...
3. **Route each finding using this fixed priority order, local-first.**
   ...
   3. **Fallback - the default prescribed private file, in the current directory.** If no existing local convention fits, don't improvise a location or invent an ad hoc filename.
      Create or append to `.stow-notes.md` in the current directory, for every finding-kind including user preferences.
   ...
   Tier 2 is the only tier that lands durable findings in a tracked/shared file; tier 3 keeps stowed findings in the private `.stow-notes.md` file and confines any optional `.gitignore` metadata edit to the current directory.
