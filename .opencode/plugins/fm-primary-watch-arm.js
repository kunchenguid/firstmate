...
async function sendPrompt(paths, client, sessionID, text, options = {}) {
  const { suppressWhileAway = false, recovery } = options;
...
async function restoreAfterActionableClose(paths, sessionID, client, predecessorArmPid) {
  ...
  for (let attempt = 0; attempt <= REARM_RETRY_LIMIT; attempt += 1) {
    ...
    if (status === "armed") return { kind: "restored", failure: "", recovery: armRecovery.get(armChild) };
    // An actionable line belongs to this arm's close handler.
    // Do not retire it before that handler can start the successor cycle.
    if (status === "wake") return { kind: "restored", failure: "", recovery: armRecovery.get(armChild) };
  ...
  armChild.on("close", (code, signal) => {
    ...
    if (classification.kind === "actionable") {
      ...
      void restoration.then((result) => {
        ...
        return sendPrompt(paths, client, sessionID, wakePrompt(message), { suppressWhileAway: true, recovery: result.recovery });
      }).catch(() => {
      });
      return;
    }
