...
async function ensureArm(paths, sessionID, client, predecessorArmPid = "", includeArmChild = false, allowJoinedRecheck = true) {
  ...
  if (joinedLaunch && allowJoinedRecheck && ["read-only", "not-needed"].includes(launchResult.status)) {
    return ensureArm(paths, sessionID, client, predecessorArmPid, includeArmChild, false);
  }
