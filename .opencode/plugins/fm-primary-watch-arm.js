...
function shouldArm(paths) {
  ...
  if (existsSync(`${paths.state}/wa-watch.check.sh`)) return true;
  try {
...
function spawnArm(paths, sessionID, client, predecessorArmPid = "") {
  ...
  const armChild = spawn("bash", ["-lc", 'config_dir="${FM_CONFIG_OVERRIDE:-$FM_HOME/config}"; [ -f "$config_dir/x-mode.env" ] && . "$config_dir/x-mode.env"; [ -f "$config_dir/wa-mode.env" ] && . "$config_dir/wa-mode.env"; exec "$FM_ROOT_OVERRIDE/bin/fm-watch-arm.sh" --restart'], {
    cwd: paths.root,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
