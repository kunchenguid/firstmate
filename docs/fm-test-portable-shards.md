...
`bin/fm-test-run.sh --list --lane portable-serial-<k>of<4>` is the authoritative current membership; timing artifacts are the authoritative measured durations.

Among the measured hints, `tests/fm-pr-check-security.test.sh` at 199573 ms is the floor for any shard count.
...
| portable serial 1-4 | job `timeout-minutes: 15` | The cap bounds each separately balanced shard as a hang tripwire; timing artifacts record healthy duration. |
