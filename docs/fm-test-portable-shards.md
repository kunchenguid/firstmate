...
The new `tests/fm-repo-concurrency.test.sh` entry carries a provisional 15000 ms hint from a 14390 ms local run on 2026-09-17, and the changed `tests/fm-spawn-treehouse-root.test.sh` entry carries a provisional 52000 ms hint from a 49970 ms local run that day, so the unmeasured-share guard remains useful until green CI timing artifacts replace them.
