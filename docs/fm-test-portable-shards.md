...
Each portable serial shard also passes `bin/fm-test-run.sh`'s `--per-script-timeout-secs 1500`, roughly twice the longest measured script (`tests/fm-watch-triage.test.sh` at about 698s) so CI load cannot cross it, and safely under the shard's 30-minute job cap, so a hung script is reported as one named failure instead of consuming the whole shard.
