...
`.github/workflows/ci.yml` runs both portable parallel lanes, the complete portable serial remainder, and the real-Herdr family through one repository-owned command policy in one serial job on the sole Water 7 runner.
