...
`tests/fm-routing-decision-negative-battery.test.sh` temporarily uses a conservative 60000 ms local hint until the next green CI timing refresh replaces the exception.
`tests/fm-spawn-dispatch-profile.test.sh` uses a locally re-estimated 120000 ms hint rather than its 57765 ms value from that CI run; a 2026-08-23 measurement at the routing-receipt head took 134154 ms.
