...
`bin/fm-lint.sh` owns two canonical CI partitions, each running the same full source-aware ShellCheck analysis with a single ShellCheck worker in CI (`--jobs 1`, a one-worker experiment to check whether intermittent exit 143 terminations stop; local runs default to two bounded workers), pinned versions, workflow validation, and backend-purity checks.
