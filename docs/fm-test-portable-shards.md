...
Taking the slowest of several CI runs rather than a single run keeps the balance honest on a slow runner, and taking them across five runs keeps one unusually slow runner from dominating the table.
...
`bin/fm-test-run.sh --check-coverage` reports the unmeasured share as `serial_unhinted=` and refuses past `PORTABLE_SERIAL_MAX_UNHINTED_PERCENT`, which catches a hint that is missing but not a hint that is wrong.
...
| portable serial 1-5 | job `timeout-minutes: 30` | Each balanced shard carries about 19.14 minutes of assignment weight and measured 13.58 to 16.95 minutes on a held-out run, so the 30-minute cap remains a hang tripwire while leaving margin for job setup and runner-speed spread. A slow runner can still push a shard well above its predicted weight. |
