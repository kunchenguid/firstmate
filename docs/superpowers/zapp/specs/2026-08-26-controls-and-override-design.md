...
Two are flagged inline as open.
...
AWS Systems Manager Parameter Store, at `/zapp/freeze`, read once per evaluation.

`aws ssm put-parameter --name /zapp/freeze --value true --overwrite` is a one-command stop that takes
effect on the next delivery, with no deploy and no PR.
...
permitted to run, so it behaves as though it is not — gate 15 fails with the read error as its
...
ships in. Only a failed *read* means frozen.
...
> **Open decision 1.** A single global flag, or per-repo freeze (`/zapp/freeze/{owner}/{repo}` with
> the global one as an override)? Global is one parameter and one read; per-repo lets one misbehaving
> repository be stopped without halting the fleet — which is what you would actually want mid-incident
> once more than one repo is enrolled.
...
> **Open decision 2.** This is the review's A5, and it is the one recorded item with a real security
> story — branch-poisoning is a known attack against auto-merge. Comfortable recording for 30 days,
> or does it warrant gating immediately despite the unknown false-positive rate?
...
No gate, no check-run row. Thirty days of data shows what fraction of would-have-approved PRs fall
outside the window, which is the number that should decide whether the window is worth enforcing —
and whether a rate budget is needed alongside it.
...
| File | Change |
|---|---|
| `src/runtime-config.ts` | New — read `/zapp/freeze`, never cached, unreadable means frozen |
...
| `infrastructure/terraform/` | Scoped `ssm:GetParameter` on `/zapp/freeze`; the parameter itself |

## Error handling

| Condition | Behaviour |
|---|---|
| SSM unreadable | **Frozen.** Gate 17 fails with the read error as its reason. |
| `/zapp/freeze` absent | Not frozen — absent is the shipped state |
| Commits fetch fails | `commitAuthorship` recorded null; no gate affected |
...
- **Freeze** — `true` fails gate 17; `false` passes; a missing parameter passes; a **failed read**
...
- **No caching** — two evaluations in one warm Lambda issue two reads, asserted by call count. A
...
- **Gating on commit authorship** — see open decision 2.
...
- [ ] `/zapp/freeze` exists; `aws ssm put-parameter` stops evaluation on the next delivery with no deploy
- [ ] The flag is read once per evaluation and never cached, proven by call count
- [ ] An unreadable flag means frozen; an absent one means not frozen
