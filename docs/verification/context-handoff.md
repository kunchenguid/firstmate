...
ok - precise Pi process liveness recovery
...
It covers required-gate prerequisite exit status, non-cancelling empty registers under an unhealthy binding, bounded classification of missing durable records, retirement of invalid compaction attempt journals, case-exact bundled MCP allowlisting, category-sensitive rejection after eligibility authorization, session-bound candidate identity, global active-state backpressure and stale-session evidence, durable receipts for unhealthy seal-time bindings, current-session-only non-empty active state that leaves foreign-session candidates non-blocking, crash-durable compaction attempt replay, exact live-generation revalidation at the sealing boundary with a probe budget materially shorter than the PreCompact hook timeout, consumable-only SessionStart announcements, exact one-create-plus-coupled Save authority, queue-before-claim crash recovery, corrupt-candidate receipts, locked collision-safe process capability with live-owner replacement refusal, proven-missing Linux owner recovery, fail-closed transient Linux probes, Darwin start-token owner recovery, deterministic byte-bounded subsets, fail-closed phase-specific Claude configuration, same-capability attempt reuse before sealing, configuration-independent per-capability terminal compaction recovery, exit-75 approval revocation and fresh inspect, current source/queue/bundle/approval enforcement at the exact MCP guard, bounded MCP errors and subprocess transport, deadline-aware idempotent idle exact-generation delivery, private descriptor publication, serialized durable state initialization, isolated Pi runtime dependencies, complete Pi lifecycle failure handling, completed-Save recovery before mutable-source validation, registration retry recovery, quarantine, disable and re-enable, disposition crash recovery, complete reviewed transaction-path verification, isolated execution of the complete installed-package dependency manifest, ambient-package and release-gate startup exclusion, orphan transaction-runtime and apply-claim recovery, correlated result-before-journal replay, transaction rollback, and model-free Claude plugin discovery.
