...
When the policy is absent, the low-level selector retains its legacy direct-selection contract; the automatic-dispatch procedure still requires an explicitly validated version 2 policy before optimized routing. An active malformed or unreadable policy makes the low-level selector fail closed with one sanitized diagnostic and no routing-state mutation.
...
The opt-in test uses a temporary home and routing state. It reports installed Claude, Codex, Pi, and quota-axi versions; checks the configured native aliases and Pi model rows through read-only catalog surfaces; reads one quota snapshot; and calls only `fm-route.sh select` in simulation mode. It never calls spawn, prints credentials, mutates a repository, or touches production routing state. A missing runtime, model, catalog, or quota-axi fails closed with a named sanitized diagnostic; without the opt-in it records a clear gate skip.
