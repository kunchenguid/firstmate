...
The same variable is also the delivery guard: `fm_busy_lines_match` in `bin/fm-composer-lib.sh` selects it for `harness=grok`, and that guard is the busy gate in `pane_is_busy` (`bin/fm-supervise-daemon.sh`), `fm_pane_busy_state` (`bin/fm-tmux-lib.sh`), `fm_pending_reply_backend_observation` (`bin/fm-pending-reply-lib.sh`), and `bin/backends/herdr.sh`, so a supervised 1.0.13 crewmate mid-turn reads non-busy and a nudge or away-mode injection can be typed into a running turn rather than merely mislabelled in a crew-state readout.
