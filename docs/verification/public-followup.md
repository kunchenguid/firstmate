...
5. A public loop whose work lives in a REMOTE secondmate home delivers and retires only after readable and writable remote state proves either that no link exists or that the matching bound legacy Relay link was cleared; unreadable or non-writable state, an identity mismatch, or unconfirmed completion retains the loop, and `--force` still covers only the unresolved obligation.
