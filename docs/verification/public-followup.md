...
5. A public loop whose work lives in a REMOTE secondmate home delivers and retires after readable remote state proves no link exists, regardless of writability, or after a matching bound legacy Relay link is cleared from writable state; unreadable state, a matching link in non-writable state, an identity mismatch, or unconfirmed completion retains the loop, and `--force` still covers only the unresolved obligation.
...
ok - retire completes without a link in non-writable remote state
...
The non-writable-state case proves a matching link refuses before lock acquisition rather than waiting indefinitely, while the readable no-link case proves an already-cleared remote task still retires idempotently even when the state directory is non-writable.
