...
It executes only a content-addressed evaluator from the capture-and-scoring evidence group, compares its genuine restored-tree output hash to the archive, and requires a different successful output after changing a manifest-declared scalar through a form-preserving JSON-value perturbation.
The portable declaration fixtures refuse missing perturbation contracts, and the execution fixture proves that a valid structured mutation reaches a strict parser while a parser rejection is reported as inconclusive rather than as evidence of an input-invariant evaluator.
