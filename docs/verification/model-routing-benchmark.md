...
Three further fixtures cover what echo rejection alone does not: an evaluator whose answers are keyed on the frozen input path matches nothing once the gate blinds that path; an evaluator that normalises away the first perturbable scalar but derives its records from the rest of the input passes, because the gate escalates to a later scalar; and an evaluator that normalises away every perturbable scalar is refused as input-blind even though it derives every record correctly.
The launch fixtures additionally prove that an unreadable evidence file and an empty declared judge panel each produce a named refusal rather than a Python traceback.
