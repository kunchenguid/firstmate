...
Two further fixtures cover what echo rejection alone does not: an evaluator whose answers are keyed on the frozen input path matches nothing once the gate blinds that path, and an evaluator that normalises away the scalar the gate edits is refused as input-blind even though it derives every record correctly.
