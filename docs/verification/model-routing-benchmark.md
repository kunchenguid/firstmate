...
The launch contract distinguishes provider-capable entrant confinement from network-disabled evaluator replay, and the entrant form requires a pinned runtime image plus a Docker-internal network whose only inspected member is the credential-holding proxy.
The preflight execution fixture content-addresses the frozen evaluator program and rejects records that its confined execution does not reproduce.
Promotion fixtures rebuild composites from content-addressed archive evidence and refuse a re-addressed record whose judge panel differs from the plan.
