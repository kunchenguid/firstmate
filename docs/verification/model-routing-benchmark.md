...
Four further fixtures cover what echo rejection alone does not: an evaluator whose answers are keyed on the frozen input path matches nothing once the gate blinds that path; an evaluator that looks up its answer by content in the frozen expected outputs is refused on its first record, because `evaluator/execution.json` and `ground-truth/` are unreachable from the gate-owned scratch root the cage exposes; an evaluator that normalises away the first perturbable scalar but derives its records from the rest of the input passes, because the gate escalates to a later scalar; and an evaluator that normalises away every perturbable scalar is refused as input-blind even though it derives every record correctly.
The launch fixtures additionally prove that an unreadable evidence file and an empty declared judge panel each produce a named refusal rather than a Python traceback, and the manifest and evaluator fixtures prove the same for a non-numeric declared sample count and a non-numeric declared unit price, which the gate prices before any cost-model check runs.
Freeze fixtures prove the same for a private input the gate can open but not read - which `is_file()` accepts and only the hash rejects - naming that file in both `freeze` and `freeze-check`, and promotion fixtures prove it for a declared baseline whose preregistered veto strata are empty, which `promote-evaluate` reaches without ever running the plan check.
Restore-drill fixtures prove that neither a perturbed archived evaluator's stderr nor a declared archive path containing a newline can forge an extra `BENCH_CHECK` or `BENCH_RESULT` line in the gate's line-oriented output.
An end-to-end fixture clears a complete benchmark through `preflight`, has the launch boundary honour the minted receipt, and then proves that a symlinked private input - which the freeze hashes through `is_file()` and only an lstat classifies - refuses at `preflight` instead of minting a clearance the spawn boundary is guaranteed to reject.
The same fixture proves that evidence a stage can open but not read still produces a named file-specific refusal with its verdict line, withdraws the standing clearance, and leaves no passing receipt on disk, rather than aborting the run.
Two further cage fixtures assert from inside the confinement that a multi-file evaluator reaches its sibling module and its frozen calibration while `evaluator/execution.json` and `ground-truth/evaluator-outputs/` are absent, and that a symlink planted under `scoring/` is refused rather than staged.
A differing pair of dry-runs is refused whether or not a tolerance file declares the difference acceptable.
The launch fixtures take their receipts from the gate's own `evidence-digest` rather than recomputing that digest, then prove a cleared launch is withdrawn when a capture record is deleted, when the measured allowance is lowered, and when an artifact is replaced by a symlink.
