...
The preflight execution fixture content-addresses the frozen evaluator program and every separate golden, mutation, and capture input, and a cat evaluator is behaviorally refused because it cannot derive the expected record from the frozen input envelope.
...
The archive and promotion fixtures preserve a real void with only central failure and timing evidence, keep its rerun as a separate attempt, and require the rerun's explicit supersession link before the planned sample is complete.
