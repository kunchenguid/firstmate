...
The genuine restored copy retains its archived bytes, while each separate perturbed copy changes only the declared value, token, or filtered pixel byte and must produce a different successful evaluator output.
