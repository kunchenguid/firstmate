...
It covers TOON-first ranking by usable remaining quota, unstarted session windows, and `spendPriority` among candidates that pass eligibility, reasoning-class, and runway-feasibility gates, explicit accounting for unmeasurable runway, the strongest-reasoning constraint, the runway feasibility floor over a higher `spendPriority`, honoring an explicit captain override when the captain insists, and the agent spontaneously surfacing a concise recommendation for an abundant alternative when a tight-quota explicit request has no override given yet.
