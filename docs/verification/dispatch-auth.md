...
`tests/fm-bootstrap.test.sh` owns the quota-axi version-floor diagnostic and the configured Codex home's validation diagnostics.
`tests/fm-control-relaunch.test.sh` owns carrying that recorded home into a same-harness replacement and dropping it on a harness switch.
