...
Its endpoint-override cases exercise the real resolver and real `curl` against a loopback-only fake Jev server with a dummy credential and deterministic responses.
Those cases cover the [endpoint configuration contract](../configuration.md#typed-dispatch-resolution-env-typesafe_api_key), successful response parsing and ordinary profile selection, and disabled-mode inertia; hosted-default destinations are checked through fake `curl` without contacting the hosted service.
This is transport and fixture-response coverage, not evidence of real local model inference.
