...
It proves the section check, the scout line, the sections, and the whole-brief fallback all come from one snapshot of the brief taken before any of them is read, so a brief rewritten mid-resolve still sends its first-read text.
