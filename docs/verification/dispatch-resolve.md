...
It proves an absent key across the environment, `.env`, and Keychain prints one stderr line, nothing on stdout, exits 0, and never invokes `curl` or `quota-axi`.
...
It proves the documented starter configuration resolves its Pi default through the declared Claude provider, a stubbed Keychain key turns the tool on, unavailable `security` and a missing Keychain item keep it off, and the environment and `.env` win over the Keychain.
...
`tests/fm-bootstrap.test.sh` proves bootstrap ignores resolver-only fields without the typed key, validates each malformed shape when the environment or home `.env` activates typed resolution, applies the same validation and harness routing from a readable Keychain secret while a missing item or unreadable secret leaves them off, and prevents an environment-provided key from reaching child processes.
