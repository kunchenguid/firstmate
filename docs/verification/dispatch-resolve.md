...
The 2026-09-25 offline regression runs also covered a `custom:foundry-ic` quota row winning by `spendPriority`, a rule floor using that row, an unrelated profile resolving with the custom row present, malformed custom identities failing validation, typed bootstrap acceptance, and a path-safe custom-provider quota watch.
The exact commands were `bin/fm-test-run.sh tests/fm-dispatch-resolve.test.sh`, `bin/fm-test-run.sh tests/fm-bootstrap.test.sh`, `bin/fm-test-run.sh tests/fm-quota-choose.test.sh tests/fm-procevent-quota.test.sh`, and a final `bin/fm-test-run.sh tests/fm-procevent-quota.test.sh`; each returned `FM_TEST_SUMMARY ... failed=0`.
