...
It also proves dynamic `discover` expands fixture-backed catalog rows, applies provider, task-fit, and reasoning-fit gates, preserves provider-qualified model IDs and catalog provenance, leaves preferences out of quota ranking, collects dynamic defaults only for a below-floor fallback, and reports unsupported harnesses, missing results, unknown quota, and catalog timeouts.
