...
It proves the documented starter configuration activates the declared Sol quality route under near-reset or comfortable Codex headroom and falls back to Luna when the preference is inactive, a `.env` key turns the tool on, and the environment wins over it.
