...
Every row added by the third pass - the retry check, both halves of the durable landing evidence, the confirmation's freshness and its authenticity, and the returning revision resolution - was verified to fail with only its own fix reverted, so none of them passes vacuously.
A fourth pass found that the immutable origin had made the documented opt-out unmergeable for any task the bridge had linked; the release rows below were revert-tested the same way.
One of those reverts is how the reason validator's own defect surfaced: `grep '[[:cntrl:]]'` cannot see a newline, because a line-oriented tool treats it as the separator rather than as content, so the check that existed to stop a reason forging further `key=value` lines did not stop it.
...
| making the origin immutable stranded work: after a revoke, every task the bridge ever linked refused to land forever, with no recovery | one named task can be released explicitly, and the release only appends to the task record so the origin and the reason stay readable | `test_a_revoked_pairing_does_not_strand_a_linked_task` |
| the release reached only the local landing path: the pull-request path demanded a resolvable revision first, and `pr_head` is routinely absent (every GitLab task, and any GitHub task whose forge CLI cannot supply it), so a released task still refused there | the release is evaluated before any revision requirement, because a released task has no confirmation for a revision to be checked against; an unreleased task still refuses on an unresolvable revision | `test_a_released_task_lands_without_a_resolvable_pr_head`, `test_telegram_linked_merge_refuses_when_the_revision_cannot_be_resolved` |
| a release could have become a routine way past the publish gate | it needs `--yes` and one line of real reason, refuses a task that never came from the bridge, is recorded once and never rewritten, is refused while a peer is pinned for that project or a preview is armed, is superseded by a later preview, and is refused to a gate agent | `test_release_demands_an_explicit_flag_a_reason_and_a_task_to_release`, `test_release_refuses_while_the_ordinary_confirmation_path_is_open`, `test_a_fresh_preview_supersedes_an_earlier_release`, `test_gate_agent_cannot_reach_the_channel` |
