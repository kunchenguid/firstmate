...
| neither landing helper enforced the publish confirmation | both refuse a Telegram-linked task without a live confirmation bound to the task, approving person, project, landing target and revision they really land, and consume it atomically | `test_telegram_linked_merge_*` (9 cases), `test_local_merge_*` (4 cases) |
