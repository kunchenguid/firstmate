...
`tests/fm-update.test.sh` additionally drives the public updater through real semantic refreshes: an upstream `AGENTS.md` addition lands exactly once in its deferred optimized owner, upstream skill bodies follow upstream while explicitly registered compact descriptions survive even after upstream convergence, and the installed refs remain unmoved until approval.
It accepts Psych-proven plain, quoted, folded, literal, and explicit- or implicit-indent description scalars, while malformed, duplicated, or unproven boundaries report the unresolved semantic and produce no candidate ref.
...
`tests/fm-instructions.test.sh` also requires the exact reviewed candidate graph, descendant reviewer and bound transformer provenance, rejects a descendant candidate producer and unattested lineage drift, and compares the reconstructed lineage with the reviewed candidate generation.
