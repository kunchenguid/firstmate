...
### 2026-09-19 Claude Code 2.1.278 pin evidence

Evidence produced 2026-09-19 on Linux 6.6.87.2-microsoft-standard-WSL2 x86_64, Claude Code 2.1.278, Node v24.18.0, with `CLAUDE_CODE_PIN` in `hooks/branch.ts` at `2.1.278` and the plugin test suite importing its `PIN` from the module.
The 2.1.278 changelog was read in full; it has two entries, both about auto mode: the auto-mode permission classifier now defaults to the server-side classifier for API, Enterprise, Bedrock, Vertex, Foundry, and gateway users, with `CLAUDE_CODE_AUTO_MODE_SERVER=0` as the opt-out, and `/status` gained an `Auto mode server` row.
Nothing announced touches the function-hooks module API, `$.tool.register`, `$.agent.spawn`, `$.prompt.submit`, the Stop hook contract, or `--plugin-dir` directory loading.

- Version and strict validation: `claude --version` printed `2.1.278 (Claude Code)`, and `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 claude plugin validate --strict .claude/mods/fm-branch-mod` printed `✔ Validation passed`.
  `bin/fm-test-run.sh tests/fm-branch-claude-mod-plugin.test.sh` printed `FM_TEST_BEGIN 2026-09-19T10:16:18Z tests/fm-branch-claude-mod-plugin.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.278 (Claude Code) validates the supervision-branch mod strictly: the documented hooks, the spawn, the classifier, and only the home-resolution environment`, `ok - Claude Code 2.1.278 (Claude Code) runs the supervision-branch mod's plugin test suite clean: pin refusal, classification records, captain hand-back, and routine spawn`, and `FM_TEST_END 2026-09-19T10:16:20Z tests/fm-branch-claude-mod-plugin.test.sh exit=0 duration_ms=2039 gate_skip=false`, measuring 2039 ms.
- Live run: `FM_BRANCH_MOD_LIVE=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` printed `FM_TEST_BEGIN 2026-09-19T10:16:26Z tests/fm-branch-claude-mod-live-e2e.test.sh family=live-harness-optin expected_gate_skip=live-capability`, `ok - Claude Code 2.1.278 loads the supervision-branch mod enabled and main holds the session lock`, `ok - the first routine wake passes the classifier and spawns the branch agent, which reports it routine`, `ok - the captain-class wake is passed to main by the classifier with a covering outcome row, and the next routine wake reaches the same agent through SendMessage`, `ok - one spawn, every send successful, no dropped hand-back, no backstop delivery across the run`, and `FM_TEST_END 2026-09-19T10:19:36Z tests/fm-branch-claude-mod-live-e2e.test.sh exit=0 duration_ms=190274 gate_skip=false`, measuring 190274 ms end to end with exit 0.
- Live run, persistence and child-session cases (later on 2026-09-19, same host and pin, after the test grew to two labs): `FM_BRANCH_MOD_LIVE=1 FM_BRANCH_MOD_LIVE_KEEP=1 tests/fm-branch-claude-mod-live-e2e.test.sh` printed the four lines above plus `ok - a wake after the in-memory window reaches the same persisted agent: the send succeeds, no rotation` and `ok - a minutes-later wake on an inherited CLAUDE_CODE_CHILD_SESSION rotates to a fresh agent (why=unresumable) and keeps the wake`, exit 0 in 474 s.
  Lab 1 (scrubbed launch, transcript saving on) logged one `agent.spawn`, two `agent.send` both carrying `"success":true,"message":"Resuming agent fm-branch"` to the same `resumedAgentId`, three `wake.delivered` (`"via":"spawn"`, then `"via":"send"` twice), and no `agent.rotated`.
  Lab 2 (the tmux server started with `CLAUDE_CODE_CHILD_SESSION=1` in its global environment) logged three `agent.spawn`, two `agent.rotated` with `"why":"unresumable"` and `"ok":true` whose `agent.send` detail read `could not be resumed: No transcript found for agent ID`, three `wake.delivered` all `"via":"spawn"`, no `wake.passed`, and no `handback.dropped` in either lab.
...
Evidence produced 2026-09-19 on Linux 6.6.87.2-microsoft-standard-WSL2 x86_64, Claude Code 2.1.278, against the head at which `checkPin` reads the version of the binary hosting the session and records which source decided, `session.start` carries `persistenceOn` / `persistenceCause`, and every `agent.send` names the resume target (`agentId`, `sessionId`).
...
- Engine-hosted suite: `bin/fm-test-run.sh tests/fm-branch-claude-mod-plugin.test.sh` printed `ok - Claude Code 2.1.278 (Claude Code) validates the supervision-branch mod strictly: the documented hooks, the spawn, the classifier, and only the home-resolution and transcript-persistence environment` and `ok - Claude Code 2.1.278 (Claude Code) runs the supervision-branch mod's plugin test suite clean: pin refusal, classification records, captain hand-back, and routine spawn`. The kit suite at this head asserts the split case (`pinSource: "running binary"`, probe `2.1.278 (Claude Code)`), the no-proc PATH fallback (`pinSource: "PATH claude"`, empty probe), a refusal record naming its source, `persistenceOn` / `persistenceCause` on `session.start` for the default, marker, and force cases, and `sessionId` on `agent.send` equal to the kit's mocked `$.session.id`.
- Live run: `FM_BRANCH_MOD_LIVE=1 FM_BRANCH_MOD_LIVE_KEEP=1 bin/fm-test-run.sh tests/fm-branch-claude-mod-live-e2e.test.sh` printed all four `ok` lines (mod loads enabled, routine spawn, SendMessage reuse, one spawn with every send successful) with `exit=0 duration_ms=144239`. In the kept event log the `session.start` data was `"version":"2.1.278","pinSource":"running binary","probe":"2.1.278 (Claude Code)","persistenceOn":true,"persistenceCause":"default"` and the first `agent.send` carried `"to":"fm-branch","agentId":"ae1c1fc8ee36ba51c","sessionId":"e8f0e455-4624-4d42-9e0c-add28209b618"`, so `$.session.id` answers a real transcript id on 2.1.278 rather than the `unavailable (...)` marker.
...
- Split run, the outage's configuration reproduced once by hand at this head (no permanent live case): the live guard's launch line with the pinned binary named by absolute path while PATH names another release, in a scratch home with `state/.branch-mod-mode` present and the guard's `tmux` shim. `$SHIM/claude` was a symlink to `~/.local/share/claude/versions/2.1.277`, and the session was started with `tmux -L fm-split new-session -d -c "$HOME_DIR" "env -u CLAUDECODE -u CLAUDE_CODE_… -u FM_… PATH='$SHIM:$PATH' CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 FM_HOME='$HOME_DIR' FM_ROOT_OVERRIDE='$HOME_DIR' CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 ~/.local/share/claude/versions/2.1.278 --model sonnet --plugin-dir .claude/mods/fm-branch-mod --settings '$LAB/settings.json' --strict-mcp-config --dangerously-skip-permissions --debug-file '$LAB/debug.log'"`. The module logged no `pin.refused`; its load line was `{"t":"2026-09-19T11:44:43.339Z","kind":"session.start","data":{"cwd":"/tmp/fm-split.XpgLWe/home","home":"/tmp/fm-split.XpgLWe/home","state":"/tmp/fm-split.XpgLWe/home/state","enabled":true,"generation":"cc1789818283005","version":"2.1.278","pinSource":"running binary","probe":"2.1.278 (Claude Code)","persistenceOn":true,"persistenceCause":"default"}}`, so `$PPID` inside the hook-spawned `sh` is the Claude Code process itself and the probe, not PATH, decided.
