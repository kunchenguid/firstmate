...
[Grok mid-turn Enter](grok-queued-enter.md) records the 2026-09-06 real-CLI queueing result for Grok 1.0.13, its manual refresh commands, and the measured `Esc:cancel` busy footer that refreshed the per-harness Grok delivery signature; it does not widen the harness-less union and does not refresh the composer-classification matrix.
