...
On 2026-09-19 a live grok 1.0.34 idle composer on tmux 3.7c (Linux, isolated private socket, no prompt submitted) rendered a same-width titled bottom `Grok 4.5 (high) · always-approve` and SGR-2 dim `❯ Type a message...`.
The ASCII-only title fold left U+00B7 in the residue, so `_fm_composer_titled_bottom_ok` rejected a complete box and both styled and unstyled captures classified `unknown`.
After mapping the middle-dot as title punctuation, the same live 20-row tail classifies `empty`; a same-width box with bright `deploy the fix` classifies `pending`.
`test_matrix_grok_titled_bottom_border` and `test_grok_1_0_34_dim_placeholder_in_autonomy_titled_box_is_not_pending` pin those two directions.
