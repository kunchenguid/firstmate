...
The Muse version in its reported line is read from the pane's own `pane process-info` body, whose `muse-bin-<version>` argv exec name names the build that actually ran, and falls back to `version-unknown` when that body yields no argv surface carrying a non-empty version.
The truncated kernel `.name` is deliberately not a fallback there: a 15/16-byte cut of a real `muse-bin-<version>` still carries the prefix, so accepting it would name a build that never existed.
