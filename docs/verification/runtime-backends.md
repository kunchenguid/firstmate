...
The Muse version in its reported line is read from the pane's own `pane process-info` body, whose `muse-bin-<version>` exec name names the build that actually ran, and falls back to `version-unknown` when that body yields nothing.
