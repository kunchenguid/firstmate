...
That pre-check is read at the top of every readiness tick and ends the wait the moment the registration appears, so the readiness gate above never gets to fail red over an argv identity or composer shape on a pairing whose steer would not have used them.
