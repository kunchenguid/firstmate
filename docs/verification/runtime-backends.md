...
`_fm_composer_titled_separator_row` in `bin/fm-composer-lib.sh` now admits a rule that carries a label written INTO it, the same tolerance a box's titled bottom border already had: the rule must open and close the row with `─`, and the label between those two runs must be spaced away from both and carry no further box structure, so prose that merely contains dashes still cannot pass.
That shape is the whole test: no width floor is held against a titled rule, because such a rule spends on its label the width a floor would count, so any floor - one contiguous run, the two runs summed, or the row's own length - closes the pair on a wide pane and stops closing it on a narrow one, which is the same false negative reached by splitting the window.
The 8-column floor still applies to a solid rule, which spends its whole width on `─`.
...
What it will prove when it runs is bounded: its readiness gate requires the pane to carry a live `muse-bin-<version>` argv identity and the shared classifier to read Muse's idle composer as empty before it steers, and it then requires the steer to confirm empty and the token to render somewhere in the captured pane window, which is the non-vacuity anchor beneath that verdict rather than a second proof of delivery: the capture is scrollback-bounded rather than de-duplicated, so where in the pane a rendered token came from is not something the leg can claim.
...
Its native pre-check is a refusal to attribute a verdict rather than a claim about which branch produced one, and it is keyed to the baseline `fm_backend_herdr_classify_submit_agent_status` derives: `idle` and `done` take the adapter's idle baseline, where a native that never leaves idle hands the verdict to the composer, so a registered idle agent runs the leg exactly as no registration at all does.
Only a pane already busy before the steer - `working` or `blocked`, the two statuses that classify `busy` - is skipped with an explicit unverified line, exactly as an absent Muse is reported, because the shape that run would measure is not the idle-composer steer this leg is about.
A raw `unknown` does not skip: it is the placeholder Herdr writes the moment it notices the harness, before the harness reports `idle` (see [Stale agent registration](#stale-agent-registration)), and the readiness loop polls straight across that transition, so it waits through it exactly as it waits for the composer to paint; an `unknown` still standing at the steer takes the adapter's footer-baseline branch, where the composer decides the verdict all the same.
That check is read at the top of every readiness tick, so the gate above never gets the chance to fail red over an argv identity or composer shape such a run would never have measured, and it is read once more immediately before the steer, because the readiness signals cost round trips in which the pane can leave idle.
Herdr 0.9.0 registers the Muse agent as `idle`, so that pairing runs the leg rather than skipping it; this entry stays unexecuted because no guarded run has been made, not because the pairing forbids one.
