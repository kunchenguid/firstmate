...
Only a pane already non-idle before the steer - `working`, `blocked`, or a status the adapter does not classify - is skipped with an explicit unverified line, exactly as an absent Muse is reported, because the shape that run would measure is not the idle-composer steer this leg is about.
