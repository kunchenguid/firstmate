...
The portable regressions pin the tolerance itself: with `FM_COMPOSER_LEFTBAR_FOOTER_RE` overridden to the pre-change regex, `tests/fm-composer-lib.test.sh` fails as `not ok - opencode --auto idle footer on tmux: expected empty, got 'pending'`, and it passes with the shipped regex.
