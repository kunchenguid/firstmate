...
A real ship spawn launched against a second config store, `<captain-home>/.claude-enterprise`, and the store it actually used was read back from the launched process's own environment rather than inferred from the command text.
