...
The companion `tests/fm-backend-adapter-source.test.sh` and `tests/fm-backend.test.sh` regressions cover missing and unreadable adapter files across tmux, Herdr, Zellij, Orca, and cmux, proving that each caller reaches its refusal branch.
