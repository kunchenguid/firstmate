...
It also proves that the private custody directory stays mode 0700 and every private record mode 0600 without widening the umask Pi inherits, that every completed recovery releases its recovery lock instead of leaving dead ownership, and that malformed, non-regular, or custody-contradicting lease records surface through `status` rather than an error exit.
It further proves that a wrapper killed with SIGKILL while holding the recovery lock leaves a complete private PID/start guard that the next recovery reclaims, and that an identity-less or non-regular object at that path refuses as ambiguous ownership instead of being guessed at.
The lifetime lease carries the same proof: a SIGKILLed wrapper leaves a single regular mode 0600 record holding exactly version, token, pid, and start, which the next recovery reclaims into a fresh token, so interruption can never publish an ownerless permanent blocker.
It separately proves that `status` reports an absent-custody dead lease as `stale` and reserves `contradictory` for a lease that disagrees with a readable custody record, and that a restart into an apostrophe-bearing specialized `FM_STATE_OVERRIDE` home pins that exact path into the generated pane command while a stray override in the pane's own shell is never followed.
