...
Orca has a recovery-grade agent-state classifier for the harnesses it hooks (claude, codex) through its `worktree ps` agent model, but like cmux it does not support secondmate spawns, so neither needs a secondmate process-identity matcher.
