...
Firstmate's real-environment run at gate tip `8f7295cf` passed the canonical lifecycle guard with the READY-first ordering, exactly-one-ring doorbell assertion, and post-doorbell idle-and-empty settle at widths 220 (`FM_TEST_END 2026-09-12T21:24:58Z exit=0 duration_ms=125646`), 120 (`FM_TEST_END 2026-09-12T21:26:56Z exit=0 duration_ms=118344`), and 80 (`FM_TEST_END 2026-09-12T21:28:32Z exit=0 duration_ms=96132`).
The same run passed the composer-matrix guard with six ok lines including both AGY cases and passed `ok - harness liveness: agy 1.2.2 classifies alive`; the detailed exact output is maintained in [`docs/verification/agy.md`](agy.md).
