...
`tests/fm-control-relaunch.test.sh` holds the same rule on the relaunch path, where a task recorded before `tasktmp=` existed makes the relaunch create the root itself: an abort before the replacement record is published removes it, and a failure after publication leaves it for teardown.
