...
The dedicated `fm-devenv-protocol-lab` snapshot and the ambient `default` snapshot were identical before and after the round trip once the one tolerated presentational title animation, the leading animated spinner frame, was removed from `terminal_title` and `terminal_title_stripped`.
That was confirmed against real `herdr --session default api snapshot` bytes, and so was its limit: a captured pair straddling an `Action Required` marker blink differed in the `[ . ]`/`[ ! ]` marker characters and in a pane `revision` advance that tracks the blink one for one, so the `revision` advance, not the marker, is why an ambient session holding an `Action Required` agent is refused and why the documented prerequisite excludes one.
