...
The fix declares both footer rows composer furniture beneath a proven bordered box (`FM_COMPOSER_KIMI_FOOTER_MODE_RE_DEFAULT` and `FM_COMPOSER_KIMI_FOOTER_CONTEXT_RE_DEFAULT` in `bin/fm-composer-lib.sh`), matched on the captured layout widened only for shapes Kimi is expected to render but that were not captured: the mode row's `<mode>  <model> thinking: low|medium|high|max` opening, where the model cell may be a spaced display name (a plain `kimi --auto` on herdr showed the configured default as `Never Ask  K2.8 Preview thinking: max  ~`, idle and after submit at `context: 2% (19.8k/1M)`), followed by two spaces or the end of the row (a narrow pane can drop the cwd and hint cells), and the anchored `context: <pct>% (<used>/<total>)` usage cell, whose counts may be decimal or carry a lowercase suffix once usage is nonzero (for example `(81.9K/1M)` or `(82k/1M)`, which the post-submit delivery gate must still read).
...
`test_matrix_kimi_202_footer_furniture` in `tests/fm-composer-lib.test.sh` carries the captured shapes byte-faithfully (both footer rows, each row alone, a foreign row below the box and in the footer's place, and the out-of-set effort word) across the herdr, zellij, and plain capability profiles under both locales, plus the anticipated but uncaptured shapes (nonzero-usage context rows, a narrow mode row that ends at the effort cell, and a refused context row with trailing text) under the herdr profile.
