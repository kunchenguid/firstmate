...
The crewmate/scout-only GitHub Copilot CLI 1.0.88 adapter was verified separately on 2026-09-24 against tmux on macOS arm64.
Its live pane held comm `node` (`node /Users/manijoshi/.npm-global/bin/copilot -i ...`) plus a native child whose basename is `copilot` (`.../@github/copilot-darwin-arm64/copilot -i ...`), so the shared classifier anchors the exact `copilot` name and reaches the loader shim through the structural argv rule in `bin/fm-copilot-lib.sh`, the gemini shape; `copilot-helper`, `mycopilot`, a bare `node`, and a copilot name in a later flag value all remain ambiguous in the portable regression.
[`copilot.md`](copilot.md) owns the fuller record, including the marker, trust, busy-fold, interrupt, and composer evidence captured in that same live tmux session.
