...
`tests/fm-standing-worker.test.sh` pins the surrounding logic portably with no harness: the working-to-idle wake and its bounded untrusted excerpt, the stored-status debounce that keeps one stop to one wake, a vanished pane, an unreadable status, a failed or timed-out Herdr read that is never a vanish, a hung session that does not starve a healthy one, the worker already stopped at registration, the wrong-session registration refusal and the sessions it names, a secondmate home publishing a keyed, defused stop event on its parent channel, the Stop hook merge and its tracked-file refusal, the hook event that catches a turn shorter than the poll interval, the hook's silence for a foreign cwd, project directory, or pane, a turn ending in a subdirectory that is delivered only when the pane id proves the worker, a pane that vanishes while its worker is stopped, and the turn-end line read as actionable by the watcher's own classifier.
