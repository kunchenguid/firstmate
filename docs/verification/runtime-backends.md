...
The 2026-09-15 OpenCode `--auto` worker posture ([OpenCode worker permission posture](#opencode-worker-permission-posture)) renders the mode footer as `Build auto · <model>`, which the leftbar footer tolerance `^(Build|Plan)[[:space:]]+·[[:space:]]+` no longer matched, so an idle `--auto` worker read `pending` and would have starved steering for every opencode crewmate.
The footer regex now takes an optional `auto` qualifier (`FM_COMPOSER_LEFTBAR_FOOTER_RE_DEFAULT` in `bin/fm-composer-lib.sh`), pinned by one portable regression in `tests/fm-composer-lib.test.sh`; the other backend and typed-composer dimensions of that code path are already pinned by the pre-existing non-auto cases beside it.
...
A rerun of the changed guard on 2026-09-15 printed `ok - opencode (1.18.31): real idle composer classifies empty` against a real `opencode --auto` pane, with claude 2.1.259 and codex-cli 0.154.0 passing and pi 0.85.1's pre-existing trust-dialog failure persisting in the guard's bounded-expiry form (`not ok - pi (0.85.1): bounded wait expired after 45 readiness polls at 1s waiting for the idle composer to classify empty; last verdict: unknown`), so that run still ends nonzero on its pi leg alone.
The guard's readiness wait is bounded at `FM_COMPOSER_MATRIX_LIVE_POLLS` one-second polls and its expiry names the harness, version, waited-for state, and budget, so a pane that never reaches the waited-for state fails fast and loudly rather than degrading quietly.
That regression pins the tolerance itself: with `FM_COMPOSER_LEFTBAR_FOOTER_RE` overridden to the pre-change regex, `tests/fm-composer-lib.test.sh` fails as `not ok - opencode --auto idle footer on tmux: expected empty, got 'pending'`, and it passes with the shipped regex.
