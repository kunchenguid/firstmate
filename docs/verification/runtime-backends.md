...
The turn-end hook adds a second harness-dependent signal: its pane check compares the `HERDR_SESSION` and `HERDR_PANE_ID` a process inside a pane sees against the session and pane id recorded at registration.
...
`tests/fm-standing-worker.test.sh` pins the surrounding logic portably with no harness: the working-to-idle wake and its bounded untrusted excerpt, the stored-status debounce that keeps one stop to one wake, a vanished pane, an unreadable status, a failed or timed-out Herdr read that is never a vanish, a hung session that does not starve a healthy one, the worker already stopped at registration, the wrong-session registration refusal and the sessions it names, a secondmate home publishing a keyed, defused stop event on its parent channel, the Stop hook merge and its tracked-file refusal, the hook event that catches a turn shorter than the poll interval, the hook's silence for a foreign cwd, project directory, or pane, a turn ending in a subdirectory that is delivered only when the pane id proves the worker, a pane that vanishes while its worker is stopped, the idle-to-blocked, idle-to-done, blocked-to-done, and blocked-to-idle wakes, a pane left at `done` after a hook-reported stop staying silent, a matching pane id in a foreign Herdr session that records nothing, a capture longer than its cap keeping the closing question, and the turn-end line read as actionable by the watcher's own classifier.
