...
Firstmate's real-environment run against gate tip `13ad48dc` and Antigravity CLI 1.2.2 passed the canonical lifecycle guard at widths 120, 80, and 220, the composer-matrix guard, and the liveness/marker guard before the deterministic lifecycle-guard ordering and post-doorbell settle changes; the exact dated outputs and the follow-up rerun note are recorded in [`docs/verification/agy.md`](agy.md).
