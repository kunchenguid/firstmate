...
The steering channel's one behavioral assumption - a real worker agent follows the constant self-describing doorbell line (list the inbox, read and act on its records in numeric order, moving each into `handled/` as soon as it is read, that move confirming receipt rather than completion) - was verified on 2026-08-23 against every installed verified harness, on tmux 3.6a, macOS arm64, on an isolated private socket, driving the REAL `bin/fm-send.sh` end to end (durable record plus doorbell, with one mid-wait re-ring playing the watcher's role).
...
Quote correction: the parenthesised doorbell wording above was rewritten in place to match the line `bin/fm-task-inbox-lib.sh` ships today, which spells out that the `mv` acknowledgement is due as soon as a record is read and does not wait on the work it asks for. The 2026-08-23 run recorded here exercised the earlier phrasing of that same contract (list, read and act, `mv` into `handled/`); no harness has been re-run against the current wording, so the evidence and its date stand as originally recorded and the guard is owed a rerun at the next harness refresh.
