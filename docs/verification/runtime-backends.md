...
The Muse version in its reported line is that same gate-read identity, so a reported line always names the build that actually ran; `version-unknown` survives only in the messages emitted before the gate opens, which never carry a verdict.
The truncated kernel `.name` is deliberately not a fallback there: a 15/16-byte cut of a real `muse-bin-<version>` still carries the prefix, so accepting it would name a build that never existed, and it would also let the readiness gate open on a truncated identity.
