...
None of the three is identified now, while `node /path/to/claude/cli.js` and `node /opt/tools/codex/cli.js` still are, which is the node-hosted install whose bin entry is not named after the harness and the reason this rule reads path components rather than a basename.
...
The recorded run above predates that check and the wording change that came with it, so the kind each release reports is unobserved here, its per-harness note lines predate the `verified for <harness>` wording they now carry, and it records no cross-harness kind distinctness line at all.
