...
agy joined this guard's harness list on 2026-08-14, after the run above, so it has no line in that output yet; meanwhile its separated `>` pair is pinned by the portable captures in `tests/fm-composer-lib.test.sh` and by the real-process identity round trip in `tests/fm-tmux-agent-liveness.test.sh`, which also proves the identical screen over a live Pi never reads `empty`.
