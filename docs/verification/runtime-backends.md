...
Orca uses its own native `connected`/`agentIdentity` terminal fields ([`orca-backend.md`](../orca-backend.md#recovery)) and needs no process-name branch either. Zellij has no verified recovery-grade agent process probe, and cmux does not support secondmate spawns, so those two retain their existing generic ordinary-launch semantics without a new liveness matcher.
