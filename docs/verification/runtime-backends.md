...
A titled rule is read when its title is well-formed, single-width, non-structural text of ANY script, and that boundary was widened deliberately from the ASCII-printable-only limit an earlier attempt imposed.
...
The widening works by making the column proof correct for non-ASCII text rather than by tolerating leftover bytes, so `fm_composer_column_spaces` refuses strictly more than the ASCII pass it replaced: a structural box-drawing glyph from another container's edge, a control byte, DEL or ESC, an invalid UTF-8 lead byte, an orphan continuation byte, and a truncated multibyte sequence.
Four live shapes still refuse, and each refusal is a deliberate `unknown` - a refusal to read - which is the safe direction.
...
Malformed bytes refuse, which is what keeps the widening from becoming a general tolerance for junk rows.
A title that precedes the row's opening 8-column dash run refuses, because this change widened the character boundary and not the title POSITION boundary.

One column-counting proof serves all three callers that ask the same question, so the boundary cannot drift between them: the titled-rule predicate, grok's titled bottom border, and the box content-row geometry check.
Those two other callers therefore widen with it, which is deliberate rather than incidental, and each is now pinned in `tests/fm-composer-lib.test.sh`.
A grok bottom border whose title carries a non-ASCII glyph is tolerated as a title at the same border width, and a bordered content row carrying a single-width non-ASCII character now proves its box's geometry instead of failing it, which moves that row from `pending-unproven` to `pending`.
Neither direction widens what may be typed into, because `empty` is still the only verdict that authorizes an injection.
No other pinned verdict moved when the three callers were consolidated, measured on 2026-08-14: `tests/fm-composer-lib.test.sh` (31 ok), `tests/fm-composer-ghost.test.sh` (33 ok), `tests/fm-backend-herdr.test.sh` (175 ok), `tests/fm-backend-cmux.test.sh` (61 ok), `tests/fm-tmux-agent-liveness.test.sh` (14 ok), `tests/fm-kimi-harness.test.sh` (17 ok), `tests/fm-gotmp.test.sh` (3 ok), `tests/fm-send-settle.test.sh`, `tests/fm-tmux-submit-busy.test.sh`, and `tests/fm-send-secondmate-marker.test.sh` all pass.
`tests/fm-backend-orca.test.sh` and `tests/fm-backend-zellij.test.sh` each fail one teardown-family assertion, byte-identically with this change reverted, so those two belong to the pre-existing failures recorded for this repository rather than to this consolidation.
A titled composer rule whose closing rule sits more than one row below the composer row - which a ghost suggestion wrapping onto a second row produces on a narrow pane - is a separate scoped gap that this change does not close: the sandwich exception requires adjacency on both edges, so that shape still reads `unknown`, the same safe direction as before.
...
When a live titled rule is declined on one of the documented boundaries above, the guard notes that pane with the concrete reason and names the leftover the proof could not count, rather than reporting a classifier behaving as specified as a regression; a refusal for any other reason still fails loudly.
Its closing note counts the panes actually examined and reports plain rules, documented refusals, and read titled rules separately, so it can no longer claim a titled rule was absent from zero panes while the run above it failed.
