...
Issue #4306's recorded Herdr `pane read` of Kimi Code CLI 0.42.0 on Herdr 0.9.0 (0.41.0 draws the same layout) reproduced a second cursorless rejection on 2026-09-13.
Kimi 0.4x draws its status footer (`Never Ask  K3-256k thinking: high  …/scratchpad/lab/wt  master`, then a right-aligned `context: 0% (0/256k)`) directly under the bordered composer's bottom border, so the cursorless stale-box rule read a genuinely empty composer as `unknown` and every Herdr Kimi spawn failed readiness or delivery confirmation, while the same screen with a tmux cursor on the box read `empty`.
The classifier now tolerates only that permission-mode footer row directly below a bordered box (`FM_COMPOSER_KIMI_STATUS_RE_DEFAULT` in `bin/fm-composer-lib.sh`, overridable through `FM_COMPOSER_KIMI_STATUS_RE`), so any other contiguous activity below a box still proves it stale.
The portable regressions in `tests/fm-composer-lib.test.sh` feed the real capture through the Herdr profile and the tmux cursor profile and prove idle is `empty` while typed text above the same footer stays `pending`, and `tests/fm-kimi-harness.test.sh` drives the spawn readiness and delivery gates over the same footer shape.
Those captures are this change's evidence; the live guard above remains the refresh command for the matrix's Kimi row, which is still unverified live.
