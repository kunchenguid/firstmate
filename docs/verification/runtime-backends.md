...
The classifier now reads that overhang as variable: a bottom at the top's own width tolerates any residue-free embedded title, and a wider bottom is accepted only when its embedded text is a valid grok model title, `<model> (<effort>)` with an optional middot-separated permission cell (`_fm_composer_grok_title_ok` in `bin/fm-composer-lib.sh`). The wrong verdict was reproduced live on 2026-09-21 on a real narrow-pane Grok render on the Herdr backend, which read `unknown` while the same bordered empty composer on a wide pane read `empty`. A constructed 1.0.41 fixture encodes that narrow-pane shape: it reuses the 1.0.5 capture's box geometry, swaps in the current `<model> (<effort>) · <permission>` title, and widens the bottom well past the old three-column cap; it is not a verbatim capture. The portable regressions feed both the 1.0.5 capture and that fixture through both the shared Herdr capability profile and `fm_backend_herdr_composer_state`, and prove idle is `empty`, typed content is `pending`, and an unrecognized oversized title remains `unknown`.
Grok was not installed on the verification machine for this 2026-09-14 change, so the live guard still owes a refresh against the current release rather than treating the portable capture as current live evidence; the variable-overhang read is pinned only by the portable 1.0.5 capture and the constructed 1.0.41 fixture, and may need adjustment if Grok's title shape changes.
