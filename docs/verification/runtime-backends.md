...
- Both-postures refresh: on 2026-09-09, on macOS 26 (Darwin 25.5.0) arm64, Node v24.13.1, after the away-posture dispatch, branch, and guarded-script changes, `FM_PI_BRANCH_LIVE_E2E=1 bin/fm-test-run.sh tests/fm-pi-branch-live-e2e.test.sh` against the globally installed `@earendil-works/pi-coding-agent` 0.81.1 printed all six current lines, including `ok - real Pi SDK 0.81.1 accepts the branch session construction and preserves an unpromptable wake`, `ok - real Pi SDK 0.81.1 rejects a post-construction 429 to watcher-owned main delivery without losing its durable row`, `ok - real Pi SDK 0.81.1 immediately renders appendEntry in the active transcript, persists it across reopen, and excludes it from model context`, and `ok - real Pi SDK 0.81.1 queues a streaming-time watcher wake without before_agent_start, keeps the successor chain, and surfaces consumption of both follow-ups`, with exit 0 and no provider call.
  The same day `bin/fm-test-run.sh tests/fm-pi-branch-extension.test.sh tests/fm-pi-watch-extension.test.sh` passed with the posture cases added (the record-present wake carries the read-back at its tail and never in the prefix, a captain outcome accumulates unprocessed until the archive, a check row is claimed unscoped, and the dispatcher offers every row and alarms instead of waking main), and `bin/fm-test-run.sh tests/fm-pi-primary-types.test.sh` against that same installed 0.81.1 failed on the pre-existing `ModelsRefreshOptions.providers` mismatch that the base commit fails identically, so the strict typecheck's current evidence remains the CI lane's public Pi package rather than this host's stale install.
