...
## Anstoss-Automat working markers

The nudge detector's pane vocabulary reuses only adapter-verified tokens (claude/claude-ox "esc to interrupt", grok "Ctrl+c:cancel", cursor "ctrl+c to stop"); the portable matrix in `tests/fm-anstoss.test.sh` pins each token's silencing effect and the markerless-harness boundary.
Dated live observation, 2026-08-24, firstmate home, tmux 3.6a, harness `claude-ox` (Ox Alpha wrapper): during an active turn the lane `6:fm-fm-brett-bemerkung-zwingend` rendered `esc to interrupt` in its bottom keybind row and was correctly excluded (`fm_backend_agent_state` = alive, CPU accruing), while the standing lane `6:fm-fm-send-fenster-zielaufloesung` rendered a free `❯` composer with no token anywhere in the full `tmux capture-pane -p` surface.
One real `bin/fm-anstoss.sh check` cycle against that standing lane delivered the stage-1 steer through the durable inbox (`state/fm-send-fenster-zielaufloesung.inbox/001.msg`), which the worker moved to `handled/` within minutes and resumed work with a fresh spinner and the token back on screen; the simultaneously-working control lane received no counter and no inbox record.
Refresh this record after any harness upgrade that touches a rendered busy surface by re-running the real-cycle sweep against a knowingly busy and a knowingly idle lane (`FM_HOME=<home> bin/fm-anstoss.sh check`, then inspect `.anstoss-count-*` and inbox records); the fleet-wide rendered-surface drift guards remain the ones listed under each harness above.
