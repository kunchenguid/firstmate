...
- The **steering inbox** is a directory root for every kind, because the record names are allocated by firstmate after launch and cannot be named ahead of time. Every brief kind carries the same inbox section (`bin/fm-brief.sh`), whose acknowledgement is a `mv` of the handled record into `state/<id>.inbox/handled/`; without write on that directory a sandboxed worker could read its steering messages and never acknowledge one, and the re-ring ladder in `bin/fm-task-inbox-lib.sh` would escalate it as stuck for complying. `bin/fm-spawn.sh` creates the inbox and its `handled/` at launch so the root resolves.
- A **scout** gets the same per-task state set a ship crewmate gets, plus two directory roots a ship never needs: `state/.locks/<id>/` for its completion gate, and its own `data/<id>/` for the report. `state/.locks/<id>/` is the whole home of the task's metadata lock since `fm_meta_lock_path` (`bin/fm-wake-lib.sh`) moved the lock under a per-task directory: the lock symlink, `fm_lock_owner_dir`'s `mktemp -d "<lockpath>.owner.XXXXXX"` owner directories, and the `.steal` lock all land inside it, so granting that one directory serves the gate's every lock artifact while `state/` itself stays denied. The directory is pre-created at spawn because a sandboxed scout cannot create it - that would need write on `state/`, the exact reach the per-task layout removed - and `fm_lock_owner_dir` skips its mkdir entirely when the directory is already there. The former whole-`state/` scout grant is what this replaced: it made the gate work, but it let one scout overwrite every OTHER task's status, inbox, lock, and watcher records. The scout report at `data/<id>/report.md` does not exist yet at launch, while the task's `data/<id>/` directory already does (it holds `brief.md`), so the report grant names that directory; the completion attestation the gate writes lands in the same directory as `data/<id>/captain-review`.
...
So a ship crewmate gets its two `state/<id>.*` files, its own `state/<id>.inbox/`, and the git dir; a scout gets those same paths plus its `state/.locks/<id>/` and its own `data/<id>/`, and the git dir; a secondmate gets the parent's `state/<id>.status` file and its `state/<id>.inbox/` - never the shared `data/` root, never the shared `state/` directory for any kind, and never `$FM_HOME` itself, which keeps `.env`, `config/`, `projects/`, and every other home denied.
...
The captain-hold completion gate runs inside the scout's task-scoped set: its lock artifacts are confined to the granted `state/.locks/<id>/`, and its attestation lands in the granted `data/<id>/captain-review`, so `complete` needs no write on the shared `state/` directory at all. A ship crewmate never runs the gate, which is why its narrower per-path grant is sufficient. One gate write is deliberately left outside the grant: the self-announced status append advances the watcher's `state/.seen-<id>_status` marker through a temp file beside it, and that sibling creation is not granted, so the marker is left alone and the watcher surfaces the gate's own `captain-held` transfer line as ordinary new activity. That is the append helper's documented fail-toward-waking direction - the line itself still lands through the granted status file - so over-reporting, not a lost completion, is the cost of keeping `state/` denied.
...
|---|---|
| The composed launch grants a ship crewmate only its own `state/<id>.status` + `state/<id>.turn-ended` files, its own `state/<id>.inbox/`, and the out-of-tree git common dir; a scout those same paths plus its `state/.locks/<id>/` and its own `data/<id>/`, and the git dir; a secondmate only the parent's `state/<id>.status` file and its `state/<id>.inbox/`. The shared `data/` root and the whole `state/` directory are never granted to any kind | `tests/fm-codex-sandbox-grant.test.sh` (portable, no codex) |
...
| The scout set is sufficient for a real status append, report, captain-hold completion gate, and commit, and each root is load-bearing; the scout and ship sets both keep the shared `state/` unwritable so a sibling task's status, inbox, and lock records cannot be reached | `tests/fm-codex-sandbox-grant.test.sh`, by making everything outside the emitted roots unwritable |
