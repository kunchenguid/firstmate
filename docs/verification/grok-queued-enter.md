...
Nothing is required BETWEEN the two keys: this session ran without `--always-approve`, which `bin/fm-spawn.sh` launches every real grok worker with and which renders its own footer segment at a position this record never captured, so requiring an exact separator there would classify a working production pane idle.
