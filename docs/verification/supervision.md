...
So identity is used only to GRANT ownership: on any mismatch the decision falls back to the unchanged contiguous-ancestry test, which still owns the lock for that re-identified process and still refuses a live owner outside this ancestry.
That fallback is read-only in the shared predicate, because run membership is a weaker proof than an identity match; the re-identified record is republished only by `bin/fm-lock.sh`, under the acquisition lock and after its own authoritative ownership confirmation.
`tests/fm-sessionstart-nudge.test.sh` covers that end to end against the real SessionStart wrapper: one live Claude-named process opens `startup`, then `clear`, then `compact` with three different `session_id` values, and each re-emit keeps the lock, avoids the read-only and competing-session paths, and republishes the record under the current identity.

The same bridge exports the identity into every ordinary command of the owning session, so a nested session of the same harness inherits it.
Provenance, not process shape, is what separates the two: SessionStart also publishes `FM_SESSION_PUBLISHER_PID`, the harness pid that published the identity, and `fm_current_session_identity` refuses an identity that carries no publisher pid or whose publisher hosts this process from outside its own contiguous harness run.
A nested session that ran its own SessionStart bridge therefore keeps full identity ownership - including the reparenting refresh - because it published its own pid, while a nested session whose bridge never ran presents its host's pid, is detected across that gap, and is judged by ancestry alone.
The walk is anchored on that pid rather than on harness names, so an unrelated harness process in between cannot hide the host.
The Stop auto-arm applies the same rule to its own payload identity, using its own harness ancestry as provenance instead of anything inherited.
