...
`tests/fm-turnend-guard.test.sh` now pins the tracked `.claude/settings.json` Stop entries against a real grok 1.0.0 hook environment so the inertness contract is covered deterministically rather than only by the opt-in live matrix.
