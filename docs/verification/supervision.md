...
5. End the real handling turn, confirm its synchronous Stop guard retires the prior `state/.claude-rewake-turn` even if the async auto-arm hook does not execute, confirm a concurrent actionable auto-arm publishes its replacement only after that boundary, and confirm the PID-strict turn-end guard still requires a live successor or a current auto-arm claim.
