...
`tests/fm-pi-watch-extension.test.sh` drives the tracked Pi extension and OpenCode plugin with real arm children: an away-mode arm request starts no cycle and delivers no wake, a close landing in the AFK-entry window parks without delivery and automatically re-arms after the flag clears, clearing the flag mid-restoration cannot replay the suppressed wake, pending retries resume without replay, and queued captain-relevant records remain available to daemon housekeeping.
