...
The change was rebased onto upstream main at `d977128` on 2026-09-02 and re-verified there with Node v22.23.1.
That base delivers an actionable wake through the acknowledgement contract added by `fix(pi): preserve watcher continuity across session replacement (#3498)`: a tokenized wake settles only when Pi reports the docked row back through `before_agent_start`, so the coalescing fakes now announce consumption that way, and a coalesced ordinary wake reports as presented by the row already docked rather than as an undelivered pending close.
The discarded-dock regression was removed on this base: a row that is never consumed leaves the pending-delivery loop waiting on its acknowledgement, so no later wake reaches presentation at all and the latch is no longer what that scenario observes; the `agent_settled` clearing edge stays covered by the busy-path re-arm test.
Setting the latch after the send instead of before it no longer fails any test on this base, because `before_agent_start` settles delivery before `agent_start` clears the latch. The set-before-send ordering is kept as the defensive one the extension header states, not as one these tests distinguish.
...
There is no rejection-rollback counterfactual: that behavior and its test were removed once the `void` surface showed the extension can never see a rejection.
