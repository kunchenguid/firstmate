...
Acquisition in `bin/fm-lock.sh` applies that same membership test, so a session is refused only by a live harness outside its own run; comparing the holder to one resolved pid instead reported a session as a foreign competitor to itself and left it read-only in its own home (verified 2026-08-06).
...
That basename is taken by parameter expansion rather than by `basename`, because a macOS login shell reports itself as `-zsh` and every `basename` implementation reads a leading dash as options.
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, drives real acquisition against a self-held and a foreign live holder, and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
