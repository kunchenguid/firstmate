...
The Claude desktop app (Code tab) uses a second version-named tree, `~/.claude/remote/ccd-cli/<version>` (for example `2.1.275`); that path has no harness-name component either, so `fm_harness_path_name` matches only a parent ending in `/.claude/remote/ccd-cli` plus a dotted-digit basename.
