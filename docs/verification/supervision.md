...
An owner record that cannot pin its process through `fm_pid_identity` is never written, because a pid-only record could not later prove a recycled pid was a mismatch; the acquire fails read-only instead.
