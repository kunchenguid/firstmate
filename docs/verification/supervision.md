...
| Grok | Model-driven tracked background `bin/fm-watch-arm.sh` task | Established-state defect fixed: the arm parks without completing once `state/.afk` is observable, then resumes after return. Its completion boundary shares the accepted one-instant residual below and can contribute that transition's single bounded extra wake. |
