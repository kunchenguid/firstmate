...
Session-lock ownership in `bin/fm-session-lock-lib.sh` consults a verified Codex thread-writer flock first, then decides every other harness against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
