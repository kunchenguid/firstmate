...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table - including a macOS login shell, whose argv[0] comm is a leading-dash token such as `-zsh` that an unguarded `basename` rejects as an option - and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
