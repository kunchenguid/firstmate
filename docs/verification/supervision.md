...
Which pid space that ancestry is read in is a third, per-host question, owned by the process backend in the same library and selected by probing whether `ps` answers `-o <field>= -p <pid>` rather than by testing `uname`.
Git Bash/MSYS answers no, and needs the Win32 process table instead, for two independent reasons: its `ps` implements none of those options, and its pid space ends at the MSYS boundary, where a shell launched by a native `claude.exe` is reported with ppid 1 - so the harness is absent from MSYS ancestry rather than merely misnamed, and `kill -0` on a Win32 pid answers about an unrelated MSYS process.
The Win32 chain alone is not sufficient either, because MSYS emulates `fork()` through an intermediate process that exits immediately, leaving every forked child with a dangling Win32 parent that Windows never reparents; the walk therefore follows MSYS `/proc` parentage while it lasts, mapping each hop through `/proc/<pid>/winpid`, and continues up the Win32 chain from the last MSYS process.
On that host the pid written to `state/.lock` is consequently a Win32 pid, which is also what Node-based harnesses such as Pi already report as `process.pid`.
`tests/fm-session-lock-windows.test.sh` covers the backend from any host by forcing `FM_PROC_BACKEND`, injecting a Win32 process table through `FM_WIN_PROC_TABLE`, and standing up MSYS's `/proc` as a fixture behind `FM_PROC_ROOT_OVERRIDE`, so none of it requires Windows or PowerShell to stay covered on CI.
