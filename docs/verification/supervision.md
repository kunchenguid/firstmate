...
A verified harness command name alone is not ownership: the lock is held only while `state/.lock-owner`, written by an acquire in that same home and pinned to the exact process through `fm_pid_identity`, still names the live pid.
That closed a reported lockout in which the ChatGPT desktop app's background `codex` app-server, unrelated to any firstmate home, was accepted as a live lock owner and took the home read-only.
One predicate, `fm_session_lock_reclaimable`, decides every takeover for the acquire, the status report, and both turn-end hooks: a home is taken from the pid in `state/.lock` only on positive evidence that pid is not a live session here - it is gone or is not a verified harness, or a record for that exact pid proves a different process occupies it now.
The same suite pins each branch with real processes: a recycled pid and a live non-harness pid are reclaimed, the recycled one with a note naming the process that had been holding the home; a recorded live owner still refuses a second session and keeps a competing Stop hook inert; and a live harness pid this home can neither confirm nor disprove - no record names it, or no identity can be read to compare - is reported unconfirmed and refused rather than reclaimed, naming the process and both ways out, with its own next acquire repairing the record in place.
An identity that cannot be read records as empty and still acquires: refusing there would put a session read-only over a home nothing else is holding, so the caution lives in the verification, which never reclaims from an owner it cannot disprove.
An acquire publishes and verifies `state/.lock` before it records ownership, and removes the lock it just published if that record fails, so the suite's interrupted-reclaim case proves a claim killed between the two writes leaves a home that still recovers itself rather than one no session can confirm or reclaim.
