...
| Codex | codex-cli 0.145.0 | No Codex lifecycle source; record-free Herdr native `busy` is a positive exception | See below; classifies `unknown codex-unverified` except when Herdr independently reports native `busy`, which classifies `busy herdr-native`; native idle and invalid records remain unknown. |
