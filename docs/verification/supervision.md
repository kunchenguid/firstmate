...
| Codex | `tests/fm-codex-continuity-live-e2e.test.sh` (token-free tier), plus `FM_CODEX_LIVE_STOP_E2E=1` for the interactive tier and `FM_CODEX_LIVE_CHECKPOINT_E2E=1` for the foreground-checkpoint fallback tier | codex-cli 0.154.0, 2026-09-13: `codex queue --thread/--message` is present and refuses an impossible thread id; a real interactive Stop hook fired with a `session_id` in its payload, `"async": true` was honored so the turn ended while the hook still ran, `codex queue` into that recorded thread was accepted while the session was live, and the async hook's exit-2 stderr was never delivered to the model. |
