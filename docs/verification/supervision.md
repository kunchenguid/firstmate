...
The table summarizes current blocking and bounded-follow-up evidence across seven harnesses, with Claude's replacement Stop-owned path revalidated on 2026-09-03, Cursor's stop-hook park validated on 2026-08-13, and omp's blocking `session_stop` hook validated on 2026-09-05.

| Harness | Version verified | Mechanism | Observed result |
| --- | --- | --- | --- |
| Claude | 2.1.259 | Cooperative blocking `Stop` guard plus `asyncRewake` auto-arm | See the current credentialed result and detached-daemon evidence boundary below. |
...
Session-lock ownership in `bin/fm-session-lock-lib.sh` uses the session's contiguous harness ancestry.
Detached Claude Stop delivery is not authorized by `--spawned-by` metadata; [`watcher-continuity.md`](../watcher-continuity.md#ownership) documents the fail-closed boundary instead of treating detached daemon ancestry as the foreground session.
...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees, while it and `tests/fm-claude-stop-autoarm.test.sh` cover detached `--spawned-by` non-authority and zombie-owner recovery visibility.
The detached-daemon paths currently have deterministic evidence only: the suite proves that `--spawned-by` metadata is inert for live foreground owners and fails closed with a durable failed epoch and notice for stale owners instead of reclaiming supervision silently.
The current credentialed live guard observed foreground-ancestry Stop delivery on `claude -p` and authenticated every real Stop delivery, so it neither exercised detached daemon delivery nor produced an unbound failure episode.
...
| --- | --- | --- |
| Claude | `FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` | See the current credentialed result and its detached-daemon evidence boundary under [Turn-end guard](#turn-end-guard). |
