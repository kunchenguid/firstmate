...
| Codex | codex-cli 0.145.0 | None usable | See below; the semantic source remains `unknown codex-unverified`, while `fm-crew-state.sh` now applies its separate bounded tmux-pane fallback. |
...
The crew-state fallback does not claim a structured lifecycle source: portable cases in `tests/fm-crew-state.test.sh` prove that a Codex tmux busy surface reports working, an empty or draft composer reports idle, and a bare shell without an agent reports stopped, while an unreadable agent surface remains unknown.
