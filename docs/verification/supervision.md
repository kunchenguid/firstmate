...
The ordering the fake models was read from the installed `@earendil-works/pi-coding-agent` runtime rather than assumed.
On an idle agent `sendUserMessage` runs the whole handling turn inside the call: `agent_start` is emitted at `pi-agent-core/dist/agent-loop.js:49` and `:67`, before the returned promise resolves.
On a busy agent the row docks and the call resolves at once, and the run already in flight drains that row inline (`agent-loop.js` sets `pendingMessages` from the follow-up queue and continues), so a docked row is consumed with no second `agent_start`; a row can also be discarded without ever being consumed, which is what Escape while streaming does through `clearQueue`.
`agent_settled` is the one boundary both busy-path outcomes share: `agent-session.js:784` emits it from the `_runAgentPrompt` `finally`, and its type doc records that it fires after a run has fully settled with no automatic retry, compaction, or queued continuation left.
An earlier draft of this record claimed the fake reproduced the runtime ordering on the busy path by having a continuation run emit `agent_start` for a docked row; that was wrong, and the fake now leaves the busy-path transitions (inline drain, queue clear, settle) to the test driver instead of treating a docked row as a no-op.

Each regression was run against deliberately broken variants of the extension to establish it is not vacuous.
...
The unmodified extension fails only the burst test; setting the latch after the delivery await fails only the idle-consumed re-arm test; classifying urgency by call site alone fails both restoration-failure tests; dropping the rejection rollback fails only the rejected-delivery test; sharing one latch across generations fails only the session-replacement test; removing urgency entirely fails five tests, the four failure-bypass tests plus the rejected-delivery test; and clearing the latch on `agent_start` alone, which is the defect this record supersedes, fails exactly the two busy-path re-arm tests.
Clearing on `agent_settled` alone fails no test in the suite: no regression currently distinguishes it, because the `restoring` single-flight guard holds across delivery and so suppresses a second actionable close for as long as an idle-path send is still awaiting its run.
The `agent_start` edge is kept regardless, per the presentation contract in the extension header: it re-arms presentation at the moment an idle-path row is consumed rather than at the end of the run that row started.
...
Observed guarantee: `ok - Pi ordinary burst coalesces to one dock row and preserves every durable event`, `ok - Pi idle-consumed row re-arms presentation so a genuinely later wake presents one new row`, `ok - Pi busy-path consumed row re-arms presentation so a genuinely later wake presents one new row`, `ok - Pi discarded dock row re-arms presentation so a genuinely later wake still presents`, `ok - Pi restoration exhaustion surfaces separately from a pending ordinary row`, `ok - Pi restoration-time lock loss surfaces separately from a pending ordinary row`, `ok - Pi retry-path lock loss surfaces separately from a pending ordinary row`, `ok - Pi refused handshake surfaces separately from a pending ordinary row`, `ok - Pi rejected ordinary delivery rolls the presentation latch back`, and `ok - Pi session replacement discards the ordinary latch so a new session presents fresh`, inside a 47-test extension suite at exit 0, with the watcher-arm (14), recovery-loop (2), branch-extension (31), Calm-extension (9), and turn-end-guard (70) suites also at exit 0, `fm-lint.sh` green under the pinned ShellCheck 0.11.0 and actionlint 1.7.12, and `fm-doc-audience-check: ok surfaces=89 local_links=300`.
