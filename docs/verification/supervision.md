...
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, proves recycled PIDs and wrong-home legacy locks cannot preserve stale ownership while same-home legacy locks remain conservative, and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
