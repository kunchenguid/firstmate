...
The headless probes and the interactive probe are compared separately, each against a pre-run copy taken immediately before it, so a difference names the phase that caused it and the run stops before the next phase gets a live home that is already dirty and another chance to write.
Each comparison runs from the guard's cleanup on every exit path, so a probe that fails partway through - the case where a bypassed gate is most likely to have started writing - still reports the verdict for whichever phase was live, and both the pre-run copies and the comparisons sit outside the throwaway lab that cleanup deletes.
...
| Codex | codex-cli 0.146.0 (re-evaluated 2026-08-08) | None usable | See below; classifies `unknown codex-unverified`. |
