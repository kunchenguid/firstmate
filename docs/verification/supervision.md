...
| Herdr supervisor backend | Uses the shared retirement path plus hard deadlines for supervisor target existence, composer state, and submit calls; worker reads keep their existing timeout behavior, and the guarded live run above covered confirmed, unconfirmed, pre-retirement TERM, post-retirement TERM, repeated scans, and buffer cleanup. |
