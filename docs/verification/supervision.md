...
The Claude desktop app (Code tab) uses a second version-named tree, `~/.claude/remote/ccd-cli/<version>` (for example `2.1.275`); that path has no harness-name component either, so `fm_harness_path_name` matches only a parent ending in `/.claude/remote/ccd-cli` whose basename carries nothing but digits and dots with a digit at each end, which keeps every released numbering identifiable (`2.10.0`, `100.1.1`) while a suffixed, dot-edged, or deeper path stays unmatched.
