...
The top-level `"version": 1` key in `.cursor/hooks.json` is load-bearing: without it cursor silently discards the file and every hook is inert.
...
The same deterministic suite pins Cursor session-lock ancestry through an exact `cursor-agent` name or Cursor install-path evidence behind `MainThread` and the legacy `agent` alias, while rejecting unrelated `agent` binaries, decoy `agent/` paths, and later-argument lookalikes.
...
```text
fm-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
fm-doc-audience-check: ok surfaces=68 local_links=241
