...
The adapter therefore refines startup-reason continuation, resume-selection, existing explicit-session, and fork CLI invocations to the context-preserving wrapper source. For `--session-id`, a populated session or a session header older than the current Pi process proves restoration; a newly created ID remains a true startup. A continuation never records or replaces the true-start baseline; if the next compact encounters the prior process identity or a missing baseline, it conservatively injects current AGENTS.md before the digest.
