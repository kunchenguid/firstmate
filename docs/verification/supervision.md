...
Session-lock ownership in `bin/fm-session-lock-lib.sh` is decided against a session's whole contiguous harness ancestry rather than one pid chosen from the walk, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
A session pid the harness itself publishes (`CLAUDE_PID`) also settles ownership directly and is the identity source that remains when the process-tree walk is severed; [`claude-stop-autoarm-windows.md`](claude-stop-autoarm-windows.md) owns that evidence.
