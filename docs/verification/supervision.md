...
The same suite pins each branch with real processes: a recycled pid and a live non-harness pid are reclaimed, the recycled one with a note naming the process that had been holding the home; a recorded live owner still refuses a second session and keeps a competing Stop hook inert; and a live harness pid with no record, with a record about some other pid, or with a recorded identity that cannot be compared is refused rather than reclaimed, with its own next acquire repairing the record in place.
