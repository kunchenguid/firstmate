...
The unmodified extension fails only the burst test; setting the latch after the send instead of before it fails only the run-start consumption re-arm test; clearing the latch on `agent_settled` alone fails that same test, because a row consumed at run start would otherwise stay latched for the rest of that run; clearing it on `agent_start` alone, which is the defect an earlier round of this change carried, fails exactly the two busy-path re-arm tests; classifying urgency by call site alone fails both restoration-failure tests; removing urgency entirely fails all four failure-bypass tests; and sharing one latch across generations fails only the session-replacement test.
