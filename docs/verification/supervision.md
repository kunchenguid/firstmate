...
Ancestry alone cannot answer it when Claude Code serves the call from a per-user worker pool that is reparented to init, because the chain then terminates at pid 1 and never reaches the interactive session that acquired the lock.
The session pid Claude Code publishes as `CLAUDE_PID` closes exactly that gap, and it only ever widens acceptance: a lock the session does not already record as its own is still decided by the ancestry walk, and the published pid is trusted only while it is still a live harness.
