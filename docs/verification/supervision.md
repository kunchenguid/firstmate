...
Session-lock ownership in `bin/fm-session-lock-lib.sh` is decided against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain when the session itself hosts that chain, or an inner pid when a harness-named daemon parents the session.
That ancestry is only decisive when the chain actually belongs to the session.
Claude Code also hosts an `asyncRewake` hook inside the worker chain of its shared per-user daemon (`claude daemon run` -> `claude bg-pty-host` -> `claude bg-spare`), a branch that never contains the session pid `state/.lock` records, and those processes carry the same `claude` branding in `argv[0]` as a session does.
The infrastructure subcommands are therefore excluded from both session ancestry and lock-owner liveness, so such a firing reports unresolvable ownership instead of confidently reporting a competing session, and `bin/fm-claude-stop-autoarm.sh` records that fault in `state/.claude-autoarm-unresolved-ancestry` for the synchronous guard to name rather than repeating a silent refusal on every turn.
Excluding them only makes them transparent to the ancestry walk, so the shared daemon is additionally a hard stop for the pre-match climb: reaching it before any session has matched proves the caller is in the shared infrastructure chain, and the walk terminates unresolved rather than climbing past it and reporting whatever harness sits above the daemon as this session's own ancestry.
The same narrowing means a lock written before it can record the shared daemon's pid, which no longer reads as a live harness, so the Stop auto-arm treats an infrastructure pid in `state/.lock` as stale but NOT claimable: a session lock whose owner is merely not live would otherwise be claimable, and a fleet self-update would open that window on every home at once, permitting exactly the two-live-sessions-one-home failure the lock exists to prevent.
The record is left for the ordinary session-start acquisition to replace with a real session pid.
Every identity decline reached with no resolvable ancestry records the fault, not only the live-recorded-owner branch, because a missing or malformed lock and a failed recovery are equally permanent for a firing that cannot see its own session; the record is written only after the AFK and need gates, so an idle or away home stays inert.
The record names which lock condition the firing declined on, and only a recorded owner verified live supplies a pid, so the blind-turn banner can never present a shared-daemon pid or an already-dead pid as a competing live session.
The guard reads that fault record only while its own recorded `updated_at` is inside the same freshness bound the epoch ledger is read under, so a stale record cannot explain a later, different failure.
A live process table showing that shape was observed on 2026-08-10 with Claude Code 2.1.227, whose daemon, `bg-pty-host`, and `bg-spare` processes sat in a different branch from the session recorded in `state/.lock`; `tests/fm-session-lock-ancestry.test.sh` pins the identity decision behind a deterministic process table of the same shape and pins both end-to-end outcomes with real processes.
