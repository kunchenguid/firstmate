...
The same suite pins each direction with real processes: a recycled pid is reclaimed with a note naming the process that had been holding the home; a recorded live owner still refuses a second session and keeps a competing Stop hook inert; a live pid this home holds no record for is refused rather than reclaimed, and its own next acquire repairs the missing record in place.
