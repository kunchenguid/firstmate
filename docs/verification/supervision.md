...
That basename evidence is also read case-insensitively, because the macOS Claude desktop app reports its executable as `Claude` in both `comm` and args, and a case-sensitive test there finds no harness anywhere in the ancestry, so every desktop-app session in the home is refused its own lock, runs read-only, and never re-arms supervision.
Only the reported name's case is folded: the harness pattern keeps its exact `pi` and `pi-signed` anchors, and no other evidence path is widened.
`tests/fm-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table, covers the capitalized command name and the anchored exact names, and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
