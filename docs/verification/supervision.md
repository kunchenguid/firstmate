...
| OpenCode | 2.0.10 plugin load, 2.0.11 event probe | V2 terminal `session.execution` subscription (`.succeeded`/`.failed` for the guard, plus `.interrupted` for the watcher arm) | All tracked primary plugins loaded through V2 definitions and passed the deterministic adapter regressions. A 2.0.11 plugin-boundary probe of a full turn observed the `session.execution` lifecycle and no `session.idle`/`session.status`, which is why the guard keys off execution completion. Interactive idle-driven follow-up delivery remains V1-only evidence pending a live 2.x rerun; headless remains fail-open. |
