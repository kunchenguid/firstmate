...
That is why both `bin/fm-harness.sh` and the shared process-name vocabulary in `bin/fm-process-identity-lib.sh` match the anchored prefix `muse-bin-*` rather than an exact name, and why neither can rely on an install-path component: `~/.local/bin/muse-bin-<version>` contains no `muse` path component.
