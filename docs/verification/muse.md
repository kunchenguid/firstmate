...
Whether a run reaches either surface depends on the pane's state at the steer, not on whether Herdr registered an agent for it: an `idle` or `done` native status takes the adapter's idle baseline, where the composer verdict still decides, so the leg runs on it exactly as it runs where Herdr registers nothing.
Herdr 0.9.0 does register that agent and reports it `idle` (measured 2026-09-17 against Muse Code 1.3.0), so that pairing runs the leg; only a pane already busy before the steer - `working` or `blocked` - reports unverified and skips, because that run would not be the idle-composer measurement the leg makes.
Herdr's `unknown` registration placeholder is not one of those: the leg waits through it rather than skipping on it.
...
[`runtime-backends.md`](runtime-backends.md#submit-confirmation) owns that leg's shape and still marks it authored and not yet executed, because no guarded run has been made yet; the first one on a Herdr-capable host with `muse` installed is what turns it into a measurement.
