...
Herdr 0.9.0 does register that agent and reports it `idle` (measured 2026-09-17 against Muse Code 1.3.0), so that pairing runs the leg; only a pane already non-idle before the steer - `working`, `blocked`, or a status the adapter does not classify - reports unverified and skips, because that run would not be the idle-composer measurement the leg makes.
