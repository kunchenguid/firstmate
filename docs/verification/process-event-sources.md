...
| condition->action repeat mode | the same suite proves a `--repeat` watch's successful fire is classified `fired` yet is neither terminal nor announced, is journalled and self-acknowledged as handled with no wake reaching the durable queue, leaves the watch registered and re-armed so a second real condition change rings the action again, stops ringing once retired, and still produces a normal terminal wake when its action fails |
...
| state-root validity guard | `arm` refuses to register a watch against a state root that fails fm-procevent.sh's own private-directory check, writing no spec, trust record, or registration before refusing, and succeeds once the root is private again (`tests/fm-procevent-when.test.sh`) |
| reconcile failure surfaced once per episode | a state root that regresses to group- or world-writable and fails that same private-directory check is reported by the watcher as a single actionable `check: process-event reconcile failed:` wake per failure episode - never swallowed, never repeated every cycle - and the episode marker clears once reconcile succeeds again (`tests/fm-watch-triage.test.sh`) |
| symlinked state-root leaf diagnosed by resolved mode | a state root that is itself a symlink to a directory regressed to group-writable is diagnosed as `bad-mode`, not `missing`: the diagnostic resolves a symlinked leaf to its physical target before checking mode, matching what the private-directory validity check already does (`tests/fm-procevent.test.sh`) |
