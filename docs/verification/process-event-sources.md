...
| silence fails closed | the adapter's published `silent` command suppresses only an `ended` session with no queued content block, and announces a real answer, freeform prose, any recognized content block regardless of its declared count, a malformed top-level content header, a `waiting` or `missing` session, a server error, an unreadable result, and indented payload text imitating an empty content block; the `remote-reply` adapter, which implements no `silent` command, announces every result, and the `when` adapter's own `silent` command (proven in "condition->action repeat mode" below) suppresses only a repeat watch's successful fire, announcing every other result |
...
| condition->action action environment | the same suite proves a registered `--action-env` assignment reaches the action, an assignment with a non-identifier NAME is refused before arming, and a shell-safe NAME that could instead hijack an interpreter or loader (`LD_PRELOAD`, `PATH`, and the same short denylist) is refused too, naming the offending option in every refusal |
| condition->action repeat mode | the same suite proves a `--repeat` watch's successful fire is classified `fired` yet is neither terminal nor announced, is journalled and self-acknowledged as handled with no wake reaching the durable queue, leaves the watch registered and re-armed so a second real condition change rings the action again, never refires on a level that never went false while still ringing again on a genuine subsequent edge, stops ringing once retired, and still produces a normal terminal wake when its action fails or when either the fire journal or the edge marker cannot be written |
| condition->action repeat dedup and its --edge exemption | the same suite proves a plain `--repeat` watch never refires the action on a level that never went false, yet still fires again on a genuine false-then-true edge; and separately, with a self-differencing condition that rewrites its own snapshot on every poll (the shape `bin/fm-nm-state-condition.sh` uses), a `--repeat --edge` watch fires on the very first poll of a fresh runner started after a restart even when no false poll was ever observed in between, and still does not refire on a poll that finds no change |
