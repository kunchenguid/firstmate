...
- a silent first-registration baseline for older history, plus an arm-time boundary that still announces activity arriving before a multi-page baseline completes;
...
- the bounded monitoring-loss contract: only a canonical captured result with its immutable `pr-follow` sidecar can count as adapter-produced invalidity and reach quarantine, while caller-selected or foreign files stay loudly refused as tampering without a latch;
