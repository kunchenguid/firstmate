...
- the bounded monitoring-loss contract: only a canonical captured result with its immutable `pr-follow` sidecar can count as adapter-produced invalidity and reach quarantine, terminal refusals release their in-flight marker, orphan markers recover before polling, and caller-selected or foreign files stay loudly refused as tampering without a latch;
