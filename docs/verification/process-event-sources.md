...
It verifies atomic token upgrade of a matching legacy registration, physical canonicalization of a host status path supplied through a symlink-spelled home, private token-stamped staging, blank-line append, stdin and file input, one multiline `--agent-reply` argv element, stable in-flight recovery after a pre-launch crash, append-safe stale recovery, restoration after termination before any response and after prolonged process liveness, refusal to commit an empty successful exit, commit only after a successful nonempty poll response including a terminal response, pending and in-flight acknowledgement fallback during explicit retirement, home-sweep fallback, final-session acknowledgement in the host status log, refusal and status logging when terminal retirement is followed by an identical re-arm that deliberately reuses the registration inode, fresh built-in registration tokens, exact-token restart including late child-record publication, capture of the old child's final queued output, and immediate replacement of that runner generation.
