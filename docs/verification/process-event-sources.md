...
Detached-launch confirmation was re-verified on 2026-09-22 on Linux through the real remote-reply relay with its forking launcher delayed beyond the confirmation window.
...
| confirmed launches only | `reconcile` synchronously waits for the tiny detached launcher to fork before starting its confirmation window, then counts a launch as `started` only after the source is observed owned or its launch-pacing stamp has moved; the real remote-reply relay stays healthy when that launcher is delayed beyond the confirmation window, while a registration that cannot claim is reported `failed=` with a non-zero exit, remains listed `none`, and announces exactly once per failure episode; a source that claimed, ran and exited before confirmation looked is still `started`, a zero-padded confirm window reads as base 10, and an unusable `FM_PROCEVENT_LAUNCH_CONFIRM_SECONDS` is refused by name before any runner is launched |
