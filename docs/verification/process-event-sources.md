...
| GPT Deep Research terminal-watch handoff | `tests/fm-procevent-gpt-deep-research.test.sh` arms isolated existing report-watch records through the public adapter and real runner, proving active waits stay quiet, incomplete in-place record rewrites retry without a false terminal result, verified collected and cleanup-pending archives remain reviewable, manual, blocked, and uncertain outcomes wake independently, a missing collected archive becomes uncertain, re-arms do not add a child or completion, unhandled captures survive restart reconciliation until acknowledged, and prompt or browser-secret fixture fields never enter the captured result |
