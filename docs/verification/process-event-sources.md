...
The identity-aware process-group read - poll-child record, proved pid reuse, and the preserving uncertain shapes - was verified on 2026-09-11 on macOS (Darwin 25.3.0) with `bin/fm-test-run.sh tests/fm-procevent.test.sh`, 110 cases passing; the kernel guarantee it rests on was read from `bsd/kern/kern_fork.c` (`forkproc` skips a candidate pid held by `pghash_exists_locked` or `session_find_locked`) in `apple-oss-distributions/xnu` and from `kernel/pid.c` (`__change_pid` frees a pid number only once `pid_has_task` is false for every pid type) in the Linux tree.
...
| crashed leader with a live group | `SIGKILL` on only the runner leader leaves its blocking child group alive; reconcile preserves that leaderless claim without starting or signalling anything, `start` runs nothing beside it, the strand is queued as one `check` wake keyed by source and claim token that names the recorded poll child it found alive and that a second cycle does not repeat, and reconcile still reclaims a generation with no leader and no surviving group |
| stale leader with a live generation | a claim whose recorded pid is alive under a different identity while the runner's own poll child is still alive is listed `orphaned`, is never relaunched by `reconcile` across cycles, is announced once naming the `start` command that clears it, and `start` reclaims it while the dead generation's leftovers can be tidied and refuses with `cannot claim source`, replacing nothing, when they cannot |
| proved pid reuse | a claim naming a live stranger's pid under the ended runner's identity, with a poll-child record naming a process that has exited and a claim record dated before the stranger started, is reclaimed by `reconcile` in one cycle: the replacement source runs, the claim no longer names the stranger, the stranger and its process group are never signalled, no strand is announced, and the source lists `live` |
| unproved pid reuse preserves | the same claim shape with a stranger that started before the claim was written is not relaunched, is reported `uncertain=1`, is announced once naming the `start` command, and is reclaimed on its own once the stranger's group is gone |
| live poll child recognized | the runner writes a token-bound poll-child record beside its claim; a child that leaves the runner's process group and exec's into another program - its process image changed while its recorded start identity still matches - keeps its generation owned after the runner's whole group is killed, `reconcile` starts nothing beside it and names that child's pid as alive, `list` reports `orphaned`, and the generation is reclaimed on its own once that child ends |
| unidentified members preserve | a claim whose leader and recorded poll child are both gone while a group with the runner's number still has members is not relaunched, its group is not signalled, the strand says the members cannot be identified and names the check a human makes rather than a command, and the source is reclaimed once that group empties |
...
| PID-reuse safety | retirement refuses a live PID whose identity differs from the claim before signalling, and a surviving generation keeps `reconcile` and `retire` from cleaning up the stale generation on both ordinary and failed reservation-removal paths; the stale-leader row above owns what a deliberate `start` does there |
