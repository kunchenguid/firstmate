...
| private state containment | every public command refuses a symlinked or non-directory state path it can reach, and a symlinked per-source runner record, before it reads or writes, and a refused start releases the source for retry |
| containment is scoped, not blanket | each command is held to exactly the leaves it touches: `retire` and `sweep-home` reach only `state`, `state/procevent`, and the machine-wide claim root, so a damaged `state/procevent-inbox` alone leaves them working and never strands the teardown sweep, while a damaged `state` or `state/procevent` still refuses them with nothing changed behind the symlink |
| damaged state is loud, never silent | supervision counts only sources this home physically holds, so a source reached through a symlinked `state` or `state/procevent` is never its wait, while a source that genuinely lives in the home still counts even when a sibling leaf is damaged; the unserviceable path is reported as a named, actionable diagnostic on a channel the reporting path actually surfaces - `bin/fm-guard.sh` prints the full banner on every run, including the no-supervision-needed early exit, and defers the repair in a read-only session, and `tests/fm-turnend-guard.test.sh` proves the turn-end guard names it while still blocking on an in-home source, and that on the non-blocking allow path it reaches Claude's stdout `systemMessage`, Pi's displayed custom message, and OpenCode's probed toast once per episode rather than once per turn, and the legacy Grok adapter's own unlatched stderr line, none of them forcing a continuation, with the Claude latch proven to survive repeats yet re-announce on a restart, a new damaged path, a missing session id, or damage recurring after a repair |
