...
| typed untrusted ingress | the public ingress privately captures bounded payload bytes before publishing a payload-free wake, never writes the captain inbox or executes payload text, durably deduplicates and coalesces one source-scoped delivery revision, preserves later revisions, recovers publication failure through reconciliation, replays after restart, and suppresses handled retries; `tests/fm-linear-reconcile.test.sh` drives the one-shot authoritative Linear producer through workspace pagination, active Bug filtering, canonical ingestion, unchanged-revision deduplication, and fail-closed incomplete responses |
