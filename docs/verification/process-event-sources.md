...
| generation-bound source readiness | the Signal adapter reaches the blocking fake receive before `send` returns, immediate account-discovery failure cannot impersonate readiness, and only the live runner generation can publish or clear its marker |
