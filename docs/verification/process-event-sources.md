...
## Session-derived reply and listener routing

Verified on 2026-09-22 on macOS with `lavish-axi` 0.1.75 using two private servers sharing an isolated session store and a separate Chrome profile.
The saved session contained the artifact's canonical `file` and the URL of the server that opened it.
As a negative control, `lavish-axi poll` against the other server persisted an agent reply while the opened browser remained Working and did not display that reply.
Running the Firstmate poll adapter under that same wrong environment instead derived the opened server from the session: the reply appeared live and Working cleared to listening, with the browser's `performance.timeOrigin` unchanged.
Only synthetic feedback on the isolated board was used; no existing review was touched.
This verifies routing across the reported same-filesystem split, not Lavish server replacement or its own discovery/failover policy.
...
| session-derived Lavish routing | the three-round worker fixture starts and recovers listeners under conflicting host/port environments and configuration, and proves every reply/poll uses the board's saved session endpoint; direct polls cover Unicode artifact paths, hostnames, IPv6, session endpoint changes, quiet retries, and refusal before reply consumption when session evidence is absent or invalid; spawn coverage still proves the configured opening address enters the worker launch |
