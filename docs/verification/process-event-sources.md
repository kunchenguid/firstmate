...
| typed untrusted ingress | `tests/fm-procevent-external-event.test.sh` proves the public ingress privately captures bounded payload bytes before publishing a payload-free wake, never writes the captain inbox or executes payload text, owns the canonical Linear issue-revision encoding, durably deduplicates and coalesces one source-scoped delivery revision, preserves later revisions, recovers publication failure through reconciliation, replays after restart, and suppresses handled retries |
