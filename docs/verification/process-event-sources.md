...
| automatic Lavish registration | another primary watcher cannot claim a Lavish artifact inside the Studio home, while the Studio watcher discovers and registers that session through the single process-event owner, captures its pending `Send & End` feedback without invoking destructive poll, and publishes the ordinary Lavish wake |
| Lavish discovery failure | repeated malformed-store discovery failures publish one bounded check wake for the failure episode, and a repaired store clears that episode so discovery can resume |
| complete Lavish recovery acknowledgement | repeated captures under the same fixed byte bound emit distinct contiguous chunks that reconstruct the exact oversized snapshot, retain the source before the final chunk, present the full prompt and attachment across those chunks, and remove the snapshot and cursor only after the final durable capture |
