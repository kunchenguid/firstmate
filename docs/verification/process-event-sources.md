...
| single delivery per generation | after that first proactive wake, a still-unhandled result keeps being re-announced onto the durable queue but never wakes the watcher again, and an acknowledged result is neither re-announced nor reported |
| proactive-delivery crash and drain boundaries | dotted and underscored source ids at the same sequence receive distinct markers; a concurrent drain cannot consume between queue revalidation and marker commit; failed output and a crash before marker commit replay, while a crash after successful output and marker commit suppresses a duplicate |
