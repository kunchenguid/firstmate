...
| automatic Lavish registration | one primary watcher reconciliation discovers an unregistered Lavish session in the local store, registers it through the single process-event owner, captures its pending `Send & End` feedback without invoking destructive poll, and publishes the ordinary Lavish wake |
