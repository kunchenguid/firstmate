...
| rebind-all preserves repeat and the action environment | `publish_spec` republishes every field `spec_load` parsed - not only the action hash - so a rebound `--repeat --action-env` watch still fires with its assigned environment and its non-terminal `repeat: continues` outcome instead of silently downgrading to a one-shot watch with no environment on its next fire (`tests/fm-procevent-when.test.sh`) |
