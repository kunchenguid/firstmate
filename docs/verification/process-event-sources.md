...
| shared state-root establishment | a state root another firstmate script created first under a host umask that left it group-writable is established private on first use rather than refused, the tightening leaves no group or other write bit and converges on repeat, and a root that cannot be tightened fails closed through the owned interface |
