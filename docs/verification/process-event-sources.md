...
| single delivery per source and sequence | after that first proactive wake, a still-unhandled result keeps being re-announced onto the durable queue but never wakes the watcher again through the surfaced marker; once a drain has presented and consumed the queued records and the source result is acknowledged, it is neither re-announced nor reported |
...
| bounded re-announcement until handled | a durably captured result with no handled acknowledgement is re-announced by `reconcile` with the same source and sequence on every call - not only the first restart after a crash - so a presented-but-unhandled result re-enters the durable queue for the next presentation after a simulated replacement session |
