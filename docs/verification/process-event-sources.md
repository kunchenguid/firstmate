...
| adapter-owned application of a captured result | a remote-secondmate reply captured through the real relay in an isolated home reaches that secondmate's local status channel, settles its correlated pending-reply expectation, re-arms the next cursor-anchored source, and is acknowledged with no handler step; its published `check` wake remains the announcement even when every input line is quarantined, and replaying that capture through the adapter is idempotent |
...
| malformed-correlation isolation and announcement | `tests/fm-remote-reply.test.sh` drives one captured delta whose malformed correlation tokens appear first, in the middle, and last, with valid correlated lines between them: every valid line ingests exactly once in source order and resolves its pending request, every malformed line is quarantined privately, the cursor advances past the whole delta, and the generation announces its quarantined lines exactly once on the wake queue; replaying the generation, duplicating it under a second sequence, and replaying from the quarantine, valid-effect, and unproven-announcement boundaries each repeat no effect |
| producer-side correlation refusal | `tests/fm-pending-reply-lib.test.sh` proves the shared token producer, the outbound request embedder, and `bin/fm-secondmate-report.sh` all refuse a correlation that is not exactly 16 hexadecimal characters, emitting no bytes and creating no status file, while `tests/fm-remote-reply.test.sh` composes that real helper with the real capture, quarantine, correlation-resolution, and cursor owners end to end |
| identity-bound automatic handling | `tests/fm-remote-reply.test.sh` automatically applies a captured reviewer-home generation through its immutable `remote-reply` adapter identity, reaches only `state/reviewer.status` through the existing validated ingest, resolves the pending correlation, and remains idempotent on replay |
