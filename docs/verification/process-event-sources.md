...
It verifies private staging, blank-line append, stdin and file input, one multiline `--agent-reply` argv element, stable in-flight recovery after a pre-launch crash, append-safe stale recovery, restoration after termination inside the delivery grace window, removal after the grace window, exact-registration restart, capture of the old child's final queued output, and immediate replacement of that runner generation.
