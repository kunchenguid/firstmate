...
| complete Lavish recovery acknowledgement | a recovery snapshot whose first capture is bounded remains present after that truncated result, then a complete retry captures the full prompt and attachment before the adapter removes the snapshot |
