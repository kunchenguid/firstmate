...
## The visible Lavish answer-receipt lifecycle

Verified on 2026-08-31 on macOS (Darwin 27.0.0) with `lavish-axi` 0.1.53, against a live Lavish session driven with `chrome-devtools-axi`, plus the behavior tests in `tests/fm-procevent-lavish-receipt.test.sh` and the updated `Send & End` regression in `tests/fm-procevent.test.sh`.

Observed against the published poll interface, live:

- A poll armed with `--agent-reply "<text>"` presents that text as an agent message in the review page's Conversation panel for as long as the session lives, and the page's standing "Your agent is not listening" warning is absent while the armed poll is connected.
- After the reviewer presses `Send & End`, one more poll armed with `--agent-reply` returns immediately (rc 0, about one second) with an empty `ended` session and still presents the reply in the ended page. This is what makes the final receipt deliverable before retirement instead of a post-end impossibility.
- A poll of a missing session returns an error result instead, so nothing can be displayed; the adapter retires such a source with its receipt durably queued and undelivered.

The receipts record backs every visible state, and the tests prove each state exists only after its owning fact: `Received` after durable capture, `Saved` only from the keyed-answer intake's returned verdict (rejected rows reported as counts), `Applying` and `Complete` only from the handler's explicit `applying` and `complete` calls, an exact content-digest replay of an already-captured submission stated as already received with no second intake effect, a resubmission whose own intake outcome closed an answer its first delivery could not, stated and saved as the new action it was, and a round carrying only a written comment acknowledged as that comment. The `Send & End` regression proves the retired review leaves exactly the human's feedback wake behind: the pure receipt-delivery capture is acknowledged before publication and wakes no handler. A runner killed between its durable capture and the seam is proven recoverable: reconcile journals that round's `Received` from the capture it can prove, states no save it cannot, re-applies no answer, and the review retires only after the recovered receipt was presented on the next armed poll. A runner that instead died after its keyed-answer intake returned is proven to keep that round's `Saved` verdict, recovered from the outcome the dead generation staged and refused when the staged outcome names any other generation.

The loss limitation is unchanged and the receipt never contradicts it: the journal proves what reached Firstmate, never what the source delivered to the browser, so a submission lost in the published poll's destructive clear-before-return window still shows nothing, and no receipt text claims at-least-once, no-loss, or lossless delivery.
