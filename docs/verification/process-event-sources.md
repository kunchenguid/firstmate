...
Exercised by `tests/fm-procevent.test.sh` against a fake blocking source whose completion is a process event, not a timer; by `tests/fm-procevent-external-event.test.sh` through the public typed ingress and existing reconciliation path; for the two supervision-delivery rows below, by `tests/fm-watch-triage.test.sh` driving a real `bin/fm-watch.sh` over a real capture; and for adapter-owned application, by `tests/fm-remote-reply.test.sh` driving the real remote-reply relay end to end in an isolated home:
...
| healthy-home invariance | homes with no registration, unhandled result, or owned runner claim retain ordinary inert supervision and teardown behavior |
| source-only supervision | a registered source with no task metadata trips the shared predicate and general guard |
| result-only supervision | an unhandled captured result with no registration or task metadata trips the shared predicate; its handled acknowledgement removes that supervision need |
...
| typed untrusted ingress | `tests/fm-procevent-external-event.test.sh` proves the public ingress privately captures bounded payload bytes before publishing a payload-free wake, never writes the captain inbox or executes payload text, uses the exact canonical Linear delivery identity, durably deduplicates and coalesces independent producer deliveries of one revision, preserves later revisions, recovers publication failure through process-event reconciliation, replays after restart, and suppresses handled retries; automatic Linear detection, external reconciliation, and routing are outside this repository boundary and are not proven |
| inertness | a home with neither a registered source nor an unhandled result generates no process-event state, starts no process-event child, and does not need supervision |
