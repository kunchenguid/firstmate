...
| stream ceiling sensitivity | the `stream` fixture's first 16763-byte chunk alone exceeds the declared 16762-byte ceiling and is held until the source is released, so the real bounds stream it while the source is open; the same fixture driven against a copy of the adapter with the bound widened to 20000 - above the chunk but below the 32768-byte payload - streams nothing early and then delivers the whole payload, which is the red state the ceiling assertion catches |
