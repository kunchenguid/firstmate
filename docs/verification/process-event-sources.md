...
Verified 2026-09-02 on macOS (Darwin 27.0.0, bash 3.2.57) with ShellCheck 0.11.0, jq 1.x, and no forge credentials, entirely through `tests/fm-procevent-pr-follow.test.sh` (31 scenarios, one green run against this head), whose stub `gh` and `glab` executables serve representative REST payloads as raw JSON while the adapter's own production projections (its real `--jq`/`-q` filters and its real `JSON::PP` programs) produce every asserted row, so no fixture can drift from what the forge path emits.

The suite proves the load-bearing guarantees end to end through the adapter's public commands plus the generic runner:

- a silent first-registration baseline (`arm` -> first poll stores maxima, head, and state maps, emits nothing);
...
- the per-document event bound: with `FM_PR_FOLLOW_MAX_EVENTS=1`, a poll carrying three GitLab thread resolutions and a merge truncates the review events and marks the document `dropped: 1`, while the `pr-state` line the cursor advance depends on is exempt from the bound and still announced, and the truncated remainder is announced on a later poll instead of being lost; a configured event or map bound larger than what the apply step accepts is refused before the first poll rather than composing a document this adapter's own apply would reject;
- the deterministic aggregate bound: with five tracked PRs sharing a two-second rotation slot, reconciling the whole roster at once still keeps the startup polls within one burst per elapsed slot (plus one boundary) until the last baseline lands, and a later eight-slot window shows every source polled (no starvation) while total poll bursts stay within one per slot plus one;
...
```sh
bin/fm-test-run.sh tests/fm-procevent-pr-follow.test.sh          # 31 scenarios, one green run
...
Two environment-only failures on this macOS host are pre-existing and identical with and without this change: `tests/fm-teardown.test.sh` case `herdr-preflight-missing-adapter` and `tests/fm-wake-queue.test.sh` case `a subshell reclaimed its parent's live hold` both fail on plain `upstream/main` and on the candidate A base under bash 3.2.57, because a `source` of a missing file bypasses `|| return 1` under `set -e` in that shell; the repository's Linux CI runs newer bash where both pass.
