...
| `fm-session-lock-lib.sh` | Shared owner of the `state/.lock` record and the session-ownership decision (identity, ancestry walk, holder liveness) for fm-lock.sh and the Claude Stop auto-arm |
