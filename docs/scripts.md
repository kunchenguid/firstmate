...
| `fm-wake-drain.sh`       | Present and acknowledge the current actor's claimed wake rows alongside status, outcome-backstop, decision, divergence, recovery, and supervision checks; print the home's measured `tasks-axi ready` count as `ready=N` on every drain (`ready=unknown` if unavailable), even when status presentation is locked |
