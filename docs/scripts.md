...
| `fm-tasks-axi-lib.sh`    | Shared `tasks-axi` compatibility probe                                              |
...
| `fm-pr-merge.sh`         | Record PR metadata, merge a task's canonical full GitHub URL, then refuse an outcome it cannot prove landed or queued |
