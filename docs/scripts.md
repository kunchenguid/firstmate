...
| `fm-adversarial-review.sh` | Run the adversarial-review loop on a Firstmate-raised PR: derive its tier, stage each round's evidence into the PR, reconcile every lens finding, and own the loop-green marker `fm-pr-merge.sh` requires |
