...
| `fm-gate-lock.sh`        | Serialize heavy validation gates, k3d resets/baselines, and image builds per test host with a kernel flock ([test-host-gates.md](test-host-gates.md)) |
| `fm-gate-lock-entry.py`  | The flock-holding engine behind `fm-gate-lock.sh`, including its local and SSH-remote execution paths |
