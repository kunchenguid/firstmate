...
| `fm-config-push.sh`      | Push declared inherited local material to live local, fleet-peer, or remote secondmates and send the placement-specific config reread when changed |
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture and `+yolo` flag from `data/projects.md` |
...
| `fm-codex-primary.sh` | Capture and validate the authoritative Codex primary thread binding for one home and session-lock generation |
| `fm-codex-queue-wake.sh` | Ring the validated Codex thread with one coalesced drain doorbell without consuming durable wakes |
...
| `fm-config-inherit-lib.sh` | Shared primary-to-secondmate inherited local-material propagation and config-reread delivery, including `config/harness-overrides.json` |
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector and `tasks-axi` compatibility probe                  |
| `fm-scm-lib.sh`          | Shared GitHub/Codebase PR/MR provider seam for URL/remote detection, head/state reads, branch discovery, and merge invocation |
