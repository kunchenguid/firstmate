...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for fm-lock.sh, the Claude Stop auto-arm, and the session-start nudge; Windows/Git Bash resolves identity via the harness WINPID |
