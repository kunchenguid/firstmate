...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (POSIX `ps -o` ancestry; Windows MSYS uses Win32 NtQuery walk + session cache + junction claim links via fm-wake-lib) for fm-lock.sh and the Claude Stop auto-arm |
