...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, optional `--base`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-merge-local.sh`      | Fast-forward a `local-only` task's local delivery target branch after approval       |
