...
| `fm-nm-run-lib.sh`       | Shared branch-and-code-identity attribution, executing-run test, and progress fingerprint for no-mistakes runs |
...
| `fm-wake-drain.sh`       | Present durable watcher wakes, unread informational status lines, OPEN DECISIONS, and captain-call RECORD DIVERGENCE, consume the presented rows in the same invocation, retire the matching recovery generation, then assert supervision health |
| `fm-wake-lib.sh`         | Shared durable wake queue, recovery generations, portable locks, and watcher identity/health helpers |
| `fm-anstoss.sh`          | Anstoss-Automat: state-based detection of silently exited lanes with an automatic stage-1 nudge ladder and O-0018 API-error co-reporting (header owns the contract) |
| `fm-classify-lib.sh`     | Shared wake-classification vocabulary, durable keyed-decision folds and scans, unread informational status-line selection, and the escalation-moment run-progress probe |
...
| `fm-wartebedingungen.sh` | Probe the backlog's deposited waiting conditions read-only and report the waits reality has already ended |
...
| `fm-miniapp-lib.sh`      | Shared settings loading and single-variable bot-token reading for the Telegram Mini App scripts |
| `fm-miniapp-deploy.sh`   | Install, inspect or remove the Mini App service, page and reverse-proxy block on their host ([telegram-miniapp.md](telegram-miniapp.md)) |
| `fm-miniapp-serve.py`    | Serve the Mini App decision page, verify its initData, and write accepted answers to the inbox |
| `fm-telegram-verify.py`  | Check a Telegram Mini App initData signature, its freshness, and its sender          |
| `fm-miniapp-ask.sh`      | Store one decision question on the host and send its Mini App button to the captain  |
| `fm-miniapp-inbox-check.sh` | Fetch accepted Mini App answers into the local inbox and print one wake line when any arrived |
...
## Resolving a crew-dispatch profile array (Stand 2026-08-26)

When a matched rule or default in `config/crew-dispatch.json` resolves to more than one profile candidate, the choice is still made by judgment rather than by a script.
The retired `quota-array-dispatch` skill owned that procedure; it is recorded here as one paragraph so the shape survives until it moves into tool code (open P3 item: teach the dispatch resolution to measure each candidate's own account with `env CLAUDE_CONFIG_DIR=<candidate> quota-axi`, so the mechanical part needs no judgment layer at all).
Read `quota-axi`'s default TOON once per intake and reuse it for every candidate; `spendPriority` in the `quota[]` block is the quota-perspective ranker and already computes the economics, so never rebuild it from headroom, pace, reserve, or window-id lists.
Fall back to a single `quota-axi --json` call only when that TOON is genuinely ambiguous or the installed build is below the floor, and read `quota-axi auth --json` only when a candidate's credential surface is in question.
Apply three cheap orthogonal gates before ranking, and let `spendPriority` rank only among candidates that pass all three: eligibility (the candidate's own authoritative model catalog lists the model, quota applies at the granularity the vendor supplies, and only concrete contradictory evidence blocks - an unmeasurable scope or a missing window is disclosed uncertainty, not ineligibility; `bin/fm-vendor-auth-probe.sh` is the only approved credential probe and only `authenticated`/`unauthenticated` are ground truth), reasoning-class fit (never let remaining quota silently downgrade the required class - report instead), and the runway feasibility floor (`through_reset` passes, `projected_exhaustion` is compared against the likely-completion horizon, and unknown runway stays eligible but is never assumed to pass).
Then pick the highest known `spendPriority`, treating absent or `unknown` as neither zero nor healthy, account for every candidate visibly with its evidence, report genuine ties to the captain instead of breaking them by array order, and escalate rather than route when no candidate can be ranked.
`harness-adapters` owns harness identity, model and provider discovery, and effort fallback; `quota-axi` stays data-only and never recommends, selects, ranks, or infers a route.
