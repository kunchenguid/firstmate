...
| `fm-herdr-endpoint-bind.sh` | Bind verified live legacy Herdr endpoint metadata to its task without runtime mutation |
| `fm-install-shellcheck.sh` | Install the exact ShellCheck pin from a verified official host-platform asset       |
...
| `fm-supervision-instructions.sh` | Render the session-start primary-harness supervision block, the one-line `--repair-line` instruction, or the non-blocking `--liveness-line` continuity instruction |
