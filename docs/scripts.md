...
| `fm-fleet-snapshot.sh`   | Print structured fleet snapshot JSON, with a read-only mode that only consumes its parent-side remote-ledger cache and never contacts remote homes (schema `fm-fleet-snapshot.v1`) |
