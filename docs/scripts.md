...
| `fm-primary-scope-lib.sh` | Shared primary-home predicate (marker, plain checkout, or linked worktree on lock evidence) for tracked hooks and the OpenCode watch-arm plugin |
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for fm-lock.sh, the Claude Stop auto-arm, and primary scope |
