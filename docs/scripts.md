...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, holder liveness, and the durable session-identity binding a reparented worker pool cannot reach through ancestry) for fm-lock.sh and the Claude Stop auto-arm |
