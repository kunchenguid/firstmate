...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for fm-lock.sh and the Claude Stop auto-arm, plus the per-host process backend they read it through (POSIX `ps`, or the Win32 process table on Git Bash/MSYS) |
