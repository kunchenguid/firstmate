...
| `fm-notify.sh`           | First backend of the pluggable issue #106 notifier: fire a best-effort native OS notification (persistent `scenario="reminder"` WinRT toast on WSL/Windows, `osascript` on macOS, `notify-send` on Linux); with `--focus` the toast carries a click-to-focus action wired to the `firstmate:` protocol; `install` arms that protocol + hidden launcher and records the pane id; non-blocking, gated by `FM_NOTIFY`, fired by the watcher on a captain-relevant signal in normal mode and by the daemon on a confirmed away-mode escalation |
