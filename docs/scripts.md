...
| `fm-update.sh`           | Orchestrate optional private-upstream integration and guarded fast-forwards of firstmate and local or remote secondmate homes |
| `fm-private-update.sh`   | Integrate and validate a public upstream in an isolated clone, then publish only private origin/main |
...
| `fm-hermes-primary.sh`   | Launch a persistent Hermes classic CLI as the Firstmate primary manager                |
| `fm-harness-process-lib.sh` | Shared process-identity predicates for harness detection and session-lock ownership |
...
| `backends/codex-app.sh`  | Experimental Codex App worker adapter                                                |
| `backends/codex-app-client.mjs` | Unix-socket WebSocket client for the Codex App adapter                      |
