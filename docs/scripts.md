...
| `fm-workspace-execution-config.sh` | Validate and resolve optional workspace-placement and command-execution defaults |
| `fm-workspace-placement.sh` | Dispatch host or Docker Sandbox placement and exact placement-handle lifecycle |
| `placements/*.sh`         | Provide the host and Docker Sandbox workspace-placement adapters                 |
| `fm-command-execution.sh` | Dispatch the explicit local or Crabbox command-execution adapter                 |
| `executors/*.sh`          | Provide the local and Crabbox command-execution adapters                          |
| `fm-exec.sh`              | Run task-scoped command check, run, inspect, logs, and release verbs              |
| `fm-sandbox-bridge-lib.sh` | Validate and synchronize the host-private Docker Sandbox status and turn-end bridge |
| `fm-spawn-cleanup-lib.sh` | Record and reconcile exact spawn placement cleanup state                           |
