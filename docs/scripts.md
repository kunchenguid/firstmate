...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll and register the contribution observer; refuses a GitHub draft |
