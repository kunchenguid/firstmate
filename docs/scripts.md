...
| `fm-preview-pr.sh`       | Refresh a reusable project PR preview worktree, stop only the prior owned backend/frontend dev servers for that project+PR, restart them on stable available ports, verify readiness and optional expected text, and print the frontend/backend URLs |
