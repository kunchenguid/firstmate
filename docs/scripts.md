...
| `fm-dashboard.sh`        | Create or attach a read-only three-pane tmux observer for one session; never wakes or mutates the target (docs/tmux-backend.md) |
| `fm-dashboard-view.sh`   | Render one read-only dashboard pane (coordinator capture, roster, or details) from tmux and the fleet snapshot |
| `fm-dashboard-loop.sh`   | Refresh one local observer pane on an interval without mutating the observed home |
...
| `fm-dispatch-select.sh`  | Resolve a matched dispatch rule to one concrete profile, owning `quota-balanced` and `primary-available` selection |
