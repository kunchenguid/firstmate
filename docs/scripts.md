...
| [`fm-wake-drain.sh`](../bin/fm-wake-drain.sh) | Present and acknowledge the current actor's claimed wake rows alongside status, outcome-backstop, decision, divergence, supervision-host outcome, recovery, supervision checks, and local secondmate checkout-drift warnings; the script header owns presentation scope and limits |
