...
| `fm-usage-harvest.sh`    | Append one finished task's token counts, elapsed time, and turn estimate to the gitignored usage ledger (sole owner of the ledger line schema) |
| `fm-usage-report.sh`     | Read the usage ledger into a plain-text per-model and per-task usage report |
