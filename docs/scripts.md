...
| `fm-routine-setup.sh`     | Provision the private routines registry and its authenticated watcher check    |
| `fm-routine-scan.sh`      | Emit due routine items and manage their cadence fire state                   |
...
| `fm-skill-map.sh`        | Generate the private cross-repo skill discovery map from skill frontmatter           |
| `fm-skill-compose.sh`    | Symlink a curated skill subset into a per-home Claude composition overlay            |
...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode` and `--web`/`--no-web` surface declaration, with the visual gate for web work), scout, secondmate-charter, and Herdr-lab briefs   |
...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend, enforcing ship-brief delivery and surface contracts |
...
| `fm-supervision-lib.sh`  | Shared supervision-need and watcher-beacon predicate                                 |
