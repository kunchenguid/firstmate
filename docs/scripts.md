...
| `fm-telemetry.sh`        | Arm, disarm, or report the never-auto-armed detached host resource-snapshot recorder that keeps durable pruned daily logs under `state/telemetry/`, and run the watcher's fseventsd footprint early-warning check |
