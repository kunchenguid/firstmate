...
| `fm-watch-arm.sh`        | Verified per-home watcher re-arm; normalizes inherited SIGHUP handling so cleanup traps work, reports `started`, `healthy`, or `FAILED`; `--restart` relaunches only this home's watcher |
...
| `fm-teardown.sh`         | Return a clean, landed ship worktree or retire/release a secondmate home; requires scout reports, checks child work, refuses while unrelated no-mistakes runs or no-mistakes cwd matches make treehouse return unsafe, removes firstmate-owned hook artifacts, and prints the backlog-backend reminder |
