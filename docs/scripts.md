...
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated pull-request, merge-request, Bitbucket pull-request, and Gerrit-change poll sidecars |
...
| `fm-pr-merge.sh`         | Record PR metadata, merge a task's canonical full GitHub, GitLab, or Bitbucket Cloud URL, refuse a Gerrit change because firstmate never submits one, then refuse an outcome it cannot prove landed or queued |
