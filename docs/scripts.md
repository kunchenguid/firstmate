...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, optionally an existing PR plus asserted head and no-mistakes base branches), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
