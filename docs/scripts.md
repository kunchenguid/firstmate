...
| `fm-harness-adapter.sh`  | Agent-harness registry, adapter selection, and per-harness fact dispatch on the harness axis |
| `harnesses/<name>.sh`    | Per-harness fact adapters (currently the verified busy signature) dispatched by `fm-harness-adapter.sh` |
