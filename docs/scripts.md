...
| `fm-procevent-pr-follow.sh` | Follow every PR firstmate raised through its whole lifecycle - comments, reviews, head moves, check transitions, merge, close, reopen - as durable wakes until an explicit retirement |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll and durable lifecycle follow-through |
