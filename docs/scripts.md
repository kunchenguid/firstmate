...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, plus the project's registered `--forge`), scout, secondmate-charter, Herdr-lab, and Herdr-retire-session briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-herdr-session-retire.sh` | Guardedly stop one explicitly named, pre-existing, empty Herdr session Firstmate did not provision (stop-only, never adopts a session `fm-herdr-lab.sh` created) |
