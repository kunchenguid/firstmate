...
| `fm-herdr-task-tab.sh`   | Repaint the one real Herdr sibling-worker tab's marker (● ◐ ? ! ✓) from a task's durable record |
| `fm-herdr-completed-pressure.sh` | Report the one completed-worker resource-pressure episode from durable Herdr task records, never mutating a retained worker |
