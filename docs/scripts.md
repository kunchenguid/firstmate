...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll, seed its movement-poll baseline reading, and register contribution observation |
