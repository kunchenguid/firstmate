...
| `fm-dod-lib.sh`          | One owner of the ship task's mode-specific definition of done and project-instructions block, rendered by both the brief scaffold and a scout promotion |
...
| `fm-promote.sh`          | Promote a scout task in place to a protected ship task with an explicit delivery mode, and write the ship instructions carrying the project-instructions block and that mode's definition of done |
