...
| `fm-preview-pr.sh`       | Refresh a reusable project PR preview worktree from the latest GitHub PR head, stop only the prior owned backend/frontend dev servers for that project+PR, restart them on stable available ports, verify readiness plus `--expect` text, and print the canonical frontend/backend `ready:` URLs |
