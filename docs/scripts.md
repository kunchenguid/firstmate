...
| `fm-fleet-dashboard.sh`  | Generate `.lavish` fleet dashboard HTML, Markdown, and the `data/dashboard.md` pointer |
| `fm-fleet-dashboard-refresh.sh` | Refresh and open the Lavish fleet dashboard read surface                    |
...
| `fm-lane-governor.sh`    | Refuse unsafe spawns by checking lane capacity, spawn leases, memory pressure, and orphaned harnesses |
...
| `fm-mcp-broker`          | Run the local MCP credential broker, render secret-free configs, and proxy relays    |
| `backends/tmux.sh`       | Verified tmux session-provider adapter                                               |
| `backends/herdr.sh`      | Experimental herdr session-provider adapter                                          |
| `backends/herdr-workspace-move.py` | Whitelisted herdr `workspace.move` raw-protocol helper for presentation-space projection (docs/herdr-backend.md) |
...
| `fm-slack-dm.mjs`        | Send one configured AFK escalation Slack DM through `chat.postMessage`               |
