...
| `fm-usage-harvest.sh`    | Best-effort teardown harvest of token usage, elapsed time, and turn estimates; script header owns the gitignored ledger schema |
| `fm-usage-report.sh`     | Read the usage ledger into per-model totals and per-task usage rows; no monetary cost calculation |
