...
| `fm-reviewed-toolchain.sh` | Single owner of the exact reviewed npm toolchain versions, checks, and installs, including the chrome-devtools-mcp global entrypoint |
| `fm-tasks-axi-lib.sh`    | Shared backlog-backend selector and `tasks-axi` compatibility probe                  |
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` exact reviewed-version check for the bootstrap diagnostic         |
