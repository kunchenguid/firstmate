...
| `fm-deploy-trigger.sh`   | Decide and perform the automatic deploy that follows a confirmed merge, and finish one that was refused only because the commit's build had not landed; inert without a deploy policy |
...
| `fm-dod-lib.sh`          | Render the shared ship review rule and mode-specific definitions of done for briefs and promotions; one owner of the no-mistakes `--intent` contract |
...
| `fm-supervision-lib.sh`  | Shared supervision-need predicate (in-flight work, Relay poll, event source, or ready work with a free slot) and the idle-capacity report behind it |
