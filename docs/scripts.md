...
| `fm-backend-hometag-lib.sh` | Shared per-installation home-tag derivation for zellij tab titles, cmux workspace titles, and per-task temp roots |
...
| `fm-task-tmp-lib.sh`     | Single owner of the per-task temp root's path shape, its guarded creation, and its guarded task-scoped removal |
