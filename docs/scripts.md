...
| `fm-watch-checkpoint.sh` | Run one bounded foreground watcher checkpoint; Codex's fallback supervision path whenever its Stop hook cannot be shown to be firing |
...
| `fm-gram.sh`             | Automatic Gram intake: read-only `poll` of `herdr gram list` that captures owner messages addressed to this home and hands each new one to `fm-inbox.sh note` |
