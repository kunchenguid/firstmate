...
| `fm-mission-control.sh`  | Render the fleet snapshot as the self-contained mission control HTML board ([`docs/mission-control.md`](mission-control.md)) |
| `fm-autonomous-action.sh` | Append one narrow, recent autonomous decision or PR-merge record for Mission Control |
...
| `fm-decision-hold.sh`    | File, gate, check readiness, verify, complete, retract, and resolve durable captain-held decisions |
...
| `fm-captain-commitment.sh` | Keep an interrupted explicit captain request visible until completion or backlog disposition |
...
| `fm-browser-session-guard.sh` | Refuse a task-scoped browser command that has no per-task session, via `bin/shims/chrome-devtools-axi` (docs/browser-session-isolation.md) |
...
| `fm-secondmate-restart.sh` | Retire and relaunch one live local secondmate with changed launch flags, keeping its persistent home |
...
| `fm-captain-action-notify.sh` | Compare the current fleet action set with its immediate predecessor and best-effort notify only on new decisions or PRs |
...
| `fm-main-divergence-lib.sh` | Shared fetch-free check for a primary default branch diverged from `origin/<default>` |
...
| `fm-provider-continuity.sh` | Record classified provider failures, report outage eligibility and tiered candidates, and license a cross-provider task handoff |
| `fm-wake-drain.sh`       | Atomically drain queued watcher wakes, emit bounded status, OPEN DECISIONS, and outstanding captain-request sections, then assert supervision health |
