...
| `fm-pr-lib.sh`           | Own canonical task and PR validation plus private atomic PR-poll publication, merge-notification identity, merge-queue ejection identity, automatic enqueue identity, stale poll-program refresh, and retirement |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR-poll sidecars           |
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll |
| `fm-pr-enqueue.sh`       | Re-queue a GitHub pull request ejected from the merge queue, or escalate; never merges |
