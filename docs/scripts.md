...
| `fm-classify-lib.sh`     | Shared watcher absorb predicates and captain-relevant wake classification vocabulary |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge/close poll |
