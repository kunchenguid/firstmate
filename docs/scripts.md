...
| `fm-codex-fabric-env.sh` | Acquire a fresh Fabric token with a hard time bound from an existing Azure CLI session for one explicitly enabled Codex worker process |
