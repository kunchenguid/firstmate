...
| `fm-ship-end-to-end.sh`  | Publish and verify private ship preflight approval for `pr-only` delivery         |
| `fm-agent-bridge-ship-preflight.sh` | Validate and publish one bridge-delivered `pr-only` ship preflight       |
