...
| `fm-spawn.sh`            | Spawn crewmates, scouts, `id=repo` batches, and secondmates on the resolved harness and runtime backend; `--resume-worktree` continues an authenticated existing worktree instead of provisioning a new one |
| [`fm-worktree-identity.sh`](../bin/fm-worktree-identity.sh) | Read-only owner of one worktree's deterministic git identity, content and fingerprint facts |
