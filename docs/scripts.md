...
| `fm-specgraph-capture.sh` | Validate and atomically publish private `specgraph.snapshot.v0.1` semantic drafts    |
...
| `fm-specgraph-lib.sh`    | Fail-open best-effort SpecGraph capture adapter for existing lifecycle write points ([contract](specgraph-export.md)) |
