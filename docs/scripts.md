...
| `fm-agy-trust.sh` | Pre-register agy's workspace trust for one isolated task worktree, or refuse; `--remove` retires it at teardown |
| `fm-agy-turnend-hook.sh` | Install or remove agy's guarded global crew turn-end hook                |
