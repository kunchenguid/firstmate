...
| `fm-supervisor-panes.sh` | Reconcile the zellij-only supervisor tab for active non-secondmate crews             |
| `fm-supervisor-pane-loop.sh` | Render one live zellij supervisor pane for a task until its metadata is gone     |
