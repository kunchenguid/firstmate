...
| `fm-spawn.sh`            | Spawn one task, several `id=repo` pairs, or a persistent secondmate with `--secondmate`; accepts concrete `--harness`, `--model`, and `--effort` profile axes; ship/scout spawns require an explicit resolved harness when dispatch profiles are active and an isolated treehouse worktree, install per-harness turn-end signaling, record the tmux layout and under `FM_TMUX_LAYOUT=pane` split firstmate's current window (recording `pane=`) instead of opening a new task window, and secondmate spawns resolve the secondmate harness, locally sync the home, and propagate declared inheritable config before launch |
...
| `fm-tmux-lib.sh`         | Shared tmux pane primitives for busy detection, dim-ghost-aware and border-aware composer detection, verified submit retry, and pane-aware resolution between a task's `state/<id>.meta` (`pane=` when present, else `window=`) and its live tmux target |
