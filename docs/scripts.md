...
| `fm-fleet-sync.sh`       | Refresh project clones with shallow-history repair, safe fast-forwards, self-heals, `STUCK:` reports, branch pruning, and bounded recovery from an orphaned `.git/packed-refs.lock` |
...
| `fm-parent-channel-lib.sh` | Single owner of this home's parent escalation channel, so a decision closes in the channel where it opened |
...
| `fm-project-depth-lib.sh` | Detect and additively repair shallow project history for fleet sync and fresh ship/scout spawns |
...
| `fm-secondmate-report.sh` | Optional helper to append a correlated parent status or document-pointer report, and the `--escalate` path for a decision the mate raises itself |
...
| `fm-afk-start.sh`        | Launcher-only sourceable entry for the foreground away-mode daemon                      |
...
| `fm-nm-compose-reap.sh`  | Safely removes Compose projects owned by vanished no-mistakes worktrees             |
...
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, remove exact worktree-owned Compose containers and networks, require completed scout deliverables, retire secondmate homes |
