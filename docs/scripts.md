...
| `fm-gh-resolve.sh`       | Pin `gh`'s base repo to the firstmate checkout's own `origin` so default forge calls never fall through to a fork parent |
...
| `fm-context-mode-webfetch-fix.sh` | Reapply the narrow `claude.ai` native-WebFetch compatibility patch to context-mode's marketplace and active cache copies |
