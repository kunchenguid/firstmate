...
| `fm-github-read-pause.sh` | Durably retire automatic GitHub readers while preserving their resumable local records |
...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically publish a static merge poll unless GitHub reads are paused; refuses a GitHub draft |
