...
| `fm-next.sh`              | Deterministically choose the highest-value captain action, compose its preparation packet, and request skill-side preparation when shell evidence is incomplete |
...
| `fm-callsigns-lib.sh`     | Resolve canonical task ids, cooldown-recycled `t1`-`t99` references, and human names |
...
| `fm-task-data-migrate.sh` | Guardedly migrate eligible legacy `data/<id>/` task artifacts to canonical `data/tasks/<id>/` storage and update report links |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture and source relationship from `data/projects.md` for fleet sync, home seeding, and update routing |
| `fm-maintained-fork.sh`  | Integrate a specifically selected upstream release in isolation, validate it, and require separate local acceptance before landing |
