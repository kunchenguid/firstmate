...
| `fm-link-intake.sh`       | Atomically retain and validate private searchable link-intake records, and prepare their queued ingest scouts       |
...
| `fm-update.sh`           | Fast-forward-only self-update of firstmate and local or remote secondmate homes from the publish remote |
...
| `fm-test-run.sh`         | Behavior-test runner: selection, portable lanes, runtime gates, proven-isolated `--jobs`, coverage guard, timing artifacts, and opt-in durable progress journals |
| `fm-test-run-stock-bash.sh` | Run the focused runner contract under stock macOS Bash 3.2, or emit explicit local skip evidence |
...
| `fm-calm.sh`             | Read or atomically update the effective Firstmate Calm presentation preference       |
| `fm-claude-calm-nudge.sh` | Print Claude's effective Calm presentation state for a genuine primary              |
...
| `fm-refill.sh`           | Print the bounded, fail-open refill evidence the heartbeat wake payload carries       |
...
| `fm-ops-inbox-lib.sh`    | Shared operational alert inbox watch resolution: config, thresholds, scan, dedupe record |
| `fm-ops-inbox-poll.sh`   | One bounded read of the operational alert inbox: wake with a digest, or stay silent  |
