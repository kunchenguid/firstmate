...
| `fm-pr-poll.sh`          | Extract and format destination-qualified ready/merged outcomes and provide the byte-static PR/MR poll |
| `fm-pr-check-migrate.sh` | Quarantine older task polls without execution and rebuild only canonical polls       |
| `fm-pr-check.sh`         | Record validated PR identity and branch evidence, print the qualified destination outcome (`--pre-merge` marks it a pre-action reading), then atomically arm a merge poll |
