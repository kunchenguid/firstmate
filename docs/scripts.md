...
| `fm-fleet-snapshot.sh`   | Print structured fleet snapshot JSON, with a read-only mode that consumes but never refreshes its parent-side remote-ledger cache (schema `fm-fleet-snapshot.v1`) |
