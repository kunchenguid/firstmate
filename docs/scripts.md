...
| `fm-update.sh`           | Official-origin fast-forward-only self-update of firstmate and local or remote secondmate homes, with an exact expected-SHA contract |
| `fm-daily-upstream.sh`   | Own guarded daily assessment, pinned updates, public-channel metadata, morning reports, private retention, and static per-user LaunchAgent deployment |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture or upstream-maintenance posture from `data/projects.md` for fleet sync, home seeding, and guarded daily upstream maintenance |
