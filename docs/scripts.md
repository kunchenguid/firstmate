...
| `fm-merge-local.sh`      | Fast-forward a `local-only` project's recorded landing branch, or its default branch, after approval, including a bare repository |
| `fm-review-diff.sh`      | Review a crewmate branch or resolved PR head against the recorded integration branch, or the authoritative default |
