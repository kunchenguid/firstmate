...
| `fm-fleet-sync.sh`       | Refresh registered project clones with safe fast-forwards, self-heals, `STUCK:` reports, branch pruning, and bounded recovery from an orphaned `.git/packed-refs.lock` |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture or project path from `data/projects.md` for mechanical consumers |
