...
| `fm-pr-lib.sh`           | Own canonical task and PR validation plus private atomic PR-poll publication, merge- and review-ready notification identity, and retirement |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR-poll sidecars           |
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, then atomically arm a static merge poll |
| `fm-notify.sh`           | Post one best-effort, idempotent Discord review-ready message for a task PR when `config/discord-webhook` is configured |
