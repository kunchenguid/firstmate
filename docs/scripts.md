...
| `fm-adopt.sh`            | Register an already-running cmux workspace as a supervise-only `kind=adopted` task (no worktree, no branch, no delivery mode, no PR); cmux-only and experimental |
