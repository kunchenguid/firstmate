...
| `fm-backend-hometag-lib.sh` | Shared per-installation home-tag derivation for zellij tab, cmux workspace, and Ryder session names |
| `fm-process-identity-lib.sh` | Shared agent/shell/other process-name vocabulary used by the tmux and Ryder liveness classifiers |
...
| `backends/ryder.sh`      | Experimental Ryder persistent-PTY session-provider adapter                           |
