...
| `fm-secondmate-inbox-lib.sh` | Secondmate steering-backlog measurement and threshold decision only; never moves or handles records |
| `fm-secondmate-melt-lib.sh` | Secondmate commander-model melt decisions, quota-checked replacement profiles, and still-recorded-pin cooldown evidence |
...
| `fm-watch.sh`            | Singleton-safe watcher: absorb benign wakes, run secondmate health gates, detect stalled local-secondmate wake queues, and exit on actionable ones |
