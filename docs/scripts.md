...
| `fm-guard.sh`            | Warn when the primary checkout is tangled, when queued wakes are pending, or when a stale or missing watcher needs a prominent banner; treats a missing watcher as expected during an enabled overnight blackout |
...
| `fm-watch-arm.sh`        | Verified per-home watcher re-arm; reports `started`, `healthy`, or `FAILED`; `--restart` relaunches only this home's watcher; becomes a zero-token, singleton-safe sleeper during an enabled overnight blackout and auto-restarts the real watcher at window end |
| `fm-watch.sh`            | Singleton-safe always-on watcher and default pull-based event source; uses backend-native busy state when available before the shared regex fallback, absorbs no-verb signal and stale wakes only when the crew is provably working, queues and exits for actionable wakes, exits cleanly on crossing into an enabled overnight blackout, and reverts to daemon-owned one-shot behavior while `state/.afk` exists |
| `fm-blackout-lib.sh`     | Shared, unit-testable `fm_in_blackout` predicate for the opt-in overnight quiet-hours blackout (config file, env overrides, injectable clock, evening-extend override), sourced by the watcher, arm, guard, and daemon |
| `fm-blackout-extend.sh`  | Defer an active or upcoming blackout into the evening on demand (`HH:MM`, `+Nh`/`+Nm`, `--clear`, `--status`) by writing a future epoch to `state/blackout-override`                                                |
| `fm-supervise-daemon.sh` | Presence-gated sub-supervisor for walk-away (`/afk`) supervision: wraps `fm-watch.sh`, uses the shared wake classifier and backend-aware stale rechecks, self-handles routine wakes in bash, escalates only captain-relevant events as one verified, batched, single-line digest prefixed with a sentinel marker, and defers injection during an enabled overnight blackout |
