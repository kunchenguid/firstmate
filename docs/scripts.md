...
| `no-mistakes`            | Argv-level dispatch boundary that reserves pipeline-driving `axi run` and `axi respond` operations for marked workers in linked task worktrees |
