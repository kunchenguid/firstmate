...
| `fm-update.sh`           | Merge upstream origin into fork/main, then fast-forward local or remote homes to the fork |
...
| `fm-main.sh`             | Launch the captain's main firstmate session with its standing `FM Main` display name |
...
| `fm-secondmate-command.sh` | Move a persistent secondmate between firstmate command and captain command, and report which state each lane is in |
| `fm-secondmate-command-lib.sh` | Shared read of a lane's recorded command state and its derived per-home copy         |
...
| `fm-ff-lib.sh`           | Shared guarded fast-forward helper for fork/origin pulls and local secondmate syncs  |
