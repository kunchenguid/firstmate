...
| `fm-treehouse-slot-lib.sh` | Shared Treehouse pool-slot recognition and the slot-owner claim spawn writes and teardown releases |
| `fm-pool-leak-lib.sh`    | Report-only session-start scan for pool slots still claimed by a task that is not using them |
