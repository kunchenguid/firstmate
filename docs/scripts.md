...
| `fm-watch-arm.sh`        | Verified home-scoped watcher arm wrapper: launch the watcher detached, follow it, report honestly |
| `fm-watch-checkpoint.sh` | Run one bounded foreground watcher checkpoint for Codex-style supervision            |
| `fm-watch.sh`            | Singleton-safe always-on watcher: absorb benign wakes, queue and exit on actionable ones |
| `fm-afk-start.sh`        | Run the common sourceable away-mode daemon entry, in the foreground or (`--detach`) detached |
...
| `fm-detach-lib.sh`       | Shared portable detachment so supervision survives a reaped harness background task  |
