...
| `fm-pr-lib.sh`           | Own canonical task and PR validation, the per-task browser session name, environment pins and browser-tool capability probe that spawn and teardown share, plus private atomic PR-poll publication and identity-bound retirement |
