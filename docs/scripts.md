...
| `fm-usage-ledger.sh`     | Own the durable, home-private task-usage ledger: its schema, safety rules, event identity, and retention |
| `fm-usage-ledger-lib.sh` | Own the call policy every lifecycle script uses to record into that ledger |
