...
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, report every other open pull request on the recorded head branch, then atomically arm a static merge poll |
| `fm-pr-merge.sh`         | Record PR metadata, merge a task's canonical full GitHub or GitLab URL after refusing a stacked-branch rewrite or an ambiguous head branch, then refuse an outcome it cannot prove landed or queued |
