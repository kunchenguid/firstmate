...
| `fm-lavish-dock-check.sh` | Automatically armed primary-home Lavish dock-reply copy; the watcher copies unpolled prompt occurrences, while the process-event adapter snapshots pending prompts before destructive polling and publishes them through its single process-event wake |
