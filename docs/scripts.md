...
| `fm-pr-lib.sh`           | Own canonical task/PR validation, GitHub check-rollup semantics, private atomic PR-poll publication, merge-notification identity, and retirement |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR-poll sidecars           |
| `fm-pr-review.sh`        | Collect, record, settle, and live-verify evidence-backed GitHub feedback triage     |
| `fm-pr-check.sh`         | Record and watch a validated PR, then require live GitHub review verification before publishing readiness |
| `fm-pr-merge.sh`         | Register PR metadata, verify live merge and review evidence, merge a canonical GitHub or GitLab URL, then prove it landed or queued |
