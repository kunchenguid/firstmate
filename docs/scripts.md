...
| `fm-pr-merge.sh`         | Refuse a merge whose recorded evidence commit is not the PR head, then record PR metadata and merge |
| `fm-evidence-record.sh`  | Record the commit a task's reported verification evidence was measured on           |
