...
| `fm-merge-attribution-lib.sh` | Own the durable, firstmate-private merge-provenance record stamped by fm-pr-merge.sh and fm-merge-local.sh |
| `fm-merge-attribution.sh` | Report whether a task's merge went through the recorded merge path, or reads unattributed |
