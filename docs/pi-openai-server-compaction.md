...
By default it posts remote compaction requests to `https://chatgpt.com/backend-api/codex/responses`; a model with the `openai-codex` provider may instead supply a different base URL, from which the `/codex/responses` path is derived.
It adds Codex identity and beta headers, and extracts the ChatGPT account ID claim from the existing Codex token for the request header.
...
It clears request-shape state on session start or before session switch, fork, and tree navigation; clears live response IDs and WebSocket managers at those boundaries and after compaction; and rebuilds remote history from the active branch after session start, tree navigation, and compaction.
Model selection clears live response IDs and WebSocket managers, while shutdown releases all continuation state and WebSocket sessions.
Pi's ordinary branch summary and tree behavior remains in place because the package's `session_before_tree` handler only clears its runtime state before navigation.
...
| Filesystem access | `src/config.ts` reads the global and project JSON config files. `src/remote-compaction.ts` reads or creates `$CODEX_HOME/installation_id`, falling back to `~/.codex/installation_id`, for Codex identity headers used by remote compaction and the optional direct-OpenAI WebSocket path. | An enabled direct-OpenAI WebSocket request or remote compaction request can create that UUID file if it is absent. The extension does not otherwise enumerate or send repository files itself, but conversation tool results can already contain repository content. |
...
| Outbound endpoints | The custom WebSocket endpoint is fixed at `wss://api.openai.com/v1/responses`. Direct remote compaction accepts the official `api.openai.com` host (or an absent base URL); Codex remote compaction accepts the `openai-codex` provider or `chatgpt.com` host and derives the `/codex/responses` path from the selected model's base URL. | OpenAI or the selected Codex base URL receives the system prompt, conversation context, tool schemas, and model settings during remote compaction. Ordinary direct turns also use server storage because of `store: true`. |
| Provider override | The factory calls `registerProvider("openai", { api: "openai-responses", streamSimple: ... })`. When disabled, or when response-ID continuation is false, it delegates to Pi's built-in HTTP Responses stream; when response-ID continuation is enabled it can use the custom WebSocket stream. | A streaming compatibility bug can affect direct official OpenAI Responses models while the package's custom WebSocket mode is enabled. The stronger rollback is package autoload false. |
