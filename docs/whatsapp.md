...
O encaminhamento publica uma nota durável e um evento `check` pelo proprietário [fm-inbox.sh](../bin/fm-inbox.sh).
O watcher anuncia notas novas também nos ciclos sucessores; no Pi, a extensão principal usa a entrega nativa para iniciar um turno quando ocioso ou enfileirar um `followUp` durante trabalho em andamento.
Esse caminho depende de o watcher do principal usar a revisão atual, independentemente do checkout estável escolhido para a ponte.
Uma tarefa em andamento pode adiar o tratamento; não há prazo fixo nem interrupção forçada do principal.
Uma sessão viva não prova que existe supervisão contínua, e nenhum comando da ponte inicia uma sessão ausente.
O diagnóstico da ponte continua conservador: usa a prova de sessão de `fm-session-lock-lib.sh`, informa `checkpoint` ou `unavailable`, e sempre `unattended_wake_verified:false`.
Essa prova de processo não mede a entrega de uma nota; [Verificação](verification/whatsapp.md) distingue a regressão nativa da aceitação WhatsApp instalada.
...
O principal recebe pedidos pelo fluxo descrito em [Limite de disponibilidade](#limite-de-disponibilidade) e mantém a responsabilidade exclusiva por executar trabalho e publicar resultados.
