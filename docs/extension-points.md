...
- A hook cannot stall firstmate through its output either, because a hook never inherits firstmate's stdout or stderr.
  ...
  `FM_HOOK_STDERR_MAX_BYTES` (default 65536) bounds that capture at both ends: firstmate drains the hook's stderr through a capped writer that keeps at most that many bytes on disk and discards the rest, and relays at most that many bytes with an explicit truncation warning.
  So a chatty or looping hook can neither flood a firstmate script whose output another script captures nor fill `TMPDIR` - and neither can a background descendant that outlives the hook still holding that stderr open.
...
| `pr-ready` | `bin/fm-pr-check.sh`, `bin/fm-pr-merge.sh` | the task's PR URL is recorded (`pr=`) and its merge poll armed; at least once, and normally exactly once, per (task, PR URL), so re-runs do not re-fire it | `$1` task id, `$2` PR URL | `FM_HOOK_TASK_ID`, `FM_HOOK_PR_URL` |
...
The fire is recorded in the task's `state/<id>.meta` as `pr_ready_hook=<url>`, so it survives a merge that fails after the recording: the retried merge does not re-fire the hook, and a PR whose merge never succeeds still fires it once.
That marker is written after the hook returns and only when a `pr-ready` hook is installed, which has two consequences worth knowing.
A home with no hooks records nothing, keeping a task's meta exactly what it was before hook points existed; and firstmate being hard-interrupted inside a `pr-ready` hook leaves no marker, so the next run fires the hook again.
The realized guarantee is therefore at-least-once rather than exactly-once - the deliberate side to err on, since recording the fire first would instead drop it entirely on that same interrupt - so a `pr-ready` hook should be idempotent for a (task, PR URL) pair, as a `post-spawn` hook already must be for a task id.
