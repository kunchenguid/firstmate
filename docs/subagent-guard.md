...
A home is in scope when it has `AGENTS.md`, a `bin/` directory, an existing state directory, and one of: a valid `.fm-secondmate-home` marker, a plain checkout where git-dir equals git-common-dir, or a linked worktree that is its own operational home because the state directory is its own `state/` and the calling session owns `state/.lock`.
The Guard Predicates section of [`turnend-guard.md`](turnend-guard.md#guard-predicates) owns that rule.
...
A crewmate's disposable task worktree is a linked git worktree, which is the shape `bin/fm-spawn.sh` always hands out, and no session owns a lock there, so it is out of scope.
