...
`bin/fm-supervision-lib.sh` also recognizes a registered process-to-event source or an armed standing poll (Relay or operational alert inbox watch) as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without an independent source or standing-poll need, unaccounted primary work therefore reads as idle rather than suspicious.
