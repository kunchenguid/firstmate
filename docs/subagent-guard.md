...
`bin/fm-supervision-lib.sh` also recognizes an X-mode or private Telegram poll as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without an independent remote-channel need, unaccounted primary work therefore reads as idle rather than suspicious.
