...
The other tracked Claude hook entries in `.claude/settings.json` refuse to run under Grok's Claude-compatible settings loading, mostly because Grok already covers those events through its own `.grok/hooks/` registration and running both creates a duplicate path; docs/turnend-guard.md "Harness integrations" owns that guarded inventory and the one entry guarded for a different reason.
