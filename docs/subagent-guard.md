...
A ship or writer scout's disposable task worktree is a linked git worktree, so it is out of scope.
A reader scout's scratch directory is not a firstmate checkout and is out of scope for the same reason as any non-firstmate directory.
A crewmate using delegation tools inside its own task environment is legitimate and stays allowed.
