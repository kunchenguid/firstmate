...
`bin/fm-supervision-lib.sh` also recognizes non-task supervision needs - a registered custom check, a process-event source, or a Relay poll - but unaccounted primary work still contributes nothing to that predicate.
Without one of those independent needs, unaccounted primary work therefore reads as idle rather than suspicious.
