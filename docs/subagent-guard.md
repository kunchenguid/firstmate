...
`bin/fm-supervision-lib.sh` recognizes the explicit registered needs owned by [`watcher-continuity.md`](watcher-continuity.md#eligibility), but unaccounted primary work still contributes nothing to that predicate.
Without an independent registered need, unaccounted primary work therefore reads as idle rather than suspicious.
