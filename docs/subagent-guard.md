...
`bin/fm-supervision-lib.sh` also recognizes a Relay poll, a persistent routine check, or a registered process-event source as supervision need, but unaccounted primary work still contributes nothing to that predicate.
Without one of those independent supervision needs, unaccounted primary work therefore reads as idle rather than suspicious.
