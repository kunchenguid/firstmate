...
A rule about names has gaps of its own, and five rounds found five: a row whose `provider` cell is empty carries no name to sweep by, and a model-scoped row beside an account-scoped one for the same provider is skipped by the sweep because its provider is already there.
A name reaching the output does not mean the row did.
So the invariant is an identity over ROWS: every row the gauge emitted, in every table the reading consults, is either READ into the reading or DECLINED with a reason, and `HEADROOM_ROWS` publishes `emitted = read + declined` on every reading, in prose and in `--json` alike.
Unmeasurable readings carry it too, because a reading that measured nothing is exactly where a reader most needs to know whether anything was accounted for; a reading that never saw a report reports all zeroes rather than omitting the line.
...
That absent marker is deliberately not a layout signal: it means "no value here" and cannot distinguish an undeclared field from an empty cell, so the one place that needs the layout question - may the percentage take the runway's window, because this gauge publishes only one? - asks the block header directly.
Reading it off the marker instead labelled a percentage with the window bounding the runway while keeping its own row's reset, on a report whose sibling row named a different window outright.
