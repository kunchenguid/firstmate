...
It is distinct from the crewmate harness: the harness is the agent process firstmate launches (`claude`, `opencode`, or `pi`), while Orca owns the task worktree and terminal endpoint underneath that process.
