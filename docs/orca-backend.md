...
It is distinct from the crewmate harness: the harness is the agent process firstmate launches (any verified adapter, such as `claude` or `codex`), while Orca owns the task worktree and terminal endpoint underneath that process.
