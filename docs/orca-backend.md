...
It is distinct from the crewmate harness: the harness is the agent process firstmate launches (`claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, or `devin`), while the Orca app owns the task worktree and terminal endpoint underneath that process.
