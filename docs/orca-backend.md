...
- Orca's worktree shape is unverified against Firstmate's spawn-time repository-ownership guard, which refuses any path that is not a linked git worktree sharing the project's git common dir before any harness launches.
- Claude also applies its separate workspace-trust check in `bin/fm-claude-trust.sh`.
