...
A bare shell row is `unknown`, not an empty agent composer; a bare `→` row is recognized separately for classifier parity, while Cursor launches remain unsupported on Orca.
