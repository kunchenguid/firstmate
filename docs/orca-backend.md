...
- `git` with GitHub auth, `no-mistakes`, `gh-axi`, `chrome-devtools-axi` as the cross-harness browser fallback, and `lavish-axi` - the same universal requirements as tmux, minus `tmux` and `treehouse` (Orca replaces both).
