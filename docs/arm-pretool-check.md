...
| OpenCode | `event.input.command` | `.opencode/plugins/fm-primary-pretool-check.js` registers the V2 `execute.before` tool hook, passes one `--command` argument, and throws only for exit 2. |
