...
| Hermes primary | `pre_tool_call` `terminal` args | `.hermes/plugins/firstmate-primary` passes the exact terminal command to the checker and returns Hermes's `{"action":"block"}` directive on exit 2. |
