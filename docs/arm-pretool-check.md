...
The classifier follows recognized wrappers, control-flow command positions, and literal nested-shell execution, so `env`, `nice`, shell or system `time`, named or unnamed `coproc`, `if`/loop/`case` bodies, and `bash -lc` cannot evade the ownership boundary.
