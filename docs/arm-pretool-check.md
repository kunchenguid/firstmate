...
The classifier follows recognized wrappers, control-flow command positions, and literal nested-shell execution, so `env`, `time -p`, `coproc`, `if`/loop bodies, and `bash -lc` cannot evade the ownership boundary.
