...
`bin/fm-arm-command-policy.mjs` is also the sole owner of firstmate's shell classification: it exports the tokenizer and command-position analysis, which the sibling cd-guard seatbelt (`bin/fm-cd-pretool-check.sh`, `docs/cd-guard.md`) and the worker PR-merge policy (`bin/fm-worker-command-policy.mjs`, [`architecture.md`](architecture.md#project-modes-are-explicit)) reuse instead of duplicating shell lexing.
