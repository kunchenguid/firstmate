...
A cadence path names one of the generated watcher cadence files, `config/x-mode.env` for Relay or `config/wa-mode.env` for the inbound WhatsApp channel.
The allowed forms of each are the relative `config/<name>.env`, the relative `./config/<name>.env`, and an absolute path that normalizes to `<active-firstmate-home>/config/<name>.env`.
