...
| `relaunch` | Replace the running agent with a new one in the same worktree - and the same endpoint whenever that endpoint still exists - on the exact recorded adapter or an explicitly chosen harness, model, effort, and codex provider profile. | The new agent is alive on the endpoint the task's record now names, and that record names the harness that is actually running. |
...
1. **Resolve the profile.**
   An explicit `--harness`, `--model`, `--effort`, or `--codex-profile` wins.
   Otherwise a `kind=secondmate` task re-resolves its durable `config/secondmate-harness` pin for its own id, including that file's optional model and effort tokens, exactly as every other respawn does - so setting the pin and relaunching is the ordinary way to move a secondmate's runtime.
   ...
   The codex provider profile is a codex-only axis: an explicit `--codex-profile` for a non-codex target is refused before the agent is stopped, and with no explicit value a codex task that stays on codex carries its recorded `codex_profile=` forward while any harness change clears it, so a relaunch never silently drops the provider.
2. **Safe checkpoint.**
