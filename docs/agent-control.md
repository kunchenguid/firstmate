...
1. **Resolve the profile.**
   An explicit `--harness` chooses the replacement runtime without consulting a `kind=secondmate` task's stored launch pin, and explicit `--model` or `--effort` values win for their matching axes.
   Without an explicit harness, a secondmate re-resolves its durable launch pin - its own `config/secondmate-harness.d/<id>` when present, else `config/secondmate-harness` - including that file's optional model and effort tokens, exactly as every other configured-profile respawn does - so setting the pin and relaunching is the ordinary way to move a secondmate's runtime.
   In that configured-profile path, an unusable per-secondmate pin refuses here, before anything is stopped, rather than falling through to the global file or the recorded harness.
