...
It is not deterministic across the verified adapters: codex prints a thread id that is not a verified resume handle, grok requires a session id printed at exit, opencode continues the most recent session for the cwd, and claude, pi, pi-signed, and kimi have no verified pane-resume contract.
