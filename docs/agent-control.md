...
- A backend that cannot deliver a required semantic key is never sent a different key.
  Lifecycle control refuses unsupported interrupt and composer-clear keys, while the launch path owns fail-closed handling for its own keys.
