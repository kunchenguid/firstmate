...
It is not deterministic across the verified adapters: codex and grok resume only from a session id printed at exit, agy exposes conversation-id and latest-conversation resume forms, opencode continues the most recent session for the cwd, and claude, pi, pi-signed, omp, and kimi have no current verified pane-resume contract.
