...
- An adapter that is not verified for this task's kind is refused **before** the running agent is stopped, not after.
  ...
  The home's `config/unverified-secondmate-harness` acceptance widens this predicate exactly as it widens the launch owner, so the two never disagree about what may start ([`docs/configuration.md`](configuration.md)).
