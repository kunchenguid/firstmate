...
| `relaunch` | Replace the running agent in the same worktree, normally at the recorded endpoint, on the exact recorded adapter or an explicitly chosen harness, model, and effort. A structurally missing Herdr pane is the narrow exception: it receives one verified replacement endpoint. | The new agent is alive at the recorded or verified replacement endpoint, and the durable record names the harness that is actually running. |
