...
5. **Launch the replacement** through its single owner, `bin/fm-spawn.sh --relaunch`, which adopts the recorded endpoint and worktree instead of creating either, clears the previous harness's per-task wiring, and arms a fresh busy generation.
   A merge poll armed before the relaunch still validates afterwards, because the republished record keeps the `pr=` identity contract owned by `fm_pr_metadata_identity_parse` in [`bin/fm-pr-lib.sh`](../bin/fm-pr-lib.sh).
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, an armed merge poll still validating after the replacement record is published, checkpoint refusals, and rollback after a failed launch.
