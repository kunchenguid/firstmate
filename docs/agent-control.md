...
- An adapter that is not verified for this task's kind is refused **before** the running agent is stopped, not after.
  Muse and prime-agent are crewmate and scout adapters only, so relaunching a secondmate onto either refuses while its agent is still up rather than leaving that secondmate with no agent when the launch owner refuses.
- A verified adapter registered in these tables is still bounded by whether the recorded backend can attribute its pane.
  prime-agent is not yet in tmux's process-name classifier, so a live prime-agent pane reads `ambiguous` there: `exit` and `relaunch` refuse before acting, and `interrupt` delivers its key and then fails its own still-running postcondition.
  Herdr classifies from its own agent registry rather than process names, so it is unaffected.
