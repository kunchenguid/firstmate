...
- `exit` and `relaunch` require a backend with a recovery-grade agent-state classifier - tmux, herdr, and thurbox - because without one the "the agent stopped" postcondition cannot be proven.
  zellij, orca, and cmux are refused rather than reported as successful blind.
  thurbox is verified: its adapter classifies the endpoint from the pane's live foreground process.
