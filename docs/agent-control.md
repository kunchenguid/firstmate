...
- A backend that cannot deliver the harness's interrupt key, or the composer clear that key needs, is refused rather than sent a different key.
  Orca's and Superset's terminal APIs each expose only an interrupt and an Enter, so neither can deliver Escape or Ctrl+U.
- `exit` and `relaunch` require a backend with a recovery-grade agent-state classifier - tmux and herdr - because without one the "the agent stopped" postcondition cannot be proven.
  zellij, orca, cmux, and superset are refused rather than reported as successful blind.
