...
| `relaunch` | Replace the running agent with a new one in the same worktree - and the same endpoint whenever that endpoint still exists - on the exact recorded adapter or an explicitly chosen harness, model, effort, and Claude account. | The new agent is alive on the endpoint the task's record now names, and that record names the harness that is actually running. |
...
1. **Resolve the profile.**
   ...
   A recorded [Claude account](configuration.md#claude-accounts-configclaude-accounts) is kept on a Claude relaunch and dropped on a switch off Claude unless `--account <name>` moves the task onto another account or `--account default` clears it, and an account that does not resolve refuses before the checkpoint.
2. **Safe checkpoint.**
