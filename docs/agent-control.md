...
1. **Resolve the profile.**
   ...
   A ship or scout recorded with the Claude Agent Teams opt-in keeps it, and `--agent-teams --teammate-mode in-process` adds it, so a relaunch onto any other harness refuses before the old agent stops; [`bin/fm-spawn.sh`](../bin/fm-spawn.sh)'s header owns that opt-in.
2. **Safe checkpoint.**
