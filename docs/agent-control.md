...
5. **Launch the replacement** through its single owner, `bin/fm-spawn.sh --relaunch`, which adopts the recorded endpoint and worktree instead of creating either, clears the previous harness's per-task wiring, and arms a fresh busy generation.
   Its metadata writer places `control_relaunch_tx=` and, when trace context is enabled, `traceparent=` before the preserved PR registration metadata (`pr=`/`pr_head=`), so a registered PR poll remains authenticated across replacement publication.
...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, checkpoint refusals, registered PR-poll authentication across replacement publication, and rollback after a failed launch.
