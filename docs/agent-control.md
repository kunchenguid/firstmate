...
1. **Resolve the profile.**
   ...
   A Claude or Pi replacement must also pass the home's [worker account pin](configuration.md#worker-account-pin-configclaude-account-configpi-account), so a missing file, a pin that no longer resolves, or a signed-out account refuses before the old agent stops.
2. **Safe checkpoint.**
