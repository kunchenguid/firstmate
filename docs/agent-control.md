...
1. **Resolve the profile.**
   ...
   A Claude or Pi profile must also pass the home's [worker account](configuration.md#worker-accounts-configclaude-account-configpi-account) selection and check here, so an undeclared or unusable account refuses before the old agent stops.
2. **Safe checkpoint.**
