...
1. **Resolve the profile.**
   ...
   Before the safe checkpoint or stopping the existing agent, the resolved Codex model and `max` effort capability must pass the launch preflight; an unsupported or unknown Codex max profile refuses while the existing agent remains running.
2. **Safe checkpoint.**
