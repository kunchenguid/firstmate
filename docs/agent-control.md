...
- `tests/fm-control-opencode-herdr-live-e2e.test.sh` - the live guard for the composer-empty precondition above, default-on wherever herdr, jq, and opencode are installed: a real idle OpenCode in an isolated Herdr lab must classify `empty`, and `exit` must then stop it while leaving the pane and the local copy in place. [`verification/runtime-backends.md`](verification/runtime-backends.md) records the version-scoped evidence.
