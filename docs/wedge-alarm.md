...
When injection cannot confirm a submit past `FM_MAX_DEFER_SECS`, `inject_wedge_alarm` raises a loud, rate-limited alarm: macOS Notification Center when available, the native wedge watch or a configured Herdr channel on Linux, otherwise only the buffered escalation itself.
