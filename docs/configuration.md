...
Primary-session turn-end guard integrations for harnesses that expose a verified turn-end mechanism are tracked as repo-level hook files and documented in [`docs/turnend-guard.md`](turnend-guard.md).
...
Cursor and hermes are hookless crewmate adapters today; supervision relies on their verified busy footers plus stale-pane detection.
Hermes also differs from the prompt-taking adapters because `hermes chat` accepts no positional prompt, so `fm-spawn.sh` launches the TUI and sends a single-line pointer to the on-disk brief as the first interactive message.
