...
Because a damaged path makes every command refuse and `bin/fm-watch.sh` swallows that refusal, `bin/fm-guard.sh` and `bin/fm-turnend-guard.sh` name the offending path in a separate diagnostic rather than letting the home fall silent; repair it by replacing that path with a private directory owned by the home.
