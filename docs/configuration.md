...
FM_OVERLOAD_REGEX='API Error: 529'   # overload footer signature; a match is parked like a quota wait with an FM_RATELIMIT_FALLBACK reset
FM_RATELIMIT_FALLBACK=3600   # seconds added to now for a parked pane's reset when no reset time can be parsed from the footer
