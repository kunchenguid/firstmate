...
`config/` holds local gitignored operating choices, and `projects/` is the default clone root for legacy registry entries and explicit `projects/...` paths; registered clones elsewhere remain subject to the same narrow guarded and concrete captain-approved exceptions in `AGENTS.md`.
