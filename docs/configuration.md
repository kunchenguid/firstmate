...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb, and a crew parked on a still-open keyed captain decision stays on the pause cadence even while a run step reports it working
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared external wait re-surfaces for a recheck in the watcher or away-mode daemon; the watcher applies the same cadence to a verified captain-held transfer and to a still-open keyed captain decision
