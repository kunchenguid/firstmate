...
The runner holds the per-source boundary from the durable capture itself through the seam, so a concurrent reconcile can never publish a capture before the adapter has acknowledged what it will acknowledge.
After a result is durably captured and any keyed-answer feed has returned - still under that boundary - the runner calls `bin/fm-procevent-<adapter>.sh receipt <source-id> <sequence> <result-file> <outcome-file>`, where the outcome file states exactly what the intake returned - `not-fed`, or `fed <exit>` with its bounded output - so an adapter that acknowledges a capture toward its own audience can journal what was received and what was saved, never more than that fact.
