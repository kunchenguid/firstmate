...
FM_VERIFY_DONE_GH_TIMEOUT=30  # seconds allowed for the one forge read that establishes a terminal claim in bin/fm-verify-done.sh
FM_VERIFY_DONE_NM_TIMEOUT=20  # seconds allowed for the one no-mistakes query that reads the validated commit in bin/fm-verify-done.sh
FM_PR_CHECK_VERIFY_TIMEOUT=20  # seconds bin/fm-pr-check.sh allows its advisory claim verification; exceeding it prints nothing and changes nothing it registered or armed
FM_DRAIN_UNRECOGNISED_MAX=10  # unrecognised status lines printed per TASK in UNREAD STATUS, so one drain may print this many for each task with unread prose; the rest are counted and held back to the next drain rather than dropped; clamped to a minimum of 1, because a cap of 0 would hold every span's first line and stall that task's cursor
...
FM_CAPTAIN_RE='^[[:space:]]*done:|^[[:space:]]*ready:|^[[:space:]]*needs-decision:|^[[:space:]]*blocked:|^[[:space:]]*failed:|PR ready|checks green|ready in branch|merged'   # captain-relevant status regex; verb tokens are anchored to the leading position they are documented to occupy so worker prose containing one is not absorbed as a state, while the legacy free-text tokens stay unanchored; nonterminal progress verbs remain excluded even when their prose matches
FM_CLASSIFY_PAUSED_VERB=paused     # leading status verb for a declared external wait; excluded from FM_CAPTAIN_RE and distinct from blocked
FM_CLASSIFY_READY_VERB=ready       # leading status verb for a no-mistakes worker's pre-validation handoff; captain-relevant so firstmate sees it, but never terminal and never a claim
