...
FM_BUSY_DIALOG_INSPECT_SECS=300    # minimum age without meaningful progress before the watcher inspects the already-captured pane once for an explicit approval/question dialog; ordinary busy output is remembered as inspected until the progress marker changes
