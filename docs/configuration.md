...
When that work lives in a REMOTE secondmate home, delivering or retiring the loop first clears its bound legacy link over that route's SSH transport only when the remote link's Relay request identity matches the registration, so closing such a loop needs a reachable host with a Firstmate copy that supports the guarded clear; an identity mismatch, an unverifiable identity, an older remote copy, or a host that never confirms the clear leaves the loop retained for reconciliation rather than closed here.
