...
FM_IDLE_EXIT_POLLS=40   # consecutive idle polls (~10 min at the default FM_POLL) before the detached watcher self-exits; idle means no task in flight, no armed check shim, and away mode off (bin/fm-watch.sh). `off` disables the resting condition for a caller that already bounds the watcher, as bin/fm-watch-checkpoint.sh does with its timeout
...
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm follows a detached watcher cycle: one it started, or an existing healthy one it attached to
FM_AFK_DETACH_CONFIRM_TIMEOUT=10   # seconds fm-afk-start.sh --detach waits for the detached daemon to take its lock before failing and killing it
