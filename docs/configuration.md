...
FM_IDLE_EXIT_POLLS=40   # consecutive idle polls (~10 min at the default FM_POLL) before the detached watcher self-exits; idle means no task in flight, no armed check shim, and away mode off (bin/fm-watch.sh)
