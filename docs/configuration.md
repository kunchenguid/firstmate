...
`lsof` is required for the full teardown leaked-process reaper: teardown scans every process whose current working directory is the task worktree or tasktmp via `lsof -a -d cwd` and TERM/KILLs survivors whose process identity still matches (Fix 2 in `bin/fm-teardown.sh`).
Without `lsof` teardown falls back to the tmux pane process-group reaper, which cannot see a detached process.
The cursor worker-server reaper (Fix 4) is pid-based and lsof-independent, but a cursor primary/crew host should still install `lsof` so the generic cwd scan also covers other leaked descendants.
