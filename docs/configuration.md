...
Posting that reply is best effort by design: the listener consumes the staged file before it polls, so a rare crash in that window drops that one round's reply rather than posting it twice, and nothing here keeps a receipt, retry, or idempotency record - robust reply delivery waits on lavish-axi's exclusive listener.
