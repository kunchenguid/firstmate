...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per bounded external query inside fm-crew-state.sh: each no-mistakes query and the read-only GitHub PR check lookup that verifies a checks-green claim
