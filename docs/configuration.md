...
It refuses a missing required deliverable the same way: the registration records the obligation's required keys for a direct emit, and `brief` repeats them as `--require-deliverable` flags for a staged one, so an incomplete result never travels either. A `failed` outcome is exempt only from a key it could not carry anyway, so a promise whose expected final IS the failure still needs its `error_code`; no other outcome type excuses a missing required value.
The registration also records what the promise expects, so a direct emit refuses an outcome that cannot satisfy it (only the expected outcome, or `failed`, may answer it) and refuses `superseded`, which `tasks-axi` takes only with a successor a typed terminal result does not carry.
