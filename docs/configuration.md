...
FM_BUSY_TURN_MAX_SECS=3600         # seconds a busy pane may run with no completed turn (state/<id>.turn-ended, or its spawn record before any turn completes) before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart
