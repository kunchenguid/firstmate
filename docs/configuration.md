...
`bin/fm-procevent-lavish.sh` is the first built-in adapter; its listener wraps the currently published `lavish-axi poll` interface, while its reviewer-link command reads the published session listing and uses `curl` to probe the health endpoint without opening or polling a session.
