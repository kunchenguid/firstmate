...
FM_COMPOSER_CAPTURE_LINES=20   # fleet-wide bound for tail-capture composer reads; tmux instead supplies its bounded visible pane, while the other adapters use this small window so stale scrollback banners stay out of the candidate set. The same bound sizes the two endpoint-shell marker reads that share this window: the dead-shell arm in bin/fm-busy-lib.sh when its caller passed no pre-captured tail, and bin/fm-spawn.sh's re-read of a marker send left stranded on a zellij or cmux input line
