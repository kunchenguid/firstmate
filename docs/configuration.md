...
FM_RATELIMIT_REGEX='Claude[[:space:]]+usage[[:space:]]+limit[[:space:]]+reached\.[[:space:]]+Your[[:space:]]+limit[[:space:]]+will[[:space:]]+reset[[:space:]]+at[[:space:]]+[^[:cntrl:]]+'   # Claude quota footer line signature; matched as a full rendered footer line, never a transcript substring
FM_OVERLOAD_REGEX='[^[:alnum:][:space:]][^[:alnum:]]*API Error: 529[[:space:]]+\{[^[:cntrl:]]*"type"[[:space:]]*:[[:space:]]*"overloaded_error"[^[:cntrl:]]*\}'   # overload footer line signature (best-effort default), also full-line matched; requires a glyph/punctuation prefix before "API Error: 529" and a trailing overloaded_error JSON payload so a bare "API Error: 529" transcript line does not park; a match is parked with the shorter FM_OVERLOAD_FALLBACK reset, since overloads are transient. Verify/tune against a real idle-529 render and override if the actual footer differs
...
FM_OVERLOAD_FALLBACK=120     # seconds added to now for an API Error: 529 overload-parked pane's reset; shorter than the hourly quota fallback
