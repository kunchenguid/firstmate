...
Both checks are per harness, not per model: an effort the harness accepts can still be unsupported by the specific model in the profile, in which case the harness itself refuses the launch loudly instead of dropping the value, and `harness-adapters` owns the verified per-harness effort support and its current per-model notes.
