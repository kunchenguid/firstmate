...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^(❯|›)'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's › (alternation, not a [❯›] class, because bash 3.2/grep mishandle multi-byte UTF-8 in character classes; docs/herdr-backend.md "Incident (2026-07-07)")
