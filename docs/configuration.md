...
FM_TASK_INBOX_BLOCKED_MAX=1   # composer-protected doorbell skips before the task surfaces as a stale wake, since a held composer will not clear itself
FM_TASK_INBOX_UNHANDLED_MAX_SECS=900   # absolute unhandled bound that escalates a steering-inbox record whatever the pane is doing, so a busy pane can never mean unread forever
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status answers for another crew's branch; the scan stops at this branch's newest row
