...
On the primary Studio Firstmate home, every watcher cycle discovers active Lavish sessions whose canonical artifact is inside that home and registers them through this same process-event owner before reconciliation, so another home cannot claim its dock replies.
If discovery fails, the watcher queues one bounded check wake for that failure episode and retries on later cycles.
Secondmate homes do not perform that discovery.
...
The runner passes the normalized bound to the invoked adapter so an adapter that supports bounded pieces can size each result to the same capture contract.
An adapter capture callback runs only for a complete result, so Lavish recovery emits an oversized source snapshot as ordered bounded chunks and advances its private cursor only after each chunk is durably captured instead of repeating or acknowledging truncated feedback.
