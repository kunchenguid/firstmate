...
DeepSeek Harness (`dsh`) is a verified PRIMARY adapter only, and is refused for crewmate, scout and secondmate launches: it exposes no endpoint, no interrupt key, no exit command and no per-task busy-state source, so a dispatched worker could not be steered, inspected or stopped. [`docs/supervision-protocols/dsh.md`](supervision-protocols/dsh.md) owns its supervision protocol and the death window it cannot close from inside, and [its adapter reference](../.agents/skills/harness-adapters/references/harness/dsh.md) owns the launch boundary, the sandbox and instruction-budget requirements, and the hook shape changes (UserPromptSubmit rather than SessionStart, lowercase `bash` matchers, no `asyncRewake`).
