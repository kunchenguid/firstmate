...
An absent, unreadable, or unparseable file passes no model override, preserving Pi's existing behavior: a reopened branch restores its recorded model, while a new branch takes the same default model main takes.
...
The branch resolves the pin on every branch build - the first wake of a cold start and the reopen after `/new`, `/resume`, `/fork`, or reload - and the explicit pin takes precedence over a reopened session's recorded model, so the choice survives all of them.
