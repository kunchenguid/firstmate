...
`devenv` is an experimental remote control backend that is known but deliberately non-spawn until resident execution is implemented; see [`docs/devenv-backend.md`](devenv-backend.md).
...
Any value other than `tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `devenv` is rejected until another adapter is implemented and verified.
...
`fm-spawn.sh` refuses `devenv` with its missing resident-execution capability instead of falling through to another backend.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`) or remote control transport (`ssh` for `devenv`), `jq` for the JSON-emitting experimental adapters and remote control backend, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
...
FM_BACKEND=             # optional runtime backend override; tmux/herdr/zellij/orca/cmux support ship/scout spawns, devenv is control-plane-only, codex-app is not accepted
