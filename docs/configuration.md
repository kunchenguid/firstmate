...
`herdr` is the verified reference backend and fully supported baseline for secondmate homes (see [`docs/herdr-backend.md`](herdr-backend.md)); `tmux` is the fully supported legacy alternative, while `zellij`, `orca`, and `cmux` remain experimental spawn backends (see [`docs/tmux-backend.md`](tmux-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), and [`docs/cmux-backend.md`](cmux-backend.md)).
...
Auto-detected tmux prints a stderr notice naming `config/backend` and `--backend herdr` as opt-outs; auto-detected herdr stays silent because it is the default, and auto-detected cmux retains its notice naming the explicit tmux opt-out.
...
New default-path tasks explicitly record `backend=herdr`, while an absent `backend=` in any recorded endpoint continues to mean `tmux` for legacy compatibility.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`), `jq` for Herdr and the JSON-emitting experimental Zellij and cmux adapters whose spawn and liveness paths parse JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
Backend tool availability uses the adapter's own executable resolver, so bootstrap and spawn agree on supported non-`PATH` locations such as cmux's bundled CLI.
An unknown resolved backend emits `BACKEND_INVALID` and blocks dispatch instead of silently dropping its dependency delta or falling through to the Herdr default.
