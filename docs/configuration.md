...
If terminal retirement and an identical re-arm replace the registration first, neither the restart nor the replacement poll may post the stale acknowledgement; the adapter removes that generation's reply record and appends the acknowledgement plus the generation-change reason to its recorded host status log.
