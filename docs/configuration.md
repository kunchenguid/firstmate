...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared external wait re-surfaces for a recheck in the watcher or away-mode daemon; also bounds the watcher's one live-agent surface per declared pause, so a repainting idle pane cannot mint a fresh surface per pane hash
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added
FM_WEDGE_ACTIVITY_DEFER_MAX=3      # consecutive wedge escalations a working status row alone may defer once the wedge timer expires; an active pipeline run defers without this bound, and 0 disables status-row-only deferral
FM_CLASSIFY_WORK_ROW_RE=           # replaces the status-row signatures (running background shells, an in-progress elapsed-work row) that count as working evidence at wedge-escalation time
FM_CLASSIFY_WORK_ROW_LINES=6       # trailing non-blank pane lines treated as the status row when reading that evidence
