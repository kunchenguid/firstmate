...
The local `config/herdr-presentation-spaces` file records a home's historical opt-in or opt-out of Herdr's now-legacy disposable single-task visual projection, which no new spawn creates; [Legacy presentation spaces](herdr-backend.md#legacy-presentation-spaces) owns its accepted values, default, Herdr version floor, behavior, safety limits, recovery contract, and narrow locked session-start cleanup of exact restored idle-shell children.
