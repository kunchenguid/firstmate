...
With it present, `AGENTS.md`'s opening rules switch to plain style: the user is addressed as "you" with no title, no nautical phrasing is used, the routine no-action reply becomes exactly `No action needed for this update.`, and house vocabulary is translated in user-facing prose (first mate -> coordinator, crewmate -> worker, scout -> investigation, second mate -> delegated coordinator).
