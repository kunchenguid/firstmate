...
When no setting or primary-runtime marker matches, `fm-spawn.sh` refuses before endpoint creation and names `config/backend` plus the authorized per-task `--backend` remedy.
