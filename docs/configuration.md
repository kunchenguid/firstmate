...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, explicitly installed content-addressed extension packages under `data/extensions/packages/`, and the per-project material that must reach a worker without ever entering the project's history under `data/project-local/`.
