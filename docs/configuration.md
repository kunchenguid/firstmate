...
An agy launch that names no model resolves to the captain's standing pin `--model gemini-3.1-pro-high`, never carries an `--effort` flag, and is refused outright when the requested model is `claude-*`.
...
FM_COMPOSER_PI_MAX_LINES=8     # fleet-wide: maximum rows admitted between an identity-corroborated separator pair (Pi's, and agy's verified `>` shape); taller or ambiguous candidates stay unknown
