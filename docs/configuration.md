...
FM_TASK_INBOX_RING_MAX=3      # watcher delivery attempts on an idle pane without an acknowledgement before the task surfaces as a stale wake for recovery; an attempt a mid-turn Kimi queued and steered spends none (bin/fm-task-inbox-lib.sh owns the ladder)
