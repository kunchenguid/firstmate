...
FM_TERMINAL_PAUSE_SCAN_MAX=20      # status lines of backward lookback, counting the last line itself, when deciding whether a log ending in `paused:` is a worker that finished into an external wait; the scan walks back across consecutive trailing declared-pause lines looking for the `done:`/`failed:` line they follow, and exhausting the budget reads as no terminal report rather than a guess; a non-numeric or non-positive value falls back to the default
