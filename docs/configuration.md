...
Every newly spawned ship worker or scout receives a deterministic task/home-specific `CHROME_DEVTOOLS_AXI_SESSION` before its agent starts, while fixed ports, external browser attachment, and shared profile selection are cleared from that worker environment.
Firstmate preserves the order of every operator-supplied whitespace-separated `CHROME_DEVTOOLS_AXI_CHROME_ARGS` token except exact `--no-sandbox` occurrences, removes all such occurrences, then appends exactly one for a root worker and none for a non-root worker.
Firstmate does not add `--disable-dev-shm-usage`, alter the captain's interactive Chrome configuration, or claim a generic Chrome DevTools AXI-to-Playwright translation.
The generated ship/scout instructions own the one-restart `Target closed` recovery and explicit task-specific Playwright handoff, while [`bin/fm-chrome-axi-lib.sh`](../bin/fm-chrome-axi-lib.sh) owns exact naming, metadata validation, and cleanup mechanics.
Successful cleanup stops only the recorded named session after the existing destructive-cleanup authorization checks pass; legacy records never fall back to the default session.
See the [maintainer verification record](verification/chrome-devtools-axi.md) for current source, regression, and concurrent-session evidence.
