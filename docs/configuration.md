...
Pi's bash tool applies a short default timeout when a call carries none, which cuts long legitimate firstmate commands short.
Firstmate's Pi extensions therefore fill a default timeout into bash tool calls that arrive with no valid explicit timeout, and leave valid explicit timeouts untouched; they never replace the bash tool itself.
...
- `0` (in any leading-zero spelling), `off`, or `none`: injection is disabled, so Pi's own default applies.
