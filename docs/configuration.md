...
FM_MERGE_GH_TIMEOUT=20  # seconds per bounded gh read in the shared merge-attribution library (per-task check.sh and heartbeat backstop)
...
FM_CAPTAIN_RE='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged|unattributed-merge'   # status regex that makes watcher and daemon signal/stale/scan output captain-relevant
