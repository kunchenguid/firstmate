...
Treehouse remains the worktree provider for ship and writer scout tasks on tmux, herdr, zellij, and cmux, since herdr, zellij, and cmux are session providers only; reader scouts use checkout-free scratch directories, while Orca provides both the task worktree and terminal endpoint.
...
`backend=orca` also refuses reader scout spawns (`--access reader`), which avoid the managed worktree Orca allocates per task.
...
When `config/crew-dispatch.json` exists, crewmate and scout spawns require an explicit resolved harness plus a dispatch attestation (`--dispatch-resolved` or `--dispatch-override-reason`) instead of automatically falling back to `config/crew-harness`.
...
For grok, `fm-spawn.sh` installs one firstmate-owned global turn-end hook under `$GROK_HOME/hooks/`, or `~/.grok/hooks/` when `GROK_HOME` is unset, and drops a per-task `.fm-grok-turnend` pointer in the task root, with teardown removing the task token and pointer.
For Kimi crews, `fm-spawn.sh` runs `fm-kimi-turnend-hook.sh install`, drops a per-task `.fm-kimi-turnend` pointer in the task root, and records the matching private registry token for teardown.
...
Because Kimi asks for workspace trust per task root, `fm-spawn.sh` pre-trusts the validated worktree or reader scratch before launch by writing Kimi's own record under `~/.kimi-code/workspace-trust/`, so a spawned crewmate never dies on that dialog; [`harness-adapters`](../.agents/skills/harness-adapters/references/harness/kimi.md#operating-facts) owns the record format and workspace-id derivation.
That record is validated by `bin/fm-kimi-trust-check.sh`, which requires `jq`, and a Kimi spawn refuses loudly rather than launching when trust cannot be established or cannot be evaluated at all.
...
Dispatch profiles never carry a task class, an exploration decision, or routing provenance: `--task-class`, `--exploration`, and `--routing-source` are firstmate's per-spawn flags, owned by `harness-adapters`, and they never influence which profile matched.
When the file exists, `fm-spawn.sh` enforces that contract by refusing crewmate and scout spawns that lack an explicit harness (`--harness`, a positional adapter, or a raw launch command).
That explicit harness must also carry a dispatch attestation naming how it was chosen: `--dispatch-resolved` when the profiles were consulted, or `--dispatch-override-reason "<why>"` for routing that did not come from consulting them - a deliberate departure from the matched profile, or an escalation-ladder recovery relaunch (`stuck-crewmate-recovery`).
A bare explicit harness, and passing both attestations at once, are refused; the override reason must be a single line, because it is recorded as one `dispatch_override_reason=` line in `state/<id>.meta` alongside `dispatch=override` (a resolved spawn records `dispatch=resolved`).
`fm-spawn.sh` never re-reads `config/crew-dispatch.json` to check the harness against the rules; the attestation proves resolution happened, and routing precedence stays firstmate's.
Batch spawns satisfy both requirements with a shared `--harness` and a shared attestation, forwarded unchanged to every pair.
...
[`bin/fm-spawn.sh`](../bin/fm-spawn.sh)'s launch-axis preflight owns refusal of unsupported or unrepresentable explicit settings before attempt intake; metadata records requested axes, not observed runtime support.
...
Reader scouts have one access-specific dependency outside that home-wide delta: `fm-spawn.sh` requires `sandbox-exec` on macOS or `bwrap` on Linux and refuses reader launch on any other platform or when the platform sandbox cannot pass its confinement preflight.
Bootstrap does not report this dependency because writer tasks on the same backend do not need it; `fm-spawn.sh` checks it before creating the reader endpoint.
...
`board` maintains one permanent live status message and edits it in place via `chat.update`, recording its identity under `state/slack-board/`.
A legacy home whose board identity still lives under `state/slack-board.meta/` is migrated to `state/slack-board/` by `bin/fm-slack-board-migrate.sh`, a lazy, idempotent, no-network owner that runs one phase-aware coherent-layout validator before any rename or network call. `bin/fm-slack-lib.sh` is the single authoritative owner of the Slack board store contract: the entry sequence `fms_board_store_assert` (probe legacy/canonical/snapshot presence, refuse ambiguous and orphan-store states, enforce directory rules, run the complete directory inventory, validate snapshot once-file bytes, assert the phase layout, then the single atomic rename), the phase model `fms_board_layout_assert`, and every shared predicate — identity (`fms_board_identity_valid`), daily state (`fms_board_state_valid`), journal (`fms_board_journal_valid`), the canonical once-file reader (`fms_board_once_valid`), the calendar oracle (`fms_date_calendar_valid`), and the directory inventory (`fms_board_dir_inventory_assert`). The contract, its invariants, and its remediation text live in the `fm-slack-lib.sh` header and are specified there in full; `bin/fm-slack-board-migrate.sh` is reduced to argument parsing, lock acquisition, calling `fms_board_store_assert`, and diagnostics. The directory is renamed atomically under `.slack-board.lock`; bootstrap invokes the helper before its first `state/*.meta` enumeration so a complete legacy board is moved out of the task-record namespace before any consumer can scan it as a phantom task, and the `board` command invokes the same helper under the board lock before any Slack API call. Partial, duplicate, malformed, wrong-mode, non-regular, or both-paths layouts, an orphan snapshot store with no board directory, any impossible or conflicting journal/identity/state combination, and any move failure, refuse with nonzero status, zero network calls, and every byte and path unchanged; only the absence of both directories is a fresh-create state. Scanners stay strict rather than learning to ignore this one directory, because startup migration removes the collision once for every consumer and preserves fail-closed detection of malformed task records.
The `bin/fm-slack-post.sh` header is the authoritative owner of the `board` rollover contract, including snapshot timing, deduplication, and no-synthetic-snapshot behavior.
...
`bin/fm-procevent.sh` owns the generic contract; built-in adapters retain their tracked `bin/fm-procevent-<adapter>.sh` commands, including `bin/fm-procevent-lavish.sh` for the published `lavish-axi poll` interface and `bin/fm-procevent-slack-socket.sh` for the long-lived Slack worker, while an explicitly bound external adapter routes through the trusted host contract above.
...
The remote-secondmate reply adapter does not declare `self-announcing`: a fully quarantined delta may add no parent status bytes, so its already-published `check` wake remains the durable announcement and a later adapter replay is idempotent; it implements automatic handling so every remote home, including a dedicated reviewer home, reaches its ordinary parent status channel through the same reply ingest and correlation owner without depending on a later manual dispatch.
...
FM_SESSION_START_STATUS_TAIL=0   # state/*.status tail lines projected per task in the session-start digest; 0 (default) prints one on-demand pointer line per task instead, and a positive value restores the bounded tail
...
FM_SPEC_KIT_SCRIPTS_DIR=     # Spec Kit scripts directory owning the linked-run seal predicate fm-teardown.sh calls; default $HOME/.claude/skills/artemis-spec-kit-plugin/scripts, where an absent install skips the check, while an explicitly configured directory without list_worktrees.py refuses teardown
FM_KIT_SCRATCH_ROOT=         # root scanned for the linked Spec Kit run of a worktree being torn down; default $HOME/.claude/scratch
...
FM_LOCK_WAIT_SECS=600   # default deadline for every lock acquisition; a live holder returns a loud timeout with its pid
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm waits to confirm a fresh watcher before reporting FAILED; default 30 on macOS and Git Bash/MSYS so pre-lock migration plus the first beat can finish under ordinary scheduler delay; a watcher that never produces that first healthy beat still fails with confirmation-timeout
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm is attached to an existing healthy watcher cycle
FM_OPENCODE_ARM_READY_TIMEOUT_MS=12000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; default 35000 on macOS and Windows so the wait stays above the 30s confirm budget
FM_PI_ARM_READY_TIMEOUT_MS=12000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached; default 35000 on macOS and Windows so the wait stays above the 30s confirm budget
...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of every busy verdict except exact busy claude-hook, measured from its latest state/<id>.turn-ended marker or state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a declared external wait or verified captain-held transfer takes the FM_PAUSE_RESURFACE_SECS recheck below instead
FM_BUSY_SILENT_SECS=3600           # seconds of unchanged status record and pane capture before a busy worker is recorded as stalled and interrupted once
FM_BUSY_SILENT_RECOVERY_SECS=300    # seconds after the one interrupt before stalled-after-interrupt is recorded for MAIN
