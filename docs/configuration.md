...
Aggregate polling is bounded by the single scheduler, which durably records the served wall slot and last roster identity before each attempt. A restart cannot repeat a slot, elapsed slots never skip roster identities, and merged or closed PRs are polled every `FM_PR_FOLLOW_SETTLED_EVERY`-th roster round.
