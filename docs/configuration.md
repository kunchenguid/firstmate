...
FM_COMPOSER_AGENT_GLYPHS='❯ ›'   # fleet-wide: whitespace-separated list of LITERAL agent prompt glyphs (claude's ❯, codex's ›) that read as a genuine empty AGENT composer, bordered or bare; it is the default for every backend, including herdr's own FM_BACKEND_HERDR_BARE_PROMPT_GLYPHS. Never a regex: a bracket class over multibyte glyphs degenerates into a byte class under a C/POSIX locale, so this set is compared with a locale-invariant case. Shell prompt glyphs (> $ % #) are NOT agent glyphs and cannot be made into them: one placed in this set is REJECTED with a loud stderr warning, and the classifier evaluates the shell rule before the configurable agent match, so a bare shell prompt still reads unknown rather than empty either way - telling a dead shell apart from an empty agent composer is the injection safety rule. A set left with NO agent glyphs by that rejection falls back to the built-in default, warning again: an empty set would strip no glyph at all, leaving every idle agent composer stuck on pending and away-mode deferring every escalation (bin/fm-composer-lib.sh)
