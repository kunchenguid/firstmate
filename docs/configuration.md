...
`bin/fm-spawn.sh --help` owns the exact per-edge behavior, including which states refuse the spawn and which degrade to a warning that names the file to fix by hand.
