...
FM_WATCHER_REAP_SECS=20            # seconds the shutdown trap waits for the watcher child after SIGTERM before killing it hard; default is FM_POLL + 5 with a floor of 20 so the bound clears one watcher poll cycle; invalid or zero values use that default; an explicit value is used verbatim with no floor
