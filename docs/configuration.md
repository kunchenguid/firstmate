...
Copilot launches on Zellij, cmux, and Orca and ordinary text steers still ride the durable inbox there, but explicit typed delivery confirmation and any empty-composer-dependent injection remain unconfirmed until those backends expose a live Copilot identity probe; the shared half-box classifier now fails closed to `unknown` without that identity.
...
FM_COPILOT_TRUST_POLLS=40   # maximum capture polls fm-spawn uses to find and verify Copilot's session-only trust dialog before and after sending Enter
FM_COPILOT_TRUST_POLL_INTERVAL=0.25   # seconds between those Copilot trust polls
FM_COPILOT_WATCH_RECEIPT_MAX_AGE=300   # seconds a Copilot watcher completion receipt remains claimable for the notification follow-up path; invalid or zero values use 300
