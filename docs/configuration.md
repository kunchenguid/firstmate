...
FM_RATELIMIT_REGEX='Claude[[:space:]]+usage[[:space:]]+limit[[:space:]]+reached\.[[:space:]]+Your[[:space:]]+limit[[:space:]]+will[[:space:]]+reset[[:space:]]+at[[:space:]]+([0-9]{1,2}(:[0-9]{2})?[[:space:]]*([AP]\.?M\.?)|[0-9]{1,2}:[0-9]{2})[[:space:]]+(\((UTC|GMT|Z|[A-Z]{2,5}|[+-][0-9]{2}:?[0-9]{2}|(UTC|GMT)[+-][0-9]{1,2}(:?[0-9]{2})?|[A-Za-z]+/[A-Za-z0-9_+\-]+(/[A-Za-z0-9_+\-]+)?)\)|(UTC|GMT|Z|[A-Z]{2,5}|[+-][0-9]{2}:?[0-9]{2}|(UTC|GMT)[+-][0-9]{1,2}(:?[0-9]{2})?|[A-Za-z]+/[A-Za-z0-9_+\-]+(/[A-Za-z0-9_+\-]+)?))\.?'   # Claude quota footer line signature; matched as a full rendered footer line and bounded to reset time plus timezone so quoted prose after the footer phrase does not park a pane
FM_OVERLOAD_REGEX=           # optional overload footer line signature; empty by default so transient API Error: 529 auto-resume is off unless explicitly calibrated against a real idle-529 render and enabled
FM_RATELIMIT_FALLBACK=3600   # seconds added to now for a quota-parked pane's reset when no reset time can be parsed from the footer
FM_OVERLOAD_FALLBACK=120     # seconds added to now for an opted-in API Error: 529 overload-parked pane's reset; shorter than the hourly quota fallback
FM_RATELIMIT_MARGIN=60       # grace seconds after a parked pane's reset epoch before an idle limit/empty pane is auto-resumed with "continue"
FM_RATELIMIT_MAX_RESUMES=3   # auto-resume submit attempts before firstmate writes state/<id>.ratelimit.failed and escalates
