...
`data/` holds durable private fleet records such as the project and secondmate registries, each registered lane's recorded command state, captain preferences, optional shared captain preferences, learnings, backlog, briefs, and scout reports.
`state/` holds volatile runtime records such as task metadata, append-only status events, endpoint signals, watcher and wake-queue coordination, away-mode state, generated X-mode artifacts, private secondmate config-reread generations with their retry and quarantine state, parent-owned secondmate pending-reply records under `state/pending-replies/` (`bin/fm-pending-reply-lib.sh`), and the open command-handback request recorded for a lane the captain holds.
...
`bin/fm-brief.sh` owns the per-task brief and the scout report under `data/<id>/`, while `bin/fm-secondmate-command.sh` owns a lane's command-position records under the same directory, its handback request record under `state/`, and every rewrite of the derived command copy that `bin/fm-home-seed.sh` first writes into the lane's own home; `bin/fm-spawn.sh` and `bin/fm-teardown.sh` own a task's turn-end marker, harness hook-registry tokens, and scratch; `bin/fm-check-register.sh` and `bin/fm-pr-check.sh` own registered custom checks and the PR merge poll with its sidecars; `bin/fm-pr-check-migrate.sh` owns the legacy-check quarantine and the migration markers; `bin/fm-wake-lib.sh` and `bin/fm-wake-drain.sh` own the wake queue, its record layout, and its serialization lock; `bin/fm-watch.sh` and `bin/fm-watch-arm.sh` own the watcher lock, liveness beacon, absorbed-wake log, and suppression markers; `bin/fm-claude-stop-autoarm.sh` owns the Claude auto-arm records and `bin/fm-turnend-guard.sh` owns the Claude guard-budget record; `bin/fm-afk-launch.sh` and `bin/fm-afk-start.sh` own the away-mode flag while `bin/fm-supervise-daemon.sh` owns the sub-supervisor coordination markers; and `bin/fm-x-lib.sh` owns the generated X-mode artifacts.
Wake, watcher, away-mode, and X-specific state mechanics remain with those named scripts and reference sections rather than being duplicated into one exhaustive state tree here, which is why `AGENTS.md` keeps only the four top-level directories, their read/write posture, and the standing prohibition on hand-editing or removing those coordination records.
...
That sharing also covers secondmate homes: `config/backend` is inherited, so a secondmate home lands on the primary's zellij selection and spawns its own ship and scout tabs into the same session.
...
An optional trailing `command:` field records whether that lane is under firstmate command (the default, and what an absent field means) or captain command, and it is the single owner of that answer; `bin/fm-secondmate-command.sh` is the only writer, and the [`secondmate-command-transfer` skill](../.agents/skills/secondmate-command-transfer/SKILL.md) owns the two-way transfer procedure and everything that changes while the captain holds a lane.
The seeded home's `data/command.md` is a derived, main-authoritative copy of that answer, read-only in the secondmate home so the lane knows which state it is in; a disagreement between it and the registry is reported rather than resolved, because the registry is authoritative.
Re-seeding an existing home is provisioning, never a command decision: it carries a recorded `command:` field forward the same way it preserves `label:`, leaves a captain-commanded lane's own copy untouched, and refuses outright on an unrecognized value.
...
A standalone-clone home cannot receive a primary-local commit through that no-fetch sync, so it receives the rule through `/updatefirstmate`'s fork refresh instead.
...
FM_SPAWN_WORKTREE_POLLS=60   # polls a ship/scout spawn waits for its window to reach the isolated worktree; whole number from 2 to 86400, the floor because acceptance needs two agreeing reads and the ceiling because the window is a real wait; it only bounds waiting, never relaxes the isolation check
FM_SPAWN_WORKTREE_POLL_INTERVAL=1   # seconds between those polls; number from 0.001 to 60, written as whole seconds, a decimal (0.5), or the leading-dot form (.5); pairs with FM_SPAWN_WORKTREE_POLLS as the interval half of that count/interval poll pair, and the two are also refused when their PRODUCT exceeds the 86400-second settle-window ceiling
...
FM_COMPOSER_IDLE_RE=    # optional empty-composer regex, applied after ghost, border, and blank stripping
