...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a stale pane whose crew cannot be proven alive escalates as a possible wedge; stale panes whose crew is not provably working surface immediately unless they declare the pause verb; deliberately not the knob for quieting long pipeline steps, since raising it slows detection of a genuinely wedged crew - a crew whose own validation reports progress is suppressed by FM_RUN_ACTIVITY_TIMEOUT's probe below instead
...
FM_RUN_ACTIVITY_TIMEOUT=10         # wall-clock seconds the wedge detector's run-activity probe allows its one bounded `no-mistakes axi status` call, taken only at the moment a wedge escalation would otherwise fire; proven recent activity suppresses that escalation outright with no wake and restarts the idle window, while an active step that reported nothing recently, a run that is not this crew's or has concluded, and an unresponsive CLI all read as no evidence and leave the escalation schedule exactly as it was
