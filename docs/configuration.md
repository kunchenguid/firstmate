...
It widens muse, gemini, and agy alone, never rovo, in both `bin/fm-spawn.sh` and the `bin/fm-control-lib.sh` capability predicate the control plane asks before it stops anything, so the launch owner and the relaunch gate cannot disagree about what may start.
