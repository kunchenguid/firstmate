...
FM_POLL=15              # seconds between watcher poll cycles; a durable wake append also taps the sleeping watcher so the append is surfaced at once (bin/fm-wake-lib.sh fm_wake_tap_watcher), leaving this cadence as the backstop and the bound for poll-detected wakes
