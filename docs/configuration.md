...
agy (Google Antigravity CLI) is likewise verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate, because agy has no verified primary supervision protocol; [`docs/verification/antigravity.md`](verification/antigravity.md) owns that evidence.
...
For agy crews, `fm-spawn.sh` pre-trusts the exact worktree path in `~/.gemini/antigravity-cli/settings.json` through a surgical jq edit that refuses on a missing jq, a symlinked settings file, or malformed JSON, then writes one per-task `.agent/hooks.json` in the worktree (gitignored via info/exclude, removed by relaunch retirement and teardown); it refuses the spawn when the project tracks that exact path.
Pre-trust exists because agy's workspace hooks load only when the workspace was already trusted at launch, and `--dangerously-skip-permissions` does not suppress the trust dialog; [`docs/verification/antigravity.md`](verification/antigravity.md) owns the evidence.
The trust entry is retired by worktree path rather than by the task's recorded harness, so teardown removes it even after a relaunch switched the task to another harness, and a relaunch that switches away from agy retires it together with the agy hook wiring.
