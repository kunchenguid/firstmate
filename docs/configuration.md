...
That inheritance covers the two paths that restart an existing task from its own record, the `relaunch` verb and a `--secondmate` spawn, which is the shape firstmate's own liveness recovery uses.
