...
When the sections are sent, one line naming the brief's kind and delivery mode comes first, taken from the scaffold's fixed contract lines: `Brief kind: ship, mode=<mode>` from its `Delivery contract: mode=<mode>` line, or `Brief kind: scout (report only)` from its scout contract line; a brief with neither line gets no kind line.
