...
FM_WEDGE_ACKED_BACKOFF_MAX_SECS=1800   # ceiling on the quiet interval an unchanged pane must clear before repeating a wedge escalation the first mate has already drained and acknowledged; the interval doubles per acknowledged repeat from FM_STALE_ESCALATE_SECS up to this bound, while an escalation still queued unacknowledged keeps the FM_STALE_ESCALATE_SECS cadence so an unread ladder still reaches demand-deep-inspection
