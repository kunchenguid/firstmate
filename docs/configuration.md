...
omp (Oh My Pi) is a candidate crewmate/scout adapter rather than a verified one, and every runnable launch remains refused until ATX-2170 empirically verifies First Mate interrupt, exit, and relaunch control.
Firstmate remains its sole supervisor and Orca remains its execution backend.
The [`fm-spawn.sh` usage contract](../bin/fm-spawn.sh) owns selection, version, model, backend, relaunch, trace, and tool enforcement; the [harness-adapters skill](../.agents/skills/harness-adapters/SKILL.md) owns adapter capability and safety details; and [runtime backend verification](verification/runtime-backends.md#omp-candidate-tool-name-compatibility) owns current empirical evidence.
