...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, task-local GitHub review evidence (`data/<id>/pr-review-snapshot.json` and `data/<id>/pr-review.json`, owned by `bin/fm-pr-review.sh`), and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
Firstmate's script dependencies come in two parts: a universal toolchain every home needs regardless of backend, and a per-backend delta that follows the runtime backend actually resolved for this home.
The essential universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.46.0 or newer, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
[`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) owns the essential version checks plus the retained optional gh-axi and lavish-axi floors, while [`bin/fm-tasks-axi-lib.sh`](../bin/fm-tasks-axi-lib.sh) and [`bin/fm-quota-axi-lib.sh`](../bin/fm-quota-axi-lib.sh) hold their own tools' floor constants.
This section is the single owner of the universal script-toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
Workers select native tools or command-line fallbacks by capability from the common [tool-selection contract](../.agents/skills/harness-adapters/references/common/tool-selection.md) and their active worker tool inventory.
`gh-axi` and `chrome-devtools-axi` are optional operation-specific fallback clients, not startup dependencies; their absence does not block work when a native or other supported surface provides the required semantics.
...
An operation that explicitly requires gh-axi may use the silent read-only `bin/fm-bootstrap.sh gh-axi-compatible` probe and request the retained install command under the normal consent policy.
An absent or incompatible `lavish-axi` reports `PRESENTATION_UNAVAILABLE` with its required floor, install command, and explicit statement that native tools and plain-text decisions remain available; [`bootstrap-diagnostics`](../.agents/skills/bootstrap-diagnostics/SKILL.md) owns the response and compatibility check before Lavish-specific use.
