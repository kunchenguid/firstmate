...
If terminal or explicit retirement ends the source before delivery, or an identical re-arm replaces its registration first, neither the restart nor the replacement poll may post the stale acknowledgement; the adapter removes each pending or in-flight record only after appending its acknowledgement and the retirement or generation-change reason to the recorded host status log.
