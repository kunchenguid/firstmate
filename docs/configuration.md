...
The file holds one line with the prefix itself - letters, digits, and dashes only, no slash, no leading dash, since git refuses branch names that begin with a dash - written without the trailing slash, for example `ardy`.
A single trailing newline is accepted; an empty value, a leading dash, a slash anywhere, whitespace, multi-line content, CRLF endings, a NUL byte, or a symlinked, unreadable, or otherwise non-regular file is refused loudly before any consumer acts.
