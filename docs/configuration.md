...
The correct delivery path for this change is an upstream Firstmate PR.
After it lands, successful producer invocations automatically gain durable capture, duplicate coalescing, prompt supervision wakes, and restart replay through Firstmate's existing process-event machinery.
Operator setup of the authenticated immediate producer and hourly reconciliation producer remains required, and there is no delivery or overnight-discovery guarantee until those producers successfully invoke the boundary.
