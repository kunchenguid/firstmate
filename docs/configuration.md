...
FM_CLAUDE_VERIFY_POLLS=40   # bounded pane captures a Claude spawn or relaunch accepts before failing the launch as activity-timeout
FM_CLAUDE_VERIFY_INTERVAL=0.5   # seconds between those Claude launch-verification pane captures
FM_CLAUDE_DIALOG_ACCEPTS=3   # bounded Enter auto-accepts of a trust or bypass-permissions dialog before the Claude launch fails as unresolved
