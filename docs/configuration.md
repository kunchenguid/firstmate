...
codebuddy is verified for primary, crewmate, and scout launches: it is a Claude-compatible harness whose project settings live in a tracked `.codebuddy/settings.json`, so a codebuddy primary runs the same Stop-hook `asyncRewake` auto-arm and turn-end guard as Claude Code; [`docs/supervision-protocols/codebuddy.md`](supervision-protocols/codebuddy.md) owns that protocol and [its adapter reference](../.agents/skills/harness-adapters/references/harness/codebuddy.md) owns the remaining boundaries.
It is still deliberately refused for a secondmate, and its worker busy-state source remains unverified, until a codebuddy secondmate lifecycle survives the supervised trial.
