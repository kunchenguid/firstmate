...
The explicit experimental native Windows launcher adds top-level `owner-probe.json` and `owner-receipts.jsonl` records, while [`native-windows-codex.md`](native-windows-codex.md) owns their supported scope and their producer code owns record mechanics.
