...
One captured document announces at most `FM_PR_FOLLOW_MAX_EVENTS` events, plus the head and pr-state lines, which are exempt from that bound because their cursor advance is never re-announced; both configured bounds are refused when they exceed what the apply step accepts (at most 198 events, so a document with its two exempt lines stays inside the 200-event limit, and at most 2048 map entries, with every stored map additionally trimmed to the 8192-character limit the cursor validators enforce), so a poll can never compose bytes its own apply would refuse; per-collection maxima and numeric state maps advance only to what was announced, so an overflowed poll re-announces its remainder next poll instead of losing it; numeric state maps keep the `FM_PR_FOLLOW_MAP_LIMIT` highest ids, and an id the bound evicts stays known through the cursor's durable per-collection maximum, while opaque GitLab discussion states use individual private records so map eviction cannot hide a later resolved or unresolved transition.
GitLab discussion pages rotate newest-first through a durable page cursor, so a newly added tail page is read immediately and every older page is revisited within `ceil(total-pages / FM_PR_FOLLOW_MAX_PAGES)` polls; GitHub review submissions come from the GET-only REST endpoint's newest bounded pages.
...
Worst case at the defaults (a full 18-fetch burst per 300-second slot) is 216 forge requests per hour for the whole home independent of the tracked count, with a per-PR poll interval of at most roster-size x slot while open and roster-size x slot x `FM_PR_FOLLOW_SETTLED_EVERY` once settled; a child's first poll waits for its own slot too, so restarting or reconciling every retained source at once stays inside the same bound, and is exempt only from the settled throttle so a baseline or post-restart verification lands within one roster cycle; registration is never capped because retirement stays explicit.
