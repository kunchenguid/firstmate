...
The inventory preserves its repo-root annotations: bare section references and "this file" refer to `AGENTS.md`, while `docs/` paths are relative to the repository root.
...
fork-divergences.json  tracked intent for every patch the personal-fork main retains beyond official upstream
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  fork-integration/  private separate clone and no-mistakes registration for validated personal-fork main PRs; see docs/fork-main.md
...
state/               runtime records and signals; gitignored
  ...
  .fork-upstream-check  epoch of the last successful daily official-upstream probe; unsafe file types are refused
  terminal-outcomes/  inactive terminal-outcome receipts awaiting upstream proof and reconciliation; bin/fm-inactive-reconcile.sh
  .fm-inherited-config-reread* .fm-inherited-config-reread-retry/ .fm-inherited-config-reread-quarantine/  private secondmate config-reread generations with their retry and quarantine state; bin/fm-config-inherit-lib.sh
...
An absent or malformed regular-file value in `state/.fork-upstream-check` causes another official-upstream probe.
...
The inventory above names wake, watcher, away-mode, and Relay state, while their mechanics remain with the named scripts and reference sections instead of being restated here.
...
Older documentation and some emitted lines call this integration "X mode", and its identifiers retain the `FMX_`, `x-`, and `fm-x-` spellings.
...
FM_CLAUDE_TURNEND_BLOCK_BUDGET=3   # fresh verified automatic-failure epochs before the one-time attended fail-open; ordinary no-claim blocks use the fixed two-identical-block escalation
...
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before an endpoint-recorded local secondmate that is blocked, idle, or of unknown liveness produces one durable parent wake-loop-stall notification; only fm_backend_agent_state=alive together with an exact semantic busy verdict stays quiet because otherwise the busy record can be stale and silently strand the queue; zero or invalid values use 60
