...
A final Lavish result the adapter cannot completely parse is indeterminate rather than empty, so it keeps the source armed instead of retiring it unacknowledged.
...
A concurrent reconcile can never publish a capture before the adapter has acknowledged what it will acknowledge, and a staged marker rather than one unbroken hold is what orders that.
Under the per-source boundary the runner stages the receipt outcome file beside a note naming the exact source and sequence it speaks for, then releases the boundary and runs the keyed-answer feed unlocked, so a hung intake can wedge no reconcile, publication, or retirement of its source.
It re-acquires a fresh hold for the seam, calling `bin/fm-procevent-<adapter>.sh receipt <source-id> <sequence> <result-file> <outcome-file>`, where the outcome file states exactly what the intake returned - `not-fed`, or `fed <exit>` with its bounded output, and an `incomplete` quality when the adapter could not extract its own answers - so an adapter that acknowledges a capture toward its own audience can journal what was received and what was saved, never more than that fact.
Publication ordering is enforced against that marker: a generation whose staged outcome exists with no record that the seam has seen it is declined by publication until the seam has run, whether by its own live runner or by the recovery below.
An adapter without a `receipt` command, and any failure inside the seam, changes nothing about publication or handling.
The runner records that the seam had its one chance at a generation as part of that same hold, so a runner killed between its durable capture and the seam is recoverable: reconcile gives the seam that chance for any captured generation still missing it, before it publishes anything or starts a replacement runner, over whatever that generation can still prove.
A runner that died after its intake returned left the outcome staged beside a note naming the exact source and sequence it speaks for, and recovery hands the seam that recorded verdict; a runner that died before it returned leaves no verdict to state, and the seam is handed an empty outcome.
A staged verdict is an obligation that outlives its poll, so it is still recovered after the source's own registration is gone; a retired source with nothing staged has no poll left to prove anything new and is left alone.
Recovery leaves a generation to a live runner only when that runner staged it itself, so a replacement that reclaimed a dead claim never holds the older generation's owed verdict behind its own poll.
The keyed-answer intake is never re-fed there, so recovery can never apply an answer a second time, and a generation whose seam already ran is left alone.
For Lavish this seam feeds a per-source receipts record (`state/procevent/<id>.receipts`, layout owned by the runner, bytes owned by the adapter) that backs the visible answer-receipt lifecycle the adapter presents through the published poll's `--agent-reply` surface: `Received` states only a durable capture, `Saved` only the intake's verdict with rejected rows reported rather than presented as saved, `Applying` and `Complete` only the handler's explicit `applying` and `complete` calls, a round that carried no answers is acknowledged as the written comment it was rather than as a count of answers nobody asked for, and an exact replay of an already-captured submission is stated as already received - unless that resubmission's own intake outcome proves a new effect, in which case it is stated and saved as the new action it was.
The registered poll presents the newest recorded truth once per armed poll - a quiet retry of the same interrupted poll never re-sends it, so one round is acknowledged once - and an ended review is never retired before its final receipt was displayed by a later poll or became impossible to display because the session is gone; a missing session retires with its receipt durably queued and undelivered.
`arm` resets the record so a reused artifact never inherits an earlier session's rounds, and it refuses to arm at all while any captured result of that artifact is still unhandled, so that reset can never orphan a still-live round from the `Applying` and `Complete` it has yet to record.
The record deliberately outlives the adapter-owned terminal retirement of its own source so the last round of an ended review can still record `Applying` and `Complete`; explicit `retire` and this home's `sweep-home` teardown remove it, each taking the adapter's own record lock with a bounded wait rather than an unbounded one, and reporting a record they could not lock instead of a clean retirement, so a later sweep reclaims it.
...
FM_SNAPSHOT_BUDGET=5                # one total seconds budget for all concurrent remote home-ledger reads and any mixed-fleet fallback they start
FM_SNAPSHOT_LEDGER_MODE=on          # on consumes home ledgers with cache/fallback behavior; off retains the bounded legacy per-home summary path for diagnosis
