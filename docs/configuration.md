...
FM_COMPOSER_GHOST_GRAY_LUMA_MAX=180   # fleet-wide: the higher luminance ceiling a NEAR-ACHROMATIC foreground gets instead of FM_COMPOSER_GHOST_LUMA_MAX, because gray is where harnesses draw placeholders (kiro at 158, rovo at 163) while chromatic real glyphs stay out of reach. Applies to a near-achromatic truecolor run and to a 256-colour index in the standard xterm 232-255 greyscale ramp, which is how the same harness draws the same gray on a pane with no truecolor; the 6x6x6 cube's r==g==b diagonal is grey arithmetically but is deliberately NOT tested, so this ceiling never reaches it. It only ever RAISES the applicable ceiling - a near-gray run takes the higher of the two, so raising FM_COMPOSER_GHOST_LUMA_MAX past this still widens the strip on gray text. Near-achromatic means a channel spread (max channel minus min channel) within a fixed bound of 12, which is a constant rather than a knob because every threshold from 5 to 164 classifies the whole measured fleet identically; a gray palette index has zero spread by definition
