...
While the away-posture record `state/.afk-contract` exists the branch takes every row and main receives none ([pi-supervision-branch.md](pi-supervision-branch.md#postures)); a legacy `state/.afk` daemon flag means nothing on Pi, and a broken branch still falls back to today's wake-to-main path while attended.
The branch's role stays bounded exactly as the captain-approved architecture set it: while attended it cannot merge a PR, land local work, freshly spawn, or answer a decision, and every existing captain gate remains unchanged; under the away-posture record those actions accept the branch only with a validated `--posture` or `--clause <id>` justification, which relocates main's standing authority without widening it (`bin/fm-lease-lib.sh`).
