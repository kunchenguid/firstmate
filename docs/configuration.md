...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr`, `zellij`, `orca`, `cmux`, and `superset` are experimental spawn backends (see [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), [`docs/cmux-backend.md`](cmux-backend.md), and [`docs/superset-backend.md`](superset-backend.md)).
Treehouse remains the worktree provider for tmux, herdr, zellij, and cmux, since herdr, zellij, and cmux are session providers only; Orca and Superset each provide both their own task worktree and terminal endpoint.
...
Zellij, Orca, and Superset are never auto-detected; select them by putting the name in a local `config/backend` file, by exporting `FM_BACKEND=<name>`, or by telling the first mate in chat.
Any value other than `tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `superset` is rejected until another adapter is implemented and verified.
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, `cmux`, and `superset` for ship and scout tasks; `backend=orca`, `backend=cmux`, and `backend=superset` all still refuse `--secondmate` until secondmate launch semantics are designed for each.
...
A Superset task additionally records `superset_workspace_id=` and `superset_terminal_id=`, with `window=fm-<id>` kept as the shared firstmate alias exactly like Orca.
...
A metadata-routed selector returns the recorded backend target (`terminal=` for Orca, the reconstructed `<workspace_id>:<terminal_id>` for Superset, otherwise `window=`), and matching explicit targets can still recover the recorded backend when metadata contains the same endpoint.
...
Selecting any other supervisor backend, including `zellij`, `orca`, `cmux`, or `superset`, refuses at daemon startup instead of trying tmux injection primitives against a non-tmux pane.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `superset`), `jq` for the JSON-emitting experimental adapters (`herdr`, `zellij`, `cmux`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
...
Superset provides both the task worktree and terminal endpoint the same way, so `backend=superset` requires only `superset` (parsed with node, already a universal toolchain requirement) and likewise skips `treehouse` and every other backend's session CLI.
...
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/herdr/zellij/orca/cmux/superset support ship/scout spawns, codex-app is not accepted
