...
FM_TREEHOUSE_READY_POLLS=300   # waits for `treehouse get` to settle a fresh task worktree: polls (default 300) at FM_TREEHOUSE_POLL_INTERVAL seconds each, a ~5 minute default budget; a cold worktree on a large repository can exceed the old fixed 60s, while a genuinely stuck spawn still surfaces within minutes
FM_TREEHOUSE_POLL_INTERVAL=1   # seconds between those settle polls; a full worktree pool is detected fast regardless and fails naming treehouse's own reason instead of burning the budget
