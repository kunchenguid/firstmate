...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait whose current state cannot name it still surfaces once before that cadence bounds repeats, and one whose current state does name it is admitted to that cadence immediately with no first stale wake
