...
FM_TEARDOWN_EVIDENCE_PATH_LIMIT=10   # unmatched paths listed in an unlanded-work refusal before the remainder is summarized as a count
FM_TEARDOWN_EVIDENCE_COMMIT_LIMIT=5   # commits a merged PR head does not contain listed in that same refusal before the remainder is summarized as a count
...
FM_CLAUDE_TURNEND_BLOCK_BUDGET=3   # auto-arm generations of --claude guard re-blocking before the verified one-time attended fail-open becomes reachable; it gates that fail-open rather than capping blocking itself, so an unarmable watcher keeps blocking past Claude Code's 8-block override (docs/turnend-guard.md)
FM_CLAUDE_GUARD_ARM_CONFIRM=10   # seconds the --claude guard's last-resort arm waits for the watcher it spawned to pass the strict predicate before falling back to the bounded re-block; default 30 on Git Bash/MSYS, derived from the same switch as FM_ARM_CONFIRM_TIMEOUT so the guard never confirms in a tighter window than the arm layer it confirms; the arm itself is never budget-limited
