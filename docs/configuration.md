...
Every Treehouse-backed crewmate or scout spawn requires `jq` so `fm-spawn.sh` can inspect Treehouse's JSON status before allocation and refuse an available pool slot that still holds work.
...
Independently of that backend-output requirement, every Treehouse-backed crewmate or scout spawn requires `jq` for its pre-allocation pool inspection.
