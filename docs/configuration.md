...
FM_BACKEND_HERDR_COMPOSER_LINES=20  # herdr-only: tail lines scanned by composer-state guard/fallback paths; native submit confirmation uses agent-state (docs/herdr-backend.md "Current transport behavior")
...
FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6  # herdr-only: minimum per-Enter agent-state confirmation budget for a ready-like pre-injection baseline (docs/herdr-backend.md "Current transport behavior")
