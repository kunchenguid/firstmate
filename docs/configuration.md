...
`VENT.md` is the private home workflow-feedback log; [`bin/fm-teardown.sh`](../bin/fm-teardown.sh) owns importing ordinary workers' feedback, durable snapshots at `data/<id>/VENT.md`, import markers, the home-local `.vent-import.lock` lock, and temporary `.vent-*` publication files.
The root log and its temporary publication files are gitignored; [`bin/fm-brief.sh`](../bin/fm-brief.sh) owns the worker-facing feedback instructions.
