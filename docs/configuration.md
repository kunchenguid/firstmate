...
Cursor ship and scout credential setup is owned by the [`FM_CURSOR_AUTH_ENV` entry](#environment-variables) below.
...
FM_CURSOR_AUTH_ENV=$HOME/.config/crew-router/env  # sourced shell env for cursor ship/scout auth; must be readable and set a non-empty CURSOR_API_KEY; secondmates do not read it
