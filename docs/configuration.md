...
Codex secondmate launches use `fm-spawn.sh`'s silent process-local notification override, so they do not inherit the machine-global generic turn-completion notifier or change its configuration.
...
FM_CAPTAIN_ATTENTION_EXEC=      # optional notifier override; see bin/fm-captain-wait.sh's authoritative header
