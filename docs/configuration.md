...
Rule `approval` and `floor`, and profile `provider` and `floor` are optional declarations that only [typed dispatch resolution](#typed-dispatch-resolution-env-typesafe_provider) applies in code; without that opt-in they are inert, and firstmate's own intake reads them as ordinary hints.
...
## Typed dispatch resolution (.env TYPESAFE_PROVIDER)
...
`TYPESAFE_PROVIDER` selects the service that serves Jev: `typesafe` (the default, using `TYPESAFE_API_KEY` against the typesafe.ai API) or `openrouter` (using `OPENROUTER_API_KEY` against OpenRouter's Decisions endpoint).
Each setting is read from the calling environment first and then from the home's gitignored `.env`; the environment wins, matching the Relay and mail-plane contracts, and the Relay accessor in `bin/fm-env-lib.sh` reads the lines.
It is off unless the selected service's key is non-empty in either place.
Selecting `openrouter` is explicit on purpose: `OPENROUTER_API_KEY` is a general gateway credential other tools share, so its presence alone never activates the tool, and an unknown non-empty `TYPESAFE_PROVIDER` is an actionable exit 2 configuration error rather than a silent fallback.
...
The resolver copies its selected key into a non-exported private variable and unsets both `TYPESAFE_API_KEY` and `OPENROUTER_API_KEY` from its own process before launching children, so neither secret reaches its `curl` or `quota-axi` calls.
Bootstrap also unsets an environment-provided `TYPESAFE_API_KEY` from the firstmate process, but it deliberately leaves `OPENROUTER_API_KEY` there because other harnesses share that gateway credential; a `.env`-only OpenRouter key never enters the environment, while an exported one stays available to firstmate's children.
...
The resolver fixes the endpoint, model, confidence floor at 0.6, and request timeout at 5 seconds: `https://api.typesafe.ai/v1/systemone` with model `jev-latest` for `typesafe`, and `https://openrouter.ai/api/alpha/decisions` with model `typesafe/jev-1.13` for `openrouter`.
`TYPESAFE_PROVIDER` and the selected service's key are its only resolver-specific environment settings.
...
TYPESAFE_PROVIDER=typesafe   # typed dispatch service: typesafe (default) or openrouter
TYPESAFE_API_KEY=       # typed dispatch resolution opt-in for the typesafe service, from the environment or .env; absent means bin/fm-dispatch-resolve.sh is off (docs/configuration.md "Typed dispatch resolution")
OPENROUTER_API_KEY=     # typed dispatch resolution key when TYPESAFE_PROVIDER=openrouter; a bare ambient key never activates the tool
