...
Every new task records `endpoint_task_id=` as the cleanup binding between the metadata filename and its opaque runtime endpoint, including a preserved Orca record emitted after incomplete spawn rollback.
...
`fm-teardown.sh <id>` takes a task id directly, may first run the bounded read-only compatibility proof below, and then validates the complete metadata-only endpoint identity before any cleanup runtime dispatch or cleanup mutation.
Missing, empty, duplicate, malformed, backend-inconsistent, or task-mismatched endpoint records are preserved and refused.
Legacy tmux metadata remains cleanup-compatible when its exact window name is `fm-<id>`.
At cleanup only, a complete ordinary-task unbound record from the immediately preceding spawn schema can gain `endpoint_task_id=` after `fm-endpoint-binding-migrate.sh` proves its exact live identity from backend-owned facts: Herdr requires exact topology, task and unambiguous home or projection labels, and the live foreground worktree; Zellij and cmux require their exact home-scoped task title and topology.
The updater itself does not rewrite operational state, so this bounded cleanup-time migration keeps a fast-forwarded home compatible without weakening the metadata-only cleanup validator.
Legacy Orca and secondmate records remain preserved and refused because the verified API or old parent-side schema does not expose the additional identity relation they require, and malformed, ambiguous, cross-home, copied, or runtime-mismatched records remain unmodified.
Recovery accepts only a project this home contains, meaning the firstmate code root for a home that is its own project, or a project under `$FM_HOME/projects` (`FM_PROJECTS_OVERRIDE` when set).
A record naming any other project stays refused unless the local, gitignored `config/endpoint-binding-recovery-allow` file holds the exact `<task-id> <metadata-sha256> <canonical-project-path>` line the refusal prints, which authorizes that one unchanged record, cannot be replayed against an edited or different record, and relaxes no other proof.
`fm-teardown.sh <secondmate-id> --force` runs the same bounded proof as a separate preflight, scoped to the home that owns each record, before it authorizes or cleans up that home's children, and one unbindable child record refuses the whole retirement.
