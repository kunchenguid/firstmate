...
FM_WORKTREE_WRITE_MAXDEPTH=6       # depth that same probe walks below the recorded worktree; it runs only at the moment a wedge escalation would otherwise fire, never on every poll; neither knob applies to a secondmate, whose recorded worktree is a provisioned home the probe skips entirely
