...
FM_BACKEND_HERDR_IDLE_RE='^Type a message\.\.\.$'  # herdr-only: empty-composer placeholder regex after shared ghost extraction plus border and prompt stripping; write it glyph-free and ASCII-only, per FM_COMPOSER_IDLE_RE below
FM_BACKEND_HERDR_BARE_PROMPT_GLYPHS=  # herdr-only: whitespace-separated list of LITERAL verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›. Defaults to the fleet-wide FM_COMPOSER_AGENT_GLYPHS, so widening that widens herdr too; set this only to give herdr a set the rest of the fleet does not share. Matched with a locale-invariant case, honored for row detection AND the shared classification, so an added glyph's idle composer reads empty rather than wedging on pending; shell glyphs are rejected from the set and remain unknown rather than empty, and the shared glyph matcher refuses to match one whatever set it is handed, which is what keeps a shell glyph out of this backend's structural row scan (that scan has no classifier beneath it, so a dead-shell prompt row reaching it would register as a bare composer row and, dim-rendered, read empty) (a set left with no agent glyphs by that rejection falls back to the built-in default rather than to nothing), and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)")
# FM_BACKEND_HERDR_BARE_PROMPT_RE is RETIRED and no longer honored; setting it warns loudly on stderr. It was a regex ('^[❯›]'), and a regex over multibyte glyphs cannot be matched safely: under the C/POSIX locale the supervise daemon runs with, an ERE bracket class degenerates into a BYTE class, every box-drawing glyph starts with byte E2, and a composer box's own border row then reads as a bare prompt row. Use the literal glyph set above.
...
FM_BACKEND_ORCA_IDLE_RE='^Type a message\.\.\.$'  # orca-only: empty-composer placeholder regex after border/prompt stripping; write it glyph-free and ASCII-only, per FM_COMPOSER_IDLE_RE below
...
FM_BACKEND_CMUX_IDLE_RE='^Type a message\.\.\.$'  # cmux-only: empty-composer placeholder regex after border/prompt stripping; write it glyph-free and ASCII-only, per FM_COMPOSER_IDLE_RE below
...
FM_COMPOSER_IDLE_RE=    # optional empty-composer regex, applied after ghost and border stripping. This is the one composer knob that is still a regex, so it is the one that can still be written unsafely: the match is pinned to the C/POSIX locale (the locale the supervise daemon runs under) so a pattern behaves the same in a hand test as it does in the daemon. Write it WITHOUT a leading prompt glyph - the glyph is stripped for you and the pattern is matched again against the bare placeholder text. If a pattern must name a glyph, spell it as a LITERAL or an alternation of literals ('^(❯|›) ?Type a message\.\.\.$'), NEVER as a bracket class ('^[❯›]...'): under C a bracket class over multibyte glyphs degenerates into a byte class, matching any byte of any box-drawing glyph and none of the glyphs it names
...
FM_COMPOSER_AGENT_GLYPHS='❯ ›'   # fleet-wide: whitespace-separated list of LITERAL agent prompt glyphs (claude's ❯, codex's ›) that read as a genuine empty AGENT composer, bordered or bare; it is the default for every backend, including herdr's own FM_BACKEND_HERDR_BARE_PROMPT_GLYPHS. Never a regex: a bracket class over multibyte glyphs degenerates into a byte class under a C/POSIX locale, so this set is compared with a locale-invariant case. Shell prompt glyphs (> $ % #) are NOT agent glyphs and cannot be made into them, in three layers: one placed in this set is REJECTED with a loud stderr warning; the glyph matcher refuses to match a shell glyph whatever set it is handed, which is what keeps a shell glyph out of a backend's structural composer-row detection (that scan has no classifier beneath it to catch the mistake); and the classifier evaluates the shell rule before the configurable agent match. A bare shell prompt therefore still reads unknown rather than empty by every route - telling a dead shell apart from an empty agent composer is the injection safety rule. A set left with NO agent glyphs by that rejection falls back to the built-in default, warning again: an empty set would strip no glyph at all, leaving every idle agent composer stuck on pending and away-mode deferring every escalation (bin/fm-composer-lib.sh)
