...
devin is verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate, because its primary session-start and watcher protocol is not yet verified; [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md) owns that evidence.
