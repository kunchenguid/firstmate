...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, receipt-backed model-routing outcomes under `data/model-routing/`, and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
Receipt-backed measurement and shadow recommendations for these profiles are optional private records owned by [`model-routing-outcomes.md`](model-routing-outcomes.md); they do not change this schema or replace the dispatch decision procedure.
