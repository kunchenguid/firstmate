...
FM_WORKTREE_DRIFT_INTERVAL=60   # seconds between the watcher's scans for a live worker running outside its recorded worktree (bin/fm-worktree-drift.sh); invalid values use 60
FM_WORKTREE_DRIFT_CONFIRM_SECS=1   # seconds between the two reads that must both place a worker outside its worktree before bin/fm-worktree-drift.sh treats it as drift
