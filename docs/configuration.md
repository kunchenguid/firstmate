...
FM_TASK_METRICS_GH_TIMEOUT=15   # seconds allowed per forge read inside fm-task-metrics.sh; reaching it leaves the affected metrics fields null
FM_TASK_METRICS_TOKEN_TIMEOUT=20   # seconds allowed for the durable per-session token-store read; reaching it leaves the token figure null with the reason in token_note
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; a full current-code-matched active run step refreshes this timer only while its last_activity is newer than the threshold; every non-positive run-state result preserves escalation
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; run-step activity never refreshes this bound, and it is inspection-only, never an automatic interrupt or restart
