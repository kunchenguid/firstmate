...
FM_COMPOSER_IDLE_RE=    # optional fleet-wide idle-placeholder regex override (bin/fm-composer-lib.sh); applied after ghost and border stripping and after every non-ASCII Unicode whitespace code point (U+00A0 among them) is normalized to a plain space and trimmed. A match alone does not prove emptiness because shape-specific position and ANSI de-emphasis safety gates still apply
