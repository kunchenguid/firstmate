...
`FM_HOME` determines Herdr's supervisor home label: the primary home uses `firstmate`, and a secondmate home marked by `.fm-secondmate-home` uses `2ndmate-<secondmate-id>`.
[`herdr-backend.md`](herdr-backend.md#watching-and-task-containers) owns the separate role-neutral worker container, parent resolution, collision handling, and recovery behavior.
