...
Every axi-family floor is the current latest published version of that tool, which the captain re-bumps periodically to keep the whole fleet on the newest axi tools; it is not the minimum version that introduced some depended-on behavior, and the floors are expected to drift upward as new versions ship.
[`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh)'s header owns that policy and the exact gh-axi and lavish-axi floors, while [`bin/fm-tasks-axi-lib.sh`](../bin/fm-tasks-axi-lib.sh) and [`bin/fm-quota-axi-lib.sh`](../bin/fm-quota-axi-lib.sh) own their own tools' floors under the same policy, plus the separate tasks-axi feature probes.
