...
The `tea` login firstmate uses for an instance is always derived from that base URL as `firstmate-<host>` (the host with `.` replaced by `-`), so `tea login add --name firstmate-<host> --url <base-url> --token <token>` is the matching one-time setup; the token needs the `write:repository` and `read:user` scopes.
