...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before wedge tracking starts; a still-busy pane below the hard ceiling is suppressed rather than escalated
FM_BUSY_TURN_HARD_MAX_SECS=21600   # hard ceiling on the same no-completed-turn age: a still-busy pane past it escalates anyway (possible hung turn), then re-escalates no more than once per hard interval; inspection-only, never an automatic interrupt or restart
