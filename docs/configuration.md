...
omp is reachable only through an explicit `--harness omp` or a later explicit local `config/crew-harness` value; it is never a coordinator, a backend, or an implicit default.
