...
Bootstrap validates resolver-only `approval`, `floor`, and present `provider` values only while typed resolution is active; without the key those inert fields are ignored and bootstrap behavior is unchanged.
Bootstrap accepts `gemini` with or without the key because AGENTS.md section 4 verifies it for crewmate and scout dispatch.
