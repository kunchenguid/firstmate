...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb, but only once per distinct situation (state/.stale-surfaced-*; docs/architecture.md), never per poll or pane-hash tick
