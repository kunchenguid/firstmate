...
The universal list also carries perl's `JSON::PP` module, which the captain-hold, procevent-lavish, and extension-capture surfaces decode task and event JSON with.
It ships with a full perl distribution but is packaged separately on minimal installs, so bootstrap probes it directly and reports it as `MISSING_MANUAL: perl JSON::PP module` with per-platform package commands instead of an install URL.
The check is detect-only like the rest of bootstrap; until the module is installed, `bin/fm-captain-hold.sh`'s decoding subcommands (`hold`, `answer`, `answers`, `complete`, `verify`, `diverged`, `open`, `reconcile-requests`) fail with the same install hint rather than reading silently empty fields, while `bind`, `unbind`, and `binding` stay usable.
...
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded rechecks of a declared external wait or verified captain-held transfer, and between repeated new-hash stale alarms for an ordinary crew task with an open backlog captain call; a structured until time can make an external-wait recheck occur sooner but cannot extend this bound; this includes a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS, while the away-mode daemon uses the same setting and ages its window against the crew's own latest status line rather than pane busy state, except a paused: line claiming a no-mistakes run, which the daemon rechecks on FM_STALE_ESCALATE_SECS instead; a captain-held transfer is never rechecked while the away-posture record exists
