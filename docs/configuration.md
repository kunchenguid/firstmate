...
A captain-facing (verdict `captain`) branch outcome persists as one exact, sequence-keyed visible transcript entry without opening a model turn.
...
The generated [Pi supervision protocol](supervision-protocols/pi.md) owns main's event ownership and conversational treatment for merged outcomes, while the persisted entry itself owns captain visibility.
