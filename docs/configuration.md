...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; ordinary stale panes whose crew is not provably working surface immediately, while docs/architecture.md owns the declared-pause exception
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; docs/architecture.md owns when a declared wait takes the FM_PAUSE_RESURFACE_SECS recheck below instead
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks after docs/architecture.md's declared-pause triage selects that cadence; the away-mode daemon uses the same setting for declared external waits
