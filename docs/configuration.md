...
FM_CREW_STATE_RUN_ACTIVITY=   # set to 1 by the watcher's wedge timer only; makes fm-crew-state.sh lead a working run-step detail with `activity: recent` when the live axi status active_steps table has no quiet prefix (never a negative spelling, and no field during the ci step), so the default supervisor-facing line never carries it
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; a pane whose attributed run currently reports recent pipeline activity (crew_run_activity_is_recent) is deferred rather than escalated, as is a pane whose own task worktree was written during the quiet window, and both re-surface on FM_PAUSE_RESURFACE_SECS measured on one shared deferral clock that a change of reason does not restart; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once before that cadence bounds repeats
