...
Set `FM_SECONDMATE_REMOTE_HOME` to the remote home's absolute path when scaffolding a remote route's charter by hand, so its steering inbox and parent channel render as `<remote-home>/state/parent-route/<id>.inbox` and `<remote-home>/state/parent-replies.status` instead of this home's `state/` paths, which name nothing on that host; `fm-remote-home-seed.sh` passes it for a charter it scaffolds itself.
