...
## Plain style (config/plain-style)

`config/plain-style` is an optional local, gitignored presence flag that switches the agent's chat with the captain from the default captain address and optional nautical seasoning to a plain colleague style.
Only the file's presence is read, so its contents are ignored; create it with `touch config/plain-style` under the effective `FM_HOME` and remove it to return to the default.
With it absent, behavior is exactly the default stated in [`AGENTS.md`](../AGENTS.md): the "captain" address in every chat message, optional nautical phrasing, and the exact `Captain, shipshape.` routine reply.
With it present, `AGENTS.md`'s opening rules switch to plain style: the user is addressed as "you" with no title, no nautical phrasing is used, the routine no-action reply becomes exactly `No action needed for this update.`, and house vocabulary is translated in user-facing prose (first mate -> coordinator, captain -> you / the user, crewmate -> worker, scout -> investigation, second mate -> delegated coordinator).
Exact command names, skill names, paths, schema fields, status markers, and quoted evidence are never translated, and the flag changes presentation only: no safety rule, approval boundary, routing rule, or the secondmate parent-channel rule changes with it.
The session-start digest's bootstrap section surfaces a present flag on every path, including read-only sessions, as `BOOTSTRAP_INFO: plain style active (config/plain-style present)`; the flag is silent when absent, and the agent honors it only through that digest line so a mid-session edit takes effect at the next session start or instruction refresh.
The flag is inherited by secondmate homes through the inherited-local-material contract in [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md), because a communication preference belongs to the captain rather than to one home; the primary's presence or absence converges into every validated secondmate home like the other declared items in `bin/fm-config-inherit-lib.sh`.
