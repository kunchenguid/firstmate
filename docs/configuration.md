...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.48.0 or newer, compatible gh-axi, chrome-devtools-axi, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
FM_DAEMON_CAPTURE_TIMEOUT_SECS=5  # maximum seconds for each backend capture the daemon reads (busy probe, stale/pause recheck) before it is cut off and treated as an unreadable endpoint; invalid or zero values use 5
