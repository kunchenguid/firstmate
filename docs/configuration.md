...
claude, codex, opencode, pi, pi-signed, grok, and kimi are empirically verified for crewmate and secondmate launches; cline, cursor-agent, copilot, and agy are verified for crewmate launches only and are not yet wired for secondmate use.
...
FM_HERDR_PRESENTATION_LOCK_RETRIES=50    # herdr-only: 0.1s attempts fm-teardown.sh waits for the named-session presentation lock, which serializes concurrent teardowns, before refusing as contended
