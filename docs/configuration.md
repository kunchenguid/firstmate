...
An Antigravity or other OpenAI-compatible model lane on Pi is a custom `models.json` provider plus `--model provider/id` on the existing Pi adapter, not a new harness; [`docs/verification/pi-custom-providers.md`](verification/pi-custom-providers.md) owns the isolated-edit evidence.
