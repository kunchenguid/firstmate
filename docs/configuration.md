...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex selects native Stop delivery or foreground checkpoints through the [shared capability gate](watcher-continuity.md#codex-native-stop-ownership), Pi and pi-signed use the same two tracked primary extensions, omp uses its own two tracked `.omp/extensions/` files with a blocking `session_stop` turn-end hook, and OpenCode uses its TUI plugin.
...
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per explicit foreground watcher diagnostic checkpoint
