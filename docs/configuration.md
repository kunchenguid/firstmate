...
`state/` holds volatile runtime records such as task metadata, append-only status events, endpoint signals, watcher and wake-queue coordination, pre-spawn lane-governor leases, away-mode state, generated X-mode artifacts, private secondmate config-reread generations with their retry and quarantine state, and parent-owned secondmate pending-reply records under `state/pending-replies/` (`bin/fm-pending-reply-lib.sh`).
...
`.lavish/` holds generated Lavish artifacts, including the fleet dashboard HTML and Markdown whose read pointer is `data/dashboard.md`.
...
In a home marked with `.fm-secondmate-home`, bootstrap also emits `SECONDMATE_SELF_CHECK:` when routed-work prerequisites are missing, unauthenticated, or too old.
That line is visible only inside the secondmate home that printed it, so the secondmate itself must report the condition as a `blocked:` status append on its own status file; that append is the relay that reaches the routing home, and the main firstmate treats the secondmate as unavailable for routed work until the secondmate reports it resolved.
...
FM_LANE_MIN_AVAILABLE_RAM_GB=1 # refuse new spawns below this available-RAM line (Linux MemAvailable; macOS kern.memorystatus_level free percentage of hw.memsize, falling back to memory_pressure); 0 disables the RAM threshold
