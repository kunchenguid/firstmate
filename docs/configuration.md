...
FM_PERMISSION_DIALOG_REGEX='Do you want to proceed\?'   # permission-confirmation dialog phrase, shared by watcher, fm-crew-state pane fallback, and tmux helper
FM_PERMISSION_DIALOG_CONFIRM_REGEX='^[0-9]+\.[[:space:]]*(Yes|No)\b|Permission rule .*requires confirmation'   # compound signature required alongside the phrase above to avoid false-matching ordinary CLI confirmation prompts
