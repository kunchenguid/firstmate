...
[`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) owns the axi-family floor policy and the gh-axi, installed glab-axi, and lavish-axi floors, while [`bin/fm-tasks-axi-lib.sh`](../bin/fm-tasks-axi-lib.sh) and [`bin/fm-quota-axi-lib.sh`](../bin/fm-quota-axi-lib.sh) hold their own tools' floor constants.
...
`glab-axi` remains task-scoped rather than universal, so homes without GitLab work remain usable and an absent binary is reported only when a GitLab operation needs it.
When `glab-axi` is installed, bootstrap requires 0.2.0 or newer for guarded immediate squash merge and the current normalized MR JSON fields; an older or unparseable build reports `MISSING: glab-axi (install: glab-axi update)` and waits for captain-approved self-update.
GitLab MR recording, guarded merge, and cleanup also require `jq` to validate one complete normalized JSON document, while the existing byte-static read poll retains its separately bounded plain-glab transport.
