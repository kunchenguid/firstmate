...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `psmux`, `herdr`, `zellij`, `orca`, and `cmux` are experimental spawn backends (see [`docs/psmux-backend.md`](psmux-backend.md), [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), and [`docs/cmux-backend.md`](cmux-backend.md)).
`psmux` is a native-Windows, tmux-compatible multiplexer that reuses the tmux adapter; it is firstmate's Windows spawn backend where no Unix/macOS backend runs.
Treehouse remains the worktree provider for tmux, psmux, herdr, zellij, and cmux, since psmux, herdr, zellij, and cmux are session providers only; Orca provides both the task worktree and terminal endpoint.
...
Zellij, Orca, and psmux are never auto-detected; select them by putting the name in a local `config/backend` file, by exporting `FM_BACKEND=<name>`, or by telling the first mate in chat (psmux is Windows-only, and setting `config/backend=psmux` is the normal way to make firstmate spawn workers on a Windows home).
Any value other than `tmux`, `psmux`, `herdr`, `zellij`, `orca`, or `cmux` is rejected until another adapter is implemented and verified.
`fm-spawn.sh` accepts `tmux`, `psmux`, `herdr`, `zellij`, `orca`, and `cmux` for ship and scout tasks; `backend=orca` and `backend=cmux` both still refuse `--secondmate` until secondmate launch semantics are designed for each.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `psmux`, `herdr`, `zellij`, `orca`, or `cmux`), `jq` for the JSON-emitting experimental adapters (`herdr`, `zellij`, `cmux`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `psmux`, `herdr`, `zellij`, `cmux`).
...
A psmux, herdr, zellij, or cmux home is therefore never told `tmux` is missing, and the `treehouse` durable-lease upgrade check runs only for the backends that actually use treehouse.
...
FM_LOCK_OS_OVERRIDE=     # force session-lock harness-ancestry resolution to posix|unix or windows (fm-session-lock-lib.sh), for exercising the POSIX contract on Windows and in CI; does not affect the real-filesystem lock symlink mode
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/psmux/herdr/zellij/orca/cmux support ship/scout spawns (psmux is Windows-only), codex-app is not accepted
