...
Every listed channel fires independently, so a missing or failing one is logged as a failed channel without suppressing the others or crashing the daemon.
...
FM_BACKEND_HERDR_COMPOSER_LINES=40  # herdr-only: tail lines scanned by composer-state guard/fallback paths; idle-baseline submit confirmation uses agent-state
...
FM_WATCHER_REAP_SECS=20            # seconds the shutdown trap waits for the watcher child after SIGTERM before killing it hard; default is FM_POLL + 5 with a floor of 20 so the bound clears one watcher poll cycle; invalid or zero values use that default; an explicit value is used verbatim with no floor; the daemon publishes the resolved value in state/.supervise-daemon.reap-secs (removed on shutdown) and the away-mode stop path waits that long plus 10s of headroom, never less than 40s
