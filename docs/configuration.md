...
FM_CLAUDE_AUTOARM_OWNER_WAIT_MS=2000   # milliseconds a firing Claude Stop auto-arm waits out a guard-held auto-arm owner lock (or an unstamped holder) before standing down; only a role naming another arm stands it down immediately
FM_CLAUDE_AUTOARM_STRUCTURAL_RECHECK_MS=500   # milliseconds the --claude guard rechecks recovery after recording a structurally unclaimed episode, so a late arm that could only claim afterwards still releases the stop
