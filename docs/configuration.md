...
`FM_TOOL_UPDATE_INTERVAL` (default 900 seconds, `0` to probe on every run) sets how often probes actually run, `FM_TOOL_UPDATE_PROBE_SECS` (default 5) bounds one probe, and `FM_TOOL_UPDATE_BUDGET_SECS` bounds a whole sweep.
The budget defaults to the larger of the documented 20 seconds and one probe bound per watched tool, so a tool late in a long sweep keeps a full slice instead of only what the earlier probes left; an explicit value from 1 to 120 overrides that.
