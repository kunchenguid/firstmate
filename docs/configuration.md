...
omp (Oh My Pi) is a CANDIDATE crewmate/scout adapter rather than a verified one: the adapter is wired and deterministically proven by `tests/fm-omp-harness.test.sh`, but its first live pilot is separately approval-gated, so it must not be described as live-verified or given real work until that pilot passes.
`fm-spawn.sh` refuses omp for a secondmate on adapter identity alone, whatever its model, before the watcher guard runs and before any lock, endpoint, worktree, state, or config mutation, because omp has no primary supervision protocol.
That identity refusal also covers a raw launch command whose first non-assignment word is omp, the same as `--harness omp`.
omp is reachable only through an explicit `--harness omp`, a raw launch command whose first non-assignment word is omp, or a later explicit local `config/crew-harness` value; it is never a coordinator, a backend, or an implicit default, and Orca remains the execution backend for it like every other adapter.
`fm-spawn.sh` resolves the backend read-only before any mutation, retains that value, and refuses any resolved backend other than `orca` at the same early gate as the model pin.
omp launches are pinned to one release: `fm-spawn.sh` resolves the executable named `omp` from `PATH` and refuses the launch unless it reports exactly `omp/17.2.9`, so an absent, drifted, or substituted build fails loudly instead of running unproven.
Every omp launch is also pinned to one model: each spawn must carry an explicit `--model <provider>/<model>` flag of its own, validated structurally as exactly one slash between two segments of letters, digits, dot, underscore, or dash, then passed through byte-for-byte on omp's `--model` flag.
...
An absent, unqualified, malformed, or ambiguous value refuses the launch before the watcher guard runs and before any lock, endpoint, worktree, state, config, registry, metadata, or extension mutation, whether omp was selected explicitly, positionally, from configuration, in a batch, or by a raw launch command whose first non-assignment word is omp.
...
The launch clears every foreign marker `bin/fm-harness.sh` tests before omp's own - `CLAUDECODE`, `PI_CODING_AGENT`, `GROK_AGENT`, `FM_PI_HARNESS`, `CURSOR_AGENT`, and `CURSOR_INVOKED_AS`, plus `TRACEPARENT`, sets the firstmate-owned `FM_OMP_HARNESS=1` marker, passes `--no-extensions`/`--no-skills` so the only loaded extension is the firstmate-owned `state/<id>.omp-ext.ts` busy-state file, and passes a `--tools` allowlist of core coding tools that excludes omp's `task` subagent delegation and its browser, computer, web-search, and MCP surface.
