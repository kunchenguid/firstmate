...
Delegated launches preload FirstMate's tracked startup guard before Pi imports its CLI, hiding the project `.pi/settings.json` and legacy `.pi/commands` migration surfaces during Pi 0.81.1's pre-flag startup phase.
