...
FM_PR_POLL_LOCK_TIMEOUT=5   # seconds the watcher waits for one task's PR-poll publication lock before leaving that check to the cycle's rejected-check wake and polling the other tasks
FM_PR_ENQUEUE_ATTEMPT_CEILING=3   # automatic enqueuePullRequest attempts bin/fm-pr-enqueue.sh allows for one armed pull request across all its merge-queue ejections; past it the ejection escalates to the captain instead of being re-queued; invalid or zero values use 3
FM_PR_ENQUEUE_UNKNOWN_BUDGET_SECS=60   # seconds that script waits for GitHub to recompute mergeability after an ejection advanced the base branch; a still-UNKNOWN pull request escalates and is never treated as mergeable; invalid values use 60
FM_PR_ENQUEUE_UNKNOWN_SLEEP_SECS=1     # first delay between those reads; a nonzero delay doubles up to 30 seconds, a zero delay never grows, and a wait ended by the 8-read ceiling before the budget elapses says so rather than claiming the budget was spent; invalid values use 1
