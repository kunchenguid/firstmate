...
FM_PIPELINE_STATUS_TIMEOUT=10      # wall-clock seconds crew_pipeline_step_active's own `no-mistakes axi status` call may take; it runs only at the moment a wedge escalation would otherwise fire, never on every poll; hitting the bound, a missing run, no active_steps table, or a `quiet`-prefixed last_activity all read as no evidence, which leaves the escalation schedule exactly as it was; a value that is not a positive integer falls back to the default
