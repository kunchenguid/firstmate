...
FM_DECLARED_WORK_STALL_SECS=900    # seconds a secondmate's unresumed declared work may sit on an idle endpoint before it surfaces as a stall, once per declared-work episode; a declared pause, a terminal last event, a proven-busy endpoint, or a task with no local state/<id>.turn-ended marker to read an idle clock from never counts
