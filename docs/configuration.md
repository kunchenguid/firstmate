...
FM_TELEGRAM_BACKOFF_SECONDS=5  # telegram adapter only: the pause before re-polling after a transient network or server error, the fallback when an HTTP 429 carries no usable retry_after, and the wait between re-polls while a batch already handed to the runner stays unacknowledged - that last one is the steady state for as long as a handler takes, because Telegram answers those requests at once instead of long-polling, so 0 is an unpaused request loop against the Bot API rather than a faster retry
