...
Each source has one private mode `0600` pending-reply file and one stable in-flight reply file under `state/procevent/`; each record carries the existing registration generation and host status-log path, and a second unconsumed reply for that generation appends after one blank line instead of overwriting the first.
