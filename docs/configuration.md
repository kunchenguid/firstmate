...
## Progress phase and remaining-time guess (data/phase-history.jsonl)

Every live task carries a display-only progress phase and a remaining-time guess, shown as a suffix on the worker's Herdr workspace label and as the `phase` and `eta_guess` columns of `/bearings`.
...
`data/phase-history.jsonl` is the gitignored per-home history: one JSON line per finished task, appended by teardown through `bin/fm-progress.sh record` with the task's per-phase seconds and fix-round count, never rewritten, and dropped without recording on a forced teardown or an Orca cleanup recovery.
`state/.progress-<id>` is the per-task observation record that the watcher tick alone advances and teardown retires; a `/bearings`, fleet-snapshot, or `bin/fm-progress.sh show` read derives the phase in memory and never writes it, so two observers can never overwrite each other's record, and deleting it only restarts that task's accumulators from its spawn instant.
`state/.progress.label-<id>` beside it is the label refresh's own bookkeeping (the suffix last applied, the pending retry, and the reason last warned about), written by the refresh alone, bound to the same `spawn_gen`, and retired with the record; the tick never writes it and the refresh never writes the record, so neither can overwrite the other.
...
That cadence is keyed on the tick's own stamp inside the observation record, so a `/bearings` or fleet-snapshot read between two ticks never postpones the next label refresh, and the single flight holds across watcher restarts through `state/.progress-tick.pid`, which names the last tick child and counts as a running tick only while that pid is alive, its command line is the tick, and the marker is younger than `FM_PROGRESS_TICK_MAX_SECS`; anything else is reclaimed, with one warning when the age bound is what expired. A reclaimed tick that later finishes its read finds the record already stamped by its replacement and drops its own observation, so the record's clock never runs backwards and a stale pass cannot revert the phase or label the replacement published; that check and the write are one step under `state/.progress.lock-<id>`, a directory the tick holds for the milliseconds of its write and retires with the record (a hold left by a tick that died is broken after ten seconds), so two passes that both read the record before either wrote it still cannot both write; a label refresh that resumes after a slow Herdr call writes only its own bookkeeping file, never the record, and the next cadence re-applies the current suffix if that late rename overtook the replacement's.
