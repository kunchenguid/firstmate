...
Write `on` into the local, gitignored `config/claude-remote-control` file, and every claude-harness crewmate, scout, and secondmate launches with Claude Code's `--remote-control <task-id>` flag, so the ordinary interactive session is also reachable from claude.ai/code and the Claude mobile app under that task's name.
Publishing a worker beyond the local machine is reachability the home has to grant, so it stays off by default: absent, empty, or `off` launches claude workers exactly as before, and an unrecognized value warns and keeps them local-only rather than failing a spawn over a reachability setting.
Nothing else about an opted-in session changes: the folder-trust prompt, the per-task turn-end notification, and the semantic busy-state hooks all behave exactly as they do without the flag, and `fm-peek` and `fm-send` read and steer such a pane unchanged; [runtime backend verification](verification/runtime-backends.md#claude-remote-control) owns that evidence.
...
This preference is local to each Firstmate home and is not part of secondmate inherited configuration, so a secondmate home that wants Remote Control needs its own file.
