...
Edit that local registry to change routines; the header of [`bin/fm-routine-scan.sh`](../bin/fm-routine-scan.sh) is the single owner of the exact record schema, accepted cadence and delivery values, diagnostics, due-output shape, and `state/.routine-fired` mechanics.
...
The deferred watcher check retains due records in `state/.routine-pending` and only acknowledges them into fire state after the complete due output has entered the wake rail, so an interrupted or failed emission remains available for at-least-once surfacing.
