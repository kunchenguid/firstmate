...
FM_DRAIN_UNRECOGNISED_MAX=10  # unrecognised status lines printed per TASK in UNREAD STATUS, so one drain may print this many for each task with unread prose; the rest are counted and held back to the next drain rather than dropped, and 0 prints the count alone
