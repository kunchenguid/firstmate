...
FM_TEARDOWN_STALE_CONTENT_SCAN_LIMIT=500  # commits of HEAD ancestry fm-teardown.sh scans when proving a dirty worktree is only a stale copy of an already-landed commit; a smaller bound can only cost a refusal, and a non-numeric or zero value falls back to the default rather than disable the proof
