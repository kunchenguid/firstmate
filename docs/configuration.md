...
A primary launched through `bin/fm-primary-pi.sh` also owns private exact-session custody, lifetime-lease, recovery-lock, and post-launch-attestation records under `state/primary-pi/`; that script's header owns their complete schemas and [`herdr-backend.md`](herdr-backend.md#primary-pi-persistence-and-exact-recovery) owns operator setup and limits.
