...
FM_SECONDMATE_WAKE_STALL_SECS=60   # minimum age of the oldest valid foreign wake-queue row before an endpoint-recorded local secondmate produces one durable parent wake-loop-stall notification unless that mate's own fixed-identity delivery-progress marker proves an actionable-wake handoff at or after the row was enqueued; zero or invalid values use 60
