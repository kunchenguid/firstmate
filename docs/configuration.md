...
FM_SUPERVISOR_REFRESH_SECS=20  # zellij-only: seconds between supervisor-pane redraws; blank, non-numeric, or 0 uses 20 (docs/zellij-backend.md "Supervisor surface"; unrelated to the away-mode FM_SUPERVISOR_BACKEND/FM_SUPERVISOR_TARGET pair below)
FM_SUPERVISOR_STATE_REFRESH_SECS=60  # zellij-only: seconds between the expensive per-crew run-state lookups a supervisor pane makes; the last line is reused between them; blank, non-numeric, or 0 uses 60 (docs/zellij-backend.md "Supervisor surface")
FM_SUPERVISOR_PEEK_LINES=20  # zellij-only: peek lines each supervisor pane renders; blank, non-numeric, or 0 uses 20 (docs/zellij-backend.md "Supervisor surface")
