...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per authoritative no-mistakes query inside fm-crew-state.sh; the live run's best-effort gate inspection (home view, per-run status) keeps its own fixed 3-second bound so a slow enrichment cannot stretch a supervision read
