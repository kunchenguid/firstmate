...
It changes no runtime state and publishes the binding only after two exact live correlations of the recorded pane, tab, workspace, unique task label, and physical foreground worktree.
It holds the task's spawn lock and then the shared per-task metadata lock every other metadata mutator takes across that whole snapshot-through-publish span, so a concurrent cleanup, PR-check, promotion, spawn, or decision-hold write can be neither lost nor resurrected; both locks refuse rather than wait, so a busy task asks the operator to retry.
Missing, stopped, exited, renamed, duplicated, moved, or unreadable endpoints remain refused without changing metadata, a task on another backend is refused by its real backend name, and an unverified Herdr client refuses with the same version diagnostic every other Herdr entrypoint reports rather than as a stale endpoint.
...
Because `bin/fm-watch.sh` reads `FM_CHECK_INTERVAL` only at process start, a cadence transition - opt-in while a watcher is already running, or opt-out - takes effect on the next home-scoped watcher cycle the emitted harness protocol starts; bootstrap deliberately never restarts the watcher itself.
The bootstrap transition line is routine continuity rather than a supervision failure, so it appends the non-blocking continuity instruction whose contract [`turnend-guard.md`](turnend-guard.md#harness-integrations) owns.
