...
`commands.test` stays changed-file-scoped and must never become a complete `tests/*.test.sh` walk: `--changed` selects only the families the branch's changed files map to, runs the proven-isolated ones with bounded concurrency, keeps every unproven stateful script serial, and applies its own generous per-script bound, so the gate never has to guess a shorter one.
