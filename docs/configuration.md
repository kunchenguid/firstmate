...
FM_PANE_READY_TIMEOUT=5  # tmux-only: seconds a launch's literal send waits for the target pane to start reading input before refusing to type a line the pane would silently truncate (docs/tmux-backend.md "Launch command delivery"); a nonpositive or unparseable value checks once and then refuses
