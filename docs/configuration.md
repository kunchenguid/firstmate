...
FM_SESSION_START_STATUS_TAIL=5   # state/*.status lines printed per task in the session-start digest; each line is capped by bin/fm-line-cap-lib.sh; also part of the fleet-state fingerprint key, so changing it invalidates every state/.session-start-seen-<task> marker and reprints every task's full block on the next session start
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; each window advances the escalation count, but only the first crossing and the round reaching FM_WEDGE_DEMAND_INSPECT_COUNT wake at once, every other round being throttled to the FM_PAUSE_RESURFACE_SECS reminder cadence below; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence, while a live idle declared wait still surfaces once before that cadence bounds repeats
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait or verified captain-held transfer, including a live idle pane after its first inconclusive stale wake, a live busy pane past FM_BUSY_TURN_MAX_SECS, and the reminders of a continuing wedge episode past its first escalation; lowering it makes a still-wedged pane report closer to once per FM_STALE_ESCALATE_SECS again, raising it makes a genuinely hung task remind that much more rarely, though neither the first crossing nor the FM_WEDGE_DEMAND_INSPECT_COUNT round is ever throttled; the away-mode daemon uses the same setting, ageing its window against the crew's own latest status line rather than pane busy state
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; reaching it is also the second and last escalation of an episode that wakes regardless of the FM_PAUSE_RESURFACE_SECS reminder throttle, the first crossing being the other
