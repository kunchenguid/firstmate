...
Compatible means the shared bootstrap probe accepts `tasks-axi --version` as `FM_TASKS_AXI_MIN` (0.2.2) or newer, `tasks-axi update --help` exposes `--archive-body`, and `tasks-axi mv --help` exposes `[<id>...]` for the atomic multi-ID move introduced in 0.2.2 and required by handoff delegation.
`bin/fm-tasks-axi-lib.sh` is the single owner of that floor constant and probe; this section summarizes the contract so other documents can point here instead of restating the gates.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, gh-axi v0.1.29 or newer, chrome-devtools-axi, lavish-axi, compatible tasks-axi v0.2.2 or newer per "Backlog backend" above, and quota-axi v0.1.16 or newer.
...
An absent or too-old `gh-axi` reports `MISSING: gh-axi (install: npm install -g gh-axi && gh-axi setup hooks)`; non-interactive PR merges need the restored bare `--squash` shorthand on that floor.
