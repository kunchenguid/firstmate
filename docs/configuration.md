...
FM_NM_PROGRESS_TIMEOUT=10          # wall-clock seconds for each of the two no-mistakes calls that measure the current step, taken only at the moment a wedge escalation would otherwise fire; a timed-out call reads as no progress evidence, leaving the escalation schedule exactly as it was
