...
When an allowlisted config item changes for an already-running local home, it sends the literal-content reread pointer described in [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md); a payload that matches that home's latest delivered generation sends no pointer unless a previous delivery is pending.
