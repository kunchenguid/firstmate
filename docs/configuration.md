...
When that work lives in a REMOTE secondmate home, delivering or retiring the loop first clears its bound legacy link over that route's SSH transport, so closing such a loop needs the host reachable; a host that never confirms the clear leaves the loop retained for reconciliation there rather than closed here.
