...
Everything after the answer runs in code: the confidence floor, the matched rule's `approval` and `floor`, each candidate's `provider` and `floor`, every applicable account-wide and model/product row from one `quota-axi --json` snapshot (schema 5 or schema 6; under schema 6 a candidate binds to the provider row whose `accountKey` is `codex-home` for native Codex regardless of model or its Pi lane prefix, such as `openai-codex-work`, else the `default` row, and an expanded provider with neither row leaves it eligible but unranked), and the numeric `spendPriority` argmax over candidates using each candidate's limiting row.
