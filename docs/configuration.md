...
FM_AUTOARM_SESSION_LEASE_TIMEOUT=1   # seconds a Stop auto-arm waits for the session-acquisition lease before owner-fenced failure publication; invalid or zero values use 1 and values above 5 clamp to 5
FM_AUTOARM_TRANSITION_GRACE=2   # seconds before an identity-verified, kernel-stopped auto-arm transition holder may be retired; age alone never retires a live holder; invalid or zero values use 2
FM_AUTOARM_TRANSITION_ATTEMPTS=150   # 20ms attempts for ordinary auto-arm transition acquisition; invalid or zero values use 150 and values above 250 clamp to 250
FM_AUTOARM_FAILURE_TRANSITION_ATTEMPTS=150   # 20ms attempts before failure publication uses its lockless reset- and ledger-fenced fallback; invalid or zero values use 150 and values above 250 clamp to 250
