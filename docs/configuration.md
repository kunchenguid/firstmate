...
A home that works on a GitLab-origin project also needs `glab` authenticated for that origin host before it can open or watch a merge request.
This section is the single owner of the universal and forge-specific toolchain requirements; backend guides' prerequisites point here and add only their backend-specific tools.
