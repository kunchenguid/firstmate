...
`fm-spawn.sh` pre-registers every crewmate or scout kimi worktree in Kimi's own per-root trust store through `bin/fm-kimi-trust.sh` and refuses the launch when that fails, because Kimi trusts each exact folder separately and never inherits trust from a parent; [its adapter reference](../.agents/skills/harness-adapters/references/harness/kimi.md#workspace-trust) owns the trust handling.
A kimi secondmate home is the one launch kind that is not pre-registered, so that pane can still meet the folder-trust dialog and still depends on the live Enter backstop; [the same workspace trust section](../.agents/skills/harness-adapters/references/harness/kimi.md#workspace-trust) owns that gap.
It still refuses kimi on cmux and Orca at preflight, because the live backstop that answers a folder-trust dialog shown anyway needs a verified viewport-only capture those backends lack; [the readiness section](../.agents/skills/harness-adapters/references/harness/kimi.md#readiness-gated-start) owns that gate.
