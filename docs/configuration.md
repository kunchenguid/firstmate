...
## Pipeline-activity wait deferral (config/wedge-defer-pipeline)

The optional local, gitignored `config/wedge-defer-pipeline` presence flag opts this home into a default-off third form of wait evidence in the watcher's wedge timer.

### When an active validation step defers an alarm

With it present, a provably-working pane about to escalate is also deferred to the `FM_PAUSE_RESURFACE_SECS` recheck cadence when its crew's own validation pipeline reports one of its steps still producing output, and the recheck names the pipeline as what the lane is waiting on instead of reporting a suspected wedge.
A validation step's agent works in the pipeline's separate checkout, so neither the crew's pane nor its worktree can show that progress, while a step the pipeline itself reports quiet keeps the ladder.
It stays opt-in because it trades a shorter false-alarm stream for a longer worst-case delay before a crew wedged mid-validation is reported, which is a home's choice to make.

With the flag absent the wedge timer spends no current-state read for it, `bin/fm-crew-state.sh` publishes no `pipeline-activity: recent` component, no record is written, and escalation keeps the unchanged schedule, reasons, and `demand-deep-inspection` wording.
Unlike the parked-gate flag, it is inherited by secondmate homes: secondmates drive validation runs of their own and meet the same false alarms, whereas a parked gate's decision is owed to each home's own supervisor.

[`architecture.md`](architecture.md) owns the wait-evidence contract; `bin/fm-watch.sh`'s `wedge_wait_evidence` and `bin/fm-classify-lib.sh`'s `crew_pipeline_activity_is_recent` own the exact derivation.
