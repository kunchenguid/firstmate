...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; other stale panes surface immediately unless they declare the pause verb or are parked at a current run-step-backed validation decision
FM_VERIFIED_WAIT_RECHECK_SECS=      # seconds between liveness rechecks for a current run-step-backed validation decision; unset defaults to FM_STALE_ESCALATE_SECS
