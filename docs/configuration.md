...
FM_RATELIMIT_REGEX='(Claude[[:space:]]+)?((AI[[:space:]]+)?usage[[:space:]-]+limit|rate[[:space:]-]+limit|quota)[^[:cntrl:]]*(reached|exceeded|reset|try again)|limit[[:space:]-]+reached'   # Claude quota footer signatures, matched only against the rendered footer tail, never the full transcript
FM_OVERLOAD_REGEX='API Error: 529'   # overload footer signature; a match is parked with the shorter FM_OVERLOAD_FALLBACK reset, since overloads are transient
