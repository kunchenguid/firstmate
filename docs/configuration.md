...
Registration records an arm-time boundary, then baselines older items through bounded rotating scans while keeping later forge activity announceable, so a new PR replays no creation noise without losing activity that arrives during a multi-poll baseline. Registration with `--backfill`, or the guarded resumable `bin/fm-procevent-pr-follow.sh backfill` sweep over PR metadata, scans threads through an independent cursor, aggregates complete thread history across pages, and surfaces each currently unanswered inline review thread exactly once.
...
At the defaults, the largest burst is 31 GET requests while an unanswered-thread backfill scan is active, which is at most 372 forge requests per hour for the whole home; ordinary GitHub polling is at most 21 requests per burst. With `T=max(FM_PR_FOLLOW_ROTATION_SLOT, 31 x FM_PR_FOLLOW_FETCH_TIMEOUT)`, an open PR starts within roster-size x T and a settled PR within roster-size x T x `FM_PR_FOLLOW_SETTLED_EVERY`; registration remains uncapped because retirement is explicit.
