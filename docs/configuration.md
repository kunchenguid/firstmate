...
FM_LOOP_PARKED_SECS=900            # positive integer interval for the parked-gate looping proxy; zero, leading-zero, or invalid values use 900; docs/architecture.md owns the criterion, distinct-reads false positive, and inspection-only handling
FM_LOOP_PROBE_SECS=60              # minimum seconds between gate-state reads within a looping episode; zero checks each poll, invalid values use 60; docs/architecture.md owns episode resets
...
FM_WORKTREE_WRITE_PRUNE='.git node_modules .venv venv __pycache__ .mypy_cache .pytest_cache .ruff_cache .tox target dist build .next .cache vendor'   # directory names the shared wedge and looping task-worktree write probe skips; the default keeps .git out so a supervisor's own read-only git command can never look like crew progress; set it to the empty string to prune nothing, which widens the probe to the whole depth-bounded tree rather than disabling it
FM_WORKTREE_WRITE_MAXDEPTH=6       # depth that same probe walks below the recorded worktree; it runs only when a wedge or looping escalation would otherwise fire, never on every poll; no probe knob applies to a secondmate, whose recorded worktree is a provisioned home the probe skips entirely
