...
devin is likewise verified for crewmate and scout launches ONLY, refused for a secondmate for the same reason - no primary supervision protocol; [`docs/verification/devin.md`](verification/devin.md) owns that evidence, including the spawn-time worktree trust pre-registration through `bin/fm-devin-trust.sh`, the firstmate-owned per-task `--config` hook wiring, and tmux pane liveness (Herdr carries no native devin recognition).
...
Typed resolution additively recognizes `gemini` and `devin` because AGENTS.md section 4 verifies them for crewmate and scout dispatch.
