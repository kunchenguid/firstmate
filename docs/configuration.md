...
FM_TEARDOWN_NM_TIMEOUT=10    # seconds allowed per no-mistakes query or abort when fm-teardown.sh or fm-control.sh relaunch concludes the task's own parked run
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed directly
FM_TEARDOWN_NM_RUNS_LIMIT=200  # recent no-mistakes run rows scanned to prove an unresolved-head parked run belongs to the task being torn down or relaunched
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage
FM_WTPROC_CREW_STATE_TIMEOUT=20   # seconds bounding the per-task corroborating current-state read that fm-orphan-reap.sh needs before it calls a local copy's worker gone; fm-session-start.sh's leftover-process section lowers it to 5 so two slow tasks cannot spend the whole digest budget
FM_WTPROC_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for that corroborating reader
FM_WTPROC_GRACE=3       # seconds between the TERM and the KILL when fm-worktree-proc-lib.sh stops the processes left in a task's local copy
FM_WTPROC_KILL_SETTLE=1   # seconds it then waits before confirming that KILL
