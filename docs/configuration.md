...
When completion carries a confirmed GitLab merge-request URL, Firstmate preserves the link in the backlog body instead of passing it through `tasks-axi`'s GitHub-only `--pr` flag; ordinary teardown and pending-close replay share that conversion and retire the pending-close record only after the transition lands.
