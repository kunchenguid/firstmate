...
A response too large to be a quiet absence is streamed through undecided and the runner's own output bound applies to it, so only those two small envelopes are ever held back.
...
For Lavish that verdict covers exactly one shape - a session the adapter classifies `ended` that carries no queued content block at all, which is a review surface closed with nothing said - and the listener consults that same command rather than restating it, because a quiet absence is waited out before the runner ever sees it (see the board-file contract above).
