...
A tmux task records `tmux_socket=`, the tmux server its window lives on, and every reader addresses that server rather than the one it happens to be running in; a meta written before the field existed has none and resolves to the ambient fleet socket, which is what those readers used before (see [`docs/tmux-backend.md`](tmux-backend.md) "Which tmux server the fleet uses").
...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses its queue-first present daemon with terminal/checkpoint fallbacks, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
No selection step consults these readings, and the profile-array procedure never reads this file at all, so a quota number can never move work onto a different billing identity: only the captain's word or an explicit config change switches identities.
...
Both `use` and the optional top-level `default` accept either one profile object or a non-empty ordered array of profile objects.
The single-object form stays fully backward-compatible, and every profile needs `harness`.
Profile `model`, `effort`, and `launch` fields and rule `why` are optional.
...
For every other array, when quota data cannot be read or no candidate can be established, the procedure reports what it could not establish instead of selecting blind.
...
Every other profile array is an implicit quota-aware choice resolved through `quota-array-dispatch`.
...
Because no-mistakes development builds may identify themselves only by commit hash, bootstrap accepts those builds only after bounded read-only help probes confirm both `watch --pr` and `axi run --intent`; an old SemVer or failed capability probe is reported as installed but incompatible, while `MISSING` is reserved for an absent command.
...
When the fleet uses the Codebase (`code.byted.org`) provider - detected from the firstmate home's own origin or any registered project clone - bootstrap also requires `bytedcli`, so an internal home learns it is missing at session start rather than at its first Codebase MR; a GitHub-only home never sees this requirement.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, `backend`, `harness-overrides.json`, `herdr-presentation-spaces`, `startup-memory-budget`, `trace-context`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread send failures.
