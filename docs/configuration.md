...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb; a provably-working pane whose task carries a still-open captain decision re-surfaces on the longer pause cadence instead of escalating
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before an idle declared external wait or a dead-agent captain hold re-surfaces for a recheck in the watcher or away-mode daemon; the watcher alone also applies it to a provably-working crew parked on a still-open captain decision, a case the away-mode daemon has no notion of
