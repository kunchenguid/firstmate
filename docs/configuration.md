...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence or their terminal status line is already recorded as delivered, while a live idle declared wait still surfaces once before that cadence bounds repeats
