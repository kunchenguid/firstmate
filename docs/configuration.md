...
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, and `cmux` for ship and scout tasks; `backend=cmux` still refuses `--secondmate` until secondmate launch semantics are designed for it, while `backend=orca` now supports `--secondmate` ([`orca-backend.md`](orca-backend.md#recovery)).
