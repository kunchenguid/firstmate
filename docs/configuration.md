...
agy (Antigravity CLI) is verified for crewmate and scout launches ONLY, on the same boundary and for the same reason as muse, and `fm-spawn.sh` refuses it for a secondmate; [`docs/verification/agy.md`](verification/agy.md) owns that evidence.
agy also needs `jq` before spawning, because its turn-end hook reads a JSON payload whose `fullyIdle` field is what separates a finished turn from one still waiting on backgrounded work, and the spawn refuses rather than installing a hook that could never fire.
agy is verified on Herdr only, has no worker-state source and therefore reports its busy state as unknown, and carries no `spendPriority`, so it is selectable only as an explicit per-task choice rather than inside a quota-balanced profile array.
That restriction is enforced rather than advisory: a crewmate or scout spawn that resolves agy from `config/crew-harness` refuses and asks for an explicit `--harness agy`, and a `config/crew-dispatch.json` profile naming agy is reported by bootstrap as `unverified harness: agy` even though the adapter itself is verified.
...
For agy crews, `fm-spawn.sh` installs a firstmate-owned plugin in the operator's own home under `~/.gemini/config/plugins/fm-turn-end/` and drops a per-task `.fm-agy-turnend` pointer in the worktree; that root is fixed and has no environment override, so spawn and teardown resolve the same registry by construction.
Teardown retires only that task's registry token and pointer, so the plugin stays installed and enabled for every agy session on the host until it is removed by hand.
An agy spawn also answers agy's workspace-trust dialog with one Enter when that dialog appears, which makes agy add the task worktree to `trustedWorkspaces` in `~/.gemini/antigravity-cli/settings.json`; teardown deliberately does not remove those entries, because they are the operator's own trust store, so they accumulate one per worktree path that has ever hosted an agy task and are pruned by hand.
Because treehouse recycles pooled worktree paths, only the first agy task in a given path sees that dialog; a later spawn into the same path finds it already trusted, draws no dialog, and waits about a minute for the poll to end before the spawn completes, with the agent already working on its brief throughout.
That wait is a deliberate trade recorded in [`docs/verification/agy.md`](verification/agy.md#workspace-trust).
