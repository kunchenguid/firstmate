...
`data/` holds durable private fleet records such as the project and secondmate registries, per-project external agent contracts under `data/project-contracts/`, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
