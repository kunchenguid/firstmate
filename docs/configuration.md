...
The watcher's external reconciliation, secondmate scan, backend and Herdr event probes, authenticated checks, shared current-state triage, remote observation, and recovery-delivery calls use deadlines owned by `bin/fm-watch.sh`, `bin/backends/herdr.sh`, `bin/fm-classify-lib.sh`, and `bin/fm-pending-reply-lib.sh`; only the pre-existing authenticated-check deadline is operator configurable.
