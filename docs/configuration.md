...
At the same time the poll records a durable per-request reply context at `state/x-context/<request_id>.json` (`{request_id, platform, reply_max_chars}`) from the same authoritative relay payload, best-effort and keyed by `request_id` so concurrent requests never overwrite each other; it survives the inbox cleanup that follows the acknowledgement, so a delayed follow-up can recover the original platform and split budget even with no task link.
...
Platform/budget resolution is layered and independent of the task link (`bin/fm-x-lib.sh:fmx_resolve_reply_context` owns the order): a per-axis `FMX_REPLY_PLATFORM` / `FMX_REPLY_MAX_CHARS` override (how `bin/fm-x-followup.sh` passes a recorded link's context) wins, otherwise the durable per-request registry is consulted first, then the still-present inbox payload, then - for a follow-up posted live by request_id - an authoritative relay lookup via `POST /connector/request-context` (`{request_id}` in, `{platform, reply_max_chars}` back).
...
In that case the link is still recorded but `bin/fm-x-link.sh` prints a loud warning; and when a follow-up's platform/budget cannot be authoritatively resolved from any source, `bin/fm-x-reply.sh` refuses (fail-safe exit 8) to split it rather than silently defaulting to the X 280-char budget and wrongly threading a possibly-Discord reply - firstmate holds and retries it once the platform is recoverable.
...
It treats `fm-x-reply.sh`'s fail-safe refusal (exit 8: platform/budget unresolved and the reply would split) differently: that is a retryable hold, so the link is KEPT and the follow-up is retried once the platform can be recovered, never posted at the wrong split.
