...
It refuses a missing required deliverable the same way: the registration records the obligation's required keys for a direct emit, and `brief` repeats them as `--require-deliverable` flags for a staged one, so an incomplete result never travels either.
