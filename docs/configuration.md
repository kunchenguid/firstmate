...
`bin/fm-lint.sh` runs ShellCheck one file at a time and bounds each invocation by a wall-clock timeout (`FM_LINT_FILE_TIMEOUT`, a positive integer number of seconds, default `120`) and a memory ceiling (`FM_LINT_FILE_MEM_KB`, a positive integer number of KiB).
The memory ceiling runs ShellCheck inside its own `systemd-run --user --scope` cgroup on a host with a live `systemd --user` session, and goes unenforced (only the timeout bounds ShellCheck) on a host without one, such as some CI runners or non-Linux; `bin/fm-lint.sh` reports that gap once per process rather than claiming an enforcement that is not happening.
An earlier version of this ceiling used `ulimit -v`, which caps virtual address space rather than resident memory; ShellCheck's GHC runtime reserves a very large virtual region up front regardless of a file's real memory use, so that version killed nearly every file, including tiny ones, with "out of memory" (caught by CI on this same change, 2026-09-06).
`--external-sources` (which this script always passes in full CI mode) makes ShellCheck recursively analyze every sourced file, so a heavily-cross-sourcing hub script legitimately peaks far higher than its own size suggests: measured directly (`systemd-run` + `/usr/bin/time -v`, 2026-09-06) against every file a flat 1 GiB ceiling had false-failed, 23 real files peaked between 1.0 and 3.9 GiB and finished in under a minute each, and 4 more never stabilized even under a 4 GiB / 180 s probe.
`FM_LINT_FILE_MEM_KB`'s default is therefore not one flat number: it targets `6291456` KiB (6 GiB, clearing the largest measured real peak with headroom) but is capped by 70% of this host's own `MemTotal` divided by the concurrent shard count (`FM_LINT_JOBS`, default 2), so the ceiling can never authorize more concurrent memory across shards than a small CI runner actually has, which is exactly how a false-positive on one file turned into collateral damage on someone else's build in the first place. An explicit `FM_LINT_FILE_MEM_KB` always overrides this computed default untouched. `fm_lint_default_file_mem_kb` in `bin/fm-lint.sh` owns the exact formula.
A file that hits either bound retries once without `--external-sources` - the same file, a purely local analysis, no sourced-file traversal - before being reported as a failure; only a fallback that also hits a bound is a real failure.
[`docs/fm-lint-external-sources-fallback.md`](fm-lint-external-sources-fallback.md) tracks which tracked files are currently known to need that fallback and why, so a newly pathological file is visible as a diff to that list rather than blending into routine lint output; `bin/fm-lint.sh`'s `fm_lint_run_one_file` owns the exact fallback mechanism.
A file whose fallback also fails is reported by name as a lint failure and the run moves on to the next file instead of stalling or growing without limit; a single pathological file (extended-analysis dataflow blowing up on a large or deeply-nested script, or one that never converges even without recursing into its sources) can no longer starve the host the way it did on 2026-09-05, when one `fm-lint` run sat 21 minutes in disk sleep at 2.7 GB resident and got an unrelated production build killed by the memory guard four times.
Both variables are read fresh on every invocation and rejected (exit 2) if not a positive integer.
`bin/fm-lint.sh`'s own header comment owns the exact per-file mechanism (a SIGALRM deadline and its interaction with the ambient-process-group signal cleanup `test_worker_trees_stop_on_signal` proves, plus why the memory ceiling is a systemd scope rather than `ulimit -v` or ShellCheck's own runtime option).
