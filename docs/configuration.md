...
Every Astra crewmate, and every effort of an exact profile that sets this field, refuses before a worker endpoint, worktree, or backlog record changes unless Firstmate supplies a current version-4 selection receipt.
...
For a named Codex home, the snapshot is `quota-axi --provider codex --full --json` output so Firstmate can compare only the account object's hash with a fresh read from that home without recording account bytes.
