...
FM_SPAWN_WORKTREE_TIMEOUT=60   # seconds a spawn waits for treehouse get to enter the task worktree; raise it when a project's first-ever worktree outlasts the default. Unset or blank means 60; the minimum accepted value is 2 because the detection loop needs two polls to confirm the pane settled, and any other value refuses the spawn before a window is created
