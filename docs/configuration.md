...
claude, codex, opencode, pi, pi-signed, grok, kimi, and cursor are empirically verified for crewmate and secondmate launches, while agy, cline, copilot, and muse are verified for crewmate and scout launches only and a `--secondmate` spawn refuses them; [README requirements](../README.md#requirements) own the set supported for the primary session.
...
[`docs/verification/muse.md`](verification/muse.md) owns the evidence behind muse's verification scope.
muse also needs a worker-reachable credential before spawning, and the portable fleet path is the `<config>/muse/auth.json` credential stored by `muse login`, because a caller-only `META_API_KEY` does not cross a long-lived backend daemon.
New harnesses get verified through a supervised trial task before joining the set.
The verified adapter knowledge - each harness's busy-state source, interrupt and exit mechanics, skill-invocation syntax, and per-harness quirks - lives in [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md).
...
FM_BACKEND_HERDR_IDLE_RE=  # herdr-only: empty-composer placeholder regex after shared ghost extraction plus border and prompt stripping; unset uses the shared fleet-wide FM_COMPOSER_IDLE_RE_DEFAULT (bin/fm-composer-lib.sh)
FM_BACKEND_HERDR_BARE_PROMPT_RE=  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row; unset uses Herdr's alternation default ('^(❯|›|→)': Claude's ❯, Codex's ›, cursor-agent's →) instead of a bracket expression so a C-locale byte-decomposed match can never misfire on an unrelated multibyte glyph; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text reads empty through shared fm_composer_strip_ghost (docs/herdr-backend.md "Composer and injection safety")
...
FM_BACKEND_ORCA_IDLE_RE=  # orca-only: empty-composer placeholder regex after border/prompt stripping; unset uses the shared FM_COMPOSER_IDLE_RE_DEFAULT (bin/fm-composer-lib.sh)
FM_BACKEND_ORCA_BARE_PROMPT_RE=  # orca-only: agent glyphs promoted as an unbordered composer row; unset uses the shared FM_COMPOSER_BARE_PROMPT_RE_DEFAULT ('^[❯›→]')
...
FM_BACKEND_CMUX_IDLE_RE=  # cmux-only: empty-composer placeholder regex after border/prompt stripping; unset uses the shared FM_COMPOSER_IDLE_RE_DEFAULT (bin/fm-composer-lib.sh)
FM_BACKEND_CMUX_BARE_PROMPT_RE=  # cmux-only: agent glyphs promoted as an unbordered composer row; unset uses the shared FM_COMPOSER_BARE_PROMPT_RE_DEFAULT ('^[❯›→]')
...
FM_COMPOSER_IDLE_RE=    # optional empty-composer regex, applied after ghost and border stripping; unset uses the shared fleet-wide FM_COMPOSER_IDLE_RE_DEFAULT (bin/fm-composer-lib.sh)
