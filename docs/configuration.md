...
Target detection uses `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then `"${HERDR_SESSION:-default}:${HERDR_PANE_ID}"` under herdr, then the legacy `firstmate` session fallback (tmux's active window in that session, so a non-default `base-index` still resolves) with a warning.
