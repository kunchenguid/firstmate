...
`tmux` is the verified reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr`, `zellij`, `orca`, `cmux`, and `thurbox` are experimental spawn backends (see [`docs/herdr-backend.md`](herdr-backend.md), [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), [`docs/cmux-backend.md`](cmux-backend.md), and [`docs/thurbox-backend.md`](thurbox-backend.md)).
Treehouse remains the worktree provider for tmux, herdr, zellij, cmux, and thurbox, since those are session providers only; Orca provides both the task worktree and terminal endpoint.
thurbox can own worktrees natively but is deliberately not used that way - see [`docs/thurbox-backend.md`](thurbox-backend.md) "Why not the worktree provider".
A thurbox home also needs one thurbox `agents.toml` entry that launches an interactive shell, named by `config/thurbox-agent` (default `shell`), because firstmate runs Treehouse and launches the harness itself.
`config/thurbox-agent` is inherited into secondmate homes under the same primary-authoritative contract as `config/backend`, so a home that inherits `backend=thurbox` also inherits the agent entry that backend needs.
New spawns choose the backend in this order: an explicit `--backend` flag that current authority for that exact task alone has authorized (a present captain instruction or the task's own accepted brief; never later-task precedent by analogy), then `FM_BACKEND`, then the first non-empty line of local gitignored `config/backend`, then runtime auto-detection from `THURBOX_SESSION`, `$TMUX`, `HERDR_ENV=1`, or cmux runtime signals, then default `tmux`.
...
A thurbox task additionally records `thurbox_session_id=` and `thurbox_pane_id=`, where the session UUID is the durable identity and the pane id is only a cache the adapter re-resolves from it (see [`docs/thurbox-backend.md`](thurbox-backend.md#the-identity-model-the-uuid-is-durable-the-pane-id-is-a-cache)).
...
thurbox has no session layer to stand up either - its database is the container, one session per task, with no per-home split - and session names are scoped by the same home tag as zellij tabs and cmux workspaces; no thurbox test may touch a real thurbox at all, per [`docs/thurbox-backend.md`](thurbox-backend.md#isolation-and-its-limit).
...
Selecting any other supervisor backend, including `zellij`, `orca`, `cmux`, or `thurbox`, refuses at daemon startup instead of trying tmux injection primitives against a non-tmux pane.
...
For the thurbox backend, `FM_CONFIG_OVERRIDE` overrides where `config/thurbox-agent` is read from, while `FM_HOME` and the same `FM_ROOT` hash form the home tag embedded in session names; there is no per-home container split.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, `cmux`, or `thurbox-cli`), `jq` for the JSON-emitting experimental adapters (`herdr`, `zellij`, `cmux`, `thurbox`) whose spawn and liveness paths parse the backend's JSON output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`, `thurbox`).
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, `backend`, `herdr-presentation-spaces`, `thurbox-agent`, `startup-memory-budget`, `trace-context`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread send failures.
...
FM_ROOT_OVERRIDE=        # override firstmate repo root, tangle-guard target, and zellij/cmux/thurbox home-title hash; also legacy whole-root override when FM_HOME is unset
...
FM_BACKEND=             # optional runtime backend override for new spawns; tmux/herdr/zellij/orca/cmux/thurbox support ship/scout spawns, codex-app is not accepted
...
FM_COMPOSER_CAPTURE_LINES=20   # fleet-wide bound for tail-capture composer reads, so stale scrollback banners stay out of the candidate set; it does not apply to the cursor-anchored backends, which must supply the visible pane itself (tmux's bounded capture, thurbox's `session capture --lines 0`) or the cursor row would address the wrong line
...
FM_COMPOSER_GHOST_LUMA_MAX=128   # fleet-wide: max perceived luminance (0.299R+0.587G+0.114B, 0-255) for a TRUECOLOR foreground to count as de-emphasised ghost/placeholder text and be stripped; dim/faint (SGR 2) is stripped regardless. Assumes a dark terminal theme (bin/fm-composer-lib.sh's fm_composer_strip_ghost, used by styled tmux, herdr, Zellij, and thurbox reads)
