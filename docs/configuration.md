...
`FM_HOME` determines Herdr's home label: the primary home uses `firstmate`, and a secondmate home marked by `.fm-secondmate-home` uses `2ndmate-<secondmate-id>`, unless the local, gitignored, non-inherited `config/herdr-workspace-label` holds a non-empty (trimmed) override, which then wins for that home except when the secondmate marker is present (`bin/backends/herdr.sh`'s `fm_backend_herdr_workspace_label`); the override applies only to workspaces created from that point on, not to a home's already-existing workspace.
...
FM_WATCH_REARM_RETRY_LIMIT=5   # Pi/omp/OpenCode adapter continuity retries before surfacing restoration failure; in Pi and omp the bound counts consecutive unhealthy cycles (docs/watcher-continuity.md "Ownership")
