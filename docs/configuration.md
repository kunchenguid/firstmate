...
FM_COMPOSER_KIMI_STATUS_RE=   # optional override for the Kimi status-footer row that a cursorless bordered-box read (herdr, zellij, orca, cmux) tolerates directly below the box (bin/fm-composer-lib.sh); the default matches only the captured "Never Ask" permission-mode row, and any other text there still marks the box stale
