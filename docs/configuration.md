...
FM_SECONDMATE_REMOTE_PROBE_TIMEOUT=60   # seconds bounding one remote secondmate state probe, so a host that stopped serving work cannot stall the poll; a probe that hits it counts as not completed; zero or invalid values use 60
FM_SECONDMATE_UNREACHABLE_PROBES=3   # consecutive remote state probes that did not complete before the watcher wakes once that the mate has stopped accepting work; any completed probe starts a new episode; zero or invalid values use 3
