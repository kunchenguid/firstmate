...
Those verified templates are executed through `bash -c` inside the runtime pane, so the captain's interactive pane shell can be fish, zsh, or bash without parsing the template's bash substitutions itself.
Raw launch commands used while verifying a new adapter are passed through unchanged.
