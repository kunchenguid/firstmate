...
`bin/fm-procevent-lavish.sh` is the first built-in adapter and wraps only the currently published `lavish-axi poll` interface; [`lavish-review-lifecycle`](../.agents/skills/lavish-review-lifecycle/SKILL.md) owns the direct-worker versus registered-listener transition and same-tab revision procedure.
