...
`chrome-devtools-axi` carries one extra contract beyond being installed: every crewmate and scout is launched with its own non-default `CHROME_DEVTOOLS_AXI_SESSION`, and with a task-private launcher first on its `PATH` that forces that session onto every browser call resolved through `PATH` and records that the task now has a bridge to reclaim.
Only the bare invocation, `setup`, and a lone `-h`/`--help`/`-v`/`-V`/`--version` are treated as unable to open one; any of those flags in front of a real command still records the call.
A `stop` the tool reports as successful clears that record again, so a worker that shuts its own bridge down costs teardown no browser call at all, while a stop the tool reports as failed leaves the task eligible for reclamation.
That clearing is declined for as long as any of the task's own browser calls is still running, because such a call has already recorded its bridge while the bridge itself is still coming up, and a stop issued in that window covers nothing it opened; the task simply stays eligible for reclamation until the call returns.
A call that reaches the tool some other way - an absolute install path, a harness that resolved the binary once at startup, an MCP or hook entry naming the install path - never passes through the launcher: the pane-level `CHROME_DEVTOOLS_AXI_SESSION` export still isolates whatever bridge it opens, but nothing records the start, so teardown will not reclaim it.
Teardown reclaims that task's bridge by asking the resolved `chrome-devtools-axi` about that exact session name, but only for a task whose launcher recorded such a call - the session name alone isolates the bridge without reclaiming it.
When spawn reports that it could not create the launcher, the task keeps its isolated session and no bridge it opens will be stopped automatically.
A resolved `chrome-devtools-axi` that is a wrapper or dispatcher which drops `CHROME_DEVTOOLS_AXI_SESSION` and routes every call onto one shared bridge cannot be used to reclaim anything: firstmate proves before every stop that the tool answers separately for a session name nothing ever started, and when that proof fails it makes no stop call at all, because the only bridge such a wrapper would stop is the captain's own browser.
On such a host task-scoped bridge reclamation is inert - teardown says so on stderr whenever a task that started a bridge gets a readable answer for that session other than gone, and a browser bridge that outlives its task must be stopped by hand - until the wrapper passes `CHROME_DEVTOOLS_AXI_SESSION` through to the real tool.
That stderr line reports what was observed - the probe was not reported gone - and names a pass-through wrapper as the usual cause rather than the certain one, because a tool that words its unknown-session answer differently, or refuses the name outright, fails the same proof for a different reason.
While the one shared bridge such a wrapper pins every call to happens to be idle, the task's session reads gone instead, so no stop is needed and no line is printed.
Every session name firstmate mints, the task's own and the probe, is kept within 64 characters of `[A-Za-z0-9._-]` starting alphanumeric, which is the longest a dispatcher in this fleet accepts; a wrapper with a tighter limit will fail the proof and report inert reclamation until it is widened.
...
FM_CHROME_BRIDGE_TIMEOUT=20   # 1..120 seconds allowed for one chrome-devtools-axi call during task teardown; a non-numeric or below-1 value uses 20, and anything above 120 is cut to 120. Only a task whose launcher recorded a bridge start makes any browser call at all, so an unused task never waits on the tool
