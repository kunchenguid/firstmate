...
The launch half ignores caller-supplied receipt paths and launch-input bytes, resnapshots the task inputs, validates the selected and emitted route, derives the deterministic generation paths, and verifies the exact published receipt, brief, and latest consumed-ledger row through the no-follow boundary.
