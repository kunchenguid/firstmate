...
prime-agent runs every session in a detached daemon worker that outlives both the pane and an explicit quit, so task cleanup retires the worker bound to that task directory first ([`bin/fm-prime-agent-lib.sh`](../bin/fm-prime-agent-lib.sh)); without that a task would keep a live session-lock holder and land read-only.
The worker's liveness and retirement details remain owned by [`bin/fm-prime-agent-lib.sh`](../bin/fm-prime-agent-lib.sh).
