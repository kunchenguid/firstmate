...
It refuses a missing required deliverable the same way: the registration records the obligation's required keys for a direct emit, and `brief` repeats them as `--require-deliverable` flags for a staged one, so an incomplete result never travels either. Only a `failed` or `superseded` outcome is exempt, because those report that the promise could not be kept as promised; no other outcome type excuses a missing required value.
