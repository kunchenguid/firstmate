...
The receipt gate accepts a direct Slack permalink only in the canonical shape `https://<workspace>.slack.com/archives/<conversation-id>/p<message-id>`, where the workspace is a DNS-safe Slack subdomain, the conversation ID starts with `C`, `D`, or `G` and has at least eight more uppercase letters or digits, and the message ID is exactly 16 digits.
