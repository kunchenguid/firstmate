...
`paseo` is an experimental backend name that is REGISTERED BUT NOT YET USABLE: its selection, detection, refusals, and endpoint-record validation exist, but its lifecycle adapter does not, so a paseo spawn refuses at endpoint creation instead of creating anything.
...
`fm-spawn.sh` creates ship and scout endpoints on `tmux`, `herdr`, `zellij`, `orca`, and `cmux`; `backend=orca`, `backend=cmux`, and `backend=paseo` all refuse `--secondmate` until secondmate launch semantics are designed for each.
`backend=paseo` passes backend selection and every refusal ahead of it, then refuses to create any endpoint while its lifecycle adapter is absent, so a home that auto-detects Paseo should pin `config/backend` until that adapter lands.
