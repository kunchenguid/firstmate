...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader behind no-verb signal triage and task-bound stale classification
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before the away-mode daemon rechecks a stale candidate; the normal watcher uses task-bound state transitions instead, while secondmates use this as their pause-state recheck interval
FM_BUSY_TURN_MAX_SECS=3600         # outer bound on every disconfirmer that proves only rendering or readability (a busy pane footer, a harness-turn verdict, an unreadable unknown state): maximum age of this task's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the window surfaces once as a turn-bound candidate; inspection-only, never an automatic interrupt or restart
FM_CREW_STATE_READ_TIMEOUT=15      # hard cap in seconds on one bin/fm-crew-state.sh read from the classifier, so a hung backend cannot stretch a supervision cycle; a capped read classifies as unknown, never as a healthy disconfirmer
FM_PAUSE_RESURFACE_SECS=3600       # seconds an idle declared external wait stays quiet before exactly one awaiting-external recheck; applies to ordinary normal-mode workers (through the task-bound candidate dedup), secondmates, and away-mode supervision alike, anchored on the task's status-log mtime so pane churn cannot reset the window
