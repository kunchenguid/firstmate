...
FM_CREW_STATE_NM_TIMEOUT=10   # Must be a positive integer from 1 through 86400 seconds for each no-mistakes query in fm-crew-state.sh; an explicitly invalid value is refused with exit 2.
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates, unless its pipeline provably advanced during that window; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
