...
A result `consume` refuses is quarantined with a reason naming the specific deliverable, outcome, or missing key where one is identifiable, and the same poll wakes the owning home once per refusal with a `public-followup rejected <event-id> ...` line carrying that reason.
