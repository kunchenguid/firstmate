...
The pull-request follow-through adapter (`bin/fm-procevent-pr-follow.sh`) turns this channel into durable PR lifecycle tracking. One home-level `pr-follow-scheduler-<hash12>` process polls every retained PR through `gh` or `glab` and announces new issue comments, inline review comments and replies, review submissions and state changes, head replacement, check-run or pipeline transitions, and merge, close, or reopen for the PR's whole retained lifetime.
Each PR keeps a canonical tracking id, `prf-gh-<hash12>` or `prf-gl-<hash12>`, derived by SHA-256 from its validated provider-tagged identity. Its private cursor and per-item state records live under `state/pr-follow/`, while only the scheduler is registered under `state/procevent/`; tracking therefore survives task cleanup, merge, restart, duplicate or reordered pages, and bounded state-map eviction. Merge never retires anything, and the explicit off switch is `bin/fm-procevent-pr-follow.sh retire <tracking-id> [--force]`.
Registration records an arm-time boundary and, when the registering caller already verified the published head, that head, then baselines older items through bounded rotating scans while keeping later forge activity announceable, so a new PR replays no creation noise, loses no activity arriving during a multi-poll baseline, and never reports a head replacement the head never made. Registration with `--backfill`, or the guarded resumable `bin/fm-procevent-pr-follow.sh backfill` sweep over PR metadata, scans threads through an independent cursor, aggregates complete thread history across pages, and surfaces each currently unanswered inline review thread exactly once.
...
One captured document announces at most `FM_PR_FOLLOW_MAX_EVENTS` events, plus exempt head and PR-state lines. Durable page cursors revisit every page, while individual private records retain every observed comment identity and every review, check, pipeline, approval, backfill, and GitLab thread state; the bounded cursor maps are compatibility caches, not loss-prone ownership of transition state. If a document fills, unapplied items remain discoverable on the next rotation and only surviving events advance their records.
GitHub comments, inline replies, reviews, and check runs and GitLab discussions and pipelines rotate newest-first through independent durable page cursors. Page growth does not reset an older traversal; the current sweep finishes before incorporating the new tail, so continuous growth cannot starve pages already in progress. All forge access is GET-only.
...
A canonical captured document whose immutable `pr-follow` sidecar is present but whose complete grammar fails validation is an adapter defect rather than tampering. After `FM_PR_FOLLOW_APPLY_BOUND` such refusals, the failing capture is acknowledged, polling pauses, and one fixed monitoring-loss document surfaces the quarantine; a caller-selected or foreign file cannot assert that provenance or quarantine a PR.
Aggregate polling is bounded by the single scheduler, which durably records the served wall slot and last roster identity before each attempt. A restart cannot repeat a slot, changing `FM_PR_FOLLOW_ROTATION_SLOT` rescales that record into the new cadence rather than dropping it - so a shrunk slot cannot replay a window already served - elapsed slots never skip roster identities, and merged or closed PRs are polled every `FM_PR_FOLLOW_SETTLED_EVERY`-th roster round.
At the defaults, the largest burst is 31 GET requests while an unanswered-thread backfill scan is active, which is at most 372 forge requests per hour for the whole home; ordinary GitHub polling is at most 25 requests per burst. With `T=max(FM_PR_FOLLOW_ROTATION_SLOT, 31 x FM_PR_FOLLOW_FETCH_TIMEOUT)`, an open PR starts within roster-size x T and a settled PR within roster-size x T x `FM_PR_FOLLOW_SETTLED_EVERY`; registration remains uncapped because retirement is explicit.
Captured wakes use the home scheduler id printed by `bin/fm-procevent-pr-follow.sh scheduler-id`; the result's validated `target:` field names the PR tracking id that `handle` applies, and retirement retains applied receipts until their queued delivery can be acknowledged safely.
...
FM_PR_FOLLOW_MAX_EVENTS=60            # per-document event bound for follow-through results (1-198)
FM_PR_FOLLOW_BACKFILL_SCAN_CAP=500    # metadata records visited per resumable migration pass
FM_PR_FOLLOW_MAP_LIMIT=40             # bounded review/check and thread-cache map size per follow-through cursor (1-2048)
