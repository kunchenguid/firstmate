...
`tmux` is the default reference backend (see [`docs/tmux-backend.md`](tmux-backend.md)); `herdr` has its own required CI lane and the largest backend-specific test suite in this repo (see [`docs/herdr-backend.md`](herdr-backend.md)); `zellij`, `orca`, and `cmux` remain experimental spawn backends with no CI lane (see [`docs/zellij-backend.md`](zellij-backend.md), [`docs/orca-backend.md`](orca-backend.md), and [`docs/cmux-backend.md`](cmux-backend.md)).
