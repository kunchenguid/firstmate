...
FM_COMPOSER_IDLE_RE=    # optional fleet-wide override of the empty-composer placeholder set, applied after ghost and border stripping on every backend (bin/fm-composer-lib.sh)
FM_COMPOSER_CAPTURE_LINES=20   # fleet-wide: bounded tail rows every backend captures for a composer read; the composer is bottom-anchored, so a small window keeps stale scrollback banners out of the candidate set
