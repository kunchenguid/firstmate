...
Those paths are private to the home: every command refuses before reading or writing when a state path it can touch is a symlink or not a directory, or when a per-source runner record is a symlink, so a damaged home reports an actionable error - naming the offending path - instead of touching files outside itself.
Each command is held to exactly the leaves it can reach: `retire` and `sweep-home` only ever touch `state`, `state/procevent`, and the machine-wide claim root, so a damaged `state/procevent-inbox` alone never refuses them and never strands the teardown sweep below; every other command additionally requires a private `state/procevent-inbox`.
Supervision counts only the sources a home physically holds, so a source reached through a symlinked `state` or `state/procevent` is never that home's wait; a source that genuinely lives in the home still counts even when a sibling leaf is damaged.
Because a damaged path makes the affected commands refuse and `bin/fm-watch.sh` swallows that refusal, the offending path is named rather than letting the home fall silent: `bin/fm-guard.sh` prints a banner on every run, and `bin/fm-turnend-guard.sh` warns without ever blocking on that alone - see [turnend-guard.md](turnend-guard.md#unserviceable-process-event-state) for exactly which channel each harness surfaces.
Repair it by replacing that path with a private directory owned by the home.
