...
A response that still outgrows that bound never becomes a silent no-op that stays armed: the adapter never claims silence for a response the listener could not hold whole, so the runner announces it once and then retires the source, while a large real payload is still streamed, delivered, and announced unchanged.
Only an artifact that no longer resolves ends a Lavish source: `lavish-axi` refuses to resolve that path before it looks for a session, and the adapter reads that exact refusal as its terminal verdict.
...
For Lavish that verdict covers an artifact that no longer resolves and nothing else, because the board file is the source's lifetime rather than any one session on it.
