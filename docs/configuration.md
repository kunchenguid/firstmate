...
`paseo` is an experimental backend name that is REGISTERED BUT NOT SPAWN-CAPABLE: its name exists, but its lifecycle adapter and its own `docs/paseo-backend.md` are still to come, so every paseo spawn refuses at backend validation instead of creating anything.
`paseo` is also never auto-detected; like Zellij and Orca it is selected only deliberately.
...
A Paseo agent session exports `PASEO_AGENT_ID` into every process it starts, including a tmux server, so a tmux pane spawned by a Paseo-hosted firstmate carries that marker even though Paseo is not the layer executing; that leak is why paseo is never auto-detected, and [`verification/runtime-backends.md`](verification/runtime-backends.md#paseo) owns the dated evidence.
...
`backend=paseo` is refused for every spawn kind by `fm_backend_validate_spawn`, the single boundary all spawn callers share, so selecting it explicitly today refuses rather than creating anything until its lifecycle adapter lands.
