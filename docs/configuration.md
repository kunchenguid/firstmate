...
The session-start secondmate liveness sweep and ordinary-worker recovery use the recovery-grade `fm_backend_agent_state` classifier.
Tmux and Herdr use process evidence; cursorless Zellij, Orca, and cmux accept only a bottom-most bare shell prompt as rendered death evidence and leave every other uncertain shape unverified.
...
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded rechecks of a declared external wait or verified captain-held transfer, between repeated new-hash stale alarms for an ordinary crew task with an open backlog captain call, and between the recovery rechecks of an ordinary crew whose endpoint the backend confidently reports dead, so preserved work or an open captain call waits on this cadence instead of re-alarming as a wedge; a structured until time can make an external-wait recheck occur sooner but cannot extend this bound; this includes a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS, while the away-mode daemon uses the same setting and ages its window against the crew's own latest status line rather than pane busy state; a captain-held transfer is never rechecked while the away-posture record exists
