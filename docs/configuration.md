...
FM_COMPOSER_KIMI_FOOTER=   # set to 1 ONLY by a caller that positively identified the pane as kimi (bin/fm-spawn.sh's kimi gate, for its own pane's readiness/submit/delivery reads); unlocks the below-box kimi footer furniture tolerance below. Unset (default) keeps the strict blank-only below-box staleness walk for every harness and caller, so a screen that merely resembles kimi's footer can never be discarded as furniture unidentified
FM_COMPOSER_KIMI_FOOTER_RE=   # optional regex override for the composer-furniture rows the empty-composer staleness walk steps over below a kimi-identified pane's box; the default recognizes only kimi 0.40.1's verified permission-badge/model-token and right-aligned context-meter shapes, never arbitrary text below a box. If a future kimi footer out-drifts the pattern the pane's gate fails LOUD (never false-confirms) and this knob carries it until the new shape is verified and the default re-anchored (bin/fm-composer-lib.sh)
