...
FM_DEPS_PROBE_TIMEOUT=30   # seconds allowed for each read-only Linux dependency probe
FM_DEPS_LOCK_TIMEOUT=5   # seconds dependency checks and Firstmate checkout advances wait for per-account, per-host ownership
FM_DEPS_IDENTITY_TIMEOUT=3   # seconds allowed for each account or machine identity probe used to resolve that ownership
