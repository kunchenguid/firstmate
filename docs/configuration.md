...
For agy crews, `fm-spawn.sh` installs a firstmate-owned plugin in the operator's own home under `${FM_AGY_CONFIG_HOME:-$HOME/.gemini/config}/plugins/fm-turn-end/` and drops a per-task `.fm-agy-turnend` pointer in the worktree.
