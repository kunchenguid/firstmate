...
antigravity is verified for crewmate and scout launches ONLY and refused for a secondmate for the same reason, and hermes runs the same worker bridge but stays uncredentialed on this host, so it is not verified for dispatch; [`docs/verification/runtime-backends.md`](verification/runtime-backends.md) owns that evidence, and the [antigravity](../.agents/skills/harness-adapters/references/harness/antigravity.md) and [hermes](../.agents/skills/harness-adapters/references/harness/hermes.md) references own each bridge's operating limits.
Both need `python3` and their native CLI (`agy` for antigravity, `hermes` for hermes) on PATH at spawn time, and `fm-spawn.sh` refuses the launch when either is missing.
...
Except for `ultra`, which refuses unsupported profiles under the native-effort contract above, and for `antigravity`, which refuses any level outside `low`, `medium`, and `high` before it builds a launch command, an effort value the chosen harness does not accept is recorded as `effort=` in task meta for traceability but omitted from the launch flags.
