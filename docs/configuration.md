...
Bootstrap applies the same relative `FM_HOME` resolution only when embedding that home in a generated Relay or self-hosted Discord poll shim; other transient consumers retain their existing shell-relative behavior.
...
This section is the single owner of the Relay cadence contract: Relay polls every 30 seconds instead of the default 300 through `config/x-mode.env`, and the session-start supervision operating block includes that cadence instruction when the file exists.
...
FM_CHECK_INTERVAL=300   # seconds between slow checks (authenticated merge polls, custom checks, Relay dispatch, or self-hosted Discord dispatch)
