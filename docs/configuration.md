...
openhands is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because a headless SDK driver has no primary surface at all; its pane runs the firstmate-owned `bin/fm-openhands-worker.py` under a resolved OpenHands venv interpreter, so busy state is the driver's own run-log fold and the turn-end signal is the driver's own marker touch, with no hook layer anywhere.
An openhands spawn refuses before endpoint creation when the venv interpreter is missing, when it cannot import the SDK, or when `config/openhands-llm.env` does not carry a non-empty `LLM_API_KEY` and `LLM_MODEL`, because a headless driver that fails inside the pane reads as a wedged worker; [`docs/verification/openhands.md`](verification/openhands.md) owns that evidence, and [`fm-spawn.sh --help`](../bin/fm-spawn.sh) owns the interpreter resolution order (FM_OPENHANDS_PY, then `config/openhands-python`, then `~/.config/openhands/venv/bin/python`).
...
The opted-in resolver has authoritative single-provider mappings for `claude`, `codex`, `grok`, `kimi`, `cursor`, `agy`, and `muse`; every other verified harness must declare `provider` explicitly, including multi-provider `pi`, `pi-signed`, `omp`, `opencode`, and `openhands`, and unmapped `gemini` and `rovo`.
openhands is multi-provider like omp because the SDK takes any litellm model string, so its provider is read from the model prefix (`fireworks_ai/*` is `fireworks`, `anthropic/*` is `claude`) and an unrecognized prefix is refused rather than guessed.
