...
openhands also needs `LLM_API_KEY` before spawning, taken from the environment or from the optional gitignored `config/openhands-llm.env`, and `LLM_MODEL` from `--model` or the environment (a LiteLLM id such as `fireworks_ai/accounts/fireworks/models/deepseek-v4p1-flash`).
