...
agy is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because its native hook surface is worker-scoped and has no primary supervision protocol; [`docs/verification/agy.md`](verification/agy.md) owns its current marker, hook, composer, control, and CLI-surface evidence.
