...
Past `FM_MAX_DEFER_SECS` the sub-supervisor force-delivers on a not-busy pane (clearing any stale self-injected composer text), and only a genuinely busy pane, an `unknown` composer, or text that will not clear raises a loud, rate-limited alarm.
...
FM_MAX_DEFER_SECS=300              # max buffered escalation age before force-delivery, then wedge alarm if a busy pane or unclearable composer still blocks it; 0 disables
