...
FM_BACKLOG_ROW_TIMEOUT_SECS=10   # seconds bounding each backlog row read (bin/fm-backlog-transition-lib.sh); nonpositive or invalid values fall back to 10; the first bound hit latches the sweep so later reads return immediately, each still naming its own item; the captain-hold gate's closed-task archive read takes the same bound but stops that command by name instead of latching (docs/captain-hold-lifecycle.md)
