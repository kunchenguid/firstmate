...
- Internal identifiers are removed rather than refused. Absolute paths; machine `key=value` fragments for tasks, worktrees, branches, harnesses, delivery modes, or endpoints; `fm/<name>` branch refs; and the verified harness names `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `muse` are stripped from a card, so a genuine failure still reaches the captain without them. A PR URL survives, because a review card without a link is close to useless on a phone.
