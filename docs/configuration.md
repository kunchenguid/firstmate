...
claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, and omp are empirically verified for crewmate and secondmate launches; gemini and agy are verified for crewmate and scout launches only, and [README requirements](../README.md#requirements) own the set supported for the primary session.
...
agy is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because its native hook surface is worker-scoped and has no primary supervision protocol; [`docs/verification/agy.md`](verification/agy.md) owns its marker, hook, composer, control, resume, and model/effort evidence.
