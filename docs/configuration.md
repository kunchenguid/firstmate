...
`fm-spawn.sh` fails a grok launch that stops at Grok's project-folder trust dialog instead of answering it, because that trust lets the task worktree's own project configuration and hooks run with additional authority; [its adapter reference](../.agents/skills/harness-adapters/references/harness/grok.md) owns the detection contract and the one measured case it deliberately leaves open.
