...
An absent, unreadable, or unparseable file means no pin, and the branch then follows main's own current model, applied explicitly.
...
Only when main's own model is unknown, or this home's stored credentials cannot run it in the isolated branch runtime, does a build fall back to passing no override at all, which is the behavior from before this file existed; the branch is never refused over model choice alone, and the command says so plainly instead of reporting a change that did not take effect.
