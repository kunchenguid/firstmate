...
FM_BACKLOG_ROW_TIMEOUT_SECS=10   # seconds bounding each backlog row read, the active-backlog read (bin/fm-backlog-transition-lib.sh) and the done-archive fallback read (bin/fm-tasks-axi-lib.sh, which owns how this value is read) alike; nonpositive or invalid values fall back to 10; the first active-read bound hit latches the sweep so later reads return immediately, each still naming its own item
