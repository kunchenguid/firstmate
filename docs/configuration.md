...
FM_NM_PROGRESS_WINDOW_SECS=180     # seconds a validation-run step measurement must have aged before an identical re-reading counts as a stall rather than one quiet stretch between step-log writes; a shorter window is not judged at all, which delays a real stall by at most this long and never suppresses one
FM_NM_PROGRESS_TIMEOUT=10          # wall-clock seconds for each of the two no-mistakes calls that measure the current step, taken only at the moment a wedge escalation would otherwise fire; a timed-out call reads as no progress evidence, leaving the escalation schedule exactly as it was; a value that is not a positive integer falls back to the default
