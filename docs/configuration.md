...
The optional local, gitignored `config/wedge-defer-parked-gate` presence flag opts this home into default-off parked-gate wait evidence in the watcher's wedge timer.
...
This supervisor-decision preference does not control CI-wait deferral; see the wait-evidence contract below.
With the flag absent the wedge timer skips parked-gate evidence and continues through its other threshold probes.
...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for watcher current-state reads; bin/fm-watch.sh owns call sites and bounds
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane reaches the wedge probes; docs/architecture.md owns escalation, wait deferral, and dead-endpoint reporting
FM_BUSY_TURN_MAX_SECS=3600         # maximum age without a completed turn or explicit native-harness progress (bin/fm-watch.sh owns marker selection), before the shared wedge probes apply; inspection-only, never an automatic interrupt or restart; docs/architecture.md owns deferrals
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded wait rechecks and repeated new-hash stale alarms for an ordinary crew task with an open backlog captain call; docs/architecture.md owns watcher eligibility, timing, and away-posture exceptions; .agents/skills/afk/SKILL.md owns daemon routing
