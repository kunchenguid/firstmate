...
FM_WEDGE_ESCALATE_MAX_SECS=900     # ceiling for the repeat-escalation backoff on an unchanged pane; the first escalation still fires at FM_STALE_ESCALATE_SECS, and this also bounds how long an absorbed pane can go without a fresh crew-state read
FM_WEDGE_UNREADABLE_SURFACE_COUNT=3  # consecutive unreadable crew-state probes on a stale pane before the reader failure itself is first surfaced (never as a wedge or a stop); repeats are then spaced by the same doubling backoff as the escalation ladder up to FM_WEDGE_ESCALATE_MAX_SECS, and both counts reset on any readable verdict
