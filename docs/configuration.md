...
FM_COMPOSER_PI_MAX_LINES=8     # fleet-wide: maximum rows admitted between the identity-corroborated separator pair of the separated shape (Pi and agy); taller or ambiguous candidates stay unknown
...
FM_AGY_CONFIG_DIR=      # firstmate's own agy config-dir override, for test isolation only; agy resolves its root from $HOME alone and honours no override, so any other value installs a hook agy never loads. Defaults to ~/.gemini/config
