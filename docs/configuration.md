...
FM_PAUSE_RESURFACE_SECS=3600       # seconds between bounded rechecks of a declared external wait, verified captain-held transfer, at-threshold validating run-step deferral, or at-threshold worktree-write deferral, including a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting for declared waits and holds, ageing its window against the crew's own latest status line rather than pane busy state
...
FM_WEDGE_RUN_STEP_TIMEOUT=10       # wall-clock seconds the wedge detector's at-threshold run-step consult may take, so a hung no-mistakes or pane read cannot stall the watcher poll that started it; a working run-step defers the escalation instead of climbing the wedge ladder on pane idleness alone; hitting the bound, a parked prompt, and every other non-working verdict leave the escalation schedule exactly as it was; a value that is not a positive integer falls back to the default
