...
Each harness's machine-consumed rendered busy signature lives in its `bin/harnesses/<name>.sh` adapter, dispatched by [`bin/fm-harness-adapter.sh`](../bin/fm-harness-adapter.sh).
