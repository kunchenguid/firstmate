...
That inheritance covers every path that restarts an existing task from its own record, including firstmate's own secondmate liveness recovery, not only the `relaunch` verb.
