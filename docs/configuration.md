...
FM_SESSION_START_REMINDERS_TIMEOUT=20   # backstop bound on the captain-reminders refresh a locked session start makes; invalid or zero values use 20
FM_REMINDERS_TIMEOUT_SECS=10   # maximum seconds for each individual Reminders step before its process group is terminated and the step reported as failed; invalid or zero values use 10
