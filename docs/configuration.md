...
**Declared-state fast path:** a worker cannot self-trigger `/compact` (it is a terminal built-in, not a tool), so a no-mistakes ship brief tells the worker to append the status line `paused: awaiting compaction before validation` right after its implementation commit, then end its turn - before starting `no-mistakes axi run`.
