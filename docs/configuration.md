...
The flag forwards nothing by itself: a home that also enables `config/launch-env-allowlist` lists the names the adviser needs, `TYPESAFE_API_KEY` and `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS`, in that allowlist itself, exactly as it would any other ambient name.
