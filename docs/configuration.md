...
The signature in the fetch output is authoritative over the exit code: some git versions exit 0 when the branch fetch succeeded but the `--prune` ref rewrite failed on the lock, and such a signature-matching "success" still enters the same recovery path so the lock cannot persist forever.
...
`fm-fleet-sync.sh` applies the same shape to an orphaned `.git/packed-refs.lock`: it retries only fetches whose output shows Git's `Unable to create '...packed-refs.lock': File exists` signature, up to `FM_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES` times (nonnegative integer; unset, blank, or invalid uses the default of 3), waiting `FM_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS` seconds (nonnegative whole or fractional; invalid falls back to 1 second) before each.
That output signature is authoritative over the exit code - some git versions exit 0 when the branch fetch succeeded but the `--prune` ref rewrite failed on the lock - so a signature-matching "success" still enters the retry path, and a retry counts as recovered only when the fetch exits 0 with no lock signature in its output.
