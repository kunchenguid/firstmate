...
`config/` holds local gitignored operating choices, and `projects/` holds the home-owned project clones that Firstmate reads but changes only through the narrow guarded and concrete captain-approved exceptions in `AGENTS.md`.
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately when they carry new captain-facing information, unless they declare the pause verb
