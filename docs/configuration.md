...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the shared current-state reader in bin/fm-classify-lib.sh, also used by the watcher's parked-gate wait consult
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before an absorbed stale pane is rechecked for escalation; docs/architecture.md "Event-driven supervision" owns health suppression, wait deferrals, first alerts, and dead-record reporting
FM_BUSY_TURN_MAX_SECS=3600         # maximum busy-turn age before the same wedge recheck; bin/fm-watch.sh's busy_turn_over_age owns marker selection, and docs/architecture.md owns inspection-only routing and deferrals
