...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, gh-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, and quota-axi.
This section is the single owner of that universal toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
In that list, no-mistakes runs the validation pipeline, gh-axi and lavish-axi cover GitHub and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch.
Browser work runs through the BrowserClaw MCP server, which is configured as an MCP server rather than an installed axi CLI, so it is not part of this bootstrap-installed toolchain.
