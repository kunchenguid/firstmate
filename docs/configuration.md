...
When pi-signed is selected, Firstmate launches the executable named `pi-signed` from `PATH` and refuses the launch if it is unavailable rather than falling back to pi.
