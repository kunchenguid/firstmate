...
A standalone-clone home cannot receive a missing primary-local commit through that routine no-fetch sync, so `/updatefirstmate` imports its exact fork-and-canonical update target from the primary code root before applying the same guarded fast-forward.
