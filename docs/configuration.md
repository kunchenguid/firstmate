...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of the newest observable activity on a busy pane - a completed turn or explicit harness progress, whichever is newer (bin/fm-watch.sh owns marker selection) - before the same wedge escalation used for a provably-working non-busy stale takes over, with its own clause naming the silence it measured - that nothing has run inside the still-open turn when explicit progress is the newer anchor, and otherwise that no turn has completed for that long and this worker runtime reports no activity to tell from; set it above the longest single tool call a worker may legitimately make, not above the longest turn, because an autonomous crewmate's turn stays open for the whole job; inspection-only, never an automatic interrupt or restart; a declared external wait, an attended verified captain-held transfer, or - where config/wedge-defer-parked-gate arms it - a validation gate of the crew's own awaiting the supervisor's still-unanswered decision takes the FM_PAUSE_RESURFACE_SECS recheck below instead
