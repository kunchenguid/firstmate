...
agy is verified for crewmate and scout launches only and is refused for secondmates for the same reason; [its adapter reference](../.agents/skills/harness-adapters/references/harness/agy.md) owns the trust pre-registration, hooks wiring, and raw-launch limitations.
