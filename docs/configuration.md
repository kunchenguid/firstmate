...
`FM_HOME` determines Herdr's home label: the primary home uses `firstmate`, and a secondmate home marked by `.fm-secondmate-home` uses `2ndmate-<secondmate-id>`, unless the local, gitignored, non-inherited `config/herdr-workspace-label` holds a non-empty single-line (trimmed) override, which then wins for that home except when the secondmate marker is present (`bin/backends/herdr.sh`'s `fm_backend_herdr_workspace_label`); the override applies only to workspaces created from that point on, not to a home's already-existing workspace, and a file whose content spans more than one line is refused in favour of the default because the label has to round-trip through one line of the projection journal.
