...
FM_LOOP_PARKED_SECS=900           # positive interval for the parked-gate looping proxy; zero or invalid values use 900; docs/architecture.md owns the criterion, distinct-reads false positive, and inspection-only handling
FM_LOOP_PROBE_SECS=60              # minimum seconds between the looping check's current-state reads of one busy pane, and the minimum age of its validated semantic busy record before any read
