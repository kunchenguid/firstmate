...
prime-agent is verified for crewmate and scout launches, and `fm-spawn.sh` refuses it for a secondmate because that launch needs the home's own primary supervision extensions, which are not wired yet.
prime-agent runs every session in a detached daemon worker that outlives both the pane and an explicit quit, so a torn-down prime-agent task leaves its worker resident until the retirement path lands.
