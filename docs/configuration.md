...
Auto-detected herdr or cmux prints a stderr notice naming `config/backend` and `--backend tmux` as opt-outs.
Auto-detected tmux stays silent because the primary already runs inside that visible tmux surface.
Recovery of an existing local second mate reuses that exact task's recorded backend unless an authorized per-task `--backend` explicitly replaces it.
When no selection or primary-runtime marker matches any other spawn, `fm-spawn.sh` refuses before endpoint creation and names `config/backend` plus the authorized per-task `--backend` remedy.
...
Task meta records `backend=` only for a non-tmux backend; an absent `backend=` means `tmux`, preserving existing tmux-path meta files.
...
The bootstrap dependency resolver retains its non-spawn compatibility fallback to `tmux` after `FM_BACKEND`, `config/backend`, and runtime auto-detection, so a home still receives one concrete dependency result before task intake selects an endpoint.
