...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, compatible gh-axi, playwright-cli, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
...
In that list, no-mistakes runs the validation pipeline, gh-axi, playwright-cli, and lavish-axi cover GitHub, ad hoc browser, and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-aware array dispatch.
Project-owned automated end-to-end suites continue to use their existing Playwright test runners rather than playwright-cli.
...
An absent `playwright-cli` reports `MISSING: playwright-cli (install: npm install -g @playwright/cli@latest)`.
