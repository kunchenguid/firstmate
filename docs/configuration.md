...
FM_BACKEND_HERDR_BARE_PROMPT_GLYPHS='❯ ›'  # herdr-only: whitespace-separated list of LITERAL verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›; matched with a locale-invariant case, honored for row detection AND the shared classification, so an added glyph's idle composer reads empty rather than wedging on pending; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)")
...
FM_COMPOSER_AGENT_GLYPHS='❯ ›'   # fleet-wide: whitespace-separated list of LITERAL agent prompt glyphs (claude's ❯, codex's ›) that read as a genuine empty AGENT composer, bordered or bare. Never a regex: a bracket class over multibyte glyphs degenerates into a byte class under a C/POSIX locale, so this set is compared with a locale-invariant case. Shell prompt glyphs (> $ % #) are deliberately NOT in this set and are not configurable - telling them apart from agent glyphs is the dead-shell injection safety rule (bin/fm-composer-lib.sh)
