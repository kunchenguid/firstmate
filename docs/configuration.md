...
FM_LOOP_PARKED_SECS=600           # how long one busy turn of an ordinary no-mistakes crew may keep seeing the same parked gate, with no completed turn, run-step change, or task worktree write, before it wakes the supervisor as looping (docs/architecture.md owns the rule); inspection-only, never deferred by a declared wait, re-surfaced once per this interval while it persists; 0 turns the check off
FM_LOOP_PROBE_SECS=60              # minimum seconds between the looping check's current-state reads of one busy pane, and the minimum open-turn age before any read
