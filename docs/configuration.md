...
When a selected profile requires a primary selection receipt, firstmate also passes its current `--selection-receipt <path>`.
...
Every Astra crew or scout launch, and every non-Astra crew or scout launch whose matching harness/model profile sets this field, refuses before a worker endpoint, worktree, or backlog record changes unless Firstmate supplies a current version-4 selection receipt.
The receipt binds the task and selected harness/model/effort, the exact effective worker model, selected Codex home, task-fit rationale, candidate accounting, catalog evidence, and quota evidence that identifies `quota-axi`, that model, the selected Codex home, the SHA-256 of one saved `quota-axi` snapshot, and, for a named Codex home, the SHA-256 of its source-owned quota-axi account object.
`fm-spawn.sh` and `fm-control.sh` verify the schema, tuple, effective-worker-model equality, exactly one selected candidate that matches the tuple, quota-evidence source/model/snapshot binding, receipt and snapshot digests, applicable provider availability, and a 15-minute maximum age by default (`FM_DISPATCH_RECEIPT_MAX_AGE_SECONDS` may narrow or extend that local bound).
...
For a named Codex home, the snapshot must include quota-axi's full Codex account object, such as `quota-axi --provider codex --full --json` output, so Firstmate can compare only that object's hash with a fresh read from that home without recording account bytes.
...
Every guarded relaunch requires a newly current receipt too, rather than reusing the task's prior dispatch evidence.
...
Each profile in those facts renders its slash-separated harness, model, and effort axes followed by `@<home>` when the candidate names one, so the two sides of a two-account Codex rule stay distinguishable; only the configured path is printed and nothing inside that home is read.
