...
Herdr workspace labels use the shared home-tag derivation (`bin/fm-backend-hometag-lib.sh`): the active `FM_HOME` readable prefix plus a short hash of the resolved `FM_HOME` path - the primary home uses `firstmate-<hash>`, and a secondmate home marked by `.fm-secondmate-home` uses `2ndmate-<secondmate-id>-<hash>`.
Per-task herdr tab labels stay the bare `fm-<id>`, because the per-home workspace is itself the namespace and only the workspace-container label carries the tag.
...
For the herdr backend, `FM_HOME` determines the readable home prefix of the workspace label used by the adapter.
The full herdr workspace label also includes a short hash of the resolved `FM_HOME` path.
...
```sh
FM_HOME=                 # optional operational home for most scripts, unset means this repo root; fm-send requires it explicitly; the herdr/zellij/cmux home-tag hash is taken over it
FM_ROOT_OVERRIDE=        # override firstmate repo root and tangle-guard target; also legacy whole-root override when FM_HOME is unset, in which case it selects the home that tag hashes too
