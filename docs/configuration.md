...
Before every claude crewmate or secondmate launch, including a relaunch, Firstmate records the resolved worktree as trusted in `<CLAUDE_CONFIG_DIR>/.claude.json` when `CLAUDE_CONFIG_DIR` is set or `$HOME/.claude.json` otherwise.
This pre-accept is limited to the exact `claude` adapter and does not cover Claude Code's separate bypass-permissions warning or first-machine onboarding.
The spawn fails closed instead of risking the Claude configuration when the trust store is malformed, symlinked, unwritable, or keeps changing during the bounded update, and it preserves unrelated keys and project entries when the update succeeds.
