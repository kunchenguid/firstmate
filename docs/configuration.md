...
## Pi bash default timeout (config/pi-bash-timeout)

Pi's bash tool applies no timeout when a call carries none, so a hung command runs unbounded (verified against pi 0.80.10: `resolveTimeoutMs` returns `undefined` for an omitted timeout and the schema documents "Timeout in seconds (optional, no default timeout)").
Firstmate's Pi extensions therefore fill a default timeout into bash tool calls that arrive with no valid explicit timeout, so no command can hang forever, and leave valid explicit timeouts untouched; they never replace the bash tool itself.
`bin/fm-pi-bash-timeout.sh` is the single owner of that value's precedence, disable, and invalid-value semantics, and both the primary extension (`.pi/extensions/fm-primary-turnend-guard.ts`) and the crewmate extension generated by `bin/fm-spawn.sh` call it.

`FM_PI_BASH_TIMEOUT_SECS` wins when it is set; otherwise the resolver reads `pi-bash-timeout` under the effective config directory (`FM_CONFIG_OVERRIDE`, else `$FM_HOME/config`).
Accepted values, after surrounding whitespace is trimmed:

- absent file and no environment override: `900` seconds, the default.
- a positive integer up to `2147483` (Pi's maximum): that many seconds.
- `0` (in any leading-zero spelling), `off`, or `none`: injection is disabled, restoring Pi's unbounded no-timeout behavior.
- anything else, including a value above `2147483`: falls back to `900` seconds rather than failing the tool call.

An elapsed timeout is reported by Pi as an error for that one bash call; it does not terminate the agent session.
The file is local and gitignored, and it is one of the allowlisted config items propagated from the primary to secondmate homes by bootstrap and `bin/fm-config-push.sh`.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, `herdr-presentation-spaces`, `pi-bash-timeout`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread send failures.
...
FM_PI_BASH_TIMEOUT_SECS=       # override config/pi-bash-timeout with the default seconds Pi extensions fill into bash calls that carry no valid explicit timeout; 1..2147483 = that timeout, 0/off/none disables injection, invalid falls back to 900
