...
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and the fleet dashboard
...
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, the primary turn-end guard, and the fleet dashboard treat a watcher beacon as stale
