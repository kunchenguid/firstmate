...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for Linux process-identity reads in fm-wake-lib.sh and fm-teardown.sh, and for MSYS winpid/ppid reads in fm-session-lock-lib.sh, mainly for tests
FM_PROC_BACKEND=         # force the session-lock process backend: posix (ps -o) or windows (Win32 process table); unset probes ps capability, mainly for tests
FM_WIN_PROC_TTL=3        # seconds a Win32 process-table snapshot is reused before it is refreshed (windows backend only)
FM_LOCK_SYMLINKS=        # force the fm-wake-lib.sh lock shape: 1 symlink, 0 plain directory; unset probes the filesystem, which is what selects directory mode on Git Bash/MSYS without Developer Mode
