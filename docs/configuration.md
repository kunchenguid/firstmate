...
FM_GUARD_GRACE=300      # watcher beacon freshness for arm health, turn-end blocking, and persistent/extension/read-only pull-guard warnings; writable auto-arm pull guards use their ledger-driven repair policy instead
...
FM_WATCHER_STALE_GRACE=300   # defaults to FM_GUARD_GRACE; seconds a live watcher lock may have a stale beacon before the watcher reports the typed busy-holder outcome
