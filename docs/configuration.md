...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface once, then stay quiet until the pane hash, busy state, or a captain-relevant status verb changes, unless they declare the pause verb
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared paused: external wait for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; leftover held after inspect stays quiet even when this cadence is due; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer, ageing its window against the crew's own latest status line rather than pane busy state
