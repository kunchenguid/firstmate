...
FM_BACKEND_HERDR_DIALOG_LINES=80  # herdr-only: pane lines captured when classifying a claude/codex first-launch dialog at spawn (docs/herdr-backend.md "Guarded first-launch dialog handling")
FM_BACKEND_HERDR_DIALOG_POLLS=40  # herdr-only: polls awaiting a classifiable dialog or a corroborated started agent before the spawn fails; non-numeric or 0 falls back to 40
FM_BACKEND_HERDR_DIALOG_POLL_SLEEP=0.5  # herdr-only: seconds between those polls
FM_BACKEND_HERDR_DIALOG_KEY_RETRIES=3  # herdr-only: key attempts per dialog step before the spawn fails unverified; non-numeric or 0 falls back to 3
FM_BACKEND_HERDR_DIALOG_KEY_SLEEP=0.2  # herdr-only: seconds after each dialog key before the verifying pane readback
