...
FM_INACTIVE_RECONCILE_SECS=900  # 60..1800-second direct-report scan cadence and terminal inactivity threshold; locked session start also scans immediately
FM_INACTIVE_RECONCILE_BUDGET_SECS=10  # 1..30-second aggregate bound per direct-report scan
FM_PROGRESS_STALL_SECS=7200  # ordinary no-meaningful-progress suspicion bound; minimum 60
FM_PROGRESS_LONG_PHASE_SECS=21600  # longer bound for test/build/CI/render/deploy phases; at least FM_PROGRESS_STALL_SECS
FM_PROGRESS_ACTIVITY_SCANS=3  # live turn-activity observations without meaningful progress before suspicion; minimum 2
FM_PROGRESS_REPEAT_ROUNDS=3  # repeated same-theme blocked/decision rounds before suspicion; minimum 2
...
FM_CONTINUATION_CREW_TIMEOUT=3   # seconds allowed per worker-state read inside bin/fm-continuation-check.sh; non-numeric or 0 resets to 3
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and the watcher's direct-report reconcile scan
