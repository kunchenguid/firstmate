...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, OMP uses the tracked `.omp/extensions` pair, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
For OMP secondmate launches, `fm-spawn.sh` starts `omp` with `-e` pointed at the secondmate home's own tracked `.omp/extensions/fm-primary-omp-watch.ts` and `.omp/extensions/fm-primary-turnend-guard.ts`; Pi and pi-signed use the corresponding tracked `.pi/extensions` pair.
