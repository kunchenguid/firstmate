...
Untracked files and directories whose names begin with `scratchpad` are also gitignored, so temporary scratch never shows up as a home change in porcelain-based status checks.
