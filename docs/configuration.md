...
In either destination the emitter refuses a deliverable `tasks-axi` would refuse, such as an absolute `report_path`, naming the key, the value, and the expected format, so a bad value never travels to the owning home.
