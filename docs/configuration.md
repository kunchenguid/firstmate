...
FM_SPAWN_ACQUIRE_TIMEOUT=600             # seconds fm-spawn.sh keeps waiting while `treehouse get` (or its git fetch) still holds the task pane's foreground; the 60s path-settle bound applies only once the foreground is a shell again; non-numeric or zero uses the default; the fm-spawn.sh header owns the wait
