...
A provider that exists only because an extension registered it inside the captain's session, such as pi-devin-auth's `devin`, is offered and resolved by copying that registration into the branch's isolated runtime, so custom streaming protocols reach the branch unchanged, while stored OAuth and API-key credentials retain their native credential type because Firstmate never copies, converts, installs, or overwrites credentials for the branch runtime.
