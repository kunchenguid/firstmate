...
`min_confidence` is a number from 0 through 1 that replaces the resolver's global 0.6 confidence floor for that one rule; set it high on a rule whose wrong pick is costly and low on a rule that is a safe runner-up.
...
The task-specific text is the brief's `## Captain's intent` and `## Firstmate spec` sections that `bin/fm-brief.sh` scaffolds, each running to the next level-one or level-two heading outside a code fence; a brief with neither heading is sent whole.
...
The confidence floor is 0.6 unless the picked rule declares `min_confidence`.
When the picked rule declares its own floor and the answer's confidence is below it, the tool takes the most probable other option whose probability clears that option's floor (a rule's `min_confidence`, otherwise 0.6, and 0.6 for the neutral option), prints a `fallback:` line naming both floors, and resolves that rule as though it had been picked; no qualifying option, or two equally probable ones, is `ambiguous`.
A picked rule without `min_confidence` whose confidence is below 0.6 is `ambiguous` exactly as before, so a file with no declared floors behaves as it did.
