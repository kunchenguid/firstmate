...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^[❯›→]'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row - claude's ❯, codex's ›, Cursor Agent's →; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)")
...
FM_BACKEND_ORCA_BARE_PROMPT_RE='^[❯›→]'  # orca-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row; bare shell prompts stay unknown
...
FM_BACKEND_CMUX_BARE_PROMPT_RE='^[❯›→]'  # cmux-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row; bare shell prompts stay unknown
