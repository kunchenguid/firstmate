...
A tmux spawn additionally requires `jq`, like every other treehouse-backed backend (see "Toolchain" below); the pool-slot allocation guard that needs it is owned by the `bin/fm-spawn.sh` header.
...
That delta is owned in code by `fm_backend_required_tools` in `bin/fm-backend.sh`: the resolved backend's own session-provider CLI (`tmux`, `herdr`, `zellij`, `orca`, or `cmux`), `jq` for every treehouse-backed backend (`tmux`, `herdr`, `zellij`, `cmux`) because spawn parses `treehouse status --json` before allocating a pool slot and the JSON-emitting adapters (`herdr`, `zellij`, `cmux`) also parse their backend's output, and the `treehouse` worktree provider for every session-provider-only backend (`tmux`, `herdr`, `zellij`, `cmux`).
