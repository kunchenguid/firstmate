...
FM_WAKE_DELIVERY_RETRY_BASE_MS=250   # OpenCode wake-delivery (.opencode/plugins/lib/fm-wake-delivery.js) base delay before retrying an unaccepted promptAsync; separate budget from FM_WATCH_REARM_RETRY_BASE_MS
FM_WAKE_DELIVERY_RETRY_MAX_MS=2000   # cap for that exponential wake-delivery retry delay
FM_WAKE_DELIVERY_RETRY_LIMIT=2   # wake-delivery retries after the first attempt before declaring failure through bin/fm-wake-delivery-alarm.sh
FM_WAKE_DELIVERY_TIMEOUT_MS=10000   # milliseconds each wake-delivery promptAsync attempt waits for acceptance before counting as failed
...
FM_WAKE_DELIVERY_KEEP_LINES=200   # lines kept in state/.wake-delivery-failures when bin/fm-wake-delivery-alarm.sh trims it past FM_WAKE_DELIVERY_MAX_BYTES
FM_WAKE_DELIVERY_MAX_BYTES=131072   # size that triggers that trim
FM_WAKE_DELIVERY_ALARM_COOLDOWN_SECS=600   # minimum seconds between active wedge-alarm-channel alerts for wake-delivery failures; the durable record is always written regardless; 0 disables the cooldown
