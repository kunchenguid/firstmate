...
If that send fails, or is deferred because the secondmate waits on its own open decision or blocker, bootstrap keeps an idempotent retry marker and emits `NUDGE_SECONDMATES:` with the reason.
