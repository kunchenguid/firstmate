...
FM_SNAPSHOT_SECONDMATE_STATE_TIMEOUT=3  # seconds allowed per no-mistakes query in a home summary's up-front child-state wave (one axi status per ship child plus one coarse run inventory per distinct child repository, run concurrently, at most FM_SNAPSHOT_SECONDMATE_CHILDREN in flight), so the whole wave costs this bound rather than one query per child; set it in the environment of the home that RUNS the summary - a remote home summary executes under the remote job worker's fixed environment, so a local export never reaches it and the remote home uses its own value or this default
