...
Normal completed runs keep local-only and no-origin skips silent, and a registry-flagged `+unsynced` project (see `bin/fm-project-mode.sh`'s header) prints nothing at all, so there is never a line to relay for it.
