...
FM_CODEX_BIN=codex                      # executable used by both the Codex app-server version gate and the owning worker client
FM_BUSY_CODEX_TURN_DEADLINE_SECS=28800  # absolute per-turn deadline duration for an owned Codex worker; expiry interrupts, then terminates only its owned process group
