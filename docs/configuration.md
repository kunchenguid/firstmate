...
A removed file is measured the same way: the relaunch would use what an unpinned launch uses (for Claude, firstmate's own `CLAUDE_CONFIG_DIR` when set, otherwise the ordinary account; for Pi, the ambient `PI_CODING_AGENT_DIR` when set, otherwise `~/.pi/agent`), so it refuses when that is a different account directory from the recorded one.
A pin change therefore reaches new work immediately and existing tasks never; the same recorded account, a relaunch onto a harness that reads the other pin file, and a task with no recorded account in a home with no pin each relaunch exactly as before.
