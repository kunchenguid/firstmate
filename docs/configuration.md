...
FM_COMPOSER_CAPTURE_LINES=20   # fleet-wide default for tail-capture composer reads; tmux supplies its bounded visible pane, and Herdr's Claude post-type payload proof may retry at the pane's visible height when the tail selects no composer (see herdr-backend.md#claude-composer-proof)
