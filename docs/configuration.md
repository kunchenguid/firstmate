...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, OpenCode uses its TUI plugin, and Cursor's `stop` hook owns the arm from inside the synchronous hook (see `docs/supervision-protocols/cursor.md`).
...
`lsof` is required for the full teardown leaked-process reaper: teardown scans every process whose current working directory is the task worktree or tasktmp via `lsof -a -d cwd` and TERM/KILLs survivors whose process identity still matches (Fix 2 in `bin/fm-teardown.sh`). Without `lsof` teardown falls back to the tmux pane process-group reaper, which cannot see a detached process. The cursor worker-server reaper (Fix 3) is pid-based and lsof-independent, but a cursor primary/crew host should still install `lsof` so the generic cwd scan also covers other leaked descendants.
...
FM_COMPOSER_IDLE_RE=    # optional empty-composer regex, applied after ghost and border stripping; fm-send and the away-mode daemon default it to cursor's idle placeholder ('^Add a follow-up$') when the target or primary harness is cursor
