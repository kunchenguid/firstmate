...
- Internal identifiers are removed rather than refused. The scrubber classifies whitespace-delimited tokens before changing them, removes absolute paths even when punctuation or a `name=value` or `name:value` prefix surrounds them, and matches internal names only as whole tokens or whole names. It strips `fm/<name>` branch refs; the internal names `child`, `task`, `fingerprint`, `key`, `mode`, `yolo`, `harness`, `backend`, `window`, `worktree`, `branch`, `report`, `pane`, and `session`; and the verified harness names `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `muse`, so a genuine failure still reaches the captain without them. A token whose content begins with a URL scheme survives byte-identical, including punctuation within it, because a review card without a usable link is close to useless on a phone.
