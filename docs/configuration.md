...
When the worker uid is root, Firstmate preserves the operator's whitespace-separated `CHROME_DEVTOOLS_AXI_CHROME_ARGS` and adds the exact `--no-sandbox` token only when absent; non-root workers receive the operator arguments unchanged.
