...
On acceptance, no-clobber publication applies `O_NOFOLLOW` to the final receipt and brief path components and binds the exact validated bytes to `data/<task-id>/routing-generation.<generation>/brief.md` and the validated receipt to `data/<task-id>/routing-generation.<generation>/receipt.json`, where `<generation>` is the SHA-256 of the validated receipt bytes.
The routing filesystem helper applies `O_NOFOLLOW` only to the final path component supplied to each operation and does not reject an intermediate symlink, notably `FM_HOME/data`.
...
The launch half ignores caller-supplied receipt paths and launch-input bytes, resnapshots the task inputs, validates the selected and emitted route, derives the deterministic generation paths, and verifies the exact published receipt, brief, and latest consumed-ledger row through those final-component no-follow checks.
That validation does not re-age a timestamp already accepted by the preflight, so the bounded control handoff does not widen or restart the five-minute acceptance window.
