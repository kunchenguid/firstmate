...
FM_PROGRESS_FOOTER_LINES=6         # non-blank lines from the end of a captured pane that the progress reader scans for counters; deliberately narrow, matching the footer size the busy match and the acknowledgement scan already use, so token-shaped or price-shaped text in displayed content cannot be read as a progress meter; a zero or non-numeric value falls back to this default
FM_BUSY_NO_PROGRESS_SECS=1500      # seconds a BUSY pane may go with every rendered progress counter unchanged before the watcher reports it as busy-but-not-advancing; measured from the harness's own token, spend, and context meters (bin/fm-progress-lib.sh), never from elapsed time or the pane hash, so a worker that keeps generating resets it continuously however long its turn runs; report-only, with its own wording, and never an interrupt, signal or restart; a zero, negative, or non-numeric value falls back to this default rather than reporting every poll or silently disabling the measure; it stands down entirely for away mode, a declared external wait, a verified captain-held transfer, a task whose authoritative state is an actively running no-mistakes run-step, a task whose own worktree was written during the frozen window, and any pane whose numbers have never been observed advancing - that last case covers a harness that renders no counters and, in the ordinary case, one whose only counter-shaped text is displayed content, since arming requires a rendered number to RISE rather than merely to change, a bar a real meter clears on poll after poll while scrolled text clears it only by chance; it is not a guarantee for a counter-free harness, because a reading keeps the largest value per kind, so counter-shaped text that scrolls in higher than the number read before can still arm the measure, which is one reason this report takes no action; the blind-spot, run-step, and worktree-write stand-downs are each recorded in the watcher's absorbed-wake log, while away mode and a declared wait simply drop the tracking; a stood-down pane keeps FM_BUSY_TURN_MAX_SECS as its only backstop
