...
Slack `chat.postMessage` rejects a `text` longer than 40000 characters, so the daemon caps the built digest by item count and byte budget with an `…and N more` tail: an oversized backlog always delivers a partial digest that fits rather than failing on every retry, and the undelivered overflow is retained in the buffer for the next batch flush and for return catch-up (a single pathological item is hard-cut so header plus one item plus tail always fits). Caps default to 40 items / 38000 bytes and are overridable via `FM_SLACK_DM_MAX_ITEMS`, `FM_SLACK_DM_MAX_BYTES`, and `FM_SLACK_DM_MAX_ITEM_BYTES`.
...
claude, codex, opencode, pi, grok, and omp are all empirically verified; new harnesses get verified through a supervised trial task before joining the set.
...
The inherited-local-material contract is owned by [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md); for harness behavior, its propagated config items make a secondmate's own crewmates, dispatch profiles, Codex MCP posture, and backlog backend use the primary values.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `crew-codex-mcp`, `backlog-backend`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero only for real propagation errors.
...
FM_TREEHOUSE_AUDIT_TIMEOUT=10   # seconds allowed for bootstrap's whole read-only dirty-idle pool sweep across the firstmate repo and every project clone; unset/blank/non-numeric defaults to 10, and 0 turns the sweep off silently
