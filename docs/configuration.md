...
FM_UNOBSERVABLE_POLLS=3 # consecutive failed observations of one status log before its one check wake per failure episode (bin/fm-classify-lib.sh owns the bound)
FM_UNOBSERVABLE_MIN_GAP=20 # seconds one failed observation must be apart from the last counted one, so several observers in one window count once
