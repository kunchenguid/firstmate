...
The check is detect-only like the rest of bootstrap; until the module is installed, `bin/fm-captain-hold.sh`'s always-decoding subcommands (`hold`, `answer`, `answers`, `reconcile-requests`, `complete`, `diverged`) fail with the same install hint rather than reading silently empty fields, `verify`, `open`, and `reconcile close`/`reconcile note` fail the same way only on the call path that actually needs to decode a field, and `bind`, `unbind`, `binding`, and `reconcile list` stay usable regardless.
