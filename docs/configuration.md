...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a stale pane escalates as a possible wedge when its crew is not affirmatively provably working; stale panes whose semantic busy source affirmatively reports working suppress escalation and reset the timer
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a non-busy stale takes over; inspection-only, never an automatic interrupt or restart
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive possible-wedge escalations on the same unchanged pane before demand-deep-inspection is added
