...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait or verified captain-held transfer for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer; a declared external wait on a task whose own validated merge poll is armed is not rechecked at all, and records state/.<id>.pause-poll-covered instead, until that poll observes the PR closed without merging and the recheck resumes
