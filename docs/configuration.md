...
FM_TASK_ID=             # internal task-worker marker fm-spawn.sh exports into ship and scout panes, never set by hand; authorizes worker-owned no-mistakes axi run/respond through bin/no-mistakes, while bin/fm-test-run.sh refuses a marked worker in the repository primary checkout
