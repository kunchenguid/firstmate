...
omp is verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate, because it has no primary watcher-supervision protocol under `docs/supervision-protocols/` yet; [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md) owns that evidence.
