...
That identity refusal also recognizes assignment-prefixed and `env`-wrapped raw commands that name omp.
omp is reachable only through an explicit `--harness omp`; positional, configured, and raw omp launch forms are refused before mutation.
It is never a coordinator, a backend, or an implicit default, and Orca remains the execution backend for it like every other adapter.
