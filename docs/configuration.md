...
`bin/fm-tasktmp-lib.sh` owns the claim-first random temporary-root format, validation, transfer, startup reconciliation, relaunch reuse, and teardown removal contract; `tests/fm-gotmp.test.sh` and the hostile-path spawn case in `tests/fm-kimi-harness.test.sh` are its executable proof.
