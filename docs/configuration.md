...
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone, and the fm/<task-id> backstop sweep (requires its tip to be an ancestor of the local default branch) that also runs for local-only-mode and no-origin projects
