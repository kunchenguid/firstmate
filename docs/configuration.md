...
FM_ENDPOINT_GONE_CONFIRM_SECS=30   # elapsed seconds an unreadable pane's endpoint must stay absent, with the task's state/<id>.meta still present, before the watcher raises its one endpoint-gone wake; wall-clock rather than a poll count so a short FM_POLL cannot outrun the teardown race this window exists to survive
