...
FM_CLASSIFY_DECLARED_WORK_VERB=working   # leading status verb that declares work still to come; a declaration is an intention, never proof the work is running. One definition for every reading of it: the declared-work silence check and the keyed activity phases the fleet view renders both follow the override
FM_DECLARED_WORK_SILENCE_SECS=1500   # seconds a secondmate's own status stream may stay silent after declaring work before that silence surfaces for reconciliation, once per declared-work episode; generous because status appends are sparse supervisor-actionable events, not routine progress. Reads the status log only - no endpoint, no launch wiring, so a remote mate is covered by the same reading of the parent-side copy its escalations are ingested into, which is why a relay that stopped delivering is named in the wake as one of the causes a cheap check has to rule out; a declared pause or a terminal last event never counts. A value that is not a whole number of seconds is rejected on stderr and in the watcher's triage log, and supervision continues on the default rather than leaving the check disabled
