...
FM_CREW_STATE_RUN_STATUS=  # internal one-hop handoff of a crew's already-collected `axi status` output; set (even empty) suppresses that query in fm-crew-state.sh
FM_CREW_STATE_RUNS_SNAPSHOT=  # internal one-hop handoff of an already-collected coarse `no-mistakes runs` inventory, used only as a supplement to the status read
FM_CREW_STATE_RUNS_SNAPSHOT_REPO=  # repository scope that inventory was captured in; fm-crew-state.sh reuses the inventory only when it matches the crew worktree's own scope
FM_CREW_STATE_CI_LOG=  # internal one-hop handoff of the ci step log; set but empty means collection already failed, so ci readiness stays fail-closed unknown
