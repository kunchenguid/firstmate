...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows fetched at most once per crew-state read; bounds the coarse scan used when axi status cannot be attributed directly (the pipeline-custody exemption reads no rows: it takes a run's age from the run id axi status already reports)
FM_NM_CUSTODY_MAX_AGE_SECS=21600  # oldest a pipeline-owned no-mistakes run may be and still bind without head equality, so a stranded pipeline falls back to pane and status evidence instead of reading as working; a run parked at a gate binds regardless of age, and a malformed or non-positive value uses 21600
