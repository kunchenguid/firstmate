...
FM_NM_PROCESS_NAME=no-mistakes     # executable basename the wedge detector's live-validation probe accepts as a validation run; that probe defers an escalation only when such a process is ALSO bound to the task's own worktree by working directory, so neither the name nor the directory alone ever silences a wedge
FM_NM_PROCESS_SHARED_SUBCOMMANDS=daemon   # first arguments that mark shared no-mistakes infrastructure serving every lane at once rather than one task's own run; excluded from that probe even when bound to a task worktree; set it to the empty string to exclude nothing
FM_NM_PROCESS_TIMEOUT=10           # wall-clock seconds that one process scan may take, on the same rule as the worktree walk above: it runs only at the moment a wedge escalation would otherwise fire, and hitting the bound - like an absent lsof or a failed scan - reads as no evidence and leaves the escalation schedule exactly as it was
