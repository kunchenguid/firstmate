...
For Pi and pi-signed secondmate launches, `fm-spawn.sh` passes one `-e` for the generated parent-bound busy-state extension under `state/`, plus `-e` entries for the secondmate home's own tracked `.pi/extensions/fm-primary-pi-watch.ts` and `.pi/extensions/fm-primary-turnend-guard.ts`.
For omp secondmate launches, `fm-spawn.sh` passes one `-e` for the generated state-resident busy-state extension and relies on omp auto-discovering the home's tracked `.omp/extensions/` with no trust gate; naming a discovered file with `-e` as well loads it twice. Every omp launch also carries the tracked `.omp/fm-worker-overlay.yml` posture overlay through `--config`, which [`fm-spawn.sh --help`](../bin/fm-spawn.sh) owns.
...
`auto` replaces that flag with `--permission-mode auto`, Claude Code's classifier-reviewed permission mode, for a captain who refuses to run workers in bypass mode; ordinary task-worker and scout launches retain their existing environment prefix, inline settings, model, and effort flags.
A persistent Claude secondmate instead passes `--settings` the Firstmate-owned `state/<id>.claude-settings.json` artifact, which combines the attribution policy with that secondmate's supervision hooks; Firstmate removes this artifact during aborted setup or teardown and does not use the secondmate home's `.claude/settings.local.json` for it.
...
For ordinary task-worker and scout launches, that attribution policy is carried in inline `--settings` JSON; persistent secondmates use the Firstmate-owned `state/<id>.claude-settings.json` artifact passed by path.
...
FM_SNAPSHOT_SKIP_REMOTE=0           # internal/read-only session-start switch: skip remote home reads and cache writes, disclosing omitted homes in secondmate_current.remote_skipped
