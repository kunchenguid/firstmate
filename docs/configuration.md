...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a stale pane reaches the possible-wedge boundary; an authoritative active pipeline step resets that timer, while every other current-state verdict proceeds to the existing worktree-write check and alarm; stale panes whose crew is not provably working surface immediately unless admitted directly to the declared-wait cadence
