...
A herdr task using presentation additionally records `crew_label=` (e.g. `C7`, `X3`): the harness-letter-plus-running-number prefix of its presentation workspace label, allocated once from a per-home, per-letter counter in `state/.crew-label-seq-<letter>` and reused unchanged on every later relaunch or presentation rebuild instead of being renumbered.
...
## Stale-escalate threshold (config/stale-escalate-secs / FM_STALE_ESCALATE_SECS)

`config/stale-escalate-secs` (local, gitignored) overrides `FM_STALE_ESCALATE_SECS` for this home: the idle seconds before a provably-working stale pane escalates as a possible wedge, and the recheck interval `pause_state_class` (`bin/fm-watch.sh`) uses to re-verify a declared external wait against authoritative crew state before continuing to trust it.
A file present and holding a valid positive integer wins over the environment variable, which is the opposite of most `config/*` knobs in this repo; this one exists specifically so an operator can raise a home's threshold without also having to export the env var into every backend that launches the watcher and the away-mode daemon.
An absent, unreadable, `0`, or non-numeric file falls through to `FM_STALE_ESCALATE_SECS`, then to the caller's own default (240) - it never means "escalate immediately" or "never escalate."
...
Before `fm-brief.sh`, `fm-promote.sh`, `fm-spawn.sh`, or `fm-afk-launch.sh` persists a path or passes it to another process, it resolves each applicable relative `FM_HOME`, `FM_STATE_OVERRIDE`, or `FM_DATA_OVERRIDE` directory against the caller's working directory, preserves accepted absolute spellings unchanged, and rejects an unresolvable relative directory with the offending variable named.
