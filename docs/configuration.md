...
FM_FLEET_BOARD_LIVE=0   # fleet board: set to 1 to default in-flight state to the reconciled --live crew-state probe instead of the cheap status tail
FM_FLEET_BOARD_USE_SNAPSHOT=0   # fleet board: set to 1 to gather via fm-fleet-snapshot.sh --json (slow per-task reconciled read); falls back to direct gather on schema/parse trouble
