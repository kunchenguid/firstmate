...
The Pi Calm extension stores the captain's home-local presentation choice in gitignored `config/calm` under the effective `FM_HOME`, or under `FM_CONFIG_OVERRIDE` when that test and specialized-setup override is present.
