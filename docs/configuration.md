...
Delegated launches preload FirstMate's tracked startup guard before Pi imports its CLI, hiding the project `.pi/settings.json`, legacy `.pi/commands`, and deprecated `.pi/hooks` and `.pi/tools` surfaces during Pi 0.81.1's pre-flag startup phase.
They also pin `PI_CODING_AGENT_SESSION_DIR` to the configured agent directory's normal project-specific session path, so a project setting cannot redirect session storage.
The guard clears its own preload environment before the agent starts subprocesses.
After startup, `--no-approve` and `--no-extensions` disable project-local Pi resources and all discovered extensions so neither can alter compaction.
