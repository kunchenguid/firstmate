...
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the same wedge escalation used for a provably-working non-busy stale takes over; inspection-only, never an automatic interrupt or restart; a pane with a current declared pause routes to FM_PAUSE_RESURFACE_SECS instead
FM_PAUSE_RESURFACE_SECS=3600       # seconds before a still-declared external wait re-surfaces for a recheck; both the watcher and the away-mode daemon track it whether the pane is idle or held busy by the wait
