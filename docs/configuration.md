...
Worst case at the defaults (a full 13-fetch burst per 300-second slot) is 156 forge requests per hour for the whole home independent of the tracked count, with a per-PR poll interval of at most roster-size x slot while open and roster-size x slot x `FM_PR_FOLLOW_SETTLED_EVERY` once settled; a child's first poll waits for its own slot too, so restarting or reconciling every retained source at once stays inside the same bound, and is exempt only from the settled throttle so a baseline or post-restart verification lands within one roster cycle; registration is never capped because retirement stays explicit.
...
FM_PR_FOLLOW_MAP_LIMIT=40             # bounded review/check/thread map size per follow-through cursor (1-2048)
