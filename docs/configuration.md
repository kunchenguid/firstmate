...
Every agy spawn also answers agy's workspace-trust dialog with one Enter, which makes agy add that task worktree to `trustedWorkspaces` in `~/.gemini/antigravity-cli/settings.json`; teardown deliberately does not remove those entries, because they are the operator's own trust store, so they accumulate one per task worktree and are pruned by hand.
