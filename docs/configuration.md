...
The local `config/herdr-presentation-spaces` file selects Herdr's worker presentation: `tabs` explicitly places directly delegated transient workers beside their launcher, while `on`, an empty file, and the supported unconfigured default retain the disposable single-task workspace projection, and `off` disables that projection; [Presentation spaces](herdr-backend.md#presentation-spaces) owns the accepted values, default, Herdr version floor, lifecycle, safety limits, recovery contract, and narrow locked session-start cleanup of exact restored idle-shell children.
