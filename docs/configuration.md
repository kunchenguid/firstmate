...
FM_CHECKPOINT_ROLLOVER_GRACE=60   # seconds a completed checkpoint's single-use rollover marker lets the immediate wake drain skip the watcher-down alarm; invalid or zero values use 60
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before the watcher re-reads a stale-absorbed-as-working pane's authoritative current state (still-working renews the timer, a current pause moves to the pause cadence, stopped/done/unknown surfaces as plain stale) and before the away-mode daemon escalates such a pane; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before possible-wedge escalation takes over; inspection-only, never an automatic interrupt or restart
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive over-age busy-turn wedge escalations on the same pane before demand-deep-inspection is added
