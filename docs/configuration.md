...
Spawn records whether it created the chosen root or borrowed a pre-existing hookless project directory; teardown and relaunch remove only the Firstmate-generated hook file (never a project-authored `hooks.json`) and only remove a root Firstmate itself created.
