...
A captain-facing (verdict `captain`) branch outcome persists as one exact, sequence-keyed visible transcript entry; `action:main` (default, and legacy omitted) then opens one sequence-keyed processing turn on main that stays open until main acknowledges that sequence through its `fm_branch_processed` tool, while display-only `action:none` finished results advance the processed marker without that turn ([pi-supervision-branch.md](pi-supervision-branch.md)).
...
FM_TURNEND_OUTCOME_ABSORB_SECS=900 # longest one endpoint's bare turn-ends may ride an already-handled outcome-index coverage proof before one wake surfaces and the window restarts; default-on
