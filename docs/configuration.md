...
Away mode still declines every wake offer, and a broken current-generation branch still falls back to today's wake-to-main path.
...
The branch prompt's "Verdict: routine, captain, or firstmate-action" section owns the distinction between captain-facing results, authorized MAIN actions, unsolicited routine outcomes, and unchanged-review outcomes.
...
On Zellij, cmux, and Orca a typed-plane Cursor send (a harness-native invocation or an explicit backend target; ordinary text steers ride the durable inbox and exit 0 at enqueue) lands, but `fm-send` reports an unconfirmed submission and exits non-zero because their shared submit core does not consult the busy footer; [runtime backend verification](verification/runtime-backends.md#cursor-agent-cli) owns the evidence and transcript-state boundary.
...
`bin/fm-teardown.sh` refuses to clean up a task while this home still owes a public reply for exactly that work, unless `--force` is used under hard rule 3 in [`AGENTS.md`](../AGENTS.md) with explicit authorization for the named contents.
...
Within one drain, queued `check` records are coalesced only when their kind, durable key, and payload are identical; a changed payload remains a separate delivery even when its key, including a source-and-sequence identity, is unchanged.
...
FM_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6  # herdr-only: minimum per-Enter confirmation budget; also the delay between cursorless Pi composer captures
...
FM_RENDERED_BUSY_RECOVERY_POLLS=3  # byte-identical Claude rendered-busy rows required after a max-defer alarm before Herdr+Claude rechecks the composer; invalid or zero uses 3
