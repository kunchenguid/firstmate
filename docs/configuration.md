...
Rule `approval` and `floor`, and profile `provider` and `floor` are optional declarations that only [typed dispatch resolution](#typed-dispatch-resolution-configjev-mode-and-env-openrouter_api_key) applies in code; without an active Jev mode and key they are inert, and firstmate's own intake reads them as ordinary hints.
...
Bootstrap validates resolver-only `approval`, `floor`, and present `provider` values only while typed resolution is active through both an active mode and a key; otherwise those inert fields and the pre-existing verified-harness baseline preserve bootstrap behavior.
...
While typed resolution is active, malformed `approval`, `floor`, and present `provider` declarations receive the same diagnostic; without an active mode plus key those inert declarations preserve the pre-existing bootstrap behavior.
Missing `jq` is reported through the normal `MISSING: jq` install-consent flow.
While the file remains present, no crewmate or scout spawn may proceed without an explicit resolved harness; malformed configuration must be reported and corrected rather than selected around.
Secondmate homes inherit this file from the primary, so a secondmate's own crewmates apply the same dispatch profile behavior.

## Typed dispatch resolution (config/jev-mode and .env OPENROUTER_API_KEY)
...
When shadow or on and at least one rule exists, the tool sends the project name and the whole brief as state and asks one Choice question whose options are every rule's `when` plus the fixed neutral option for no matching rule; the model never sees quota, catalogs, `why`, `use`, or approvals.
...
In on mode the result is one of `clear` (a `profile:` line ready for `fm-spawn.sh`), `ambiguous` (confidence below the floor), `escalate` (an approval-gated rule, unverifiable rule floor, nothing rankable, or a genuine tie), or `error` (API, network, malformed response metadata, rendering, or quota-axi failure), and every one of them exits 0.
Shadow mode instead emits `status: shadow`, the underlying `decision_status`, a non-applicable `shadow_profile:` when one was selected, and the private record path.
...
The resolver, mode command, and bootstrap copy an environment-provided key into a non-exported private variable and unset `OPENROUTER_API_KEY` before launching child processes, so the secret is absent from child environments.
...
The resolver fixes the endpoint at `https://openrouter.ai/api/alpha/decisions`, model at `~typesafe/jev-latest`, confidence floor at 0.6, and request timeout at 5 seconds; `OPENROUTER_API_KEY` is its only resolver-specific environment setting.
...
The `github` profile requires `gh`, compatible `gh-axi`, and a successful `gh auth status`.
The `forgejo` and `local-only` profiles omit those GitHub-only prerequisites while retaining Git, no-mistakes, browser helpers, backlog tools, and the resolved runtime backend's dependencies.
...
Required tools come in three parts: an SCM-independent base toolchain, an SCM-profile delta, and a per-backend delta that follows the runtime backend actually resolved for this home.
The essential base toolchain is node, git, no-mistakes v1.46.0 or newer, chrome-devtools-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
The `github` SCM profile additionally requires gh with GitHub auth via `gh auth login` and compatible gh-axi; the `forgejo` and `local-only` profiles do not.
...
On the `github` profile, an absent or incompatible `gh-axi` reports `MISSING: gh-axi (install: npm install -g gh-axi && gh-axi setup hooks)`.
...
OPENROUTER_API_KEY=     # credential for Jev through OpenRouter; config/jev-mode separately selects off, shadow, or on (docs/configuration.md "Typed dispatch resolution")
