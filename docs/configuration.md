...
Every routed clone is independently revalidated through the public guarded command boundary and requires an exact conventional destination under either the active home's `projects/` directory or the selected secondmate home's `projects/` directory.
