...
Registration baselines every page of each collection through bounded rotating scans before it becomes live, so a new PR replays no creation noise without treating a bounded window as the whole history. Registration with `--backfill`, or the guarded resumable `bin/fm-procevent-pr-follow.sh backfill` sweep over PR metadata, then surfaces each currently unanswered inline review thread (last comment author is not the PR author and GitLab does not mark the thread resolved) exactly once; capped sweeps and thread scans retain their progress until enumeration finishes.
...
GitHub comments, inline replies, reviews, and check runs and GitLab discussions and pipelines rotate newest-first through independent durable page cursors. A newly added tail page is read immediately, and every older page is revisited within `ceil(total-pages / FM_PR_FOLLOW_MAX_PAGES)` polls. All forge access is GET-only.
...
Aggregate polling is bounded by the single scheduler and a deterministic modular rotation over `FM_PR_FOLLOW_ROTATION_SLOT`-second slots: the sorted cursor roster assigns each slot to one PR, so only one poll burst starts per slot, every open PR is visited once per roster cycle, and merged or closed PRs are polled every `FM_PR_FOLLOW_SETTLED_EVERY`-th visit.
At the defaults, a full 25-request burst per 300-second slot is at most 300 forge requests per hour for the whole home, independent of the tracked count. An open PR's starvation bound is roster-size x slot, a settled PR's is roster-size x slot x `FM_PR_FOLLOW_SETTLED_EVERY`, and registration remains uncapped because retirement is explicit. Captured wakes use the home scheduler id printed by `bin/fm-procevent-pr-follow.sh scheduler-id`; the result's validated `target:` field names the PR tracking id that `handle` applies.
