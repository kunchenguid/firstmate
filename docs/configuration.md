...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns completion evidence, Bitbucket PR recording, missing-evidence refusals, and captain-held retention instead of closure.
Session start reconciles this home's own books after an interrupted run; [`bin/fm-backlog-transition-lib.sh`](../bin/fm-backlog-transition-lib.sh)'s header owns close-marker replay and legacy Bitbucket marker migration.
