...
`bin/fm-run-evaluation-export.sh` is the only producer of `state/cockpit-run-evaluation.json`; its header owns source selection, fixed destination, validation, redaction, limits, and atomic publication.
