...
Real feedback, any other `SERVER_ERROR`, and that same interruption still standing once the bound is spent are all captured and announced normally; `FM_LAVISH_POLL_RETRY_DELAY` is a bounded 1 to 60 second test override for the interval only, and the runner itself stays adapter-agnostic.
...
A Lavish source tracks the BOARD FILE, not the session, and that is the adapter's own operating contract.
`lavish-axi <file>` opens, resumes, and reopens a session for the same artifact as often as anyone likes, and the Lavish server also stops when it is idle, so a session reaching `ended` - or an artifact having no active session at all - says nothing about whether that board can deliver again.
The listener therefore waits such a response out rather than capturing it: it prints nothing and exits nonzero, which is the runner's own documented no-result path, so the registration stays armed, no result is stored, and no wake is published.
The two shapes it waits out are an ended session carrying no queued content at all and the published not-found error, and either one would otherwise store a fresh result on every supervision cycle.
A response too large to be a quiet absence is streamed through undecided and the runner's own output bound applies to it.
The hold-back bound is derived from the longest artifact path the OS can name rather than fixed, because the largest possible quiet absence is a path: lavish-axi's ended-session envelope is 378 bytes of fixed text plus the artifact realpath four times, so `378 + 4 x PATH_MAX` covers every envelope that can exist.
A quiet absence that still outgrows that bound never becomes a silent no-op that stays armed: the adapter never claims silence for a response the listener could not hold whole, so the runner announces it once and then retires the source, while every other over-bound response - a large real payload above all - is still streamed, delivered, and announced unchanged and keeps the source armed.
An artifact that no longer resolves is the one normal thing that ends a Lavish source: `lavish-axi` refuses to resolve that path before it looks for a session, and the adapter reads that exact refusal as its terminal verdict, with the over-bound quiet absence above as the one pathological exception.
A board file that is deleted therefore retires its source on its own, while a board that is merely finished stays armed, so retire a Lavish source explicitly once the board itself is no longer wanted.
...
For Lavish that verdict covers exactly one shape - a session the adapter classifies `ended` that carries no queued content block at all, which is a review surface closed with nothing said - and it is shared with the listener's own quiet-absence test rather than restated, because a quiet absence is waited out before the runner ever sees it (see the board-file contract above), while a response larger than the hold-back bound is never claimed as silence.
...
A `Send & End` close carrying the captain's answer arrives as `status: feedback` with `session_ended`, so it classifies `feedback` and is announced unchanged, as is any `ended` result that still carries content, and every `waiting`, `unknown`, or unreadable result.
...
For Lavish that verdict covers an artifact that no longer resolves and the over-bound quiet absence above, because the board file is the source's lifetime rather than any one session on it.
