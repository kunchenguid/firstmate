...
FM_CHECK_INTERVAL=300   # seconds between slow checks (authenticated merge polls, registered local validation gates and custom checks, or Relay dispatch)
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script
FM_VALIDATION_PARKED_SECS=240   # seconds a matching no-mistakes run may await an agent while parked before its registered validation check wakes firstmate
