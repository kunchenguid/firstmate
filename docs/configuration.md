...
Its `invalidate` token additionally records an unchanged graph stale, which is what a remote PR merge needs while the local clone still lags; the token is recorded in the project's own pending request, so coalescing a repeat into a running refresh cannot downgrade it.
