...
Secondmate spawns are exempt and still resolve through `config/secondmate-dispatch.json` when a matching profile exists, otherwise through `config/secondmate-harness` and its optional model and effort tokens.
