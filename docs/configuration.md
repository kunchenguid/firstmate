...
Which build "on `PATH`" means is resolved deterministically by that same library rather than by `PATH` order alone: sourcing it keeps the first `tasks-axi` on `PATH` when it meets the version floor, otherwise probes every other `tasks-axi` on `PATH` in order and selects the first that does, prepending that build's directory to the process `PATH` and exporting it as `FM_TASKS_AXI_BIN`, so every firstmate script and every child it starts uses one build even when a shell rc file has put a stale install first.
`bin/fm-spawn.sh` exports the same `FM_TASKS_AXI_BIN` and `PATH` prefix into each crew pane before launch, and an exported `FM_TASKS_AXI_BIN` that still meets the floor is honored by every script the crew runs.
...
An absent or incompatible `tasks-axi` reports `MISSING: tasks-axi (install: npm install -g tasks-axi)`, followed by the rejected candidates' paths and versions when any `tasks-axi` was found; when `config/backlog-backend` is not `manual`, a home with a backlog refuses lifecycle mutation until compatible `tasks-axi` is on `PATH`, while a manual-backend home keeps its backlog hand-edited.
When several `tasks-axi` builds are on `PATH`, the resolution rule in "Backlog backend" above decides which one every firstmate script and crew pane uses, and bootstrap prints the resolved path and version whenever that differs from `PATH` order.
