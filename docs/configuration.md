...
The central resolver maps a project argument to a path in this order: a `projects/<name>` argument prefers the legacy `$FM_HOME/projects/<name>` clone then resolves `<name>` as an alias; a bare alias resolves through `data/project-paths.json`, then `<projects-root>/<alias>`, then - in a `config/projects-root` home - the projects root itself when it is the git work-tree root named `<alias>` (the repository a per-project `firstmate init` registers), then `$FM_HOME/projects/<alias>`; any other path, and an alias that resolves nowhere, passes through unchanged.
