...
FM_TASK_INBOX_DEFER_HORIZON_SECS=450   # derived from the two settings above as grace * (ring max + 2); seconds of continuously observed lifecycle-lock deferral of one steer before it surfaces once as a stale wake, since a deferred doorbell types nothing and spends none of the attempts above
...
FM_TASK_CONTROL_LOCK_WAIT=5   # seconds a lifecycle entry point (fm-control, fm-spawn --relaunch, fm-teardown, fm-promote) waits for a task's control lock, so a steering doorbell's brief hold is waited out instead of reported as a concurrent lifecycle action
