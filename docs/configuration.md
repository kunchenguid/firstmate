...
FM_CREW_STATE_NM_TIMEOUT=10   # positive integer from 1 through 86400 seconds for each no-mistakes query in fm-crew-state.sh; an explicit blank, zero, nonnumeric, or out-of-range value exits 2
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before possible-wedge escalation; the normal watcher resets this window on provable attributed pipeline progress, while away-mode housekeeping escalates at the bound
