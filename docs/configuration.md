...
The primary-scoped session hooks are the one exception: when the override resolves to a checkout other than the one the hook itself runs from, they treat the whole inherited set as a parent session's and ignore it, so pointing them at a foreign root has no effect - see [turnend-guard.md](turnend-guard.md#guard-predicates).
...
FM_ROOT_OVERRIDE=        # override firstmate repo root, tangle-guard target, and zellij/cmux home-title hash; also legacy whole-root override when FM_HOME is unset; ignored by primary-scoped hooks when it names another checkout (docs/turnend-guard.md)
