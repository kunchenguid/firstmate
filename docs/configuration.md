...
FM_WAKE_ACTIONABLE_TIMEOUT_MS=3000   # milliseconds the Pi watcher extension allows each receipt capture or batched final validation
FM_WAKE_ACTIONABLE_LOCK_ATTEMPTS=200   # queue-lock attempts allowed for one non-consuming wake receipt operation
FM_WAKE_ACTIONABLE_LOCK_SLEEP=0.01   # seconds between wake receipt queue-lock attempts
