...
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, and `cmux` for ship and scout tasks; `--secondmate` additionally launches on `cmux` (see [`docs/cmux-backend.md`](cmux-backend.md#secondmate-launch)), while `backend=orca` still refuses `--secondmate` until secondmate launch semantics are designed for it.
