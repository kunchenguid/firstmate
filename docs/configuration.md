...
FM_FLEET_PRUNE=1        # set to 0 to skip pruning local branches whose upstream is gone, and the fm/<task-id> backstop sweep (a branch's tip is an ancestor of the local default branch, or its upstream is gone) that also runs for local-only-mode and no-origin projects
