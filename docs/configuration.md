...
`fm-spawn.sh` unsets all five `FM_*_OVERRIDE` variables in the launch line of every spawned pane, regardless of task kind, so a pane can never inherit a foreign home's overrides from the launching process tree and resolves its own defaults instead.
The reset leaves each variable genuinely unset rather than set to empty, because consumers distinguish the two, and it takes effect both in POSIX pane shells and in fish.
On a fish pane it clears the copy the pane inherited from the launching process tree; a value the operator exported as a universal fish variable (`set -Ux`) is machine-wide operator configuration rather than a foreign home leaking in, and deliberately survives the reset.
Detecting fish costs a fish pane one cosmetic line of output - the interpreter path printed by the shell probe the reset uses - ahead of the launched command; no other pane shell prints anything for the reset.
A secondmate launch additionally redirects `FM_HOME` to that secondmate's own home alongside the reset; an ordinary ship or scout worker has no home of its own and receives the reset only.
