...
FM_SECONDMATE_HEALTH_INTERVAL_SECS=60  # seconds between primary-watcher secondmate health samples (commander-model melt and steering-backlog alarm); the durable mtime marker survives watcher restarts, and zero or invalid values use 60 (docs/remote-secondmates.md)
FM_SECONDMATE_INBOX_MAX=20  # unhandled *.msg count that must be exceeded before the secondmate steering-backlog alarm fires; invalid values use 20 (bin/fm-secondmate-inbox-lib.sh)
FM_SECONDMATE_INBOX_AGE_SECS=7200  # oldest unhandled *.msg age in seconds that must be exceeded before that same backlog alarm fires; invalid values use 7200
FM_SECONDMATE_MELT_EVIDENCE_COUNT=2  # distinct quota-error pane snapshots required before pane evidence alone melts a live secondmate commander; zero or invalid values use 2 (bin/fm-secondmate-melt-lib.sh)
FM_SECONDMATE_MELT_COOLDOWN_SECS=3600  # seconds a melt cooldown marker suppresses relaunch for the same still-recorded harness/model pair after an unsuccessful attempt; successful replacements with an updated profile are not cooled; a restored different pin is not suppressed; invalid values use 3600
