...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; leftover idle surfaces once, then exponential backoff capped at once per hour; dead or unknown leftover, dead+done, and dead+paused stay silent after classify
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a live declared paused: external wait for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; leftover held after inspect stays quiet even when this cadence is due; dead+paused stays silent after classify; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer, ageing its window against the crew's own latest status line rather than pane busy state
FM_IDLE_BACKOFF_SECS=60            # leftover-idle exponential backoff base after the first inspect; cap is FM_IDLE_BACKOFF_CAP_SECS
FM_IDLE_BACKOFF_CAP_SECS=3600      # leftover-idle recheck cap: at most once per hour for the same pane and condition
FM_LEFTOVER_LIST_SECS=86400        # once per day, one leftover list of long-inactive panes for a captain decide-or-discard; notes younger than this stay off the digest; the watcher never auto-teardowns
