...
FM_LAVISH_POLL_TIMEOUT_MS=   # optional milliseconds for one bounded fm-lavish-poll.sh `lavish-axi poll`; unset derives (FM_CHECK_TIMEOUT - 1) seconds, capped at 25000
FM_LAVISH_ACK_TEXT=     # optional override for the concise acknowledgement fm-lavish-poll.sh sends back into the Lavish session when reviewer prompts are picked up
FM_LAVISH_ACK_TIMEOUT_MS=500   # milliseconds allowed for that acknowledgement `lavish-axi poll --agent-reply` call; must be numeric
