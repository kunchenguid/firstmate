...
FM_CHECK_INTERVAL=300   # seconds between slow checks (authenticated merge/close polls, custom checks, or X-mode dispatch)
FM_CHECK_TIMEOUT=30     # seconds allowed per slow check script
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per foreground watcher checkpoint in Codex primary supervision
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by watcher absorb triage
FM_MERGE_READY_HEAD_TIMEOUT=10   # seconds allowed for the live PR head/check verdict used by merge-ready stale suppression
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; other stale panes surface immediately unless an expected-idle pause or merge-ready absorb class applies (docs/architecture.md "Event-driven supervision")
