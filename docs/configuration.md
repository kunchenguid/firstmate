...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; at that threshold a current declared wait, a quiet crew's working run step with recent pipeline activity, or in the attended watcher a current task-worktree write defers the alarm to the FM_PAUSE_RESURFACE_SECS recheck below; quiet or missing run activity keeps the ordinary wedge schedule, and a live idle declared wait still surfaces once before the longer cadence bounds repeats
...
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded rechecks of a declared wait, a quiet pane explained by a working run step with recent activity, the attended watcher's validation-or-write deferral window, or repeated new-hash stale alarms for an ordinary crew task with an open backlog captain call; a structured until time can make an external-wait recheck occur sooner but cannot extend this bound; the away-mode daemon applies this setting to declared waits and live validation runs but does not probe worktree writes, while a captain-held transfer is never rechecked while the away-posture record exists
