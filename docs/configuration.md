...
FM_BRAIN_EVENT_TIMEOUT=10   # seconds allowed for one lifecycle event before it is abandoned with a warning; enforced through timeout, gtimeout, or a perl fallback, so a host with none of the three runs the event unbounded; blank or non-numeric values use 10
