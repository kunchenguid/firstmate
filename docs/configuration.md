...
FM_ARM_ATTACH_VERIFY=2  # seconds an attach candidate must keep verifying as this home's live watcher before fm-watch-arm reports the attach; 0 and non-numeric fall back to 2
FM_ARM_ATTACH_RETARGET_MAX=2   # times one fm-watch-arm attach may retarget onto a healthy successor that took the lock mid-verification; 0 and non-numeric fall back to 2
FM_ARM_RESTART_MAX=1    # times one fm-watch-arm invocation may re-execute itself as --restart after a failed attach
FM_ARM_ATTACH_REPLACEMENT_MAX=20   # consecutive replacement candidates that may fail verification while fm-watch-arm is attached before it stops re-attempting and returns a terminal failure; reset by any candidate that verifies; 0 and non-numeric fall back to 20
FM_ARM_STDERR_TAIL_LINES=5   # lines of the watcher's stderr quoted in a failure line
