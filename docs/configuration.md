...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread send failures.
...
FM_PROVIDER_SESSION_AVOID_PCT=70   # fresh advisory session avoid pressure refuses normal launches; unknown/stale remains unconfirmed, not quota-confirmed
FM_PROVIDER_SESSION_HANDOFF_PCT=85 # fresh advisory handoff pressure refuses normal launches and triggers a planned safe-checkpoint handoff review
FM_PROVIDER_WEEK_AVOID_PCT=90      # fresh advisory weekly/model-window avoid pressure refuses normal launches; unknown/stale remains unconfirmed
FM_PROVIDER_WEEK_AVOID_PCT_OPENAI=50 # fresh advisory weekly avoid pressure for OpenAI specifically refuses normal launches at this lower reserve floor; verified-exhaustion holds stay authoritative
...
FM_SPAWN_OVERRIDE_PRESSURE=1   # deliberate escape hatch: allow fm-spawn.sh to proceed through a fresh avoid/handoff advisory pressure snapshot; the override is logged as evidence and the verified-exhaustion hold remains the stronger refusal
