...
A project whose shared git config sets `core.bare` or `core.worktree` (for example a bare repository with linked worktrees) is refused before any config write unless `extensions.worktreeConfig` is already enabled, because enabling it would apply that setting to every worktree.
