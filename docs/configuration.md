...
After a board host applies and re-serves one feedback batch, `FM_LAVISH_HOST_STATUS_FILE=<host-task-status-file> bin/fm-procevent-lavish.sh reply <artifact.html> <text>` stages a short acknowledgement for that board's existing listener and returns without running a poll in the host's turn.
The command also accepts multiline text on stdin or through `--file <path>`.
The command canonicalizes an absolute host status path through a symlink-spelled home to the physical state root before validating and recording it.
When a final Send & End batch has already retired the source, the command exits successfully after appending the acknowledgement as an informational note to that host status log, and the host preserves the applied final changes in its report because the closed board cannot display another reply.
Each source has one private mode `0600` pending-reply file and one stable in-flight reply file under `state/procevent/`; each record carries the built-in registration's fresh random token and host status-log path, and a second unconsumed reply for that token appends after one blank line instead of overwriting the first.
Before staging against a matching legacy built-in registration, `reply` asks the generic runner to atomically republish that exact adapter and argv record with a fresh runner-owned token, so already-open boards adopt generation-bound replies without a manual re-arm.
The command requests an immediate restart bound to that exact built-in registration generation, and the fresh adapter invocation atomically renames the pending file to the in-flight file only when the live registration still carries that generation before launching `lavish-axi poll <artifact.html> --agent-reply <text>`.
If terminal or explicit retirement ends the source before delivery, a home sweep retires it, or an identical re-arm replaces its registration first, neither the restart nor the replacement poll may post the stale acknowledgement; the generic runner invokes the built-in adapter's optional retirement cleanup under the source lock before removing the exact registration, and the Lavish adapter removes each pending or in-flight record only after appending its acknowledgement and the retirement or generation-change reason to the recorded host status log.
The adapter removes the in-flight file only after `lavish-axi poll` exits successfully with a nonempty response, which is observable progress beyond its earlier agent-reply POST; process liveness alone never commits the reply.
An exit, launch failure, or restart before that response, or a stale in-flight file left by a crash, restores the acknowledgement to pending, appending it before any newer pending reply.
This is deliberately at least once: a poll that posted the acknowledgement and then failed before returning a response can repeat that short acknowledgement, while an unproven delivery cannot silently lose it.
The generic restart hook discovers the identity-checked registered-command child before or during its bounded wait, signals it once published, lets the runner drain and durably capture any result bytes that already arrived, and starts the replacement only after the prior generation releases ownership.
...
Registration writes one private record with a fresh non-reusable registration token under `state/procevent/`, and a completed result plus its immutable adapter identity are captured under `state/procevent-inbox/` before any announcement or event can reference it.
...
After capture - and after initial `check` publication for the default ordering - the runner asks the immutable captured owner through the built-in `terminal` command or external `result.terminal` operation and retires the registration on exit 0 alone, dropping only the exact registration generation captured by its claim and releasing that claim only after adapter cleanup and removal succeed under one source boundary; before terminal, explicit, or home-sweep removal of a built-in registration, its optional `retirement-cleanup` command runs under that same boundary, where exit 0 completes cleanup, exit 2 declares none, and any other exit retains the registration for retry; a missing terminal command, an error, or any other terminal exit keeps the source armed, so an adapter with no notion of ending needs no change.
