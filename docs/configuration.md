...
FM_LOCK_STALE_AFTER=2   # grace seconds for missing or nonnumeric lock-owner PIDs (minimum 2s); a numeric PID proven dead, or proven recycled onto another process by its recorded start token, has no age grace
FM_STATUS_PRESENTATION_LOCK_TIMEOUT=10   # positive whole seconds one bounded presentation-path lock wait may take (blank, nonnumeric, or zero uses 10); a wake drain skips the contended step and retries on the next drain, while teardown's status-row retire waits a live holder out across many such windows; queue mutation locks stay blocking
