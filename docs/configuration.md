...
**Declared-state fast path:** a worker cannot self-trigger `/compact` (it is a terminal built-in, not a tool), so when this host has `config/idle-compact` configured, a no-mistakes ship brief tells the worker to append the status line `paused: awaiting compaction before validation` right after its implementation commit, then end its turn - before starting `no-mistakes axi run`. A brief rendered for a host without `config/idle-compact` configured skips this instruction entirely and tells the worker to drive `no-mistakes axi run` itself instead, since nothing would ever resume a worker that paused and waited here.
