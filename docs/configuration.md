...
claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, and omp are empirically verified for crewmate and secondmate launches; gemini, muse, rovo, and agy are verified for crewmate and scout launches only, and [README requirements](../README.md#requirements) own the set supported for the primary session.
...
agy is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because it has no primary supervision protocol; its semantic hooks require the narrow, task-scoped `.agents/hooks.json` exception owned by [its adapter reference](../.agents/skills/harness-adapters/references/harness/agy.md), and Firstmate never writes agy's machine-global configuration.
