...
When no setting or primary-runtime marker matches, the hard tmux fallback still creates or reuses the detached `firstmate` session, but `fm-spawn.sh` first announces that choice, the session name, the configuration paths, and `tmux attach -t firstmate` so the worker cannot land off-screen silently.
