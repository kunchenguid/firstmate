...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex uses bounded foreground checkpoints, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin (`session.idle` → `bin/fm-watch-arm.sh --restart` → `client.session.promptAsync` `watcher` operational input). For herdr-backed OpenCode primaries (`config/backend herdr` / `HERDR_ENV=1`) the watcher combines a bounded poll (`FM_POLL 15`, `SIGNAL_GRACE 30`, `FM_CHECK_INTERVAL 300`, `HEARTBEAT 600→7200`) with a `pane.agent_status_changed` push; the foreground must drain with `bin/fm-wake-drain.sh` at the top of every wake-handling turn - even when the prior turn was a user Q&A - and translate `done: PR <url>` per `AGENTS.md §9` (full `https://` URL, outcome, risk level) before the generation-bound `WAKE_ACK_REQUIRED` ack, under one live cycle, no `pkill -f bin/fm-watch.sh`, and daemon-owned `state/.afk` (see `docs/supervision-protocols/opencode.md` and `docs/watcher-continuity.md`).
