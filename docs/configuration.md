...
`bin/fm-branch-outcome.sh` owns durable outcome formats and processing receipts, `bin/fm-lease-lib.sh` owns task-mutation leases, and [Pi supervision architecture](pi-supervision-branch.md) owns extension session state.
[Watcher continuity](watcher-continuity.md) owns per-actor wake claims and their acknowledgement.
