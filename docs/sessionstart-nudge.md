...
Before printing, the nudge wrapper reads `state/.lock` and walks at most eight parents from its own pid in its own separate, hard-coded loop, independent of `bin/fm-lock.sh`'s own ownership decision (`bin/fm-session-lock-lib.sh`, which walks up to sixteen parents, can extend past a claude-named match to a still-more-ancestral one, and additionally accepts a lock held by a co-located session that provably started this one or was provably started by it) and of Pi's `lockOwnership()`.
A wrapper that misses one of those shapes prints a second nudge, which the once-per-session start is designed to absorb, so it is deliberately not kept in step with that owner.
