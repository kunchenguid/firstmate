...
Before printing, the nudge wrapper reads `state/.lock` and walks at most eight parents from its own pid in its own separate, hard-coded loop, independent of `bin/fm-lock.sh`'s fuller identity resolution (`fm_harness_ancestry_pid()` in `bin/fm-session-lock-lib.sh`, which walks deeper and can fall back to the session pid the harness publishes) and of Pi's `lockOwnership()`.
