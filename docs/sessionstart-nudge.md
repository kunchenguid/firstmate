...
| Harness | Tier | Tracked transport | Compatibility |
| --- | --- | --- | --- |
...
| Kimi | Uncovered | None. | Session-start and post-compaction delivery remain outside the current tracked transport family. |
...
It pins that exact output for a plain primary and a marked linked secondmate primary, then executes the tracked Claude `compact` hook command to prove `post-compact` contradiction output, consistent-record silence, mutation-free behavior, owned-lock delivery, and fail-open handling of an unknown wrapper mode.
...
Its separate `FM_PI_SESSIONSTART_RACE_LIVE_E2E=1` mode uses real Pi with an offline deterministic provider and barrier-controlled fresh-start and `/new` digests, proving the fresh first prompt plus immediate and completed-before-prompt controls make their first provider call with exactly one native startup context and no manual execution; when Calm is loaded, the fresh path also requires visible preflight feedback.
