...
The Guard Predicates section of [`turnend-guard.md`](turnend-guard.md#guard-predicates) owns marker validation, plain-checkout detection, required Firstmate-shaped paths, and the rule that every primary-scoped hook takes its scope from the checkout it runs from rather than from an inherited `FM_ROOT_OVERRIDE`.
