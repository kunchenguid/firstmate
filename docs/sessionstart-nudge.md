...
Codex 0.150.1's interactive TUI does not fire the tracked project `SessionStart` hook and is not covered by either tier.
...
| Codex interactive TUI | Uncovered | None. | Codex 0.150.1 does not fire the tracked project `SessionStart` hook in its interactive TUI; Firstmate ships no global hook, has no tracked compaction or re-emit channel, and does not claim instruction-refresh delivery for this surface. |
...
`tests/fm-codex-queue-wake.test.sh` proves that a native `CODEX_THREAD_ID` becomes an authoritative binding only when its home, lock generation, owner PID, and process identity agree, that compaction-compatible rebinding preserves a stable thread UUID, and that resume or restart may replace it authoritatively after the required bootstrap runs.
