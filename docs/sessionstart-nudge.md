...
| Cursor | Run | `.cursor/hooks.json` registers `sessionStart` and `preCompact`, both anchored through `$CURSOR_PROJECT_DIR` with a 180s timeout, invoking `bin/fm-sessionstart-cursor.sh`. | Cursor's payload has no `source` field, so the registration supplies `--source` itself. `sessionStart` returns the digest as `additional_context`, which Cursor injects into model context; `preCompact` cannot, so it stages instead. Project hooks load only when the workspace is launched with `--trust`. |
...
`tests/fm-cursor-primary.test.sh` proves the Cursor adapter's two routes over real processes: `sessionStart` emitting the whole digest as `additional_context` with a caller-supplied `--source`, `preCompact` staging it instead, silence in a child worktree, and the run wrapper standing down on the Cursor-delivered duplicate.
