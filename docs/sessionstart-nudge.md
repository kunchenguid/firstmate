...
| Nudge | Asks the agent to run the digest through the native adapter or the tracked session-start instruction. | Grok, OpenCode, Codex interactive TUI, Cursor interactive TUI, and run-tier sources routed to the nudge |
...
| Cursor interactive TUI | Nudge | The tracked `AGENTS.md` session-start instruction remains visible; `.cursor/hooks.json` registers only the `stop` and `preToolUse` supervision hooks. | Cursor has no tracked session-open adapter, so the agent-owned instruction remains its session-start floor. |
