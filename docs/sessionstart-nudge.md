...
| Codex exec | Run | `.codex/hooks.json` anchors to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and pipes the hook payload into the wrapper with a 180s timeout. | Native stdout context injection is supported under `codex exec`. Codex 0.154.0 discovers the tracked `PreCompact` registration but dispatches no `PreCompact`/`PostCompact` events around an exec `/compact`, so the stow gate activates only where a codex surface actually dispatches it. |
...
Claude Code and Codex both expose `PreCompact` and `PostCompact` hook events, and neither delivers either event's hook output to the model, so compaction coverage splits by what each half can actually do.
`bin/fm-precompact-stow.sh` is the tracked `PreCompact` registration for Claude (`.claude/settings.json`) and Codex (`.codex/hooks.json`); its header owns the full runner contract, and the tier-level facts are these.
Every compaction, manual or auto, performs stow instead of asking for it: the hook launches a detached headless agent that resumes the compacting session's context where the payload carries a session id and executes the stow skill's required startup-memory pass and knowledge sweep, then the compaction proceeds immediately - the hook never blocks, delays, or prints an instruction, so no compaction can wedge on it.
A completed pass inside `FM_PRECOMPACT_STOW_SECS` (default 3600) is not relaunched, a still-running pass is not stacked onto, and a failed, timed-out, or unavailable agent records no cooldown so the next compaction retries.
The runner stands down for a no-mistakes gate agent, an unmarked worktree, and a foreign-host payload through the same eligibility owners as the session-open wrappers, and its bookkeeping writes are guarded so a state directory that rejects writes loses bookkeeping only.
...
The same portable suite proves the pre-compact stow runner performs the pass itself on both harness modes with the marked prompt, the cooldown window and stale-timestamp re-arm, the auto path performing without ever blocking, no stacking onto a live pass, retry after a failed pass, the never-wedging unwritable-state path, and its silent stand-downs for a gate agent and an unmarked worktree.
...
It verifies context-preserving reopen sources for those adapters and context-reset delivery wherever their tracked TUI surface is reachable, drives a real `/compact` whose pre-compact runner performs a stubbed stow pass without blocking before asserting compact-source re-injection, and proves a codex exec reopen still fires the session-open channel after a real compaction.
