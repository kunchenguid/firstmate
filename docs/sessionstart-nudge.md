...
Claude Code and Codex both expose `PreCompact` and `PostCompact` hook events, and neither delivers either event's hook output to the model, so compaction coverage splits by what each half can actually enforce.
`bin/fm-precompact-stow.sh` is the tracked `PreCompact` registration for Claude (`.claude/settings.json`) and Codex (`.codex/hooks.json`); its header owns the full gate contract, and the tier-level facts are these.
...
The same portable suite proves the pre-compact stow gate's one-shot manual block and retry on both harness output modes, the never-blocking auto path, stale-marker re-arming, and its silent stand-downs for a gate agent and an unmarked worktree.
