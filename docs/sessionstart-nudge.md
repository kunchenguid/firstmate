...
| Antigravity | Nudge | `.agents/hooks.json` registers `PreInvocation`, anchored through `${ANTIGRAVITY_PROJECT_DIR:-$PWD}` with a 180s timeout, invoking `bin/fm-antigravity-hook.sh sessionstart`. | Supported in interactive `agy` TUI sessions launched with `--add-dir "$PWD"`. Without `--add-dir`, Antigravity defaults its workspace to `~/.gemini/antigravity-cli`, never loads `.agents/hooks.json`, and silently suppresses the session-start nudge and foreground watch. |
...
Antigravity's tracked `.agents/hooks.json` registers a `PreInvocation` hook that calls `bin/fm-antigravity-hook.sh sessionstart`.
Because Antigravity CLI does not automatically adopt the shell's current working directory as its workspace root, launching without `--add-dir "$PWD"` causes Antigravity to default to `~/.gemini/antigravity-cli`.
In that un-added state, Antigravity never discovers `.agents/hooks.json` and suppresses the session-start nudge.
The session appears conversational and interactive, but without the startup reminder to run `bin/fm-session-start.sh` and foreground watch (`bin/fm-watch.sh`), fleet events remain unmonitored.
