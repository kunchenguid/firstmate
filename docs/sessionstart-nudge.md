...
| Cursor | `.cursor/hooks.json` registers `sessionStart` invoking `bin/fm-sessionstart-nudge-cursor.sh`, which returns `additional_context` when the shared nudge prints. | Verified 2026-07-23 on cursor-agent 2026.07.20-8cc9c0b: wrapper returns `{}` when the home lock is already held, and returns `additional_context` containing the `fm-session-start.sh` instruction when not. |
