...
Codex's tracked interactive surfaces are version-dependent.
Codex 0.150.1 on native Windows fires the tracked project `SessionStart` hook, while the version-scoped evidence below still limits verified context-reset delivery to the surfaces it names.
...
| Codex exec | Run | `.codex/hooks.json` resolves the containing Git root, dispatches through `bin/fm-codex-hook.sh` on POSIX hosts and `bin/fm-codex-hook.cmd` on native Windows, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the run wrapper with a 180s timeout. | Native stdout context injection is supported under `codex exec`; the Windows command wrapper avoids `cmd.exe` parsing Bash syntax. |
| Codex interactive TUI | Startup transport present, reset coverage version-dependent | The same tracked project registration and platform dispatcher. | Codex 0.150.1 on native Windows fires project `SessionStart`; compaction and re-emit delivery remain unclaimed until the live context-reset guard records them. |
