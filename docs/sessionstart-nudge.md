...
Pi is the only adapter that injects a message or ephemeral context rather than hook stdout, so whatever it injects must carry operational provenance or the Ahoy skill would have to guess whether it was captain-authored.
...
Compaction shares that same generation ownership and cancellation - a stale or superseded compaction cannot deliver - but is deliberately different from startup delivery: a persistent full custom message would refill the context Pi just reduced and would itself enter a later summarization span, and an automatic retry may have no new `before_agent_start` to await.
...
The extension sizes each delivered refresh against a UTF-8 byte count, a valid upper bound on its token count since every token encodes at least one byte. When post-compaction provider usage is known, it subtracts that prior delivery back out of authoritative usage before replacing it, using a conservative maximum bytes-per-token divisor so the subtracted amount is a lower bound on tokens and base usage is never understated; immediately after compaction, when Pi reports usage as unknown, it estimates the rebuilt messages, effective system prompt, and active tool definitions with the same UTF-8 byte upper bound.
...
`tests/fm-sessionstart-instruction-refresh-live-e2e.test.sh` is the separate opt-in authenticated real-Pi guard for a post-start AGENTS.md update followed by compaction.
`tests/fm-pi-compaction-context.test.sh` is the credential-free real-Pi/local-provider guard for near-limit automatic and manual compaction, bounded large-digest delivery, hard-limit controls, current-instruction refresh, and independence from guard or watcher activity.
