...
Before printing, the nudge wrapper reads `state/.lock` and walks at most eight parents from its own pid in its own separate, hard-coded loop, independent of `bin/fm-lock.sh`'s binding-aware identity resolver (`fm_harness_ancestry_pid()` in `bin/fm-session-lock-lib.sh`; the remote route is documented in [`remote-secondmates.md`](remote-secondmates.md)) and of Pi's `lockOwnership()`.
