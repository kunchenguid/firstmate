...
| Codex exec | Run | `.codex/hooks.json` anchors to the spawn-set `FM_CODEX_HOOK_ROOT` when present (otherwise the hook process working directory), verifies a Firstmate-shaped hook-bearing root, and pipes the hook payload into the wrapper with a 180s timeout. | Native stdout context injection is supported under `codex exec`. |
