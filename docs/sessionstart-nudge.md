...
When post-compaction provider usage is known, the extension subtracts its prior ephemeral delivery from that authoritative usage before replacing it; immediately after compaction, when Pi reports usage as unknown, it estimates the rebuilt messages, effective system prompt, and active tool definitions with Pi's documented character heuristic.
