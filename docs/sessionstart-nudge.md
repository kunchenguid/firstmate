...
Before printing, the nudge wrapper reads `state/.lock` and walks at most eight parents using `bin/fm-session-lock-lib.sh`'s shared process-identity reader, independent of `bin/fm-lock.sh`'s ancestry walk (`fm_harness_ancestry_pid()` in that same file, which walks up to sixteen parents and can extend past a claude-named match to a still-more-ancestral one) and of Pi's `lockOwnership()`.
