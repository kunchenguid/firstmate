...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` through the native session-open adapter and gates its ordered digest into model context before the first turn. | Claude, `codex exec`, OMP, Pi / pi-signed, Cursor |
...
| OMP | Run | `.omp/extensions/fm-primary-turnend-guard.ts` maps `session_start` reasons to wrapper sources, starts one generation, and makes `before_agent_start` await that generation before returning a persistent custom message. | Project extension discovery and explicit `--extension` loading are supported; OMP remains a distinct adapter and does not use Pi's `session_compact` or provider-prerequisite path. |
...
OMP and Pi inject a persistent custom message rather than hook stdout, so what they inject carries operational provenance and cannot be mistaken for captain-authored input.
For `session_start`, each extension activates a monotonic-generation owner synchronously, starts the wrapper once, and makes `before_agent_start` await that same generation before returning the harness's persistent `message` result.
Replacement ignores stale generations; Pi additionally retires the matching process group on shutdown.
An eligible native failure returns the exact manual instruction rather than allowing native and manual startup to run concurrently.
...
The portable OMP extension suite proves session-start delivery through the public event surface together with the pre-tool and native `session_stop` guard paths.
