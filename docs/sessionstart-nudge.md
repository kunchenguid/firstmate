...
| Codex exec | Run on POSIX; explicit diagnostic on native Windows | `.codex/hooks.json` invokes `bin/fm-codex-hook.mjs session-start`; the adapter delegates to `fm-sessionstart-run.sh` on POSIX and emits `NATIVE_WINDOWS_RUNTIME` on Windows without claiming a lock or fleet mutation. | Native stdout context injection is supported under `codex exec`; the fleet bootstrap itself remains POSIX-only. |
