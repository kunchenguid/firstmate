...
| Codex | `.codex/hooks.json` resolves the wrapper through the [trusted-root boundary](architecture.md#codex-project-hook-root-anchoring) and uses the agent worktree only for primary-scope classification. | Native stdout context injection is supported. |
