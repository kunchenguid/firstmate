...
| Pi / pi-signed | Run | `.pi/extensions/fm-primary-turnend-guard.ts` maps `session_start` reasons `startup`, `new`, `resume`, and `fork` onto wrapper sources, refines startup-reason restored continuation, resume-selection, explicit-session, and fork CLI invocations to `resume` or `fork`, handles `session_compact` as the compaction equivalent, and injects the output with `pi.sendMessage`. Create-if-missing flags remain startup unless existing entries or a pre-process session header prove restoration. | The custom message reaches model context without racing an initial positional prompt. Pi's `reload` reason is deliberately unmapped, as it always was. |
