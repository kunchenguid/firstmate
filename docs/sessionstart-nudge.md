...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` in the hook and lets its ordered digest land in model context before the first turn. | Claude, `codex exec`, OMP, Pi / pi-signed, Cursor |
...
| OMP | Run | `.omp/extensions/fm-primary-turnend-guard.ts` maps `session_start` to the session-start wrapper and delivers the encoded output through `pi.sendMessage` with `display: false`. | `.omp/extensions` auto-load without a trust gate. |
| Pi / pi-signed | Run | `.pi/extensions/fm-primary-turnend-guard.ts` maps `session_start` reasons `startup`, `new`, `resume`, and `fork` onto wrapper sources, refines a Pi-reported `startup` to `resume` only when a continuation, resume-selection, or explicit-session flag accompanies a session header older than the current process, maps a fork flag to `fork`, handles `session_compact` as the compaction equivalent, and injects the output with `pi.sendMessage`; setup-created entries such as `--name` are not treated as a continuation. | Project trust is required before the extension auto-loads. |
