...
| Codex exec | Run | `.codex/hooks.json` resolves the wrapper through the [trusted-root boundary](architecture.md#codex-project-hook-root-anchoring), uses the agent worktree only for primary-scope classification, verifies a Firstmate-shaped hook-bearing root, and pipes the hook payload into the wrapper with a 180s timeout. | Native stdout context injection is supported under `codex exec`. |
