...
| Cursor | Run | `.cursor/hooks.json` registers `sessionStart` and `preCompact`, anchored through `$CURSOR_PROJECT_DIR` with 180s timeouts, invoking the Cursor adapters. | Session start returns the digest as `additional_context`; preCompact stages a compact-source digest that the next stop delivers as a typed follow-up because preCompact itself cannot inject context. Project hooks load only when the workspace is launched with `--trust`. |
...
Cursor's `preCompact` response can return only `user_message` and cannot inject additional context, so its adapter stages the compact-source digest in `state/.cursor-pending-context` for exactly-once session-start-typed delivery by the next stop hook.
...
`tests/fm-cursor-primary.test.sh` proves the Cursor adapters over real processes: `sessionStart` emits the whole digest as `additional_context` with a caller-supplied `--source`, stays silent in a child worktree, lets the run wrapper stand down on the Cursor-delivered duplicate, and carries staged preCompact context through the next stop exactly once.
