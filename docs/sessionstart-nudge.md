...
| OpenCode | Nudge | `.opencode/plugins/fm-primary-sessionstart-nudge.js` subscribes to `session.created`, runs once per session id, and calls `ctx.session.prompt` only when the wrapper prints a nudge. | Interactive TUI delivery is supported; headless `opencode run` is intentionally fail-open because the process can exit before the queued turn. That early exit is also why OpenCode cannot use the run tier. |
