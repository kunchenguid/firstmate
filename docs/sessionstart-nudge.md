...
| Codex exec | Run | `.codex/hooks.json` anchors to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, clears the verified identity markers of other harnesses at that Codex-owned boundary, and pipes the hook payload into the wrapper with a 180s timeout. | Native stdout context injection is supported under `codex exec`; a Codex process nested under another harness is classified from its nearer process ancestry rather than an inherited foreign marker. |
...
It also proves the Codex-owned SessionStart boundary clears every classifier-owned foreign harness marker while preserving the original hook payload.
