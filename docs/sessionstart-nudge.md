...
| Devin | `.devin/config.json` registers a project `SessionStart` hook and resolves the trusted project root through `DEVIN_PROJECT_DIR` with a Git-root fallback. | Native stdout context injection is supported. |
