...
Each step reads the parent through `fm_ps_ppid()` rather than `ps -o ppid=`, so the loop also resolves on a Cygwin-family host whose `ps` has no `-o` option, and a Windows-tagged holder - which is not in this process table at all - is deferred to `fm_session_lock_owned_by_self()`, the owner of harness identity, because a local ancestry comparison cannot answer for it.
