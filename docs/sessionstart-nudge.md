...
| Codex exec | Run | `.codex/hooks.json` execs the wrapper directly with a 180s timeout; the wrapper owns its own root anchoring. | Native stdout context injection is supported under `codex exec`. |
