...
Before an external invocation can open its runner-output staging file, core validates the effective state directory and its `state/procevent/` registry as same-user, non-link private directories with safe modes, judged on their resolved canonical form: the directory itself may not be a symlink, while an ancestor symlink (macOS resolves `/tmp` and `/var` through `/private`) is resolved rather than refused.
