...
These external-only checks refuse before a staging or capture write when they detect a post-registration link, an ownership or canonical-path substitution, or mode drift in the registry or destination, while the legacy four-argument built-in capture path retains its existing behavior.
