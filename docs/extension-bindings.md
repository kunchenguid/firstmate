...
Before an external invocation can open its runner-output staging file, core validates the effective state directory and its `state/procevent/` registry as same-user, non-link private directories with safe modes, refusing a symlink on the path rather than resolving it: the directory itself may not be a symlink, and a symlinked ancestor component is refused as well - the only exceptions are the operating system's own `/tmp` and `/var` aliases and their `/private` targets, so a macOS temp-directory home validates while any other ancestor redirection fails.
