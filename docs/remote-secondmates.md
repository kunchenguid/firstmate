...
A bare `<project>` is still accepted when this machine has a Git clone at that project's registered resolved path, whose configured origin is then read instead of being retyped.
