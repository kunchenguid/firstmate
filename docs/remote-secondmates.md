...
The primary passes `<harness> <model|default|-> <effort|default|->` explicitly, resolved for that mate's own id and using `default` when an axis has no parent pin, because neither `config/secondmate-harness` nor a per-secondmate `config/secondmate-harness.d/<id>` pin is inherited into a second mate's home and the files on that host belong to a different home; letting the far side re-resolve them would silently move the mate onto another runtime.
