...
A bare `<project>` is still accepted when this home's own registry resolves that name to a clone here - a `projects/<project>` directory, a `config/projects-root` sibling, or a `data/project-paths.json` alias - whose configured origin is then read instead of being retyped.
