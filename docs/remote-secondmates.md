...
Raw launch commands and an explicit `--env` path are not accepted for remote secondmates; [`docs/configuration.md`](configuration.md#worker-launch-environment-configlaunch-env-allowlist-and---env) owns the launch-environment contract those refusals belong to.
