...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse`, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, or `cursor-agent`. Cursor's user-local install at `~/.local/bin/cursor-agent` is accepted by the doctor even when it is absent from the login PATH, matching the local spawn path.
...
Cursor is refused as a remote secondmate harness until herdr + cursor is verified: cursor's composer and busy semantics are tmux-verified only, and remote secondmates run exclusively on herdr, so the exclusion is deliberate and fail-closed rather than an allowlist oversight.
