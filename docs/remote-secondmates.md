...
A mate provisioned before that rewrite keeps the supervising home's path in its existing `data/charter.md`, and relaunching it is not enough, because the launch re-serves that same file.
Re-run `bin/fm-remote-home-seed.sh` with the same id, host, root and home to republish the charter, then relaunch the mate.
