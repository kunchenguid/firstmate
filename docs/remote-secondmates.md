...
- Each worker runtime's own `/login`, and any keychain password prompt that login needs. OMP uses the remote account's own `HOME` and OMP store; no provider credential is copied from the primary.
