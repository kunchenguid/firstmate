...
Abandoned calls are cancelled when detected or bounded by their deadlines; the [job library header](../bin/fm-remote-job-lib.sh) owns cancellation, caller-identity checks, and the multiplexed-SSH detection limitation.
