...
Only a structured `report=data/....md` pointer offers a document; a bare path inside prose is a mention, so writing about a document - including one that belongs to another home, or one the mate has not created yet - never asks this channel to fetch it.
...
If the confined remote reader cannot deliver an offered document, the mate's line is mirrored with its original pointer and the adapter appends one keyed escalation naming the gap, the reader's own reason, and each missing document exactly once, instead of stalling the stream.
That gap is a durable, re-attemptable obligation rather than a verdict: the reader cannot tell a report that is still being written from one that will never exist, so the channel re-attempts every outstanding document on its own poll and on every later delta, and retires the escalation with a matching `resolved` line naming the local copy as soon as the document arrives.
A mirrored line carries the mate's own pointer until its document is delivered, and the same remote line still mirrors at most once whichever pointer form it was first mirrored under.
