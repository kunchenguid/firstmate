...
cmux is a session provider only, exactly like herdr and zellij (unlike the Orca backend, which bypasses treehouse and creates the task git worktree directly).
...
- **`--secondmate` spawns are refused** - no per-home container design (a herdr-style workspace-per-home split, Orca's persistent-session model, or similar) has been designed or verified for cmux yet.
