...
| Duplicate task check | `cmux list-windows --json --id-format uuids`, then `workspace list --window <id>` for every window, matching the home-scoped `.title` | cmux enforces NO title uniqueness for workspaces OR surfaces/tabs - verified live: two workspaces, and two surfaces within one workspace, all created successfully sharing one title. The adapter requires exactly zero matches before create and refuses ambiguous or unreadable global state instead of selecting the first match. |
...
| Workspace/surface id resolution | global per-window title lookup, then `cmux list-panes --workspace <id> --json --id-format uuids` (`.panes[0].selected_surface_id`) | Resolution requires exactly one title match across every window. A freshly created workspace already has exactly one surface, so no separate `new-surface` call is needed. `--id-format uuids` (or `both`) is required to get a bare `id` field in JSON; the default JSON shape returns only short `ref` strings like `"workspace:2"`. |
...
| Recorded-target recovery | global per-window lookup by the expected home-scoped title, then `list-panes` for the surface id | Title-based recovery never trusts a stored workspace uuid blindly because ids do not survive an app relaunch. It requires one global title match, so a workspace parked in a non-current window is recoverable while ambiguous matches fail closed. |
| Unrecorded list-live sweep | `cmux workspace list --json --id-format uuids` in the current window, filter this home's title prefix, then `list-panes` per match | The sweep prints the plain `fm-<id>` label after stripping the readable home tag and `FM_ROOT` hash, but it still cannot discover an unrecorded workspace parked in a non-current window. |
...
`workspace list --json` without `--window` is scoped to the current window only (verified live).
`fm_backend_cmux_window_of_workspace`, duplicate detection, creation-result validation, and recorded-target label recovery now enumerate `list-windows --json` and pass each window explicitly, so those operations see parked non-current workspaces and fail closed on unreadable or ambiguous state.
`fm_backend_cmux_list_live` still uses the unqualified current-window list, so unrecorded orphan discovery retains a non-current-window blind spot.
...
- **Windows cannot be closed over the control socket** - owned by "Closing the last workspace in a window" above. Teardown of a last-in-window task workspace therefore leaves that window a fresh default workspace rather than closing it.
- **The unrecorded list-live sweep remains current-window scoped.** Recorded-target recovery and duplicate detection enumerate every window, but `fm_backend_cmux_list_live` can still miss an unrecorded orphan parked in a non-current window.
