...
It recognizes either a row enclosed by matching `│`, `┃`, or plain ASCII `|` borders, or an unbordered row beginning with Cursor Agent's `→` glyph specifically, scanning forward and keeping the last match so earlier transcript content cannot outrank the bottom-anchored composer.
The generic agent glyphs `❯` and `›` are deliberately NOT accepted bare here: a dead shell running a starship/pure-style `❯` prompt must stay `unknown`, never a safe injection target.
A found composer row matching the shared busy regex (Cursor renders `ctrl+c to stop` on the composer row itself while generating) short-circuits to `empty` - the submit landed and a turn is running - so the Enter-retry loop never re-sends Enter into a live turn, mirroring `fm_tmux_composer_state`.
After that adapter-owned row finding, cmux delegates the shared `empty`/`pending`/`unknown` decision to `bin/fm-composer-lib.sh`; a bare shell prompt reads `unknown`, not empty.
