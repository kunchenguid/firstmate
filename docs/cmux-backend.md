...
GitHub account routing is activated in `bin/fm-spawn.sh` before backend dispatch, so cmux receives the common sanitized project context and does not own account selection.
`bash tests/fm-backend-cmux.test.sh` ended with every deterministic adapter assertion passing, and `bash tests/fm-spawn-dispatch-profile.test.sh` ended with all shared hostile-environment launch assertions passing.
`/Applications/cmux.app/Contents/Resources/bin/cmux --version` printed `cmux 0.62.2 (77) [6c203b514]`, below FirstMate's verified `0.64.17` minimum, so no live task lifecycle was attempted.
