...
Every execution is blinded: the gate copies the frozen input to a fresh opaque directory and file name and passes that path, so the evaluator never learns which record it is being asked to derive and an answer table keyed on the input path matches nothing.
...
Input dependence is executed too: the gate applies its own form- and length-preserving edit to one scalar of that golden input and requires the output to move, so an evaluator that reproduces its records without reading them is refused.
