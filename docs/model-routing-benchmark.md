...
| `promote-evaluate` | Only a six-of-six paired sweep with the predeclared margin, no blocker-class failure, and no regression below the fixed zero mean floor or beyond the declared zero-or-one loss bound is eligible; a retained void is complete only when the same role, candidate, and packet has a scored replacement, and the captain still gives the word. |
| `archive-verify` / `restore-drill` / `cleanup-gate` | A git bundle alone cannot be rejudged after cleanup; every evidence file except its sample's own top-level manifest is content-addressed beneath its sample directory, including the executable evaluator; every restored sample must carry a valid evaluator declaration and scored-input set, while bounded execution reruns a deterministic sample in an opaque confinement that must reproduce the genuine result; a sound declared JSON-value perturbation adds a confined differential dependence proof, while samples without one remain restorable with dependence recorded as unproven; cleanup is authorised only while the archive verifies both before and after the rerun. |
...
For a declared JSON-value perturbation, both restored copies pass through identical canonical preparation before the named scalar changes, and the gate verifies that no other JSON value differs.
Only two successful runs with identical output prove that an evaluator ignored its declared perturbable input.
A scored source, image, or other input without a sound form-preserving perturbation still receives the confined genuine replay and recorded-hash comparison, but the gate output and drill receipt mark its dependence as unproven.
