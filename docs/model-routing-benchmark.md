...
| `evaluator-verify` | A described evaluator is not a reproducible measurement. The environment lock, published score map, zero-weight validity gates, identical dry-runs, finite positive per-dimension mutation calibration, and bound reference captures from named calibration fixtures are all required. |
...
A host-side relay forwards that task’s generation-bound busy events and turn-end notifications to the existing supervisor readers; sibling state never enters the container.
Entrant containers retain stdin and allocate a terminal when launched from a pane, while evaluator replay stays noninteractive.
...
Its capture records describe named calibration fixtures, without candidate or packet assignments, so calibration can finish before any entrant runs.
Actual candidate captures are required after execution by the archive’s planned identity coverage and tree bindings.
