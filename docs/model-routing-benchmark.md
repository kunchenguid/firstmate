...
Every execution is blinded: the gate copies the frozen evaluator program and the frozen input into a fresh gate-owned scratch root under opaque file names and exposes only that root to the confinement, so the evaluator never learns which record it is being asked to derive, an answer table keyed on the input path matches nothing, and the frozen expected outputs and the `evaluator/execution.json` mapping that keys them are outside the cage entirely.
