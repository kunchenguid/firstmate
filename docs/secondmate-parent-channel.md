...
|---|---|---|
| Ship child PR ready | the child's `done: published; PR <url>` or `done: green-CI; PR <url> checks green` line; `pr=` in the child's record once registered | `bin/fm-inactive-reconcile.sh` on the next poll with the child's line; `bin/fm-pr-check.sh` at registration with the canonical URL |
