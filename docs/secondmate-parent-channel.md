...
The ledger delivery reads files only: it calls no harness, forge, or current-state reader, so the delivered line is identical for every harness and runtime backend.
When the terminal producer records its exact run id in a `[run=<id>]` tag, the ledger receipt copies that producer-owned identity; a line without the tag remains deliverable without a run id, and delivery never guesses from a later current-state read that may describe another run.
...
That producer-recorded run id lets the same scan's workerless run observation recognize an already-delivered completion instead of publishing it a second time.
...
It also covers the reboot case in which a terminal ledger line and its completed run are first seen together: the producer-recorded run id keeps that one delivery single, while a genuinely newer terminal run on the same child is still reported.
