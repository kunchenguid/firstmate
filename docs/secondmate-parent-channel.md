...
For a scout, delivering that factual parent outcome does not consume the separate local guarded-cleanup obligation: matching current-incarnation completion evidence keeps a main-owned cleanup check active in the secondmate home until teardown retires the child metadata.
...
`tests/fm-inactive-reconcile.test.sh` covers the ledger delivery against real ledgers with no harness: immediate done and failed delivery with note, PR, mode, posture, and report pointer, once-only delivery across polls, a line still being appended, the remote route, non-scout inactive reconciliation yielding to a terminal ledger, scout parent delivery retaining its local cleanup obligation, and the real watcher poll driving it.
