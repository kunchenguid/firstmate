...
| Devin | `.devin/config.json` PreToolUse hook matcher-scoped to `exec` and forwarding stdin through `--claude` | Blocks the tool call; stderr deny object, stdout empty. |
...
Every block and allow case runs through Codex-shaped stdin, Claude-shaped stdin, Devin-shaped stdin, Grok-shaped stdin, OpenCode-shaped CLI, and Pi-shaped CLI entry forms.
