...
| Codex | `.codex/hooks.json` PreToolUse hook that uses the spawn-set `FM_CODEX_HOOK_ROOT` when present (otherwise `pwd -P`), verifies that hook-bearing root, and forwards the payload | Blocks on exit 2 and displays stderr. |
