...
Every tracked harness registration for this hook also short-circuits on `FM_TASK_ID` before invoking the script at all, because a firstmate-repo task worktree inherits the registration from the primary checkout ([`tests/fm-primary-hook-scope.test.sh`](../tests/fm-primary-hook-scope.test.sh)).
