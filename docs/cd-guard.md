...
| Codex | `.codex/hooks.json` invokes `bin/fm-codex-hook.mjs cd`, which delegates to the shell transport on POSIX and imports the same policy owner directly on native Windows | Blocks on exit 2 and displays stderr. |
