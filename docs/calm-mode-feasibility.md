...
It covers persisted preference restoration across every session-start reason and a real restart, proves the working-ship presentation and Calm-off stock working row through a deterministic provider held active until capture, asserts no Calm status row, verifies operational messages remain exact ordinary user-role session entries and complete exports, and drives genuine 100 by 44, 160 by 36, and 180 by 44 terminal fixtures.
...
`tests/fm-pi-primary-types.test.sh` performs strict no-emit TypeScript checking against the installed Pi declarations; [runtime backend verification](verification/runtime-backends.md) records version-scoped results.
