...
Pi 0.81.1 was installed when Calm was first built, Pi 0.82.0 was the later broad reverification target, and Pi 0.83.0 was the current rendering-regression target.
The inspected Pi CHANGELOG shows no relevant presentation API introduced between 0.81.1 and 0.82.0, so those versions remain verification evidence rather than compatibility bounds.
...
The fix installs one idempotent presentation adapter, verified on Pi 0.81.1 through 0.83.0, on the exported `AssistantMessageComponent.updateContent` method.
...
The fix installs a separate idempotent presentation adapter, verified on Pi 0.81.1 through 0.83.0, on the exported `InteractiveMode.addMessageToChat` method.
The adapter probes for that exact method and, per the [compatibility contract](calm.md#pi-compatibility), degrades independently with a diagnostic rather than gating on a version number.
...
For `/export` and `/share`, Calm cycles tool expansion and restores its original value before Pi dispatches the command, while the ordinary transcript renderers are still active.
It then enables the stock tool renderers only for export generation and clears them afterward, so Pi 0.83's adjacent transcript-status coalescing cannot overwrite the built-in completion feedback and the final `Ctrl+O` state remains unchanged.
...
| Policy class | Pi transcript path | Calm result (verified on Pi 0.81.1 through 0.83.0) |
| --- | --- | --- |
...
| --- | --- | --- |
| Claude Code 2.1.223 | Feasible for response presentation only. | Project SessionStart hooks supply persisted-preference context, project custom commands provide `/calm`, and project output styles modify the response system prompt, while no inspected supported surface exposes a transcript-row renderer or transcript-wide redraw API. |
...
Its lifecycle fixture proves disposed-session events with a stale UI context are no-ops while the same handlers still present through a live context.
...
On Pi 0.83.0, the adjacent case waits for the initial dispatch's visible result without retrying the command, then asserts exactly one persisted and rendered captain answer and one processing result.
The path also asserts exact user-role operational envelopes in order, no replacement custom messages, zero operational transcript rows, and the two-row neighboring-assistant geometry for live, adjacent, and restart paths.
...
## 2026-08-05 Claude Calm adapter verification
...
$ bin/fm-doc-audience-check.sh
fm-doc-audience-check: ok surfaces=66 local_links=197
```
