...
It also proves the `config/calm-scene` candles scene: only an exact `candles` value selects it, the boat scene stays byte-for-byte the boat, every width down to one column fills exactly in two rows of one-colored candles and blank gaps, the chart scrolls one column per boat move, reflows, freezes and resumes, and Pi and the Raster packing paint it in their own greens and reds.
...
`tests/fm-calm-claude-mod-live-e2e.test.sh` is the opt-in credentialed guard in a real Claude Code TUI under tmux: flag off is a complete no-op with the preference already on, flag on with `config/calm-scene` set to `candles` shows the scrolling candles in the working row in the theme's green and red, flag on without that setting shows the moving boat, hides tool and operational rows, toggles and persists through `/calm`, and `claude --continue` restores the hidden rows.
`tests/fm-calm-pi-extension.test.sh` drives the same setting through the extension's session and run events, proving two exact-width green and red candle rows down to a single column, settle and resume, and the boat for an unknown, absent, or unreadable value.
