...
ok - Pi Calm hides thinking, tools, and operational bookkeeping without numbered replacements, tickers, blank geometry, or repaint residue, while preserving assistant commentary, history, exports, stock working row, and preference
ok - Pi Calm keeps assistant commentary exactly once, suppresses persisted thinking across reload and repeated Calm toggles, preserves final replies and message data, ignores every /calm argument, and restores a legacy persisted max as ordinary Calm on
ok - Pi operational follow-up E2E processes exact user-role notifications once while Calm hides current and adjacent rows, Calm off and absent render them, and restart preserves semantics
ok - Pi Calm native /skill:ahoy geometry keeps every collapsed thinking and tool block at zero height while preserving expansion, history, restart, and Calm-off rendering
