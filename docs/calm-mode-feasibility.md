...
| `fm_branch_processed` | Main-session custom tool in `fm-branch-supervision.ts` | Its call and result are always empty because acknowledgement is private bookkeeping; it never emits a captain-facing row in Calm, Calm-off, or stock export rendering. |
| `fm_branch_report` | Branch-session custom tool supplied directly to `createAgentSession` | It runs only in the headless supervision session and has no main-session `ToolExecutionComponent`; successful execution writes the outcome store, keeps routine results private, and opens only a private processing request for captain interventions, so the tool cannot emit a dump-shaped row in the captain's transcript. |
...
| `working-status` | `WorkingStatusIndicator`, or the Calm working-ship widget while Calm is active | Visible for genuine captain-authored runs. Calm off leaves Pi's stock row untouched; Calm on hides that row for a normal logical agent run and renders the working ship instead, while typed operational runs keep both the stock row and ship at zero height. |
