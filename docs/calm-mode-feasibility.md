...
`@oh-my-pi/pi-coding-agent` on this install also exports `InteractiveMode.addMessageToChat`, `UserMessageComponent`, `AssistantMessageComponent.updateContent`, and `ToolExecutionComponent.render` with `updateArgs`/`setArgsComplete`/`setExecutionStarted`/`updateResult`, which the shipped OMP Calm adapters probe for operational user rows, Calm-on thinking collapse (a presentation-layer filter of the message passed to `updateContent`), restored-transcript working-note classification, and zero-height operational (FIRSTMATE_OP / watcher / drain) tool rows. OMP keeps a tool row's name and arguments in private constructor fields and, on the live InteractiveMode path, calls `updateResult` with `{ ...result, isError }` (no `toolName`), so the tool-row adapter instead records `toolCallId -> { name, command }` from the unfiltered assistant message's `toolCall` blocks (carried by both the extension message events and `InteractiveMode.addMessageToChat`) and binds the row as soon as OMP hands it that `toolCallId` through `setArgsComplete`/`setExecutionStarted` (which the live tool start calls before the row is added) or `updateArgs`/`updateResult`; that hides a running row from its first render and classifies live and rebuilt rows identically whenever OMP still tracks the tool call or has completed it, including the watcher-arming tool `fm_watch_arm_omp`, which supplies no bash command. OMP's transcript rebuild does not call `setArgsComplete`/`setExecutionStarted`, so a row whose paired result is missing receives no `toolCallId` before it renders: a session resumed after the process died during an operational tool call can show that single rebuilt row at full height. A version-specific binding seam that would close that residual was considered and declined as net-negative coupling. It never inspects a row's rendered result output, so an ordinary row whose output merely mentions a Firstmate script stays visible. The shipped extension registers no `registerAssistantThinkingRenderer`: it would add supplemental UI rather than collapse thinking, and thinking collapse is already handled by the `updateContent` filter.
...
The working-ship / boat path was not probed for OMP and is not part of the OMP slice.
