...
[`calm.md`](calm.md) owns the current mid-turn visibility contract; `.pi/extensions/lib/fm-calm-assistant-layout.ts` owns signature validation.
...
| `genuine-agent-response` | Assistant text in `AssistantMessageComponent`, classified under the [mid-turn visibility contract](calm.md) | Visible, including the phase-marked pre-tool fixture. |
| `assistant-working-note` | Remaining mid-turn assistant text under the [mid-turn visibility contract](calm.md) | Removed from a shallow presentation copy before layout; the unsigned-only fixture occupies zero rows (verified on Pi 0.84.4). |
...
Its `mid-turn` entry point runs both fresh installation and an in-process upgrade from the previous adapter, then repeats reloads to check visibility and Calm-off renderer parity without changing message data.
...
They cover persisted preference restoration across every session-start reason and a real restart, check the working-ship presentation and Calm-off native working row through a delayed deterministic provider with a fixture-owned label set through Pi's public API, assert no Calm status row, verify operational messages remain exact ordinary user-role session entries and complete exports, and drive genuine 100 by 44, 160 by 36, and 180 by 44 terminal fixtures.
