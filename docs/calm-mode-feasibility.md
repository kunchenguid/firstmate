...
| OpenCode 1.18.30 (superseding the 1.17.18 server-lane row, which found no transcript-row renderer in events or tool hooks) | Best-effort first pass through the TUI plugin lane: `/calm` and the working-strip boat, without row hiding. | A file TUI plugin (`fm.calm`) persists the shared `config/calm` preference, listens to `session.status`, and paints the shared sprite in `session_prompt_right`; the supported slot surface cannot hide the stock spinner, verb, or transcript rows, which stay visible by design. |
