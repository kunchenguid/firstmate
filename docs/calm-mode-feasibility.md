...
Pi 0.84.4's public `@earendil-works/pi-ai` declarations expose `TextContent.textSignature` and the versioned `TextSignatureV1` shape, including the `commentary` and `final_answer` phases.
Calm accepts only a JSON v1 signature with a string `id` and one of those explicit phases when retaining a mid-turn text block.
It treats absent, legacy opaque, phase-less, malformed, unknown-version, and unknown-phase signatures as ordinary mid-turn narration without consulting prose, provider names, model names, or session topology.
...
| `genuine-agent-response` | Response-ending assistant text, plus a mid-turn `AssistantMessageComponent` text block carrying a valid Pi `TextSignatureV1` whose phase is `commentary` or `final_answer` | Visible, including explicitly phase-marked explanations immediately before a tool call. |
| `assistant-working-note` | Every other text block in an `AssistantMessageComponent` message the model did not end its response with, identified by its own `stopReason` of `toolUse`, or of `length` with tool calls present | Ordinary text blocks are removed from the shallow presentation copy before layout, so a `toolUse` message carrying only unsigned narration occupies zero rows (verified on Pi 0.84.4); valid phase-marked blocks beside them remain, malformed or unrecognized signatures do not widen visibility, and a still-streaming `pending` message is never filtered. |
...
The focused assistant-row fixture passes Pi's public text-signature type through the installed renderer and verifies captain-facing pre-tool explanation plus supervision/return guidance, unsigned narration beside it, phase-less and invalid signatures, response-ending text, Calm off, redraw, and every session-start reason including reload.
The native persisted-session fixture exercises the same mixed pre-tool message through Pi's real TUI, stock HTML export, Calm repaint, process restart, and Calm-off restoration without inspecting implementation source or mutating messages.
Together the suites exercise redraw of already-rendered tool, thinking, current operational-user, legacy synthetic, and phase-marked assistant rows, and cover every policy class.
They cover persisted preference restoration across every session-start reason and a real restart, prove the working-ship presentation and Calm-off stock `Working...` row through a delayed deterministic provider, assert no Calm status row, verify operational messages remain exact ordinary user-role session entries and complete exports, and drive genuine 100 by 44, 160 by 36, and 180 by 44 terminal fixtures.
