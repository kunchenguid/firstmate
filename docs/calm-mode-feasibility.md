...
| Cursor Agent | Preference and conversation-policy only; no TUI row hiding or working-ship widget. | Project hooks expose `sessionStart` `additional_context`, `stop` follow-ups, and `preToolUse` decisions, not a transcript-row renderer or working-indicator replacement; shipped as `/calm` plus session-open policy injection over the shared `config/calm` file. [`calm.md`](calm.md#cursor) owns the captain-facing gap. |
