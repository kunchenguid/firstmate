...
Firstmate never sets that flag in any project or user settings; enabling it is each captain's own explicit opt-in, and without that exact value the mod is a complete no-op even if Claude Code's rollout flag loads the module: there is no `/calm` command, no preference, scene, or transcript read, no timer, and every drawing stays exactly as Claude Code draws it, whatever `config/calm` says.
