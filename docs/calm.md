...
Calm hides collapsed thinking labels, mid-turn assistant working notes, the shells for Pi's seven main-session built-in tools, the `fm_watch_arm_pi` and `fm_branch_outcomes` tool shells, and canonically classified Firstmate operational user rows.
...
Calm registers no tool and changes presentation only, so input delivery, ordering, tool execution, model context, session storage, diagnostics, and `/export` and `/share` operation remain unchanged.
...
The collapsed-thinking, operational-user-row, and built-in-tool-row presentation adapters probe the exact Pi API seam they patch when Calm loads.
...
Calm's built-in tool presentation (`bash`, `read`, `edit`, `write`, `grep`, `find`, `ls`) registers no tool of its own, so it never competes for one of those names.
Pi resolves same-name tool registrations by first registration in extension load order and discards the loser's complete definition, execution included, so a Calm-owned built-in name would silently disable another extension's override of that tool.
Calm presents those seven rows through the method Pi itself uses to hand a tool row its renderers, `InteractiveMode.getRegisteredToolDefinition`, and owns that seam for those names only.
An extension that overrides one of them keeps its own tool and its own rendering: the row hides while Calm is on, and returns to that extension's rendering when Calm is off.
The toggle's redraw reaches rows already on screen, so turning Calm on collapses them and turning it off restores them.
Every other tool row, including arbitrary custom tools, is left exactly as its owner drew it.
...
`.pi/extensions/lib/fm-calm-visibility.ts` owns the visibility policy, `.pi/extensions/lib/fm-calm-operational-user-layout.ts` owns the zero-height operational-user row adapter, `.pi/extensions/lib/fm-calm-tool-row-layout.ts` owns the built-in tool-row presentation, and `.pi/extensions/lib/fm-calm-working-ship.ts` owns Pi's animated working presentation over the sprite geometry both harnesses share in `.claude/mods/firstmate-calm/lib/fm-calm-working-ship-sprite.ts`.
