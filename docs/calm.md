...
Calm hides collapsed thinking labels, ordinary mid-turn assistant working notes, the shells for the Pi built-in tool names Calm owns, the `fm_watch_arm_pi` and `fm_branch_outcomes` tool shells, and canonically classified Firstmate operational user rows.
A mid-turn assistant message is one the model did not end its response with, identified by that message's own `stopReason` of `toolUse`, or of `length` with tool calls present.
Within one of those messages, Calm keeps any text block whose valid Pi `TextSignatureV1` explicitly marks the `commentary` or `final_answer` phase, so captain-facing explanations remain visible when a tool call immediately follows them.
Unsigned text and text carrying a legacy opaque, phase-less, malformed, unknown-version, or unknown-phase signature remain ordinary working narration and hide.
The distinction uses Pi's versioned text metadata rather than wording, provider names, model names, or session layout.
Text that is still streaming is never hidden, because suppressing it would also stop a genuine reply from streaming, so ordinary working narration is briefly visible before its row collapses.
