...
The family follows the `theme` setting by its prefix, `dark` or `light`, is re-read when the theme changes, and falls back to the dark set for `auto` and custom themes; the Pi extension keeps its standard ANSI blue and yellow.
