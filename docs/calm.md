...
Calm hides collapsed thinking labels, short single-line mid-turn assistant working notes, the shells for the Pi built-in tool names Calm owns, the `fm_watch_arm_pi` and `fm_branch_outcomes` tool shells, and canonically classified Firstmate operational user rows.
...
Short single-line mid-turn text is hidden when its trimmed length is below `CALM_PRESERVE_MIN_CHARS` (240), while text containing a newline or meeting that threshold is preserved as substantive captain-facing content.
Hiding short working notes removes the narration a model emits alongside its tool calls, while substantive content and the genuine reply that ends a response stay visible.
...
`.pi/extensions/lib/fm-calm-visibility.ts` owns the visibility policy, `.pi/extensions/lib/fm-calm-preservation.ts` shares the substantive mid-turn text rule with Claude Code Calm, `.pi/extensions/lib/fm-calm-operational-user-layout.ts` owns the zero-height operational-user row adapter, and `.pi/extensions/lib/fm-calm-working-ship.ts` owns Pi's animated working presentation over the sprite geometry both harnesses share in `.claude/mods/firstmate-calm/lib/fm-calm-working-ship-sprite.ts`.
