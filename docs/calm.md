...
Run [`bin/fm-omp-calm-install.sh`](../bin/fm-omp-calm-install.sh) to install or refresh the standalone copy; its header owns installation paths, overrides, unloading, and legacy-copy migration, including refusal to overwrite an existing backup.
The standalone installation survives moving or removing the checkout, and must be refreshed after updating Firstmate.
The package lives outside `.omp/extensions/` to avoid loading both a project copy and the global plugin in the same session.
