...
The accumulated steps reset when a new agent run, session, Calm disable, or shutdown begins, so each completed response owns only its run's display-only step list.
