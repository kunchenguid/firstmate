...
Calm hides assistant thinking blocks on its own (independent of Pi's hide-thinking / Ctrl+T setting), mid-turn assistant working notes, the shells for the Pi built-in tool names Calm owns, the `fm_watch_arm_pi` and `fm_branch_outcomes` tool shells, and canonically classified Firstmate operational user rows.
