...
While Calm is active, Pi's built-in `Working...` activity remains visible and Calm installs no custom working widget or separate status row.
While Calm is off, Pi's stock working row is also left exactly as Pi renders it.
