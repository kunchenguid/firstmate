...
The same GitHub-only live read also supplies the head branch and that branch's own repository, which the registration path uses to report every other open pull request on the branch; the GitLab path reads no head branch at all.
