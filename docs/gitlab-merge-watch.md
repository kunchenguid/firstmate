...
Consequently, a GitLab task cannot use `bin/fm-teardown.sh`'s forge-based squash proof, which requires a recorded `pr_head=`; its header owns the remaining landed-work proofs.
`bin/fm-review-diff.sh` still resolves the head from the remote when none is recorded.
