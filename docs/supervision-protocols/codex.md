...
When that turn is allowed to end, `bin/fm-codex-idle-continuity.sh` keeps process-event reconciliation running until the Codex process exits, and queues an actionable close back into the same thread.
