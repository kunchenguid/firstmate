...
1. Drain first with `bin/fm-wake-drain.sh`.
   Handle every emitted wake, reconcile every `UNREAD STATUS` line and open decision, then run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`; until then the work remains durable for idempotent re-handling after interruption.
...
3. An actionable close wakes you as a follow-up turn carrying the `watcher` operational kind.
   ...
   Reconcile any `UNREAD STATUS` lines before acknowledging the drain.
