...
4. The captain keeps control while the hook is parked.
   ...
   The private supersession records are `state/.cursor-park-claim`, `state/.cursor-park-claim.lock`, and `state/.cursor-park-owner`, with transient `state/.cursor-park-claim.commit.*` files used for atomic output commits.
   Staged compaction context is atomically claimed from its owner-scoped `state/.cursor-pending-context.<session-owner-pid>` file before delivery.
