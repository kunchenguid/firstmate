...
4. The captain keeps control while the hook is parked.
   ...
   The private supersession records are `state/.cursor-park-claim`, `state/.cursor-park-claim.lock`, `state/.cursor-park-owner`, and `state/.cursor-park-owner.lock`.
   Staged compaction context is serialized by `state/.cursor-pending-context.lock`.
