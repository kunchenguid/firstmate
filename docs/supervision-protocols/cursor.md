...
7. The turn-end guard (`bin/fm-turnend-guard.sh`, via the cursor shim) remains the final backstop.
   ...
   If the arm fails or closes without a typed actionable reason while supervision is still needed, the shim emits the loud guard banner so hook registration and startup remain diagnosable.
    ...
    That silent `{}` is refused while durable evidence of an interrupted park or an undelivered wake exists: the arm's signal traps write `state/.cursor-hook-interrupted` (timestamp, signal, and arm origin) whenever the hook-owned arm is torn down while supervising, and a non-empty `state/.wake-queue` means a wake landed without a live hook to deliver it.
