...
7. The turn-end guard (`bin/fm-turnend-guard.sh`, via the cursor shim) remains the final backstop.
   ...
   If the arm fails or closes without a typed actionable reason while supervision is still needed, the shim emits the loud guard banner so hook registration and startup remain diagnosable.
    ...
    That silent `{}` is refused while durable evidence of an interrupted park or an undelivered wake exists: the Cursor stop-hook adapter arms with the interrupted-park marker requested (`FM_WATCH_ARM_INTERRUPT_MARKER`), so the arm's signal traps write `state/.hook-arm-interrupted` (timestamp, signal, and arm origin) whenever this hook-owned arm is torn down while supervising, and a non-empty `state/.wake-queue` means a wake landed without a live hook to deliver it.
    In either case the shim emits a loud drain-and-reconcile follow-up (`bin/fm-wake-drain.sh`) instead of `{}`, so the next stop is a recovery vehicle rather than a blind end; `bin/fm-wake-drain.sh` is the single owner of clearing the marker on any successful drain.
   The `loop_limit: null` registration leaves continuation budget to the shim, which keeps separate session-scoped chain records under `state/` (keys `session`, `total`, `fail`, `wake`, `reason`, and one `wake_reason` line per reason already seen in this chain): consecutive loud arm-failure follow-ups (`FM_CURSOR_TURNEND_BLOCK_BUDGET`, default 3) and consecutive repeats of the same canonical wake reason (`FM_CURSOR_WAKE_CHAIN_BUDGET`, default 5), with a diagnostic follow-up when that chain's ceiling is reached.
   The ceilings and the unified bound are PER CHAIN, not a global cap on continuation: the ceiling diagnostic resets the chain, so the next stop starts a fresh chain, and every stop while supervision is still needed re-evaluates and may emit a follow-up (the guard never ends a needed turn blind).
   Dynamic stale and check diagnostics stay attached to their stable wake identity, so changing diagnostic details do not evade the current chain's re-fire bounds; a new distinct wake reason is progress and does not add to `total` (it resets the same-reason repeat count but keeps the chain), while failures and re-firing any previously seen reason advance the current chain's count.
   ...
   One unified bound sits over both branches within each chain: alternating actionable and failed closes cannot evade `FM_CURSOR_TURNEND_BLOCK_BUDGET + FM_CURSOR_WAKE_CHAIN_BUDGET` in a single chain, and reaching the chain's ceiling emits a diagnostic follow-up instead of `{}` and resets the chain for the next stop.
