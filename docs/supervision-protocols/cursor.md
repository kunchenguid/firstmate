...
1. Drain first with `bin/fm-wake-drain.sh`.
   After handling all emitted wakes, reading all `UNREAD STATUS` entries, and reconciling open decisions, run the exact `--ack-through` command printed as `WAKE_ACK_REQUIRED`; until then the work remains durable for idempotent re-handling after interruption.
...
3. An actionable close wakes you as a follow-up turn carrying the `watcher` operational kind.
   ...
   Do not invent a wake from an attach-status line alone; drain and act only on real wake records, the drain's `OPEN DECISIONS` or `UNREAD STATUS` sections, or a real watcher reason line.
