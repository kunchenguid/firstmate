...
2. Source `__FM_TRELLO_MODE_ENV__` first when Trello mode is active, then `__FM_X_MODE_ENV__` when X mode is active.
3. First cycle: arm with Grok's tracked background tool, as its own call:

   `run_terminal_command` with `background: true` on:
   `[ -f __FM_TRELLO_MODE_ENV_SH__ ] && . __FM_TRELLO_MODE_ENV_SH__; [ -f __FM_X_MODE_ENV_SH__ ] && . __FM_X_MODE_ENV_SH__; exec bin/fm-watch-arm.sh`
