...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call while supervision is still needed (work in flight, X-mode polling, or a registered custom check).
