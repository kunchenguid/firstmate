...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight, the local routines check is installed, or Relay still needs polling.
