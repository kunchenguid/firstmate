...
6. Failure or missing cycle only: `watcher: FAILED ...` means supervision is down; fix and re-arm.
   `watcher: cycle closed with a delivered wake ...` is neither: another arm owned that cycle and already reported its wake, so do not act on it as a wake, and re-arm because no live cycle remains.
