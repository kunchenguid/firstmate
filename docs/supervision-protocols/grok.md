...
1. Drain first with `/bin/bash bin/fm-wake-drain.sh`.
...
3. First cycle: arm with Grok's tracked background tool, as its own call:
   ...
   `[ -f __FM_X_MODE_ENV_SH__ ] && . __FM_X_MODE_ENV_SH__; exec /bin/bash bin/fm-watch-arm.sh`
...
1. Run `/bin/bash bin/fm-wake-drain.sh` first.
...
4. Ordinary wake: re-arm the next cycle with the same background `/bin/bash bin/fm-watch-arm.sh` call if work remains in flight or Relay still needs polling.
