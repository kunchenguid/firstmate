...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight, X mode still needs polling, or the Telegram topic board is active.
