...
2. Source `__FM_X_MODE_ENV__` first only when the session-start instructions include the generated secure-mode Relay cadence file.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight or a generated Relay poll still needs polling.
