...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if actively progressing work remains in flight or X mode still needs polling.
