...
2. Source `__FM_X_MODE_ENV__` first when that file exists.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call whenever this home still needs supervision, which covers work in flight, Relay polling, an armed inbound message channel, and any registered event source.
