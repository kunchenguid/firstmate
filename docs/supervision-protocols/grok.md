...
3. Source `__FM_PORTAL_MODE_ENV__` first when the portal connector is active.
4. First cycle: arm with Grok's tracked background tool, as its own call:

   `run_terminal_command` with `background: true` on:
   `[ -f __FM_X_MODE_ENV_SH__ ] && . __FM_X_MODE_ENV_SH__; [ -f __FM_PORTAL_MODE_ENV_SH__ ] && . __FM_PORTAL_MODE_ENV_SH__; exec bin/fm-watch-arm.sh`
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/fm-watch-arm.sh` call if work remains in flight or either messaging connector still needs polling.
