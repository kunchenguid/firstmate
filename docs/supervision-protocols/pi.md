...
Regression example, kept verbatim as this contract's test case and never condensed or summarized away: the listed outcome `[seq 41] claude-mod: implementation complete, checks green, ready for review and merge approval` is finished requested work, so the correct processing turn tells the captain the Claude mod is done, gives the PR's recorded URL, and asks for their review and merge word before calling `fm_branch_processed`; answering `Captain, shipshape.` fails that turn even though the anchor already shows the substance.
