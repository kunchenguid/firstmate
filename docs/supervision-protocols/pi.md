...
A no-change heartbeat outcome explicitly reported with `task=fleet` and `silent=true` is delivered silently with no rendered note, while every other routine outcome returns as an appended note that leads with ⛵ then the dim outcome text when visible under Pi's current presentation mode.
