...
6. After an actionable child close, the extension rechecks session-lock ownership and verifies one successor before it delivers a hidden `firstmate-watcher` custom message with `triggerTurn: true` and `deliverAs: "followUp"`; its bounded fallback is defined in `docs/watcher-continuity.md`.
