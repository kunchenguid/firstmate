...
4. If the extension says no live session holds the lock, run `/bin/bash bin/fm-session-start.sh` to reclaim the session lock, then call `fm_watch_arm_pi` again.
