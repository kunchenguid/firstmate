...
A no-change heartbeat outcome explicitly reported with `task=fleet` and `silent=true` is delivered silently with no rendered note, and a later declared-external-wait recheck is explicitly recorded silent only when its condition, consequence, and required action are unchanged after the first notice, while every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
