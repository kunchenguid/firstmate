...
When that outcome or MAIN's review or merge ask involves a PR, the visible reply carries the PR's full `https://` URL as `AGENTS.md` section 9 requires; "awaiting your merge call" alone, leaning on the dim anchor for the URL, fails the same way.
Because Calm hides assistant text emitted in the same step as a tool call as a working note, put that captain-facing text in the final visible assistant reply of the processing turn, after the `fm_branch_processed` call, and keep it to the URL plus a short outcome or ask rather than a re-paste of the anchor.
