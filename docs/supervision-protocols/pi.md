...
Outcome visibility, including silent no-change heartbeats and unchanged declared-external-wait rechecks after the first notice, follows the [two-stage noise filter](../pi-supervision-branch.md#two-stage-noise-filter); every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
