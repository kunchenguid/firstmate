...
The supervision branch is default-on: when this session owns the fleet lock, the watcher extension offers eligible task-local rows and selected heartbeat reviews to the branch while main-only rows remain queued for this conversation.
[Away mode](../pi-supervision-branch.md#away-mode) owns posture-dependent branch dispatch, including quiet mode and legacy daemon flags.
...
This conversation still receives every other fleet-wide or unresolvable wake, wakes the branch declines or cannot handle, and every watcher-failure alarm, so the arm and repair contract above is unchanged.
