...
6. After an actionable child close, the extension captures the matching durable event, rechecks session-lock ownership, and verifies one successor before delivery.
   If Pi is busy, the extension waits for the next idle boundary, revalidates that the event and its source are still current, and only then starts the handling turn; its bounded fallback is defined in `docs/watcher-continuity.md`.
