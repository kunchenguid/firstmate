...
Regression example, kept verbatim as this contract's test case and never condensed away: `[seq 41] claude-mod: implementation complete, ready for review` requires telling the captain the Claude mod is done and asking for review; `Captain, shipshape.` fails even though the anchor shows the substance.
