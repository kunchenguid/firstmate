...
6. A Pi session replacement such as `/new` stops the previous child and retry state, keeps that child as the occupied arm slot until its close event, then restores one process-exit cleanup listener and allows `fm_watch_arm_pi` to arm the new session in the same process.
