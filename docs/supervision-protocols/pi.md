...
Initial verification on 2026-07-09 used Pi 0.80.5, an isolated `PI_CODING_AGENT_DIR`, an isolated `FM_HOME`, and the dedicated tmux socket `fm-pi-q6-lab`.
...
The extension keeps a one-shot process `exit` listener as the fail-closed cleanup path because Pi 0.80.5 did not reliably emit `session_shutdown` for `/quit`; when `session_shutdown` does run, it still stops the owned watcher group and removes that listener.
...
The live primary then repeated the same failure under arm child 4: the nested `fm-watch.sh` bash chain remained present for 32 minutes while the watcher beacon aged to 1,922 seconds, and `fm_watch_arm_pi` still reported healthy from its non-null child reference.
Terminating that exact task-owned process chain and re-arming started arm child 5, advanced the beacon again, and returned `bin/fm-guard.sh` to silent healthy output.
The repeat isolates two cooperating root causes rather than a transient process exit: an unbounded native event capability or wait call can wedge the watcher inside nested bash processes, and inherited pipe holders can suppress the arm child's `close` event indefinitely, while the old Pi extension mistakes the surviving JavaScript child reference for health without checking the shared lock identity and beacon predicate.
