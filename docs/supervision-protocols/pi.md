...
6. After an actionable child close, the extension rechecks session-lock ownership and verifies one successor before it delivers the follow-up wake; its bounded fallback is defined in `docs/watcher-continuity.md`.
   It revalidates the durable queue before delivery and coalesces later closes behind one pending or active handling turn, then suppresses an obsolete follow-up when the real drain has already consumed every queued event.
