...
A no-change review explicitly reported with `silent=true` (a fleet-wide heartbeat or a task-local wake that changed nothing and took no action) is delivered silently with no rendered note, while every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
