...
While the away-posture record `state/.afk-contract` exists the branch takes every row instead, this conversation receives no wakes and no processing request, main's standing authority relocates to the branch under the record, and a failure the branch cannot take becomes the away alarm (`bin/fm-afk-alarm.sh`) rather than a wake here; the first run boundary after the record is archived presents what accumulated (docs/pi-supervision-branch.md "Postures").
...
While attended, this conversation still receives every other fleet-wide or unresolvable wake, the branch's wakes when it is unavailable, and every watcher-failure alarm regardless, so the arm and repair contract above is unchanged.
