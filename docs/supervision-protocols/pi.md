...
A no-change heartbeat outcome explicitly reported with `task=fleet` and `silent=true` is delivered silently with no rendered note, while every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text, except a routine summary unchanged from the same task's most recent routine outcome, which is filtered before delivery and never reaches this conversation ([docs/pi-supervision-branch.md](../pi-supervision-branch.md) owns the filter).
