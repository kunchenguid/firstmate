...
A routine outcome reported with `silent=true` is delivered silently with no rendered note, covering a no-change heartbeat (`task=fleet`) and a no-new-fact follow-up after a terminal captain-facing outcome; every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
...
That request is hidden operational input, not captain chat: do not quote its instructions, and never emit a truncated https:// URL.
Process each listed outcome: answer or escalate a decision, act on a blocker or failure, or record that no further action is needed, then call the `fm_branch_processed` tool with the highest sequence the request listed, exactly once.
A ready-for-review, merge, or other outcome that `AGENTS.md` section 9 says needs a reply requires MAIN's own reply with the full PR URL; that is not a no-further-action case.
...
A merge ask with no URL that leans on the dim anchor, or a captain-facing reply that truncates an https:// URL, violates `AGENTS.md` section 9.
