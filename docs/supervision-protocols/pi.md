...
Only a successful acknowledgement closes the outcome; a refusal means nothing was acknowledged, so inspect its diagnostic and use only the sequence listed in the current processing request, never a sequence inferred from an outcomes read.
An unrelated, empty, or paraphrased answer leaves the outcome open, and the current unprocessed sequence set is presented again at the next run boundary and at session start until it is acknowledged.
