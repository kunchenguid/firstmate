...
An ordinary task-scoped routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text, while a fleet-wide routine outcome is delivered silently with no rendered note.
