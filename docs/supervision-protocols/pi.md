...
A no-change heartbeat outcome explicitly reported with `task=fleet` and `silent=true` is delivered silently with no rendered note, while every other current routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text.
The delivery-time freshness treatment for a superseded routine or captain outcome is owned by `docs/pi-supervision-branch.md`.
