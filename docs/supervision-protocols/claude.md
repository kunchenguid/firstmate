...
9. The continuity PreToolUse gate allows wake drain, watcher arm recovery, and fail-closed teardown, and refuses only other `bin/fm-*.sh` fleet commands while actively progressing work is in flight and no identity-matched live watcher holds the home lock.
10. The turn-end guard remains the final backstop and is not replaced by this command gate; its shared predicate requires checkpoints only for actively progressing work.
