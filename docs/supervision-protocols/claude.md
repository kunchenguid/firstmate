...
9. The continuity PreToolUse gate allows wake drain, watcher arm recovery, fail-closed teardown, and the allowlisted wake-handling tools while a wake is queued or was recently drained.
   It still refuses unrelated `bin/fm-*.sh` fleet commands while tasks are in flight and no identity-matched live watcher holds the home lock.
