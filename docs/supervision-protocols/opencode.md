...
4. After an actionable child close, the plugin rechecks session-lock ownership and verifies one singleton successor before any `client.session.promptAsync` delivery; `docs/watcher-continuity.md` owns the queue-revalidated fallback and coalescing boundary.
