...
3. The plugin listens for `session.idle`, spawns `bin/fm-watch-arm.sh` without awaiting it in the idle handler, and calls `client.session.promptAsync` when the child exits with an actionable watcher reason or failure.
