...
While away mode is active the plugin parks ordinary supervision instead: an idle event arms nothing, and a cycle still live when away mode begins delivers no wake prompt and records an owner-scoped resume that re-arms after return, because the away-mode daemon owns the watcher (`docs/watcher-continuity.md` "Away-mode parking").
