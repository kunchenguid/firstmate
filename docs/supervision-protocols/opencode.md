...
3. The plugin listens for `session.idle`, spawns `/bin/bash bin/fm-watch-arm.sh --restart` without awaiting it in the idle handler, and owns every later successor launch.
...
7. Failure or missing cycle only: if the plugin reports a watcher failure, drain queued wakes, inspect the failure text, and use `/bin/bash bin/fm-watch-arm.sh` manually only as a short recovery probe.
