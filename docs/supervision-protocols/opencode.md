...
3. The plugin listens for `session.execution.succeeded` and `session.execution.failed`, spawns `bin/fm-watch-arm.sh --restart` without awaiting it in the turn-end handler, and owns every later successor launch.
