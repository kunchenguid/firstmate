...
4. After an actionable child close, the plugin rechecks session-lock ownership and verifies one singleton successor before it calls `client.session.promptAsync` on the most recently idle captain session (a subagent child's idle resolves to its top-level ancestor; detail in the OpenCode harness reference); its bounded fallback is defined in `docs/watcher-continuity.md`.
