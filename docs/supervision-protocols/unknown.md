...
Ordinary wake: drain, handle all emitted wakes, and reconcile open decisions and unread status lines (presented records are consumed at presentation; a still-open decision re-appears on every drain until answered), then repeat that verified wait while supervision is still required.
