...
2. Away (the record exists and no daemon runs): the host hands each wake to a headless away session that runs the supervision branch's contract under the record, and you are parked.
   ...
   After the return, `supervision-host: the captain returned while the away session was handling this wake ...` means that wake is already handled but its outcomes, on the `supervision-host: outcome ...` lines after it (all of them in `bin/fm-branch-outcome.sh list`), missed the return brief: relay them to the captain, then drain and acknowledge as usual.
