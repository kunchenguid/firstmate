...
An `FM_ROOT_OVERRIDE` that names some other checkout is an inherited parent environment rather than an operator-directed one, so `fm_primary_scope_resolve_env` (`bin/fm-primary-scope-lib.sh`) discards it together with `FM_HOME` and the `FM_*_OVERRIDE` paths that travel with it, in this process and in the helpers the hook spawns.
The state dir argument is still the effective `FM_HOME`/`FM_STATE_OVERRIDE` one, which is how the split root/home remote model keeps working; discarding the foreign set is what stops root and state coming from different sessions.
A child crew or scout worktree that inherited the parent primary's environment therefore stays inert, and a secondmate home guards its own state instead of the parent's.
