...
A due pause counts actionable at most once per cadence window - a per-id `state/.actionable-paused-resurfaced-<id>` marker (mirroring `bin/fm-watch.sh`'s own once-per-window `.paused-resurfaced-<key>`) is touched the first time it fires, so an unchanged pause that has crossed the threshold does not force a fresh checkpoint on every single subsequent turn end.
