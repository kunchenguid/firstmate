...
The one loud attended fail-open opens on either of two verified conditions, and in both cases only when the block budget is exhausted and a final check finds neither a healthy watcher nor an automatic continuation.
The first is an exhausted failure episode: the auto-arm recorded an exhausted failure and its one notice is already consumed.
The second is a verified ABSENT auto-arm: its epoch ledger is missing entirely, or its last entry is older than one event epoch.
The second condition exists because the first cannot cover its own precondition.
