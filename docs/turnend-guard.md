...
Only another auto-arm is a competing single-flight owner of that lock, so an arm that reaches its claim inside one of the guard's own bounded `terminal-check` or `structural-failure` sections waits it out (`FM_CLAUDE_AUTOARM_OWNER_WAIT_MS`, default 2000ms) instead of standing down; a merely slow but healthy arm is never converted into an inert one.
