...
The refusal banner says that to the model on the paths where priming actually ran: it still reports the missing generation claim, but instead of inferring that nothing is recovering it names the primed detached start and asks for a live watcher to be verified before the Stop hook is treated as broken.
A refusal that did not prime - no `bin/fm-claude-stop-autoarm.sh` to run, or a claim that opened inside the race window - keeps the original wording, because there the absent claim really is the whole story.
