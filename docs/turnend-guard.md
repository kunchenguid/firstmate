...
That ownership proof is `fm_extension_owns_supervision` in `bin/fm-wake-lib.sh`, which accepts either the Pi pair (`fm_pi_extension_owns_supervision`) or the omp pair (`fm_omp_extension_owns_supervision`): both primary extensions of one family must have state markers naming their current on-disk builds and the live session PID recorded in `state/.lock`; omp never inherits the Pi tolerance because its proof is keyed on its own two files and markers.
An omp extension running in a nested worker publishes the recorded session PID only when it finds that PID in its own process ancestry; a missing, malformed, stale, or foreign lock cannot authorize publication.
On fresh startup, the turn-end extension retries publication after the startup supervisor succeeds, using the same ancestry check against the newly acquired lock.
