...
The same early ownership check makes passive harness guards stand down before they update any supervision state.
Ownership is the shared `fm_session_lock_owned_by_self` verdict in `bin/fm-session-lock-lib.sh`: it accepts the recorded harness ancestry, a trusted same-session Claude id beside a live anchor, or the recorded Codex thread identity while its writer lock is held.
