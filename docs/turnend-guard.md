...
- Codex registers a `Stop` hook in `.codex/hooks.json` that execs `bin/fm-turnend-guard.sh` directly and passes the original payload to the shared guard; the script owns its own root anchoring.
