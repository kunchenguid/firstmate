...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the spawn-set `FM_CODEX_HOOK_ROOT` when present (otherwise the hook process working directory), verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard.
