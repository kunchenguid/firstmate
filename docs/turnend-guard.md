...
- Codex registers two `Stop` hooks in `.codex/hooks.json`, each anchored to the hook process working directory and each verifying a Firstmate-shaped hook-bearing root before running: the shared guard, and `bin/fm-codex-stop-autoarm.sh` with `"async": true` and `timeout: 28800`.
