...
The mid-turn pull guard uses the model-aware supervision verdict and repair policy described below, while the turn-end guard keeps the PID-strict watcher predicate.
...
`bin/fm-guard.sh`, the pull guard, instead uses the model-aware `fm_watcher_supervision_verdict` from the same library, because it fires mid-turn when the auto-arm model runs no watcher at all.
Under the auto-arm model an aged or absent beacon is an ordinary mid-turn state and never produces the passive watcher-down banner in a writable session.
When tasks are in flight and the auto-arm ledger has stopped advancing past its fixed generous bound, the writable guard attempts a home-scoped tracked-owner re-arm, stays silent on positive evidence that the successor beat or delivered a wake, and prints one concise failure warning at most once per episode only when repair produces no such evidence.
Process-event or Relay-only supervision need cannot trigger that mutation, and a read-only guard cannot repair, so it retains the advisory watcher-down banner without changing episode state.
...
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the persistent-model fresh-leftover-beacon negative control, the auto-arm model's removal of the passive stale-beacon banner, fresh-ledger quiet-turn silence, successful and immediately actionable self-heal silence, once-per-episode repair failure, read-only advisory behavior, and the extension model's live-watcher path, ownership-qualified fresh hand-off, held-lock failures, independently broken ownership signals, stale-beacon alarm, queued-wake warning, and Pi and pi-signed harness routing.
