...
The claim is the ledger entry itself: the epoch sequence in `state/.claude-autoarm-epoch` is a monotonic claim generation, line 1 records the generation and each new claim's authenticated foreground session owner, and line 2 records the claiming process's mandatory pid-identity (`fm_autoarm_claim_open` and `fm_autoarm_claim_next` in `bin/fm-wake-lib.sh` own the contract).
...
[`watcher-continuity.md`](watcher-continuity.md#ownership) owns the Stop hook's session-lock proof, including authenticated Claude daemon delivery outside the foreground process ancestry.
...
`tests/fm-session-lock-ancestry.test.sh` and `tests/fm-claude-stop-autoarm.test.sh` cover Claude daemon ownership bridging, including wrong-root and wrong-pid negatives, zombie-owner recovery visibility, the auto-arm's failed-claim marker path, and lockless publication races against recovery reset and ledger advance.
