...
- OpenCode V2 subscribes to `session.idle` in `.opencode/plugins/fm-primary-turnend-guard.js`, lets the watcher coordinator act first, and calls `ctx.session.prompt` once when the guard returns 2.
