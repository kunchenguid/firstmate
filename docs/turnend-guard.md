...
An eligible hook that cannot recover the session lock or publish its generation claim commits a fresh `failed` or `failed-suppressed` epoch through the same progression before it asks Claude for another Stop-owned retry.
