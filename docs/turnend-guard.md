...
Away and quiet mode are the one place the turn-end guard accepts a different supervisor: in either mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`), a live identity-matched daemon with a fresh beacon satisfies that boundary in place of a watcher process holding the lock.
The `state/.afk` flag alone never does, because that flag answers the posture question and outlives the daemon under every signal; every check across the tree that means "the daemon has this covered" goes through `fm_afk_daemon_owns_supervision` for the same reason, so a standing flag with a dead daemon restores ordinary supervision and turn-end blocking at the very next turn.
...
Detection is still only as prompt as the next turn boundary, which is why `bin/fm-afk-daemon-run.sh` keeps the daemon alive in its tracked terminal rather than waiting for a turn to notice the gap: in away mode turns exist because the daemon injects them, and a home out of quota cannot complete one at all.
