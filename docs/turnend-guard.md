...
The claim is the ledger entry itself: the epoch sequence in `state/.claude-autoarm-epoch` is a monotonic claim generation, line 1 records the generation and its authenticated foreground session owner, and line 2 records the claiming process's mandatory pid-identity (`fm_autoarm_claim_open` and `fm_autoarm_claim_next` in `bin/fm-wake-lib.sh` own the contract).
A claim is open while its bound foreground session still owns `state/.lock`, its outcome is `arming`, its owner pid is alive, its recorded identity successfully recomputes and matches that pid, and it is not stuck - stuck meaning the entry and the watcher beacon are both older than the guard grace, which proves the owner hung mid-arm (a healthy hours-long foregrounded cycle keeps the beacon beating, and every arming phase with no watcher is bounded in seconds).
...
Claude daemon-delivered Stop hooks can run outside the foreground session's process ancestry, so `fm_session_lock_owned_by_self` reads boundary-preserved argv through the declared Node toolchain and macOS system process interface and accepts that bridge only when `daemon` and `run` are its first command tokens plus exactly one `--spawned-by` argument whose unambiguous JSON names label `claude`, the live foreground pid in `state/.lock`, and the same resolved project root.
...
Every auto-arm session-acquisition lease wait is bounded, including stale-lock recovery, generation claims, terminal writes and resets, and independent failure publication; when a required lease remains held, the hook publishes an immutable failure record and episode marker bound to the exact authenticated or unchanged stale session owner so a successor ignores the obsolete episode.
...
The synchronous guard consumes the newest independent failed epoch for the current fence only after the main claim ledger still matches its recorded pre-attempt snapshot on a final recheck, so either a later claim-ledger transition or recovery reset supersedes the failure while repeated failures against one unchanged baseline still advance the bounded progression.
...
In Claude mode, positive watcher recovery clears the block budget, independent failure epoch, failure notice, and attended alarm together under the existing budget lock before either hook reports ordinary recovery.
...
Its alarm marker is bound to the failure reset and authenticated foreground session owner, so a predecessor session's alarm cannot suppress the successor's auto-arm or attended fail-open progression.
...
`tests/fm-session-lock-ancestry.test.sh` and `tests/fm-claude-stop-autoarm.test.sh` cover Claude daemon `--spawned-by` ownership bridging, including wrong-root and wrong-pid negatives, the auto-arm's failed-claim marker path, and lockless publication races against recovery reset and ledger advance.
