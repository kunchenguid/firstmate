...
Their separate owners are [`arm-pretool-check.md`](arm-pretool-check.md), [`cd-guard.md`](cd-guard.md), [`subagent-guard.md`](subagent-guard.md), and, for the ordinary-worker PR-merge guard, [`architecture.md`](architecture.md#project-modes-are-explicit).
...
- That crew wake path is dormant today: an ordinary Kimi worker or scout spawn refuses before endpoint creation because Kimi has no verified blocking shell-tool hook for the worker PR-merge guard (`bin/fm-worker-guard-install.sh`), so no crew reaches the registration step.
...
`tests/fm-kimi-harness.test.sh` covers the separate Kimi crew hook's format preservation, idempotence, and refusal cases, plus the ordinary-worker spawn refusal that keeps that hook dormant.
