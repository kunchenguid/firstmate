...
The short transition boundary serializes claim-ledger writes, failure publication, and positive recovery reset, and a later participant retires an identity-verified holder that stalls beyond its bounded lease before continuing the transaction.
...
For upgrade compatibility, the guard accepts a baseline-less legacy failure only until a terminal main-ledger transition carries a demonstrably later update timestamp.
