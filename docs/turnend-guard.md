...
Under the Stop auto-arm model (Claude, Codex, and Cursor) a beacon fresh within grace is healthy even with no live watcher process.
A stale beacon is still healthy while `fm_autoarm_midturn_healthy` in `bin/fm-wake-lib.sh` proves the running primary's own auto-arm rewake explains the mid-turn gap: the rewake is bound to the current recovery generation and live session-lock owner, and no later watcher beacon or exhausted-failure marker supersedes it, because that session's turn-end will re-arm.
Each Stop auto-arm keeps its own ledger, so `fm_autoarm_prefix_for_primary` in the same library decides which one that proof reads; an explicit `FM_AUTOARM_PREFIX` wins, and reading the other harness's ledger would alarm on healthy supervision rather than merely miss a claim.
...
- Codex registers two `Stop` hooks in `.codex/hooks.json`, each anchored to the hook process working directory and each verifying a Firstmate-shaped hook-bearing root before running: `bin/fm-turnend-guard.sh` with no flag, and `bin/fm-codex-stop-autoarm.sh` with `"async": true` and `timeout: 28800`.
  Codex honors `async` on a Stop hook - the turn completes while the hook is still running, and the hook survives across later turns - but it discards an async hook's exit status and stderr, so the Codex auto-arm cannot use Claude's exit-2 rewake and delivers its wake with `codex queue --thread <session_id>` instead (`bin/fm-codex-stop-autoarm.sh` owns that contract).
  Codex starts a fresh async hook instance per Stop with no deduplication, exactly as Claude does, which is why the auto-arm keeps its own single-flight generation ledger under `state/.codex-autoarm-epoch`.
  The guard resolves the Codex cooperative mode from the running harness rather than from a flag, so its registered command stays byte-identical and its Codex hook approval survives an upgrade; only the genuinely new auto-arm entry needs approving.
  That mode is the same Stop-owned auto-arm cooperation `--claude` uses, reading the Codex ledger rather than Claude's, so the turn that handles a wake does not block on the watcher its own Stop hook is bringing back up.
  An explicit `--codex` is still accepted and still wins where it is passed; a harness that cannot be resolved keeps the pre-existing default mode, so an inconclusive answer never allows a stop the guard would have blocked.
  Unlike `--claude` it keeps the shared `stop_hook_active` loop guard, because a Codex wake arrives as an ordinary queued user message rather than a stop-hook continuation, so one forced continuation per turn still bounds the guard without depending on a harness-side consecutive-block override.
...
In the guard's default mode, a true value lets the second stop finish after one forced continuation.
