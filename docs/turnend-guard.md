...
Positive recovery advances a monotonic reset fence before clearing the episode, and every independent failure record plus notice election is bound to the fence it observed, so a lockless publisher crossing that reset remains historical rather than recreating or suppressing current failure state.
