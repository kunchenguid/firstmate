...
Two further bounds keep Claude mode from blocking a session indefinitely, and neither changes the ordinary contract above.
The first is the lock-refused stand-down.
A session whose harness ancestry does not include the pid in `state/.lock` is read-only by the session-start contract, so it may not arm, steer, or repair supervision, and the Stop auto-arm exits without claiming on that same evidence (`fm_session_lock_owned_by_self` in `bin/fm-session-lock-lib.sh`).
Waiting for a claim there waits forever: on 2026-09-12 such a session was blocked on every turn for about thirty turns because the epoch ledger never advanced and the budget above never counted.
When `state/.lock` names a live harness process outside this session's ancestry, the guard blocks exactly once per (session, lock owner) with the owner pid and the supervision need, so the model reports it to the captain, and then allows every later stop while that owner holds the lock, printing the same evidence as a `systemMessage` each time.
The owner it blocked for is recorded as `refused_owner=` in `state/.turnend-claude-blocks`; a different live owner gets one new block, and the ordinary path drops the record.
A dead owner, a missing lock, and a malformed lock are deliberately not this case: those remain recoverable by the auto-arm's guarded lock recovery or by the model, so they keep the ordinary blocking path.
A healthy watcher still allows silently before any of this is consulted.
The second is the block ceiling, `FM_CLAUDE_TURNEND_BLOCK_CEILING` (default 6, always raised above the budget when configured at or below it).
The budget only advances when the auto-arm ledger does, so a Stop hook that never runs at all - unregistered, parked, or crashing before its claim - would otherwise re-block every turn forever.
Every blocked stop increments `blocks=` in the budget file; once the count reaches the ceiling, the stop is allowed with a loud `systemMessage` naming the supervision need and the count, and every later stop stays allowed and loud until positive watcher recovery clears the budget file, which resets the ceiling with it.
The ceiling never consumes the verified-failure attended alarm and never shortens the budget's bounded progression.
The maximum a Claude session can be held at the turn boundary is therefore the ceiling's blocked stops, and a lock-refused session is held for exactly one.
