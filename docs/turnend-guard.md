...
When work, a process-event source, a registered custom check, or Relay polling needs supervision at that boundary and neither an accepted supervision proof nor the read-only foreign-owner exception below applies, the harness integration must either block the turn end or force one bounded follow-up that uses the recovery instruction from the emitted session-start protocol.
The mid-turn pull warning uses the model-aware supervision verdict described below, while the turn-end guard keeps the PID-strict watcher predicate as its primary proof.
Two other supervision owners satisfy that boundary in place of a watcher process holding the lock, and each still requires a fresh watcher beacon.
While `state/.afk` exists, in either mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`), the daemon owns supervision, so a live identity-matched daemon satisfies the boundary.
On an extension-model primary a proven live extension pair owns its own tear-down-and-respawn hand-off, so a genuinely unheld lock satisfies the boundary there.
...
The turn-end guard needs that strict check under the auto-arm model because it fires at the turn boundary, where the auto-arm is bringing a fresh watcher up for the upcoming idle period, and it cooperates with that arm rather than trusting a beacon left by the cycle that just ended.
That rationale is specific to the auto-arm: an extension-model primary arms on the watcher's own actionable close rather than at the turn boundary, so there is no arm in progress to cooperate with, and the guard accepts the hand-off described below instead.
...
An extension-model primary hands off the same way with no away posture at all, so the turn-end guard accepts the `fm_extension_owns_supervision` proof defined above on that path too.
It requires all three of a beacon fresh within grace, `fm_watcher_lock_unheld`, and that ownership proof, which keeps a lock that IS held but unhealthy on the strict predicate instead - the one place this branch is tighter than the away-mode branch, whose live daemon outweighs a dead watcher lock its exited child left behind.
It uses the flat `FM_GUARD_GRACE` default rather than the poll-derived grace, so this guard and `bin/fm-guard.sh` return the same verdict for the same instant; before that, a turn settling inside a hand-off blocked as blind while the pull warning called the identical state healthy.
The branch deliberately does not consult `fm_supervision_model`: the ownership proof is a structural fact about this home, while harness detection reads the guard child's own ancestry, which is evidence this predicate does not need.
Every other refusal is unchanged, so a stale beacon, an unloaded or version-drifted extension, a half-loaded pair, and an exited session all still block.
