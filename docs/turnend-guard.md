...
When none of those proofs appears, the guard records a typed `failed-unclaimed` episode under the auto-arm owner lock before consuming the block budget.
This covers a recovery path that exits inert before it can claim the home or record its own exhausted attempt, including a future session-ownership regression.
That guard-recorded episode is marked by its own `state/.claude-autoarm-failure-unclaimed` record rather than the auto-arm's `state/.claude-autoarm-failure-notified` record, because nothing was reported to the operator when the guard wrote it.
An auto-arm that claims late and then genuinely fails in the same episode therefore still delivers its one loud failure notice; either marker opens the failure episode for budget accounting, and positive recovery clears both.
Only another auto-arm is a competing single-flight owner of that lock, so an arm that reaches its claim inside one of the guard's own bounded `terminal-check` or `structural-failure` sections - or inside the window between any holder's acquire and its role stamp - waits it out (`FM_CLAUDE_AUTOARM_OWNER_WAIT_MS`, default 2000ms) instead of standing down; only a role that positively names another arm stands a firing down, so a merely slow but healthy arm is never converted into an inert one.
Because that wait means a late arm can only claim after the structural record is written, the guard re-checks recovery for a bounded window after writing it (`FM_CLAUDE_AUTOARM_STRUCTURAL_RECHECK_MS`, default 500ms) and releases the Stop - clearing the episode when the watcher is healthy - instead of blocking with a claim that recovery is not under way.
Each later unclaimed Stop advances that episode once, so the guard re-blocks at most `FM_CLAUDE_TURNEND_BLOCK_BUDGET` consecutive times (default 3, below Claude's 8-block override) before permitting one loud attended fail-open.
An arm that turns inert only after a real failure stops publishing epochs of its own, so the guard also advances the episode when the recorded epoch is the one this session's budget already spent on an earlier Stop; an epoch published during the current Stop is left to its own producer.
In Claude mode, positive watcher recovery clears the block budget, both failure-episode markers, and the attended alarm together under the existing budget lock before either hook reports ordinary recovery.
The one loud attended fail-open is available only when either the auto-arm has recorded an exhausted failure or the guard has recorded structurally unclaimed recovery, the block budget is exhausted, and a final check finds neither a healthy watcher nor an automatic continuation.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, a structurally inert ownership path that blocks exactly three times before the attended fail-open, the same bound for an arm that turns inert after one real failure, preservation of a late-claiming arm's one loud failure notice across a guard-recorded structural episode, post-alarm continuation suppression, positive recovery reset, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
