...
Outside away mode, the guard spawns the home-scoped watcher singleton detached (own process group, `nohup`, stdio detached) and verifies it within `FM_CLAUDE_GUARD_ARM_CONFIRM` seconds (default 5) against the same strict watcher predicate.
