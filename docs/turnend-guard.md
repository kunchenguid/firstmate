...
A session whose home lock is held by a different live verified harness is read-only and may not arm a watcher or repair supervision, which is also why the Stop-owned auto-arm stays inert there and the auto-arm epoch never moves.
Neither bound above can be reached in that state: the consecutive-block count only advances when the epoch moves, a frozen ledger pins it at its initial value, and the attended fail-open additionally needs a verified failure episode that a session with no auto-arm never produces.
Such a session therefore takes its own terminal path: at most `FM_CLAUDE_TURNEND_READONLY_ADVISORIES` (default 2) read-only advisories naming the live lock holder, counted once per block instead of once per epoch, and then the guard stands down for the rest of that session.
A contended budget lock stands the turn down as well, because this path exists so a session that cannot act is never wedged.
The advisory replaces this guard's own repair banner in every mode, since no mode may demand a repair the session is forbidden to perform.
Grok, OpenCode, Pi, and omp forward that stderr after their own generic turn-end prefix and keep their existing per-turn follow-up bound, so only `--claude` needs the advisory count.
Cursor never reaches this path at all: `bin/fm-turnend-guard-cursor.sh` already stands down before its park whenever another live harness holds the lock.
A missing, malformed, or dead-owner lock is deliberately not this state: that session may claim or reclaim the lock, so it keeps the ordinary repair path and its unbounded blocking.
The advisories share the session's consecutive-block count rather than keeping a second one, so a session that later acquires the lock resumes an already-advanced progression.
That is the deliberate trade: total nagging stays bounded per session even if lock ownership flaps, at the cost of reaching an already-verified failure episode's attended fail-open a turn or two earlier than a fresh count would.
