...
An eligible hook bounds its session-acquisition lease wait when claiming a generation, and if that lease stays held it publishes an immutable failure record and episode marker bound to the exact authenticated or unchanged stale session owner so a successor ignores the obsolete episode.
