...
One frozen-epoch shape cannot be repaired from inside the session at all: `state/.lock` names a live harness process outside the session's own ancestry (`fm_session_lock_owned_by_self` in `bin/fm-session-lock-lib.sh`).
That session is read-only by the session-start contract, may not arm, steer, or repair supervision, and its own Stop auto-arm exits without claiming, so its budget is charged against a ledger that can never advance and re-blocking past the budget only wedges a session that cannot act (about thirty blocked turns on 2026-09-12).
Every re-block on that path names the live owner and tells the model it is read-only and must report to the captain.
Once the budget is spent and no verified failure episode exists, the guard releases the turn with an ownership-unresolved escalation: a `systemMessage` naming the live owner, the supervision need, and the beacon age, stating that this is not a completion attestation and that the lock-holding session's own guard is responsible.
The release consumes no attended alarm, writes no failure marker, and leaves the ledger and the owning session's state untouched; every later stop under the same live owner is released the same way, loudly.
A dead owner, a missing lock, and a malformed lock are deliberately not this case, because the session can still take the lock, so they keep the keep-blocking contract above; away mode never releases; and a healthy watcher still allows silently and clears the episode.
The residual is an owner that is alive but never ends a turn: supervision is then genuinely down and neither session blocks, which the release reports rather than hides, because wedging the read-only session would not supervise anything either.
