...
Outside away mode, the guard spawns the home-scoped watcher singleton detached (own process group, `nohup`, stdio detached), verifies it within `FM_CLAUDE_GUARD_ARM_CONFIRM` seconds (default 5) against the same strict watcher predicate, and on success blocks exactly once with the restored-cycle banner so one handling turn drains what the lapse queued; the following healthy stop is then allowed normally and is what resets the failure episode.
...
That restored-cycle block spends one `FM_CLAUDE_TURNEND_BLOCK_BUDGET` slot exactly like a repair re-block, accounted before the arm runs; it deliberately does not reset the failure episode, because that reset deletes the block-budget file and the failure notice together, which would erase the progression bounding it and leave the terminal fail-open permanently unverifiable.
Once the budget is spent the last resort stands down instead of arming again, but read the reach of that bound precisely, because it is narrower than per session.
`budget_account_current_epoch` advances the count only when the ledger's `epoch=N` differs from the one already recorded in the block-budget file, so a slot is spent per auto-arm generation rather than per stop.
An auto-arm whose generation never advances - the hook disabled, or the script erroring before its first ledger write - is therefore not bounded by it, and in that state the guard can re-arm and re-block past Claude's 8-block override with no attended alarm ever fired.
That limit comes from the per-epoch budget itself and applies equally to the ordinary repair re-block, so the last-resort arm does not introduce it.
When that last resort cannot verify a cycle, or has no budget slot left to spend, the guard re-blocks with the repair banner instead, within the same `FM_CLAUDE_TURNEND_BLOCK_BUDGET` (default 3, intended to stay below Claude's 8-block override) that bounds every Claude-mode block for one auto-arm generation, under the frozen-generation caveat above.
