...
A stale or absent beacon is still healthy while `fm_autoarm_midturn_healthy` in `bin/fm-wake-lib.sh` proves a Claude auto-arm generation explains the mid-turn gap: an open arming claim, or a rewake outcome with no exhausted-failure marker and a live session lock, because that session's turn-end will re-arm.
