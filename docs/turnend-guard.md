...
Claude daemon-delivered Stop hooks can run outside the foreground session's process ancestry, so `fm_session_lock_owned_by_self` accepts that bridge only when the daemon's `--spawned-by` metadata names label `claude`, the live foreground pid in `state/.lock`, and the same resolved project root.
...
An eligible hook that cannot recover the session lock or publish its generation claim commits a fresh `failed` or `failed-suppressed` epoch and marker through a bounded handoff that does not acquire the claim mutex before it asks Claude for another Stop-owned retry.
