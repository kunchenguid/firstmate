...
An eligible hook that cannot recover the session lock or publish its generation claim atomically creates an immutable record keyed by its pre-attempt claim signature under `state/.claude-autoarm-failure-epochs/` without acquiring or rewriting the claim mutex and ledger, then uses an independent transition boundary and atomic marker creation to elect at most one notice before it asks Claude for another Stop-owned retry.
The synchronous guard consumes that independent failed epoch only while the main claim ledger still matches its recorded pre-attempt snapshot, so any later claim-ledger transition supersedes the failure, and positive recovery clears it with the rest of the failure episode.
