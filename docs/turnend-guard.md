...
An eligible hook that cannot recover the session lock or publish its generation claim atomically creates an immutable record keyed by its pre-attempt claim signature under `state/.claude-autoarm-failure-epochs/` without acquiring or rewriting the claim mutex and ledger, then uses an identity-fenced short transition boundary and atomic marker creation to elect at most one notice before it asks Claude for another Stop-owned retry.
The short transition boundary serializes claim-ledger writes, failure publication, and positive recovery reset, and a later participant either retires an identity-verified holder that stalls beyond its bounded lease or discards an identity-mismatched stale lock without signalling its reused pid.
