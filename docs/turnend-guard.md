...
The mid-turn pull warning uses the model-aware supervision verdict described below.
The turn-end guard still requires the PID-strict live-holder check, then accepts the same extension-handoff proof (`fm_extension_handoff_healthy`) that verdict already uses, so a Pi or omp tear-down-and-respawn window is not a blind-turn alarm.
...
Otherwise it calls `fm_watcher_healthy <state-dir> <watch-path> [grace-seconds] [home]` from `bin/fm-wake-lib.sh`, the same PID-strict identity-matched lock and fresh-beacon check used by `bin/fm-watch-arm.sh`: a stale beacon blocks even when a watcher pid is live, and a held lock whose pid is missing, dead, or identity-mismatched still blocks even when the leftover beacon is fresh.
When that strict check fails, the turn-end guard also accepts `fm_extension_handoff_healthy` from the same library: a genuinely unheld lock, a beacon still inside grace, and a live Pi or omp session that owns continuity.
That is the extension's own tear-down-and-respawn window, which the pull warning already tolerated and which the turn-end guard used to sample as "supervision off" (a leftover beat of a few seconds with `lock_after=pid:none`).
The Claude Stop path still needs the strict live-holder check because it fires at the turn boundary, where the auto-arm is bringing a fresh watcher up for the upcoming idle period, and it cooperates with that arm rather than trusting a beacon left by the cycle that just ended.
