...
As the root argument it passes the running hook script's own checkout, never a raw `FM_ROOT_OVERRIDE`.
An `FM_ROOT_OVERRIDE` that resolves to some other checkout is an inherited parent environment rather than an operator-directed one, so `fm_primary_scope_resolve_env` (`bin/fm-primary-scope-lib.sh`) discards it together with `FM_HOME` and the `FM_*_OVERRIDE` paths that travel with it, in this process and in the helpers the hook spawns.
An override that cannot be resolved at all proves nothing and is left intact, so a broken environment stays with the scope check rather than silently rebinding the hook to its own checkout's state.
The state dir argument is still the effective `FM_HOME`/`FM_STATE_OVERRIDE` one, which is how the split root/home remote model keeps working; discarding the proven-foreign set is what stops root and state coming from different sessions.
A child crew or scout worktree that inherited the parent primary's environment therefore stays inert, and a secondmate home guards its own state rather than the parent's.
That holds for leaks carrying `FM_ROOT_OVERRIDE`, which is how every launch path that sets a child's `FM_HOME` sets it; a hypothetical `FM_HOME`-only leak is indistinguishable from the split root/home model and is not rejected here.
...
`tests/fm-cursor-primary.test.sh` covers the Cursor park end to end over real processes with no harness installed: each tracked Claude-shaped entrypoint standing down on a Cursor payload, both follow-up sources, the bounded repair nag and its reset, the nested loop bounds, supersession, away-mode and lock-ownership inertness, Pi-host stand-down without Cursor identity and continued parking when `PI_CODING_AGENT` leaks alongside `CURSOR_AGENT` or `CURSOR_INVOKED_AS`, child-worktree exclusion including under an inherited parent-primary `FM_ROOT_OVERRIDE`/`FM_HOME`, and that the adapter never exits 2.
