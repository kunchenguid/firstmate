...
Claude, Codex, and traex can block a Stop directly with exit status 2 and stderr.
All three payloads carry `stop_hook_active`.
In the default Codex mode, which traex shares as a codex fork, a true value lets the second stop finish after one forced continuation.
...
`tests/fm-turnend-guard.test.sh` also covers the queue-aware block message: a pending queue keeps the block and adds drain-first delivery instructions, while an empty queue keeps the plain message.
