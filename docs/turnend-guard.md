...
If that wait does not produce recovery proof, the guard primes `bin/fm-claude-stop-autoarm.sh --ensure-watcher` immediately before refusing.
Priming is refusal-path only: running it before the wait turned every ordinary Claude Stop into a handling successor and suppressed the once-per-generation re-presentation of an unacknowledged downtime episode.
The bounded re-block budget and the attended fail-open described below are both decided before that call, so a Stop the fail-open allows leaves no primed cycle: a successor this Stop had forked could otherwise consume the unacknowledged downtime episode and win the home lock in time to answer the health check that decision reads, silencing the alarm that says supervision is broken.
The one refusal that does not prime is a Stop whose block-budget accounting cannot take `state/.turnend-claude-blocks.lock`: it refuses on the spot, before the fail-open and before the priming call.
That lock is a micro-mutex held across a few file operations, so a Stop that loses it detaches nothing and the next Stop primes normally.
...
The primed arm starts the watcher as a handling successor so a leftover downtime marker is not re-announced into a one-poll resurface.
Nothing reads the detached cycle's stdout once the hook has exited, so it is discarded and `state/.watch-cycle-exits.log` is the single record of that cycle: a primed cycle that cannot start still appends its lifecycle row, whose `exit_code`, `reason`, and `lock_before=pid:<n>|identity:<...>` fields name the holder that blocked it.
...
The refusal banner does not say that to the model, and deliberately says nothing about the detached start's prospects: it reports that the Stop-owned auto-arm did not claim this home, so no generation claim owns recovery for this Stop.
That one sentence is printed on every refusal, because it is true whether or not this Stop detached a cycle - priming deliberately takes no generation claim, and the guard has forked a process whose outcome it cannot know.
It stops at the missing claim rather than telling the reader that nothing is recovering and that restoring supervision is theirs: on a refusal that just detached a cycle that is false, and it invites a second arm over a recovery already under way.
The banner therefore reads nothing back from the priming call, and `--ensure-watcher` reports nothing to it.
Priming does not close every way a session can reach a turn end that is refused again and again, and four entry conditions stay open.
With `python3` absent from `PATH` there is nothing to detach through, so `--ensure-watcher` exits without starting an arm at all.
With a live lock holder whose beacon is stale past `FM_WATCHER_STALE_GRACE`, `bin/fm-watch.sh` deliberately refuses to start a second watcher over it, so the primed cycle names the blocking holder in its lifecycle row and exits without restoring supervision.
With `state/.lock` missing, holding something non-numeric, or naming a harness pid `fm_harness_pid_alive` still reports live for another session - or with the stale-owner recovery through `bin/fm-lock.sh` failing, or its result still not proving self-ownership - the auto-arm's session-lock identity gate stands every firing of it down, and this guard has no session-lock gate of its own to match: it keeps refusing on the strict watcher predicate while both the priming call and the registered `asyncRewake` firing exit at that gate without forking anything, and because that firing never publishes an epoch or its one failure notice, `failure_episode_verified` refuses as well, so the attended fail-open cannot fire - the block count climbs with nothing to spend it on and every turn end refuses.
With away mode active and the away daemon's watcher gone, every path closes at once: the away-mode allow is skipped because it still requires a fresh beacon, the registered auto-arm and the priming call both exit at the auto-arm's `state/.afk` gate without forking anything, and `failure_episode_verified` refuses while `state/.afk` exists, so the attended fail-open cannot fire either - every turn end refuses and nothing bounds it.
That last condition is left to its own owner rather than handled here: the attended fail-open is attended because a person is there to notice, away mode means nobody is, and firing it there would widen the escape precisely when no one can catch what it lets through.
A stale-beacon away home is the away daemon's to recover by restarting its own watcher, or the guard's to reach through that daemon's ownership, which is a change in a mode this priming path does not touch.
...
Each epoch identity is accounted at most once per Stop under the budget lock, and a Stop that reaches the refusal with no epoch progress still advances the count, so an auto-arm epoch that cannot change can no longer pin the bound (2026-09-06: `count=1 epoch=165` for forty minutes).
