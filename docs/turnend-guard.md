...
An eligible hook that cannot recover the session lock or publish its generation claim commits `state/.claude-autoarm-failure-epoch` through a bounded handoff that does not acquire or rewrite the claim mutex and ledger, then uses atomic marker creation to elect at most one notice before it asks Claude for another Stop-owned retry.
