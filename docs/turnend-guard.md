...
The refusal banner says that to the model, but only when a cycle was really detached: it still reports the missing generation claim, and then names the primed detached start and asks for a live watcher to be verified before the Stop hook is treated as broken.
The claim is keyed on the fork rather than on the call, because `--ensure-watcher` exits 0 only after the fork that hands a process to `bin/fm-watch-arm.sh` and non-zero at every gate that returns without forking.
