...
`bin/fm-wake-lib.sh` owns the recorded rejection-reason fields and every condition that sets them, documented at their definitions, and `bin/fm-guard.sh`'s header owns the three reported states.
