...
The claim is the ledger entry itself: the epoch sequence in `state/.claude-autoarm-epoch` is a monotonic claim generation, line 1 records the generation, terminal outcome, each new claim's authenticated foreground session owner, and a rewake's session-lock pid and watcher recovery generation, and line 2 records the claiming process's mandatory pid-identity (`fm_autoarm_claim_open`, `fm_autoarm_midturn_healthy`, and `fm_autoarm_claim_next` in `bin/fm-wake-lib.sh` own the contract).
A claim is open while its outcome is `arming`, its owner pid is alive, its recorded identity successfully recomputes and matches that pid, and it is not stuck - stuck meaning the entry and the watcher beacon are both older than the guard grace, which proves the owner hung mid-arm (a healthy hours-long foregrounded cycle keeps the beacon beating, and every arming phase with no watcher is bounded in seconds); each new owner-bound claim also closes as soon as its foreground session loses `state/.lock`, while unbound pre-upgrade entries retain the older proof.
...
[`watcher-continuity.md`](watcher-continuity.md#ownership) owns the Stop hook's session-lock proof and the detached Claude daemon boundary.
...
The irrevocable commit point of a translation is the exit status, because the harness delivers the collected stderr banner only on exit 2, so an owned terminal commit decides the exit: markerless outcomes commit with the claim ledger write, while an owned generation's once-per-episode failure notice commits only when its marker is created after the winning failed write in the same critical section.
A generation whose required marker cannot be created is refused and exits 0 silently even after printing; its terminal claim-ledger entry is superseded by a later firing, which retries the notice.
Every auto-arm session-acquisition lease wait is bounded, including stale-lock recovery, generation claims, terminal writes and resets, and independent failure publication; when a required lease remains held, the hook publishes an immutable failure record bound to the unchanged lock owner as its mutation fence and scopes the episode marker to the authenticated claimant when one exists.
An owned generation that cannot commit its terminal outcome uses the same independent failure publication against its exact arming signature, while only verified supersession may end silently.
Positive recovery compacts obsolete failure-sequence tokens into a durable monotonic high-water value while retaining newer in-flight tokens for reset fencing.
The short transition boundary serializes claim-ledger writes, failure publication, and positive recovery reset, including the guard's final healthy-watcher or fresh-beacon-and-away-daemon proof before reset, and a later participant either retires an identity-verified holder whose kernel-stopped state is rechecked after lock detachment immediately before signalling, discards an identity-mismatched stale lock without signalling its reused pid, or abandons the transition after a bounded wait when the recovery lock remains unavailable.
Age alone never authorizes signalling a live transition holder, so slow running or sleeping transactions retain ownership while the bounded caller uses its reset-fenced failure fallback.
Positive recovery marks its reset transaction before advancing the monotonic fence, clears the transaction only after episode cleanup completes, and binds every independent failure record plus notice election to an idle fence snapshot, so a lockless publisher crossing that reset remains historical rather than recreating or suppressing current failure state.
The synchronous guard consumes and budgets the newest independent failed epoch for the current fence through the shared failure-episode selector only after the main claim ledger still matches its recorded pre-attempt snapshot on a final recheck, so either a later claim-ledger transition or recovery reset supersedes the failure while repeated failures against one unchanged baseline still advance the bounded progression.
Notice and alarm ownership follow the selected failure record's authenticated claimant when present, but `fm_autoarm_failure_ledger_current` and `fm_autoarm_failure_episode_current` in `bin/fm-wake-lib.sh` and the guard's `failure_episode_current` wrapper do not yet authenticate or filter by the current guard claimant, so an unbound episode retains compatibility selection through its stale session-lock owner.
Every serialized main-ledger transition retires any baseline-less legacy failure record before publishing its new ledger state, so coarse legacy timestamps cannot override a later handoff.
Only a ledger transition away from that hook's exact pre-attempt snapshot proves that a competing participant handled the claim; an unrelated fresh terminal epoch from a prior cycle cannot suppress the current failure.
...
A legacy build's lock-holding claim (recognizable by its `autoarm` role file) still defers or reclaims under the legacy abandonment proof, with a bounded pre-publication window for a ledger mismatch, a live identity-verified stuck owner retired via TERM before its lock is removed, and an unverified pid never signalled; an unmatched hold beyond that window publishes the independent failure epoch instead of remaining silent.
...
In Claude mode, positive watcher recovery rechecks watcher health and clears the block budget, independent failure epoch, failure notice, and attended alarm together under the transition boundary and budget lock before either hook reports ordinary recovery.
...
Its alarm marker is bound to the failure reset and authenticated failure claimant when present, with the episode's session owner as the unbound fallback, separately from any stale lock-owner mutation fence, so a predecessor session's alarm cannot suppress an authenticated successor's auto-arm or attended fail-open progression.
While that attended alarm and its failure episode remain current, later claim or terminal-write failures stop without allocating more sequence tokens or immutable failure records.
...
`tests/fm-session-lock-ancestry.test.sh` and `tests/fm-claude-stop-autoarm.test.sh` cover ancestry-only Claude ownership, detached daemon `--spawned-by` non-authority, zombie-owner recovery visibility, the auto-arm's failed-claim marker path, and lockless publication races against recovery reset and ledger advance.
...
[`verification/supervision.md`](verification/supervision.md#turn-end-guard) records the active cross-harness empirical evidence, including the current credentialed Claude Stop-delivery and `asyncRewake` revalidation.
