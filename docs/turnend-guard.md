...
The refusal banner does not say that to the model, and deliberately says nothing about the detached start's prospects: on a Stop that really detached a cycle it reports only that no generation claim owns recovery yet, because the guard has forked a process and cannot know its outcome.
It once also told the model the refusal could not guarantee the next one and to check whether a watcher took the home lock, and both halves are false in the wedged stale-beacon holder condition below - there the primed cycle refuses to start over the live holder and exits, so the next refusal IS guaranteed, while a live pid does hold `state/.watch.lock` and makes the named check answer yes over dead supervision.
Which of the two wordings appears is still keyed on the fork rather than on the call, because `--ensure-watcher` exits 0 only after the fork that hands a process to `bin/fm-watch-arm.sh` and non-zero at every gate that returns without forking.
So a refusal where nothing was detached - no `bin/fm-claude-stop-autoarm.sh` to run, a claim that opened inside the race window, or any of the priming gates below, away mode and an absent `python3` included - keeps the original wording, because there the absent claim really is the whole story.
