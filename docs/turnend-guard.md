...
That path uses the Stop hook's harness ancestry, starts a session-detached watcher, and takes no generation claim, so a refused Stop cannot abort the only process that could restore a watcher.
