...
With a live lock holder whose beacon is stale past `FM_WATCHER_STALE_GRACE`, `bin/fm-watch.sh` deliberately refuses to start a second watcher over it, so the primed cycle names the blocking holder on its output file and exits without restoring supervision.
