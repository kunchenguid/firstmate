...
Every mode treats an armed standing poll - `state/x-watch.check.sh` or `state/ops-watch.check.sh` - as supervision need, so Relay polling and operational alert monitoring remain guarded without an in-flight task.
