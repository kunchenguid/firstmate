...
The block banner names those two failure modes distinctly so recovery is unambiguous: a lock that fails the ownership proof (missing, dead, or identity-mismatched) reads "daemon pid gone" and points at `bin/fm-afk-launch.sh start` (or `start-native`), while a live, identity-matched daemon whose beacon has passed grace reads "beacon stale" and calls for inspecting the daemon instead of relaunching it; outside away mode the banner is unchanged.
