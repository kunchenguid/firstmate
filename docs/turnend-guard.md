...
An episode closes - clearing the failure notice, alarm, and block budget together for a future independent episode - when supervision is proven healthy again, either by a positively verified healthy watcher or by supervision no longer being needed at all.
The second condition is the stronger proof, because nothing remains for the mechanism to be failing at, and without it an episode opened before the fleet went quiet outlived the condition it described and muted the next last-resort notice permanently.
`fm_failure_episode_reset` in [`../bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns that record set and both closing conditions.
