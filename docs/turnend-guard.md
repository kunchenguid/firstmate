...
An eligible hook that cannot recover the session lock or publish its generation claim makes a bounded identity-recorded attempt to serialize an immutable failure record and episode marker, then publishes both without that boundary when process identity or the boundary itself is unavailable, allocating a distinct monotonic failure epoch for every eligible attempt against its exact pre-attempt claim signature.
