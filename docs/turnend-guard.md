...
Which of the two wordings appears is keyed on the fork rather than on the call, because `--ensure-watcher` exits 0 only after the fork that hands a process to `bin/fm-watch-arm.sh` and non-zero at every gate that returns without forking.
