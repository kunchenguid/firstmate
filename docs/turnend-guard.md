...
The synchronous guard consumes that independent failed epoch only after the main claim ledger still matches its recorded pre-attempt snapshot on a final recheck, so any later claim-ledger transition supersedes the failure.
...
`tests/fm-session-lock-ancestry.test.sh` and `tests/fm-claude-stop-autoarm.test.sh` cover Claude daemon `--spawned-by` ownership bridging, including wrong-root and wrong-pid negatives, and the auto-arm's failed-claim marker path.
