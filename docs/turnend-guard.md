...
When that clearing cannot be recorded, supervision is healthy but the recovery is not provably recorded, so the guard holds the turn open rather than reporting a recovery it cannot prove.
That reset runs on every healthy Claude-mode stop, so reaching it does not imply an open failure episode: with none of its markers present the only thing left that can refuse is its lock, most often held by the auto-arm's own reset on the same Stop event, and that contention clears on the next turn.
Every blocking exit of both Stop hooks names its cause on stderr, including that one: exit 2 is the operator's only channel here, so a block that printed nothing left a home re-waking forever with no message naming why (the state directory that cannot hold restricted modes is the condition that produced it), and only the full failure notice is deduplicated per episode, never the block.
