...
- Hermes registers `on_session_end` in `.hermes/plugins/firstmate-primary`, calls the shared predicate after every persistent CLI turn (including failed and interrupted turns), and uses `ctx.inject_message()` with the canonical operational-input envelope for one bounded recovery turn when the guard returns 2.
...
OpenCode, Pi, pi-signed, and Hermes expose passive callbacks for this purpose.
...
- Hermes is supported only through `bin/fm-hermes-primary.sh` in the persistent classic CLI; safe mode, ignored rules or user config, one-shot, TUI, gateway, desktop, ACP, and alternate-root launches are outside the integration.
