...
When work, a registered custom check, a process-event source, or Relay polling needs supervision at that boundary and no identity-matched watcher has a fresh beacon, the harness integration must either block the turn end or force one bounded follow-up that uses the recovery instruction from the emitted session-start protocol.
...
At least one custom `state/<name>.check.sh` whose current bytes match its valid private `state/<name>.check-trust` binding also requires supervision even though no task is in flight.
The shared predicate delegates that decision to `fm_custom_check_registered` through `bin/fm-check-lib.sh`, so an unregistered, changed, or otherwise invalid check never creates supervision need.
...
- A valid secondmate home is in scope; an idle secondmate endpoint with no registered custom check, process-event source, Relay poll, or work in flight remains healthy because it has no supervision need.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, authenticated custom-check-only need, invalid custom-check rejection, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
