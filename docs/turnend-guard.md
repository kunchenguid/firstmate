...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The sixth guarded entry, the `UserPromptSubmit` registration for `bin/fm-plainenglish-hook.sh`, carries the same marker guard for a different reason: its whole payload is hook stdout, which Grok discards from model context ([`sessionstart-nudge.md`](sessionstart-nudge.md#harness-transports) "Harness transports"), so running it under Grok could only spend a process to produce nothing.
