...
The away-mode case gets its own headers, because a lingering flag with nothing behind it is worse than supervision being plainly off.
There are two of them, chosen by how strong the evidence actually is, because a detector built because away mode asserted supervision it did not have must not assert an absence it has not proven either.
`fm_afk_daemon_absence_is_proven` in `bin/fm-wake-lib.sh` owns that split, and `bin/fm-supervision-instructions.sh` receives it as the `--afk-absence-proven` flag alongside `--afk-daemon-down`, defaulting to the weaker claim so a caller that cannot tell the two apart never overstates.
Absence is proven when there is no daemon lock, no recorded pid, a recorded pid that is not alive, or a recorded identity that demonstrably does not match the process now at that pid, which is what a recycled pid looks like; the pull guard then prints `AWAY MODE FLAGGED, BUT NO SUPERVISOR IS RUNNING`.
Supervision is merely unconfirmed when ownership cannot be established either way, principally a live daemon that never recorded its own identity because `ps` was unreadable at startup and kept supervising anyway; the pull guard then prints `AWAY MODE FLAGGED, BUT SUPERVISION CANNOT BE CONFIRMED`.
The turn-end guard prints the same split under its own prefix, as `TURN WOULD END BLIND - AWAY MODE FLAGGED, BUT NO SUPERVISOR IS RUNNING` and `TURN WOULD END BLIND - AWAY MODE FLAGGED, BUT SUPERVISION CANNOT BE CONFIRMED`.
Its block decision is unchanged and still refuses either way; only the wording and its repair line moved, so that banner can no longer say away mode owns watcher supervision three lines under a header saying it does not.
...
It covers the away-mode short-circuit on both sides of the blind window, alarming when the flag is set with no live daemon whether the beacon is still fresh or long stale, and staying silent when a live identity-matched daemon owns the home.
It covers both provability headers against the evidence that selects them: `AWAY MODE FLAGGED, BUT NO SUPERVISOR IS RUNNING` for a dead or absent daemon and for a recycled pid whose recorded identity does not match, and `AWAY MODE FLAGGED, BUT SUPERVISION CANNOT BE CONFIRMED` for a live daemon that recorded no identity.
