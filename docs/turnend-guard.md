...
Its banner names the true failing condition, a missing live watcher process, a live process that fails identity verification, or a genuinely stale beacon with its real age, and keys the once-per-episode dedup on that condition rather than the beacon mtime.
...
`tests/fm-guard-stale-banner.test.sh` covers the pull-guard predicate, including the persistent-model fresh-leftover-beacon negative control, the auto-arm model's healthy fresh-beacon-without-a-watcher case and its stale-beacon alarm, the true-reason banner wording for a missing process and for a live process that fails identity verification, and the reason-keyed episode dedup surviving a beacon mtime change.
