...
- Codex registers a `Stop` hook in `.codex/hooks.json`, dispatches through the tracked POSIX or native Windows hook entrypoint, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard.
