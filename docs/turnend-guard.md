...
The watcher's own stale-holder refusal and eviction, the healthy-watcher check the arm and Stop-hook auto-arm rely on, and the supervision verdict measure the beacon by its awake age, `fm_beacon_age` in `bin/fm-wake-lib.sh`, because no process runs while the host sleeps: on macOS the most recent system sleep is subtracted, so the first arm after a wake no longer refuses or evicts a healthy watcher that simply had not resumed yet, while a watcher that stalls while the host is awake still crosses the grace and the hard bound as before; hosts that expose no sleep record keep wall-clock ages.
