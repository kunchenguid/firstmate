...
An eligible hook that cannot recover the session lock or publish its generation claim holds the session-acquisition lease while authorizing and publishing its immutable failure record and episode marker, requiring either the same authenticated owner or the exact unchanged stale owner observed before recovery.
