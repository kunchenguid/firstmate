...
An eligible hook that cannot recover the session lock or publish its generation claim makes a bounded identity-independent attempt to serialize an immutable failure record and episode marker, then publishes both against its exact pre-attempt claim signature without that boundary when the boundary itself is unavailable so the synchronous guard can consume the failure without a stale automatic retry.
