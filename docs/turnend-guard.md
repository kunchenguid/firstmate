...
- Agy registers a `Stop` hook in `.agents/hooks.json` and delegates its native continuation response to `bin/fm-turnend-guard-agy.sh`.
...
- The direct-blocking, native continuation, and bounded passive-follow-up split is limited to the primary integrations listed above.
