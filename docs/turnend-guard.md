...
Claude daemon-delivered Stop hooks can run outside the foreground session's process ancestry, so `fm_session_lock_owned_by_self` accepts that bridge only when the daemon has exactly one valid `--spawned-by` JSON value naming label `claude`, the live foreground pid in `state/.lock`, and the same resolved project root.
...
The synchronous guard consumes that independent failed epoch ahead of an unrelated main-ledger terminal outcome, and positive recovery clears it with the rest of the failure episode.
