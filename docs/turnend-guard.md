...
A session that does not own the session lock must not arm, drain, or repair supervision, and `bin/fm-claude-stop-autoarm.sh` also stays inert in that case, so blocking it would demand a repair it may never perform and would re-block every turn indefinitely.
`fm_session_lock_live_foreign_owner` in `bin/fm-session-lock-lib.sh` is the single owner of the guard's foreign-owner question, and `fm_session_lock_pid` there is that library's only reader of `state/.lock`, so the two hooks cannot drift on how the lock file is parsed.
...
The auto-arm deliberately decides its own narrower question through `fm_session_lock_stale_owner` rather than this predicate, because it must stay inert on an empty or malformed lock instead of treating it as a lock nobody owns; [`watcher-continuity.md`](watcher-continuity.md#ownership) owns that recovery boundary.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open including its frozen-epoch case, the live-foreign-lock-owner allow with its unresolvable-ancestry and dead-owner blocking controls, post-alarm continuation suppression, positive recovery reset, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
