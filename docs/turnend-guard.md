...
`bin/fm-watch.sh` touches `state/.last-watcher-beat` once per cycle, at the top of the cycle, so a healthy watcher's beacon can legitimately age by one full cycle, including its terminal wait (`event_wait_or_sleep`) of up to `FM_POLL` seconds.
...
Every check that turns the beacon into a liveness or abandonment verdict - the watcher's own stale-holder refusal and eviction, the healthy-watcher check the arm and Stop-hook auto-arm rely on, the auto-arm claim's stuck proof, the away-mode daemon-ownership branch, and the supervision verdict with the beacon age its banner shows - measures the beacon by its awake age, `fm_beacon_age` in `bin/fm-wake-lib.sh`, because no process runs while the host sleeps: on macOS the most recent system sleep is subtracted, so the first arm after a wake no longer refuses or evicts a healthy watcher that simply had not resumed yet, while a watcher that stalls while the host is awake still crosses the grace and the hard bound as before; hosts that expose no sleep record keep wall-clock ages.
Report-only ages, such as the away-mode return brief's watcher-beat gap, stay on wall-clock time, because a gap while the host slept truly means supervision did not run.
