...
- Cursor registers a `stop` hook in `.cursor/hooks.json` and delegates the whole turn boundary to `bin/fm-turnend-guard-cursor.sh`, the park described below.
  ...
  The `UserPromptSubmit` voice presenter `bin/fm-voice-pending.sh` is deliberately not in that set: `.cursor/hooks.json` registers no counterpart for that event, so standing down there would remove the turn-start voice check from Cursor entirely rather than deduplicate it, and the presenter is read-only with no rewake, so the wedge rationale below does not apply.
...
- Grok registers a `Stop` hook in `.grok/hooks/fm-primary-turnend-guard.json` and delegates capability selection to `bin/fm-turnend-guard-grok.sh`.
  ...
  The same marker guard carries every tracked `.claude/settings.json` entry whose event Grok already covers through its own `.grok/hooks/` registration, which is both `Stop` entries, the `SessionStart` entry, and the two `PreToolUse` Bash entries; `bin/fm-subagent-pretool-check.sh` is one deliberate unguarded exception because no Grok registration covers the subagent-spawn event, recorded in [`subagent-guard.md`](subagent-guard.md) "Known residual gap".
  The `UserPromptSubmit` voice presenter `bin/fm-voice-pending.sh` is the second deliberate unguarded exception, on Grok for the same reason as on Cursor above: no `.grok/hooks/` registration covers that event, so guarding it would remove the turn-start voice check from Grok entirely rather than deduplicate it, and the presenter is read-only with no `asyncRewake`, so the wedge rationale that justifies guarding the `Stop` entries does not apply.
