...
- Codex registers a `Stop` hook in `.codex/hooks.json`, resolves the containing Git root from the hook process working directory, dispatches through the tracked POSIX or native Windows hook entrypoint, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard.
