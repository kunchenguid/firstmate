...
A regular `config/desired-concurrency` file also counts, so the refill detector keeps running with no in-flight work (docs/configuration.md "Desired concurrency").
