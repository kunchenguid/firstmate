...
When none of those proofs appears, the guard restores the standing cycle itself rather than merely re-blocking: Claude runs the whole Stop batch in parallel and a guard exit 2 never suppresses the `asyncRewake` sibling ([`verification/supervision.md`](verification/supervision.md#turn-end-guard) records the dated semantics), so an unclaimed epoch at this point means the auto-arm fired and is wedged, gated, or dead, and re-blocking alone can never bring supervision back.
Outside away mode, the guard spawns the home-scoped watcher singleton detached (own process group, `nohup`, stdio detached) and verifies it within `FM_CLAUDE_GUARD_ARM_CONFIRM` seconds (default 10, 30 on Git Bash/MSYS) against the same strict watcher predicate.
That default is derived from the same OSTYPE switch `bin/fm-watch-arm.sh` uses for its own arm window rather than set separately, because a confirm window tighter than the arm layer it confirms would make the guard report a failure the arm had not yet had time to avoid.
That arm is unconditional and is never limited by the block budget, because arming and blocking are different actions with different risks: restoring a watcher is recovery, and only blocking needs a ceiling.
A blocked turn is visible and bounded, while a fleet left with no watcher is silent, so standing the arm down for a spent budget would trade a bounded annoyance for the far worse failure.
On success with a budget slot left, the guard blocks exactly once with the restored-cycle banner so one handling turn drains what the lapse queued, and the following healthy stop is then allowed normally and is what resets the failure episode.
The watcher queues every actionable wake durably before closing, so a cycle restored this way loses nothing even while the auto-arm's rewake channel stays broken, and the singleton lock makes a race with a late-claiming auto-arm harmless.
That restored-cycle block spends one `FM_CLAUDE_TURNEND_BLOCK_BUDGET` slot exactly like a repair re-block; it deliberately does not reset the failure episode, because that reset deletes the block-budget file and the failure notice together, which would erase the progression pacing it and leave the terminal fail-open permanently unverifiable.
Once the budget is spent the arm still runs and still restores the cycle at every turn end, and what stops is the blocking: the guard allows the stop and says so, on stderr and again as a `systemMessage` because the harness delivers collected stderr only on exit 2.
Allowing there is safe because the cycle is live and verified and the watcher queues every actionable wake durably, so the next drain presents anything the lapse queued.
When that last resort cannot verify a cycle at all, the guard re-blocks with the repair banner instead and then considers the attended fail-open.
`FM_CLAUDE_TURNEND_BLOCK_BUDGET` (default 3) gates reaching that fail-open rather than capping blocking itself: with no verified failure episode the terminal check refuses and the guard blocks again, so a watcher that can never be armed keeps blocking on every later stop, past Claude's 8-block override.
Two things narrow the budget further, and this is the single place that owns the caveat: `budget_account_current_epoch` advances the count only when the ledger's `epoch=N` differs from the one already recorded in the block-budget file, so a slot is spent per auto-arm generation rather than per stop, and a generation that never advances - the hook disabled, or the script erroring before its first ledger write - never advances the count at all.
...
- No harness adapter manufactures supervision with an unverified fire-and-forget shell ampersand; the Claude guard's last-resort arm spawns the watcher detached and claims recovery only after verifying the live cycle.

## Regression coverage

`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` open-generation claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, generation and legacy claim cases that must block or clear instead of allowing a blind stop, the guard-owned last-resort arm (a lapsed cycle restored at the next turn end, the unverifiable-arm fallback to the bounded re-block, a spent budget stopping the blocking but never the arming, a restorable cycle being restored rather than alarmed over, the default confirm window being the arm layer's own 10 s rather than a tighter hand-set 5 s - with the replaced 5 s re-applied as the control that still gives up on the same slow watcher - and its widening to 30 s under `OSTYPE=msys` with `bin/fm-watch-arm.sh`'s, and away-mode stand-down), away-mode daemon ownership between watcher cycles and over a watcher lock left behind by an exited watcher, plus its dead, pid-reused, absent, stale-beacon, and away-mode-off negatives, the away-mode beacon's poll-derived grace widening for a live daemon still mid-cycle and its bound against a dead daemon, a beacon older than that wider grace, and FM_POLL's inapplicability with away mode off, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
`tests/fm-watcher-lock.test.sh` and `tests/fm-claude-stop-autoarm.test.sh` pin the start-time-hardened micro-mutex steal that keeps a pid-reuse-wedged hold from silently freezing the auto-arm's claim ledger, in both directions (a recorded start time the live pid no longer matches is reclaimed without signalling the reused pid; a live holder whose start time still matches defers).
The same suite pins why holder liveness compares the start time rather than the fuller process identity: a holder that exec's into another program keeps its pid and start time while replacing its command, and an identity comparison would read that live holder as dead and hand its lock to a contender.
It also covers the `TZ` pin on `fm_pid_start_identity`'s `ps -o lstart` fallback: a live holder keeps its lock against a contender running under a different timezone, and the fixture first asserts that the unpinned rendering diverges between the two zones so the case cannot pass vacuously.
...
[`verification/supervision.md`](verification/supervision.md#turn-end-guard) records the active cross-harness empirical evidence, including the 2026-07-24 Claude `asyncRewake` revalidation and the 2026-08-30 Stop-hook execution-semantics facts the last-resort arm relies on.
