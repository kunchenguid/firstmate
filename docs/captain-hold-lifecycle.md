...
One retained-delivery gap remains bounded by tasks-axi 0.2.6 and is recorded for separate upstream work rather than representing a defect introduced by this branch.
An interrupted local-only retention preserves its `local-landing:<branch>` marker inside the retained deliverable line, so answering the hold or replaying the pending transition can publish the local landing note.
...
A local-only landing's note is written by `tasks-axi done --note` as the last of the row's indented body lines rather than into the row title, using the explicit `local-landing:<branch>` marker for the recorded landing base or the dynamically resolved default branch, with exact `local main` compatibility; teardown may retain that marker inside `Deliverable of the finished work: ...`, and the snapshot accepts either completed form whether or not an earlier captain resolution record exists and publishes the accepted note.
The explicit `local-landing:<branch>` marker and its retained deliverable form are teardown metadata rather than captain prose, while the legacy bare `local main` fallback remains resolution-guarded so a decision worded `local main` does not become a delivery artifact.
