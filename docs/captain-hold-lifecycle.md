...
When a distinct task is held with `--origin`, the origin is linked before the hold attempt releases the shared publication lock, and a failed attempt reconciles that link against the resulting durable hold state.
This makes the origin's current captain-hold state a reliable transition guard without leaving a failed hold as a permanent false blocker.
