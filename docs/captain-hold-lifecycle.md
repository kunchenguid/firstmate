...
Repeats are idempotent, a closed task is refused rather than reopened, an id that tasks-axi has already pruned into the done archive is refused rather than silently resurrected under a fresh live task, and `--until` stores the captain's own deferral date through tasks-axi's date gate.
...
Once tasks-axi has pruned the closed task into the done archive, that retroactive repair is mechanically impossible - an archived-only id has no live update path - so re-answering it is limited to an idempotent replay whose decision digest and `Resolution mode:` both match, and an archived call with no recorded answer fails the replay rather than being invented.
...
It accepts `--none` as an explicit semantic inventory result, refused while the origin still has a lifecycle-open keyed status decision, and verifies every listed task - live in the backlog, or carrying its resolution in the done archive - before recording completion.
...
`verify` requires the recorded attestation, requires every recorded inventory entry to still be durable - actively captain-held, closed with a recorded answer in the live backlog, or carrying that answer in the done archive - and fails on any keyed status decision that opened after the last `complete`, which makes re-running `complete` the repair.
An archived id with no recorded answer fails the gate exactly as an unresolved live task would: only done tasks are ever archived, so an archived call without its answer is genuinely unresolved, and the safety guarantee is unchanged, not relaxed.
