...
Queued forge merges cannot be covered locally because the forge performs the merge asynchronously after the local command has returned, when no lock this code could hold would still be held; while the away-posture record exists, `fm-pr-merge.sh` therefore refuses GitHub auto-merge and required or unreadable queue rules, refuses GitLab scheduled and caller-requested auto-merge, and forces an otherwise synchronous GitLab merge with a final `--auto-merge=false`. This closes the common synchronous live-owner TOCTOU, not every adversarial timing: a GitHub queue-rule or PR-base change after the queue-free preflight can still enqueue before the forge call, and a killed lock-owning shell can leave its forge child alive after stale-owner recovery admits archive or replacement. Those are accepted confused-agent-grade limitations, in the sense documented by `bin/fm-lease-lib.sh`, not oversights; this change deliberately adds neither landing re-verification and durable authority nor child ownership handoff.
...
The suite does not test the accepted merge-to-cleanup re-hold window because it occurs after the locally serialized merge command has returned; asynchronous queued-forge landing is attended-only and is refused while the away-posture record exists.
