...
The `complete` subcommand unions the reviewed captain-held task ids into `decision_keys=` and writes `decisions_reviewed=1` while originating task metadata is live, placing the attestation before any preserved `pr=` identity line so completing over a PR-armed origin never breaks its armed merge poll.
