...
That stub prints the task body the way tasks-axi prints a scalar needing escapes - as a quoted JSON string - so the same case also pins the show-scalar decode that [`bin/fm-backlog-transition-lib.sh`](../bin/fm-backlog-transition-lib.sh) owns for every backlog reader: the hold must round-trip the body's embedded quotes, backslashes, and line structure on the host's own JSON::PP.
