...
A local-only landing's note is written by `tasks-axi done --note` as the last of the row's indented body lines rather than into the row title, using the explicit `local-landing:<branch>` marker with exact `local main` compatibility; teardown may retain that marker inside `Deliverable of the finished work: ...`, and the snapshot accepts either completed form whether or not an earlier captain resolution record exists and publishes the accepted note.
