...
A local-only landing's note is written by `tasks-axi done --note` as the row's indented body line rather than into the row title, so the snapshot reads that note from the body as well as from the title and the landing is published carrying its recorded note.
