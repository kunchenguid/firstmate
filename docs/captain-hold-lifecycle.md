...
The same settlement retracts a `complete` transfer - the `captain-held [key=<k>]` event naming the task ids it tracks - that still stands on some lane's log and names the settled task or sits on the settled task's own log: it appends `resolved [key=<k>]` once neither a task the transfer names nor the lane's own task is still an open captain call.
