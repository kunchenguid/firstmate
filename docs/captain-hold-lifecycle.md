...
Watcher and away-mode classification read the last event line of `state/<id>.status` rather than the backlog, so when the held task has a worker lane (`state/<id>.meta`), `hold` also declares the hold there as `captain-held [key=captain-hold-<task>-<n>]: <reason>`, attributed to the hold command through that keyed operator verb rather than disguised as a worker line; a repeated hold leaves the standing declaration untouched.
...
Every settlement path - close, release, repair, or an evidence-backed reconcile close - retracts the declaration itself with the matching keyed `resolved` line whenever it is still the log's last event, so a stopped worker can never leave the lane reading as an answer still owed, and a worker that wrote anything newer keeps that newer state.
The same settlement retracts a `complete` transfer, `captain-held [key=<k>]: tracked by <ids>`, that is still some lane's last event and names the settled task or sits on the settled task's own log: it appends `resolved [key=<k>]` once neither a task the transfer names nor the lane's own task is still an open captain call.
...
While the hold or transfer stands, the watcher and away-mode daemon read it as the lane's last line; once a retraction is the log's latest event, `last_status_line` in `bin/fm-classify-lib.sh` itself reads past every line the hold command wrote - mirror, transfers, and retractions - to the worker's last event, so a settled lane is done, paused, or failed again, or back on the `needs-decision` its transfer answered, to every raw reader instead of looking like a quiet worker.
