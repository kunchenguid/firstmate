...
For rollout safety, the adapter also rejects the old answer shape when its structured selection is empty and only its note carries text, so a board opened before this change cannot turn the old annotation workaround into a final answer.
...
Current rows require the versioned shape, the `choice` tag, and a non-empty selection; a time-limited rollout branch accepts ordinary answers from the old question/answer shape but refuses its bare or separator-annotated reconcile values from both intakes because those rows do not separate the selected option from its note.
