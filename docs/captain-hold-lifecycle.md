...
While the hold or transfer stands, the watcher and away-mode daemon read it as the lane's last line; once a retraction is the log's latest event, `last_status_line` in `bin/fm-classify-lib.sh` itself reads past every line the hold command wrote - mirror, transfers, and retractions - to the worker's last event, so a settled lane is done, paused, or failed again, or back on the `needs-decision` its transfer answered, to every reader that routes through that function instead of looking like a quiet worker.
