...
The `complete` subcommand unions the reviewed captain-held task ids into `decision_keys=` and writes `decisions_reviewed=1` while originating task metadata is live; over a record with a preserved `pr=` identity line it drops any earlier attestation pair and places the single fresh pair before that line, so completing over a PR-armed origin never breaks its armed merge poll, and a pair stranded after that identity block by an older append is repaired rather than refused.
