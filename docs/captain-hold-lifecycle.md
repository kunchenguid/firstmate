...
The pending-close record teardown already stages before destructive cleanup carries that intent as a `mode=retain` line, so an interrupted cleanup replays the retention at the next session start through the same record, validator, and lock as an ordinary close and never closes the row.
If the captain answers before replay, `answer` validates that record and applies its retained artifact before closing: supported pull requests and reports use the row's structured artifact fields, while a GitLab merge request or local-only note is preserved as a deliverable line in the task body; replay then retires the record.
