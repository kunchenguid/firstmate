...
On 0 only the close changes: after cleanup and still under the task's own lock, teardown records one `Deliverable of the finished work: ...` line at the end of the task body, copies a supported pull request or canonical `data/tasks/<id>/report.md` into the row's structured artifact fields, and runs `tasks-axi reopen`, so the row returns to Queued with its hold intact and remains on the appropriate Captain's Call or Charted Next decision surface instead of reading as work still under way.
...
A relocated retained report reaches the row through the canonical `data/tasks/<id>/report.md` link accepted by the installed tasks-axi version; migration updates existing stored links and refuses an ambiguous or unsupported backlog mutation rather than rewriting the backlog by hand.
