...
`bin/fm-procevent-lavish.sh answers` is one such adapter command; it reads only rows tagged `choice` that carry a non-empty structured selection, relays a card's declared close mode, and can never let freeform captain prose forge a task id or a mode.

## Bearings board controls

Every Captain's Call card with multiple effective options also has a separate freeform control labeled `Ask a question or give another instruction` by default.
...
A payload may opt in a card with fewer choices, and a non-empty `freeform_hint` may replace the default label.
...
A freeform submission cannot close or release a task, create a reconcile request, dispatch work, or authorize a merge.
Only an explicit option keeps those existing powers, including the exact `merge` option on a merge card.
The adapter rejects every schema-less `question`/`answer` row, so an old board cannot turn freeform text into authority.
An old board must be rebuilt before its explicit choices can enter captain-answer intake.
...
An explicit board option emits `fm-bearings-answer.v1` context with the slug-shaped selection and a separate note field, so a note on a captured pre-change Reconcile selection cannot turn it into an ordinary answer value.
`bin/fm-procevent-lavish.sh answers` emits only an exact non-reconcile selection, while `reconciles` emits only task ids whose structured selection is Reconcile and carries any captured note as request provenance.
Rows require the versioned shape, the `choice` tag, and a non-empty selection.
Schema-less old question/answer captures and every other structurally uncertain capture feed neither intake, remain announced, and cannot forge a task id from freeform prose.
...
The captured-source coverage proves Lavish deduplicates each card before separating versioned structured selections from notes, freeform prompt rows, stale empty-selection choice rows, and schema-less question/answer rows leave their captain-held tasks open, bare and annotated Reconcile choices never reach keyed answers, genuine versioned choices still close normally, schema-less reconcile values feed neither intake, mixed repeated selections preserve every other card's final value, the generic runner creates a request only through a verified bound source, chat reconcile text creates none, and the resulting board request authorizes evidence-backed closure.
The board's half is pinned in `tests/fm-bearings-board.test.sh` and `tests/fm-bearings-board-render.test.sh`: cards with multiple effective options get the separate freeform control while a card left with one effective option requires an explicit opt-in, every published decision card carries exactly one reconcile option, authored options reserve that value across every card type, recommendations name authored options, freeform text reaches structured supervisor presentation without counting as an answer or entering keyed-answer or reconcile output, explicit options keep their existing outputs and answered state, a decision card whose structured subject appears in the payload's landed rows is dropped while a genuinely open one is kept even when an unrelated landed id contains its key after a newline, a build requires a fresh authoritative listed-open result before binding or arming, a reopen retires the pre-reopen source generation and waits for a fresh live listener, and a rebuild of an already-armed board with no live listener starts one.
