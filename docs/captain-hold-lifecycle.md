...
`--surviving` names the task that still carries the live question; that task must already exist, must itself be (or have been, through a close) an actual captain call, must not itself already be retired as a duplicate (a chain to a dead end is refused, naming the task it points to instead), and is never held, answered, or otherwise modified by this command.
...
`verify` requires the recorded attestation, requires every recorded inventory entry to still be durable (actively captain-held, carrying a recorded answer, or retired as a duplicate), and fails on any keyed status decision that opened after the last `complete`, which makes re-running `complete` the repair.
