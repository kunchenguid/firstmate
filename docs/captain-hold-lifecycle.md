...
A standing mirror also stays the lane's declared wait (`status_declared_wait_line`) when later lines such as an answer for another key land on top of it, because only settling the captain's call lifts the hold; a transfer counts as the declared wait only while it is the lane's last event.
