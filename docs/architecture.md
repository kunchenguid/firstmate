...
A slot leased under a task id that no home's task record names (a secondmate home is leased under its registered secondmate id, not a task id) is the leftover of a spawn killed outright between leasing and publishing its record (nothing reclaims it, because teardown and relaunch both start from the record): release it by hand from the project with `treehouse return --force --if-lease-holder <task-id> <slot-path>`, and never read it as a live task.
