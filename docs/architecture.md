...
A worker that finishes into a wait - a `done:` line immediately followed by one or more `paused:` lines, the shape `bin/fm-brief.sh`'s status protocol instructs for exactly this case - is trusted on that shape alone and skips the confidently-dead liveness wait entirely, on the condition that no steering-inbox record has landed after the status log's last append; a newer inbox delivery lapses that trust and restores ordinary run-step wedge tracking instead, since a steer never gets a plain `working:` acknowledgement the log could otherwise show (`bin/fm-watch.sh`'s `terminal_then_paused` and `no_inbox_activity_since_status`).
