...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when the run is bound to the crew's own branch and current code identity, then keeps that run-step authoritative even if the pane has closed.
A run in flight keeps its pipeline fix commits in the gate repository, so its head is not resolvable in the crew worktree at all; that run is bound instead by no-mistakes' own `branch_sync` verdict naming this branch, this exact HEAD, and the same run id, which keeps a live run from being rejected and replaced by a superseded run on the same branch.
`bin/fm-nm-run-lib.sh` owns that attribution as one rule asked by both readers, so `bin/fm-teardown.sh` sees the same live run before it removes a worker, and each script header owns how its reader applies the verdict.
