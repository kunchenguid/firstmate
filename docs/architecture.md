...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, unless that task carries a live declared merge wait (`state/<id>.merge-wait`), provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits, declared merge waits, and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A checks-green ship task waiting on a merge nobody in this fleet can perform takes that same absorb from a durable declaration (`state/<id>.merge-wait`, owned by `bin/fm-merge-wait.sh`) rather than a status verb, because its last status line is a `done:` report and not a pause.
That absorb (`handle_merge_wait_stale`) re-surfaces once per `FM_PAUSE_RESURFACE_SECS` window rather than going quiet, and it is admitted only while authoritative crew state still reports the run step's terminal checks-green outcome and the task's merge poll is still armed, so a withdrawn, superseded, or no-longer-green declaration returns the pane to ordinary wedge aging.
