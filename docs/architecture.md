...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written and, in a home that opted in with `config/wedge-defer-pipeline`, without their validation pipeline reporting an active step, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
In a home that opted in with `config/wedge-defer-pipeline`, a pane whose worktree is silent is then asked one further question, on the same at-threshold budget: does the validation pipeline itself report one of its own steps as currently producing output?
That flag is default-off and gates both halves, the deferral and the marker `bin/fm-crew-state.sh` publishes for it, so a home that has not set it takes no deferral, reads no marker, spends no pipeline read, and escalates on exactly the prior schedule, evidence, and reasons.
That case is invisible to every other liveness input, because a validation round runs in the pipeline's separate checkout, so the crew's pane renders nothing and the crew's worktree is never touched, while a run record says `running` whether or not the step behind it is still alive.
The pipeline's own recency verdict is the authority: it marks a step quiet once no step log or native-agent lifecycle event has arrived for longer than its configured quiet warning, so a stalled step withdraws the evidence on the pipeline's schedule rather than on a second threshold invented in firstmate.
A positive answer takes the same bounded deferral as write evidence, on the one shared progress-deferral chain both sources age, so a quiet stretch re-surfaces once from its first deferral however its evidence alternates, and every other answer - a quiet step, no activity report, a coarse verdict describing some other run, an unreadable verdict, an unreachable daemon, or any state that is not a working run step - leaves the escalation schedule exactly as it was.
A busy pane is otherwise exempt from staleness, but only until its last completed turn or explicit native-harness progress reaches `FM_BUSY_TURN_MAX_SECS` (`bin/fm-watch.sh` owns marker selection); past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, progress deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
Away-mode housekeeping has no progress deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or whose validation pipeline reports an active step, still escalates as a possible wedge at that bound.
