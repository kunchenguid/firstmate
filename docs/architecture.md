...
The confirming words are read from the stored record of a named inbound message rather than from a path, and that message must be authentic, still open, and received after the preview was armed - so an approval is something the paired person actually sent, and arming and confirming in one turn is impossible rather than merely forbidden.
