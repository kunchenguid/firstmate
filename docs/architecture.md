...
A `https://github.com/<owner>/<repo>/pull/<n>` URL is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, and green at the current head, then `gh pr merge` binds that verified head with `--match-head-commit`.
...
An attended `--allow-red <check-name>` waives only the named GitHub checks and is refused while the away-posture record exists.
