...
`local-only` projects stay with the main first mate by default because they merge into the main local checkout instead of a remote-backed PR path; a local route may supervise one only through the guarded same-filesystem capability and branch return owned by the [`secondmate-provisioning` skill](../.agents/skills/secondmate-provisioning/SKILL.md#guarded-local-only-branch-return), with landing still main-firstmate work.
...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's `local-only` guard and no-mistakes initialization, and the guarded local branch-return path.
