...
The same library owns the task record's `pr=` identity contract, which every writer of `state/<id>.meta` has to keep: the `pr=` block stays the record's trailing block, and a key appended past it costs that task its merge poll on every later cycle.
