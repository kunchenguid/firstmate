...
The one further exception exists only under the opt-in idle-worker pre-compaction sweep: the two turns that sweep induces would each otherwise surface as an actionable turn-end wake, so `bin/fm-idle-compact.sh` identifies exactly those per episode and the watcher absorbs them, and [configuration.md](configuration.md#idle-worker-pre-compaction-configidle-compact) owns that contract along with the sweep's own bounded cadence on this poll.
