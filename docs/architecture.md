...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, one agent-gone stale wake per endpoint the backend's recovery-grade classifier proves holds no live agent, and heartbeat backstop hits.
That agent-gone verdict (`dead` or `missing` from `fm_backend_agent_state`) is confirmed across two polls, surfaced once, and then suspends stale tracking for the identity until the agent reads alive again, so an exited or torn-down pane cannot re-alarm through hash churn, wedge escalations, or native blocked edges.
A task record retired mid-poll ends its identity's classification without a wake, and the same poll sweeps the per-window marker family for any window that no longer has a record, so a recycled pane identity never inherits a dead task's escalation state.
...
Every absence of write evidence, including a missing worktree record, a torn-down worktree, a walk that outlives its wall-clock bound on a hung mount, and a failed walk, leaves the existing escalation schedule untouched, so a crew that writes nothing still escalates exactly as before unless the agent-gone probe at that same threshold moment has already replaced the escalation.
