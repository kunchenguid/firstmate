...
When the away-posture record existed, that outcome also names the authority the merge ran under, on the poll's row as much as on the attended one, so a merge the forge queued or landed out of band stays as auditable as one firstmate merged directly.
[`bin/fm-merge-authority-lib.sh`](../bin/fm-merge-authority-lib.sh)'s header owns that answer and its structured sources; the merge path gates on it and the watcher only records it, so reading the authority never becomes a second way to authorize a merge.
