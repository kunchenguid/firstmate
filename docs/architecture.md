...
paseo is a registered experimental backend name with no adapter under `bin/backends/` yet: its runtime detection exists, but it is absent from the spawn-capable set, so every paseo spawn refuses at `fm_backend_validate_spawn` and every `backend=paseo` cleanup record refuses at `fm_backend_validate_task_endpoint` until that adapter lands and a `docs/paseo-backend.md` owns it; [`verification/runtime-backends.md`](verification/runtime-backends.md#paseo) owns the active detection-ordering evidence in the meantime.
