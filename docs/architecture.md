...
The same cursor-backed unread span also prints `note:` lines and reserved-key pending-reply resolutions once, because those verbs never enter the OPEN DECISIONS fold and a latest-line-only annotation would otherwise drop a substantive answer sitting directly above a later routine line. This presentation path is deliberately fail-open: an I/O failure or concurrent status replacement may rarely re-present a handled line, but it never acknowledges bytes that might not have been shown; teardown retires a task's cursor generation before that task ID can be reused.
