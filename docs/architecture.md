...
Both supervisors classify the status BYTES appended since they last classified that log, never its last line alone, so a later routine append cannot hide the decision, blocker, failure, or finish that arrived just before it; each keeps its own presentation marker (the watcher's `.seen-*` and `.hb-surfaced-<task>` markers, the daemon's `.subsuper-seen-status-<task>` marker) with independent reported-signature and classified-position fields, and an unusable position re-reads the whole log rather than losing events.
A `needs-decision` or `blocked` event in that span is retired only when the whole-file decision fold proves its key closed, which is the same open/closed rule the durable OPEN DECISIONS surface uses.
