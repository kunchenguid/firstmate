...
The delivery-mode closed set is itself machine-checked: six scripts (`fm-brief.sh`, `fm-spawn.sh`, `fm-promote.sh`, `fm-project-mode.sh`, `fm-remote-home-provision.sh`, `fm-remote-home-seed.sh`) each independently validate it with their own case statement, and `bin/fm-delivery-mode-check.sh` cross-checks their actual case arms against the single declared inventory in `bin/fm-delivery-mode-sites.json`, failing on drift in either direction; `tests/fm-delivery-mode-sites.test.sh` owns the regression coverage.
