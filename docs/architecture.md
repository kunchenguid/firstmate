...
Its later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change, because the throttle and its age are keyed to the declaration (the declared wait's verb and reason) and not to the pane an idle parked worker keeps ticking, so a status write that leaves that event unchanged neither restarts the cadence nor wakes early, while a new or changed declaration surfaces at once.
