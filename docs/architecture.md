...
Away mode is a posture of the one supervision session, recorded in `state/.afk-contract` by `bin/fm-afk-contract.sh` after the captain confirms a read-back of their away words and mandate clauses.
The independent `state/captain-presence` HOME/AWAY record controls only proactive Hermes/Telegram routing and never changes that supervision posture, authority, permissions, ownership, approval semantics, logging, or queue durability.
...
Ordinary workers land as tabs directly in the home's durable stable workspace binding; [Stable home spaces and ordinary worker tabs](herdr-backend.md#stable-home-spaces-and-ordinary-worker-tabs) owns that binding and its validation, while [Legacy presentation spaces](herdr-backend.md#legacy-presentation-spaces) owns the disposable-workspace projection no new spawn creates, retained for existing tasks, its version floor, and its narrow home-local restored-shell cleanup at locked session start.
