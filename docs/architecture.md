...
A `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, or the real `gh` binary instead of `gh-axi` when a live pre-merge head read succeeds and the merge binds to it with `--match-head-commit` (`gh-axi` silently drops that flag), defaults to `--squash`, and preserves explicit merge-method flags.
