...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, the built-in fseventsd footprint early warning, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
That fseventsd early warning runs on the same bounded slow-check cadence and wakes only when the host's fseventsd memory footprint newly reaches a warning, action, or emergency level, so a level that is merely still true never re-wakes.
`bin/fm-telemetry.sh` owns its sampling, its bounded history under `state/telemetry/`, and the thresholds themselves.
