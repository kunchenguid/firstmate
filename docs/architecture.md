...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, a declared external wait, a verified captain-held transfer, or a still-open decision that remains outstanding past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A crew that declares `paused:` for a known external wait, carries a verified `captain-held` transfer, or still has a decision open in the durable fold is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
An open decision qualifies because firstmate itself owes that answer, so the quiet endpoint is expected rather than wedge-suspect, and its recheck names firstmate as the one who can clear it.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake and then applies that cadence, and that one-shot surface is keyed on the declared wait's own identity - the pause reason for a `paused:` wait, the durable open-decision set for a decision - rather than on the pane hash or on a whole-file signature, so neither a live harness redrawing a spinner or a token counter nor the worker's own later progress lines appended while the wait is still open can buy another surface for the same wait.
That one-shot's suppressor is published only after its wake is durable in the queue, so a watcher that exits between the two can at most repeat the surface on its next round, never hide the gate behind the long cadence.
Live or inconclusive liveness still lets that initial surface through, and a secondmate's endpoint liveness is never read at all; a mate is admitted to that same cadence only to serve a declared `paused:` or `captain-held` wait's bounded re-surface, so a forgotten pause or captain hold on a mate cannot rot invisibly.
...
The watcher additionally reads that library's single declared-wait triage, which adds a still-open decision as a third kind of expected quiet endpoint, so its several stale-emission paths cannot each decide on their own whether an idle endpoint is a wedge; away mode keeps the narrower verb predicate because the daemon escalates an open decision to the captain rather than absorbing it.
...
That surface names every listed decision by the key that answers it, including the shared `default` key an unkeyed line opens, and flags a differently-keyed token inside a summary as prose, because a `[key=...]` a summary merely mentions is never that decision's key and answering it would otherwise refuse.
