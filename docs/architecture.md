...
This repo uses that setting, and its own `.no-mistakes/` directory remains local state that stays gitignored and is rejected by CI if tracked; [`configuration.md`](configuration.md) owns the setting.
 ...
 The main home also runs `bin/fm-pr-delivery.sh` on the watcher poll cadence and at locked session start as a bounded adjunct that discovers open PRs for merge-capable registered projects, classifies heads with live gh evidence, maintains a reason-coded blocked queue, and queues `check: pr-delivery` when a merge-eligible or post-merge obligation appears; handling is owned by the [`pr-delivery` skill](../.agents/skills/pr-delivery/SKILL.md).
 ...
 The main home also runs `bin/fm-pr-delivery.sh` on the watcher poll cadence and at locked session start as a bounded adjunct that discovers open PRs for merge-capable registered projects, classifies heads with live forge evidence, maintains a reason-coded blocked queue, and queues `check: pr-delivery` when a merge-eligible PR appears; handling is owned by the [`pr-delivery` skill](../.agents/skills/pr-delivery/SKILL.md).
 ...
 The main home also runs `bin/fm-pr-delivery.sh` on the watcher poll cadence and at locked session start as a bounded adjunct that discovers open PRs for merge-capable registered projects, classifies heads with live forge evidence, maintains a reason-coded blocked queue, and queues `check: pr-delivery` when a merge-eligible PR appears; handling is owned by the [`pr-delivery` skill](../.agents/skills/pr-delivery/SKILL.md).
 The helper requires a full canonical URL, defaults GitHub merges to `--squash`, preserves explicit merge-method flags, rejects malformed URLs and repo override flags, and binds delivery merges to the scanned head with `--expected-head` and `--match-head-commit`.
 Standard GitHub merges invoke `gh-axi pr merge <n> --repo <owner>/<repo>`.
 GitLab merge requests use the URL-derived project and are guarded by a live open, mergeable, conflict-free, discussion-resolved state with a successful pipeline at the current head; a well-formed GitLab URL is never sent to GitHub.
