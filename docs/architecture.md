...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr or cmux prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij, orca, and superset are never auto-detected (only explicit selection).
...
tmux, zellij, orca, cmux, and superset expose no native busy primitive at all, so a task on those backends is classified purely from its adapter's own lifecycle record.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij remains unverified, and Orca, cmux, and Superset do not support secondmate spawns.
...
Superset is experimental and selected only explicitly: like Orca it owns both workspace and terminal lifecycle, records `superset_workspace_id=` and `superset_terminal_id=` (reconstructed into a live `<workspace_id>:<terminal_id>` target, cmux's composite-addressing shape), and removes worktrees through `superset ws delete` only after the usual firstmate teardown checks pass.
[`superset-backend.md`](superset-backend.md) owns current behavior and limitations, while [`verification/runtime-backends.md`](verification/runtime-backends.md#superset) owns active smoke evidence.
Codex App support is recorded in `docs/codex-app-backend.md`; it is not selectable as a runtime backend.

## Worktrees, not branches in your checkout

Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, and cmux tasks, while Orca creates its own worktrees for `backend=orca` and Superset creates its own for `backend=superset`.
...
Fleet state lives in each task's session-provider backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca/superset when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
