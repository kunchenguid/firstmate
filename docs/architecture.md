...
`fm_backend_source` checks each not-yet-loaded adapter file for readability before invoking the shell source builtin, returning non-zero so callers reach their fail-closed refusal path consistently on stock macOS Bash and newer Bash.
