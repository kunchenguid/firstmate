...
A `<base-url>/<owner>/<repo>/pulls/<n>` Gitea (or Forgejo) URL invokes `tea pr merge <n> --repo <owner>/<repo> --login firstmate-<host>` and defaults to `--style squash`; the `tea` login is re-derived from the base URL and the base URL must be listed in `config/gitea-instances` (see [configuration.md](configuration.md)) before any side effect, because a plain-http self-hosted origin cannot be trusted from its shape alone.
