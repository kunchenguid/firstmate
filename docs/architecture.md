...
An ordinary Herdr ship or scout worker with a replayable verified harness profile has one optional bounded child layer owned by `bin/fm-child.sh`: at most three response-verified panes share the exact parent tab and Treehouse worktree while private records bind disjoint writable paths, the inherited harness profile, reports, and cleanup.
Those children are not fleet tasks or supervised reports, cannot delegate, and never own Git mutation, final validation, publication, or captain communication; the parent remains the sole integration and delivery authority.
A child pointer send that returns `pending` may be confirmed only after exact inherited registered-agent identity plus the harness-scoped rendered-busy matcher also used by `fm-crew-state.sh`, never transcript text by itself.
