...
The same coverage identity also bounds terminal-status stale re-hashes after a finished result was already delivered, under the shared `.paused-resurfaced-*` throttle with declaration `outcome-covered:<seq>:<status-sig>`, re-surfacing once per `PAUSE_RESURFACE_SECS` so a forgotten teardown still reaches the long cadence.
