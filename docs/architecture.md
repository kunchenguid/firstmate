...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, exact trust dialogs awaiting a keystroke, possible-wedge state transitions, and heartbeat backstop hits.
The stale detector reads the task's current state through `bin/fm-crew-state.sh` before it fires.
An active pipeline, a run parked at a gate, a live harness turn, or a declared external pause with a live substrate disconfirms a wedge for that task.
An unrelated watcher heartbeat or another task's run is never progress evidence.
Unreadable current state remains `unknown` in fleet views and does not alarm in normal-mode stale triage.
The disconfirmers that prove only rendering or readability - a busy pane footer, a harness-turn verdict, an unreadable `unknown` verdict - are bounded by `FM_BUSY_TURN_MAX_SECS` against the task's own latest `state/<id>.turn-ended` marker, or its `state/<id>.meta` spawn record before any turn completes; past that bound the window surfaces once as a turn-bound candidate for inspection only, never an automatic interrupt, signal, or restart.
Each current-state read is itself capped at `FM_CREW_STATE_READ_TIMEOUT`, and a capped read classifies as `unknown` rather than as a healthy disconfirmer, so a hung backend can neither stretch a supervision cycle nor fake progress.
The normal-mode watcher records each task-bound stale candidate in `state/.stale-candidate-<key>` as `<task>:<class>`, so identical candidates are deduplicated across pane churn, a replacement task reusing the same backend target is a transition rather than an inherited verdict, and only a candidate transition wakes firstmate.
...
The always-on watcher uses that library's absorb classification on no-verb signals and its task-bound supervision classification on stale panes, while the daemon maintains distinct wedge and declared-pause recheck cadences.
