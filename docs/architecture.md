...
Endpoint death is the only process-level override: a busy record on a positively absent local endpoint yields dead, while a remote, unreadable, or ambiguous endpoint keeps its recorded verdict, and a backend with no recovery-grade classifier answers unverified, so the failed presence probe itself is that positive absence; child processes, CPU, process sleep state, and marker modification times are not state signals.
