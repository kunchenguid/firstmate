...
Delivered work waiting on a merge nobody in the fleet controls takes the same bound only when its last status verb is `done:` and the watcher verifies both an authenticated merge-poll snapshot and a successful bounded forge lookup returning a recognized unmerged state, then revalidates the snapshot after that lookup.
`bin/fm-pr-lib.sh` owns snapshot authentication, including the registration and exact static template; poll-file presence alone never qualifies, and `bin/fm-pr-poll-health.sh` supplies lookup evidence without changing the registered poll template or requiring re-registration.
The first new hash still alarms, subsequent new hashes are absorbed on the same cadence, and the declaration carries the pull request plus the status-log signature, so a different pull request or any new status line re-alarms immediately.
An open captain call owns the cadence first; the merge-poll bound does not cover blockers, open decisions, failures, deliveries without a poll, invalid registrations, or failed or unrecognized forge lookups, which retain their existing stale-wake routing.
`tests/fm-watch-triage.test.sh` covers healthy throttling, broken polls with files still present, and previously registered baseline polls through the updated watcher.
...
`fm_pr_metadata_identity_parse` in `bin/fm-pr-lib.sh` owns which metadata appended after a PR identity preserves poll authentication.
...
[`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh)'s header owns the no-run fallback, including positive agent-death precedence over retained busy records and the preservation of non-working status-log states.
