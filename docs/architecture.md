...
The most recent recognized ci log marker wins, and only a positive green marker reports done: a later re-arm, failed-check, or issue marker returns the crew to working, and so do a PR with no CI checks configured at all and an unreadable ci log.
