...
A recorded clause acts only when the supervision session cites it to a guarded gate that accepts clause citations (`bin/fm-lease-lib.sh`).
On Pi and pi-signed the away daemon is never launched: the one supervision session continues under the record in the away posture, taking every row with main parked and its standing authority relocated to the session ([pi-supervision-branch.md](pi-supervision-branch.md#postures)).
