...
Herdr's durable default container shape is workspace-per-home plus tab-per-task, with recovery and list-live scoped to the current `FM_HOME`'s workspace; [Watching and task containers](herdr-backend.md#watching-and-task-containers) owns the visible labels and upgrade-compatibility contract.
