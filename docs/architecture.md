...
When a task declared a non-default intended base with `fm-brief.sh --base` (recorded as `base=` in meta and flowing through the `data/<id>/base` sidecar promoted by `fm-spawn.sh`), `fm-pr-check.sh` refuses fail-closed before recording `pr=` or arming the merge poll unless the freshly fetched PR head is rooted in that base's unmerged history and the PR's base label targets it.
Rootedness is tested as "the merge-base of the PR head and the base is a commit the default branch cannot reach", not as "the head descends from the base's current tip", because a stacked PR whose own base is still under review sees that base advance routinely and a merely-behind head is correctly based and safe to merge.
...
The guard does not steer the no-mistakes pipeline, which has no base mechanism and always rebases onto the repo default and opens the PR there; the recovery is to retarget the PR's base with `gh-axi pr edit <n> --base <branch>`, after which the pipeline's monitor re-rebases the branch onto the new base and force-pushes a clean head.
The guard's contribution is that a wrong-based PR cannot merge unnoticed: it blocks at the PR-ready check instead.
A declared base that no longer exists on origin is a third state and is not a failure: it is the normal end-state of a stacked PR, whose base merges and is then auto-deleted, so no unmerged feature history is left to drag into the default branch and the hazard is gone with the branch.
Both `fm-pr-check.sh` and `fm-review-diff.sh` therefore stand the base handling down with a loud notice and fall back to ordinary default-branch behavior, rather than deadlocking a legitimate merge behind a recovery (`gh-axi pr edit --base <branch>`) that a deleted branch makes impossible to follow.
An origin that cannot be asked at all is not the same thing and still fails closed, and [`bin/fm-base-lib.sh`](../bin/fm-base-lib.sh) owns that present/absent/probe-failed distinction (`git ls-remote --exit-code`) so the two consumers cannot drift.
`fm-review-diff.sh` consumes the same `base=` so a based task's pre-merge review diffs against its intended base rather than presenting the feature base's unmerged history as the crewmate's change.
