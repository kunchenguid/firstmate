...
An exact busy verdict reports working, an exact idle verdict permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
