...
Where authoritative state cannot name the wait - a captain hold, which has no current-state mapping, or a worker standing at a harness approval gate it cannot report - the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint; in that unnamed case the pause classification itself is recovered without that first wake only when the backend confidently reports an ordinary crew's agent dead.
Live or inconclusive liveness remains fail-open at that initial surface of an unnameable wait, so a worker genuinely waiting on a decision its own state could not report is never silenced by a declaration it left behind.
