...
A task whose intended base is a non-default branch declares it with `fm-spawn.sh --base`, which records `base=` into `state/<id>.meta`, the single source of truth for a task's base (`fm-brief.sh --base` only shapes the brief's prose and records nothing).
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh) then guards the PR's base before recording `pr=` or arming the merge poll, and [`bin/fm-review-diff.sh`](../bin/fm-review-diff.sh) reviews a based task against that base rather than presenting the feature base's unmerged history as the crewmate's change.
Both consumers, and `fm-spawn.sh`'s own accept-or-refuse check, decide on one shared predicate: [`bin/fm-base-lib.sh`](../bin/fm-base-lib.sh)'s liveness question, the single owner of what a task's base is (still on origin AND still carrying at least one commit the default branch lacks), so the merge gate and review can never tell different stories about one head.
The guard covers the live-base case only: a base the default branch has already absorbed is verified as the ordinary default-branch PR it now is, and a base that is gone from origin, or unresolvable, is handed to a human rather than adjudicated, because a merged base and an abandoned one are not distinguishable from git without a guess.
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh)'s header owns the guard contract in full, including what it deliberately declines to decide.
With no `base=` recorded, the guard is skipped and default-branch tasks are unchanged.
