...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, unless a live declared merge wait (`state/<id>.merge-wait`) absorbs that wake as it is emitted, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, declared merge waits that remain declared past that same cadence wherever an alarm attempt recurs, and heartbeat backstop hits.
...
A checks-green ship task waiting on a merge nobody in this fleet can perform is absorbed from a durable declaration (`state/<id>.merge-wait`, owned by `bin/fm-merge-wait.sh`) rather than a status verb, because its last status line is a `done:` report and not a pause.
That absorb is deliberately not a classification: `emit_stale_wake` consults the declaration at the single point every suppressible stale wake is emitted, after whichever owner already classified the window, so it can never outrank the hash-changed, paused, captain-held, provably-working, terminal, or wedge-timer owners, and a crew's own `paused:` or `captain-held` line keeps its wait and its wording.
Each emit site states which alarm class it belongs to, and only the idle-pane stale class may be absorbed: the busy-pane completed-turn bound above keeps escalating a possible hung foreground call, which a pending merge does not explain, and away mode absorbs nothing at all because the daemon owns that triage.
It absorbs only the wake itself, and only while the record still names the pull request the task records, that task's own merge poll for that pull request is still armed (its published registration, not merely some armed check occupying the slot), and authoritative crew state still reports the run step's terminal checks-green outcome; that verdict is always the one read for this alarm rather than a cached one, so a withdrawn, superseded, or no-longer-green declaration reaches a real alarm on the attempt it stops holding.
One reminder per `FM_PAUSE_RESURFACE_SECS` passes through wherever an alarm attempt recurs, which covers a pane whose hash keeps changing and the non-terminal wedge window, but a byte-static pane on the terminal branch attempts no alarm once its hash is recorded, so no reminder fires there.
What covers that pane instead, and why no separate timer exists for it, is the merge poll: it stays armed and is what reports the merge, and `bin/fm-merge-wait.sh` refuses to declare without it, naming `bin/fm-pr-check.sh` when the slot holds some other armed check instead.
That is one cover rather than two, and thinner for it: the `heartbeat:` fleet review does not reach this population on its own, because the absorbed wake still marks the green status line surfaced and the fleet review only runs in response to a heartbeat wake that therefore never arrives, so it helps only when unrelated fleet activity happens to raise one.
A declaration whose poll has died is the known exposure; a fleet-level check that every declared merge wait still has a live poll is filed as separate work rather than added here.
That matters because a recorded terminal checks-green outcome is immutable at the same head, so no crew-state verdict can see a pull request that later goes red or is closed there.
