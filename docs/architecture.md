...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-control.sh`, `fm-teardown.sh`, and `fm-orphan-reap.sh reap` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
Only `fm-orphan-reap.sh`'s `reap` verb refuses; its read-only `scan` reports without stopping anything, so the session-start digest keeps working from inside a gate.
