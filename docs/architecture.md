...
A `done:` event is read as a landing only when the task's recorded delivery mode actually ends in one: on a `no-mistakes` or `direct-PR` ship with no PR known from the line, the task's `pr=` record, or its own status log, that event reports `parked` and says the pipeline still owes a PR, because the generated contract asks the worker to report completion on its implementation commit before the pipeline has produced anything.
