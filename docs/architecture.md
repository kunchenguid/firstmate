...
It absorbs only the wake itself, passing one reminder through per `FM_PAUSE_RESURFACE_SECS` rather than going quiet, and only while the record still names the pull request the task records, the merge poll is still armed, and authoritative crew state still reports the run step's terminal checks-green outcome; every refusal is per-alarm, so a withdrawn, superseded, or no-longer-green declaration reaches a real alarm on the next attempt.
Because a recorded terminal checks-green outcome is immutable at the same head, that recheck cannot see a pull request that later goes red or is closed there, which is what the bounded reminder exists to catch.
