...
Paseo uses one shared workspace per project, titled `firstmate` (or `2ndmate-<id>` in a secondmate home) when Firstmate creates it, with one terminal tab per task; the terminal's home-scoped NAME is the routing authority, the title only adopts the shared workspace, and Paseo refuses `--secondmate` spawns, mirroring Orca and cmux.
