...
Away mode is the one exception to that routing, and the declared-pause cadence below owns it.
...
The away-mode sub-supervisor extends that cadence to a declared-paused pane that reads busy: it absorbs the busy-pane turn-age wedge escalation described above, ages the pause window against the crew's own latest status line rather than against pane busy state, and so re-surfaces a pane that keeps rendering busy behind a stuck external wait once per `FM_PAUSE_RESURFACE_SECS` instead of going silent.
The always-on watcher does not extend it that far, so a declared-paused pane that stays busy past `FM_BUSY_TURN_MAX_SECS` still wakes the first mate on the possible-wedge reason.
