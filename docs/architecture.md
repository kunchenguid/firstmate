...
Teardown is fail-closed for every task worktree, scout lanes included: an uninspectable recorded path refuses, a worktree held by another task's branch refuses, dirty worktrees refuse, and committed work must be landed before the worktree is returned.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the worktree-ownership proof, the landed-work proofs, the PR-discovery fallback, and the stale-lock recovery procedure.
