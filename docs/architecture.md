...
A declared clearing time (`paused: ... until <UTC ISO 8601>`) that has already passed stops counting as that account, and what the pane gets next is decided by evidence independent of the declaration rather than by the declaration itself: an attributed active run, observed tool progress inside the quiet window, or worktree writes - never the pane, whose unchanged hash is what the ladder is judging.
That evidence is deliberately narrower than any sign of life: an attributed active run, the recorded progress marker, or a worktree write counts, while a busy footer on a pane whose hash has not moved for a whole escalation window does not, because that pane is the wedge suspect the ladder exists to judge.
With that evidence the lane is rechecked on the long cadence as a slipped estimate, named as one, because a worker who declares a time must not be alarmed harder than one who stays vague; without it a lane whose own wait is over keeps the unchanged escalation schedule, climbing count and `demand-deep-inspection` wording exactly as a lane that never declared one does - an expired declaration with nothing to show is more suspicious than no declaration, not less - and its escalation additionally names the time that lane declared and how long ago it passed, so the reader can weigh the alarm without opening the status log.
The slipped-estimate deferral leaves the escalation counter exactly where it was, so the ladder resumes from that count the moment the evidence stops rather than restarting at one, and a lane that works in bursts cannot hold the alarm off forever.
...
Every RECHECK a declared wait produces - the bounded absorber, the wedge threshold's deferral, and the live first-sight surface - names that wait: which verb declared it, which human it blocks on, and the action that clears it, so a reader can separate a bounded recheck from a possible wedge without opening the status log, and the half reached while liveness is fail-open reports only that the agent is not confirmed gone, which is all that path establishes: it is reached for a proven-alive agent and for every inconclusive read alike, so claiming the agent is live would read as reassurance nothing verified.
A wedge escalation and the away-mode handoff deliberately carry neither verb nor human nor action: the escalation is not a recheck and says only what was declared and that its time passed, and the handoff queues the bare stale identity because the away-mode daemon classifies the declaration itself.
Every declared time these paths print is the worker's own token character for character, never a re-rendering of it, and it is the only worker-authored text any of them carries into the durable wake queue - accepted first by the shared UTC reader, so its two exact shapes bound what can reach a tab-separated row.
A lane that declared nothing keeps the unnamed stale identity and the unchanged wedge ladder, so naming a declared wait narrows which alarms carry that wording and never what counts as a wedge.
