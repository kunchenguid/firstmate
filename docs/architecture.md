...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working: an active no-mistakes run attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes only the crew branch's newest no-mistakes run, active or terminal, and only when that run matches the crew's current code identity, then keeps that run-step authoritative even if the pane has closed.
A newer run supersedes every older run on the branch, so a newest run that fails that identity check leaves the crew with no attributed run instead of reviving an older, superseded verdict.
