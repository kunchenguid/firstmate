...
A matched no-mistakes CI monitor is passive rather than active when its structured run state records a PR, has completed every pre-CI step, has only `ci,running` attached, and has no gate, fixing state, or worker activity.
A `paused:` event newer than that run may override only that passive state, and only when the run's own structured head is exactly the worktree's current head and the semantic worker is affirmatively idle or confidently gone; no pause prose is parsed, so an approval note carries no authority of its own.
The watcher absorbs that one canonical run-step pause and re-surfaces it only on the longer pause cadence, rather than treating it as a possible wedge, because the current-state reader has already proven both the head binding and worker idleness.
Every other pause keeps the live-worker safeguard: for an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint only when the backend confidently reports its agent dead.
Live or inconclusive liveness remains fail-open at that initial surface, so an active external-decision gate is never hidden behind the pause cadence, and the secondmate idle-endpoint exemption is unchanged.
...
Every file timestamp and metadata read in that supervision graph, and in the fleet read path, goes through `bin/fm-stat-lib.sh`, which probes the real `stat` binary once and binds the matching format flag, then refuses any value that is not well formed.
The host's kernel name does not determine which `stat` is first on `PATH`, and a format-flag fallback chain can exit successfully with a diagnostic dump spliced into its output, so neither may decide the format; the readers that age the watcher beacon, prove a git lock abandoned, bound a pause recheck, age a fleet status event, gate the herdr presentation lock namespace, and validate single-link private artifacts all depend on that one probe.
`tests/fm-stat-flavor.test.sh` enumerates exactly which files that claim covers and fails when one of them reads `stat` directly again.
That owner also normalizes the two flavors' disagreement about what a mode is: it always reports the full permission word, so a setuid, setgid, or sticky bit makes a `0600` or `0700` private-artifact check fail on macOS exactly as it already does on Linux.
