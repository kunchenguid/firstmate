...
The once-marker records the pane state it was reported for, so it re-arms when that endpoint reads live again and when the pane is replaced: a replacement launched into the same window escalates as a wedge normally and its own later death is reported again, even when no threshold probe reads the replacement alive in between.
