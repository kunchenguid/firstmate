...
A recorded endpoint that a successful backend inventory no longer lists surfaces once on its own evidence, before and independently of any declared wait, because `paused:` declares a pending external dependency and never that the worker itself may disappear.
