...
On Codex surfaces where background-task completion is not a verified assistant wake channel, `bin/fm-watch-loop.sh` is the present-mode fallback: it holds a singleton `state/.watch-loop.lock`, keeps `state/.watch-loop-beat` fresh while a child watcher blocks, drains queued wakes through `bin/fm-wake-drain.sh`, and immediately re-arms without enabling away-mode injection semantics.
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if tasks are in flight and watcher supervision stops running, if queued wakes are waiting to be drained, or if Codex has only a fresh one-shot watcher on an unverified wake channel.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work is in flight and no acceptable watcher supervision is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
