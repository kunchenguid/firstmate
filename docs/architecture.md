...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without an in-progress validation run-step and without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pane whose attributed no-mistakes run-step is still working (`bin/fm-crew-state.sh` reporting `working` from source `run-step`) is deferred instead of escalated, because an in-progress pipeline round legitimately leaves the pane idle between gates while a background shell holds the blocking call; a parked decision prompt is not that signal and keeps the unchanged wedge schedule.
That consult is wall-clock bounded (`FM_WEDGE_RUN_STEP_TIMEOUT`) and taken only in the branch that was about to escalate, never on every poll; a timed-out or failed read is no evidence, so a prompt still alarms.
A pane holding a file newer than the start of its own quiet window, anywhere in the worktree recorded for that task, is also deferred, because a crew writing source, then tests, then documentation behind a static pane is liveness the pane cannot show.
Both deferrals re-surface on the same `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait, with a reason naming the run-step or write evidence rather than a wedge, and the write probe is bounded to one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) taken only in that same at-threshold branch.
...
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge path, with the identical reason, escalation count, run-step deferral, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
