...
A crew that declared an external wait (`paused:`) is the one exception to that bound: the declaration itself identifies the live endpoint's long-running external wait, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation regardless of endpoint-liveness reads.
A verified captain-held transfer still relies on its busy verdict to identify the long-running foreground call and takes that bounded recheck only under its stricter endpoint-liveness gate.
...
For an ordinary crew, an unchanged `paused:` declaration takes that cadence whatever the backend reports about endpoint liveness, because the worker itself declared the external wait its live endpoint is sitting in.
Reading liveness instead let a live declared wait enqueue a bare stale wake on the first sight of every new pane hash, so a ticking harness footer cost one handling turn per tick while the polls in between logged the same declaration as absorbed.
A durable `captain-held` transfer keeps the stricter gate and takes the cadence only when the backend confidently reports its agent dead, so a live decision gate still surfaces one stale wake first and live or inconclusive liveness remains fail-open there.
Once the current pause recheck window lapses - it is bounded by `FM_STALE_ESCALATE_SECS`, not by the longer pause cadence - an authoritative working verdict outranks either declaration and restores the wedge timer, and lifting the declaration returns the pane to ordinary wedge handling on the next poll.
