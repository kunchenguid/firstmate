...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own validation reporting progress or their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pane whose own no-mistakes run reported activity inside its quiet window is suppressed outright, with no wake at all, because escalating a crew the supervisor can prove is working is a contradiction.
This exists because a fix round takes custody of the branch and works in its own gate clone, which silences the pane and the worktree together and leaves the run's own activity clock as the only source that separates a long foreground validation from a wedge; a review step routinely runs far longer than `FM_STALE_ESCALATE_SECS`, so without it a busy fleet produces a steady stream of false escalations.
Raising that threshold is deliberately not the answer, because it would buy quiet by slowing detection of a genuinely wedged crew; instead the threshold governs only the case where nothing can prove liveness.
Mere presence of an active step is not that proof: a frozen run keeps one indefinitely, so only a recent activity age suppresses, the idle window restarts from that proof, and a run that stops reporting progress escalates on the unchanged schedule.
A pane holding a file newer than the start of its own quiet window, anywhere in the worktree recorded for that task, is instead deferred, because a crew writing source, then tests, then documentation behind a static pane is liveness that neither pane quietness nor the run step can show; that deferral re-surfaces on the same `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait, with a reason naming the write evidence rather than a wedge.
Each probe is bounded and taken only in the branch that was about to escalate, never on every poll: one bounded `no-mistakes axi status` call (`FM_RUN_ACTIVITY_TIMEOUT`), and one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`).
Every absence of evidence, including a missing worktree record, a torn-down worktree, a run that is not this crew's or has concluded, an unresponsive CLI, a walk that outlives its wall-clock bound on a hung mount, and a failed walk, leaves the existing escalation schedule untouched, so a crew that proves nothing still escalates exactly as before.
A pipeline that keeps logging indefinitely without converging is the accepted residual: this detector does not surface it, and it is bounded instead by no-mistakes' own idle timeout and fix-round caps, plus the status append any gate or terminal state produces.
A secondmate is never probed by either, because the worktree recorded for it is a provisioned firstmate home rather than a validating code tree: its own supervision keeps writing inside it whether or not the mate produces anything, and it never runs a validation of that home, so its panes keep escalating on the unchanged schedule.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, run-activity suppression, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
