...
Any `.status` file in the batch, an absent or invalid index, an identity mismatch, new status bytes, steering-inbox activity newer than the covering outcome, a pane seen busy again after a covered stale absorb, or an exhausted bound surfaces exactly as before.
...
An unchanged pane under that throttle is rechecked every poll, so a static finished pane re-surfaces on the same cadence, and the first idle sight after the pane was seen busy again surfaces instead of being absorbed.
