...
A task whose workspace was already released (`workspace_state=released`, `worktree=` cleared) is reviewed in the durable project clone with no checkout and no reconstructed workspace: the helper fetches the exact `refs/pull/<n>/head` and the PR's actual recorded base branch (`workspace_base=`, so a stacked PR diffs against the branch below it rather than trunk) into `refs/fm-review/<task-id>/`, requires the fetched head to equal the recorded `pr_head=` (or the journaled `workspace_head=` when none was recorded), and refuses on missing, mismatched, or unfetchable identity instead of falling back.
