...
At the stale escalation boundary, normal and away-mode supervision re-read that owner and refresh the timer only when a full branch-and-current-code-matched active step reports `last_activity` newer than `FM_STALE_ESCALATE_SECS`; that age is the whole test, so no-mistakes' own `quiet` marking is not a separate veto, and a coarse, terminal, mismatched, absent, malformed, or unreadable run preserves escalation.
