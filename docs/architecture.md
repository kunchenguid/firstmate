...
The tracked primary hooks stand down under that marker when a firstmate-repo task worktree inherits them, and `bin/fm-test-run.sh` refuses to execute the behavior suite from the primary checkout while it is set; the runner's header owns that refusal.
[`tests/fm-primary-hook-scope.test.sh`](../tests/fm-primary-hook-scope.test.sh) pins both hook directions, and [`tests/fm-test-run.test.sh`](../tests/fm-test-run.test.sh) pins the runner refusal.
