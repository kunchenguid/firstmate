...
Once an endpoint is flagged paused, its recheck window is measured from the status log and a re-surface throttle marker rather than the pane hash, so a parked pane that keeps redrawing rechecks on the same configured interval as a static one; only a real transition - the status no longer declaring the wait, the crew becoming provably working again, or a proven busy worker - drops that tracking and returns the pane to ordinary stale escalation.
