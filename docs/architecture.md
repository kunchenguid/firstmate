...
A `.status` or `.turn-ended` file with no matching `state/<id>.meta` is an orphan left behind by a torn-down task (or a leaked turn-end hook writer that outlived teardown's best-effort process kill); the watcher never classifies it at all, removing it before it can wake anyone, even under afk.
A `.check.sh` file is instead gated by the check-trust registration in `fm-check-register.sh`; an unregistered check is rejected and surfaced, not silently absorbed - the one named exception is `state/x-watch.check.sh`, the X-mode relay shim, which is validated separately and has no per-task meta by design.
