...
After the reset plus `FM_RATELIMIT_MARGIN`, the watcher (or, in away mode, the daemon) submits `continue` to a still-idle limit/empty pane, clears the marker on a confirmed empty composer, and appends a silent `ratelimited-resumed` queue record; if the submit cannot be confirmed after `FM_RATELIMIT_MAX_RESUMES` attempts it writes `state/<id>.ratelimit.failed` and escalates, and from then on stops parking that pane so it returns to normal stale/wedge escalation rather than being suppressed into silence.
