...
A run head the task copy cannot resolve locally is attributed when the pipeline's own runs ledger proves it is an active continuation of the submitted head, so a pipeline fix round never reads as an older failed run, or when the run is parked at a gate, whose head is routinely unpushed for the whole review..lint window, so a crew waiting there is never absorbed as a busy or idle pane; a terminal run still has to prove its head.
