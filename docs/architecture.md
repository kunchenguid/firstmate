...
`data/projects.md` records each project's standing posture and optional `+yolo` merge flag as the captain's default and as context for that decision, plus an optional declared clone path; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
`bin/fm-project-mode.sh` remains the one registry parser for mechanical consumers, including fleet sync, local and remote home seeding, and the spawn's registry-deviation notice.
