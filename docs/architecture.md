...
A crew that has declared `paused:` is the one exception, and the declared-pause cadence below owns what the away supervisor does with those escalations.
...
A crew that declares `paused:` for a known external wait is separately absorbed and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, whether its pane reads idle or busy.
Away mode holds that contract at the supervisor too: a current declaration absorbs the busy-pane turn-age wedge escalation described above, and the away supervisor ages the pause window against the crew's own latest status line rather than against pane busy state, so a pane that keeps rendering busy behind a stuck external wait still re-surfaces once per `FM_PAUSE_RESURFACE_SECS` instead of going silent.
