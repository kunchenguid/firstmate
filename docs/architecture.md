...
Tracked-file dirty clones, non-default branches, detached HEADs with unique commits, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched, as is a clone whose re-attach checkout or fast-forward Git refused (for example over a colliding untracked file), with Git's reason appended.
...
Targets with tracked-file changes, uniquely diverged, offline, and off-default targets are reported and left untouched, and genuine secondmate divergence remains visible through a durable reconciliation record until a later successful convergence clears it.
