...
Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) provides clean worktrees for tmux, herdr, zellij, and cmux tasks, while Orca creates its own worktrees for `backend=orca`.
Treehouse pools are deterministically scoped by owning Firstmate home, and task workspaces have zero idle retention plus remote-branch reconstruction for later PR continuation; [Task workspace scope, retention, and reconstruction](configuration.md#task-workspace-scope-retention-and-reconstruction) is the single contract owner.
...
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation and base freshness, [`tests/fm-workspace.test.sh`](../tests/fm-workspace.test.sh) for scope/release/reconstruction, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving or reconstructing the recorded copy on relaunch.
...
A task whose workspace was already released (`workspace_state=released`, `worktree=` cleared) is reviewed in the durable project clone with no checkout and no reconstructed workspace. The forge's current canonical PR identity is authoritative there, because a head can advance and GitHub retargets an upper stacked PR once the branch below it merges: the helper reads the PR's current head commit, base branch, and base commit, requires the answer to be for the recorded `pr=` URL, fetches `refs/pull/<n>/head` and that base branch into `refs/fm-review/<task-id>/`, requires the fetched head to equal the forge head and the forge base commit to lie on the fetched base branch, then atomically refreshes `pr_head=` and `workspace_base=` and prints a `changed:` line for an advanced head or a retargeted base. Unavailable identity, forge access, fetch, or hash proof refuses instead of falling back. `bin/fm-pr-check.sh` likewise reads a released record's head from the project clone so a rerun keeps `pr_head=` current, and final teardown retires the task's `refs/fm-review/<task-id>/` and `refs/fm-workspace/<task-id>/` refs.
