...
For every fresh non-Orca ship or scout, `fm-spawn.sh` acquires the pooled worktree with `treehouse get --lease --lease-holder <task-id>`, durably leasing the slot to the task rather than relying on pid liveness, so an idle or concurrently live worker's worktree is never recycled; `fm-teardown.sh`'s `treehouse return` frees it once the task lands, and `fm-spawn.sh`'s own abort cleanup returns it directly if the spawn itself fails after acquiring the lease but before recording the task's `state/<id>.meta`.
