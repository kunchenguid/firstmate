...
That section states the base as a fact for the direct-PR and no-mistakes paths only, since `bin/fm-merge-local.sh` still fast-forwards the default branch for a local-only task.
