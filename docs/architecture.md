...
Verified harness templates are wrapped in `bash -c` before being sent into tmux, so firstmate does not depend on the pane's interactive shell understanding bash-only template syntax.
