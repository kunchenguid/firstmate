...
Those repeats are driven by evidence rather than by elapsed time alone: the watcher re-reads the crew's authoritative state every `FM_STALE_ESCALATE_SECS`, escalates at once when that verdict changes, and otherwise doubles the wait before the next escalation up to `FM_WEDGE_ESCALATE_MAX_SECS`, so a legitimately long quiet validation step is not re-reported as a fresh possible wedge every few minutes while the first escalation still lands at `FM_STALE_ESCALATE_SECS`.
A state read that fails outright is a distinct unreadable verdict rather than a stopped crew: it re-arms the probe schedule without advancing the ladder, without shortcutting the backed-off window, and without any wake claiming the work signal is gone, and a re-read that comes back `paused` is handed to the pause cadence instead of escalated.
A reader that stays broken is still surfaced rather than silently absorbed: `FM_WEDGE_UNREADABLE_SURFACE_COUNT` consecutive unreadable probes wake the first mate with a reason that names the failing reader and asserts neither a wedge nor a stop, repeats of that wake are spaced by the same doubling backoff the escalation ladder uses, and any readable verdict drops both counts.
...
Live or inconclusive liveness still surfaces once at that initial sighting, labeled as the declared pause it is rather than as a bare stale, and the secondmate idle-endpoint exemption is unchanged.
That one sighting is recorded against the declared pause itself, not against the pane's contents, so an idle harness pane that merely repaints cannot ask for it again, while a genuinely different pause later declared on the same window is a new gate and gets its own single sighting rather than inheriting the previous one's suppression.
...
The always-on watcher also uses that library's absorb classification on no-verb signals and first-sighting stale panes before status-log terminality is trusted, and as the bounded periodic probe that paces repeat wedge escalations on fresh evidence, while the daemon maintains distinct wedge and declared-pause recheck cadences.
