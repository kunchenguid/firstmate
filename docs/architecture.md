...
thurbox is the second backend with a native one: its `hook_state` is written by agents' own lifecycle hooks (`thurbox-cli session signal`) in herdr's exact `working`/`blocked`/`done`/`idle` vocabulary, so a native busy verdict is trusted on the same terms as herdr's, while native idle and a not-yet-signalled null both stay `unknown`.
