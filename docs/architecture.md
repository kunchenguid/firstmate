...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint; the pause classification itself is recovered when the backend confidently reports its agent dead, or confirms the agent alive but its busy verdict cannot be resolved, because a well-formed declared pause is status-log evidence that is believed even when the terminal cannot be classified.
Live liveness with a resolvable idle or busy verdict, and liveness itself reading inconclusive, both remain fail-open at that initial surface, so a worker genuinely waiting on a decision - or one whose endpoint cannot even be confirmed alive - is never silenced.
