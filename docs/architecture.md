...
Review diffs go through `bin/fm-review-diff.sh`, which refreshes the authoritative base and, when task meta records `pr=`, asks the `bin/fm-scm-lib.sh` provider seam for the current remote head before using a reachable recorded `pr_head=` as an offline fallback and finally the local branch with a warning.
