...
Orca is experimental and selected only explicitly: ship and scout tasks get firstmate-owned git worktrees plus daemon sessions, while secondmates attach one persistent daemon session to their pre-existing home.
It records `orca_worktree_id=` and `terminal=`, and removes ship/scout worktrees through guarded git worktree cleanup plus `bin/fmod kill` only after the usual firstmate teardown checks pass.
Its current behavior and limitations are recorded in `docs/orca-backend.md`.
...
cmux's container shape is one workspace per task with one surface, no per-home container split; workspace titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path, and `--secondmate` spawns are refused until a per-home design is verified.
