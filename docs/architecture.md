...
For capable herdr sessions, the same watcher replaces its terminal sleep with a bounded native event wait that immediately surfaces `blocked`; the watcher script header owns the outer deadline and exact-process-tree cleanup, while [herdr-backend.md](herdr-backend.md#native-paneagent_status_changed-push-escalation-immediate-blocked-wake) owns the backend mechanism, capability gates, and verification evidence.
