...
The existing turn-end guard remains the final backstop for every verified primary protocol, with pi-signed sharing Pi's protocol, OMP selecting its own lifecycle events through the shared extension core, the `--claude` mode cooperating with the auto-arm claim, and Cursor's `--cursor` mode rendering a block as one bounded follow-up because its `stop` step cannot be blocked.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, omp, grok, kimi, cursor, and muse while preserving the requested profile for later audit.
