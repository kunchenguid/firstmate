...
The terminal state of that escalation is a backend PROOF that the recorded endpoint no longer exists, read only where triage first classifies a distinct stale hash and in the branch about to escalate, so it costs at most one backend read per stale hash rather than one per poll, and only a successful, exact read counts as proof: the window's per-window watcher records are retired, no wake is emitted, one triage line is written, and the window is skipped before its capture for as long as that retirement holds (`state/.retired-<key>`, which records the mtime of the metadata it was taken against).
Any other mtime there releases the marker and the window rejoins ordinary triage, so a relaunch onto the same recorded target is supervised again, and a steering-inbox instruction written after a retirement still escalates for worker recovery; every ambiguity - an unreachable server, a failed read, a husk that still exists - escalates on the unchanged schedule instead.
...
The arm layer records one bounded lifecycle row per observed cycle in `state/.watch-cycle-exits.log`, and keeps each cycle's own stderr - the reason line a failed or signaled cycle prints - in the bounded `state/.watch-arm-stderr.log`, because an adapter that holds the arm's stderr in memory loses it when the arm exits; `state/.watch-triage.log` remains exclusively the absorbed-wake debug log.
