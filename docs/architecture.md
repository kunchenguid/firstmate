...
Only a backend that confidently reports the agent dead breaks the declaration, since nothing is then left to end the wait, so that pane surfaces once for reconciliation and its per-hash suppressor and pause markers return it to the bounded cadence immediately afterwards.
