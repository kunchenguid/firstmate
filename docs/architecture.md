...
A pane hash covers the pane's CONTENT, not the harness drawing its own status, so what counts as a new hash is a change in the worker's output rather than in the harness's chrome.
pi's footer is a surface pi hands to extensions, so a status line showing a quota countdown, a clock, or a token counter repaints on a timer while the transcript above it never moves; hashing those rows made a finished worker look changed once a minute and re-alarmed its terminal status on every tick.
The hashed region therefore ends at the harness's own composer boundary - for pi, the closing separator of its separated composer, matched by [`bin/fm-composer-lib.sh`](../bin/fm-composer-lib.sh)'s structural row owner rather than by any footer text - and every other harness, plus a pi capture whose composer cannot be found, hashes the whole capture exactly as before.
The capture itself stays whole for the busy read, which needs the rendered footer the hash drops.
One derivation is shared by both hash consumers, so the staleness backbone and the turn-end pane-churn proof can never disagree about whether a pane moved.
...
A known bound: the recheck throttle is scoped to the pane hash, so the long cadence holds for a lane whose pane is genuinely static, while a lane whose content churns - new output redrawing the transcript - drops the throttle with each new hash and is rechecked once per idle window instead.
...
A home that creates `config/turnend-churn-absorb` lets each eligible bare turn-ended task that lacks either authoritative proof use a third form: pane content that changed since the previous poll, compared against the same `state/.hash-*` marker the staleness backbone records through the same content-hash derivation, which still needs no adapter cooperation and still reads rendered bytes rather than a harness work verdict.
...
That bound is load-bearing rather than cosmetic: churn and staleness share the same content-hash derivation, so a pane whose hashed region renders continuously - transcript output, a spinner inside the content area, a shell heartbeat, or a harness that leaves a background renderer alive after its agent yields - never reaches the staleness backbone's two-identical-hashes test either, and an unbounded churn absorb would leave a genuinely stopped worker behind such a renderer with no path left to surface it.
