...
A doorbell and a lifecycle action - `fm-control`, `fm-spawn --relaunch`, `fm-teardown`, or `fm-promote` - serialize on the same per-task control lock so neither types into an endpoint the other is changing: the lifecycle entry points wait a doorbell's brief hold out (`bin/fm-wake-lib.sh`), while a doorbell the lock defers types nothing, spends no re-ring budget, and is bounded by its own observed-deferral horizon instead (`bin/fm-task-inbox-lib.sh` header).
