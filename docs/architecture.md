...
Session start leaves those logs unchanged and emits a deterministic historical projection per task, reusing `fm-classify-lib.sh`'s existing authoritative open-decision fold rather than introducing a second one.
Per-event, per-task, and fleet-wide text budgets bound that prose so one long status log cannot dominate a session, and every truncation carries a receipt naming its original byte count and source path.
State verbs, decision keys, full PR URLs, exact heads, and run identities are lifted into structured fields before text is cut, so bounding prose never costs restart-recovery identity.
`bin/fm-session-start.sh`'s header owns the exact projected records, counts, and limits.
The projection never turns historical status into current state, and targeted `bin/fm-crew-state.sh <id>` reconciliation remains required whenever current state matters.
