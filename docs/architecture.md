...
A PR-requiring ship `done:` is captain-relevant only after the task's own branch is on the remote and a bounded forge read confirms an open PR in that task's repository; otherwise the watcher drops that completion and steers the worker to push (`bin/fm-done-guard-lib.sh`).
...
A secondmate retains a durable receipt for its idempotent report through the established parent route, and main-home captain presentation retains a separate receipt; neither path polls a PR or merge state, and the bounded ship-done gate inside that current-state read is the only forge call the scan can reach.
...
Terminal verbs remain captain-relevant, apart from a ship `done:` the shared done gate above refuses for both supervisors alike, while a nonterminal progress verb cannot become terminal merely because its prose contains a legacy free-text token such as `merged`; bare legacy free-text lines remain compatible.
...
`bin/fm-done-guard-lib.sh` owns the control-plane check that a PR-requiring ship `done:` names a pushed branch and an open PR.
