...
After those three probes, and still only in the branch that was about to escalate, Jev classifies the remaining candidate as `pipeline_wait`, `true_wedge`, or `healthy_idle`.
Only `true_wedge` still fires today's possible-wedge wake; `pipeline_wait` and `healthy_idle` suppress it, restart the idle timer, and leave the escalation counter untouched, which is what stops a ship waiting on CI or a healthy idle secondmate from climbing the ladder.
Any missing key, timeout, HTTP error, or malformed answer fail-opens to the unchanged escalate path and stamps `jev_triage.unavailable`.
`config/jev-wake-triage=off` disables the gate entirely.
`bin/fm-jev-wake-triage.sh` owns the request, the coarse class-only telemetry, and the first-20 calibration log; [`configuration.md`](configuration.md) "Jev stale-escalation triage" owns the operator contract.
A busy pane is otherwise exempt from staleness, but only until its last completed turn or explicit native-harness progress reaches `FM_BUSY_TURN_MAX_SECS` (`bin/fm-watch.sh` owns marker selection); past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write deferral, Jev triage, and `demand-deep-inspection` marker for a live agent and the same dead-record report when the endpoint is proven gone, for inspection only - never an automatic interrupt, signal, or restart.
...
Away-mode housekeeping has no worktree-write deferral and no Jev triage of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or waiting on a pipeline, still escalates as a possible wedge at that bound.
