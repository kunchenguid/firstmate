...
Two consumers read it: `fm-pr-check.sh` guards the PR's base fail-closed before recording `pr=` or arming the merge poll, and `fm-review-diff.sh` diffs a based task against its intended base rather than presenting the feature base's unmerged history as the crewmate's change.
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh)'s header owns the guard contract in full, and [`bin/fm-base-lib.sh`](../bin/fm-base-lib.sh) owns the shared declared-base predicates the two consumers decide on and must not drift apart on: the `git ls-remote` present/absent/probe-failed probe, whether the base's work has landed in the default branch, and whether a head is rooted in the base's unmerged history.
