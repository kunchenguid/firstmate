...
`data/projects.md` records each project's standing posture, optional `+yolo` flag, and optional `base=<branch>` override as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip and home seeding's refusal and no-mistakes initialization, and it owns the exact bracket-token contract including `base=`.
