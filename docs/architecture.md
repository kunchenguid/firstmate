...
Only when no matching run exists does it consult semantic busy state; before that fallback, a recovery-grade tmux or Herdr endpoint read reconciles a readable shell-only endpoint, so stale progress is unknown while terminal or blocked events remain supervision-relevant. Exact busy reports working, and exact idle permits fallback to the log's resolved current declaration - the newest decision the fold still holds open, otherwise the latest recognized event - when its verb maps to a recognized run-state; an unknown endpoint or stale progress on a dead pane stays unknown, while terminal or blocked status remains supervision-relevant when the agent is proven gone.
...
Snapshot completeness is conservative: an in-flight child whose current state is `blocked` remains visible in the home's `holds` surface, while intentionally skipped remote homes count as omitted coverage and keep Bearings `complete` and `proven_clear` false.
...
Supported non-raw secondmate launches whose harness has a verified semantic source use the same generation-bound lifecycle wiring for their parent-owned busy record, so the launch turn opens the record and the adapter's turn-close or shutdown event can close only that incarnation.
