...
A fourth rendered-text check also stays outside this contract, at the crew-state resolution layer rather than in `fm_busy_classify`: `bin/fm-crew-state.sh` overrides an already-busy `claude-hook` verdict to paused when the pane's own tail shows Claude Code's account/usage-limit banner, scoped to `harness=claude` and owned by `fm_busy_claude_limit_banner` (`bin/fm-busy-lib.sh`).
