...
That last exemption is what keeps displayed content from ordinarily being read as a meter: matching a counter shape proves only that the text looks like one, and a transcript line inside the footer region matches exactly as a real meter does while telling nothing about whether this worker is advancing.
A pane's numbers must therefore be seen ADVANCING at least once - one of them rising, which a real meter does within the first seconds of generation and on poll after poll thereafter, while text scrolling through the footer region rises only by chance and falls just as readily - before the short fuse applies to it.
That bar is not absolute: a reading keeps the largest value found per kind, so counter-shaped text that scrolls in higher than the number read before can still arm a pane that renders no meter, which is part of why this path only ever reports, keeps its other stand-downs, and records both the arming and the report in the absorbed-wake log.
