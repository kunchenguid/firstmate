...
The heartbeat cadence backs off exponentially while idle; a fleet whose every recorded in-flight window is `kind=secondmate` uses a slower default cadence than an ordinary crewmate fleet, because secondmate panes are expected to idle and parent supervision of a secondmate is status-write driven rather than pane-staleness driven (`FM_HEARTBEAT_SECONDMATE_ONLY`, `FM_HEARTBEAT_SECONDMATE_ONLY_MAX`).
Signal coalescing likewise extends past its base grace window in bounded steps while a burst of signals keeps landing, up to `FM_SIGNAL_COALESCE_MAX`, so a status write and a delayed turn-end marker from the same turn still collapse into one wake even when they land more than one grace period apart; each linger step refreshes the watcher's liveness beacon so supervision guards never see it as lapsed.
