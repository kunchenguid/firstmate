...
Unsupported effort values are still recorded in task meta when passed to `fm-spawn.sh`, but the launch template omits any effort flag that the selected harness does not accept. Codex `max` is the model-qualified exception: Firstmate emits it only for current catalog models that advertise `max`, and refuses before metadata or launch for a known non-supporting or unknown model.
