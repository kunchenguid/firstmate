...
If fleet-lock ownership is not verified for this session, including an uncertain Codex writer-lock state, both surfaces keep the alarm but switch to read-only wording with no repair command.
