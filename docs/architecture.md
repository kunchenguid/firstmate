...
That evidence is deliberately narrower than any sign of life: an attributed active run or the recorded progress marker counts, while a busy footer on a pane whose hash has not moved for a whole escalation window does not, because that pane is the wedge suspect the ladder exists to judge.
