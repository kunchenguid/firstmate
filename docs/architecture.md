...
A tmux pane whose own foreground process has kept running the whole quiet window is deferred the same way, on the same cadence, because screen quiescence alone cannot distinguish a wedged worker from one blocked on a long foreground command (a gate-response call, a bounded poll loop); the pane's foreground PID is recorded when the stale timer opens, and the deferral holds only while that same PID stays alive as the pane's current foreground process.
Every absence of that evidence, including a non-tmux backend, a pane with no readable foreground process, and a foreground process that has changed or died, leaves the existing escalation schedule untouched exactly like the write-deferral fallback above.
