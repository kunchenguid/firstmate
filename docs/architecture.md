...
`fm-classify-lib.sh` owns the decision (`status_declares_work`, `status_work_declared_unresumed`, `status_declared_work_stalled`) for the watcher and the away-mode daemon alike, and the endpoint read runs only after the pure status and idle-age gates already hold.
A declared `paused:` wait, a terminal last event, and a proven-busy endpoint each keep their existing meaning and never become a stall; an unresolved stall re-surfaces once per threshold window so it cannot rot invisibly.
