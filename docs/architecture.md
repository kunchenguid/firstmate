...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written and without recent pipeline activity, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Independently, a pane whose attributed no-mistakes run currently reports recent pipeline activity is deferred the same way: the one checkable condition is `crew_run_activity_is_recent`, which requires `bin/fm-crew-state.sh` to report `state: working`, `source: run-step`, and `activity: recent`.
That recency field is the live `axi status` `active_steps` table with no `quiet` prefix, the pipeline's own verdict; it is emitted only when the watcher asks for it (`FM_CREW_STATE_RUN_ACTIVITY=1`) and has no negative spelling, so the default crew-state line that `bin/fm-fleet-snapshot.sh` and `bin/fm-bearings-snapshot.sh` render as the `doing` column never carries it.
A working record that survived a relaunch, a coarse runs-list status word, a missing table, a quiet-prefixed step, a terminal or parked run, and no run at all keep the existing escalation schedule.
This deferral covers a worker waiting on an active pipeline round; a worker waiting on the `ci` step is deliberately not covered, because no live `active_steps` table exists during that phase, so its idle pane still escalates on the existing schedule.
The live-run deferral re-surfaces on the same `FM_PAUSE_RESURFACE_SECS` cadence with a reason naming the pipeline rather than a wedge, and the crew-state read runs only in that same at-threshold branch.
Both deferrals share one clock: a window's `state/.deferred-since-*` marker opens with the first deferral of the current quiet stretch and is not restarted when the reason changes, so a crew that hands off from a live pipeline round to writing its worktree, or back, re-surfaces at the same bound as if the reason had never changed; only a real escalation, where neither reason defers, or the pane going active clears it.
...
Away-mode housekeeping has no worktree-write or live-run deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or whose pipeline run is still active, still escalates as a possible wedge at that bound.
