...
A finished ship or scout whose agent has actually exited is closed through `bin/fm-teardown.sh --close-pane` instead of a stale wake, so an empty pane waiting on merge does not keep alarming while the isolated copy stays until landed teardown.
