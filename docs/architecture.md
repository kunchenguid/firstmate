...
Composer classification has one shared owner, `bin/fm-composer-lib.sh`: tmux, herdr, Zellij, Orca, cmux, and Paseo contribute only a screen capture plus declarative styled, cursor, identity, and row capabilities, while the shared classifier owns every shape and the `empty`/`pending`/`pending-unproven`/`unknown` verdict.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's and Paseo's primary markers and their documented fallback signals; auto-detected herdr, cmux, or Paseo prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij and orca are never auto-detected (only explicit selection).
...
tmux, zellij, orca, cmux, and Paseo expose no native busy primitive at all, so a task on those backends is classified purely from its adapter's own lifecycle record.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij remains unverified, and Orca, cmux, and Paseo do not support secondmate spawns.
...
Paseo is experimental, GUI-first, macOS-only, and can be selected explicitly or by runtime auto-detection from its primary `PASEO_AGENT_ID` marker plus a bundle-id fallback: Treehouse remains its worktree provider, [`paseo-backend.md`](paseo-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#paseo) owns active source and live evidence.
Paseo uses one shared workspace per project, titled `firstmate` (or `2ndmate-<id>` in a secondmate home) and adopted by cwd and title, with one terminal tab per task; the terminal's home-scoped NAME is the routing authority, the title only adopts the shared workspace, and explicitly selected Paseo refuses `--secondmate` spawns, mirroring Orca and cmux.
