...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating the task branch named by the home's task-branch prefix (`fm/<id>` by default; docs/configuration.md "Task-branch prefix"), then stop with a blocked status if it landed in the primary checkout.
