...
A crew that declares `paused:` for a known external wait, or carries a verified `captain-held` transfer, is separately absorbed while idle rather than being treated as a possible wedge; `handle_paused_stale` and `resurface_absorbed` in [`bin/fm-watch.sh`](../bin/fm-watch.sh) own its bounded recheck timing and away-posture suppression.
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that bounded recheck to an unchanged `paused:` or durable `captain-held` endpoint while attended; the pause classification itself is recovered only when the backend confidently reports its agent dead.
