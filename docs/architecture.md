...
A GitHub pull request additionally requires adversarial-review loop-green evidence: `bin/fm-pr-merge.sh` refuses once before arming any merge poll, so a refusal leaves no poll behind, and again against the live head it then binds with `--match-head-commit`, so the head the loop reviewed and the head that merges are one value rather than two reads a push can slip between.
That evidence also carries the tier the loop ran and the tier the reviewed change required, which makes the second check the minimum-tier enforcement at this boundary.
The marker is GitHub-shaped, so a GitLab merge request is outside what the loop covers today rather than waived, and the branch becomes provider-agnostic when the loop gains GitLab support.
The loop starts itself instead of waiting to be remembered: a single PR-open when-watch is re-armed by session startup and by every GitHub `bin/fm-pr-check.sh` registration, and its action dispatches every PR still missing a loop rather than only the first.
[`bin/fm-adversarial-review.sh`](../bin/fm-adversarial-review.sh)'s header owns the loop - tier derivation and floor, the per-round evidence and results posted into the PR itself, seat independence, findings carried across rounds, the captain-granted T0 waiver, and the marker's write and revocation - while the merge path only enforces its answer.
