...
The mate's own `wake()` transition atomically advances that durable timestamp alongside its bounded delivery log, so a handling turn stays quiet however long it runs, while an absent, unreadable, malformed, older, or future marker never suppresses an abandoned queue and the primary only reads the foreign marker and queue.
