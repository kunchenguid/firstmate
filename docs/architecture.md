...
Because a terminal outcome is a captain-facing claim, two rules bound what may be built from that read, and which one applies depends on where the reader sourced the state. A state sourced from the pipeline's own RUN STEP is authoritative: the crew's last self-declared word never vetoes it, so a run that failed after the crew wrote `done:` still reaches the captain instead of reaching nobody. A state sourced from the STATUS LOG is built only when that same self-declared word does not contradict it, because there the two records are genuinely comparable and a disagreement is left to ordinary supervision. Either way its pull-request identity comes from the same record as the state rather than from a separately recorded task PR, and a scout, which delivers no pull request, carries none on either path.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes an active or terminal no-mistakes run under the shared run-attribution contract, then keeps that run-step authoritative even if the pane has closed, except that a `blocked:` event reporting a refused or missing daemon socket outranks any attributed run record, active or terminal, and every terminal-precedence answer built from it, only while that socket-down declaration is itself the log's latest recognized event, since any later event, including another `blocked:` one, means the crew moved on.
...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh) owns branch, head, and pipeline-custody attribution, plus complete same-branch run selection, optional inventory lookup, ambiguity reporting, and the single structural validity rule for a runs-ledger row, so a row one ledger reader rejects is never evidence for another.
[`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh)'s header also owns the exact terminal-reading precedence and pull-request attribution rules.
The stable safety boundary is that a stale terminal reading - a failure or a success - cannot outrank newer authoritative evidence, uncertain later history reads unknown, and a terminal run publishes no pull request while a ledger row that exists contradicts it, though an absent row - routine for a long-inactive crew whose run has left the listing window - leaves the run's own pull request publishable.
