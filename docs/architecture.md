...
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root distinct from both the spawning project and its repository's primary checkout, including when spawning from a linked home.
...
Its header owns the exact refusal mechanics.
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation, shallow repair, and base freshness, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
