...
`no-mistakes` projects run the full validation pipeline, `direct-PR` projects open PRs (GitHub) or MRs (GitLab) without that pipeline, and `local-only` projects stay local until firstmate performs an approved fast-forward merge.
Review diffs go through `bin/fm-review-diff.sh`, which refreshes the authoritative base and, when task meta records `pr=`, compares against the reachable recorded `pr_head=` or a freshly fetched head ref - `refs/pull/<n>/head` for GitHub, `refs/merge-requests/<iid>/head` for GitLab (host inferred via `bin/fm-git-host-lib.sh`) - before falling back to the local branch with a warning.
...
PR- and MR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before merging.
The git host is inferred from the URL via `bin/fm-git-host-lib.sh`: a full `https://github.com/<owner>/<repo>/pull/<n>` URL invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, while a `https://<host>/<namespace>/-/merge_requests/<iid>` URL invokes `glab mr merge <iid> -R https://<host>/<namespace> --yes`; both default to `--squash`, preserve explicit merge-method flags, and reject malformed URLs or repo override flags before recording merge state.
