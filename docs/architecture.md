...
`fm_backend_source` refuses before invoking the shell source builtin when an adapter file is missing or unreadable, so callers reach their fail-closed refusal path consistently on stock macOS Bash and newer Bash.
