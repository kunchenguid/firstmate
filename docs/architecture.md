...
The optional Signal integration keeps account ownership inside one generic process-event runner generation and publishes authenticated readiness immediately before its foreground blocking receive without creating an adapter-owned background process.
That time-based boundary leaves a narrow race in which send can return just before receive reacquires the account, while fully closing the race would require the adapter-owned source backgrounding that this architecture forbids; [configuration.md](configuration.md#process-to-event-sources-stateprocevent) owns setup and operator mechanics.
Home-scoped lifecycle locking also leaves a residual cross-home race after retirement releases the claim and before the originating home restores it; closing that interval requires a machine-wide account-lifecycle serialization boundary, which this optional adapter does not provide.
