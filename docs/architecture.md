...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, metadata-only cleanup identity validation, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the verified reference adapter ([`docs/tmux-backend.md`](tmux-backend.md)), and `bin/backends/herdr.sh` (P2), `bin/backends/zellij.sh` (P3), `bin/backends/orca.sh` (P4), and `bin/backends/cmux.sh` (P5) are experimental task-spawn adapters, joined by `bin/backends/thurbox.sh` ([`docs/thurbox-backend.md`](thurbox-backend.md)), whose sessions are tmux windows on thurbox's own socket but which drives them entirely through `thurbox-cli` (session identity and lifecycle, plus the unsubmitted input, named keys, ANSI capture and cursor row that thurbox 2.10 exposes), so it runs no tmux command and does not require a tmux client.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr, cmux, or thurbox prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij and orca are never auto-detected (only explicit selection).
thurbox is checked before `$TMUX`, the one deliberate exception, because it is a tmux server plus a session database rather than a layer inside tmux; the `THURBOX_SESSION` marker is paired with a socket match, so a nested tmux on a different socket still resolves to `tmux`.
...
thurbox is the second backend wired to a native one: its `hook_state` uses herdr's exact `working`/`blocked`/`done`/`idle` vocabulary, so when a busy verdict is present it is trusted on the same terms as herdr's, while native idle and a not-yet-signalled null both stay `unknown`. That signal is written only by `thurbox-cli session signal`, which nothing in a default firstmate-spawned thurbox session calls, so in practice the verdict is `unknown` until an operator wires their harness's hooks to it - see [`docs/thurbox-backend.md`](thurbox-backend.md) "Active limits".
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij and thurbox remain unverified, and Orca and cmux do not support secondmate spawns.
...
thurbox's container shape is one session per task, no per-home container split - its database is the container, and `thurbox-cli` is fully headless, so there is nothing to stand up: session names are scoped by the shared home tag, the durable identity is the session UUID (the tmux pane id is re-resolved from it on every operation, since `session restart` moves the pane), and Treehouse stays the worktree provider even though thurbox could own worktrees itself.
...
Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, cmux, and thurbox tasks, while Orca creates its own worktrees for `backend=orca`.
...
Fleet state lives in each task's session-provider backend (tmux by hard default, herdr, cmux, or thurbox when selected or auto-detected, zellij/orca when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
