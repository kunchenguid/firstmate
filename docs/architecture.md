...
Two independent proofs can establish that identity, either one sufficient: the run head's ancestry relative to the local tip, or no-mistakes' own `branch_sync` statement that a live pipeline holds this exact branch and head, which is what binds a run whose gate-fix commits live in the gate repo rather than the crew worktree.
`bin/fm-nm-run-lib.sh` owns both rules and the exact conditions each requires.
