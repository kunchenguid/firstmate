...
Before merging, it resolves the real GitHub PR head the same way `bin/fm-review-diff.sh` does and runs `bin/fm-evidence-check.sh` against it, refusing the merge on a byte-identical or unpairable before/after evidence image pair; `bin/fm-evidence-check.sh`'s header owns the exact pairing convention and opt-out. If the task's recorded worktree is missing, not a git repository, or no ref can be resolved at all, the check cannot run: `fm-pr-merge.sh` prints a loud warning to stderr and still proceeds with the merge rather than failing closed.
