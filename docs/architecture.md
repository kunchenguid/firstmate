...
Both supervisors classify the status BYTES appended since they last classified that log, never its last line alone, so a later routine append cannot hide the decision, blocker, failure, or finish that arrived just before it; each keeps its own position record (the watcher's `.seen-*` signature and `.hb-surfaced-<task>` marker, the daemon's `.subsuper-seen-status-<task>` marker) because the two classify the same stream independently, and an unusable position re-reads the whole log rather than losing events.
