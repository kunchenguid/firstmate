...
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation and base freshness, [`tests/fm-spawn-pool-preflight.test.sh`](../tests/fm-spawn-pool-preflight.test.sh) for refusing a spawn the treehouse pool cannot serve with the cause named, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
