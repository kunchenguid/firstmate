...
The [`nathan-coding-loop` skill](../.agents/skills/nathan-coding-loop/SKILL.md) is the single owner of that loop, including role independence, GitHub line-comment evidence, repeated review of the current full diff, PR-path escalation, and the boundary between reviewer satisfaction and merge authority.
