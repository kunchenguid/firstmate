...
Structured secondmate summaries carry nonterminal working, parked, paused, and blocked children with each child's canonical `spawn_gen` when available; an unproven generation drops mutable child evidence and receives only snapshot-scoped browser identity so refresh selection cannot cross incarnations.
