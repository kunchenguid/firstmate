...
A `needs-decision` or `blocked` event in that span is retired only when the whole-file decision fold proves that exact opening closed, which preserves same-key reopening and uses the same open/closed rule as the durable OPEN DECISIONS surface.
