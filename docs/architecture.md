...
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns new-task backend selection precedence and authorization.
...
Fleet state lives in each task's selected or detected session-provider backend (tmux, herdr, cmux, zellij, or Orca), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
