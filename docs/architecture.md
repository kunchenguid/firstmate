...
An ordinary Herdr ship or scout worker has one optional bounded child layer owned by `bin/fm-child.sh`: at most three response-verified panes share the exact parent tab and Treehouse worktree while private records bind disjoint writable paths, inherited harness profiles, reports, and cleanup.
...
A child pointer send that returns `pending` may be confirmed only by the existing `fm-crew-state.sh` corroboration pattern: exact inherited registered-agent identity plus that harness's verified rendered-busy signature, never transcript text by itself.
