...
`bin/fm-project-mode.sh` remains the one registry parser for fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and spawn's registered-local-only fresh-base eligibility and advisory.
