...
PR- and MR-based task merges go through `bin/fm-pr-merge.sh`, which requires the resolved `captain-explicit` or `standing-yolo-green` authority class and records `pr=` plus the provider head through `bin/fm-pr-check.sh` before invoking either provider owner.
The helper requires a full canonical URL and rejects malformed URLs, repository overrides, and duplicate authority before recording merge state.
A `https://github.com/<owner>/<repo>/pull/<n>` URL keeps the existing `gh-axi pr merge <n> --repo <owner>/<repo>` behavior, including its `--squash` default, explicit merge methods, extra arguments, and queue-aware outcome verification.
A canonical `https://<host>/<nested-path>/-/merge_requests/<n>` URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) accepts only optional explicit squash, records and revalidates one exact source head and branch pair from strict bounded `glab-axi mr view` JSON, and invokes the guarded immediate-squash `glab-axi mr merge` primitive exactly once with URL-derived host/project/IID, canonical URL, expected head, exact source and target branches, and authority.
Workers cannot invoke that primitive; firstmate's wrapper owns task identity, authority, expected-head recording, one-shot invocation, strict result consumption, and lifecycle receipt persistence, while `glab-axi` owns provider preflight, the mutation, response validation, and ambiguity reconciliation.
A GitLab result is accepted only when one complete normalized JSON document binds the success action, canonical identity, source and target branches, expected head, successful head pipeline, authority, squash commit, and resulting target commit.
GitLab cleanup requires that current provider evidence together with exactly one task-bound guarded-squash receipt, an exact canonical MR-head fetch, squash/result reachability from the freshly fetched target, and an independent content-in-target proof, so legacy, malformed, stale, open, conflicting, mismatched, or merely similar landed work preserves the isolated copy.
After either provider path proves a landing, the script records the same durable landed outcome; a refusal or ambiguous result records none and leaves its poll armed.
