...
Claude keeps its tracked background-task protocol and adds a narrow PreToolUse continuity gate that allows drain, arm recovery, fail-closed teardown, and allowlisted wake handling while a wake is queued or was recently drained.
The gate still refuses unrelated fleet commands when tasks are in flight and no identity-matched live watcher holds the home lock.
