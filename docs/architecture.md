...
A stale pane whose rendered footer, not its transcript text, shows a Claude usage-limit or `API Error: 529` overload message is parked as a `ratelimited` wake instead of escalated as a wedge: the watcher records `state/<id>.ratelimit` with the parsed reset epoch (falling back to now plus `FM_RATELIMIT_FALLBACK` for a quota wait, or the shorter `FM_OVERLOAD_FALLBACK` for a transient overload, when no reset time can be read), window, and harness, then suppresses further stale wakes for that pane while it waits.
After the reset plus `FM_RATELIMIT_MARGIN`, the watcher (or, in away mode, the daemon) submits `continue` to a still-idle limit/empty pane, and clears the marker only once the pane genuinely recovers to a render that no longer shows the limit footer, appending a silent `ratelimited-resumed` queue record then; an accepted `continue` whose limit footer persists is counted as a resume attempt rather than a success, and if a still-limited pane is not resumed after `FM_RATELIMIT_MAX_RESUMES` attempts it writes `state/<id>.ratelimit.failed` and escalates, and from then on stops parking that pane so it returns to normal stale/wedge escalation rather than being suppressed into silence.
