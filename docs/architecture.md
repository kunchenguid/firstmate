...
One wake covers every status byte appended since the previous one, so both supervisors judge that whole span rather than only its newest line: an event firstmate must see, followed by any routine append, is still that file's captain-relevant event, and reading last-event-wins let such a batch be absorbed as routine with no supervisor turn at all.
The per-wake signal path bounds that span by the drain's presentation cursor, so an event already presented cannot re-fire it while a repeated identical line still counts as new; the away-mode daemon and its fleet backstop extend that same per-file position with their existing seen marker while an escalation awaits presentation, so repeated content cannot hide an earlier unseen event.
If a cursor cannot be read, classification scans the whole status file rather than risking a hidden event.
A signaled status path that is unreadable, non-regular, or a symlink (including a broken symlink) surfaces as a trust refusal instead of being absorbed as routine, and the away-mode fleet backstop also surfaces status entries excluded from its trusted snapshot.
