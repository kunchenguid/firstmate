...
A project with no origin remote, which a `local-only` project may be, has no remote tip to be stale against, so its base is the local default branch the linked task worktree already sees and no fetch is attempted; a project that does have an origin still refuses when that origin is unreachable.
