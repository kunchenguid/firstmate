...
The existing turn-end guard remains the final backstop for every enabled primary harness protocol, with pi-signed sharing Pi's protocol and the `--claude` mode cooperating with the auto-arm claim.
...
That keeps spawn launch compatible across claude, codex, copilot, grok, pi, opencode, and kimi while preserving the requested profile for later audit.
