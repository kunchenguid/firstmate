...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) still extends this for walk-away supervision on the other harnesses: `bin/fm-afk-launch.sh start` runs it via `bin/fm-afk-start.sh` inside a dedicated tracked terminal, kept alive across a death from any cause by `bin/fm-afk-daemon-run.sh`'s restart loop (`docs/herdr-backend.md` "Away-mode supervisor support"), after which the watcher reverts to one-shot mode while a live daemon provably owns supervision (`fm_afk_daemon_owns_supervision`, never the bare record alone; [`turnend-guard.md`](turnend-guard.md) owns that predicate) and the daemon self-handles routine wakes in bash.
