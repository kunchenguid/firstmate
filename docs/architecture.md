...
Repeated provably-working stale escalations on the same unchanged pane are one continuing episode, keyed to the window rather than to the ever-changing escalation count: a full wake fires immediately on the first crossing and again on reaching `FM_WEDGE_DEMAND_INSPECT_COUNT`, when the reason also carries a `demand-deep-inspection` marker, but every other escalation in between or after is throttled to one resurface per `FM_PAUSE_RESURFACE_SECS` instead of a fresh wake every `FM_STALE_ESCALATE_SECS`; those throttled reminders still carry the escalation count the silent rounds kept advancing, and the `demand-deep-inspection` marker once the threshold is behind them.
Any genuine state change ends that episode and opens a new one that reports immediately again: the pane going busy again below its `FM_BUSY_TURN_MAX_SECS` turn bound, its capture hash changing, a declared pause or captain-held transfer taking over, and the crew writing its own task worktree all drop the episode's resurface throttle, so a pane that goes from deferred-writing back to genuinely wedged surfaces on the next escalation rather than waiting out the previous episode's `FM_PAUSE_RESURFACE_SECS` clock.
A pane that renders busy while already past that turn bound is the deliberate exception: it stays inside the same episode on the same throttle, because a hung foreground call hiding behind a busy signature must not be able to flicker its way back to a full report every `FM_STALE_ESCALATE_SECS`.
...
That deferral re-surfaces on its own `FM_PAUSE_RESURFACE_SECS` cadence as a declared wait does, with a reason naming the write evidence rather than a wedge, and it leaves the escalation count untouched while clearing the wedge resurface throttle, so a later genuine escalation keeps the deep-inspection history it had earned yet still reports at once, and it is bounded to one pruned, depth-bounded, wall-clock-bounded walk (`FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`) taken only in the branch that was about to escalate, never on every poll.
...
That handoff is keyed on the declaration itself (the status log's signature) rather than on the pane capture, so a harness footer that ticks on every poll wakes the daemon once per declaration instead of once per poll, and it clears the whole wedge episode - timer, escalation count, resurface throttle, and worktree-write deferral - exactly as the normal-mode absorber does, so an undeclared busy phase's timer does not resume when the declaration lifts.
...
That evidence clears the pane's prior stale classification, its wedge-escalation count, and that episode's resurface throttle, so a pane that wedges again after churning reports at once rather than waiting out the previous episode's clock, then defers such a wake rather than swallowing it, since a crew that has stopped renders nothing further and its now-static pane surfaces through the staleness backbone within a poll or two, even if its final bytes match an earlier stale render.
