...
Pi and OpenCode verify session-lock ownership and launch one singleton successor from their child-close handlers before any actionable wake prompt delivery, with durable-queue revalidation, one-prompt coalescing through the handling turn, and bounded exponential retry for failed restoration.
