...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, a secondmate whose own status stream goes silent past `FM_DECLARED_WORK_SILENCE_SECS` after declaring work, and heartbeat backstop hits.
...
Status files are append-only, so the log's own age is the time since its last event - a durable record this host already holds, needing no launch wiring, and covering a remote mate identically because its escalations are ingested into this same state directory.
`fm-classify-lib.sh` owns the decision and its vocabulary (`status_declares_work`, `status_work_declared_unresumed`, `status_declared_work_stalled`, `FM_CLASSIFY_DECLARED_WORK_VERB`) in the one library both supervisors source, so the away-mode daemon reads the same verb and receives the resulting wake through the same single supervision cycle; the last status line the poll already read decides first, so an ordinary fleet pays essentially nothing per poll.
A declared `paused:` wait and a terminal last event each keep their existing meaning and never become a stall.
The wake is reconciliation rather than an alarm, because silence is not proof: the mate may be doing long legitimate work with nothing supervisor-actionable to report, may have finished a phase without closing its record, or may genuinely be stopped, and from outside those are indistinguishable.
...
That single bounded wake is what the last-event rule honestly supports: it narrows, rather than eliminates, the unsuperseded-start-event false signal the registered-secondmate current-state section below records, and whichever reading holds, the mate's own stream disagrees with reality, so the wake states all three and suppression is deliberately not built.
