...
The slot's own owner claim covers a slot reassigned to a task that left no record the scan could reach: a claim naming a different task releases nothing - teardown warns, names the claimant, and finishes only the task's own cleanup - while the spawn that takes a slot writes the claim under the allocation lock, because Treehouse's own live process lease cannot answer that question once the worker's exit releases it.
