...
Before a ship spawn creates any worktree or endpoint, `bin/fm-preflight-gate.sh` independently proves four preconditions rather than assuming them - a reachable push path, a delivery mode that can actually complete for this repo, measured quota headroom for the resolved harness, and host load/memory inside a safe ceiling - and refuses, naming exactly which one failed, when any does not hold; its own header owns the exact checks and their stated scope limits.
