...
The normal-mode watcher records each task-bound stale candidate in `state/.stale-candidate-<key>`, so identical candidates are deduplicated across pane churn and only a candidate transition wakes firstmate.
