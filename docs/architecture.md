...
On `attached` it stays live across identity-matched successors, and an unexplained clean close by its own child either attaches to a verified healthy successor or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
An attached arm never holds the followed watcher's stdout, so it cannot read that cycle's wake reason and takes the singleton over in place when the cycle ends with no verified successor, reaching the same typed failure only after its bounded attempts are spent.
