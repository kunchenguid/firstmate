...
An existing-PR ship brief additionally records the canonical PR URL, resolves its head through `gh-axi` in a disposable probe clone, verifies that the PR is open and its head is origin-hosted and not the default branch, and replaces new-branch and new-PR instructions with same-branch checkout and non-force push rules.
An optional `--branch` is a checked assertion against the forge-reported head rather than manually substituted shell text, and an existing-PR no-mistakes brief is refused at spawn until it records a valid base branch that setup verifies against the live PR before passing it to the pipeline's `--base-branch` target.
Spawn rejects the base placeholder anywhere in the generated setup, rules, or definition of done, so editing only its metadata copy cannot launch a broken delivery command.
...
Both delivery paths revalidate the open PR immediately before their push boundary, including the live base for no-mistakes, and origin must have exactly one matching push URL.
The disposable probe resolves the forge head without colliding with a linked worktree that already holds the PR branch, while the task checkout still honors Git's refusal to attach that shared branch twice.
The probe lives under the system temporary directory and runs inside a subshell with exit and interruption cleanup, so a stopped setup does not leave a nested repository in the project worktree or replace the worker shell's traps.
When a synchronized local branch is reusable, setup binds its upstream to the exact origin branch before work begins.
