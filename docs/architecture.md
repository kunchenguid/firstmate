...
Generation-scoped PR wakes carry a stable publication identity that a successful drain retains in `state/.wake-publications`, making a retry idempotent after queue consumption.
...
Merge polling is generation-scoped and one-shot: publication and terminal handling share a task lock, the watcher revalidates the exact generation and durably queues its identified wake before recording a retryable retirement receipt, and a replacement generation remains armed.
