...
`bin/fm-worker-state-lib.sh` owns this reconciliation and the exact run-head ancestry rules; `bin/fm-crew-state.sh` and `bin/fm-peek.sh` both render its one computed projection.
