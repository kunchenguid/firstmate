...
On the coarse runs-ledger route, which names no run id, an unbound crew is withheld branch-level credit entirely while another task binds a run whose own branch, as no-mistakes reports it, is that branch; the sibling's worktree is never consulted, so a sibling that has since detached or moved still owns the run it bound, and a same-branch answer another task has bound settles the question by itself.
That withhold also applies when the sibling's binding is stale, since the ledger cannot tell the branch's current row from a terminal run; a branch carrying no binding at all keeps the existing branch-based behaviour so no existing home regresses.
