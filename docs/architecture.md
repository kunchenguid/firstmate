...
Hermes's tracked primary plugin admits only the managed `terminal(background=true, notify_on_complete=true)` arm shape, receives the close in the persistent CLI, and injects one bounded repair turn when the shared predicate detects a blind boundary.
The existing turn-end guard remains the final backstop for every harness-engine protocol, with pi-signed sharing Pi's protocol, the `--claude` mode cooperating with the auto-arm claim, Cursor's `--cursor` mode rendering a block as one bounded follow-up because its `stop` step cannot be blocked, and Hermes mapping the same block to one plugin-injected follow-up.
