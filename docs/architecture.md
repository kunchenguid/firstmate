...
`fm-watch.sh` polls each window's backend for a busy state: tmux, zellij, orca, and cmux have no native primitive and always report unknown; herdr's `agent.get` semantic state (working/idle/done/blocked) is consulted first for stale detection, with unknown native states falling back to the pane-tail regex. For tmux specifically, that regex fallback routes through `bin/fm-tmux-lib.sh`'s shared `fm_pane_is_busy` (the same function `bin/fm-crew-state.sh` uses), which corroborates a tail-window match against the pane's live cursor row first so a leftover background helper's stale busy-looking text does not outlast the harness's own return to an idle composer or terminal prompt (docs/turnend-guard.md "Tests").
...
Ship tasks change projects and ship by project mode (`direct-PR` by default, or opt-in `no-mistakes`, or `local-only`); scout tasks investigate, plan, reproduce bugs, or audit, then leave a report at `data/<id>/report.md` and never push.
