...
The PR metadata identity boundary has an explicit post-`pr=` allowlist: validated `pr_head=`, Relay-link fields, and captain-completion `decisions_reviewed=1` and `decision_keys=` are accepted, while an unknown post-PR field still invalidates the poll.
The control-plane relaunch ordering that keeps relaunch-owned metadata ahead of the preserved PR registration metadata is owned by [`docs/agent-control.md`](agent-control.md#transactional-relaunch).
`tests/fm-pr-check-security.test.sh` pins the captain-completion allowlist.
