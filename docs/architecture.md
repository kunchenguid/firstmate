...
Two things cover that pane instead, which is why no separate timer exists for it: the merge poll stays armed and is what reports the merge (`bin/fm-merge-wait.sh` refuses to declare without one), and the `heartbeat:` fleet review reads every task in flight whether or not its pane changes.
