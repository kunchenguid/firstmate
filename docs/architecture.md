...
Its shared `bin/fm-wake-lib.sh` lock helper keeps symlink publication wherever `ln -s` yields a readable owner link, and when Git Bash/MSYS returns success while materializing a copied owner directory it adopts that publication through the existing legacy directory-lock path; stale-lock recovery removes only generated nested owner directories, preserving the same exclusive-ownership, stale-recovery, and steal-lock fencing guarantees.
