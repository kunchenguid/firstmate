...
A pool worktree is only returned after teardown passes the slot-ownership proof: a contradictory task record, a supported live endpoint, or the slot's own owner claim naming a different task refuses without touching either task, and no discard authority relaxes that.
The claim is what covers a slot reassigned to a task that left no record the scan could reach; the spawn that takes a slot writes it under the allocation lock, and Treehouse's own live process lease cannot answer that question because the worker's exit releases it.
