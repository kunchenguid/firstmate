...
Herdr's durable default container shape is workspace-per-home plus tab-per-task; [`herdr-backend.md`](herdr-backend.md#watching-and-task-containers) owns the resolved home workspace label, and recovery/list-live scope to the current `FM_HOME`'s workspace.
