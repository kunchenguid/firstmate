...
`fm-primary-pi.sh` and `fm-primary-pi-install.sh` refuse the same way before primary-custody or local-install mutation, so a gate agent cannot take custody of the captain's own Pi primary either.
