...
`bin/fm-public-followup.sh` composes those three and adds nothing of its own beyond the activation gate, a private terminal-event inbox, the idempotent delivery sequence, and explicit loop closure: delivering a final stamps the registration delivered and leaves the public loop open until `retire`.
