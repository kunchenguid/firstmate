...
A stale pane that is not provably working is still exempt from every possible-wedge escalation, permanently and with no wedge timer, when the current state reports the task terminal-done through the run-step source and its merge poll is armed and validated: that idle wait is the expected wait for the captain's merge, not a wedge, and the same recheck also disarms an already-running wedge timer the moment a provably-working pane finishes into that state on an unchanged pane hash.
