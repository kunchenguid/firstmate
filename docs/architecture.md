...
The existing turn-end guard remains the final backstop for the primary harness-engine protocols that wire a blocking or passive turn-end integration, with pi-signed sharing Pi's protocol and the `--claude` mode cooperating with the auto-arm claim.
