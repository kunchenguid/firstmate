...
`data/projects.md` records each project's standing posture, optional `+yolo` merge flag, and optional declared path as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
`bin/fm-project-mode.sh` remains the one registry parser for mechanical consumers that have no task in hand, including fleet sync, local and remote home seeding, and the spawn's registry-deviation notice.
