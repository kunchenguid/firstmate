...
PR readiness and ordinary merge monitoring first pass the lifecycle owned by [`pr-publication-check`](../.agents/skills/pr-publication-check/SKILL.md): the responsible worker attests a fresh complete public body and exact head, then `fm-pr-check.sh` repeats the authenticated readback before it can publish a poll.
