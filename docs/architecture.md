...
A finished ship or scout whose agent has actually exited is closed through `bin/fm-teardown.sh --close-pane` instead of a stale wake, so an empty pane waiting on merge does not keep alarming while the isolated copy stays until landed teardown; the watcher only does this automatically once the captain opts in with `config/close-exited-panes` (or `FM_CLOSE_EXITED_PANES=1`), so upgrading never starts closing panes on its own.
