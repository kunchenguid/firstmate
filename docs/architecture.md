...
Merge polling is generation-scoped and one-shot: publication and terminal handling share a task lock, the watcher revalidates the exact generation and durably queues its wake before recording a retryable retirement receipt, and a replacement generation remains armed.
