...
Idle secondmate panes are healthy; teardown is explicit, retirement takes per-target authority naming the exact home (`--retire-secondmate <id>`, which `--force` never substitutes for; see the teardown ownership paragraph under Delivery modes are explicit per task), and teardown refuses while the secondmate home has in-flight work unless the captain has approved discard with `--force`.
