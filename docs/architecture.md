...
Runtime auto-detection is innermost-first except for thurbox, which is checked first because it runs its panes on its own tmux server: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr, cmux, or thurbox prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij and orca are never auto-detected (only explicit selection).
...
thurbox exposes one, but only for an agent thurbox itself launched: its adapter reports a busy verdict solely from a live, uncontradicted agent-hook state and otherwise falls back to the same lifecycle record.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, Herdr, and thurbox have verified endpoint classifiers, Zellij remains unverified, and Orca, cmux, and thurbox do not support secondmate spawns.
...
thurbox is experimental and can be selected explicitly or by runtime auto-detection when Firstmate itself runs inside a thurbox pane: Treehouse remains its worktree provider, [`thurbox-backend.md`](thurbox-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#thurbox) owns active live evidence.
thurbox has no session layer either - one thurbox session per task, named with the same home label and `FM_ROOT` hash cmux and Zellij use - and it is the second push-capable backend after Herdr, supplying its own event subscriber as a CLI verb rather than needing a sidecar reader.
Its detection arm is the one that precedes tmux, because thurbox runs its panes on its own tmux server and is distinguished from a nested plain tmux by that server's socket.
