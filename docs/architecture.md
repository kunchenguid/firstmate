...
A stale pane whose status log looks terminal is bounded to one wake per episode per `FM_PAUSE_RESURFACE_SECS`, where an episode ends when the status log gains a line, the crew completes another turn, its agent confidently exits, or the pane goes busy again because resumed work makes the next quiet stretch a situation the supervisor has not been told about.
"Firstmate must act" is learned on the first wake, so a crew parked on a `blocked:` line cannot spend a handling turn every time its idle pane's footer ticks, and the window's end still re-surfaces the episode so it cannot go unreported.
That first wake carries the plain `stale: <window>` identity, while the end-of-window recheck names itself as `stale: <window> (<verb> Ns, already reported, rechecked on a long cadence not a wedge)` so a revisit of an already-handled blocker is not mistaken for a new one.
The verb slot holds the crew's own declared verb, and a captain-relevant line that declares none reads as `terminal status` instead.
