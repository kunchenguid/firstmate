...
Any `.status` file in the batch, an absent or invalid index, an identity mismatch, new status bytes, steering-inbox activity newer than the status bytes the outcome covers, a pane seen busy under the covering outcome, or an exhausted bound surfaces exactly as before.
A steer is work the status log has not reported yet, so coverage breaks on it and stays broken until the worker appends status again, whoever sent the steer and whenever the covering outcome landed relative to it: the branch steers in one step and reports in the next, so an outcome newer than the steer proves nothing about the turn that steer started.
