...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as a merge-attribution alarm or an X mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, and heartbeat backstop hits.
...
Before the merge call it writes a durable `merged_by_firstmate=` attribution marker to the task's meta, keyed by the unique PR URL rather than the branch name, rolls that attempt's marker back on a definite merge failure, and records a best-effort `merged_commit=` after success.
Under an unattended `yolo=on` merge it refuses when the PR's recorded CI check count (`pr_checks=`, captured by `bin/fm-pr-check.sh`) is 0, unknown, or missing, because a green with no CI checks validated nothing; an explicit captain-approved merge passes `--captain-approved` instead.
The per-task check script that `bin/fm-pr-check.sh` generates sources the shared `bin/fm-merge-attribution-lib.sh`: it stays silent for firstmate's own marked merges, wakes with `unattributed-merge:` when a tracked PR merged without the marker, warns with `merge-state-unknown:` when the PR state cannot be read rather than assuming the PR is unmerged, and fires once per distinct outcome via a `state/<id>.merge-seen` dedupe record.
The watcher's heartbeat fleet-scan reuses the same library as a backstop, surfacing an unattributed merge even when a task's check script is gone and deduplicating with `state/<id>.merge-hb-surfaced` markers.
