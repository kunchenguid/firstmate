...
That same acquisition gate is also where the task worktree gets the captain's checkout-local credential file: no worktree provider seeds git-ignored files, so the shared `fm-env-local-lib.sh` lifecycle, invoked by `fm-spawn.sh`, copies the project's git-ignored `.env.local` into every fresh ship and scout worktree, new or reissued, and refreshes or retires that one copy on each acquisition rather than letting a pooled slot serve an earlier task's credential.
Firstmate authors a file inside a worktree it does not own, so the boundary is deliberate rather than incidental: a tracked `.env.local` is the project's own version-controlled content and is never seeded over or deleted, a path the project does not ignore is reported instead of seeded because an untracked copy would wedge that slot's own clean check and its teardown, and a state that cannot be positively established refuses the spawn instead of assuming the benign case.
`bin/fm-env-local-lib.sh` is the single owner of that whole lifecycle, because `fm-teardown.sh` needs the same decision: a task can drop the ignore rule mid-flight, and the copy firstmate seeded then reads as untracked work that teardown would otherwise refuse forever.
Authorship is proved by a record the seed writes into the worktree's own git directory, never by content, because a task can legitimately author a `.env.local` whose bytes equal the project checkout's.
Teardown asks that question read-only, leaves only that one proven path out of its uncommitted-work verdict, and removes the file only after every work-preservation check has passed - so a file one of those checks would have refused is never already gone by the time it runs.
That library, the callers' headers, and `tests/fm-spawn-pool-base-freshen.test.sh` own the mechanics and the regression coverage, while [`configuration.md`](configuration.md#project-local-environment-file-envlocal) owns what a captain sets for it.
