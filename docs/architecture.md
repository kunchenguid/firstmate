...
A stale pane whose status log looks terminal is bounded to one wake per episode per `FM_PAUSE_RESURFACE_SECS`, where an episode ends when the status log gains a line, the crew completes another turn, or its agent confidently exits.
