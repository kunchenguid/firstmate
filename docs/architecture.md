...
The terminal state of that escalation is a backend PROOF that the recorded endpoint no longer exists, taken only in the branch about to escalate and only from a successful, exact read: the window's per-window watcher records are retired, no wake is emitted, one triage line is written, and the window is skipped before its capture for as long as that retirement holds (`state/.retired-<key>`, which records the mtime of the metadata it was taken against).
