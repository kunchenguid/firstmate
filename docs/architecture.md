...
That tip is the fetched `origin/<default>` wherever an origin is configured, and the clone's own default-branch tip when the project is a local-only clone with no origin to fetch from.
