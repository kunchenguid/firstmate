...
A registered check that reaches `FM_CHECK_TIMEOUT` is reported as `check: ... timed out after <seconds>s` instead of producing a silent cycle, so the timeout becomes actionable without changing the cadence or the handling of healthy long-running work and declared waits.
