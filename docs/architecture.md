...
Review diffs go through `bin/fm-review-diff.sh`, which refreshes the authoritative base and, when task meta records `pr=`, compares against the reachable recorded `pr_head=` or a PR head re-resolved from the PR host (via `bin/fm-scm-lib.sh`: `refs/pull/<n>/head` for GitHub, `az` for Azure DevOps) before falling back to the local branch with a warning.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which merges only GitHub PRs: it refuses an Azure DevOps PR URL and points the captain at the ADO UI before any recording or host call, since firstmate never completes an ADO PR (see `docs/ado-backend.md`).
For a GitHub PR it records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
...
Landed work is accepted when `HEAD` is reachable from any remote-tracking branch, when a merged PR's head (resolved from the PR host through `bin/fm-scm-lib.sh`) contains the current local work, or when the worktree content is already present in the freshly fetched default branch.
...
A PR-host lookup error falls back to the content check and still refuses if that check is inconclusive.
If no `pr=` was ever recorded, teardown can still discover a merged PR by matching the worktree branch name and fetching its head from the PR host when the head branch was deleted.
