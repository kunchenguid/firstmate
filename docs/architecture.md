...
`bin/fm-tasks.sh` adds callsigns and ordered table rendering to that projection, while `bin/fm-task.sh`, `bin/fm-next.sh`, `bin/fm-close.sh`, and `bin/fm-history.sh` compose the same lifecycle owner for detail, evidence-bound human-action ranking, guarded archival, and recall.
The lifecycle projection tells `/next` which task deserves attention, while the task brief's durable review plan tells it what the captain should concretely do; [`task-lifecycle.md`](task-lifecycle.md#human-action-translation) owns that boundary.
