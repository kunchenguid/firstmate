...
`bin/fm-env-local-lib.sh` is the single owner of that whole lifecycle, because `fm-teardown.sh` needs the same decision: a task can drop the ignore rule mid-flight, and the copy firstmate seeded then reads as untracked work that teardown would refuse forever, so teardown retires a copy it can prove is firstmate's own before its uncommitted-work check and leaves everything else for that check to refuse exactly as before.
