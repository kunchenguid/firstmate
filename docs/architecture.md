...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working and carry new captain-facing information, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The latest explicit `paused:` event remains the external-wait authority while the endpoint idles; an authoritative active run overrides it as working, and a later status event clears pause tracking.
...
When several declared-pause rechecks are due in one scan, each stale window is queued durably but the watcher exits once with a batched reason.
...
A declared-pause pane whose validation run reaches a newly actionable outcome escalates the same way.
...
`bin/fm-project-mode.sh` remains the one registry parser for standing posture lookups by name or project path, including fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and spawn's registered-posture notice.
