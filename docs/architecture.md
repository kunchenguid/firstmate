...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if the home needs supervision - tasks in flight or an armed X-mode relay poll, the shared predicate in `bin/fm-supervision-lib.sh` - and that watcher stops running or queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when that same supervision-need predicate holds and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
