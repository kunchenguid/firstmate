...
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge; an authoritative `paused` verdict from `bin/fm-crew-state.sh` is honored whether or not the agent is still alive, because a waiting worker normally keeps its agent at the idle prompt.
For an ordinary crew whose declared wait or durable `captain-held` endpoint that read cannot confirm (a stopped or unknown current state), the normal-mode watcher first surfaces one stale wake, then applies that same cadence to the unchanged endpoint only when the backend confidently reports its agent dead.
