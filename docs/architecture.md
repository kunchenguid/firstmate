...
Its shared `bin/fm-wake-lib.sh` lock helper keeps symlink publication on platforms where `ln -s` yields a readable owner link, but on Git Bash/MSYS it uses the existing legacy directory-lock path and stale-lock recovery removes only generated nested owner directories, preserving the same exclusive-ownership, stale-recovery, and steal-lock fencing guarantees.
