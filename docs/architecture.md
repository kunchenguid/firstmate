...
A `https://<host>/<owner>/<repo>/pulls/<n>` URL (see [docs/forgejo-tea-integration.md](forgejo-tea-integration.md)) addresses the repository through a `tea login` resolved fresh from the host on every run, because `tea` takes a repository slug rather than a URL; the merge itself calls Forgejo/Gitea's REST API directly (`tea api ... pulls/<n>/merge` with `head_commit_id`) rather than `tea pulls merge`, which exposes no head-binding flag, and that call's own exit status is never trusted because it reports an HTTP-level failure as success, so a follow-up read of `merged` is the only accepted evidence of a landed merge.
That path merges only after one live read confirms the pull request is open, mergeable, and has a successful combined commit status at the current head; an absent status is read as failing, not as nothing to check.
