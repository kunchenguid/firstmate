...
The explicit resolution is written by the actor that answers, not the busy worker: `fm-send`'s `--resolve-key` appends the closing `resolved` line to every live copy owned by the answered task at answer time, which covers crewmates, local secondmates, and remote secondmates identically because a remote mate's escalations reach this home's copy through the parent-replies ingest and only the answer message itself crosses the transport.
A secondmate home holds a second live copy whenever it relayed a worker's decision with that worker's task provenance onto its parent escalation channel, so the same answer closes that copy too and a decision always closes in the channel where it opened; an upstream copy with missing or different task provenance refuses the answer before delivery rather than conflating unrelated decisions that reuse a key.
`bin/fm-parent-channel-lib.sh` owns which channel that is for a given home, and the `fm-send.sh` header owns the propagation and refusal contract.
