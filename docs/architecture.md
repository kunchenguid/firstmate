...
`bin/fm-pr-poll.sh`'s and `bin/fm-teardown.sh`'s own headers own the git-based landed-work test that `pr_dest=` feeds: git is the primary detector on every forge, including Bitbucket, which has no forge API integration here at all, with each forge's own API kept only as a fallback.
