...
On `attached` it stays live across identity-matched successors, and a clean close it could not read a reason line for either attaches to a verified healthy successor, reports the wake the durable queue proves the cycle delivered, or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
