...
The optional Signal integration keeps account ownership inside one generic process-event runner generation, whose authenticated readiness boundary confirms the adapter reached its blocking receive operation without creating an adapter-owned background process; [configuration.md](configuration.md#process-to-event-sources-stateprocevent) owns setup and operator mechanics.
