...
The interval before each repeat is evidence-driven rather than fixed: an escalation still queued unacknowledged keeps the `FM_STALE_ESCALATE_SECS` detection cadence, while one the first mate has already drained and acknowledged doubles the required quiet interval per acknowledged repeat up to `FM_WEDGE_ACKED_BACKOFF_MAX_SECS`, because re-firing an alarm the supervisor already inspected and cleared is churn rather than evidence.
A recorded endpoint that a successful backend inventory no longer lists on two consecutive polls surfaces once on its own evidence, before and independently of any declared wait, because `paused:` declares a pending external dependency and never that the worker itself may disappear.
The second poll is what separates a lost worker from an ordinary cleanup, which closes the runtime endpoint before it removes the task record and so leaves one poll's worth of a window that is genuinely absent and genuinely still recorded.
...
That one prompt surface is spent per DECLARATION, not per pane hash: a quiet pane that keeps redrawing under a declaration the first mate has already been told about stays on the long cadence, and only a fresh append re-arms the prompt surface.
