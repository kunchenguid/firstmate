...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and fallback-path `fm-send` sends do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow. The same opencode busy-queue case is a known gap on the herdr adapter and is recorded in `docs/herdr-backend.md` rather than patched here. For a task whose harness has a trusted semantic lifecycle source, `fm-send` submit confirmation instead reads the busy-record contract below and the rendered core is only its explicitly labelled fallback ([tmux backend](tmux-backend.md)); the daemon's context-free supervisor injection keeps the rendered core unchanged.
...
Submit confirmation on the tmux path is a read-only consumer of this contract for harnesses with a trusted source: it confirms delivery from a turn opening in the task's busy record after Enter, consults the rendered acknowledgement only as an explicitly labelled fallback, and compares both signals during bake-in with a loud disagreement warning ([tmux backend](tmux-backend.md)).
