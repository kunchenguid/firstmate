...
`bin/fm-watch-arm.sh` remains the verified arm wrapper for protocols that call it; it forks the watcher as a tracked child and verifies it is genuinely alive with a fresh liveness beacon.
...
Claude's `bin/fm-claude-stop-autoarm.sh` hook fires on every Stop and, when the home is eligible and still needs supervision, claims one home-scoped cycle, foregrounds the arm wrapper, and translates an actionable close or typed nonzero alarm into one exit-2 rewake.
[`watcher-continuity.md`](watcher-continuity.md) owns Claude's residual active-turn coverage, the arm-layer cycle-close contract, and the lifecycle-ledger boundary.
...
At the one poll where the watcher's wedge timer would otherwise fire, it asks whether that crew's attributed pipeline advanced during the elapsed window, since a long validation step leaves the pane visually static and elapsed time alone escalates a healthy worker on the same schedule as a frozen one.
Positive proof of advancement resets the timer; an unreadable, unparseable, or timed-out progress read escalates unchanged, so the check can only quiet an alarm that was demonstrably false.
