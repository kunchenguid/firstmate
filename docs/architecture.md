...
Codex classifies unknown behind explicit probes until a semantic source is live-verified, except that a record-free Codex task on Herdr accepts independently verified native `busy` as positive activity.
Standalone Kimi classifies unknown behind explicit probes until a semantic source is live-verified, and Grok, Rovo, and AGY each keep one clearly isolated rendered-tail fallback that can only ever classify their own task.
