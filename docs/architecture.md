...
A declared external wait on a task that already carries an armed, validated merge poll is the one exception to that recheck: the poll wakes on the merge by itself, so re-surfacing the wait would only re-ask an answered question.
Both supervisors decide that through one shared predicate and record a `state/.<id>.pause-poll-covered` note, printed under that task in the session-start fleet digest, so a wait that goes quiet is visibly quiet on purpose.
Anything short of a fully validated poll for that exact task keeps the ordinary recheck, and only the declared external-wait verb is ever offered the drop - a verified captain-held transfer always takes its recheck, because a merge poll says nothing about whether the captain acted.
Coverage also lapses the moment the poll positively observes its PR closed without merging: the watcher records that terminal outcome in `state/<id>.pr-poll-terminal`, the coverage note is withdrawn, and the ordinary recheck resumes, so a rejected wait can never go quiet forever.
That observation is dropped instead of woken on only once its record is durably written; one that cannot be recorded surfaces as an ordinary `check:` wake, because with no record nothing else would ever report it.
Silence, a forge error, or any other inconclusive reading is never a terminal outcome, so an unreachable forge only keeps the recheck rather than losing it.
...
A declared external wait whose own armed merge poll already covers it is the single exception described above; a captain-held transfer never is.
