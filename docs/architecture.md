...
Herdr uses one tab per task by default, with an opt-in pane split inside the launching agent's own tab owned by [Crew placement](herdr-backend.md#crew-placement); [Watching and task containers](herdr-backend.md#watching-and-task-containers) owns launcher-bound workspace placement, the label-only fallback, and recovery scope.
