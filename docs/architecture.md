...
Every alarm a declared wait does produce names that wait - which verb declared it, which human it blocks on, and the action that clears it - so a reader can separate a bounded recheck from a possible wedge without opening the status log, and the live half additionally reports that the lane still has an agent to answer.
