...
Codex's `bin/fm-codex-stop-autoarm.sh` is that same hook-owned shape on a Codex primary, keeping its own generation ledger and delivering an actionable close as a `codex queue` message rather than an exit-2 rewake; [`watcher-continuity.md`](watcher-continuity.md) and [`turnend-guard.md`](turnend-guard.md) own that contract and its foreground-checkpoint fallback.
...
The existing turn-end guard remains the final backstop for every harness-engine protocol, with pi-signed sharing Pi's protocol, omp's blocking `session_stop` hook compelling one continuation per turn, the `--claude` mode and Codex's flagless cooperative mode each cooperating with their own auto-arm claim, and Cursor's `--cursor` mode rendering a block as one bounded follow-up because its `stop` step cannot be blocked.
