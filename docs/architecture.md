...
The mate's own `wake()` transition holds queue serialization across its bounded output and durable progress publication before the diagnostic delivery ledger runs, recording a monotonic transition timestamp and the authoritative queue-sequence watermark, so clock changes cannot override causal order, a handling turn stays quiet however long it runs, and an absent, unreadable, malformed, or lower-watermark marker never suppresses an abandoned queue while the primary only reads the foreign marker and queue.
