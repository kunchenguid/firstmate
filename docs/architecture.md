...
A worktree whose HEAD is provably unborn - `git init` or `checkout --orphan` with nothing committed - names no commit, so it passes through like an ordinary directory; a ref store that cannot answer that question still refuses.
