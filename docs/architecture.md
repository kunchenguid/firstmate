...
A stale pane whose rendered footer, not its transcript text, shows a Claude usage-limit or `API Error: 529` overload message is parked as a `ratelimited` wake instead of escalated as a wedge: the watcher records `state/<id>.ratelimit` with the parsed reset epoch (falling back to now plus `FM_RATELIMIT_FALLBACK` when no reset time can be read), window, and harness, then suppresses further stale wakes for that pane while it waits.
After the reset plus `FM_RATELIMIT_MARGIN`, the watcher (or, in away mode, the daemon) submits `continue` to a still-idle limit/empty pane, clears the marker on a confirmed empty composer, and appends a silent `ratelimited-resumed` queue record; if the submit cannot be confirmed after `FM_RATELIMIT_MAX_RESUMES` attempts it writes `state/<id>.ratelimit.failed` and escalates rather than looping forever.
