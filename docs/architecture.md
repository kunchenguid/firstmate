...
That claim is read before the contradictory-record refusal, so an old dead record and the replacement that took its slot can both name the same path: the claim naming the replacement is the positive proof the old record is stale, so its cleanup proceeds with every slot step skipped instead of refusing on the collision.
