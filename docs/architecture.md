...
The library header owns the accepted status grammar and anomaly contract; the drain prints a companion STATUS LINE ANOMALIES section for malformed lifecycle lines, honored misplaced key tokens, and closures for keys nothing opened, so a writer that drifts off the grammar surfaces on the next wake instead of quietly leaving a decision open.
