...
Its Definition of done also opens with a fixed `Surface contract: web|non-web` line, and a web brief carries the canonical website deployment completion gate from `bin/fm-web-gate-lib.sh` directly under it.
That helper owns the gate text and fence-aware structural validation, while the spawn accepts the declaration only at that boundary and ignores fenced or task-description text, so a missing, misplaced, or incomplete web gate stops the spawn.
