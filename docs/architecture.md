...
A busy pane is otherwise exempt from staleness, but only up to `FM_BUSY_TURN_MAX_SECS` with no completed turn (`state/<id>.turn-ended`, or its spawn record before any turn completes); past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
