...
When `bin/fm-crew-state.sh` reports a not-working stale crew as parked, the stale wake reason includes the parked detail, such as an ask-user gate, so the first mate can start with the decision point instead of only a pane peek.
