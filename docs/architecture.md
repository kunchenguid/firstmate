...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: with an origin configuration, the clean task worktree must match the fetched tip of origin's resolved default branch; without one, a clean task worktree in either a separate pool repository or the primary repository's linked worktree is reset to the primary checkout's default-branch commit, fetching that commit only for a separate pool repository.
Any unsafe or unverifiable base stops the spawn.
