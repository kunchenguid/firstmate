...
It also owns the worktree base section rendered into a launch brief and into a promoted scout's ship instructions when a spawn placed that slot on the project's registered working branch, because only the placement itself knows which branch the worker is on and a brief written before the slot exists asserts the default branch.
That section states the base as a fact for every path except local-only, a scout included, since `bin/fm-merge-local.sh` still fast-forwards the default branch for a local-only task.
...
The same registry line optionally records the project's working branch, for the projects whose working branch is not the remote's own default branch.
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and the working branch a fresh ship or scout spawn places its pooled worktree on.
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` refreshes the authoritative base and, when task meta records `pr=`, always fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
That authoritative base is the branch the spawn recorded as `base_branch=` in the task record, so a task's own review diff is taken against the same working branch its slot was placed on, and only falls back to the project's default branch for a task whose record names no branch.
