...
That state stays local to the home that read it: a secondmate's upward parent-channel append carries only `done` or `failed`, because the parent's status stream is folded by the open-decisions semantics above where `blocked` opens a durable keyed decision and this path never emits its resolution, so the precise state and the quota detail ride along in the line's text instead of becoming a decision the parent cannot close.
