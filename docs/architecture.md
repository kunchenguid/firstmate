...
Because teardown reaps leaked processes by the recorded worktree path, a crewmate or scout task outside Orca whose recorded worktree cannot be proven a Treehouse pool slot of its recorded project refuses with nothing changed, even under `--force`, so a stale or wrong `worktree=` or `project=` can never aim cleanup at another directory or its processes.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, slot-ownership proof, pre-reap pool-slot proof, PR-discovery fallback, pre-teardown run conclusion, and stale-lock recovery procedure; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh) and [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh) pin the slot-collision boundary, and [`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh) pins the pool-slot refusal.
