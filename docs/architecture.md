...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, pane captures that fail without authoritative work or a trusted pause, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A declared external wait (`paused:`) or verified captain-held transfer that remains trusted after current run-step and agent-liveness reconciliation is the one exception to that bound: its busy verdict identifies the long-running foreground call as the expected wait, but does not establish agent liveness, so it takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation.
Lifting the declaration restores the unchanged busy-pane wedge path, while a trusted pane that is no longer busy returns to the existing idle declared-wait classification.
...
A crew whose `paused:` declaration remains trusted for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
A declared `paused:` or durable `captain-held` endpoint keeps that bounded cadence while its agent is live, and also while the backend's liveness read is inconclusive, because the declaration is the worker's own evidence and an unreadable, ambiguous, or unverified read is a measurement gap rather than proof the worker is gone.
Declared-pause triage derives current run-step authority and liveness before pane capture and busy-state guards can bypass reconciliation.
An active run-step supersedes the older declaration and returns the endpoint to ordinary working-state tracking, while capture failure alone is never treated as death.
Only a backend that confidently reports the agent dead breaks the declaration, since nothing is then left to end the wait, so the endpoint surfaces once per continuous death episode even when capture fails, the pane appears busy, or its hash keeps changing.
An inconclusive liveness read neither invalidates the pause nor ends an already-surfaced death episode, while a confirmed alive read ends that episode so a later distinct death can surface once again.
After the immediate death surface, the declared wait returns to the bounded pause cadence until either current run-step authority or a new alive-to-dead episode changes its verdict.
