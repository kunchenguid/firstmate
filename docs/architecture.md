...
Each home also atomically publishes that same bounded home summary with freshness epoch metadata at `state/home-summary.json` after a locked session start, a watcher-observed status change, task spawn, and task teardown; `bin/fm-home-summary-refresh.sh` owns the publication mechanics.
