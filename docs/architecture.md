...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake and then applies that cadence, and that one-shot surface is keyed on the declaration rather than the pane hash, so a live harness redrawing a spinner or a token counter cannot buy another surface for the same wait.
