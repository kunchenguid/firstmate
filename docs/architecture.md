...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, check-script output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, quota-parked (`ratelimited`) panes, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A stale Claude pane whose rendered footer line, not its transcript text, shows the exact Claude usage-limit message or an explicitly opted-in `FM_OVERLOAD_REGEX` match while its composer is empty is parked as a `ratelimited` wake instead of escalated as a wedge: the watcher records `state/<id>.ratelimit` with the parsed reset epoch (falling back to now plus `FM_RATELIMIT_FALLBACK` for a quota wait, or the shorter `FM_OVERLOAD_FALLBACK` for an opted-in transient overload, when no reset time can be read), window, and harness, then suppresses further stale wakes for that pane while it waits.
After the reset plus `FM_RATELIMIT_MARGIN`, the watcher (or, in away mode, the daemon) submits `continue` only when the pane still shows the limit footer and `fm_backend_composer_state` reports an empty composer, and clears the marker only once the pane genuinely recovers to a render that no longer shows the limit footer, appending a silent `ratelimited-resumed` queue record then; an accepted `continue` whose limit footer persists is counted as a resume attempt rather than a success, a pending composer is counted as a blocked attempt without typing anything, and if a still-limited pane is not resumed after `FM_RATELIMIT_MAX_RESUMES` attempts it writes `state/<id>.ratelimit.failed` and escalates, and from then on stops parking that pane so it returns to normal stale/wedge escalation rather than being suppressed into silence.
The whole marker set (`state/<id>.ratelimit`, `.attempts`, `.failed`) is also cleared the moment the pane content changes to a render that no longer shows a quota footer or opted-in overload footer, since the crew is active again; a render still showing the footer keeps the marker, so a genuinely new rate-limit episode later starts fresh instead of reusing a past reset or inheriting a stuck `.failed` that would forbid parking for the rest of the task.
...
Its housekeeping loop runs the same quota-parking auto-resume as the always-on watcher, and also detects and parks a rate-limited supervisor pane of its own, so an away-mode fleet still resumes parked crews on schedule.
The supervisor pane's own auto-resume `continue` is prefixed with the same terminal-safe U+2063 sentinel marker, so it reads as an internal escalation rather than a captain return that would prematurely exit away mode; crew-pane resumes stay a plain, unmarked `continue`.
While a pane stays parked the watcher reuses the reset already recorded in `state/<id>.ratelimit` instead of re-parsing the footer every poll.
