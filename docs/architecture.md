...
Ryder recognizes its own composer row from an ANSI snapshot's style and cursor channels, then hands the stripped content to that same owner through `fm_composer_classify_content`, so the verdict is never a private copy.
