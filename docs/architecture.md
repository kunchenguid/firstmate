...
Orca is experimental and selected only explicitly: Orca owns both worktree and terminal lifecycle, records the composite `orca_worktree_id=<repo uuid>::<absolute worktree path>` and `terminal=`, and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass.
...
When that poll finds the PR still open and the task's recorded merge posture is `yolo=on`, the watcher invokes `bin/fm-pr-merge.sh` itself, so a green mergeable PR lands without waiting for a firstmate turn; the merge path's own live verification is the only green check, and a red, unmergeable, or unverifiable PR is refused there and keeps polling.
