...
A leading WAKE CLASS line labels each non-empty drain's whole raw batch ROUTINE or ATTENTION without changing queue contents, eligibility, or acknowledgement: a heartbeat row is always routine, any check-kind row (including an inactive-outcome or inactive-reconcile terminal-outcome notice) is always attention on every presentation, and a signal or stale row is attention only when its status span since the last presentation cursor carries a newly unread captain-relevant record, with a missing, unreadable, symlinked, or otherwise unsafe-to-read status file also failing toward attention. A per-fingerprint "already surfaced" downgrade to routine was rejected: a plain, non-acknowledging drain can be issued by an automated caller (for example the AFK supervise daemon's own internal housekeeping) that never shows its output to the captain, so marking a fingerprint surfaced there would risk hiding the captain's own first look at a finished-or-failed child behind a routine label.
