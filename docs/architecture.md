...
Merge polling is generation-scoped and one-shot: publication and terminal retirement share a task lock, the watcher revalidates the exact generation before neutralizing its runnable check and queuing one wake, and a replacement generation remains armed.
