...
While a stale key remains durably queued and unacknowledged, equivalent detections are absorbed at the shared queue boundary without another primary turn or escalation-count increase; acknowledgement starts a full fresh cadence before that unchanged key may surface again.
Successive acknowledged provably-working stale escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
