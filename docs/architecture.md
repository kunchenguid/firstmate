...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, unless a live declared merge wait (`state/<id>.merge-wait`) absorbs that wake as it is emitted, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits, declared merge waits, and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
It absorbs only the wake itself, and only while the record still names the pull request the task records, the merge poll is still armed, and authoritative crew state still reports the run step's terminal checks-green outcome; that verdict is always the one read for this alarm rather than a cached one, so a withdrawn, superseded, or no-longer-green declaration reaches a real alarm on the attempt it stops holding.
...
What covers that pane instead, and why no separate timer exists for it, is the merge poll: it stays armed and is what reports the merge, and `bin/fm-merge-wait.sh` refuses to declare without one.
