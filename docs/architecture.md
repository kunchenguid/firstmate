...
The working proof attributes a no-mistakes run by branch (`bin/fm-crew-state.sh`), so when several crews' worktrees sit on one long-lived feature branch - the shape a one-PR-per-repo posture produces - a crew that has exited otherwise inherits a co-branch crew's active run, reads as provably working on every poll, and is escalated as a possible wedge indefinitely; a dead agent cannot be running that pipeline, so its own declaration is the better evidence.
