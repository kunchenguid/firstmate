...
The receipt makes retirement safely retryable across restarts: fixed-path recovery revalidates the same evidence, removes the runnable check first, removes its registration and data sidecars, removes the receipt last, and preserves task metadata including `pr=`, `pr_head=`, and `ready_head=`.
...
`bin/fm-task-custody-lib.sh` is the single worktree-custody owner: it derives identity from both the physical worktree root and its worktree-specific absolute Git directory, then scans the home's authoritative task metadata so symlink aliases, a stale record on a reused pool path, duplicate fields, and two task ids cannot claim one isolated copy.
Fresh spawns serialize only that scan through metadata publication, while same-task recovery reuses the recorded copy rather than allocating a replacement and requires the recorded endpoint to be recovery-grade `dead` or `missing`.
The check applies after allocation on every spawn-capable backend; automatic same-task recovery is available on tmux and Herdr, while Zellij, Orca, and cmux preserve the record and refuse because their agent-process state is deliberately unverified.
[`verification/task-lifecycle-identity.md`](verification/task-lifecycle-identity.md) records the current public-interface evidence for custody, recovery, readiness, and merge revision binding.
...
`ready_head=` is the immutable delivery-readiness identity for every ship task.
For a PR task, `bin/fm-pr-check.sh` resolves the current source revision from GitHub or GitLab, atomically replaces `pr=`, `pr_head=`, and `ready_head=` with one consistent binding, and republishes the merge poll; an unavailable or malformed forge head refuses without a local fallback.
For a local-only task, `bin/fm-merge-local.sh <id> --prepare` records `ready_head=` only while the task branch and clean isolated copy agree exactly.
`bin/fm-review-diff.sh` refreshes the authoritative base but compares only the bound commit, first proving that the current PR or local branch still resolves to it; movement refuses with the old and new revisions instead of silently changing the reviewed diff.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which consumes but never refreshes the existing readiness binding.
It requires the full canonical GitHub URL to match metadata, re-reads the current forge head and check rollup, refuses moved, pending, red, unavailable, closed, or malformed state, and passes `--match-head-commit <ready_head>` to `gh-axi pr merge` so a final head-movement race cannot merge a different revision.
It defaults to `--squash`, preserves explicit merge-method flags, and rejects repository or head override flags; a well-formed GitLab merge request URL is refused rather than sent to the wrong forge.
The local merge path similarly verifies the current task branch and isolated-copy HEAD equal `ready_head=`, then fast-forwards to that commit object rather than the movable branch name.
An intentional pipeline fix or rebase remains ordinary: rerun the readiness owner to bind the resulting revision, review it, and apply merge authority again.
