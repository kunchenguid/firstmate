...
Uninspectable active-home projects, malformed or unreadable Treehouse state, and failed skill inventories invalidate the healthy coverage heartbeat until discovery and hygiene inspection complete.
...
The shared mutation path holds one canonical repository lock for refresh, Treehouse acquisition, and every authorized Treehouse return, and proves the fetched default ref against the live upstream `HEAD`, while teardown requires its locked pre-fetch and post-fetch probes to agree, so scheduler, preflight, teardown, and merged-PR wake callers neither race nor trust stale `origin/HEAD`.
