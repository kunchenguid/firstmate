...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and current code identity - including a run the pipeline has pushed ahead of the worktree, via no-mistakes' own branch_sync binding - then keeps that run-step authoritative even if the pane has closed.
The script header and `bin/fm-nm-run-lib.sh` own the exact matching rules.
