...
It owns task endpoint creation, bounded capture, text/key sends, current-path reads that prove the task pane has entered its leased slot when the backend does not create the worktree itself, live-window fallback lookup, agent-process liveness probes where verified, and endpoint teardown.
...
Firstmate takes every crewmate and scout slot as a durable Treehouse lease under the task id (`treehouse get --lease --lease-holder`), then has the task pane open a nested shell in it (so the pane's top shell survives teardown's slot cleanup until the endpoint's own close), and Treehouse's own persistent state is the one record of which task holds a slot: a worker that exits or dies leaves its slot leased, the pool never hands a leased slot to a later task, and teardown returns it with the holder check.
[`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns that lease contract and its read, and [`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh) reports a Treehouse install without both lease flags as missing before any dispatch.
The [`fm-spawn.sh` header](../bin/fm-spawn.sh) owns ship/scout worktree isolation, the lease-then-enter sequence, and fresh-base refusal rules, including spawns from linked homes.
A slot leased under a task id that no home's task record names (a secondmate home is leased under its registered secondmate id, not a task id) is the leftover of a spawn that stopped between leasing and publishing its record without returning the lease - killed outright, or its lease `get` stopped at its `FM_TREEHOUSE_LEASE_TIMEOUT` bound after Treehouse had already written the lease, a case the spawn reports rather than returns because the task-id label alone cannot tell that lease from one a live spawn under the same id holds; nothing reclaims it, because teardown and relaunch both start from the record, so release it by hand from the project with `treehouse return --force --if-lease-holder <task-id> <slot-path>`, and never read it as a live task.
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation, the spawn-side lease, and base freshness, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch; [`tests/fm-treehouse-lease-e2e.test.sh`](../tests/fm-treehouse-lease-e2e.test.sh) proves the lease lifecycle against the real Treehouse and tmux binaries wherever both are installed, and [`verification/runtime-backends.md`](verification/runtime-backends.md#treehouse) records the dated Treehouse measurements that guard refreshes.
...
The slot's durable Treehouse lease, taken by the spawn under the task id and owned by [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh), covers a slot reassigned to a task that left no record the scan could reach: a lease naming another holder releases nothing - teardown warns, names the holder, and finishes only the task's own cleanup - while a slot leased to the task is returned with `--if-lease-holder`, so Treehouse itself refuses if the lease changed hands after the read, and a slot with no lease (taken before spawns leased, or already returned) keeps the record-scan protection alone.
