...
Ship briefs prompt crewmates to create or update those files through the normal delivery path, but only when that task itself has something to record, so an unrelated change never carries a memory-file restructure into its PR; `data/projects.md` stays a thin private registry.
