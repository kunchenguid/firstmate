...
Before a `no-mistakes` ship launch, `bin/fm-spawn.sh` refreshes the project's shared gate hook via `bin/fm-nm-gate.sh` (an idempotent `no-mistakes init`), so a worktree push creates a pipeline run instead of failing on a stale hook; the refresh is best-effort and bounded by `FM_NM_GATE_TIMEOUT` seconds.
