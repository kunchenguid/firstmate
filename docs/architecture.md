...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr uses native agent-state submit confirmation on idle baselines, a pre-Enter rendered-footer transition when that baseline is unavailable, and the shared composer classifier for pending-input guards, submit fallback, and the Pi-only idle-delivery acknowledgement ([herdr-backend.md](herdr-backend.md)).
