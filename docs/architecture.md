...
Those marked requests also receive a privacy-safe correlation id and a durable parent-owned pending-reply expectation under `state/pending-replies/` before delivery (`bin/fm-pending-reply-lib.sh`): successful transport never resolves the expectation, a correlated parent status or document pointer does, the watcher observes backend busy/idle turn completion without reading the secondmate conversation, one automatic recovery repost is sent after grace, and a second miss escalates once while retaining the unresolved record.
