...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` owns provider-specific PR-head resolution, authoritative-base refresh and offline fallback warnings.
...
Azure DevOps Services support and its current limits are owned by [azure-devops.md](azure-devops.md).
After a forge completion request returns, the script confirms the PR or MR actually landed, and only a confirmed landing records a landed outcome; a queued or unconfirmed request records none and leaves its poll armed.
