...
A `done:` event is read as a landing only when the task's recorded delivery mode actually ends in one, and then only when that line itself carries the PR or merge-request URL, which is the single acceptance path: on a `no-mistakes` or `direct-PR` ship whose completion line carries no URL, the event reports `parked` and says the pipeline still owes a PR, because the generated contract asks the worker to report completion on its implementation commit before the pipeline has produced anything.
Neither an earlier status line mentioning a PR nor the task's recorded `pr=` is accepted as evidence, because the first would let one mention authorize every later completion and the second survives a relaunch into a new incarnation's metadata.
`bin/fm-dod-lib.sh` owns which modes end in a PR and `bin/fm-classify-lib.sh` owns the reading, so every firstmate-facing surface that renders a status line - the drain annotations and its `STATUS OUTCOME BACKSTOP` section, the session-start tails, the watcher's actionable reasons, the away-mode daemon's escalation digest, and a secondmate's upward ledger report - refuses that landing through the same test, while a `local-only` ship's ready branch and a scout's report stay accepted untouched.
