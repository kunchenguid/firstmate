...
Because a `local-only` task's approved work lands on the local default branch and is never pushed, which ref carries a task's landed work follows that task's own recorded mode rather than the registry; [`bin/fm-pool-base-lib.sh`](../bin/fm-pool-base-lib.sh) is the one owner of that rule and its limits, shared by the pooled base a spawn launches on and the base a branch is reviewed against.
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` refreshes that authoritative base and, when task meta records `pr=`, always fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
