...
A pane no longer named by any recorded `state/<id>.meta` never advances stale or wedge state even if a prior snapshot already emitted it or a backend still returns one final cached capture; the watcher retires its `.hash-*`, `.count-*`, `.stale-*`, `.stale-since-*`, `.wedge-escalations-*`, and pause records instead.
The same snapshot race applies to the signal-grace wait: a `.status` or `.turn-ended` path that vanishes before its grace elapses is dropped from that batch, and its `.seen-*` and `.hb-surfaced-*` suppression markers retire once no status, turn-ended, or meta source remains for that task.
