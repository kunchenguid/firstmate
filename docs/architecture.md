...
A pane whose no-mistakes run reports a fresh `active_steps` `last_activity` (via one bounded `axi status` call, `crew_pipeline_step_active` in `bin/fm-classify-lib.sh`) is deferred the same way, because a long-running step - a multi-minute test or review pass - legitimately renders nothing new to the pane for its whole duration, while the run-step read that classified the stale hash in the first place only covers the first poll of that stretch.
A `quiet`-prefixed `last_activity`, no `active_steps` table, no attributable run, or an unavailable or erroring `axi status` call all leave the existing escalation schedule untouched, so a pipeline that has genuinely stopped moving still escalates exactly as before; that call is bounded by `FM_PIPELINE_STATUS_TIMEOUT` and taken only in the branch that was about to escalate, never on every poll.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write and pipeline-activity deferrals, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
