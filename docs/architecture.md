...
Independently, the fleet-lifecycle entrypoints - currently `fm-spawn.sh`, `fm-send.sh`, `fm-control.sh`, `fm-teardown.sh`, `fm-herdr-endpoint-bind.sh`, and `fm-sessionstart-nudge.sh` - source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
That helper's header owns the current entrypoint list, so treat it rather than this paragraph as the authoritative inventory when adding one.
