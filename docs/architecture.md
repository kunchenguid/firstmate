...
That evidence defers such a wake rather than swallowing it, since a crew that has stopped renders nothing further and its now-static pane surfaces through the staleness backbone within a poll or two; a wake naming any status file, a secondmate, an unresolvable endpoint, an ambiguous marker key, a missing prior hash, or a capture that fails or returns empty surfaces exactly as before.
