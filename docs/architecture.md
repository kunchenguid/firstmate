...
`bin/fm-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and the guarded daily upstream owner's separate `--upstream-posture` eligibility query ([daily-upstream.md](daily-upstream.md)).
