...
Firstmate remains the dispatch decision owner: it resolves the selected rule's profile array from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, then passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
[`configuration.md`](configuration.md#typed-dispatch-resolution-env-typesafe_api_key) owns optional typed rule matching; neither that advisory match nor shell validation owns array selection or authorizes a spawn.
