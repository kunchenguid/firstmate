...
That state read is bounded per crew per threshold but not per housekeeping pass, so a pass makes at most `FM_STALE_WORKING_GATE_READS` of them and leaves any remaining aged marker untouched for the next pass, which delays such an escalation by a tick rather than dropping it.
