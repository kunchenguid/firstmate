...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress; the daemon marker is the sole wedge cadence owner, and the watcher re-hands off an unchanged non-paused stale hash only after that marker clears.
