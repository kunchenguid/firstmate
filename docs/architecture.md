...
The watcher atomically publishes an exact-instance actionable result before releasing its singleton lock, so every attached arm surfaces the same reason and exit status while a missing result remains a failure.
The arm layer records one bounded diagnostic lifecycle row per observed cycle in `state/.watch-cycle-exits.log`; `state/.watch-triage.log` remains exclusively the absorbed-wake debug log.
