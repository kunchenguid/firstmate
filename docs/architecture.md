...
A crew that declared an external wait (`paused:`) or a verified captain-held transfer is the one exception to that bound: its long-running foreground call IS the declared wait, so a live busy pane under that declaration takes the bounded `FM_PAUSE_RESURFACE_SECS` recheck instead of a wedge escalation, and the escalation returns unchanged the moment the declaration is lifted or the pane stops proving liveness.
