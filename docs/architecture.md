...
Past `FM_MAX_DEFER_SECS` a stalled escalation is force-delivered on a not-busy pane (clearing any stale self-injected composer text that would otherwise read as `pending` input and defer forever, via the backend-dispatched `fm_backend_clear_composer`), and only a genuinely busy pane, an `unknown` composer, or text that will not clear writes `state/.subsuper-inject-wedged` and attempts a configured backend-independent active alert instead of silently deferring forever.
