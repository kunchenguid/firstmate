...
A task whose intended base is a non-default branch declares it with `fm-spawn.sh --base`, which records `base=` plus the base's spawn-time tip as `base_sha=` into `state/<id>.meta`, the single source of truth for a task's base (`fm-brief.sh --base` only shapes the brief's prose and records nothing).
Four scripts act on that declaration: `fm-spawn.sh` decides whether the task may be launched against the base at all, `fm-brief.sh` tells the crewmate what to root its branch on, `fm-pr-check.sh` guards the PR's base fail-closed before recording `pr=` or arming the merge poll, and `fm-review-diff.sh` diffs a based task against its intended base rather than presenting the feature base's unmerged history as the crewmate's change.
None of them asks its own version of the question.
[`bin/fm-base-lib.sh`](../bin/fm-base-lib.sh) owns what a declared base means, and its header owns why branch existence answers nothing: whether the base's work has landed in the default branch and whether a head is rooted in the base's own commits are two independent questions, and the single verdict every consumer decides on comes from asking both.
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh)'s header owns the guard contract in full.
