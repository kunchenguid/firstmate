...
When that same poll instead returns exactly `closed`, for a PR or MR closed without merging, the watcher appends the same kind of durable notification but publishes no retirement receipt: a closed-unmerged result alone does not prove the work landed elsewhere, so the poll stays armed and keeps re-notifying on every check cycle until firstmate or the captain reconciles the lane.
A concurrent replacement remains armed, every other non-terminal or invalid observation remains unchanged, and retirement never performs task or persistent-secondmate cleanup.
