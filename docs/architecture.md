...
Its shared `bin/fm-wake-lib.sh` lock helper keeps symlink publication on platforms where `ln -s` yields a readable owner link, but on Git Bash/MSYS it adopts a copied owner directory through the existing directory-lock path and scrubs only pid-matched nested owner artifacts from raced publications, preserving the same exclusive-ownership, stale-recovery, and steal-lock fencing guarantees.
