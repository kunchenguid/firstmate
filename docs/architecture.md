...
When the home-local `config/wake-context-presentation` opt-in is present, Claude, Codex, and Pi actionable notifications call `fm-wake-context.sh` once to attach a bounded `fm-wake-context.v1` projection of that durable presentation, including its post-handling acknowledgement, so the primary does not reconstruct the same fleet context in later model turns.
Without that opt-in, the same notifications preserve the actionable reason and direct the primary to one ordinary human drain without publishing wake-context state.
