...
A second, narrower exception turns on freshness rather than on any particular status word, giving the precedence `fresh run-step > status-log > stale run-step`.
A run object is a durable record rather than a heartbeat: a reboot, a killed agent, or an abandoned daemon leaves it frozen mid-step, still answering `running` with an in-flight step row, so a run that has stopped moving cannot describe what is happening now however confidently it still claims to.
...
This governs every verdict a stale run would otherwise supply about being mid-work, including what its ci step row and ci.log tail say about CI, because those stopped at the same instant the run did; a CI regression after a run freezes is the PR poll's job in `bin/fm-pr-check.sh`, not this reader's.
