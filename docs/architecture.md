...
`bin/fm-dod-lib.sh` is the one owner of each mode's definition of done and the destination contribution-guidance contract shared by PR delivery modes.
It renders that contract into a generated ship brief, the ship instructions a promoted scout receives, and that scout's own `brief.md` so a later relaunch reads the same contract and a promoted worker cannot be handed weaker instructions than a briefed one.
