...
Herdr's container shape is workspace-per-home plus tab-per-task: workspace labels use the shared home-tag - the active home label plus a short hash of the resolved `FM_HOME` path, so the primary home uses `firstmate-<hash>` and secondmate homes use `2ndmate-<secondmate-id>-<hash>` - while per-task tab labels stay the bare `fm-<id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
...
Zellij's container shape is simpler than herdr's: one shared `firstmate` session, one tab per task, with no per-home workspace split; visible tab titles are scoped by the active home label plus a short hash of the resolved `FM_HOME` path.
...
cmux's container shape is one workspace per task with one surface, no per-home container split; workspace titles are scoped by the active home label plus a short hash of the resolved `FM_HOME` path, and `--secondmate` spawns are refused, mirroring Orca.
