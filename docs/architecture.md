...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh) owns branch, head, pipeline-custody, and submitted-head attribution, plus complete same-branch run selection, optional inventory lookup, and ambiguity reporting.
[`tests/fm-crew-state.test.sh`](../tests/fm-crew-state.test.sh) covers run selection; its capture provenance and live-evidence limits for [v1.70.1](../tests/captures/no-mistakes-v1.70.1/README.md) and [v1.75.2](../tests/captures/no-mistakes-v1.75.2/README.md) distinguish recorded inputs from composed scenarios.
