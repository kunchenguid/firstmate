...
Every new ordinary task claim writes `task_owner_marker=1` into its record alongside the marker, so an absent `.fm-task-owner` on a path whose record carries that bit refuses by task id, path, and missing marker rather than falling back to the task branch or project membership.
`spawn_gen=` predates owner markers and is not a marker-awareness signal: existing records are never migrated, and every record without the explicit bit keeps the legacy task-branch and project-membership fallbacks unchanged while normal task turnover retires that compatibility exposure.
A duplicated, empty, unreadable, or unsupported awareness bit refuses rather than being inferred absent, and a marker-aware record also refuses when its spawn generation is duplicated, empty, or unreadable.
...
That retirement begins at all only once the slot's marker is proved absent or exactly this record's own; any other entry there - another task's, malformed, unreadable, or not a regular file - refuses first with the record's claim, that entry, and the slot untouched, so resolving the entry and rerunning is an ordinary recovery rather than a drill.
Once it has begun, only a confirmed provider release completes it automatically; every failure or interruption preserves the claim and marker copies, leaves the record claimless, refuses every further destructive lifecycle action, and prints one deliberate manual-recovery drill rather than trying to infer whether restoration became safe.
Its dirty and unlanded-work gates all inspect the slot's HEAD, which is not the same thing as the `fm/<task-id>` ref, so teardown deletes that ref only when the ref itself proves every commit on it is on a remote-tracking branch or in the landed target that run already accepted; anything less retains the branch and says which commits went unproved, in every mode.
The durable half of that binding goes down first: before it stamps the marker, `fm-spawn.sh` publishes an ownership record into `state/` naming the task, the spawn generation, and the slot, so a spawn killed before it can publish its task record leaves a marker an operator can still attribute and clear rather than one that refuses the slot forever.
...
A relaunch restamping a marker this same task already owns records the generation it is replacing alongside the one it is stamping, which makes the rewrite one exact transition: a crash between the restamp and the metadata advance leaves a marker one generation ahead of the record, and that handoff is what proves it is still the task's own worktree rather than a slot recycled to a later spawn.
A record that does not say clearly enough who owns a slot - a missing, duplicated, or unusable spawn generation or worktree line - is unknown ownership, never evidence that a marker is stale, because that marker may be the only thing protecting a live worker.
