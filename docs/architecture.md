...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR movement polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` with neither a wait their own worker declared nor their own task worktree being written, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, except a wait on a still-open pull request that a live movement poll demonstrably covers, and heartbeat backstop hits.
...
In that status-log fallback, a declared external wait reports the distinct `paused` state with its reason and a verified `captain-held` transfer reports `parked`, each read as the standing declaration described below rather than as the log's last line.
...
`resolved:` is what a crew appends to lift a wait without ending its task, and it retracts only a declaration carrying its own decision key, so a resolution for an unrelated decision cannot cancel the wait; every terminal captain verb retracts whatever key it carries, so a crew that declares itself blocked never reads back as still paused.
The routed-work activity fold deliberately differs - it key-scopes every verb and treats `captain-held` as a closer rather than a declarer - because it answers what work is open for the decision surface, not what wait a crew is standing in.
...
The daemon's declared-wait window ages against the crew's standing declaration rather than against pane busy state, because a declared wait can legitimately hold a pane busy, and only the crew's own retraction described above ends that routing and restores wedge detection.
...
The daemon escalates captain-relevant events, plus a bounded recheck for a declared external wait that is still declared, as one batched digest; captain-held transfers remain silent until return while the posture record exists.
It writes that digest to a `state/.subsuper-digest-*` file and types only one short line naming the file, using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
The digest never travels through the composer because a long typed line reaches the harness in several terminal reads and loses its head, prefix included ([evidence](verification/runtime-backends.md#away-mode-transport)).
Every queued escalation logs its source, and every injection attempt, deferred, refused, failed, or delivered, logs one line with the digest size and outcome.
