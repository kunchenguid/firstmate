...
The parent guards every reply-expected marked request against a missing correlated report without reading the secondmate conversation, while an explicitly classified `fm-send.sh --delivery-only` action resolves on confirmed delivery; `bin/fm-pending-reply-lib.sh` owns the correlation, recovery, escalation, explicit operator resolution, and retention contract.
