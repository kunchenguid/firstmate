...
The same cursor-backed unread span also prints `note:` lines and reserved-key pending-reply resolutions once, because those verbs never enter the OPEN DECISIONS fold and a latest-line-only annotation would otherwise drop a substantive answer sitting directly above a later routine line.
