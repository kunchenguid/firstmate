...
Paseo's container shape is one workspace per task with one terminal, no per-home container split; the terminal's home-scoped NAME is the routing authority and the workspace's title is decoration only, and `--secondmate` spawns are refused, mirroring Orca and cmux.
