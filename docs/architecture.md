...
A `https://github.com/<owner>/<repo>/pull/<n>` URL requires `gh` and `jq`, is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, every configured required check has reported at the current head, and every unwaived check is green at that head, then `gh pr merge` binds that verified head with `--match-head-commit`.
...
An attended `--allow-missing <check-name>` may appear once, waives only that configured required check when it has not reported at the current head, still requires every other required check to report and every check to be green, and is refused while the away-posture record exists.
Required checks come from the base branch's forge protection response, and an unreadable response refuses the merge rather than treating the branch as having no requirements.
