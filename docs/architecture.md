...
A secondmate is never probed at all, and neither is any task whose recorded worktree is a marked firstmate home, because such a home's own supervision keeps writing inside it whether or not the mate produces anything, so this deferral never relaxes the schedule those panes already had.
...
A crew that has gone quiet without declaring any wait, and that is not provably working, still surfaces one stale wake at once, on the unchanged schedule.
The normal-mode watcher applies the bounded cadence to a still-declared `paused:` or durable `captain-held` endpoint whether that crew's agent is still live or has already exited, because the declaration rather than a backend liveness verdict is what makes the idling expected; a repainting idle pane stays on that one-recheck-per-window cadence instead of rearming a full-rate wake.
