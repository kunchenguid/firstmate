...
On a Pi primary, supervision is default-on: the watcher extension hands each wholly in-scope ordinary actionable wake, plus every fleet-wide heartbeat scan, to a persistent in-process supervision conversation instead of the captain's, which handles it, stores the outcome durably, and merges an append-only note back.
