...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is neither provably working nor parked at a current run-step-backed validation decision whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
An authoritative current run step parked at a no-mistakes validation decision is a verified gate wait, so its unchanged pane remains absorbed without wedge escalation or the declared-external-wait cadence while the worker stays live.
The watcher rechecks positive worker or endpoint liveness on the bounded `FM_VERIFIED_WAIT_RECHECK_SECS` cadence and surfaces the stale pane when that liveness is lost or the current run step is no longer parked.
...
Fresh stale panes use the same current-state read before trusting the status log, so an active run, busy pane, or verified parked validation decision outranks an old captain-relevant status-log line left behind before validation.
Watcher signal, stale, and heartbeat scans require current task metadata, and teardown retires the task's watcher suppressors, so collected notifications from a torn-down identity cannot wake the model or suppress an identical notification after that task ID is reused.
...
Pi and OpenCode verify session-lock ownership and launch one singleton successor from their child-close handlers before delivering an actionable wake, with bounded exponential retry for failed restoration.
