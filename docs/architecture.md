...
The helper requires a full `https://github.com/<owner>/<repo>/pull/<n>` or `https://<host>/<owner>/<repo>/pulls/<n>` URL, invokes `gh-axi pr merge <n> --repo <owner>/<repo>` for GitHub or `tea pulls merge <n> --repo <owner>/<repo> --login <name>` for Gitea/Forgejo, defaults to a squash merge, preserves explicit merge-method flags, and rejects malformed URLs or repo/login override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/forge-merge-watch.md](forge-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge, because no owner/repository pair can address GitLab's arbitrary-depth namespace.
