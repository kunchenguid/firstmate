...
The helper requires a full `https://github.com/<owner>/<repo>/pull/<n>` URL, invokes `gh pr merge <n> --repo <owner>/<repo>`, passes a verified head with `--match-head-commit` when one is required, defaults to `--squash`, preserves explicit merge-method flags, and rejects malformed URLs, repository overrides, requirements-bypass flags, or caller head overrides before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
