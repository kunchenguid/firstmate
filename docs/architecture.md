...
A full `axi status` run object judged frozen (its `active_steps` `last_activity` older than `FM_RUN_STEP_LIVENESS_MAX`, or a steps table with nothing left running or fixing) is demoted below the crew's own later `paused:`/`done:` line once the branch's PR is recorded in meta, and can no longer claim to have superseded a declared `needs-decision:`/`blocked:` gate; terminal outcomes, gate parks, and the coarse runs-list fallback are never demoted.
The `bin/fm-crew-state.sh` header is the single owner of that rule, its argument, and the limitations accepted with it.
