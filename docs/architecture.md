...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without fresh structured validation-log activity or their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
At the short-wedge boundary, a changed current-step log token from the same fully attributed active no-mistakes run restarts the short timer instead of treating a static endpoint as idle.
The token is progress evidence only, never a replacement for structured current state, and a missing, parked, terminal, pane-only, unreadable, or unchanged token follows the existing wedge schedule.
Continuously advancing validation still re-surfaces on `FM_PAUSE_RESURFACE_SECS`, so log churn cannot hide a worker indefinitely.
...
Pi-only append tickets and per-actor post-output presentation receipts let its delayed follow-up revalidate that exact durable event and task incarnation after successor restoration; [`watcher-continuity.md`](watcher-continuity.md#actionable-wake-ordering) owns the full delivery contract and safe exceptions.
...
A direct approval or fix-review wait on a nonterminal structured run also outranks a stale terminal outcome field, while a terminal top-level status remains terminal.
Its `--activity-token` mode reuses that attribution and emits an opaque current-step log identity only for an actively working full run; all parked, terminal, coarse, remote, pane-only, and unknown states emit nothing.
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the exact branch, head, pipeline-custody, agent-wait, and newest-first attribution rules.
...
[docs/pi-supervision-branch.md](pi-supervision-branch.md) owns row eligibility and dispatch architecture, while [watcher continuity](watcher-continuity.md#actionable-wake-ordering) owns Pi's delayed-delivery revalidation and the generated [Pi supervision protocol](supervision-protocols/pi.md) owns MAIN's captain-visible response and merged-event handling; every other harness keeps the wake-to-main path unchanged.
