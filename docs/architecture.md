...
`bin/fm-delivery-contract-lib.sh` is the single owner of each mode's worker contract, so `bin/fm-brief.sh` and `bin/fm-promote.sh` cannot drift: PR modes forbid worker merges, require every review submission and inline thread to be handled, and return the full green PR URL for captain approval, while local-only work stops on the ready branch for a Firstmate-owned approved merge.
Promotion first renders the canonical contract to `data/<id>/ship-contract.md`, verifies its exact bytes, binds its path, mode, and SHA-256 into task metadata, and changes the task to ship only after that association succeeds.
Ordinary production workers also receive a backend-independent spawn-time merge guard: a private workspace registration and harness-native blocking shell hook use `bin/fm-worker-command-policy.mjs`, whose shell tokenization comes from `bin/fm-arm-command-policy.mjs`, while task-local `gh`/`gh-axi` PATH wrappers enforce the same boundary at execution.
The guard denies direct, path-qualified, wrapper-qualified, shell-keyword-position, generic-exec-wrapper, nested-shell, eval, substitution, alias, REST merge-endpoint, and GraphQL merge execution before GitHub is called, while leaving push, PR creation, review, comment, and CI operations available.
A harness without a verified blocking shell hook is refused before its endpoint launches, an ordinary worker spawn refuses every raw launch command because part of the guard rides the verified typed launch template, and the task-local PATH never reaches Firstmate's own `bin/fm-pr-merge.sh` process.
Each harness-native hook dispatches through the private spawn-time binding - workspace pointer to mode-0600 auth file to the recorded `checker=` path - so a workspace copy of the checker can never stand in for the Firstmate-owned one, and a workspace with no registration leaves the hook inert.
