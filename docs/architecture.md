...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if tasks are in flight and the watcher's liveness beacon (`state/.last-watcher-beat`, touched every poll cycle) is missing or older than `FM_GUARD_GRACE` (300s), or queued wakes are waiting to be drained.
That predicate is beacon-mtime only and never inspects the watcher lock, so a watcher that has died while its beacon is still within grace stays silent for up to `FM_GUARD_GRACE`; the identity-matched live-lock backstop that catches that case at the next turn end is the push-based backstop below, not this pull-based guard.
