...
A task whose intended base is a non-default branch declares it with `fm-brief.sh --base`, which writes the `data/<id>/base` sidecar that `fm-spawn.sh` promotes into meta as `base=` plus the base's spawn-time tip as `base_sha=`.
