...
The guard covers the live-base case only: a base that is gone from origin, or unresolvable, is handed to a human rather than adjudicated, because a merged base and an abandoned one are not distinguishable from git without a guess.
