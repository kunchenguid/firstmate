...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns reassigned-slot handling and completion-evidence requirements; [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh) owns the slot claim and why a Treehouse process lease cannot prove task ownership.
