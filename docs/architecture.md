...
A terminal failed run-step from an obsolete run defers to a later declared paused or blocked status event only once a structural check confirms the branch's custody was returned; the script header owns the exact precedence and custody-check rule.
