...
An explicit `pipeline_owned` branch-sync state preserves that attribution while pipeline commits have not yet synchronized into the crew worktree, preventing an older locally resolvable run from masking the current pipeline-owned run.
