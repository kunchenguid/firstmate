...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes outside a live crew's declared wait that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
An authoritative active-run verdict does not retire that declaration: a crew routinely declares its wait and then idles while the very run it is waiting on is still attributed to its code, so the run step and the declaration describe one expected idleness rather than a disagreement, and treating the run step as the stronger record put the commonest healthy shape on the fleet into the wedge ladder.
A declaration is categorically stronger evidence than the run-step or pane state a wedge is inferred from, because it is the crew's own statement that the pane waits by design, so an active run under one is reconciled exactly as a declared-pause verdict is on both the busy and the idle path.
That reconciliation is withheld from a crew whose agent the backend confidently reports dead: a run step attributed to a crew with no agent left is an orphaned runner rather than a wait, nobody remains to answer a recheck asking whether the wait still holds, and such a pane keeps the ordinary wedge ladder.
Because a live crew's declared wait is answered from liveness alone, that far more common window costs no authoritative crew-state read at all, so the read stays on first sight of an undeclared pane and on the dead-agent declaration.
...
A captain call deferred with `bin/fm-captain-hold.sh hold --until <date>` is the exception: the watcher asks `open --deferred` only when that recheck, or a held call's stale alarm, is about to ring, spends the due window silently while tasks-axi still reports the hold active, and resumes the ordinary cadence on the first window on or after the date.
