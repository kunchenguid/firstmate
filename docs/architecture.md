...
After the watcher's first signal scan starts the `FM_SIGNAL_GRACE` coalescing window, its final classification uses only a fresh post-grace scan, so an answerer's `--resolve-key` close whose seen marker commits during that window does not re-wake its own home while a later worker append still surfaces.
