...
When two records name one pool slot, [`bin/fm-teardown.sh`](../bin/fm-teardown.sh) can stage `--correct-stale-association` only after the claim and exactly one current-owner record agree under the project lock; it retains an exact private copy of the stale record, leaves the current owner and copy untouched, and requires a second ordinary teardown invocation that rechecks the evidence before cleanup.
