...
Both report a pull request only alongside the `pr_landing=` verdict recorded at arming time, and read `unverified` where metadata carries none or the URL came from a status log, so a pull request nothing checked cannot reach the captain worded as a merge decision.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review; ahead of every other source it consults `bin/fm-allowance-lib.sh`, the single owner of the account-allowance-park verdict, because a worker parked on the account allowance still renders a live pane and a live endpoint and would otherwise be misread as working ([docs/verification/supervision.md](verification/supervision.md#allowance-park-detection) owns the per-harness signal evidence). Once that reports not parked, it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and current code identity, then keeps that run-step authoritative even if the pane has closed.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=`, any available `pr_head=`, and the landing-target verdict `pr_landing=` through `bin/fm-pr-check.sh` before calling the forge CLI.
The helper requires a full canonical URL and rejects malformed URLs or repo override flags before recording merge state.
Recording is also where a pull request is declared a landing path, so it is refused when the forge reports the viewer cannot merge into the repository hosting it; [`bin/fm-pr-landing-lib.sh`](../bin/fm-pr-landing-lib.sh)'s header owns that question and [`docs/verification/pr-landing-target.md`](verification/pr-landing-target.md) holds its forge evidence.
Raising or pushing a pull request against a read-only upstream stays untouched: only treating one as a landing path, or reporting it as awaiting a merge decision, is refused.
