...
The retirement receipt makes poll cleanup safely retryable across restarts: fixed-path recovery revalidates the same evidence, removes the runnable check first, removes its registration and data sidecars, removes the receipt last, and preserves task metadata including `pr=`, `pr_head=`, and `pr_dest=`.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` and `pr_dest=` through `bin/fm-pr-check.sh` before calling the forge CLI.
`bin/fm-pr-poll.sh`'s and `bin/fm-teardown.sh`'s own headers own the git-based landed-work test that `pr_dest=` feeds: git is the primary detector on every forge, including Bitbucket, which has no forge API integration here at all, with each forge's own API consulted as a safety net that can only add a detection the git test missed and never override one it already confirmed.
