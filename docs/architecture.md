...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter for the two harnesses with verified Enter-while-busy queuing (opencode 1.18.4 and cursor-agent; see `docs/tmux-backend.md`), reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane, or any other harness, keeps the `pending` verdict as a genuine swallow. The herdr adapter has its own native-busy fallback (`fm_backend_herdr_busy_state`) for the same exhausted-retry busy-queue case, covering opencode and cursor on herdr.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, muse, and cursor while preserving the requested profile for later audit.
