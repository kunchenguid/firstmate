...
A crew whose latest status event declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
An authoritative active run overrides that pause as working, while any later non-pause status event clears the pause classification and restores normal stale handling.
For an ordinary crew with a durable `captain-held` endpoint, the normal-mode watcher first surfaces one stale wake and applies the bounded cadence only when the backend confidently reports its agent dead.
Live or inconclusive liveness for `captain-held` remains fail-open at that initial surface, and the secondmate idle-endpoint exemption is unchanged.
