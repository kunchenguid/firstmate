...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while Herdr's confirmation paths are owned by [herdr-backend.md](herdr-backend.md#current-transport-behavior).
