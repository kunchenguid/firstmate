...
A live run's head is normally a commit the crew's checkout has never seen, because the pipeline keeps its own gate-fix commits private until it pushes, so pipeline custody rather than local ancestry settles code identity there, and only the branch's newest run is ever eligible.
The script header owns the exact run-head ancestry and custody rules.
