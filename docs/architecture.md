...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, the primary appends one keyed `check` wake naming the mate, row sequence, and observed age unless the mate's delivery-progress marker proves its supervision delivered an actionable wake at or after that row was enqueued; parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes, while empty and younger queues remain silent.
The mate's own `wake()` transition uses bounded queue and progress-lock acquisition, publishes a short-lived in-flight record before its bounded output, and commits the monotonic transition timestamp and authoritative queue-sequence watermark only after direct output succeeds or the arm/checkpoint wrapper confirms its unchanged forwarding succeeded or the away daemon confirms durable handling succeeded, while serialization or forwarding failure leaves progress unusable, so clock changes cannot override causal order, an active publication suppresses detection only within a tight live-owner window, and an absent, unreadable, malformed, future-dated, unconfirmed, abandoned, or lower-watermark marker never suppresses an abandoned queue while the primary only reads the foreign marker and queue.
This marker proves a delivered supervision handoff rather than entry into a handling turn, so a home whose watcher delivers an actionable wake but whose handling turn never begins remains quiet under this detector.
