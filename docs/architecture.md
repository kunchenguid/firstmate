...
A done/failed verdict read straight from bare `axi status` is then cross-checked against the branch's own newest-first runs list and folds to a still-active run's state when one supersedes it (the shape typical right after custody recovery); [`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh)'s header owns the exact fold and inconclusive-lookup rules.
Only when no matching run exists does it consult semantic busy state; exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log - a terminal done/failed log verb specifically also requires the run lookup to have genuinely completed and found nothing, not merely timed out or crashed.
