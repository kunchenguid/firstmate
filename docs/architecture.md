...
That handoff is keyed on the declaration itself (the latest status event's verb and reason) rather than on the pane capture, so a harness footer that ticks on every poll wakes the daemon once per declaration instead of once per poll, and it clears the wedge timer, escalation count, and worktree-write deferral exactly as the normal-mode absorber does, so an undeclared busy phase's timer does not resume when the declaration lifts.
...
Its later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change, because the throttle and its age are keyed to the declaration (the latest status event's verb and reason) and not to the pane an idle parked worker keeps ticking, so a status write that leaves that event unchanged neither restarts the cadence nor wakes early, while a new or changed declaration surfaces at once.
