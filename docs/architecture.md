...
On the coarse runs-ledger route, which names no run id, an unbound crew is withheld branch-level credit entirely while another task binds a run from a worktree on that branch; a branch carrying no binding at all keeps the existing branch-based behaviour so no existing home regresses.
