...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose current authoritative state is no longer working, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, busy turns that exceed their separate duration bound, and heartbeat backstop hits.
A stale pane first absorbed as working is reconciled again after `FM_STALE_ESCALATE_SECS`; current work renews the timer without a wake, a current pause moves to the pause cadence, and a stopped, done, or unknown state surfaces as plain stale rather than as a possible wedge.
Repeated over-age busy-turn wedge escalations on the same pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker.
...
A completed Codex foreground checkpoint is the one exception: it publishes a fresh single-use rollover marker that suppresses only its immediate required drain, while the turn-end guard and every later operation continue to treat the watcher gap as unhealthy until the next checkpoint starts.
