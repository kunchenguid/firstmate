...
Hermes and Antigravity expose no such vendor contract, so Firstmate owns the turn instead: their crew and scout endpoint is `bin/fm-worker-bridge.py`, which runs each native CLI turn itself and publishes the matching generation-bound busy and idle events around it.
...
Unsupported effort values are still recorded in task meta when passed to `fm-spawn.sh`, but the launch template omits any effort flag that the selected harness does not accept, which keeps a launch compatible across adapters while preserving the requested profile for later audit.
[`docs/configuration.md`](configuration.md#crew-dispatch-profiles-configcrew-dispatchjson) owns that rule and the adapters that refuse an unsupported level instead of omitting it.
