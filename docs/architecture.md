...
Approved implementation obligations use this same loop and queue through `bin/fm-task-execution.sh`, rather than a second scheduler.
That owner derives an accountable next action even when no endpoint exists, and leaves recovery, decisions, validation, and landing with their existing authority owners.
Its processing receipt proves execution of the current handoff instruction in the isolated worker copy; current activity still comes from `bin/fm-crew-state.sh`, and neither a launch seed nor an old working event proves progress.
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` with neither a wait their own worker declared, nor their own task worktree being written, nor a recorded step still inside its `FM_BUSY_TURN_MAX_SECS` bound, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The same bound carries a second trigger on the recorded herdr+devin path: a worker whose native agent state proves no turn in flight while its pane keeps rendering never forms a stable hash, so `bin/fm-watch.sh` keys that stopped-worker signature on native idle plus continued churn and routes it through the identical bound and wedge escalation.
That same bound also holds an idle-looking pane's `FM_STALE_ESCALATE_SECS` escalation while its recorded step is still inside it, so a healthy long silent foreground step - an in-flight sleep-based status poll, a long foreground command - is not misreported as a wedge; the hold's anchor is evidence every harness produces rather than a marker only one adapter writes, and it re-arms the idle timer so the at-threshold probes above keep their once-per-`FM_STALE_ESCALATE_SECS` cadence, while a genuinely wedged step still escalates within one stale interval of that bound crossing.
...
The drain collapses genuine repeats in first-seen order, but never two distinct check results that share a key: a check's key is only the channel that produced it while its payload is the deliverable, so grouping those by key alone would silently drop the earlier event.
...
Before classifying coalesced signals, the watcher coordinates turn-end publication with teardown through the task's metadata lock, retaining that lock through queue publication and seen-marker updates.
A turn-end whose lock is unavailable is deferred to a later scan; one whose signal file teardown has removed is discarded, so a snapshot captured before teardown cannot announce the retired signal or recreate its seen marker afterward.
Teardown removes the turn-end signal and its seen marker using the shared naming helper, including for dotted task IDs.
This retirement filter applies only to turn-end signals, and an empty filtered batch still falls through to pane-staleness and heartbeat checks.
[`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh) covers concurrent teardown during signal coalescing, dotted task IDs, and unrelated heartbeat delivery while teardown holds the metadata lock.
...
[`configuration.md`](configuration.md#crew-dispatch-profiles-configcrew-dispatchjson) owns the schema, judgment versus opt-in typed resolution, spawn enforcement, and bootstrap diagnostics.
Spawn launch stays compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, rovo, omp, agy, and Herdr-only devin while preserving the requested profile for later audit.
...
A pool worktree is only taken or returned after the slot-ownership proof passes: a task record naming the assigned path refuses a fresh ship or scout before it claims the slot and refuses teardown before it returns one, a supported live endpoint refuses teardown, and no discard authority relaxes either.
[`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh)'s header owns the shared scan over every local home's task records and the canonical spelling it compares, while [`bin/fm-spawn.sh`](../bin/fm-spawn.sh)'s header owns the spawn-side refusal.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, teardown-side slot-ownership refusals, endpoint-close refusal, PR-discovery fallback, pre-teardown run conclusion, and stale-lock recovery procedure; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh) and [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh) pin the return-side slot-collision boundary, and [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) pins the spawn-side refusal.
