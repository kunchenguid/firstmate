...
Zellij alone also reconciles a supervision-only container next to those task tabs: one display tab per home whose panes render existing crew reads and hold no task endpoint or lifecycle authority, with the other backends no-op for it; [Supervisor surface](zellij-backend.md#supervisor-surface) owns that mechanism.
