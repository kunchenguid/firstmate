...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling, a Relay mention, or an inbound WhatsApp message, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, Relay polling, or an armed inbound message channel has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, a process-event source, Relay polling, or an armed inbound message channel needs supervision and no identity-matched watcher lock with a fresh beacon is live, blocking-capable Stop hooks block and nonblocking turn-end integrations force one bounded follow-up.
...
## Optional inbound WhatsApp channel

The captain can also reach firstmate from his phone, and that direction is opt-in per home in exactly the shape Relay uses.
A home enables it by writing a non-empty `FM_WA_CAPTAIN` into its gitignored `config/whatsapp.env`; with no such file every WhatsApp entry point is a hard no-op, so a home that never opts in polls nothing and behaves exactly as before.
`bin/fm-wa-setup.sh arm` writes `state/wa-watch.check.sh` and binds it through the ordinary registered-custom-check path, plus the generated `config/wa-mode.env` cadence, so nothing in `bin/fm-watch.sh`, `bin/fm-watch-arm.sh`, `bin/fm-wake-lib.sh`, or the away-mode daemon changes.
An armed shim counts as supervision need in `bin/fm-supervision-lib.sh` exactly as a Relay poll does, because the captain messages precisely when nothing else is running.
Outbound stays on mudslide's existing linked device while `bin/fm-wa-listen.sh` runs a second linked device of its own under `state/wa-auth/`, because baileys allows one live connection per credential folder and arming the channel must not be able to break `mudslide send`.
That listener holds the one long-lived connection and stashes accepted messages as `state/wa-inbox/<message-id>.json`; `bin/fm-wa-poll.sh` is a local directory read that announces the pending set as one `wa-message ...` check line, restarts a dead listener on a bounded budget, reports a channel fault as `wa-channel-error ...`, and prunes the channel's own records.
Only the configured numbers are accepted, on either the phone-number or LID form of a direct chat, and inbound text is treated as untrusted data that never reaches a command line; the `wa-respond` agent-only skill owns what to do with a message once it lands, and replies go back out through `bin/fm-wa-send.sh`.
[whatsapp-channel.md](whatsapp-channel.md) owns the operator-facing setup, pairing, dry-run, and opt-out contract, and the [inbound WhatsApp configuration reference](configuration.md#inbound-whatsapp-channel-configwhatsappenv--configwa-modeenv) owns the keys and generated artifacts.
