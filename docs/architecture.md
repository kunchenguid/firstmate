...
Its `fm_nm_select_run` owns complete same-branch run selection and ambiguity reporting, using an optional Python 3 SQLite reader only when AXI caps its overview; `tests/fm-crew-state.test.sh` covers replacement gates, competing live runs beyond that window, and preservation of newer failures.
