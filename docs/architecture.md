...
The drain also advances the per-signal acknowledgement cursor `state/.wake-acked-cursor` (a private list of signal keys appended to by the watcher on every queued wake and removed by the drain on every consumed key) so the arm layer can distinguish a cycle that exits on a NEW signal from one that exits on the same signal set the previous cycle already queued. The arm captures a strictly-before-the-cycle snapshot of the cursor under `state/.wake-acked-cursor.snapshot.<watcher-pid>` and records the cycle as `reason=actionable-signal-unacked` (instead of `reason=actionable-signal`) when the wake keys are in the snapshot, with the existing lifecycle-ledger format staying byte-stable for the rest of the tooling. Consecutive unacked exits bump `state/.wake-unacked-streak`; the binary `actionable-signal-unacked` is the cycle-by-cycle verdict, the streak is the supervision-level alarm aggregator. Three consecutive unacked exits (`FM_WATCH_UNACKED_THRESHOLD`, default 3) arm `state/.guard-watcher-unacked-banner` with the unread task ids, and `bin/fm-guard.sh` prints the unacked-exit banner with the same episode dedup as the watcher-stale banner, naming the unread task ids (issue 2028).
