...
If firstmate's own pane cannot be verified from an explicit target, `$TMUX_PANE`, or herdr markers, `/afk` refuses before setting `state/.afk` instead of using the blind legacy `firstmate:0` guess.
...
Startup refusals after the daemon owns its singleton lock also clear and verify any stale `state/.afk` flag before claiming the fleet has returned to full per-wake supervision.
