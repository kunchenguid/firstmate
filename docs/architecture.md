...
Herdr's durable default container shape is workspace-per-home plus tab-per-task: the primary home prefers workspace label `FirstMate Crew` and falls back to an existing legacy lowercase `firstmate` workspace during upgrades, secondmate homes use `2ndmate-<secondmate-id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
