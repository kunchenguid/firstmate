...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, confirmed vanished endpoints, and heartbeat backstop hits.
...
A pane the backend cannot read is never silently skipped: reads never start a backend server, so a failed capture is real evidence rather than a self-healing hiccup.
The watcher separates a transient read error from a vanished endpoint with the side-effect-free existence probe and raises exactly one `stale: <window> (endpoint gone)` wake, so a fleet that disappeared is reported instead of dropping out of triage.
That verdict must hold for `FM_ENDPOINT_GONE_CONFIRM_SECS` of real elapsed time (default 30) with the task's `state/<id>.meta` still present, because a deliberate teardown kills the pane several steps before it removes that meta and would otherwise look identical; the confirmation is wall-clock rather than a poll count because the poll interval is configurable and a fast poll would confirm nothing.
