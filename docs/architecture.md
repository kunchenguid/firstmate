...
Fresh Treehouse allocation first checks retained task records, actual durable reservations and orphan claims under the canonical project lock; [`bin/fm-treehouse-inventory.py`](../bin/fm-treehouse-inventory.py) owns the read-only admission checks and supported configuration boundary.
An aborted launch retains its claim for recovery, and a later launch cannot replace that evidence.
This conservative admission guard does not convert legacy process reservations into durable task allocations or authorize disposal; it refuses unresolved ownership instead.
