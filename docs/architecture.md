...
The bounded pause cadence applies whether the underlying agent is alive (the ordinary case for a declared pause) or has confidently exited (a captain-held or paused crew whose agent is gone); agent liveness only promotes an unchanged `paused:` or durable `captain-held` endpoint that has no authoritative paused verdict of its own, and only when the backend confidently reports its agent dead - it never demotes an authoritative paused verdict back to an immediate surface. Inconclusive liveness with no authoritative paused verdict remains fail-open (surfaces), and the secondmate idle-endpoint exemption is unchanged.
