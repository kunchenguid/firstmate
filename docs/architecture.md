...
The watcher repeats that receipt, body, and head verification before every merge-state lookup, so drift wakes Firstmate as `publication-invalid` and cannot produce a merged outcome.
