...
Herdr's container shape is workspace-per-home plus tab-per-task: workspace labels use the shared home-tag - the active home label plus a short hash of the resolved `FM_ROOT` path, so the primary home uses `firstmate-<hash>` and secondmate homes use `2ndmate-<secondmate-id>-<hash>` - while per-task tab labels stay the bare `fm-<id>`, and recovery/list-live scopes to the current `FM_HOME`'s workspace.
