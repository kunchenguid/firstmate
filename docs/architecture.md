...
[`bin/fm-pr-check.sh`](../bin/fm-pr-check.sh)'s header owns the guard contract in full, and [`bin/fm-base-lib.sh`](../bin/fm-base-lib.sh) owns the shared declared-base rules the two consumers must not drift on: the `git ls-remote` present/absent/probe-failed probe, and whether a base that is gone from origin actually merged.
