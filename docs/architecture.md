...
A second, narrower exception: an uncorroborated plain "still validating" full-status reading defers to a later `paused:`/`done:` status-log line once the branch's PR is already recorded in meta, so a stale run object that never advanced past `running` cannot mask a declared pause or completion.
A reading counts as corroborated only when some step is both reading running or fixing and still live by the `active_steps` `last_activity` age, because a step row is a durable record that a reboot or a killed agent leaves frozen at `running` indefinitely; `FM_RUN_STEP_LIVENESS_MAX` (default 30 minutes) sets that window and `active_for` is deliberately ignored.
