...
In normal mode a crew that declared `paused:` is routed onto the pause cadence below at that bound instead of onto the wedge escalation; the never-interrupt guarantee is the same on either route.
...
A crew that declares `paused:` for a known external wait is separately absorbed - whether its pane sits idle or stays busy past that turn-age bound - and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
Both consumers absorb the busy-pane turn-age wedge escalation described above, and the away-mode sub-supervisor additionally ages that pause window against the crew's own latest status line rather than against pane busy state, so a pane that keeps rendering busy behind a stuck external wait still re-surfaces once per `FM_PAUSE_RESURFACE_SECS` instead of going silent.
Only a later non-paused status append ends that routing and restores normal wedge detection.
...
The daemon escalates captain-relevant events, plus a bounded recheck for a still-declared pause, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages.
