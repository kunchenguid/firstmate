...
When the canonical validated PR outcome path returns an exact merged result, the watcher appends its destination-qualified human outcome before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
`bin/fm-pr-poll.sh` is the single extractor and formatter for both ready and merged outcomes: it reads the forge-reported base for both, defers the repository-default lookup until an exact merge needs classification, and explicitly leaves default-branch delivery unverified when either value is unavailable.
It distinguishes open review from a still-draft, closed-without-merging, or locked result rather than folding them together, and a forge it cannot read at all is reported as unavailable while arming still records the canonical identity and arms the watch.
Every sentence it emits names the object by its own forge, `PR <url>` on GitHub and `MR <url>` on GitLab, and the arming refusal for a missing provider CLI names the same object.
Every arming path passes through `fm_pr_poll_prepare`, which refuses to publish a poll whose provider CLI (`gh`, or `glab` and `jq`) is absent, so neither registration nor a migration rebuild can arm a watch that could only ever fail silently.
The receipt makes retirement safely retryable across restarts: fixed-path recovery revalidates the same evidence, removes the runnable check first, removes its registration and data sidecars, removes the receipt last, and preserves task metadata including `pr=`, `pr_base=`, `pr_default=`, and `pr_head=` when present.
...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records the canonical PR identity and any available destination, repository-default, and head evidence through `bin/fm-pr-check.sh --pre-merge` before calling the forge CLI.
That flag keeps the observed state visible while marking it `before merging:`, so a pre-merge reading is never presented as the PR's standing outcome.
