...
After every wake decision of a cycle, the poll also launches the display-only progress tick owned by `bin/fm-progress-lib.sh` as a detached single-flight child, which re-reads each task's phase on a bounded cadence and refreshes its Herdr workspace label suffix only when the phase or rounded estimate changed, so its current-state reads never sit on the loop and can never delay a wake; [configuration.md](configuration.md#progress-phase-and-remaining-time-guess-dataphase-historyjsonl) owns that model.
