...
`bin/fm-dod-lib.sh` is the one owner of that mode's definition of done and of the project-instructions block that carries a project's own committed agent instructions and its issue-linking moment, rendered both into a generated ship brief and into the ship instructions a promoted scout receives, so a promoted worker cannot be handed a weaker contract than a briefed one.
