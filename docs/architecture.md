...
Orca also exposes a native busy verdict (its agent-status `working`/`done`/`blocked`, correlated to the task terminal), used the same evidence-of-activity way; tmux, zellij, and cmux expose no native busy primitive, so a task on those backends is classified purely from its adapter's own lifecycle record.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux, Herdr, and Orca have verified classifiers for secondmate recovery, while Zellij and cmux do not, so a dead secondmate on those two backends is reported inconclusive rather than auto-respawned.
...
cmux's container shape is one workspace per task with one surface, no per-home container split; workspace titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path, and `--secondmate` spawns are refused (cmux is now the only spawn backend that still refuses them).
