...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence while a `paused:` or durable `captain-held` declaration remains current; the pause classification itself is recovered only when the backend confidently reports its agent dead.
...
Its later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change or still-declaring status append.
The live-agent inspection is spent once while the declaration remains current, so the declaration - not a rendered pane hash or status-file signature - owns the cadence across watcher restarts until the last status line stops declaring a wait or authoritative state proves work resumed.
