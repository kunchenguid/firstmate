...
A measurement younger than that window is not judged at all and does not overwrite its own baseline, so the window always matures and a real stall is delayed by at most `FM_NM_PROGRESS_WINDOW_SECS` rather than suppressed.
