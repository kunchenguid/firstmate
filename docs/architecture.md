...
For whole-fleet read-only review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, current crew state, durable delivery-continuation records, endpoint probes, PR/report pointers, scout reports, bounded current summaries from registered secondmate homes, and secondmate return-channel guidance.
An attributed active or terminal no-mistakes run remains the primary task state when its validation instruction is still unacknowledged; without that attribution, Bearings keeps the pending instruction, any unverified busy source, or an exact-head conflict visible instead of presenting activity as pipeline-owned.
