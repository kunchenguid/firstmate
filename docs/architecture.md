...
The same coverage identity also bounds terminal-status stale re-hashes after a finished result was already delivered, under this proof's own `state/.outcome-resurfaced-*` throttle with declaration `outcome-covered:<seq>:<status-sig>`, re-surfacing once per `PAUSE_RESURFACE_SECS` so a forgotten teardown still reaches the long cadence without claiming the shared pause marker.
An unchanged pane under that throttle is rechecked every poll, so a static finished pane re-surfaces on the same cadence, and the first idle sight after the pane was seen busy under the same identity surfaces instead of being absorbed.
