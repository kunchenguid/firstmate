...
`bin/fm-nm-run-lib.sh` owns that attribution rule in full, and `bin/fm-teardown.sh` asks the same rule so it sees the same live run before it removes a worker.
Each script header owns how its own reader applies the verdict.
