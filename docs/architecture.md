...
At each escalation moment, and only there, both supervisors first ask `fm-classify-lib.sh`'s `crew_run_progressed` whether that crew's own no-mistakes run is executing and has advanced since the previous ask; if it has, the aging window restarts instead, because a crew blocked on an advancing pipeline agent renders nothing and its quiet endpoint is what a healthy validation looks like.
