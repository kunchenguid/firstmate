...
That recency field is the live `axi status` `active_steps` table with no `quiet` prefix, the pipeline's own verdict, so a working record that survived a relaunch, a coarse runs-list status word, a missing table, a quiet-prefixed step, a terminal or parked run, and no run at all keep the existing escalation schedule.
