...
The absorb is bounded rather than permanent silence, and the accepted limit of that bound has exactly one owner: the `merge_wait_absorbs_alarm` header in `bin/fm-watch.sh` states in full which alarm attempts the `FM_PAUSE_RESURFACE_SECS` reminder can reach, what covers a pane that attempts none, why the `heartbeat:` fleet review is not a second cover, the residual exposure and the follow-up filed for it, and what a crew-state verdict can and cannot catch.
Read it there rather than here, because two statements of that one contract would drift the moment only one was edited.
