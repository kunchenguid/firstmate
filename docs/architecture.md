...
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation, base freshness, and pool-ownership refusal, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
...
Before refreshing a pool worktree for fresh allocation, Firstmate refuses if any reachable local task record still names the selected copy or if the reachable ownership inventory or record cannot be read.
Fresh allocation writes or refreshes the slot-owner claim only when an existing claim names the same task ID and physical Firstmate home; a foreign or unreadable claim refuses rather than being overwritten.
A pool worktree is only returned after teardown passes the same record inventory plus the claim and branch proofs: a record collision or unreadable ownership evidence refuses before cleanup, and no discard authority relaxes that.
Any recorded worktree checked out on another task's `fm/<task>` branch refuses before direct or forced descendant cleanup unless a pool owner claim already proves reassignment to a different task ID.
A slot's own owner claim, written by the spawn that takes it under the allocation lock and owned by [`bin/fm-wake-lib.sh`](../bin/fm-wake-lib.sh), binds the task ID and physical Firstmate home identity and covers a slot reassigned to an owner that left no record the scan could reach: a claim naming a different task ID releases nothing - teardown warns, names the claimant, and finishes only the task's own cleanup - because Treehouse's own live process lease cannot answer ownership once the worker's exit releases it.
A claim naming the same task ID in a different physical home refuses because endpoint and task-temp cleanup remain task-ID-scoped, while canonical aliases of the same physical home remain the task's own claim.
