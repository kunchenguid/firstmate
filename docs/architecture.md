...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written or their validation run's current step advancing, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A crew parked on a no-mistakes validation run defeats pane quietness, the run step, and the worktree probe at once, because the pipeline takes the turns and commits its fix rounds in its own checkout, so the watcher separately measures the run's own current step: it resolves the active step from the run record, then compares that step's log size and its agent process's CPU time against a baseline recorded when the quiet window opened.
Resolving the step on every measurement is what keeps a finished step's permanently flat log from reading as a wedge once the run has moved on.
Any measured advance defers the escalation on the same `FM_PAUSE_RESURFACE_SECS` cadence with a reason naming the step, while a step that is still claimed to be executing and has not moved across `FM_NM_PROGRESS_WINDOW_SECS` escalates exactly as before, and so does every absence of evidence - no run, no active step, a run that names another branch than the worktree's own, a missing or torn-down worktree, a secondmate home, and either bounded call timing out at `FM_NM_PROGRESS_TIMEOUT`.
A measurement younger than that window is not judged at all and does not overwrite its own baseline, and the window's first usable measurement is likewise a baseline rather than a verdict - the seed probe can find no active step, and a timer matured in that gap must not turn the step that starts afterwards into a wedge - so in both cases the window always matures and a real stall is delayed by at most `FM_NM_PROGRESS_WINDOW_SECS` rather than suppressed.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, worktree-write and step-progress deferrals, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
Away-mode housekeeping has neither the worktree-write nor the step-progress deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or whose validation run is advancing, still escalates as a possible wedge at that bound.
