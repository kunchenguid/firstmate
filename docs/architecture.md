...
The whole marker set (`state/<id>.ratelimit`, `.attempts`, `.failed`) is also cleared the moment the pane content changes to a render that no longer shows a quota/overload footer, since the crew is active again; a render still showing the footer keeps the marker, so a genuinely new rate-limit episode later starts fresh instead of reusing a past reset or inheriting a stuck `.failed` that would forbid parking for the rest of the task.
