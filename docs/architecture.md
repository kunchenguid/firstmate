...
codex-app is experimental and selected only explicitly: Firstmate creates a guarded detached git worktree, starts a Codex Desktop thread there through `bin/fm-codex-bridge`, validates the returned cwd, and waits for a status-file return-channel line before treating the thread as supervised.
