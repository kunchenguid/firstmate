...
A confirmed batch is retired only by its original flush path, and termination requests shutdown cooperatively so an in-flight confirmation reaches that retirement before cleanup while an unconfirmed batch stays buffered for return catch-up.
Bash can dispatch TERM after a confirmation command substitution returns but before its caller clears the buffer, so a cleanup-time flush at that boundary would see and submit the same batch again.
Cleanup never starts a second submit, which keeps a delivered batch outside both retry and catch-all paths after confirmation.
The daemon's watcher child runs in its own verified process group, so shutdown can stop that exact child tree within a bound and reap its leader without signaling another home's supervision.
...
The away daemon gives each Herdr supervisor-transport CLI call its own hard deadline so one stalled vendor call cannot hold the cooperative shutdown boundary indefinitely.
