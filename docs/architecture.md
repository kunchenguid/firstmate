...
The script first replays the single pending packet byte-identically, even after opt-out, a new wake, or an attempted manual drain; with no pending transaction, a regular non-symlinked `wake-context-presentation` file under `FM_CONFIG_OVERRIDE`, or otherwise `$FM_HOME/config`, authorizes a bounded `fm-wake-context.v1` projection of the durable presentation, including its post-handling acknowledgement.
The opt-in is home-local and never inherited into secondmate homes. When neither that opt-in nor a previously published transaction exists, the notification preserves the actionable reason and emits one ordinary human-drain instruction without draining, caching, staging a cursor, publishing a receipt, or acknowledging anything.
This presentation contract makes no claim about reduced real provider-token usage; that requires telemetry outside the transaction mechanism.
...
The packet embeds a bounded projection of the drain's human presentation, replays byte-identically until its exact acknowledgement, and advances the unread-status presentation cursor only when that acknowledgement is applied.
Before any `WAKE_ACK_REQUIRED` is published, the drain validates the status snapshot identity and either stages its cursor for wake-context or commits it durably for an ordinary drain, including when the wake queue or status set is empty.
If that identity changes during presentation, the drain withholds the acknowledgement and leaves the durable queue and live cursor unchanged so a retry can present the replacement status.
If collection fails after the cursor stage exists, the fallback returns the complete human presentation and persists a minimal acknowledgement receipt before publishing the acknowledgement, so acknowledging that fallback cannot replay already handled unread status.
Status and queue byte volume is checked before presentation. A published packet or fallback receipt remains the sole transaction until its exact acknowledgement retires it, and a manual drain cannot supersede its receipt, cache, or staged cursor.
...
Behavioral coverage in [`tests/fm-wake-context.test.sh`](../tests/fm-wake-context.test.sh) pins the acknowledgement ordering for nonempty and empty queues, empty status sets, invalidated identities, retries, and post-staging timeout fallback.
Drain and acknowledgement are actor-scoped: main claims every queue row not reserved by a live branch grant, while branch may present and consume only the exact granted sequence snapshot; an absent or stale claim is never guessed.
...
A queued signal annotation prints every status line still unread at that cursor, while the fleet-wide UNREAD STATUS section prints `note:` lines and reserved-key pending-reply resolutions even on an empty-queue drain because those verbs never enter the OPEN DECISIONS fold; the wake-context acknowledgement boundary described above determines when those lines cease replaying.
