...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge/close polling or an X-mode mention, ordinary stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A merge-ready park is the third expected-idle absorb class: when `crew_is_merge_ready` in `bin/fm-classify-lib.sh` confirms its fail-closed poll, live-PR, current-state, and endpoint conditions, the idle pane is absorbed without stale wakes or wedge escalation because the authenticated poll owns the merge/close wait.
The predicate's source comment owns its exact trust and identity checks; any failed condition restores ordinary stale detection, including current check regression, PR-head mismatch, or known endpoint death.
...
The always-on watcher also uses that library's absorb predicates on no-verb signals and first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge and declared-pause recheck cadences.
