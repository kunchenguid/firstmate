...
Ryder does its own capture and composer-row recognition first, because it is the only backend whose snapshot carries both a style channel and a cursor position, then hands the recognized row to that same owner for the verdict.
The agent/shell/other process-name vocabulary those liveness classifiers read has its own shared owner, `bin/fm-process-identity-lib.sh`, used by the tmux and Ryder adapters, so a newly verified harness is taught to the fleet once.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over Ryder's `RYDER_SESSION_ID`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr, cmux, or ryder prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij and orca are never auto-detected (only explicit selection).
...
Ryder reports `busy` only on positive proof, when a process that is neither a harness nor a shell owns the pty, and never claims idle, which process identity cannot establish.
...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, Zellij remains unverified, and Orca, cmux, and Ryder do not support secondmate spawns.
...
Ryder is experimental and is the first backend with no container layer at all: one detached PTY host process per session, the sessions directory as the registry, and an endpoint target that is a single session id derived from the home-scoped task label rather than a composite that must be split.
It ships pull-only, so the watcher's poll loop remains its event source; [`ryder-backend.md`](ryder-backend.md) owns current setup and limits, and [`verification/runtime-backends.md`](verification/runtime-backends.md#ryder) owns active evidence.
Codex App support is recorded in `docs/codex-app-backend.md`; it is not selectable as a runtime backend.

## Worktrees, not branches in your checkout

Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, cmux, and ryder tasks, while Orca creates its own worktrees for `backend=orca`.
...
Fleet state lives in each task's session-provider backend (tmux by hard default, herdr, cmux, or Ryder when selected or auto-detected, zellij/orca when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
