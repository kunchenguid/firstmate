...
A wait is read from both of its records: the status line a worker declared, and the backlog hold `bin/fm-captain-hold.sh` recorded once firstmate handed the work to the captain.
So a delivered task whose last line stays `done: PR ...` takes the same bounded `FM_PAUSE_RESURFACE_SECS` cadence rather than re-alarming on every pane tick for the length of the captain's decision.
Only that repetition is bounded: a task with no open captain call keeps alarming unchanged, a held crew whose pane then goes genuinely quiet still reaches its wedge escalation on the non-terminal path, and the window's end re-surfaces held work once so a forgotten hold cannot rot invisibly.
