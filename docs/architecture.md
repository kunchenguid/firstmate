...
The daemon injects only into an affirmatively `empty` composer AND a pane with a confirmed live agent process, so a bare dead-shell prompt cannot receive an escalation even when it shows the `❯` glyph that reads `empty` (starship's default prompt is byte-identical to claude's composer glyph); the complete policy is in [Composer-emptiness safety](herdr-backend.md#composer-emptiness-safety-2026-07-10-fleet-wide-across-all-four-backends) and [Agent-liveness injection guard](herdr-backend.md#agent-liveness-injection-guard-2026-07-14-the-starship-glyph-hole).
Unsupported supervisor backends refuse at daemon startup.
That liveness guard fails closed, so a supervisor the backend cannot attribute at all (pi runs as a generic `node`) receives no escalation for the whole away session - which is announced rather than left to be discovered: `bin/fm-afk-canary-lib.sh` owns one probe that `bin/fm-afk-launch.sh` re-derives in the foreground as away mode is armed and `bin/fm-bootstrap.sh` re-derives at every session start as an `AFK_INJECTION_DISABLED:` line, always live, never from a stored marker, exactly like the worktree-tangle guard below.
Stalled escalation delivery writes `state/.subsuper-inject-wedged` and attempts a configured backend-independent active alert after `FM_MAX_DEFER_SECS`, naming the cause `inject_msg` recorded rather than assuming a busy pane, instead of silently deferring forever.
A supervisor pane that vanishes outright cannot reach that housekeeping alarm at all, so the main loop's backoff raises its own `state/.subsuper-pane-gone` alarm - separate marker, one best-effort alert per absence episode, with the reach and the deferred follow-ups spelled out in [`wedge-alarm.md`](wedge-alarm.md).
...
It has two consumers with opposite failure modes, so both refuse to act on `unknown` but only one loses a capability by doing so: session-start secondmate recovery, where a wrong `dead` would spin up a duplicate supervisor, and away-mode injection above, where a wrong `alive` would type a command into the captain's shell.
