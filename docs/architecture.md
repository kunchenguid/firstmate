...
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge timer, where a still-busy verdict suppresses the escalation until the `FM_BUSY_TURN_HARD_MAX_SECS` no-completed-turn ceiling, and only past that ceiling does it surface with the reason, escalation count, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
