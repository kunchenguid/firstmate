...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the current tip of the resolved default branch, and any unsafe or unverifiable base stops the spawn.
That tip is the fetched `origin/<default>` wherever an origin is configured, and the clone's own default-branch tip when the project is registered as `local-only` in `data/projects.md` and so has no origin to fetch from.
A missing origin on any other project is a damaged pool, not a local-only clone, so the spawn is refused rather than started from a local tip no remote confirmed.
