...
That tip is fetched from `origin` when the project has one, so an unreachable `origin` still refuses the spawn; a project with no `origin` at all, which a `local-only` project may be, has no remote tip to be stale against and starts from the local default branch its linked task worktree already sees.
