...
Because the pool decides slot reuse from live-agent presence rather than recorded task ownership, `fm-spawn.sh` also refuses a resolved worktree that any other task's `state/<id>.meta` still claims, so a crash- or reboot-orphaned but still-owned slot cannot be silently handed to a new spawn and have its unlanded work reset.
