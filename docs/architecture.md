...
`bin/fm-dod-lib.sh` owns which modes end in a PR and `bin/fm-classify-lib.sh` owns the reading, so every firstmate-facing surface that renders a status line - the drain annotations, the session-start tails, the watcher's actionable reasons, and a secondmate's upward ledger report - refuses that landing through the same test, while a `local-only` ship's ready branch and a scout's report stay accepted untouched.
