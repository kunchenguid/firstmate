...
`fm-spawn.sh` acquires each pooled worktree with `treehouse get --lease --lease-holder <task-id>`, durably leasing the slot to the task rather than relying on pid liveness, so an idle or concurrently live worker's worktree is never recycled; only `fm-teardown.sh`'s `treehouse return` frees it.
