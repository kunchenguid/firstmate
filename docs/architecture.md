...
The guard denies direct, path-qualified, wrapper-qualified, nested-shell, eval, substitution, alias, REST merge-endpoint, and GraphQL merge execution before GitHub is called, while leaving push, PR creation, review, comment, and CI operations available.
A harness without a verified blocking shell hook is refused before its endpoint launches, and the task-local PATH never reaches Firstmate's own `bin/fm-pr-merge.sh` process.
