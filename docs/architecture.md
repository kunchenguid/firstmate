...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge` (GitHub) or `glab mr merge` (GitLab).
The helper requires a full `https://github.com/<owner>/<repo>/pull/<n>` or `https://<host>/<group path>/-/merge_requests/<n>` URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md) for the GitLab host and path shape), invokes `gh-axi pr merge <n> --repo <owner>/<repo>` or `glab mr merge <n> --repo https://<host>/<group path>` accordingly, defaults to `--squash`, preserves explicit merge-method flags, and rejects malformed URLs or repo override flags before recording merge state.
