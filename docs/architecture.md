...
A presence-gated sub-supervisor (`bin/fm-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill enters through `bin/fm-afk-launch.sh`, which owns lifecycle state and either prepares a harness-native background job or creates a non-visible tracked terminal for the common foreground entry `bin/fm-afk-start.sh`; the watcher then reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash.
