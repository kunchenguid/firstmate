...
A declared wait that the crew's authoritative current state NAMES takes that cadence immediately, whatever the agent's liveness, with no first stale wake, because that state has already reported where the crew is standing.
Trusting that report costs bounded latency rather than a lost wedge: a worker that declares a pause and only then wedges - parked at a harness permission or trust dialog it cannot report, so its idle pane leaves current state falling back to the declaration and still naming the pause - is caught by the `FM_PAUSE_RESURFACE_SECS` recheck instead of the shorter `FM_STALE_ESCALATE_SECS` ladder.
Where authoritative state cannot name the wait - a captain hold, which has no current-state mapping, or a crew whose own no-mistakes run has since parked at a gate, which its current state reports as that park rather than as the declared pause - the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` or durable `captain-held` endpoint; in that unnamed case the pause classification itself is recovered without that first wake only when the backend confidently reports an ordinary crew's agent dead.
Live or inconclusive liveness remains fail-open at that initial surface of an unnamed wait, so a worker standing somewhere its stale declaration does not name is never silenced by that declaration.
