...
`bin/fm-dod-lib.sh` is the one owner of each mode's delivery contract, rendered both into a generated ship brief and into the ship instructions a promoted scout receives; both PR modes therefore carry the same guarded pre-delivery and live-published-text scan requirement, while `local-only` carries none.
