...
Two further reads protect work a merge would otherwise break silently: an open pull request whose base is that head branch makes a squash or rebase rewrite the commits it is stacked on, so that merge is refused and the stacked pull requests and the `--merge` that preserves them are named, and another open pull request on the same head branch makes the task's own pull request ambiguous, so that merge is refused and every open pull request on the branch is named.
Both guards are GitHub reads, because a GitLab merge request's method is the project's own setting rather than a default this path chooses.
