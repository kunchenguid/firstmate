...
Plain `arm` also self-heals a WEDGED incumbent - a process still alive and still holding this home's lock with a matching identity, but whose beacon has gone stale past `FM_GUARD_GRACE` because it stopped beating - by running the same home-scoped stop `--restart` does and then forking a fresh watcher, all in one invocation, instead of returning `FAILED` and forcing a manual `--restart`.
That wedge predicate requires the beacon to exist and be stale, so a startup-race peer that just took the lock and has not beaten yet is excluded and left to beat; a healthy incumbent stays a clean no-op, and a live lock held by something that is not this home's watcher (identity mismatch) is never signalled and stays the honest `FAILED` case.
Its `--restart` mode and this arm-self-heal share one home-scoped stop that signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting or self-healing one home cannot kill sibling secondmate watchers.
