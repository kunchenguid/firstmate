...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without a declared wait, a live no-mistakes run step with recent activity, or a current task-worktree write, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A quiet pane whose current state is a `working` `run-step` with `activity: recent` also defers the pending wedge escalation because the pipeline is executing the crew's work outside the pane.
The deferral restarts the `FM_STALE_ESCALATE_SECS` timer and re-surfaces as a validation recheck after `FM_PAUSE_RESURFACE_SECS`, rather than as a possible wedge.
The ordinary wedge schedule remains in effect for a run step whose activity is quiet or missing, a coarse run with no activity table, a parked gate, a failed or terminal run, and a busy pane that has crossed `FM_BUSY_TURN_MAX_SECS`.
...
The attended watcher gives validation and worktree-write deferrals one quiet-window anchor and one re-surface throttle, so switching between those two forms of evidence cannot restart the bound or produce two rechecks inside one cadence.
...
Every working run-step verdict ends with `activity: recent` or `activity: quiet`, using the pipeline's active-steps table as the single source of that fact; the coarse runs-ledger fallback always reports quiet because its status response can belong to another run.
