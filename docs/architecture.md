...
A fourth, default-on proof absorbs a bare turn-end when the task's bounded branch-outcome index already covers its whole current status log under a matching identity: the exact state was already handled, so another turn boundary with no new status bytes is noise.
That proof is tracked per endpoint in `state/.turnend-covered-since-*` and bounded by `FM_TURNEND_OUTCOME_ABSORB_SECS` (default 900) for the same reason the churn bound is mandatory.
Any `.status` file in the batch, an absent or invalid index, an identity mismatch, new status bytes, a steering-inbox record (`*.msg` under `<task>.inbox/` or `handled/`) newer than the status bytes the outcome covers, a pane seen busy under the covering outcome, or an exhausted bound surfaces exactly as before.
A steer is work the status log has not reported yet, so coverage breaks on the record's send time and stays broken until the worker appends status again, whoever sent the steer and whenever the covering outcome landed relative to it: the branch steers in one step and reports in the next, so an outcome newer than the steer proves nothing about the turn that steer started, while handled/ moves and ring-ladder cleanup that only bump the inbox directory mtime do not.
A covered pane seen busy is additionally recorded in `state/.outcome-busy-*` against the covering outcome's identity, which keeps that record out of the shared `.paused-resurfaced-*` throttle whose gates the declared-wait and captain-call cadences own; the next bare turn-end and the next idle sight both surface, a surfaced stale wake clears it, and a newer outcome retires it with the identity it names.
The same coverage identity also absorbs terminal-status stale re-hashes after a finished result was already delivered: only that new-hash repetition is bounded, a stable hash stays silent after its first terminal alarm, and the first idle sight after the pane was seen busy under the same identity surfaces instead of being absorbed.
...
A captain-facing outcome persists as one exact, sequence-keyed visible transcript entry; `action:main` (default, and legacy omitted) then opens one sequence-keyed processing turn on main that only main's sequence-bound acknowledgement closes, while display-only `action:none` finished results stop at the visible entry.
