...
The lifecycle projection tells `/next` which task deserves attention, while its structured packet and the task brief's durable review plan bound read-only preparation before the smallest captain handoff; [`task-lifecycle.md`](task-lifecycle.md#attention-allocation) owns that boundary.
