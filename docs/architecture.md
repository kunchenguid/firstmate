...
Before that longer wedge bound, the watcher performs one bounded rendered-dialog inspection after `FM_BUSY_DIALOG_INSPECT_SECS` without meaningful progress, using the shared composer classifier's explicit approval/question shape; a confirmed choice wakes firstmate for manual inspection, while ordinary busy output is recorded as inspected until meaningful progress changes.
...
`bin/fm-tasks.sh` adds callsigns and ordered table rendering to that projection, while `bin/fm-task.sh`, `bin/fm-next.sh`, `bin/fm-close.sh`, and `bin/fm-history.sh` compose the same lifecycle owner for detail, deterministic attention ranking and preparation evidence, guarded archival, and recall.
The lifecycle projection tells `/next` which task deserves attention, while its structured packet, skill-side preparation request, and the task brief's durable review plan bound read-only preparation before the smallest captain handoff; [`task-lifecycle.md`](task-lifecycle.md#attention-allocation) owns that boundary.
...
Ship tasks change projects and ship by project mode (`no-mistakes`, `direct-PR`, or `local-only`); scout tasks leave standalone investigation reports at `data/tasks/<id>/report.md` and never push.
