...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is neither provably working nor under a declared external-wait pause, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A busy pane whose crew declared `paused:` takes the long pause cadence at that bound instead of the wedge escalation, so a crew idling on its own background work is rechecked rather than reported as a possible wedge every `FM_STALE_ESCALATE_SECS`.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only when `bin/fm-crew-state.sh` reports positive evidence that the crew is still working - an actively running no-mistakes step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract - or reports a declared external-wait pause.
Absorbing that second class is what keeps declaring or refreshing a `paused:` line from waking the supervisor that declared it.
...
Its status signals are absorbed by the no-verb path in normal mode, while away mode self-handles that routine signal and owns the later recheck.
