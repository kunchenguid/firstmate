...
A run lookup killed before it answers is not an answered "no run": it never reaches that log fallback, reporting unknown and naming the timeout unless an exact busy pane overrides it (`bin/fm-crew-state.sh`'s header owns the exact precedence).
