...
Unread instructions, declared waits, and an authoritative attributed run keep their existing paths, while the pane busy signature and the status-log verb left behind by the stopped worker describe that dead worker rather than the present and never revive its inherited wedge timer; the bounded wait and its declaration identity are owned by `bin/fm-watch.sh`.
