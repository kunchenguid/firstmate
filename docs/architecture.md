...
It also owns the worktree base section appended to a launch brief and to a promoted scout's ship instructions when a spawn placed that slot on the project's registered working branch, because only the placement itself knows which branch the worker is on and a brief written before the slot exists asserts the default branch.
