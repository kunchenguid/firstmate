...
It leads with a prominent bordered tangle banner, while `bin/fm-guard.sh` owns the watcher banner/notice/reminder policy so repeated guarded commands stay noisy without reprinting the full watcher-down banner in the same episode.
That policy separates a watcher that completed a cycle and is awaiting re-arm, which the watcher is designed to do after every delivered wake, from one that is genuinely gone, and states the condition the health check actually rejected on rather than a fixed explanation.
