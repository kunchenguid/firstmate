...
PR readiness and task merges go through `bin/fm-pr-check.sh`, which first requires `bin/fm-independent-review.sh` to produce a different-family clear verdict for the forge's exact current head, then records `pr=` and that reviewed `pr_head=`; `bin/fm-pr-merge.sh` uses the same boundary before calling `gh-axi pr merge`.
That verdict's provenance is cooperative task identity: a submission is accepted only when it names a reviewer task bound to the round and recorded as a fresh scout in the resolved different family, which enforces provenance against mistakes and drift rather than against a deliberately dishonest agent.
Every firstmate agent runs under the same uid, so a builder that chose to lie could still record its own clear verdict; that sits outside the accepted honest-but-fallible threat model and is deliberately not defended against.
