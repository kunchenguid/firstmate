...
Claude keeps its tracked background-task protocol and adds a narrow PreToolUse continuity gate that allows drain, arm recovery, and fail-closed teardown while refusing only other fleet commands when actively progressing work or queued wakes need supervision and no identity-matched live watcher holds the home lock.
The turn-end guard remains the final backstop for all five harness protocols, now using the shared active-only supervision predicate.
Its `--restart` mode signals only the watcher recorded in the current home's `state/.watch.lock`, so restarting one home cannot kill sibling secondmate watchers.
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if actively progressing work or queued wakes need supervision and that watcher stops running.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when actively progressing work remains and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up; parked, terminal, blocked, and declared-pause tasks do not force that checkpoint.
...
Ship tasks change projects and ship by project mode (`fast-preview`, `no-mistakes`, `direct-PR`, or `local-only`); scout tasks leave standalone investigation reports at `data/<id>/report.md` and never push.
...
`bin/fm-project-mode.sh` owns exact mode names and parsing.
The global default is `fast-preview`: implement, run only the minimum smoke needed for a safe runnable result, start a human-test preview, and stop at review with a complete inspection path.
Missing registry entries, bare legacy lines without `[mode]`, and unknown mode tokens inherit `fast-preview` rather than forcing `no-mistakes`.
