...
A slot's own owner claim, written by the spawn that takes it under the allocation lock and owned by [`bin/fm-treehouse-slot-lib.sh`](../bin/fm-treehouse-slot-lib.sh), covers a slot reassigned to a task that left no record the scan could reach: a claim naming a different task releases nothing - teardown warns, names the claimant, and finishes only the task's own cleanup - because Treehouse's own live process lease cannot answer ownership once the worker's exit releases it.
The claim is the stronger evidence, so it is read before the record scan; the scan itself never consults it, because a claim belongs to a slot rather than to a record and cannot say which of two records naming that slot is stale. A record-vs-record collision therefore refuses in both directions and a human moves the stale record aside, the command the refusal prints.
The same claim answers the inverse question at session start: a slot whose claimant has no live worker is a leak the whole pool pays for, so bootstrap names each one and what was observed about it without ever returning, resetting, or re-claiming a slot, or even printing a command that would, because a held slot can still carry work no branch holds ([`bin/fm-pool-leak-lib.sh`](../bin/fm-pool-leak-lib.sh)).
A fresh spawn onto a task whose record still holds a claimed slot refuses for the same reason: replacing that record would leave the slot claimed by a task no record names, which no cleanup command can reach.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, slot-ownership proof, endpoint-close refusal, PR-discovery fallback, pre-teardown run conclusion, and stale-lock recovery procedure; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh) and [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh) pin the slot-collision boundary, while [`tests/fm-pool-leak.test.sh`](../tests/fm-pool-leak.test.sh) and [`tests/fm-spawn-slot-record-clobber.test.sh`](../tests/fm-spawn-slot-record-clobber.test.sh) pin leak detection and the spawn refusal.
