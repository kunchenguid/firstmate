...
The helper requires a full GitHub pull request URL on `github.com` or an enterprise host allowlisted in [`config/github-hosts`](configuration.md#github-enterprise-hosts-configgithub-hosts), invokes `gh-axi pr merge <n> --repo <owner>/<repo>` with the parsed enterprise hostname selected through `GH_HOST` when applicable, defaults to `--squash`, preserves explicit merge-method flags, and rejects malformed URLs or repository and hostname override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
