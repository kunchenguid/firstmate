...
Once that row reaches `FM_SECONDMATE_WAKE_STALL_SECS`, a latest `blocked:` status always wakes the primary, an exact semantic busy verdict keeps the watcher quiet for however long the turn legitimately runs, and idle or unknown liveness conservatively wakes; the primary appends one keyed `check` wake naming the mate, row sequence, and observed age, while parent receipts and queued-key deduplication suppress repeats for the same row across watcher and handling crashes and empty or younger queues remain silent.
