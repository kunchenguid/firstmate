...
A fourth rendered-text check also stays outside this contract, at the crew-state resolution layer rather than in `fm_busy_classify`: `bin/fm-crew-state.sh` overrides an already-busy `claude-hook` verdict to paused when the pane's own tail shows Claude Code's account/usage-limit banner, scoped to `harness=claude` and owned by `fm_busy_claude_limit_banner` (`bin/fm-busy-lib.sh`). An attributed no-mistakes run stays authoritative - parked and active runs keep their own state and only gain a `worker pane limit-blocked` note - and the resulting pause is accepted as a real pause by the watcher and the supervise daemon through `crew_pause_still_live` (`bin/fm-classify-lib.sh`).
