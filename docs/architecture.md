...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, a secondmate whose unresumed declared work sits on an idle endpoint past `FM_DECLARED_WORK_STALL_SECS`, and heartbeat backstop hits.
...
The same poll therefore checks every secondmate for declared work that no later event superseded while its endpoint stayed idle past `FM_DECLARED_WORK_STALL_SECS`, and surfaces that combination as its own `check: secondmate-stalled <id>` reason rather than as a stale pane, so the supervisor reconciles the mate and pushes the work forward.
`fm-classify-lib.sh` owns the decision and its vocabulary (`status_declares_work`, `status_work_declared_unresumed`, `status_declared_work_stalled`, `FM_CLASSIFY_DECLARED_WORK_VERB`) in the one library both supervisors source, so the away-mode daemon reads the same verb and receives the resulting wake through the same single supervision cycle; the endpoint read runs only after the pure status and idle-age gates already hold.
A declared `paused:` wait, a terminal last event, and a proven-busy endpoint each keep their existing meaning and never become a stall.
Neither does a task with no locally readable `state/<id>.turn-ended` marker: that marker is the only idle clock this host can read, and a remote mate's turns complete on another host, so without it there is no evidence of idleness and absence of evidence never becomes evidence of a stall - a rule about what this host can observe, not a remote special case, so a local harness that installs no turn-end hook is silenced by the same reasoning.
A stall surfaces exactly once per declared-work episode, latched until a later status event or a completed turn ends the episode, because the durable queue is acknowledged only after the handling turn and so one wake cannot be lost.
That single bounded wake is also what the last-event rule honestly supports: it narrows, rather than eliminates, the unsuperseded-start-event false signal the registered-secondmate current-state section below records, since a phase the mate finished without closing its record is indistinguishable from unresumed work from outside, and both readings mean the mate's own stream disagrees with reality, so the wake states both and suppression is deliberately not built.
