...
devenv is experimental, selected only explicitly, and is a control plane rather than a session provider: it owns no task endpoint, no worktree, and no spawn path, and instead drives fixed-command SSH requests against existing Expanly OrbStack VMs for registry discovery, commit-pinned runtime installation, inspection, queueing, and token-guarded leases.
Selecting it for a task spawn refuses loudly instead of falling back, because resident execution is not implemented; [`devenv-backend.md`](devenv-backend.md) owns current setup, protocol, and safety boundary, and [`verification/runtime-backends.md`](verification/runtime-backends.md#devenv) owns active live evidence.
