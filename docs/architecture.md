...
That fold only accepts lines that follow the status grammar the library header documents, and the drain prints a companion STATUS LINE ANOMALIES section naming every line it refused, every misplaced key token it had to honor, and every closure for a key nothing opened, so a writer that drifts off the grammar surfaces on the next wake instead of quietly leaving a decision open.
