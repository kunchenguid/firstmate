...
A worktree whose HEAD is provably unborn - nothing ever checked out or committed in it, so it has no HEAD reflog - names no commit and passes through like an ordinary directory; a ref store that cannot answer that question, or a branch ref that vanished from a worktree that has one, still refuses.
