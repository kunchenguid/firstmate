...
The helper requires a full `https://<host>/<owner>/<repo>/pull/<n>` URL where `<host>` is `github.com` or a host listed in the local `config/pr-hosts` allowlist (see [configuration.md](configuration.md#pr-hosts-configpr-hosts)), invokes `gh-axi pr merge <n> --repo <owner>/<repo>` with `GH_HOST` scoped to a non-`github.com` host, defaults to `--squash`, preserves explicit merge-method flags, and rejects malformed URLs, disallowed hosts, or repo override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
