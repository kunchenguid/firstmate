...
Because a review round moves the PR head, ship briefs also require the worker to fast-forward a reviewed branch to the actual PR head before editing and to run `bin/fm-push-guard.sh` immediately before every push.
That guard fails closed on any push it cannot prove preserves every remote-reachable commit; its header owns the exact contract and [`verification/push-safety.md`](verification/push-safety.md) owns the active evidence.
