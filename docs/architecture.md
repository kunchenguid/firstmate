...
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress, and the watcher routes an unchanged stale hash through its bounded wedge timer so each escalation re-arms at `FM_STALE_ESCALATE_SECS` while the pane remains frozen.
