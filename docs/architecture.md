...
A `no-mistakes` project may also carry the optional `+tiered` flag (and the paired `+ci-tests`), resolved and passed explicitly the same way: a deterministic classifier (`bin/fm-tier.sh`, pure git diff, no agent) buckets the finished diff so a small, low-risk change runs a lighter gate - docs-only diffs take the `direct-PR` path, and a diff at or under 100 changed lines and 5 files that is entirely code/test/docs with no high-risk paths and a test-presence signal skips the `document` step (and the local `test` step too under `+ci-tests`, meaning CI already runs the same suite); `review` and CI are never skipped at any tier, and any doubt escalates to the full gate.
