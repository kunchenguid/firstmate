...
For a Codex primary, the present-mode daemon treats the installed CLI's `codex queue` command as a doorbell to the exact thread bound from the live Codex lifecycle payload and tool environment, never as the wake payload or acknowledgement.
The private binding is tied to the current home, session-lock generation, owner PID, and process identity, while one outstanding record coalesces delivery for the bound session, recovery generation, and delivered wake-sequence boundary until the ordinary drain acknowledgement retires that covered boundary.
If a higher sequence survives acknowledgement, `fm-wake-drain.sh` invokes the daemon's serialized one-shot queue-first delivery entry so the survivor has a doorbell without starting a second watcher.
Missing or invalid native delivery falls through to the existing guarded terminal injection and then the bounded foreground checkpoint without changing queue durability or composer safety.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, muse, and traex while preserving the requested profile for later audit.
...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` refreshes the authoritative base and, when task meta records `pr=`, asks the `bin/fm-scm-lib.sh` provider seam for the current remote head before using a reachable recorded `pr_head=` as an offline fallback and finally the local branch with a warning.
...
Codebase MRs continue through `bin/fm-scm-lib.sh`, default to a real merge commit, and refuse to squash a merge-commit head whose second parent must remain available as a future upstream merge base.
The Codebase provider contract is documented in [codebase-scm.md](codebase-scm.md), while exact accepted flags live in the script headers.
After a forge command returns, the script confirms the PR or MR actually landed, and only a confirmed landing records a landed outcome; a queued or unconfirmed request records none and leaves its poll armed.
