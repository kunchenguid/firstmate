...
Acquiring a pooled worktree starts with a command typed into the task's endpoint, so `fm-spawn.sh` owns one further boundary on every backend: before each fixed spawn-time command - `treehouse get`, and later the environment exports and the agent launch line - it proves the endpoint's shell is executing what is typed into it, because a shell still running its own startup swallows input while the pty echoes it anyway, which makes pane text no proof that a command ran.
Every way that proof can fail stops the spawn naming the endpoint state it left behind, because an unproven shell, an unreachable endpoint, and a probe the backend left sitting in that shell's input line each need a different repair; its header owns those refusal classes, what makes the proof itself trustworthy, and the tuning knobs, while `tests/fm-spawn-shell-ready.test.sh` owns the portable regression coverage.
