...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the rendered submit core that the tmux send backend now uses as its labelled fallback, while herdr uses native busy state, native agent-state submit confirmation on idle baselines, and its ANSI-aware structural composer classifier for pending-input guards and submit fallback.
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and fallback-path `fm-send` sends do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow. The same opencode busy-queue case is a known gap on the herdr adapter and is recorded in `docs/herdr-backend.md` rather than patched here. For a task whose harness has a trusted semantic lifecycle source and whose record proves an idle baseline, `fm-send` submit confirmation instead requires an observed idle-to-busy transition in the busy-record contract below, and the rendered core is only its explicitly labelled fallback ([tmux backend](tmux-backend.md)); mid-turn sends stay on that labelled fallback, the daemon's context-free supervisor injection keeps the rendered core unchanged, and send-to-turn correlation and concurrent-send serialisation are deliberately deferred to the fm-submit-correlation follow-up.
...
Three rendered-text readers deliberately remain outside this contract because they answer delivery questions: the fallback submit acknowledgement and away-mode supervisor-pane busy guard in `bin/fm-tmux-lib.sh`, and the secondmate delivery-confirmation observation in `bin/fm-pending-reply-lib.sh`.
All are harness-scoped rather than a global pattern union, and none is a recorded worker state source.
Submit confirmation on the tmux path is a read-only consumer of this contract for harnesses with a trusted source: it confirms delivery only from an observed idle-to-busy transition in the task's busy record after a delivered Enter, consults the rendered acknowledgement only as an explicitly labelled fallback, and during bake-in compares both signals, records disagreements durably, and downgrades a contradicted confirmation to unconfirmed ([tmux backend](tmux-backend.md)).
