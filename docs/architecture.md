...
The snapshot's explicit `--read-only` mode keeps live reads and valid existing-cache reads while disabling cache creation and refresh; `--project-status-source` additionally suppresses task endpoint and terminal observation plus parent event collection, retaining only attributable structured current-run fields for local main tasks, without CI-log or status-prose reconciliation, and otherwise reporting unknown.
