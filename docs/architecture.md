...
For whole-fleet review, `bin/fm-fleet-snapshot.sh --json` emits schema `fm-fleet-snapshot.v1` from the backlog, task metadata, local current crew state, supervision-owned endpoint evidence, PR/report pointers, scout reports, current summaries from registered secondmate homes, and secondmate return-channel guidance.
Each home atomically publishes its home summary with freshness epoch metadata at `state/home-summary.json` after a locked session start, a watcher-observed status change, task spawn, task teardown, and on a recurring live-watcher cadence; `bin/fm-home-summary-refresh.sh` owns the publication mechanics.
The fleet snapshot and Bearings paths use the concurrent remote-ledger collection, cache, per-surface bounds, delivery retention, unreadable-home disclosure, and remote-liveness boundary owned by `bin/fm-fleet-snapshot.sh`'s header.
...
Cross-home reads validate the seeded identity and operational-directory boundaries and never revive a parent event as current work; [`bin/fm-fleet-snapshot.sh`](../bin/fm-fleet-snapshot.sh)'s header owns collection, cache selection, unreadable-home behavior, and partial retention under inventory mismatches.
