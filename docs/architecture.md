...
Every generated task brief, ship and scout alike, closes on a second, separate store whenever the crewmate's runtime provides one - the harness's per-repo auto-memory, agent-local rather than committed - so a durable non-obvious finding outlives a discarded worktree; a runtime with no such store satisfies the same closing item by saying so in its closing status line.
That store's index loads whole into every future session under a size cap that drops the overflow silently - a magnitude observed on one measured version, not a version-independent guarantee - so briefs require a one-line pointer there and keep all detail in the memory file itself.
