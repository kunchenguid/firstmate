...
Actionable wakes include signals whose newly appended status content classifies as wake under `bin/fm-classify-lib.sh`'s content rule (`status_span_wake_class`: an opened-and-still-open decision, terminal or legacy captain-relevant lines, any secondmate stream content), routine-content signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, the liveness probe's alarms described next, and heartbeat backstop hits (unclassified actionable content, or an open decision due its bounded re-surface).
The liveness probe replaced the old pane-stillness stale path (plan v3 U1.4): window stillness is not a signal, and liveness is measured at the process.
Per recorded window, on a per-window cadence (`FM_PROBE_INTERVAL_SECS`, doubled per refuted alarm up to `FM_PROBE_BACKOFF_MAX` doublings), the watcher reads `fm_backend_agent_state` and then looks for progress evidence: a CPU delta across the probe interval (`fm_busy_cpu_progress` over the backend's pane CPU sample), a file written under the task's recorded worktree since the previous probe (`crew_worktree_written_since`, one pruned, depth-bounded, wall-clock-bounded walk per due probe - `FM_WORKTREE_WRITE_PRUNE`, `FM_WORKTREE_WRITE_MAXDEPTH`, `FM_WORKTREE_WRITE_TIMEOUT`), or pipeline movement (`crew_run_progressed`).
At those due probe moments, and only there, `crew_run_progressed` asks whether the crew's own no-mistakes run is executing and has advanced since the previous ask, because a crew blocked on an advancing pipeline agent renders nothing and burns no local CPU; that probe is one bounded read-only run read that can start or influence nothing, its fingerprint excludes elapsed timers, and absorbing requires movement, so a frozen run and every crew without an executing run of its own alarm exactly as before.
A confident agent-free verdict (dead or missing) is the agent-gone alarm, fired once per observed death; any progress evidence proves liveness, and if an unrefuted alarm stood, the evidence clears it and doubles that window's probe interval, so a false alarm makes the probe quieter, never louder.
A whole interval with no evidence splits on the semantic turn state: an idle crew with no attributed run surfaces once per idle identity (it may have finished through an interactive menu without a report), an idle crew whose authoritative current state is a vendor-derived external wait such as the Claude account-limit pause stays quiet on the bounded re-surface cadence, and everything else is the no-progress alarm - fired once, then re-surfaced at most once per `FM_PAUSE_RESURFACE_SECS` while unrefuted.
A declared wait is the machine field `state/<id>.wait` with reason and deadline, written by `bin/fm-wait.sh` and owned by `bin/fm-wait-lib.sh`: while active it silences every probe alarm for that task, even while a run or busy pane makes the worker look occupied, and at its deadline the watcher checks the task exactly once per field identity rather than once per poll.
A prose `paused:` line alone no longer damps the probe, the old precedence of surface or run-step busy-ness over the worker's own declaration is abolished, and so is the escalation counter with its `demand-deep-inspection` decoration.
A verified captain hold lives in the backlog as a held item, so its re-surfacing belongs to the heartbeat's bounded open-decision re-surface and the fleet review, not to the window probe.
A secondmate is never probed at all - an idle mate endpoint is healthy by design - and only its wait field's expiry check runs.
Every probe alarm is inspection-only: never an automatic interrupt, signal, or restart of the worker or its tool process.
...
A crew whose authoritative current state is an external wait keeps its quiet treatment through the probe rules above; the declaration itself already surfaced once through the signal path when it was appended.
No-change heartbeats are also benign.
Separately from heartbeat backoff and probe alarms, the watcher poll runs `bin/fm-inactive-reconcile.sh` on its own bounded cadence, while locked session start performs the same bounded local scan immediately.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, or if work, process-event sources, or Relay polling has an unhealthy model-aware supervision verdict; queued rows get no standing warning because a drain presents and consumes them.
The drain script calls that guard after presenting the queue; presented records are consumed at presentation, and the delivery guarantee for anything that must survive an interrupted handling turn is carried by the cursor-backed OPEN DECISIONS and UNREAD STATUS folds plus each upstream obligation's own durable record.
...
The always-on watcher also uses that library's absorb classification on no-verb signals and the liveness probe's flat split, while the away-mode daemon maintains distinct wedge and declared-wait recheck cadences of its own.
...
A fourth rendered-text check also stays outside this contract, at the crew-state resolution layer rather than in `fm_busy_classify`: `bin/fm-crew-state.sh` overrides an already-busy `claude-hook` verdict when the pane's composer region shows Claude Code's blocking account/usage-limit widget - to paused for a self-resolving widget, to blocked for its give-up rendering (a bare limit notice only annotates the working detail - `fm_busy_claude_limit_banner`'s header owns that deliberate limitation), scoped to `harness=claude` and owned by `fm_busy_claude_limit_banner` (`bin/fm-busy-lib.sh`). An attributed no-mistakes run stays authoritative - parked and active runs keep their own state and only gain a `worker pane limit-blocked` note - and the resulting pause keeps the watcher's probe quiet through the flat split's `paused` class, while the supervise daemon accepts it through `crew_pause_still_live` (`bin/fm-classify-lib.sh`).
...
For a pooled worktree of the firstmate repo itself - the repository the running `bin/` scripts belong to - the fresh base is instead the local default-branch head, because that origin is read-only and lags local main.
...
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
