...
Actionable wakes include signals whose newly appended status content classifies as wake under `bin/fm-classify-lib.sh`'s content rule (`status_span_wake_class`: an opened-and-still-open decision, terminal or legacy captain-relevant lines, any secondmate stream content), routine-content signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits (unclassified actionable content, or an open decision due its bounded re-surface).
...
At each STALE escalation moment, and only there, both supervisors first ask `fm-classify-lib.sh`'s `crew_run_progressed` whether that crew's own no-mistakes run is executing and has advanced since the previous ask; if it has, the aging window restarts instead, because a crew blocked on an advancing pipeline agent renders nothing and its quiet endpoint is what a healthy validation looks like.
That probe is one bounded read-only run read that can start or influence nothing, its fingerprint excludes elapsed timers, and absorbing requires movement, so a frozen run and every crew without an executing run of its own escalate exactly as before.
Next, a pane holding a file newer than the start of its own quiet window, anywhere in the worktree recorded for that task, is deferred instead of escalated, because a crew writing source, then tests, then documentation behind a static pane is liveness that neither pane quietness nor the run step can show.
...
That bound is deliberately exempt from the run-progress absorb above and still escalates on its own schedule, because it exists to catch a hung foreground call, which an advancing pipeline says nothing about.
...
The declaration is read fresh on every poll, so it damps a wedge escalation chain already under way and retires that chain's timer and counter, and it bounds itself: once the declaring status line has stood unrefreshed for `FM_PAUSE_DEMAND_INSPECT_COUNT` recheck windows, the recheck keeps its declared-wait identity but also carries the `demand-deep-inspection` marker, so a worker wedged behind its own declaration cannot stay silenced indefinitely.
...
A crew whose authoritative current state is an external wait - a declared `paused:` line, the pane-derived Claude account-limit pause described under the busy-state contract below, or a verified `captain-held` transfer - is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
...
Live or inconclusive liveness keeps that unconfirmed declaration surfacing, and a secondmate's endpoint liveness is still never read at all; a mate is admitted to that same cadence only to serve a declared wait's bounded re-surface, so a forgotten pause or captain hold on a mate cannot rot invisibly.
