...
[`bin/fm-pr-url-lib.sh`](../bin/fm-pr-url-lib.sh) parses the PR URL's shape to pick the forge, never a hostname allowlist: a singular `https://github.com/<owner>/<repo>/pull/<n>` URL is GitHub, a plural `https://<any-host>/<owner>/<repo>/pulls/<n>` URL is Gitea, and a hyphenated plural `https://<any-host>/<workspace>/<repo>/pull-requests/<n>` URL is Bitbucket (bitbucket.org for Cloud, a self-hosted host for Server/Data Center).
...
A Bitbucket URL has no CLI equivalent, so after recording `pr=` it prints open-URL guidance and exits non-zero - the merge happens manually in the browser, and teardown's content-in-default fallback verifies it landed.
All paths reject malformed URLs or repo override flags before recording merge state.
