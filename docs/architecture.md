...
Orca is experimental and selected only explicitly: the Orca backend creates firstmate-owned git worktrees directly, owns the daemon terminal lifecycle, records `orca_worktree_id=` and `terminal=`, and removes worktrees through guarded git worktree cleanup plus `bin/fmod kill` only after the usual firstmate teardown checks pass. Its current behavior and limitations are recorded in `docs/orca-backend.md`.
