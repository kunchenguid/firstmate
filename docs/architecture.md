...
Pane existence, busy checks, composer checks, capture, and verified submit route through `bin/fm-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr combines native agent-state submit confirmation with conservative composer fallback.
The [tmux backend guide](tmux-backend.md#current-behavior-and-safety) and [Herdr backend guide](herdr-backend.md#current-transport-behavior) own their exact delivery proofs, numbered composer-queue guard, Enter-retry semantics, and active exceptions.
