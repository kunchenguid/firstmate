...
Two guards settle ownership before either attribution route runs: the run binding `nm_run=<run-id>`, which the crew's own no-mistakes contract records through [`bin/fm-run-bind.sh`](../bin/fm-run-bind.sh) as soon as its run reports an id, and the delivery-mode gate, since a `direct-PR` or `local-only` crew never drives a pipeline of its own.
A bound crew is credited only its own run id, an unbound crew is credited only a run no other task has bound, and a branch carrying no binding at all keeps the existing branch-based behaviour so no existing home regresses.
