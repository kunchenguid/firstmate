...
Its shared `bin/fm-wake-lib.sh` lock helper keeps symlink publication on platforms where `ln -s` yields a readable owner link, but on Git Bash/MSYS it uses the existing legacy directory-lock path without invoking `ln -s`.
Stale-lock recovery there reclaims only positively identified generated nested owner directories with known lock files and refuses empty, malformed, or foreign nested content, while preserving the same exclusive-ownership, stale-recovery, and steal-lock fencing guarantees.
