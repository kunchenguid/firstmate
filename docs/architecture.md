...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the authoritative base selected by the explicit task mode and Git ancestry, and any unsafe or unverifiable base stops the spawn.
...
Without `origin`, Firstmate resolves the default only from the repository-local `init.defaultBranch` when it names an existing local branch, then from `main`, then from `master`, and refuses with remediation when none resolves.
