...
When a canonical validated PR poll returns exactly `merged` or exactly `closed`, the watcher appends that durable notification before publishing a private receipt bound to the poll's registration, bytes, file identities, metadata, provider, URL, and task ID.
`closed` means the watched pull request was closed without merging, which is a normal outcome rather than a failure: the change may have landed in a successor pull request, so the wake exists for the first mate to reconcile where the work went, and the receipt records which of the two results was observed so a closed pull request is never recoverable as a merge.
...
An exact `closed`, which only the GitHub path emits, means the watched pull request was closed without merging, which is a normal outcome rather than a failure: the change may have landed in a successor pull request, so the wake exists for the first mate to reconcile where the work went.
That result deliberately does not retire the poll, because a closed pull request can be reopened and merged and retirement would lose the merge wake; the poll stays armed and its repeated closed reading is held to a single wake by a `state/.seen-pr-closed-<id>` marker written after the durable wake and cleared whenever a poll is published for that task.
The marker is also cleared when the poll reads an exact `open`, the one reading that positively proves the pull request is no longer closed, so a pull request that is reopened and closed again wakes a second time; `open` itself never queues a wake, and silence never clears the marker because silence is also an unreadable pull request or a failed forge lookup.
Neither result exists on the GitLab path, which emits `merged` alone and stays silent on every other reading, because the token `glab` prints for a closed merge request is unverified and an unverified token would make that half a silent no-op.
