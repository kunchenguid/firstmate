...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as a due business-agenda item, PR merge polling, or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The business agenda uses the same authenticated custom-check rail rather than adding a scheduler service: locked primary bootstrap publishes one private registered check, the watcher invokes it on the existing slow-check cadence, and the scanner's local fire state makes repeated scans and restarts idempotent.
[Business agenda](configuration.md#business-agenda) owns current operator behavior and the local-only boundary.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, the persistent business-agenda check, process-event sources, or Relay polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
On every verified primary harness, tracked hook integration gives the primary session a push-based backstop: when work, the persistent business-agenda check, a process-event source, or Relay polling needs supervision and no identity-matched watcher lock with a fresh beacon is live, direct Stop hooks block and passive turn-end hooks force one bounded follow-up.
