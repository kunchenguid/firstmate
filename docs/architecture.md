...
When herdr env markers are absent, the daemon may recover the primary from a safe firstmate-home Codex match in `herdr agent list`, but refuses a legacy tmux fallback if herdr has agents and no safe primary is discoverable.
