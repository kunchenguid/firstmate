...
The explicit resolution is written by the actor that answers, not the busy worker: `fm-send`'s `--resolve-key` appends the closing `resolved` line to this home's own copy of the ledger, which covers crewmates, local secondmates, and remote secondmates identically because a remote mate's escalations reach that local copy through the parent-replies ingest and only the answer message itself crosses the transport.
When that line is written is a separate invariant, because delivering an answer and the worker reading it are different facts: on the local inbox plane the doorbell is deliberately skipped whenever the composer visibly holds pending text, so the closure is parked beside its durable record and committed only when the worker acknowledges that record, and until then the decision keeps reading open everywhere.
That is the safe direction - an open decision re-surfaces, while a decision closed on an unread answer makes a stopped worker read as a moving one - and the steering inbox's own bounded escalation is what keeps the unread case from being merely quiet.
An OPEN DECISIONS row whose answer is already enqueued but still unacknowledged says so on the row, so reconciliation neither treats it as unanswered nor re-answers a decision that is only waiting to be read.
The typed plane closes on its confirmed submit, and the remote plane on the remote enqueue, since a parent cannot observe a remote home's acknowledgement without a network round trip per poll and every marked remote request already carries a pending-reply expectation that recovers on its own; `bin/fm-send.sh`'s header owns the full contract.
...
[`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh)'s header owns the exact branch, head, and pipeline-custody rules, while `bin/fm-crew-state.sh` owns which run row those rules are applied to.
Only a branch's newest run row can describe its current state, so attribution binds that row or attributes nothing: an `axi status` answer for the crew's own branch never falls through to the coarse runs list, and the coarse list itself stops at that branch's newest row instead of walking back to an older one.
Surfacing a superseded run in a live one's place is what reported healthy validating crews as terminally failed.
