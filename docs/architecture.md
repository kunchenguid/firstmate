...
On `attached` it stays live across identity-matched successors and, if that non-owned target ends with no successor, takes over ownership and starts a fresh watcher rather than reporting a false failure; [`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the full arm-layer cycle contract, including the narrower owned-child case that still surfaces the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
