...
Structured secondmate summaries keep `active_children` limited to working children and carry every bounded child's lifecycle plus canonical `spawn_gen` through endpoint inventory; Project Cockpit joins parked, paused, and blocked children from that inventory, while an unproven generation drops mutable child evidence and receives only snapshot-scoped browser identity so refresh selection cannot cross incarnations.
