...
The most recent whole-line CI marker is treated as an observation, and `forge_zero_check_verdict` in `bin/fm-crew-state.sh` owns the exact terminal-evidence rule before the active monitor reports done or no CI configured.
