...
The arm layer records one bounded lifecycle row per observed cycle in `state/.watch-cycle-exits.log`, opened when the cycle begins so an untrappable kill still leaves evidence, with [watcher-continuity.md](watcher-continuity.md#arm-layer-cycle-contract) owning that record contract; `state/.watch-triage.log` remains exclusively the absorbed-wake debug log.
