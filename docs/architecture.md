...
That handoff is keyed on the declaration itself (the declared wait's verb and reason) rather than on the pane capture, so a harness footer that ticks on every poll wakes the daemon once per declaration instead of once per poll, and it clears the wedge timer, escalation count, and worktree-write deferral exactly as the normal-mode absorber does, so an undeclared busy phase's timer does not resume when the declaration lifts.
...
Its later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change, because the throttle and its age are keyed to the declaration (the declared wait's verb and reason) and not to the pane an idle parked worker keeps ticking, so a status write that leaves that event unchanged neither restarts the cadence nor wakes early, while a new or changed declaration surfaces at once. Because that identity is the declaration content, leaving a wait resets it only when a watcher poll observes the non-wait event: a word-for-word re-declaration written within one poll of leaving the wait, or while the watcher is down, inherits the previous wait's recheck timing and age, although its status write still wakes firstmate at once.
...
Only when no matching run exists does it consult semantic busy state; exact busy reports working, exact idle permits fallback to the log's resolved current declaration - the newest decision the fold still holds open, otherwise the latest recognized event, read through the declared-wait read below so a `note:` under a declared wait leaves that wait current - when its verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log.
...
Every decision about whether a declared wait is still in force goes through the library's declared-wait read (`status_wait_line`, which owns the contract): an informational `note:` appended under a declared wait keeps that wait, its cadence and its age, while any other event ends or replaces it as before, and the `note:` itself stays the latest event for every other reader.
