...
`config/backend` is inherited as each secondmate home's local runtime-backend default for future spawns, with deliberate per-home overrides preserved and explicit `--backend`/`FM_BACKEND` still stronger.
