...
Any `.status` file in the batch, an absent or invalid index, an identity mismatch, new status bytes, steering-inbox activity the covering outcome is not strictly newer than, a pane seen busy under the covering outcome, or an exhausted bound surfaces exactly as before.
A steer is new information the outcome never saw, so the worker's first stop after one surfaces even when it writes no status.
Ordering between a steer, the worker's acknowledgement, and the branch's own report is a race no writer serializes, so a covered pane seen busy is recorded against the covering outcome's identity on every poll, whether or not a stale sight armed the throttle first; the next bare turn-end and the next idle sight both surface, and a newer outcome retires the observation with the identity it names.
