...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is neither provably working nor declared idle, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared idle endpoints - an external wait, a captain-held transfer, or a still-open keyed captain decision - that remain absorbed past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The cadence applies only while the crew is still parked on that decision, meaning the line that opened the key is still its last status line, so a crew that resumes and moves again is wedge-detectable and backstop-visible immediately even when nobody closed the key.
A holder that dies is surfaced once and only once, since only a confident live verdict retires that one-shot record and an ambiguous backend read leaves it alone, and its later rechecks then name the exited agent rather than asking firstmate for an answer that would restart nothing.
Closing the key remains the only thing that closes the decision itself, and the fold keeps surfacing it until then.
