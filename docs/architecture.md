...
The projector marks each browser identity scope explicitly: canonical backlog and landed-history rows remain stable across snapshot timestamps, proven active rows use their generation, and unproven active rows remain snapshot-scoped so selection cannot cross an uncertain incarnation.
