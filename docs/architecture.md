...
A `<base-url>/<owner>/<repo>/pulls/<n>` Gitea (or Forgejo) URL invokes `tea pr merge <n> --repo <owner>/<repo> --login firstmate-<host>-<port>` and defaults to `--style squash`; the `tea` login is re-derived from the base URL (host plus port, so instances on one host do not collide) and the base URL must be listed in `config/gitea-instances` (see [configuration.md](configuration.md)) before any side effect, because a plain-http self-hosted origin cannot be trusted from its shape alone.
That path merges only after one live read confirms the pull request is open, not already merged, and mergeable; `tea pr merge` exposes no head binding, so it follows the GitHub model rather than GitLab's `--sha` and only reports a recorded `pr_head` that disagrees.
After a forge command returns, the script confirms the PR or MR actually landed - GitHub through gh, GitLab and Gitea through their own live JSON read - and only a confirmed landing records a landed outcome; a queued or unconfirmed request records none and leaves its poll armed.
On GitLab an auto-merge-queued or unconfirmed request is reported without failing the run.
On Gitea, which merges synchronously with no queue, a `tea` success that does not read back as merged is refused loudly, while a state that cannot be read leaves the poll armed.
