...
At fleet intake, the dispatch file is intentionally judgment-based: firstmate reads the natural-language rules, chooses the best matching rule, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
Native-child classification follows the generated-brief contract owned by [`configuration.md`](configuration.md#nested-delegation).
