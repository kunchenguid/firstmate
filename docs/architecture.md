...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is neither provably working nor under a standing `paused:` or `captain-held` declaration, whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` with neither a wait their own worker declared nor their own task worktree being written, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The normal-mode watcher applies that cadence to a standing `paused:` or `captain-held` declaration whatever the backend reports about the agent - alive, unknown, or dead - and only a provably working crew outranks the declaration.
So a declared wait is absorbed from its first sight, bounded only by the `FM_PAUSE_RESURFACE_SECS` recheck or an earlier declared `until` time, and a worker that dies or wedges under a standing declaration is left to that recheck rather than surfaced promptly.
Every sight is held to that same bounded cadence rather than re-alarming on every pane-hash change, because both the recheck age and its throttle are keyed to the declaration and not to the pane an idle parked worker keeps ticking.
A secondmate's endpoint liveness is never read on this path either; a mate reaches it only through a status-declared wait, so a forgotten `paused:` declaration, or an attended `captain-held` declaration, cannot rot invisibly.
