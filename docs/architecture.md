...
Each task's mode and `yolo` delivery posture are firstmate's decision at intake.
...
`bin/fm-dod-lib.sh` is the one owner of that mode's definition of done and its yolo-specific pull-request readiness instructions, rendered at the spawn or promotion boundary into worker launch briefs, promoted ship instructions, and promoted `brief.md` so a later relaunch reads the same contract and a promoted worker cannot be handed a weaker contract than a briefed one.
It is also the one owner of the no-mistakes `--intent` contract those workers follow.
`data/projects.md` records each project's standing posture and optional `+yolo` delivery flag as the captain's default and as context for that decision, including the conditional `no-mistakes-prod-only` policy; a ship spawn that drops below the registered rigor prints a deviation notice and continues.
