...
A confidently ended ordinary worker is classified at the shared liveness boundary and again at the shared wedge boundary, so preserved state remains a recovery wait rather than repeated evidence of a stuck agent.
Tmux and Herdr use process evidence; Zellij, Orca, and cmux conservatively accept only a bottom-most bare shell prompt as rendered death evidence, while every other unreadable or ambiguous shape remains unknown.
If a confirmed dead or missing endpoint cannot provide pane bytes, the watcher reconciles it by recorded task identity instead of skipping it at capture; an open captain call names that wait as captain-owned, otherwise it produces one recovery notice on the same bounded cadence.
Unread instructions, declared waits, and an authoritative attributed run keep their existing paths, while the pane busy signature and ordinary status-log verb left behind by the stopped worker never revive its inherited wedge timer.
Positive daemon socket failure remains an explicit daemon-recovery alert in both the stale watcher and workerless-run observer rather than collapsing into generic worker-loss or lost-attribution text.
The bounded wait and its declaration identity are owned by `bin/fm-watch.sh`.
