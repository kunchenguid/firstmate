...
The mate's own `wake()` transition independently publishes a durable transition timestamp and queue-sequence watermark under its own serialization before the bounded diagnostic delivery ledger runs, so equal-second ordering follows the watermark, a handling turn stays quiet however long it runs, and an absent, unreadable, malformed, older, future, or unorderable marker never suppresses an abandoned queue while the primary only reads the foreign marker and queue.
