...
On `attached` it stays live across identity-matched successors, and a clean child close is accepted without a captured reason only when the verified cycle advanced the durable wake sequence while its beacon remained fresh; otherwise it attaches to a verified healthy successor or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
