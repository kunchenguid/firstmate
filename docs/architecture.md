...
Delivered work waiting on a merge nobody in the fleet controls takes the same bound from a different record: while a `done:` task's merge poll is armed, that poll owns the next event for it, so new pane hashes are absorbed on the same cadence and the declaration carries the pull request plus the status-log state, which re-alarms at once on a re-arm against a different pull request or any new status line.
Neither bound reaches a blocker, an open decision, a failure, or a delivery with no armed poll: those keep alarming on every new hash, because a bound that swallowed an unreported event would be worse than the churn it removes.
...
A task record's PR identity is the single `pr=` URL matching the sidecar, not its position in the file: the record is an append-friendly key=value file with many writers, so ordinary well-formed fields added after that line carry no identity and keep the poll authenticated, while a second `pr=`, an invalid `pr_head=` after it, and any content that is not a field still refuse the poll.
...
When no run is attributed and the reading falls back to the status log, one log state is checked against the endpoint before it is trusted: `working` claims the crew is producing work right now, so on the two backends that own a recovery-grade classifier it is refused when that classifier proves the endpoint outlived its agent, because the bare shell a departed agent leaves behind answers every cheap probe and would otherwise report whatever the worker last wrote.
Every other log state describes work that is not in progress and stays true after the agent exits, so a delivery, a failure, a blocker, an open decision, and a declared wait all still read from the log.
