...
Diverged defaults remain `STUCK:` except for a clean checked-out default whose exact root tree matches the freshly fetched remote commit; that tree-identical squash case preserves the old local head at the script-owned durable ref before an expected-old atomic reconciliation.
