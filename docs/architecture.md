...
Every generated task brief, ship and scout alike, closes on a second, separate store as well - the harness's per-repo auto-memory, agent-local rather than committed - so a durable non-obvious finding outlives a discarded worktree; its index loads whole into every future session under a hard silent size cap, so briefs require a one-line pointer there and keep all detail in the memory file itself.
