...
It also records a fixed `Surface contract: web|non-web` line, and web briefs carry the generated website deployment completion gate owned by the brief and web-gate helper; a missing or invalid surface declaration or incomplete web gate stops the spawn.
