...
That verdict must hold on two consecutive polls with the task's `state/<id>.meta` still present, because a deliberate teardown kills the pane several steps before it removes that meta and would otherwise look identical.
