...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as forge change-request merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
`AGENTS.md` section 7 owns the current delivery behavior for `no-mistakes`, `direct-PR`, and `local-only`, including forge-specific change-request guidance.
...
When a selected delivery path calls for a diff, [`bin/fm-review-diff.sh`](../bin/fm-review-diff.sh)'s header owns provider-specific change-request head resolution and local-branch fallback behavior.
For target project repos shipped through their own no-mistakes pipeline, commits under `.no-mistakes/evidence/` are the pipeline's forge-change-request-viewable validation evidence and are expected to stay in the crew branch until the evidence-hosting design changes.
...
GitHub pull-request task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, GitHub pull-request-discovery fallback, and stale-lock recovery procedure.
...
The locked session-start bootstrap step, forge change-request teardown, and merged-change-request wake handling refresh remote-backed project clones when the clone is safe to move.
