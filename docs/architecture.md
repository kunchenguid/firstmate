...
That block owns the live wait shape for the running primary harness: Claude, Grok, and OMP use background-notify cycles, Codex uses bounded foreground checkpoints, Pi uses its two tracked primary extensions, and OpenCode uses its TUI plugin.
...
When local `config/afk-slack-dm` exists, the daemon flushes escalations as a Slack DM through `bin/fm-slack-dm.mjs` instead of pane injection; failed delivery preserves the buffer and writes `state/.subsuper-slack-dm-failed`, and persistent failure past max-defer fires the same active alert throttled by `state/.subsuper-slack-dm-wedged` (docs/configuration.md "Away-mode Slack DM delivery").
...
That keeps spawn launch compatible across claude, codex, grok, pi, opencode, and omp while preserving the requested profile for later audit.
