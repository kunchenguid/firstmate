...
The branch's current nonterminal run keeps that ownership even when the pipeline has advanced its mutable head beyond the unchanged worktree, while terminal and coarse historical attribution stays strictly head-bound; the script header owns the exact run-selection and head-binding rules.
