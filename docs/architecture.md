...
On `attached` it stays live across identity-matched successors, and an unexplained clean child close either attaches to a verified healthy successor, recovers the ended cycle's reason from the durable wake queue when one was delivered, or becomes the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result; [`watcher-continuity.md`](watcher-continuity.md#arm-layer-cycle-contract) owns the full arm layer's successor, terminal-delivery, and typed clean-close failure contract.
