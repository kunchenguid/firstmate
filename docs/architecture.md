...
`config/backend` is inherited as each secondmate home's local runtime-backend default for future spawns, with private provenance preserving deliberate per-home overrides and explicit `--backend`/`FM_BACKEND` still stronger.
