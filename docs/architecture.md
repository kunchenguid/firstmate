...
Every durable append also taps this home's watcher (`fm_wake_tap_watcher` in `bin/fm-wake-lib.sh`, a signal to the identity-matched lock holder that advertises the tap), so a producer outside the watcher - a captain inbox note, a voice capture, a process-event result - is surfaced by the next cycle at once rather than after the remaining `FM_POLL` sleep, and the poll cadence remains the backstop for a missed tap.
