...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign only on positive evidence that the crew is still working: a currently attributed active no-mistakes step, or an exact busy verdict from the semantic busy-state contract, both read through `bin/fm-crew-state.sh`.
A home that creates `config/turnend-churn-absorb` opts a wake carrying only bare turn-ended markers into a third form, because neither of those is reachable for an adapter whose semantic busy state has no verified source: pane content that changed since the previous poll, compared against the same `state/.hash-*` marker the staleness backbone records, which claims no harness semantics and needs no adapter cooperation.
...
That evidence clears the pane's prior stale classification and defers such a wake rather than swallowing it, since a crew that has stopped renders nothing further and its now-static pane surfaces through the staleness backbone within a poll or two, even if its final bytes match an earlier stale render; a wake naming any status file, a secondmate, an unresolvable endpoint, an ambiguous marker key, a missing or malformed prior hash, or a capture that fails or returns empty surfaces exactly as before.
...
If two recorded endpoints derive the same per-window marker key, that marker is not attributable churn evidence for either endpoint, so the bare turn-ended wake surfaces without changing or migrating existing marker state.
