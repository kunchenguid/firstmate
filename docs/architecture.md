...
A stale pane is classified once, when its pane hash is first seen, so the wedge timer expiring is not on its own proof of a wedge: before escalating, both the watcher and the away-mode daemon re-read the crew for fresh evidence it is still working, because a crew blocked on a long foreground pipeline step renders an unchanging pane for minutes while its run is genuinely active.
An active run or exact busy verdict defers the escalation and restarts the timer, a still-working rendered status row defers it for at most `FM_WEDGE_ACTIVITY_DEFER_MAX` consecutive rounds because rendered text only corroborates, and no evidence escalates on the unchanged schedule.
`bin/fm-classify-lib.sh` owns that evidence contract and its bound for both supervisors.
...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow. The herdr adapter carries its own fallback for the same busy-queue shape, built on native agent-state rather than the tmux path's rendered footer; see `docs/herdr-backend.md`.
...
That `pr=` recording exists only for teardown's landed-work check, so a task that teardown already cleaned up merges without it and records the confirmed merge in `data/<id>/merge.md` instead; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns how a torn-down task is told apart from an unknown one.
