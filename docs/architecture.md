...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then holds that unchanged `paused:` or durable `captain-held` endpoint on the same cadence; recovering the paused classification for a crew whose current state has fallen back to stopped or unknown needs the backend to confidently report its agent dead.
...
Once the normal-mode watcher flags an endpoint paused, its recheck window is measured from the status log and a re-surface throttle marker rather than the pane hash, so a parked pane that keeps redrawing rechecks on the same configured interval as a static one.
Only a real transition - the status no longer declaring the wait, the crew becoming provably working again, a proven busy worker, or away mode taking over triage - drops that tracking and returns the pane to ordinary stale escalation.
Fresh stale panes use the same current-state read before trusting the status log, so an active run or a proven busy worker outranks an old captain-relevant status-log line left behind before validation.
No-change heartbeats are also benign.
Absorbed wakes advance their suppression markers, log to `state/.watch-triage.log`, and keep the watcher blocking without a queue record or LLM turn.
A parked pane's repeated absorb line is rate-limited to one per pause window, so a long or churning wait cannot roll the rest of that log away.
