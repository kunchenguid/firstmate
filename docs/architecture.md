...
Task metadata identity is the single `pr=` URL matching the sidecar, so well-formed keys that later writers append are not part of the binding, while malformed content after `pr=` still refuses the poll.
