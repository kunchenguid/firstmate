...
A busy pane is otherwise exempt from staleness, but only the exact Claude lifecycle verdict `busy claude-hook` remains exempt after its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes.
Every other busy verdict, including the initial `busy fm-spawn` seed, is routed through the same wedge escalation at that bound, with the identical reason, escalation count, worktree-write deferral, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
When the home configures automatic quota drain, the same ordinary watcher cycle gives `bin/fm-auto-quota-drain.sh` exactly one `quota-axi --json` snapshot before later event checks or supervised agent invocations.
That owner is silent on healthy effective pools, persists warning and action episodes under `state/auto-quota-drain/`, and returns at most one bounded warning, completed action, or bad-data diagnostic for the existing watcher queue.
At the configured action-threshold crossing, its journal composes the existing marked secondmate reply, `/stow` completion receipt, graceful harness exit, backend park, routing cooldown authorization, telemetry intake, and `fm-spawn.sh --secondmate` owners in that order.
Each watcher invocation advances at most one durable lifecycle transition and never sleeps waiting for a receipt, exit, park, or relaunch, so later cycles keep touching the watcher heartbeat while `fm_pending_reply_tick` resolves the checkpoint.
The exact post-drain harness/model/effort comes only from durable `config/secondmate-harness`, whose provider/model-family relation is checked through the explicit home-local model catalog before planning.
Nonterminal journals resume ahead of current config or quota decisions and bind the original position, seat, home, reasoning class, candidate, and old endpoint, while a complete unoverridden durable tuple is the only launch that records `routing_source=secondmate-config`.
It adds no daemon, scheduler, alternate agent selector, alternate launch configuration, or alternate handoff path, and the session-start error-triggered secondmate recovery remains independent.
[`configuration.md`](configuration.md#automatic-quota-drain-configauto-quota-drainjson) owns the thresholds and per-position trigger/fact schema, while the script header owns exact state and lifecycle mechanics.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the executing checkout lags its local default branch, if the primary checkout is tangled, if work, process-event sources, or Relay polling has an unhealthy model-aware supervision verdict, or if queued wakes are waiting to be drained.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and writer scout: no worker starts until its clean task worktree matches the fetched tip of origin's resolved default branch, and any unsafe or unverifiable base stops the spawn.
Its header owns the exact refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` owns the portable regression coverage.

The firstmate repo has one extra exposure because it can dispatch crewmates to work on itself.
Its operating checkout (`FM_ROOT`) and the disposable writer task worktrees are all linked git worktrees of the same repository, so the valid discriminator is branch state, not whether the checkout is linked.
...
It also classifies an executing checkout as stale only when its `HEAD` is a strict ancestor of the local default-branch tip, leaving current, ahead, and diverged checkouts alone.
`fm-guard.sh` reports both full commit SHAs without advancing the running checkout.
...
The pipeline's required-check wait during PR validation is owned by no-mistakes' `internal/scm/github/github.go` `(*Host).GetChecks`; installed v1.53.0 invokes plain `gh pr checks` rather than `gh pr checks --required`, and its failure path discards stderr, while current upstream reads the head-SHA GraphQL status rollup plus same-head workflow runs and retains the plain CLI fallback.
...
`probe` (with no `--task`) already calls `reconcile` for every task that has a `state/<id>.meta` file, on the watcher's normal cadence, regardless of pause state, so a task's record already advances through `dispatched` -> `pr-registered` -> `merged` on its own as those artifacts appear; a fresh spawn's own immediate `reconcile` attempt (below) usually populates the record before the first probe cycle even runs, and a refusal there can persist past it, since spawn itself never retries.
The record surface is fully wired now: the schema, migration from the legacy cache, `reconcile`, `retire`, and the `board-json`/`steps` read surfaces, plus the existing probe cycle calling `reconcile` automatically (PR 209); an explicit best-effort `reconcile` call wired directly into `bin/fm-pr-check.sh`'s registration seam right after registration (PR 216); and explicit best-effort `reconcile`/`retire` calls wired into `bin/fm-spawn.sh` (both fresh spawn and relaunch, right after the task record publishes) and `bin/fm-teardown.sh` (both normal and backlog-close removal, before the metadata lock releases, skipped silently when the state directory is already gone) (PR 222).
Every one of these calls is best-effort and never changes the caller's own result: when `reconcile` or `retire` is refused (a bounded lock timeout, a malformed record), the caller prints a warning and proceeds and the old record is left as is.
For the pr-check and spawn `reconcile` calls, the task's `.meta` and `.status` files still exist afterward, so the next probe cycle retries automatically (a malformed record keeps refusing until it is repaired by hand). Teardown's `retire` call is different: by the time it runs, teardown has already removed the task's `.meta` file, so a refused retire is never retried by any later probe cycle (`probe_all` only visits tasks with a surviving `.meta` or `.status` file); `bin/fm-pipeline.sh retire <id>` by hand is the only path to clean up that record.
...
On an ordinary row, `since=` is seconds since the task's *current record step* began (the record's own `ts=`, read by `pipeline_since_from_ts`), not seconds since an observer first noticed the wait or since the task itself started waiting; a later pause in the same step does not reset it. Demonstrated by backdating a record's `ts=` by 3,661 seconds in a disposable directory: the next probe reports `since=` approximately 3,661. This is not a real age on every row: an `evidence=scan:activity-read-failed` row, a `scan:deadline` row, and a row whose record time is missing, invalid, or in the future all pass a fixed `since=0` fallback instead of a measured duration (demonstrated: the same 3,661-second-old record, probed again with only its status file made unreadable, reports `since=0`, not `3661`).
A keyed wait's resume closes with exactly one `probe=ok` row citing the resume line, never repeated by a later probe, but only once that occurrence has actually been observed while still paused (a probe cycle that reached it and cached it), with the record still on the same generation and the record clock still valid at closure time; a resume that changes generation refuses as foreign-gen, and an invalid record clock reports unknown instead of closing. A pause and its same-key resume that both land in the status file before any probe ever observed the pause produce no event at all: there is nothing cached to close. A resume is a status line carrying the *same key* as the pause (for example `working [key=vendor]: ...` after `paused [key=vendor]: ...`); an unkeyed line such as a plain `working: ...` does not close a keyed pause. Demonstrated: probing a task paused with `[key=vendor]` first (so the pause is cached) yields `probe=unknown`; appending `working [key=vendor]: vendor released, resuming` and probing again yields exactly one new row, `probe=ok` at the resume line; a third probe adds nothing further for that occurrence.
...
`bin/fm-graph-board.sh arm` registers the separate `graph-board` check, which captures `fm-pipeline.sh board-json` in a private state-directory temporary file and rebuilds the HTML quietly; `disarm` records its disabled marker before retiring the registration.
This is the file-refresh half of S4b, and production arming is an explicit one-time operator action rather than an automatic watcher-start or build/path effect.
Open-page refresh remains an explicit, still-owed M4 obligation: replacing the HTML file does not update an already-open browser tab.
...
Every crewmate and scout spawn also carries `--routing-source captain|profile|fallback`, recording which of those three authorities resolved the tuple, because the recovery escalation ladder (`bin/fm-harness.sh escalate`) reads it as its precedence guard and leaves routing untouched when it is absent.
A persistent-secondmate launch adds the separate `secondmate-config` value automatically only when a complete durable `config/secondmate-harness` tuple owns every launch axis and no explicit or positional tuple override was supplied; the telemetry validator accepts that value, and the ordinary escalation ladder still refuses secondmate records.
...
This repo keeps no-mistakes test evidence in a temporary directory; [`configuration.md`](configuration.md) owns that setting.
PR-based task merges go through [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh), which establishes the live pull request identity, records it through `bin/fm-pr-check.sh`, and only then calls the forge, so a merge whose outcome cannot be read afterwards still leaves teardown a PR to verify against while a request the forge does not confirm records nothing.
The architectural commitment is that a merge is only ever performed on a head this run verified, and that nothing the forge cannot prove landed is reported as landed.
That script's header is the single owner of what follows from it: accepted URLs, the caller flags each forge allows, the live pre-merge conditions, the review evidence a Firstmate merge requires, the captain-authorized escapes and their receipts, and every refusal's exact wording.
...
[`bin/fm-pr-lib.sh`](../bin/fm-pr-lib.sh) owns the per-task metadata lock those receipt writes share with every other metadata writer, so one task's read-stage-publish transactions serialize instead of overwriting each other.
Teardown is fail-closed for ship worktrees: committed work must be landed before the worktree is returned, and a dirty worktree refuses unless [`bin/fm-worktree-unique-content.sh`](../bin/fm-worktree-unique-content.sh) proves every differing path holds only content already reachable from refs that survive teardown.
That distinction exists because dirtiness is a proxy that inverts in one supported situation: a task branch's ref being rewritten in place beneath a live worktree - the no-mistakes pipeline returns a rebased branch exactly this way, as a bare ref write with no checkout - leaves the index and working tree at the pre-rewrite state, so `git status` presents landed work as deletions to preserve.
...
A sealed terminal also names why no token usage was captured, instead of leaving one null that reads alike whether the harness exposes no verified usage log, no session matched the attempt, or the matched session reported no tokens.
