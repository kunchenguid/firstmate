...
PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh --register-only` before calling the forge CLI.
The helper requires a full canonical URL and rejects malformed URLs or repo override flags before recording merge state.
A `https://github.com/<owner>/<repo>/pull/<n>` URL requires `gh` and `jq`, is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, and every unwaived check is green at the current head, then a fresh `bin/fm-pr-review.sh verify` binds complete feedback triage to that same head before `gh pr merge --match-head-commit`.
A check run is green when its current run is green, because GitHub leaves a cancelled run in the rollup beside the passing re-run it triggered when the base branch advanced; `bin/fm-pr-lib.sh`'s `fm_pr_github_checks_not_green` owns the shared rule, which uses `startedAt` to clear only an older completed check run that a passing run with the same name provably replaced, while unfinished check runs and non-green status contexts stay red.
...
An attended `--allow-red <check-name>` may appear once, waives only GitHub checks with that exact name, never waives feedback coverage or freshness, and is refused while the away-posture record exists.
