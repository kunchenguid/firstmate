...
At session start, confirmed-dead secondmate agent endpoints are closed and relaunched through the same secondmate spawn path only after local endpoint cleanup is confirmed, while ambiguous liveness reads are left untouched to avoid duplicate supervisors.
