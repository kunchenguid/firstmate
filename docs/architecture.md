...
Its later sights are still held to that same bounded cadence rather than re-alarming on every pane-hash change or busy blip, because the throttle is keyed to the declaration and not to the pane a parked worker keeps ticking and briefly renders busy while its declared wait stands.
A provably-working verdict does not retire it either, because that verdict is read from a capture taken a moment after the staleness loop read the same pane idle, so it can catch a parked worker mid-blip rather than prove the wait ended; the declaration is retired by the status line that ends it, and the wedge timer still bounds a run that really is frozen.
