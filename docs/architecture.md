...
`bin/fm-fleet-dashboard.sh` is a third consumer of the same snapshot: it renders the Lavish dashboard HTML, its Markdown twin, and the `data/dashboard.md` read pointer, and `bin/fm-fleet-dashboard-refresh.sh` regenerates and opens it.
That dashboard is a read surface only, never a decision input.
