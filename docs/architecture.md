...
`bin/fm-dod-lib.sh` is the one owner of that mode's definition of done. `bin/fm-spawn.sh` strips any authored or legacy definition of done from the source brief and renders the current one into every ship launch brief, so a hand-written or legacy source cannot bypass it; `bin/fm-promote.sh` renders the same block into the ship instructions a promoted scout receives and into that scout's own `brief.md` so a later relaunch reads the same contract, and a promoted worker cannot be handed a weaker contract than a briefed one.
