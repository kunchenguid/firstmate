...
That confident `dead` verdict is weighed BEFORE the run-based working proof, and settles the classification on its own.
A dead agent cannot answer a pipeline's gates or act on its result - the run is a background daemon run and may still be executing, whoever owns it - so the crew's own declaration is the better evidence about the crew, and the stale path then absorbs it on the declared-wait cadence instead of escalating an empty pane as a possible wedge indefinitely.
Live or inconclusive liveness remains fail-open at that initial surface and still consults the working proof first, so a worker genuinely waiting on a decision is never silenced and a live crew's declaration never silences its own running pipeline.
...
That override is offered only while the backend does not confidently report the crew's agent dead: a dead agent cannot answer the run's gates or act on its result, whether the run credited to it by branch is its own or a co-branch sibling's, so its terminal-looking pane is surfaced once as the genuinely terminal stale it is and never absorbed or wedge-escalated, while a live or ambiguous agent keeps the override.
...
Branch and head identity cannot separate concurrent crews whose worktrees sit on ONE long-lived feature branch - the shape a one-PR-per-repo posture produces - because no-mistakes exposes nothing that names the worktree a run was invoked from, so two such crews both satisfy the head rule for whichever run is current.
Two guards settle ownership on each attribution route: the run binding `nm_run=<run-id>`, which the crew's own no-mistakes contract records through [`bin/fm-run-bind.sh`](../bin/fm-run-bind.sh) as soon as its run reports an id, and the delivery-mode gate, since a `direct-PR` or `local-only` crew never drives a pipeline of its own.
The ownership rule itself lives in [`bin/fm-nm-run-lib.sh`](../bin/fm-nm-run-lib.sh) and is shared with `bin/fm-teardown.sh`, whose pre-removal abort of a parked run applies the same rule, names the owning task when it declines, and never aborts a sibling's run.
On the `axi status` route a bound crew is credited only its own run id, and when the repo's current run is another id its own run is fetched by id instead; an unbound crew is credited only a run no other task has bound.
That by-id fetch is not the end of the story: a binding whose run has reached a terminal outcome is DECLINED when the repo's current run is a different id on the same branch, and the crew then falls through to the unbound branch guess, so a stale binding degrades to a guess rather than to a confident wrong answer and a missed re-bind is self-healing.
The decline is scoped to terminal bindings only - an active bound run always keeps precedence over whatever the repo happens to be reporting - and both consumers apply it at their own by-id fetch, so a crew that restarted validation without re-binding is neither reported from its own finished row nor left with that live run orphaned when teardown removes its worktree.
On the coarse runs-ledger route, which names no run id, an unbound crew is withheld branch-level credit entirely while another task binds a run and that task's recorded worktree currently sits on the branch; the sibling's branch is one local git ref read per bound record, never a no-mistakes CLI call, because this read runs on the watcher's per-poll path for every task, and a same-branch answer another task has bound settles the question by itself.
That withhold also applies when the sibling's binding is stale, since the ledger cannot tell the branch's current row from a terminal run; the accepted limit of the local read is that a bound sibling whose worktree has since detached, moved, or been torn down no longer withholds on that route, though it still owns the run it bound by id.
A bound crew is never handed a ledger row on the strength of its binding: it enters the ledger only to confirm that its own same-branch run, whose head the task copy cannot resolve, is the branch's current row by that run's head, so a stale own run is never reported as current on a sibling's evidence; a branch carrying no binding at all keeps the existing branch-based behaviour so no existing home regresses.
