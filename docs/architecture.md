...
Runtime auto-detection is innermost-first, so the runtime layer actually executing wins over any outer runtime whose markers leaked into its environment; [`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns the exact marker order, which backends are auto-detected at all, and which of them print an opt-out notice.
...
paseo is a registered experimental backend name with no adapter under `bin/backends/` yet: like Zellij and Orca it is never auto-detected, and it is absent from the spawn-capable set, so every paseo spawn refuses at `fm_backend_validate_spawn` and every `backend=paseo` cleanup record refuses at `fm_backend_validate_task_endpoint` until that adapter lands and a `docs/paseo-backend.md` owns it; [`verification/runtime-backends.md`](verification/runtime-backends.md#paseo) owns the dated marker-leak evidence behind that explicit-only choice in the meantime.
