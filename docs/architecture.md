...
Verified harness templates are wrapped in `bash -c` before being sent into the runtime endpoint, so firstmate does not depend on the pane's interactive shell understanding bash-only template syntax.
Raw launch commands remain raw for adapter-verification work.
