...
The mate's own `wake()` transition uses bounded queue and progress-lock acquisition, publishes a short-lived in-flight record before its bounded output, commits the monotonic transition timestamp and authoritative queue-sequence watermark after successful delivery, and falls back to delivery without optimistic progress when serialization is unavailable, so clock changes cannot override causal order, active publication is retried only within a tight live-owner window, and an absent, unreadable, malformed, future-dated, abandoned, or lower-watermark marker never suppresses an abandoned queue while the primary only reads the foreign marker and queue.
