...
A terminal failed run-step from an obsolete run defers to the status log's last declared paused or blocked line only once a branch-scoped structural check confirms that branch's custody was returned, with no run-id binding and no timestamp comparison against the run's own completion; the script header owns the exact precedence and custody-check rule.
