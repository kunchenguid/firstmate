...
A check is green when its current run is green, because GitHub leaves a cancelled run in the rollup beside the passing re-run it triggered when the base branch advanced; `bin/fm-pr-merge.sh`'s `github_checks_not_green` owns the rule, which clears a failure only when a later run of the same check provably replaced it and never clears a check whose current run failed or is pending.
