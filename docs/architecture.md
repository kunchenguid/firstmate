...
A housekeeping capture that still fails after bounded retries asks the backend's recovery-grade agent state: only an authoritatively missing endpoint is dropped without escalation, so a torn-down pane stays quiet while a present or unreadable pane is surfaced and kept on the same cadence.
