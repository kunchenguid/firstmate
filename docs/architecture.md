...
That library, `fm-spawn.sh`'s header, and `tests/fm-spawn-pool-base-freshen.test.sh` own the mechanics and the regression coverage, while [`configuration.md`](configuration.md#project-local-environment-file-envlocal) owns what a captain sets for it.
