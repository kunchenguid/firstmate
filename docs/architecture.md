...
For non-orca spawns, `fm-spawn.sh`'s worktree-discovery poll adds one more guard: a freshly created pane can transiently report its parent `FM_ROOT` before the shell finishes cd'ing into the project, so `FM_ROOT` is rejected as a destination unconditionally alongside the project path itself, with no ordering/timing gate between the two checks (a departure to `FM_ROOT` would tangle the primary, which the distinct-worktree-root check cannot catch since `FM_ROOT` is itself a valid worktree root).
