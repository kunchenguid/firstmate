...
Only when no matching run exists does it consult semantic busy state; exact busy reports working, exact idle permits fallback to a status-log event whose verb maps to a recognized run-state, and unknown or a dead pane stays unknown instead of trusting a stale log, except that a live Herdr registration with `agent_status=working` supplies a backend current-state fallback when the harness-specific source is missing or unverified.
...
The Herdr agent-status fallback reports only live `working` activity, names `herdr-agent-status` as its source, and never maps `done` or another Herdr status to a durable crew outcome.
