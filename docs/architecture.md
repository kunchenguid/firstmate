...
The optional delegated Pi profile is a fail-closed exception to those generic dispatch rules; [configuration.md](configuration.md#delegated-pi-profile-configpi-delegated-profile) owns its compatibility impact.
...
An explicit per-spawn harness, or a raw launch command when no delegated Pi profile is active, does not inherit model or effort tokens from `config/secondmate-harness`.
