...
omp's replacement follows the same generation-owner contract in `.omp/extensions/fm-primary-omp-watch.ts`, whose header owns the differences: omp reports no shutdown reason, so every shutdown with a pending actionable close persists the handoff for the next owning `session_start` to replay, and one accepted but unconsumed follow-up covers every later durable actionable until that follow-up is consumed.
Covered handoff entries retire without another follow-up, while a close after consumption creates a new one.
