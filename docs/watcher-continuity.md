...
The arm forks the confirmed watcher into its own process group (`set -m` around the fork) while keeping it a normal waitable child, so `wait` and the confirm-poll still see its real pid and exit status.
A graceful, catchable HUP/TERM/INT to the arm still explicitly tears that child down through the arm's trap handlers, so intentional stops, `--restart`, and the singleton-lock discipline are unchanged; only an uncatchable, group-wide kill of the arm (for example a harness memory guard misfiring on a transient reading) no longer takes an already-confirmed watcher down with it, so fleet supervision survives that case.
...
It also covers the process-group survivability contract above: a real arm launched as the leader of its own fresh process group and then SIGKILLed by process-group id, exactly like the misfiring guard, must leave the detached watcher alive and still beating its beacon, while a spawn that never becomes healthy is cleaned up without leaking a running process.
