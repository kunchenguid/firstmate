...
A numeric session-lock owner this home can disprove through the shared `fm_session_lock_live_owner` predicate is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a verified live owner, absent lock, or malformed lock keeps the competing hook inert.
That predicate requires an owner record written by an acquire in this home and pinned to the exact process, so a recycled pid cannot hold supervision inert; `bin/fm-session-lock-lib.sh` owns the record and its verification.
A live harness pid this home holds no record for at all is missing evidence rather than proof, so it is refused and left alone instead of reclaimed - that is a still-running pre-upgrade session's shape, and it repairs its own record the next time it touches the lock.
