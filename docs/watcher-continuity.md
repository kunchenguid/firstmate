...
The tracked Claude-shaped `UserPromptSubmit` entry is deliberately left unguarded on Grok and Cursor, because neither host registers a counterpart for that event, so a primary on those hosts that loads this repository's Claude-shaped settings also gets the turn-start presenter, while primaries whose hosts never fire that event present voice notes through the drain at session start and at wake-handling turns.
