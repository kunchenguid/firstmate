...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, atomic watcher-lock publication, reclaim of a pid-only dead lock in this home, the live-pid refusal, and the other-home refusal that holds only while the beacon is fresh.
