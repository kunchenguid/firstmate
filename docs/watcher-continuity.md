...
Once those gates pass, a recovery-claim failure makes a bounded identity-independent attempt to serialize an immutable record and episode marker, then publishes both against the exact pre-attempt claim-ledger snapshot without that boundary when the boundary itself is unavailable so the synchronous guard can consume the failure without a stale automatic retry.
