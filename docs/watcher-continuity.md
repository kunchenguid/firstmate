...
The arm layer recovers an ordinary poll-blocked wedged watcher instead of refusing it.
...
When `arm` finds THIS home's own watcher live but its beacon stale past grace, it sends that watcher a home-scoped SIGTERM and arms a fresh cycle after it exits, so the Claude Stop-hook auto-arm self-heals the normal poll-blocked wedge at the next turn boundary rather than only reporting the failure.
...
`stop_this_home_watcher`, shared with `--restart`, resolves and signals only the pid recorded in this home's lock, and only after `fm_watcher_lock_matches_pid` proves it is this home's own watcher.
Ordinary `arm` never escalates beyond SIGTERM: after `FM_WATCH_EVICT_TERM_GRACE` seconds, a watcher that remains identity-matched and beacon-stale while deferring its TERM trap produces a typed failure for the Claude hook's attended failure path.
Only explicit `--restart` may escalate that stale holder to SIGKILL, because the operator requested replacement; it revalidates the recorded lock and process identity first and never uses a name-matching kill.
Arm-mode recovery revalidates identity and beacon staleness immediately before SIGTERM, so a watcher that recovered in the race is attached to instead and a healthy watcher is never evicted by automatic recovery.
The watcher's own terminal poll wait uses an interruptible sleep so a poll-blocked watcher honors SIGTERM at once; a genuine foreground wedge stops safely pending explicit restart authorization.
...
It also covers wedged-watcher recovery against real processes: `arm` evicting a poll-blocked wedged watcher via SIGTERM and re-arming, refusing destructive escalation for a frozen (SIGTERM-deferring) watcher until explicit `--restart`, rechecking a recovered watcher before automatic SIGTERM while explicit `--restart` still stops it, refusing SIGKILL after lock identity changes, attaching to a healthy watcher without automatic eviction, and a poll-blocked watcher honoring SIGTERM promptly through the interruptible sleep.
