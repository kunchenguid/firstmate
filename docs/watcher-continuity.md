...
Because that exclusion makes those rows invisible to main, `bin/fm-guard.sh`'s queued-wake warning counts only the rows the calling actor can itself present or retire (`fm_wake_actor_pending_count`, whose implementation `bin/fm-wake-lib.sh` owns alongside the grant row-list and owner-record reads that the drain and `bin/fm-wake-grant.sh` share): a row a live grant reserves warns its own owner and nobody else, so main is never sent to a drain that can only print nothing.
...
The same suite pins the counted-equals-presentable invariant against `bin/fm-guard.sh` and `bin/fm-wake-drain.sh` together: a branch-held row warns only its owner and is presented with its acknowledgement command as soon as the grant clears, and structurally unusable rows are retired by main alone while every remaining row stays presentable and acknowledgeable.
