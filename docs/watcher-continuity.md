...
Its latch-release regression proves `session.deleted` or `session.error` for the outstanding session reopens delivery while foreign session events leave the latch intact.
