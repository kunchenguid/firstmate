...
If a Pi recovery successor emits another actionable close before confirmation finishes, Pi folds that close into the original main-owned wake and starts one replacement only after its handling turn settles.
This prevents a recovery resurface from flooding the follow-up queue without leaving supervision stopped.
...
The same suite reproduces a recovery successor that emits `check: rearm-resurface` during confirmation, proves that Pi keeps one main follow-up, and proves that handling settlement starts exactly one replacement.
