...
`tests/fm-watch-beacon.test.sh` drives a real watcher with SIGPIPE ignored against a backend whose every call fails and one whose every call hangs, and covers the beacon still advancing while the cycle stays alive and silent in both, an unwritable beacon ending the cycle rather than leaving a live lock-holder with a frozen beat, and the fleet-scaling shared loops beating once per pending-reply record and per triaged signal task through `FM_LIVENESS_BEAT_HOOK` while an unset hook stays a no-op.
