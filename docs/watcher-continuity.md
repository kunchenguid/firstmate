...
A cycle with no matching delivery record needs repair rather than an alarm, so the attached arm re-enters arm mode in place and takes the singleton over, keeping its own pid so the harness still tracks and notifies the same task.
`FM_ARM_TAKEOVER_MAX` bounds those attempts, and only once they are spent does the arm emit `watcher: FAILED - cycle ended without an actionable reason` and exit nonzero.
This matters most under Claude, whose successor arm starts only at the next Stop: without the delivery ledger and the takeover every attached arm would report that failure while the fleet was healthy.
...
The same suite runs two concurrent arms against one real wake to prove the owning arm returns the reason while the attached arm closes the delivered cycle cleanly with no false failure, proves an attached arm takes the singleton over when the cycle it follows dies undelivered, and pins `FM_ARM_TAKEOVER_MAX=0` on the successor-gap cases to keep the loud typed failure covered.
