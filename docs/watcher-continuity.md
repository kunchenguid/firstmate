...
The hook fires on every Stop except when `FM_TASK_ID` marks the pane a task worker, and an eligible primary with supervision need admits one home-scoped owner that foregrounds `bin/fm-watch-arm.sh` inside the hook-owned process tree.
