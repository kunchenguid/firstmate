...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, a watcher signalled while parked on both the singleton and wake-queue locks handing back every lock it holds (the singleton only through the recovery marker), the record-before-visible ordering invariant behind that handback, and `fm_lock_acquire_wait` taking over a dead holder's lock while never taking one a live holder still holds.
