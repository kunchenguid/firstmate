...
A numeric session-lock owner that fails the shared, PID-reuse-hardened `fm_session_lock_pid_verified_alive` predicate (`bin/fm-session-lock-lib.sh`) is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a live, identity-verified owner, absent lock, or malformed lock keeps the competing hook inert.
`fm-turnend-guard-cursor.sh`'s park uses the identical predicate before its own reclaim delegation to `bin/fm-lock.sh`.
That predicate layers process-start identity evidence (`fm_pid_identity`, `bin/fm-wake-lib.sh`) on top of the plain `fm_harness_pid_alive` liveness check, recorded in a `state/.lock-identity` sidecar `bin/fm-lock.sh` writes alongside every `state/.lock` acquisition: a live pid whose recorded identity no longer matches is proven to be an unrelated process that inherited a dead session's number, and is reclaimed exactly like a dead pid, while a live pid whose identity still matches - or one with no identity evidence at all - is never touched. The sidecar also carries a unique per-acquisition `token`, minted only by a genuine new acquisition and never by a renewal of the same ownership, and a `heartbeat_at` lease that the owning session's own routine per-turn touchpoints (Claude Stop, Cursor park) renew in place; one owner per contract still holds, but a live, identity-matched owner whose lease has expired (`FM_SESSION_LOCK_LEASE_GRACE`, default far looser than `FM_GUARD_GRACE` to tolerate a primary's legitimately unbounded idle gaps between turns) is now also reclaimable, covering the rarer hung-but-alive-forever failure mode that dead/reused-pid identity alone cannot. `bin/fm-session-lock-lib.sh`'s own header owns the exact sidecar format and grace rationale.
Before that hardening (2026-09-09), a dead session's harness pid reused by any other live harness process anywhere on the host was indistinguishable from the genuine owner to the bare liveness check, so the async auto-arm hook and the Cursor park both refused recovery indefinitely, and the synchronous turn-end guard blocked every subsequent Stop because neither participant ever advanced past that gate to arm a watcher or claim a fresh autoarm-epoch generation - `bin/fm-session-lock-lib.sh`'s own header owns the sidecar's exact format and the missing-evidence fallback contract.
