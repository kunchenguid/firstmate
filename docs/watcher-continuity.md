...
`tests/fm-watcher-lock.test.sh` also covers `bin/fm-wake-lib.sh`'s Git Bash/MSYS copied-directory publication adoption, including simultaneous single-winner acquisition, refusal to reclaim a stale primary lock while a live steal mutex is held, clean losing contenders, stale recovery of pre-fix generated nested owner directories, refusal of foreign nested content, release, and MINGW or Cygwin staying on symlink publication when `ln -s` really links.
