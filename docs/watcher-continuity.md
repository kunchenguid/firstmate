...
Once those gates pass, a recovery-claim failure atomically creates an immutable record keyed by the exact pre-attempt claim-ledger snapshot and an atomically elected episode marker instead of contending on or overwriting the claim ledger, so stale publishers cannot erase a current failure and a later claim-ledger transition supersedes older records.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, Claude daemon `--spawned-by` ownership bridging, unchanged AFK and need boundaries, independent failed-claim epochs, atomic notice election, bounded legacy pre-publication, prior-terminal isolation, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
