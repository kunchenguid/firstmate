...
While `state/.afk` exists, the `/afk` contract makes the daemon the single injection owner; Pi's continuity callback preserves successor verification and bounded retry but returns before `pi.sendUserMessage`.
...
It also drives both AFK-present and AFK-absent modes against the same durable wake, proving the successor arm and durable queue behave identically while only AFK-present suppresses the typed wake delivery.
