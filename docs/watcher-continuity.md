...
A session lock that names no live harness owner, meaning a numeric owner failing the shared `fm_harness_pid_alive` predicate, an absent lock, or a malformed lock, is claimed through `bin/fm-lock.sh` before auto-arm state changes, while a live owner the session does not own keeps the competing hook inert.
The absent and malformed cases must be claimed rather than left inert, because the turn-end guard blocks exactly that home and the hook is the only Claude recovery owner.
