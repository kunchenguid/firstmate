...
Only the first of those is reclaimed silently, because a holder that no longer exists is the ordinary path every session start takes; the second reclaims a process that was still present when the predicate classified it and is therefore announced, and it arises when a lock an older firstmate wrote names a still-live session whose install shape the current identity rules do not identify, which [`verification/runtime-backends.md`](verification/runtime-backends.md#session-lock-identity-and-the-suspended-holder) owns as a bounded adoption-window limit.
