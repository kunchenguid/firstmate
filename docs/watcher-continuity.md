...
An attached arm follows verified identity-matched successors; when that chain ends with no successor, it first recovers the ended cycle's actual reason from the durable wake queue (the cycle it followed has no redirected child output of its own to read) and reports that reason as an ordinary wake, reporting the typed failure only when no such reason exists (firstmate/863).
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and an attached arm recovering a genuinely delivered wake reason from the durable queue (a peek, not a consume) instead of reporting the typed failure.
