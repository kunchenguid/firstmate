...
An acknowledgement carries two separable facts: queue-row consumption is bound to a monotonic through-sequence (further scoped per actor - see "Per-actor acknowledgement" below), while only retiring the episode is bound to a recovery generation.
Both, plus the actor they were presented to, travel inside the one opaque receipt the drain prints and `--ack` consumes; `bin/fm-wake-lib.sh`'s "acknowledgement receipts" section owns how they are packed and why, and the deprecated `--ack-through <SEQ> --recovery-generation <GEN>` pair spells out the same two facts for the deprecation window.
...
A main acknowledgement deletes only claimed main rows at or below its receipt's cutoff, while a branch acknowledgement deletes only claimed branch rows at or below its own.
...
An acknowledgement whose cutoff removes none of the actor's rows while a presented row above the cutoff still waits is reported as having acknowledged nothing, together with the exact `--ack` command carrying the receipt for that presented row; the presented set is read before any re-claim, so a row that arrived after presentation is never named for unseen acknowledgement.
A receipt that is not one this build issued, or whose actor is not the acknowledging actor, is refused outright and consumes nothing, so a mistyped or borrowed token can never read as a successful acknowledgement.
