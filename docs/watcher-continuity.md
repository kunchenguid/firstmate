...
When that Stop would otherwise refuse, the guard primes `bin/fm-claude-stop-autoarm.sh --ensure-watcher` immediately before the refusal, which detaches the same `bin/fm-watch-arm.sh` owner every other arm path uses as a handling successor, so aborting the registered `asyncRewake` hook cannot starve the watcher back into existence, a primed cycle does not re-announce downtime into a one-poll resurface, and a primed cycle that fails still leaves its lifecycle record ([`turnend-guard.md`](turnend-guard.md) owns that priming contract).
Ordinary Claude Stops that the wait allows do not prime, so the registered `asyncRewake` cycle remains a non-successor and still re-presents an unacknowledged downtime episode once.
...
It also covers generation-claim single-flight, stuck-claim supersession, superseded-owner silence, notice-marker refusal and retry, ownership-atomic episode reset, the legacy upgrade shim, and `--ensure-watcher` priming that detaches the arm owner without taking a generation claim, reports by exit status whether it forked, carries the hook's resolved `FM_GUARD_GRACE` into that fork, and leaves a lifecycle record even when a wedged lock holder refuses the primed cycle; [`turnend-guard.md`](turnend-guard.md) owns those behavior contracts.
