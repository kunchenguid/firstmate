...
A single-flight claim left behind by an owner killed mid-arm is stale and reclaimed at a later firing even when an unrelated process has since reused its recorded pid: only a claimant still running the hook (or the guard's terminal check) counts as the live owner, and any uncertainty stands the firing down instead.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight with stale-claim reclaim (dead owner and reused pid) and a live hook-owner stand-down control, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
