...
It also covers `--restart` stale-lock recovery over real processes: a reused-pid lock recorded under an equivalent home spelling is cleared, while a distinct home's lock and a lock whose recorded watcher script is momentarily absent are both left held and unsignalled.
...
It also drives `bin/fm-guard.sh` against real watcher processes to prove the recorded home and watcher script are accepted by filesystem identity independently of each other, with distinct-home, alias-repointed-script, and missing-target negatives; it drives the guard from separate working directories to prove a recorded relative spelling of either one resolves in the checking process's cwd, accepted where it names the real target and down where it names another real object; the two case-dependent cases (a mixed-case spelling accepted, and case-distinct real objects rejected) each skip explicitly on the filesystem that cannot stage them, and [`turnend-guard.md`](turnend-guard.md#guard-predicates) owns that equivalence contract.
