...
Coalescing never costs a wake: a consumed row re-arms presentation, so a genuinely later wake still presents one new row, and a replacement session presents fresh.
...
The pending-row latch lifecycle and the `watcher: FAILED` urgency marker every failure text must carry follow the presentation-coalescing contract in `.pi/extensions/fm-primary-pi-watch.ts`.
...
The same suite covers ordinary wake presentation coalescing against the real extension through a production-timing follow-up fake: a four-event burst presents one docked row while the durable queue keeps every event and every close completes its handshake, an idle-consumed wake leaves presentation re-armed so a genuinely later wake presents exactly one new row, continuity-restoration exhaustion and restoration-time lock loss each present their own row while an ordinary row is pending, retry-path lock loss and a refused handling handshake do the same, a rejected ordinary delivery rolls the latch back so the next wake still presents, and session replacement discards the latch with its generation.
