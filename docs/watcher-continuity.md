...
Hermes's primary protocol uses one managed-terminal cycle with `background=true` and `notify_on_complete=true`; its completion re-enters the persistent CLI and directs exactly one successor, while the tracked plugin blocks unsafe arm shapes and catches a blind turn end. Shell `&` is not used.
