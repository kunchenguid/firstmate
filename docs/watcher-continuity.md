...
Once those gates pass, a recovery-claim failure atomically creates an immutable record keyed by the exact pre-attempt claim-ledger snapshot and an atomically elected episode marker instead of contending on or overwriting the claim ledger, while an independent transition boundary orders that publication against claim-ledger transitions so stale publishers cannot erase or announce over a later success.
