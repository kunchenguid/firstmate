...
`bin/fm-lock.sh` itself refuses a lock whose bytes cannot be read at all, and a readable record that does not parse but whose first line still names a live harness process outside this session's ancestry - that is a genuinely competing owner and keeps the existing refusal wording. Only a readable record that names no such owner is unowned, and acquisition reclaims it under the acquisition lock exactly like a dead owner rather than leaving the home permanently read-only; a session whose own record was mangled proves ownership through ancestry and reclaims it.
