...
Readiness is not a lifetime health claim: the arm keeps verifying its owned watcher's lock identity and beacon while waiting, and loss of health produces a typed failure for the harness continuity owner after bounded retirement of that child.
The arm never refreshes the beacon on the watcher's behalf or signals an unrelated holder; `bin/fm-watch-arm.sh` owns the health-check cadence and retirement mechanics.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, and a five-task turn-end reproduction that distinguishes a live PID from a stale beacon and requires the owned arm to detect and retire it without manual intervention.
