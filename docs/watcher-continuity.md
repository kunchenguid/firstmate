...
When Claude delivers the Stop hook from its daemon/background-spare tree, the hook may use the daemon's `--spawned-by` metadata as session-lock proof only if it names the live foreground Claude owner and the same resolved project root.
...
Once those gates pass, a recovery-claim failure records a failed auto-arm epoch and the episode marker instead of leaving the next reader with no claim and no failure record.
