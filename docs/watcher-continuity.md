...
Pi starts that liveness restoration per close independently of its serialized wake-delivery path, so a successor that closes actionably while an earlier branch settlement remains pending gets its own verified successor immediately without starting a parallel arm chain.
Once a Pi arm has queued actionable output, dependent delivery treats that arm as superseded even before its close callback runs: it waits for that exact close and joins the token-bound post-close restoration rather than accepting the still-open arm as its own successor.
...
At Pi's pre-delivery checkpoint, it also revalidates the exact successor arm and its watcher through the shared home, path, PID-identity, and fresh-beacon health boundary; a successor that fails that check sends the wake to main with a typed failure instead of being offered to the branch, and any associated failure close retains its ordinary bounded retry after the serialized delivery settles.
It waits at most one readiness timeout per attempt, then sends TERM and waits a bounded retirement confirmation before the next lock-verified exponential retry.
If the unready arm does not retire within that bound, the adapter keeps ownership, does not spend another readiness timeout on that child, starts no overlapping retry, and delivers the typed fallback immediately.
When that retained arm later closes, its actual close is classified as a new supervised event without replaying the earlier fallback.
After the configured retry bound is exhausted, it delivers the original wake with a typed continuity-restoration failure even if every successor arm hung without reporting readiness.
A later successful owned repair supersedes that failed restoration for any delivery still serialized behind an earlier branch settlement, so the pending wake joins the repaired child's readiness instead of surfacing a stale failure.
...
Pi's arm tool distinguishes a newly spawned arm whose readiness is still pending from an already-owned child whose watcher is verified ready.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi's first-cycle-or-explicit-repair tool metadata and readiness-qualified redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers, blocks prompt delivery to prove the successor launches first, closes two successors while the earlier branch settlement remains pending to prove the latest successor starts immediately and all three wakes deliver once in either settlement order, and proves queued pre-close output cannot make an arm its own successor.
The same suite proves a successful manual repair supersedes a failed cached restoration, rechecks an exact successor's watcher against dead-PID, stale-beacon, and PID-identity-mismatch counterfactuals before branch delivery, verifies single-flight behavior, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure without spending a second readiness budget on a retained child.
