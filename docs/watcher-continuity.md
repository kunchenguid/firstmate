...
The owner record also carries the reserved sequence numbers as its durable list, written with every publication and cleared by every release, and activation is refused while a live different-pid owner holds the grant, so a descendant of the primary that walks to the same lock-holder verdict can neither take the grant over nor strand it by exiting.
Any actor's drain or acknowledgement rebuilds a lost or corrupt `state/.branch-eligible-rows` from that record's list under the queue lock before presenting, acknowledging, claiming, or reclaiming, keeping a snapshot lost to descendant-process churn from wedging the queue and from letting a main drain claim a live grant's rows.
...
A missing or empty branch snapshot that no live owner record rebuilds is refused loudly rather than read as "nothing eligible", because reaching the drain without the non-empty handoff promised by the extension is a wiring bug.
...
The same suite drives a full accepted branch wake cycle with the eligible snapshot lost between publication and acknowledgement: the drain and the acknowledgement rebuild it from the live owner record while the restored reservation still fences main out of the granted rows, grant activation is refused from a live different pid while the same pid may reactivate and a dead owner stays replaceable, a released grant cannot be resurrected from its cleared record, and main's own acknowledgement still completes when its presented claim is lost or corrupt.
