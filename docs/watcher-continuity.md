...
`tests/fm-watcher-lock.test.sh` also covers `bin/fm-wake-lib.sh`'s Git Bash/MSYS directory-lock publication, including simultaneous single-winner acquisition, refusal to reclaim a stale primary lock while a live steal mutex is held, clean losing contenders, stale recovery of pre-fix generated nested owner directories, refusal of foreign nested content, release, and Cygwin staying on symlink publication.
