...
The shared supervision loops whose wall time scales with the fleet - the pending-reply tick and the provably-working triage of a signal batch - reach the same beat through `FM_LIVENESS_BEAT_HOOK`, which the watcher points at its own beat function; the hook runs in the watcher's own process, so no helper process gains the ability to freshen the beacon.
That hook is a command word plus arguments, invoked directly and never evaluated as shell source text, so the many scripts that source the shared classifier for its pure status reads do not start running environment-derived strings as shell code.
...
`tests/fm-watch-beacon.test.sh` drives a real watcher with SIGPIPE ignored against a backend whose every call fails and one whose every call hangs, and covers the beacon still advancing while the cycle stays alive and silent in both, an unwritable beacon ending the cycle rather than leaving a live lock-holder with a frozen beat, and the fleet-scaling shared loops beating once per pending-reply record and per triaged signal task through `FM_LIVENESS_BEAT_HOOK` while an unset hook stays a no-op and a hook carrying shell metacharacters is word-split into arguments rather than evaluated.
