...
A successor emits `check: rearm-resurface` for an accepted down stretch only when the recovery presentation actually holds something: an unacknowledged queue row, a still-open decision, or an unread informational status line, and otherwise retires the episode silently.
Proving that none of them holds anything always means no wake, whatever the recovery marker says, so a recovery pending through the watcher's own stale-lock reclaim and one already moved to handling are as silent as a published downtime episode.
...
It pins the same two halves for the reclaim path, where the watcher takes over a dead predecessor's lock itself instead of being handed one an arm already cleared: that reclaim stays live and silent over an empty fleet and still resurfaces a real open decision.
