...
It also drives the OpenCode plugin's primary scope: arming in a marked linked secondmate home and in an unmarked linked-worktree home that owns its lock, one not-primary failure per session for another refused Firstmate-shaped root, and silence in a task worktree, a read-only second session, and a crewmate whose `FM_HOME` names its parent home.
