...
`tests/fm-wake-queue.test.sh` also covers `bin/fm-wake-lib.sh`'s Git Bash/MSYS copied-owner compatibility, including ownership-fenced adoption of a copied lock directory and cleanup of raced nested owner artifacts under both directory and symlink lock holders.
