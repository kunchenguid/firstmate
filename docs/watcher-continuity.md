...
Deduplication applies to the full notice, never to the block itself: exit 2 blocks the turn and the collected stderr is the operator's only view of why, so every blocking firing of both Stop hooks names what refused, why the turn is held, and the epoch ledger to read.
A blocking path that printed nothing was undiagnosable by construction, and the arm's typed lines are not the only possible cause, so a refusal that reaches the hook without those prefixes is still reported from the arm's plain output.
