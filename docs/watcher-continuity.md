...
Each adapter checks current session-lock ownership at launch, preserves one child or scheduled retry at a time, and applies bounded exponential retry after an unexpected or failed close.
Pi starts the next arm before delivering an ordinary wake, but coalesces any recovery resurface produced while that wake is pending and defers its replacement arm until the handling run settles.
OpenCode starts the next arm before delivering the wake prompt.
...
After an ordinary actionable Pi or OpenCode child close, the adapter starts and verifies one singleton successor before it delivers the original wake.
It waits at most one readiness timeout per attempt, then sends TERM and waits a bounded retirement confirmation before the next lock-verified exponential retry.
Pi marks the original wake pending before that restoration starts.
If the handling successor then closes on `check: rearm-resurface`, or another durable actionable row arrives before the queued handling run settles, Pi folds that close into the already queued drain instead of enqueueing another follow-up.
It starts one replacement from `agent_settled`, after the handling run has had the opportunity to retire the recovery generation.
A standalone recovery resurface is delivered once and uses the same settle-before-rearm path.
...
The same suite reproduces a handling successor that immediately emits `check: rearm-resurface`, proves only the original wake enters Pi's follow-up queue, proves one successor starts after `agent_settled`, and proves a standalone recovery resurface delivers its single wake before rearming through the same settle path.
