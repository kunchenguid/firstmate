...
Its PreToolUse continuity gate allows wake drain, arm recovery, independently fail-closed teardown, and the narrow wake-handling toolset while a wake is queued or was recently drained.
It refuses unrelated fleet commands while tasks are in flight and no identity-matched live watcher holds the home lock, regardless of wake state.
...
`tests/fm-continuity-pretool-check.test.sh` reproduces the topic-board watcher-exit deadlock, proves the drained wake remains handleable, proves unrelated mutation remains blocked without a wake, and preserves the existing Stop registration.
