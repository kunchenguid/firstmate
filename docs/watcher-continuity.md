...
The pull guard treats that residual window as healthy even after the ordinary beacon grace only when `state/.claude-autoarm-epoch` records an actionable `rewake`, the matching `state/.claude-rewake-turn` proof has not been cleared by the next Stop, both records bind to the same numeric session owner in `state/.lock`, that owner is still a live verified harness process, the guard invocation descends from that owner, and the detected harness is Claude.
