...
Both turn-end hooks share one session-lock recovery boundary: a numeric session-lock owner this home can disprove through the shared `fm_session_lock_reclaimable` predicate is reclaimed through `bin/fm-lock.sh` before any arm or state change, while any owner it cannot disprove, an absent lock, or a malformed lock keeps the competing hook inert.
That predicate reclaims on positive evidence only: the pid is gone or is not a verified harness, or an owner record for that exact pid proves a different process now occupies it; `bin/fm-session-lock-lib.sh` owns the record and its verification.
A live harness pid this home can neither confirm nor disprove - no record names it, or no identity can be read to compare it against - is missing evidence rather than proof, so it is refused and left alone instead of reclaimed; that is a still-running pre-upgrade session's shape, and it repairs its own record the next time it touches the lock.
