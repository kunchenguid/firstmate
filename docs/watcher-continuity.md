...
It pins the same two halves for the reclaim path, where the watcher takes over a dead predecessor's lock itself and recovers the episode its own steal published, instead of being handed a lock an arm already cleared: that reclaim stays live and silent over an empty fleet and still resurfaces a real open decision.
