...
Once those gates pass, a recovery-claim failure records an independent auto-arm failure epoch with the exact pre-attempt claim-ledger snapshot and an atomically elected episode marker instead of contending on or overwriting the claim ledger, and a later claim-ledger transition supersedes that failure.
