...
A session-lock owner with a valid identity - a local pid or a Windows-tagged `win:<pid>` - that fails the shared `fm_harness_pid_alive` predicate is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a live owner, an absent lock, a malformed lock, or an unreadable Windows process table keeps the competing hook inert.
