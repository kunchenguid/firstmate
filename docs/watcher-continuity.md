...
Machine-emitted `signal:`, `stale:`, `check:`, and `heartbeat` reason lines are presented as `<local timestamp> — <unchanged reason>`, where the short timestamp uses the durable queue row's event epoch when one exists rather than the later presentation time.
The durable queue remains the five-field `epoch<TAB>seq<TAB>kind<TAB>key<TAB>payload` schema, and arm and Stop-hook consumers accept both timestamped reasons and legacy unprefixed watcher delivery records.
...
`tests/fm-watch-arm.test.sh` covers durable queue replay, timestamped and legacy unprefixed reason compatibility, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
`tests/fm-wake-queue.test.sh` covers local-time rendering, delayed queue presentation using the stored event epoch, byte-exact preservation of the reason after its prefix, and the unchanged five-field durable schema.
