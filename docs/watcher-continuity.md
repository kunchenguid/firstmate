...
Once those gates pass, a recovery-claim failure makes a bounded identity-recorded attempt to serialize an immutable record and episode marker, then publishes both against the exact pre-attempt claim-ledger snapshot without that boundary when process identity or the boundary itself is unavailable and requests another Stop-owned turn only while that baseline remains current.
