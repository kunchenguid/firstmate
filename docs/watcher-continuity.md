...
OpenCode retargets its existing singleton arm to the newest idle TUI conversation, and completion of a wake-injected turn in an older conversation cannot reclaim later delivery after that handoff.
...
`tests/fm-pi-watch-extension.test.sh` checks Pi's first-cycle-or-explicit-repair tool metadata and ownership-based redundant-call no-ops, then simulates actionable and empty child closes against the actual Pi and OpenCode close handlers, blocks prompt delivery to prove the successor launches first, verifies OpenCode conversation retargeting and handoff-race ownership, verifies single-flight behavior, changes the session lock before close to prove ownership is rechecked, and hangs each successor arm to prove bounded fallback delivery includes the typed restoration failure.
