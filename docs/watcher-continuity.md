...
When Claude delivers the Stop hook from its daemon/background-spare tree, the hook may use exactly one valid `--spawned-by` JSON value as session-lock proof only if it names the live foreground Claude owner and the same resolved project root.
...
Once those gates pass, a recovery-claim failure records an independent auto-arm failure epoch and atomically elected episode marker instead of contending on or overwriting the claim ledger.
