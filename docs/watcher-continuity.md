...
The hook accepts the session lock only when the lock owner appears in the hook process's contiguous harness ancestry.
Claude daemon `--spawned-by` metadata is not an ownership proof.
When Claude delivers a Stop hook from a detached daemon/background-spare tree outside the foreground process ancestry, a live foreground owner keeps the hook inert and a stale owner produces durable failure state instead of reclaiming through inferred authority.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale, zombie, and live session owners, detached Claude daemon delivery failing closed without `--spawned-by` authority, unchanged AFK and need boundaries, independent failed-claim epochs, reset-fenced atomic notice election, stopped transition recovery with stopped-state revalidation and without age-only revocation of slow valid work, bounded legacy pre-publication, prior-terminal isolation, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
