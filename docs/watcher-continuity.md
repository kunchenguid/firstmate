...
`tests/fm-watcher-lock.test.sh` also covers `bin/fm-wake-lib.sh`'s Git Bash/MSYS directory-lock publication, including simultaneous single-winner acquisition, clean losing contenders, release and reacquire, refusal to reclaim a stale primary lock while a live steal mutex is held, stale recovery of pre-fix generated nested owner directories, refusal of empty or foreign nested content during migration cleanup, and Cygwin plus other real-symlink platforms staying on symlink publication through the nested owner-link race cleanup path.
