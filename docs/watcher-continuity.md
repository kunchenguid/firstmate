...
Codex owns the same shape through its own async Stop hook (`bin/fm-codex-stop-autoarm.sh`), which delivers the wake as a queued message rather than an exit-2 rewake, and retains its bounded foreground checkpoint protocol as the fallback wherever that hook is not firing.
...
`tests/fm-codex-stop-autoarm.test.sh` covers the Codex hook's same ownership boundaries (scope, session lock, AFK, need) plus what only it does: queueing an actionable close into the payload's own thread, keeping its ledger separate from Claude's, refusing an unbindable rewake, leaving no recovery claim behind a rejected push, retrying a rejected failure notice, and standing down with no thread or no `codex` binary.
`tests/fm-codex-continuity-live-e2e.test.sh` is the live guard for the vendor facts underneath it; [`verification/supervision.md`](verification/supervision.md#watcher-continuity) owns its tiers and their opt-in variables.
`tests/fm-turnend-guard.test.sh` covers the cooperative guard in both `--claude` and the Codex mode, including monotonic failed-epoch progression, the integrated bounded fail-open, post-alarm continuation suppression, and positive recovery reset; [`turnend-guard.md`](turnend-guard.md#regression-coverage) lists that suite's full generation and legacy claim coverage.
...
Claude depends on the Stop `asyncRewake` rewake, Codex depends on `codex queue` reaching the thread named by the `session_id` in that Stop payload and falls back to bounded foreground checkpoints when that hook is not firing, Cursor depends on its awaited stop-hook park, and Grok retains native background-completion notifications.
