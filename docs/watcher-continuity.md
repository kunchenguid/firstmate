...
Eviction (`stop_this_home_watcher`, shared with `--restart`) resolves and signals only the pid recorded in this home's lock, and only after `fm_watcher_lock_matches_pid` proves it is this home's own watcher: SIGTERM first, then, after `FM_WATCH_EVICT_TERM_GRACE` seconds, SIGKILL for a watcher that remains identity-matched and beacon-stale while deferring its TERM trap in a foreground syscall.
It never signals a live PID that is not this home's watcher, revalidates the recorded lock and process identity before SIGKILL, and never uses a name-matching kill.
...
Explicit `--restart` still sends SIGTERM to a verified healthy watcher because the operator requested its replacement, but its SIGKILL escalation remains beacon-stale-gated.
The watcher's own terminal poll wait uses an interruptible sleep so a poll-blocked watcher honors SIGTERM at once, keeping the SIGKILL escalation for a genuine foreground wedge.
...
It also covers wedged-watcher recovery against real processes: `arm` evicting a poll-blocked wedged watcher via SIGTERM and re-arming, evicting a frozen (SIGTERM-deferring) wedged watcher via the bounded SIGKILL escalation and re-arming, rechecking a recovered watcher before automatic SIGTERM while explicit `--restart` still stops it, refusing SIGKILL after lock identity changes, attaching to a healthy watcher without automatic eviction, and a poll-blocked watcher honoring SIGTERM promptly through the interruptible sleep.
