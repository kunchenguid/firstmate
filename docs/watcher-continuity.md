...
Pi same-process session replacement follows the generation-owner contract in `.pi/extensions/fm-primary-pi-watch.ts`: an owning `session_start` arms the replacement generation without waiting for a model turn, and a state-scoped replacement handoff carries every actionable close whose delivery overlapped `session_shutdown`, including a retiring child that reports after the bounded shutdown wait.
A main follow-up counts as delivered once Pi accepts it, never once the model reads it, because a follow-up queued while main is streaming joins the running run without a `before_agent_start`; an accepted close is never re-sent, so a replacement replays only a close Pi never accepted, and only after the replay gate (`bin/fm-watch-replay-gate.sh`) confirms the durable wake queue and recovery episode still name an unhandled wake - the extension header owns how consumption is observed and that replay contract.
...
The same suite also pins the replay bound: a replacement never re-sends a close Pi accepted, an unaccepted close replays once only while the replay gate (`bin/fm-watch-replay-gate.sh`) still names an unhandled wake, and a gate drop whose handoff cleanup fails marks the record delivered and stays bounded.
The gate is consulted off Pi's UI thread as a bounded awaited child: a pending answer keeps the session responsive, and a child that never answers is killed after its bound and fails open to exactly one presentation.
`tests/fm-watch-replay-gate.test.sh` pins the gate's verdict table.
