...
A numeric session-lock owner that fails the shared live, non-zombie `fm_harness_pid_alive` predicate is reclaimed through `bin/fm-lock.sh`'s bounded auto-arm mode before auto-arm state changes, while a live owner, absent lock, or malformed lock keeps the competing hook inert.
