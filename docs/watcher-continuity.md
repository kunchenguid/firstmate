...
A session-lock owner that the shared `fm_session_lock_stale_owner` predicate proves gone is reclaimed through `bin/fm-lock.sh` before auto-arm state changes, while a live owner, absent lock, or malformed lock is uncertainty rather than stale-owner evidence and keeps the competing hook inert.
