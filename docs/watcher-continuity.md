...
Only an exhausted failure with no verified watcher commits one last-resort notice for the continuous failure episode; a refused notice commit stays silent for a later retry, and after a successful notice later Stop cycles exit 2 with a shorter suppressed-continuation line instead of the full notice, until the turn-end guard consumes the attended fail-open.
Deduplication applies to the full notice, never to the block itself: exit 2 blocks the turn and the collected stderr is the operator's only view of why, so every later failure block still names the current refusal and the epoch ledger to read, and a healthy home whose failure-episode reset cannot be recorded is told that the refused bookkeeping write, not the arm, is the cause.
A blocking path that printed nothing was undiagnosable by construction, and the arm's typed lines are not the only possible cause, so the evidence is the typed lines plus a bounded tail of any untyped output rather than a choice between them; an arm that said nothing, or whose output the hook could not even capture, is reported as exactly that.
The Claude turn-end guard owns that notice commit contract, the rule that no blocking exit of either Stop hook may be empty, the monotonic failure progression, one-time attended fail-open, post-alarm continuation suppression, and positive recovery reset described in [`turnend-guard.md`](turnend-guard.md#harness-integrations).
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, exit-2 translation, host-timeout HUP/TERM/INT translation into the same durable failure handoff, and that no blocking firing is ever empty - including on a real filesystem that reverts `chmod 600`, when the host offers one.
