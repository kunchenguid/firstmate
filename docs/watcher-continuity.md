...
`tests/fm-wake-queue.test.sh`, `tests/fm-watch-triage.test.sh`, and `tests/fm-supervision-events.test.sh` cover atomic same-key stale coalescing, unrelated-notification progress, a full post-acknowledgement cadence, activity reset, acknowledged deep-inspection escalation, and the Herdr push-transition path.
