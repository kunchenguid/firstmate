...
Any other child return - zero/empty, or nonzero from a startup race the PR-check migration guard or lock acquire rejected, or a genuine crash/kill - rechecks the home lock and beacon, attaches to a verified healthy successor when one exists, or resolves the close against the watcher's bounded terminal-delivery ledger.
...
`tests/fm-watch-arm.test.sh` covers an attached arm reporting a delivered wake instead of a false failure, including after the handling turn drains the durable queue, a cycle that truly delivered nothing still failing loudly, and an arm whose own forked child loses a startup race (a PR-check migration ownership rejection) to a live sibling still reporting that sibling's delivered wake.
