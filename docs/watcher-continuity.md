...
Supervision need includes a registered custom check whose current bytes match its private trust binding, even when no tasks are in flight.
...
Once started, `bin/fm-watch.sh` does not close a cycle merely because the home has no tasks in flight.
Its normal idle loop keeps refreshing the watcher beacon and evaluating authenticated registered checks at `FM_CHECK_INTERVAL`, so a check-only home remains supervised until a check produces an actionable result or the owning lifecycle stops the watcher.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK boundaries, empty-home inertness, authenticated custom-check-only need, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, and exit-2 translation.
