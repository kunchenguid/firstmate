...
Today's registry is the single lane `fireworks-glm`, using Fireworks' GLM 5.2 Fast selector `accounts/fireworks/routers/glm-5p2-fast` through the direct Fireworks endpoint.
Fireworks documents that selector as the same GLM 5.2 model at the same quality on a serving path targeting more than 100 generated tokens per second: https://docs.fireworks.ai/serverless/serving-paths.
...
The guest independently retains the terminal stream contract too: at least one turn, a final assistant `stop`, and exactly one completed agent are required.
The final terminal event must report the exact `fireworks-glm` provider and `accounts/fireworks/routers/glm-5p2-fast` model selector requested by the compartment; reporting the former Standard selector or another route fails before a verdict can publish.
Pi's explicit `auto_retry_start` may open a continuation only after a completed attempt executed a turn and did not stop successfully; the continuation starts with empty terminal state and must execute its own turn before completing.
`model_guest_pi_verdict_unit` executes the exact shipped heredoc and byte-compares its pure fence parser to the host copy so the self-contained image artifact cannot drift back to `json.loads(final)`.
When Pi itself exits zero with a safe terminal stream but the artifact is malformed, non-object, or lacks a dictionary `verdict` or `evidence_files`, the guest permits exactly one protocol-correction attempt in the same credentialed compartment.
It deletes the first result and event files, prepends a fixed trusted correction that requires silent reasoning and `{` as the first output byte, then appends the untouched original prompt so its exact schema remains last.
No first-output byte enters that prompt, and a nonzero Pi exit or provider/stream failure never earns this correction.
Each invocation's stderr stays in a separate attempt file and is never emitted raw; every surfaced stderr or artifact failure summary is control-escaped and byte-bounded, and the common wrapper gate still rechecks the accepted result.
Honest limit, corrected five times. Four live 2026-08-21 cross-family attempts for PR #285 provisioned model compartments and reached the guest, but none returned a valid verdict. The first produced one fenced JSON object and the guest's then-naive `json.loads(final)` refused it. The second, from the repaired parser at `b987aa9e`, passed that parser but returned a wrapper with `verdict` and no `evidence_files`, so the guest correctly refused the missing evidence manifest. The third and fourth ran after both the parser and exact outer-schema fixes; each stopped successfully but returned prose-only final text beginning `Looking at this PR...`, so the parser correctly refused malformed JSON. Pi has no `--output-schema`; the prompt called the Crosscheck verdict schema "supplied" without embedding the outer Azure wrapper schema's bytes. The prompt now ends, after the untrusted packet, with the exact compact schema requiring both `verdict` and `evidence_files`; Codex still receives the same schema through its native flag as well. The new one-shot correction has not yet completed a live review. Until the first attempt, both reasons given for the lane not running were wrong. It is not that the image lacks `pi` (stale), and it is not ONLY that the lane is switched off. `$FM_HOME/config/crosscheck-azure.json` carries `"enabled": false`, set by an operator on 2026-08-20 - but flipping that flag would NOT have restored the codex-family path, because a second, independent, in-code blocker sat behind it: the archive gate derived the executing-account identity a second time and returned the BARE account id while the admitted identity carried a `codex:` / `openai-codex:` prefix, so `archived_identity != reviewer_account_identity` was structurally always true. A live run refused there at 04:43Z, before any billable resource. NO codex-family compartment review has ever run; every one of the resource group's historical `fm-crosscheck-model-*` deployments is on the cross-family path, which passed only because both sides there read one shared value. Fixed 2026-08-21, and the FIRST attempt at that fix was incomplete in a way worth recording: it made the host reader and the host archive gate share one derivation, but the model guest carried a THIRD copy that still derived the bare account id, so the refusal simply moved from staging into a booted, paid VM. All three now derive the prefixed identity; the guest cannot import the others because it ships self-contained onto the VM, so `model_guest_executing_account_unit` EXECUTES the guest's own credential block against the host readers to prove they agree, and is red on either the two- or the three-derivation form. A compartment review has still not completed; the only cross-family verdicts accepted so far came from the local Pi reviewer.
