...
A `missing` verdict additionally lets `bin/fm-control.sh <id> relaunch` recreate the endpoint before launching: the launch owner, `bin/fm-spawn.sh --relaunch`, ensures the container and creates the window through this adapter's own functions, rooted in the recorded worktree, under the preconditions its header owns.
