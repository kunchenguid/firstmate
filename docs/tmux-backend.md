...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, AGY, and Polytoken process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
AGY, Devin, and Polytoken are anchored to the exact `agy`, `devin`, and `polytoken` identities for the same reason, so unrelated names containing those fragments remain ambiguous.
