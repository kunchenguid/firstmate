...
Copilot is anchored to the exact `copilot` identity for the same reason; its node loader shim carries no name identity and is reached through the structural argv rule in `bin/fm-copilot-lib.sh` instead.
