...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, AGY, and Devin process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
Devin is anchored to the exact `devin` identity for the same reason, so unrelated names containing that fragment remain ambiguous.
...
tests/fm-devin-harness.test.sh
