...
A target-existence check proves only that the recorded pane exists, and it must prove that rather than assume it: tmux resolves a `-t` target loosely, so an absent window can be answered from an unrelated live pane, and a session name as well as a window name can match by prefix.
...
Exact matching applies to names only, so a window index, an `@window-id`, and a pane-qualified name still resolve, a colonless `%pane` id keeps the plain form because a bare pane id rejects `=` and already resolves exactly, and a target naming no window at all (empty, or `session:`) is refused outright because tmux would answer it from whatever window is current.
