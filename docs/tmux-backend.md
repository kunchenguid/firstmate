...
A target-existence check proves only that the exact recorded endpoint exists, using exact-session window inventory for `session:window` targets or server-wide pane inventory for stable `%pane` targets so tmux's absent-target active-window fallback cannot mask a missing endpoint.
