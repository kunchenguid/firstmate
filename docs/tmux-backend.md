...
A target-existence check proves only that the recorded pane exists, and it must prove that rather than assume it: tmux resolves a `-t` target loosely, so an absent window can be answered from an unrelated live pane, and a window name can also match by prefix.
The presence check therefore binds a recorded `session:window-name` to the window tmux actually resolved, and [runtime backend verification](verification/runtime-backends.md#target-resolution) owns the evidence.
