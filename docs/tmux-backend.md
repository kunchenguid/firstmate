...
A fresh spawn additionally proves the worker started before publishing `state/<id>.meta`, so a pane that never becomes a live agent stays unwindable and one automatic retry can reuse the preserved worktree. [`bin/fm-dispatch-breaker-lib.sh`](../bin/fm-dispatch-breaker-lib.sh) owns that start-proof.
