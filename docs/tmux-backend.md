...
An ordinary local `fm-send.sh` text steer and every remote text steer no longer ride this verified submit as their delivery proof: they become durable steering-inbox records plus best-effort constant doorbell lines (`bin/fm-task-inbox-lib.sh`), acknowledged only by the worker's move into `handled/`.
That doorbell still rides this submit core, and an exact `pending` verdict surviving it is read there as proof that the doorbell's own typed line is stranded unsubmitted in the composer, never as delivery; `bin/fm-task-inbox-lib.sh`'s header owns what the delivery ladder does with that.
The verdicts above decide delivery only for the local typed plane - harness-native invocations and explicit backend targets - where `fm-send.sh` still never retypes or assumes a confirmed submit for an unconfirmed verdict; its header owns the distinct delivered-unconfirmed exit status and operator response.
