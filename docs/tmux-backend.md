...
agy is anchored to the exact `agy` name because it ships as a single binary with no version-named or wrapped form, so the three-letter substring never claims an unrelated command.
