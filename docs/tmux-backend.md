...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, and Hermes/Antigravity worker-bridge process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
The Hermes and Antigravity endpoints run under a shell trampoline launched as `fm-worker-bridge`, so that exact name is anchored the same way: the live process is a bash whose install path carries no bridge component, and the rewritten argv[0] is the only field that names the agent.
