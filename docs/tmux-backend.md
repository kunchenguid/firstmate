...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, scoped OMP, Grok, Kimi, Cursor, and Muse process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
OMP runs under generic `bun`, so a bare Bun process remains ambiguous unless the task record says `harness=omp` and the foreground argv identifies its launcher or pi-coding-agent package.
