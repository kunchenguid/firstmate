...
humanlayer is anchored to the exact `humanlayer` identity for the same reason (the node shim reaches detection at interpreter-args strength, never process-name strength), so unrelated names containing that fragment remain ambiguous.
