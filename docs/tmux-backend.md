...
A target-existence check proves only that the exact recorded window exists, using the same session inventory that prevents tmux's absent-target active-window fallback from masking a missing endpoint.
