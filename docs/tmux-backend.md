...
agy is anchored to the exact `agy` identity: its launch execs the bare binary, so both name sources read exactly `agy`, and `agy` is short enough that a substring match would claim ordinary names such as `legacy` and `agyneja`.
