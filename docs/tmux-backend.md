...
Arrows, such as the ones needed to move a selection inside a harness confirmation dialog, therefore reach the pane exactly as written; Enter, Escape, Ctrl-C, and C-u are the only names this repository's own evidence exercises (C-u is the composer-clear key the muse interrupt path sends after Escape, from `fm_control_interrupt_clear_key` through `fm_send_clear_after_interrupt`), and any other name is resolved by tmux itself.
