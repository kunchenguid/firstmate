...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, omp, and agy process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
agy is anchored to the exact `agy` identity for the same reason, so `magyar` and `agyw` remain ambiguous.
