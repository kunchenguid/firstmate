...
Both were observed during verification and cleaned up by hand. This is why `tests/fm-backend-thurbox.test.sh` drives a **stubbed** `thurbox-cli`, why `tests/thurbox-test-safety.sh` fails closed unless the binaries in use live inside the test's own fixture (see "Regression entry points"), and why - unlike cmux and zellij - this backend ships **no real-binary smoke test**.
...
- `tests/fm-backend-thurbox.test.sh` - the stubbed-CLI unit suite, covering the version and socket gates, naming and length limits, the shell-agent requirement and its `[[agents]]`-scoped literal name match (including ordinary trailing comments, TOML whitespace, and a `#` inside a quoted name), create and its duplicate refusal, the full target-resolution model (pane re-resolution after restart, name-mismatch and remote refusals, recovery by label, fail-closed cases), the `send_literal`-never-auto-submits rule, capture and composer routing including the Cursor reclassification and its process-identity gate, a pane read that fails surfacing as a failure rather than as an empty screen, the `hook_state` mapping and the queued-Enter conversion it gates (converting only on an affirmative busy), forced teardown of a row whose window is already gone and of a secondmate's child session under the child home tag, discovery scoping, endpoint-metadata validation, both halves of the detection rule, both halves of the same rule in away-mode captain-pane discovery, and the operator-level path - `bin/fm-spawn.sh --backend thurbox` end to end plus `fm-peek`, `fm-send`, and `fm-crew-state` driven against a recorded `backend=thurbox` task, proving that spawn creates the home-scoped session and records the durable UUID, that the dispatcher routes into this adapter, and that the steering doorbell reaches the pane through `session send --no-enter` plus `session key enter`.
- `tests/thurbox-test-safety.sh` - the fail-closed guard that keeps any thurbox test away from a real thurbox: the `thurbox-cli` in `FM_THURBOX_BIN` must be a stub inside the test's own fixture, and so must the `tmux` that would be resolved from `PATH`. The tmux half is deliberate defence in depth rather than a leftover - the adapter runs no tmux command, so a regression that reintroduced one would otherwise reach the operator's real thurbox server with this guard still reporting safe. `tests/fm-backend-thurbox.test.sh` keeps a tmux tripwire in its fixture that fails on any invocation, so both would have to be defeated.
