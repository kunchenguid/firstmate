...
The never-started proof accepts either the exact empty preflight stub or an exact-key, exact-script update that Azure left `Updating` with a stable `Pending` instance view and no start time, end time, output, error, execution message, or result marker.
It tolerates a non-succeeded sibling monitor or bootstrap child without weakening ordinary create or execute readiness, but it still refuses incomplete or conflicting resource identity, any command or instance-view mismatch, execution evidence, staging drift, changed observations, or a non-exact durable marker.
The never-started exit durably marks the exact key and every future execute refuses that marker; ordinary dark-VM cleanup then cascades the retired Run Command child.
