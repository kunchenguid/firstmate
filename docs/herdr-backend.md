...
Every Herdr invocation goes through `fm_backend_herdr_cli`, which uses a live pane's injected Herdr server identity and otherwise passes an explicit trailing `--session <name>`.
