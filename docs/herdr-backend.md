...
The recorded `herdr_tab_id` is authoritative: while that tab appears in the listing, only it maps to `fm-<id>`, and an out-of-band tab wearing the same label is never mapped (Herdr enforces no label uniqueness against tabs firstmate did not create); the unique-label fallback applies only when the recorded tab is absent from the listing.
...
| Recovery / list-live | `herdr tab list --workspace <id>`, then direct `fm-<id>` recognition or `label=` metadata mapping | Label-based, with the recorded tab id authoritative for a human label while its tab is listed, and a unique matching meta label as the restart fallback only when the recorded tab is absent. `<id>` is always THIS home's own workspace (`fm_backend_herdr_workspace_find`), so recovery never sees a sibling home's tabs. |
