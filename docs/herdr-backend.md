...
An `unknown` registration is recoverable only when its echoed pane id matches the recorded pane and the process view positively proves shell-only; a running agent, non-shell foreground, unreadable process view, or contradictory pane id still refuses recovery.
That record need not carry an `agent` label: once shell output follows an orderly Codex exit, Herdr 0.8.2 re-detects the pane and drops the label while keeping the record, so `agent get` reads `agent_status` `unknown` with no `agent` and only `agent_session` still naming Codex (measured 2026-09-15 - [verification](verification/runtime-backends.md) "Stale agent registration"), and requiring a label there refused exactly the pane an exited worker leaves behind.
