...
A new `FM_SUPERVISOR_BACKEND` override (`tmux`|`herdr`) resolves independently, mirroring `bin/fm-backend.sh`'s own `fm_backend_detect`: `$TMUX_PANE` set selects tmux (even nested inside herdr, matching the innermost-first rule); `$HERDR_ENV=1` with `$HERDR_PANE_ID` present selects herdr, composing the target as `"${HERDR_SESSION:-default}:${HERDR_PANE_ID}"`.
Absent both an override and any live-pane marker, the only target left is the legacy `firstmate:0` guess (a crewmate tmux session, not firstmate's input); as of 2026-07-11 both `/afk` and the daemon refuse loudly and abort on that blind fallback rather than injecting there and wedging silently (see `bin/fm-supervisor-target-lib.sh`).
