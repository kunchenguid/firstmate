...
That released label survives the launch, so it can never stand in as proof for a replacement on another runtime either: a cross-runtime relaunch out of this state is accepted only after exactly one independent target-harness process, with no Pi engine anywhere below the same pane shell, is stable in the recorded pane and copy within `FM_CONTROL_LAUNCH_WAIT`.
Independence is decided over the whole ancestry up to the pane shell, so a harness that runs its own helpers - even behind an intermediate shell - still counts as one replacement, while two target processes neither of which descends from the other are the duplicate worker this refuses; the accepted process's ancestry is part of the stable sample, and an ancestry that cannot be read is a refusal rather than a root.
...
Because every process witness in that proof is derived from one `pane process-info` response, an unreadable pane is never read as an absent engine: the read is retried up to `FM_CONTROL_HERDR_READ_RETRIES` total attempts, and a pane whose process state still cannot be read is answered by a second witness that needs no Herdr response at all - a live, non-zombie Pi engine in the operating-system process table carrying this task's own generated `state/<id>.pi-ext.ts` extension argument, which no other task's worker can carry.
Only when both witnesses stay silent does the endpoint remain ambiguous, and ambiguity still refuses without terminal input.
