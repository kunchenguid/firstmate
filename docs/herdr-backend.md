...
The persisted `.focused` pointer is not a live viewer: when `herdr terminal title clear` reports `no_foreground_client`, cleanup proceeds on that tab because no human is attached.
Herdr currently has no atomic client-aware close, so the foreground-client checkpoint runs immediately before the close; a client can still attach in the residual probe-to-close window, and a durable atomic close is deferred until Herdr exposes that primitive.
