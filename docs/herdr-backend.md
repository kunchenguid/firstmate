...
Discovery starts from a title ending in the exact ` · p:<22-character-token>` grammar (currently `<L><n> <title> · p:<token>`, e.g. `C7 my task · p:...`, with legacy `└ <concise-task> · p:<token>` labels still accepted; see `fm_backend_herdr_projection_workspace_label`), but a title or token alone is never mutation authority.
The token must occur exactly once across the named-session snapshot and must match the token recorded by exactly one valid presentation journal in this home's own `state/`; a version 2 journal additionally must bind this exact physical home, named session, workspace, tab, and pane.
