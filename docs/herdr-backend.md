...
Every recorded option is retained: `option_count`, `options_retained`, `unsupported_options`, and `options_complete` describe the set, an option whose text cannot be displayed safely is shown as an explicit `Unsupported option (select explicitly)` entry rather than dropped, and a decision that recorded any option never collapses to a binary `Igen`/`Nem` card.
