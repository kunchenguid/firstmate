...
Resolved backend evidence, including the symlinked-project-prefix isolation fix, is kept in the same follow-up log for auditability.
...
- **RESOLVED: worktree-discovery isolation guard was symlink-fragile for a project path under a symlinked prefix (e.g. macOS's `/tmp` -> `/private/tmp`).** Discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project. `fm-spawn.sh`'s `PROJ_ABS` is a LOGICAL `cd && pwd` (symlink components kept), while herdr's `foreground_cwd` (and real tmux's `pane_current_path`, on the same OS-level cwd primitive) report the PHYSICALLY resolved path.
  When the project itself lived under a symlinked directory, the very first worktree-discovery poll saw two different strings for the identical starting directory: its old break condition (`current_path != PROJ_ABS`, compared as raw strings) fired immediately on the still-in-project pane, and the isolation guard then false-refused the spawn as "not isolated" before `treehouse get` ever moved the pane - backend-agnostic, not specific to herdr. Fixed in `fm-spawn.sh` by hoisting the isolation invariant into a shared `is_isolated_worktree()` helper that resolves both the candidate path and `PROJ_ABS` physically (`pwd -P`) before comparing, and using that helper as the discovery poll's own break condition (not just `validate_spawn_worktree`'s final assertion). The poll now compares physically-resolved forms on both sides, so a symlinked project prefix no longer collapses to a false non-match and the poll correctly keeps waiting for the genuine isolated worktree instead of breaking on the still-in-project pane. The test still resolves its scratch `TMP_ROOT` through `pwd -P` up front, but that is now defensive rather than load-bearing.
