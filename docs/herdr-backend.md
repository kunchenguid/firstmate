...
- Recovery of an existing presentation journal waits for the shared presentation lock instead of falling back flat or refusing, so concurrent recoveries serialize and a recovery spawn can block while another holder keeps the lock; dead-owner reclaim stops the wait when a holder crashed.
