...
Options are compared after leading and trailing whitespace is trimmed, so choices that differ only in surrounding space collapse into one row and are counted as duplicates rather than rendered as two rows an operator cannot tell apart.
Internal whitespace is significant and is never collapsed for comparison, so `zip x` and `zip  x` remain two distinct choices; a run of two or more internal spaces is rendered as a visible ` _ ` marker so those choices read differently instead of relying on a positional suffix.
