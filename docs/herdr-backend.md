...
Recovery carries it only after proving over `worktree list` on the journal's exact parent, or for a version 1 journal on the owning home, that the recorded checkout belongs to that exact project, so a legacy top-level projection whose checkout was only a process lease is never adopted.
An open linked checkout must still be that journal's child; a linked checkout that the same listing no longer renders as an open child is carried only when the durable slot-owner claim names the recovering task, because its parked work would otherwise be stranded by a second allocation.
