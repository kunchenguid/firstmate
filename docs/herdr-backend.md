...
That deferral is bounded in turn: once deferrals for one steer span `FM_TASK_INBOX_DEFER_HORIZON_SECS` (default: one full re-ring ladder plus a grace), a lock nothing is going to release - a leaked lock directory, a wedged lifecycle action - surfaces the unread steer once as an ordinary stale wake, and the steer stays pending so it is still rung normally if control does release.
