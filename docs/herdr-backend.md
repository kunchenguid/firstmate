...
Options are deduplicated on the raw recorded value rather than on their display label, so an exact repeat is counted as a duplicate and never reported as a lost choice, while two different options that both fail the display check stay distinct.
An option whose text cannot be displayed safely is shown as a numbered `Unsupported option N (select explicitly)` entry keyed to its position in the distinct set, so distinct unavailable choices are never merged into one row and `unsupported_options` counts each of them.
