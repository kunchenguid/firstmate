...
`bin/fm-spawn.sh --label <text>` optionally gives a task a human-readable tab label instead of the default `fm-<id>` display.
The `fm-spawn.sh` header and `--help` output own normalization, validation, uniqueness, batch, and relaunch behavior.
The sanitized value is recorded as `label=` in `state/<id>.meta`; task ids and recorded endpoint ids remain the operational identity.
Other backends retain their stable `fm-<id>` runtime names even when metadata carries a display label.
