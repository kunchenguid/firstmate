...
That lock lives in a namespace whose name carries the account's own numeric user id, read from the shell itself rather than from a command on `PATH` so that two processes of one account always serialize against each other, and it is shared by every Firstmate home of one account because no other account's Firstmate ever creates or uses that name.
