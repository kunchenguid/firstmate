...
Projected cleanup therefore runs under the same session lock, captures the exact active tab, refuses to delete the active tab (except in the opt-in "retreat" mode fm-teardown uses for the task's own operator-requested termination, which first retreats focus to the terminating process's launcher tab and otherwise falls back to the plain confirmed close's legitimate focus move rather than leaving the doomed tab open), and treats a workspace-emptying close as a focus-safe removal: it verifies the close would empty the workspace, repositions the doomed workspace behind the focused one through the verified `workspace.move` transport when needed, proves the pane holds one lone idle shell, and ends that shell so Herdr removes the emptied workspace through its focus-preserving pane-death path.
...
`tests/fm-backend-herdr-focus-flash-e2e.test.sh` reproduces the raw explicit-close focus steal on the installed release, proves the focus-safe emptying-close plan removes a doomed workspace with no wrong-focus interval, and proves both active-target modes on the real binary - the default refusal leaves the focused doomed pane intact while the opt-in "retreat" mode closes it; [`verification/runtime-backends.md`](verification/runtime-backends.md#workspace-removal-focus-safety) owns the active versioned evidence.
