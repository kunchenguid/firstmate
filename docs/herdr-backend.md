...
The adapter is a thin capture: it hands a bounded ANSI tail plus Herdr's capability facts to the fleet-wide classifier in `bin/fm-composer-lib.sh`, which owns every shape, including muse's `⟩`, which the adapter's retired local pattern silently omitted.
This adapter pioneered the identity-gated separator pair; the classifier owns which idle or done identities that pair admits.
...
A working identity, pending middle row, missing identity, incomplete separator pair, or over-tall candidate remains unknown or pending.
