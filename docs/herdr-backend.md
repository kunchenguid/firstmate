...
A spawn that aborts while its task pane survives a refused close keeps its leased slot rather than force-returning a worktree whose pane may still hold an agent or unlanded work, and records that slot under `state/.treehouse-lease-retained/`; `bin/fm-bootstrap.sh` prints one `TREEHOUSE_LEASE:` line per record at session start, carrying both the reclaim command and the record to remove. Reclaiming such a slot is deliberately a human step - automatic reclamation is not implemented.
A spawn that aborts after its launch was delivered keeps the slot the same way, whatever the layout, because the launched agent is running in it.
A same-identity respawn over that husk takes the slot the task already holds (`treehouse status --json` reports it leased to `fm-<task-id>`) instead of leasing a second one, so it replaces the pane without stranding the worktree the previous worker left its unlanded work in, and skips the pooled-slot base refresh exactly as a relaunch does.
If that respawn then aborts, its record is rolled back and the reused slot is retained and recorded; it STILL HOLDS THE PREVIOUS WORKER'S UNLANDED WORK, and a retried respawn of the same task does not find it and starts in a FRESH slot. `treehouse return --if-lease-holder` DISCARDS that work, so the retained slot must be INSPECTED, and anything worth keeping landed or saved, BEFORE it is reclaimed.
A restored same-labeled tab with a missing pane or no registered agent is a husk.
Create replaces only a confidently dead or no-agent husk, creates the replacement before closing the old tab, and refuses live or unknown states.
This prevents closing the workspace's last tab before a replacement exists.

A registration alone never proves an agent.
Herdr keeps a Pi registration (`agent get` still reports `agent=pi` with its last status) after the Pi process has exited to a plain shell whenever a nested interactive shell sits under the pane's top shell (measured on Herdr 0.9.0 - [verification](verification/runtime-backends.md) "Stale agent registration"; upstream issue #4115).
A task pane's own top shell is its worktree shell, because the pane is created in the leased worktree and is sent no interactive `treehouse get`, so any such nesting is whatever the pane itself ran.
