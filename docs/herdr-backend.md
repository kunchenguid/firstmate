...
The adapter is a thin capture: it hands a bounded ANSI tail plus Herdr's capability facts to the fleet-wide classifier in `bin/fm-composer-lib.sh`, which owns every shape - bordered boxes, bare agent-glyph rows (including muse's `⟩`, which the adapter's retired local pattern silently omitted), opencode's left bar, and Pi's separated or compact region.
A Pi region proves an empty composer only when native `agent get` identity is exactly Pi and state is idle or done.
The compact Pi shape is a rounded status header, one unboxed input row, and one lower solid separator followed by no visible content in the capture; its empty row must be exactly one reverse-video blank cursor cell.
...
A working Pi, pending input, an unstyled compact row, missing or contradictory identity, truncated capture, incomplete separator pair, over-tall candidate, or trailing shell remains unknown or pending.
Identity stays a lazy second read, consulted only after the classifier selects a recognized Pi structure.
