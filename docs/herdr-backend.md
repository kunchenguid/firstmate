...
- **RESOLVED: worktree-discovery isolation guard's symlinked-project-prefix false refusal.** Originally discovered while building the runtime-backend-auto-detection real smoke test (`tests/fm-backend-autodetect-smoke.test.sh`), which needed a scratch project.
  ...
  The wait loop's symlinked-project behavior is additionally regression-covered at the behavior level by `tests/fm-spawn-symlink-wait.test.sh`, which drives the real `bin/fm-spawn.sh` through a symlinked project dir against a stateful fake tmux whose pane keeps reporting the project's physical cwd for the first polls before moving to the worktree.
