...
The floor is read from the selected named session's server protocol number and version while that server is running, or from the installed client's signals only when status positively reports no running server and that client will start it; either protocol or version can establish a supported release on its own, and an unreadable server state or release is treated as unsupported rather than guessed at.
