...
`bin/fm-spawn.sh --label <text>` adds an optional human display label, for example `--label "#5 bpi-auth"`.
The `fm-spawn.sh` header and `--help` output own the normalization, validation, reuse, and batch-dispatch contract, while `fm_task_display_label_sanitize` in `bin/fm-backend.sh` is the shared implementation.
The sanitized value is recorded as `label=` in `state/<id>.meta`, and a respawn without an explicit flag reuses it.
A label beginning with `fm-` is refused because that namespace remains the stable task identity.
A Herdr spawn also refuses a display label already recorded by another Herdr task in the same home, so duplicate-label husk handling cannot replace or block another task's tab.

Without `--label`, Herdr retains the `fm-<id>` tab name byte-for-byte.
With it, both flat and optional presentation-space task tabs use the human label while the task id, metadata filename, `endpoint_task_id=`, `window=<session>:<pane-id>`, and backend ids remain authoritative.
Teardown still closes the recorded pane id.
Flat-workspace list-live discovery maps a human-labeled tab back to `fm-<id>` through matching `label=`, session, workspace, and tab metadata, and skips an unmapped human label rather than guessing.
The line-framed discovery path strips label control characters before matching so an out-of-band label cannot forge a recovery record.
[`verification/runtime-backends.md`](verification/runtime-backends.md#human-readable-task-labels) records the Herdr 0.7.3 probe and the reviewed applicability limits for the other backends.
...
The normal `fm-<id>` task tab, or its metadata-bound human display label, is created in the exact new workspace returned by Herdr.
...
label=<optional-sanitized-display-label>
