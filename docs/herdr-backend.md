...
When the selected named server is not running, the adapter launches it without inherited Firstmate home and directory overrides, harness identity markers, the supervision-model override, or the launcher's color control (`NO_COLOR`, `FORCE_COLOR`, `CLICOLOR`, `CLICOLOR_FORCE`).
