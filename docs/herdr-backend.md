...
The adapter is a thin capture: it hands a bounded ANSI tail plus Herdr's capability facts to the fleet-wide classifier in `bin/fm-composer-lib.sh`, which owns every shape - bordered boxes, bare agent-glyph rows (including muse's `⟩`, which the adapter's retired local pattern silently omitted), opencode's left bar, and the separator region this adapter pioneered for Pi (agy draws the same pair around a `>` prompt row), admitted only when a native `agent get` identity is exactly Pi or agy and state is idle, done, or blocked.
A working agent, pending middle row, missing identity, incomplete separator pair, or over-tall candidate remains unknown or pending.
