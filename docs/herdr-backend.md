...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call; used for the three fixed spawn-time commands (`treehouse get`, the PWD normalization, the `GOTMPDIR` export) exactly where tmux used one `send-keys ... Enter` call. |
