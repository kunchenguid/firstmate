...
A working Pi, missing identity, incomplete separator pair, or over-tall candidate remains unknown or pending.
A pending middle row is empty rather than pending when the pi is idle or done and every non-blank row is pi-vimmode's `─ INSERT y:x ─` status furniture, not submitted text (issue #3819); any other content in that row keeps it pending.
