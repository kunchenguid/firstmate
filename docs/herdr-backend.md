...
Because every process witness in that proof is derived from one `pane process-info` response, an unreadable pane is never read as an absent engine: the response is retried for `FM_CONTROL_HERDR_READ_RETRIES` attempts, and a pane whose process state still cannot be read is answered by a second witness that needs no Herdr response at all - a live, non-zombie Pi engine in the operating-system process table carrying this task's own generated `state/<id>.pi-ext.ts` extension argument, which no other task's worker can carry.
