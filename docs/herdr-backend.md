...
The floor is read from the Herdr client's own reported protocol number and version, either of which can establish a supported release on its own, and a release that reports neither readably is treated as unsupported rather than guessed at.
