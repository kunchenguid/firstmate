...
Task cleanup also runs the active-tab focus precheck (`fm_backend_herdr_projection_pane_focus_precheck`) before that same return, so a captain-focused target refuses the whole teardown up front instead of surfacing only once the close itself runs, which would otherwise leave the copy already reclaimed against retained records.
That precheck cannot close the window between it and the actual close: if the captain refocuses the exact tracked tab while the destructive steps in between run, the close still fails once the copy is already reclaimed.
There, cleanup finishes instead of retaining records: it removes every durable record and leaves the pane for the captain to close by hand, since retaining them would only invite a rerun that could reset a slot already reassigned to a new task.
Any other unconfirmed-close reason once the copy is reclaimed - an ambiguous post-close presence read, an actual close-command failure - keeps the ordinary retain-for-retry behavior, since only the focus-unsafe refusal reproduces that unsafe rerun.
