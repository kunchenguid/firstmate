...
| Send + submit atomically | `herdr pane run <pane> <command>` | Runs and submits a command in one call once the pane's shell is ready. A newly created pane can exist before its shell accepts Enter, so immediate post-creation callers must prove readiness or retry; the real-Herdr E2E fixtures do this with a marker probe. |
