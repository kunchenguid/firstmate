...
The same suite covers the start-up poll on both sides: instant cold-socket failures polled through to a server that comes up, and consecutive bound hits cut short instead of multiplying the bound by the poll budget.
