...
An `unknown` registration is recoverable only when the response type is `agent_info`, its echoed pane id matches the recorded pane, and the process view positively proves shell-only; a running agent, non-shell foreground, unreadable process view, contradictory response type, or contradictory pane id still refuses recovery.
That record need not carry an `agent` label: in the measured Herdr 0.8.2 run, `agent get` returned `agent_status` `unknown` with no `agent` and only `agent_session` still naming Codex after an orderly Codex exit and later shell output (measured 2026-09-15 - [verification](verification/runtime-backends.md) "Stale agent registration"), and requiring a label there refused exactly the pane an exited worker leaves behind.
...
The response shape the adapter parses (`result.type` of `pane_process_info`, `process_info.shell_pid`, and `foreground_processes` entries carrying `name`, `argv0`, `argv`, and `cmdline`) is verified live on Herdr 0.9.0, with the idle-shell proof's narrower parse previously verified on 0.7.5.
Herdr 0.8.2 emits those entries with `name`, `argv`, and `cmdline` and no `argv0` (measured 2026-09-15 - [verification](verification/runtime-backends.md) "Stale agent registration"), which the parse covers by reading argv[0] from `argv` ahead of `argv0`.
A server response below 0.8.2 has not been measured for this parse.
