...
Herdr keeps a Pi registration (`agent get` still reports `agent=pi` with its last status) after the Pi process has exited to a plain shell whenever a nested interactive shell sits under the pane's top shell, the shape an interactive `treehouse get` or `treehouse enter` leaves behind (measured on Herdr 0.9.0 - [verification](verification/runtime-backends.md) "Stale agent registration"; upstream issue #4115); a spawned crew pane now enters its leased slot with a plain `cd` in its top shell, so the nesting no longer comes from a spawn, but the classifier still has to handle it.
