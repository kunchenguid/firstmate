...
So a **relaunch** preserves the binding instead of fighting it: before the Pi-family launch line is composed, `bin/fm-spawn.sh` reads the recorded pane's session reference through `fm_backend_herdr_pane_agent_session_ref` - for a crewmate or scout, before that pane is replaced by one created in its worktree - and passes it back as Pi's own `--session <path-or-id>` (`relaunch_resume_args`; `bin/fm-control-lib.sh`'s `fm_control_relaunch_resume_flag` owns which adapters and which registration labels qualify).
