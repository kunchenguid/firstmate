...
It defaults to the fleet-wide `FM_COMPOSER_AGENT_GLYPHS` rather than to the built-in glyph set, so widening the fleet-wide knob widens herdr with it; set the herdr knob only to give this backend a set the rest of the fleet does not share.
Defaulting to the built-in set instead would make herdr the one backend that ignores the fleet-wide knob, and since herdr hands its own set to the shared classifier it would override the fleet-wide value there too, leaving a glyph every other backend reads as an empty composer stuck on pending here.
Shell prompt glyphs (`>`, `$`, `%`, `#`) are rejected from either set with a loud stderr warning, because a bare shell prompt is a dead shell rather than an empty agent composer.
The rejection is not the only guard, and must not be treated as if it were: the shared glyph matcher also refuses to match a shell glyph whatever set it is handed.
That refusal is what protects the structural row scan below, which has no classifier beneath it to catch a mistake.
A shell glyph reaching that scan would make a dead-shell prompt row register as a bare composer row, and a dim or dark-truecolor prompt (an ordinary muted theme) then ghost-strips to nothing and reads empty, handing the away-mode injector a live shell to type into.
That rejection never leaves the set empty: a set with no agent glyphs left in it falls back to the built-in `❯ ›`, also with a warning, because an empty set would strip no glyph at all and leave every idle claude or codex composer reading pending forever.
...
**The glyph sets (2026-07-13).** The AGENT glyph set is configurable, the SHELL glyph set is not, and both are matched as literal glyphs rather than as a regex.
`FM_COMPOSER_AGENT_GLYPHS` (fleet-wide, default `❯ ›`) and herdr's own `FM_BACKEND_HERDR_BARE_PROMPT_GLYPHS` (default: the fleet-wide set) are whitespace-separated lists of literal glyphs.
A backend that recognizes a further harness's prompt glyph hands the same set to the shared classifier, so its structural row detection and this classification can never disagree; a glyph honored for detection but not for classification would leave that harness's idle composer reading `pending` forever, and away-mode would defer every escalation behind input that was never there.
A regex form is deliberately unavailable: under the C/POSIX locale `bin/fm-supervise-daemon.sh` runs with, an ERE bracket class over multibyte glyphs degenerates into a byte class, every box-drawing glyph starts with byte `E2`, and a composer box's own border row then matches as a bare prompt row.
Literal glyphs compared with a `case` are locale-invariant, which leaves the per-harness idle placeholder (`FM_COMPOSER_IDLE_RE` and the per-backend `*_IDLE_RE` knobs) as the one remaining regex; it is matched under a pinned `LC_ALL=C` so a hand-tested pattern behaves the same inside the daemon, and it must be written glyph-free, or naming a glyph as a literal rather than as a bracket class.
Shell glyphs cannot be configured into the agent set, in three layers: a shell glyph in any set read from the environment is rejected with a loud stderr warning; `fm_composer_leading_agent_glyph` refuses to MATCH a shell glyph whatever set it is handed; and `fm_composer_classify_content` evaluates the shell rule before the configurable agent match.
The middle layer is the load-bearing one, because it is the single chokepoint every consumer goes through, including a backend's structural row scan - which has no classifier beneath it, so a dead-shell prompt row reaching it would register as a bare composer row and, rendered dim or dark-truecolor, ghost-strip to nothing and read `empty`.
That rejection never leaves a set empty: a set with no agent glyphs left in it falls back to the built-in default, warning again, because an empty set strips no glyph at all and leaves every idle claude or codex composer reading `pending` forever - the same wedge from the other direction.
All three layers read the shell set through the single `FM_COMPOSER_SHELL_GLYPHS` owner in `bin/fm-composer-lib.sh` and nowhere else, so teaching the fleet a further dead-shell prompt glyph is a one-line edit that cannot leave a layer behind.
`FM_BACKEND_HERDR_BARE_PROMPT_RE`, the retired regex spelling of herdr's knob, is no longer honored and warns loudly on stderr.
...
The glyph-set contract is pinned there too: `test_glyph_strip_is_locale_invariant`, `test_caller_supplied_glyph_set_is_honored`, `test_shell_glyph_cannot_be_configured_into_agent_set`, `test_shell_only_glyph_set_falls_back_to_default`, `test_shell_glyph_set_has_one_owner`, and `test_idle_regex_match_is_locale_pinned`.
`tests/fm-backend-herdr.test.sh` pins herdr's thread-through of that set: `test_composer_state_border_row_is_not_a_bare_prompt_under_c_locale`, `test_composer_state_fleet_wide_glyph_knob_layers_into_herdr`, `test_composer_state_herdr_glyph_knob_overrides_fleet_wide`, `test_composer_state_shell_glyph_is_never_a_bare_composer_row`, and `test_bare_prompt_re_is_retired_loudly`.
