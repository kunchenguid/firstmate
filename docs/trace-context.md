...
The injection sits at the unconditional pre-launch export site, so it covers every ship and scout spawn `bin/fm-spawn.sh` performs on any adapter, plus every Secondmate spawn it accepts.
The crewmate/scout-only adapters it refuses for a Secondmate are owned by [`docs/configuration.md`](configuration.md#harness-support).
