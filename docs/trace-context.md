...
The dormant candidate `omp` adapter is the one deliberate exception: its candidate launch contract forces trace off and strips `TRACEPARENT`, while `bin/fm-spawn.sh` refuses every runnable selection before mutation until ATX-2170 verifies First Mate lifecycle control; [`configuration.md`](configuration.md) owns that dormancy boundary.
