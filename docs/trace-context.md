...
Ship and scout spawns reach that site on every pane-based spawn backend (`tmux`, `herdr`, `zellij`, `orca`, `cmux`), while a `t3` thread has no pane shell and receives the same value through its worktree's Claude settings `env` map ([`t3-backend.md`](t3-backend.md)); a Secondmate reaches it on every backend that accepts a Secondmate spawn (`tmux`, `herdr`, `zellij`), because `bin/fm-spawn.sh` rejects a Secondmate on `orca`, `cmux`, and `t3`.
