...
The dormant candidate `omp` adapter is the one deliberate exception: its candidate launch contract forces trace off and strips `TRACEPARENT`, while [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) owns the refusal of every runnable selection before mutation until ATX-2170 verifies First Mate lifecycle control.
