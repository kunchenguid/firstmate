...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns across `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, `gemini`, `muse`, `rovo`, and `agy`, plus Secondmate spawns across that same set except always-crewmate/scout-only `rovo` and, unless that home's [`config/unverified-secondmate-harness`](configuration.md#unverified-secondmate-harness-configunverified-secondmate-harness) accepts them, `gemini`, `muse`, and `agy`.
