...
The dormant candidate `omp` adapter is the one deliberate exception: its candidate launch contract forces trace off and strips `TRACEPARENT`, while [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) owns the refusal of every runnable selection before mutation until both the supported session-free OMP 17.2.9 consumer-containment proof and the separate ATX-2170 First Mate lifecycle-control proof pass.
