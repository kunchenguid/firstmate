...
It also pins the states in between - a live deadline, an accepted termination the listing does not confirm, an unreadable pod list, a pod id that was never in the account, a ceiling anchored to the pod's own start that re-arming cannot extend, a carried anchor already spent that terminates nothing on first sight, a re-arm whose `status` reports `unresolved` instead of the retired watch's figures, a ceiling whose anchor cannot be read and is therefore not enforced, an unseen pod's decision that keeps naming a pod still missing when another one turns up, alarms that leave a crewmate's open decision intact, a verified stop that leaves no open decision at all, an alarm the watchdog raises and then closes itself once the API comes back, a shared `.env` whose neighbouring values cannot take the credential away, a corrupt pid file that cannot signal the caller's process group, the credential never reaching a written file, and the armed watchdog surviving the death of the shell that armed it.
