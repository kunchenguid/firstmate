...
The [authoritative worktree lifecycle](architecture.md#worktrees-not-branches-in-your-checkout) now gives the pane a plain top-level `cd`, which this field tracks; the marker probe stays in place regardless as the already-proven discovery mechanism.
