...
Limitations: zellij is experimental, has no per-home workspace split (all tasks share one tab bar, unlike herdr), has no verified agent-process liveness classifier for an existing secondmate pane, and still carries the known gaps documented below (no native busy-state signal and a narrow focus-steal race on tab creation) - see "Known gaps left for a follow-up" at the end of this document.
...
| Agent liveness | Session and pane/tab inventory only | `fm_backend_agent_state` keeps a label-owned task tab with any terminal pane `ambiguous`, but reports `missing` after a readable session inventory omits the recorded session or a readable tab inventory contains no home-scoped or uniquely legacy task-label match. Numeric pane and tab IDs are treated only as correlators, so reuse by a foreign tab cannot mask absence or authorize touching that foreign tab. A uniquely label-owned task tab with no terminal panes is `dead`; session start resolves and closes that exact ghost tab before relaunch even when its current tab ID differs from metadata. Inventory failures and duplicate task-label matches remain `unreadable`, so they never authorize recovery. |
| Kill | `zellij action close-tab-by-id <id>` (tab id resolved fresh from the pane id when possible; callers pass recorded `zellij_tab_id` plus the expected caller-facing `fm-<id>` label when the pane is already gone) | Unlike herdr (where closing a tab's only pane also closes the tab), closing a zellij pane with `close-pane` does NOT close the now-empty tab - it survives as an empty "ghost" entry in `list-tabs`. `close-tab-by-id` on a LIVE tab (with its pane still running) verified to cleanly remove both pane and tab in one call. Kill resolves the owning tab and closes by tab id; if an expected label is supplied, every numeric candidate must match through the home-scoped-title or unambiguous legacy-title check, and a stale numeric ID falls back to the unique current label-owned tab. Best-effort (`\|\| true`), matching tmux's `kill-window` and herdr's `pane close` contract. |
...
This is why `fm_backend_zellij_kill` resolves the owning tab id from the pane when possible, accepts a recorded `zellij_tab_id` as a fallback when the pane has already gone, resolves a unique current label-owned tab when numeric IDs are stale, verifies the expected caller-facing `fm-<id>` label through the home-scoped-title or unambiguous legacy-title check, and calls `close-tab-by-id`, rather than mirroring herdr's simpler "close the pane, the tab follows" contract.
...
- **No verified agent-process liveness classifier for an existing terminal pane.** The session-start secondmate liveness sweep therefore leaves every existing Zellij pane untouched unless structural inventory proves that the recorded task tab is absent or is an empty ghost.
  A bare shell or ambiguous process inside an existing pane still requires manual recovery, avoiding the worse failure mode of duplicating a live supervisor.
