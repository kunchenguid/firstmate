...
| Agent liveness | Session and pane/tab inventory only | `fm_backend_agent_state` keeps an existing label-owned task tab `ambiguous`, but reports `missing` after a readable session inventory omits the recorded session or a readable tab inventory contains no home-scoped or uniquely legacy task-label match. Numeric pane and tab IDs are treated only as correlators, so reuse by a foreign tab cannot mask absence or authorize touching that foreign tab. A verified task tab with no terminal panes is `dead`; session start closes that exact label-checked ghost tab through its recorded tab ID before relaunch. Inventory failures and duplicate task-label matches remain `unreadable`, so they never authorize recovery. |
