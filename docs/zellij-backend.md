...
`pane_cwd` follows a top-level shell `cd`, which is how a spawn enters its leased Treehouse slot, but not a nested foreground subshell such as the one an interactive `treehouse get` or `treehouse enter` opens.
