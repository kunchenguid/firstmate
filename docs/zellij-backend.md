...
A `--secondmate` launch is refused outright, before any secondmate home sync or inherited local-material propagation, because Zellij's plain `dump-screen` capture cannot strip claude's ghost text from a secondmate composer.
[`configuration.md`](configuration.md#runtime-backend-configbackend--fm_backend) owns that refusal contract, and the harness-adapters skill owns the ghost-text reasoning behind it.
No legacy Zellij secondmate is expected, because this backend is experimental and explicit-only, and a pre-existing one can still be torn down and recreated on tmux.
