...
Herdr later adopted that same shared derivation for its per-home workspace labels (`docs/herdr-backend.md` "Label derivation"), so all three adapters now resolve one tag from one place and cannot drift apart; a change to the hash length, algorithm, or fallback order moves zellij's tab titles, cmux's workspace titles, and herdr's workspace labels together.
