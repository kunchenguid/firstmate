...
| Agent liveness | Session and pane/tab inventory only | `fm_backend_agent_state` keeps every existing terminal pane `unverified` or `ambiguous`, but reports `missing` after a readable session inventory omits the recorded session or a readable task-tab inventory omits the recorded tab. A verified task tab with no terminal panes is `dead`; session start closes that exact label-checked ghost tab through its recorded tab ID before relaunch. Inventory failures and metadata contradictions remain `unreadable`, so they never authorize recovery. |
