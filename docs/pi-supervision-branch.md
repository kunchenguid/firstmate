...
- Committed-ready delivery: `bin/fm-delivery-continue.sh` is the narrow leased owner for a `ship` task's recorded `mode=no-mistakes` committed receipt.
  ...
  The shared Pi delivery preflight scans both relevant unread wakes and durable committed-ready task state before startup or watcher intake presents the work.
  A pending captain inbox note defers continuation until that explicit request is presented and acknowledged, while an active origin or inventory captain hold, keyed decision, or blocker refuses publication.
  Queue-backed results remain bound to the exact sequence numbers inspected, and a durable-only transient result is requeued so lease, attribution, captain-hold-state, or inbox-delivery failure cannot strand the transition.
...
- Consistency: `bin/fm-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/fm-lease.sh` is the command surface.
  ...
  Pi assigns distinct operation owners to MAIN and branch turns and releases only that exact owner's leases when each run settles, while generation activation clears only legacy and operation-owned turn leases and preserves continuation custody.
