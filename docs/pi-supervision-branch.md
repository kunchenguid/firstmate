...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every row other than a check row is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); a decision-owned signal or stale row counts as resolvable here too - its own task must still be known even though its content stays excluded from what the branch may claim - so an unresolvable decision-owned row still defers the whole heartbeat to main exactly like an ordinary unresolvable row. Every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
  ...
  Heartbeat keeps its own all-or-nothing recheck over the rows it can claim: it takes every branch-ownable unread row or none of them, and an unresolvable task-local row - a decision-owned signal or stale row included, since it must still resolve to a known task even though its content is excluded from the claim - still defers the whole review to main.
...
A heartbeat is never vetoed or ridden into main by a co-present check row, or by a decision-owned signal/stale row whose own task still resolves.
...
What all-or-nothing still guarantees is unchanged: the branch takes every branch-ownable unread row or none of them, and an unresolvable task-local row, an unknown row kind, or an unreadable queue still defers the whole review to main - a decision-owned signal or stale row included, since excluding its content from the claim never excuses skipping the check that its own task still exists.
