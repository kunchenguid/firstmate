...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  The repeated cleanup reminder for a current-state-proven completed scout is one such check: while attended the branch may report the completion outcome, but main owns the report review, captain-call inventory, and guarded teardown until metadata retirement stops the reminder.
...
`tests/fm-inactive-reconcile.test.sh` covers current-state-proven completed-scout cleanup checks, acknowledgement replay, metadata retirement, report integrity failures, relaunch history, trailing decision resolution, and unchanged ship reconciliation.
