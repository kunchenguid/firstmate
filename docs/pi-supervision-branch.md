...
A no-change review explicitly reported with `silent=true` (a fleet-wide heartbeat or a task-local wake that changed nothing and took no action) is delivered silently with no rendered note, while every other `routine` outcome stays rendered with its sailboat prefix.
...
A review that found literally nothing worth reporting uses verdict `routine` and `silent=true` so it has no rendered note, with `task=fleet` for a fleet-wide heartbeat or the wake's own task for a task-local wake, while a routine action omits `silent` and keeps its rendered sailboat note.
