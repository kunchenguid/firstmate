...
Its credential-free deterministic processing probe also exercises active delivery behind an idle next-prompt copy, stale-copy exclusion in real model context, and refused versus successful acknowledgements through the real agent-core tool executor.
`test_new_captain_outcomes_reactivate_idle_next_turn_without_overlapping_requests` in the portable extension suite covers multiple arrivals, triggered versus consumed requests, retry and preflight intervals, partial acknowledgement, run settlement, replacement, and preserved durable rows.
