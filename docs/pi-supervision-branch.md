...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  After wake rows are claimed, a branch prompt counts as handled only when `fm_branch_report` appends a durable outcome before that prompt settles; every settled turn releases the branch actor's per-task leases through `bin/fm-lease.sh release-actor`, while a settled provider error or a settled prompt with no report also releases the wake grant and rejects delivery ownership back to the watcher.
  ...
  Two consecutive settled provider errors latch the branch broken and surface a one-line health note only on that initial trip; the settlement cleanup releases any per-task leases the branch claimed before it stopped acting, so main can reclaim finished work without a session restart.
