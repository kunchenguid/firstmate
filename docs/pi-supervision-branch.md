...
- Committed-ready delivery: `bin/fm-delivery-continue.sh` is the narrow leased owner for an eligible `ship` task's recorded `mode=no-mistakes` committed receipt. The extension invokes it before the branch receives the wake, so routine classification follows a durable instruction, an attributed pipeline, or a typed refusal rather than model compliance. Its inbox delivery never grants merge authority.
