...
- Committed-ready delivery: `bin/fm-delivery-continue.sh` is the narrow leased owner for a `ship` task's recorded `mode=no-mistakes` committed receipt.
  The primary Pi watcher invokes it at the common wake-dispatch boundary before choosing main versus branch, binds the result to the exact queue sequences it inspected, retains a live-lease conflict as a retry obligation, and dispatches only after the result is a durable instruction or a proven stop.
...
- Consistency: `bin/fm-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/fm-lease.sh` is the command surface.
  ...
  Pi assigns distinct operation owners to MAIN and branch turns and releases only that exact owner's leases when each run settles, while generation activation retains a broad same-actor recovery sweep.
