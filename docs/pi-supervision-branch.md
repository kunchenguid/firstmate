...
A routine outcome whose summary is semantically unchanged from the SAME task's most recent routine outcome is filtered before it reaches main: its durable row is stored and its cursor advanced, but no merge note enters main's model context, so an unchanged operational state that the branch observes again cannot spend the lead model's context on text that says nothing new.
The first occurrence is always delivered (a task's first routine row has no predecessor), and a materially changed summary, a changed `silent` flag, a captain verdict, and every decision, failure, or safety escalation always deliver.
`bin/fm-branch-outcome.sh repeated --seq <seq>` is the comparison's single owner; it ignores case and whitespace runs and treats an unreadable store as "not a repeat", so an uncertain read errs toward delivering rather than silently swallowing a note.
