...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  The branch loader keeps `noExtensions: true` so project extensions and settings packages never load, but when the branch model uses the Cursor provider it adds only the installed `pi-cursor-sdk` extension through `additionalExtensionPaths`, resolved from the live Pi agent dir (see docs/configuration.md "Pi supervision branch model and effort").
  Because `createAgentSession` resolves its initial model before extension providers register, the branch then re-selects the configured model from `/supervision-model` or main's settings and applies it with `setModel` once the Cursor provider is available.
