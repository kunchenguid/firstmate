...
A provider an extension registered only into main's runtime, such as pi-devin-auth's `devin`, reaches the isolated branch runtime by copying that registration across at model-resolution time, so a custom `streamSimple` transport works unchanged and the branch can pin or follow a devin model.
