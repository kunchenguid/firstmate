...
- The branch itself: `.pi/extensions/fm-branch-supervision.ts` creates and reopens the persistent branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  Two consecutive settled provider errors latch the branch broken and surface one health note.
  Main keeps every wake during a five-minute cooldown, after which one wake may probe the branch while concurrent wakes still stay on main; each failed provider probe doubles the next cooldown up to one hour.
  A durable `fm_branch_report` from the current branch generation and model or effort selection clears both the latch and provider-error streak and surfaces one recovery note, while a session replacement or branch model or effort change resets the recovery state immediately.
