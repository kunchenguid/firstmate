...
Captain-relevant branch outcomes persist as exact, sequence-keyed visible transcript entries; `action:main` (and legacy) rows then open one sequence-keyed processing turn on main that stays open until main acknowledges that sequence, while display-only `action:none` finished results stop at the visible entry.
...
Stage one remains the bash watcher, which absorbs everything provably fine at zero token cost, including already-handled outcome-covered bare turn-ends and terminal-status re-hashes ([architecture.md](architecture.md)).
...
The captain entry contains the store sequence, task, verdict, exact summary, silent flag, and processing `action`, and its renderer presents the exact task and summary with an anchor prefix.
...
Display is only half of a captain outcome that still needs main action; the other half is processing, because a blocker, a decision, or work main must still drive needs main to act, not only the captain to see it.
A captain outcome carries `action: "main" | "none"` (legacy and omitted fields read as main): `none` is display-only finished delivery whose anchor entry is enough, while `main` keeps the processing turn.
After the visible entry exists and the read cursor has passed it, the extension closes leading `action:none` rows through `bin/fm-branch-outcome.sh mark-processed --through N --display-only` with no main turn, and hands every still-unprocessed `action:main` captain row to main as one hidden, typed `fm-branch-process` request (kind `branch-outcome`) listing each `[seq N] task: summary`, and that request opens exactly one main turn.
Main closes an `action:main` request only by calling `fm_branch_processed` with the highest sequence the request listed, which advances a processed marker that `bin/fm-branch-outcome.sh` keeps separately from the read cursor and never moves past it or backwards.
