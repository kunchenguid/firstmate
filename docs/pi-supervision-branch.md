...
The [branch prompt](../bin/fm-branch-prompt.sh)'s "Verdict: routine or captain" section owns the verdict and no-change criteria, including requested results, progress updates, and retained previews.
...
Outcome disposition follows the [two-stage noise filter](#two-stage-noise-filter).
...
`test_retained_preview_no_change_delivery` in `tests/fm-pi-branch-extension.test.sh` exercises explicitly supplied silent task reports, durable storage, delivery-cursor advancement, continued HTTP preview availability, and visible captain reports; it does not test automatic model classification or wake acknowledgement.
