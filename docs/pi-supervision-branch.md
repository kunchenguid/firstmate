...
Its "PR identity: copy or abstain" section owns where a PR URL in a summary or tool argument may come from: a durable record, verbatim, or else only the identifier the branch actually has, with `bin/fm-pr-check.sh` refusing a URL the task's own ready line contradicts.
