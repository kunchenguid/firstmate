...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  `bin/fm-watch-arm.sh` refuses branch-owned arm and restart modes after parsing while preserving branch handling confirmation, so shell syntax or argument expansion cannot bypass that main-owned repair path.
...
- Consistency: `bin/fm-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/fm-lease.sh` is the command surface.
  The guards are wired into `fm-send.sh`, `fm-control.sh`, and `fm-teardown.sh` (overlap, lease-checked, with claim serialization retained through the mutation), into `fm-pr-merge.sh`, `fm-merge-local.sh`, and `fm-spawn.sh` (main-owned, branch refused; a relaunch through `fm-control` stays branch-legal recovery), and into the watcher repair boundary described under Wake dispatch above.
...
`tests/fm-watch-arm.test.sh` covers branch-side bare and explicit arm refusal, explicit and variable-expanded restart refusal, and allowed branch handling confirmation at the parsed script boundary.
