...
Every completed scout signal or stale trigger also stays on main, which owns report delivery, the captain-call completion gate, and guarded cleanup as one lifecycle.
...
- Wake dispatch: `.pi/extensions/fm-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/fm-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  It also excludes every signal or stale row whose current task metadata says `kind=scout` and whose latest meaningful status is `done`, leaving the main session to follow the scout-completion contract in `AGENTS.md` section 7 rather than accepting a branch outcome as the end of fleet handling.
...
- Autonomy: supervision is default-on for every task once a Pi primary session owns the fleet lock (docs/configuration.md "Pi supervision branch"); no captain grant file is required.
  A fleet-wide heartbeat is separately eligible only when every row other than a check or main-owned signal/stale row (a decision event or completed scout lifecycle event) is a heartbeat row or a resolvable task-local row (see "Heartbeat routing" below); every other fleet-wide or unresolvable wake, and every watcher-failure alarm, stays on main.
...
A heartbeat is never vetoed or ridden into main by a co-present check row or main-owned signal/stale row (a decision event or completed scout lifecycle event).
...
Its direct dispatch classifier case covers decision-owned and completed-scout signal/stale rows' exclusion from `eligibleSeqs`, active-scout branch eligibility, the single `mainOwnedKeys` index, task alias resolution, reserved-key configuration, status-log race and symlink refusal, non-vetoing behavior for unrelated eligible rows, and main-only queues reading as ordinary absence.
