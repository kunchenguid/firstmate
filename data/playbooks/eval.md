...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript under the active worktree's run-transcript location (the harness names this path). Do not glob across other sessions' private logs; that crosses worktree boundaries and reads private logs from unrelated work. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
