...
**You own the program, never the code. Author briefs, drain the queue, keep the frontier green, decide.** For a whole project handed to one standing coordinator chat: multi-day, many stacked PRs, dozens to hundreds of workers, the supervisor checking in twice a day instead of every five minutes. One task driven to a predicate is Autonomous run. One ambitious run needing a bespoke workflow is a custom plan built from multi-phase-plan.md. Route here when the work outlives any single agent. Work one agent could finish inside the session's budget is not a program; measured head-to-head, this playbook's ceremony turned a half-hour 12-unit job into 1 landed unit while a plain agent landed all 12. Below that line, route to Autonomous run.
...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the supervisor report, makes judgment calls. It never authors or edits code and never merges: conflicted merges, restacks, landings, and code changes are always tasks or merge-authority actions. The coordinator never lands work itself: landing belongs to the merge authority after its explicit go-ahead. Queueing finished work without a landing decision is how a deadline harvests nothing. The loop is agentic end to end. Workers are spawned, resumed, and drained only through the supervisor's dispatch plane. State reads and writes go through the supervisor's program ledger at drain points, one update in and one line out, to conserve context.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting works to depth 3, and a nested spawn has the full spawn schema including `environment`). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Always `environment: "worker"` unless the task needs this machine: runtime verification that drives the real surface; reading local transcripts under `run transcripts/`; simulators and local machine state; auth that exists only here. Workers receive self-contained briefs that inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker where the fleet has one.

Depth stays at coordinator, track, worker. Author the track decomposition per project (build, landing, and verification are common cuts, not a required shape); hard-coded fan-out trees were tried and parked as too rigid.

#### Store layout

Create `orchestrate/<project-slug>/` in the current agent's store (path the harness names). Every file has exactly one writer; owners publish facts, readers aggregate at read time. Keep bookkeeping in the program ledger's plain TSV and JSON so it stays readable without any CLI.

- `preferences.md` is the standing-orders register: numbered lines, one constraint each (model policy, stack shape and count, verification bar, forbidden paths, escalation policy). Paste it verbatim into every spawn and every resume; directives decay across resumes, and each dropped one costs a supervisor turn. When you catch yourself restating an instruction, append the line before you act (principle-encode-lessons-in-structure).
...
- `inbox/` holds completion pointers. `gates.md` parks supervisor gates (question, options, default on no answer) so a completion flood cannot wipe decision state.
...
2. **Install the runtime.** Initialize the program ledger (`ledger.tsv`, `units.tsv`, `frontier.json`). Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs.
...
- On a completion notification, record `<agent> <unit> <status> [--report PATH]` in the program ledger and return to what you were doing. Never deep-review inline; a completion that needs review becomes a verifier unit. Never review a diff inside a drain.
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (armed as a supervision wait, with a long heartbeat fallback), and before a supervisor report. Begin each batch by draining the ledger inbox. Arrivals during a drain wait for the next one.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), records the resulting rows in the ledger (`units.tsv`, `ledger.tsv`), then spawns the next wave in one message.
...
- A drain turn ends with a three-line status: counts against the states, what changed, gates open. Detail lives in `status.md`; the full reply contract applies at checkpoints and close.

#### Stack safety

- The frontier is a computed object, never narrative. Recompute the frontier from the forge after every merge and stack mutation because base refs drift mid-restack while the stack tracker is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where the stack tracker knows the stack, normally the stacker's clone; a checkout whose local tracking never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may touch stack topology, serialized within its stack; record the holder in the standing orders. Large restacks run as their own task, never inline in a supervision pass.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the worker's status in the supervisor status view. Transcript mtime is not liveness.
...
- After a session restart: local passes are dead, spawned work continues. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach spawned work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; the ledger replaces a lock whose holder pid is gone.

#### Escalation

Reaches the supervisor, batched into the status page rather than per item: irreversible actions (force-push to shared branches, deploys, deletions, closing someone else's PR), genuine product or preference calls no experiment settles, a standing order that contradicts observed reality, a program-level dead end that survived a replan. Park each as a `gates.md` entry before asking, and route work around it.

Never reaches the supervisor: frontier nudges, restack mechanics, retries, CI flake triage, review-thread triage, format fixes, scope the brief already forbids (refuse and continue), and "should I keep going". When in doubt, act and log; deferring is the measured failure mode.
...
**Reply:** at checkpoints and close: the predicate and the count against it from `units.tsv` and `ledger.tsv`, tracks and what each landed, the frontier (PR list plus SHAs), verdicts summary, what was abandoned and why, gates awaiting the supervisor (the only asks), the store path, and the trail path. Numbers from the tables, not narrative. Include PR links.
