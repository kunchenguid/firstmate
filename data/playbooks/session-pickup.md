...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", a handoff URL, or a pushed branch you are meant to continue.
...
1. Locate the prior trail. A local transcript under the active worktree's run-transcript location (the harness names the path; do not glob across other sessions' private logs, that crosses worktree boundaries and reads private logs from unrelated work), a handoff URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Summarize a long transcript into a reduced timeline and keep that timeline in the main thread (the **principle-guard-the-context-window** skill).
