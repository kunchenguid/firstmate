...
tests/fm-brief.test.sh                    # fm-brief.sh bash -n parse regression guard (issue #166) and clean no-mistakes/direct-PR/local-only/scout/secondmate brief generation tests
...
tests/fm-bash32-parse.test.sh             # repo-wide bash -n parse sweep over bin/, bin/backends/, and tests/, using /bin/bash 3.2 on macOS; paired with ci.yml's docker bash-3.2 job on Linux
...
tests/fm-gotmp.test.sh                    # per-task GOTMPDIR temp root: fm-spawn's tasktmp= meta and pane-export contract, and fm-teardown's temp-root cleanup
...
tests/fm-backend-tmux-smoke.test.sh       # real (private-socket) tmux smoke test for the tmux adapter: create/duplicate-refuse, all-digit session name targeted by name not window index, send text + Enter, send literal + key, bounded capture, live-window resolve, kill
