...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, pinned actionlint workflow lint, and the tasks-axi interface boundary that keeps the shell surface off the beads CLI directly), and both CI and the no-mistakes pre-push gate invoke it with no arguments.
