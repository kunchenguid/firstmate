...
Local `.no-mistakes/` state and test evidence stay out of this repo; `.no-mistakes.yaml` keeps evidence in a temp directory and pins the gate's lint command to `bash bin/fm-lint.sh`, matching the Linux CI lint job.
The interpreter prefix is required, not cosmetic: no-mistakes runs that string through cmd.exe on Windows, which cannot exec a POSIX script path.
