...
Reach the real `tasks-axi` through `tests/lib.sh`'s `fm_test_tasks_axi`, never by invoking it directly as a bare command or through a resolved path: an unbounded call lets a wedged backlog backend hang the whole run until CI kills it instead of failing the case.
A path resolved only to probe whether `tasks-axi` is installed, or to hand a fake shim the real binary the subject under test then runs under its own bound, is not such a call.
