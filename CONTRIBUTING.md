...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `scripts/`, `.agents/skills/`, and `skills/`.
...
- Helper scripts in `bin/` and `scripts/` are plain bash.
  Each starts with a usage header comment; keep it accurate when you change behavior.
  Test scripts and helpers in `tests/` are plain bash too.
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and pinned actionlint workflow lint).
  ...
  Its header and `--help` output own the exact local lint modes and flags, while `.github/workflows/ci.yml` and `bin/fm-ci.sh` own the hosted-primary and fallback invocation policies.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `scripts/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and require an explicit merge approval.
...
bin/fm-lint.sh   # full local lint for the toolbelt, behavior tests, and GitHub workflows via pinned actionlint
...
bin/fm-test-run.sh --changed   # changed-file-informed path with automatic bounded concurrency
...
Shared test helpers live in `tests/lib.sh` for reporters, temporary roots, and git fixtures; `tests/fixtures.sh` for fake toolchains and spawn-world builders; and the subject-specific helper files.
Source those helpers instead of copying a fake toolchain into a new suite.
