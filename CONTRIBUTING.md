...
- Helper scripts in `bin/` are plain bash.
  ...
  Scripts must also RUN under the stock macOS Bash 3.2.57, not merely parse there, and the `macos-stock-bash` CI lane enforces that on the suites it lists.
  The trap that has caught this repo repeatedly is `errexit`: Bash 3.2 does not apply it to a redirection that fails to open on a compound command (`{ ...; } > "$f"`, `while ...; done > "$f"`), so the body is skipped and the script runs on, while Bash 5.x aborts.
  Create the file with a simple command first (`: > "$f" || { ...; exit 1; }`) and then fill it, because simple-command redirections fail identically on both versions.
  Do not guard the compound form with `|| ...` or `if ! ...`: a group's exit status reports its last command rather than the redirection, and a redirection failure is not negated by `!` on either version.
