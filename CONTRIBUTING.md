...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-install-shellcheck.sh <directory>` installs exactly that pinned build, checksum-verified, on macOS and Linux for both aarch64 and x86_64, so lint is verifiable locally without a package manager.
