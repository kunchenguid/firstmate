...
**Direct pull requests are allowed only when both the head and base are the private `vaskarvelas/firstmate-private` repository.**
Pull requests targeting any other repository, including the original upstream repository, are forbidden.
Human-authored pull requests from an external head repository must be raised through [`no-mistakes`](https://github.com/kunchenguid/no-mistakes) and must still target this private repository.

`no-mistakes` puts a local git proxy in front of your real remote.
Pushing through it runs an AI-driven review/test/lint pipeline in an isolated worktree, forwards the push to the configured private-fork target only after every check passes, and opens a clean PR automatically.

A GitHub Actions check (`Require no-mistakes`) first rejects any base other than the exact private repository.
It accepts same-repository direct pull requests and otherwise requires the deterministic signature that `no-mistakes` writes.
...
GitHub Actions and Dependabot are exempt so their automation keeps working.

## Workflow

1. Verify that `origin` resolves to `vaskarvelas/firstmate-private` for both fetch and push.
2. Create a feature branch and make the change.
3. Commit and validate the change.
4. Push the feature branch to that private `origin`.
5. Open a pull request from the feature branch into `main` in the same private repository.
6. Merge only after the required checks and approval pass.

External-head contributors must use the [no-mistakes quick start](https://kunchenguid.github.io/no-mistakes/start-here/quick-start/) and configure `vaskarvelas/firstmate-private` as the pull-request target.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux` plus experimental `herdr`, `zellij`, `orca`, `cmux`, and `codex-app`.
  It does not make `data/` tracked.
- Helper scripts in `bin/` are plain bash except for the Node.js standard-library WebSocket client used by the Codex App backend.
