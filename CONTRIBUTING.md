...
tests/fm-brief.test.sh                    # fm-brief.sh bash -n parse regression guard (issue #166), clean no-mistakes/direct-PR/local-only brief generation tests, and the no-mistakes brief's merge-safety contract (never-merge, honest CI-posture done lines, do-not-fight-a-merge)
...
tests/fm-pr-check.test.sh                 # fm-pr-check.sh merge-attribution arming: pr_checks= recording (0/N/unknown, re-arm replacement), generated check.sh outcomes (attributed silent, unattributed-merge once per distinct outcome, unknown warns, sibling-PR marker disambiguation), and the heartbeat fleet-scan helper
tests/fm-pr-merge.test.sh                 # fm-pr-merge.sh records pr= and available pr_head= before merging, writes the pre-merge merged_by_firstmate= attribution marker and rolls it back on a failed merge, records best-effort merged_commit=, refuses an unattended yolo merge with unconfirmed CI unless --captain-approved, parses PR URLs into gh-axi number/--repo calls, defaults to squash, preserves explicit merge methods, rejects malformed URLs and repo overrides, and propagates real merge failures
