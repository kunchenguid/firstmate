...
- Helper scripts in `bin/` are plain bash.
  ...
  The rendered-DOM assertions in `tests/fm-calm-pi-extension.test.sh` require a Chrome or Chromium executable native to the test host; put it on `PATH`, or set `FM_CHROME_BIN` to its absolute executable path.
  A Windows Chrome executable reached through WSL interoperability is not a substitute because its file-URL and profile-path semantics differ from a native Linux browser; install Chrome or Chromium inside the WSL distribution before running that test.
