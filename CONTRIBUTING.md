...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  The root `.tasks.toml` is tracked `tasks-axi` config for default-home `data/backlog.md`; `bin/fm-tasks-axi.sh` binds routine commands to the active home's backlog, with the compatibility definition owned by [`docs/configuration.md`](docs/configuration.md) ("Backlog backend").
