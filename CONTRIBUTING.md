...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml.example`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  [`docs/configuration.md`](docs/configuration.md) ("Backlog backend") owns per-home tasks-axi configuration, template materialization, and compatibility requirements.
...
- Cleanup authority spans the refusals in `bin/fm-teardown.sh`, the kind-labeled candidate report in `bin/fm-fleet-view.sh`, and the retirement contract in `.agents/skills/secondmate-provisioning/SKILL.md`; a change to one of those keeps the other two accurate, because retiring a persistent secondmate home takes authority naming that exact home and must never follow from a path-matched target list.
