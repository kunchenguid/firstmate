...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-install-shellcheck.sh <destination-directory>` installs exactly that pin for the host platform, verifying the official release SHA-256 and refusing unless the installed binary reports the pinned version; it covers Linux x86_64, aarch64, armv6hf, and riscv64 plus macOS x86_64 and aarch64, and CI uses the same script.
