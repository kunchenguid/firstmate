...
When Firstmate coordinates a tracked code change, a second mate coordinates separate implementation and read-only review workers through the single task PR; the agent-only [`nathan-coding-loop` skill](.agents/skills/nathan-coding-loop/SKILL.md) owns that loop without changing human contribution intake or merge authority.
