...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, and `.agents/skills/`.
  ...
  A local `config/backend` file selects the runtime session-provider backend for new tasks and stays gitignored; accepted values are `tmux` and experimental `herdr`.
...
tests/fm-tangle-guard.test.sh             # primary-checkout tangle detection, spawn/brief isolation, and fm-spawn tmux window construction (append-form creation, name pinning, window-id targeting) tests
