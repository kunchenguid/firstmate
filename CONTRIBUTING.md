...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  Everything personal to one captain's fleet (`.env` and its local key backups, `data/`, `state/`, `config/`, `projects/`, `.no-mistakes/`) is gitignored; never commit it.
  The root `.gitignore` owns the exact patterns, and `tests/gitignore-env-bak.test.sh` pins the key-backup rules so a plain `git add -A` cannot stage a secret.
