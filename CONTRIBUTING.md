...
4. Push your branch to your fork and open a PR against the parent repo.
5. `.github/workflows/ci.yml` runs lint and tests on the PR; product-facing, mixed, or uncertain changes also get a Golden Rules review pass before merge (see [`docs/configuration.md`](docs/configuration.md) "Nubank Best Practices").
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through `direct-PR` on a feature branch, with a Golden Rules review pass for product-facing changes, and require an explicit merge approval.
