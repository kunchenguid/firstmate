...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; [`docs/configuration.md`](docs/configuration.md) ("Runtime backend") owns the accepted spawn-capable values, and `codex-app` is documented only in `docs/codex-app-backend.md`.
