...
tests/fm-watcher-lock.test.sh             # watcher singleton, lock-race, PID identity stability, watch-arm liveness, Codex degraded-wake diagnostics, and guard-warning tests
tests/fm-turnend-guard.test.sh            # shared supervision predicate plus Claude/Codex Stop-hook scoping, loop guard, fail-open, live watcher health, and Codex degraded-wake block tests
