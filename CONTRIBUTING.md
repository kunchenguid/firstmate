...
tests/fm-spawn-dispatch-profile.test.sh   # concrete dispatch profile flags: active-profile backstop, harness/model/effort meta, bash-wrapped launch templates, raw launch passthrough, batch forwarding, and secondmate exemption
...
tests/fm-backend.test.sh                  # runtime-backend abstraction: fm-backend.sh selection/meta/dispatch helpers, old-vs-new fake-tool command-log conformance, and fm-spawn bash-wrapped launch assertions
