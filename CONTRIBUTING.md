...
tests/fm-fleet-board.test.sh              # fm-fleet-board.sh read-only HTML fleet board: in-flight/queued/done rows and chips, live crew-state chip and activity line, full PR urls and scout report paths, HTML escaping, self-contained output, empty/missing-backlog and missing-meta degradation, read-only invariant, default output path, and the snapshot seam schema/fallback
