...
Each run judges the body as GitHub holds it at that moment rather than the snapshot in its own event, and a head synchronization waits up to three minutes for the pipeline to publish the attestation for the commit it just pushed before judging whatever body it has.
