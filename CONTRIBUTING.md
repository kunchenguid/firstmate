...
tests/fm-bootstrap.test.sh                # bootstrap dependency, feature-probe, fleet-sync timeout, crew-dispatch reporting, and update-availability check tests (prerelease filtering, offline/timeout silence, cache hit/read-only-no-write, corrupted-cache guard, and per-tool staleness lines)
