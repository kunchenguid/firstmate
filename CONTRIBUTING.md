...
- Helper scripts in `bin/` are plain bash.
  ...
  Every `bin/*.sh` must also parse under stock macOS Bash 3.2.57, which the macOS CI lane enforces with `/bin/bash -n` over the whole directory, so a local `bash -n` under a newer Bash will not catch syntax that 3.2 rejects.
