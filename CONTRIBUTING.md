...
That diet has a hard budget: `AGENTS.md` must stay at or under 40000 bytes, Claude Code's agent-instruction warning threshold, so check `wc -c AGENTS.md` before proposing an addition and expect to pay for a new line by removing another.
