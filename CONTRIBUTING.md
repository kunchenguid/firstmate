...
Shared test helpers live in `tests/lib.sh` (reporters, temp roots, git fixtures, and the host-proven named-process decoy every harness-detection suite needs), `tests/fixtures.sh` (fake toolchain and spawn-world builders), `tests/wake-helpers.sh`, and `tests/secondmate-helpers.sh`.
