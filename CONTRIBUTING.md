...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through the `no-mistakes` pipeline on a feature branch and merge only under the merge authority owned by [`AGENTS.md` hard rule 2](AGENTS.md#1-identity-and-prime-directives).
A change to any surface the merge authority rests on - hard rule 2, the captain-instruction precedence section, section 7's selected delivery path and approval authority, the recorded-posture form in [`docs/configuration.md`](docs/configuration.md#captain-preferences-datacaptainmd--datacaptain-sharedmd), the `pr-shepherd` skill's merge-authority section, or the `stow` skill's merge-posture marker rule - is carved out there and always needs a fresh explicit captain word for that PR, and hard rule 2 owns the full list.
