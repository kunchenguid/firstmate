...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - follow the authoritative delivery-mode rule in [`AGENTS.md`](AGENTS.md#intake-and-authority), with the default `no-mistakes` path requiring an explicit merge approval.
