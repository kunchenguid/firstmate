...
A change to any surface the merge authority rests on - hard rule 2, the captain-instruction precedence section, the recorded-posture form in [`docs/configuration.md`](docs/configuration.md#captain-preferences-datacaptainmd--datacaptain-sharedmd), the `pr-shepherd` skill's merge-authority section, or the `stow` skill's merge-posture marker rule - is carved out there and always needs a fresh explicit captain word for that PR.
