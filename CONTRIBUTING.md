...
When Firstmate coordinates a tracked code change, a second mate coordinates separate implementation and read-only review workers through the single task PR; the agent-only [`secondmate-code-review` skill](.agents/skills/secondmate-code-review/SKILL.md) owns that loop without changing human contribution intake or merge authority.
