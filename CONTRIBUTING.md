...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `skills-lock.json`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  The README's "Two-tier skill layout" owns what belongs in `.agents/skills/` versus `skills/`, including the vendored third-party skills that `skills-lock.json` locks and that are never hand-edited in this repo.
