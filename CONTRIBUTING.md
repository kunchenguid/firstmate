...
- Helper scripts in `bin/` are plain bash.
  ...
  Never open a heredoc inside a command substitution (`VAR=$(cat <<EOF ... EOF)`): the stock macOS `/bin/bash` 3.2 mis-lexes it as soon as the body holds an unbalanced quote character and then fails to parse the whole file.
  Emit the body from a function with `cat <<DELIM` and capture it as `VAR=$(fn)`; [`tests/fm-stock-bash-parse.test.sh`](tests/fm-stock-bash-parse.test.sh) owns that guard over `bin/`.
...
CI owns broad regression across required portable parallel shards, the portable serial lane, the Herdr lane, lint, invariants, the coverage guard, and the stock macOS Bash lane in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
That macOS lane runs the toolbelt-wide stock-Bash parse guard as well as the snapshot and Bearings consumers, so a Mac-only parse break cannot pass Linux-only checks.
