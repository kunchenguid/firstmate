...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  `.agents/skills/` holds agent-loaded skills that assume a live firstmate home and carry `metadata.internal: true` so installers such as [skills.sh](https://skills.sh) hide them from discovery, except the `fleet` skill, which deliberately omits the flag so installers ship it; `skills/` holds standalone, installer-facing public skills with no firstmate dependency (see the README's "Two-tier skill layout").
...
tests/fm-fleet-board.test.sh              # fm-fleet-board.sh read-only HTML fleet board: in-flight/queued/done rows and chips, fast default in-flight state from the status tail with the reconciled crew-state chip and activity line opt-in via --live, full PR urls and scout report paths, PR-chip promotion only for genuine pull-request links with free-form urls kept visible, HTML escaping, self-contained output, empty/missing-backlog and missing-meta degradation, read-only invariant, default output path, the default-off snapshot seam's opt-in gate and schema/fallback, and the fleet wrapper's forwarding
