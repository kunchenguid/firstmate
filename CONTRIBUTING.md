...
tests/fm-teardown.test.sh                 # fm-teardown.sh landed-work safety and reminder checks: fork-remote allow, squash/content landings, dirty and unlanded refusals, PR-head metadata, no-pr= branch discovery, Azure DevOps completed-allow/active-refuse parity, tasks-axi/manual backlog reminder, --force override, stale-vs-live worktree git index.lock recovery
tests/fm-review-diff.test.sh              # fm-review-diff.sh authoritative review diff coverage: recorded pr_head=, fetched refs/pull/<n>/head, Azure DevOps PR-head resolution via az, no-pr local branch behavior, and warning fallback
tests/fm-pr-merge.test.sh                 # fm-pr-merge.sh records pr= and available pr_head= before merging, parses PR URLs into gh-axi number/--repo calls, defaults to squash, preserves explicit merge methods, rejects malformed URLs and repo overrides, refuses Azure DevOps PR URLs, and propagates real merge failures
tests/fm-scm-lib.test.sh                  # fm-scm-lib.sh SCM provider abstraction: github/ado/unknown detection from URL and origin remote, GitHub and Azure DevOps PR URL parsing, and normalized pr_state/pr_head/pr_state_head/pr_number_for_branch against gh and az mocks
