...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  Such a mod loads only while Claude Code's default-off `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS` flag is on; Firstmate never sets that flag in any settings file, and [`docs/calm.md`](docs/calm.md) owns the shipped Calm mod's contract.
