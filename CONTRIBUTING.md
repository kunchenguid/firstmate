...
The same check also requires the structured pipeline step attestation comment that no-mistakes v1.46.0+ writes beside that signature, with `review`, `test`, and `document` each completed, so pushing through an older gate raises a PR the check rejects.
