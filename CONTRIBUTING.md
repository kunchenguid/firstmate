...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (firstmate expects **no-mistakes v1.31.2+**; without a fork, plain `no-mistakes init` still works for maintainers with push access).
   If an existing gate was created by an older no-mistakes and a push reports `invalid gate path`, re-run the same init command; init is idempotent and refreshes the post-receive hook.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux` plus experimental `herdr`, `zellij`, and `orca`.
...
- Changes to runtime session backends (`bin/fm-backend.sh`, `bin/backends/`, and the scripts that dispatch through them) need empirical adapter notes in the relevant backend guide: `docs/tmux-backend.md`, `docs/herdr-backend.md`, `docs/zellij-backend.md`, or `docs/orca-backend.md`.
...
tests/fm-nm-gate.test.sh                  # no-mistakes gate refresh helper: usage errors, non-git/no-origin/no-mistakes-absent skips, idempotent init refresh, non-fatal init-failure handling, and spawn timeout behavior
