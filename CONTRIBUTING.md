...
- Helper scripts in `bin/` are plain bash.
  ...
  One 3.2 trap that sweep cannot always catch: a command substitution is scanned for its closing paren without honoring an enclosed here-document, so `var=$(cat <<'X' ... X)` ends early if the body holds an unbalanced `)` such as a `case` pattern.
