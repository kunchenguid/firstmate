...
tests/fm-teardown.test.sh                 # fm-teardown.sh landed-work safety and reminder checks: fork-remote allow, squash/content landings, dirty and unlanded refusals, PR-head metadata, no-pr= branch discovery, Codebase merged-MR/branch-discovery allows and lookup-error content fallbacks, tasks-axi/manual backlog reminder, --force override, stale-vs-live worktree git index.lock recovery
tests/fm-review-diff.test.sh              # fm-review-diff.sh authoritative review diff coverage: recorded pr_head=, fetched GitHub refs/pull/<n>/head, Codebase MR head resolved through the provider seam, no-pr local branch behavior, and warning fallback
tests/fm-pr-merge.test.sh                 # fm-pr-merge.sh records pr= and available pr_head= before merging, parses GitHub PR URLs into gh-axi number/--repo calls and Codebase MR URLs into bytedcli number/-R calls, defaults to squash, maps Codebase merge-method shims, preserves explicit merge methods, rejects malformed URLs, repo overrides, and unsafe Codebase repo paths, and propagates real merge failures
