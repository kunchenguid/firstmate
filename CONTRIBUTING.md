...
Load `mobile-tablet-ui` before implementing responsive web layout or input adaptations for phones and tablets.
Load `ui-quality-evidence` when producing UI acceptance evidence against authoritative templates.
Load `api-contract-evidence` when producing endpoint response and persisted-effect evidence against authorization and error contracts.
