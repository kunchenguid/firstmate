...
- Changes to harness adapters (the registry and per-harness runtime facts in `bin/fm-harness-adapter.sh` and `bin/harnesses/`, detection in `bin/fm-harness.sh`, launch and hook mechanics in `bin/fm-spawn.sh`, semantic busy sources and trust gates in `bin/fm-busy-lib.sh`, cleanup in `bin/fm-teardown.sh`, and facts in `.agents/skills/harness-adapters/SKILL.md`) must be verified empirically against the real harness, never written from documentation alone.
  A harness's rendered busy-footer signature belongs to its adapter in `bin/harnesses/`, never to a backend library: `bin/fm-harness-adapter.sh` is the single owner, and `bin/fm-tmux-lib.sh` keeps only the tmux pane probe that consumes it.
