...
Open a pull request against `main` and let the repository's required CI checks report on the proposed changes.

## Workflow

1. Fork the repo or clone it if you have push access.
2. Create a branch and make your changes.
3. Run the focused tests for the touched behavior and the lint checks below.
4. Commit your changes and push the branch to your fork or repository.
5. Open a pull request against `main` and address the required CI results.

The repository CI workflow owns broad deterministic regression coverage.
...
Tracked changes to firstmate itself - `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/` - ship through the normal branch and pull-request path.
...
Reach for `bin/fm-test-run.sh` with the subjects you care about rather than chaining `bash tests/a.test.sh && bash tests/b.test.sh`, because a list of script paths gets the same bounded concurrency as `--changed`.
...
bin/fm-lint.sh   # lint that shell surface plus GitHub workflows via pinned actionlint; CI uses the same owner
