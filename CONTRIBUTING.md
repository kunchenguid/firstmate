...
- Helper scripts in `bin/` are plain bash.
  ...
  `bin/fm-lint.sh` must pass: it is the single owner of the lint definition (the shellcheck file set, config, pinned shellcheck version, and bounded worker policy), and both CI and the no-mistakes pre-push gate run it, so local and CI can never diverge.
...
bin/fm-lint.sh   # lint the toolbelt and behavior tests; the bounded single owner CI and the no-mistakes gate both run
...
bin/fm-test-run.sh --list --shard 1/4   # validate the complete manifest and inspect one isolated CI shard
bin/fm-test-isolation-proof.sh --list   # proven concurrent local candidate set (separate from CI sharding)
...
`bin/fm-test-run.sh` is the single owner of behavior-suite selection, shard-manifest completeness, per-script timing markers, family totals, deterministic failure aggregation, and the optional JSON timing artifact.
Its header and `--help` own the flags, family labels, and changed-file map; this section only documents the entry points.
Each runner invocation is serial; CI runs four manifest shards in isolated GitHub jobs, while `--all`, family, changed-file, and explicit-script selections remain available locally.
`bin/fm-test-isolation-proof.sh` separately owns the bounded concurrent local isolation proof; it does not add general local `--jobs` to the canonical runner.
See `docs/fm-test-isolation-proof.md` for the archived isolation candidate evidence and [`docs/ci-performance.md`](docs/ci-performance.md) for reproducible CI and lint timing evidence.
Local no-mistakes Test stays intent-targeted and must not wire `commands.test` to `--all` or a `tests/*.test.sh` walk; the CI Behavior matrix calls every completeness-checked shard for broad regression.
