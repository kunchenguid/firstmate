...
- `AGENTS.md` and the supervision protocol blocks are the always-loaded startup surface every fleet session pays for on every turn, so a required CI job enforces a byte budget on them; run `bin/fm-startup-surface-budget.sh` before proposing an addition to either, and route conditional detail to its owner rather than raising the ceiling.
