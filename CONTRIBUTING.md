...
- Helper scripts in `bin/` are plain bash.
  ...
  That same path runs `bin/fm-voice-guard.sh`, which reads back every commit message not yet on the default branch and refuses an address to the repository owner or an account of how the change was produced before the first push; its header owns the rule set and `--list-patterns` prints it.
