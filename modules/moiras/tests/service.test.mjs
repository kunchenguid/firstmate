...
const wait = async check => { for (let i = 0; i < 100; i++) { if (check()) return; await new Promise(resolve => setTimeout(resolve, 50)); } throw Error('Observable update missing'); };
test('real state -> module rules -> registered capture; shared messaging is explicitly fake', { timeout: 90000 }, async () => {
  ...
  try {
    ...
    observer = await start(); assert.ok(!fs.existsSync(path.join(state, `procevent-inbox/moiras-${event.id}.2.result`)));
