...
Operator reference for firstmate's orca backend. Orca owns both the task worktree and the terminal endpoint; the orca adapter (`bin/backends/orca.sh`) talks to the orca daemon directly via `bin/fmod` because the `orca` CLI cannot run while the GUI holds the single-instance lock.
...
- It does not register the captain's project with orca. The daemon-direct adapter does not touch the orca repo registry; the project must be a real git repo (adapter checks) and treehouse- or orca-worktree-ready.
