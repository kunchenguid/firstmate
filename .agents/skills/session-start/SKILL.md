...
description: >-
  ...
  AGENTS.md section 3 owns the run-once, visible-digest, no-persisted-reread, and lock-refusal invariants that apply with this skill unloaded.
...
`AGENTS.md` section 3 is the always-loaded owner of the rules that must hold before this skill loads: run or confirm `bin/fm-session-start.sh` exactly once, treat its visible complete digest as authoritative, do not re-read a persisted digest automatically, and remain read-only after lock refusal.
...
1. **Lock** - acquires the per-home session lock before anything mutates shared state, then starts the deferred startup stage for network checks and inactive-outcome reconciliation.
...
3. **Wake queue** - when locked, presents the durable queue and prints the raw records as this turn's first work queue without running inactive-outcome reconciliation inline.
   ...
   A main drain may also print a one-shot `STATUS OUTCOME BACKSTOP` when the newest captain-facing event lacks a covering supervision outcome; handle it as a recovered wake even when no queue row remains.
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`, every `state/<id>.meta`, a bounded tail of each task's `state/<id>.status` labeled as wake-EVENT history with the full log path printed, the away posture in `state/.afk-contract` plus the daemon flag where applicable, and one cheap alive-or-dead read of each task's recorded backend endpoint.
...
`AGENTS.md` section 5 keeps durable records and live backend reality authoritative after restart.
...
When `state/.afk-contract` exists, let `/afk` own the away posture; when `state/.afk` exists, route by its mode through `/afk` or `/quiet` instead of arming competing supervision.
