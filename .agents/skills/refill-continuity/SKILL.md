...
1. Run `bin/fm-refill.sh status`, then `tasks-axi ready` against this home's configured backlog.
2. Read each ordinary direct report with `bin/fm-crew-state.sh <id>`.
...
3. Reconcile terminal work through its existing lifecycle.
...
4. After every successful teardown, re-run `tasks-axi ready` and dispatch one highest-priority dependency-cleared task through the normal intake and `bin/fm-spawn.sh` path.
   Re-read `bin/fm-refill.sh status` after each successful spawn and stop when its active count reaches the desired count.
5. Treat each candidate independently.
...
6. If no safe candidate remains, leave every refusal and ambiguity durable and let the deficit detector re-surface the unchanged condition at its bounded cadence.
7. Resume this harness's supervision protocol before ending the turn.
