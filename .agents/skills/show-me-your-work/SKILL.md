...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for autonomous or multi-phase runs, or work a supervisor reviews after stepping away."
...
For work a supervisor reviews after the fact, a decision trail lets them reconstruct what was decided, why, and on what evidence, without rerunning the work or reading the whole transcript. Keep one canonical log so the trail is consistent and a future agent can find it.
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under the active worktree's run-transcript location (the harness names the path). Don't glob across other sessions' private logs; that reads unrelated private logs. Walk the log against what actually happened:
...
Before handing back, get a reviewer pass on a different model family from the one that did the work, via the supervisor where the harness supports it. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The reviewer reads the audit trail and the run's transcript, then flags what the supervisor should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
