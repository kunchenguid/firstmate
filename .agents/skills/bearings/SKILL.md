...
3. **Write the dated report file so it persists, then surface the mandatory four-section digest in chat.**
   ...
   - For a richer review surface, optionally offer a Lavish board through the supervised helper `bin/fm-lavish-review.sh` (never raw `lavish-axi`) when the report has enough structure to deserve one, but the markdown file is the required artifact and the four-section chat digest is the required minimum.
