...
- Every Underway row carries an explicit `name` field: the durable task name from the snapshot's `in_flight` rows, which the board's row leads with while keeping the run status on its second line.
Use null or an empty string only as the deliberate no-known-name marker, in which case the row falls back to the run status.
Never fabricate a name; the snapshot's durable name is the only source.
- A Charted Next row carries `filed`, the durable filed date from the snapshot's `gates` rows (`YYYY-MM-DD`, or that date with a UTC timestamp), and the board orders the section by it, newest filed first.
Omit it or pass null for a row with no durable filed date - the main-inventory warning, an unavailable secondmate home, a queued row filed before dates were recorded - and the board keeps those rows in payload order after every dated row.
