...
- Compose exactly one decision card per captain-held task id, and let that card ask exactly one question. The key is the task id, so a task carrying several questions is split into one captain-held task per question before the board is composed; never emit duplicate cards with the same task-id key, and never bundle several questions behind options like "walk me through all four" or "take my recommendation on all of them". The board flags such a card as a bundle and still deals it, because the captain answers what he can see - the flag is a reminder that the split is owed, not a substitute for doing it.
- Decision cards carry agent-authored copy: a short noun-phrase title, one-line `about` and `decide` context rows, and option labels with hints, with the recommended option marked. `about` names the concrete things at stake - which figures, which sections, which setting, where they appear, and what changes either way - because a card that says "two figures cannot be traced" without naming them cannot be answered. Find those specifics before composing the card.
- When a card goes out thin anyway, set `missing` to a short plain statement of what is not yet established, for example "the two figures are not named yet - firstmate is tracing them". The board shows it on the card as an `incomplete` flag. Publish the card either way: an unasked question is the one outcome the board never produces, and a thin card the captain can see beats a complete one he never gets.
...
- A Charted Next row's `title` is the FULL, untruncated task title, and its optional `detail` carries the task's notes: the fleet-state snapshot itself truncates titles for the chat digest, so pull the full title and notes for the board from `tasks-axi show <id> --full` (or the manual-backend backlog record) rather than copying the snapshot's shortened string. The board opens each row in place to show `title` and `detail` in full; a truncated title there defeats that.
