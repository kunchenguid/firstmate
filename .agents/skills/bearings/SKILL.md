...
- Every Underway row carries an explicit `name` field: the task-identifying label from the snapshot's `in_flight` rows, which the board's row leads with while keeping the run status on its second line. This is the durable task name when known and the snapshot-projected durable task id otherwise; never replace it with run status or invent another label.
