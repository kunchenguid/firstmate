...
- `awaiting` carries the Delivered section, one row per snapshot `awaiting` row with `nudge: false`: `id`, `repo`, `owner`, `what`, the required `pr_url`, and `age_days`. `awaiting_nudge_days` is the snapshot's `awaiting_nudge_days` verbatim, including null when the exit rule is not configured, and `build` refuses any `awaiting` row that has already reached a configured threshold. A null threshold refuses nothing for age and produces no nudge card. Never add a row the snapshot did not classify.
