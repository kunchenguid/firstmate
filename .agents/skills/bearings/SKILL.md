...
Generate a complete standalone snapshot from the fleet's current state, so Jay can resume in one read after a break, a night, or a context reset.
...
It never tears down a task, merges a PR, dispatches new work, or mutates any task state as a side effect of producing the brief - those belong to Jay's explicit word and the normal task lifecycle.
...
1. **Gather live fleet state with one deterministic command.**
   ...
   When Jay asks to include PRs, use the command's live-PR opt-in; otherwise keep the default local-only read.
...
2. **Compose the detailed report file around the four-section spine, adding the richer detail the chat leaves out.**
   ...
   - **Title** - `# Bearings - <day> <YYYY-MM-DD>` (use "Morning status" only when Jay specifically asks for a morning brief), followed by two or three sentences framing where things stand.
   - **Your Call** - every open decision summarized with its options from the structured decision record, plus each PR ready to merge and each needed credential or login, every PR with the full `https://...` URL, never a bare `#number`.
...
1. **Your Call** - ONLY items that need Jay's own action now: a decision to make, a PR to approve or merge, a credential or login to provide, or a blocker only Jay can clear.
...
4. **Charted Next** - queued or gated work waiting on the fleet or a date, plus action-free fleet-integrity warnings, never on Jay.
...
- The four buckets are mutually exclusive, so every item is forced into exactly one: needs-your-action is Your Call, done is Recently Landed, self-progressing is Underway, and not-yet-started work or an action-free fleet-integrity warning is Charted Next.
- The strict boundary keeps action-free items OUT of Your Call: a working or validating task, a queued item blocked on another task or a date, landed work, a completed scout's report pointer, a declared `paused:` external wait, and a bare recorded PR with no merge-ready signal each belong to one of the other three sections, never Your Call.
- A secondmate appears Underway only for `active_child_work`; `externally_held` belongs in Charted Next, and `unknown` belongs there as an unavailable-state gate unless its reason requires Jay's action.
...
- This report is a private internal artifact for Jay that lives in gitignored `data/`, so unlike normal chat it MAY reference task ids, PR URLs, and repo names - Jay works with these directly and needs them to resume; keep it organized and scannable, not a raw dump.
...
If the state you read suggests an action - a PR ready to merge, a queued item whose gate has arrived, a needs-decision finding - name it in its section (Jay's action under "Your Call", queued or gated work under "Charted Next") and let Jay decide, rather than taking the action from inside this skill.
