...
Decision answers need no routing from you: the runner feeds the board's binding into `bin/fm-captain-hold.sh`'s one keyed-answer intake, which closes or releases each answered captain-held task at answer time, and posts a receipt on the board naming what it recorded and what stays open; reconcile any `skipped:` key yourself with a direct `answer`, and when the captain's answer is "later", record it as a deferral with `bin/fm-captain-hold.sh hold <id> --reason "<reason>" --until <date>` instead of a closure.
A note typed with no option selected on a card that has options is the captain's comment and closes nothing ([`captain-hold-lifecycle.md`](../../../docs/captain-hold-lifecycle.md#reconcile-re-check-reality-never-a-blind-close) owns the rule): answer it in chat or on a rebuilt board and leave its call held.
A note attached to a selected option rides along with a recorded answer; read it and act on any instruction or question it carries.
