...
4. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   - **Underway** - each live direct report making progress, with its current state, how long it has been running, and the plans or main pickup pointers worth reopening (`data/<id>/report.md` files, `.lavish/*.html` boards).
...
3. **Underway** - live work progressing on its own, one line of current state per direct report, each carrying how long that worker has been running.
   Say so plainly when the snapshot reports a worker's running time as unknown, rather than guessing a figure.
