...
- Fill the optional `prs` array from the captain's open pull requests across repos with `gh search prs --author=@me --state=open --sort updated --order desc --json number,title,repository,url`, so the board's top right shows what is open right now; omit the field entirely when that query is unavailable.
