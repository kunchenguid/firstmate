...
The hold remains the authoritative Captain's Call item until the captain's answer is durably recorded and `bin/fm-decision-hold.sh resolve` closes it.
When the answer creates follow-up work, record that work in the same backlog, block it by the hold identity, and let `resolve` clear those dependency edges before closing the hold.
When the answer is accept-as-is - a documented gap, accepted residual, or deliberate non-fix - record the decision file and pass `--accept-as-is` so `resolve` closes the hold without routing any task; the decision text is the durable record of what was accepted, and no `blocked_by` edges are cleared.
...
6. After the captain decides, decide whether the answer creates follow-up work or is an accept-as-is resolution.
7. For follow-up work: record dependent work with normal tasks-axi commands, block it by the hold identity, put the captain's exact durable decision in a file, and use the script's `resolve` command with every routed task.
   For accept-as-is: put the captain's exact durable decision in a file and use the script's `resolve` command with `--accept-as-is`; no dependent work is recorded and no `blocked_by` edges are cleared.
