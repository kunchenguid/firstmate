...
description: >-
  Agent-only policy for completing investigations and visual reviews without losing unresolved decisions for Jay.
  Load before treating an investigation, scout report, structured review, or Lavish review as complete, before ending a visual review that exposed a decision, and when recording or routing Jay's answer.
...
This skill is the single policy owner for unresolved decisions for Jay discovered by an investigation or visual review.

## Policy

Every unresolved decision that belongs to Jay and is discovered while producing, reading, presenting, or ending an investigation or visual review must become a structured `captain-held` work item in the authoritative backlog of the home that owns the originating work before that work or review may be treated as complete.
...
After inventorying the whole report and review surface, run `bin/fm-decision-hold.sh complete` with every unresolved key, or with `--none` only when the reviewed surface contains no unresolved decision for Jay.
...
The hold remains the authoritative Your Call item until Jay's answer is durably recorded, dependent work is created in the same backlog and blocked by that hold, and `bin/fm-decision-hold.sh resolve` routes the answer by clearing those dependency edges before closing the hold.
Resolved findings, recommendations that need no choice from Jay, and prose that merely sounds decision-like do not create holds.
...
2. Inventory only genuine unresolved choices that require Jay.
...
5. Relay the choices to Jay as decisions from Bearings' Your Call section under `AGENTS.md` section 9; do not use the word hold in messages to Jay.
6. After Jay decides, record dependent work with normal tasks-axi commands and block it by the hold identity.
7. Put Jay's exact durable decision in a file and use the script's `resolve` command with every routed task.
