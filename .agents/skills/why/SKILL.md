---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Discovers available tool integrations and queries each evidence category (source control, issue tracker, long-form docs, real-time chat, infrastructure observability, error tracking, product analytics warehouse) in parallel, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (current task context, recent changes, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the tool integrations available in this environment and map each one to an evidence category.
...
- `model`: the why-investigators model assigned in the brief or dispatch profile
- `readonly`: `false` (agent mode). Investigators need tool access, so do not strip it. Not writing anything is a posture, not a sandbox.
...
- run each investigator as a separate agent pass with a clean-slate prompt
- `model`: the why-synthesizer model assigned in the brief or dispatch profile
- `readonly`: `false` (agent mode). The synthesizer's quality check spot-verifies citations, which can require tool access. Do not strip tool access.
...
**Sources Consulted**. One line per investigator, including the ones that returned nothing. The reader should see at a glance (a) which MCPs were queried, (b) which came back empty, and (c) which were skipped and why. This coverage map lets the reader judge breadth and redirect if something obvious was missed.
...
- Real-time team chat: skipped. No team-chat integration available in this environment. Gap: conversational record not searched.
...
After the Sources Consulted block, if the `why` question is a precursor to actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
