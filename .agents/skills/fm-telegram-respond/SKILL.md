...
4. Do not re-process inbox files the script already cleared. `deferred` lines are rate-limited commands still queued in the inbox; leave those files alone - the poll re-wakes pending inbox files each sweep, so a later drain runs them. On script `error` lines, leave evidence and escalate if the failure repeats.
