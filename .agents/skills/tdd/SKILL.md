...
- **Implementation-coupled** - tests private methods or verifies through a side channel (querying the database instead of using the interface).
  The tell: the test breaks when you refactor but behavior hasn't changed.
  The boundary and mocking rules live in [mocking.md](mocking.md).
