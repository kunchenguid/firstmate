...
- run each investigator as a separate agent pass with a clean-slate prompt
- `model`: the how-explorer model assigned in the brief or dispatch profile
- `readonly`: `true`
...
- run each investigator as a separate agent pass with a clean-slate prompt
- `model`: the how-explainer model assigned in the brief or dispatch profile
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per model in the how-critics list from the brief or dispatch profile (one per available model family from the brief or dispatch profile), all in a single message.
