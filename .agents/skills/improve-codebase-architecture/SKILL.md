...
Delegate the scan through the active environment's authorized worker path.
When running as Firstmate, commission a scout through its task lifecycle and retain the resulting report as the investigation artifact.
...
Mark it clearly in the card (e.g. a warning callout: _"contradicts ADR-0007 - but worth reopening because…"_).
...
- **Want to explore alternative interfaces for the deepened module?** Run the `/codebase-design` skill and use independent design contexts when the active environment authorizes them.
