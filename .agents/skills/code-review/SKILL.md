...
description: >-
  ...
  Runs independent review axes, in parallel when authorized, and reports them side by side.
...
Both axes run in independent contexts so they do not pollute each other, then this skill aggregates their findings.
Use parallel workers when the current environment authorizes them.

Resolve the originating specification from the task brief, linked issue, pull request, or repository documentation.
When no specification exists, say that the Spec axis is ungrounded instead of inventing requirements.
...
1. Issue references in commit messages such as `#123`, `Closes #45`, or GitLab `!67` - fetch through the repository's established forge workflow.
...
- **Speculative Generality** - abstraction, parameters, or hooks added for needs the spec doesn't have.
  Delete it and inline back until a real need shows.
...
- **Middle Man** - a class or function that mostly just delegates onward.
  Cut it and call the real target directly.
...
### 4. Dispatch both review axes

Run both read-only review axes in parallel through the current environment's authorized worker mechanism.
When running as Firstmate, follow its task-lifecycle and harness-adapter contracts and commission crewmates.
When a delegated worker is explicitly allowed to spawn sub-agents, it may use them.
Otherwise, run the two axes sequentially while keeping their contexts separate.
