...
`AGENTS.md` section 4 keeps the load trigger and a one-line reinforcement of that intake boundary.
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override.
...
Never guess, omit a candidate, or fall back silently.
