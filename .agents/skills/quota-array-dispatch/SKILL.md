...
description: >-
  ...
  array from current quota-axi output, including selection.spendPriority,
...
- applicable `selection.status`, `selection.spendPriority`, and any `selection.unmeasurableWindowIds` from the established provider/model scope
- applicable effective headroom (`effectivePercentRemaining`) from that same scope
- usable runway status, `usableRunwaySeconds`, `projectedExhaustedAt`, `limitingWindowId`, `projectionConfidence`, and any `unmeasurableWindowIds`
...
- schema notes when selection, runway, or pace fields are absent
...
Read all windows named by `boundedBy`, `limitingWindowIds`, `aheadWindowIds`, `unknownWindowIds`, and `unmeasurableWindowIds`.
`aheadWindowIds` is the conservation-pressure trigger and `unknownWindowIds` is uncertainty; both stay in default `--json`.
`behindWindowIds` and `onPaceWindowIds` are the benign complement and live in `--full` only, so do not require them to settle a candidate.
The compact default TOON intentionally omits numeric reserve.
Default `--json` retains `reservePercentPoints`, `burnMultiple`, window `pace.status`, and `pace.reason`; use those rather than `--full` cycle-progress internals.
Per-window `timeRemainingPercent`, `elapsedPercent`, `cycleBasis`, `cycleSeconds`, and per-window `projectedExhaustedAt`/`projectionConfidence` are `--full` only and are not needed once the retained reserve and pace fields are present.
There is no `projectionBasis` field; its absence means `cycle_average`.
`quotaSemantics.description` is `--full` only; read the per-scope `boundedBy` set, which states the applied bounding rule concretely.
...
2. Apply quota at the granularity the vendor actually supplies.
   ...
   Read that scope's `boundedBy`, which names every window included in the bound.
...
Read `reservePercentPoints` from default `--json`; do not recompute it from `--full` cycle-progress fields.

## Selection order

Apply only among candidates satisfying required fit and strongest reasoning class.
Never use `spendPriority`, headroom, runway, pace, or reserve to silently replace that reasoning class.

1. Concrete contradictory evidence or malformed configuration: stop and report the tuple and that evidence.
   Unmeasurable quota, a missing model-level window, an absent runway or selection field, and a credential surface quota-axi does not model are uncertainty, never this rule.
...
4. Among comparable-fit candidates, rank by `selection.spendPriority` as the primary quota-perspective factor.
   ...
   Establish that rank only from comparable known `selection.status: known` scalars.
   Never treat absent, `unknown`, or unmeasurable `spendPriority` as zero or as a healthy value; `0` means exact utilization, a different claim from unknown.
   Show the scalar, its status, and any `selection.unmeasurableWindowIds` in the rationale rather than hiding them in a score.
5. Usable runway versus the inspectable likely-completion horizon remains a hard gate that `spendPriority` cannot override.
   Known evidence that does not reach that horizon is inferior to known evidence that does, even when the short-runway candidate has a higher `spendPriority`.
   Preserve projection confidence, the limiting window, and the horizon estimate in the rationale rather than hiding them in a score or model-specific heuristic.
6. Resolve remaining uncertainty explicitly.
   An authenticated candidate with unknown or unmeasurable headroom, runway, or `spendPriority` stays eligible and cannot be silently excluded or assumed sustainable.
...
7. Use pace and signed reserve only as later diagnostic tie-break evidence among candidates still unresolved after `spendPriority` rank, runway-versus-horizon, and uncertainty.
   Pace and reserve never rescue a clearly inferior completion prospect or override a known `spendPriority` rank.
   Do not collapse these facts into an opaque composite score.
8. Older schemas or absent selection, runway, or pace fields: do not crash, fabricate runway, pace, or `spendPriority`, treat absence as healthy, or silently exclude a candidate.
