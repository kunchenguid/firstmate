...
agy has no provider family at all, so the helper discloses an `agy:` candidate on stderr, reads it as unknown quota rather than refusing the run, never selects it, and still ranks the candidates after it.
