...
A separate limitation of the same class applies to every claude candidate, helper or not: the `claude` row measures the store Firstmate's own environment resolves, not the store `config/claude-config-dir` names for crewmates, so it can read healthy while the crewmates' seat is empty; `docs/configuration.md` "Claude config store" owns that fact.
