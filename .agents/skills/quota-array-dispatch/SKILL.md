...
Match the candidate to its `quota[]` row by that established provider, its `accountKey` when the snapshot is schema 6 (a Pi lane's auth provider id such as `openai-codex-work`, or `codex-home` for native Codex including Pi's `codex-native/` adapter, then the `default` row, else unmeasured; a native `claude` candidate launches through the claude-pool proxy that fails over between its accounts, so after the `default` row it binds to the best viable account row, the one whose provider-wide scopes are not blocked or exhausted and leave the most headroom; never a row picked by position, never rows summed across accounts), and scope; a stale, auth-required, or unmeasurable scope is named in `attention[]` instead of a fabricated number.
