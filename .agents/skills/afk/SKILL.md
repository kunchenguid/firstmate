...
Treat `state/.subsuper-escalations`, its `.since` sidecar, `state/.subsuper-inject-wedged`, and the bounded-re-injection cap markers `state/.subsuper-inject-failed` and `state/.subsuper-inject-capped` as session-scoped delivery artifacts, not as the durable work record.
