...
- `signal` or `stale` whose latest status declares a wait, either a `paused:` external wait or a `captain-held` declaration, tracks the pause rather than a wedge whether its pane reads idle or busy.
  ...
  If a declared external wait is still declared past `FM_PAUSE_RESURFACE_SECS` (default four hours), housekeeping sends one recheck and resets the pause window; a captain-held declaration is never rechecked while the posture record exists.
