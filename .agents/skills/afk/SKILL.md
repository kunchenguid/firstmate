...
- **Primary-pane busy guard** - `pane_is_busy` trusts Herdr native `busy` when available, otherwise matches rendered output against only the detected primary harness's signature.
  ...
  The Herdr adapter owns the same masking once, at its native-state reader, so a self-hosted pane classifies as `unknown` and never as `idle`; see `docs/herdr-backend.md` "Current transport behavior" for the Herdr reader's contract.
