...
For herdr, normal idle-baseline submits are confirmed by native agent-state showing a real turn started, except Pi, which can consume a message while Herdr stays idle and is confirmed by the identity-corroborated structurally empty composer instead (`docs/herdr-backend.md` owns the contract); the shared classifier remains the affirmative-empty pre-injection guard and conservative fallback for non-idle or unreadable baselines.
...
- **Verified type-once submit model** - the digest is typed once (`send-keys -l`
  ...
  For herdr's normal idle-baseline path it means native agent-state observed a real turn start, or for Pi only, an identity-corroborated structurally empty composer while Herdr stays idle (`docs/herdr-backend.md`); herdr uses the shared classifier for the pre-injection composer guard, that Pi acknowledgement, and fallback paths.
