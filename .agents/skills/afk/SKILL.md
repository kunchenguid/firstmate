...
Captain-relevant events, plus a bounded recheck of a still-declared external wait, escalate to firstmate's context as one pre-read, single-line, batched digest.
...
- `signal` or `stale` for a declared `paused:` external wait -> self-handle and track the pause rather than a wedge, whether its pane reads idle or busy.
  This outranks an enriched possible-wedge reason, so a declared pause never escalates on the `FM_STALE_ESCALATE_SECS` cadence.
  If the pause is still declared past `FM_PAUSE_RESURFACE_SECS` (default 3600s), housekeeping sends one awaiting-external recheck and resets the pause window.
  That window ages against the crew's own latest status line, so only a non-paused status append ends this routing and restores normal wedge detection.
