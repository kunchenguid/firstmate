...
Always enter through `bin/fm-away-session.sh start`, which composes the launcher that clears prior-session artifacts only for a fresh entry and preserves the current session's buffer on refresh.
Always return through `bin/fm-away-session.sh return`, which composes the launcher stop that keeps `state/.afk` present through the daemon's shutdown flush and clears it last.
