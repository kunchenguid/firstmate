...
For herdr, [`docs/herdr-backend.md`](../../../docs/herdr-backend.md#current-transport-behavior) owns the native-state, rendered-footer, and Pi-specific confirmation paths; the shared classifier remains the affirmative-empty pre-injection guard and participates only in the documented fallback paths.
...
- **Verified type-once submit model** - the digest is typed once (`send-keys -l`
  ...
  For herdr, the backend-specific native-state, rendered-footer, and Pi structural-composer confirmation paths are owned by [`docs/herdr-backend.md`](../../../docs/herdr-backend.md#current-transport-behavior).
