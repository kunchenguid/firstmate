...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITHOUT one** (e.g. pi): run `bin/fm-afk-launch.sh start`. It is
     ...
     detached tmux session, records its exact id, and passes the captain pane in
     as `FM_SUPERVISOR_TARGET` so the daemon injects into the captain, not its own
     new pane.
...
The digest is typed **once** with `send-keys -l` on tmux, then submitted with Enter and
...
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  ...
  no-op.
- **Verified type-once submit model** - the digest is typed once with `send-keys -l`
...
- **Auto-discovered supervisor pane** - the daemon resolves its own tmux TARGET independently.
  Target resolution uses `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE`, then a `firstmate:0` fallback with a warning.
  The resolution source is logged at startup so a wrong-but-resolving fallback is detectable.
