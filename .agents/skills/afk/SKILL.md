...
- `signal` or `stale` whose latest status declares a wait, either a `paused:` external wait or a verified `captain-held` transfer, tracks the pause rather than a wedge whether its pane reads idle or busy.
  ...
  With no unreported actionable event, the wake self-handles, and the current declaration outranks an enriched possible-wedge reason so it never escalates on the `FM_STALE_ESCALATE_SECS` cadence - except a `paused:` line claiming a no-mistakes run that `bin/fm-crew-state.sh` does not confirm as a working run, which escalates immediately instead of tracking the pause (`docs/configuration.md` "Pipeline-state watch").
