...
4. **Use inspect-then-update.**
   ...
   The only graduation moves are promotion to tracked shared material through a PR, folding a learning into the captain-preference destination selected by `primary-runtime`, archiving a stale entry to `data/memory-archive.md`, autonomous offload of an eligible non-pinned conditional entry to an already-existing allowed owner through the reduce flow above, captain-approved offload of a pinned durable conditional entry to a JIT-loaded owner executed through the migration step above, or deletion of an entry that is a duplicate or already preserved through a stronger existing owner.
