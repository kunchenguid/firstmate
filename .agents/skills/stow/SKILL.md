...
3. **Write within firstmate's existing write boundaries.**
   ...
   - Knowledge generalizable to every firstmate user: this repo's own `AGENTS.md` (or other shared, tracked material), shipped through the direct-PR workflow owned by `CONTRIBUTING.md`, never hand-committed straight to `main`.
