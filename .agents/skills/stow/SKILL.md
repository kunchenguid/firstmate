...
8. Run `bin/fm-startup-memory-budget.sh report` again after the complete pass.
   ...
   When the remaining excess is blocked only by pinned safety, authority, or genuine captain-preference entries, create one captain-held decision that names the shortfall and each relevant pinned entry, with exactly these options: raise the effective budget, or explicitly approve offloading or trimming a named pinned entry.
...
It is an immediate reduction step for eligible non-pinned conditional material with a live allowed destination, not a deferred proposal that leaves the pass over budget.
...
- Eligible by authority: a non-pinned `aging` entry may be autonomously relocated only to an already-live allowed owner, while a `pinned` entry may be proposed only for explicit, per-item captain-approved relocation and can never be archived or autonomously offloaded for budget relief.
...
1. Reduce non-pinned material now.
   ...
   Autonomously relocate it only when that allowed JIT or owning destination is already live, then remove the memory entry in the same pass.
   A destination that needs creation, project delivery, or any other future work is not live and cannot count as relief, so continue with the next archival or eviction rung instead of leaving an over-budget proposal pending.
