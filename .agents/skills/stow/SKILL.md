...
3. **Write within firstmate's existing write boundaries.**
   ...
   - Knowledge generalizable to every firstmate user: this repo's own `AGENTS.md` or other shared tracked material, shipped through the selected branch -> PR -> captain-merge path for this repo, with direct-PR as the default and no-mistakes as an explicit opt-in, never hand-committed straight to `main`.
