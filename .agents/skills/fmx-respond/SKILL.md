...
- **Work that spawns a real, longer-running job** (dispatching a crewmate, a scout investigation, a ship task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  ...
  3. **Link it for the follow-up, before clearing the inbox.** Associate the spawned task with this mention so completion follow-ups can be posted later: `bin/fm-x-link.sh <task-id> <request_id>` (records the request id, a timestamp, a follow-up counter, and reply-platform context).
     ...
     As a durable safety net beneath all of that, the poll records a per-request reply context (`state/x-context/<request_id>.json`) that survives inbox cleanup and lets a later follow-up recover the original platform even with no task link, and `bin/fm-x-reply.sh` refuses (rather than wrongly splits) a follow-up whose platform it cannot resolve - so a delayed or concurrent request-id follow-up is never mangled into an X-style thread.
...
- If a follow-up's platform/budget cannot be authoritatively resolved (no per-request context, no inbox payload, and no relay answer) and the reply is long enough to split, `bin/fm-x-followup.sh` does NOT post it: the fail-safe holds it (the link is kept, exit non-zero) rather than risk threading a Discord follow-up at the X budget. This is a retryable hold - a later milestone wake retries it once the platform is recoverable, or link the task before the inbox drains. A follow-up short enough to fit one message still posts.
