...
- **Work that spawns a real, longer-running job** (dispatching a crewmate, a scout investigation, a ship task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  ...
  3. **Link it for the follow-up, before clearing the inbox.** Associate the spawned task with this mention so completion follow-ups can be posted later: `bin/fm-x-link.sh <task-id> <request_id>` (records the request id, a timestamp, a follow-up counter, and reply-platform context).
     ...
     `bin/fm-x-link.sh` reads the mention's platform and explicit budget from the still-present inbox payload, then fills either missing axis from an authoritative relay lookup by request_id and warns loudly if the context remains incomplete - but the local link-before-cleanup order is the fast, correct path, so keep it.
     As a durable safety net beneath all of that, the poll records a per-request reply context (`state/x-context/<request_id>.json`) that survives inbox cleanup and lets a later follow-up recover the original platform and explicit budget even with no task link, and `bin/fm-x-reply.sh` refuses a follow-up unless it can resolve both values - so a delayed or concurrent request-id follow-up is never posted with a local default.
