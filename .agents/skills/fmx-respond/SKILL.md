...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   a. Read the object: you need `request_id`, `text`, `in_reply_to`, and - when present - `in_reply_to_chain`.
      ...
      `in_reply_to_chain` is the optional surrounding-conversation transcript; the legacy Relay configuration reference owned its exact wire shape and compatibility semantics before Relay was removed from the lean fork.
