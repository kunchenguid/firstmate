...
2. For each ready commitment, run `bin/fm-public-followup.sh deliver <obligation-id>`.
   ...
   Once delivery is validated, run `bin/fm-x-followup.sh --clear <task-id>` in the bound work home so a legacy X link cannot post a second terminal reply.
