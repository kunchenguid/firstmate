...
description: >-
  Agent-only procedure for registered process-to-event sources and their notifications.
  ...
  `procevent <adapter> <source-id> <sequence>` check notification.
  Owns the arming commands, the condition->action eligibility boundary, the
  durable result read, which notifications must be routed to their adapter instead of
