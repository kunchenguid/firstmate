...
: A source whose adapter classifies the captured result as terminal has already retired itself, so an ended review needs no cleanup from you and produces no further wake. Retire any other finished source with the adapter's `retire`, which stays safe and idempotent even for one that already retired. Retirement stops future completions; it is independent of acknowledging a result already captured, which only `handled` does.
...
- a captured result is delivered proactively on the watcher's ordinary cycle, once per generation, with no manual drain and no duplicate wake while it stays queued or after it is acknowledged;
- a source whose own adapter classifies its captured result terminal captures at most one such result, retires automatically without losing that result or its announcement, and does no further poll work;
