...
: A Lavish result whose keyed answers carry `merge.` or `dispatch.` keys is a bearings board result; load the `bearings` skill's board-wake handling before acting on those keys.
