...
: A `quota` wake carries one terminal quota-check outcome: `bin/fm-procevent-quota.sh classify <result-file>` returns `low`, `exhausted`, `error`, or `unknown`. Report the provider, the ambient credential home the detail's `scope` says it was measured in, and captured quota state, decide whether the active work should continue or move, then use the generic acknowledgement above. Re-arm explicitly if continued monitoring is needed.
