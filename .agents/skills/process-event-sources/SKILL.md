...
For a terminal validation failure already proven to be exhaustion of one measurable rolling provider window, use the task-bound reset adapter instead:

```sh
bin/fm-procevent-quota-reset.sh arm <task-id> --run <run-id> --provider <provider> --window <window-id>
```

Its command refuses intake unless the clean submitted head, exact failed run, worker incarnation, and fresh exhausted window all bind, and its header and help own the complete evidence and result contract.
...
`bin/fm-procevent.sh --help`, `bin/fm-procevent-lavish.sh --help`, `bin/fm-procevent-when.sh --help`, `bin/fm-procevent-quota.sh --help`, `bin/fm-procevent-quota-reset.sh --help`, and `bin/fm-procevent-remote-reply.sh --help` own the exact commands and flags.
...
: A `quota-reset` wake is task-bound: classify it with `bin/fm-procevent-quota-reset.sh classify <result-file>`. For `reset`, run `bin/fm-procevent-quota-reset.sh handle <source-id> <sequence> <result-file>` instead of acknowledging generically; the handler re-reads authoritative task and validation state and either delivers one deduplicated retry instruction through the task inbox or records a concrete safety refusal. For `diagnosis`, inspect and report the provider evidence, then acknowledge generically without claiming a reset. Retire a stale manually abandoned watch through the adapter's `retire <task-id>` command.
