...
Reconcile reports every source it could not start as `failed=` and exits non-zero, so a source that cannot be started says so instead of looking armed, and it wakes you once per failure episode about it because the watcher discards that count; the runner never claimed there, so `start` does not fix it - run it attached to read the runner's refusal, then check the source command and adapter binary the registration names.
