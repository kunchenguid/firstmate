...
: Ask the adapter what the result means rather than parsing it yourself.
  ...
  Consume every Lavish capture with `bin/fm-procevent-lavish.sh read <result-file>` rather than grepping the raw file.
  For an ordinary capture, that command reports declared and presented item counts plus a completeness verdict, enumerates every captured queued item while retaining supplied element identity, and surfaces a `tag=message` session-ending message as its own field.
  For a recovery chunk, it instead reports the exact byte range and whether recovery is complete; handle successive sequences in order through the final chunk so the whole dock reply reaches the captain.
...
The Lavish adapter therefore atomically snapshots matching pending dock prompts before polling and retains that snapshot until the generic runner durably captures it; if the snapshot exceeds the per-result bound, successive invocations emit ordered chunks and advance a private cursor only after each chunk is durably captured, then the final chunk removes the snapshot through the same process-event source and wake owner.
