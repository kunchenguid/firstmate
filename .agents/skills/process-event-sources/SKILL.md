...
: Ask the adapter what the result means rather than parsing it yourself.
  ...
  It reads both shapes the published poll encodes queued content in, so an annotation carrying an element target is presented like any other; when it cannot read a non-empty payload it names the unread block or the failed count and exits nonzero, and that nonzero exit is a failure to investigate, never an empty review to approve.
