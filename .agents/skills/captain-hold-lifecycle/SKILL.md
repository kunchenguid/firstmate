...
Owner-qualified secondmate task routes and repository-qualified PID routes are not local task aliases: the product-decision router records delivery durably, resolves the authoritative owner, and reaches this same guarded intake in that home.
It never creates a duplicate main-home request or bypasses the owner's `reconcile-requests` binding check; load `product-decision-steward` for the PID record and route contract.
