...
Holding the work item the question gates is safe for exactly that reason: cleanup keeps such a row open and returns it to the queue, so it still reads as the captain's own call.
