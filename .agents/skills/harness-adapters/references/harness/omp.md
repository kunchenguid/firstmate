...
The same file ports the `tool_call` seatbelts and delivers the session-start digest through `before_agent_start` on the Run tier; omp's `session_start` carries no reason, so the source is derived (first start `startup`, or `resume` only when the launch line says resume and this process already owns the lock; a new process that does not own the lock uses `startup`; later in-process starts `clear`; `session_compact` as `compact`).
