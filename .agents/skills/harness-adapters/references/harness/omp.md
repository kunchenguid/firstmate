...
| Launch | Foreign markers cleared (`CLAUDECODE`, `PI_CODING_AGENT`, `GROK_AGENT`, `FM_PI_HARNESS`, `GEMINI_CLI`, Cursor's), `FM_OMP_HARNESS=omp OMP_SKIP_SETUP=1`, then `omp [--profile <OMP_PROFILE>] --config <.omp/fm-worker-overlay.yml> --auto-approve --cwd <worktree> [--model] [--thinking] -e state/<id>.omp-ext.ts <one positional brief>`; `--profile` is forwarded only when firstmate's own `OMP_PROFILE` is set and non-empty, so a worker authenticates under the same isolated profile firstmate runs under; a secondmate passes no `-e` and relies on auto-discovery. |
