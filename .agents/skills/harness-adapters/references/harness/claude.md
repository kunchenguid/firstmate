...
`../../../bin/fm-claude-trust.sh` records `hasTrustDialogAccepted` for both the worktree and its primary checkout in the selected Claude account's `.claude.json` (`~/.claude.json` for `ordinary`; `docs/configuration.md` "Worker accounts") for a ship or scout spawn; a secondmate spawn registers only its own home entry, since a secondmate home has no separate primary-checkout entry to carry import consent forward from.
