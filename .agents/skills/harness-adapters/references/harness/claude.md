...
The `<project>` argument that names that primary checkout is derived from the acquired worktree's own git common dir, not from the spawning home's registered project: a Treehouse pool is keyed by the project's resolved origin and shared by every local clone of that origin, so a pool worktree can be linked to a different home's clone of the same origin.
A registered project entry that names another home's clone rather than this home's own `projects/<name>` is therefore the correct result, not a bug.
