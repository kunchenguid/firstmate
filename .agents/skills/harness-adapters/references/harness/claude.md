...
The spawn command has no flag to suppress it: `--setting-sources` is not a documented flag of the interactive `claude` CLI at all, only a same-named Agent SDK constructor option, and it is not passed here. It gates the pane exactly like the trust dialog: cursor on "No, disable external imports", no way to move the selection from firstmate's steering plane.
