...
The standalone-`cd` shape is unavailable in a plain (non-worktree) firstmate or secondmate checkout: `docs/cd-guard.md`'s PreToolUse seatbelt (`bin/fm-cd-pretool-check.sh`) denies exactly a bare top-level `cd` there, home-relocation included, because the guard's steady state is always at the home and a return-to-home `cd` is redundant rather than necessary. In that scope, use the absolute-path invocation instead; the standalone `cd` is only available in a linked task worktree, where the guard is inert.
