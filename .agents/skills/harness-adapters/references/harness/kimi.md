...
The delivery wait therefore re-sends Enter - never retypes - on every poll the shared composer classifier still proves the composer holds the pointer, bounded by `FM_KIMI_DELIVERY_RESENDS`, and never sends into an empty or unclassifiable composer, so a confirmed or in-flight delivery is never followed by a stray Enter; the postcondition verification stays exactly as strict rather than readiness being relaxed.
A gate failure after the pane and local copy exist keeps the provisional task record, so `../../../bin/fm-teardown.sh` owns the pane, local copy, slot claim, and hook-token cleanup under its landed-work gates; rolling the record back stranded all four for hand cleanup on 2026-09-25.
