...
Verified on 2026-08-16 with Kimi Code CLI 0.36.1 for trust and submit paths.

The trust dialog, workspace-trust record format, launch, and composer/submit paths were exercised live. A full model turn was not exercised because the Kimi Code CLI credential was expired (`kimi_code_cli_credential_expired`; see [`docs/verification/dispatch-auth.md`](../../../../../docs/verification/dispatch-auth.md)). Treat real turn execution, turn-end hook firing under a live model, and busy state during a genuine turn as unverified until authentication is restored.
...
That mitigation is selected by the harness argument threaded through `fm_backend_send_text_submit`, so every caller inherits the one implementation.
...
[`docs/turnend-guard.md`](../../../../../docs/turnend-guard.md) owns its separate global hook surface and captain-approved crew wake integration.
...
Each Kimi task root receives a `.fm-kimi-turnend` pointer, excluded from Git for writer worktrees.
