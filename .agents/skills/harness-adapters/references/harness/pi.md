...
| Model flag | `--model <model>`; Firstmate also passes `--provider <the model's provider>`, which `config/pi-account` must declare, because Pi otherwise treats the model's provider prefix as a hint (`../../../docs/configuration.md` "Worker accounts"). |
...
The decision persists per path in `trust.json` inside the selected Pi account root (`~/.pi/agent` for `ordinary`; `../../../docs/configuration.md` "Worker accounts"), so later spawns in the same pooled slot under that root skip it.
