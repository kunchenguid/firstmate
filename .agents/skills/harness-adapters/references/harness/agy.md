...
`../../../tests/fm-agy-harness.test.sh` is the portable regression, the default-on `../../../tests/fm-harness-liveness-drift-live-e2e.test.sh` covers an installed `agy` for liveness drift, and the opt-in `../../../tests/fm-agy-live-e2e.test.sh` (`FM_AGY_LIVE_E2E=1`) drives spawn, both Stop hooks, the continue bound, in-session harness detection, steer, busy, interrupt, exit, and teardown against the installed binary; run it after every Agy upgrade.
