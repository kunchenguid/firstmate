...
- Answer the workspace-trust dialog with one Enter, only once its question is on screen together with `Yes, I trust this folder` as a row of its own, never on prose that quotes either, and prove no readiness after it: a launch that never starts is ordinary stuck-worker territory.
