...
| Busy state | Armed: `../../../../../bin/fm-spawn.sh` seeds the record and agy's global hook writes it, `PreInvocation` opening a turn and `Stop` closing one, both as source `agy-hook`. Arming requires positive evidence of the hook surface: the spawn probes `agy --version` and needs at least `1.2.6`, so an older, unreachable, or unrecognisable version installs nothing and stays unarmed. The `agy-regex` rendered-tail fallback is retained for the windows in which no record exists, and on Herdr the native `working` status still classifies busy. |
...
That is not a window before the first hook fires, because a managed launch is seeded busy at spawn; the fallback carries the shapes in which no hook could write a record at all, namely a raw launch, a launch whose global hook install the installer refused, a launch whose agy is older than the `1.2.6` arming floor, and a retired incarnation.
