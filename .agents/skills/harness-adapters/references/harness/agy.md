...
Verified for crew, scout, secondmate, and primary work on tmux on 2026-07-30 with Anti-Gravity CLI 1.1.8.
...
| Model flag | `--model <id>`; discover ids with `agy models`, whose verified 1.1.8 listing carried Gemini 3.6 Flash, Gemini 3.5 Flash, Gemini 3.1 Pro, Claude Sonnet 4.6, Claude Opus 4.6 Thinking, and GPT OSS 120B profiles. |
| Effort flag | `--effort <low\|medium\|high>`; 1.1.8 rejects `xhigh` and `max`, so Firstmate records either in task metadata and omits the flag. |
...
| Composer | A bare `>` row between two long horizontal separators, followed by `? for shortcuts` when idle or `esc to cancel` when busy. `../../../bin/fm-composer-lib.sh` recognizes that bare `>` only inside its complete separator pair with a verified footer; a bare unstructured `>` remains an unsafe shell prompt and returns `unknown`. |
...
`../../../tests/fm-agy-harness.test.sh` is the portable regression, and the default-on `../../../tests/fm-harness-liveness-drift-live-e2e.test.sh` covers an installed `agy` for liveness drift.
