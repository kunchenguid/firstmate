...
| Fact | Value |
|---|---|
| Busy state | The last rendered-tail fallback, isolated to Grok pending a semantic source: ASCII mid-turn `Ctrl+c:cancel`, absent from idle bar `Shift+Tab:mode │ Ctrl+.:shortcuts`, never the locale-fragile braille spinner. Known stale on 1.0.13, whose captured active footer no longer contains `Ctrl+c:cancel`, so a genuinely busy turn reads as not-working across every Grok busy consumer until the signature is re-verified. See [`docs/verification/grok-interrupt.md`](../../../../../docs/verification/grok-interrupt.md) for the captures, the affected consumers, and the re-capture caveat. |
...
| Interrupt | Single `Ctrl+C`, retained as the established path. Escape is version- and mode-dependent: on 1.0.13 one Escape also visibly cancelled the turn in `--no-alt-screen` trials, which the spawn shape does not use, and neither key is proven to stop already-running tool work on that build. See [`docs/verification/grok-interrupt.md`](../../../../../docs/verification/grok-interrupt.md). |
