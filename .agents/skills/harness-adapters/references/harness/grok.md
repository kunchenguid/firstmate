...
| Fact | Value |
|---|---|
| Busy state | The last rendered-tail fallback, isolated to Grok pending a semantic source: ASCII mid-turn `Ctrl+c:cancel` (0.2-series) or `Esc:cancel` (1.0.13 footer `Shift+Tab:mode | Esc:cancel | Ctrl+x:shortcuts`, which has no `Ctrl+c:cancel` and whose `Shift+Tab:mode` is present while thinking as well as when idle, so that token is not idle-only), never the locale-fragile braille spinner. |
