...
| Interrupt | Single `Ctrl+C`, retained as the established path. Escape is version-dependent: it only focused scrollback on 0.2.73, while on 1.0.13 one Escape also visibly cancels the turn under the production launch shape. `Ctrl+C` matches it there on every measured axis, and neither key reliably stops already-running tool work on that build, so nothing favours switching. See [`docs/verification/grok-interrupt.md`](../../../../../docs/verification/grok-interrupt.md). |
