...
| Fact | Value |
|---|---|
| Busy state | The last rendered-tail fallback, isolated to Grok pending a semantic source: the mid-turn footer row carrying `Shift+Tab:mode` and then `Esc:cancel` across the measured `│` separator (measured on 1.0.13; both keys are required on one row rather than the bare token, so prose naming them stays idle, with one intervening segment such as `· always-approve` tolerated because its footer position is unmeasured) or the older `Ctrl+c:cancel`, both absent from the idle bar (`Shift+Tab:mode │ Ctrl+x:shortcuts` on 1.0.13), never the locale-fragile braille spinner. |
