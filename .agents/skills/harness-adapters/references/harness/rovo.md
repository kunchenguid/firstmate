...
1. **Readiness gate** (`rovo_wait_for_ready` in `../../../bin/fm-spawn.sh`): poll for the fresh-launch `Welcome to Rovo!` ASCII banner, falling back to composer-empty. The banner is the primary signal because it proves a fresh launch outright, where composer-empty only proves the composer is clear; rovo's idle placeholder chip is ghost text the near-achromatic ceiling strips (see "Composer ghost text" below), so the fallback does read `empty`.
...
## Composer ghost text: stripped by the near-achromatic ceiling
...
That placeholder's foreground is `38;2;162;163;165` (luminance ~163) while real typed text in the same box is `38;2;206;207;210` (luminance ~207), so both sit above the default `FM_COMPOSER_GHOST_LUMA_MAX` of 128.
`fm_composer_strip_ghost` in `../../../bin/fm-composer-lib.sh` applies a higher ceiling, `FM_COMPOSER_GHOST_GRAY_LUMA_MAX` (default 180), to a NEAR-ACHROMATIC truecolor run only, so the chip strips as ghost text and a fresh rovo composer reads `empty`.
rovo's real typed text is also near-gray but is separated by luminance alone at ~207 and is kept, and muse's real prompt glyph is strongly chromatic (spread 165) so no luminance ceiling can reach it - which is why a gray-only ceiling is safe where raising the shared default was not.
Measured values and the four pinned cases are in `../../../../docs/verification/rovo.md`.
