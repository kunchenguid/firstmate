...
| Effort flag | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'`, verified on codex-cli 0.142.1 whose installed schema contains `model_reasoning_effort`. `max` is per model: each entry's `supported_reasoning_levels` in `${CODEX_HOME:-~/.codex}/models_cache.json`, the catalog Codex refreshes and `codex debug models` prints, says whether that model accepts it, so `../../../bin/fm-spawn.sh` passes `max` only when that catalog advertises it for the launched model and otherwise omits it with a one-line warning naming the model and the effort in force. Verified on 2026-09-25 with codex-cli 0.157.0: `codex exec -m gpt-6-astra -c 'model_reasoning_effort="max"'` reported `reasoning effort: max`. |
