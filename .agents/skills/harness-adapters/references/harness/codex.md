...
| Effort flag | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'`, verified on codex-cli 0.142.1 whose installed schema contains `model_reasoning_effort`, active config uses it, and bundled catalog advertised only the first four values while omitting `max`; Firstmate passes `max` only for a model whose installed catalog entry at `${CODEX_HOME:-~/.codex}/models_cache.json` lists `max` in `models[].supported_reasoning_levels[].effort`, read once through `../../../bin/fm-harness.sh codex-max-models`; an absent, unreadable, or malformed catalog rejects no profile: the resolver and bootstrap accept `max` with one diagnostic naming the catalog, and the launch records `max` in task metadata and omits the flag with a notice until the catalog is readable; on codex-cli 0.155.1 that catalog advertises `max` for `gpt-6-sol`, `gpt-6-luna`, `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`, while `gpt-5.5` stops at `xhigh`. |
