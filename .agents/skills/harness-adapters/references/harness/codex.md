...
| Skill invocation | `$<skill>`, for example `$bearings`; `/<skill>` is Claude-only and Codex rejects it as "Unrecognized command". |
...
This is why `$bearings` reaches a Codex worker instead of being consumed by the popup.
