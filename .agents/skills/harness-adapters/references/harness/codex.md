...
| Fact | Value |
|---|---|
| Busy state | Unknown until a semantic source is live-verified: the app-server turn lifecycle is unreachable for a pane worker, and project lifecycle hooks did not fire for a Firstmate-launched worker. A record-free Codex task on Herdr may still read working from Herdr's independently verified native `busy` verdict; native idle, malformed or mismatched records, and every non-Herdr backend remain unknown. |
