...
| Provider profile | `--profile <name>`, threaded ahead of `--model` by `fm-spawn.sh --codex-profile <name>` (codex-only). It selects an OpenAI-compatible provider profile file (e.g. DeepSeek's `~/.codex/deepseek.config.toml`, chosen by `codex --profile deepseek`), so the profile carries the model and `--model` becomes optional; an explicit `--model` still overrides it. codex-cli >= 0.154 rejects a legacy `[profiles.x]` table inside `~/.codex/config.toml`, so the profile lives in its own file. Recorded as `codex_profile=` in the task record and carried forward on a codex-to-codex relaunch. |
