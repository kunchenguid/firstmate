...
| Launch | Positional instructions in the acquired task worktree; `../../../bin/fm-spawn.sh` owns the flags and foreign-marker clearing. |
...
| Autonomy | Sandboxed automatic review; the dated bypass-mode observations in `../../../docs/verification/runtime-backends.md#cursor-agent-cli` do not verify this launch posture. |
| Trust | Workspace trust remains necessary for each fresh task path and for loading project hooks; autonomy does not replace it. |
...
The adapter adds no external writable roots; delivery of positional instructions does not establish permission to write shared Git metadata or task outcomes outside the worktree.
