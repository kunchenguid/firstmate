...
| Launch | Positional instructions in the acquired worktree; `bin/fm-spawn.sh`'s Cursor launch template owns the flags and foreign-marker clearing. |
...
| Autonomy | Launches enable the sandbox and automatic review; `bin/fm-spawn.sh` owns the exact flags. |
| Trust | Launch-time trust suppresses the dialog and loads project hooks; every task has a fresh path. |
