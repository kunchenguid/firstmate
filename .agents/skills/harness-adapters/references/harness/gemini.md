...
A raw Gemini-shaped launch is an unverified escape hatch: it receives no busy-state wiring or turn-end hook and therefore has no trusted busy state, and it cannot record the exact name `gemini`, which the spawn owner reserves for this canonical template.
