...
| Skill | `/<skill>`, for example `/bearings`; ONE Enter submits, with no popup swallow, and the turn opens with an `Activate Skill` tool call. |
...
It hides `~/.agents/skills`, so Firstmate's user skills silently disappear from that worker.
...
`~/.agents/skills/bearings` is therefore discovered as a user skill and loads even in an untrusted folder.
