...
It is deliberately NOT the worktree's `.gemini/settings.json`: like Claude's `settings.local.json`, that path can be the PROJECT's own committed settings file, so writing it would clobber a project's configuration and retiring it would delete a tracked file.
