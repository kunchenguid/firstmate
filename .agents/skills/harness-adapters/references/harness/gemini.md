...
`AfterAgent` fires on a manual Escape interrupt as well as on normal completion, while Claude emits no interrupt hook and therefore needs the explicit `fm-interrupt` correction owned by `bin/fm-busy-lib.sh`.
