...
`../../../bin/fm-spawn.sh` accepts concrete `--harness`, `--model`, `--effort`, and codex-only `--codex-profile` values selected at intake; scripts never parse natural-language dispatch rules.
