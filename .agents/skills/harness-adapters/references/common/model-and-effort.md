...
Use `low` for well-understood work with an explicit bounded path and `high` for difficult work.
Consider `xhigh` only for genuinely very difficult work, where `high` is insufficient.
