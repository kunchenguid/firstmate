...
`../../../bin/fm-spawn.sh --help` owns launch-axis refusal: never preserve an explicit model or effort only in metadata while omitting it from execution.
Choose a supported setting explicitly, or leave the axis unset to request the harness default; never silently downgrade a specified value.
Recorded requested effort is not observed effective effort; per-model runtime support still needs verification.
