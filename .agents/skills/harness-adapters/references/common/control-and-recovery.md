...
Pi gates the fresh-worktree case too; the spawn pre-registers it in Pi's own store the same way, and unlike Claude, a strict post-launch gate answers any dialog that still renders before the spawn reports success; `references/harness/pi.md` owns that recipe and where the decision persists.
