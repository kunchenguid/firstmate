...
Pi gates the fresh-worktree case too; the spawn pre-registers it in Pi's own store the same way, and unlike Claude, a dialog that still renders is answered with Enter; `references/harness/pi.md` owns that recipe and where the decision persists.
