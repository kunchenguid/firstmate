...
Codex shows a directory-trust dialog on the first run for a repository root it has no recorded decision for, and `references/harness/codex.md` owns where that decision persists and which intake step pre-registers it.
