...
Codex shows a directory-trust dialog on the first run for a repository root, so ship and scout spawns pre-register their fresh worktrees in Codex's own store.
A Codex secondmate remains excluded from that helper and keeps its persistent home's one-time trust decision.
