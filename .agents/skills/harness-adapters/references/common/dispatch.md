...
For an approved new adapter check, use the spawn owner's `--raw` escape hatch only for a trivial supervised task, stating the harness name and model it should record; it refuses to name an already-verified adapter, arms no per-harness wiring, and records `launch=raw` so the task cannot later be relaunched onto a different launch.
