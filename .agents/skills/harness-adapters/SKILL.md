...
Firstmate-launched Pi crewmates and secondmates disable ambient extension, skill, and prompt-template discovery with `--no-extensions --no-skills --no-prompt-templates`.
The launch keeps project context-file discovery enabled so the target repository's `AGENTS.md` or `CLAUDE.md` still governs the worker, and explicit Firstmate extensions continue to load through `-e`.
Verified 2026-07-21 on Pi 0.80.10: `pi --help` documents that each `--no-*` flag disables discovery while explicit `-e` paths still work; the isolated launch regression in `tests/fm-spawn-dispatch-profile.test.sh` pins the emitted command and rejects `--no-context-files`.
...
When a secondmate is launched on Pi, `fm-spawn.sh --secondmate` uses the same ambient-discovery isolation and explicitly loads both `-e .pi/extensions/fm-primary-turnend-guard.ts` and `-e .pi/extensions/fm-primary-pi-watch.ts`, both already present in the secondmate home's git worktree.
