...
If the captain asks for a new harness, propose verifying it first: spawn a trivial supervised task using `fm-spawn --unverified-adapter <executable>` with one repeated `--adapter-arg=<literal>` per argument, confirm every fact empirically, then record the mechanics in `fm-spawn`, its semantic busy source and trust gate in `bin/fm-busy-lib.sh`, any needed `FM_COMPOSER_IDLE_RE` empty-composer override plus any novel bare agent prompt glyph in `bin/fm-composer-lib.sh`'s shared composer classifier (the one fleet-wide owner of the empty/dead-shell/pending decision, so a new harness's own idle composer is not misread as a dead shell), the tmux agent-process liveness classification in `bin/backends/tmux.sh` when the harness can launch a secondmate, and the verified knowledge here.
The executable must resolve to a regular direct adapter entrypoint whose basename ends in `-agent` or `-adapter`; Firstmate delivers its resolved absolute path and literal arguments as shell-quoted argv, while interpreters, runners, shell command strings, quoted arguments, inline code, substitutions, globs, and aliases remain outside this escape path.
`bin/fm-spawn.sh --help` owns the exact literal-argv syntax and migration error.
...
Firstmate launches every Claude crewmate, scout, and secondmate with `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false`, scoped to Firstmate-launched agents through `bin/fm-spawn.sh`, so it never touches the captain's global config.
