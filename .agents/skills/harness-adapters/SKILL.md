...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Re-verified 2026-08-03 on codex-cli 0.146.0, superseding a 0.142.1 reading that recorded a low/medium/high/xhigh ceiling. `codex debug models` advertises `max` for `gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra` and `codex-auto-review`, and a live `codex exec` run recorded `collaboration_mode.settings.reasoning_effort = "max"` in its rollout. Omitting `max` silently ran the session at the model default instead, which is `medium` for `gpt-5.6-luna`. `gpt-5.5` and older still cap at `xhigh`. Sol and Terra also advertise `ultra`, which firstmate's shared effort axis does not carry. |
