...
Each adapter's `Busy state` row names only which semantic source that harness uses; `bin/fm-busy-lib.sh` owns the contract itself, including verdicts, source attribution, and the verification gates that keep an unverified harness at unknown, while each machine-consumed rendered busy-footer regex lives in the harness's `bin/harnesses/<name>.sh` adapter.
...
If the captain asks for a new harness, propose verifying it first: spawn a trivial supervised task using `fm-spawn`'s raw-launch-command escape hatch, confirm every fact empirically, then record the mechanics in `fm-spawn`, its semantic busy source and trust gate in `bin/fm-busy-lib.sh`, its rendered busy-footer signature in a new `bin/harnesses/<harness>.sh` adapter registered in `bin/fm-harness-adapter.sh`, any needed `FM_COMPOSER_IDLE_RE` empty-composer override plus any novel bare agent prompt glyph in `bin/fm-composer-lib.sh`'s shared composer classifier (the one fleet-wide owner of the empty/dead-shell/pending decision, so a new harness's own idle composer is not misread as a dead shell), the tmux agent-process liveness classification in `bin/backends/tmux.sh` when the harness can launch a secondmate, and the verified knowledge here.
`bin/fm-harness-adapter.sh` keeps two deliberately different name sets: `FM_HARNESS_KNOWN` for harnesses launchable as a crewmate or secondmate, and the narrower `FM_HARNESS_PRIMARY` for those supported as the primary session.
Add a new harness to the primary set only when its primary-session supervision is genuinely verified, since that set is what `README.md` advertises.
