...
For an unbound primary, Codex runs the Stop hook command with process PWD set to the hook-loaded project root, and no `CODEX_PROJECT_DIR`, `CODEX_WORKSPACE_ROOT`, or `CODEX_CWD` root variable is set.
Firstmate-launched workers use the trusted executable-root and worktree-scope binding owned by [architecture](../../../docs/architecture.md#codex-project-hook-root-anchoring).
