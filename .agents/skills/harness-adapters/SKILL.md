...
A crewmate's per-task hooks are written to `state/<task-id>.claude-settings.json` and loaded with `claude --settings <path>` whenever the launch command can carry that flag, instead of into the copy's `.claude/settings.local.json`.
...
Because the wiring rides that flag, a raw launch command (the unverified-adapter escape hatch) gets the flag inserted right after its command word, but only when that word's basename is exactly `claude`.
...
This is separate from the per-task crewmate hooks above, which `fm-spawn` keeps out of the project's tracked content (see the crewmate hook fact above).
