...
**Primary-session guard fact (verified 2026-07-04, Claude Code 2.1.201; preserved 2026-07-08, Claude Code 2.1.204; Stop-owned auto-arm revalidated 2026-07-30, Claude Code 2.1.220).**
...
A pull-based `WATCHER DOWN` warning inside an active turn is not evidence that the Stop owner failed, so it renders Stop-owned continuity guidance instead of an arm command; `docs/supervision-protocols/claude.md` owns when the manual background arm is genuinely authorized.
