...
| cursor | `--model <model-id>` | mapped to `cursor-grok-4.5-low`, `cursor-grok-4.5-medium`, or `cursor-grok-4.5-high` | Verified 2026-07-23 on Cursor Agent 2026.07.20-8cc9c0b. Cursor has no standalone effort flag. Effort maps to a concrete model only when no explicit model is supplied: `low` and `medium` map directly; `high`, `xhigh`, and `max` map to the highest listed Cursor Grok 4.5 model, `cursor-grok-4.5-high`. Any explicit model wins unchanged. |
