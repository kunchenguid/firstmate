...
The firstmate PRIMARY's own `.codex/hooks.json` pipes Codex's Stop payload through `bin/fm-turnend-guard.sh`, which recognizes Codex's `turn_id` and routes through `bin/fm-turnend-guard-codex.sh` for a non-blocking `systemMessage` and a bounded durable unhealthy-watcher alarm.
