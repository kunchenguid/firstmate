...
The executable must be a direct adapter entrypoint whose basename ends in `-agent` or `-adapter`; interpreters, runners, shell command strings, quoted arguments, inline code, substitutions, globs, and aliases are outside this escape path.
