...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified 2026-08-01 on codex-cli 0.146.0 with `codex debug models`, which reports low/medium/high/xhigh/max/ultra for the installed worker models. Firstmate leaves `ultra` outside the shared effort axis until its automatic delegation behavior is tested. |
