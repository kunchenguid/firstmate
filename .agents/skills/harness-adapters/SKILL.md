...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified 2026-07-29 on codex-cli 0.145.0. The model catalog advertises max on the GPT-5.6 family and a live `codex exec` run at max reported `reasoning effort: max`. An unsupported level is rejected by the API with a visible 400, never silently downgraded. `ultra` is excluded on purpose: it delegates to parallel agents, which does not belong inside firstmate supervision. |
