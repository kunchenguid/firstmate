...
| pi | `--model <model>` | `--thinking <off\|minimal\|low\|medium\|high\|xhigh\|max>` | Verified 2026-07-13 on Pi 0.80.6 (re-verified 2026-07-23 for `off` and `minimal`). `pi --help` advertises `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; `pi --print --model openai-codex/gpt-5.6-sol --thinking max 'Reply with exactly OK.'` completed successfully; `pi --print --model zai/glm-5.2 --thinking off 'Reply with exactly OK.'` and `--thinking minimal` both completed successfully on 2026-07-23. |
