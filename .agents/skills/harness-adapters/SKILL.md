...
Dialog-shaped text alone is never trusted: the handler corroborates with herdr's `agent get`, so a brief or transcript that merely quotes dialog wording sends no keys once the agent reports working; idle/done are trusted only when no dialog shape is visible, since a harness parked at a real dialog is not generating.
