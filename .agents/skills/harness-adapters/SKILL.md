...
| pi / pi-signed | `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-07-28 on Pi and pi-signed 0.82.0. Both expose the same accepted thinking levels and completed the same model-qualified max-thinking smoke. |
...
## pi and pi-signed (VERIFIED 2026-07-28)
...
Pi's shared `PI_CODING_AGENT=true` child marker is therefore disambiguated only by an exact `pi-signed` process in bounded ancestry; names such as `pi-signed-helper` remain unrelated.
