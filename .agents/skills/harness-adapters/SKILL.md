...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified 2026-08-01 on codex-cli 0.146.0. `codex debug models` shows effort support is per model, not per CLI: gpt-5.6-sol and gpt-5.6-terra report low/medium/high/xhigh/max/ultra, gpt-5.6-luna and codex-auto-review stop at max, and gpt-5.5, gpt-5.4, gpt-5.4-mini and gpt-5.3-codex-spark stop at xhigh. fm-spawn stays model-agnostic and deliberately implements no model-capability gate, because the unsupported pairing already fails closed loudly: `codex exec --model gpt-5.5 -c 'model_reasoning_effort="max"'` exits 1 on an HTTP 400 `unsupported_value` naming `reasoning.effort` and listing none, low, medium, high and xhigh, with no work performed (measured 2026-08-01). Firstmate leaves `ultra` outside the shared effort axis until its automatic delegation behavior is tested. |
