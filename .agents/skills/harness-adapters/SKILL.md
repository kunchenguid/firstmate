...
| pi | `--model <model>` | `--thinking <off\|minimal\|low\|medium\|high\|xhigh\|max>` | Verified 2026-07-13 on Pi 0.80.6. `pi --help` advertises `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; `pi --print --model openai-codex/gpt-5.6-sol --thinking max 'Reply with exactly OK.'` completed successfully. `off` and `minimal` verified working 2026-07-23: `pi --print --model zai/glm-5.2 --thinking off 'Reply with exactly OK.'` and `pi --print --model zai/glm-5.2 --thinking minimal 'Reply with exactly OK.'` both returned `OK`. |
