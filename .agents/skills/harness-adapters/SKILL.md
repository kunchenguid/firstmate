...
`fm-spawn` launches inside the task worktree (a git repo root, whether treehouse-managed or Orca-managed), so the picker never appears and grok treats the worktree as a trusted project automatically - no post-launch keystroke is needed.
