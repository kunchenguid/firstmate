...
The watcher extension defers missing/dead startup and new locks to session-start, advances them to owned-only after the first verified ready arm, reserves controlled recovery for `resume`, requires existing ownership for `fork` and `reload`, and starts and verifies every later successor before delivering an ordinary actionable wake.
The model drains and handles that wake without calling `fm_watch_arm_pi`; use the tool only for initial arming when automatic startup has not armed or for explicit recovery, never a foreground bash arm, with the full recovery and clean-exit protocol owned by `docs/supervision-protocols/pi.md`.
