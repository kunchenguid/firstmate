...
The optional local `config/crew-config-dir` knob adds a second env prefix on the same claude launch, `CLAUDE_CONFIG_DIR=<dir>`, so a claude crewmate on a multi-account machine authenticates against a captain-chosen config dir instead of stranding on the login wall when the default `~/.claude` is empty or unauthenticated.
Its value is the first non-empty, non-comment line, with a leading `~`/`~/` and any `$HOME`/`${HOME}` expanded and the result shell-quoted, so nothing in the knob is re-parsed at launch; an absent or value-less file leaves the launch byte-identical to bare `claude` on its default config dir.
Like the ghost-text variable it is a simple-command env prefix on the claude verb only, scoped to that firstmate-launched agent, so it never mutates the captain's global config and no other harness sees it (`docs/configuration.md` "Claude config dir").
