...
It only ever reclaims what that spawn itself allocated, and never at the cost of work: a worktree holding uncommitted changes (or one git cannot inspect) is left checked out with the manual return command printed, and a `/tmp/fm-<id>` that already existed is left alone because respawning a live id is a documented recovery path.
