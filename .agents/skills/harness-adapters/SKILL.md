...
| codex | `--model <model>` | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'` | Verified on codex-cli 0.147.0 (2026-08-19). The installed model catalog advertises `gpt-5.6-luna` with low/medium/high/xhigh/max, so firstmate passes `max` when explicitly selected. |
