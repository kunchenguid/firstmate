...
Every primary harness with a tracked turn-end integration has an empirically validated hook path for the "no turn ends blind" guard.
...
`cursor` and `hermes` are verified crewmate adapters but have no tracked primary turn-end integration yet, so a primary running on either relies on `fm-guard.sh` and watcher arm checks.
