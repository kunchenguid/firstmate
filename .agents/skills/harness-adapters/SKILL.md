...
`bin/fm-model-refresh.sh` runs every listing surface above that the installed harnesses expose, records a dated catalog, and names each id that is new since its previous run, so catalog drift is noticed rather than discovered by accident; run it before trusting any model claim written in this file.
A listing states only that a model is offered.
Whether it is actually usable is a separate fact that only a real one-token prompt establishes, and the two genuinely differ, so that check is the same script's opt-in `--probe` and never runs on its own.
...
| Models | Validate against `cursor-agent --list-models` for the current account rather than a fixed list; that list has already drifted twice. It carried only `-high` Grok ids when this row was first written, and on 2026-08-15 it carried a full `cursor-grok-4.6-{low,medium,high,xhigh}` ladder with `-fast` variants alongside the older `cursor-grok-4.5-*` ids, so neither "high only" nor any other remembered shape may be assumed. `bin/fm-model-refresh.sh` is what keeps this row current: it records the live catalog and names every id added since the last run. |
...
| Busy state | Its own per-session record under `<data-dir>/sessions/<session-id>/`, folded on demand by `bin/fm-busy-lib.sh` (source `cline-session`), bound to the pane by the `state/<id>.cline-session` sidecar. Structural, not rendered. TWO signals: the record's `status` (`running` -> busy, `completed`/`failed` -> idle) and, for the ambiguous `idle`, the turn log's last message role. Nothing is armed and no record is ever seeded, the same reason muse and cursor are pull sources. Requires `jq`; without it the fold reports unknown rather than reaching for a formatting-dependent text read. Backend-agnostic: Herdr has no cline integration, so its native `agent_status` for a cline pane sticks at busy forever, and once the cline-only staleness bound fires the classifier skips that uninformative guess and folds this record instead of reporting unknown. |
