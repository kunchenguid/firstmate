...
The primary integrations for `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `omp`, `grok`, and `cursor` have empirically validated hook paths for the "no turn ends blind" guard.
...
The primary integrations for `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `omp`, `grok`, and `cursor` also have wired PreToolUse-equivalent hooks that deny a watcher-arm anti-pattern (shell `&`, truncating pipe, bundling, broad `pkill -f fm-watch`) before it runs.
