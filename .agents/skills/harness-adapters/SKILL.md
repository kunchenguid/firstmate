...
The optional local `config/crew-config-dir` knob adds a second env prefix on the same launch, `CLAUDE_CONFIG_DIR=<dir>`, so a claude agent on a multi-account machine authenticates against a captain-chosen config dir instead of stranding on the login wall when the default `~/.claude` is empty or unauthenticated.
Like the ghost-text variable it is a simple-command env prefix on the claude verb only, scoped to that firstmate-launched agent, so it never mutates the captain's global config and no other harness sees it.
`docs/configuration.md` "Claude config dir" owns the file format, path expansion, missing-directory warning, and inheritance rules.
