...
| grok | `--model <model>` | `--reasoning-effort <low\|medium\|high\|xhigh>` | Verified on grok 1.0.3 (1a29d5bc12) (2026-08-15). `--effort` is an alias, but firstmate's profile axis is reasoning effort. As of 1.0.3 the accepted set is `xhigh, high, medium, low`; `max` is rejected, so firstmate omits it. |
