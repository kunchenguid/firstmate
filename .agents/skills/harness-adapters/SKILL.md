...
The watcher extension defers missing/dead startup and new locks to session-start, reserves their recovery for `resume`, requires existing ownership for `fork` and `reload`, and starts and verifies every later successor before delivering an ordinary actionable wake.
