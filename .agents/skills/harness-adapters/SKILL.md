...
The firstmate PRIMARY's own `.claude/settings.json` registers three Stop hooks: the synchronous context-refresh threshold hook `bin/fm-context-restart-claude-hook.sh` (owned by `docs/context-refresh.md`), `bin/fm-turnend-guard.sh --claude`, and the Stop-owned auto-arm `bin/fm-claude-stop-autoarm.sh` (`asyncRewake: true`, `timeout: 28800`), and exiting the guard with status 2 plus stderr reliably forces the model to continue.
