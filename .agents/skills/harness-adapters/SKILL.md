...
Copilot's inherited `COPILOT_CLI=1` marker is accepted only when a live Copilot ancestor corroborates it, and explicit Claude, Pi-family, or Grok child markers win first.
...
The primary integrations for `claude`, `codex`, `copilot`, `opencode`, `pi`, `pi-signed`, and `grok` have empirically validated hook paths for the "no turn ends blind" guard.
`claude` and `codex` block directly through Stop hooks that preserve exit status 2 and stderr from `bin/fm-turnend-guard.sh`.
`copilot` converts the same shared predicate to its native `agentStop` decision object.
...
The primary integrations for `claude`, `codex`, `copilot`, `opencode`, `pi`, `pi-signed`, and `grok` also have wired PreToolUse-equivalent hooks that deny a watcher-arm anti-pattern (shell `&`, truncating pipe, bundling, broad `pkill -f fm-watch`) before it runs.
...
- `copilot`: verified on 1.0.76-0; native `sessionStart` receives `source=new`, and `bin/fm-copilot-hook.sh` injects the wrapper line as `additionalContext`.
...
Copilot uses one standalone Bash task with `mode=async` and `detach=false` around `bin/fm-watch-arm.sh`; task completion becomes a system notification.
...
| copilot | `--model <model>` | `--effort <low\|medium\|high\|xhigh\|max>` | Verified on GitHub Copilot CLI 1.0.76-0. |
...
| copilot | Open the current interactive session's `/model` picker; `copilot --help` lists the current model identifiers and effort levels. |
...
- copilot: use natural language to invoke the skill by name; Copilot discovers repository and user skills through its native skill tool.
...
## copilot (VERIFIED 2026-07-27, GitHub Copilot CLI 1.0.76-0)

| Fact | Value |
|---|---|
| Busy-pane signature | `esc interrupt` in the active footer, observed as `◉ Working · 86 B esc interrupt`; the idle footer omits it. |
| Exit command | `/exit` |
| Interrupt | single Escape, as advertised by the active footer |
| Skill invocation | Natural-language request to invoke the named skill |
| Autonomy | `--allow-all --no-ask-user` |
| Env marker | `COPILOT_CLI=1`, accepted only with corroborating Copilot ancestry |

`fm-spawn` launches Copilot in persistent interactive mode with `-i <typed launch brief>`.
It resolves the executable from `PATH`, then from the current primary's process ancestry so a Copilot primary installed through its versioned loader can spawn workers even when no `copilot` shim is on `PATH`.

The first interactive launch in an untrusted clone displays the folder-trust dialog before any repository hook runs.
Choose the persistent trust option when appropriate so later sessions and linked worktrees share the repository approval.
After every spawn, inspect the pane and accept that dialog if it appears, then verify the brief started.

Copilot crews receive one task-specific gitignored `.github/hooks/fm-firstmate-turn-end-<id>.json` file.
Its native `agentStop` hook touches the task's private `state/<id>.turn-ended` marker.
Teardown removes the hook before returning the worktree.

**Primary-session guard fact (verified 2026-07-27, GitHub Copilot CLI 1.0.76-0).**
The tracked `.github/hooks/fm-primary.json` registers native `sessionStart`, `preToolUse`, and `agentStop`.
`bin/fm-copilot-hook.sh` converts the session-start nudge to `additionalContext`, forwards native camelCase tool payloads to the shared safety checkers, and converts a shared turn-end-guard exit 2 to `{"decision":"block","reason":"..."}`.
Copilot also reads `.claude/settings.json`, so every tracked Claude hook exits silently when `COPILOT_CLI=1` and `CLAUDE_PROJECT_DIR` is absent; a genuine Claude child still has `CLAUDE_PROJECT_DIR` and keeps the Claude path.
The live interactive probe observed one `sessionStart` payload with `source=new`, one first `agentStop` with `stop_hook_active=false`, and one forced continuation followed by `stop_hook_active=true`.

Copilot's primary watcher protocol runs `bin/fm-watch-arm.sh` as one standalone Bash task with `mode=async` and `detach=false`.
The task completion system notification is the wake.
After handling an ordinary wake, start exactly one replacement task; never use shell `&`.
