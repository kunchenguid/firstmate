...
| pi | `--provider <provider>` when a bare model id resolves in `~/.pi/agent/models.json`; `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-07-13 on Pi 0.80.6. `pi --help` advertises `off`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; `pi --print --model openai-codex/gpt-5.6-sol --thinking max 'Reply with exactly OK.'` completed successfully. Provider inference was added for VibeProxy-style bare model ids such as `gpt-5.5`. |
