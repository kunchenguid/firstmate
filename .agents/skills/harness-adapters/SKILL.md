...
Every verified primary harness except `kimi` has an empirically validated hook path for the "no turn ends blind" guard.
...
`kimi` runs without any installed firstmate hook until a safe idempotent merger for its shared `~/.kimi-code/config.toml` is verified; see the kimi section below.
...
Every verified primary harness except `kimi` (no installed hooks; see the kimi section below) also has a wired PreToolUse-equivalent hook that denies a watcher-arm anti-pattern (shell `&`, truncating pipe, bundling, broad `pkill -f fm-watch`) before it runs.
...
- `kimi`: no native adapter; firstmate installs no Kimi hooks (see the kimi section below).
...
Kimi also uses bounded foreground checkpoints through `bin/fm-watch-checkpoint.sh` (cadence `FM_KIMI_WATCH_CHECKPOINT`) because no tracked background-task wake mechanism is verified for Kimi.
