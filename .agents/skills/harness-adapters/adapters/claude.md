...
This is separate from the per-task crewmate turn-end hook in `bin/fm-spawn.sh` (that one just `touch`es a marker file in a task's own `.claude/settings.local.json`).
