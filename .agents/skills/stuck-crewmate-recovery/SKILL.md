...
Use `treehouse status --root <this-firstmate-home>` for treehouse-backed tmux, herdr, zellij, or cmux tasks - each home's task-worktree pool is rooted at that home, so a rootless `treehouse status` inspects a different pool - and use the recorded `orca_worktree_id=` and `terminal=` for Orca tasks.
