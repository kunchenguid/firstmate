...
Load `secondmate-provisioning` instead for `kind=secondmate` recovery, and `executor-dispatch` for `kind=executor`, whose dead endpoint is its expected terminal shape rather than a stuck worker.
