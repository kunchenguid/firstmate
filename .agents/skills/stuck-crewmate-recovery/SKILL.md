...
A PR or CI run matched to the crew's branch and current code remains authoritative when the endpoint is dead: handle a terminal or in-progress state through the normal lifecycle, and keep supervising active work instead of creating a duplicate worker.
