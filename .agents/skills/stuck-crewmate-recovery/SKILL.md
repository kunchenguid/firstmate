...
Use `treehouse status` for treehouse-backed tmux, herdr, zellij, or cmux tasks, `git -C <project> worktree list` for a secondmate home's copies of its own clone, and use the recorded `orca_worktree_id=` and `terminal=` for Orca tasks.
