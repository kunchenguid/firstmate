...
4. If the crewmate is genuinely wedged after redirection, or its recorded tmux endpoint is authoritatively gone while the local copy remains, relaunch it with `FM_HOME=<this-firstmate-home> bin/fm-control.sh <task-id> relaunch --note '<progress so far>'`.
   That plane stops a still-running agent, treats a positively missing endpoint as already stopped, reconstructs a missing tmux endpoint in the same local copy, and restores the prior record if the replacement cannot start; missing Herdr endpoints currently refuse until restoration exclusion is atomic ([`docs/agent-control.md`](../../../docs/agent-control.md)).
