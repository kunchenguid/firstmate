...
1. Peek the pane, and check the task's steering inbox (`state/<id>.inbox/`) for unhandled `*.msg` records - a stale wake naming an unread firstmate instruction means the worker never acknowledged a durable steer, and the record itself shows exactly what was intended.
   When that same wake instead reports that every doorbell delivery attempt was suppressed because the endpoint's input line holds unsubmitted text, the worker is not the blocker and the rest of this ladder is the wrong move: the steering plane will not submit content it did not just type, so read that input line, submit it when it is firstmate's own stranded doorbell or clear it when it is the worker's half-typed content, and let the next re-ring land.
