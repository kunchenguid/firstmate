...
Use `treehouse status` for treehouse-backed tmux, herdr, zellij, or cmux tasks, use the recorded `orca_worktree_id=` and `terminal=` for Orca tasks, and use the recorded `superset_workspace_id=` and `superset_terminal_id=` for Superset tasks.
