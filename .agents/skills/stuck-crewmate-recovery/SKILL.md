...
1. Peek the pane, and check the task's steering inbox (`state/<id>.inbox/`) for unhandled `*.msg` records - a stale wake naming an unread firstmate instruction means the worker never acknowledged a durable steer, and the record itself shows exactly what was intended.
   If the last status is `done:` or `failed:`, do not send, ring, recover, or relaunch for leftover inbox records.
   Preserve those records for audit and cleanup.
   A later nonterminal `working:` status means the task is active again and normal recovery applies.
2. If the crewmate is waiting on a question its brief already answers, answer in one line via `FM_HOME=<this-firstmate-home> bin/fm-send.sh <task-id> '<text>'` from an active firstmate session unless `FM_HOME` is already set to the active firstmate home.
