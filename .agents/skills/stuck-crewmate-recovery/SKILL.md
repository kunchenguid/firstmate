...
The same relaunch can recover a destroyed Herdr endpoint.
It proves that the pane is gone before it creates a replacement pane.
A stopped Herdr server does not prove pane loss.

The same relaunch can recover a missing tmux window from a verified seat in the recorded session.
The adapter verifies the seat's server process, pane, and session.
It then verifies that the exact recorded window is absent.
The relaunch creates only that exact session and window.
It keeps the endpoint binding unchanged.
Run this recovery from the owning firstmate seat.
An unbound shell, stale seat, or different session refuses.

Do not bypass a refusal with a new spawn.
A live agent can still own the worktree when absence is not proven.
See [`docs/agent-control.md`](../../../docs/agent-control.md) "Reclaiming a task whose endpoint is gone".
A secondmate is the exception.
Recover it through `bin/fm-spawn.sh <id> --secondmate` as above.
