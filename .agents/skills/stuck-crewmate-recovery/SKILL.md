...
For tmux and Herdr, rerunning `bin/fm-spawn.sh` with the task's original project, delivery posture, and profile reuses that exact recorded copy only after the shared custody check and a recovery-grade `dead` or `missing` endpoint verdict.
Zellij, Orca, and cmux have no recovery-grade agent-process classifier, so an existing record refuses automatic `fm-spawn.sh` recovery rather than treating an unreadable endpoint as dead; preserve it for manual reconciliation under that backend's operator guide.
Never allocate a fresh generic copy while the recorded one is unaccounted for, and never hand-create replacement metadata around the custody check.
