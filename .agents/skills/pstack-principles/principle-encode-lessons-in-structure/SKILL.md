...
Encode recurring fixes in mechanisms (tools, code, metadata, automation) instead of textual instructions. Every error, supervisor correction, and unexpected outcome is a learning signal. Capture it, route it, and close the loop.
...
**Feedback loop:**
- **Capture every correction.** When the supervisor intervenes or tests fail, decide if it's a one-off or a pattern.
