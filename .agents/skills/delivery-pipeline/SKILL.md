...
- **Slot busy** - the task is held with its approval already banked, so it is not re-approved when it starts.
  ...
  The item stays load-held and serialized the whole time it waits, and nothing on this branch releases it or reassigns its kind, because only the blocking task merging frees the slot.
  No hold sits unnoticed: keeping a long wait visible is a review responsibility, not a timer, and the heartbeat fleet review that already re-evaluates held work is where firstmate raises one that has aged past a day, as ordinary batched supervision carrying the blocking PR's full URL and the merge, re-scope, or explicit P0 override options.
