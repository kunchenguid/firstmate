...
To move a live LOCAL secondmate onto a newly pinned harness, model, or effort without a full recovery, set `config/secondmate-harness` and use the custody-gated relaunch in [`docs/agent-control.md`](../../../docs/agent-control.md#live-secondmate-context-custody); the control plane re-resolves that pin and launches the replacement in the same home.
...
For a live REMOTE one, use the custody-gated host-side relaunch in [`docs/remote-secondmates.md`](../../../docs/remote-secondmates.md); pass the profile explicitly and use `default` for an absent pin, because `config/secondmate-harness` is not inherited and the copy on that host belongs to a different home.
