...
When a local Claude-backed secondmate needs cross-repo skills, choose a curated subset and pass `--skills <name[,name]...>` on its fresh `bin/fm-spawn.sh` launch.
[`docs/skill-system.md`](../../../docs/skill-system.md) owns the map, standalone composition helper, supported launch boundaries, Claude overlay load point, and one-canonical-copy symlink rule.
