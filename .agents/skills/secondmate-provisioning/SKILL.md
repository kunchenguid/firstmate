...
description: >-
  ...
  Also use when that home is, or should be, on another fleet machine or on a configured SSH remote route.
  Covers local leases, whole-home remote routes, transactional seeding, record intake for an existing or inherited domain, project clone restrictions, secondmate harness pins, inherited local-material push, idle charter, handoff helper, teardown safety, and the cross-machine contract.
...
A whole-home SSH remote route uses:
