...
That no-fetch path is a purely local fast-forward of tracked files, never an origin fetch, and it never touches the gitignored operational dirs, so a secondmate's backlog, projects, and in-flight work are never disturbed; a linked worktree advances immediately, while a standalone clone that lacks the target receives `/updatefirstmate`'s exact selected update commit, imported from the primary code root before the same guarded fast-forward.
...
`/updatefirstmate` first reconciles that remote code root's own fork origin with the canonical upstream and stops there unless that comparison yields a clean fast-forward, then runs the guarded home sync.
