...
After a new local placement or a remote outbox receipt becomes durable, the helper sends one marked routed-work instruction through the receiving secondmate's recorded endpoint; missing or failed local delivery makes the command fail loudly with the moved work intact, while a failed remote wake remains a separately retried best-effort nudge.
[`bin/fm-backlog-handoff.sh`](../../../bin/fm-backlog-handoff.sh) owns remote outbox release after durable receipt and stable wake-correlation retry behavior.
