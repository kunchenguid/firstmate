...
`--projects-root <dir>` seeds an org-shaped home whose projects are the preexisting sibling repositories under `<dir>`: the seed writes the resolved absolute `<dir>` to `config/projects-root`, skips the `projects/` operational-directory requirement, and registers each named project in `data/projects.md` instead of cloning it - a named project that is not registered in this home, is neither at the path this home's `data/project-paths.json` names for it nor a sibling repository under that root, or (when this home pins the alias to a location through its own `config/projects-root` or `data/project-paths.json`) is not that same repository, fails loudly; a `<dir>` that resolves to a path with whitespace is refused.
A path-registered alias carries its `data/project-paths.json` entry and its delivery mode into the child (`bin/fm-home-seed.sh` header).
