...
The GitHub routing copy contains non-secret profile ids, paths, and bindings only; never copy any referenced profile directory, `hosts.yml`, Keychain or keyring material, credential-helper response, environment, or token value into a secondmate home.
...
A separate, literal-content config reread is required whenever inherited agent-readable `config/*` material changes under an already-running secondmate.
The operational `config/github-accounts.json` file is validated and sanitized during propagation but is never embedded in agent instructions.
