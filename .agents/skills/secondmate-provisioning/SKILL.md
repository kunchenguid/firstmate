...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, and the one shared captain-preference file `data/captain-shared.md`.
...
The inherited `config/backend` keeps a secondmate home's own task sessions on the primary's selected backend without changing the normal per-home backend resolution order.
