...
The same placement-specific launch and deferred bootstrap sweep also propagate the primary's local material declared by `fm_config_inherit_items` in [`bin/fm-config-inherit-lib.sh`](../../../bin/fm-config-inherit-lib.sh), subject to that library's session-scoped exclusions.
