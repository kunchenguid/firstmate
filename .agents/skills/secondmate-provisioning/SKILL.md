...
A home whose live destination bytes would produce a payload byte-identical to its latest already-delivered generation is skipped the same way, and one delivery that would carry two byte-identical generations sends only the first.
