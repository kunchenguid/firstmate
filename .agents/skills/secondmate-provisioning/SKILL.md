...
On the Herdr backend the second mate starts in its Herdr workspace's root directory with `FM_HOME` naming its home, and a harness other than Claude refuses when that root differs from the home; [second-mate working directory](../../../docs/configuration.md#second-mate-working-directory) owns the rule, including which launches it covers and how the home's contract, skills, and hooks are carried.
