...
Before launch, `fm-spawn.sh --secondmate` locally fast-forwards the home to the primary firstmate checkout's current default-branch commit when it is safe; dirty, diverged, ahead, or in-flight homes launch unchanged with a warning.
A home reads ahead when it holds the primary's commit plus newer ones, which has two causes: this primary is behind origin, or the home committed its own tracked change on top of a current primary.
Neither is a fault in the home, but only the first means the primary is stale, so check the commits in `<primary-commit>..HEAD` in the home before concluding it is; the `bootstrap-diagnostics` skill owns the operator response.
