...
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the firstmate repo itself, whose home is a firstmate worktree, whose writers take pooled worktrees of the same repo, and whose reader scouts use checkout-free scratch directories.
...
A remote home is a standalone clone on another machine, so that host imports the one commit it was given - already present, else from that host's own Firstmate copy without moving it, else from that copy's forge origin, with a distinct home origin retained for adopted layouts - and skips with an actionable reason when none of them holds it, which is what an unpushed primary commit looks like from there.
