...
Bootstrap reports successful AGENTS.md re-read sends as `BOOTSTRAP_INFO:` and only emits `NUDGE_SECONDMATES:` when that send fails or is deferred and needs retry.
...
A reread is an automatic send (`bin/fm-send.sh` header, `--automatic`), so while the secondmate waits on its own open decision or blocker it is deferred rather than sent: the retry marker stays, the diagnostic reads `CONFIG_REREAD: secondmate <id>: deferred: ...`, and the deferral is not a failure.
The deferral is flagged, and the watcher delivers the retained reread within one poll of that decision closing (`bin/fm-config-push.sh --retry-deferred`), for local and remote secondmates alike.
