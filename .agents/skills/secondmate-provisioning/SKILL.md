...
An explicit per-spawn `--harness` flag or positional harness arg starts clean on model and effort too, unless the caller also passes explicit `--model` or `--effort`; a raw launch does the same only when no delegated Pi profile is active.
...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/herdr-presentation-spaces`, `config/pi-delegated-profile`, and the one shared captain-preference file `data/captain-shared.md`.
The propagation guard requires every destination config path to be gitignored, and the primary-authoritative copy remains separate from the tracked-files fast-forward: it re-converges every live home whether or not its tracked files advanced, and it touches only the declared items.
Propagation failures warn without blocking secondmate launch or session-start continuation, except that launch and recovery require an active `config/pi-delegated-profile` to report verified convergence before creating an endpoint.
...
These config values remain defaults and rules only, except for the delegated Pi profile's deliberate fail-closed launch policy described in `docs/configuration.md`; the other inherited values must not harden `fm-spawn` to reject a deliberate runtime choice that differs from their configured defaults.
