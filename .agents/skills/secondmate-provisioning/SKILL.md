...
`/updatefirstmate` first updates the remote code root from its own origin, then updates the persistent home under that exact home's independent self-update policy; absent/default still runs this guarded sync, and `docs/configuration.md` owns the opt-in alternative.
