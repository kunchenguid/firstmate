...
`config/claude-account` and `config/pi-account` are not inherited: they name the accounts that home's own workers spend; a local secondmate agent's own launch reads the launching home instead, and a remote one reads its own remote home's file ([`docs/configuration.md`](../../../docs/configuration.md) "Worker accounts").
