...
Local recovery reuses that exact secondmate's recorded backend unless current authority for that task authorized an explicit per-task `--backend` replacement.
