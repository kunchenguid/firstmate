...
For a local route, an explicit per-spawn `--harness` flag, positional harness arg, or structured `--unverified-adapter` launch starts clean on model and effort too, unless the caller also passes explicit `--model` or `--effort`.
A remote route accepts only a verified harness adapter and refuses an unverified-adapter launch at the host boundary.
