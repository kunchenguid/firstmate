...
Resolve each project's origin yourself from the captain, the project registry, a clone that exists elsewhere, the [capability-based tool-selection contract](../harness-adapters/references/common/tool-selection.md), or an explicit paste, and name it as `<project>=<origin-url>`; the seed validates and transports what you supply.
