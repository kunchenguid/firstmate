...
For a local route, an explicit per-spawn `--harness` flag, positional harness arg, or `--raw` launch command starts clean on model and effort too, unless the caller also passes explicit `--model` or `--effort`; a `--raw` launch must pass `--model`.
A remote route accepts only a verified harness adapter and refuses a `--raw` launch command, and an explicit `--env` path, at the host boundary.
