...
`config/claude-account` and `config/pi-account` are not inherited: they name the accounts that home's own workers spend, and a secondmate agent's own launch reads the launching home instead ([`docs/configuration.md`](../../../docs/configuration.md) "Worker accounts").
