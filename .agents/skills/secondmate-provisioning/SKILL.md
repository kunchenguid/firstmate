...
A bare `<harness>` selects no model or effort token from this file; `harness-adapters` owns any adapter-specific launch default.
...
An explicit per-spawn `--harness` flag, positional harness arg, or raw launch command does not inherit model or effort tokens from this file; explicit profile flags still select those axes, and a raw launch command owns its own flags.
