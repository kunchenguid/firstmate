...
[`bin/fm-teardown.sh`](../../../bin/fm-teardown.sh)'s header and `endpoint_close_refusal` own adapter-prerequisite and endpoint-close refusals, including the limits on `--force` and record retention; [`docs/verification/runtime-backends.md`](../../../docs/verification/runtime-backends.md#endpoint-close) records the per-backend close evidence.
