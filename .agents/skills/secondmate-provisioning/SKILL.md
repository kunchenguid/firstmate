...
`config/secondmate-harness` may also pin a concrete model and effort for the secondmate agent in the SAME file rather than a new one, as a default line for every secondmate plus optional per-id pin lines that override it for one secondmate; [`docs/configuration.md`](../../../docs/configuration.md) owns the format, and a bare `<harness>` line stays fully backward-compatible.
`bin/fm-harness.sh secondmate-model <id>` and `bin/fm-harness.sh secondmate-effort <id>` print the tokens that apply to that secondmate (empty when absent, or when the file is absent/`default`/harness-only); they read only `config/secondmate-harness`, never `config/crew-harness`, which stays a bare adapter name.
