...
`config/claude-config-dir` is not inherited because a host-local absolute store path is meaningful only on the machine that holds it, so each home names its own.
