...
Before a local launch, `fm-spawn.sh --secondmate` locally fast-forwards the home to the primary firstmate checkout's current default-branch commit when it is safe, or reconciles a clean divergence whose complete local result is already present there (e.g. after a squash merge) with `reset --keep`; for an ESTABLISHED home (one with a prior `state/<id>.meta` task record in this primary, local or remote), a dirty, uniquely diverged, or in-flight home instead launches unchanged with a warning, preserving its own intentional local work, and a genuine divergence gets the same durable reconciliation record `bin/fm-ff-lib.sh` writes for `/updatefirstmate`.
A FRESH home - one with no prior `state/<id>.meta` record, which `bin/fm-ff-lib.sh`'s `secondmate_launch_is_fresh` is the single owner of detecting - gets none of that leniency: it has no local work yet to preserve, so an unconverged sync refuses the launch outright (a local refusal exits before anything is published; a remote refusal is a parent-side preflight before the host-local launch is ever requested), rather than risk running stale or diverged code that could violate the current Herdr presentation contract (`docs/herdr-backend.md` "Presentation spaces").
