...
Because these paths are gitignored, that propagation is separate from the tracked-files fast-forward: it re-converges inherited state in every live home whether or not its tracked files advanced, preserves the deliberate `config/backend` override described below, and touches only the declared items.
...
Private `state/.fm-inherited-backend` provenance stores the exact last-inherited backend bytes, allowing inherited destination state to converge byte-exact or to `ABSENT` while preserving a divergent deliberate per-home backend override.
Override preservation is reported as `skipped - preserving deliberate local override` and does not expand the operator-facing config surface.
