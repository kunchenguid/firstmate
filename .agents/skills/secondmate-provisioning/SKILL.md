...
That no-fetch path is a purely local fast-forward or redundant-divergence reconcile of tracked files, never an origin fetch; a linked worktree advances immediately, while a standalone clone that lacks the target receives firstmate updates through `/updatefirstmate`'s origin refresh.
For merge-watch continuity across those transitions, see [Self-updates stay safe](../../../docs/architecture.md#self-updates-stay-safe).
