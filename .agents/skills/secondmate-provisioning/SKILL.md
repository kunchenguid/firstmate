...
An explicit per-spawn `--harness` flag or positional named harness starts clean on model and effort too, unless the caller also passes explicit `--model` or `--effort`.
