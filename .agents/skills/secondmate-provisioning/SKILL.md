...
`config/dev_startup.env` is inherited but excluded from those literal-content instructions because only future task processes read it, and its values must not be injected into the already-running secondmate's conversation or durable instruction records.
