...
Use the architect runners assigned in the brief or dispatch profile (one per available model family from the brief or dispatch profile).
...
Default: proceed directly to implementation with the synthesized design. No supervisor checkpoint.
...
If the supervisor pushes back on the shape (in a checkpoint or after the fact), treat that as Phase A evidence. Re-ground and re-run Phase B before writing more code.
