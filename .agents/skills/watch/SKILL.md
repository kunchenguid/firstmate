...
metadata:
  internal: true
  vendored-from: "bradautomates/claude-video@83da59fa78c3eee9e20f515fe75c438bb5166efd (2026-06-30, v0.2.0)"
---

<!--
Vendored third-party skill, MIT licensed - see LICENSE in this directory.
Upstream: https://github.com/bradautomates/claude-video (skills/watch).
Kept byte-identical to upstream below this banner except for the frontmatter
metadata block above, so a future upstream sync stays a clean diff.
Firstmate's one-sentence-per-line and plain-dash style rules are deliberately
NOT applied to this file for the same reason.
...
then fails with "Unrecognized option 'vsync'" and the skill returns no frames.
Pin ffmpeg to a 7.x or 8.x RELEASE build, not master. This fleet runs
n7.1.5 installed to ~/.local/bin, verified end-to-end.
If a future upstream version switches to `-fps_mode`, this note can go.
-->
