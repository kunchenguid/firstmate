...
- `FLEET_SYNC: <repo>: tags not updated: origin moved <tags>; ...` - origin repointed tags this clone already had (typically a moving deploy tag), so git refused to overwrite them; the default branch still synced normally.
  The clone's copies of those tags are stale, so accept them with the exact command the line prints, which touches only the named tags, unless a local tag of that name is deliberately different.
