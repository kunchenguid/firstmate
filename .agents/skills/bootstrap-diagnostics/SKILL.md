...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For any reviewed npm tool - `gh-axi`, `chrome-devtools-axi`, `chrome-devtools-mcp`, `lavish-axi`, `tasks-axi`, `quota-axi` - an installed version that does not exactly match its reviewed version, whether older, newer, or malformed, is a plain reinstall request; [`bin/fm-reviewed-toolchain.sh`](../../../bin/fm-reviewed-toolchain.sh) owns the exact reviewed versions, and never argue the pin toward whatever the home happens to have installed.
