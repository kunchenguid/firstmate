...
- `ROUTINE: local recurring routine setup failed` - the private registry, its authenticated check, or its registration is not guaranteed ready.
  Preserve any existing `data/routines.md`, inspect the adjacent `routine-setup:` diagnostic, correct the named local path or permission problem, then rerun session start so idempotent setup can revalidate the check.
