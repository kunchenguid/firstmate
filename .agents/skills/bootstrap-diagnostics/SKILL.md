...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the actionable error, and require correction of the reported schema, harness, effort, or `codexHome` fault (`docs/configuration.md` "Crew dispatch profiles" owns the validated fields) rather than falling back around it or selecting a bad profile.
