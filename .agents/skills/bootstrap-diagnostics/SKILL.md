...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For any axi-family tool - `gh-axi`, `lavish-axi`, `tasks-axi`, `quota-axi` - this also covers an installed version below its floor, which is the current latest published version rather than a feature minimum, so treat it as a plain upgrade request and never argue the floor down to whatever the home happens to have installed.
