...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  It also covers a live binary that lacks the OMP agent (upstream release or plain `go install` / mise clobber of the OMP-capable fork): the printed install command is `bin/fm-install-no-mistakes-omp.sh`, which rebuilds from the local `feat/omp-agent` checkout into GOBIN and must not be replaced with `no-mistakes update` or the upstream install script.
