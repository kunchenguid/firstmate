...
- `AFK_INJECTION_DISABLED: <cause>; <remediation>` - away mode is armed, but the away-mode daemon's fail-closed agent-liveness guard cannot deliver an escalation into the pane it injects into, so nothing reaches the captain until they are back.
  Like `TANGLE:`, this is re-derived live on every run, never read from a marker, so a corrected target or a respawned agent clears it on the next session start.
  When a daemon is already running, the probed pane is the one THAT daemon injects into rather than the pane this session happens to occupy - the two differ after firstmate restarts, and the daemon is never retargeted, which is exactly the case this line exists to catch.
  The causes need different responses, and the printed line says which one it is.
  A bare shell as the supervisor target means the target points at the wrong pane, which is a fixable misconfiguration.
  A liveness verdict that could not be confirmed is ambiguous, and the line says so: it is either an unattributable harness (pi runs as a generic `node`), which is an accepted degradation, or a pane the probe could not read, which is not.
  Confirm the target first; only once it is confirmed is this an accepted degradation, and then tell the captain in plain outcome language that nothing will reach them while they are away, and that the work still surfaces on their return.
  A target that no longer exists is never an accepted degradation: a running daemon injecting into a vanished pane delivers nothing at all for the rest of the away window, and while it does raise the wedge alarm from its backoff path, an alarm is a distress signal rather than a delivery, so that one must be fixed rather than lived with.
  Follow the remediation the line prints rather than substituting one: re-arming does not retarget a running daemon (`bin/fm-afk-launch.sh start` returns early on a live daemon and refreshes only the flag), so a verdict resolved from a running daemon's endpoint says to stop away mode first and then arm it again from firstmate's own pane, while the repoint-and-re-arm form applies only to a fresh arm.
  Away mode keeps running either way - escalations buffer, the wedge alarm fires, and the afk-exit catch-up flush delivers them on the captain's return - so never treat this line as a reason to refuse to supervise.
