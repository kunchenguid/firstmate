...
- `HOME_SUMMARY: this home has never published state/home-summary.json` or `... has not been republished since <stamp>` - this home's structured summary publication has failed repeatedly, and the line carries the failure count and the newest recorded reason from `state/.home-summary-refresh.log`.
  Publication is deliberately best-effort, so nothing else in the session has been held up and none of this session's other results are in doubt; the ledger is simply absent or stale for any reader that consults it.
  Read the named record for the recorded reasons, then reproduce with a direct `bin/fm-home-summary-refresh.sh` (no `--best-effort`, which is what keeps the failure quiet) so the producer's own error reaches you.
  A recorded deadline means the producer did not finish inside `FM_HOME_SUMMARY_TIMEOUT` in a home this large; that is a defect to fix in the producer, not a bound to raise.
