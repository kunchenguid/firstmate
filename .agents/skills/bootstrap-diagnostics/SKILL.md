...
- `FLEET_SYNC: <repo>: recovered: <detail>` - the clone had drifted onto a detached HEAD with no tracked-file changes holding no unique commits and the sync self-healed it (re-attached the default branch and fast-forwarded); no action needed, it is reported only so the self-heal is visible.
- `FLEET_SYNC: <repo>: STUCK: on <state>, N commits behind <base> - needs attention` - the clone has tracked-file changes (untracked-only never counts), is on a non-default branch, is detached with unique commits, or has diverged, so the sync left it untouched (never forcing or discarding); it will keep falling behind until you look.
  A re-attach checkout or fast-forward that git refused (for example over a colliding untracked file) is STUCK too, with `(checkout failed: <git's reason>)` or `(fast-forward failed: <git's reason>)` appended, prefixed `re-attached <default>, ` when the re-attach already happened.
