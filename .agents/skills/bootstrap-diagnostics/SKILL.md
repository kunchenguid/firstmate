...
- `SECONDMATE_SYNC: secondmate <id>: skipped: <reason>` - the local-HEAD secondmate sync left a live secondmate home on its existing checkout because the home was dirty, diverged, ahead of the primary, unsafe, on the wrong branch, missing the primary target commit, or otherwise not fast-forwardable, or because inherited local-material propagation failed; bootstrap continued, but inspect the reason because the secondmate's tracked instructions, inherited settings, or shared captain preferences may be stale after a primary update.
  An `ahead of <commit> by N commits` reason is the one that indicts the PRIMARY rather than the home: the home already holds the primary's commit plus newer ones, so nothing forked and nothing is at risk, but this primary checkout is behind origin and every new home and task worktree it creates starts from that stale base.
  Nothing else reports primary staleness automatically, so treat it as the signal to update this checkout (`/updatefirstmate`) unless it is deliberately pinned.
