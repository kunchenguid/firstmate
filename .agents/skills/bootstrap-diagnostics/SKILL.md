...
- `HOME_SUMMARY: this home has never published <path>` or `<path> has not been republished since <stamp>` - this home's always-active private summary publication, or its explicitly enabled redacted observation publication, has failed repeatedly, and the line carries the failure count and the newest recorded reason from `state/.home-summary-refresh.log`.
