...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, AGY_TURNEND_HOOK, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `AGY_TURNEND_HOOK: ... could not be withdrawn - run bin/fm-agy-turnend-hook.sh remove` - the captain has declined the global agy turn-end hook but firstmate's own hook is still installed in his shared agy store, so run that command by hand and read its refusal, which names the store problem blocking the withdrawal; never re-record consent to silence the line.
  The `withdrew ...` variant is informational only and needs no action: the withdrawal already succeeded.
