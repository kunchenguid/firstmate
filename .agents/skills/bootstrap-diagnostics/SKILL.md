...
- `FLEET_SYNC: <repo>: converged rewritten <default> <before>..<after> (N local-only commit trees matched <base>)` - origin's default branch was deliberately rewritten and the sync proved each local-only commit has an identical tree at the same position on the freshly fetched remote branch, with a clean clone and clean, fully landed linked worktrees, so it converged the clone; no action needed, it is reported only so the convergence is visible.
...
- `FLEET_SYNC: <repo>: STUCK: diverged <default>; rewrite convergence refused: <condition> at commit <commit>[ (<detail>)]; N commits behind <base> - needs attention` - the clone diverged and the rewrite proof failed, so it was left untouched; the named condition and commit are the exact reason (unfetchable remote branch, mismatched or absent counterpart trees, a dirty clone, or a linked worktree that is dirty, uninspectable, or holds unlanded content).
  Resolve that specific condition by hand - land or inspect the offending commit, clean the named tree - and never force, stash, or discard to make the proof pass.
