...
- `TANGLE: <remediation>` - the primary checkout is stranded on a feature branch instead of its default branch; [`docs/architecture.md`](../../../docs/architecture.md#worktrees-not-branches-in-your-checkout) explains why this guard exists and what it protects, and `AGENTS.md` section 8 keeps the rule that resolving it must never touch unlanded work.
