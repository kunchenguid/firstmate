...
- `COMMITMENT: <id> COULD-NOT-OBSERVE - <evidence>` - the probe reached no verdict, or the entry is inadmissible, or it is an attested criterion that cannot execute.
  ...
  Evidence beginning `NOT RUN: session start executes no probe` is the one case that names no defect: session start reports the recorded state and never executes a probe to find out, because arbitrary execution on the critical path of every session is what gets session start turned off.
  Run `bin/fm-commitment-register.sh` by hand to get the observed answer, and read a `TIMEOUT:` evidence line as a probe to fix rather than as an ordinary open item, because a probe that always times out blocks its closure forever.
