...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `graphify`, the printed command installs Firstmate's own pinned home-local environment (it needs `python3`) instead of a global package, and it also covers an installed environment that no longer satisfies the pin; until it is approved and installed, briefs are scaffolded without project context (`docs/configuration.md` "Managed Graphify context" owns the contract).
