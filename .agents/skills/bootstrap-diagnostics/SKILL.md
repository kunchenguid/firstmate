...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, TREEHOUSE_LEASE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `TREEHOUSE_LEASE: pool slot <worktree> is still leased to <holder> for task <id> (<reason>); once nothing is running in it, inspect it and save any unlanded work, because reclaiming discards it; then reclaim it with: treehouse return --if-lease-holder <holder> <worktree>, then rm <record>` - a spawn aborted when its slot could still hold an agent or unlanded work (its task pane survived, its agent was already launched, or it had reused the task's own earlier lease) or when the return itself failed, so it kept that Treehouse pool slot leased on purpose and left no task record; nothing else names the slot, and `treehouse get` never hands a leased slot out again, so the pool stays one slot smaller until a human reclaims it.
  Detect-only: the pane may still hold a live agent or its unlanded work, so never force-return the slot and never remove the record to silence the line.
  Confirm the named pane or agent is gone and nothing is running in the worktree, then INSPECT the worktree before reclaiming: a slot kept by an aborted respawn that reused it still holds the previous worker's unlanded work, and a retried respawn of that task starts in a fresh slot instead of this one.
  The printed `treehouse return --if-lease-holder` command DISCARDS everything in the slot, so land or save any work worth keeping first; only then run it and remove the named record.
  Automatic reclamation is deliberately out of scope; the line is the whole recovery path.
