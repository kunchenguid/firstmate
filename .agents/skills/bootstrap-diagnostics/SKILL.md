...
- `SECONDMATE_LIVENESS: secondmate <id>: <diagnostic>` - the session-start liveness sweep skipped recovery, failed to respawn, or could not confirm replacement liveness.
  Investigate using the recovery-marker and retry contract in [`bin/fm-bootstrap.sh`](../../../bin/fm-bootstrap.sh)'s header before attempting another recovery.
