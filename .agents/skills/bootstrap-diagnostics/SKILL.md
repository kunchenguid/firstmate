...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `no-mistakes`, this line appears only when an explicit no-mistakes project or task override makes the opt-in pipeline necessary, and it also covers an installed version older than 1.31.2 because crewmate validation briefs delegate gate mechanics to no-mistakes' version-matched guidance.
