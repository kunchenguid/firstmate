...
- `SECONDMATE_LIVENESS: secondmate <id>: remote reply stream is unarmed: <reason>` - the remote secondmate's reply source could not be registered, so its reports are not reaching this home even when its endpoint is live.
  Endpoint liveness never proves the reply stream; act on the reason, and for a continuity break run the recovery command it names, which the [`bin/fm-procevent-remote-reply.sh`](../../../bin/fm-procevent-remote-reply.sh) header owns.
