...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, FMX, or CLAUDE_PRIMARY_AUTO_MODE - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `CLAUDE_PRIMARY_AUTO_MODE: ...` - this primary session is confirmed running Claude Code's auto permission mode, and firstmate's own sanctioned local landings, projects/ fast-forward merges and pulls, and path-scoped stash have no opt-in allow rules yet, so the classifier will keep denying them one at a time instead of a captain-visible, predictable pattern.
  Tell the captain in plain terms what this costs (every local landing and deploy becomes a manual handoff) and point at `docs/configuration.md` "Claude permission mode for the primary session" for the copy-pasteable rule pack; never add the rules yourself; `config/claude-permission-mode` is a different, worker-only setting and does not fix this.
  This is a one-shot nudge (`state/.claude-primary-auto-mode-notified`): it will not print again after this session holds the fleet lock once, whether or not the captain acts on it.
