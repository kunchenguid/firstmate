...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `tasks-axi`, this additionally covers an installed build that fails the separate feature probe (`bin/fm-tasks-axi-lib.sh` owns the definition); `config/backlog-backend=manual` only suppresses the verbose `BOOTSTRAP_INFO: tasks-axi available: <path> (<version>)` fact, not this missing-tool report.
  A `MISSING: tasks-axi` line is followed by `BOOTSTRAP_INFO: tasks-axi floor ... unmet by every PATH candidate: <path> <version>` when a build was found, so the upgrade request names the exact install that failed; a `BOOTSTRAP_INFO: tasks-axi resolved <path> (<version>) because PATH-first <path> reports <version> below floor ...` line means only that this build is the one in use, chosen because it clears the floor while an earlier `PATH` candidate did not - the separate feature probe still applies to it, so a `MISSING: tasks-axi` line can still follow reporting that same resolved build as incompatible, and that line is still authoritative.
