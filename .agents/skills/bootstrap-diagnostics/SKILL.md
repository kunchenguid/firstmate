...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the actionable error, and require correction of the malformed schema, unverified harness name, invalid harness/effort pair, or codex-only `profile` field on another harness rather than falling back around it or selecting a bad profile.
