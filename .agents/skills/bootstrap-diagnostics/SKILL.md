...
- `GH_RESOLVE: <detail>` - the firstmate checkout has a GitHub `origin` and no pinned base repo, so `gh` chooses the base repo itself.
  Two separate mechanisms send it to a fork parent, and only one needs a second remote: with several GitHub remotes `gh` prefers one named `upstream` over `origin`, and independently of that, when `origin` names a real GitHub fork, `gh pr create` defaults the pull-request base to that fork's parent from the GitHub API rather than from local remotes.
  ...
  A line naming a linked worktree means the pin belongs to the primary checkout it shares configuration with; repair it there rather than from the worktree.
