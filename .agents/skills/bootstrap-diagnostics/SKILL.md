...
- `FMTG: Telegram bridge on ...` / `FMTG: Telegram bridge off ...` - the same confirmation for the private Telegram bridge's poll artifacts (`docs/configuration.md` "Telegram bridge (.env)"), with the same cadence-restart rule.
  An `off` line that names a malformed `FM_TELEGRAM_BOT_TOKEN`, missing poll dependencies, or a failed artifact removal is a bridge the captain believes is live but is not; report it, and never echo the token itself.
  `FMTG: Telegram bridge armed but not paired ...` is the expected line between adding the token and completing a pairing: the artifacts are live but no one can reach the bridge yet, so surface the pairing command it prints and let the captain decide when to open the link.
