...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, SCM_HOST_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, JEV_MODE invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
The inline rules in `AGENTS.md` section 3 still bind: detect, then consent, then install - never install anything the captain has not approved in this session - and no work is dispatched until the tools it needs are present and the selected SCM host's prerequisites are good.
...
- `SCM_HOST_INVALID: <name> (known: github forgejo local-only)` - the selected SCM prerequisite profile is unknown, so do not dispatch work until `FM_SCM_HOST` or local `config/scm-host` is corrected.
- `NEEDS_GH_AUTH` - the selected SCM host is GitHub; ask the captain to run `! gh auth login` (interactive; you cannot run it for them).
...
- `JEV_MODE: invalid config/jev-mode - <reason>` - Jev's posture file is unsafe or does not contain `off`, `shadow`, or `on`; keep typed resolution off, report the correction, and use `/jev off`, `/jev shadow`, or `/jev on` to replace it through the tracked command.
