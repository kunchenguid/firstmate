...
- `TREEHOUSE_LEASE: pool slot <worktree> is still leased to <holder> for task <id> (<reason>); once nothing is running in it, reclaim it with: treehouse return --if-lease-holder <holder> <worktree>, then rm <record>` - a spawn aborted while its task pane survived, so it kept that Treehouse pool slot leased on purpose and published no task record; nothing else names the slot, and `treehouse get` never hands a leased slot out again, so the pool stays one slot smaller until a human reclaims it.
  ...
  Confirm the named pane is gone and nothing is running in the worktree, inspect it for work worth keeping, then run the printed `treehouse return --if-lease-holder` command and remove the named record.
