...
- `NUDGE_SECONDMATES: secondmate <id>: deferred: ...` - the same nudge was not sent because that secondmate is waiting on its own open decision or blocker, and automatic nudges never wake a waiting worker.
  ...
  For a remote secondmate the watcher sends the deferred nudge once that decision closes.
  For a local secondmate, add the re-read instruction to your answer so it does not resume on stale instructions.
