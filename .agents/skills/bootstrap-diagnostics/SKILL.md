...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  Bootstrap reports `quota-axi` only when the active dispatch configuration contains a profile array, and reports `gh-axi` only while a registered non-local GitHub project is present.
  GitLab merge-request operations require `glab` at their real operation boundary instead of making it a universal startup dependency.
