...
- `STARTUP_MEMORY_BUDGET: startup memory over budget - <total> estimated tokens against an allowance of <budget>; curate it with /stow` - this home's always-loaded memory files now cost more than its allowance, and every session pays that cost at startup until someone curates them.
  It is a meter reading, not a gate: nothing is blocked, and the reported total is the same conservative local estimate `bin/fm-startup-memory-budget.sh report` prints.
  Curation is judgement work owned by the `/stow` skill, so do not prune memory files from this diagnostic; tell the captain the home is carrying more startup memory than its allowance and recommend a `/stow` pass.
  A secondmate home is judged against its own inherited allowance, and an excess it cannot resolve locally - the primary-owned shared captain-preference file alone exceeding the allowance - routes to the primary owner under the `/stow` contract rather than being curated in place.
