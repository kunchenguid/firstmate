...
- `FLEET_SYNC: <repo>: skipped: <reason>` - a non-fatal refresh failure such as an offline fetch or missing remote/default branch; bootstrap continued, investigate only if it blocks work.
  `local-only` is not a skip reason: origin-backed local-only clones refresh, with their pruning protection owned by the [local-clone architecture](../../../docs/architecture.md#local-clones-stay-fresh).
