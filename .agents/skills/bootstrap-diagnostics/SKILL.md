...
description: >-
  ...
  Use whenever the session-start digest's bootstrap section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, CREW_DISPATCH invalid, FLEET_SYNC, PR_CHECK_MIGRATION, SECONDMATE_SYNC, SECONDMATE_LIVENESS, NUDGE_SECONDMATES, FMX, or PORTAL - or when a standalone bin/fm-bootstrap.sh run prints one of those lines.
...
- `PORTAL: connector on ...` / `PORTAL: connector off ...` - bootstrap confirmed, refused, recovered, or removed the private portal poll artifacts (`docs/configuration.md` "Private portal connector").
  Repair invalid mode-0600 configuration only through `bin/fm-portal-config.sh`, never print or copy its token, and apply a running watcher's cadence transition through the emitted harness supervision protocol.
