...
- `MISSING_MANUAL: <tool> (instructions: <instructions>)` - tell the captain why the tool is required and give them the printed instructions, which may be a URL or a pointer to a setup doc, but do not pass the tool to `bin/fm-bootstrap.sh install`; wait for the captain to complete the manual installation, then rerun session start to confirm the dependency is present.
