...
- `CREW_SKILLS: invalid config/crew-skills.json - <reason>` - the optional workflow-skill file exists but failed validation; stop crewmate and scout dispatch, report the schema or unknown-skill error, and require correction rather than silently skipping the configured plan or review phase.
