...
When a diagnostic needs captain attention, use `captain-communication` to report the plain consequence and requested action; name the diagnostic label only when the captain must paste it into a command or issue.
...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `quota-axi`, bootstrap requires it because firstmate reads its current output directly before resolving every crew-dispatch profile array under the agent-owned dispatch-profile array procedure in the harness-adapters skill.
  Without it, report the missing requirement and do not choose around an unexamined candidate.
