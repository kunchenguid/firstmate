...
- `SECONDMATE_SYNC: secondmate <id>: skipped: <reason>` - the local-HEAD secondmate sync left a live secondmate home on its existing checkout because the home was dirty, diverged, ahead of the primary, unsafe, on the wrong branch, missing the primary target commit, or otherwise not fast-forwardable, or because inherited local-material propagation failed; bootstrap continued, but inspect the reason because the secondmate's tracked instructions, inherited settings, or shared captain preferences may be stale after a primary update.
  An `ahead of <commit> by N commits` reason means the home holds the primary's commit plus newer ones, so nothing forked; it has two causes with opposite responses, and collapsing them is the same conflation the separate reason strings exist to prevent.
  Either this primary checkout is behind origin and the home merely holds commits this checkout has not pulled yet, or the home committed its own tracked change on top of a current primary and holds unlanded work of its own.
  Read the commits in `<commit>..HEAD` in the home to tell which before concluding the primary is stale.
  When they are commits this primary is missing, that is the only automatic signal that the primary is behind, and every new home and task worktree it creates starts from that stale base, so update this checkout (`/updatefirstmate`) unless it is deliberately pinned.
  When they are the home's own work, the primary is current and updating it is the wrong response; the unlanded work is in the home, so inspect it there.
  Either way the home is skipped and left untouched, and no data loss is possible because HEAD is never moved.
