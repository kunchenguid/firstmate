...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  For required tools, never argue the documented compatibility floor down to whatever this home happens to have installed.
  ...
  `gh-axi` is operation-specific and optional at session start: when an explicitly required fallback feature finds it absent or below the floor, use `bin/fm-bootstrap.sh gh-axi-compatible`, report that operation's missing requirement, and follow the printed install command only after consent.
...
- `PRESENTATION_UNAVAILABLE: lavish-axi ...` - explain that Lavish-specific presentation is unavailable while native visual and browser tools plus plain-text decisions and reports remain available; do not hold unrelated dispatch for installation consent.
  Do not use Lavish until it satisfies the floor owned by `bin/fm-bootstrap.sh`; when a task specifically needs Lavish hosting, request consent for the printed install or upgrade command, then rerun bootstrap to confirm compatibility before using it.
  Scout briefs check the same floor when scaffolded and fall back only from persistent Lavish hosting; native visual evidence, screenshots, browser work, and plain-text reports remain available.
