...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, MEMORY_DOCTOR, CREW_DISPATCH invalid, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `TANGLE: <remediation>` - the primary checkout is stranded on a feature branch instead of its default branch; [`docs/architecture.md`](../../../docs/architecture.md) owns why this guard exists and what it protects.
...
- `MEMORY_DOCTOR: <advisory threatening-gap summary>` - read the emitted diagnostic and the doctor's referenced evidence before deciding whether action is needed.
  Keep this diagnostic work read-only.
  Do not mutate memory, configuration, projects, or delivery state merely because the doctor reports a finding.
  Distinguish a threatening reliability gap from a stow-class finding, and route only the concrete action owned by the existing handler.
  A threatening finding may block or require captain attention only through that handler; a stow-class finding does not by itself authorize delivery changes.
