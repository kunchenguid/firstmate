...
- `NUDGE_SECONDMATES: secondmate <id>: send failed: <reason>` - the secondmate's required re-read request remains incomplete under `bin/fm-bootstrap.sh`'s guarded owner contract.
  Inspect and fix the printed reason, never delete or rewrite an existing marker under `state/.secondmate-nudge-pending/`, and rerun session start so bootstrap can retry or guardedly supersede the request under the exact contract in `bin/fm-bootstrap.sh`'s header.
