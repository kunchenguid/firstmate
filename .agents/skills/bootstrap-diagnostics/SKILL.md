...
- `FLEET_SYNC: <repo>: skipped: <reason>` - a benign one-off skip (offline, no origin, local-only); bootstrap continued, investigate only if it blocks work.
  A skip whose repo field is `fleet` is fleet-wide rather than one clone: the bounded fleet-refresh timeout (`FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT`, or a fleet-size-aware default with a 20 second floor), or a refresh that refused to run at all (an unreadable project registry or projects root, reported with its reason).
  A timeout never blocks startup, but a refusal means no clone was refreshed, so fix what it names rather than reading the quiet fleet as current.
