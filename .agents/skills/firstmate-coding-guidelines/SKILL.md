...
- Every script in `bin/`, `bin/backends/`, and `tests/` must parse under Bash 3.2, which is what `#!/usr/bin/env bash` resolves to on stock macOS; CI's `macos-stock-bash` job owns the authoritative compatibility sweep over the `bin/fm-lint.sh --list-files` inventory.
- Never nest a heredoc inside `$(...)` when its body carries free-form prose; `tests/fm-brief.test.sh` owns the rationale and focused regression for this Bash 3.2 hazard.
