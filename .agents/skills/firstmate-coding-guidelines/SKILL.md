...
- Never hand a data-scaled payload to a command on argv, including `jq --argjson` and `--arg`: execve caps one argv string at `MAX_ARG_STRLEN`, 131072 bytes on Linux, and refuses the whole command with "Argument list too long" once a payload crosses it.
  Stream any payload that grows with fleet, backlog, log, or record count through a descriptor instead - a pipe, `jq --slurpfile`, or `--rawfile` - and keep only fixed-size scalars on argv; `bin/fm-fleet-snapshot.sh`'s `JQ_PAYLOAD` helper is the worked example.
- Colocate tests with the existing pattern in `tests/`, name them `<subject>.test.sh`, and extend an existing script rather than inventing a new runner.
- A test of executable material must exercise behavior through an executable or public interface and must never assert implementation-source bytes, including through parsers, regexes, snapshots, or indirect wrappers.
  A tracked instruction surface is the exception, because `AGENTS.md`, `.agents/skills/`, and `docs/` are themselves the artifact an agent consumes, so their content has no executable interface behind which the contract could be observed.
  Assert those surfaces by content, keep each assertion tied to a named contract rather than to incidental wording, and never use a doc assertion to stand in for behavior a script can be run to prove.
- Run `bin/fm-private-material-check.sh` before publishing tracked changes anywhere public, and run it with `--history` before ANY push, not only before contributing to a third-party upstream.
- The default mode scans the working tree and HEAD, so a marker added in one commit and removed in a later commit on the same unpushed branch passes even though pushing that branch publishes it; `--history` is the mode that reaches those commits, which is why the wider trigger is load-bearing rather than cautious.
  `--history` reaches them by walking every ref - all local branches, all remote-tracking refs, and the stash - not the merge-base to HEAD range alone, so a hit may be on an abandoned branch or in an upstream commit that is not yours to rewrite; find which ref carries the commit before choosing a remedy.
  Both modes read tracked names as well as tracked content, because a file or directory named after a private project leaks it to anyone listing the tree.
  Those names are read from the index and HEAD only, and `--history` adds none of them, because `git log -G` matches patch text and a rename produces no patch text at all.
  So a path added and then renamed or removed within unpushed commits is published by the push while no mode reports it, and `--history` is not the mitigation here that it is for intermediate-commit content.
  ...
  A finding on a name that is legitimately public, such as the upstream owner this repo was forked from, is a captain decision recorded in local `config/private-material-allow` rather than a reason to edit the check; `docs/configuration.md` ("Private-material markers") owns that file and its `config/private-material-markers` sibling for a private name nothing local can derive.
