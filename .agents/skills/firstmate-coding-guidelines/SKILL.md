...
- `bin/*.sh` and `bin/backends/*.sh` must also parse under Bash 3.2, which is what `#!/usr/bin/env bash` resolves to on stock macOS; CI's `macos-stock-bash` job runs that parse sweep over the `bin/fm-lint.sh --list-files` inventory because Bash 4+ accepts constructs that Bash 3.2 rejects.
- Never nest a heredoc inside `$(...)` in a script whose heredoc bodies carry free-form English prose, because Bash 3.2 scans the command substitution for its closing `)` without understanding heredoc bodies, so one ordinary possessive can break parsing of the whole file; `tests/fm-brief.test.sh` provides a partial local structural guard, `bin/fm-brief.sh` assigns its prose blocks with `IFS= read -r -d '' ... <<EOF || true`, and CI's stock Bash 3.2 parse sweep is the authoritative compatibility check.
