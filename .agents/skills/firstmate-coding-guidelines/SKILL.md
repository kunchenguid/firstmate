...
- Keep every canonical lint root's followed source graph within roughly 10,000 inlined lines, because ShellCheck's extended analysis inlines each followed `# shellcheck source=` target at every source site and needs memory proportional to the inlined line count times the variables in scope, which is what pushed the largest runtimes past the 4 GB ShellCheck memory cap: since every production shell is already analyzed in full as its own root, a library sourced inside a function or re-sourced lazily, and the largest libraries a large runtime sources at top level, stay `# shellcheck source=/dev/null` analysis boundaries with a one-line rationale.
