...
- The default mode scans the working tree and HEAD, so a marker added in one commit and removed in a later commit on the same unpushed branch passes even though pushing that branch publishes it; `--history` is the mode that covers the range, which is why the wider trigger is load-bearing rather than cautious.
