...
- `/afk` is not an exit but a posture change: run the `afk` skill's entry as written, whose step-1 `enter` rewrites a standing `state/.afk` to `away` rather than removing it, so a running daemon keeps its presence gate and the session continues in the away posture, where the next ordinary captain message is the return.
