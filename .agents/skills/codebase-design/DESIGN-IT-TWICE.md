...
When the user wants to explore alternative interfaces for a chosen deepening candidate, use this independent-review pattern.
...
Before dispatching independent reviewers, write a user-facing explanation of the problem space for the chosen candidate:
...
Dispatch three or more reviewers in parallel through the current environment's authorized worker mechanism.
When running as Firstmate, follow its task-lifecycle and harness-adapter contracts and commission crewmates.
When delegation is unavailable or unauthorized, develop the alternatives sequentially in separate contexts.
