...
Never issue a raw removal command from Firstmate unless that preflight passes and AGENTS.md hard rule 1's concrete captain-approved project operation exception applies.
Once both conditions are met, that exception authorizes firstmate to remove the clone directly and update its registry entry to match.
