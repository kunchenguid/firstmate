...
The optional `+tiered` posture applies only to `no-mistakes` projects and lets a small, low-risk diff run a trimmed gate instead of the full pipeline, classified deterministically by `bin/fm-tier.sh` (pure git diff, no agent): a docs-only diff ships `direct-PR`; a diff at or under 100 changed lines and 5 files, entirely code/test/docs with no high-risk paths and a test-presence signal, skips the `document` step (and `test` too under the paired `+ci-tests` flag, when the project's own CI already runs the same suite); everything else, and any classifier error, takes the full gate. `review` and CI are never skipped at any tier. Default both flags off for every project, and enable them only on the captain's explicit instruction. `AGENTS.md` section 7 owns how each task's concrete `tiered`/`ci-tests` are resolved at intake and passed explicitly to the brief and the spawn, exactly like mode and yolo.
