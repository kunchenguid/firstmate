...
Use `gh-axi` for an approved GitHub operation or `glab` for an approved GitLab operation, and consult the selected tool's current help rather than relying on remembered flags.
