...
The registry records the project's standing posture, which is the captain's default for the work rather than any task's answer; `AGENTS.md` section 7 owns how each task's concrete mode, Golden Rules review requirement, and yolo are resolved at intake and passed explicitly to the brief, the spawn, and any promotion.
Choose that posture when adding or creating the project:

- `direct-PR` pushes and opens a PR; product-facing, mixed, and uncertain work additionally requires a Golden Rules review pass before merge, while genuinely internal-only tooling, automation, contributor or operator process, and release or submission work does not.
...
Existing registry entries keep the meaning they already have and are never migrated or reinterpreted, so a legacy `no-mistakes` or `no-mistakes-prod-only` entry now reads as `direct-PR` with a mandatory Golden Rules review pass.
Registering the posture is a one-time choice and never requires classifying any change; the per-task surface classification happens at each task's intake, and internal-only is never inferred from file location or project name.
...
After remote creation succeeds, clone it locally and add the registry entry.
