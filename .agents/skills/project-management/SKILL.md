...
Keep each registry description useful for identifying the project, but keep delivery posture, Jay's private state, and detailed project knowledge in their existing designated homes.
...
- `no-mistakes` runs the full validation pipeline before a PR and is the default when Jay does not specify a mode.
...
Default it off, and enable it only on Jay's explicit instruction.
Destructive, irreversible, and security-sensitive decisions still require Jay's approval when it is on.
...
Before making that remote change, propose the repository name, owner or organization, visibility, and delivery mode, defaulting visibility to private and delivery mode to `no-mistakes`, then obtain Jay's explicit consent for those values.
...
Jay's request to create that local project authorizes this local initialization, but it does not authorize an unmentioned remote repository.
...
First obtain Jay's explicit removal decision, then inspect the current digest and authoritative repositories for in-flight or queued work, registered secondmate clones, linked worktrees, dirty files, unpushed commits, and any other unlanded work.
