...
- `direct-PR` pushes its branch and opens a PR with `gh-axi`; generated completion waits for the project's required CI to be green, and merging separately requires that PR's own required CI green plus the configured merge authority without granting merge authority itself.
...
An unregistered project, or a missing registry, resolves to `direct-PR` with yolo off, and that registration gap goes to the captain.
A legacy registry entry with no bracket resolves to `direct-PR off` under `bin/fm-project-mode.sh`'s parser warning; it is never reinterpreted as another mode.
