...
- `direct-PR` pushes and opens a PR without the no-mistakes pipeline and is the default.
- `local-only` has no required remote or PR and lands only through the approved local fast-forward path.
- `no-mistakes` is not a standing project mode; it is a user-explicit per-task override on direct-PR work.
...
Initialize no-mistakes only when the captain explicitly requests that capability for future manual tasks.
...
Run no-mistakes initialization only after an explicit captain request to enable manual no-mistakes tasks:
...
Before Firstmate can manage a bounded run, the project's trusted default-branch `.no-mistakes.yaml` must canonically set `agent: codex`, and the submitted HEAD must set `auto_fix.review: 0`; v1.53.0 has no safe per-run override for either value.
