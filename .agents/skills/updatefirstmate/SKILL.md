...
Firstmate is its own repo, behind the same pull-request-and-CI discipline as any project, so new tracked material (`AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/`) reaches `main` through a PR with required CI green and then sits there until each running firstmate pulls it.
