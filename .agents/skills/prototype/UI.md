...
If the question is about logic or state rather than appearance, use [LOGIC.md](LOGIC.md) instead.
...
When implementation is authorized, fold the winner into the real code and remove the discarded variants from the production path:

- **Sub-shape A** - fold the winner into the existing page; drop the losing variants and the switcher from main.
- **Sub-shape B** - promote the winning variant to a real route; drop the throwaway route and the switcher from main.

The full set of variants is the primary source.
Preserve it through the task's authorized delivery path; do not create a branch, commit, or external pointer solely because this skill was loaded.
Variant components and the switcher left in production rot fast and confuse the next reader.
