...
6. **Capture it when done.**
   Fold any validated decision into the real code only when the task authorizes implementation.
   Preserve the prototype and its answer through the task's existing delivery contract.
   Do not create a branch, commit, issue, or outward-facing pointer unless the task explicitly authorizes that action.
   When running as Firstmate, delegate project changes and preserve every unlanded-work and outward-consent boundary.
