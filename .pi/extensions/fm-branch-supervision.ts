...
export default function (pi: ExtensionAPI) {
  ...
  function createReportTool(toolGeneration: number): ToolDefinition {
    return {
      ...
      description:
        "Record the outcome of one handled fleet event: write it durably to the outcome store, then merge it into the captain-facing main conversation. verdict captain persists an exact visible entry and opens one sequence-keyed processing turn on main that stays open until main acknowledges it; routine notes render unless silent marks an unchanged still-working update or a no-change heartbeat.",
      parameters: Type.Object({
        ...
        silent: Type.Optional(Type.Boolean({
          description: "True when this routine update is unchanged still-working narration worth recording but not worth a rendered note, or a fleet-wide heartbeat review that found literally nothing worth reporting; omit or use false whenever any action was taken or any routine result is worth a note",
        })),
      }),
...
${context.command}
  ...
  // routine note uses except an explicitly silent routine outcome.
