...
// 35s on macOS and Windows so the outer wait stays above arm's 30s confirm
// default (bin/fm-watch-arm.sh / fm_arm_confirm_timeout_seconds). A successor
// whose first beat lands between 12s and 30s must not be SIGTERMed.
export function armReadyTimeoutDefaultMs(platform = process.platform): number {
  return platform === "darwin" || platform === "win32" ? 35000 : 12000;
}
const armReadyTimeoutMs = positiveInteger(
  "FM_PI_ARM_READY_TIMEOUT_MS",
  armReadyTimeoutDefaultMs(),
);
