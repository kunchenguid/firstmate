...
const startupGraceMs = Number(process.env.FM_ARM_CONFIRM_TIMEOUT || "10") * 1000;
...
export default function (pi: ExtensionAPI) {
  function stopOwnedGroup(pid: number): void {
    spawnSync(
      "bash",
  ...
  function childIsStillStarting(arm: ArmChild): boolean {
    if (!arm.pid || !pidAlive(String(arm.pid))) return false;
  ...
  function startArm(): ArmResult {
    ...
    armChild.on("exit", () => {
      ...
      if (armChild.pid && ownedGroups.delete(armChild.pid)) stopOwnedGroup(armChild.pid);
    });
