...
import { noteCalmAssistantUserTurn } from "./fm-pi-cursor-calm-assistant-layout.ts";
...
export function installCalmOperationalUserLayout(): void {
  ...
  prototype.addMessageToChat = function (
    ...
    // Every user-role row opens an assistant turn, including operational
    // follow-ups, so a later run cannot hide the previous question's last recap.
    noteCalmAssistantUserTurn();

    const text = this.getUserMessageText(message);
    if (text && patch.isOperationalInput(text) && contentIsTextOnly(message.content)) {
