...
| `/helm`            | Show or set the compact `language` and `response_tone` captain-style preferences in `config/captain-style.json`; `language` only ever governs firstmate's own chat and captain-facing artifacts, never a project's tracked code, comments, commits, or documentation |
