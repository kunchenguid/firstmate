...
Claude Code uses a tracked Stop hook for tokenless watcher re-arm and rewake, Grok uses background-notify wake cycles, and OMP and Pi use distinct tracked primary watcher extensions.
...
OMP auto-discovers the tracked `.omp/extensions/*.ts` files when launched from the FirstMate root.
