...
- **Explicit project modes** - each project ships via `direct-PR` (default; focused local tests plus your own PR), `no-mistakes` (opt-in full validation pipeline), or `local-only`, with an optional `+yolo` autonomy flag.
