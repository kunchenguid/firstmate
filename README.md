...
| `/updatefirstmate` | Self-update the running firstmate and its secondmates from origin through guarded, never-forced pulls, then re-read instructions and nudge secondmates; fast-forward-only unless the optional held-improvement stack is configured ([docs/configuration.md](docs/configuration.md)) |
