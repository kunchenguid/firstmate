...
- **Explicit project modes** - each project ships via `direct-PR` (with a mandatory Golden Rules review pass for product-facing, mixed, or uncertain work) or `local-only`, with an optional `+yolo` autonomy flag.
