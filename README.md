...
| `/morning-brief`   | Compose a "pick up where I left off" status report from live fleet state - backlog, each direct report's current state, open PRs, scout reports, and pending decisions - writing a dated file to `data/` and surfacing a concise summary in chat; read-mostly, it never merges, tears down, or steers work |
