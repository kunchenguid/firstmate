...
| `/backlog`         | Turn a pasted wall of text into filed backlog items: split into discrete asks, classify each one's project/kind/priority, dedupe by merging into existing items, route secondmate-scope items, hold captain-owned questions, and report each item's id, outcome, and priority; never dispatches work |
