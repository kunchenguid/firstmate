...
The runtime session-provider backend is selectable through local `config/backend` for future adapters; today `tmux` is the only verified backend and omitted config defaults to it.
