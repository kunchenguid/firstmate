...
- **Optional Telegram topic board** - run a dedicated forum bot as a persistent long-poll service across multiple approved groups and senders so project messages are retained, scoped, routed by topic, and answered back into their originating group and thread without sharing or risking the direct-message bot token.
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional X mode, the optional Telegram topic board and direct-message line, the files you set, and harness support.
...
- [docs/dm-line.md](docs/dm-line.md) - the session-independent captain direct-message line, durable handling, and single-consumer safety.
