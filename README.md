...
| `/parallel-ideation` | Run several captain-supplied ideation or brainstorming skills in parallel on one subject, synthesize their reports into one document, then verify that synthesis with a readonly inspection by the separately installed `seats` skill, which pauses once for your approval of the chosen seats, before the run closes |
