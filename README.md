...
- **Captain-private business agenda** - firstmate seeds a local daily and weekly routine registry and uses the existing authenticated watcher rail to surface each due action once per local period, with no calendar sync, credentials, or outbound communication; see [Business agenda](docs/configuration.md#business-agenda).
...
Full architecture - the supervision engine, business agenda, worktree isolation, secondmates, dispatch profiles, project modes, optional Relay, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, the private business agenda, runtime backend selection, optional Relay and its X and Discord setup steps, the files you set, and harness support.
