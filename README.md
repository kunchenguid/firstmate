...
| `/updatefirstmate` | Self-update the running firstmate and its secondmates from origin under each home's policy (guarded fast-forward by default, explicit remote-authoritative replacement by per-home opt-in), then re-read instructions and nudge secondmates |
