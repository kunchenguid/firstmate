...
- **Two task shapes** - ship tasks deliver authorized changes, each bounded to one independently testable outcome so larger work arrives as separate reviewable ships rather than one sprawling PR; scout tasks leave standalone investigation reports when the intake contract warrants separate research.
