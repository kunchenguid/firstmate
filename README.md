...
Codex and OpenCode are also verified and supported as primary harnesses; Codex selects native asynchronous Stop supervision when supported and retains foreground checkpoints otherwise, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
