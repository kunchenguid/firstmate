...
- **Optional Telegram mode** - opt in with a bot token and private chat id in the same local `.env` so away-mode escalations reach your phone, and reply through a closed command grammar (`status`, `approve <key>`, `deny <key>`, `merge` of a previously notified green PR); phone messages never exit away mode, and secrets plus destructive or irreversible decisions stay desk-only. See [docs/telegram-mode.md](docs/telegram-mode.md).
...
Optional secondmates extend this to persistent second mates, dispatch profiles let you steer which harness handles which task, an opt-in X mode lets the same fleet answer public mentions, and an opt-in Telegram mode brings away-mode notifications and bounded approvals to your phone.
...
- [docs/telegram-mode.md](docs/telegram-mode.md) - opt-in phone notifications and bounded approvals over Telegram: activation, security controls, the closed command grammar, and kill switches.
