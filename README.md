...
- A verified primary agent harness: Claude Code, Grok, OMP, Pi, `pi-signed`, Codex, OpenCode, or Cursor Agent CLI.
...
**Claude Code, Grok, OMP, and Pi are equal co-primary recommendations** for running the primary firstmate session, with `pi-signed` supported as Pi's distinct signed-wrapper identity.
Claude Code uses a tracked Stop hook for tokenless watcher re-arm and rewake, Grok uses background-notify wake cycles, OMP uses its tracked OMP-native extensions, and Pi uses its tracked primary watcher extension.
All four have verified turn-end guard paths when launched with their documented setup.
...
OMP auto-loads the tracked `.omp/extensions/*.ts` files without a trust prompt.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, OMP, Pi and `pi-signed`, Grok, Cursor, and unknown harness fallback.
