...
- **Private portal messaging** - opt in with a local mode-`0600` connector file so authenticated AI Department portal requests wake the same active Firstmate session and receive one crash-safe, idempotent response in that portal conversation without mirroring unrelated chat channels.
...
Optional secondmates extend this to persistent second mates, dispatch profiles let you steer which harness handles which task, an opt-in X mode lets the same fleet answer public mentions, and an independent private portal connector carries authenticated captain requests.
`codex-app` is not a runtime backend yet; [docs/codex-app-backend.md](docs/codex-app-backend.md) owns the Codex App boundary.

Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional messaging connectors, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional X and private portal messaging, the files you set, and harness support.
