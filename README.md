...
- **A visible crew** - every crewmate works in its own tmux window, experimental herdr/zellij tab, cmux workspace, Orca terminal, or Codex Desktop-visible task; the first mate reconciles.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca/codex-app when explicitly selected); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, or Hermes.
...
Hermes is verified only as a persistent primary, launched through `bin/fm-hermes-primary.sh`; it is never a crewmate or secondmate runtime. Its tracked project plugin uses managed background-terminal completion notifications for watcher supervision and a turn-end backstop.
...
Setup guides for tmux (the default) and every other supported backend (herdr, zellij, Orca, cmux, Codex App) are linked in [Documentation](#documentation) below.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   terminal endpoints or Codex Desktop-visible tasks
...
`codex-app` is an experimental ship/scout backend for Codex Desktop-visible workers; [docs/codex-app-backend.md](docs/codex-app-backend.md) owns its setup and limits.
...
| `/updatefirstmate` | Safely integrate a configured public upstream into private origin when opted in, then fast-forward the running firstmate and secondmates, re-read instructions, and nudge updated homes |
...
- [docs/codex-app-backend.md](docs/codex-app-backend.md) - setup and limits for the experimental Codex App worker backend.
