...
Codex and OpenCode are also verified and supported as primary harnesses; Codex runs a persistent watcher bridge that injects wake doorbells into its pane, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
