...
Secondmate state that carries no repository and whose owner covers several projects is never guessed at: the card discloses it as unattributable and raises the project only to the status that state would have produced anyway, so an unplaceable queued or landed row never repaints a card.
A held in-flight backlog row reaches its card straight from the backlog, without live task metadata, the way the Bearings gate list reads it, and a backlog row that omits the optional `(repo: ...)` metadata follows its task to that task's project.
An invalid main backlog inventory, open main backlog rows or tasks that reach no registered project, a completed row whose repository label names an unregistered project, a bounded-read drop in the canonical snapshot, and any secondmate state that can reach no registered project card - work labelled with an unknown repository, or unlabelled work from a secondmate with no registered project - are disclosed board-wide rather than quietly shrinking a card.
Every item has exactly one place on the card: a captain-actionable queued row is shown as a decision, and work that is blocked or externally held appears under waiting rather than being repeated under queued next steps, returning there only once its blocker or hold clears.
A completed row still held for the captain is a resolved decision, listed in a bounded recently-resolved panel under decisions rather than mixed into landed work or pull-request history; that entry keeps its own pull request as a labelled reference, and a captain hold that is still in flight keeps its pull-request link.
That recently-resolved panel is main-home only in v1: the canonical secondmate home summary omits a secondmate's completed captain-held rows, this board does not widen that contract, and the board states that limitation once as a global scope note above the project grid rather than repeating it per card.
A captain decision that is due now appears only under decisions even when its own task has parked awaiting the answer. Only a live, answer-ready decision does that: one deferred to a future hold-until date, or set aside as superseded, is not awaiting an answer, so its parked work still reports itself under waiting.
...
A completed row that carries no repository label and whose task has already been torn down has nothing linking it to a project, so it is dropped rather than reported as a gap, and the recently-landed and pull-request panels each show at most five rows, newest completion first, with the true total in their heading; the pull-request panel lists open work ahead of completed work, counting a task whose backlog row is already done as completed.
