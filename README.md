...
Calm adds no persistent status row, and every row it controls is removed without reserving vertical space.
...
Exports and shares remain complete session artifacts: operational text hidden from the live Calm transcript remains in serialized session data and Pi 0.81.1's sidebar tree.
