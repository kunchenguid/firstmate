...
Selecting a project expands that card larger than the rest, with its active work, decisions, queued next steps, recently landed work, pull-request links, delivery mode, last-activity age, and secondmate owner.
Every item has exactly one place on the card: a decision the captain owes an answer to is shown as a decision instead of being repeated as queued work, and blocked or externally held work appears under waiting until its blocker or hold clears.
...
That panel is main-home only in v1: the canonical secondmate home summary omits a secondmate's completed captain-held rows, this board does not widen that contract, and the board states that limitation once as a global scope note above the project grid.
The board discloses what it cannot safely attribute to one project, and what a bounded read or an unreadable record left out, rather than quietly shrinking a card.
Nothing readable locally tells a merged pull request apart from one still awaiting review, so the board never claims either.
...
`bin/fm-project-dashboard-snapshot.sh --help` owns the full `fm-project-dashboard.v1` aggregation contract: attribution, disclosure, per-panel bounds, and the exact surface every kind of item lands on.
