...
- **Explicit delivery posture** - each project ships via `direct-PR` or `local-only`, with an optional `+yolo` autonomy flag; `no-mistakes` is a user-explicit bounded override for one task, never an automatic route.
