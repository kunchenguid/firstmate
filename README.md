...
- **Pi quota at a glance** - trusted Pi and `pi-signed` sessions automatically add every fresh quota window for the active model provider to Pi's built-in footer, including resets, plan, and credits when available, while narrow or unavailable displays stay explicit and compact.
