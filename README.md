...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, or Anti-Gravity CLI (`agy`).
...
Codex, OpenCode, and Anti-Gravity CLI are also verified and supported as primary harnesses.
Codex and Agy use bounded foreground checkpoints, while OpenCode uses a TUI plugin, so all three carry more harness-specific supervision tradeoffs than the three co-primaries.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, Grok, Agy, and unknown harness fallback.
