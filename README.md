...
| `/parking`         | Capture the captain's half-formed ideas mid-conversation, resurface them on a daily and weekly cadence to refine in place, and promote the ready ones into firstmate intake (and optionally an external tracker); capture is an always-on reflex on `Parking:`, `park this`, and `put this in parking`, while `/parking` runs the review ritual on demand |
