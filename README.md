...
Copilot uses tracked background Bash tasks, Codex uses bounded foreground checkpoints, and OpenCode uses a TUI plugin, so they carry more harness-specific supervision tradeoffs than the three co-primaries.
...
Copilot asks you to trust the clone on first launch; choose the persistent trust option if you want later sessions and linked worktrees to load the tracked Firstmate hooks without another prompt.
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Copilot, Codex, OpenCode, Pi and `pi-signed`, Grok, and unknown harness fallback.
