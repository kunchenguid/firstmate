...
Codex and OpenCode are also verified and supported as primary harnesses; Codex prefers an async Stop hook that re-arms the watcher and queues the wake back into its own thread, falling back to bounded foreground checkpoints wherever that hook is not firing, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
