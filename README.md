...
It routes each request to a crewmate in its own session endpoint and git worktree, supervises the fleet with a zero-token event-driven watcher or Codex-safe present-mode loop, and brings you finished PRs, approved local merges, or investigation reports.
