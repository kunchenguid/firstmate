...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` merge-autonomy flag and an optional `branch=<prefix>` override for the default `fm/` ship-branch prefix.
