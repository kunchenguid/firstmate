...
Codex and OpenCode are also verified and supported as primary harnesses; Codex uses an async Stop hook that re-arms the watcher and queues the wake back into its own thread, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
