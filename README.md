...
codex-cli 0.146.0 ships its `hooks` feature enabled by default, so the operator-visible gates for a Codex primary are the directory-trust prompt and the separate hook-review prompt; accept both or the tracked session-start and turn-end hooks never load.
A Codex process started with that feature explicitly disabled stays hookless until you restart it, and [docs/turnend-guard.md](docs/turnend-guard.md) routes to the details.
