...
| `/bearings`        | Generate a concise four-section chat digest with live verification of every named PR; use `/bearings file` to create a new dated report in `data/` without replacing existing snapshots, and add `include PRs` to discover additional open PRs |
...
- `/bearings file` creates a new dated `data/status-report-<YYYY-MM-DD>[-<HHMMSS>[-N]].md` snapshot, preserving any existing report, and links it from the four-section chat digest.
