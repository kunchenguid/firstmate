...
- **Explicit project modes** - each project ships via `direct-PR` or `local-only`, with an optional `+yolo` autonomy flag.
...
- **Guarded by construction** - the first mate is read-only over your projects except for the guarded paths authorized by [hard rule 1](AGENTS.md#1-identity-and-hard-boundaries), with fleet sync's safe branch pruning remaining part of the fleet-sync exception; crewmates make every project change behind the captain's explicit merge authority.
