...
| `/updatethecaptain`      | Keep the captain posted on every worker under way with a plain-English report - how long each has been running, what it has finished, what it is doing now, what is left, and how long it needs - once immediately and then every ten minutes, surfacing any worker's question for the captain at the top of the message, until no worker is left running or the captain stops the updates |
