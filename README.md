...
- **Pi quota at a glance** - trusted Pi and `pi-signed` TUI sessions automatically add a width-aware quota row beside Pi's unchanged built-in footer.
  For official OpenAI Codex subscription models, the row shows every still-fresh window from an account-matched provider report, with reset information and any supplied plan or credit balance; narrow layouts and unsupported, unavailable, unverified, malformed, missing, failed, or timed-out results stay explicit and compact.
  Observed auth-file writes quarantine cached quota immediately; one-second revision polling plus a bounded credential read contains missed filesystem notifications, so credential transitions have no zero-latency guarantee.
...
The crew architecture is described in [docs/architecture.md](docs/architecture.md).
