...
- **A visible crew** - every crewmate works in its own tmux window, experimental herdr/zellij tab, cmux workspace, thurbox session, or Orca terminal you can watch or type into; the first mate reconciles.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr, cmux, or thurbox when selected or auto-detected, zellij/orca when explicitly selected); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
Setup guides for tmux (the default) and every other supported backend (herdr, zellij, Orca, cmux, thurbox) are linked in [Documentation](#documentation) below.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   tmux windows, herdr/zellij tabs, cmux workspaces, thurbox sessions, or Orca terminals
...
- [docs/thurbox-backend.md](docs/thurbox-backend.md) - current setup, the shell-agent requirement, and limits for the experimental thurbox backend.
