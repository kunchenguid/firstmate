...
| `/updatefirstmate` | Guardedly update the running firstmate and its secondmates - fast-forward, or reconcile a redundant post-squash-merge divergence - then attempt a persist-gated restart for every eligible live mate left on the target commit - including already-current homes - with an honest re-read nudge when restart is unsafe |
