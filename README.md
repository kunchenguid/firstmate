...
Claude and Pi workers spend only an account the home declares, so a spawn refuses until you create `config/claude-account` or `config/pi-account`; [worker account pin](docs/configuration.md#worker-account-pin-configclaude-account-configpi-account) gives the format, including `ordinary` for the runtime's default login.
