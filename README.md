...
A held in-flight backlog row reaches its card straight from the backlog, without live task metadata, the way the Bearings gate list reads it.
An invalid main backlog inventory, a bounded-read drop in the canonical snapshot, and any secondmate state that can reach no registered project card - work labelled with an unknown repository, or unlabelled work from a secondmate with no registered project - are disclosed board-wide rather than quietly shrinking a card.
