...
- **Event-driven, zero-token supervision** - a bash watcher sleeps on the fleet and wakes the first mate only when something needs you; verified primary harnesses also get a turn-end backstop that warns, blocks, or follows up on a blind stop when work is under way and supervision is not live.
