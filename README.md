...
| `/bearings`        | Generate a concise four-section chat digest from bounded local fleet and registered-secondmate state with live verification of every named PR; use `/bearings file` to preserve any same-day report and create a collision-proof new dated report, and add `include PRs` to discover additional open PRs |
...
- `/bearings file` preserves any existing same-day report and creates a collision-proof new dated report, then links it from the four-section chat digest.
