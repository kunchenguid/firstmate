...
- **A visible crew** - every crewmate works in its own Herdr tab by default, or an explicitly selected tmux window, Zellij tab, cmux workspace, or Orca terminal you can watch or type into; the first mate reconciles.
...
- **Restart-proof** - all state lives on disk and in the active session backend (Herdr by hard default, tmux, Herdr, or cmux when auto-detected, and tmux, Zellij, Orca, or cmux when explicitly selected); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
- The CLI and dependencies for your selected runtime backend; Herdr is the reference default.
...
Install `jq` with your platform package manager before running the pinned default-backend installers.
...
bin/fm-install-herdr.sh "$HOME/.local/bin"
bin/fm-install-treehouse.sh "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
```

To opt into the fully supported legacy tmux backend instead, install tmux and follow [`docs/tmux-backend.md`](docs/tmux-backend.md).
...
Setup guides for Herdr (the default) and every other supported backend (tmux, Zellij, Orca, cmux) are linked in [Documentation](#documentation) below.
...
- [docs/herdr-backend.md](docs/herdr-backend.md) - current setup, safety boundaries, and limits for the reference Herdr backend.
- [docs/tmux-backend.md](docs/tmux-backend.md) - current setup and limits for the fully supported legacy tmux backend.
