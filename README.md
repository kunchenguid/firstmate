...
- **Restricted account tasks** - an operator-installed SSH forced-command route can submit bounded non-private work to a separately owned execution account without copying a primary home or exposing generic remote command, repair, or file authority; deterministic fixtures ship with the route, while every live cross-account installation still requires its own attended qualification.
...
- [docs/account-task-route.md](docs/account-task-route.md) - the restricted cross-account task protocol, attended installation boundary, qualification requirements, and rollback behavior.
