...
- **Captain-private recurring routine** - firstmate seeds a local daily and weekly routine registry and uses the existing authenticated watcher rail to surface due actions once per local period, retaining unacknowledged emissions for at-least-once delivery, with no calendar sync, credentials, or outbound communication; see [Recurring routine](docs/configuration.md#recurring-routine).
...
Full architecture - the supervision engine, recurring routine, worktree isolation, secondmates, dispatch profiles, project modes, optional Relay, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, the private recurring routine, runtime backend selection, optional Relay and its X and Discord setup steps, the files you set, and harness support.
