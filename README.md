...
- **Provable lifecycle claims** - an optional Z3-backed completeness gate proves each done, teardown, or merge claim consistent with the directives before the irreversible step, naming the invariant a blocked claim violated; install `z3-solver` to turn it on, and without it the gate steps aside and the existing bash safety checks remain the hard guarantee.
