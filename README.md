...
Codex, OpenCode, and Cursor are also verified and supported as primary harnesses; Codex uses bounded foreground checkpoints, OpenCode uses a TUI plugin, and Cursor arms from inside its `stop` hook, so all three carry more harness-specific supervision tradeoffs than the three co-primaries.
