...
- **Optional workspace placement and command execution** - defaults stay host-direct worktrees and local commands; operators may opt into Docker Sandbox placement for workers or secondmates and task-scoped Crabbox command execution through local `config/workspace-execution.json` or per-spawn overrides without replacing the session backend ([configuration](docs/configuration.md#workspace-placement-and-command-execution-configworkspace-executionjson), [architecture](docs/architecture.md#optional-docker-sandbox-placement)).
...
- [docs/architecture.md](docs/architecture.md) - maintainer architecture for the crew, supervision, worktrees, optional Docker Sandbox placement, task-scoped command execution, secondmates, and project modes.
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional workspace placement and Crabbox command execution, optional Relay and its X and Discord setup steps, the files you set, and harness support.
