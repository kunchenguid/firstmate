...
- A verified primary agent harness: Claude Code, Grok, Pi, `pi-signed`, Codex, OpenCode, Cursor Agent CLI, or omp (Oh My Pi).
...
omp (Oh My Pi) is verified as a primary too, using its tracked `.omp/extensions/*.ts` watcher and turn-end guard extensions and a native awaited `session_stop` continuation hook; see [`docs/supervision-protocols/omp.md`](docs/supervision-protocols/omp.md).
...
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, Grok, Cursor, omp, and unknown harness fallback.
