...
- **Private Telegram bridge** - pair one dedicated bot to one exact captain direct chat for private text requests, safely rendered rich/threaded replies, and quiet decision or outcome notifications from every supervised project through long polling, with no public webhook, listening port, raw model markup, buttons, or offline model execution.
...
Optional secondmates extend this to persistent second mates, dispatch profiles let you steer which harness handles which task, an opt-in X mode lets the same fleet answer public mentions, and the disabled-by-default [private Telegram bridge](docs/telegram-bridge.md) adds one private captain conversation.
`codex-app` is not a runtime backend yet; [docs/codex-app-backend.md](docs/codex-app-backend.md) owns the Codex App boundary.

Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional remote channels, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional remote channels, the files you set, and harness support.
- [docs/telegram-bridge.md](docs/telegram-bridge.md) - current one-captain Telegram setup, pairing, limits, disable flow, and safe-delivery behavior.
