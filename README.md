...
- **Explicit project modes** - each project ships via `direct-PR` or `local-only`, with an optional `+yolo` merge-autonomy flag; legacy `no-mistakes` registry tokens map to direct-PR.
