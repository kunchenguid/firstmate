...
- **Disposable worktrees** - each task runs in a clean [treehouse](https://github.com/kunchenguid/treehouse) git worktree, or an Orca-managed worktree when `backend=orca`, so parallel work on one repo never collides; a T3 Code thread is bound to that worktree too, never to the project root.
...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij/orca/t3 when explicitly selected); the next session reconciles after a restart, while ordinary supervision recovers confirmed-dead secondmate agents without waiting for one.
...
Setup guides for tmux (the default) and every other supported backend (herdr, zellij, Orca, cmux, T3 Code) are linked in [Documentation](#documentation) below.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    ...
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   tmux windows, herdr/zellij tabs, cmux workspaces, Orca terminals, or T3 Code threads
...
- [docs/t3-backend.md](docs/t3-backend.md) - current setup, token handling, and limits for the experimental T3 Code backend, where each worker is a thread in the repository's T3 project.
