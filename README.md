...
- **Optional inbound WhatsApp channel** - opt in with one local `config/whatsapp.env` naming your own number so you can message the first mate from your phone and have it act through the same lifecycle as a chat request, on a listener with its own linked device that never touches the existing `mudslide send` path; dry-run records would-be replies locally, and a home that never opts in polls nothing and behaves exactly as before.
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional Relay and its X and Discord setup steps, the optional inbound WhatsApp channel, the files you set, and harness support.
- [docs/remote-secondmates.md](docs/remote-secondmates.md) - current setup, routing, transfer, recovery, and safety behavior for whole-home remote second mates.
- [docs/calm.md](docs/calm.md) - current Pi `/calm` behavior and supported presentation limits.
- [docs/wedge-alarm.md](docs/wedge-alarm.md) - configure the active alert for an away-mode escalation delivery that gets stuck.
- [docs/whatsapp-channel.md](docs/whatsapp-channel.md) - current setup, re-pairing, dry-run, and opt-out for the optional inbound WhatsApp channel.
