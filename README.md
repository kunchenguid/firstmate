...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` autonomy flag; the two PR-opening modes treat the PR body as public reviewer-facing writing, and both the crewmate and the first mate read the published body back before the work is reported or merged.
