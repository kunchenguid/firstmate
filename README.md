...
- **Pi quota at a glance** - trusted Pi and `pi-signed` sessions automatically add a width-aware quota row beside Pi's unchanged built-in footer, showing every fresh window whose provider account matched the active model credential at the last bounded check plus resets, plan, and credits while narrow, unverified, or unavailable displays stay explicit and compact. Observed credential writes quarantine the row immediately; one-second revision polling plus a bounded credential read contains missed filesystem notifications, so credential transitions have no zero-latency guarantee.
