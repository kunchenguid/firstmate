...
- **Optional Relay** - opt in with one local `.env` pairing token so firstmate can answer public mentions on X and Discord or use an explicitly configured private one-to-one loopback adapter, act on normal reversible mention requests through the same lifecycle as chat requests, acknowledge durably registered work, and post audience-safe milestone and final follow-ups within seven days without changing non-Relay behavior; a final public reply promised in a thread becomes durable state that is reconciled from disk, so a restart or a compacted conversation cannot lose it; dry-run preview records would-be replies and dismissals locally before go-live.
...
Optional secondmates extend this to persistent local or whole-home remote second mates, dispatch profiles let you steer which harness handles which task, and opt-in Relay lets the same fleet answer routed requests under a public or explicitly configured private one-to-one audience.
