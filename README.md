...
- **Disposable worktrees** - each task runs in a clean `proj`-provisioned git worktree, or an Orca-managed worktree when `backend=orca`, so parallel work on one repo never collides.
...
```
 ...
 └───┬────┘   └───┬────┘      └───┬────┘
     ▼            ▼               ▼
  proj worktree, Orca worktree, or isolated secondmate home
     │
