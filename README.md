...
| `/quiet`           | Keep routine wakes off the conversation while you stay and chat: on Pi and where an [attended supervision host](docs/supervision-host.md#quiet-mode) runs this is already the ordinary posture, and elsewhere it starts the same token-saving sub-supervisor as `/afk`, which ordinary messages do not exit - use `/quiet off` to leave it (on an opted-in host home, `/quiet` under a live away record first completes the return) |
