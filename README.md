...
- **Disposable worktrees** - each task runs in a clean [treehouse](https://github.com/kunchenguid/treehouse) git worktree, or a firstmate-created git worktree attached to an Orca daemon terminal when `backend=orca`, so parallel work on one repo never collides.
