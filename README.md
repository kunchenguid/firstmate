...
- **Disposable worktrees** - each task runs in a clean, owning-home-scoped [treehouse](https://github.com/kunchenguid/treehouse) git worktree, or an Orca-managed worktree when `backend=orca`; remotely preserved PR work is reclaimed locally and reconstructed only when continuation needs it.
