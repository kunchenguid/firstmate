...
- **Pi quota at a glance** - trusted Pi and `pi-signed` sessions automatically add a width-aware quota row beside Pi's unchanged built-in footer, showing every fresh window whose provider account matches the active model credential plus resets, plan, and credits while narrow, unverified, or unavailable displays stay explicit and compact.
