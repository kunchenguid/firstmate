...
- **Restart-proof** - all state lives on disk and in the active session backend (tmux by hard default, herdr when selected or auto-detected); kill the session anytime and the next one reconciles, including confirmed-dead secondmate agents, and carries on.
...
[Calm's current behavior and supported limits](docs/calm.md) are documented separately.
...
Setup guides for tmux (the default) and the herdr backend are linked in [Documentation](#documentation) below.
