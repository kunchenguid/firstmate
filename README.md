...
| `/loopd`           | Turn the current long task into a time-boxed self-driving loop where each heartbeat is one complete dynamic workflow round |
| `/loopds`          | Build a more durable loop contract first: clarify the goal, hard verifier, ledger, budget, stop conditions, and delivery path before running |
