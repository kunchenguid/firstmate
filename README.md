...
- **Automatic PR review and reviewer feedback** - the main home slow-polls your authenticated GitHub account through the same event-driven supervision plane, with no cron job, daemon, or extra token: every new head of a firstmate-authored PR gets one private adversarial review whose supported findings go straight back to the crewmate that owns the branch, and external review feedback is verified before the fix and one evidence-backed reply in the original thread; it never publishes its own findings as a review under the PR author's identity, never approves, and never merges, and `bin/fm-pr-review.sh opt-out <pr-url>` hands any single PR back to you.
