...
- **Pi quota at a glance** - trusted Pi and `pi-signed` sessions automatically add a width-aware quota row beside Pi's unchanged built-in footer, showing every fresh window for the active model provider plus resets, plan, and credits when available while narrow or unavailable displays stay explicit and compact.
