...
A bounded-read drop in the canonical snapshot, and any secondmate state that can reach no registered project card - work labelled with an unknown repository, or unlabelled work from a secondmate with no registered project - are disclosed board-wide rather than quietly shrinking a card.
