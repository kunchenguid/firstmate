...
Pi's `/supervision-model` command pins a cheaper model for the supervision branch alone, picked from Pi's own model list, and with no pin the branch simply follows your own conversation's model; see the [configuration schema](docs/configuration.md#pi-supervision-branch-model-configsupervision-branch-model).
