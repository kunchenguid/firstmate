...
- **Explicit project modes** - each project ships via `fast-preview` (global default: implement then human-test preview), `no-mistakes`, `direct-PR`, or `local-only`, with an optional `+yolo` autonomy flag.
