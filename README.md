...
A completed row that carries no repository label and whose task has already been torn down has nothing linking it to a project, so it is dropped rather than reported as a gap, and the recently-landed and pull-request panels each show at most five rows, newest completion first, with the true total in their heading; the pull-request panel lists open work ahead of completed work.
