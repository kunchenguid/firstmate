...
Codex and OpenCode are also verified and supported as primary harnesses; Codex 0.150.1 uses a native queue-first present daemon with guarded terminal injection and bounded checkpoint fallbacks, while OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
...
- [docs/treehouse-pool.md](docs/treehouse-pool.md) - why each firstmate home gets its own worktree pool, the measurements behind it, and the supervised backfill order.
- [docs/turnend-guard.md](docs/turnend-guard.md) - the primary session's current "no turn ends blind" backstop, scope, loop safety, and compatibility limits.
- [docs/verification/supervision.md](docs/verification/supervision.md) - active maintainer verification for session-start, guard, continuity, and wedge integrations.
- [docs/supervision-protocols/](docs/supervision-protocols/) - rendered primary-harness watcher protocols for Claude, Codex, OpenCode, Pi and `pi-signed`, Grok, traex, Cursor, and unknown harness fallback.
