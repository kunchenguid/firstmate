...
- **Captain-private recurring routine** - firstmate seeds a local daily and weekly routine registry and uses the existing authenticated watcher rail to surface each due action once per local period, with no calendar sync, credentials, or outbound communication; see [Recurring routine](docs/configuration.md#recurring-routine).
