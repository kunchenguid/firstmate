...
- **Optional idle-worker pre-compaction** - opt in with a local `config/idle-compact` threshold (15 minutes by default) so a genuinely idle Claude crewmate is compacted while its prompt cache is still warm, instead of burning quota reprocessing a stale one on the next steer.
