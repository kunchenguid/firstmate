...
**Status:** applied on branch `patch/wedge-cap-2026-08-19` (off main). v5 (2026-08-25) addresses Greptile review round 4: stop clearing `.wedge-permanent-<key>-*` markers on pause-class transitions (`handle_paused_stale`, `clear_pause_tracking`). The cap is now permanent per (window, hash) until the hash actually changes — manual operator `rm` is the only way to lift it.
