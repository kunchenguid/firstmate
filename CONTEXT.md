...
For **Comparison Mode**, it also declares distinct independently isolated server-side synthetic fixture namespaces for the baseline and candidate.
...
The captain's one-time authorization through an **Evidence Review Surface** to offer one exact previewed **Evidence Bundle** or batch, its manifest hash, **Evidence Report Identity**, artifact hashes, no-mistakes run and reviewed head, and named destination to no-mistakes.

**Publication Approval**:
The no-mistakes-owned one-time authorization granted after protected staging, revalidation, and its own exact preview to publish one exact evidence batch bound to its manifest hash, **Evidence Report Identity**, artifact hashes, run, reviewed head, allowed media types, and named pull request destination.
...
The human-readable report's relative bundle path and content hash recorded in the **Portable Manifest**.
...
Separate baseline and candidate browser contexts backed by distinct independently isolated server-side synthetic fixture namespaces with equivalent initial seeds, no shared mutable state, and no asynchronous cross-writes.
...
**Privacy Finding**:
An identifier, artifact reference, suspected region, and category for sensitive content detected during a **Privacy Review**, without copying the underlying privacy value into metadata.
...
- The active trusted host controller revalidates every **Review Decision** binding before acting, including the no-mistakes run, reviewed head, exact batch, manifest hash, **Evidence Report Identity**, artifact hashes, and pull request destination before routing **Evidence Import Consent** for protected staging.
...
- A **Portable Manifest** includes the demonstrated claim, **Evidence Report Identity**, relative artifact names, artifact types, capture time, file hashes, **Capture Scope**, **Privacy Scan Result**, and relevant tool versions.
...
- Each comparison **Evidence Scenario** declares distinct independently isolated server-side synthetic fixture namespaces for the baseline and candidate.
- Both namespaces are seeded to equivalent initial synthetic state before either capture begins.
- The namespaces share no mutable state and reject asynchronous writes across the baseline and candidate boundary.
...
- Server-side capture isolation operates only on synthetic test state.
...
- Privacy test values may appear in an **Evidence Review Surface** only as pixels intentionally rendered from the private artifact under review.
- Privacy test values never appear in review-page scaffolding or metadata, **Portable Manifests**, reports, logs, or controller state.
- Version 1 does not add masking or reveal controls for artifact pixels.
...
- The active trusted host controller stores explicit acceptance in private decision state bound to the unchanged manifest hash, **Evidence Report Identity**, artifact bytes, exact **Privacy Finding** set, batch, and destination.
...
- Derivation, cropping, redaction, exclusion, report change, artifact byte change, batch change, **Privacy Finding** set change, or destination change invalidates the decision and requires a new exact preview and **Review Decision**.
...
- No-mistakes protects and revalidates the staged manifest, report, and artifacts, presents their exact identities in its own preview, and alone owns the **Publication Approval** that authorizes pull request mutation.
...
- Any report, manifest, artifact, destination, run, or reviewed-head drift, or any replay, invalidates the affected consent or approval and requires a new exact preview and authorization.
...
- A **Publication Approval** applies only to the exact no-mistakes-staged and previewed manifest hash, **Evidence Report Identity**, artifact hashes, batch, run, reviewed head, allowed media types, and named pull request destination.
...
> **Domain expert:** "No, use separate browser contexts and distinct independently isolated server-side synthetic fixture namespaces, seed both equivalently before either capture, and permit no shared mutable state or asynchronous cross-writes."
...
> **Domain expert:** "No, it only permits offering that exact batch, including its **Evidence Report Identity**, and destination to no-mistakes for protected staging, revalidation, its own preview, and a separate **Publication Approval**."
...
- How baseline and changed-state captures isolate browser and server state within a **Task-scoped Browser Profile** was unresolved - resolved: **Comparison Mode** uses separate browser contexts and distinct independently isolated server-side synthetic fixture namespaces seeded equivalently before either capture, with no shared mutable state or asynchronous cross-writes.
