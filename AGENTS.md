...
state/               volatile runtime signals; gitignored
  ...
  <id>.launch-brief.md   owner-only per-launch harness rendering of a ship brief; removed by a spawn that aborts before the worker launch is delivered, and by teardown, so a retained live pane keeps its instructions
...
Leave `__FM_NO_MISTAKES_INVOCATION__` unchanged in a no-mistakes ship scaffold; `bin/fm-brief.sh --help` owns its render, refusal, and legacy-compatibility mechanics.
