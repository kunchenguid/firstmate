...
config/claude-accounts  optional map from Claude account names that dispatch profiles select to absolute Claude config directories; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Claude accounts"
config/crew-dispatch.json  optional crewmate dispatch profiles; LOCAL, gitignored; firstmate-maintained but human-editable natural-language rules that choose a per-task harness/model/effort/account profile (section 4). Inherited by secondmate homes
