...
Specific tracked, `data/`, `state/`, `config/`, or `projects/` entries with their own authoritative owner, reached instead of one exhaustive tree anywhere: `config/crew-harness`, `config/secondmate-harness`, and `config/model-catalog.json` in [`docs/architecture.md`](docs/architecture.md), [`.agents/skills/harness-adapters/SKILL.md`](.agents/skills/harness-adapters/SKILL.md), and [`docs/configuration.md`](docs/configuration.md); and `data/secondmates.md` in [`.agents/skills/secondmate-provisioning/SKILL.md`](.agents/skills/secondmate-provisioning/SKILL.md); all other entries resolve through [operational home layout and state](docs/configuration.md#operational-home-layout-and-state) and the producing script headers named there.
To resolve a `state/<id>.meta` key, read the header of the script that writes it: `bin/fm-spawn.sh` owns the base task-metadata fields, including `routing_source=secondmate-config` when its complete durable secondmate tuple owns the launch; `bin/fm-pr-check.sh`, `bin/fm-pr-merge.sh`, and `bin/fm-merge-local.sh` own the PR and local-delivery fields, `bin/fm-x-link.sh` owns the X-mode fields, `bin/fm-backend.sh` owns backend-specific fields, and `bin/fm-teardown.sh` owns `telemetry_terminal_payload_sha256=`; [`docs/configuration.md`](docs/configuration.md#operational-home-layout-and-state) states only the schema-level rule that `bin/fm-spawn.sh` owns the base fields it emits.
Three `state/` families are agent-private runtime machinery rather than catalog entries and must never be touched directly - `.claude-autoarm.lock`, `.claude-autoarm-epoch`, `.claude-autoarm-failure-notified`, `.claude-autoarm-failure-alarmed`, `.turnend-claude-blocks`, and `.turnend-claude-blocks.lock` (Claude Stop auto-arm records); `.hash-*`, `.count-*`, `.stale-*`, `.stale-since-*`, `.paused-*`, `.wedge-escalations-*`, `.seen-*`, `.hb-surfaced-*`, `.last-*`, and `.heartbeat-streak` (watcher internals); and `.subsuper-*` and `.supervise-daemon.*` (sub-supervisor internals); see [`docs/configuration.md`](docs/configuration.md#agent-private-state-never-touch) for the prohibition.
...
When that section reports checks still in progress, treat none of them as passed until the result lands through `bin/fm-startup-network.sh report` or a `check: startup-network` wake.
...
For an ordinary ship or scout brief, use its scaffold as the contract, then replace exactly the two standalone `{TASK}` lines - under `# Task` and `# Load-bearing contract` - with a clear task description, acceptance criteria, constraints, and necessary context before dispatch.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `MEMORY_DOCTOR:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
