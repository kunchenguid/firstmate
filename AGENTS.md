...
The spawn also refuses to launch a based task against a base that has already merged, or one abandoned without merging, so a `--base` that names the wrong branch costs a command rather than a whole crewmate run; `bin/fm-spawn.sh`'s header owns those refusals and the recovery each one names.
