...
A first spawn refuses a base that does not exist on origin, so a mistyped `--base` costs a command rather than a whole crewmate run; a respawn of a task that already declares that base keeps the declaration without re-probing, because a base merging and being deleted mid-flight is the normal end-state of a stacked PR.
