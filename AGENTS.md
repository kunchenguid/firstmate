...
config/claude-account-profiles  optional home-local native Claude account aliases; LOCAL, gitignored, mode 600, and not inherited or transferred; docs/configuration.md owns setup and schema
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  quota-cooldowns.json  durable evidence-backed routing cooldowns with automatic expiry; written only by bin/fm-quota-cooldown.sh, whose recover command quarantines an invalid store beside it as quota-cooldowns.json.corrupt.<instant>
  ...
  pr-templates/<project>.md  optional private per-project PR body template; LOCAL, gitignored; rendered and filled by bin/fm-pr-body.sh, whose header owns template precedence, placeholder-fill, and refusal mechanics; a templated direct-PR ship brief requires using it (bin/fm-brief.sh)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, access=reader only for a slot-free reader scout (whose worktree= is its disposable scratch directory, not a pool worktree), mode=, yolo=, tasktmp=, base_commit= (ship/writer-scout worktree HEAD or reader-scout bare-handle HEAD before launch, preserved across same-boundary relaunch), and the opaque model-attempt identifiers telemetry_attempt= and telemetry_task_root= that teardown seals, alongside the telemetry_terminal_payload_sha256= digest teardown records for the explicit terminal payload it sealed so a rerun of the same command stays a no-op while a different payload still reaches the ledger's contradiction refusal; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); a crewmate or scout spawn records its dispatch attestation as dispatch=resolved, or dispatch=override plus dispatch_override_reason=, together with the catalog-established dispatch_provider= and dispatch_model_family= axes it was passed and the routing facts it was passed as matched_rule=, quota_decision=, quota_headroom=, and quota_runway=, while crew dispatch profiles are active or a durable routing cooldown file exists (docs/configuration.md "Crew dispatch profiles", "Routing cooldowns"); kind=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-merge-local records local_delivery_base=/local_delivery_head= around the exact net-content interval unique to an approved local-only task at landing time; fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md), carries a valid missing_review_override_ts= forward only while that recorded pr= is unchanged, while fm-pr-merge records a new timestamped value before an explicitly authorized missing-Review merge attempt and clears a stale one before an ordinary reviewed merge; fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14); --routing-source records routing_source= (captain|profile|fallback), the escalation ladder's precedence guard; --account-profile records account_profile=<alias>, the home-local native Claude account alias only and never its directory, which every escalation verdict re-emits for the recovery relaunch to pass back unchanged (.agents/skills/stuck-crewmate-recovery/SKILL.md)
  <id>.escalation    append-only routing-escalation log written only by bin/fm-harness.sh escalate, whose header owns the format and the attempt budget it encodes; removed by teardown
  ...
  slack-watch.check.sh generated Slack captain channel poll shim; present only when opted in (docs/configuration.md "Slack captain channel")
  model-telemetry-receipts/  private per-task model-attempt receipts (opaque ids only) that make ledger writes crash-recoverable; written only by bin/fm-model-telemetry.sh and removed when the attempt is sealed
...
Do not separately re-read the context, backlog, or metadata inputs it just printed, and do not bulk-read the status logs it only named; inspect a source only when it was reported absent or corrupt, older status history is specifically needed, or a targeted workflow must inspect before writing.
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; one on-demand status line per task naming the full `state/<id>.status` wake-event log path and the `bin/fm-crew-state.sh` read that answers current state (a positive status-tail override restores the bounded tail rendering); the `state/.afk` flag; and one cheap alive/dead read of each task's recorded backend endpoint.
   Its bounded `RECORD CONTRADICTIONS` subsection prints only disagreements across those records and cheap endpoint or recorded-PR ground truth, and stays absent when the records agree; `bin/fm-record-contradictions-lib.sh` owns the exact comparisons and bounds.
...
When dispatch profiles exist, consult them at every crewmate or scout intake and pass the resolved concrete profile required by `fm-spawn`, which fail-closed refuses a bare explicit harness and requires a dispatch attestation (`--dispatch-resolved` or `--dispatch-override-reason`) so the consultation cannot be silently skipped; see `bin/fm-spawn.sh`'s header for the exact flags.
Routing precedence is an explicit per-task captain override, then the best-fit configured rule, then the configured default, then the static crewmate harness.
When a provider refusal or an exhausted measured `quota-axi` window proves a limit, record its narrowest proven scope through `bin/fm-quota-cooldown.sh`; never infer a cooldown from slowness or silence, and never hand-edit its durable file.
While `data/quota-cooldowns.json` exists, pass the catalog-established `--dispatch-provider` and `--dispatch-model-family` on every crewmate or scout spawn, including the static crewmate-harness path, and carry an explicit captain instruction as `--dispatch-override-reason`; `bin/fm-quota-cooldown.sh`'s header owns what it refuses and what it records (docs/configuration.md "Routing cooldowns").
Pass the routing-decision facts firstmate already has to `fm-spawn` so the intake row carries them: `--matched-rule` (the rule id or `default`), `--quota-decision`, `--quota-headroom`, and `--quota-runway` from the `quota-axi` reading that informed the choice, plus `--routing-source` and the dispatch attestation; `fm-spawn` writes only what is passed and omits the rest, so the no-profile path stays unchanged.
Firstmate alone resolves a matched profile array: run `quota-axi --json` at that intake, evaluate every configured candidate against that current output, and choose with inspectable effective headroom, usable runway, and binding-window reserve, using a short window only as a later tie-break when the binding weekly window is on or behind pace.
Never hold ready work to conserve quota, and never weaken the required reasoning class; when every equivalent candidate is tight, report the pressure and continue under this routing contract.
`bin/fm-quota-utilization.sh` owns that widget-compatible utilization metric, binding-weekly reserve, usable runway, and end-of-window outcome table from the same snapshots.
...
Load `quota-array-dispatch` before choosing among a matched profile array; that skill is the single owner of the completion-aware selection procedure, including durable cooldown consultation and explicit captain overrides.
The generic effort fallback and its precedence are owned by `harness-adapters`: explicit captain and standing configured effort win; otherwise use low for well-understood explicit work, xhigh for ambiguous investigation or design, intermediate levels proportionally, and never max without explicit captain preference.
Do not add model-specific versions of that policy.
On a recovery relaunch after failure, routing changes (effort, model, harness) go through the classifying ladder in `stuck-crewmate-recovery`, whose mechanics `bin/fm-harness.sh escalate` owns; the ladder acts only where the generic fallback acted, never through a captain pin or configured profile, and never selects max.
...
Status logs are wake-event history surfaced through the digest's on-demand per-task pointers, never current-state truth; use targeted current-state reconciliation when the live state matters.

Reconcile only this home's recorded direct reports and their recorded backend inventory; never sweep a shared endpoint namespace for matching names or claim another home's work.
For an ordinary direct report whose endpoint is dead or metadata has no window, load `stuck-crewmate-recovery` and preserve the recorded task environment and unlanded work while reconciling ownership.
...
When the captain invokes `/gather` or explicitly asks to gather reusable ecosystem learnings, load the `gather` skill before curating or exporting any lesson.
...
- **Scout** produces knowledge in `data/<id>/report.md`, never a PR, and is appropriate for investigation, diagnosis, planning, reproduction, or audit work when the captain explicitly requests a separate knowledge or design deliverable or unresolved uncertainty could materially change whether or what to build.
  At scout intake also classify its access axis: a scout that only reads - research, monitoring, reviewing diffs - dispatches slot-free with `--access reader` (a scratch directory plus an object-store read handle, no pool worktree), while a scout that must edit or run the checkout stays on the default writer worktree; `bin/fm-spawn.sh` and `bin/fm-brief.sh` own the axis, its enforced target-project read-only boundary, and its broader no-tracked-file-writes operating contract, and a reader is never promoted in place.
...
For any custom `state/<id>.check.sh` you write yourself, keep it an ordinary single-link mode-`0700` file, print one line only when firstmate should wake, print nothing otherwise, finish before `FM_CHECK_TIMEOUT`, then bind its current bytes with `bin/fm-check-register.sh <id>` before the watcher may execute it, and if it watches a finite subject, print a final wake line when that subject reaches a terminal state so firstmate retires it on that wake with `bin/fm-check-register.sh retire <id>`.
...
When implementation is separately authorized, promote the existing writer scout through `bin/fm-promote.sh` rather than creating a duplicate task; `bin/fm-promote.sh` refuses a reader scout, whose implementation dispatches as a fresh ship task instead.
...
- Every current daemon injection uses the `away-supervisor` kind and every current `bin/fm-sessionstart-nudge.sh post-compact` re-anchor uses the `post-compact` kind from `bin/fm-operational-input.sh` after `FM_OPERATIONAL_PREFIX` (U+2063 INVISIBLE SEPARATOR followed by `FIRSTMATE_OP: `), while the `/afk` skill owns legacy bare-marker compatibility; both typed kinds are trusted operational input, and `post-compact` requires re-reading the complete captain, shared-captain, learnings, and active task metadata files without running `bin/fm-session-start.sh` before further action, while its read-only contradiction refresh stays silent when those records agree.
...
When a routine operational update's specific event requires no action but a response must be sent, reply exactly `Shipshape.` without characterizing the visible session's unrelated decisions.
...
- `quota-array-dispatch` - load before choosing among a matched crew-dispatch profile array from current quota-axi output and durable routing cooldowns.
