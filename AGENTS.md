...
It falls back to the default branch, saying why, once that base stops being a meaningful diff base - it is gone from origin, its work has already merged, or the pipeline rebased the branch under review off it - so the review is never blocked and always shows what the merge would land on the default branch.
