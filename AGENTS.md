...
- `fm-telegram-respond` - load on a `telegram-msg <update_id>` `check:` wake to drain the Telegram inbox and apply the closed Stage-2 grammar, and on a `telegram-mode-error ...` `check:` wake to report the Telegram polling blocker; relevant only when Telegram mode is on.
