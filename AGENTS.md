...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, secondmate sync and inherited local-material propagation, self-update, approved `local-only` merge paths owned by their referenced skills and scripts, the per-home worktree-pool config, and a concrete captain-approved project operation governed directly by this rule.
   The worktree-pool exception is `bin/fm-project-pool.sh` and nothing else: it writes exactly one `treehouse.toml` at a clone's root plus one line in that clone's local git exclude file, refuses rather than overwrites a `treehouse.toml` the project owns, and touches no other file.
   It is not a general licence to write configuration into a project.
...
control-root/        relay task-host scripts, committed; deployed to each remote task host by bin/fm-relay-conn.sh, never run from here (docs/relay-host.md)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=, an optional launch= naming the selected launch variant, and tmux_socket= for a tmux task; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); kind=secondmate also records home= and projects=, plus host= for a fleet-peer home and remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for an SSH remote route; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for a Relay-originated task (section 14)
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the bootstrap MUTATING sweeps - non-executing legacy PR-check migration, relay reconnect, helm task adoption, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1; the network ones among them run in the deferred stage rather than in this section.
...
Use `gh-axi` for GitHub, `bytedcli` for Codebase MR operations, `chrome-devtools-axi` for browser work, and `lavish-axi` for structured decisions or reports; consult current help rather than memorizing flags.
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `traex`, plus `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
`traex`'s crewmate/scout face is live-verified and its primary/secondmate face is wired and component-verified, with the remaining live gap recorded in `harness-adapters`.
...
A matched profile array in which any candidate declares `launch` therefore resolves to its first element without consulting quota at all; every other array follows the completion-aware procedure below.
...
- **direct-PR** has the worker push and open a PR without the no-mistakes pipeline, then waits for the configured merge authority while firstmate keeps an escalate-only no-mistakes watch on that PR.
...
`bin/fm-scm-lib.sh` and `docs/codebase-scm.md` own the Codebase provider seam, whose per-provider merge default matters here: GitHub PRs squash, while Codebase MRs take a real merge commit and are never squashed.
...
A teardown refusal - for uncommitted or unlanded work, or for a no-mistakes run the task still owns - is a stop-and-investigate result, never an obstacle to bypass.
...
Token discipline: for a crewmate's current state prefer `bin/fm-crew-state.sh <id>`; default peeks to 40 lines; never stream a pane repeatedly through yourself; batch what you tell the captain.
