...
config/crew-disabled-plugins  optional list of plugin ids to disable in claude crew sessions (one id per line, # comments allowed); LOCAL, gitignored; absent or empty = no change to plugin state; disables via --settings enabledPlugins, the only scope proven to suppress hooks; see docs/configuration.md "Crew plugin suppression". Inherited by secondmate homes
