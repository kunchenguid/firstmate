...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  tasks/<id>/brief.md      per-task crewmate brief, or per-secondmate charter brief when kind=secondmate
  tasks/<id>/report.md     scout task deliverable, written by the crewmate; survives teardown
...
- **Scout** produces knowledge in `data/tasks/<id>/report.md`, never a PR, and is appropriate for investigation, diagnosis, planning, reproduction, or audit work when the captain explicitly requests a separate knowledge or design deliverable or unresolved uncertainty could materially change whether or what to build.
...
Reply exactly `Captain, shipshape.` only for a direct captain-chat no-op that still needs an answer - an idle re-read, an empty heartbeat, or a pure acknowledgement with no consequence for the captain - without characterizing the visible session's unrelated decisions.
A private supervision processing turn that needs no new captain action or information produces no visible assistant text at all, not `Captain, shipshape.`.
For a captain-requested completion or any wake that needs the captain's review, approval, merge, or design pick, give one captain-facing outcome only when that action is genuinely needed; a durable record alone does not discharge a required review or decision.
...
When the captain will review, deliver, or monitor the result, add the structured `## Captain review plan` subsection owned by `docs/task-lifecycle.md`; derive every action and check from accepted intent, never invent criteria for `/next`.
