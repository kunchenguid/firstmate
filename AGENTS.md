...
config/crew-config-dir  Claude config dir a spawned claude crewmate authenticates with; LOCAL, gitignored; absent = bare `claude` on its default config dir, which strands a crewmate on the login wall when that dir is unauthenticated (docs/configuration.md "Claude config dir")
