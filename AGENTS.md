...
state/               runtime records and signals; gitignored
  ...
  .progress-<id> .progress-tick.pid   display-only progress observation record (phase, per-phase seconds, last applied label suffix, last tick) advanced by the watcher tick and the fleet snapshot and retired by teardown, plus the watcher's single-flight marker naming its last detached progress tick child (a dead pid is reclaimed); both safe to delete; bin/fm-progress-lib.sh owns the record, bin/fm-watch.sh the marker
