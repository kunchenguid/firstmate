...
A forge's merged-by field never identifies who decided, because firstmate, every crewmate, and the captain act through one shared forge identity; when a merge's authorization is in doubt, `bin/fm-merge-attribution.sh <id>` reports whether it went through the recorded merge path, and a merge with no matching record reads as unattributed, never as firstmate's own.
