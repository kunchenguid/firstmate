...
- `grep-loop-review-workflow` - load only for an explicitly requested bounded review or review re-check loop outside ordinary no-mistakes delivery; it owns evidence collection and reporting, never PR creation, push, merge, or background orchestration.
- `goal-prompt-builder` - load when a crewmate must refine an idea into an auditable `/goal`; it owns the one-question workflow, scenario and format selection, five-section contract, and auditability check, and never executes the goal.
- `goal-kanban` - load when a crewmate needs to organize planning cards or model an approved multi-part chain; it owns the planning states and dependency contract, and never creates a board service or orchestrates work.
