...
Never merge a red PR under either setting unless a current explicit captain instruction names each GitHub check waived through `fm-pr-merge.sh --allow-red`, repeated once per named check; that attended-only waiver still requires every other check green.
