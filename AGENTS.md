...
Set `FM_OPENCODE_VISIBILITY=headless` (default), `web`, `desktop`, or `both`; `desktop` records OpenCode Desktop's `opencode://new-session` deep link without opening it, and `terminal`/`attach`/`tui`/`app`/`chat` are aliases for that Desktop-link path rather than external terminal launchers.
OpenCode Desktop does not currently expose an attach-to-existing-server-session deep link, so FirstMate does not auto-open standalone prompt-backed Desktop chats by default; set `FM_OPENCODE_DESKTOP_OPEN=1` to explicitly launch the fallback `new-session` link, and set `FM_OPENCODE_DESKTOP_PROMPT` only when Desktop should start its own chat/model call.
Set `FM_OPENCODE_MODEL` or `FM_OPENCODE_SERVER_MODEL` to pass an explicit `provider/model` to `opencode serve` for tasks, useful when the user's default OpenCode model is unavailable for the active account.
