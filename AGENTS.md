...
state/               runtime records and signals; gitignored
  ...
  .lock-generation .codex-primary-* .codex-queue-*  private Codex primary binding and queue-doorbell generation/coalescing records; never touch
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `traex`, and `cursor`, plus `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
...
- `pr-description` - load immediately before preparing or revising any GitHub PR body or Codebase MR description, including a daily sync MR.
