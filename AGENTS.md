...
config/crew-claude-agent  optional Claude agent profile for crewmate and scout launches; LOCAL, gitignored; absent = whatever agent profile the operator's own global Claude settings select, and secondmate launches never use it. Inherited by secondmate homes (section 4)
