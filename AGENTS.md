...
Before calling `treehouse return`, teardown also checks the shared no-mistakes surface: if another branch has an active no-mistakes run, or any `no-mistakes` process has cwd inside the worktree being returned, teardown refuses so treehouse cannot terminate the shared daemon and cancel unrelated validations.
...
It also resets inherited ignored SIGHUP handling before installing cleanup traps, so validation contexts that ignore SIGHUP do not leave orphan watcher children behind.
