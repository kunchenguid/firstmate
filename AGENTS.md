...
config/pi-bash-timeout  default timeout Firstmate's Pi extensions fill into bash tool calls that carry no valid explicit timeout; LOCAL, gitignored; absent = 900 seconds, a positive integer up to 2147483 selects that many seconds, and 0/"off"/"none" disables injection so Pi's unbounded no-timeout behavior applies; FM_PI_BASH_TIMEOUT_SECS overrides it; inherited by secondmate homes (docs/configuration.md)
