...
.env                 optional Relay pairing token and typed dispatch resolution key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
`bin/fm-quota-gate.sh` is a separate deterministic backstop inside `fm-spawn`: it refuses a crewmate or scout spawn (never secondmate) when Claude-provider quota is low, with `FM_QUOTA_OVERRIDE=1` as the bypass; its own header owns the thresholds and exit-code contract.
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, as the first rule-match resource; its inspectable matched rule informs firstmate, but every result returns to the intake above for catalog, provider, authentication, reasoning-class, quota, and captain-approval gates (contract: `docs/configuration.md` "Typed dispatch resolution").
