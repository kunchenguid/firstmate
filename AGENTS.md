...
If `state/.afk` is present, load `/afk` in away mode or `/quiet` in quiet mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`); where its daemon is alive, let the daemon own supervision rather than arming another cycle, and on Pi keep the ordinary supervision session, which runs in both postures.
...
- While a LIVE daemon owns supervision (`bin/fm-wake-lib.sh`'s `fm_afk_daemon_owns_supervision`), do not arm a separate watcher; the `state/.afk` flag alone never proves that, because it outlives the daemon under every signal.
