...
config/claude-account  optional per-home Claude account pin: the absolute path of the Claude config store this home's launches bill; LOCAL, gitignored and never inherited, re-resolved at every spawn, and refused rather than silently replaced by the primary's account when unusable; see docs/configuration.md "Claude account store"
