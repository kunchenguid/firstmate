...
state/               runtime records and signals; gitignored
  ...
  periodic/          private standing periodic-check specs, their trust bindings, and durable next-due markers; written only by bin/fm-procevent-periodic.sh (section 13's process-event-sources trigger)
...
- `process-event-sources` - load before arming a long-polling source, before registering a deterministic condition->action watch (do X as soon as Y is true), before registering a standing periodic check that should run on a cadence, and on any `procevent <adapter> <source-id> <sequence>` check wake.
