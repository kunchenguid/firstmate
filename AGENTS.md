...
config/secondmate-harness.d/<id>  optional per-secondmate launch pin in that same "<harness> [<model>] [<effort>]" format, winning over config/secondmate-harness for that id's configured-profile launches and recoveries; LOCAL, gitignored, primary-only, NOT inherited; docs/configuration.md "Per-secondmate launch pins"
...
If static `config/crew-harness`, `config/secondmate-harness`, or a per-secondmate pin under `config/secondmate-harness.d/` names an unverified adapter, report it and do not launch it; any current-work fallback must be an explicit verified override rather than treating the configured value as absent.
