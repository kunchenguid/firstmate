...
config/harness-overrides.json  optional per-harness launch override; LOCAL, gitignored; per-harness command/args/env that customize HOW a harness launches without changing WHICH harness it is (section 4; full contract docs/configuration.md). Inherited by secondmate homes
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, appends pr= and provider pr_head= when available; fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
...
2. **Bootstrap** - detect-only diagnostics (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, harness-overrides validation, backlog-backend status) always run and always print.
...
Use `gh-axi` for all GitHub operations, `bytedcli` for Codebase MR operations, `chrome-devtools-axi` for all browser operations, and `lavish-axi` when a decision or report is complex enough to deserve a rich review surface.
...
To customize HOW a selected harness launches (its binary, launch args, or launch env) without changing WHICH harness is selected or losing supervision, use the optional local `config/harness-overrides.json`; the full contract lives in `docs/configuration.md`.
...
`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/harness-overrides.json` are inherited into every secondmate home; `config/secondmate-harness` is not, because secondmates never spawn secondmates.
...
Run `bin/fm-pr-check.sh <id> <PR/MR url>` - it records `pr=` and provider `pr_head=` when available in the task's meta and arms the watcher's merge poll.
...
Teardown closes the task's backlog row itself; when it prints a manual reminder instead (backlog backend `manual`, or `tasks-axi` missing) or reports the write failed, move the task to Done in `data/backlog.md` yourself with the full `https://...` PR/MR URL or local merge note and date, and keep Done to the 10 most recent.
...
- Teardown records it in Done with the report path instead of a PR link; if it printed a manual reminder or reported the write failed, hand-edit `data/backlog.md` and keep Done to the 10 most recent, then re-evaluate the queue and dispatch only queued work whose blockers are gone and whose time/date gate, if any, has arrived.
...
- Every daemon injection is prefixed with `FM_INJECT_MARK`, terminal-safe invisible separator `U+2063`, so internal escalations are distinguishable from a captain message.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints any diagnostic or capability line (`MISSING:`, `NEEDS_GH_AUTH`, `TANGLE:`, `BACKLOG_ORPHAN:`, `CREW_HARNESS_OVERRIDE:`, `CREW_DISPATCH:`, `FLEET_SYNC:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `TASKS_AXI:`, `NUDGE_SECONDMATES:`, or `FMX:`); silence needs no load.
