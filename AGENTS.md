...
- `captain-writing` - load before sending a substantive captain-facing message, before sending anything composed in the captain's name, and before writing a report or artifact copy the captain will read.
  Section 9 owns what such a message must say; that skill owns the revision pass, including handing the draft to a separate agent that never saw the reasoning behind it.
