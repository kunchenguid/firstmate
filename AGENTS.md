...
config/gh-mentions.json  optional GitHub mention plane: the repositories watched beyond this home's registered projects, the logins - permanently or for a bounded trial - whose tagged comments firstmate acts on, and the markers that select them; LOCAL, gitignored, absent means the whole plane is a complete no-op, and NOT inherited by secondmate homes; see docs/configuration.md "GitHub mentions"
config/watched-tools.json  optional list of the tools this home depends on, read by the update check armed with bin/fm-tool-update-check.sh; LOCAL, gitignored, firstmate-maintained but human-editable, and NOT inherited by secondmate homes; see docs/configuration.md "Watched tool updates"
config/x-mode.env    generated watcher cadence, carrying the one interval the fastest enabled plane asked for; LOCAL, gitignored; source before arming watcher when present; see docs/configuration.md "Watcher cadence"
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - same-home backlog reconciliation, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, Relay artifact writes, and GitHub mention poll arming - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
- `gh-mention-respond` - load on a `check: gh-mention <record-id>` wake to handle a tagged comment from a trusted account in a watched repository; relevant only when this home configured the GitHub mention plane.
