...
config/launch-wrapper  optional operator-chosen command prefix prepended to every spawned agent's launch, with __TASKID__ substituted; LOCAL, gitignored; absent = no wrapper and unchanged launches; per-home and NOT inherited into secondmate homes (docs/configuration.md "Launch wrapper")
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/merge.md      one line per PR merge confirmed at the forge by fm-pr-merge; survives teardown
...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .wedge-deferrals-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done: PR <url> checks green - merge pending, not yet landed` after CI is green, while `direct-PR` reports `done: PR <url> opened - merge pending, not yet landed` after opening the PR.
Either signal is a handoff, not a landed task: firstmate still confirms the PR can merge and lands it under the configured merge authority.
