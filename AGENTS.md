...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed routing key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh and bin/fm-jev-router.sh; docs/configuration.md "Typed dispatch resolution", "Jev intake router"); LOCAL, gitignored
