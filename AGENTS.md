...
state/               runtime records and signals; gitignored
  ...
  <id>.adversarial-review/  one round-<N>/ directory per adversarial-review round on this task's PR, holding the evidence staged before dispatch (diff, prose, file list), the per-slot reviewer prompts, each lens report and the seat that produced it, the finding resolutions, the reconciliation, and the comments posted into the PR; written only by bin/fm-adversarial-review.sh, which owns every field; teardown does not retire it, and it is safe to delete once the PR has landed
  <id>.adversarial-review-green  the loop-green marker bin/fm-pr-merge.sh requires before merging a GitHub pull request: exactly a pr=, head=, tier=, and required= line, written only by bin/fm-adversarial-review.sh on a GREEN reconciliation at that head or a captain-granted T0 waiver, and revoked by that same script when a later reconciliation of the same PR comes back RED, so the marker always says what the newest reconciliation said; any other shape is malformed and refuses the merge, as does a marker naming a different PR or head, so a surviving one after teardown cannot clear a later PR
...
Every GitHub PR Firstmate raises runs the adversarial-review loop to GREEN at the head being merged before any merge is requested, whatever the CI state or the hour, and `bin/fm-pr-merge.sh` refuses without that evidence; `bin/fm-adversarial-review.sh --help` owns the tiers, seats, round cap, the evidence and results posted into the PR itself, and the captain-granted T0 waiver.
...
Run `bin/fm-pr-check.sh <id> <PR url>` with the URL copied from that ready signal - it records `pr=` and the forge's `pr_head=` when available in the task's meta, arms the watcher's merge poll, and re-arms the PR-open watch that dispatches the adversarial-review loop, so on GitHub the loop starts itself and you drive its rounds to GREEN rather than arming it by hand.
