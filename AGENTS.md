...
A remote secondmate can use only `envelope:<name>`, because `attended` asserts that a person is at the sending pane and cannot authorize a worker on another host.
