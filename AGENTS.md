...
config/lavish-axi-host  optional one-line per-machine Lavish server address; LOCAL, gitignored, inherited by same-machine secondmate homes but never sent to a remote host, and exported into every worker launch; see docs/configuration.md "Lavish server address" for opening versus polling
