...
- `board-automation` - load before arming, killing, restarting, or supervising the board-driven automation daemon (`bin/fm-board-daemon.sh`, which auto-launches agents when the captain drags a card on the Azure board), editing the board lane->project map, or reconciling board-spawned tasks.
