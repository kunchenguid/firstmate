...
At PR ready, verify the author-controlled title and body prose against the propagated preferences, then have the delivery-mode owner correct mismatches before presentation; never replace the whole PR body or alter delivery-mode-owned content such as the no-mistakes `## Pipeline` section and its signature.
