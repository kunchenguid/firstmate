...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   A ship task whose meta records `base=` (dispatched with `--base <branch>`) treats that branch, not the default branch, as its landing target throughout these checks.
...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend, herdr and zellij are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md) - herdr can also be selected by runtime auto-detection, zellij never is (always a dedicated background session), and orca is currently primitive-only so fm-spawn refuses it; not inherited into secondmate homes
...
4. Use the `window=` values from the digest's `state/*.meta` entries as the live direct-report set, and read the digest's per-task `endpoint: alive|dead` line for each - that cheap check is already done; do not re-probe it yourself.
   Do not sweep every `fm-*` tmux window, herdr tab, or zellij tab across all sessions during recovery; another firstmate home's child endpoints may share that namespace and are not this home's orphans.
...
All truth lives in each task's backend live-task inventory (tmux by hard default, or herdr/zellij when explicitly selected - herdr can also be auto-detected, zellij never is), state files, data/backlog.md, data/captain.md, data/learnings.md, data/secondmates.md, persistent secondmate homes, and treehouse; your conversation memory is a cache.
...
- `local-only` - local branch, no remote, no PR; firstmate reviews the diff, the captain approves, firstmate merges to local `main` (or the task's recorded base branch; section 7).
...
bin/fm-spawn.sh <id> projects/<repo> --base <branch>  # ship task branched from and landing into <branch> instead of the default branch; records base= in meta
...
Each task's backend live-task inventory is the ground truth (tmux when `backend=` is absent; a task's meta may record a different `backend=` - herdr and zellij are the other implemented, spawn-capable, experimental backends today, docs/herdr-backend.md and docs/zellij-backend.md; orca is primitive-only and has no task inventory yet).
...
For a ship task dispatched with `fm-spawn.sh --base <branch>`, scaffold with the same `--base <branch>`: the Setup then has the crewmate verify the base branch exists in the worktree (fetching it from origin if needed) and branch from it, and the local-only/direct-PR landing wording targets that branch; like `fm-spawn.sh`, the flag is refused for `--scout` and `--secondmate`, and without it the scaffold is byte-identical to the default-branch brief.
