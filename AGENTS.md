...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged, and after the spawn record the profile you actually dispatched by rerunning the same script with `--record-dispatch` (contract: `docs/configuration.md` "Typed dispatch resolution").
