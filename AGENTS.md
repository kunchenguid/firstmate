...
config/fleet-crew-limit  optional positive-integer ceiling on live ship and scout workers across the local root home and every local secondmate below it, read only from that root; absent = no ceiling; LOCAL, gitignored; see docs/configuration.md "Fleet worker ceiling"
