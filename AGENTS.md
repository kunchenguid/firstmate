...
config/crew-claude-config-dir config/worker-sandbox-posture-check  optional host-local crew Claude sandbox directory and estate posture-check executable; LOCAL, gitignored, default-off, and not inherited because their absolute paths and proof are host-specific; see docs/configuration.md "Crew Claude sandbox"
