...
state/               runtime records and signals; gitignored
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests; each note appends one `check` wake and stays pending until acknowledged with `bin/fm-inbox.sh drain --ack <id>`, which moves it to inbox/handled/, and its durable outcome summary, when firstmate records one, lands in inbox/outcomes/ (docs/voice-relay.md; docs/configuration.md "Generic captain-message ingress")
...
3. For `check:`, act on the named poll result, including merges, contribution signals, Relay events, process-to-event source results, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate. When firstmate finishes processing a captain inbox note, it records a short outcome summary with `bin/fm-inbox.sh outcome <id> <short summary>` so the bridge can read progress back, then acknowledges the note with `drain --ack`.
