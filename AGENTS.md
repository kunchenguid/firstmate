...
Verified harness templates are wrapped in `bash -c` before being sent into the runtime endpoint, so fish/zsh/bare-bash pane shells all launch the same command; raw launch commands are passed through unchanged for adapter verification.
...
It also normalizes an inherited ignored SIGHUP before installing cleanup traps, so harnesses that start background commands with HUP ignored still get child cleanup when the arm task is hung up.
