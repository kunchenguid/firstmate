...
config/herdr-workspace-label  optional label for the PRIMARY home's Herdr workspace; LOCAL, gitignored; absent = "firstmate", present = its first line verbatim including a blank label; primary-only and not inherited; see docs/configuration.md "Herdr workspace label"
