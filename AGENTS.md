...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override, then rerun the same script with `--record-dispatch` for the profile you actually dispatched; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged, and record nothing (contract: `docs/configuration.md` "Typed dispatch resolution").
