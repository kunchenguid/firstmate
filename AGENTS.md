...
config/unverified-secondmate-harness  optional presence flag recording the captain's acceptance, for THIS home only, of a secondmate on an adapter not verified for that kind (muse, gemini, agy; never rovo); LOCAL, gitignored; NOT inherited into secondmate homes; see docs/configuration.md "Unverified secondmate harness"
...
The single exception is a secondmate adapter this home accepted in `config/unverified-secondmate-harness`, which launches unverified and warns; `bin/fm-spawn.sh` owns which adapters that file widens.
