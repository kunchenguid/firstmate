...
config/browser-cdp-url optional loopback-only Chromium DevTools Protocol URL for browser automation; LOCAL, gitignored, and inherited by secondmate homes; `bin/fm-brief.sh` injects its verified connection rule into new ship/scout briefs (docs/configuration.md "Browser CDP endpoint")
