...
state/               runtime records and signals; gitignored
  ...
  primary-resource/  private main-session resource-protection state (binding, alerts, proposals, receipts, outcomes); written only by bin/fm-primary-resource.sh; its standing check shim is state/primary-resource.check.sh
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and every bootstrap MUTATING sweep (inventory owned by `bin/fm-bootstrap.sh`'s header) run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, `FMX:`, or `PRIMARY_RESOURCE:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
...
- `primary-resource-handover` - load on a `primary-resource` `check:` wake (context or quota handover, or an alert-only line) for the main session's automatic resource protection.
