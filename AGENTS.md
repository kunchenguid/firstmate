...
Ship shared tracked changes through an ordinary feature-branch PR without requiring no-mistakes, with the same merge authority as any other project.
...
The verified harnesses are `claude`, `codex`, `copilot`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
