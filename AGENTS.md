...
`bin/fm-harness-adapter.sh` and `bin/harnesses/<harness>.sh` own per-harness runtime facts, including which harness names are launchable versus supported for the primary session.
