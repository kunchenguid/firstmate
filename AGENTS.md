...
- `image-review` - load before building or opening an image review session with `bin/fm-image-review.sh`, when the captain asks to review, annotate, or select among a directory of images with Lavish, or when reading returned image feedback back into image paths.
