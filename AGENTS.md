...
config/agy-turnend-hook  captain's one-time consent for the global agy turn-end hook write: "allow" permits it, "deny" refuses it and also takes back the key and hook files an earlier "allow" wrote (at the next install and at session-start bootstrap, the latter skipped in a local secondmate home whose primary owns the same store), absent means he has not been asked and is the only case that asks; LOCAL, gitignored; ONE consent per machine, recorded in the primary home, inherited into that machine's local homes, never carried to a remote home, which records its own; see docs/configuration.md "agy turn-end hook consent"
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `AGY_TURNEND_HOOK:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
