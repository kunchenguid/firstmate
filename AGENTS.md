...
config/pr-refresh  optional presence flag opting this home into the default-off branch-currency dispatch that keeps a waiting pull request current with its base automatically; LOCAL, gitignored, and not inherited; see docs/configuration.md "Branch-currency dispatch"
...
state/               runtime records and signals; gitignored
  ...
  <id>.pr-refresh-state  private dispatched/resolved receipt for branch-currency work on an open PR, paired with <id>.pr-refresh-refused, the one-refusal-per-head marker; bin/fm-watch.sh owns their lifecycle
...
In a home that opted in with `config/pr-refresh`, the merge poll also reactivates a finished GitHub PR worker when its head falls behind or conflicts with its base, while authoritative active-work or unreadable-state evidence prevents dispatch and a non-active refusal surfaces once per head for supervision; the worker re-enters its selected delivery path and no new ready report is valid until current-head checks pass.
