...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and an `artifact ready` line, which means the task waiting on that path can be steered to resume.
