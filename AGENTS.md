...
Specific tracked, `data/`, `state/`, `config/`, or `projects/` entries with their own authoritative owner, reached instead of one exhaustive tree anywhere: `config/crew-harness` and `config/secondmate-harness` in [`docs/architecture.md`](docs/architecture.md) and [`.agents/skills/harness-adapters/SKILL.md`](.agents/skills/harness-adapters/SKILL.md), and `data/secondmates.md` in [`.agents/skills/secondmate-provisioning/SKILL.md`](.agents/skills/secondmate-provisioning/SKILL.md); all other entries resolve through [operational home layout and state](docs/configuration.md#operational-home-layout-and-state) and the producing script headers named there.
To resolve a `state/<id>.meta` key, read the header of the script that writes it: `bin/fm-spawn.sh` owns the base task-metadata fields, `bin/fm-pr-check.sh`, `bin/fm-pr-merge.sh`, and `bin/fm-merge-local.sh` own the PR and local-delivery fields, `bin/fm-x-link.sh` owns the X-mode fields, `bin/fm-backend.sh` owns backend-specific fields, and `bin/fm-teardown.sh` owns `telemetry_terminal_payload_sha256=`; [`docs/configuration.md`](docs/configuration.md#operational-home-layout-and-state) states only the schema-level rule that `bin/fm-spawn.sh` owns the base fields it emits.
