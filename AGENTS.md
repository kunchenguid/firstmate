...
state/               runtime records and signals; gitignored
  ...
  archived-records/<id>.record .status  retained evidence from `fm-teardown.sh --record-only`, the explicitly selected archival operation for a completed ship or scout task whose pooled Treehouse worktree slot was already reused before ordinary teardown ran; bin/fm-teardown.sh's fm_teardown_record_only owns the contract and never touches the reused slot
