...
config/herdr-task-titles  optional presence flag opting this home into default-off human-readable Herdr task-tab labels ("<short title> (<id>)" instead of fm-<id>) for new workers; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Herdr task-title labels"
...
state/               runtime records and signals; gitignored
  ...
  <id>.herdr-task-labels  flat-path Herdr spawn's attempted task-tab labels, so a title edit cannot hide a live prior attempt from duplicate checks; compacted to the live label once the replacement is verified; removed by teardown
