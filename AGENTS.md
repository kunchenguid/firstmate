...
- `slack-browser` - load before reading or posting in Slack through the captain's own browser session, and before promising the captain a Slack action.
  Every message sent that way posts as the captain rather than as a bot, so the skill's consent gates apply even to a routine-looking reply.
