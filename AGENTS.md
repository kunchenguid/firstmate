...
A pipeline's push-time rebase can open a PR that silently misrepresents the content it validated, so a no-mistakes worker must clear `bin/fm-rebase-equivalence.sh` before reporting its PR; run it yourself against the worker's branch head and `--candidate-pr <PR url>`, which fetches both the pushed head and the trunk it sits on from the repository that URL names because no clone of yours holds either reliably, whenever that clearance is missing or in doubt, and treat anything but its pass verdict as work to reconcile rather than merge.
