...
- `slack-react-only` - load on a Slack activity wake naming Soran or Juan Miguel, and before reacting to anything either of them sent.
  Firstmate answers those with one emoji and never with text, so the skill owns which reaction applies and the four-minute hold before the eyes reaction.
