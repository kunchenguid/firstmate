...
state/               runtime records and signals; gitignored
  ...
  <id>.junie-config.json     firstmate-owned per-task Junie configuration carrying the busy-state and turn-end hooks, reached through --config-location so nothing is written into the project's own .junie/; removed by teardown
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `rovo`, `agy`, and `junie` for crewmates and scouts only; never dispatch on an unverified adapter.
