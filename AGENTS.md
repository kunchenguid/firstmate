...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and Jev's OPENROUTER_API_KEY (config/jev-mode separately selects the typed dispatch posture; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/jev-mode  Jev dispatch posture: absent or "off" = no request, "shadow" = decide and record without applying, "on" = allow clear results to drive dispatch; LOCAL, gitignored, and not inherited; see docs/configuration.md "Typed dispatch resolution"
...
config/scm-host  SCM prerequisite profile; LOCAL, gitignored; absent or "github" = require gh, gh-axi, and authenticated GitHub CLI, "forgejo" or "local-only" = no GitHub prerequisite; inherited by secondmate homes (docs/configuration.md "SCM host")
...
state/               runtime records and signals; gitignored
  ...
  <id>.meta          task metadata, including optional label= from fm-spawn --label; each producer script's header owns its exact fields and mutation contract, with docs/configuration.md routing operator-facing backend and trace-context details
...
Do not dispatch until the essential launch tools are present and the selected SCM host's prerequisites are good; presentation availability follows `bootstrap-diagnostics` and does not block nonvisual work.
...
When the captain invokes `/jev`, load the `/jev` skill; it owns status and the `off`, `shadow`, and `on` posture changes.
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, and only in `on` mode on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override; `shadow`, `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `SCM_HOST_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `JEV_MODE: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
