...
state/               runtime records and signals; gitignored
  ...
  .branch-mod-mode     Claude Code supervision-branch mod opt-in whose presence alone switches the mod and every bin/ piece it relies on; LOCAL, operator-written; docs/claude-supervision-branch.md
  .branch-mod-counters .branch-mod-passed branch-mod-events.jsonl branch-mod-classifications.jsonl  that mod's session counters keyed by the lock pid, its event log, and its durable classifier record: the queue rows passed to main and not yet acknowledged (`.branch-mod-passed`, shared with the Pi extension), and the one-record-per-call classification log read by bin/fm-branch-classifier-score.sh (`branch-mod-classifications.jsonl`, shared with the Pi extension); module-owned, never touch
  .<task>.classifier-offset  the status byte offset of the last evidence bundle the pre-branch classifier (either host) judged for the task; bin/fm-wake-evidence.sh owns it, teardown removes it
