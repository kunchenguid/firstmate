...
state/               runtime records and signals; gitignored
  ...
  .afk               durable quiet-mode flag, also the away/quiet daemon flag on harnesses that launch one; bin/fm-afk-launch.sh owns entry and exit, fm_afk_mode() in bin/fm-wake-lib.sh owns mode reading
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the away or quiet posture (see section 8); and one cheap alive/dead read of each task's recorded backend endpoint.
...
If `state/.afk` is present, load `/afk` in away mode or `/quiet` in quiet mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`); where its daemon runs, let the daemon own supervision rather than arming another cycle, and on Pi and omp keep the ordinary supervision session, which runs in both postures.
...
Each skill owns its entry and exit procedure; these safety facts remain inline for both:
...
- Pi, pi-signed, and omp keep extension-owned supervision in both postures; never launch an away daemon there.
