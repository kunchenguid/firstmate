...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `agy` (crewmate/scout only, not yet wired for secondmate launches); never dispatch on an unverified adapter.
