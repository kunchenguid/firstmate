...
config/herdr-workspace-label  optional per-home Herdr workspace label override, overriding the primary "firstmate" default; LOCAL, gitignored, not inherited; see docs/configuration.md
...
state/               runtime records and signals; gitignored
  ...
  <id>.omp-overlay.yml  firstmate-owned per-task omp posture overlay (the tracked .omp/fm-worker-overlay.yml with the task's own fail-closed retry block and dispatch-tier fallback chain) passed through --config; written by fm-spawn, regenerated on relaunch, removed by teardown
  ...
  .watch-arm-stderr.log  bounded copy of each watcher cycle's own stderr, where a failed or signaled cycle's reason line survives the arm that observed it (docs/watcher-continuity.md "Arm-layer cycle contract"); diagnostic only, safe to delete
