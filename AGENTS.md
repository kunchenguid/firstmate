...
state/               runtime records and signals; gitignored
  ...
  <id>.meta          task metadata, including optional `transport=acp` and its `session_id=`; each producer script's header owns its exact fields and mutation contract, with docs/configuration.md routing operator-facing backend and trace-context details
