...
state/               runtime records and signals; gitignored
  ...
  .session-start-seen-<id>  per-task fingerprint of the fleet-state block the last session start printed for that task (.meta content, last status line, status-log size, endpoint verdict, keyed by the status-tail bound); a match collapses that task to one compact line in the next digest's fleet-state section, written only by bin/fm-session-start.sh, removed by teardown, safe to delete (forces one full block)
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every CHANGED task's `state/<id>.meta` with a bounded tail of its `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead read of each task's recorded backend endpoint.
   A task whose `.meta`, status log, and endpoint verdict are all unchanged since the last session start collapses to ONE compact line instead - endpoint, last known verb, full status log path, its own `state/<id>.meta` path, and the exact marker to delete to force the full block back - and for such a compacted task, reading that named `.meta` and status log directly is explicitly allowed despite the read-once contract. A `--reemit` never compacts: it reprints every task in full.
