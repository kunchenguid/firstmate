...
state/               runtime records and signals; gitignored
  ...
  reconcile-requests/ private open obligations to re-check a captain call selected through a bound captured source; written only by bin/fm-captain-hold.sh, retired by its verify-then-decide outcomes or a normal answer that settles the call (section 13; docs/captain-hold-lifecycle.md)
...
Use `gh` for GitHub.
For browser work, use the harness browser tools or Playwright.
Use plain chat for structured decisions and reports.
...
For PR-based ship tasks, the ready signal is `done: PR <url>` after the worker opens the PR.
...
For the full `stuck-crewmate-recovery` trigger, including a live worker whose claim conflicts with an already-attributed legacy no-mistakes run, follow section 13.
...
`bin/fm-dod-lib.sh` owns the worker verification and delivery contract.
...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `stuck-crewmate-recovery` - load when the session-start digest reports an ordinary direct report's endpoint dead or its metadata has no window, after a stale wake, looping pane, repeated confusion, an answered-by-brief question, an unresponsive crewmate, or a failed steer, and when a live worker's claim conflicts with an already-attributed legacy no-mistakes run.
- `secondmate-provisioning` - load before creating, seeding, validating, launching, handing backlog to, recovering, pushing inherited local material into, or retiring a secondmate home, and before editing `data/secondmates.md`.
- `captain-hold-lifecycle` - load before treating an investigation or visual review as complete, before ending a visual review that exposed a captain decision, when completed ship work awaits the captain, when recording or routing the captain's answer, and on any `RECORD DIVERGENCE` line from the wake drain.
