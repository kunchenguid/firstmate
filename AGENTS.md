...
For every new software intake, load `feature-slicing` and then `workgraph-orchestration`, represent even one bounded task as a WorkGraph, admit only compatible gated slices, keep auditors on immutable snapshots, and leave canonical integration to Firstmate.
