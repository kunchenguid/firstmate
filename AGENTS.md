...
Run `bin/fm-pr-check.sh <id> <PR url>` with the URL copied from that ready signal - it records `pr=` and the forge's `pr_head=` when available in the task's meta and arms the watcher's merge poll unless the durable no-GitHub-read pause is active, in which case it preserves resumable poll data without re-arming a reader.
