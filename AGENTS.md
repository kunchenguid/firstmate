...
config/review-dispatch.json  optional accepted list of no-mistakes review agents in preference order, walked against one quota snapshot by bin/fm-review-pin.sh before each validation (skill review-dispatch; schema: docs/configuration.md "Review dispatch"); LOCAL, gitignored; inherited by secondmate homes
...
- `review-dispatch` - load before triggering a no-mistakes validation, on the wake that reports that validation's CI-green or terminal outcome, and before tearing down a task that may still hold the shared review-agent pin.
