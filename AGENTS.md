...
config/project-env/  per-project source of truth for the project's own LOCAL gitignored env files (.env.local and friends), one directory per project holding them at their repo-relative paths; LOCAL, gitignored, per-home and not inherited; fm-spawn applies it to every ship and scout worktree, and bin/fm-project-env.sh owns the commands and the never-committed proof; see docs/configuration.md "Local project env files"
