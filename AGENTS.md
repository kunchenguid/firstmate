...
Drive a worker's lifecycle through `bin/fm-control.sh <task-id> interrupt|exit|stop|relaunch`, which owns the per-runtime mechanics, verifies each action, and never tears down or discards anything ([`docs/agent-control.md`](docs/agent-control.md)).
