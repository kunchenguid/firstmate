...
It falls back to the default branch, saying why, once the branch under review is no longer rooted in that base - which is what the no-mistakes pipeline's rebase always leaves behind - so the review is never blocked.
A branch still rooted in its declared base keeps that base as the diff base even after the base has merged or its branch has been deleted from origin, because a squash merge leaves the base's own commits out of the default branch and diffing against it would dress them up as the crewmate's change.
The helper and the pre-merge guard read the same verdict from `bin/fm-base-lib.sh`, whose header owns what a declared base means, so review can never tell the captain a different story from the one the guard is about to enforce.
...
When the task declared a non-default intended base (section 11's `--base`, recorded as `base=` in meta), the check also guards the PR's base and refuses loudly before merge rather than letting a wrong-based PR through, so a based task that was never retargeted blocks here instead of landing the feature base's unmerged commits on the default branch.
Every refusal names the recovery that fits the state it found, and `bin/fm-pr-check.sh`'s header owns the full base-guard contract - what is verified, when the guard stands down, and what still fails closed - not restated here.
...
The brief scaffold uses it to root the crewmate's branch on that base and write the mode-specific path to a correctly based PR, and records no state; `bin/fm-spawn.sh` is the one owner of the durable record, writing `base=` plus the base's spawn-time tip as `base_sha=` into meta, which is what arms the PR-ready check that refuses a head rebased onto the repo default or a PR opened against it (section 7).
Because a base can merge or vanish while its crewmate works, the brief keys every base instruction on `bin/fm-base-state.sh`, which the crewmate runs as its first action and again before it targets a PR at the base; that script's header owns what each answer means.
A first spawn also refuses to launch a based task against a base whose work has already merged, and against one abandoned without merging, so a `--base` that names the wrong branch costs a command rather than a whole crewmate run.
A respawn of a task that already declares that base keeps the declaration when the base has merged since, because a base merging mid-flight is the normal end-state of a stacked PR; `bin/fm-spawn.sh`'s header owns those refusals, that carry-forward, and the recovery each one names.
A brief scaffolded with `--base` whose spawn omits it leaves the task unguarded, so `bin/fm-brief.sh` prints the matching spawn command; the flag mechanics live in each script's header.
