...
- `process-event-sources` - load before arming a long-polling source, when starting an external wait that owes a callback, and on any `procevent <adapter> <source-id> <sequence>` check wake.
