...
config/secondmate-harness.d/<id>  optional per-secondmate launch pin in that same "<harness> [<model>] [<effort>]" format, winning over config/secondmate-harness for that id only and re-resolved on every launch and recovery; LOCAL, gitignored, primary-only, NOT inherited; docs/configuration.md "Per-secondmate launch pins"
...
If static `config/crew-harness`, `config/secondmate-harness`, or a per-secondmate pin under `config/secondmate-harness.d/` names an unverified adapter, report it and fall back only to a verified adapter rather than launching it.
