...
config/project-local-material.json  optional per-project list of the untracked local material - environment files, local tool configuration, stored browser sessions - carried into every fresh ship or scout worktree so a worker can run a dev server, e2e tests, and screenshots; LOCAL, gitignored; inherited by secondmate homes; owner bin/fm-local-material.sh, schema docs/configuration.md "Project local material"
