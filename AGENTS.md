...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `grok`, and `agy`; never dispatch on an unverified adapter.
`agy` (Antigravity CLI) is verified for crewmate and scout spawns only: no turn-end hook contract exists, so `--secondmate` spawns with harness `agy` are refused by `fm-spawn`.
