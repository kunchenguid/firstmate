...
state/               runtime records and signals; gitignored
  ...
  <id>.meta          task metadata whose producer script header owns its exact fields and mutation contract; after a verified adapter launch is submitted, profile_delivery=fm-spawn.v1 with delivered_model= and delivered_effort= records its delivered profile, while local records retain requested model= and effort= and a remote parent records the remote endpoint's delivered axes, with docs/configuration.md routing operator-facing backend and trace-context details
