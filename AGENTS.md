...
- `decorrelated-review-lenses` - load only while carrying out an independently authorized review or audit that has multiple rounds or reviewer fan-out; it never adds a no-mistakes review gate.
- `merge-conflict-arbitration` - load only when the selected delivery path reaches an actual semantic merge or rebase conflict between parallel contributions; same-file overlap alone does not trigger it.
