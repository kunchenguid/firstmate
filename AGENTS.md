...
state/               volatile runtime signals; gitignored
  ...
  <id>.claude-settings.json  firstmate-owned claude crewmate turn-end Stop hook, written by fm-spawn OUTSIDE the worktree and loaded with --settings so it can never truncate a project's own tracked .claude/settings.local.json; removed by teardown. A manual crewmate relaunch (e.g. `claude --continue` during stuck-crewmate recovery) must repeat `--settings <this file>` or turn-end wakes are silently dropped for the task (.agents/skills/harness-adapters/SKILL.md)
