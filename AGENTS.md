...
state/               runtime records and signals; gitignored
  ...
  <id>.claude-settings-owned  firstmate-owned record of the exact claude hook entries fm-spawn merged into the task worktree's .claude/settings.local.json, the sole ownership authority for stripping, replacing, or restoring them (bin/fm-control-lib.sh); removed by teardown
