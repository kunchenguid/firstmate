...
1. **Never write to a project.**
   ...
   Seven sanctioned write exceptions are indexed here; their procedures live where they are used: tool-driven project initialization (section 6), fleet sync via `bin/fm-fleet-sync.sh` (sections 3, 7, and 8), local-HEAD secondmate sync via `bin/fm-bootstrap.sh` and `bin/fm-spawn.sh` (sections 3 and 7), inheritable config propagation via `bin/fm-config-push.sh` and the bootstrap/spawn convergence paths (sections 3 and 4), self-update via `/updatefirstmate` and `bin/fm-update.sh` (section 12), approved `local-only` merge via `bin/fm-merge-local.sh` (section 7), and the declared-base checks in `bin/fm-spawn.sh --base`, `bin/fm-pr-check.sh`, and `bin/fm-review-diff.sh`, which refresh the project clone's remote-tracking refs and fetch the PR head, and nothing else (sections 7 and 11).
   All are fast-forward operations, remote-tracking refreshes, guarded gitignored-config propagation, or guarded local merges that never force, stash, or discard unlanded work.
...
When the task meta records a non-default `base=` (section 11's `--base`), that base is still a live feature branch on origin, and the reviewed head is actually rooted in it, the helper diffs against that base instead of the repo default, so review shows the crewmate's own change rather than the whole feature base's unmerged history stacked underneath it.
Anything else - the base is gone from origin, origin cannot be asked, the default branch has already absorbed everything the base carries, or the head was rebased off the base onto the default branch - falls back to the default branch and says so in one line, so the review is never blocked and the substituted diff base is never silent; `bin/fm-review-diff.sh`'s header owns that contract.
...
When the task declared a non-default intended base (section 11's `--base`, recorded as `base=` in meta) and that base is still a live feature branch on origin, the check also guards the PR's base and refuses loudly before merge rather than letting a wrong-based PR through, so a based task that was never retargeted blocks here instead of landing the feature base's unmerged commits on the default branch.
A base the default branch has already absorbed is not a live feature base and has no unmerged history left to protect, so the check verifies that PR as the ordinary default-branch PR it now is instead of refusing it.
When the base cannot be verified at all - it is gone from origin, or origin cannot be asked - the check deliberately does not adjudicate it: it hands the PR back to you, saying what it could not determine and asking a human to confirm the PR's base before merging.
`bin/fm-pr-check.sh`'s header owns the full base-guard contract, including what it deliberately declines to decide, and is not restated here.
...
For a PR-based ship task whose intended base is a non-default branch (a stacked feature-branch fix), pass `--base <branch>` to BOTH `bin/fm-brief.sh` and `bin/fm-spawn.sh`.
The brief scaffold uses it to root the crewmate's branch on that base and write the mode-specific path to a correctly based PR, and records no state; `bin/fm-spawn.sh` is the one owner of the durable record, writing `base=` into meta, which is what arms the PR-ready check that refuses a head rebased onto the repo default or a PR opened against it (section 7).
A declared base is worth stacking on only while it is LIVE - still on origin, and still carrying at least one commit the default branch does not already have - and that one question is asked the same way everywhere the base comes up: at spawn, at the PR-ready check, at review, and by the crewmate itself, all through `bin/fm-base-lib.sh`, which owns it (`bin/fm-pr-check.sh`'s header owns what the guard then does).
So a first spawn refuses a base that is not live, whether it is missing from origin or the default branch has already absorbed it, and a mistyped or spent `--base` costs a command rather than a whole crewmate run; a respawn of a task that already declares that base keeps the declaration without re-checking, because a base merging mid-flight is the normal end-state of a stacked PR.
Passing the flag to only one of the two is caught from both sides: `bin/fm-brief.sh` prints the matching spawn command and has the crewmate stop when meta records no base, and `bin/fm-spawn.sh` refuses a `--base` whose brief was never written for that base, rather than launching a crewmate that would root on the default branch and have its PR refused after a whole run; the flag mechanics live in each script's header.
...
Either way the crewmate re-asks the liveness question before it points a PR at the base, and stops with `blocked:` for any answer but live, because a base most often stops being live exactly while its child is in flight - it never decides for itself what became of one.
