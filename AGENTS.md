...
config/agy-turnend-hook  captain's one-time consent for the global agy turn-end hook write: "allow" permits it, "deny" refuses it and also removes a key an earlier "allow" wrote, absent means he has not been asked and is the only case that asks; LOCAL, gitignored, inherited within one machine's local homes and never carried across machines; see docs/configuration.md "agy turn-end hook consent"
