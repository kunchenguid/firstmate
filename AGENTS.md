...
state/               runtime records and signals; gitignored
  ...
  <id>.git-hooks/    per-task git hooksPath that strips AI commit trailers at the commit object, used only on a git without config-defined hooks; written by fm-spawn, removed by teardown (bin/fm-git-strip-ai-trailers.sh)
