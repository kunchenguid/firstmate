...
Name the matched rule's route to `fm-spawn --route`, or `default` when no rule matched, so the task records the route and capability floor it was dispatched at; an undeclared route records as unresolved rather than being inferred, so a relaunch must name the same route again.
