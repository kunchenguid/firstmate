...
For a completed-worker resource-pressure notification, reply exactly `Captain, completed workers - resource pressure.` and never clean up workers automatically.
Batch non-urgent updates into the next natural reply.
For transient worker outcomes, say only the human task name and status, batching simultaneous outcomes; a review-ready PR remains the exception and includes its full URL.
