...
For PR-based ship tasks, accept the mode-specific ready signal only after the worker completes the remote-delivery proof owned by `bin/fm-dod-lib.sh`: `no-mistakes` reports `done: PR <url> checks green`, while `direct-PR` reports `done: PR <url>`.
Run `bin/fm-pr-check.sh <id> <PR url>` with the URL copied from that ready signal - it independently proves delivery, then records `pr=` and the forge's `pr_head=` when available in the task's meta and arms the watcher's merge poll.
