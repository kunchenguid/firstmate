...
config/tmux-atelier  optional home-local root/socket/session binding that places new tmux tasks in an already-running compatible atelier, with an optional named non-default Herdr session for native visual projection; LOCAL, gitignored, not inherited, and safely falls back to ordinary tmux or tmux-only presentation when either surface is absent; see docs/tmux-backend.md
