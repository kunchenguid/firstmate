...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then herdr, which is the fleet BASE backend - tmux is used only when herdr is not installed at all, and that substitution announces itself on stderr; when SET it is authoritative and fm-spawn.sh REFUSES a `--backend` naming anything else - if the configured backend is unusable, report that and stop rather than spawning the task into a different one, because a task running where the captain is not looking for it is invisible even when the machinery works; moving backends is the captain's call, via this file or FM_BACKEND; herdr is the fleet base backend (docs/herdr-backend.md), covered by six e2e suites against a real herdr plus smoke and unit suites; tmux is the verified REFERENCE backend (docs/tmux-backend.md) and the only one the unit-test fakes cover, so a spawn test whose expected outcome is SUCCESS must run on tmux or it will reach the real backend; zellij, orca, and cmux remain experimental spawn backends (docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit), and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
config/image-daily-usd-cap  DOLLARS per UTC day bin/fm-image-gen.sh may spend before refusing with exit 7; LOCAL, gitignored; absent = 5. Counted in money rather than images because an image count silently changes meaning when the model does (200 images is ~$13 on flash and ~$48 on pro at 4K); per-image prices used for the cap are deliberately conservative - the top of a model's range, and the dearest known rate for a model not in the table. Checked BEFORE the billable call and accumulated in state/image-gen-spend.tsv from images actually written, so a failed or refused call never consumes budget. This is the ONLY hard brake that exists: a Google Cloud billing budget merely sends an alert and the spend continues past it. An agent that hits the cap reports and stops; raising it is the captain's decision
config/image-model  image model for bin/fm-image-gen.sh; LOCAL, gitignored; absent = gemini-3.1-flash-image (Nano Banana 2). EVERY generation spends real money and there is no free tier for any image model, so the four differ in price by roughly 4x; the tool verifies the configured id against the API's live catalogue and FAILS LOUDLY on an unavailable model rather than substituting a cheaper one (the agy silent-downgrade defect). Credential is GEMINI_IMAGE_API_KEY in .env; see .agents/skills/image-generation/SKILL.md
config/sprint-poll.env  enables the scheduled sprint-board check (bin/fm-sprint-poll.sh); LOCAL, gitignored; ABSENT = the poll is a hard no-op and the watcher behaves exactly as before. FM_SPRINT_INTERVAL (default 3600), FM_SPRINT_HOURS (default 9-20), FM_SPRINT_DAYS (default 1-5). The signal means "time to look", never "a task exists": the board is behind MCP and unreadable outside an agent turn, so the PM discovers content itself. Every wake costs an agent turn, which is why the window exists
...
2. For `stale:`, inspect the recorded endpoint and load `stuck-crewmate-recovery` for a stopped, looping, confused, or unresponsive worker; a deep-inspection reason also requires current-state and validation-log inspection, and a worker with no obvious pane symptom is not exempt from action - judge whether that evidence shows real progress toward the brief, and steer or redirect it exactly as a wedged worker when it does not.
