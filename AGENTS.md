...
- `sos-dispatch-loop` - load on any `procevent when sos-* <sequence>` wake, before running `bin/fm-sos-intake.sh`, and when an ops-hq SOS ticket or a stack-monitor sos event needs dispatch.
