...
For PR-based ship tasks, the ready signal depends on mode: `no-mistakes` reports `done [run=<id>]: PR <url> checks green` after CI is green, carrying the validation run's own identity, while `direct-PR` reports `done: PR <url>` after opening the PR.
