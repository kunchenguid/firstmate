...
state/               runtime records and signals; gitignored
  ...
  <id>.herdr-task-labels  flat-path Herdr spawn's attempted task-tab labels, so a title edit cannot hide a live prior attempt from duplicate checks; compacted to the live label once the new tab is verified; removed by teardown
