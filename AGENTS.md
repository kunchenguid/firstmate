...
For ship and scout tasks, the script asserts the resolved worktree is a genuine isolated worktree OF THE TARGET PROJECT (same git common dir, never merely "some other git repo") distinct from the primary checkout, killing the fresh window and aborting the spawn otherwise to prevent the worktree tangle of section 8 and wrong-repo launches.
