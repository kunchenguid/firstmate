...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `prime-agent`, `rovo`, and `agy` for crewmates and scouts only; never dispatch on an unverified adapter.
