...
Run `bin/fm-pr-check.sh <id> <PR url>` - it records `pr=` and the forge's `pr_head=` when available in the task's live meta or released-task landing record and arms the watcher's merge poll.
