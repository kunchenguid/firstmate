...
state/               volatile runtime signals; gitignored
  ...
  <id>.check.sh      trusted slow poll; the watcher dispatches validated PR data and the byte-identified Relay shim through trusted repository scripts, runs registered custom and generated local validation-gate checks from hash-validated private snapshots, and rejects every other state check without execution
  <id>.check-trust   private content binding for registered custom or generated local validation-gate checks
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the six bootstrap MUTATING sweeps - registered validation-check repair within the non-executing legacy PR-check migration, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1.
...
Do not replace the generated validation-gate check for a no-mistakes ship before `bin/fm-pr-check.sh` records its PR: it owns the task's one check slot until the merge poll takes over ([delivery-mode contract](docs/architecture.md#delivery-modes-are-explicit-per-task)).
