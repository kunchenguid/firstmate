...
Resolve every ship task's concrete delivery mode and `yolo` delivery posture at intake.
...
Record the resulting mode, `yolo` delivery posture, and the one-line reason for any deviation in the backlog item note.
