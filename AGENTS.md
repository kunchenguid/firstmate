...
config/secondmate-dispatch.json  optional per-secondmate-ID dispatch profiles (ordered "use" plus "select", including "primary-available"; section 4); LOCAL, gitignored; PRIMARY-only, takes precedence over config/secondmate-harness for a matching ID, and NOT inherited into secondmate homes (docs/configuration.md)
