...
config/standing-skills  optional skill names, one per line, that every ship brief, scout brief, and secondmate charter tells the worker to load first with its harness's own skill command; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Standing skills"
