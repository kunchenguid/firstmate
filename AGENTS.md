...
state/               runtime records and signals; gitignored
  ...
  gitlab-issues.check.sh  generated GitLab issue intake poll shim and its .check-trust binding; present only after bin/fm-gitlab-issues.sh arm; its pending record .gitlab-issues-pending holds the reported issues' details for firstmate to read and clear through that script, every writer of it holding .gitlab-issues.lock
...
A `--issue` scaffold composes that subsection itself, so there replace only the `{TASK}` token, with this subtask's own work, and leave the generated issue context and scope sentence in place.
