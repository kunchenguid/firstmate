...
Load `operational-layout` before locating, inspecting, or changing private operational files or configuration fields whose ownership is unclear.
...
Match the trigger to the action being performed, not a shared word: terminal Codex work does not imply Codex Desktop coordination, drafting a PR does not imply project intake, and authoring a review finding does not mean deciding an existing ask-user finding.
...
- `operational-layout` - load before locating, inspecting, or changing private operational files or configuration fields whose ownership is unclear.
