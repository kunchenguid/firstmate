...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  cost-attribution.tsv  append-only task-to-AI-effort join; firstmate-private, best-effort appended by fm-teardown before volatile metadata is removed on every successful cleanup, by forced secondmate retirement for each discarded child, and by secondmate home removal, which lifts that home's whole record into this file - join rows into the v2 section, every other non-blank line verbatim into a preserved-legacy section naming the home - so retirement never erases what it recorded; hand-written rows are never rewritten, a failed append only warns, and bin/fm-teardown.sh's header owns the schema and append-safety contract
  .cost-attribution.lock  cost-attribution.tsv append serialization lock, with sibling .cost-attribution.lock.owner.* directories; created and released per append by fm-teardown, self-healing when stale, and never captain-edited
