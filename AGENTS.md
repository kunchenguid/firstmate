...
state/               runtime records and signals; gitignored
  ...
  .progress-<id> .progress.label-<id> .progress-tick.pid   display-only progress observation record (phase, per-phase seconds, last tick) advanced by the watcher tick alone (a fleet snapshot or bearings read never writes it), the Herdr label bookkeeping beside it (last applied suffix, pending retry, warned reason) written by the label refresh alone, both retired by teardown, plus the watcher's single-flight marker naming its last detached progress tick child (a dead, recycled, or overlong pid is reclaimed); all safe to delete; bin/fm-progress-lib.sh owns the record and the bookkeeping, bin/fm-watch.sh the marker
