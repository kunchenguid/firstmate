...
config/agy-turnend-hook  captain's one-time consent for the global agy turn-end hook write: "allow" permits it, "deny" refuses it and also removes a key an earlier "allow" wrote (at the next install and at session-start bootstrap), absent means he has not been asked and is the only case that asks; LOCAL, gitignored; ONE consent per machine, recorded in the primary home, inherited into that machine's local homes, never carried to a remote home, which records its own; see docs/configuration.md "agy turn-end hook consent"
