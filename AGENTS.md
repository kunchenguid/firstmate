...
state/               runtime records and signals; gitignored
  ...
  <id>.claude-settings.json  firstmate-owned per-task Claude settings carrying the launch policy plus the busy-state and turn-end hooks, reached through --settings so nothing is written into the project's own .claude/; removed by teardown
