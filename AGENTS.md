...
config/github-app-*  optional GitHub App worker identity used by ship and scout spawns instead of the captain's gh credentials; LOCAL, gitignored; NOT inherited by secondmate homes and never used for secondmate spawns; absent = inert (today's spawn behavior), present but unusable = refused ship and scout spawns; see docs/configuration.md "GitHub App worker identity"
