...
- `usage-limit-recovery` - load on any `USAGE_WALL:` line, whenever several workers on one provider stop within minutes of each other, whenever a stranded worker's cause is not positively explained, and before dispatching when the headroom surface reads tight, wall, or unknown.
