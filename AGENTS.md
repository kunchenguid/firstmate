...
Use normal branches and pull requests for tracked Firstmate changes, and keep the existing explicit captain approval gates for sensitive actions.
...
config/backend  removed and ignored by the lean product contract; tmux is the only runtime backend
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  secondmates.md      local secondmate routing table; firstmate-private, maintained by the local secondmate seed helpers (section 6)
...
state/               runtime records and signals; gitignored
  ...
  <id>.check.sh      authenticated slow poll; the watcher dispatches validated PR data and registered custom checks from hash-validated private snapshots, and rejects every other state check without execution
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The mutating bootstrap sweeps run only when this session actually holds the lock from step 1.
   The secondmate liveness sweep accounts for registered local secondmates and relaunches only from the recovery-grade `dead` or `missing` states.
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, each clearly delimited, followed by the closing reminder.
   ...
   The closing reminder points back to the emitted supervision block and preserves the lock, afk, and read-once reminders.
...
Resolve every ship task's delivery path and approval posture at intake, and pass the explicit result to the brief, spawn, and any scout promotion.
A current explicit captain instruction wins, and a validation result does not itself authorize a sensitive action.
Record the delivery path and the reason for any deviation from the registered posture in the backlog note.
...
When a steer answers an open keyed decision or blocker, pass `fm-send`'s `--resolve-key` so the answer itself closes that decision record at answer time (contract: `bin/fm-send.sh` header).
...
A local secondmate's routed reply returns through status or a document pointer, not by firstmate peeking into its chat.
For the parent-owned correlation, recovery, and escalation contract on marked secondmate requests, see `bin/fm-pending-reply-lib.sh`.
Supervise all live work under section 8.

### Selected delivery path and approval authority

The selected delivery path owns its own validation and review steps.
A separate review or audit is allowed only when the captain explicitly requests that deliverable or the authorized task is knowledge-only.
Validation evidence informs the captain's decision but does not authorize a push, pull request, merge, local apply, or discard.
For a pull-request path, the worker waits for explicit captain direction before pushing or opening the pull request.
For a local-only path, the worker stops with a clean ready branch and waits for explicit captain direction before the guarded local application path.
...
For every ship, run the task's relevant validation after its implementation commit.
...
If a current explicit captain instruction invalidates work under validation, stop and obtain the required direction before changing the branch again.
...
Judge validation through the current task state, not shell liveness or a stale status event.
Passed validation is evidence for the captain's next decision, and failed validation remains a task blocker.

### PR ready, landing, and teardown

For PR-based ship tasks, report the full pull request URL and the validation evidence after the captain has explicitly authorized opening it.
...
3. For `check:`, act on the named poll result, including merges and process-to-event source results.
...
When a main-side pending captain decision is worth durable tracking, file it as its own work item; use `tasks-axi hold <id> --reason "<reason>" --kind captain` for a captain-gated thread.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line; silence and `BOOTSTRAP_INFO:` need no load.
