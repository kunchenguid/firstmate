...
The verified worker harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` for crewmates and scouts only; never dispatch on an unverified adapter.
Hermes is verified only as a primary harness, never as a worker, so a Hermes primary must select a concrete verified `config/crew-harness` instead of inheriting itself through `default`.
