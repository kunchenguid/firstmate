...
config/crew-config-dir  Claude config dir every claude agent this home launches (crewmate, scout, secondmate) authenticates with; LOCAL, gitignored; absent = bare `claude` on its default config dir, which strands the agent on the login wall when that dir is unauthenticated. NOT inherited into secondmate homes, so a secondmate's own crewmates need that home's own file (docs/configuration.md "Claude config dir")
