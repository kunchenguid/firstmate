...
A `.status`/`.turn-ended`/`.check.sh` file with no matching `state/<id>.meta` is an orphan from a torn-down task, or a leaked turn-end hook writer that outlived teardown's best-effort process kill, and is absorbed and removed before it ever reaches classification - never surfaced, even under afk; `state/x-watch.check.sh` is the one named exception, since that X-mode relay shim has no per-task `state/x-watch.meta` by design.
