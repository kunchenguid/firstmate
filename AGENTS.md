...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, kind=, mode=, yolo=; kind=secondmate also records home= and projects=; mode=codespace also records codespace=, remote_worktree=, remote_state= (fm-pr-check appends pr=)
  <id>.check.sh      optional slow poll per task (e.g. merged-PR check); you write it by hand, or fm-spawn auto-generates one for a mode=codespace task that SSHes in to read the remote status file
  <id>.check.last    last line a check.sh emitted, recorded so it wakes firstmate only on change; removed at teardown
...
Command name varies by image: the Cursor CLI is `cursor-agent` on personal installs but plain `agent` in company-managed Codespaces (`$HOME/.local/bin/agent`); the `codespace` delivery mode resolves whichever exists (section 6), while a local cursor spawn uses `cursor-agent`.
...
Secondmate project lists may include `no-mistakes` and `direct-PR` projects only; `local-only` and `codespace` projects stay with the main firstmate.
...
- `codespace` - crewmate SSH-es into the project's GitHub Codespace via `gh codespace ssh`, leases a treehouse worktree inside it, and runs there; no local clone at all. The registry line carries the `owner/repo` and an optional harness (`[codespace <owner/repo> [<harness>]]`); spawn discovers the single Available Codespace, ensures prerequisites in it (creates the remote state dir and makes `treehouse` runnable - company-managed Codespaces run an older base image and often cannot run personal dotfiles, so setup is not assumed; if `treehouse` cannot be made runnable, spawn fails with the one-time command to run), leases a worktree (recording its remote path for teardown safety), copies the brief in, and launches the crewmate over SSH. The codespace crewmate runs the harness named in the bracket (default `claude`), independent of `config/crew-harness`; that agent must already be installed in the Codespace. Scout and ship both work; the scout's report is copied back over SSH at teardown.
  Company-managed Codespace specifics (all handled by `fm-spawn.sh`; verified live against Debian 11 / glibc 2.31, repo `transcend-io/main`, harness `cursor`):
  - **Runnable-treehouse gate, not a PATH check.** A `gh codespace ssh -- <cmd>` shell is non-login/non-interactive. The prebuilt `treehouse` install.sh can drop a binary that is on PATH yet crashes at runtime (`GLIBC_2.34 not found` on glibc 2.31), so spawn gates on `treehouse --version` exiting 0, and when the prebuilt does not run, falls back to a source build (`GOBIN=$HOME/.local/bin go install github.com/kunchenguid/treehouse@latest`, Go at `/usr/local/go/bin`) that links against the Codespace's own glibc and still ships `--lease`.
  - **Harness command + PATH resolution.** Spawn sources the login profiles (`.profile`/`.bashrc`/`.zprofile`/`.zshrc`) and force-adds `$HOME/.local/bin` and `/usr/local/go/bin` to PATH on every remote command, because the non-interactive SSH PATH omits them. The Cursor CLI is installed as `cursor-agent` on personal images but as plain `agent` (`$HOME/.local/bin/agent`) in company Codespaces, so the cursor launch resolves whichever exists at launch time; other harnesses are unaffected.
  - **Git credential injection.** The non-interactive SSH session has no git credentials, so `treehouse get`'s `git fetch origin` fails. When `/workspaces/.codespaces/shared/.env-secrets` exists, spawn decodes its `GITHUB_TOKEN` (`base64 -d`) and exports `GITHUB_TOKEN` + `GITHUB_SERVER_URL=https://github.com` for BOTH the lease and the harness launch (so ship tasks can commit/push). Guarded on the file's presence so personal Codespaces with working auth are untouched; token values are never printed.
  - **Status mirror for perch.** The generated `state/<id>.check.sh` mirrors a new remote status line into the local `state/<id>.status` (which perch reads), waking firstmate via the watcher's normal status-hash signal scan. It dedupes via `state/<id>.check.last` (append only on a changed line) and prints nothing to stdout, so there is no `check:`-plus-`signal:` double-wake and no wake loop.
...
- **Ship** (the default): the deliverable is a change to the project. It ships through the project's delivery mode: `no-mistakes`, `direct-PR`, `local-only`, or `codespace`.
...
- **codespace** - all work happens inside the Codespace; there is no local worktree. The crewmate stops at `done: {summary}`; trigger validation by sending `/no-mistakes` (the crewmate runs the pipeline inside the Codespace), then PR ready as usual (`fm-pr-check`, relay the PR). Teardown SSHes into the Codespace to refuse any uncommitted or unpushed work, then releases the durable worktree lease with `treehouse return`; a failed return stops with state intact rather than leaking the lease.
...
For a ship task the definition of done is shaped by the project's delivery mode (section 6): `no-mistakes` ends in the harness-appropriate no-mistakes validation pipeline, `direct-PR` has the crewmate push and open the PR itself, `local-only` has it stop at "ready in branch" for firstmate to review and merge locally, and `codespace` has the crewmate work inside the Codespace, report `done`, then run `/no-mistakes` to validate and ship a PR (its status and report paths point at the remote state directory firstmate polls over SSH).
The scaffold reads the mode via `fm-project-mode.sh`, so you do not pass it.
Ship briefs also include the project-memory contract: run `bin/fm-ensure-agents-md.sh` when the project already has agent-memory files or when the task produced durable project-intrinsic knowledge, then record proportionate learnings in `AGENTS.md`.
For scout tasks add `--scout`: the scaffold swaps the definition of done for the report contract (findings to `data/<id>/report.md`, no branch, no push, no PR) and declares the worktree scratch; scout is mode-agnostic except for `codespace`, where the report is written inside the Codespace (to the remote state directory) and copied back to `data/<id>/report.md` at teardown.
