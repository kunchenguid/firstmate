...
config/fleet-snapshot-cadence  optional cadence, in seconds, at which this home republishes its own fleet snapshot to state/fleet-snapshot.json for a surface that watches the file; LOCAL, gitignored; absent means disabled, and it is not inherited by secondmate homes; see docs/configuration.md "Fleet snapshot cadence"
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - same-home backlog reconciliation, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, Relay artifact writes, and snapshot-publisher arming - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
