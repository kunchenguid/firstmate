...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and a `secondmate-stalled` result, which reports a secondmate whose own last event still declares work nothing resumed while its endpoint idled, so reconcile that mate and push the declared work forward rather than treating the quiet endpoint as healthy.
