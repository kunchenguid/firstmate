...
Running, fixing, or CI states remain working; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed is done only when the run's `pr` and `ci` steps actually ran, because a pass with either step skipped lacks that step's delivery evidence and reports failed; failed or cancelled is failed.
