...
Use `gh-axi` for GitHub, `chrome-devtools-axi` for browser work, and `bin/fm-lavish.sh` for every Lavish live-session operation; consult current help rather than memorizing flags.
...
Use plain chat for a yes-or-no decision and `bin/fm-lavish.sh` only when several options or a structured report benefit from a visual surface.
