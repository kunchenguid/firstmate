...
A model the captain marks reserved in `data/captain.md` requires explicit approval for that task; neither task difficulty nor a configured rule or default grants it.
...
When every candidate is tight, preserve the strongest reasoning class authorized for this task rather than silently downgrading it solely to conserve quota; stop and report the tight choice if that class cannot proceed.
