...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Each secondmate home also carries a local `bin/fm-self-heal.sh` watchdog that reanimates its own dead session without depending on the primary's watcher being alive (Option B, approved 2026-07-28); it uses the same liveness state and `fm-spawn.sh --secondmate` parameters, does not create tasks or talk to the captain, and leaves lost-response detection with the primary.
