...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed dispatch resolution key TYPESAFE_API_KEY (after the process environment and this file, the resolver falls back to macOS Keychain service typesafe-api-key; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
