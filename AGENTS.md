...
state/               runtime records and signals; gitignored
  ...
  .dsh-sessionstart-delivered   DeepSeek Harness once-per-session gate holding the session id whose digest was delivered; written only after a digest was produced, so a refused or empty startup retries, and rewritten by the next session
  .turnend-dsh-blocks .turnend-dsh-blocks.lock   DeepSeek Harness guard budget (session id plus the episode's consumed continuations) and its lock; discarded once the episode ages past FM_DSH_TURNEND_BUDGET_WINDOW, so one lapse cannot leave a session permanently blocked
  .dsh-turnend-fail-open   DeepSeek Harness terminal-alarm latch: the guard raises its one model-visible alarm turn when this is absent, then allows every later stop; the session-start digest prepends it when present, and the guard's healthy reset clears it so a recovered home can alarm again
