...
It falls back to the default branch, saying why, once the branch under review is no longer rooted in that base - the base is gone from origin, or the branch was rebased onto the default branch, which the no-mistakes pipeline always does - so the review is never blocked.
A branch still rooted in its declared base keeps that base as the diff base even after the base has merged, because a squash merge leaves the base's own commits out of the default branch and diffing against it would dress them up as the crewmate's change.
