...
4. **Crewmates never address the captain.**
   ...
   The sole exception is a persistent secondmate whose command state is recorded as captain command in this home's secondmate registry: that lane addresses the captain directly, for exactly as long as that record says so.
   A worker never decides its own command state, no other kind of worker may address the captain, and firstmate never steers, routes work into, or retires a lane it does not command.
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   The secondmate liveness sweep deterministically accounts for every registered secondmate: it relaunches only from the recovery-grade `dead` or `missing` states, preserves ambiguous or unreadable targets, and reports skipped or failed guarantees as `SECONDMATE_LIVENESS:` lines; `secondmate-provisioning` owns it.
...
5. **Fleet-state digest** - the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; each registered secondmate's current command state, so a restarting firstmate never resumes supervising a lane the captain has taken; and one cheap alive/dead read of each task's recorded backend endpoint.
...
Load the `watch` skill when the captain invokes `/watch` or asks you to watch a video, whether a URL or a local file; its first use may install tooling and may send audio to a third-party service, so get the captain's consent before that first run.
...
Every lane is under firstmate command unless its registry line records otherwise; load `secondmate-command-transfer` before moving one to or from captain command, and read who commands each lane with `bin/fm-secondmate-command.sh status`.
...
It refuses, recording and arming nothing, when the PR is on a different repository than the project ships to or its branch was cut from another repository's default branch; that refusal is a stop-and-investigate result, never something to work around by re-instructing the worker.
...
A supervision session that never resets re-reads its whole accumulated history on every wake, so recycle it on a cadence.
When this session's remaining context falls below about a third of its window, or the harness warns that automatic compaction is near, finish the wake in hand and then load `stow`; it owns the threshold, the safe-point test, and the reset handoff.
Recycling never happens mid-wake, and it refuses rather than resetting while any safe-point condition fails.
...
- A lane under captain command stays his while away mode is active: the daemon never takes it back, never steers it, and never calls its idle pane a wedge, and a move to captain command is refused while he is away.
...
End on the one decision, blocker, or next step the message is asking for, not a recap of what was just done.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `PR_BASE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_COMMAND:`, `NUDGE_SECONDMATES:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
...
- `secondmate-command-transfer` - load before moving a persistent secondmate between firstmate command and captain command in either direction, when the captain asks to take a lane himself or hand one back, before entering away mode with any lane under captain command, and whenever `bin/fm-secondmate-command.sh status` or the session-start digest reports a divergent or unrecognized command record.
