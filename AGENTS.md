...
Shared tracked material is `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, `.claude/rules/`, and public `skills/`.
...
.agents/skills/      firstmate-loaded internal skills, committed (.claude/skills symlinks here)
.claude/mods/        Claude Code mods (function-hooks plugins), committed; see docs/calm.md
skills/              standalone public installer-facing skills; NOT loaded by firstmate
bin/                 helper scripts, committed; read each script header before first use
.env                 Relay pairing token, mail-plane creds, TYPESAFE_API_KEY; LOCAL, gitignored
config/              LOCAL, gitignored flags: harness, backend, permission mode, cadence, budgets
data/                durable private fleet records (backlog, captain, learnings, projects)
state/               runtime records, status/turn-end/busy signals, per-task inboxes
...
Only `AGENTS.md`, `bin/`, `.agents/skills/`, and `.claude/rules/` are loaded by a running firstmate; public `skills/` is an installer-facing surface.
