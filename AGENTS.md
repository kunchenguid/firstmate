...
If a ship task will capture before/after PR evidence images, point the crewmate at `bin/fm-evidence-check.sh --help`, which owns the naming convention (leading-ordinal or matching-suffix pairing, `.allow-identical` opt-out) now enforced as a merge-blocking check.
