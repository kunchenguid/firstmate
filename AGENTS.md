...
config/fork-url  optional complete push URL used verbatim to initialize the no-mistakes gate (https/http/ssh/git+ssh/file or user@host:path); LOCAL, gitignored; inherited by secondmate homes; invalid values fail closed; see docs/configuration.md "Push target for the no-mistakes gate"
