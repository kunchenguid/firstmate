...
Ship shared tracked changes through this repo's PR path, with the same merge authority as any other project.
...
The worker owns its own rigor through the superpowers workflow: it implements, tests, reviews, and verifies its work before opening the PR, using the installed superpowers skills named in its brief - brainstorming before creative work, writing-plans with executing-plans (or subagent-driven-development) for multi-step tasks, test-driven-development for implementation, systematic-debugging for bugs, requesting-code-review and receiving-code-review for review rounds, and verification-before-completion before claiming done.
A separate review or audit is allowed only when the captain explicitly requests that deliverable or the authorized task is a knowledge-only review; one named question remains scoped to that question.
The path's worker and captain approval remain authoritative:
...
The worker validates its own work through the superpowers skills - tests, review rounds, and verification - before opening the PR.
...
CI on the PR is advisory evidence for the captain, not a firstmate-driven gate.

### PR ready, landing, and teardown

For PR-based ship tasks, the ready signal is `done: PR <url>` after opening the PR.
...
Do not expose internal terms such as startup machinery, locks, watchers, polling, crewmates, task ids, briefs, worktrees, checkouts, status or metadata files, teardown, promotion, harness names, runtime backend names, context budgets, delivery-mode names, autonomy flags, wake types, status prefixes, decision holds, or compressed safety labels such as fail-closed, fails closed, fail-open, fails open, fail loudly, or close variants.
...
- hold, ask-user, needs-decision, blocked, or paused -> the concrete decision, wait, approval, blocker, or external delay.
- done, failed, or cancelled -> the concrete result, failed check, or stopped work.
- brief -> instructions.
...
- Findings that `ask-user-authority` escalates.
