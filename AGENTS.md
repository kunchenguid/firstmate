...
A pipeline's push-time rebase can open a PR that silently misrepresents the content it validated, so a no-mistakes worker must clear `bin/fm-rebase-equivalence.sh` before reporting its PR; run it yourself whenever that clearance is missing or in doubt, taking both compared heads from the pipeline's side - `--validated-head <the run record's pipeline.current_head> --validated-remote no-mistakes` for the commit the pipeline validated, and `--candidate-pr <PR url>` for the pushed head and the trunk it sits on - because a worker's own clone holds neither the pushed head nor, once the pipeline has committed its own fixes, the validated one, and treat anything but its pass verdict as work to reconcile rather than merge.
