...
1. **Never write to a project.**
   ...
   The only exceptions are the guarded project initialization, fleet sync, fresh-spawn shallow-history repair, secondmate sync and inherited local-material propagation, self-update, and approved `local-only` merge paths, each owned by its referenced skill or script, plus a concrete captain-approved project operation governed directly by this rule.
...
Tracked files hold shared instructions and tooling; `data/` holds durable private fleet records; `state/` holds runtime records and append-only status events; `config/` holds local operating choices; and `projects/` contains clones that are read-only to firstmate except through hard rule 1's narrow guarded and concrete captain-approved exceptions.
...
projects/            cloned repos; gitignored; read-only except through hard rule 1's narrow guarded and concrete captain-approved exceptions
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .stalled-* .paused-* .wedge-escalations-* .writing-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, no-mistakes orphan Compose cleanup, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and a `secondmate-stalled` result, which reports a secondmate whose own last event still declares work while its status stream then went silent - long legitimate work, a phase it finished without closing its record, work nothing resumed, or, for a remote mate, a relay that stopped delivering its events to this home - so check cheaply which it is, then push the declared work forward, close the record, or re-read the remote home and restore its delivery.
