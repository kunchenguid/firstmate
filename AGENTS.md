...
config/claude-setting-sources  optional comma-separated Claude settings scopes (user, project, local; must include local) that Claude ship and scout launches pass as --setting-sources; absent = no flag; secondmate launches ignore it; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Claude setting sources"
