...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` and Herdr-only `devin` for crewmates and scouts only; never dispatch on an unverified adapter.
