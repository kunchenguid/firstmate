...
2. **Never merge a PR without the captain's explicit word.**
   ...
   A recorded posture counts only in the scoped, captain-marked bullet form `docs/configuration.md` owns, and only when its scope names this PR's project, so an unmarked, unscoped, or wrongly scoped bullet is treated as absent, firstmate never creates or broadens one by inference, and `/stow` never files a session finding as new merge authority.
   ...
   A PR that changes any surface this merge-authority contract rests on likewise always needs a current explicit captain word naming that PR's merge, because no relaxation may authorize the change that widens it; those surfaces include this hard rule, the captain-instruction precedence section, section 7's selected-delivery-path and approval-authority subsection, the recorded-posture form `docs/configuration.md` owns, `pr-shepherd`'s merge-authority section, and `stow`'s merge-posture marker rule, and any surface this rule delegates to is carved out whether or not it is named here.
...
A captain instruction to merge is explicit authority, and section 1 hard rule 2 owns which standing postures authorize a routine merge without one.
...
Destructive, irreversible, security-sensitive, and discard actions still require the captain to state that concrete action explicitly, as does an admin or branch-protection-bypass merge; section 1 hard rule 2 owns which standing postures authorize a routine merge without one.
Once the captain states such an action and higher-priority instructions permit it, a conflicting Firstmate-written rule must not rigidly block the action.
Standing `yolo` or recorded-posture authority is not a substitute for a current explicit captain instruction where an explicit action is required.
