...
state/               runtime records and signals; gitignored
  ...
  procevent/         registered process-to-event sources, one private record per canonical source id; the .source registrations are written only by bin/fm-procevent.sh, and their presence alone keeps supervision required; the adapter-owned <id>.receipts acknowledgement record also lives here, outlives its source's automatic terminal retirement, and never keeps supervision required by itself (section 13; docs/configuration.md)
  procevent-inbox/   private captured results, their durable handled-acknowledgement markers, and the runner's per-generation markers recording that the adapter-owned receipt seam already had its one chance at a capture; source output lives here and never in an event line
