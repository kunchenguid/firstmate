...
config/claude-config-dir  optional one-line absolute path to the Claude config store (seat) every Claude worker launch and its trust registration use; absent falls back to firstmate's own CLAUDE_CONFIG_DIR, then the single-store default; LOCAL, gitignored; NOT inherited into secondmate homes, because an absolute store path is meaningful only on the machine that holds it, so each home names its own; see docs/configuration.md "Claude config store"
