...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; WezTerm can be selected explicitly through local `config/backend=wezterm` (docs/wezterm-backend.md), tmux is the verified reference backend (docs/tmux-backend.md), and herdr, zellij, orca, and cmux are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - herdr and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit), and codex-app is not accepted; see docs/codex-app-backend.md; not inherited into secondmate homes
...
For the WezTerm backend, project/home groups are WezTerm tabs and crewmates are panes; task metadata records `window=wezterm:<pane-id>`. For the tmux backend, the task window is always named `fm-<id>`; per-backend window/tab naming and workspace scoping live in `docs/configuration.md` ("Runtime backend") and each backend's own doc.
...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>` for tools whose install command is an actual shell command.
  `wezterm` is manual-install only: the printed line points to WezTerm's installer or `FM_WEZTERM_BIN`, and `bin/fm-bootstrap.sh install wezterm` refuses instead of trying to eval prose.
...
All truth lives in each task's backend live-task inventory (tmux by default or when legacy metadata omits `backend=`, WezTerm when explicitly selected, herdr or cmux when explicitly selected or auto-detected, and zellij/orca when explicitly selected), state files, data/backlog.md, data/captain.md, data/learnings.md, data/secondmates.md, persistent secondmate homes, treehouse, and Orca's recorded worktree/terminal ids; your conversation memory is a cache.
...
bin/fm-spawn.sh <id> projects/<repo> --backend wezterm # explicit runtime backend; can also be selected via local config/backend (docs/wezterm-backend.md)
...
If the primary selected the spawn backend through ambient `FM_BACKEND`, the secondmate agent launch clears that environment variable before setting `FM_HOME`, so the parent records the selected endpoint backend in primary metadata without inheriting `config/backend` or `FM_BACKEND` into the child firstmate home.

For ship and scout tasks, wezterm/tmux/herdr/zellij/cmux create a runtime endpoint and run `treehouse get`; Orca creates an Orca-owned worktree, validates it, then creates the terminal. In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
Each task's backend live-task inventory is the ground truth (tmux when legacy `backend=` is absent; new WezTerm tasks record `backend=wezterm`; a task's meta may record a different `backend=` - tmux, herdr, zellij, orca, and cmux are also implemented spawn backends today, docs/wezterm-backend.md, docs/tmux-backend.md, docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, and docs/cmux-backend.md; codex-app is not selectable, see docs/codex-app-backend.md).
