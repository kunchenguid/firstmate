...
Run `bin/fm-session-start.sh 1` (or `bin/fm-session-start-1.sh`), followed by `bin/fm-session-start.sh 2` (`bin/fm-session-start-2.sh`), followed by `bin/fm-session-start.sh 3` (`bin/fm-session-start-3.sh`), followed by `bin/fm-session-start.sh 4` (`bin/fm-session-start-4.sh`), followed by `bin/fm-session-start.sh 5` (`bin/fm-session-start-5.sh`) at session start across sequential turns.
...
Run-tier harness surfaces run Stage 1 for you at session open while the rest only nudge it, so confirm Stage 1 is present in this session and proceed deterministically across Stage 2, Stage 3, Stage 4, and Stage 5; `docs/sessionstart-nudge.md` owns adapter tiers, source routing, and compatibility.
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` for crewmates and scouts only, and `agy` (Antigravity CLI) for crewmates, scouts, and interactive primary sessions; never dispatch on an unverified adapter.
...
Every scaffold permanently embeds the crewmate optmem ban: workers never run `~/.optmem/memo` (wake/nap/note) because AGENTS.md exempts crewmates and any memo call wastes budget.
...
- `lv1-access` - load before running any command on, pulling files from, pushing files to, or messaging the operator side of the LV1 Windows PC (AT3 customer client).
