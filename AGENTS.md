...
The script resolves the harness (`fm-harness.sh crew` for crewmate/scout tasks only when `config/crew-dispatch.json` is absent, `fm-harness.sh secondmate` for `kind=secondmate`; section 4), resolves and validates the runtime backend (the selection order, runtime auto-detection with its innermost-first nesting, and the per-backend refusals - including `codex-app` as unknown - are owned by docs/configuration.md "Runtime backend"), owns the verified launch templates, resolves the project's delivery mode (`fm-project-mode.sh`) for ship/scout tasks, and records the task meta enumerated in section 2.
...
For `kind=secondmate`, the same script launches in the registered or explicit firstmate home instead of running `treehouse get` for a project, and uses the charter brief as the launch prompt.
...
Per-backend workspace scoping (herdr's per-home workspaces that keep secondmate tabs out of the primary space, zellij's single shared session) is owned by docs/configuration.md "Runtime backend".
