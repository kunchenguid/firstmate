...
Never merge a red PR under either setting unless a current explicit captain instruction names the single GitHub check, or the no-checks sentinel, waived through `fm-pr-merge.sh --allow-red`; that attended-only waiver still requires every other check green, and a PR carrying no checks at all is refused by that same green condition rather than counted green.
