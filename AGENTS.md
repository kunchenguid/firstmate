...
config/pavel-ops.json  optional Pavel delegated-business automation and Telegram transport config; LOCAL, gitignored, not inherited, and owned by docs/configuration.md "Pavel operations loop"
...
state/               runtime records and signals; gitignored
  ...
  pavel-ops/         private immutable Telegram events, lifecycle transitions, audit log, and outbound receipts for the opt-in Pavel loop; bin/fm-pavel-ops.sh owns the format
...
- `pavel-ops` - load on every `pavel-ops-...` check wake, before classifying or answering a Pavel event, on every milestone for a Pavel-linked task, and during Pavel migration or recovery.
