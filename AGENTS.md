...
The records you act on are each task's `<id>.meta`, its append-only `<id>.status` event log, its registered `<id>.check.sh` slow poll, its durable `<id>.inbox/` steering records, the durable wake queue, the away-mode `.afk` flag, and the private per-task and fleet directories for pending replies, process-event sources and their captured results, decision bindings, condition-to-action watches, captain inbox notes, and Relay payloads and follow-ups.
...
Everything else under `state/` is watcher, wake-queue, turn-end guard, stop-hook, sub-supervisor, PR-poll, migration, lease, and presentation-cursor internals - every lock, cursor, counter, marker, and journal among them is owned by the script that writes it.
