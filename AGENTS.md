...
Use the hardened `gh` binary for every GitHub operation so Automic Vault can apply the captain's configured access level; use `chrome-devtools-axi` for browser work and `lavish-axi` for structured decisions or reports, and consult current help rather than memorizing flags.
