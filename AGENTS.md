...
config/task-tmpdir-pin  optional presence flag pinning each launched agent's TMPDIR to its task temp root so harness scratch is torn down with the task; default-off, LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Task TMPDIR pin"
