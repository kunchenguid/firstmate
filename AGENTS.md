...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/latent/       versioned latent-worker recovery manifest, incomplete transaction journal when present, and generated resume instructions; survives hibernation and is removed only by final teardown
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); kind=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available plus a matching GitHub-only pr_ready_head= durable hibernation trigger (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-latent replaces endpoint/worktree targets with empty values and records tier=/latent_generation=/latent_manifest=/latent_saved_head=/latent_local_head= only after a successful hibernation transaction; fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for a Relay-originated task (section 14)
...
2. **Bootstrap** - detect-only checks (tool/version problems, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the seven bootstrap MUTATING sweeps - non-executing legacy PR-check migration, latent transaction recovery, fleet sync, secondmate convergence, secondmate liveness, pending remote handoff retry, and Relay artifact writes - run only when this session actually holds the lock from step 1; the four network ones among them run in the deferred stage rather than in this section.
...
A finished eligible GitHub PR worker may then hibernate through `bin/fm-latent.sh enter <id>`: the script alone owns exact-head pinning, the manifest/ref transaction, endpoint termination, Treehouse return, crash recovery, and resume; every uncertain predicate leaves the task active, and neither a `paused:` tail nor watcher throttle state is eligibility evidence.
Hibernation is not teardown: task identity, status history, PR monitoring, backlog state, and `refs/firstmate/latent/<id>` survive, while only final merged cleanup through `bin/fm-teardown.sh` may end the task and remove that recovery ref.
...
When any wake reports a merged PR for a project cloned in this home, refresh that clone through the guarded fleet-sync path; for a valid latent task, run `bin/fm-latent.sh finish <id>` so normal teardown verifies the merged head contains the protected generation before final cleanup.
