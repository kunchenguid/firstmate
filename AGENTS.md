...
A declared base that no longer exists on origin is the normal end-state of a stacked PR, whose base merges and is then auto-deleted, taking the unmerged feature history the guard protects against with it: the check says so loudly, stands the guard down, and lets the now-ordinary PR proceed, because refusing there would deadlock a legitimate merge forever.
An origin that cannot be asked at all is different and still refuses, so an auth or network failure never passes for a verified base.
