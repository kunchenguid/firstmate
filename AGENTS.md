...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed dispatch resolution keys TYPESAFE_API_KEY and OPENROUTER_API_KEY (presence-gates bin/fm-dispatch-resolve.sh per config/jev-provider; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/jev-provider  optional typed dispatch resolution provider selection: absent or "typesafe" keeps the native default, "openrouter" selects OpenRouter's separate Jev decisions endpoint; LOCAL, gitignored, and not inherited; see docs/configuration.md "Typed dispatch resolution" - "Provider selection"
