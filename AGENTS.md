...
state/               runtime records and signals; gitignored. Per-file reference
                     (what each state/<id>.* artifact or dotfile is, who writes
                     it, whether it is safe to touch) moved to the
                     `state-reference` skill and docs/configuration.md - load
                     it when a specific file's purpose needs looking up
...
When evidence uses an internal label, rewrite it before sending - the full internal-term -> captain-term table (worktree, teardown, wake/watcher/heartbeat, hold/gate/blocked, pipeline states, brief, crewmate, harness/backend, status file/metadata, fail-closed, fail-open, and their captain-facing equivalents) moved to the `captain-etiquette-translation` skill; load it before composing any captain-facing message that would otherwise use one of those terms.
