...
- `paid-traffic-ops` (`.agents/skills/ads-analytics/`) - load before diagnosing paid-media performance, pacing, winners/drains, scale/pause/hold decisions, validating historical media-decision rules, or drafting choice-model action prompts for ads; client-specific constants stay in that skill's optional `references/` overlays.
