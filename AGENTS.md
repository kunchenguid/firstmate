...
state/               volatile runtime signals; gitignored
  ...
  <id>.pr-publication  private exact-body/head publication attestation required before PR readiness and ordinary merge monitoring
...
Load `pr-publication-check` before composing promotion or successor ship instructions and before accepting any PR-ready signal or calling a PR lifecycle command.
...
Running, fixing, or CI states remain working; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed means validation is done but does not by itself prove publication readiness; failed or cancelled is failed.
...
After CI first becomes green, the worker passes publication attestation and reports the PR without waiting for merge monitoring to finish.

### PR ready, landing, and teardown

For PR-based ship tasks, create the PR or MR as a draft and let publication attestation mark it ready through the forge; `no-mistakes` reports `done: PR <url> checks green` after CI and attestation pass, while `direct-PR` reports `done: PR <url>` after the same attestation.
Run `bin/fm-pr-check.sh <id> <PR url>` - it verifies a fresh complete public body/head against the worker's private publication receipt, records canonical `pr=` and `pr_head=`, and only then arms a merge poll that repeats publication verification before every forge-state lookup.
