...
config/claude-remote-control  optional "on" opt-in to Claude Remote Control on claude-harness workers; LOCAL, gitignored, and not inherited; absent, empty, or "off" leaves them local-only, "on" also reaches them from claude.ai/code and the Claude mobile app; see docs/configuration.md "Claude Remote Control"
