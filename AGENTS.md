...
config/self-update-policy  optional per-home `/updatefirstmate` policy; LOCAL, gitignored, not inherited; absent/default keeps guarded fast-forwards, while the explicit remote-authoritative value may replace only that opted-in home's tracked Firstmate code after fetching and verifying origin; see docs/configuration.md "Self-update policy"
...
Firstmate's shared instruction surface reaches running homes only after it lands on the default branch and those homes update from origin under their per-home self-update policy.
Only `AGENTS.md`, `bin/`, and `.agents/skills/` are loaded by a running firstmate; public `skills/` is an installer-facing surface.
When the captain invokes `/updatefirstmate` or asks to update firstmate, load the `/updatefirstmate` skill.
It applies each target home's independent policy, refreshes instructions, and never touches anything under `projects/`; absent/default remains guarded fast-forward-only, while `docs/configuration.md` owns the explicit remote-authoritative opt-in.
