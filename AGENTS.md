...
- `merge-conflict-arbitration` - load when two parallel lanes have touched the same file and a merge or rebase conflict between their branches needs reconciliation.
- `decorrelated-review-lenses` - load before commissioning a captain-authorized multi-round review and before fanning out parallel reviewers on the same artifact.
