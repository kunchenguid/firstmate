...
state/               runtime records and signals; gitignored
  ...
  <id>.executor-exit  exit status the pane shell records when a kind=executor task's one-shot command returns; the structural exit fact bin/fm-executor-lib.sh reads, cleared by relaunch and teardown
