...
config/crew-memory-bound  enforced memory ceiling for a newly spawned crewmate's whole pane process tree ("<MemoryMax> [<MemoryHigh>]"); LOCAL, gitignored; absent leaves spawning unbounded, present but unenforceable makes fm-spawn refuse rather than launch unbounded, and so does a raw launch command that is not one simple command (a pipeline, list, redirection, subshell, or leading shell keyword such as time/!/coproc); crewmates only, never the secondmate agent itself, but inherited by secondmate homes so their own crewmates are bound (docs/configuration.md "Crewmate memory bound"; bin/fm-crew-memory-bound.sh)
