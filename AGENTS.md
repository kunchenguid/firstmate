...
state/               runtime records and signals; gitignored
  ...
  .afk-launching     transient away-mode entry marker: bin/fm-afk-launch.sh writes it before state/.afk and clears it once a daemon owns supervision (or on failure/interrupt), and the arm gate yields while it is present so an in-progress launch is never raced (fm_afk_launch_in_progress in bin/fm-wake-lib.sh)
