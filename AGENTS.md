...
For ship and scout tasks, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout, aborting the spawn otherwise to prevent the worktree tangle of section 8; a refusal kills the window it already launched rather than leaving it stranded in the project clone. The wait for `treehouse get` to move the pane retries across bounded rounds (`FM_SPAWN_WT_WAIT_ROUNDS`, default 3), re-sending `treehouse get` each round, because a transiently exhausted treehouse pool has been observed growing new slots asynchronously within minutes - one round timing out is not proof the pool stays exhausted, but every round timing out is a hard refusal, never a silent launch into the project clone. What gets recorded as `worktree=` (in the summary line and `state/<id>.meta`) is always the isolation assert's own freshly re-resolved, canonicalized worktree toplevel, never a raw pane-path read that can be transient or differently-formed before the pane has actually settled (`docs/configuration.md` "Environment variables").
