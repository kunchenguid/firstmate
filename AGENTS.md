...
config/crew-config-dir  Claude config dir a spawned claude crewmate authenticates with; LOCAL, gitignored; absent = bare `claude` on its default config dir, which strands a crewmate on the login wall when that dir is unauthenticated. NOT inherited into secondmate homes, so a secondmate's own claude crewmates still launch on the default config dir; propagating it is a separate follow-up tracked outside this change (docs/configuration.md "Claude config dir")
