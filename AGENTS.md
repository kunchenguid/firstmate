...
Run `bin/fm-pr-check.sh <id> <PR url>` with the URL copied from that ready signal - it records `pr=` and the forge's `pr_head=` when available, arms the merge monitor, and invokes the executable workspace owner to release a clean remotely reconstructable task copy at the earliest safe point.
A release refusal is a safety result to reconcile, never a reason to discard local work; before later CI or review-fix continuation, run `bin/fm-workspace.sh restore <id>` and then use the ordinary control-plane relaunch.
[`docs/configuration.md`](docs/configuration.md) "Task workspace scope, retention, and reconstruction" owns the full scoped-root, remote-proof, bounded-retention, legacy-audit, and reconstruction contract.
