...
state/               volatile runtime signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .stale-candidate-* .paused-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch (.stale-candidate-* holds the deduplicating "<task>:<class>" stale verdict; .wedge-escalations-* is retired and only ever removed)
