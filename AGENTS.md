...
A teardown refusal for uncommitted work, unlanded work, or committed work whose durability could not be certified is a stop-and-investigate result, never an obstacle to bypass; the committed-work refusal preserves the worktree, is not bypassed by force, and clears only once its commits are recovered.
