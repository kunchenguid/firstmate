...
After filing or materially rescoping an open backlog task, assess its risk from blast radius, reversibility, uncertainty, and security, privacy, data, payment, production, compliance, or external-user impact, then record that explicit assessment through `bin/fm-task-risk.sh` before dispatch when the tasks-axi backend is selected, or preserve the script header's owned record contract during the configured manual edit.
