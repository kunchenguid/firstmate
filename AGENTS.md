...
Additionally, load `firstmate-orca` for Orca work, `process-event-sources` before arming or handling a registered long poll, `planning-room` before commissioning an adversarial planning gap review, `firstmate-codexapp` for visible Codex Desktop coordination, `war-room` before starting or supervising a reusable program room, and `bugfix-pair-room` before dispatching a bug the captain found in the flow or any bug brief without a proven root cause.
