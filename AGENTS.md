...
If one is already alive with a fresh liveness beacon, another invocation exits cleanly instead of creating a duplicate watcher; if the live holder is a WEDGED watcher (this home's watcher by identity, but its beacon has gone stale past grace because it stopped beating), a plain arm self-heals it - running the same home-scoped stop `--restart` does, then forking and confirming a fresh watcher in one invocation - instead of returning `FAILED`, so you no longer need a manual `--restart` for that case.
A startup-race peer that just took the lock and has not beaten yet (no beacon, or one still within grace) is not a wedge and is left alone to beat, and a live lock held by something that is NOT this home's watcher stays the honest `FAILED` case.
...
A plain arm already self-heals a wedged watcher, so `--restart` is reserved for a deliberate forced restart (such as a cadence transition); when one is genuinely needed, use `bin/fm-watch-arm.sh --restart`, which stops only this home's watcher (the pid recorded in this home's `state/.watch.lock`) and starts a fresh one.
