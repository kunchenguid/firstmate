...
inbox-from-warroom/  private downward inbox of operator-handed work-order drops (*.md other than README.md); LOCAL; presence alone means pending, listed read-only at session start (section 3, bin/fm-session-start.sh header)
...
4. **Downward inbox** - after the wake queue, the digest lists pending operator-handed drops; read each drop, convert accepted work into backlog items or holds, and archive the processed drop into `data/`.
5. **Supervision operating instructions** - after the work-input sections and before both digests, the digest emits exactly one operating block for the detected primary harness, followed by the read-once contract that governs them.
