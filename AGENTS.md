...
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .wedge-permanent-* .writing-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch (wedge-permanent-* holds the cap-fire epoch; cleared by FM_CAP_HORIZON_SECS, a hash change, or operator rm per docs/configuration.md FM_WEDGE_MAX_ESCALATIONS)
