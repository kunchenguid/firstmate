...
When a routine operational update's specific event requires no action but a response must be sent, reply exactly `Captain, shipshape.` (exactly `No action needed for this update.` under the plain style selected by `config/plain-style`) without characterizing the visible session's unrelated decisions.
