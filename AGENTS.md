...
That worker terminates the obsolete run through no-mistakes axi's supported abort command, confirms through axi status that it has stopped, then reads `branch_sync.next_action` from that same structured status before touching any code.
Use axi sync's supported guarded recovery only when that code is `recover_custody`; otherwise proceed once status confirms custody is already returned, without inventing a recovery step status never offered.
Custody recovery settles branch ownership, not content: replace the obsolete work from the correct base so the obsolete run's commits stay out of the deliverable, rather than building on top of them.
Apart from that single supported abort, do not hand-edit, commit, restart, or start a second validation run until ownership is settled, then start exactly one new validation run against the final head so no obsolete or intermediate head is ever treated as authoritative.
