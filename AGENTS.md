...
config/deploy-policy/<project>  path patterns the captain approves himself before they reach the live site; LOCAL, gitignored; its presence is what makes a project deploy-managed at all, and its absence is never read as permission; see docs/configuration.md "Deploy policy and target"
config/deploy-target/<project>  the machine that serves that project, data only and never a command; LOCAL, gitignored; same doc
...
config/concurrency-cap  bare integer bounding how many workers this home runs at once regardless of any project's own free worktree slots; LOCAL, gitignored; absent or malformed defaults to 13; read by the idle-capacity report (bin/fm-supervision-lib.sh); see docs/configuration.md "Concurrency cap"
config/lane-floor  bare integer, the fewest lanes this home keeps running while any work is dispatchable without a captain decision; LOCAL, gitignored; absent or malformed defaults to 10; inherited by secondmate homes; read by the lane-floor report (bin/fm-supervision-lib.sh), escalated on every wake drain, and enforced at a Claude turn end; see docs/configuration.md "Lane floor"
...
config/stale-escalate-secs  optional home-level override of FM_STALE_ESCALATE_SECS, the wedge-escalation and declared-wait recheck threshold; LOCAL, gitignored; see docs/configuration.md "Stale-escalate threshold"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  ops-facts.md       per-project operational facts - providers, hosts, IPs, URLs, user names, and where each secret lives, never a secret value; LOCAL, gitignored; carried into every deploy or operations brief; updated with inspect-then-update
...
state/               runtime records and signals; gitignored
  ...
  deploy-ledger/<project>.jsonl  durable append-only record of every deploy attempt, refusals included, with the commit it came from, the commit it went to, and the authority it acted under; bin/fm-deploy.sh owns the format and reads it for --rollback
  deploy-pending/<project>  durable record of a deploy refused only because the merged commit's own build had not finished, kept until the watcher's deferred re-check deploys it, reports it, or gives up on it; bin/fm-deploy-trigger.sh owns the format, the supersession rule, and the give-up horizon
  ...
  .turnend-lane-floor-blocks  consecutive lane-floor turn-end blocks for one session, bounding that enforcement separately from the supervision guard budget; owned by bin/fm-turnend-guard.sh, cleared as soon as the breach clears, safe to delete
  ...
  .idle-capacity-last-escalated  last (ready ids, per-project free counts) tuple the idle-capacity report escalated, so an unchanged idle fleet nags once, not once per poll; owned by bin/fm-supervision-lib.sh's fm_idle_capacity_should_escalate, safe to delete (forces one re-escalation)
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, `data/learnings.md`, and `data/ops-facts.md`, each clearly delimited, followed by the closing reminder.
...
- Operational facts about a deployment or account - registrar, host provider, IP, URL, login user, secret location - belong in `data/ops-facts.md`, never in a brief alone and never as a secret value; a deploy or operations worker receives that project's section in its brief and returns new facts through its report for firstmate to fold in.
...
For a deploy-managed project, landing is not the last step: the confirmed-merge path itself puts merged work live when nothing in it touches a captain-reserved surface, and reports it to the captain instead when something does.
Never deploy a captain-reserved range on standing `yolo` authority; `yolo` governs merging, and this gate is about what the captain sees on his own live site.
When the captain invokes `/deploy` or asks what is not live yet, load the `deploy` skill.
...
When any wake reports a merged PR for a project cloned in this home, refresh that clone through the guarded fleet-sync path and recompute the ready frontier.
...
Every hold carries a due date or a recheck event named in its reason, and the digest names a stale hold that has neither.
...
- `lane-floor` - load on any `LANE FLOOR` line, from a wake drain or from a blocked turn end, and dispatch until the fleet reaches its floor.
