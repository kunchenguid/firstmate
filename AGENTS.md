...
Verified harness templates are wrapped in `bash -c` before being sent into tmux, so fish/zsh/bare-bash pane shells all launch the same command; raw launch commands are passed through unchanged for adapter verification.
