...
A vacuous green is not a real green: on a PR with no CI checks configured, `bin/fm-pr-merge.sh` refuses an unattended yolo merge and you escalate to the captain, because nothing was validated (absent CI is a legitimate posture for repos that gate CI on the default branch only, not a broken repo).
An explicit captain merge is always allowed even on a no-CI PR - pass `bin/fm-pr-merge.sh <id> <full GitHub PR URL> --captain-approved` once the captain confirms.
`bin/fm-pr-merge.sh` always records `pr=` and records `pr_head=` when available before merging, writes the `merged_by_firstmate` attribution marker (section 8) before the merge call so an out-of-band merge stays distinguishable, parses the full `https://github.com/<owner>/<repo>/pull/<n>` URL into `gh-axi pr merge <n> --repo <owner>/<repo>`, and defaults to `--squash` unless an explicit merge method is forwarded after `--`; this holds even on a repo with no PR CI where the "checks green" signal that normally triggers `bin/fm-pr-check.sh` never fires - do not call `gh-axi pr merge` directly for a task's PR, or the recording step can be silently skipped and a later `fm-teardown.sh` has nothing to verify a squash merge against.
...
Firstmate's wrapper stays narrow: `ask-user` findings return through `needs-decision`, captain-owned decisions go back through `no-mistakes axi respond`, crewmate validation avoids `--yes`, and CI-green completion is reported with an honest CI posture - `done: PR {url} checks green (<N> checks)` when real CI ran and passed, or `done: PR {url} NO CI CHECKS CONFIGURED - not CI-validated` when the repo runs no PR CI (the crew brief owns the exact wording).
...
As soon as you learn a tracked ship task has an open PR - even before checks-green - run `bin/fm-pr-check.sh <id> <url>` to arm the merge-attribution watch for the PR's whole lifetime, so an out-of-band merge is caught (section 8) rather than missed in the window before the crew reports the PR.
...
A merge firstmate did not perform surfaces first as an `unattributed-merge` anomaly (section 8): confirm it with the captain and reconcile records before tearing down, since the `merged_by_firstmate` marker in meta is what tells firstmate's own merges apart from these; teardown itself only checks that the work landed, never who merged it.
...
4. `check:` a per-task poll fired (usually a merge poll, or X mode when enabled); act on it.
   An `unattributed-merge:` line means a tracked PR merged that firstmate did not merge: treat it as an anomaly to confirm, never a silent proceed - tell the captain "PR X merged and I did not merge it, please confirm you intended this" - and do NOT fight it, since the captain may merge directly (prime directive 4), so reconcile records and tear down once confirmed, never revert, block, or re-open.
   A `merge-state-unknown:` line means the PR state could not be read: warn and re-check, never assume the PR is unmerged.
5. `heartbeat:` a heartbeat wake now reaches you only when the watcher's bash fleet-scan caught a captain-relevant status the per-wake path missed, or a tracked ship task's PR merged without firstmate on record (handle that as an `unattributed-merge`, item 4); no-change heartbeats are absorbed in bash, never surfaced, so treat it as "something turned up" and review the whole fleet: read each crewmate's current state with `bin/fm-crew-state.sh <id>` (the cheap first read - it reconciles the authoritative run-step over a possibly-stale status-log line, so a crewmate whose gate you already resolved no longer reads as still parked), peek panes that look off, check PR-ready tasks for merge, reconcile data/backlog.md, then re-arm the watcher.
...
The no-mistakes brief points to no-mistakes' version-matched guidance and keeps only firstmate-specific wrapper rules: `ask-user` escalation, `--yes` avoidance, a do-not-fight-a-merge rule (report `needs-decision` and stop on any merge anomaly, take no corrective GitHub action), and an honest-CI-posture done line that distinguishes real CI from a repo with no PR CI.
