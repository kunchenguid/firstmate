...
config/paused-run-failed-absorb  optional presence flag opting this home into the default-off reconciliation letting a crew's own declared paused: line outrank a failed/cancelled run-step underneath it (bin/fm-crew-state.sh); LOCAL, gitignored, and not inherited; see docs/configuration.md "Paused-run-failed absorb"
