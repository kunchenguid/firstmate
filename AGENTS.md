...
config/captain-style.json  optional compact captain-style preferences (language, response_tone) set via /helm; LOCAL, gitignored, and not inherited; canonical source for those two axes over data/captain.md prose (section 6); requires jq (accepted hard dependency, same as config/watched-tools.json and config/crew-dispatch.json); see docs/configuration.md "Captain style preferences"
