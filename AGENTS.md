...
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .wedge-permanent-* .wedge-rollback-failed-* .writing-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch (wedge-permanent-* holds the cap-fire epoch; cleared by FM_CAP_HORIZON_SECS or operator rm of BOTH window-scoped and per-hash markers per docs/configuration.md; wedge-rollback-failed-* holds the cap-marker-write-failure sentinel, expires after FM_ROLLBACK_SENTINEL_TTL_SECS)
