...
config/claude-remote-control  optional "off" opt-out from Claude Remote Control on claude-harness workers; LOCAL, gitignored, and not inherited; absent or "on" reaches those workers from claude.ai/code and the Claude mobile app; see docs/configuration.md "Claude Remote Control"
