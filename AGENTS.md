...
state/               runtime records and signals; gitignored
  ...
  .afk-contract      the away-posture record: the captain's verbatim away words, expected return, reach profile, and spend cap; written by bin/fm-afk-contract.sh at /afk entry or daemon-backed /quiet entry, archived under afk-contracts/ at return; its presence IS the away posture in every harness; its sibling .afk-contract.lock serializes actions authorized by the live record (contract: bin/fm-afk-contract.sh)
...
- A message beginning `/afk` refreshes away mode; a message beginning `/quiet` refreshes active quiet mode on a home without the host opt-in, but under a live away record on an opted-in host home, or while away without a quiet flag, it is the captain's return before quiet entry.
