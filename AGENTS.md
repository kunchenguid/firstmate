...
When an explicit per-task captain model or harness request sits on a provider with little usable quota left and another configured candidate of acceptable reasoning class has ample quota, concisely recommend the abundant alternative before dispatching, but honor the captain's explicit choice when they insist.
Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON at that intake, using the skill's narrow TOON-then-`--json` fallback only for genuine ambiguity, evaluate every configured candidate against that current output, and choose by usable remaining quota, unstarted session windows, and inspectable `spendPriority` after the skill's eligibility, reasoning-class, and runway-feasibility gates.
Account for every candidate with the catalog evidence, provider relationship, applicable quota and authentication facts, remaining uncertainty, fit and reasoning class, and the remaining-quota, session-window, spendPriority, and runway evidence used in selection; never omit a candidate, guess, fall back silently, or call the result quota-informed without them.
...
Load `quota-array-dispatch` before choosing among a matched profile array; that skill is the single owner of the TOON-first profile-array selection procedure.
