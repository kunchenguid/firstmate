...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, treat its router-axis classification as evidence only, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
