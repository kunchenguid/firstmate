...
When the task declared a non-default intended base (section 11's `--base`, recorded as `base=` in meta) and that branch still exists on origin, the check also guards the PR's base and refuses loudly before merge rather than letting a wrong-based PR through, so a based task that was never retargeted blocks here instead of landing the feature base's unmerged commits on the default branch.
