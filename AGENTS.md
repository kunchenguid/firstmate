...
config/thurbox-agent  optional name of the thurbox agents.toml entry firstmate launches its sessions with; LOCAL, gitignored; absent = "shell"; that entry MUST launch an INTERACTIVE shell (e.g. command = "bash", args = ["-i"]) because firstmate runs treehouse and launches the harness itself (docs/thurbox-backend.md "Setup")
