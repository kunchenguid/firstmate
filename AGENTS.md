...
Shared tracked material is `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `bin/`, `.agents/skills/`, and public `skills/`.
...
- `pstack-hermes` - load when the request asks for pstack or rigorous non-trivial engineering, or names a bug, feature, refactor, performance/runtime investigation, evaluation, PR readiness, multi-phase delivery, or session pickup that no more specific owner fully governs.
- `project-verification-harness` - load when a project lacks repeatable live/E2E proof, workers improvise browser or CLI verification, a Feature Map has drifted, or green tests do not prove the requested behavior.
- `implement-spec` - load when the captain provides a spec, feature design, or implementation plan that decomposes into multiple concurrent tasks with a dependency graph between them.
