...
state/               runtime records and signals; gitignored
  ...
  <id>.chrome-devtools-session  task-scoped named bridge binding written by fm-spawn; teardown stops that exact session through chrome-devtools-axi before its safety checks and again after worker exit; removed by teardown, together with its .lock mutex and .inflight registration directories, which bin/fm-chrome-devtools-lib.sh owns and which every writer of the record uses to keep a marker retirement from overwriting a bridge that is still coming up
