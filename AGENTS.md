...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  cost-attribution.tsv  append-only task-to-AI-effort join; firstmate-private, best-effort appended by fm-teardown before volatile metadata is removed on every successful cleanup, and by forced secondmate retirement for each discarded child; hand-written rows are never rewritten, a failed append only warns, and bin/fm-teardown.sh's header owns the schema and append-safety contract
