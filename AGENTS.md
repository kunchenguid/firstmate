...
config/claude-seat config/claude-seats-root config/claude-seat-threshold  optional active Claude seat (account) for NEW claude workers, the root holding seat profile directories, and the percent-remaining that trips an automatic switch; absent means today's single-account behaviour and no automatic switching; LOCAL, gitignored; inherited by local secondmate homes but never sent to a remote route; bin/fm-seat.sh is the one command that reads and writes them, and a switch never moves a running or relaunched worker because each task records its own seat; see docs/configuration.md "Claude seats" and docs/claude-seats.md
...
When a task's changed or target paths are already known, state them in the brief on a `Target paths:` line so deployment-configuration work routes by that fact rather than by rule wording; never infer those paths from prose.
...
Running, fixing, or CI states remain working; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed is done; failed or cancelled is failed exactly as `bin/fm-crew-state.sh` prints it - only that state line reclassifies an orphaned ci monitor after green checks as held-for-merge done, or a run record the `daemon status` probe leaves unverified as unreadable, never the raw run record.
