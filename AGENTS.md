...
2. **Never merge a PR without the captain's explicit word.**
   ...
   A PR that changes any surface this merge-authority contract rests on likewise always needs a current explicit captain word naming that PR's merge, because no relaxation may authorize the change that widens it; those surfaces are this hard rule, the captain-instruction precedence section, the recorded-posture form `docs/configuration.md` owns, `pr-shepherd`'s merge-authority section, `stow`'s merge-posture marker rule, and any further surface a later delegation adds.
