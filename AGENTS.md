...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then tmux; tmux is the verified reference backend (docs/tmux-backend.md), while herdr, zellij, orca, cmux, and codex-app are experimental spawn backends (docs/herdr-backend.md, docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md, docs/codex-app-backend.md) - herdr and cmux can also be selected by runtime auto-detection, while zellij, orca, and codex-app are always explicit; codex-app supports only ordinary Codex workers, not secondmates; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, and `kimi` for worker dispatch, plus `hermes` for the persistent primary; never dispatch on an unverified adapter.
If static `config/crew-harness` or `config/secondmate-harness` names an unverified adapter, report it and fall back only to a verified adapter rather than launching it.

Hermes is verified only as a persistent primary through `bin/fm-hermes-primary.sh`; it is never a Firstmate crewmate or secondmate runtime, and it dispatches visible crewmates only through `bin/fm-spawn.sh`.
