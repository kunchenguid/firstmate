...
config/check-interval  optional default seconds between the watcher's slow-check sweeps; LOCAL, gitignored; see docs/configuration.md "Check interval"
...
2. **Bootstrap** - detect-only checks (tool/version problems, selected-SCM prerequisites, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
...
Do not dispatch until the required tools are present and the selected SCM host's prerequisites are good.
...
X mode or a registered custom check may require that same live cycle with no fleet work.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `SCM_HOST_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
