...
state/               runtime records and signals; gitignored
  ...
  <id>.nm-state       snapshot of the last no-mistakes pipeline-state projection this task's deterministic watch has seen; bin/fm-nm-state-condition.sh owns the format, bin/fm-nm-watch.sh arms the watch (called by fm-spawn.sh on every spawn and relaunch), fm-teardown.sh removes it (docs/configuration.md "Pipeline-state watch")
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `PROCEVENT:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
