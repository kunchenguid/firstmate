...
config/herdr-presentation-spaces  historical "off"/"on" record for Herdr's now-legacy disposable single-task visual projection, which no new spawn creates; LOCAL, gitignored; inherited by secondmate homes; see docs/herdr-backend.md "Legacy presentation spaces"
...
state/               runtime records and signals; gitignored
  ...
  captain-presence   optional durable HOME/AWAY Telegram notification-routing mode; absent or invalid means HOME, and bin/fm-hermes-notify.sh owns the format and commands; it is independent of AFK/quiet supervision posture
  hermes-notify/     optional durable Hermes/Telegram captain-hold notification registrations, keyed to the hold's own lifecycle identity for send dedup, and read back to correlate an inbound Telegram reply to its exact hold; present only on a home with that VPS-local Hermes integration configured; bin/fm-hermes-notify.sh owns the format
