...
- **Auto-discovered supervisor pane** — the daemon resolves its injection target from `FM_SUPERVISOR_TARGET`, then `$TMUX_PANE` / `$WEZTERM_PANE` (whichever the multiplexer set in the pane that launched it; `$WEZTERM_PANE` becomes a `wezterm:<id>` handle), then a `firstmate:0` fallback with a warning; the resolution source is logged at startup so a wrong-but-resolving fallback is detectable.
