...
Never merge a PR at whose head no check has reported unless a current explicit captain instruction states that the repository runs no CI and authorizes `fm-pr-merge.sh --allow-no-checks` for that merge; that attended-only waiver is never a substitute for waiting out a cancelled or re-triggering run, and standing `yolo` cannot authorize it.
