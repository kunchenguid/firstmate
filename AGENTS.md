...
config/browser-policy.json  optional browser-custody policy for bin/fm-browser.sh; LOCAL, gitignored; absent means the browser capability is disabled, and enabling it does not authorize a real browser launch in the current core; inherited by secondmate homes; see docs/configuration.md "Browser policy"
...
Use `gh-axi` for GitHub and `lavish-axi` for structured decisions or reports; load `browser-capability` before browser work.
...
- `browser-capability` - load before browser intake, browser action, manual browser interaction, browser handoff, browser recovery, or browser cleanup.
