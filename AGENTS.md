...
config/deploy-clones.json  optional registry opting a project into automatic redeploy of its machine-local installed CLI clone on merge, run detached by bin/fm-deploy-clone.sh from the merge-landed path; LOCAL, gitignored, firstmate-maintained but human-editable; absent = no-op; see docs/configuration.md "Deployed CLI clones"
