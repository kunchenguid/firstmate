...
Only the task speaking for itself may retract: the `failed:` line must carry no correlation token and no routed `[key=...]`, so a closed routed phase or a reported child outcome cannot withdraw the task's own terminal claim.
