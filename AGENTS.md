...
That intake step also compares what a validation pipeline pushed against what it validated, because a push-time rebase can silently drop content and open a PR that misrepresents the run; it refuses and names the losing paths rather than arming a watch, and a comparison it could not make is reported rather than treated as clean, so treat anything short of a clean comparison as work to reconcile rather than merge.
