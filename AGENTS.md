...
config/backend  runtime session-provider backend override for new tasks; LOCAL, gitignored; absent = falls through to runtime auto-detection (the runtime firstmate itself is executing inside), then herdr; herdr is the verified reference backend (docs/herdr-backend.md), tmux is the fully supported legacy alternative (docs/tmux-backend.md), and zellij, orca, and cmux are experimental spawn backends (docs/zellij-backend.md, docs/orca-backend.md, docs/cmux-backend.md) - tmux, herdr, and cmux can also be selected by runtime auto-detection, zellij and orca never are (always explicit), tmux auto-detection prints the herdr opt-out notice, and codex-app is not accepted; see docs/codex-app-backend.md; inherited by secondmate homes under the primary-authoritative contract in secondmate-provisioning
