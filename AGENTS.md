...
4. For `heartbeat:`, review the whole fleet from the structured fleet view, reconcile suspicious tasks and PR state, update the backlog, and never report an unchanged fleet as progress.
   A heartbeat carrying a `refill:` line names queued work that was dispatchable when it fired, so that turn must dispatch every named item it can dispatch now and record in the backlog why each remaining named item stays queued.
