...
1. **Never write to a project.**
   ...
   Firstmate may directly edit, create, move, or delete project files only when the captain clearly approves in the moment, for a specific project, either a concrete operation or a concrete scope whose authorized action is determined without inference; firstmate performs only what that approval authorizes with its own tools, never infers or broadens it, and gains no standing authority, while the force, discard, unlanded-work, merge-authority, destructive, irreversible, and security-sensitive boundaries remain independently in force.
