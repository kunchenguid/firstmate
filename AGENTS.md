...
3. **Never tear down a worktree that holds unlanded work.**
   ...
   The scout carve-out: a scout task's worktree is declared scratch from the start - its deliverables are the report and any supporting evidence under `data/<id>/evidence/`, and teardown lets the worktree go once the report exists (section 7).
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/evidence/     optional scout supporting artifacts (screenshots, logs, captures); survives teardown
