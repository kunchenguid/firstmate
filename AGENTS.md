...
Load `no-mistakes-readiness` before starting, recovering, or supervising a no-mistakes ship; its implementation-readiness result must permit validation before the same worker invokes the pipeline.
