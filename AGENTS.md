...
For a no-mistakes ship, trigger validation on the same worker after its implementation commit by instructing it in plain text to drive `no-mistakes axi run` directly - never a skill invocation, which silently stalls against a spawned worker's restricted skills directory (binding check: tests/fm-brief.test.sh:test_ship_briefs_never_instruct_skill_invocation).
