...
Use `gh-axi` for GitHub, `chrome-devtools-axi` for browser work, and `lavish-axi` for structured decisions or reports; in Pi-family primary sessions, use `fm_lavish_poll` for Lavish feedback waits as owned by `harness-adapters`; consult current help rather than memorizing flags.
