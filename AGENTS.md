...
Sharp edge: `treehouse get`'s subshell has been observed to `chdir()` the pane into the worktree correctly (confirmed by the isolation guard's own `pwd -P` check) while leaving the pane's `$PWD` environment variable stale (e.g. literally `.`) - not reproduced on every spawn, but intermittent enough that it repeatedly broke a project's `no-mistakes` gate `post-receive` hook, whose bare `pwd` call inherited the bad `$PWD` down the `git push` -> `git-receive-pack` -> hook exec chain and failed with `invalid gate path: .`. Manually patching the generated hook file fixed it but never held, because the daemon regenerates that file (a compiled binary, not something this repo owns) on restart. Since the actual break is a stale `$PWD` env var propagated by firstmate's own launch chain, not the hook, `fm-spawn.sh` re-exports `PWD="$(pwd -P)"` into the pane right after the isolation guard passes (`pwd -P` ignores `$PWD` and resolves the real physical cwd via `getcwd()`), for ship/scout launches only - secondmate panes get their cwd from tmux's own `-c` flag at process start rather than a `treehouse get` subshell, so they are not known to hit this.
The corruption is not confined to spawn time: it has also been observed to recur later in a long-lived crewmate session, well after the one-time spawn-time export, so `fm-spawn.sh`'s spawn-time re-export alone is not sufficient. As defense-in-depth, the ship brief (`bin/fm-brief.sh`) also instructs crewmates to re-run `export PWD="$(pwd -P)"` immediately before invoking /no-mistakes or any other command that pushes to the no-mistakes gate remote.
