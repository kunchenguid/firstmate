...
state/               runtime records and signals; gitignored
  ...
  <id>.cline-session  cline busy-source binding (sessions root, task worktree, pre-existing sessions) written by fm-spawn; removed by teardown
  <id>.cline-settings.json  firstmate-owned cline settings copy forcing act mode, pointed at by CLINE_GLOBAL_SETTINGS_PATH at launch; written by fm-spawn, removed by teardown, and never the operator's own ~/.cline
