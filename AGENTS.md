...
config/provision/<project>.toml  optional per-project worktree provisioning config (env_source_dir, env_files, setup_cmd, optional setup_timeout); LOCAL, gitignored; absent = no-op (section 7 "Spawn")
...
For ship and scout tasks, tmux/herdr/zellij/cmux create a runtime endpoint and run `treehouse get`; Orca creates an Orca-owned worktree, validates it, then creates the terminal. In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the per-task turn-end signal hook, provisions the worktree run-ready (see below), records `state/<id>.meta`, and launches the agent with the brief.
Selecting `backend=codex-app` fails as an unknown backend; see `docs/codex-app-backend.md`.

**Worktree provisioning.**
A freshly-cut worktree lacks its project's env and credential files (they are gitignored, so they never come from GitHub) and sometimes its installed deps, so a crewmate cannot run the full app.
After the isolation guard passes and before the agent launches, `fm-spawn` runs `bin/fm-provision-worktree.sh <project> <worktree>` for every ship/scout spawn (never for secondmate homes) to make the worktree run-ready.
It is driven by a per-project, fleet-LOCAL, gitignored config at `config/provision/<project>.toml` (project = the registered `projects/<name>` basename), so provisioning is strictly opt-in: a project with no config file is a silent no-op and the spawn behaves exactly as before.
The config declares `env_source_dir` (absolute source dir, a leading `~`/`~/` expands to `$HOME`, typically the captain's own separate clone), `env_files` (a general single-line array of repo-relative paths to copy into the worktree, each preserving its relative path with parent dirs created - not only `.env` files but ANY file a backend needs at runtime, including credential/key files a `.env` references by relative path such as a Firebase service-account JSON pointed at by `GOOGLE_APPLICATION_CREDENTIALS`; a `.env` that names a missing service-account key is exactly what breaks a worktree backend), and `setup_cmd` (a shell command run in the worktree root to install deps, e.g. `npm install`).
`setup_cmd` is bounded by a timeout so a hanging or interactive install can never block the spawn indefinitely: it defaults to 600 seconds, and `FM_PROVISION_SETUP_TIMEOUT` overrides an optional per-project `setup_timeout` config key, which overrides the default (only a positive integer of seconds is accepted; an invalid value warns and falls back to 600); on expiry the whole setup process group is killed and the timeout is treated as a non-aborting provisioning failure.
The helper never reads or prints the copied files' contents, only copies and names them; a source file missing at `env_source_dir` is skipped with a warning rather than failing.
Provisioning is best-effort: a failure prints a `PROVISION:` warning to stderr but does NOT abort the spawn (the crewmate can still work, only running the full app is lost), and a single spawn can skip it with `FM_SPAWN_NO_PROVISION=1`.
`config/provision/` is personal fleet state firstmate writes locally after a change merges; only the committed template `docs/examples/provision.toml` lives in the repo, never a real project's actual config or any real env/credential file.
