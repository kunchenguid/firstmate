...
state/               runtime records and signals; gitignored
  ...
  procevent/         registered process-to-event sources, one private record per canonical source id, plus the Lavish adapter's private per-source pending-reply and in-flight host-acknowledgement staging files; source records written by bin/fm-procevent.sh and reply records by bin/fm-procevent-lavish.sh, and their presence alone keeps supervision required (section 13)
