...
A fresh `bin/fm-spawn.sh` without an explicit crewmate/scout profile automatically runs `bin/fm-dispatch-resolve.sh` when the dispatch file is active; it adopts only a `clear` result, while explicit profiles and relaunches are deliberate bypasses and every non-clear result returns to the existing intake (contract: `docs/configuration.md` "Typed dispatch resolution").
