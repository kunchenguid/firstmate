...
config/branch-prefix  optional per-home task-branch prefix replacing the default fm/ in scaffolded briefs, definitions of done, local landing, review, promotion, and PR-to-task mapping; one line of letters, digits, and dashes with no leading dash and no trailing slash; LOCAL, gitignored, and not inherited; see docs/configuration.md "Task-branch prefix" (bin/fm-branch-prefix-lib.sh)
