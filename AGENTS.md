...
config/herdr-workspace-label  optional Herdr workspace label for THIS primary home, so a second primary home on one Herdr session stops sharing the first home's workspace; LOCAL, gitignored; absent keeps "firstmate" and a secondmate home keeps "2ndmate-<id>"; NOT inherited (each home needs its own); see docs/configuration.md "Runtime backend"
