...
config/crew-dispatch.json  optional crewmate dispatch profiles; LOCAL, gitignored; firstmate-maintained but human-editable natural-language rules that choose a per-task harness/model/effort/account-home profile (section 4). Inherited by secondmate homes
...
When dispatch profiles exist, consult them at every crewmate or scout intake and pass the resolved concrete harness, model, effort, and optional `codexHome` or `piAgentDir` as `fm-spawn` flags.
