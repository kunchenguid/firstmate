...
state/               volatile runtime signals; gitignored
  ...
  <id>.body-scan     published-body scan verdict written by fm-pr-check so a repeat check of the same PR does not re-print an unchanged one; removed by teardown
...
Briefs for the two PR-opening modes carry the public-PR contract, which the scaffold owns in full: the PR body is public reviewer-facing writing, and the crewmate must read the live body back with `bin/fm-pr-body-check.sh <pr-url>` and clear it before reporting done.
