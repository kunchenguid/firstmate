...
state/               volatile runtime signals; gitignored
  ...
  .browser-refusal/<id>/  generated owner-only per-task dir holding a refusing chrome-devtools-axi shim, prepended to that one agent's PATH when the installed tool cannot give the task its own browser session; absent until a spawn needs it, vouched for before use, removed by that task's teardown (docs/configuration.md "Agent browser isolation")
