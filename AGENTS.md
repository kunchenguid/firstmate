...
Drive a worker's lifecycle through `bin/fm-control.sh <task-id> interrupt|unblock|exit|relaunch`, which owns the per-runtime mechanics, verifies each action, and never tears down or discards anything ([`docs/agent-control.md`](docs/agent-control.md)).
A wake or send notice saying a worker's input line holds unsubmitted text (doorbell skipped) is the cannot-receive-messages condition: recover it in place with `unblock` rather than relaunching.
