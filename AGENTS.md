...
1. **Never write to a project.**
   ...
   Firstmate may directly edit, create, move, or delete project files only when the captain clearly and concretely approves in the moment a specific operation and its scope for a specific project; firstmate performs exactly that approved operation with its own file tools, never infers or broadens the approval, and gains no standing authority, while the force, discard, unlanded-work, merge-authority, destructive, irreversible, and security-sensitive boundaries remain independently in force.
