...
config/max-active-workers     primary-authoritative per-host limit for concurrently live crew endpoints across local secondmate homes; LOCAL, gitignored, inherited into secondmate homes; absent = one; see docs/configuration.md "Worker capacity"
...
Treat file or subsystem overlap as a risk signal rather than an automatic reason to wait. Dispatch isolated work immediately when each change can be independently implemented and validated, the selected delivery path can reconcile ordinary rebases or conflicts, and the configured worker capacity has a free slot. Queue otherwise-ready work when capacity is full; do not bypass the admission guard.
