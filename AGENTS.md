...
Never merge a red PR under either setting unless a current explicit captain instruction names every GitHub check waived through a separate `fm-pr-merge.sh --allow-red <check-name>` argument; those attended-only waivers still require every other check green.
