...
2. **Never merge a PR without the captain's explicit word.**
   ...
   A PR that changes this hard rule, the captain-instruction precedence section, or `pr-shepherd`'s merge-authority section likewise always needs a current explicit captain word naming that PR's merge, because no relaxation may authorize the change that widens it.
