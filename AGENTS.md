...
Running, fixing, or CI states remain working; parked approval or fix-review states require the worker to follow the active gate help; passed or checks-passed is done; a step that judged the work and rejected it is failed, while a deliberate cancel reports aborted and a pipeline that broke without judging the work reports interrupted, both re-run conditions rather than rejected code to fix.
