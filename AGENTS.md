...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `rovo`, and `antigravity` for crewmates and scouts only; `hermes` has a bridge implementation pending the credentialed verification in its harness reference; never dispatch on an unverified adapter.
