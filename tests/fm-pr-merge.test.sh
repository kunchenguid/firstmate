...
# never fires.
#
# The GitHub matrix preserves the established gh-axi argv, squash default,
# explicit-method and extra-argument behavior, queue-aware outcome checks,
# refusal diagnostics, lifecycle recording, and durable outcome routing after
# the newly required authority declaration.
#
...
MR_SQUASH=cccccccccccccccccccccccccccccccccccccccc
MR_RESULT=dddddddddddddddddddddddddddddddddddddddd

JQ_BIN=$(command -v jq) || fail "these tests read glab-axi JSON with the real jq, which was not found"
...
# Plain glab remains available only because the byte-static read poll checks for
# it when arming. The merge wrapper itself must never invoke it.
add_gitlab_mocks() {
  local case_dir=$1
  cat > "$case_dir/fakebin/glab" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$FM_TEST_GLAB_LOG"
echo "error: plain glab must never be invoked by fm-pr-merge" >&2
exit 97
SH
...
case "${1:-} ${2:-}" in
  "mr view")
    count=0
    [ ! -f "$FM_TEST_VIEW_COUNT" ] || count=$(cat "$FM_TEST_VIEW_COUNT")
    count=$((count + 1))
    printf '%s\n' "$count" > "$FM_TEST_VIEW_COUNT"
    if [ "$count" -eq 1 ] || [ ! -e "$FM_TEST_VIEW_TWO" ]; then
      cat "$FM_TEST_VIEW_ONE"
    else
      cat "$FM_TEST_VIEW_TWO"
    fi
    exit "${FM_TEST_VIEW_RC:-0}"
    ;;
  "mr merge")
    cat "$FM_TEST_MERGE_JSON"
    exit "${FM_TEST_MERGE_RC:-0}"
    ;;
esac
echo "unexpected glab-axi invocation: $*" >&2
exit 98
SH
  chmod +x "$case_dir/fakebin/glab" "$case_dir/fakebin/glab-axi"
  ln -sf "$JQ_BIN" "$case_dir/fakebin/jq"
}

write_mr_view_json() {
  local file=$1 host=$2 path=$3 iid=$4 url=$5 head=$6
  local source=${7:-fm/task-x1} target=${8:-main} state=${9:-opened}
  local conflicts=${10:-false}
  jq -cn \
    --arg host "$host" --arg repo "$path" --arg url "$url" \
    --arg head "$head" --arg source "$source" --arg target "$target" \
    --arg state "$state" --argjson iid "$iid" --argjson conflicts "$conflicts" \
    '{
...
write_mr_merge_json() {
  local file=$1 host=$2 path=$3 iid=$4 url=$5 head=$6 authority=$7
  local source=${8:-fm/task-x1} target=${9:-main} action=${10:-merged}
  jq -cn \
    --arg host "$host" --arg repo "$path" --arg url "$url" \
    --arg head "$head" --arg source "$source" --arg target "$target" \
    --arg authority "$authority" --arg action "$action" \
    --arg squash "$MR_SQUASH" --arg result "$MR_RESULT" \
    --argjson iid "$iid" \
    '{
      schema:"glab-axi/ux-v1",
      ok:true,
      data:{merge:{
        ...
        pipeline:{id:1234,sha:$head,status:"success"},
        authority:$authority
      }},
      meta:{
        backend:"official-glab",
...
# make_gitlab_case <name> [host [path [iid [authority]]]]
make_gitlab_case() {
  local name=$1 host=${2:-$MR_HOST} path=${3:-$MR_PATH}
  local iid=${4:-7} authority=${5:-captain-explicit} case_dir url
  case_dir=$(make_case "$name")
  url="https://$host/$path/-/merge_requests/$iid"
  mkdir -p "$case_dir/wt"
  add_gh_mocks "$case_dir" cccccccccccccccccccccccccccccccccccccccc
  add_gitlab_mocks "$case_dir"
  : > "$case_dir/gh-axi.log"
  : > "$case_dir/glab.log"
  : > "$case_dir/glab-axi.log"
  write_mr_view_json "$case_dir/view-one.json" "$host" "$path" "$iid" "$url" "$MR_HEAD"
  cp "$case_dir/view-one.json" "$case_dir/view-two.json"
  write_mr_merge_json "$case_dir/merge.json" "$host" "$path" "$iid" "$url" \
    "$MR_HEAD" "$authority"
  printf '%s\n' "$case_dir"
}
...
EOF
  hash -r
  ! PATH="$dir" type -P "$omit" >/dev/null 2>&1 \
    || fail "the $omit-free search path still resolved $omit"
}

glab_merge_line() {
  grep '^mr merge ' "$1" || true
}

glab_view_count() {
  grep -c '^mr view ' "$1/glab-axi.log" || true
}

glab_merge_count() {
  grep -c '^mr merge ' "$1/glab-axi.log" || true
}
...
run_pr_merge_raw() {
  ...
  FM_TEST_GLAB_AXI_LOG="$case_dir/glab-axi.log" \
  FM_TEST_VIEW_COUNT="$case_dir/view-count" \
  FM_TEST_VIEW_ONE="$case_dir/view-one.json" \
  FM_TEST_VIEW_TWO="$case_dir/view-two.json" \
  FM_TEST_MERGE_JSON="$case_dir/merge.json" \
  PATH="${FM_TEST_PATH:-$case_dir/fakebin:$PATH}" \
    "$PR_MERGE" "$@"
}

# Existing behavior tests use one valid authority declaration by default so
# they continue to isolate the GitHub behavior they were written to pin.
run_pr_merge() {
  local case_dir=$1 id url; shift
  id=$1
  url=$2
  shift 2
  run_pr_merge_raw "$case_dir" "$id" "$url" \
    --authority "${FM_TEST_AUTHORITY:-captain-explicit}" "$@"
}
...
test_github_failed_merge_with_queue_flags_never_claims_acceptance() {
  ...
  assert_grep 'task-x1 https://github.com/example/repo/pull/74 --authority captain-explicit -- --auto --merge' "$case_dir/stderr" \
...
test_malformed_url_refuses_before_merge() {
  ...
  assert_grep 'error: invalid PR/MR merge request' "$case_dir/stderr" \
...
test_rejects_unsafe_url_segments_before_recording() {
  ...
  assert_grep 'error: invalid PR/MR merge request' "$case_dir/stderr" \
    "unsafe-url-segment: refusal did not use the fixed non-probing diagnostic"
...
assert_no_provider_call() {
  ...
  [ ! -s "$case_dir/glab-axi.log" ] \
    || fail "$label: glab-axi ran on local refusal: $(cat "$case_dir/glab-axi.log")"
  [ ! -s "$case_dir/glab.log" ] \
    || fail "$label: plain glab ran on local refusal: $(cat "$case_dir/glab.log")"
  [ ! -s "$case_dir/gh-axi.log" ] \
    || fail "$label: gh-axi ran on local refusal: $(cat "$case_dir/gh-axi.log")"
}

test_merge_authority_boundary() {
  local case_dir rc mode help
  help=$($PR_MERGE --help) || fail "fm-pr-merge --help failed"
  assert_contains "$help" 'captain-explicit|standing-yolo-green' \
    "merge help omitted the accepted authority classes"
  for mode in missing invalid duplicate; do
    case_dir=$(make_gitlab_case "authority-$mode")
    set +e
    case "$mode" in
      missing)
        run_pr_merge_raw "$case_dir" task-x1 "$MR_URL" \
          > "$case_dir/stdout" 2> "$case_dir/stderr" ;;
      invalid)
        run_pr_merge_raw "$case_dir" task-x1 "$MR_URL" --authority maybe \
          > "$case_dir/stdout" 2> "$case_dir/stderr" ;;
      duplicate)
        run_pr_merge_raw "$case_dir" task-x1 "$MR_URL" \
          --authority captain-explicit -- --authority standing-yolo-green \
          > "$case_dir/stdout" 2> "$case_dir/stderr" ;;
    esac
    rc=$?
    set -e
    case "$mode" in
      missing|invalid) expect_code 2 "$rc" "authority-$mode should be a usage refusal" ;;
      duplicate) expect_code 1 "$rc" "duplicate authority should be a safety refusal" ;;
    esac
    assert_no_provider_call "$case_dir" "authority-$mode"
    assert_no_grep '^pr=' "$case_dir/state/task-x1.meta" \
      "authority-$mode: local refusal recorded provider state"
  done
  pass "fm-pr-merge requires exactly one validated merge-authority class"
}

test_gitlab_exact_guarded_route_and_success_actions() {
  local action authority case_dir expected url output
  for authority in captain-explicit standing-yolo-green; do
    case_dir=$(make_gitlab_case "gitlab-route-$authority" gl.self-hosted.example \
      deep/group/subgroup/project 31 "$authority")
    url='https://gl.self-hosted.example/deep/group/subgroup/project/-/merge_requests/31'
    ...
    expect_code 2 "$(glab_view_count "$case_dir")" \
      "exact GitLab route should record then revalidate the head"
    expect_code 1 "$(glab_merge_count "$case_dir")" \
      "exact GitLab route should invoke one guarded merge"
    grep -qxF "pr_head=$MR_HEAD" "$case_dir/state/task-x1.meta" \
      || fail "exact GitLab route did not persist one expected head"
    grep -qxF "gitlab_guarded_squash_receipt=v1|task-x1|$url|$authority|$MR_HEAD|fm/task-x1|main|$MR_SQUASH|$MR_RESULT" \
      "$case_dir/state/task-x1.meta" \
      || fail "exact GitLab route did not persist the task-bound squash receipt"
    [ ! -s "$case_dir/glab.log" ] || fail "exact GitLab route invoked plain glab"
    [ ! -s "$case_dir/gh-axi.log" ] || fail "exact GitLab route invoked gh-axi"
  done

  for action in merged already_merged reconciled_merged; do
    case_dir=$(make_gitlab_case "gitlab-action-$action")
    jq --arg action "$action" '.data.merge.action = $action' \
      "$case_dir/merge.json" > "$case_dir/merge.tmp"
    mv "$case_dir/merge.tmp" "$case_dir/merge.json"
    if [ "$action" = reconciled_merged ]; then
      jq --arg squash "$MR_SQUASH" \
        '.data.merge.merge_commit_sha = null | .data.merge.result_commit_sha = $squash' \
        "$case_dir/merge.json" > "$case_dir/merge.tmp"
      mv "$case_dir/merge.tmp" "$case_dir/merge.json"
    fi
    run_pr_merge "$case_dir" task-x1 "$MR_URL" \
      > "$case_dir/stdout" 2> "$case_dir/stderr" \
      || fail "guarded GitLab action $action should validate"
    expect_code 1 "$(glab_merge_count "$case_dir")" \
      "guarded GitLab action $action should issue one merge call"
    output=$(tail -n +2 "$case_dir/stdout")
    printf '%s\n' "$output" | jq -e --arg action "$action" \
      '.data.merge.action == $action' >/dev/null \
      || fail "guarded GitLab action $action result was not relayed"
  done
  pass "fm-pr-merge routes exact nested GitLab MRs and accepts only verified guarded success actions"
}

test_gitlab_forbidden_arguments_are_zero_provider() {
  local spec label arg1 arg2 case_dir rc n=0
  set -- \
    'merge|--merge|' \
    'rebase|--rebase|' \
    'method|--method|squash' \
    'method-equals|--method=squash|' \
    'auto|--auto|' \
    'delete-branch|--delete-branch|' \
    'remove-source|--remove-source-branch|' \
    'repo|--repo|other/project' \
    'repo-equals|--repo=other/project|' \
  ...
  for spec in "$@"; do
    ...
    arg1=${spec%%|*}
    arg2=${spec#*|}
    case_dir=$(make_gitlab_case "forbidden-$n-$label")
    set +e
    if [ -n "$arg2" ]; then
      run_pr_merge "$case_dir" task-x1 "$MR_URL" -- "$arg1" "$arg2" \
        > "$case_dir/stdout" 2> "$case_dir/stderr"
    else
      run_pr_merge "$case_dir" task-x1 "$MR_URL" -- "$arg1" \
        > "$case_dir/stdout" 2> "$case_dir/stderr"
    fi
    rc=$?
    set -e
    expect_code 1 "$rc" "forbidden GitLab argument $label should refuse"
    assert_no_provider_call "$case_dir" "forbidden-$label"
    assert_no_grep '^pr=' "$case_dir/state/task-x1.meta" \
      "forbidden-$label: refusal recorded provider state"
  done

  case_dir=$(make_gitlab_case forbidden-duplicate-squash)
  set +e
  run_pr_merge "$case_dir" task-x1 "$MR_URL" -- --squash --squash \
    > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_gitlab_head_and_view_identity_are_strict() {
  ...
  write_mr_view_json "$case_dir/view-two.json" "$MR_HOST" "$MR_PATH" 7 \
    "$MR_URL" "$MR_STALE_HEAD"
  set +e
  run_pr_merge "$case_dir" task-x1 "$MR_URL" \
    > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
  expect_code 1 "$rc" "changed GitLab head should refuse"
  grep -qxF "pr_head=$MR_HEAD" "$case_dir/state/task-x1.meta" \
    || fail "changed GitLab head lost the recorded expected revision"
  assert_no_grep "pr_head=$MR_STALE_HEAD" "$case_dir/state/task-x1.meta" \
    "changed GitLab head silently refreshed the expectation"
  expect_code 0 "$(glab_merge_count "$case_dir")" \
    "changed GitLab head reached the merge primitive"

  for variant in malformed duplicate legacy wrong-host wrong-repo wrong-url wrong-iid \
    truncated invalid-head invalid-source conflict-type; do
    case_dir=$(make_gitlab_case "gitlab-view-$variant")
    case "$variant" in
      malformed) printf '{not-json\n' > "$case_dir/view-one.json" ;;
      duplicate)
        cp "$case_dir/view-one.json" "$case_dir/view.copy"
        cat "$case_dir/view.copy" >> "$case_dir/view-one.json" ;;
      legacy)
        jq '.data.mr.sha = .data.mr.head_sha | del(.data.mr.head_sha)' \
          "$case_dir/view-one.json" > "$case_dir/view.tmp"
        mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      wrong-host) jq '.meta.host = "other.example"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      wrong-repo) jq '.meta.repo = "other/project"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      wrong-url) jq '.data.mr.web_url = "https://gitlab.example/other/project/-/merge_requests/7"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      wrong-iid) jq '.data.mr.iid = 8' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      truncated) jq '.meta.complete = false | .meta.truncated = true' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      invalid-head) jq '.data.mr.head_sha = "not-a-sha"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      invalid-source) jq '.data.mr.source_branch = "fm|bad"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
      conflict-type) jq '.data.mr.has_conflicts = "false"' "$case_dir/view-one.json" > "$case_dir/view.tmp"; mv "$case_dir/view.tmp" "$case_dir/view-one.json" ;;
    esac
    set +e
    run_pr_merge "$case_dir" task-x1 "$MR_URL" \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
    rc=$?
    set -e
    expect_code 1 "$rc" "strict GitLab view $variant should refuse"
    expect_code 0 "$(glab_merge_count "$case_dir")" \
      "strict GitLab view $variant reached the merge primitive"
    assert_no_grep '^pr=' "$case_dir/state/task-x1.meta" \
      "strict GitLab view $variant recorded ambiguous identity"
  done
  pass "fm-pr-merge records one expected head and rejects stale or ambiguous MR views"
}

test_gitlab_provider_failures_and_results_are_single_attempt() {
  local case_dir rc variant
  for rc in 5 6; do
    case_dir=$(make_gitlab_case "gitlab-provider-$rc")
    set +e
    FM_TEST_MERGE_RC=$rc run_pr_merge "$case_dir" task-x1 "$MR_URL" \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
    actual=$?
    set -e
    expect_code "$rc" "$actual" "GitLab provider refusal $rc should propagate"
    expect_code 1 "$(glab_merge_count "$case_dir")" \
      "GitLab provider refusal $rc retried the mutation"
    assert_no_grep '^gitlab_guarded_squash_receipt=' "$case_dir/state/task-x1.meta" \
      "GitLab provider refusal $rc recorded a success receipt"
  done

  for variant in malformed duplicate false-ok bad-action wrong-host wrong-repo \
    wrong-url wrong-iid wrong-head wrong-source wrong-target wrong-authority \
    red-pipeline stale-pipeline bad-squash bad-merge bad-result missing-merge; do
    case_dir=$(make_gitlab_case "gitlab-result-$variant")
    case "$variant" in
      malformed) printf '{not-json\n' > "$case_dir/merge.json" ;;
      duplicate) cp "$case_dir/merge.json" "$case_dir/merge.copy"; cat "$case_dir/merge.copy" >> "$case_dir/merge.json" ;;
      false-ok) jq '.ok = false' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      bad-action) jq '.data.merge.action = "ambiguous"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-host) jq '.meta.host = "other.example"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-repo) jq '.meta.repo = "other/project"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-url) jq '.data.merge.web_url = "https://gitlab.example/other/project/-/merge_requests/7"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-iid) jq '.data.merge.iid = 8' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-head) jq --arg h "$MR_STALE_HEAD" '.data.merge.source_head_sha = $h' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-source) jq '.data.merge.source_branch = "fm/other"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-target) jq '.data.merge.target_branch = "release"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      wrong-authority) jq '.data.merge.authority = "standing-yolo-green"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      red-pipeline) jq '.data.merge.pipeline.status = "failed"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      stale-pipeline) jq --arg h "$MR_STALE_HEAD" '.data.merge.pipeline.sha = $h' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      bad-squash) jq '.data.merge.squash_commit_sha = "bad"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      bad-merge) jq '.data.merge.merge_commit_sha = "bad"' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      bad-result) jq --arg h "$MR_SQUASH" '.data.merge.result_commit_sha = $h' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
      missing-merge) jq 'del(.data.merge)' "$case_dir/merge.json" > "$case_dir/merge.tmp"; mv "$case_dir/merge.tmp" "$case_dir/merge.json" ;;
    esac
    set +e
    run_pr_merge "$case_dir" task-x1 "$MR_URL" \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
    rc=$?
    set -e
    expect_code 1 "$rc" "strict GitLab result $variant should refuse"
    expect_code 1 "$(glab_merge_count "$case_dir")" \
      "strict GitLab result $variant retried or skipped the guarded mutation"
    assert_no_grep '^gitlab_guarded_squash_receipt=' "$case_dir/state/task-x1.meta" \
      "strict GitLab result $variant recorded an unproved receipt"
  done
  pass "fm-pr-merge propagates provider refusals and accepts no ambiguous success result"
}

test_gitlab_missing_tools_refuse_before_recording() {
  local case_dir rc tool path expected
  for tool in glab glab-axi jq; do
    case_dir=$(make_gitlab_case "gitlab-no-$tool")
    path="$case_dir/no-$tool"
    mirror_path_without "$path" "$tool" "$case_dir/fakebin"
    case "$tool" in
      glab) expected='watching a GitLab merge request requires glab on PATH' ;;
      glab-axi) expected='recording a GitLab merge request requires glab-axi on PATH' ;;
      jq) expected='recording a GitLab merge request requires jq on PATH' ;;
    esac
    set +e
    FM_TEST_PATH=$path run_pr_merge "$case_dir" task-x1 "$MR_URL" \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
    rc=$?
    set -e
    expect_code 1 "$rc" "missing GitLab tool $tool should refuse"
    assert_grep "$expected" "$case_dir/stderr" \
      "missing GitLab tool $tool was not named"
    assert_no_grep '^pr=' "$case_dir/state/task-x1.meta" \
      "missing GitLab tool $tool recorded provider state"
    assert_no_provider_call "$case_dir" "missing-$tool"
  done
  pass "fm-pr-merge refuses before recording when a GitLab lifecycle dependency is missing"
}
...
test_gitlab_refusal_reports_nothing() {
  ...
  jq '.data.merge.action = "ambiguous"' "$case_dir/merge.json" > "$case_dir/merge.tmp"
  mv "$case_dir/merge.tmp" "$case_dir/merge.json"
  ...
  expect_code 1 "$rc" "gitlab-refusal-silent: an ambiguous GitLab result should exit non-zero"
  assert_absent "$case_dir/state/parent-replies.status" \
    "gitlab-refusal-silent: an unproved merge request was reported as landed"
  pass "an ambiguous GitLab merge result reports no landed outcome"
}
...
test_ambiguous_gitlab_merge_leaves_the_poll_armed() {
  local case_dir rc
  case_dir=$(make_gitlab_case ambiguous-gitlab-merge)
  printf '{not-json\n' > "$case_dir/merge.json"
  mkdir -p "$case_dir/home"
  ...
  rc=$?
  set -e
  expect_code 1 "$rc" "ambiguous-gitlab-merge: unproved output should refuse"
  assert_absent "$case_dir/state/.wake-queue" \
    "ambiguous-gitlab-merge: ambiguous output was reported as landed"
  [ -f "$case_dir/state/task-x1.check.sh" ] \
    || fail "ambiguous-gitlab-merge: the merge poll was not left armed"
  [ ! -e "$case_dir/state/task-x1.pr-poll-merge-notified" ] \
    || fail "ambiguous-gitlab-merge: ambiguous output was marked as reported"
  pass "an ambiguous GitLab result stays silent and leaves its poll armed"
}
