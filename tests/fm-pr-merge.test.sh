...
# Mirror a usable PATH minus one binary: link every entry from the given bin
# dirs plus BASE_PATH into <dir>, skipping <omit>, so a case can prove code
# runs without a tool without an empty PATH breaking the rest of the fixture.
mirror_path_without() {
...
test_malformed_url_refuses_before_merge() {
  ...
  pass "fm-pr-merge refuses non-GitHub PR URLs before calling gh-axi"
}
...
# the CLI expands it one character at a time, so the guard has to read the
# whole cluster.
