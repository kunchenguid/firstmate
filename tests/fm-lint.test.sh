...
# invokes, so the local lint can never diverge from CI again.
# Regression origin: a prior local validation path did not run the deterministic
...
# ShellCheck retired in 0.11.0. fm-lint.sh now pins one exact version, so
...
SH
  ...
  pass "fm-lint.sh catches a real lint defect before CI"
}
