...
test_herdr_ci_family_run_has_a_step_timeout() {
  ...
  # artifact keys cannot masquerade as the step contract. Prefer Ruby's bundled
  # parser, but accept PyYAML on Python hosts that deliberately omit Ruby.
...
step = next(
    ...
    and item.get("name") == "Run real-Herdr family (serial, required)"
)
...
PY
    ...
    fail "Ruby or python3 with PyYAML is required to parse .github/workflows/ci.yml as YAML"
  fi
