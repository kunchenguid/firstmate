...
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate claude:default --candidate bogus:default 2>&1); then
  fail "trailing unsupported harness was hidden by an earlier selection"
fi
[ "$err" = "error: unknown harness: bogus" ] || fail "trailing unsupported harness returned: $err"
# agy is a verified harness with no quota-axi provider row, so it is refused by
# name rather than reported as unknown, and a leading selection cannot hide it.
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate claude:default --candidate agy:default 2>&1); then
  fail "trailing agy candidate was hidden by an earlier selection"
fi
case "$err" in
  "error: agy has no quota-axi provider mapping"*) ;;
  *) fail "agy candidate must be refused by name, got: $err" ;;
esac
...
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate bogus:default 2>&1); then
  fail "unsupported harness unexpectedly dispatched"
fi
[ "$err" = "error: unknown harness: bogus" ] || fail "unsupported harness returned: $err"
ok "unsupported harness is rejected"

# agy is verified but has no quota-axi provider row, so a lone candidate is
# refused by name rather than dispatched or reported as an unknown harness.
if err=$(call_choose --snapshot "$LAB/captured.json" --candidate agy:default 2>&1); then
  fail "agy candidate unexpectedly dispatched"
fi
case "$err" in
  "error: agy has no quota-axi provider mapping"*) ;;
  *) fail "agy candidate returned: $err" ;;
esac
ok "agy is refused by name without a quota row"
