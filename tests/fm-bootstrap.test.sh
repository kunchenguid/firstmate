...
a non-string home is flagged^{"rules":[{"when":"x","use":{"harness":"codex","home":7}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - profile home must be a non-empty string when present
