  ...
  (
    ...
    sleep() {
      ...
      if [ "${FM_FAKE_GIT_WAIT_FOR_FLEET_START:-}" = 1 ] \
        && [ "${FM_FAKE_FLEET_START_WAITED:-0}" -eq 0 ] \
        && [ -n "${FM_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ]; then
        FM_FAKE_FLEET_START_WAITED=1
        tries=0
...
quota-balanced tuning is accepted^{"rules":[{"when":"big feature","use":[{"harness":"claude"},{"harness":"codex"}],"select":"quota-balanced"}],"quotaBalanced":{"reservePercent":5,"staleClearMargin":20,"runRate":{"minimumObservationSeconds":300,"historyMaxAgeSeconds":86400,"recentWeight":0.5},"providers":{"codex":{"reservePercent":10,"windows":[{"scope":"weekly","durationSeconds":604800,"extraResets":3}]}}}}^empty^
quota-balanced reset count must be an integer^{"quotaBalanced":{"providers":{"codex":{"windows":[{"scope":"weekly","durationSeconds":604800,"extraResets":1.5}]}}}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quotaBalanced window extraResets must be a non-negative integer
quota-balanced duration must be positive^{"quotaBalanced":{"providers":{"codex":{"windows":[{"scope":"weekly","durationSeconds":0,"extraResets":3}]}}}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quotaBalanced window durationSeconds must be a positive integer
quota-balanced recent weight is bounded^{"quotaBalanced":{"runRate":{"recentWeight":1.5}}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quotaBalanced.runRate.recentWeight must be between 0 and 1
quota-balanced weekly resets cannot target session scope^{"quotaBalanced":{"providers":{"codex":{"windows":[{"scope":"session","durationSeconds":604800,"extraResets":3}]}}}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quotaBalanced window scope must match canonical 5-hour or 7-day duration
quota-balanced duplicate windows are flagged^{"quotaBalanced":{"providers":{"codex":{"windows":[{"scope":"weekly","durationSeconds":604800,"extraResets":1},{"scope":"weekly","durationSeconds":604800,"extraResets":2}]}}}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quotaBalanced provider windows must not duplicate scope and durationSeconds
ROWS
