...
prime-agent shared efforts are accepted^{"rules":[{"when":"prime low","use":{"harness":"prime-agent","effort":"low"}},{"when":"prime max","use":{"harness":"prime-agent","effort":"max"}}],"default":{"harness":"prime-agent","model":"gpt-5.6-luna"}}^empty^
unsupported prime-agent ultra effort is flagged^{"rules":[{"when":"prime ultra","use":{"harness":"prime-agent","effort":"ultra"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: prime-agent:ultra
