...
agy dispatch profile is accepted^{"rules":[{"when":"agy work","use":{"harness":"agy","model":"gemini-3.8-flash","effort":"high"}}],"default":{"harness":"agy"}}^empty^
unsupported agy xhigh effort is flagged^{"rules":[{"when":"agy work","use":{"harness":"agy","effort":"xhigh"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: agy:xhigh
unsupported agy max effort is flagged^{"rules":[{"when":"agy work","use":{"harness":"agy","effort":"max"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: agy:max
