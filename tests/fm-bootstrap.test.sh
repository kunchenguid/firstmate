...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_QUOTA_AXI_VERSION:-0.1.49}"
  exit 0
fi
exit 0
SH
...
minimum quota-axi version is accepted^0.1.49^empty
newer quota-axi patch is accepted^0.1.50^empty
newer quota-axi minor is accepted^0.2.0^empty
newer quota-axi major is accepted^1.0.0^empty
the patch just below the floor reports an upgrade^0.1.48^missing
