...
test_crew_dispatch_active_rules_are_verbose_bootstrap_info() {
  ...
  mkdir -p "$case_dir/home/config"
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  printf '%s\n' '{"rules":[{"when":"fresh news","use":{"harness":"grok"},"why":"current context"},{"when":"big feature","use":[{"harness":"claude","model":"claude-sonnet-5","effort":"high"},{"harness":"codex","model":"gpt-5.5","effort":"high"}]},{"when":"legacy feature","use":[{"harness":"claude"},{"harness":"codex"}],"select":"quota-balanced"}],"default":[{"harness":"pi","model":"openai-codex/gpt-5.6-sol","effort":"high","codexHome":"/home/example/.codex-3"},{"harness":"grok","model":"grok-4.5","effort":"high"}]}' > "$case_dir/home/config/crew-dispatch.json"
  fakebin=$(make_fake_toolchain "$case_dir")
  add_real_jq "$fakebin"
...
pi codexHome is accepted^{"rules":[{"when":"alternate Codex account","use":{"harness":"pi","model":"openai-codex/gpt-5.6-sol","codexHome":"/home/example/.codex-3"}}]}^empty^
empty codexHome is flagged^{"rules":[{"when":"alternate Codex account","use":{"harness":"pi","codexHome":""}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile codexHome must be a non-empty string when present
non-string codexHome is flagged^{"default":{"harness":"pi-signed","codexHome":3}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - default profile codexHome must be a non-empty string when present
codexHome on another harness is flagged^{"rules":[{"when":"wrong adapter","use":{"harness":"claude","codexHome":"/home/example/.codex-3"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - codexHome is supported only for pi and pi-signed profiles
