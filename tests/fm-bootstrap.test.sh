...
cursor profile without standalone effort is accepted^{"rules":[{"when":"local Cursor work","use":{"harness":"cursor","model":"gpt-5.6-sol-high"}}]}^grep^CREW_DISPATCH: active config/crew-dispatch.json
unsupported cursor standalone effort is flagged^{"rules":[{"when":"local Cursor work","use":{"harness":"cursor","model":"gpt-5.6-sol-high","effort":"high"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: cursor:high
