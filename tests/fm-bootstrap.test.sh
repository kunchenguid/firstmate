...
ROWS
  ...
  printf '%s\n' '{"default":{"harness":"polytoken","model":"codex/gpt-6-luna","effort":"high"}}' > "$case_dir/home/config/crew-dispatch.json"
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  [ -z "$out" ] || fail "no-key bootstrap must accept the verified polytoken worker adapter, got: $out"
