...
assert_fleet_timeout_line() {
  local out=$1 expected_timeout=$2 msg=$3 line elapsed
  line=$(printf '%s\n' "$out" | grep -F "FLEET_SYNC: fleet: skipped: bootstrap refresh timed out (timeout=${expected_timeout}s elapsed=" | head -1)
  [ -n "$line" ] || fail "$msg (missing timeout=${expected_timeout}s line)"$'\n'"--- output ---"$'\n'"$out"
  elapsed=${line##* elapsed=}
  elapsed=${elapsed%s)}
  case "$elapsed" in
    ''|*[!0-9]*) fail "$msg (unparseable elapsed in: $line)" ;;
  esac
  [ "$elapsed" -ge "$expected_timeout" ] || fail "$msg (elapsed $elapsed below timeout $expected_timeout)"
}
...
test_fleet_sync_timeout_scales_with_origin_backed_project_count() {
  ...
  assert_fleet_timeout_line "$out" 59 "bootstrap timeout should scale to 59s for 18 origin-backed projects"
...
test_fleet_sync_timeout_floor_preserves_small_fleets() {
  ...
  assert_fleet_timeout_line "$out" 20 "small fleets should keep the 20s timeout floor"
...
test_fleet_sync_timeout_explicit_override_wins() {
  ...
  assert_fleet_timeout_line "$out" 7 "explicit timeout override should still win over computed default"
...
test_fleet_sync_timeout_empty_override_uses_default() {
  ...
  assert_fleet_timeout_line "$out" 59 "blank timeout env should behave like an unset override"
...
test_fleet_sync_timeout_is_computed_before_launch() {
  ...
  assert_fleet_timeout_line "$out" 20 "launch-order case should still enforce the computed timeout"
