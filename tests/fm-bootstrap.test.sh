...
custom provider profile and rule floor are accepted^{"rules":[{"when":"foundry work","floor":{"scope":"all_models","min_percent":10,"provider":"custom:foundry-ic"},"use":{"harness":"opencode","model":"azure/gpt-5.6-sol","provider":"custom:foundry-ic"}}]}^empty^
...
array profile with malformed model is flagged^{"rules":[{"when":"big feature","use":[{"harness":"codex","model":5}]}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
...
rule floor without provider is flagged^{"rules":[{"when":"hard design","floor":{"scope":"model:fable","min_percent":20},"use":{"harness":"claude"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - rule floor needs scope, min_percent 0..100, and provider matching ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z
rule floor uppercase provider is flagged^{"rules":[{"when":"hard design","floor":{"scope":"model:fable","min_percent":20,"provider":"CLAUDE"},"use":{"harness":"claude"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - rule floor needs scope, min_percent 0..100, and provider matching ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z
rule floor out of range is flagged^{"rules":[{"when":"hard design","floor":{"scope":"model:fable","min_percent":120,"provider":"claude"},"use":{"harness":"claude"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - rule floor needs scope, min_percent 0..100, and provider matching ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z
empty profile provider is flagged^{"rules":[{"when":"images","use":[{"harness":"pi","model":"openai-codex/gpt-5.6-sol","provider":""}]}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
whitespace profile provider is flagged^{"rules":[{"when":"images","use":[{"harness":"pi","model":"openai-codex/gpt-5.6-sol","provider":" claude"}]}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
newline profile provider is flagged^{"rules":[{"when":"images","use":[{"harness":"pi","model":"openai-codex/gpt-5.6-sol","provider":"claude\n"}]}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
empty custom provider slug is flagged^{"rules":[{"when":"foundry","use":{"harness":"opencode","provider":"custom:"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
uppercase custom provider slug is flagged^{"rules":[{"when":"foundry","use":{"harness":"opencode","provider":"custom:Bad"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
other colon provider is flagged^{"rules":[{"when":"foundry","use":{"harness":"opencode","provider":"x:y"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
...
default array malformed effort is flagged^{"default":[{"harness":"codex","effort":3}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - default profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present
...
ROWS
  ...
  out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  [ "$out" = 'CREW_DISPATCH: invalid config/crew-dispatch.json - use profile model and effort must be non-empty strings, and provider must match ^(custom:)?[a-z0-9]+(-[a-z0-9]+)*\z when present' ] \
