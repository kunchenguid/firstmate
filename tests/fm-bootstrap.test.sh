...
test_orca_backend_gates_orca_tool_only_when_selected() {
  ...
  fakebin=$(make_fake_toolchain "$case_dir")
  # A host system may carry an unrelated `orca` binary (GNOME's screen reader
  # ships as /usr/bin/orca on Linux desktops); mask it the same way the
  # missing-git case does so the Orca CLI is genuinely absent here.
