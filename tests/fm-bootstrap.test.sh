...
quality preference fields are accepted^{"quality_preference":{"comfortable_percent":50,"reset_within_hours":72},"default":[{"harness":"pi","model":"openai-codex/gpt-5.6-sol","provider":"codex","prefer_quality":true},{"harness":"codex","model":"gpt-5.6-luna"}]}^empty^
quality preference needs valid thresholds^{"quality_preference":{"comfortable_percent":0,"reset_within_hours":72},"default":{"harness":"codex"}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - quality_preference needs comfortable_percent in (0,100] and reset_within_hours > 0
prefer quality needs policy^{"default":{"harness":"codex","prefer_quality":true}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - prefer_quality profiles need top-level quality_preference
prefer quality only accepts true^{"quality_preference":{"comfortable_percent":50,"reset_within_hours":72},"default":{"harness":"codex","prefer_quality":false}}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - default profile prefer_quality must be true when present
