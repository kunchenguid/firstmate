...
test_announced_update_is_reported_from_the_tool_itself() {
  local home dir out report
  # demo-tool already announces its own update on stderr; read that rather
  # than reimplementing its version lookup.
  home=$(make_home announce)
  dir="$TMP_ROOT/announce/bin"
  mkdir -p "$dir"
  cat > "$dir/demo-tool-fixture" <<'SH'
#!/usr/bin/env bash
printf '1.46.0\n'
printf 'A new version of demo-tool is available: v1.46.0 -> v1.47.0\n' >&2
SH
  chmod 0755 "$dir/demo-tool-fixture"
  write_config "$home" '{"tools":[{"name":"demo-tool","command":"demo-tool-fixture","announce_pattern":"A new version of demo-tool is available: [^ ]+ -> [^ ]+"}]}'
  out="$home/out.txt"
  run_check "$home" "$(fixture_path "$dir")" "$out"
  report=$(cat "$out")
  assert_contains "$report" "demo-tool update available: A new version of demo-tool is available: v1.46.0 -> v1.47.0" "the tool's own update announcement was not reported"
...
test_announcement_is_read_from_a_second_command() {
  ...
  # The real demo-tool prints its version for --version but announces a new
...
#!/usr/bin/env bash
if [ "${1:-}" = "--version" ]; then
  printf 'demo-tool version v1.46.0\n'
  exit 0
fi
printf 'A new version of demo-tool is available: v1.46.0 -> v1.53.0\n' >&2
printf 'Usage: demo-tool <command>\n'
SH
  chmod 0755 "$dir/demo-tool-fixture"
  out="$home/out.txt"

  write_config "$home" '{"tools":[{"name":"demo-tool","command":"demo-tool-fixture","version_args":["--version"],"announce_args":["--help"],"announce_pattern":"A new version of demo-tool is available: [^ ]+ -> [^ ]+"}]}'
  run_check "$home" "$(fixture_path "$dir")" "$out"
  report=$(cat "$out")
  assert_contains "$report" "demo-tool update available: A new version of demo-tool is available: v1.46.0 -> v1.53.0" "the announcement was not read from the command that carries it"
  ...
  write_config "$quiet_home" '{"tools":[{"name":"demo-tool","command":"demo-tool-fixture","version_args":["--version"],"announce_pattern":"A new version of demo-tool is available: [^ ]+ -> [^ ]+"}]}'
...
test_unusable_announce_pattern_is_reported_not_read_as_silence() {
  ...
  make_copy "$dir" demo-tool-fixture 'demo-tool version v1.46.0'
  write_config "$home" '{"tools":[{"name":"demo-tool","command":"demo-tool-fixture","announce_pattern":"A new version of demo-tool is available: ([^ ]+ -> [^ ]+"}]}'
  out="$home/out.txt"
  run_check "$home" "$(fixture_path "$dir")" "$out"
  assert_contains "$(cat "$out")" "demo-tool check failed: announce_pattern is not a usable extended regular expression" "a pattern that cannot be used was read as nothing to announce"
...
test_one_broken_pattern_does_not_blind_the_rest_of_the_sweep() {
  ...
  make_copy "$fresh" "$TOOL" 'herdr 0.8.2'
  make_copy "$dir" demo-tool-fixture 'demo-tool version v1.46.0'
  write_config "$home" "{\"tools\":[{\"name\":\"herdr\",\"command\":\"$TOOL\"},{\"name\":\"demo-tool\",\"command\":\"demo-tool-fixture\",\"announce_pattern\":\"A new version of demo-tool is available: ([^ ]+ -> [^ ]+\"}]}"
  out="$home/out.txt"
  ...
  assert_contains "$report" "demo-tool check failed: announce_pattern is not a usable extended regular expression" "the tool whose pattern cannot be used was not named"
...
SH
  ...
  report=$(cat "$out")
  assert_contains "$report" "demo-tool check failed: the time budget ran out before the update announcement was checked" "an announcement source that was never asked was not reported"
...
test_an_announcement_probe_that_does_not_answer_is_reported() {
  local home dir out report
  # demo-tool learns about a new release from the network, so the command that
...
SH
  ...
  report=$(cat "$out")
  assert_contains "$report" "demo-tool check failed: $dir/demo-tool-fixture did not answer when asked for its update announcement" "an announcement probe that never answered was read as a clean sweep"
...
test_quiet_tool_with_announce_pattern_is_silent() {
  ...
  make_copy "$dir" demo-tool-fixture '1.46.0'
  write_config "$home" '{"tools":[{"name":"demo-tool","command":"demo-tool-fixture","announce_pattern":"A new version of demo-tool is available: [^ ]+ -> [^ ]+"}]}'
  out="$home/out.txt"
  run_check "$home" "$(fixture_path "$dir")" "$out"
