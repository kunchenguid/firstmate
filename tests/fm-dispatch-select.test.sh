...
JQ_BIN=$(command -v jq) || fail "jq is required for dispatch tests"
...
test_rule_array_without_select_invokes_quota_axi() {
  ...
  fakebin=$(dispatch_fakebin "$TMP_ROOT/implicit-command")
  ...
  cat > "$quota" <<'JSON'
{
  ...
  "providers": [
    ...
    {"provider":"cursor","state":{"status":"fresh"},"windows":[{"id":"included_usage","kind":"monthly","percentRemaining":65},{"id":"auto_usage","kind":"monthly","percentRemaining":5},{"id":"spend_limit","kind":"credits","percentRemaining":80}]},
...
JSON
  ...
  out=$(FM_DISPATCH_RANDOM_SOURCE="$RANDOM_ZERO" "$ROOT/bin/fm-dispatch-select.sh" --quota-json "$quota" \
    '[{"harness":"cursor","model":"composer-2.5"},{"harness":"codex"}]' 2>/dev/null)
  assert_profile "$out" '{"harness":"cursor","model":"composer-2.5"}' "explicit Cursor model did not use included Cursor quota"

  out=$(FM_DISPATCH_RANDOM_SOURCE="$RANDOM_ZERO" "$ROOT/bin/fm-dispatch-select.sh" --quota-json "$quota" \
    '[{"harness":"cursor","model":"auto"},{"harness":"claude"}]' 2>/dev/null)
  assert_profile "$out" '{"harness":"claude"}' "Cursor auto did not include its constrained auto-usage window"
  pass "direct and wrapped candidates map to consumed Claude, Codex, Cursor, xAI API, and Grok Build quota"
}
...
test_operational_quota_failures_use_uniform_random_fallback() {
  local fakebin quota
  fakebin=$(dispatch_fakebin "$TMP_ROOT/missing")
  ...
  fakebin=$(dispatch_fakebin "$TMP_ROOT/error")
  cat > "$fakebin/quota-axi" <<'SH'
#!/usr/bin/env bash
exit 42
SH
...
test_single_profile_and_one_element_array() {
  ...
  fakebin=$(dispatch_fakebin "$TMP_ROOT/single")
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" "$ROOT/bin/fm-dispatch-select.sh" \
    '{"harness":"cursor","model":"cursor-grok-4.5-low"}' 2>/dev/null)
  assert_profile "$out" '{"harness":"cursor","model":"cursor-grok-4.5-low"}' \
    "Cursor single profile should preserve its exact model ID"
