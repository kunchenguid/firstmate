...
# fm-watch.sh reclaims that lock ITSELF rather than being handed one an arm
# already cleared. The reclaim still arrives on a published episode:
# fm_lock_try_acquire publishes downtime as part of stealing the watch lock.
# What differs is WHO published it, and the guards below pin that the planted
# lock is still there, unreclaimed and unpublished, when the watcher starts.
start_reclaiming_watcher() {  # <home> <state> <fakebin> <watch-out>
  ...
  [ "$(cat "$state/.watch.lock/pid" 2>/dev/null || true)" = "$dead" ] \
    || fail "the stale-lock reclaim fixture did not plant its lock, so this case proves nothing"
  [ ! -e "$state/.watcher-down" ] \
    || fail "the stale-lock reclaim fixture published a recovery episode, so this case proves nothing"
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_STATE_OVERRIDE="$state" \
    FM_POLL=1 FM_SIGNAL_GRACE=0 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$WATCH" > "$out" &
  ARM_PID=$!
