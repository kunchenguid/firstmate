...
# Attach a real arm to the live cycle. The optional takeover-max lets a fixture
# spend the takeover budget up front so a cycle that delivered nothing still
# exercises the loud typed failure rather than an in-place takeover.
start_attached_arm() {  # <state> <fakebin> <arm-out> <confirm-timeout> [takeover-max]
  local state=$1 fakebin=$2 armout=$3 confirm=$4 takeover_max=${5:-3} i
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_ARM_ATTACH_POLL=0.1 \
    FM_ARM_CONFIRM_TIMEOUT="$confirm" FM_ARM_TAKEOVER_MAX="$takeover_max" "$WATCH_ARM" > "$armout" &
  ARM_PID=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_attached_arm_still_fails_on_a_wake_it_did_not_deliver() {
  ...
  armout="$dir/arm.out"
  start_seed_watcher "$state" "$fakebin" "$out"
  # FM_ARM_TAKEOVER_MAX=0 spends the takeover budget up front: a cycle that
  # delivered nothing would otherwise be repaired by an in-place takeover
  # instead of reaching the loud typed failure this test pins.
  start_attached_arm "$state" "$fakebin" "$armout" 1 0
