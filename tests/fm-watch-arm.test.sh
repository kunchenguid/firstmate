...
# What differs is WHO published it, and that is what the guards below pin, so
# the planted lock and the absent episode are both checked before the watcher
...
start_reclaiming_watcher() {  # <home> <state> <fakebin> <watch-out>
  ...
  [ "$(cat "$state/.watch.lock/pid" 2>/dev/null || true)" = "$dead" ] \
    || fail "the stale-lock reclaim fixture did not plant the dead predecessor's lock"
  [ ! -e "$state/.watcher-down" ] \
    || fail "the stale-lock reclaim fixture published the recovery episode, so this case proves nothing"
  PATH="$fakebin:$PATH" FM_HOME="$home" FM_STATE_OVERRIDE="$state" \
    FM_POLL=1 FM_SIGNAL_GRACE=0 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$WATCH" > "$out" &
  ARM_PID=$!
