...
case "${1:-}" in
  ...
  display-message)
    for a in "$@"; do
      case "$a" in
        *cursor_y*) printf '1\n'; exit 0 ;;
        *pane_tty*) [ "${FM_FAKE_TMUX_ENDPOINT:-}" = dead ] && { printf '\n'; exit 0; } ;;
        *pane_current_command*) [ "${FM_FAKE_TMUX_ENDPOINT:-}" = dead ] && { printf 'zsh\n'; exit 0; } ;;
      esac
    done
    printf 'fakepane\n'; exit 0 ;;
  capture-pane)
    ...
    exit 0 ;;
  list-windows)
    [ "${FM_FAKE_TMUX_ENDPOINT:-}" = missing ] || printf 'fm-t1\n'
    exit 0 ;;
esac
exit 0
SH
...
state=stood-down
EOF
  run_send "$dir" "$err" FM_FAKE_TMUX_ENDPOINT=missing -- t1 "recover this missing endpoint"; rc=$?
