...
const pi = {
  ...
  sendUserMessage() { throw new Error("watcher wakes must not use user-message delivery"); },
};
const mod = await import(pathToFileURL(process.env.EXT).href);
mod.default(pi);
...
if (sent[0].m.customType !== "firstmate-watcher-wake" || sent[0].m.display !== true) throw new Error(`unexpected custom wake shape: ${JSON.stringify(sent[0].m)}`);
...
if ("attribution" in sent[0].m) throw new Error("wake must retain default agent attribution");
if (sent[0].o?.deliverAs !== "followUp" || sent[0].o?.triggerTurn !== true) throw new Error("wake must trigger a follow-up turn");
// The wake is consumed when omp starts its custom message.
await handlers.get("message_start")({ type: "message_start", message: { role: "custom", customType: "firstmate-watcher-wake", content: sent[0].m.content } }, {});
await handlers.get("session_shutdown")({}, {});
...
EOF
  ...
  pass ".omp watch extension: fm_watch_arm_omp arms once, repeats as a no-op, and delivers an actionable close as one custom-message follow-up"
}
