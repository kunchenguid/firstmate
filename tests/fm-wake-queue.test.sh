...
SH
  ...
  # Give the full watcher source graph time to reach each tick under load; the
  # fake clock, not this process deadline, owns the tested no-progress interval.
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NOW_FILE="$dir/now" FM_HOME="$dir" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    "$ROOT/bin/fm-watch-checkpoint.sh" --seconds 3 > "$dir/watch-first.out" 2> "$dir/watch-first.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NOW_FILE="$dir/now" FM_HOME="$dir" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    FM_SECONDMATE_WAKE_STALL_SECS=1 FM_POLL=1 FM_SIGNAL_GRACE=0 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$ROOT/bin/fm-watch-checkpoint.sh" --seconds 3 > "$dir/watch-progress.out" 2> "$dir/watch-progress.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NOW_FILE="$dir/now" FM_HOME="$dir" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    FM_SECONDMATE_WAKE_STALL_SECS=1 FM_POLL=1 FM_SIGNAL_GRACE=0 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$ROOT/bin/fm-watch-checkpoint.sh" --seconds 3 > "$out" 2> "$dir/watch-stalled.err" || true
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NOW_FILE="$dir/now" FM_HOME="$dir" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    FM_SECONDMATE_WAKE_STALL_SECS=1 FM_POLL=1 FM_SIGNAL_GRACE=0 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$ROOT/bin/fm-watch-checkpoint.sh" --seconds 3 > "$dir/watch-next.out" 2> "$dir/watch-next.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" FM_FAKE_NOW_FILE="$dir/now" FM_HOME="$dir" FM_ROOT_OVERRIDE="$ROOT" \
    ...
    FM_SECONDMATE_WAKE_STALL_SECS=1 FM_POLL=1 FM_SIGNAL_GRACE=0 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 \
    "$ROOT/bin/fm-watch-checkpoint.sh" --seconds 3 > "$dir/watch-refrozen.out" 2> "$dir/watch-refrozen.err" || true
...
test_unconsumable_rows_are_retired_instead_of_wedging_the_queue() {
  ...
  # A branch actor never repairs or claims from a malformed queue. Main owns
  # retirement, and branch publication fails closed until that repair happens.
  FM_STATE_OVERRIDE="$state" "$GRANT" activate "$$" retire-scope || fail "branch owner activation failed"
  rc=0
  FM_STATE_OVERRIDE="$state" "$GRANT" publish retire-scope 1 > "$dir/branch.out" 2> "$dir/branch.err" || rc=$?
  [ "$rc" -ne 0 ] || fail "branch grant accepted a malformed queue"
  [ ! -e "$state/.branch-eligible-rows" ] || fail "malformed rows received a branch grant snapshot"
  [ "$(awk 'END { print NR }' "$state/.wake-queue")" -eq 3 ] \
    || fail "a refused branch grant changed the malformed queue"
...
test_extra_field_row_cannot_be_granted_or_consumed() {
  ...
  state="$dir/state"
  printf '1700000000\t1\tsignal\ttask-a.status\tsignal: task-a.status\textra\n' > "$state/.wake-queue"
  before="$dir/before"
