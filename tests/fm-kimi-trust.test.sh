...
# The store is $HOME/.kimi-code and nothing in firstmate reads KIMI_CODE_HOME,
# but Kimi itself does, so an ambient value is dropped rather than left looking
# like it selects a fixture's store: each case's isolation is its own HOME.
...
# make_case <name>: a project with one linked worktree plus a fake HOME carrying
# its own .kimi-code, so nothing resolves to the operator's own store. Echoes
...
# run_trust <fake-home> <args...>: invoke against an isolated store. The store is
# $HOME/.kimi-code, so the fixture home is the whole isolation.
run_trust() {
  local fake_home=$1
  shift
  HOME="$fake_home" "$TRUST" "$@" 2>&1
}
...
test_worktree_is_registered_with_the_vendor_record_shape() {
  ...
  out=$(run_trust "$FAKE_HOME" "$WT" "$PROJ")
...
test_a_symlinked_argument_registers_the_resolved_path() {
  ...
  ln -s "$WT" "$link"
  out=$(run_trust "$FAKE_HOME" "$link" "$PROJ")
...
test_seeded_secondmate_homes_are_registered() {
  ...
  out=$(run_trust "$fake_home" --secondmate-home "$home" n1)
  ...
  out=$(run_trust "$fake_home" --secondmate-home "$leased" n2)
