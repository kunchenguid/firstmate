...
workflow_contract() {
  ...
  assert_grep "if: always() && needs.behavior-test-plan.outputs.mode == 'full'" "$CI" \
    "the full-suite completeness proof is not confined to full mode"
  ...
  pass "CI runs focused test and lint jobs for owned Azure service diffs and preserves full fallback"
}
