...
#
# Durability: this library puts a small git wrapper early on PATH so
# hook-bearing subcommands (commit/push/merge/...) send their stdout to stderr
# by default. Instruments stay visible; command-substitution path capture
# cannot pick them up. New tests can keep writing the obvious `dir=$(helper)`
# form and stay correct.
#
...
#!$FM_TEST_BASH
# Generated by tests/lib.sh - relocates hook-bearing git stdout to stderr.
set -u
...
case "\$subcmd" in
  commit|push|merge|am|rebase|cherry-pick|pull)
    ...
    # stdout -> stderr: hooks and porcelain stay human-visible, never join a
    # caller's command-substitution capture of a fixture path.
    exec "\$real" "\$@" 1>&2
    ;;
  *)
    exec "\$real" "\$@"
...
fm_test_tmproot() {
  ...
  # Trap already installed at library load for the git wrap dir.
