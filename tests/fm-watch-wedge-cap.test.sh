...
test_wedge_cap_validates_invalid_override() {
  ...
  # v20 (2026-09-24): FM_WEDGE_MAX_ESCALATIONS=0 is now VALID and means
  # "cap disabled" (the unconfigured path). No warning logged, no cap fires
  # - every escalation produces a wake. The v18 anti-pattern where 0 fell
  # back to 10 with a warning is gone: 0 is the explicit opt-out, not a
  # bad config.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_STALE_ESCALATE_SECS=999 FM_POLL=1 FM_SIGNAL_GRACE=1 \
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || true
  # v20: 0 is valid (cap disabled), so no fallback warning. Assert no
  # 'falling back' warning for the 0 case specifically.
  if grep -F "FM_WEDGE_MAX_ESCALATIONS='0'" "$state/.watch-triage.log" 2>/dev/null | grep -F "falling back" >/dev/null; then
    fail "FM_WEDGE_MAX_ESCALATIONS=0 should be valid (cap disabled), not produce a 'falling back' warning"
  fi
  grep -F "PERMANENTLY-WEDGED" "$out" >/dev/null && fail "FM_WEDGE_MAX_ESCALATIONS=0 should NOT fire PERMANENTLY-WEDGED (cap is disabled when 0)"
  ...
  unset FM_FAKE_CREW_STATE
  pass "FM_WEDGE_MAX_ESCALATIONS=0 means cap disabled (no warning, no cap fires); FM_CAP_HORIZON_SECS rejects 0 and non-integer; FM_WEDGE_MAX_ESCALATIONS rejects non-integer (v20 semantics: opt-in by setting N>=1)"
}

test_wedge_cap_disabled_when_zero_v20() {
  # v20 (2026-09-24): the unconfigured path is now FM_WEDGE_MAX_ESCALATIONS=0
  # which means "no cap" - the pre-PR behavior, every escalation produces a
  # wake. Captains opt in by setting FM_WEDGE_MAX_ESCALATIONS=N (N>=1).
  # This test pins that semantics: many wedge escalations past the prior
  # default of 10 do NOT write a cap marker, do NOT fire PERMANENTLY-WEDGED,
  # and the escalation counter climbs past 10 unbounded.
  local dir state fakebin out capture_file window key pane_hash sig pid n max marker_a ewf_after
  dir=$(make_case wedge-cap-disabled-v20); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:fm-wedge-cap-disabled-v20"
  printf 'idle wedged content' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/wedge-cap-disabled-v20.meta"
  printf 'working: still wedged\n' > "$state/wedge-cap-disabled-v20.status"
  sig=$(seen_sig "$state/wedge-cap-disabled-v20.status"); printf '%s' "$sig" > "$state/.seen-wedge-cap-disabled-v20_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle wedged content")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  max=15  # way past the prior default of 10 - if the cap were on, this would fire
  marker_a="$state/.wedge-permanent-$key-${pane_hash:0:12}"
  marker_w="$state/.wedge-permanent-$key"
  ...
  # Assert NO cap markers exist (cap was off)
  [ -e "$marker_a" ] && fail "per-hash cap marker written despite FM_WEDGE_MAX_ESCALATIONS=0 (cap should be disabled when 0; v20 semantics)"
  [ -e "$marker_w" ] && fail "window-scoped cap marker written despite FM_WEDGE_MAX_ESCALATIONS=0 (cap should be disabled when 0; v20 semantics)"

  # Assert NO PERMANENTLY-WEDGED wake in any of the rounds
  for f in $(ls -1 "$dir"/round-*-watch.out 2>/dev/null || true); do
    grep -F "PERMANENTLY-WEDGED" "$f" >/dev/null && fail "PERMANENTLY-WEDGED fired in round output despite FM_WEDGE_MAX_ESCALATIONS=0 (cap should be disabled when 0; v20 semantics)"
  done
  # Also check the final watch output as a fallback
  grep -F "PERMANENTLY-WEDGED" "$out" >/dev/null && fail "PERMANENTLY-WEDGED fired in final watch despite FM_WEDGE_MAX_ESCALATIONS=0 (cap should be disabled when 0; v20 semantics)"

  # Assert the escalation counter climbed past 10 (i.e. cap machinery ran, just didn't fire)
  ewf_after=$(cat "$state/.wedge-escalations-$key" 2>/dev/null || true)
  case "$ewf_after" in
    ''|*[!0-9]*) fail "wedge-escalation counter missing or non-integer after $max rounds with FM_WEDGE_MAX_ESCALATIONS=0 (cap machinery not running)" ;;
    *)
      if [ "$ewf_after" -lt "$max" ]; then
        fail "wedge-escalation counter climbed only to $ewf_after (expected $max) with FM_WEDGE_MAX_ESCALATIONS=0; cap machinery is not iterating"
      fi
      ;;
  esac

  unset FM_FAKE_CREW_STATE
  pass "FM_WEDGE_MAX_ESCALATIONS=0 disables the cap: $max escalations, no cap marker, no PERMANENTLY-WEDGED, counter climbed to $ewf_after (v20 opt-in semantics)"
}
