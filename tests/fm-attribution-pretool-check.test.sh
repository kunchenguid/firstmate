...
test_allows_human_messages_and_reads() {
  ...
  expect_allow $'git commit -m "a\n\nCo-authored-by: Jean-Claude <jc@example.fr>"' "a human co-author whose name ends in Claude"
  expect_deny $'git commit -m "a\n\nCo-authored-by: Jean-Claude <noreply@anthropic.com>"' "a Claude-like name with a vendor address"
  expect_allow 'git commit -m "fix(bin): parse the JSON generated by codex exec"' "a subject naming codex output"
  expect_allow 'git commit -m "feat: handle output generated by gemini 2.5"' "a subject naming gemini output"
  expect_allow 'git commit -m "docs: note fixtures are generated with an AI-free script"' "a subject naming a generator"
  ...
  expect_deny $'gh api repos/o/r/pulls/1 -f body="to read, pass -X GET.\n\nGenerated with Claude Code"' \
...
test_message_file_mode() {
  ...
  printf 'fix(bin): parse the JSON generated by codex exec\n\nCo-authored-by: Jean-Claude <jc@example.fr>\n' >"$dir/human"
  "$GUARD" --message-file "$dir/human" >/dev/null 2>&1
  expect_code 0 $? "a subject naming generated output and a human Jean-Claude are not attribution"
  printf 'fix: x\n\n**Generated by Codex CLI**\n' >"$dir/footer"
  "$GUARD" --message-file "$dir/footer" >/dev/null 2>&1
  expect_code 2 $? "a Generated by footer line is attribution"
