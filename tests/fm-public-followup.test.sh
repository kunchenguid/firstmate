...
EOF
  ...
  fm_test_write_active_treehouse_fake "$home/fakebin"
...
test_outcome_text_is_bounded_without_corrupting_characters() {
  ...
  # jq counts codepoints under every locale; ${#text} would count BYTES wherever
  # the suite runs without a UTF-8 locale (e.g. CI), reading 600 é as 1200.
  codepoints=$(jq -r '.public_safe_outcome | length' "$event") \
    || fail "an over-long outcome must still produce valid JSON"
  [ "$codepoints" -le 600 ] || fail "the outcome text was not bounded, got $codepoints characters"
  case "$text" in
...
test_secondmate_teardown_resolves_parent_from_durable_record_when_env_lost() {
  ...
  fm_test_write_active_treehouse_fake "$child/fakebin"
...
test_cleanup_refuses_while_a_public_reply_is_owed() {
  ...
  fm_write_meta "$home/state/ship-task.meta" \
    ...
    "kind=ship" \
    "mode=no-mistakes" \
    "treehouse_lease=lease-ship-task" \
...
test_retention_creates_no_false_teardown_refusal() {
  ...
  fm_write_meta "$home/state/ship-retain.meta" \
    ...
    "kind=ship" \
    "mode=no-mistakes" \
    "treehouse_lease=lease-ship-retain" \
    "treehouse_slot=slot-fixture"
...
test_x_request_teardown_warns_when_final_unposted() {
  ...
  fm_write_meta "$home/state/linked-task.meta" \
    ...
    "kind=ship" \
    "mode=local-only" \
    "treehouse_lease=lease-linked-task" \
    "treehouse_slot=slot-fixture" \
