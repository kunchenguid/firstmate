...
test_claude_busy_signature_uses_real_capture_shapes() {
  ...
  printf 'Working...\n' > "$composer"
  pane_busy omp omp && fail "OMP print-mode output without its TUI esc footer must stay idle"
  printf ' Working… [escape]\n' > "$composer"
  pane_busy omp omp && fail "OMP matcher must require the exact esc footer"
  printf 'The crewmate is Working through the requested change.\n' > "$composer"
  pane_busy omp omp && fail "ordinary Working prose must not classify as OMP busy"
  printf 'Ctrl+c:cancel\n' > "$composer"
