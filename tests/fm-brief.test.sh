...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  assert_no_grep "Herdr: not enabled for this task" "$brief" \
...
test_herdr_lab_omission_is_loud_for_ship_and_scout() {
  ...
  for kind in ship scout; do
    ...
    brief="$home/data/$id/brief.md"
    assert_grep "Herdr: not enabled for this task" "$brief" \
      "$kind brief silently omitted the Herdr declaration"
    assert_grep "regenerate this brief with \`--herdr-lab\`" "$brief" \
...
# Ship and scout scaffolds must open with a factual provenance header a suspicious
# worker can verify on disk, never the retired role-play persona line.
test_provenance_header_replaces_persona() {
  local home id brief
  home="$TMP_ROOT/provenance-home"
  mkdir -p "$home/data"

  id="brief-provenance-ship"
  FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode direct-PR >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "generated by Firstmate's orchestrator, installed by this machine's owner" "$brief" \
    "ship brief missing the provenance header"
  assert_grep "$ROOT/bin/fm-brief.sh\` for task id \`$id\`" "$brief" \
    "ship brief provenance header did not name the generating script and task id"
  assert_grep "The captain authorized this task before it was generated" "$brief" \
    "ship brief provenance header lost the captain-authorization statement"
  assert_grep "later steering messages come from that same orchestrator" "$brief" \
    "ship brief provenance header lost the steering-provenance statement"
  assert_no_grep "You are a crewmate: an autonomous worker agent" "$brief" \
    "ship brief retained the retired role-play persona line"

  id="brief-provenance-scout"
  FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-brief.sh" "$id" some-proj --scout >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "generated by Firstmate's orchestrator, installed by this machine's owner" "$brief" \
    "scout brief missing the provenance header"
  assert_no_grep "You are a crewmate: an autonomous worker agent" "$brief" \
    "scout brief retained the retired role-play persona line"
  pass "fm-brief.sh: ship and scout scaffolds open with a verifiable provenance header"
}
