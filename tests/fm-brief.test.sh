...
# The scaffold primes every worker with the fleet's internal role vocabulary
# while the publication surfaces its guidance governs (commit messages, PR
# text, review comments, issues, files committed to the project) are
# colleague-facing, so without an explicit rule a worker can echo an internal
# role label into published text. Every crewmate brief - all three ship modes
# and both scout accesses - must carry the role-neutral publication rule ahead
# of its task body. The secondmate charter is accounted for rather than
# assumed unchanged: a secondmate publishes only through its private status
# file and through its own crewmates, whose briefs are generated by this same
# script, so the charter itself carries no publication guidance.
test_publish_rule_covers_crewmate_briefs() {
  local home brief heading_line task_line id mode
  home="$TMP_ROOT/publish-rule-home"
  mkdir -p "$home/data"

  for id in brief-pub-ship-nm brief-pub-ship-dpr brief-pub-ship-lo; do
    case "$id" in
      *-nm) mode=no-mistakes ;;
      *-dpr) mode=direct-PR ;;
      *-lo) mode=local-only ;;
    ...
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode "$mode" >/dev/null 2>&1 \
      || fail "fm-brief.sh failed to scaffold the $mode ship brief"
    brief="$home/data/$id/brief.md"
    assert_grep "# What you publish" "$brief" \
      "$mode ship brief omitted the role-neutral publication rule"
    assert_grep "captain, first mate, crewmate, scout, second mate" "$brief" \
      "$mode ship publication rule must name the covered internal role vocabulary"
    ...
    [ -n "$heading_line" ] && [ -n "$task_line" ] && [ "$heading_line" -lt "$task_line" ] \
      || fail "$mode ship brief: publication rule must precede # Task"
  done

  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-pub-scout firstmate --scout >/dev/null 2>&1 \
    || fail "fm-brief.sh failed to scaffold the writer scout brief"
  brief="$home/data/brief-pub-scout/brief.md"
  assert_grep "# What you publish" "$brief" \
    "writer scout brief omitted the role-neutral publication rule (a promoted scout keeps operating from this brief)"
  assert_grep "including a no-mistakes \`--intent\`" "$brief" \
    "writer scout publication rule omitted the promoted no-mistakes intent boundary"

  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-pub-reader firstmate --scout --access reader >/dev/null 2>&1 \
    || fail "fm-brief.sh failed to scaffold the reader scout brief"
  brief="$home/data/brief-pub-reader/brief.md"
  assert_grep "# What you publish" "$brief" \
    "reader scout brief omitted the role-neutral publication rule"
  heading_line=$(grep -n -Fx "# What you publish" "$brief" | head -n 1 | cut -d: -f1)
  task_line=$(grep -n -Fx "# Task" "$brief" | head -n 1 | cut -d: -f1)
  [ -n "$heading_line" ] && [ -n "$task_line" ] && [ "$heading_line" -lt "$task_line" ] \
  ...
  FM_SECONDMATE_CHARTER='Supervise the alpha domain.' \
    ...
    || fail "fm-brief.sh failed to scaffold the secondmate charter"
  brief="$home/data/brief-pub-sm/brief.md"
  assert_no_grep "# What you publish" "$brief" \
    "secondmate charter carries publication guidance it does not need: it publishes only through private status and its own crewmates' generated briefs"
  pass "fm-brief.sh: every crewmate brief carries the role-neutral publication rule; the charter is accounted for"
}
