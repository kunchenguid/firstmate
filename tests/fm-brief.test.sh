...
# Every ship scaffold now has to name the captain order it hangs from and the
# product goal it serves (bin/fm-brief-product-lib.sh owns those flags; their
# own behavior is covered by tests/fm-brief-v2.test.sh). These fixture values
# satisfy that contract so the cases below keep testing what they were written
# for - delivery modes, Herdr isolation, wording - and not the v2 gate.
V2_SHIP=(--order O-0001 --ziel 'fixture goal|some-proj/fixture-anker')
...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    id=${id_mode%%:*}
    mode=${id_mode##*:}
    FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode "$mode" "${V2_SHIP[@]}" >/dev/null 2>&1; status=$?
...
test_ship_mode_is_explicit_not_registry() {
  ...
  write_registry "$home"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-explicit-a5 direct-proj --mode no-mistakes "${V2_SHIP[@]}" >/dev/null 2>&1 \
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-explicit-a6 never-registered --mode local-only "${V2_SHIP[@]}" >/dev/null 2>&1 \
...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" direct-proj --mode direct-PR "${V2_SHIP[@]}" >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" local-proj --mode local-only "${V2_SHIP[@]}" >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
...
test_no_mistakes_dod_wording() {
  ...
  mkdir -p "$home/data"
  id="brief-wording-b1"
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode no-mistakes "${V2_SHIP[@]}" >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes "${V2_SHIP[@]}" --herdr-lab >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
...
test_herdr_lab_omission_is_loud_for_ship_and_scout() {
  ...
  for kind in ship scout; do
    ...
    if [ "$kind" = scout ]; then
      ...
      FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes "${V2_SHIP[@]}" >/dev/null 2>&1
    fi
    brief="$home/data/$id/brief.md"
    ...
    case "$kind" in
      ship)
        FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=awaiting \
          "$ROOT/bin/fm-brief.sh" "$id" firstmate --mode no-mistakes "${V2_SHIP[@]}" >/dev/null 2>&1
        ;;
      scout)
        FM_HOME="$home" FM_CLASSIFY_PAUSED_VERB=awaiting \
...
test_no_agent_coauthor_rule_in_every_scaffold() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" "$id" some-proj --mode local-only "${V2_SHIP[@]}" >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
