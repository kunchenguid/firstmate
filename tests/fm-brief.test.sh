...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    ...
    assert_grep "including its final line when the record has no terminating newline" "$brief" \
      "$id: generated inbox instructions must preserve an unterminated final line"
