...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    ...
    assert_grep "blocked: worktree is not on the base firstmate reset it to" "$brief" \
      "$id: brief missing the base assertion for an ordinary project"
    assert_grep "refs/remotes/origin/main" "$brief" \
      "$id: brief did not name origin's tip, which is the base fm-spawn.sh resets an ordinary project's worktree to"
    assert_no_grep "refs/heads/main" "$brief" \
      "$id: brief demanded the local default branch on a project fm-spawn.sh resets to origin's tip"
...
# The ref a ship brief tells the worker to compare HEAD against, taken from the
# generated brief so the check that runs below is the one the worker is given.
brief_base_ref() {  # <brief>
  # shellcheck disable=SC2016  # single quotes are deliberate: this is a sed script whose backticks and \1 must reach sed verbatim.
  sed -n 's/.*must equal `git rev-parse \([^`]*\)`.*/\1/p' "$1" | head -n 1
}

# bin/fm-spawn.sh resets an ordinary project's task worktree to origin's fetched
# tip and firstmate's own task worktree to the local default branch. A brief
# whose base assertion names the other ref makes the worker refuse a task it was
# correctly launched for, so run the brief's own rev-parse check against both
# real bases instead of trusting the prose.
test_ship_brief_base_check_passes_on_the_base_spawn_leaves() {
  local base home project origin publisher worktree fmrepo fmorigin fmpub fmwt
  local brief ref head expected local_main

  fm_git_identity
  base="$TMP_ROOT/base-assert"
  home="$base/home"
  project="$base/project"
  origin="$base/origin.git"
  publisher="$base/publisher"
  worktree="$base/worktree"
  mkdir -p "$base" "$home/data"

  # Ordinary project: origin advances past the clone's local main between
  # fleet-sync and spawn, so the two refs genuinely disagree.
  ...
  git -C "$publisher" commit -qm advance-main
  git -C "$publisher" push --quiet origin main
  git -C "$project" fetch --quiet origin
  git -C "$project" worktree add --quiet --detach "$worktree" refs/remotes/origin/main
  head=$(git -C "$worktree" rev-parse HEAD)
  local_main=$(git -C "$worktree" rev-parse refs/heads/main)
  [ "$head" != "$local_main" ] \
    || fail "fixture did not make the spawn base differ from the clone's local default branch"

  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" base-ordinary "$project" --mode no-mistakes >/dev/null 2>&1 \
    || fail "scaffolding a brief for an ordinary project failed"
  brief="$home/data/base-ordinary/brief.md"
  ref=$(brief_base_ref "$brief")
  [ -n "$ref" ] || fail "ordinary-project brief named no base ref for the worker to verify"
  expected=$(git -C "$worktree" rev-parse "$ref" 2>/dev/null) \
    || fail "ordinary-project brief named '$ref', which the task worktree cannot resolve"
  [ "$head" = "$expected" ] \
    || fail "ordinary-project brief would block the worker: HEAD $head is not $ref ($expected)"

  # Firstmate on itself: local main is the fleet's tree and origin/main carries
  # an unreviewed parent commit that must never become a branch base.
  fmrepo="$base/fmrepo"
  fmorigin="$base/fmorigin.git"
  fmpub="$base/fmpublisher"
  fmwt="$base/fmworktree"
  fm_git_init_commit "$fmrepo"
  git -C "$fmrepo" branch -M main
  mkdir -p "$fmrepo/data"
  git clone --quiet --bare "$fmrepo" "$fmorigin"
  git -C "$fmrepo" remote add origin "file://$fmorigin"
  git clone --quiet "file://$fmorigin" "$fmpub"
  printf 'unreviewed-parent\n' > "$fmpub/upstream.txt"
  git -C "$fmpub" add upstream.txt
  git -C "$fmpub" commit -qm 'unreviewed parent commit'
  git -C "$fmpub" push --quiet origin main
  printf 'local-authoritative\n' > "$fmrepo/local.txt"
  git -C "$fmrepo" add local.txt
  git -C "$fmrepo" commit -qm 'local main is authoritative'
  git -C "$fmrepo" fetch --quiet origin
  git -C "$fmrepo" worktree add --quiet --detach "$fmwt" refs/heads/main
  head=$(git -C "$fmwt" rev-parse HEAD)
  [ "$head" != "$(git -C "$fmwt" rev-parse refs/remotes/origin/main)" ] \
    || fail "firstmate fixture did not diverge origin/main from local main"

  FM_HOME="$fmrepo" "$ROOT/bin/fm-brief.sh" base-firstmate "$fmrepo" --mode no-mistakes >/dev/null 2>&1 \
    || fail "scaffolding a brief for firstmate's own repository failed"
  brief="$fmrepo/data/base-firstmate/brief.md"
  ref=$(brief_base_ref "$brief")
  [ -n "$ref" ] || fail "firstmate brief named no base ref for the worker to verify"
  expected=$(git -C "$fmwt" rev-parse "$ref" 2>/dev/null) \
    || fail "firstmate brief named '$ref', which the task worktree cannot resolve"
  [ "$head" = "$expected" ] \
    || fail "firstmate brief would block the worker: HEAD $head is not $ref ($expected)"
  case $ref in
    refs/heads/*) ;;
    *) fail "firstmate brief accepted a remote-derived base ref '$ref'" ;;
  esac

  pass "fm-brief: the ship base assertion passes on the base fm-spawn.sh actually leaves"
}
