...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  # happened): a branch with a commit, a follow-up fix commit pushed on
...
# --- Fix 2: reap leaked descendant processes rooted under the task's own
# worktree/tasktmp - exercised through the real teardown path
# (bin/fm-teardown.sh), never by matching its source text. ------------------

# Land a shippable commit on the task branch and push it to origin, so these
# cases exercise the reap step on a real successful teardown rather than a
# refusal path.
