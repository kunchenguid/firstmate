...
seed_backlog_in_flight() {
  ...
  # Force markdown: an ambient repo .tasks.toml may select beads and its
  # prefix rules, which would make bare tasks-axi add of task-x1 fail silently
  # under >/dev/null and leave the fixture with an empty backlog.
  TASKS_AXI_BACKEND=markdown tasks-axi add task-x1 "teardown fixture task" --kind "$kind" \
    --file "$case_dir/data/backlog.md" >/dev/null \
    || fail "could not seed teardown fixture backlog item task-x1"
  TASKS_AXI_BACKEND=markdown tasks-axi start task-x1 --file "$case_dir/data/backlog.md" >/dev/null \
    || fail "could not start teardown fixture backlog item task-x1"
}

backlog_row_state() {
  local case_dir=$1
  TASKS_AXI_BACKEND=markdown tasks-axi show task-x1 --file "$case_dir/data/backlog.md" 2>/dev/null |
