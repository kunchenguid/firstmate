...
test_clean_task_container_prints_no_warning() {
  ...
  pass "teardown stays silent when no task container is leaked"
}
...
test_docker_absent_skips_container_warning_silently() {
  ...
  rc=0
  PATH="$path_without_docker:$case_dir/fakebin" run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
