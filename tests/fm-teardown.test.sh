...
#   (q2) direct-PR + squash-merged, local followed branch rebase -> ALLOW
#   (q3) direct-PR + squash-merged, same file, different content   -> REFUSE
#   (q4) direct-PR + squash-merged rebased local plus extra commit -> REFUSE
...
# Squash-merged history whose branch rebased onto a newer main that
...
test_direct_pr_origin_remote_allows() {
  ...
  expect_code 0 "$rc" "direct-pr-origin: teardown should succeed when HEAD is on origin"
...
test_direct_pr_truly_unpushed_refuses() {
  ...
  grep -q REFUSED "$case_dir/stderr" || fail "direct-pr-unpushed: no REFUSED line in stderr"
  pass "direct-PR worktree with genuinely unlanded work is refused (safety preserved)"
}

# A removed or invalid persisted ship mode must refuse before endpoint or
# worktree cleanup, even when the work itself is safely landed.
test_invalid_persisted_ship_mode_refuses_before_cleanup() {
  local case_dir mode rc head
  for mode in no-mistakes no-mistakes-prod-only invalid; do
    case_dir=$(make_case "invalid-ship-mode-$mode")
    write_meta "$case_dir" "$mode" ship
    land_shippable_commit "$case_dir"
    head=$(git -C "$case_dir/wt" rev-parse HEAD)
    cp "$case_dir/state/task-x1.meta" "$case_dir/meta-before"
    printf 'working: retain this lifecycle record\n' > "$case_dir/state/task-x1.status"
...
#!/usr/bin/env bash
case "\${1:-}" in
  kill-window|kill-pane|kill-session) printf 'endpoint cleanup\n' >> "$case_dir/cleanup.log" ;;
esac
exit 0
EOF
    ...
    [ "$rc" -ne 0 ] || fail "$mode: invalid persisted ship mode was accepted"
    assert_refusal_retained_task_state "$case_dir" "$mode" "$head"
    cmp -s "$case_dir/meta-before" "$case_dir/state/task-x1.meta" \
      || fail "$mode: refusal modified task metadata"
    cmp -s "$case_dir/status-before" "$case_dir/state/task-x1.status" \
      || fail "$mode: refusal modified the lifecycle record"
    assert_absent "$case_dir/cleanup.log" "$mode: refusal performed lifecycle cleanup"
  done
  pass "invalid persisted ship modes refuse before lifecycle cleanup"
}

# A missing legacy mode retains direct-PR landed-work safety: unlanded work
# refuses, while a matching merged PR permits cleanup without remote reachability.
test_missing_ship_mode_preserves_landed_work_protection() {
  local case_dir rc head
  case_dir=$(make_case missing-ship-mode)
  fm_write_meta "$case_dir/state/task-x1.meta" \
    "window=firstmate:fm-task-x1" \
    "endpoint_task_id=task-x1" \
    "worktree=$case_dir/wt" \
  ...
  rc=0
  run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] || fail "missing mode bypassed ship landed-work protection"
  assert_refusal_retained_task_state "$case_dir" missing-ship-mode "$head"

  append_pr_meta_for_current_head "$case_dir"
  add_gh_pr_merged_for_head "$case_dir" "$head"
  rc=0
  run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  expect_code 0 "$rc" "missing mode should accept a matching merged PR"
  assert_absent "$case_dir/state/task-x1.meta" "completed teardown retained task metadata"
  pass "missing ship mode preserves direct-PR landed-work protection"
}
...
test_squash_merged_pr_allows_when_head_ancestor_of_pr_head() {
  ...
  local_head=$(git -C "$case_dir/wt" rev-parse HEAD)
  pr_head=$(commit_tree_from_wt_head "$case_dir" "$local_head" "review follow-up")
  add_gh_pr_merged_for_head "$case_dir" "$pr_head"
...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  # happened): a branch with a commit, a review-fix commit pushed on
  ...
  wt_commit_file "$case_dir" feature.txt hello "add feature"
  local_head=$(git -C "$case_dir/wt" rev-parse HEAD)
  pr_head=$(commit_tree_from_wt_head "$case_dir" "$local_head" "review fix")
...
test_squash_merged_rebased_branch_allows() {
  ...
  expect_code 0 "$rc" "squash-rebased: teardown should succeed when the worktree followed the branch rebase"$'\n'"$(cat "$case_dir/stderr")"
  ! grep -q REFUSED "$case_dir/stderr" || fail "squash-rebased: teardown printed a REFUSED line"
  pass "squash-merged task whose local branch followed the rebase is torn down"
}
...
test_squash_merged_same_file_different_content_refuses() {
  ...
  write_meta "$case_dir" direct-PR ship
  # The branch rebase produced a different blob for shared.txt than the stale
...
# The local branch followed the rebase, so without later.txt this is the
...
test_legacy_record_without_the_flag_refuses() {
  ...
  write_legacy_meta "$case_dir" direct-PR ship
  seed_backlog_in_flight "$case_dir"
...
test_secondmate_pr_registration_publishes_ready_line() {
  ...
  assert_grep "done [key=child-pr-task-x1]: child task-x1 PR ready: $url mode=direct-PR" "$channel" \
...
# Reap leaked descendants rooted under the task's own worktree or tasktmp.
# Land the work first so these cases exercise successful teardown.
