...
test_teardown_removes_submit_disagreement_record() {
  local case_dir rc
  case_dir=$(make_case submit-disagreement-cleanup)
  write_meta "$case_dir" local-only ship
  printf 'v1 ts=1 harness=claude semantic=turn-opened composer=pending action=downgraded\n' \
    > "$case_dir/state/task-x1.submit-disagreement"
  ...
  expect_code 0 "$rc" "submit-disagreement-cleanup: forced teardown failed"
  assert_absent "$case_dir/state/task-x1.submit-disagreement" \
    "teardown left the task's durable submit-confirm disagreement evidence behind"
  pass "teardown removes the semantic submit-confirm disagreement record"
}
