...
# The load-bearing case: a NESTED SUBSHELL's own cd. Verified real finding
# (docs/cmux-backend.md finding #2): current_directory stays frozen at wherever
# the surface's shell was when it launched the subshell as a foreground
# command - it never follows the subshell's own cd. Seeing through that is the
# whole point of fm_backend_cmux_current_path's active pwd-probe, so it must be
...
pass "real cmux: current_path tracks a NESTED SUBSHELL's own cd (the case a bare cwd read cannot see)"
