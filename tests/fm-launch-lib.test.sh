...
test_ship_and_scout_templates_are_pinned() {
  ...
  for kind in ship scout; do
    ...
    assert_template "$kind" pi 'FM_PI_HARNESS=pi __PIBIN____PITUIMODE__ __MODELFLAG____EFFORTFLAG__-e __PIEXT__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    assert_template "$kind" pi-signed 'FM_PI_HARNESS=pi-signed __PIBIN____PITUIMODE__ __MODELFLAG____EFFORTFLAG__-e __PIEXT__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
...
test_secondmate_templates_are_pinned() {
  ...
  assert_template secondmate pi 'FM_PI_HARNESS=pi __PIBIN____PITUIMODE__ __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
  # shellcheck disable=SC2016
  assert_template secondmate pi-signed 'FM_PI_HARNESS=pi-signed __PIBIN____PITUIMODE__ __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
  local h
