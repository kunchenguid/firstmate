...
test_outcome_startup_replay_preserves_silence() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-branch-outcome.sh" append \
    --task task-a --verdict routine --summary 'task steady, no action taken' --silent true >/dev/null \
  ...
  assert_not_contains "$replay" "task steady, no action taken" "startup replay printed a silent task-scoped outcome"
