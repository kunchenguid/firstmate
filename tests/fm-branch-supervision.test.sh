...
test_branch_prompt_is_byte_stable_and_above_cache_floor() {
  ...
  case "$out_a" in
    *"# PR identity: copy or abstain"*"copied verbatim from the task's \`done: published; PR <url>\` or \`done: green-CI; PR <url> checks green\` status line, a legacy \`done: PR <url> ...\` line, or its \`pr=\` metadata field"*"Never assemble an owner, repository, host, or number"*"report the identifier you do have"*) ;;
