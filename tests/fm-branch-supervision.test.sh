...
test_branch_prompt_is_byte_stable_and_above_cache_floor() {
  ...
  case "$out_a" in
    *"Report verdict captain for the finished result of work the captain requested, even when that result is healthy."*"A start or still-working update on requested work that brings no new artifact, finding, or decision is verdict routine."*"Keep an unsolicited routine outcome as verdict routine"*"Keep an unchanged fleet review silent"*"A follow-up that adds no new captain-facing fact after a terminal captain-facing outcome already reported for that task still gets a report, with silent true so it is not captain-visible."*"never internal mechanics like wake kinds, status prefixes, worktrees, close-out, teardown, or state file names."*) ;;
...
test_outcome_startup_replay_preserves_silence() {
  ...
  assert_contains "$out" "silent outcomes must be routine" "silent captain refusal lost its diagnostic"
  [ ! -e "$store" ] || fail "refused silent captain outcome changed the durable store"

  FM_HOME="$home" "$ROOT/bin/fm-branch-outcome.sh" append \
    --task task-a --verdict routine --summary 'worker idle after the ready PR' --silent true >/dev/null \
    || fail "silent task-scoped follow-up append failed"
  FM_HOME="$home" "$ROOT/bin/fm-branch-outcome.sh" append \
  ...
  assert_not_contains "$replay" "worker idle after the ready PR" "startup replay printed a silent task-scoped follow-up"
