...
make_originless_case() {  # <name> <id> [advance-primary] [default-branch]
  local name=$1 id=$2 advance=${3:-0} default=${4:-main} case_dir home project pool fakebin initial
  case_dir="$TMP_ROOT/$name"
  home="$case_dir/home"
  project="$case_dir/project"
  pool="$case_dir/pool"
...
test_originless_pool_refreshes_non_main_default_branch() {
  local rec id out status current branch_head
  id='pool-originless-trunk-stale-r1'
  rec=$(make_originless_case originless-trunk-stale "$id" 1 trunk)
  read_case_record "$rec"
  ...
  status=$?
  expect_code 0 "$status" \
    "spawn should refresh an origin-less pool from a non-main primary branch"$'\n'"$out"
  assert_contains "$out" "spawned $id" "spawn did not report success for the origin-less trunk pool"
  branch_head=$(git -C "$POOL_DIR" rev-parse HEAD)
  [ "$branch_head" = "$current" ] || fail "spawn left the origin-less pool on stale non-main history"
  assert_grep 'originless advanced main' "$POOL_DIR/advanced-local.txt" \
    "the refreshed origin-less trunk pool omitted the advanced content"
  pass "a stale origin-less pool refreshes from a non-main primary default branch"
}
