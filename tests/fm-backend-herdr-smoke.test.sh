...
pass "real herdr: container_ensure starts the isolated session's server, creates the FirstMate Crew workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing FirstMate Crew workspace, reports no seeded default tab on adoption)"
