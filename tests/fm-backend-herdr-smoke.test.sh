...
pass "real herdr: container_ensure starts the isolated session's server, creates the primary 'fm' workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing primary 'fm' workspace, reports no seeded default tab on adoption)"
...
POST_PRIMARY_ID=$(printf '%s' "$POST_LIST" | jq -r '.result.workspaces[]? | select(.label == "fm") | .workspace_id')
