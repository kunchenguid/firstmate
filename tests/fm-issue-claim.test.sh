...
# logs every call so read-only and corpus-reuse behavior can be asserted. An
...
JSON
  ...
  # 300: fixed on main, visible only to a --symbol search.
  ...
  put "repos/o/r/issues/400/timeline?per_page=100" '[
    {"event":"cross-referenced","source":{"issue":{"number":70,"state":"closed","title":"part one","user":{"login":"dan"},"repository":{"full_name":"o/r"},"pull_request":{"merged_at":"2026-01-05T00:00:00Z"}}}},
...
test_history_cite_and_symbol_fix_on_main() {
  local out rc=0
  out=$(run_claim --symbol 300:magic_symbol 200 300) || rc=$?
  expect_code 0 "$rc" "fixed-on-main verdicts"
  assert_contains "$(block_of "$out" 200)" "verdict: fixed-on-main" "a main commit citing the issue is merged evidence"
  ...
  assert_contains "$(block_of "$out" 300)" "verdict: fixed-on-main" "a --symbol pickaxe hit is merged evidence"
  assert_contains "$(block_of "$out" 300)" "merged: commit $FIX300_SHA" "the removing commit is named"
  ...
  assert_equals 1 "$(block_of "$out" 300 | grep -c '^merged: ')" "only the commit removing the symbol matches"
  pass "history finds citing commits and caller symbols since the issue opened"
}
...
test_fork_branch_heading_a_merged_pr() {
  ...
  assert_contains "$block" "verdict: fixed-on-main" "a fork branch whose PR merged is merged evidence"
  assert_contains "$block" "merged: PR #80 merged" "the branch resolves to its PR"
  assert_not_contains "$block" "no PR found" "the branch is not a separate unlinked claim"
  pass "a fork branch resolves to the PR it heads"
}
...
test_corpus_is_fetched_once_and_reusable() {
  local out saved="$TMP_ROOT/corpus.json" n
  ...
  : > "$FIX/calls.log"
  PATH="$FAKEBIN:$PATH" "$SCRIPT" --repo o/r --save-corpus "$saved" >/dev/null \
    || fail "saving a corpus was refused"
  jq -e '.schema == "fm-issue-claim-corpus.v1" and .fetched == 5 and .total_count == 5' "$saved" >/dev/null \
    || fail "the saved corpus does not carry its counts"
  out=$(run_claim --corpus "$saved" 4018)
  n=$(grep -c 'pulls?state=open\|is_pr\|is:pr' "$FIX/calls.log" || true)
  assert_equals 0 "$n" "a reused corpus is not refetched"
  assert_contains "$out" "claim: PR #52" "a reused corpus still matches"
  assert_contains "$out" "open-PR corpus reused from" "a reused corpus discloses its age"

  jq '.total_count = 99' "$saved" > "$saved.short"
  out=$(run_claim --corpus "$saved.short" 100) || true
  assert_contains "$out" "corpus=truncated(5/99)" "a reused short corpus stays short"

  printf '{"schema":"fm-issue-claim-corpus.v1","repo":"x/y","fetched":0,"prs":[]}\n' > "$saved.other"
  local rc=0
  run_claim --corpus "$saved.other" 100 >/dev/null 2>&1 || rc=$?
  expect_code 2 "$rc" "a corpus for another repository is refused"
  pass "the corpus is fetched once per run and reusable across runs"
}
...
test_sweep_dispositions() {
  ...
  assert_contains "$out" "sweep: #4018 leave-open state=open verdict=claimed coverage=complete link=#52" \
    "an unlinked body-citing PR is a link candidate and the issue stays open"
  ...
  assert_contains "$out" "sweep: #200 close-candidate state=open verdict=fixed-on-main coverage=complete link=$FIX200_SHA" \
    "a citing commit is a close and link candidate"
  assert_contains "$out" "sweep: #300 close-candidate state=open verdict=fixed-on-main coverage=complete link=-" \
    "a symbol-only fix is a close candidate but never a link candidate"
