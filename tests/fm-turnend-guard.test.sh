...
install_guard_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/fm-primary-scope.sh" "$dir/bin/fm-primary-scope.sh"
  ...
  chmod +x "$dir/bin/fm-primary-scope.sh" "$dir/bin/fm-turnend-guard.sh" "$dir/bin/fm-turnend-guard-grok.sh" "$dir/bin/fm-supervision-instructions.sh" "$dir/bin/fm-harness.sh"
}
...
test_pi_extension_forces_followup() {
  ...
  assert_contains "$content" 'spawnSync(lockScript, ["ownership"]' "pi extension must use the lock owner's read-only API"
  assert_not_contains "$content" 'function parentPid' "pi extension kept a duplicate ancestry classifier"
  assert_not_contains "$content" 'function pidAlive' "pi extension kept a duplicate holder-liveness classifier"
  assert_not_contains "$content" "readFileSync(\`\${state}/.lock\`" "pi extension still classifies lock bytes itself"
