...
cmux_expected_identity_hash() {  # <identity>
  local identity real
  identity=$1
  real=$(cd "$identity" && pwd -P) || return 1
...
cmux_expected_home_label() {  # [home] [identity]
  local home=${1:-$ROOT} identity=${2:-$ROOT} marker id prefix
  ...
  printf '%s-%s' "$prefix" "$(cmux_expected_identity_hash "$identity")"
}

cmux_expected_scoped_title() {  # <fm-task-label> [home] [identity]
  local label=$1 home=${2:-$ROOT} identity=${3:-$ROOT} rest
  case "$label" in
    fm-*) rest=${label#fm-} ;;
    *) rest=$label ;;
  esac
  printf 'fm-%s-%s' "$(cmux_expected_home_label "$home" "$identity")" "$rest"
}
...
test_scoped_title_uses_primary_home_label() {
  ...
  pass "fm_backend_cmux_scoped_title: scopes a primary task title with firstmate plus identity hash"
}
...
test_scoped_title_uses_secondmate_home_label() {
  ...
  pass "fm_backend_cmux_scoped_title: scopes a secondmate task title with the home marker plus identity hash"
}

test_scoped_title_tracks_home_identity_not_code_root() {
  local dir home root_one root_two identity_one identity_two out_one out_relocated out_two expected_one expected_two
  dir="$TMP_ROOT/scoped-title-identity-hash"; home="$dir/home"; root_one="$dir/root-one"; root_two="$dir/root-two"
  identity_one="$dir/identity-one"; identity_two="$dir/identity-two"
  mkdir -p "$home" "$root_one" "$root_two" "$identity_one" "$identity_two"
  expected_one=$(cmux_expected_scoped_title fm-task1 "$home" "$identity_one")
  expected_two=$(cmux_expected_scoped_title fm-task1 "$home" "$identity_two")
  out_one=$( FM_HOME="$home" FM_HOME_IDENTITY="$identity_one" FM_ROOT_OVERRIDE="$root_one" bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_scoped_title fm-task1' "$ROOT" )
  out_relocated=$( FM_HOME="$home" FM_HOME_IDENTITY="$identity_one" FM_ROOT_OVERRIDE="$root_two" bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_scoped_title fm-task1' "$ROOT" )
  out_two=$( FM_HOME="$home" FM_HOME_IDENTITY="$identity_two" FM_ROOT_OVERRIDE="$root_two" bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_scoped_title fm-task1' "$ROOT" )
  [ "$out_one" = "$expected_one" ] || fail "scoped title should include identity-one hash as $expected_one, got '$out_one'"
  [ "$out_relocated" = "$expected_one" ] || fail "scoped title changed when only the installed code root relocated"
  [ "$out_two" = "$expected_two" ] || fail "scoped title should include identity-two hash as $expected_two, got '$out_two'"
  [ "$out_one" != "$out_two" ] || fail "scoped titles should differ for distinct FM_HOME_IDENTITY paths"
  pass "fm_backend_cmux_scoped_title: tracks home identity independently of the installed code root"
}
