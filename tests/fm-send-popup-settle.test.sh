...
# for a leading `$<skill>` invocation (e.g. `$afk`). Submitting before the
...
first_settle 1.2 'codex $skill -> long settle' codex '$afk'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$afk'

# The `/` slash case stays universal and unchanged: long settle regardless of
# harness (here a non-codex claude target).
first_settle 1.2 'claude /command -> long settle (slash unchanged)' claude '/afk'
