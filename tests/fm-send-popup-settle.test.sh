...
# for a leading `$<skill>` invocation (e.g. `$compact`). Submitting before the
...
first_settle 1.2 'codex $skill -> long settle' codex '$compact'
...
first_settle 1.2 'codex $skill exact task id -> long settle' codex '$compact' exact

# Same `$` message to claude keeps the fast path: `$` is ordinary text there.
first_settle 0.3 'claude $-message -> fast path' claude '$compact'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$compact'
