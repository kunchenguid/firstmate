...
test_spawn_template_mentions_pi_watch_placeholder() {
  ...
  assert_contains "$text" "pi --no-extensions --no-skills --no-prompt-templates __MODELFLAG____EFFORTFLAG__-e __PITURNEND__ -e __PIWATCH__" \
    "Pi secondmate launch template is not hermetic or does not include both primary extensions"
  assert_not_contains "$text" "pi --no-context-files" "Pi launch template must preserve project context-file discovery"
