...
test_tracked_extension_present_and_self_hashing() {
  ...
  assert_contains "$text" "[ -f \\\"\$config_dir/telegram-mode.env\\\" ] && . \\\"\$config_dir/telegram-mode.env\\\"" "tracked extension does not source the effective Telegram config"
...
test_opencode_primary_watch_plugin_sources_effective_config() {
  ...
  printf 'export FM_HEARTBEAT=8\n' > "$home/config/telegram-mode.env"
  cat > "$repo/bin/fm-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'poll=%s\n' "${FM_POLL:-missing}" >> "${FM_ARM_LOG:?}"
printf 'heartbeat=%s\n' "${FM_HEARTBEAT:-missing}" >> "${FM_ARM_LOG:?}"
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" FM_HOME="$home" FM_ARM_LOG="$log" node 2>&1 <<'EOF'
...
const text = readFileSync(process.env.FM_ARM_LOG, "utf8");
if (!text.includes("poll=7") || !text.includes("heartbeat=8")) {
  console.error(text);
  process.exit(1);
}
EOF
  ...
  pass "OpenCode watcher plugin sources both effective remote-channel configs"
}
