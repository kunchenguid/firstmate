...
test_pi_recovery_resurface_coalesces_and_resumes() {
  local repo home plugin log stop out status
...
#!/usr/bin/env bash
if [ "${1:-}" = --handling-delivered ]; then
  printf 'confirmation-rejected generation=%s watcher=%s\n' "$2" "$4" >> "${FM_ARM_LOG:?}"
  exit 1
fi
printf 'arm=%s predecessor=%s\n' "$$" "${FM_WATCH_PREDECESSOR_ARM_PID:-none}" >> "${FM_ARM_LOG:?}"
count=$(grep -c '^arm=' "$FM_ARM_LOG")
...
if (!prompts[0].includes("handling delivery confirmation was rejected")) throw new Error(`confirmation failure was not surfaced: ${prompts[0]}`);
