...
SH
  ...
  out=$(PLUGIN="$plugin" FM_HOME="$home" FM_ROOT_OVERRIDE="$repo" node "$FM_TEST_NODE_NO_TS_WARNING" --input-type=module 2>&1 <<'EOF'
...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  out=$(PLUGIN="$plugin" FM_HOME="$home" FM_ROOT_OVERRIDE="$repo" FM_CLEANUP_LOG="$cleanup_log" FM_CHILD_PID_FILE="$pid_file" node "$FM_TEST_NODE_NO_TS_WARNING" --input-type=module 2>&1 <<'EOF'
