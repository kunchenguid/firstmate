...
case "$*" in
  *"#{pane_current_path}"*)
    ...
    # worktree. Model that: the first poll reports FM_FAKE_PROJ_PATH, later polls
    # FM_FAKE_PANE_PATH (FM_FAKE_POLL_FILE, truncated per run, is the counter).
    __n=1
...
run_grok_spawn() {
  ...
  FM_ROOT_OVERRIDE='' FM_HOME="$home" \
    ...
    FM_SPAWN_NO_GUARD=1 FM_FAKE_PANE_PATH="$wt" FM_FAKE_PROJ_PATH="$proj" \
