...
# binary. Skip cleanly when it is absent (matching the backend smoke suites) or
# when the installed version predates atomic multi-ID mv (0.2.2+), which the
# delegated handoff path refuses to run without.
...
fm_tasks_axi_compatible || { echo "skip: tasks-axi is not compatible (0.2.2+ with atomic multi-ID mv is required by the delegated handoff path)"; exit 0; }
