...
setup_remote_handoff() {  # <name> -> echoes "<here> <peer>"
  ...
  # The peer's own firstmate root: a real machine's whole bin/, which is exactly
  # what a peer has. A derived library list was tried twice and rotted twice: the
  # 2026-08-11 upstream sync added two more `. "$SCRIPT_DIR/..."` lines to
  # fm-backlog-handoff.sh, and the 2026-08-25 sync added fm-pending-reply-lib.sh,
  # whose own siblings are one level further out and spell their directory four
  # different ways ($SCRIPT_DIR/, $(dirname ...)/, $ROOT/bin/, $VAR/backends/).
  # Both times this peer died with "No such file or directory" mid-handoff.
  # Copying bin/ wholesale removes that whole class of rot.
  rm -rf "$peer/fmroot/bin"
  cp -R "$ROOT/bin" "$peer/fmroot/bin"
  [ -f "$peer/fmroot/bin/fm-backlog-handoff.sh" ] \
    || fail "peer fixture: bin/fm-backlog-handoff.sh did not reach the peer root"
  ...
  local sm_abs peer_fakebin
  sm_abs=$(cd "$TMP_ROOT/$name-sm" && pwd -P)
  # A live receiver on the PEER. The landing runs the peer's own handoff script,
  # which since the 2026-08-25 upstream sync also wakes the receiving endpoint
  # and refuses to report success when no live endpoint is recorded. A routed
  # secondmate is running on its own machine in real life, so the peer records
  # its endpoint exactly as this side's setup_homes does for a local one.
  cat > "$peer/state/far.meta" <<EOF
window=firstmate:fm-far
kind=secondmate
harness=claude
backend=tmux
home=$sm_abs
worktree=$sm_abs
EOF
  # The stub relay runs the peer's verb with an EMPTY environment, and the host
  # config carries a fixed key set that has no room for the fake tmux's own
  # variables. So the peer gets its own front-of-PATH shim that supplies them and
  # then delegates to the shared fake tmux.
  peer_fakebin="$peer/fakebin"
  mkdir -p "$peer_fakebin"
  cat > "$peer_fakebin/tmux" <<EOF
#!/usr/bin/env bash
set -u
export FM_FAKE_TMUX_WINDOW='firstmate:fm-far'
export FM_FAKE_TMUX_LOG='$peer/tmux.log'
export FM_FAKE_TMUX_CAPTURE='$HANDOFF_FAKEBIN/../default-fake/pane.txt'
exec '$HANDOFF_FAKEBIN/tmux' "\$@"
EOF
  chmod 755 "$peer_fakebin/tmux"
  {
    printf 'FM_ROOT=%s\n' "$peer/fmroot"
    printf 'FM_HOME=%s\n' "$peer"
    printf 'HOME_DIR=%s\n' "$peer"
