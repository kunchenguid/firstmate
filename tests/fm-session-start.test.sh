...
EOF
  ...
  # The fakebin deliberately does not provide tasks-axi, and BASE_PATH excludes
  # user-local package-manager installs.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # The fake toolchain intentionally omits tasks-axi so bootstrap emits one of
  # its real MISSING diagnostics without depending on host-level node installs.
