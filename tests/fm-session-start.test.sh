...
EOF
  ...
  # Use lavish-axi (an npm package) rather than node: node is often present in
  # the system PATH (/usr/bin/node) on CI hosts, which would prevent bootstrap
  # from reporting it missing and silently break this ordering assertion.
