...
EOF
  ...
  # The missing tool must be one no host ships in the system BASE_PATH dirs:
  # node exists as a distro package on some hosts, so removing the fake would
  # not force anything there.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # The forced-missing tool must be one no host ships in the system BASE_PATH
  # dirs: node exists as a distro package on some hosts, so removing the fake
  # would not force anything there.
  rm -f "$fakebin/chrome-devtools-axi"
