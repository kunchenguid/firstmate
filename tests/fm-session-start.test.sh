...
EOF
  ...
  assert_contains "$out" "each task's status line names its full log path" \
    "read-once contract does not describe the on-demand status pointer"
  ...
  assert_contains "$out" "a bounded tail of every state/*.status" \
    "tail-mode read-once contract does not describe bounded status tails"
...
status_tail_oracle() {  # <session-start-bin> <home> <root> <path>
  ...
  case "$off" in
    *"each task's status line names its full log path"*) : ;;
    *) printf 'oracle: default run missing the on-demand read-once contract\n'; return 1 ;;
  esac
  ...
  case "$on" in
    ...
    *) printf 'oracle: tail run missing the bounded-tail read-once contract\n'; return 1 ;;
  esac
...
#!/usr/bin/env bash
set -u
if [ "${1:-}" = --worker-liveness ]; then
  printf '%s\n' 'liveness: absent · source: endpoint'
  exit 0
fi
