...
EOF
  ...
  # Remove an axi tool rather than node: node can be a real system binary on the
  # base PATH, which would defeat the forced-missing check.
  rm -f "$fakebin/gh-axi"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Remove an axi tool (never a base-system binary) so the forced-missing line is
  # reliable even where node is installed on the base PATH.
  rm -f "$fakebin/gh-axi"
