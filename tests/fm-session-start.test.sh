...
EOF
  ...
  # independent of real tools on the host PATH.
  rm -f "$fakebin/wezterm"

  printf 'window=fm-sess:w1\nkind=ship\n' > "$home/state/task-a.meta"

  out=$(FM_WEZTERM_BIN="$home/no-such-wezterm" run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  missing_line=$(printf '%s\n' "$out" | grep -n 'MISSING: wezterm' | head -1 | cut -d: -f1)
...
EOF
  ...
  assert_contains "$out" "MISSING: wezterm (install:" "fm-bootstrap.sh's real detect line did not appear verbatim"
