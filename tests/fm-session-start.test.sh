...
if [ "${FM_BOOTSTRAP_NETWORK:-}" = only ]; then
  printf '%s\n' 'deferred' >> "${FM_FAKE_STATE_READ_CALLS:?}"
else
  printf '%s\n' 'blocking' >> "${FM_FAKE_STATE_READ_CALLS:?}"
fi
...
ticks=0
while [ ! -e "${FM_FAKE_STATE_READ_RELEASE:?}" ] && [ "$ticks" -lt 300 ]; do
  sleep 0.1
  ticks=$((ticks + 1))
done
...
SH
  ...
  out=$(FM_BACKEND=tmux FM_FAKE_HARNESS_PID="$SESSION_START_TEST_HARNESS_PID" \
    FM_FAKE_STATE_READ_CALLS="$calls" FM_FAKE_STATE_READ_RELEASE="$release_gate" \
