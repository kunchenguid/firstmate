...
EOF
  ...
  missing_line=$(printf '%s\n' "$out" | grep -n 'MISSING: playwright-cli' | head -1 | cut -d: -f1)
...
EOF
  ...
  assert_contains "$out" "MISSING: playwright-cli (install:" "fm-bootstrap.sh's real detect line did not appear verbatim"
