...
# Markers today: CODEX_THREAD_ID/CODEX_SESSION_ID (Codex, verified by its writer
# lock), CLAUDECODE (claude), PI_CODING_AGENT plus FM_PI_HARNESS (Pi family),
# and GROK_AGENT (grok). Without this, a local harness session fails cases that
# pin a different fake harness while CI
# (no ambient markers) still passes.
run_session_start() {
  local home=$1 root=$2 path=$3 pi_harness=${4:-}
  if [ -n "$pi_harness" ]; then
    env -u CODEX_THREAD_ID -u CODEX_SESSION_ID -u CLAUDECODE -u GROK_AGENT PI_CODING_AGENT=true FM_PI_HARNESS="$pi_harness" \
...
SH
  ...
  out=$(env -u CODEX_THREAD_ID -u CODEX_SESSION_ID -u CLAUDECODE -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT \
