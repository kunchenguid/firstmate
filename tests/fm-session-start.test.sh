...
# Desktop), PI_CODING_AGENT plus FM_PI_HARNESS (Pi family), GROK_AGENT (grok).
# Codex CLI and opencode have no env markers (ancestry only). Without this, a local
# claude/pi/grok session fails cases that pin a different fake harness while CI
# (no ambient markers) still passes.
run_session_start() {
  local home=$1 root=$2 path=$3 pi_harness=${4:-}
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u CODEX_CI -u CODEX_THREAD_ID -u GROK_AGENT PI_CODING_AGENT=true FM_PI_HARNESS="$pi_harness" \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u CODEX_CI -u CODEX_THREAD_ID -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT \
      FM_HOME="$home" FM_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}
...
SH
  ...
  out=$(env -u CLAUDECODE -u CODEX_CI -u CODEX_THREAD_ID -u PI_CODING_AGENT -u FM_PI_HARNESS -u GROK_AGENT \
