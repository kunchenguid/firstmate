...
test_output_ordering_diagnostics_lead() {
  local rec root home fakebin out lock_line boot_line wake_line context_line fleet_line next_line diagnostic_line
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # Force a deterministic actionable diagnostic without depending on which
  # optional tools happen to exist in the host's system PATH.
  printf 'unverified-test-backend\n' > "$home/config/backend"
  ...
  diagnostic_line=$(printf '%s\n' "$out" | grep -n '^BACKEND_INVALID:' | head -1 | cut -d: -f1)
  [ -n "$diagnostic_line" ] || fail "actionable bootstrap diagnostic did not appear at all"
  [ "$diagnostic_line" -lt "$fleet_line" ] \
    || fail "actionable bootstrap diagnostic was buried after the bulk fleet-state digest"
