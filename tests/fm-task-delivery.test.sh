...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  instructions_path="$home/data/tasks/promote-d1/ship-instructions.md"
...
test_promote_refuses_a_symlinked_task_record() {
  ...
  assert_absent "$home/data/tasks/promote-sym/ship-instructions.md" \
...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    fill_brief_subsections "$home/data/tasks/$id/brief.md" \
    ...
    rm "$home/data/tasks/$id/brief.md"
    ...
    awk '/^# Definition of done$/ { emit=1 } emit' "$home/data/tasks/$id/brief.md" > "$brief_dod"
  ...
  assert_grep 'needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file='"$home/data/tasks/promote-dod-no-mistakes/nm-<run>-findings.txt" "$payload" \
...
EOF
  ...
  assert_present "$home/data/tasks/$id/launch-brief.md" \
  ...
  assert_grep "supersedes every earlier brief instruction about constructing \`--intent\`" \
    "$home/data/tasks/$id/launch-brief.md" \
  ...
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit && /^$/ { exit } emit { print }' "$home/data/tasks/$id/launch-brief.md")
...
EOF
  ...
  assert_absent "$home/data/tasks/$id/launch-brief.md" "unmarked legacy no-mistakes spawn serialized unauthorized intent"
  ...
  assert_absent "$home/data/tasks/$id/ship-instructions.md" \
...
EOF
  ...
  brief="$home/data/tasks/$id/ship-instructions.md"
...
EOF
  ...
  fill_brief_subsections "$home/data/tasks/$id/brief.md" "$words" 'This build constraint must not become intent.'
  out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
  assert_present "$home/data/tasks/$id/launch-brief.md" "plain intent was not serialized"
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$home/data/tasks/$id/launch-brief.md")
  ...
  printf '# Task\n## Captain'"'"'s intent\n%s\n\n## Firstmate spec\nDo not paraphrase.\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' "$words" > "$home/data/tasks/intent-literal/brief.md"
  ...
  authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$home/data/tasks/intent-literal/launch-brief.md")
  ...
  for marker in 'Captain:' "Captain's words:" "Captain's ask:" "Captain's intent:" 'Captain,'; do
    ...
    printf '# Task\n## Captain'"'"'s intent\nKeep the original request intact.\n  %s preserve its provenance.\n\n## Firstmate spec\nDo not paraphrase.\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' \
      "$marker" > "$home/data/tasks/$id/brief.md"
    out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
    status=$?
    ...
    assert_absent "$home/data/tasks/$id/launch-brief.md" "$marker: addressed intent was serialized"
    assert_absent "$home/state/$id.meta" "$marker: addressed intent spawn wrote task metadata"
    assert_grep "  $marker preserve its provenance." "$home/data/tasks/$id/brief.md" "$marker: refusal rewrote the brief"
  done
  ...
  assert_absent "$home/data/tasks/$id/ship-instructions.md" "promotion published addressed intent"
  ...
  for marker in '[captain]' 'Captain:' "Captain's words:" "Captain's ask:" "Captain's intent:"; do
    ...
    printf '# Task\n%s %s\nDo not include this build constraint.\n%s %s\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' \
      "$marker" 'Keep the original request intact.' "$marker" 'Preserve its provenance.' > "$home/data/tasks/$id/brief.md"
    out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
    assert_present "$home/data/tasks/$id/launch-brief.md" "$marker: provenance was not accepted"
    authorized=$(awk '$0 == "## Captain intent authorized for --intent" { emit=1; next } emit { print }' "$home/data/tasks/$id/launch-brief.md")
...
EOF
  ...
  for project_kind in firstmate unrelated; do
    ...
    for kind in no-mistakes direct-PR local-only scout; do
      ...
      assert_no_grep '# Current worker role contract' "$home/data/tasks/$id/brief.md" "spawn rewrote the source brief"
