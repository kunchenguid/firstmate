...
# (AGENTS.md section 7), so a downgrade there is announced too. A legacy
# no-mistakes or no-mistakes-prod-only registry annotation now maps to
# direct-PR (bin/fm-project-mode.sh), so it compares the same way.
...
legacy no-mistakes project shipped direct-PR|- proj [no-mistakes] - fixture (added 2026-01-01)|direct-PR|quiet|direct-PR
legacy no-mistakes project shipped local-only|- proj [no-mistakes] - fixture (added 2026-01-01)|local-only|notice|direct-PR
local-only project shipped direct-PR|- proj [local-only] - fixture (added 2026-01-01)|direct-PR|quiet|local-only
legacy conditional policy shipped direct-PR|- proj [no-mistakes-prod-only] - fixture (added 2026-01-01)|direct-PR|quiet|direct-PR
unregistered project resolves to the direct-PR standing default|- other [no-mistakes] - fixture (added 2026-01-01)|local-only|notice|direct-PR
ROWS
...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  out=$(FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode nope --yolo off 2>&1)
  status=$?
  [ "$status" -ne 0 ] || fail "promotion on an unknown mode should exit non-zero"
  assert_contains "$out" "must be one of direct-PR, local-only" "promote did not refuse an unknown task mode"
...
# The registry parser survives for the mechanical consumers only. A legacy
# no-mistakes or no-mistakes-prod-only annotation, from before those were
# retired as task modes, is always mapped to direct-PR - never migrated in the
# registry file itself, but always resolved to its current meaning.
...
EOF
  ...
  [ "$out" = "direct-PR off" ] || fail "legacy conditional policy did not map to direct-PR (got '$out')"
  err=$(FM_HOME="$home" "$PROJECT_MODE" prodproj 2>&1 >/dev/null)
  [ -z "$err" ] || fail "a registered legacy annotation still warned as unknown: $err"

  out=$(FM_HOME="$home" "$PROJECT_MODE" yoloproj 2>/dev/null)
  [ "$out" = "direct-PR on" ] || fail "legacy conditional policy dropped its +yolo posture (got '$out')"

  out=$(FM_HOME="$home" "$PROJECT_MODE" flatproj 2>/dev/null)
  [ "$out" = "direct-PR off" ] || fail "a flat registered mode was altered (got '$out')"

  out=$(FM_HOME="$home" "$PROJECT_MODE" typoproj 2>/dev/null)
  [ "$out" = "direct-PR off" ] || fail "a typo'd mode no longer falls back to the default"
  ...
  pass "fm-project-mode: a legacy conditional-policy annotation always maps to direct-PR"
}
