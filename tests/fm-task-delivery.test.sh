...
STUB
  ...
  for mode in no-mistakes direct-PR local-only; do
    ...
    # Compare the public outputs of both real generation paths. The ordinary
    # brief repeats its task after the Definition of done as a load-bearing
    # bookend, so stop at that separate section instead of treating it as part
    # of the shared delivery contract.
    ...
    awk '
      /^# Definition of done$/ { emit=1 }
      !emit { next }
      /^# Load-bearing contract$/ { exit }
      /^$/ { blanks=blanks "\n"; next }
      { printf "%s", blanks; blanks=""; print }
    ' "$home/data/$id/brief.md" > "$brief_dod"
