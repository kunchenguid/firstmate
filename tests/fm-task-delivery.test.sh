...
test_promotion_delivers_the_real_definition_of_done() {
  ...
  local named_proj named_wt origin_only local_only_proj status stale_remote_sha remote_sha
...
STUB
  ...
  named_wt="$TMP_ROOT/promote-dod/named-base-wt"
  git clone -q "$named_proj.origin.git" "$named_wt"
  git -C "$named_wt" fetch -q origin
  for mode in no-mistakes direct-PR local-only; do
  ...
  git -C "$named_wt" update-ref refs/remotes/origin/develop "$stale_remote_sha"
  id=promote-refreshes-remote-base
  printf 'window=fm-%s\nkind=scout\nworktree=%s\nproject=%s\nbase_branch=develop\n' \
    "$id" "$named_wt" "$named_proj" > "$home/state/$id.meta"
  FM_HOME="$home" "$BRIEF" "$id" fixture-project --scout --base-branch develop >/dev/null 2>&1 \
  ...
  [ "$(git -C "$named_wt" rev-parse refs/remotes/origin/develop)" = "$remote_sha" ] \
    || fail "promotion left the task worktree refs/remotes/origin/develop stale"
