...
test_existing_launch_templates_are_byte_pinned() {
  # claude carries one template per kind: a secondmate is captain-facing, so it
  # keeps the native prompt suggestion and takes the session display-name flag,
  # while a ship/scout crewmate launches with the suggestion disabled. Both are
  # pinned so the kimi adapter cannot disturb either one.
  assert_source_line "        printf '%s' 'claude --dangerously-skip-permissions __MODELFLAG____EFFORTFLAG____NAMEFLAG__\"\$(__OPINPUT__ encode launch-brief < __BRIEF__)\"'"
