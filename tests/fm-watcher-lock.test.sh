...
test_watch_restart_reports_healthy_peer_without_attaching() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/restart.out"
  # The peer must already have its SIGTERM handler installed when the restart's
  # kill -TERM lands; node takes ~100ms of cold start to register it, so wait for
  # the ready marker (written synchronously after process.on) before proceeding.
  node -e 'process.on("SIGTERM", () => {}); require("fs").writeFileSync(process.argv[1], ""); setTimeout(() => {}, 300000)' "$dir/peer-ready" &
  peer=$!
  i=0
  while [ "$i" -lt 50 ] && [ ! -e "$dir/peer-ready" ]; do
    sleep 0.1
    i=$((i + 1))
  done
  [ -e "$dir/peer-ready" ] || fail "peer never reported its SIGTERM handler ready"
