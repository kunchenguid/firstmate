...
test_watch_restart_reports_healthy_peer_without_attaching() {
  ...
  out="$dir/restart.out"
  mark_pr_check_migration_complete "$state"
  # The peer must already be TERM-resistant when the restart arm signals it, and
  # node only registers the handler after interpreter startup - so have the peer
  # signal readiness and wait for it, or the arm's TERM can land first and kill
  # a handler-less peer (turning this into the started-a-fresh-watcher path).
  ...
  [ -s "$ready" ] || fail "peer never signaled SIGTERM-handler readiness"
