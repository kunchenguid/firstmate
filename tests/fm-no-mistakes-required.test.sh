...
test_duplicate_steps_fail_closed() {
  local body output rc
  body="$SIGNATURE
<!-- no-mistakes-pipeline-attestation:v1 {\"head_sha\":\"$NEW_SHA\",\"steps\":[{\"step\":\"review\",\"status\":\"completed\"},{\"step\":\"review\",\"status\":\"failed\"},{\"step\":\"test\",\"status\":\"completed\"},{\"step\":\"document\",\"status\":\"completed\"}]} -->"
  rc=0
  output=$(run_verifier "$body" "$NEW_SHA") || rc=$?
  [ "$rc" -ne 0 ] || fail "shared action accepted duplicate attestation steps"
  assert_contains "$output" "duplicate step review" \
    "duplicate-step failure did not identify the conflicting step"
  pass "shared action rejects duplicate attestation steps"
}

test_malformed_step_fails_closed() {
  local body output rc
  body="$SIGNATURE
<!-- no-mistakes-pipeline-attestation:v1 {\"head_sha\":\"$NEW_SHA\",\"steps\":[{\"step\":\"review\",\"status\":\"completed\"},{\"step\":\"test\",\"status\":\"completed\"},{\"step\":\"document\",\"status\":\"completed\"},{\"status\":\"completed\"}]} -->"
  rc=0
  output=$(run_verifier "$body" "$NEW_SHA") || rc=$?
  [ "$rc" -ne 0 ] || fail "shared action accepted a malformed attestation step"
  assert_contains "$output" "structured pipeline step attestation" \
    "malformed-step failure did not identify the invalid attestation"
  pass "shared action rejects malformed attestation steps"
}
