...
printf '%s\n' "$pane" | grep -Fq "Working..." \
  || fail "Calm did not keep Pi's stock working row visible on the credentialed provider path"
printf '%s\n' "$pane" | grep -Fq '\__/' \
  && fail "Calm rendered a custom working widget on the credentialed provider path"
wait_for_exact_line "CALM_LIVE_WORKING_VISIBLE" 120 \
  || fail "Pi did not settle the Calm stock working-row provider probe"
pane=$(capture)
printf '%s\n' "$pane" | grep -Fq '\__/' \
  && fail "Calm left custom working-widget output after the run settled"
...
printf 'ok - Pi %s live E2E covered Calm with Pi stock working activity, Ahoy first/later messages, legacy transcripts, near misses, and watcher continuity\n' "$PI_VERSION"
