...
if [ "$WATCH_ONLY" -eq 0 ]; then
  ...
  printf '%s\n' "$pane" | grep -Fq ' o  o  o ' \
    || fail "Calm did not show the working presentation on the credentialed provider path"
  ...
  pane=$(capture)
  printf '%s\n' "$pane" | grep -Fq ' o  o  o ' \
    && fail "Calm left the working presentation on screen after the run settled"
