...
test_relaunch_reorders_control_relaunch_tx_before_a_preserved_pr_identity() {
  ...
  # Mirrors what bin/fm-pr-check.sh actually leaves behind: every other field
  # first, pr= and pr_head= last, because fm-pr-check.sh:132-139 always strips
  # and re-appends them at the end of the file.
  {
  ...
  # fm_pr_metadata_identity_parse (bin/fm-pr-lib.sh) is the exact check
  # fm_pr_poll_artifacts_content_valid runs against the live task meta to
  # validate the merge poll - it is what the watcher's state check depends on.
  fm_pr_metadata_identity_parse "$meta" \
    || fail "the relaunched record no longer validates as a PR identity; the merge poll would silently stop being watched"
  ...
  pass "fm-control relaunch: control_relaunch_tx lands before a preserved pr= block, so the merge poll's identity still validates"
}
