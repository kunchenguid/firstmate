...
# A fresh ship or scout spawn is refreshed to the current default-branch tip before
# it branches (bin/fm-spawn.sh), and since that tip may now be the primary
# checkout's own branch, a relaunch must not follow it: a relaunch replaces the
# AGENT, never the work, and moving a task's branch onto a base it was not written
# against would displace unlanded work.
test_spawn_relaunch_keeps_its_base_when_the_primary_advanced() {
  ...
  pass "fm-spawn --relaunch: the task keeps its branch and work while the primary's default branch moves on"
}
