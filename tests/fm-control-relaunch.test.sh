...
case "\$cmd \$sub" in
  ...
  "pane process-info")
    ...
    if [ -f "\$D/launched" ]; then
      printf '{"result":{"type":"pane_process_info","process_info":{"pane_id":"%s","shell_pid":%s,"foreground_process_group_id":%s,"foreground_processes":[{"pid":%s,"name":"claude","argv0":"claude","argv":["claude"],"cmdline":"claude"}]}}}\n' \
        "\$pane" "\$\$" "\$\$" "\$\$"
    else
