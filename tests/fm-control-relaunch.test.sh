...
case "${args[0]:-} ${args[1]:-}" in
  ...
  'server ')
    ...
    ;;
  'tab get')
    if [ "${args[2]:-}" = w1:t3 ]; then
      case "$case_name" in
        postcreate-tab-unreadable)
          exit 1
          ;;
  ...
  'pane get')
    ...
    if [ "${args[2]:-}" = w1:p3 ]; then
      case "$case_name" in
        postcreate-pane-unreadable)
          exit 1
          ;;
        postcreate-tabs-unavailable|postcreate-tabs-malformed|postclose-tabs-unavailable|postclose-tabs-malformed)
          if [ -e "$dir/herdr-new-pane-closed" ]; then
            printf '%s\n' '{"error":{"code":"pane_not_found"}}'
          else
  ...
  'agent get')
    ...
    if [ "${args[2]:-}" = w1:p3 ] && { [ "$case_name" = postcreate-tabs-unavailable ] || [ "$case_name" = postcreate-tabs-malformed ]; }; then
      printf '%s\n' '{"error":{"code":"agent_not_found"}}'
      exit 0
    fi
  ...
  'pane list')
    ...
    ;;
  'workspace list')
    if [ "$case_name" = postcreate-conflicting-pane ] \
  ...
  'tab create')
    ...
    case "$case_name" in
      postcreate-tab-unreadable|postcreate-pane-unreadable|postcreate-tabs-unavailable|postcreate-tabs-malformed|postclose-tabs-unavailable|postclose-tabs-malformed)
        printf '%s\n' "${args[*]}" >> "$dir/herdr-mutations"
        printf '%s\n' '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}'
        exit 0
        ;;
    ...
    if [ "${args[0]:-} ${args[1]:-} ${args[2]:-} ${args[3]:-}" = 'pane send-keys w1:p3 Enter' ]; then
      for lock in /tmp/firstmate-herdr-presentation/order-*.lock; do
        ...
        if [ -n "$lock_pid" ] && kill -0 "$lock_pid" 2>/dev/null; then
          : > "$dir/herdr-launch-lock-observed"
          break
        fi
      done
    fi
  ...
  'tab close'|'pane close')
    ...
    if [ "${args[0]:-} ${args[1]:-} ${args[2]:-}" = 'tab close w1:t2' ] \
       && { [ "$case_name" = postclose-tabs-unavailable ] || [ "$case_name" = postclose-tabs-malformed ]; }; then
      exit 0
    fi
    exit 1
    ;;
...
test_spawn_relaunch_preserves_a_postcreate_herdr_conflict_claim() {
  local dir out rc before claim_before mutations
  ...
  assert_not_contains "$mutations" 'pane close w1:p3' \
    "the post-create conflict must retain its exact claimed replacement pane"
  ...
  [ -e "$dir/home/state/.hr1.herdr-relaunch-claim" ] \
    || fail "the post-create conflict must retain its durable recovery claim"
  claim_before="$dir/claim-before"
  cp "$dir/home/state/.hr1.herdr-relaunch-claim" "$claim_before"
  assert_contains "$(cat "$claim_before")" 'tab_id=w1:t3' \
    "the post-create conflict claim must name its verified replacement tab"
  assert_contains "$(cat "$claim_before")" 'pane_id=w1:p3' \
    "the post-create conflict claim must name its verified replacement pane"
...
test_spawn_relaunch_handles_postcreate_herdr_read_failures() {
  ...
  for case_name in postcreate-tab-unreadable postcreate-pane-unreadable postcreate-tabs-unavailable postcreate-tabs-malformed postbind-tabs-unavailable postbind-tabs-malformed postcreate-zero-match postclose-tabs-unavailable postclose-tabs-malformed final-tabs-unavailable final-tabs-malformed final-tabs-stale; do
    ...
    cp "$dir/home/state/hr1.meta" "$before"
    out=$(FM_FAKE_HERDR_CASE="$case_name" run_spawn "$dir" hr1 --relaunch --harness claude); rc=$?
    expect_code 1 "$rc" "a post-create Herdr $case_name read must refuse"
    cmp -s "$before" "$dir/home/state/hr1.meta" \
...
test_spawn_relaunch_holds_herdr_session_lock_through_submission() {
  local dir out rc
  dir=$(new_case herdr-launch-lock hr1)
  add_herdr_missing_pane_task "$dir" hr1
