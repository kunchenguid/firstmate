...
test_respond_unsafe_inbox_is_quarantined() {
  local home out fakebin qcount mode uid
  home="$TMP_ROOT/resp-unsafe-inbox"; mkdir -p "$home/state"
  write_env "$home" "FM_TELEGRAM_DRY_RUN=1"
  fakebin=$(make_fake_curl "$home")
  uid=47
  for mode in 644 000; do
    uid=$((uid + 1))
    seed_inbox "$home" "$uid" "status"
    chmod "$mode" "$home/state/telegram-inbox/$uid.json"
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_TELEGRAM_API_URL="https://api.test" \
      FAKE_POLL_CODE=200 FAKE_POLL_BODY='{"ok":true,"result":[]}' \
      "$ROOT/bin/fm-telegram-poll.sh")
    assert_contains "$out" "telegram-msg $uid" "mode-$mode inbox file must wake for quarantine"
    out=$(FM_HOME="$home" "$ROOT/bin/fm-telegram-respond.sh" 2>/dev/null)
    assert_contains "$out" "quarantined $uid unsafe-inbox" "mode-$mode inbox file must quarantine"
    assert_absent "$home/state/telegram-inbox/$uid.json" "mode-$mode inbox file must leave live inbox"
    qcount=$(find "$home/state/telegram-inbox-quarantine" -name "$uid.*.json" -type f 2>/dev/null | wc -l | tr -d ' ')
    [ "$qcount" -eq 1 ] || fail "expected one mode-$mode quarantine artifact, got $qcount"
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$home" FM_TELEGRAM_API_URL="https://api.test" \
      FAKE_POLL_CODE=200 FAKE_POLL_BODY='{"ok":true,"result":[]}' \
      "$ROOT/bin/fm-telegram-poll.sh")
    assert_not_contains "$out" "telegram-msg $uid" "quarantined mode-$mode file must not re-wake"
  done
  assert_grep "unsafe-inbox" "$home/state/telegram-audit.log" "unsafe-inbox quarantine must be audited"
  pass "poll wakes readable and unreadable unsafe inbox files once for quarantine"
}
