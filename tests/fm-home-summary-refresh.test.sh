...
cleanup() {
  ...
  for pid in "$WATCH_PID" "$SLOW_WRITER_PID" "$SLOW_STATE_READER_PID" "$LOCK_HOLDER_PID"; do
    [ -n "$pid" ] || continue
...
SLOW_MARKER="$TMP_ROOT/slow-state-reader.pid"
PATH="$FAKEBIN:$PATH" \
  FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$HOME_DIR" \
  FM_SNAPSHOT_NOW="$NOW_THREE" FM_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  FM_TEST_STATE_READ_MARKER="$SLOW_MARKER" FM_TEST_STATE_READ_SLEEP=30 \
...
SLOW_STATE_READER_PID=$(cat "$SLOW_MARKER" 2>/dev/null || true)
writer_pgid=$(ps -o pgid= -p "$SLOW_WRITER_PID" 2>/dev/null | tr -d '[:space:]')
ancestor=$SLOW_STATE_READER_PID
child_pgid=
i=0
while [ "$i" -lt 20 ]; do
...
SUCCESS_MARKER="$TMP_ROOT/success-state-reader.pid"
PATH="$FAKEBIN:$PATH" \
  FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$HOME_DIR" \
  FM_SNAPSHOT_NOW="$NOW_THREE" FM_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  FM_TEST_STATE_READ_MARKER="$SUCCESS_MARKER" FM_TEST_STATE_READ_SLEEP=1 \
