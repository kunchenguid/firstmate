#!/usr/bin/env bash
# Behavior tests for the generic process-to-event runner.
#
...
# depends on a discovery timer. The adapter names below are inert fixtures; no
# external poll service is started.
#
# Delivery is deliberately NOT asserted as at-least-once or lossless: the
# source-side durability belongs to each adapter; this suite tests only the
# runner's own boundary - output that reached the runner is stored before it is
# announced.
set -u
...
# Generic runner cases use an inert fixture adapter supplied by the test, not
# a product adapter or an external poll tool.
FIXTURE_ROOT="$TMP_ROOT/fixture-root"
mkdir -p "$FIXTURE_ROOT/bin"
cat > "$FIXTURE_ROOT/bin/fm-procevent-fixture.sh" <<'SH'
#!/usr/bin/env bash
exit 2
SH
chmod +x "$FIXTURE_ROOT/bin/fm-procevent-fixture.sh"
...
pe() {
  FM_ROOT_OVERRIDE="$FIXTURE_ROOT" FM_HOME="$1" "$ROOT/bin/fm-procevent.sh" "${@:2}"
}
...
out=$(pe_register "$H1" fixture src-one -- "$BLOCKER" "$TRIG" "payload one")
...
assert_contains "$payload" "procevent fixture src-one 1" "completion publishes the committed result sequence"
...
assert_grep 'fixture' "${RESULT%.result}.adapter" "the captured result retains its immutable adapter"
...
pe_register "$HPG" fixture direct-src -- "$BLOCKER" "$DIRECT_TRIGGER" "direct result" >/dev/null
...
exit 0;
PL
pe_register "$HPG" fixture shared-src -- "$BLOCKER" "$SHARED_TRIGGER" "shared result" >/dev/null
...
assert_contains "$(wake_payloads "$H2")" "procevent fixture src-cut 7" "durable adapter identity survives without a registration"
...
assert_contains "$(wake_payloads "$H2")" "procevent fixture src-cut 7" "the repeat wake preserves its deduplication identity"
...
pe_register "$HW" fixture restart-cut-src -- "$BLOCKER" "$TRIGW" "restart cut payload" >/dev/null
pe "$HW" reconcile >/dev/null
sleep 0.5
: > "$TRIGW"
wait_for "$HW/state/.wake-queue" || fail "the restart-cut source published no event"
assert_contains "$(wake_payloads "$HW")" "procevent fixture restart-cut-src 1" \
...
' _ "$ROOT" "$HP")
expected=$(printf '%s\n' \
  'check: procevent fixture ordered-src 1' \
  'check: procevent fixture ordered-src 2' \
  'check: procevent fixture ordered-src 10')
...
pe_register "$HA" fixture shared-src -- "$BLOCKER" "$TRIG2" "shared" >/dev/null
pe_register "$HB" fixture shared-src -- "$BLOCKER" "$TRIG2" "shared" >/dev/null
...
pe_register "$HZ" fixture orphan-src -- "$BLOCKER" "$TRIG4" "orphan" >/dev/null
...
printf '%s\n%s\nstale-token\nstale-identity\n' "$HC" "999999" > "$CLAIM"
chmod 0600 "$CLAIM"
pe_register "$HC" fixture stale-src -- /bin/echo recovered >/dev/null
...
pe_register "$HC_NEW" fixture cross-home-src -- /bin/echo recovered >/dev/null
...
pe_register "$HR" fixture race-src -- "$RACE_BLOCKER" "$RACE_LOG" "$RACE_TRIGGER" >/dev/null
...
pe_register "$HG" fixture orphan-src -- \
...
pe_register "$HG2" fixture dead-gen-src -- "$RACE_BLOCKER" "$DEAD_LOG" "$DEAD_TRIGGER" >/dev/null
...
pe_register "$HJ" fixture torn-src -- "$BLOCKER" "$TORN_TRIGGER" "torn" >/dev/null
...
pe_register "$HK" fixture retire-start-src -- "$START_BLOCKER" "$START_LOG" >/dev/null
...
pe_register "$HI" fixture reused-src -- /bin/true >/dev/null
sleep 60 &
innocent_pid=$!
...
pe_register "$HL" fixture identity-src -- "$BLOCKER" "$IDENTITY_TRIGGER" "identity" >/dev/null
...
pe_register "$HM" fixture sweep-one -- "$BLOCKER" "$SWEEP_TRIGGER_ONE" "sweep one" >/dev/null
pe_register "$HM" fixture sweep-two -- "$BLOCKER" "$SWEEP_TRIGGER_TWO" "sweep two" >/dev/null
...
pe_register "$HN" fixture foreign-src -- "$BLOCKER" "$FOREIGN_TRIGGER" "foreign" >/dev/null
pe_register "$HO" fixture foreign-src -- "$BLOCKER" "$FOREIGN_TRIGGER" "foreign" >/dev/null
...
pe_register "$HU" fixture sweep-uncertain -- "$BLOCKER" "$SWEEP_UNCERTAIN_TRIGGER" "uncertain" >/dev/null
...
pe_register "$HD" fixture argv-src -- "$BLOCKER" "$TRIG3" "one arg with spaces" "second; rm -rf /tmp/nope" >/dev/null
...
newline_status=0
newline_out=$(pe_register "$HD" fixture newline-src -- /bin/echo $'first\nsecond' 2>&1) || newline_status=$?
...
pe_register "$HE" fixture fail-src -- /bin/sh -c 'exit 7' >/dev/null
...
pe_register "$HF" fixture big-src -- /bin/sh -c 'printf "x%.0s" $(seq 1 5000)' >/dev/null
...
SH
chmod +x "$NOISY"
pe_register "$HG" fixture noisy-src -- "$NOISY" "$NOISY_PID" >/dev/null
...
pe_register "$HBAD" fixture bad-limit -- /bin/true >/dev/null
bad_limit_status=0
