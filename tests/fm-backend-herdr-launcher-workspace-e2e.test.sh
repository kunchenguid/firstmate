...
LAUNCH_COMMAND="$TMP_ROOT/launcher-ws-command"
...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      "$ROOT/bin/fm-spawn.sh" "$id" "$proj" "$LAUNCH_COMMAND --run" --backend herdr "$@" \
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/fm-spawn.sh" dupC "$PROJ" "$LAUNCH_COMMAND --run" --mode no-mistakes --yolo off --backend herdr \\
