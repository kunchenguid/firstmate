...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      "$ROOT/bin/fm-spawn.sh" "$id" "$proj" "/bin/echo launcher-ws-ok" --backend herdr "$@" \
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/fm-spawn.sh" dupC "$PROJ" "/bin/echo launcher-ws-ok" --mode no-mistakes --yolo off --backend herdr \\
