...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      "$ROOT/bin/fm-spawn.sh" "$id" "$proj" --raw "sh -c 'echo launcher-ws-ok'" \
        --harness launcher-sh --model default --backend herdr "$@" \
...
FM_SPAWN_NO_GUARD=1 FM_HOME="$PRIMARY_HOME" FM_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/fm-spawn.sh" dupC "$PROJ" --raw "sh -c 'echo launcher-ws-ok'" --harness launcher-sh --model default --mode no-mistakes --yolo off --backend herdr \\
