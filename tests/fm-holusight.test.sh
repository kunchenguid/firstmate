...
printf '%s\n' 'base' > "$brief"
printf '%s\n' '#!/usr/bin/env bash' 'set -u' 'printf "%s" "$HOLUS_EGRESS|$HOLUSIGHT_EGRESS|$FLEET_HOLUSIGHT_EGRESS" > "$FM_FAKE_MARKER"' 'printf "%s" "{coverage: sufficient, egress: {occurred: false}, evidence: [{source: src/main.py, location: line 4}]}"' > "$fake"
chmod +x "$fake"
...
[ "$(cat "$TMP/on.marker")" = '0|0|0' ] || fail 'egress deny signals were not passed'
...
pass 'benchmark reports invocation, elapsed time, and unknown token/usability dimensions'
