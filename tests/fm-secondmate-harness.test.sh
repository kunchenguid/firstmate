...
# `#{pane_current_path}` probe from FM_FAKE_PANE_PATH, and the leasing
# treehouse stub reports that same path, so this same fakebin works for a
# crew/scout (non-secondmate) spawn's lease-then-settle sequence.
