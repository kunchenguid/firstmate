...
#      config/backlog-backend, and config/backend - down into each secondmate
#      home's config/, so the secondmate's OWN crewmates, dispatch profiles,
#      backlog backend, and runtime backend inherit the primary's settings.
#      It is primary-authoritative (re-pushed at
...
test_propagate_lib() {
  ...
  [ "$(cat "$dest/backend")" = herdr ] || fail "runtime backend not propagated"
  ...
  [ "$(cat "$dest/backend")" = zellij ] || fail "changed runtime backend did not converge"
  ...
  rm -f "$src/crew-dispatch.json" "$src/crew-harness" "$src/backlog-backend" "$src/backend"
  propagate_inheritable_config "$src" "$dest"
  ...
  [ -e "$dest/backend" ] && fail "runtime backend absence not mirrored downstream"
  ...
  [ "$(cat "$d/dest2/backend")" = herdr ] || fail "runtime backend not propagated alongside"
...
test_bootstrap_sweep_propagates_and_reconverges() {
  ...
  [ "$(cat "$w/sm/config/backend" 2>/dev/null)" = herdr ] \
    || fail "sweep: runtime backend not pushed into the live home"
  ...
  [ "$(cat "$w/sm/config/backend" 2>/dev/null)" = zellij ] \
    || fail "sweep: home did not re-converge to the primary's new runtime backend"

  # Mirror absence: primary clears inherited config; the home's copies are removed.
  rm -f "$w/home/config/crew-dispatch.json" "$w/home/config/crew-harness" "$w/home/config/backlog-backend" "$w/home/config/backend"
  run_bootstrap "$w" >/dev/null
  ...
  [ -e "$w/sm/config/backend" ] \
    && fail "sweep: home runtime backend not removed after the primary cleared it"
...
test_config_push_propagates_reports_without_ff_or_nudge() {
  ...
  assert_contains "$out" "backend: pushed" \
    "config push did not report runtime backend as pushed"
  [ "$(cat "$w/sm/config/backend" 2>/dev/null)" = herdr ] \
    || fail "config push did not propagate runtime backend into the live home"
  ...
  assert_contains "$out2" "backend: unchanged" \
    "idempotent config push did not report runtime backend as unchanged"
