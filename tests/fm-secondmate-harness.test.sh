...
NODE_BIN_DIR=$(dirname "$(command -v node)") || fail "test needs node"
JQ_BIN_DIR=$(dirname "$(command -v jq)") || fail "test needs jq"
...
test_remote_control_rejects_legacy_cursor_alias() {
  local home="$TMP_ROOT/remote-cursor-alias" out status
  mkdir -p "$home/bin"
  printf 'remote-seat\n' > "$home/.fm-secondmate-home"
  printf '# Test checkout\n' > "$home/AGENTS.md"

  out=$(FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-remote-secondmate-control.sh" launch remote-seat cursor - - herdr 2>&1)
  status=$?
  expect_code 1 "$status" "remote control should reject the noncanonical cursor alias"
  assert_contains "$out" "unverified remote secondmate harness: cursor" \
    "remote control did not explain the noncanonical cursor alias refusal"
  pass "remote control accepts only the canonical cursor-agent harness name"
}
...
#!/usr/bin/env bash
printf '%s\n' '{"schemaVersion":3,"providers":[{"provider":"claude","quotaSemantics":{"effectiveAvailability":[{"scope":"all_models","status":"known","effectivePercentRemaining":100}]}},{"provider":"codex","quotaSemantics":{"effectiveAvailability":[{"scope":"all_models","status":"known","effectivePercentRemaining":100}]}}]}'
SH
...
test_spawn_fallback_chain_and_crew_scout_unaffected() {
  ...
  status=$?
  expect_code 0 "$status" "crew-unaffected spawn should succeed"$'\n'"$out"
...
sm_presentation_verdict() {  # <config-dir> -> on|off
  bash -c '
    ...
    fm_backend_herdr_presentation_default_supported() { return 0; }
