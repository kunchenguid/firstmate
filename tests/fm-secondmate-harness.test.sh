...
test_propagate_lib() {
  ...
  printf 'manual\n' > "$src/backlog-backend"
  printf 'tmux\n' > "$src/backend"
  printf 'github.com/example-org example-user\n' > "$src/github-accounts"
  ...
  cmp -s "$src/github-accounts" "$dest/github-accounts" || fail "GitHub mapping not propagated"
  ...
  printf 'tasks-axi\n' > "$src/backlog-backend"
  printf 'zellij\n' > "$src/backend"
  printf 'github.com/example-org another-user\n' > "$src/github-accounts"
  propagate_inheritable_config "$src" "$dest"
  ...
  cmp -s "$src/github-accounts" "$dest/github-accounts" || fail "changed GitHub mapping did not converge"
  ...
  rm -f "$src/crew-dispatch.json" "$src/crew-harness" "$src/backlog-backend" \
    "$src/backend" "$src/herdr-presentation-spaces" "$src/trace-context" "$src/github-accounts"
  propagate_inheritable_config "$src" "$dest"
  ...
  [ -e "$dest/github-accounts" ] && fail "GitHub mapping absence not mirrored downstream"
