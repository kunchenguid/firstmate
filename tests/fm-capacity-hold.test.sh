...
# Force markdown regardless of an ambient repo .tasks.toml that selects beads
# (this house keeps a beads-backed .tasks.toml). Fixture seeds and shows must
# not inherit that backend or its id-prefix rules.
tx_md() { TASKS_AXI_BACKEND=markdown tasks-axi "$@"; }
...
make_spawn_case() {  # <name> <manual 0|1>
  ...
  if [ "$manual" = 1 ]; then
    ...
    tx_md add cap-x1 "capacity fixture task" --kind ship \
...
test_pool_full_refusal_holds_and_exits_two() {
  ...
  local show
  show=$(tx_md show cap-x1 --file "$HOME_DIR/data/backlog.md" 2>/dev/null)
...
SH
  ...
  tx_md add task-x1 "teardown fixture task" --kind ship --file "$backlog" >/dev/null
  tx_md start task-x1 --file "$backlog" >/dev/null
  tx_md add task-a "oldest held" --kind ship --file "$backlog" >/dev/null
  tx_md add task-b "newer held" --kind ship --file "$backlog" >/dev/null
  tx_md add task-c "other pool" --kind ship --file "$backlog" >/dev/null
  tx_md add task-d "external" --kind ship --file "$backlog" >/dev/null
  tx_md hold task-a --reason "$a_reason" --kind load --file "$backlog" >/dev/null
  tx_md hold task-b --reason "$b_reason" --kind load --file "$backlog" >/dev/null
  tx_md hold task-c --reason "pool /some/other-pool full 1/2" --kind load --file "$backlog" >/dev/null
  tx_md hold task-d --reason "awaiting captain" --kind external --file "$backlog" >/dev/null
  printf '%s\n' "$case_dir"
}

held_field() {  # <case-dir> <id>
  tx_md show "$2" --file "$1/data/backlog.md" 2>/dev/null |
...
test_teardown_releases_oldest_capacity_hold_for_the_pool() {
  ...
  [ "$(tx_md show task-a --file "$case_dir/data/backlog.md" 2>/dev/null | sed -n 's/^  state: *//p' | head -1)" = queued ] \
