...
# Portable mtime in epoch seconds. Flavor-detected, never the `stat -f || stat -c`
# fallback (which writes a partial filesystem dump with GNU stat; see fm-watch.sh).
file_mtime() {
  if stat -c %Y "$ROOT" >/dev/null 2>&1; then stat -c %Y "$1" 2>/dev/null; else stat -f %m "$1" 2>/dev/null; fi
}

# Signature a primed .seen-* marker must hold so the per-poll signal scan does not
# fire on a pre-existing status (mirrors fm-watch.sh's stat_sig exactly).
seen_sig() {
  if stat -c %Y "$ROOT" >/dev/null 2>&1; then stat -c '%s:%Y' "$1" 2>/dev/null; else stat -f '%z:%Fm' "$1" 2>/dev/null; fi
}
