...
test_working_note_not_working_surfaced() {
  ...
  status_file="$state/task.status"
  printf 'working: compiling step 2\n' > "$status_file"
  # A non-direct-PR crew (no run) whose pane went idle: fm-crew-state falls back
...
# log gets no new entry once firstmate hands it to a direct-PR validation
...
test_stale_terminal_status_overridden_by_active_run() {
  ...
  printf 'validation still running...' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/validating.meta"
  # The crew reported done BEFORE firstmate triggered direct-PR validation;
  ...
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "validation still running...")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  export FM_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
...
# non-direct-PR crew, or any crew with no running pipeline) are never left hanging.
