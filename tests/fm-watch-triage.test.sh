...
test_already_surfaced_terminal_stales_absorbed_until_new_status_signal() {
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional absorbed-stale stop"
...
test_identical_run_outcomes_surface_per_validation_generation() {
  ...
  for item in ready failed; do
    ...
    reap "$pid"
    ack_stopped_cycle "$state" || fail "could not acknowledge the intentional $item duplicate-A stop"
...
test_same_hash_run_outcomes_surface_once_after_working_state() {
  ...
  for item in ready failed; do
    ...
    reap "$pid"
    ack_stopped_cycle "$state" || fail "could not acknowledge the intentional $item same-hash working stop"
...
test_declared_pause_and_exited_captain_hold_use_bounded_cadence() {
  ...
  ack_stopped_cycle "$state" || fail "could not acknowledge the bounded-cadence recheck surface"
