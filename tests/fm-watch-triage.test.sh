...
test_declared_pause_and_exited_captain_hold_use_bounded_cadence() {
  ...
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"
    fail "live declared pause surfaced before its long cadence: $(cat "$out")"
  fi
  ...
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional absorbed-pause watcher stop"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "live declared pause did not re-surface on the bounded cadence"
  wakes=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w { n++ } END { print n + 0 }' "$state/.wake-queue")
  bare=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w && $5 == "stale: " w { n++ } END { print n + 0 }' "$state/.wake-queue")
  ...
  ack_stopped_cycle "$state" || fail "could not acknowledge the declared-pause cadence surface"
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "resolved pause did not resume normal stale behavior"
