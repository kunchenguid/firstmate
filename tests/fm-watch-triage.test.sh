...
test_orphan_check_sh_never_runs() {
  ...
  # No state/orphan.check-trust registration (bin/fm-check-register.sh) and no
  # state/orphan.meta. bin/fm-pr-check-migrate.sh --checks-safe runs before the
  # watcher's own check.sh loop even starts and quarantines any unrecognized
  # legacy-shaped check.sh into state/.pr-check-quarantine/ instead of leaving it
  # in place to be authenticated (or rejected) by the loop; it never runs either
  # way. Quarantining is a safe, non-actionable migration outcome (no durable
  # wake queued), reported only as a one-time stdout notice at watcher startup.
...
SH
  ...
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher exited for a quarantined legacy check.sh (should stay up, nothing actionable): $(cat "$out")"
  fi
  assert_no_grep 'should never run' "$out" "watcher executed an unrecognized, unquarantined check.sh"
  assert_grep 'PR_CHECK_MIGRATION' "$out" "startup migration did not report the quarantine outcome"
  [ ! -s "$state/.wake-queue" ] || fail "a safe migration quarantine enqueued a durable wake record"
  [ ! -e "$state/orphan.check.sh" ] || fail "orphan check.sh was not migrated out of state/"
  find "$state/.pr-check-quarantine" -maxdepth 1 -name 'orphan.check.*' -type f 2>/dev/null | grep -q . \
    || fail "orphan check.sh was not preserved in the quarantine tree for captain review"
  reap "$pid"
  pass "an unrecognized check.sh with no matching state/<id>.meta never runs and is quarantined, not executed"
}
...
test_x_watch_check_sh_survives_orphan_guard_and_runs() {
  local dir state fakebin out pid fakeroot
  dir=$(make_case x-watch-check-sh); state="$dir/state"; fakebin="$dir/fakebin"; out="$dir/watch.out"
  fakeroot="$dir/root"
  # No state/x-watch.meta - by design, this shim is never task-keyed. The shim
  # must be byte-exact trusted content (bin/fm-x-lib.sh's fmx_poll_shim_content,
  # mode 0700): fm-pr-check-migrate.sh's --checks-safe pre-flight (run before the
  # watcher's own loop) only exempts x-watch.check.sh from its legacy-check
  # quarantine sweep when the bytes validate, same gate the watcher's own check
  # loop uses. The watcher never runs the shim script itself - a valid shim only
  # authorizes it to invoke $FM_ROOT/bin/fm-x-poll.sh directly, so fake that
  # target under an isolated FM_ROOT_OVERRIDE to observe surfaced output without
  # depending on real X-mode configuration.
  mkdir -p "$fakeroot/bin"
  cat > "$fakeroot/bin/fm-x-poll.sh" <<'SH'
#!/usr/bin/env bash
echo "check: x-mention deadbeef"
SH
  chmod +x "$fakeroot/bin/fm-x-poll.sh"
  ( . "$ROOT/bin/fm-x-lib.sh"; fmx_poll_shim_content "$fakeroot" "$fakeroot" ) > "$state/x-watch.check.sh"
  chmod 0700 "$state/x-watch.check.sh"
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_ROOT_OVERRIDE="$fakeroot" FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=1 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if wait_live "$pid" 30; then
    reap "$pid"; fail "watcher never fired for x-watch.check.sh's actionable output (should surface): $(cat "$out")"
  fi
  assert_grep 'x-mention deadbeef' "$out" "x-watch.check.sh's actionable output did not produce a wake reason"
  [ -e "$state/x-watch.check.sh" ] || fail "x-watch.check.sh was wrongly deleted as an orphan or quarantined as an unrecognized check"
  pass "x-watch.check.sh survives the orphan and migration-quarantine guards and keeps running with no state/x-watch.meta"
}
