...
# --- non-terminal stale, LIVE agent behind a declared pause: healthy idleness,
#     absorbed exactly like the dead-agent case ---------------------------------
# Reproduction of the false-stale incident: an ordinary crew that correctly
# appended paused: and kept its agent alive at the idle prompt - the NORMAL
# paused shape, since a waiting worker has no reason to exit - was surfaced as
# `stale: <win>` on every new pane hash, because pause_state_class demanded a
# CONFIDENTLY DEAD agent before honoring even an authoritatively confirmed
# pause. A churny idle pane (a ticking clock or token counter) produced a fresh
# false stale wake per hash. A declared external wait is healthy idleness: the
# bounded pause cadence owns it, and the long resurface still bounds a
# forgotten pause.
test_live_agent_declared_pause_absorbed_not_surfaced() {
  local dir state fakebin out capture_file statusf window key pane_hash sig pid round wakes
  dir=$(make_case live-agent-paused); state="$dir/state"; fakebin="$dir/fakebin"
  ...
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-held_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  # crew-state authoritatively confirms the declared pause, and the agent
  # process is still alive at the idle prompt (grok, not a bare shell).
  export FM_FAKE_CREW_STATE='state: paused · source: status-log · holding for the upstream tool release'

  # One long-running watcher over a churny idle pane (a ticking clock or token
  # counter): prime the first hash as already stably stale, then keep replacing
  # the pane content so each new hash reaches stale triage in turn. None may
  # surface, start a wedge timer, or leave the pause cadence.
  printf 'idle, holding for upstream (tick 0)' > "$capture_file"
  pane_hash=$(hash_text "idle, holding for upstream (tick 0)")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  while [ "$round" -le 4 ]; do
    ...
    printf 'idle, holding for upstream (tick %s)' "$round" > "$capture_file"
    round=$((round + 1))
  done
  sleep 4
  if ! kill -0 "$pid" 2>/dev/null; then
    fail "live-agent declared pause surfaced on a churny idle pane (false stale alarm): $(cat "$out")"
  fi
  [ -e "$state/.paused-$key" ] || { reap "$pid"; fail "live-agent declared pause did not enter the pause cadence"; }
  [ ! -e "$state/.stale-since-$key" ] || { reap "$pid"; fail "live-agent declared pause started a wedge timer"; }
  wakes=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w { n++ } END { print n + 0 }' "$state/.wake-queue" 2>/dev/null)
  [ "${wakes:-0}" -eq 0 ] || { reap "$pid"; fail "live-agent declared pause raised $wakes stale wakes across churny hashes"; }
  reap "$pid"
  unset FM_FAKE_CREW_STATE
  pass "a live agent behind a declared pause is absorbed as healthy idleness, never surfaced as stale"
}
...
# A still-live agent behind an authoritatively confirmed declared pause is the
# NORMAL paused shape (a waiting worker keeps its agent at the idle prompt): it
# is absorbed on the same bounded cadence, and a re-armed wedge timer must be
# discarded instead of escalating.
test_exited_declared_pause_is_bounded_and_live_agent_pause_absorbed() {
  ...
  window="test:fm-gate"
  printf 'idle, declared external wait with a live agent\n' > "$capture_file"
  printf 'window=%s\nkind=ship\nharness=grok\nbackend=tmux\n' "$window" > "$state/gate.meta"
  printf 'paused: waiting on the upstream release\n' > "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-gate_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle, declared external wait with a live agent")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"

  # First sight of the stale hash with the agent still alive and crew-state
  # authoritatively paused must absorb - never surface a false stale.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  pid=$!
  if ! wait_live "$pid" 30; then
    fail "live-agent declared pause surfaced on first sight (false stale alarm): $(cat "$out")"
  fi
  [ -e "$state/.paused-$key" ] || { reap "$pid"; fail "live-agent declared pause did not enter the pause cadence"; }
  reap "$pid"
  ack_stopped_cycle "$state" || fail "could not acknowledge the intentional live-agent pause stop"

  # Re-arm with the stale timer already beyond the wedge threshold. This is the
  # unchanged-hash fallback while paused: it must retain the pause cadence and
  # discard any residual wedge timer instead of emitting a possible-wedge wake.
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live-agent declared pause escalated on the wedge timer: $(cat "$out")"
  fi
  [ -e "$state/.paused-$key" ] || { reap "$pid"; fail "live-agent declared pause lost its pause cadence marker"; }
  [ ! -e "$state/.stale-since-$key" ] || { reap "$pid"; fail "live-agent declared pause retained the wedge timer"; }
  reap "$pid"
  wakes=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w { n++ } END { print n + 0 }' "$state/.wake-queue")
  bare=$(awk -F '\t' -v w="$window" '$3 == "stale" && $4 == w && $5 == "stale: " w { n++ } END { print n + 0 }' "$state/.wake-queue")
  [ "$wakes" -eq 0 ] || fail "live-agent declared pause raised $wakes stale wakes"
  [ "$bare" -eq 0 ] || fail "live-agent declared pause left a bare stale queued"
  pass "exited declared-pause and captain-held panes use bounded pause cadence, and a live-agent declared pause is absorbed"
}
