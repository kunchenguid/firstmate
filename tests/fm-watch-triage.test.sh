...
run_hold() {  # <dir> <args...>
  local dir=$1
  shift
  PATH="$dir/fakebin:$PATH" FM_HOME="$dir" FM_STATE_OVERRIDE="$dir/state" FM_DATA_OVERRIDE="$dir/data" \
    FM_CONFIG_OVERRIDE="$dir/config" "$ROOT/bin/fm-captain-hold.sh" "$@" >/dev/null 2>&1
}

make_hold_home() {  # <name> <status-line> <hold|nohold>
  local name=$1 line=$2 hold=$3 dir state tasks_axi
  dir=$(make_case "$name"); state="$dir/state"
  mkdir -p "$dir/data" "$dir/config"
  tasks_axi=$(command -v tasks-axi) || return 1
  {
    # shellcheck disable=SC2016 # ${1:-} belongs to the generated wrapper.
    printf '%s\n' '#!/usr/bin/env bash' 'if [ "${1:-}" = --version ]; then printf "0.2.4\\n"; exit 0; fi'
    printf 'exec %q "$@"\n' "$tasks_axi"
  } > "$dir/fakebin/tasks-axi"
  chmod +x "$dir/fakebin/tasks-axi"
  cp "$ROOT/.tasks.toml" "$dir/.tasks.toml" || return 1
  printf '## In flight\n\n## Queued\n\n## Done\n' > "$dir/data/backlog.md"
  (cd "$dir" && PATH="$dir/fakebin:$PATH" tasks-axi add held-merge 'delivered work' --file data/backlog.md) >/dev/null 2>&1 \
    || return 1
  if [ "$hold" = hold ]; then
