...
# The same bounded cadence must hold when the crew's agent is very much ALIVE.
# pause_state_class's liveness gate exists only to RECOVER a paused classification
# for a crew fm-crew-state reports as stopped/unknown; an authoritative `paused`
# verdict must reach the pause cadence on its own. The 2026-08-13 fm-key-position
# incident is the live case: a healthy worker deliberately parked on an upstream
# maintainer's approval, whose live agent forced the class to `none` so every
# fresh pane hash stale-escalated it again.
test_live_agent_declared_pause_absorbed_not_surfaced() {
  ...
  printf 'idle, PR open awaiting upstream approval' > "$capture_file"
  printf 'window=%s\nkind=ship\nharness=grok\nbackend=tmux\n' "$window" > "$state/parked.meta"
  statusf="$state/parked.status"
  printf 'paused: PR 2302 open, CI withheld pending upstream approval - external wait\n' > "$statusf"
  sig=$(seen_sig "$statusf"); printf '%s' "$sig" > "$state/.seen-parked_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle, PR open awaiting upstream approval")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"

  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_FAKE_TMUX_CURRENT_COMMAND=grok \
    FM_FAKE_CREW_STATE='state: paused · source: status-log · PR 2302 open, CI withheld pending upstream approval - external wait · obsolete run superseded (custody returned)' \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" FM_PAUSE_RESURFACE_SECS=999 FM_POLL=1 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "watcher surfaced a live agent's declared pause (should absorb): $(cat "$out")"
  fi
  [ ! -s "$out" ] || fail "a live agent's declared pause printed a wake reason during absorb"
  [ ! -s "$state/.wake-queue" ] || fail "a live agent's declared pause enqueued a wake during absorb"
  [ "$(cat "$state/.stale-$key" 2>/dev/null || true)" = "$pane_hash" ] || fail "stale suppressor not advanced on a live paused absorb"
  [ -e "$state/.paused-$key" ] || fail "paused flag not recorded for a live agent's declared pause"
  [ ! -e "$state/.stale-since-$key" ] || fail "a live paused absorb must not start the wedge timer"
  reap "$pid"
  pass "an authoritative declared pause is absorbed even while the crew's agent is alive"
}
...
# A still-live agent at an external-decision gate takes the pause cadence from
# its own authoritative paused verdict instead of the dead-agent recovery, and a
# residual wedge timer left by an earlier surface must not escalate it.
test_exited_declared_pause_is_bounded_and_live_gate_uses_pause_cadence() {
  ...
  # First sight takes the pause cadence straight from the authoritative paused
  # verdict - the live agent must not demote it to an immediate stale surface.
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
  ...
  pid=$!
  if ! wait_live "$pid" 30; then
    reap "$pid"; fail "live external-decision gate surfaced instead of taking the pause cadence: $(cat "$out")"
  fi
  [ -e "$state/.paused-$key" ] || { reap "$pid"; fail "live external-decision gate did not record the pause cadence marker"; }
  [ ! -s "$state/.wake-queue" ] || { reap "$pid"; fail "live external-decision gate queued a stale wake on first sight"; }
  reap "$pid"
  ...
  # exact unchanged-hash fallback after the first sighting: it must retain the
  # pause cadence and discard any residual wedge timer instead of emitting a
  # possible-wedge wake.
  ...
  if ! wait_live "$pid" 30; then
    reap "$pid"
    fail "live external-decision gate escalated on the residual wedge timer: $(cat "$out")"
  fi
  ...
  reap "$pid"
  [ ! -s "$state/.wake-queue" ] || fail "live external-decision gate queued a wake while inside its pause window"
  pass "exited declared-pause and captain-held panes use bounded pause cadence, and so does a live decision gate"
}
