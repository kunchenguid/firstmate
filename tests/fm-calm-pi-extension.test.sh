...
test_stock_working_presentation() {
  local fixture out status version
  if ! command -v node >/dev/null 2>&1 || ! command -v npm >/dev/null 2>&1; then
    echo "skip: node or npm not found for Pi Calm stock-working test"
    return 0
  fi
  if [ ! -f "$PI_PACKAGE_DIR/package.json" ]; then
    echo "skip: installed @earendil-works/pi-coding-agent package not found"
  ...
  version=$(node -p "require('$PI_PACKAGE_DIR/package.json').version")
  record_pi_version_evidence "$version" "Pi Calm stock-working assumptions"

  fixture="$TMP_ROOT/stock-working"
  mkdir -p "$fixture/home" "$fixture/lib" "$fixture/node_modules/@earendil-works"
  cp "$EXT" "$fixture/fm-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/fm-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/fm-calm-operational-user-layout.ts"
...
const extension = await import(`${pathToFileURL(process.env.EXT).href}?stock=${Date.now()}`);
extension.default(pi);
check(!!calmCommand, "Calm command was not registered");
check(handlers.has("session_start"), "Calm did not register its session-start behavior");
...
const ui = {
  ...
  setWorkingVisible(visible) {
    ...
    this.workingCalls.push(visible);
  },
  ...
  setToolsExpanded(value) {
    this.expanded = value;
    this.expansionCalls.push(value);
  },
  notify() {},
};
const ctx = { ui };
...
const assertStockPresentation = (label) => {
  check(ui.workingVisible === true, `${label} did not keep Pi's stock working indicator enabled`);
  check(
    ui.workingCalls.every((visible) => visible === true),
    `${label} hid Pi's stock working indicator: ${JSON.stringify(ui.workingCalls)}`,
  );
  check(
    ui.widgetCalls.length === 0,
    `${label} installed a custom working widget: ${JSON.stringify(ui.widgetCalls)}`,
  );
};

await fire("session_start", { reason: "startup" });
assertStockPresentation("Calm-off startup");
check(
  ui.hiddenThinkingLabels.at(-1) === undefined,
  "Calm-off startup changed the collapsed-thinking presentation",
);

for (const outcome of ["active", "settled", "abort", "failure"]) {
  await fire("agent_start", { outcome });
  assertStockPresentation(`${outcome} start`);
  await fire("agent_settled", { outcome });
  assertStockPresentation(`${outcome} settle`);
}

await calmCommand.handler("", ctx);
assertStockPresentation("Calm activation");
check(readFileSync(`${process.env.FM_HOME}/config/calm`, "utf8") === "on\n", "Calm activation did not persist on");
check(ui.hiddenThinkingLabels.at(-1) === "", "Calm activation did not hide collapsed-thinking labels");
check(
  registeredTools.length === 7 &&
    ["bash", "edit", "find", "grep", "ls", "read", "write"].every((name) => registeredTools.includes(name)),
  `Calm activation did not retain all built-in wrappers: ${JSON.stringify(registeredTools)}`,
);
check(
  ui.statuses.some(([key, value]) => key === "firstmate-calm" && value === undefined),
  "Calm activation did not retain status-row cleanup",
);
check(
  JSON.stringify(ui.expansionCalls.slice(-2)) === "[true,false]",
  `Calm activation did not retain transcript redraw: ${JSON.stringify(ui.expansionCalls)}`,
);

for (const outcome of ["active", "settled", "abort", "failure"]) {
  await fire("agent_start", { outcome });
  assertStockPresentation(`Calm-on ${outcome} start`);
  await fire("agent_settled", { outcome });
  assertStockPresentation(`Calm-on ${outcome} settle`);
}

for (const reason of ["reload", "new", "resume", "fork"]) {
  await fire("session_shutdown", { reason });
  assertStockPresentation(`session shutdown ${reason}`);
  await fire("session_start", { reason });
  assertStockPresentation(`session start ${reason}`);
}

await calmCommand.handler("", ctx);
assertStockPresentation("Calm deactivation");
check(readFileSync(`${process.env.FM_HOME}/config/calm`, "utf8") === "off\n", "Calm deactivation did not persist off");
check(
  ui.hiddenThinkingLabels.at(-1) === undefined,
  "Calm deactivation did not restore collapsed-thinking labels",
);
JS
)
  status=$?
  [ "$status" -eq 0 ] || fail "Pi Calm stock-working checks failed: $out"
  [ -z "$out" ] || fail "Pi Calm stock-working test printed output: $out"
  pass "Pi Calm installs no custom working widget, keeps Pi's stock indicator enabled through active, settled, abort, failure, reload, replacement, and toggle paths, and preserves persistence, built-in activation, transcript redraw, thinking labels, and status cleanup"
}

test_interactive_terminal_e2e() {
  local project config home session_file export_file export_dom default_snapshot expanded_snapshot hidden_snapshot active_before_snapshot active_hidden_snapshot export_snapshot restored_snapshot working_snapshot working_response_snapshot active_settled_snapshot restarted_snapshot resumed_restored_snapshot hash_before hash_after now version chrome chrome_pid chrome_wait active_wait active_screen_wait
  if ! command -v pi >/dev/null 2>&1 || ! command -v tmux >/dev/null 2>&1; then
  ...
  active_settled_snapshot="$TMP_ROOT/active-settled.txt"
...
export default function (pi: ExtensionAPI): void {
  pi.registerProvider("calm-e2e", {
    ...
    models: [
      ...
      {
        id: "delayed-active",
        name: "Long-delay Calm active-run fixture",
        reasoning: false,
        input: ["text"],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
        contextWindow: 4096,
    ...
    streamSimple(model, _context, options) {
      ...
      void (async () => {
        ...
        await new Promise<void>((resolve) => {
          const timer = setTimeout(resolve, model.id === "delayed-active" ? 90000 : 1500);
  ...
  pi.registerCommand("calm-active-e2e", {
    description: "Start the long-delay Calm active-run fixture.",
    handler: async (_args, ctx) => {
      const model = ctx.modelRegistry.find("calm-e2e", "delayed-active");
      if (!model || !(await pi.setModel(model))) {
        throw new Error("could not select the long-delay Calm E2E model");
      }
      await pi.sendUserMessage("CALM_ACTIVE_E2E_PROMPT");
    },
  });
...
JS
  ...
  # Calm on plus a genuinely active run keeps Pi's stock working row.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/calm-active-e2e"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  active_screen_wait=0
  ...
  assert_contains "$(cat "$working_snapshot")" "Working..." \
    "Calm did not keep Pi's stock working row visible during a real provider wait"
  assert_not_contains "$(cat "$working_snapshot")" '\__/' \
    "Calm rendered the removed working-ship hull during a real provider wait"
  assert_not_contains "$(cat "$working_snapshot")" '<|' \
    "Calm rendered the removed working-ship rightward sail during a real provider wait"
  assert_not_contains "$(cat "$working_snapshot")" '|>' \
    "Calm rendered the removed working-ship leftward sail during a real provider wait"
  assert_not_contains "$(cat "$working_snapshot")" "calm transcript" \
    "the real provider wait showed a persistent Calm status row"
  assert_not_contains "$(cat "$working_snapshot")" \
    "FIRSTMATE WATCHER WAKE: signal: /tmp/probe.status" \
    "the real provider wait restored a hidden operational row"

  # The stock presentation leaves ordinary editor focus and Escape abort unchanged.
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "FOCUSPROBE"
  wait_for_text "$working_snapshot" "FOCUSPROBE" \
    || fail "keyboard input did not reach the editor while Pi's stock working row was active"
  i=0
  while [ "$i" -lt 10 ]; do
  ...
  active_screen_wait=0
  while [ "$active_screen_wait" -lt 200 ]; do
    tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$active_settled_snapshot"
    if ! grep -Fq "Working..." "$active_settled_snapshot"; then
      break
    fi
    sleep 0.05
    active_screen_wait=$((active_screen_wait + 1))
  done
  assert_not_contains "$(cat "$active_settled_snapshot")" "Working..." \
    "Escape did not settle Pi's stock working row"
  assert_not_contains "$(cat "$active_settled_snapshot")" '\__/' \
    "Escape left removed working-widget output"
  assert_not_contains "$(cat "$active_settled_snapshot")" "CALM_WORKING_E2E_RESPONSE" \
    "the long-delay fixture settled instead of aborting on Escape"
  assert_not_contains "$(cat "$active_settled_snapshot")" "FOCUSPROBE" \
    "the editor kept the focus probe text after Escape"

  # A normally settled Calm-on run also uses only Pi's stock working row.
  ...
  assert_contains "$(cat "$working_snapshot")" "Working..." \
    "a normally settled Calm-on run did not show Pi's stock working row"
  assert_not_contains "$(cat "$working_snapshot")" '\__/' \
    "a normally settled Calm-on run showed removed working-widget output"
  ...
  tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" >"$active_settled_snapshot"
  assert_not_contains "$(cat "$active_settled_snapshot")" '\__/' \
    "a settled run left removed working-widget output"
  [ "$(cat "$home/config/calm")" = on ] \
    || fail "the active and settled working-row probes changed the Calm preference"
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/quit"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  pass "Pi calm native E2E keeps Pi's stock working row visible with no custom widget through active, settled, failed, and aborted runs, keeps captain turns visible, hides exact operational user rows without changing persistence, survives restart, and preserves export plus Ctrl+O behavior"
}
