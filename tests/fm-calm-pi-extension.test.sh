...
JSON
  ...
  # Assistant narration remains conversation even when it precedes a tool call.
  assert_contains "$(cat "$hidden_snapshot")" "I will run one command." "/calm removed assistant text before a tool call"
...
JS
  ...
  assert_contains "$(cat "$export_settled_snapshot")" "I will run one command." \
    "/export removed assistant text before a tool call"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
