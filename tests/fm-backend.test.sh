...
# The pre-launch PWD normalization this branch added (export PWD="$(/bin/pwd)",
# defending no-mistakes' git-push gate against a poisoned relative PWD=.;
# data/nm-gatepath-g2) is covered by tests/fm-spawn-batch.test.sh's static
# log-order and /bin/sh-behavior assertions and by tests/fm-backend-tmux-smoke.test.sh's
# real-tmux end-to-end scenario, not by this retired conformance test.
