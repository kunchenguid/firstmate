...
#      and accepts every implemented adapter.
#
...
test_spawn_refuses_incompatible_codex_app_harness() {
  local out status
  out=$(FM_ROOT_OVERRIDE='' FM_HOME='' FM_STATE_OVERRIDE='' FM_DATA_OVERRIDE='' \
    FM_PROJECTS_OVERRIDE='' FM_CONFIG_OVERRIDE='' FM_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/fm-spawn.sh" nope-codex-app-z1 projects/none claude --mode no-mistakes --yolo off --backend codex-app 2>&1)
  status=$?
  [ "$status" -ne 0 ] || fail "fm-spawn --backend codex-app with claude should refuse"
  assert_contains "$out" "backend=codex-app supports only the verified codex harness" "fm-spawn did not enforce the Codex App harness binding"
  pass "fm-spawn.sh: codex-app refuses an incompatible harness before project allocation"
}
