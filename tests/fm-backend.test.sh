...
OLD_BIN_OPTIONAL_SIBLINGS="fm-pending-reply-lib.sh fm-secondmate-command-lib.sh"
...
test_teardown_conformance_old_vs_new() {
  ...
  # as its re-handed-slot guard (assert_treehouse_worktree_owner). The legacy
  # fixture pre-dates that guard, so the probe is asserted on the new log only
  # rather than compared across the two runs.
  ...
  pass "fm-teardown.sh: the re-handed-slot guard runs and treehouse return remains compatible while tmux cleanup uses exact selectors"
}
