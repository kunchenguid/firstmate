...
# inside (a real tmux pane, a normal case for a captain's session).
test_backend_detect_precedence() {
  ...
  pass "fm_backend_detect: no markers -> undetected, HERDR_ENV=1 -> herdr, \$TMUX -> tmux, nested markers resolve innermost-first"
}
...
test_backend_validate_refuses_unknown() {
  ...
  local out
  # bogus names a backend with no adapter at all; tmux and herdr are the known
  # adapters and spawn-supported.
...
test_backend_validate_spawn_accepts_spawn_backends() {
  ...
  fm_backend_validate_spawn zellij 2>/dev/null \
    && fail "fm_backend_validate_spawn should refuse the retired zellij backend"
  fm_backend_validate_spawn orca 2>/dev/null \
    && fail "fm_backend_validate_spawn should refuse the retired orca backend"
  fm_backend_validate_spawn cmux 2>/dev/null \
    && fail "fm_backend_validate_spawn should refuse the retired cmux backend"
  ...
  pass "fm_backend_validate_spawn: the implemented lifecycle backends are spawn-supported and retired ones refuse"
}
