...
test_kimi_grok_and_agy_install_no_unverified_wiring() {
  local state out
  ...
  [ -z "$(fm_busy_sources_for_harness agy)" ] \
    || fail "Agy must trust no semantic source while it uses its isolated rendered fallback"
  ...
  [ "$out" = "busy agy-regex" ] || fail "Agy must classify through its isolated fallback, got '$out'"
  pass "Kimi, Grok, and Agy install no unverified semantic wiring and classify through their own gates"
}
