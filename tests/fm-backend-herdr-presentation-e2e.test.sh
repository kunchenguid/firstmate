...
#
# Every restart/recovery scenario below stops and reprovisions the session
# (simulating a firstmate restart), which leaves earlier fixtures' agents gone
# but their slots still claimed in metadata. fm-spawn now refuses to launch onto
# a slot another task's metadata still claims, so run all of these scenarios
# against their own dedicated project - and therefore their own treehouse pool.
# Each scenario tears its tasks down before the next runs, so this pool only ever
# holds free (unclaimed) slots for a fresh projected spawn, and reclaims resume
# their own worktree; the durable firstmate workspace parent still comes from the
# scenario's home. The projections here are unaffected by - and never collide
# with - the earlier fixtures' orphaned slots left in the main project pool.
...
for RESTART_ID in fm-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  if [ "$RESTART_ID" = fm-hibit-resume-r1 ]; then
    ...
    # A same-identity reclaim resumes its own recorded worktree, so the repeated
    # reclaim reuses the prior one rather than acquiring a fresh slot. Only return
    # the prior worktree when it is genuinely distinct from the now-active one;
    # force-returning the active worktree would terminate the live task before its
    # teardown runs below.
    [ "$PRIOR_RESTART_WT" = "$NEW_RESTART_WT" ] \
