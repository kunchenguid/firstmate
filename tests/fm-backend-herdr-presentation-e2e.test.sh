...
spawn_task() {  # <id> <home> <project>
  ...
  FM_GATE_REFUSE_BYPASS=1 FM_SPAWN_NO_GUARD=1 FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/fm-spawn.sh" "$id" "$project" "sh -c 'while :; do sleep 60; done'" --mode direct-PR --yolo off --backend herdr
}
...
cat > "$SECOND_HOME_A/.fm-secondmate-parent" <<EOF
schema=fm-secondmate-parent.v1
route=local
parent_home=$HOME_DIR
parent_role=root
EOF
cp "$SECOND_HOME_A/.fm-secondmate-parent" "$SECOND_HOME_B/.fm-secondmate-parent"
