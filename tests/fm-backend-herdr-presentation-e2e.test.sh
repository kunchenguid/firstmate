...
normalize_meta() {  # <meta>
  sed -E \
    ...
    -e 's|^started_at=.*$|started_at=<task-start>|' \
    "$1"
}
...
pass "real Herdr lab: Treehouse commands and metadata shape are byte-identical except for endpoint IDs, task start, and spawn incarnation"
