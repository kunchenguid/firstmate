...
test_conditional_stanzas() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" "$RENDER" --harness codex --read-only 1 --afk 1 --x-mode 1 --telegram-mode 1)
  ...
  assert_contains "$out" "- Telegram bridge: active" "Telegram stanza missing"
  assert_contains "$out" "$config/telegram-mode.env" "Telegram stanza did not render the effective config path"
  assert_contains "$out" 'Mode: Codex foreground checkpoint.' "codex snippet missing"
  assert_not_contains "$out" "Source \`config/x-mode.env\`" "snippet kept the repo-relative x-mode config path"
  pass "renderer includes read-only, afk, and effective remote-channel current-state stanzas"
}
...
test_repair_lines() {
  ...
  out=$(FM_HOME="$home" FM_CODEX_WATCH_CHECKPOINT=7 "$RENDER" --harness codex --x-mode 1 --telegram-mode 1 --repair-line)
  assert_contains "$out" "source '$home/config/x-mode.env' and '$home/config/telegram-mode.env' first" "combined remote-channel repair line did not source both effective cadence configs"
  assert_contains "$out" "bin/fm-watch-checkpoint.sh --seconds 7" "combined remote-channel repair line lost the checkpoint helper"
...
test_grok_is_background_notify() {
  ...
  assert_not_contains "$out" "__FM_TELEGRAM_MODE_ENV" "renderer leaked a Telegram path placeholder"
...
test_grok_command_sources_effective_config() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" "$RENDER" --harness grok --x-mode 1 --telegram-mode 1)
  assert_contains "$out" "[ -f '$config/x-mode.env' ] && . '$config/x-mode.env'; [ -f '$config/telegram-mode.env' ] && . '$config/telegram-mode.env'; exec bin/fm-watch-arm.sh" "grok arm command did not use both effective remote-channel config paths"
  pass "grok rendered command sources both effective remote-channel configs"
}
