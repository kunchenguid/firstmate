...
run_render() { /bin/bash "$RENDER" "$@"; }

test_selected_harness_block_only() {
  local out
  out=$(run_render --harness codex)
...
test_unknown_fallback() {
  local out
  out=$(run_render --harness not-real)
...
test_conditional_stanzas() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" run_render --harness codex --read-only 1 --afk 1 --x-mode 1)
...
test_repair_lines() {
  ...
  out=$(FM_HOME="$home" FM_CODEX_WATCH_CHECKPOINT=7 run_render --harness codex --repair-line)
  assert_contains "$out" "/bin/bash bin/fm-watch-checkpoint.sh --seconds 7" "codex repair line did not bypass the checkpoint script shebang"

  out=$(FM_HOME="$home" run_render --harness claude --queue-pending 1 --repair-line)
  ...
  out=$(FM_HOME="$home" FM_CODEX_WATCH_CHECKPOINT=7 run_render --harness codex --x-mode 1 --repair-line)
  assert_contains "$out" "source '$home/config/x-mode.env' first" "x-mode repair line did not source the effective cadence config"
  assert_contains "$out" "/bin/bash bin/fm-watch-checkpoint.sh --seconds 7" "x-mode codex repair line did not bypass the checkpoint script shebang"

  out=$(FM_HOME="$home" run_render --harness opencode --read-only 1 --repair-line)
  assert_contains "$out" "session holding the fleet lock" "read-only repair line missing"

  out=$(FM_HOME="$home" run_render --harness pi --repair-line)
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  out=$(run_render --harness claude --repair-line)
  ...
  out=$(run_render --harness grok --repair-line)
  ...
  out=$(run_render --harness codex --repair-line)
...
test_pi_signed_preserves_identity_with_pi_supervision_protocol() {
  local out ordinary
  out=$(run_render --harness pi-signed)
  ...
  out=$(run_render --harness pi-signed --repair-line)
...
test_grok_command_sources_effective_config() {
  ...
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" run_render --harness grok --x-mode 1)
  assert_contains "$out" "[ -f '$config/x-mode.env' ] && . '$config/x-mode.env'; exec /bin/bash bin/fm-watch-arm.sh" "grok arm command did not bypass the tracked arm script shebang"
...
test_pi_snippet_uses_effective_extension_path() {
  ...
  mkdir -p "$home/state" "$home/config"
  out=$(FM_HOME="$home" run_render --harness pi)
