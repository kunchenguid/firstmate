...
test_worktree_create_defers_pathless_cleanup_to_verified_rollback() {
  ...
  assert_contains "$out" $'wt-no-path::/orca/wt-no-path\t\tterm-no-path' \
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  out=$(cd "$host" && HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" FM_ORCA_CWD_PATH="$wt" \
