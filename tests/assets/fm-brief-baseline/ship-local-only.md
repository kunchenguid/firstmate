...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '{STATE}/ship-local-only.status'`
...
Firstmate steers you through durable message files in '{STATE}/ship-local-only.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '{STATE}/ship-local-only.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '{STATE}/ship-local-only.inbox'/NNN.msg '{STATE}/ship-local-only.inbox'/handled/`.
