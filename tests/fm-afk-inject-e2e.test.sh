...
submit_line() {
  ...
  # Byte-exact prefix match: ${_line:0:1} counts LOCALE characters, so under a
  # C/POSIX locale it compares only the first byte of the 3-byte MARK and
  # misclassifies a genuinely injected digest as user input.
  case "$_line" in
    "$MARK"*) _c="injection" ;;
    *) _c="user" ;;
  esac
