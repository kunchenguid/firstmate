...
test_dc_recording_and_head() {
  ...
  sidecar=$(cat "$dir/home/state/task-a.pr-poll")
  [ "$sidecar" = $'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/37\n~se.kostrov\nyapi-mcps\n37' ] \
...
test_dc_static_poll_contract() {
  ...
  fm_pr_poll_prepare "$state" task-a "$url" "$owner" "$repo" "$number" "$POLL" \
  ...
  cp "$POLL" "$state/task-a.check.sh"
  printf '%s\n%s\n%s\n%s\n' https://github.com/o/r/pull/1 o r 1 > "$state/task-a.pr-poll"
  chmod 0600 "$state/task-a.check.sh" "$state/task-a.pr-poll"
  : > "$dir/bkt.log"
  out=$(FM_TEST_BKT_STATE=MERGED run_dc_poll "$dir")
  ...
  # A malformed (five-line) DC sidecar is rejected before any provider query.
  printf '%s\n%s\n%s\n%s\n%s\n' "$url" "$owner" "$repo" "$number" extra > "$state/task-a.pr-poll"
...
test_dc_migration_rebuild() {
  ...
  [ "$(sed -n '1p' "$state/task-a.pr-poll")" = 'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/7' ] \
    || fail "DC sidecar url line was not exact"
  [ "$(sed -n '2p' "$state/task-a.pr-poll")" = '~se.kostrov' ] || fail "DC sidecar owner was not the project key"
  [ "$(sed -n '3p' "$state/task-a.pr-poll")" = 'yapi-mcps' ] || fail "DC sidecar slug was not exact"
  [ "$(sed -n '4p' "$state/task-a.pr-poll")" = '7' ] || fail "DC sidecar number was not exact"
