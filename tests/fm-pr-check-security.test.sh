...
# install_bkt_stub <dir>: drop a bkt fake onto <dir>/fakebin that serves the
# three call shapes the DC poll and pr_head fetch make (context list, pr view
# state, pr view head), driven by FM_TEST_BKT_* env so a test can pin each
# outcome. FM_TEST_BKT_FAIL short-circuits every call to a nonzero exit.
install_bkt_stub() {
  local dir=$1
  cat > "$dir/fakebin/bkt" <<'SH'
#!/usr/bin/env bash
[ -n "${FM_TEST_BKT_LOG:-}" ] && printf '%s\n' "$*" >> "$FM_TEST_BKT_LOG"
[ "${FM_TEST_BKT_FAIL:-0}" = 0 ] || exit 1
case "$*" in
  *'context list'*)
    printf '%s\n' "${FM_TEST_BKT_CONTEXT:-* yadro (host: b.yadro.com)}"
    ;;
  *'.pull_request.state'*)
    printf '%s\n' "\"${FM_TEST_BKT_STATE:-OPEN}\""
    ;;
  *'.pull_request.fromRef.latestCommit'*)
    printf '%s\n' "\"${FM_TEST_BKT_HEAD:-0123456789abcdef0123456789abcdef01234567}\""
    ;;
  *)
    exit 1
    ;;
...
SH
  chmod +x "$dir/fakebin/bkt"
  : > "$dir/bkt.log"
}

# run_dc_poll <dir>: run the published DC task-a.check.sh with the bkt fake on
# PATH. The caller pins behavior with FM_TEST_BKT_STATE/FAIL/CONTEXT/HEAD, which
# propagate to the poll subprocess as command-prefix assignments.
run_dc_poll() {
  local dir=$1
  FM_TEST_BKT_LOG="$dir/bkt.log" FM_TEST_GH_LOG="$dir/gh.log" PATH="$dir/fakebin:$BASE_PATH" \
    bash "$dir/home/state/task-a.check.sh"
}
...
test_dc_parser_matrix() {
  local row url owner repo number
  while IFS='|' read -r url owner repo number; do
    [ -n "$url" ] || continue
    fm_pr_url_parse "$url" || fail "DC parser rejected canonical URL: $url"
    [ "$FM_PR_URL" = "$url" ] || fail "DC parser changed canonical URL"
    [ "$FM_PR_PROVIDER" = bitbucket-dc ] || fail "DC parser did not set provider=bitbucket-dc for $url"
    [ "$FM_PR_HOST" = b.yadro.com ] || fail "DC parser returned wrong host for $url (got '$FM_PR_HOST')"
    [ "$FM_PR_OWNER" = "$owner" ] || fail "DC parser returned wrong owner for $url (got '$FM_PR_OWNER')"
    [ "$FM_PR_REPO" = "$repo" ] || fail "DC parser returned wrong repository for $url (got '$FM_PR_REPO')"
    [ "$FM_PR_NUMBER" = "$number" ] || fail "DC parser returned wrong PR number for $url (got '$FM_PR_NUMBER')"
  done <<'EOF'
https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1|~se.kostrov|yapi-mcps|1
https://b.yadro.com/users/team-user/repos/repo.with-dots/pull-requests/42|~team-user|repo.with-dots|42
https://b.yadro.com/projects/PROJ/repos/some-repo/pull-requests/7|PROJ|some-repo|7
https://b.yadro.com/projects/MY.TEAM/repos/yapi-firstmate/pull-requests/123456|MY.TEAM|yapi-firstmate|123456
EOF
  # shellcheck disable=SC2016 # Literal rejected URL bytes are parser test data.
  for row in \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1/' \
    ' https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1 ' \
    'https://b.yadro.com/users/se.kostrov/repos/./pull-requests/1' \
    'https://b.yadro.com/users/se.kostrov/repos/../pull-requests/1' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/0' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/01' \
    'https://b.yadro.com/teams/x/repos/r/pull-requests/1' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1/files' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1?q=x' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull/1' \
    'https://b.yadro.com/projects/PROJ/repos/yapi-mcps/merge-requests/1' \
    'http://b.yadro.com/users/u/repos/r/pull-requests/1' \
    'https://b.yadro.com/users//repos/r/pull-requests/1' \
    'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/' \
    'https://byadrocom/users/u/repos/r/pull-requests/1' \
    'https://b.yadro.com./users/u/repos/r/pull-requests/1' \
    'https://b.yadro.com-/users/u/repos/r/pull-requests/1' \
    ; do
    ! fm_pr_url_parse "$row" || fail "DC parser accepted a rejected URL: $row"
  done
  # A real GitHub URL is still classified github even though a DC host capture
  # could in principle also match the dotted github.com host: github must win.
  fm_pr_url_parse 'https://github.com/o/r/pull/1' || fail "github parse regressed under the DC extension"
  [ "$FM_PR_PROVIDER" = github ] || fail "github URL was misclassified as $FM_PR_PROVIDER"
  [ "$FM_PR_HOST" = github.com ] || fail "github host was misclassified as $FM_PR_HOST"
  pass "DC parser accepts both URL shapes with provider/host and rejects the adversarial matrix"
}

test_dc_recording_and_head() {
  local dir expected sidecar
  dir=$(make_case dc-recording)
  install_bkt_stub "$dir"
  write_task_meta "$dir"
  expected=0123456789abcdef0123456789abcdef01234567
  : > "$dir/gh.log"
  FM_TEST_BKT_HEAD=$expected FM_TEST_BKT_LOG="$dir/bkt.log" \
    run_check_entry "$dir" task-a 'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/37' \
    > "$dir/stdout" 2> "$dir/stderr" || fail "valid DC check failed: $(cat "$dir/stderr")"

  grep -qxF 'pr=https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/37' "$dir/home/state/task-a.meta" \
    || fail "canonical DC pr metadata was not exact"
  grep -qxF "pr_head=$expected" "$dir/home/state/task-a.meta" || fail "DC pr_head metadata was not exact"
  cmp -s "$POLL" "$dir/home/state/task-a.check.sh" || fail "DC published check was not byte-for-byte static"
  [ "$(file_mode "$dir/home/state/task-a.check.sh")" = 600 ] || fail "DC published check mode was not 0600"
  fm_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
    || fail "DC published poll provenance or metadata binding was invalid"
  sidecar=$(cat "$dir/home/state/task-a.pr-poll")
  [ "$sidecar" = $'bitbucket-dc\nhttps://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/37\nb.yadro.com\n~se.kostrov/yapi-mcps\n37' ] \
    || fail "DC sidecar bytes were not exact (got: $sidecar)"
  assert_grep 'pr view 37 --project ~se.kostrov --repo yapi-mcps' "$dir/bkt.log" \
    "DC head fetch did not invoke bkt with the split project/repo/number"
  [ ! -s "$dir/gh.log" ] || fail "DC recording invoked the GitHub gh tool"

  # The head SHA came from bkt (no worktree needed); a multiline head is rejected.
  dir=$(make_case dc-recording-newline-head)
  install_bkt_stub "$dir"
  write_task_meta "$dir"
  FM_TEST_BKT_HEAD=$'0123456789abcdef0123456789abcdef01234567\nwindow=unexpected' \
    run_check_entry "$dir" task-a 'https://b.yadro.com/projects/PROJ/repos/some-repo/pull-requests/8' \
    >/dev/null 2>/dev/null || fail "valid DC check with malformed remote head failed"
  assert_no_grep 'pr_head=' "$dir/home/state/task-a.meta" "multiline DC head reached metadata"
  grep -qxF 'pr=https://b.yadro.com/projects/PROJ/repos/some-repo/pull-requests/8' "$dir/home/state/task-a.meta" \
    || fail "DC pr metadata was not recorded alongside a rejected head"

  # Recording still proceeds without pr_head when bkt is absent.
  dir=$(make_case dc-recording-no-bkt)
  write_task_meta "$dir"
  : > "$dir/gh.log"
  run_check_entry "$dir" task-a 'https://b.yadro.com/projects/PROJ/repos/some-repo/pull-requests/5' \
    > "$dir/stdout" 2> "$dir/stderr" || fail "DC check without bkt failed"
  grep -qxF 'pr=https://b.yadro.com/projects/PROJ/repos/some-repo/pull-requests/5' "$dir/home/state/task-a.meta" \
    || fail "DC pr metadata was not recorded without bkt"
  assert_no_grep 'pr_head=' "$dir/home/state/task-a.meta" "DC recording wrote a pr_head without bkt available"
  [ ! -s "$dir/gh.log" ] || fail "DC recording without bkt invoked gh"
  fm_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
    || fail "DC poll without bkt was not armed"
  pass "DC recording stores exact pr=/pr_head= via bkt and arms a static poll; GitHub tools stay untouched"
}

test_dc_static_poll_contract() {
  local dir state url owner repo number out state_val
  dir=$(make_case dc-poll-contract)
  state="$dir/home/state"
  install_bkt_stub "$dir"
  url='https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/1'
  owner='~se.kostrov'
  repo='yapi-mcps'
  number=1
  write_poll_meta "$state" task-a "$url"
  fm_pr_poll_prepare "$state" task-a bitbucket-dc "$url" b.yadro.com "$owner/$repo" "$number" "$POLL" \
    || fail "could not prepare DC poll sidecar"
  fm_pr_poll_publish_prepared || fail "could not publish DC poll sidecar"
  cmp -s "$POLL" "$state/task-a.check.sh" || fail "DC published check was not byte-for-byte static"

  out=$(FM_TEST_BKT_STATE=MERGED run_dc_poll "$dir")
  [ "$out" = merged ] || fail "DC poll did not emit exactly one merged line for MERGED (got: '$out')"
  for state_val in OPEN DECLINED '' NOTASTATE; do
    out=$(FM_TEST_BKT_STATE="$state_val" run_dc_poll "$dir")
    [ -z "$out" ] || fail "DC poll emitted for non-merged state '$state_val' (got: '$out')"
  done

  out=$(FM_TEST_BKT_FAIL=1 run_dc_poll "$dir")
  [ -z "$out" ] || fail "DC poll emitted after a bkt failure"
  out=$(PATH="$BASE_PATH" bash "$state/task-a.check.sh")
  [ -z "$out" ] || fail "DC poll emitted when bkt was absent from PATH"

  out=$(FM_TEST_BKT_CONTEXT=$'* yadro (host: b.yadro.com)\n  other (host: b.yadro.com)' run_dc_poll "$dir")
  [ -z "$out" ] || fail "DC poll emitted for an ambiguous context list"
  out=$(FM_TEST_BKT_CONTEXT='* other (host: other.example.com)' run_dc_poll "$dir")
  [ -z "$out" ] || fail "DC poll emitted when no context matched the host"

  # A GitHub-shaped sidecar never reaches the DC branch even with bkt present.
  cp "$POLL" "$state/task-a.check.sh"
  printf '%s\n%s\n%s\n%s\n%s\n' github https://github.com/o/r/pull/1 github.com o/r 1 > "$state/task-a.pr-poll"
  chmod 0600 "$state/task-a.check.sh" "$state/task-a.pr-poll"
  : > "$dir/bkt.log"
  out=$(FM_TEST_BKT_STATE=MERGED run_dc_poll "$dir")
  [ -z "$out" ] || fail "GitHub sidecar reached the DC bkt branch"
  [ ! -s "$dir/bkt.log" ] || fail "GitHub sidecar invoked bkt"

  # A malformed (six-line) DC sidecar is rejected before any provider query.
  printf '%s\n%s\n%s\n%s\n%s\n%s\n' bitbucket-dc "$url" b.yadro.com "$owner/$repo" "$number" extra > "$state/task-a.pr-poll"
  chmod 0600 "$state/task-a.pr-poll"
  out=$(FM_TEST_BKT_STATE=MERGED run_dc_poll "$dir")
  [ -z "$out" ] || fail "DC poll emitted from a multiline sidecar"

  pass "DC static poll emits one merged line for MERGED and stays silent otherwise"
}

test_dc_migration_rebuild() {
  local dir state
  dir=$(make_case dc-migration)
  state="$dir/home/state"
  fm_write_meta "$state/task-a.meta" \
    'window=fm-task-a' \
    'pr=https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/7'
  printf 'legacy DC bytes\n' > "$state/task-a.check.sh"

  FM_HOME="$dir/home" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err" \
    || fail "DC migration failed: $(cat "$dir/migrate.err")"
  [ "$(cat "$dir/migrate.out")" = 'PR_CHECK_MIGRATION: canonical polls rebuilt and armed; resume supervision for this home' ] \
    || fail "DC migration did not report an armed canonical poll"
  cmp -s "$POLL" "$state/task-a.check.sh" || fail "DC migration did not rebuild a canonical static poll"
  [ "$(file_mode "$state/task-a.check.sh")" = 600 ] || fail "migrated DC check mode was not 0600"
  fm_pr_poll_artifacts_valid "$state" task-a "$POLL" \
    || fail "DC migration did not leave a validated armed poll"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  [ "$(sed -n '1p' "$state/task-a.pr-poll")" = bitbucket-dc ] \
    || fail "DC sidecar provider line was not exact"
  [ "$(sed -n '2p' "$state/task-a.pr-poll")" = 'https://b.yadro.com/users/se.kostrov/repos/yapi-mcps/pull-requests/7' ] \
    || fail "DC sidecar url line was not exact"
  [ "$(sed -n '3p' "$state/task-a.pr-poll")" = 'b.yadro.com' ] || fail "DC sidecar host was not exact"
  [ "$(sed -n '4p' "$state/task-a.pr-poll")" = '~se.kostrov/yapi-mcps' ] || fail "DC sidecar path was not exact"
  [ "$(sed -n '5p' "$state/task-a.pr-poll")" = '7' ] || fail "DC sidecar number was not exact"
  pass "non-executing migration rebuilds a DC pr= into an authenticated static poll"
}
