...
# A tmux window id (@N) is unique server-wide, so it is a valid kill target on
# its own: it carries no session to qualify and no name that could fuzzy-match a
# lookalike window, which is exactly why fm-spawn's settle abort addresses the
# window it created by id rather than by the fm-<id> name. The id must reach tmux
# verbatim - wrapping it in the '=' exactness markers the NAME form needs would
# ask tmux for a window literally called "@7".
test_tmux_window_id_target_is_killed_verbatim() {
  local dir rc
  dir=$(make_case window-id)
  set +e
  FM_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    bash -c '. "$1/bin/fm-backend.sh"; fm_backend_source tmux; fm_backend_tmux_kill "@7"' _ "$ROOT" \
    > "$dir/stdout" 2> "$dir/stderr"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "a tmux window id target was refused: $(cat "$dir/stderr")"
  grep -Fqx "tmux <kill-window> <-t> <@7>" "$dir/runtime.log" \
    || fail "window id target did not reach tmux verbatim: $(cat "$dir/runtime.log")"
  pass "tmux backend: a window id target is killed verbatim, without session qualification or '=' markers"
}

# The id form must stay as narrow as the name form: anything that merely starts
...
test_tmux_malformed_window_id_refuses_without_invocation() {
  ...
  for target in '@7:x' '@' '@7a' '@7@8' '@1@'; do
    ...
    set +e
    FM_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
      bash -c '. "$1/bin/fm-backend.sh"; fm_backend_source tmux; fm_backend_tmux_kill "$2"' _ "$ROOT" "$target" \
      > "$dir/stdout" 2> "$dir/stderr"
    rc=$?
    set -e
    [ "$rc" -ne 0 ] || fail "malformed window id '$target' unexpectedly succeeded"
    [ ! -s "$dir/runtime.log" ] || fail "malformed window id '$target' invoked tmux: $(cat "$dir/runtime.log")"
  done
  pass "tmux backend: a malformed window id refuses without invoking tmux"
}
