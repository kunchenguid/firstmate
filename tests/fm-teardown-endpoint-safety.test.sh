...
test_metadata_lock_serializes_destructive_cleanup() {
  local dir id=metadata-locked-task lock ready release holder teardown_pid i=0 rc occupancy_json
  dir=$(make_case metadata-lock)
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:fm-$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout" \
    "treehouse_lease=lease-metadata-lock" "treehouse_slot=slot-metadata-lock"
  occupancy_json=$(jq -cn --arg path "$dir/worktree" --arg holder "$id" \
    '[{name:"slot-metadata-lock",path:$path,status:"leased",lease_id:"lease-metadata-lock",lease_holder:$holder}]')
  ...
  FM_FAKE_TREEHOUSE_STATUS_JSON="$occupancy_json" run_case "$dir" "$id" > "$dir/stdout" 2> "$dir/stderr" &
  teardown_pid=$!
  sleep 0.2
