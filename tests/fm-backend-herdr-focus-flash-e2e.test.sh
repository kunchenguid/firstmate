...
# One persistent child of the pane's shell, started outside any worktree so
# nothing reaps it, is enough to fail the proof on every sample. Keep it in the
# foreground so POSIX shells cannot reparent an asynchronous job on Linux.
