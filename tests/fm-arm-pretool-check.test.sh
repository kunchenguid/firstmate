...
test_pi_extension_carries_pretool_check() {
  ...
  assert_contains "$content" 'String(event.input?.command ?? "")' "pi must extract and string-coerce event.input.command exactly"
  assert_contains "$content" 'applyDefaultBashTimeout(event.input)' "pi must apply the default timeout to validated bash calls"
  assert_not_contains "$content" 'pi.registerTool' "pi must not replace the active bash tool"
