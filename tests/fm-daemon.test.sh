...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  ...
  : > "$state/.validation-since-$watcher_key"
  : > "$state/.validation-resurfaced-$watcher_key"
  ...
  [ ! -e "$state/.validation-since-$watcher_key" ] || fail "terminal signal retained watcher validation tracking"
  [ ! -e "$state/.validation-resurfaced-$watcher_key" ] || fail "terminal signal retained watcher validation recheck tracking"
  FM_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
