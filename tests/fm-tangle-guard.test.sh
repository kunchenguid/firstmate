...
# (so the spawn's worktree-settle loop resolves to a path we control), names
...
#     are disabled so the fm-<id> name survives the pane cd'ing into its leased
#     worktree;
...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): the worktree-entry cd and the worktree wait loop target the
  # stable id.
