...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): the worktree acquire and the wait loop target the stable id.
  # The acquire carries this home's own worktree root, so match around it rather
  # than pinning the bare command.
  grep -E "send-keys -t @spawnwid treehouse( --root [^ ]+)? get Enter" "$rec" >/dev/null \
    || fail "the worktree acquire must be sent to the stable window id"
