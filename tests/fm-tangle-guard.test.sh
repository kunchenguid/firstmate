...
SH
  ...
  # told to enter it, and the refusal names the path and why it was rejected.
  # The assertions pin which cause fired, not the operator wording that
  # explains it.
...
#   - the cd into the leased slot and the pane-arrival wait loop target that
#     stable window id, never the (possibly-renamed) name - a lost name would let
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "display-message -p -t @spawnwid #{pane_current_path}" "$rec" \
    "the pane-arrival wait loop must query the stable window id, not the name"
