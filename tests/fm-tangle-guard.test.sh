...
SH
  ...
  # Candidate paths require two stable reads before the isolation guard runs,
  # so make that confirmation sleep deterministic and instant.
...
test_spawn_isolation_abort() {
  ...
  assert_contains "$out" "did not yield an isolated worktree" "non-worktree spawn lacked the isolation refusal"
  ...
  assert_contains "$out" "did not yield an isolated worktree" "primary-checkout spawn lacked the isolation refusal"
