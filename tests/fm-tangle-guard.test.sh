...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): scoped treehouse-get and the worktree wait loop target the stable id.
  assert_grep "send-keys -t @spawnwid treehouse --root" "$rec" \
    "scoped treehouse get must be sent to the stable window id"
  assert_grep "get Enter" "$rec" \
    "the scoped Treehouse command must acquire a worktree"
