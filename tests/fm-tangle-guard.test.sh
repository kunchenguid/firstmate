...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "send-keys -t @spawnwid fm_slot=\$(treehouse get --lease --lease-holder" "$rec" \
    "treehouse get must be sent to the stable window id"
  assert_grep "( cd -- \"\$fm_slot\" && exec \"\${SHELL:-/bin/sh}\" ) Enter" "$rec" \
    "the treehouse lease acquisition line sent to the stable window id must enter the leased slot in a child shell"
