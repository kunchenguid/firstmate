...
test_allowed_message_becomes_a_note() {
  ...
  # the chat: the text ack it replaced reached the captain before firstmate had
  # even read the message, and arrived as a separate line to read.
  [ "$(sent_count)" = 0 ] \
...
test_a_non_message_update_is_consumed() {
  local home
  home=$(make_home nonmessage)
  reset_sent
  set_failures 0
  # An update that is not a new message (an edit, here) must still be
  # acknowledged. Left pending, every later getUpdates returns it immediately,
  # so the collector spins instead of long-polling and never moves past it.
  set_updates "$(jq -nc '[{update_id: 150, edited_message: {message_id: 9, chat: {id: 555}, text: "edited"}}]')"
  run_poll "$home" 555 >/dev/null 2>&1
  [ "$(cat "$home/state/telegram/offset" 2>/dev/null)" = 151 ] \
    || fail "a non-message update was never acknowledged"
  [ "$(note_count "$home")" = 0 ] || fail "a non-message update became a note"

  set_updates "$(jq -nc '[{update_id: 151, edited_message: {message_id: 9, chat: {id: 999}, text: "x"}}]' \
    | jq -c --argjson m "$(message_json 152 555 'after the edit')" '. + $m')"
  run_poll "$home" '' >"$TMP_ROOT/nonmessage-out" 2>&1
  grep -q 'status: unconfigured' "$TMP_ROOT/nonmessage-out" \
    || fail "discovery did not report the real message behind a non-message update"
  grep -q 'chat_id: 555' "$TMP_ROOT/nonmessage-out" \
    || fail "discovery reported a non-message update's chat instead of the message's"
  pass "an update that is not a message is acknowledged rather than replayed forever"
}
