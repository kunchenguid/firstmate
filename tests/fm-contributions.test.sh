...
SH
  ...
  if out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" poll 2>&1); then
    fail 'an empty contribution input was reported as a successful poll'
  fi
  printf '%s' "$out" | grep -F 'fm-contributions: contribution snapshot was empty' >/dev/null \
    || fail "an empty contribution input did not surface as an error: $out"
  pass 'an empty contribution input is an error, not zero-record coverage'
}
