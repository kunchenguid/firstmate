...
test_large_backlog_contribution_input_survives_argv_limit() {
  # Regression: a large analytics-home backlog produces a structured document
  # far larger than Linux's 128 KiB single-argv limit (MAX_ARG_STRLEN). The
  # earlier --argjson contribution-input path died with "jq: Argument list too
  # long" and emitted nothing at every session start. The read must now stage
  # the document and return every record.
  local home out structured_bytes records
  home=$(new_home large-backlog-argv)
  local i
  for i in $(seq 1 400); do
    printf -- '- [ ] task-%04d - Ship the analytics dashboard refactor number %04d for the recurring session, involving many subsystems and long descriptive prose to inflate structured size https://github.com/org/repo/pull/%d (repo: analytics-home-sample) (kind: ship)\n' \
      "$i" "$i" "$i" >> "$home/data/backlog.md"
  done
  with_home "$home" "$ROOT/bin/fm-fleet-snapshot.sh" --contribution-input > "$home/input.json" \
    || fail 'contribution-input read failed on a large backlog (argv overflow?)'
  structured_bytes=$(jq -c '.backlog' "$home/input.json" | wc -c)
  [ "$structured_bytes" -gt 131072 ] \
    || fail "structured backlog only $structured_bytes bytes; too small to exercise the argv limit"
  records=$(jq '.backlog.records | length' "$home/input.json")
  [ "$records" -eq 400 ] || fail "expected 400 backlog records, read $records"
  out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" snapshot "$home/input.json" --all) \
    || fail 'consumer could not project the large contribution input'
  printf '%s' "$out" | jq -e '.known == 400 and .unreadable_records == 0' >/dev/null \
    || fail "large contribution input did not reach the consumer intact: $out"
  pass 'a backlog exceeding the argv limit is staged and read with every record intact'
}
...
failures=0
for test_name in test_actor_coverage test_stale_verdict test_unchecked_is_not_silence test_newest_check_has_no_verdict test_comment_wake test_review_wake test_inline_wake test_ready_issue_wake test_fresh_issue_requires_maintainer test_missing_lane_remains_missing test_partial_freshness_keeps_measured_rows test_malformed_record_cannot_prove_silence test_issue_timeline_and_exact_ack test_verdict_retains_judged_head test_observed_replacement_refreshes_verdict test_unobserved_head_leaves_verdict_unknown test_large_backlog_contribution_input_survives_argv_limit test_away_yolo_is_fleet_work test_away_yolo_cross_home_is_fleet_work test_retired_and_unsupported_coverage test_unsupported_forge_is_not_fleet_work test_held_unsupported_forge_is_not_captain_work test_shared_contribution_signal_wakes_once test_watcher_keeps_diagnostics_separate_from_contribution_wakes test_expired_child_unsupported_forge_stays_unmeasured test_watcher_surfaces_new_contribution_once test_home_summary_coverage test_unreadable_pending_is_not_empty test_budget_refusal_between_calls test_budget_bounded_call_timeout test_genuine_failure_near_deadline_is_unavailable test_shared_url_observed_once test_terminal_contribution_settles test_late_owner_inherits_terminal_observation test_done_task_open_pr_still_observed test_failure_wakes_once_per_episode test_late_owner_keeps_failure_episode_suppressed; do
