...
test_contribution_input_handles_large_payload() {
  local home
  home=$(new_home large-input)
  head -c 2200000 /dev/zero | tr '\0' 'x' >> "$home/data/backlog.md"
  printf '\n' >> "$home/data/backlog.md"
  with_home "$home" "$ROOT/bin/fm-fleet-snapshot.sh" --contribution-input > "$home/input.json" 2> "$home/input.err" \
    || fail 'the contribution input command rejected a large local payload'
  jq -e '.backlog.present == true and (.backlog.records | length) > 0 and (.tasks | type) == "array"' \
    "$home/input.json" >/dev/null \
    || fail 'the large contribution input was not valid structured JSON'
  [ ! -s "$home/input.err" ] || fail "large contribution input emitted an error: $(cat "$home/input.err")"
  pass 'large contribution input uses file transport'
}
