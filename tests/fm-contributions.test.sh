...
test_snapshot_rejects_invalid_input() {
  local home input mode out
  home=$(new_home invalid-snapshot-input)
  input="$home/input.json"
  for mode in missing empty whitespace malformed null array missing-fields wrong-backlog wrong-tasks multiple; do
    case "$mode" in
      missing) ;;
      empty) : > "$input" ;;
      whitespace) printf ' \n' > "$input" ;;
      malformed) printf '{' > "$input" ;;
      null) printf 'null\n' > "$input" ;;
      array) printf '[]\n' > "$input" ;;
      missing-fields) printf '{}\n' > "$input" ;;
      wrong-backlog) printf '{"backlog":[],"tasks":[]}\n' > "$input" ;;
      wrong-tasks) printf '{"backlog":{},"tasks":{}}\n' > "$input" ;;
      multiple) printf '{"backlog":{"present":true,"records":[]},"tasks":[]}\n%.0s' 1 2 > "$input" ;;
    esac
    if out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" snapshot "$input" 2> "$home/error"); then
      fail "snapshot accepted $mode input: $out"
    fi
    [ -z "$out" ] || fail "snapshot emitted coverage for $mode input: $out"
    grep -F 'fm-contributions: contribution snapshot' "$home/error" >/dev/null \
      || fail "snapshot did not explain $mode input rejection"
  done
  printf '{"backlog":{"present":true,"records":[]},"tasks":[]}\n' > "$input"
  out=$(with_home "$home" "$ROOT/bin/fm-contributions.sh" snapshot "$input") \
    || fail 'snapshot rejected valid zero-ownership input'
  printf '%s' "$out" | jq -e '.known == 0 and .checked == 0 and .unreadable_records == 0' >/dev/null \
    || fail "valid zero-ownership input did not project cleanly: $out"
  pass 'snapshot rejects missing, empty and invalid input but accepts valid empty ownership'
}

test_empty_contribution_input_is_an_error() {
  local home out mode expected
  home=$(new_home empty-contribution-input)
  ...
  case "$arg" in
    */contribution-tasks.json)
      case "$INPUT_FAULT" in
        empty) exit 0 ;;
        invalid) printf '{}\n'; exit 0 ;;
        failed) printf '{"backlog":{"present":true,"records":[]},"tasks":[]}\n'; exit 42 ;;
      esac
      ;;
  esac
done
...
SH
  ...
  for mode in empty invalid failed; do
    case "$mode" in
      empty) expected='contribution snapshot was empty' ;;
      invalid) expected='contribution snapshot was invalid' ;;
      failed) expected='contribution snapshot unavailable' ;;
    esac
    if out=$(INPUT_FAULT="$mode" with_home "$home" "$ROOT/bin/fm-contributions.sh" poll 2> "$home/error"); then
      fail "$mode contribution input was reported as a successful poll"
    fi
    [ "$out" = "contributions: $expected" ] \
      || fail "$mode contribution input did not surface on the stdout diagnostic channel: $out"
    grep -F "fm-contributions: $expected" "$home/error" >/dev/null \
      || fail "$mode contribution input lost its stderr diagnostic"
  done
  pass 'empty, invalid and failed contribution input produce stdout diagnostics and fail'
}
...
failures=0
for test_name in test_actor_coverage test_stale_verdict test_unchecked_is_not_silence test_newest_check_has_no_verdict test_snapshot_rejects_invalid_input test_empty_contribution_input_is_an_error test_comment_wake test_review_wake test_inline_wake test_ready_issue_wake test_fresh_issue_requires_maintainer test_missing_lane_remains_missing test_partial_freshness_keeps_measured_rows test_malformed_record_cannot_prove_silence test_issue_timeline_and_exact_ack test_verdict_retains_judged_head test_observed_replacement_refreshes_verdict test_unobserved_head_leaves_verdict_unknown test_away_yolo_is_fleet_work test_away_yolo_cross_home_is_fleet_work test_retired_and_unsupported_coverage test_unsupported_forge_is_not_fleet_work test_held_unsupported_forge_is_not_captain_work test_shared_contribution_signal_wakes_once test_watcher_keeps_diagnostics_separate_from_contribution_wakes test_expired_child_unsupported_forge_stays_unmeasured test_watcher_surfaces_new_contribution_once test_home_summary_coverage test_unreadable_pending_is_not_empty test_budget_refusal_between_calls test_budget_bounded_call_timeout test_genuine_failure_near_deadline_is_unavailable test_shared_url_observed_once test_terminal_contribution_settles test_late_owner_inherits_terminal_observation test_done_task_open_pr_still_observed test_reservation_defers_later_url_when_fifteen_seconds_do_not_remain test_three_second_pr_reads_complete_fresh_in_one_cycle test_unavailable_forge_records_error_and_wakes_once_per_episode test_late_owner_keeps_failure_episode_suppressed; do
