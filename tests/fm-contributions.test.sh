...
test_rejected_forge_host_narrows_coverage_without_failing_reads() {
  ...
  grep -F 'not a host/' "$home/warnings.txt" >/dev/null \
    || fail 'the rejected-host warning did not name the offending line'
...
test_forge_reads_only_reach_allowlisted_hosts() {
  local home url='https://precision-it.ghe.com/o/r/pull/8' unlisted='https://code.acme.test/o/r/pull/19'
  ...
  printf '# Backlog\n\n## Queued\n- [ ] delivery - Contribution delivery %s (repo: sample) (kind: ship)\n- [ ] unlisted - Filed %s (repo: sample) (kind: ship)\n' \
    "$url" "$unlisted" > "$home/data/backlog.md"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null \
    || fail 'a listed enterprise contribution poll failed'
  jq -e --arg now "$NOW" '.records[0].checked_at == $now and .records[0].error == null' \
    "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'a listed enterprise observation was not recorded as measured'
  grep -Fx 'api --hostname precision-it.ghe.com repos/o/r/pulls/8' "$home/forge/calls" >/dev/null \
    || fail 'a listed enterprise API read fell back to the default GitHub host'
  ! grep -F 'code.acme.test' "$home/forge/calls" >/dev/null \
...
test_forge_host_rejection_reaches_the_captain_through_the_watcher() {
  ...
  forge_home "$home"
  with_home "$home" "$ROOT/bin/fm-pr-check.sh" delivery https://github.com/o/r/pull/8 >/dev/null \
    || fail 'could not register delivery for the forge-host refusal watcher wake'
  registered_checks "$home" >/dev/null
  printf 'precision-it.ghe.com\nnot a host/\n' > "$home/config/forge-hosts"
  out="$home/watcher-refusal.out"
  rc=0
  with_home "$home" env FM_POLL=1 FM_SIGNAL_GRACE=0 FM_CHECK_INTERVAL=0 FM_HEARTBEAT=999999 \
