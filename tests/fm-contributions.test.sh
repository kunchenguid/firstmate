...
case "$*" in
  ...
  'api repos/o/r/pulls/8')
    jq -n --arg head "$(cat "$FORGE/head")" --arg state "$(cat "$FORGE/state" 2>/dev/null || printf open)" \
      --arg head_repo "$(cat "$FORGE/head-repo" 2>/dev/null || true)" '
      {state:(if $state == "open" then "open" else "closed" end),user:{login:"author"},
       head:({sha:$head} + (if $head_repo == "" then {} else {repo:{full_name:$head_repo}} end)),draft:false,
...
test_cross_repo_checks_use_head_repo() {
  local home
  home=$(new_home cross-repo)
  forge_home "$home"
  wrap_forge "$home"
  printf 'fork/r\n' > "$home/forge/head-repo"
  with_home "$home" "$ROOT/bin/fm-contributions.sh" poll >/dev/null || fail 'cross-repo poll failed'
  jq -e --arg now "$NOW" '.records[0].checked_at == $now and .records[0].error == null
    and (.records[0].observation.checks | length) == 1' "$home/data/delivery/contributions.json" >/dev/null \
    || fail 'a cross-repo observation was not recorded fresh'
  grep -qF "repos/fork/r/commits/$HEAD_A/check-runs?" "$home/forge/calls" \
