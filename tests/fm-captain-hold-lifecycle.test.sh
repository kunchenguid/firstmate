...
# the floor bin/fm-tasks-axi-lib.sh owns.
...
# tasks-axi quotes multi-entry blocked_by values as "a,b,c". The compatibility
# shim must normalize that serialization before matching the final edge.
test_legacy_resolve_matches_quoted_blocked_by_tail() {
  local home origin first last task blocked_line show
  home=$(make_home legacy-quoted-blockers)
  origin=sample-legacy-quoted-review
  mkdir -p "$home/data/$origin"
  tasks_in "$home" add "$origin" "Review quoted legacy blockers" \
    --kind scout --repo sample --start >/dev/null \
    || fail "could not create the quoted-blocker origin"
  write_origin_meta "$home" "$origin"
  first=$(run_shim "$home" hold "$origin" first-choice \
    --title "Choose the first option" --reason "captain first choice pending" --repo sample) \
    || fail "could not create the first legacy hold"
  last=$(run_shim "$home" hold "$origin" last-choice \
    --title "Choose the last option" --reason "captain last choice pending" --repo sample) \
    || fail "could not create the last legacy hold"
  task=sample-legacy-quoted-work
  tasks_in "$home" add "$task" "Apply the quoted legacy choice" \
    --kind ship --repo sample --blocked-by "$first" >/dev/null \
    || fail "could not create the quoted-blocker work item"
  tasks_in "$home" block "$task" --by "$last" >/dev/null \
    || fail "could not append the final legacy blocker"
  ...
  printf 'Use the last quoted option.\n' > "$home/quoted-answer.txt"
  run_shim "$home" resolve "$origin" last-choice \
    --decision-file "$home/quoted-answer.txt" --routed-to "$task" >/dev/null \
    || fail "the shim could not resolve the last quoted blocked_by edge"
  show=$(tasks_in "$home" show "$task" --full)
  blocked_line=$(printf '%s\n' "$show" | sed -n 's/^  blocked_by: //p' | head -1)
  assert_not_contains "$blocked_line" "$last" "the shim left the final quoted blocker attached"
  assert_contains "$blocked_line" "$first" "the shim removed an unrelated quoted blocker"
  pass "the compatibility shim resolves a final edge in quoted multi-value blocked_by"
}
