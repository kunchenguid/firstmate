...
run_teardown() {  # <home> <id>
  ...
  PATH="$home/fakebin:$PATH" FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" \
    FM_STATE_OVERRIDE="$home/state" FM_DATA_OVERRIDE="$home/data" \
    FM_CONFIG_OVERRIDE="$home/config" FM_PUBLIC_FOLLOWUP_PRIMARY_HOME='' "$TEARDOWN" "$id"
}
...
configure_merged_github() {  # <home>
  ...
  git -C "$home/projects/sample" remote add origin git@github.com:sample/sample.git
  ...
  *mergeable_state*) printf 'true\n' ;;
  "api /repos/"*"/pulls/"*)
    printf '%s\n' \
      ...
      'merged=false' \
      'ref=fm/sample' \
      'headrepo=sample/sample' \
      'baserepo=sample/sample'
    ;;
esac
SH
...
test_pr_merge_entrypoint_refuses_a_captain_held_task() {
  ...
  assert_no_grep 'api PUT /repos/sample/sample/pulls/31/merge ' "$home/gh-axi.log" \
...
test_pr_merge_entrypoint_separates_an_unreadable_record_from_an_absent_one() {
  ...
  assert_no_grep 'api PUT /repos/sample/sample/pulls/43/merge ' "$home/gh-axi.log" \
  ...
  merge_count=$(grep -c 'api PUT /repos/sample/sample/pulls/43/merge ' "$home/gh-axi.log" || true)
...
test_merge_entrypoints_refuse_a_reused_task_incarnation() {
  ...
  assert_no_grep 'api PUT /repos/sample/sample/pulls/42/merge ' "$home/gh-axi.log" \
...
test_merge_entrypoints_serialize_forced_teardown_before_task_reads() {
  ...
  wt="$home/projects/$id"
  fm_git_worktree "$repo" "$wt" "fm/$id"
  git -C "$repo" remote set-url origin git@github.com:sample/sample.git
...
SH
  ...
  assert_grep 'api PUT /repos/sample/sample/pulls/33/merge ' "$home/gh-axi.log" \
