...
# Done retention pinned into the copy. Several fixtures hold more than a handful
# of captain calls against one origin, complete them, and then read each record
# back by id. `done_keep` is an operator tunable the tracked config is free to
# lower, and inheriting it would archive those completed records out from under
# assertions that are about captain-hold lifecycle, not Done retention.
write_tasks_config() {  # <home> [done-keep]
  local home=$1 keep=${2:-100}
  sed "s/^done_keep = .*/done_keep = $keep/" "$ROOT/.tasks.toml" > "$home/.tasks.toml" \
    || fail "could not write the fixture backlog config"
  grep -q "^done_keep = $keep\$" "$home/.tasks.toml" \
