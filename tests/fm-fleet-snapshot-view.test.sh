...
case "${1:-}" in
  list-windows)
    ...
    ;;
  list-panes)
    # The endpoint-presence check answers from the exit status alone; every window
    # here is present, including the dead-secondmate fixture, whose pane is live
    # and whose exited AGENT the pane_current_command branch below reports.
    printf '%%1\n'
    ;;
  display-message)
    case "$*" in
