...
EOF
  ...
  fakebin=$(make_fake_spawn_toolchain "$w")
  fm_fake_exit0 "$fakebin" node gh-axi lavish-axi gh treehouse no-mistakes tasks-axi quota-axi
...
EOF
  fakebin=$(make_fake_spawn_toolchain "$w")
  fm_fake_exit0 "$fakebin" node gh-axi lavish-axi gh treehouse no-mistakes tasks-axi quota-axi pgrep
