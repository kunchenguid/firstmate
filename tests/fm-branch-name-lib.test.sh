...
[ "$(fm_ship_branch_name 'fm/' 've-1263-deploy-uat')" = "fm/VE-1263-deploy-uat" ] ||
  fail "already-dashed ticket was not uppercased: $(fm_ship_branch_name 'fm/' 've-1263-deploy-uat')"
pass "a leading ve-1263 ticket becomes VE-1263"

[ "$(fm_ship_branch_name 'fm/' 've1262-backoff-retry')" = "fm/VE-1262-backoff-retry" ] ||
  fail "dashless ticket was not canonicalized: $(fm_ship_branch_name 'fm/' 've1262-backoff-retry')"
pass "a dashless ve1262 ticket gains its canonical dash and case"

[ "$(fm_ship_branch_name 'fm/' 'visto-pipelines-ve1262-backoff-retry')" = "fm/visto-pipelines-VE-1262-backoff-retry" ] ||
  fail "an embedded ticket was not normalized in place: $(fm_ship_branch_name 'fm/' 'visto-pipelines-ve1262-backoff-retry')"
pass "a ticket embedded mid-id is normalized without moving it"

[ "$(fm_ship_branch_name 'fm/' 'es-pipelines-ve1247-mysql2-tls')" = "fm/es-pipelines-VE-1247-mysql2-tls" ] ||
  fail "ticket next to a version-like slug was mishandled: $(fm_ship_branch_name 'fm/' 'es-pipelines-ve1247-mysql2-tls')"
pass "mysql2 in the same id is left alone while the ticket is normalized"

[ "$(fm_ship_branch_name 'fm/' 'dark1063-token')" = "fm/DARK-1063-token" ] ||
  fail "a second Jira project key was not canonicalized: $(fm_ship_branch_name 'fm/' 'dark1063-token')"
pass "another Jira project key (dark) is canonicalized the same way"

[ "$(fm_ship_branch_name 'fm/' 'VE-1263-already')" = "fm/VE-1263-already" ] ||
  fail "an already-canonical ticket must be idempotent: $(fm_ship_branch_name 'fm/' 'VE-1263-already')"
...
for plain in 'pim-ci-tag-slug-fix' 'fm-opencode-arm-not-needed-falso-falha' 'pim-sdk-release-v121' 'bmsantander'; do
  got=$(fm_ship_branch_name 'fm/' "$plain")
...
[ "$(fm_ship_branch_name 'fm/' 'pim-sdk-release-v121')" = "fm/pim-sdk-release-v121" ] ||
  fail "v121 was rewritten as a ticket"
[ "$(fm_ship_branch_name 'fm/' 'upgrade-php8')" = "fm/upgrade-php8" ] ||
...
[ "$(fm_ship_branch_name '' 've-1263-x')" = "VE-1263-x" ] ||
  fail "an empty prefix must yield a bare normalized id: $(fm_ship_branch_name '' 've-1263-x')"
[ "$(fm_ship_branch_name 'release/' 've-1263-x')" = "release/VE-1263-x" ] ||
  fail "a custom prefix must be preserved: $(fm_ship_branch_name 'release/' 've-1263-x')"
...
BRANCH=$(fm_ship_branch_name 'fm/' 'visto-system-ve1263-deploy-uat')
case "$BRANCH" in
  *'VE-1263'*) : ;;
