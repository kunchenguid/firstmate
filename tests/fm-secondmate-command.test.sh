...
test_fm_send_refuses_a_lane_whose_command_record_is_missing() {
  ...
  # is not an ordinary crewmate. A work steer refuses either way: steering is
  # frequent and cheap to retry, so the conservative default costs nothing here.
  # (Retirement is the one action that proceeds on this state - see
  # test_an_unrecorded_lane_is_retireable_and_never_silently_so - because it is
  # the only path that reclaims a home, worktree and lease.)
  local home fb out rc
  ...
  assert_contains "$out" "no registry line" "the refusal must name the absent line, not an unreadable field"
...
# The operational exemption is about the KIND of message, not the command state:
# an infrastructure nudge tells a lane about its own files. It must reach an
# unrecorded lane - otherwise a mid-session config or instruction re-read can
# never land on one - while a work steer to that same lane still refuses, and an
# UNREADABLE record exempts nothing at all.
test_operational_exemption_covers_unrecorded_but_never_unreadable() {
  local home fb rc
  home=$(make_fleet send-unrecorded)
  fb=$(make_send_stubs "$home")
  printf '# Secondmates\n\n' > "$(reg_of "$home")"
  ...
  env PATH="$fb:$PATH" FM_HOME="$home" FM_ROOT_OVERRIDE="$home" FM_SEND_SETTLE=0 \
    "$SEND" sm-alpha "do the thing" >/dev/null 2>&1 || rc=$?
  [ "$rc" != 0 ] || fail "a work steer to an unrecorded lane must still refuse"

  rc=0
  env PATH="$fb:$PATH" FM_HOME="$home" FM_ROOT_OVERRIDE="$home" FM_SEND_SETTLE=0 \
    FM_SECONDMATE_COMMAND_OPERATIONAL=sm-alpha \
    "$SEND" sm-alpha "CONFIG_REREAD: /somewhere/generation" >/dev/null 2>&1 || rc=$?
  [ "$rc" = 0 ] || fail "an infrastructure nudge must still reach an unrecorded lane, got $rc"

  # An unreadable command field exempts nothing, operational or not.
  sed -i.bak 's/added 2026-01-01)/added 2026-01-01; command: whoknows)/' "$(reg_of "$home")" 2>/dev/null || true
  printf -- '- sm-alpha - Owns the alpha domain. (home: %s; scope: a; projects: a; added 2026-01-01; command: whoknows)\n' \
    "$(lane_of "$home")" >> "$(reg_of "$home")"
  rm -f "$(reg_of "$home").bak"
  rc=0
  env PATH="$fb:$PATH" FM_HOME="$home" FM_ROOT_OVERRIDE="$home" FM_SEND_SETTLE=0 \
    FM_SECONDMATE_COMMAND_OPERATIONAL=sm-alpha \
    "$SEND" sm-alpha "CONFIG_REREAD: /somewhere/generation" >/dev/null 2>&1 || rc=$?
  [ "$rc" != 0 ] || fail "an unreadable command field must exempt nothing, not even an infrastructure nudge"
  pass "the operational exemption covers unrecorded, never unreadable, and never a work steer"
}
...
# UNRECORDED and UNREADABLE are two different states and must never collapse
# into one condition. This pair pins them apart from opposite sides.
#
# UNRECORDED: no registry line exists at all. Command state is representable
# ONLY as that line's trailing field, so no transfer could ever have been
# recorded and the lane is firstmate-commanded by definition. Retirement is the
# only path that reclaims a home, worktree and never-recycled lease, so it
# proceeds - out loud. Steering and routing stay strict, being cheap to retry.
test_an_unrecorded_lane_is_retireable_and_never_silently_so() {
  local home lane out rc
  home=$(make_fleet unrecorded-record)
  lane=$(lane_of "$home")
  printf '## Queued\n\n- [ ] alpha-item - do alpha work\n' > "$home/data/backlog.md"
  printf '## Queued\n' > "$lane/data/backlog.md"
  # A lane the meta proves exists, with no registry line anywhere.
  printf '# Secondmates\n\n' > "$(reg_of "$home")"

  # Assert the still-strict paths first: retirement below is now allowed to
  # succeed, and a successful teardown clears the very records these read.
  rc=0
  out=$(env FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" FM_DATA_OVERRIDE="$home/data" \
    "$HANDOFF" sm-alpha alpha-item 2>&1) || rc=$?
  [ "$rc" != 0 ] || fail "firstmate must not route work into a lane whose command record it cannot read"
  ...
  assert_contains "$out" "command: unrecorded" \
    "the digest must never be silent about a lane with no command record"

  out=$(env FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" FM_STATE_OVERRIDE="$home/state" \
    FM_DATA_OVERRIDE="$home/data" "$TEARDOWN" sm-alpha 2>&1 || true)
  assert_not_contains "$out" "REFUSED: sm-alpha" \
    "an unrecorded lane must not be blocked from retirement - that strands its home, worktree and lease forever"
  assert_contains "$out" "no registry line" \
    "proceeding must be reported, or a deliberate decision looks like a missing check"
  assert_contains "$out" "firstmate-commanded" \
    "the note must say what it treated the lane as"
  pass "an unrecorded lane is retireable, says so out loud, and stays unsteerable"
}

# UNREADABLE: a registry line EXISTS but its command field cannot be parsed.
# Captain command may be exactly what is unreadable, so an authority that cannot
# be read is not authority and retirement still refuses.
test_an_unreadable_command_field_still_blocks_retirement() {
  local home out rc
  home=$(make_fleet unreadable-record)
  # The line exists; only its command field is damaged. This is NOT the absent
  # line above, and the two must not share a code path or a message.
  sed -i.bak 's/added 2026-01-01)/added 2026-01-01; command: whoknows)/' "$(reg_of "$home")"
  rm -f "$(reg_of "$home").bak"

  rc=0
  out=$(env FM_HOME="$home" FM_ROOT_OVERRIDE="$ROOT" FM_STATE_OVERRIDE="$home/state" \
    FM_DATA_OVERRIDE="$home/data" "$TEARDOWN" sm-alpha 2>&1) || rc=$?
  [ "$rc" != 0 ] || fail "an unreadable command field must still block retirement - captain command may be what is unreadable"
  assert_contains "$out" "REFUSED: sm-alpha" "the refusal must be explicit"
  assert_contains "$out" "cannot be parsed" "the refusal must name the unparseable field, not a missing record"
  assert_not_contains "$out" "no registry line" \
    "an unreadable field must never be reported as an absent line - they are different states"
  pass "an unreadable command field still blocks retirement, distinctly from an absent line"
}
