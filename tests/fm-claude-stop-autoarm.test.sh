...
test_busy_holder_expiry_is_consumed_without_a_second_wait() {
  ...
  grep -Eq "outcome=busy-holder .*holder_pid=$pid lock_age=0s$" \
    "$dir/state/.claude-autoarm-epoch" \
    || fail "the typed busy-holder result lost its holder or truthful lock age: $(head -1 "$dir/state/.claude-autoarm-epoch")"
