...
test_arms_for_telegram_intake_without_inflight() {
  local dir out status
  dir=$(make_primary_dir "$TMP_ROOT/telegram-need")
  mkdir -p "$dir/data/fm-telegram-topics"
  : > "$dir/data/fm-telegram-topics/config.env"
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "enabled Telegram intake must keep the auto-arm active with zero tasks in flight"
  [ -e "$dir/state/arm-ran" ] || fail "hook did not arm for Telegram intake"
  pass "auto-arm: Telegram intake arms the cycle even with no tasks in flight"
}
