...
run_autoarm_from_claude_daemon_with_spawned_by() {  # <dir> <foreground-lock-owner-pid>
...
test_zombie_session_owner_recovery_failure_is_visible() {
  ...
  out=$(FM_PROC_ROOT_OVERRIDE="$fakeproc" \
    run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$successor" 2>/dev/null); status=$?
  failure=$(failure_epoch_path "$dir") \
  ...
  [ "$(cat "$state/.lock" 2>/dev/null || true)" = "$owner" ] \
    || fail "daemon delivery changed the stale zombie-shaped session owner"
  ...
  pass "auto-arm: zombie session owners cannot hide daemon-delivered recovery failures"
}
...
test_claude_daemon_spawned_by_foreground_lock_owner_is_inert() {
...
SH
  ...
  out=$(PATH="$dir/no-interpreters:$PATH" run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status=$?
  lock_after=$(cat "$dir/state/.lock" 2>/dev/null || true)
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true

  expect_code 0 "$status" "a Claude daemon with only spawned-by metadata must not claim a live foreground-owned home"
  [ "$lock_after" = "$owner" ] || fail "daemon delivery rewrote the foreground session lock: expected $owner, got $lock_after"
  assert_absent "$dir/state/arm-ran" "daemon-delivered Stop hook armed through spawned-by metadata"
  assert_absent "$dir/state/.claude-autoarm-epoch" "daemon-delivered Stop hook wrote an epoch through spawned-by metadata"
  [ -z "$out" ] || fail "daemon-delivered Stop hook emitted output while a live foreground owner held the lock: $out"
  pass "auto-arm: Claude daemon spawned-by metadata is not an ownership proof"
}

test_claude_daemon_spawned_by_cannot_reclaim_stale_lock() {
  ...
  out=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status=$?
  lock_after=$(cat "$dir/state/.lock" 2>/dev/null || true)
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true

  expect_code 2 "$status" "a detached daemon with only spawned-by metadata must fail visibly instead of reclaiming a stale lock"
  [ "$lock_after" = 9999999 ] \
    || fail "detached daemon delivery replaced the stale owner: expected 9999999, got $lock_after"
  assert_absent "$dir/state/arm-ran" \
    "detached daemon delivery armed through spawned-by metadata"
  [ "$(failure_epoch_field "$dir" session_owner_pid)" = 9999999 ] \
    || fail "daemon-delivered failure was not fenced to the unchanged stale owner"
  assert_contains "$out" "stale session lock recovery failed" \
    "detached daemon delivery did not report its unreclaimed stale owner"
  pass "auto-arm: detached Claude daemon delivery fails visibly without ownership authority"
}
...
test_stale_recovery_claimant_owns_failure_suppression() {
  ...
  first_out=$(FM_AUTOARM_SESSION_LEASE_TIMEOUT=1 \
    run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); first_status=$?
  failure=$(failure_epoch_path "$dir") \
  ...
  repeat_out=$(FM_AUTOARM_SESSION_LEASE_TIMEOUT=1 \
    run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); repeat_status=$?
  ...
  second_out=$(FM_AUTOARM_SESSION_LEASE_TIMEOUT=1 \
    run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); second_status=$?
...
test_recovery_revalidates_daemon_delivered_owner_after_session_lease_wait() {
  ...
  run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" > "$out" 2>&1 &
  hook=$!
  i=0
  ...
  if [ ! -e "$dir/state/session-lease-waiting" ]; then
    ...
    fail "daemon-delivered recovery did not reach the session lease wait"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  ...
  expect_code 2 "$status" "a daemon-delivered owner dying during recovery lease wait must fail visibly"
  [ "$(cat "$dir/state/.lock" 2>/dev/null || true)" = 9999999 ] \
  ...
  pass "auto-arm: stale recovery revalidates daemon-delivered ownership after lease waits"
}
...
test_successor_session_gets_own_failure_notice() {
  ...
  out1=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner1" 2>/dev/null); status1=$?
  ...
  out2=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner2" 2>/dev/null); status2=$?
...
test_successor_session_ignores_predecessor_attended_alarm() {
  ...
  owner2=$!
  printf '%s\n' "$owner2" > "$dir/state/.lock"
  out=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner2" 2>/dev/null); status=$?
  kill "$owner2" 2>/dev/null || true
  wait "$owner2" 2>/dev/null || true
...
test_claude_daemon_losing_session_owner_cannot_commit() {
  ...
  if [ ! -e "$dir/state/arm-waiting" ]; then
    kill "$hook" "$owner" 2>/dev/null || true
    wait "$hook" "$owner" 2>/dev/null || true
    fail "daemon delivery did not reach the terminal authority boundary"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  "$FAKE_CLAUDE" -c 'sleep 60; :' &
  ...
  pass "auto-arm: daemon delivery revalidates session authority before terminal commit"
}
...
test_claude_daemon_losing_owner_during_terminal_lock_wait_cannot_commit() {
  ...
  if [ ! -e "$dir/state/terminal-lock-waiting" ]; then
    kill "$hook" "$owner" 2>/dev/null || true
    wait "$hook" "$owner" 2>/dev/null || true
    fail "daemon delivery did not block inside terminal lock acquisition"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  "$FAKE_CLAUDE" -c 'sleep 60; :' &
...
test_terminal_publish_holds_session_acquisition_lease() {
  ...
  if [ ! -e "$dir/state/terminal-publish-waiting" ]; then
    kill "$hook" "$owner" 2>/dev/null || true
    wait "$hook" "$owner" 2>/dev/null || true
    fail "daemon delivery did not pause after terminal authority validation"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
...
test_claude_daemon_losing_owner_during_record_lock_wait_cannot_mutate() {
  ...
  if [ ! -e "$dir/state/terminal-lock-waiting" ]; then
    kill "$hook" "$owner" 2>/dev/null || true
    wait "$hook" "$owner" 2>/dev/null || true
    fail "daemon delivery did not block inside quiet-record lock acquisition"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  "$FAKE_CLAUDE" -c 'sleep 60; :' &
...
test_claude_daemon_losing_owner_during_reset_lock_wait_cannot_reset() {
  ...
  if [ ! -e "$dir/state/terminal-lock-waiting" ]; then
    ...
    fail "daemon delivery did not block inside recovery-reset lock acquisition"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  "$FAKE_CLAUDE" -c 'sleep 60; :' &
...
test_claude_daemon_losing_session_owner_cannot_rearm() {
  ...
  if [ ! -e "$dir/state/arm-waiting" ]; then
    kill "$hook" "$owner" 2>/dev/null || true
    wait "$hook" "$owner" 2>/dev/null || true
    fail "daemon delivery did not reach the retry authority boundary"
  fi
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
  "$FAKE_CLAUDE" -c 'sleep 60; :' &
  ...
  pass "auto-arm: daemon delivery revalidates session authority before every arm"
}
...
test_failed_cycles_notify_once_and_keep_retrying() {
  ...
  owner=$!
  printf '%s\n' "$owner" > "$dir/state/.lock"
  out1=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status1=$?
  out2=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status2=$?
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
...
test_post_alarm_claim_failures_do_not_grow_episode_state() {
  ...
  owner=$!
  printf '%s\n' "$owner" > "$state/.lock"
  run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" >/dev/null 2>&1
  status=$?
...
test_failure_notice_marker_write_refuses_delivery_and_retries() {
  ...
  rm -f "$marker"
  out2=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status2=$?
  out3=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$owner" 2>/dev/null); status3=$?
  kill "$owner" 2>/dev/null || true
  wait "$owner" 2>/dev/null || true
...
test_superseded_owner_goes_silent_and_never_double_translates() {
  ...
  session_owner=$!
  printf '%s\n' "$session_owner" > "$dir/state/.lock"
  run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$session_owner" > "$a_out" 2>&1 &
  a_pid=$!
  i=0
  ...
  b_out=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$session_owner" 2>/dev/null); b_status=$?
  ...
  touch -t 202001010000 "$dir/state/.claude-autoarm-epoch"
  touch -t 202001010000 "$dir/state/.last-watcher-beat"
  c_out=$(run_autoarm_from_claude_daemon_with_spawned_by "$dir" "$session_owner" 2>/dev/null); c_status=$?
