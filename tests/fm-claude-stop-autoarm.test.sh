...
test_settings_registers_autoarm_with_multi_hour_timeout() {
  ...
  grep -Fq '[ -f "$CONFIG/x-mode.env" ] && . "$CONFIG/x-mode.env"' "$ROOT/bin/fm-claude-stop-autoarm.sh" \
    || fail "auto-arm must source the effective X-mode cadence config"
  grep -Fq '[ -f "$CONFIG/telegram-mode.env" ] && . "$CONFIG/telegram-mode.env"' "$ROOT/bin/fm-claude-stop-autoarm.sh" \
    || fail "auto-arm must source the effective Telegram cadence config"
  pass "settings.json registers the asyncRewake auto-arm with remote cadence configs, timeout >= 28800, and a foreground arm"
}
