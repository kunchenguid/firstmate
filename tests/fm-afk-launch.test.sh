...
# ---------------------------------------------------------------------------
# UNIT 1: fm_afk_clear_stale_artifacts removes the away-session delivery artifacts.
# ---------------------------------------------------------------------------
unit_clear_stale() {
  local st
  ...
  if [ ! -e "$st/state/.subsuper-escalations" ] \
    ...
    pass "clear-stale: removes escalation delivery state and the queue-observation cursor"
  else
...
unit_fresh_vs_refresh() {
  ...
  if [ -e "$st/state/.subsuper-escalations" ] \
    && [ -e "$st/state/.subsuper-inject-wedged" ] \
    && [ "$(cat "$st/state/.subsuper-seen-wake-seq")" = 12 ]; then
...
unit_failed_start_rolls_back_state() {
  ...
  if FM_HOME="$st" FM_STATE_OVERRIDE="$st/state" FM_SUPERVISOR_TARGET=unused \
    ...
    && [ "$(cat "$st/state/.subsuper-seen-wake-seq")" = 19 ]; then
