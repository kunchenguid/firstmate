...
test_x_artifact_identity_accepts_overlay_file_device() {
  local dir fakebin file
  dir="$TMP_ROOT/x-artifact-overlay-device"
  fakebin=$(fm_fakebin "$dir")
  file="$dir/private-artifact"
  printf 'private\n' > "$file"
  chmod 0600 "$file"
  cat > "$fakebin/stat" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  FM_TEST_REAL_STAT=$(command -v stat) PATH="$fakebin:$BASE_PATH" bash -c '
    . "$1/bin/fm-x-lib.sh"
    fmx_single_link_file_valid "$2" 39
  ' _ "$ROOT" "$file" || fail "X artifact identity rejected an overlay file-layer device"
  pass "X artifact identity accepts an overlay file-layer device bound to its parent"
}
