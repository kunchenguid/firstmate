...
phase_spawn() {
  ...
  # Launch ran in the subhome, with the persistent charter, the Codex hook scope
  # bound to that home, every other operational override cleared, and no
  # project-style treehouse get.
  assert_grep "FM_HOME='$SUB_ABS'" "$LOG" "secondmate launch did not set FM_HOME to the subhome"
  assert_grep "FM_ROOT_OVERRIDE='$SUB_ABS' FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE=" "$LOG" \
    "Codex secondmate launch did not bind only its root scope override"
