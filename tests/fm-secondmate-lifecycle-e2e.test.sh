...
#   - cloned projects are created only under the new subhome and parent project
#     clones are never mutated
...
EOF
  ...
  # One fakebin serves tmux + treehouse for spawn, send, and teardown.
...
phase_seed() {
  ...
  for project in alpha beta gamma; do
    [ -z "$(git -C "$HOME_DIR/projects/$project" status --porcelain --untracked-files=all)" ] \
      || fail "seed mutated the parent $project project"
  done
  ...
  [ "$(FM_HOME="$SUB" "$ROOT/bin/fm-project-mode.sh" gamma)" = "direct-PR off" ] \
    || fail "default direct-PR delivery mode not preserved in the subhome"
  FM_HOME="$HOME_DIR" "$ROOT/bin/fm-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"

  pass "seed: registry scope+projects, charter copied, clones+origins and modes preserved"
}
