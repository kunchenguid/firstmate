...
phase_spawn() {
  ...
  assert_grep 'notify=[\"true\"]' "$LOG" "secondmate codex launch did not suppress the machine-global generic notifier"
  assert_no_grep 'touch __TURNEND__' "$LOG" "secondmate codex launch included the parent turn-end notify hook"
