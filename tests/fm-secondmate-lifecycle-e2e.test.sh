...
phase_spawn() {
  ...
  assert_grep "FM_ROOT_OVERRIDE='$SUB_ABS' FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE=" "$LOG" "Codex secondmate launch did not bind its home as guard scope"
  assert_grep "FM_CODEX_HOOK_ROOT='$SUB_ABS'" "$LOG" "Codex secondmate launch did not bind its trusted hook root"
