...
phase_send() {
  ...
  # The session name is this home's own (fleet isolation names it after the home
  # directory, not a fixed "firstmate"), so the doorbell must ring exactly the
  # window the meta records - the same name phase_respawn asserts below.
  assert_grep 'send-keys -t main home:fm-design -l Firstmate instruction waiting:' "$LOG" "send did not ring the window recorded in this home's meta"
...
phase_recovery() {
  ...
  assert_grep 'window=main home:fm-design' "$meta" "respawn did not reconstruct the direct-report window"
