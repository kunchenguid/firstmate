...
phase_send() {
  : > "$LOG"
  : > "$PANE"
  # The meta window (firstmate:@1, the stable id captured at spawn) must win
  # over a foreign same-named window returned by list-windows.
  ...
  assert_grep 'send-keys -t firstmate:@1 -l [fm-from-firstmate]' "$LOG" "send did not use the window recorded in this home's meta, or did not mark the secondmate request"
...
phase_recovery() {
  ...
  assert_grep 'window=firstmate:@1' "$meta" "respawn did not reconstruct the direct-report window"
