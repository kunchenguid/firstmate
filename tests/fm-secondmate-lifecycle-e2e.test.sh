...
phase_handoff() {
  ...
  # incompatible - the handoff path requires atomic multi-ID mv, 0.2.2+ - (the
  ...
  if ! fm_tasks_axi_compatible; then
    echo "skip: tasks-axi lacks atomic multi-ID mv support (0.2.2+, required by the handoff path)"
    return 0
  fi
