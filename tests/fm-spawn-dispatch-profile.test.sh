...
test_codex_threads_max_effort() {
  ...
  status=$?
  expect_code 0 "$status" "codex spawn at max effort should succeed"
  assert_meta_profile "$HOME_DIR/state/$id.meta" codex gpt-5 max
  launch=$(cat "$LAUNCH_LOG")
  # The regression this pins: max used to be dropped here, so a crewmate
  # dispatched at max silently ran at the model default instead.
  ...
  pass "codex threads max reasoning effort instead of silently dropping it"
}
