...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI env -u CLAUDE_CODE_CHILD_SESSION CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' $CLAUDE_CONTROL_CHANNEL_FLAG \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
test_claude_forwards_firstmate_config_dir_when_set() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$CASE_DIR/claude-work' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI env -u CLAUDE_CODE_CHILD_SESSION CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}'" \
...
# Claude Code exports CLAUDE_CODE_CHILD_SESSION=1 into every Bash-tool command, so
# a runtime server started from inside a Claude session hands it to every pane,
# and a claude launched there stops saving its transcript. Run the rendered launch
# with the marker in the pane environment and prove the harness never sees it.
test_claude_launch_strips_inherited_child_session_marker() {
  local rec id kind sm out status launch seen
  for kind in ship scout secondmate; do
    id="profile-child-session-$kind-z30"
    rec=$(make_spawn_case "profile-child-session-$kind" claude "$id")
    read_case_record "$rec"
    seen="$CASE_DIR/child-session-seen"
    cat > "$FAKEBIN_DIR/claude" <<'SH'
#!/usr/bin/env bash
...
SH
    chmod +x "$FAKEBIN_DIR/claude"
    case "$kind" in
    ship) out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR") ;;
    scout) out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --scout) ;;
    secondmate)
      sm="$CASE_DIR/secondmate-home"
      make_seeded_secondmate_home "$sm" "$id"
      out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$sm" --secondmate)
    ...
    status=$?
    expect_code 0 "$status" "claude $kind spawn should succeed"$'\n'"$out"
    launch=$(cat "$LAUNCH_LOG")
    CLAUDE_CODE_CHILD_SESSION=1 FM_CHILD_SESSION_SEEN="$seen" PATH="$FAKEBIN_DIR:$PATH" \
      bash -c "$launch" || fail "the claude $kind launch command failed"
    [ "$(cat "$seen")" = unset ] ||
      fail "the claude $kind launch passed an inherited CLAUDE_CODE_CHILD_SESSION to claude"
  done
...
claude_expected_launch() {  # <home> <id> <permission-flag>
  ...
  printf '%s' "export COMPACT_ADVISER_DISABLE=1; env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI env -u CLAUDE_CODE_CHILD_SESSION CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude $flag --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' $CLAUDE_CONTROL_CHANNEL_FLAG \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$home/data/$id/launch-brief.md')\""
}
