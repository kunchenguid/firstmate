...
test_codex_forwards_max_effort() {
  local rec id out status launch
  ...
  assert_contains "$launch" "codex --model 'gpt-5' -c 'model_reasoning_effort=\"max\"' --dangerously-bypass-approvals-and-sandbox" \
    "codex launch did not forward max reasoning effort"
  pass "codex forwards max reasoning effort through model_reasoning_effort"
}
