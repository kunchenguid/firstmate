...
test_pi_delegated_profile_pins_command_model_effort_and_resources() {
  ...
  assert_contains "$launch" "NODE_OPTIONS=''" "Pi launch did not clear hostile ambient Node options"
  assert_contains "$launch" "node --require '$ROOT/bin/fm-pi-startup-guard.cjs'" "Pi launch did not preload the controlled startup guard through Node arguments"
