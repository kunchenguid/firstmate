...
test_hermes_is_not_a_crewmate_or_secondmate_runtime() {
  local rec id out status
  id=profile-hermes-primary-only-z20
  rec=$(make_spawn_case profile-hermes-primary-only hermes "$id")
  read_case_record "$rec"

  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" --harness hermes)
  status=$?
