...
test_codex_threads_max_effort() {
  ...
  status=$?
  expect_code 0 "$status" "codex spawn with explicit max effort should succeed"
  ...
  assert_contains "$launch" "codex --model 'gpt-5.6-luna' -c 'model_reasoning_effort=\"max\"' --dangerously-bypass-approvals-and-sandbox" \
    "codex launch did not thread explicit max reasoning effort config"
  pass "codex receives explicit max model_reasoning_effort profile flags"
}
