...
test_pi_threads_model_and_max_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi '$FAKEBIN_DIR/pi' --tui-mode regular --provider 'openai-codex' --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --provider 'openai-codex' --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --provider 'fake' --model 'fake/test' -e '$sm/.pi/extensions/fm-primary-turnend-guard.ts' -e '$sm/.pi/extensions/fm-primary-pi-watch.ts'" \
