...
test_native_pi_ultra_is_explicit_and_model_scoped() {
  ...
  pass "Ultra is explicit for native Pi and Pi-signed in both supported delivery modes, and refuses unsupported profiles before provisioning"
}
...
SH
    ...
    if [ "${FM_TEST_EVIDENCE:-0}" = 1 ]; then
      ...
      cat "$prompt"
      printf 'authored brief remains unchanged\n# evidence end\n'
    fi
  done
  done
