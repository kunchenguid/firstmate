...
SH
  ...
  # fm-worktree-proc-lib.sh: teardown sources it to reap leftover processes
  # under the task copy without depending on lsof.
  ln -s "$ROOT/bin/fm-worktree-proc-lib.sh" "$fake/bin/fm-worktree-proc-lib.sh"
