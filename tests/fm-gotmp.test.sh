...
make_fake_root() {
  ...
  # fm-github-lib.sh: teardown sources the optional routing owner even when its
  # private config is absent and execution remains byte-compatible legacy mode.
  ln -s "$ROOT/bin/fm-github-lib.sh" "$fake/bin/fm-github-lib.sh"
...
test_teardown_skips_gracefully_without_tasktmp() {
  ...
  # fm-pr-lib.sh: teardown uses its canonical task-ID validator for poll cleanup.
  ln -s "$ROOT/bin/fm-pr-lib.sh" "$fake/bin/fm-pr-lib.sh"
  # fm-github-lib.sh: teardown sources the optional routing owner in legacy mode.
  ln -s "$ROOT/bin/fm-github-lib.sh" "$fake/bin/fm-github-lib.sh"
  cat > "$fake/bin/fm-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
