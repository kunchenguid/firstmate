...
make_fake_root() {
  ...
  # fm-composer-lib.sh: fm-tmux-lib.sh sources it at load time, so the tmux adapter
  # cannot be sourced without it. Symlink the REAL file; a missing source under the
  # script's `set -eu` would abort teardown before the tasktmp cleanup even runs.
