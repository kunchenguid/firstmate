...
#
# fm-spawn gives each writer task a temp root /tmp/fm-<id>/ with Go's build temp nested at
...
make_fake_root() {
  ...
  # fm-backend.sh and home-tag derivation are real, while the tmux adapter is
  # stubbed so this temp-cleanup test cannot mutate a host tmux server.
