...
make_fake_root() {
  ...
  # fm-session-lock-lib.sh: the tmux adapter sources it at load. A missing
  # sibling is fatal to the whole shell under Bash 3.2 (macOS), where `.` of an
  # absent file exits rather than returning non-zero, so the kill dispatch would
  # abort teardown before it ever reaches the tasktmp removal under test.
  ...
  # fm-stat-lib.sh: the one portable file-metadata owner the lock, wake, x, and
  # pr libs above each source.
