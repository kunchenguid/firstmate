...
test_fm_home_parameterization() {
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-a.status' \"{state}: {one short line}\"" "$brief" >/dev/null \
    || fail "brief did not shell-quote the serialized FM_HOME status command"
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-b.status' \"{state}: {one short line}\"" "$brief" >/dev/null \
    || fail "scout brief did not shell-quote the serialized FM_HOME status command"
  ...
  grep -F "'$ROOT/bin/fm-status-append.sh' '$home_one/state/task-c.status' \"{state}: {one short line}\"" "$brief" >/dev/null \
    || fail "secondmate brief did not shell-quote the serialized FM_HOME status command"
