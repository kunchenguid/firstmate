...
SH
  ...
  grep -F 'tab create' "$log" >/dev/null && fail "spawn created a tab before validation"
...
test_secondmate_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    grep -F 'tab create' "$log" >/dev/null && fail "spawn created a tab before unsafe $opdir directory validation"
  done
...
test_fm_send_refuses_bare_window_without_home_meta() {
  ...
  # foreign same-named pane that the backend inventory happens to return.
  ...
  grep -F 'pane send-text' "$log" >/dev/null \
...
EOF
  ...
  grep -F 'pane close fm-child' "$log" >/dev/null || fail "force teardown did not close child pane"
  grep -F 'pane close fm-domain' "$log" >/dev/null || fail "force teardown did not close parent pane"
...
EOF
    ...
    grep -F 'pane close fm-domain' "$log" >/dev/null || fail "force teardown did not close the parent pane for inside $opdir symlink"
  done
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown closed a pane before symlinked state refusal"
...
SH
    ...
    grep -F 'pane close' "$log" >/dev/null && fail "teardown ($row) closed a pane before validation"
  done <<'ROWS'
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown closed a pane before nested-home refusal"
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "teardown closed a pane before child-registry refusal"
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown closed panes before subhome validation"
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown closed panes before child validation refusal"
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown closed panes before repo child validation refusal"
...
EOF
  ...
  grep -F 'pane close' "$log" >/dev/null && fail "force teardown closed panes before unregistered child refusal"
