...
test_busy_lifecycle_locks_never_hold_up_the_digest() {
  ...
  for label in reconcile control meta; do
    ...
    notify=$!
    # Wait for the observable command completion, with enough scheduling budget
    # for a loaded CI host. The old fixed 200 ms sample could report a correct
    # nonblocking path as wedged before the process had been scheduled.
