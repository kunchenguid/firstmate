...
test_return_gate_orders_catchup_before_bearings() {
  ...
  : > "$dir/home/state/.subsuper-busy-empty-streak-since"
  {
  ...
  [ ! -e "$dir/home/state/.subsuper-busy-empty-streak-since" ] || fail "successful check left the busy+empty streak marker behind for the next away session"
