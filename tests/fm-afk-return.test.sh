...
#!/usr/bin/env bash
# Fake drain with the U1.3 contract: presented rows are consumed at
# presentation, and the legacy acknowledgement flag is a no-op.
file="$FM_HOME/state/.fake-drain"
if [ "${1:-}" = --ack-through ]; then
  printf 'wake drain: presented wakes are consumed at presentation; no acknowledgement is needed\n' >&2
  exit 0
fi
if [ -s "$file" ]; then
  cat "$file"
...
test_return_gate_orders_catchup_before_bearings() {
  ...
  [ ! -s "$dir/home/state/.fake-drain" ] || fail "blocked return left presented wakes unconsumed"
  ...
  [ ! -s "$dir/home/state/.fake-drain" ] || fail "successful return left presented wakes unconsumed"
...
test_evidence_publication_failure_preserves_wake_evidence() {
  ...
  [ ! -s "$dir/home/state/.fake-drain" ] || fail "the presenting drain left the wake unconsumed"
  [ -s "$gate" ] || fail "publication failure did not retain the catch-up gate"
  grep -F $'evidence\twake\t1784074271' "$gate" >/dev/null \
    || fail "publication failure lost the presented wake from the durable gate evidence"

  out=$(run_return "$dir" check) || fail "publication retry did not complete catch-up: $out"
  assert_contains "$out" 'catch-up wake: 1784074271' "publication retry did not re-present the preserved wake evidence"
  [ ! -e "$gate" ] || fail "successful publication retry left the catch-up gate pending"
  pass "AFK return preserves presented wake evidence in the durable gate across publication failure"
}
