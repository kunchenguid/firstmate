...
test_return_brief_composes_from_record_store_and_held_set() {
  ...
  contract_in "$dir" propose --words 'merge the windows fix when green, then cut a prerelease' \
    ...
    --action prerelease --object 'repo release pipeline' --when 'after clause 1' \
  ...
  assert_contains "$out" '2. prerelease repo release pipeline when after clause 1 - recorded, not executed by this release' "the second clause was not listed"
