...
test_projection_journal_v2_binds_and_advances_exact_endpoint() {
  ...
  out=$(bash -c '
    ...
    fm_backend_herdr_projection_journal_bind \
      "$journal" fm-hibit-r1 "$home" lab-session w2 w2:t2 w2:p2 w1 firstmate "$label" "#5 bpi-auth" || exit 1
  ...
  [ "$(sed -n 's/^task_label=//p' "$state/fm-hibit-r1.herdr-presentation")" = "#5 bpi-auth" ] \
    || fail "version 2 projection journal did not retain the human task label"
...
test_kill_labeled_task_closes_recorded_pane_id() {
  ...
  PATH="$fb:$PATH" FM_HOME="$home" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_presentation_session_lock_path() { printf "/tmp/fm-herdr-labeled-test-lock"; }
      fm_lock_try_acquire() { return 0; }
      fm_lock_release() { return 0; }
