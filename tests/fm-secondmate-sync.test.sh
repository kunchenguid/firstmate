...
# --- R3: the target is imported from the host code root's forge origin ---------
test_remote_sync_imports_from_origin() {
  ...
  [ "$REMOTE_SYNC_RC" -eq 0 ] || fail "remote sync could not import from the host code root's forge origin: $REMOTE_SYNC_OUT"
  [ "$(head_of "$w/sm")" = "$c2" ] || fail "remote home did not advance to the origin-imported commit"
  pass "R3 a target neither local repository holds is imported from the host code root's forge origin"
}
