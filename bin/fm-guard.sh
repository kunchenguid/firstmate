...
# read-only advisory mode when session start did not acquire the fleet lock.
...
if "$queue_pending"; then
  if [ "$READ_ONLY" -eq 1 ]; then
    echo "WARNING: queued wakes pending - left untouched because this session did not acquire the fleet lock." >&2
  else
