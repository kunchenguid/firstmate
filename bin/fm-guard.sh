...
# Then, if any task is in flight or a messaging connector requires polling and the watcher's
...
# Compute in-flight count, connector monitoring need, and watcher-beacon
# freshness via the shared grace-based predicate (bin/fm-supervision-lib.sh).
# Act only when work or messaging is riding on an absent watcher.
fm_supervision_status "$STATE" "$GRACE" "$CONFIG"
in_flight=$FM_SUP_IN_FLIGHT
watcher_fresh=$FM_SUP_WATCHER_FRESH
beacon_desc=$FM_SUP_BEACON_DESC
if [ "$FM_SUP_MONITORING_REQUIRED" != true ]; then
  # Leave the unhealthy state (no work or connector riding on the watcher): clear so a later
...
if [ "$watcher_fresh" = false ]; then
  ...
  if [ "$print_full_banner" -eq 1 ]; then
    ...
    portal_mode=0
    [ -f "$CONFIG/portal-mode.env" ] && portal_mode=1
    ...
    rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
    {
      ...
      if [ "$in_flight" -gt 0 ]; then
        ...
        printf '●  Messaging is active, but no watcher has a fresh beacon (last beat: %s, grace %ss).\n' "$beacon_desc" "$GRACE"
      fi
      if [ "$READ_ONLY" -eq 1 ]; then
