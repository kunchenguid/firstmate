...
# Then, if the shared supervision predicate (bin/fm-supervision-lib.sh) reports
# supervision need and supervision is not healthy, prints a loud, clearly
...
# FM_GUARD_GRACE) is a genuine lapse; under every persistent-watcher harness a
# live identity-matched watcher with a fresh beacon is required. The banner names
# the true failing condition (a missing live watcher process vs a genuinely
# stale beacon). The full banner is emitted once per distinct down-episode in
# this FM_HOME (keyed to the failing condition, not the beacon mtime, which a
# healthy between-turns watcher advances every poll);
