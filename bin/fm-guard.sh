...
# Then, if any task is in flight (a state/<id>.meta exists), warn when the
# watcher's liveness beacon (state/.last-watcher-beat, touched every poll cycle)
# is missing or older than FM_GUARD_GRACE seconds, or when Codex has only a fresh
# one-shot watcher on an unverified background-completion wake channel.
# The warning is loud and clearly delimited so the agent cannot skim past it in
# the tool output of whatever it was doing - the one channel every harness has.
# Normal wake handling (watcher briefly down between a wake and its re-arm) stays
