...
# message and starts nothing, so it observes too. A local Claude deny list may
# still remove these from the schema; this shipped guard deliberately stays
# narrower so it can never be the reason a runaway task cannot be stopped.
