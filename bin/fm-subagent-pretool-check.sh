...
# turn-end guard do; bin/fm-primary-scope-lib.sh owns the rule. A primary home
# operates a fleet and must dispatch through it, while a crewmate's task
# worktree, the linked shape bin/fm-spawn.sh always hands out, owns no session
# lock and stays out of scope, so a crewmate using delegation tools inside its
# own task worktree is legitimate and stays allowed. Any failure to confirm the
