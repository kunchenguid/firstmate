...
usage() {
  cat <<'EOF'
Usage: fm-supervision-instructions.sh [--harness <name>] [--read-only 0|1] [--afk 0|1] [--x-mode 0|1] [--trello-mode 0|1] [--repair-line] [--queue-pending 0|1]
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --x-mode)
      ...
      ;;
    --trello-mode)
      [ "$#" -gt 1 ] || { echo "error: --trello-mode requires 0 or 1" >&2; exit 2; }
      TRELLO_MODE=$(bool_value "$2")
      shift 2
      ;;
    --queue-pending)
...
trello_mode_env="$CONFIG/trello-mode.env"
...
trello_mode_env_sh=$(shell_quote "$trello_mode_env")

if [ "$X_MODE" -eq 0 ] && [ -f "$x_mode_env" ]; then
  X_MODE=1
fi
if [ "$TRELLO_MODE" -eq 0 ] && [ -f "$trello_mode_env" ]; then
  TRELLO_MODE=1
fi
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    line=${line//__FM_TRELLO_MODE_ENV_SH__/$trello_mode_env_sh}
    line=${line//__FM_TRELLO_MODE_ENV__/$trello_mode_env}
    printf '%s\n' "$line"
  done < "$SNIPPET"
}
...
repair_line() {
  ...
  if [ "$TRELLO_MODE" -eq 1 ]; then
    prefix="${prefix}source ${trello_mode_env_sh} first, then "
  fi
  if [ "$X_MODE" -eq 1 ]; then
...
if [ "$TRELLO_MODE" -eq 1 ]; then
  printf '%s%s%s\n' '- Trello mode: active; source ' "$trello_mode_env" ' before the X-mode config, if any, so the 60s cadence is inherited without overriding X mode.'
else
  printf '%s\n' '- Trello mode: inactive.'
fi
if [ "$X_MODE" -eq 0 ] && [ "$TRELLO_MODE" -eq 0 ]; then
  printf '%s\n' '- Control-plane polling: inactive; use the default watcher cadence.'
fi
ordinary_wake_line
printf '\n'
render_snippet
