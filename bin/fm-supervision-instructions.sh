...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|cursor|omp|jcode) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    jcode)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own jcode background bash task with wake: true set explicitly, never shell &.'
      ;;
    cursor)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    jcode)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm.sh jcode background bash task, with wake: true set explicitly, as directed below.'
      ;;
    cursor)
