...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    copilot)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as one standalone Copilot Bash task using mode=async and detach=false, never shell &.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    copilot)
      printf '%s\n' '- Ordinary wake: start exactly one new bin/fm-watch-arm.sh Copilot Bash task using mode=async and detach=false after handling the wake.'
      ;;
    pi|pi-signed)
