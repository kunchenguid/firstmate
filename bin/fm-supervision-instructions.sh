...
cursor_checkpoint_seconds=${FM_CURSOR_WATCH_CHECKPOINT:-180}
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    cursor)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$cursor_checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
