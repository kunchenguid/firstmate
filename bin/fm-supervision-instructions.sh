...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|agy) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    agy)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own background run_command task, never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    agy)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm.sh background task as directed below.'
      ;;
    *)
