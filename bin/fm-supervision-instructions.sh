...
# Either opt-in channel needs the 30s watcher cadence.
FAST_CADENCE=0
...
repair_line() {
  ...
  if [ "$FAST_CADENCE" -eq 1 ]; then
    # Name only the cadence files that actually exist (and when both).
    cadence_hint=
    if [ -f "$x_mode_env" ] && [ -f "$telegram_mode_env" ]; then
      cadence_hint="source ${x_mode_env_sh} and ${telegram_mode_env_sh} first, then "
    elif [ -f "$x_mode_env" ]; then
      cadence_hint="source ${x_mode_env_sh} first, then "
    elif [ -f "$telegram_mode_env" ]; then
      cadence_hint="source ${telegram_mode_env_sh} first, then "
    else
      # Flag said fast cadence but files not on disk yet - keep a generic hint.
      cadence_hint="source the X/Telegram cadence env when present, then "
    fi
    prefix="${prefix}${cadence_hint}"
  fi
...
if [ "$TELEGRAM_MODE" -eq 1 ]; then
  printf '%s%s%s\n' '- Telegram mode: active; source ' "$telegram_mode_env" ' before launching any watcher process so the 30s cadence is inherited.'
else
  printf '%s\n' '- Telegram mode: inactive.'
fi
if [ "$FAST_CADENCE" -eq 0 ]; then
  printf '%s\n' '- Watcher cadence: default (no X/Telegram opt-in cadence file).'
fi
