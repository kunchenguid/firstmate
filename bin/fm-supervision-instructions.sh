...
kimi_checkpoint_seconds=${FM_KIMI_WATCH_CHECKPOINT:-180}
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    kimi)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$kimi_checkpoint_seconds" '.'
      ;;
    pi)
