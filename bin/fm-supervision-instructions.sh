...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|cursor-agent) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    cursor-agent)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/fm-watch-arm.sh as its own Cursor Shell command, never shell &.'
      ;;
    *)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    grok)
      ...
      ;;
    cursor-agent)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/fm-watch-arm.sh Cursor Shell command as directed below.'
      ;;
    *)
