...
# and claude, omp, and kimi have no verified pane-resume contract at all.
...
# instruction. The one RELAUNCH-time exception is
# fm_control_relaunch_resume_flag below: where the endpoint's runtime records
# the exact session its own status authority is bound to, that reference (not a
# guess about which session is recent) is handed to the replacement so the
# runtime's state reports keep landing.
