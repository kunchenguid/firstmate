...
# is exact for the same reason on an even shorter name, `copilot` is exact
# because a `copilot*` prefix would claim unrelated commands, and an
...
# copilot, and devin are crewmate/scout adapters only: none has a primary
...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|gemini|rovo|agy|copilot|devin) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}

# The key that cancels a running turn. Escape for every adapter except grok,
# whose Esc only moves focus to the scrollback, and copilot, whose footer
# advertises `esc interrupt` but whose single and double Escape showed no
# verifiable effect on a running model or tool turn across three live trials
# (copilot 1.0.88); both cancel on Ctrl+C instead. gemini names its own key
# in the running turn's status row (`(esc to cancel, <n>s)`), and a single
# Escape was verified to cancel it.
...
# Herdr). copilot cancels on a single Ctrl+C, recording abort user_initiated
# in its session events with an idle composer and no repollution (verified
# live, copilot 1.0.88). omp (Oh My Pi) shares Pi's single Escape, empty
...
# or @path/to/file` placeholder. copilot was checked the same way and also
# does not repollute: after a single Ctrl+C its composer shows only the
# empty `❯` row (verified live, copilot 1.0.88). Prints the key or nothing;
# a harness with no verified mechanics returns nonzero, matching the tables
# above.
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|copilot|devin) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # copilot's session events DO record abort user_initiated, but for the
    # same parity reason this stays 'none': the control plane's postcondition
    # is proven through the backend agent-state classifier, not the fold.
    claude|codex|opencode|pi|pi-signed|omp|grok|kimi|cursor|gemini|rovo|agy|copilot|devin) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    copilot)
      # copilot installs no hook: its busy source is its own session event
      # log, bound to the pane by this firstmate-owned sidecar. A relaunch
      # ONTO copilot rewrites it, but a relaunch AWAY from copilot must
