...
# is exact because the shipped binary is always the bare word `agy` (verified,
# agy 1.1.28: a natively compiled ELF with no versioned launcher), and an
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # agy's `Interrupted` line is the same class: rendered, not recorded.
