...
# is exact for the same reason on an even shorter name, and `devin` is exact
# for the same reason again, and an
...
# through Herdr). devin is the one verified adapter that needs TWO: its
# running turn names the key itself (`(esc twice to interrupt)`), and a double
# Escape was verified to cancel it, printing `Canceled. What should Devin do?`
# with an idle composer and no repollution (verified live, devin 3000.10.31).
...
# Escape, and so does devin; every other verified adapter interrupts on a
# single press.
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # devin prints "Canceled. What should Devin do?" on double Escape and
    # fires no hook for it, so it stays 'none' for the same reason: the next
    # UserPromptSubmit or Stop is what settles the devin-hook record.
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    # file the launch reaches through devin's --config flag, so retiring that
    # one file retires the whole incarnation's wiring. Nothing is written
    # into the worktree, and the captain's own user config is never mutated.
