...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    # claude's hooks live in a firstmate-owned settings file the launch
    # reaches through --settings, never in the worktree, whose own
    # .claude/settings.local.json may be the project's committed file.
