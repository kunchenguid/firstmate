...
fm_control_harness_family() {  # <recorded-harness>
  case "${1-}" in
    ...
    # agy is exact like pi: the installed binary is exactly `agy`, and an
    # `agy*` prefix would claim unrelated raw-command basenames.
    agy) printf 'agy' ;;
...
# bin/fm-spawn.sh refuses a --secondmate launch on both. The control plane asks
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|grok|kimi|agy) printf 'none' ;;
    *) return 1 ;;
  esac
}
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    muse)
      ...
      ;;
    agy)
      # agy's busy and turn-end wiring is one per-task workspace hook file in
      # the singular `.agent/` discovery root (the plural `.agents/` root is
      # left to the project; agy merges hooks from both). fm-spawn refuses the
      # spawn when the project tracks this exact path, so the file here is
      # always firstmate-written and safe to retire.
      printf '%s\n' "$wt/.agent/hooks.json"
      ;;
  esac
}
