...
# exit, agy exposes conversation-id and latest-conversation resume forms,
# opencode resumes the most recent session for the cwd with --continue, and
# claude, pi, pi-signed, omp, and kimi have no verified pane-resume contract at
...
# harness= that way), except raw `agy`, which is recorded as `raw-agy` so it
# cannot inherit the verified adapter's mechanics. This is the one place that
# prefix rule is stated. `pi` and
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    # agy loads hooks from a firstmate-owned customization root supplied by
    # --add-dir, so the whole directory is the retired incarnation's wiring.
    agy) printf '%s\n' "$state/$id.agy-hooks" ;;
  esac
}
