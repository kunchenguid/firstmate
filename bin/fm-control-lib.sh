...
fm_control_harness_supported() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|agy) return 0 ;;
  esac
  return 1
}
...
# crewmate/scout adapters only: none has a primary supervision protocol, and
# bin/fm-spawn.sh refuses a --secondmate launch on any of them. A secondmate IS a
# firstmate, so it needs the emitted primary protocol in
# bin/fm-supervision-instructions.sh and a verified way to keep a watcher cycle
# alive; agy's worker mechanics being verified says nothing about either. The
...
fm_control_interrupt_key() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|kimi|cursor|gemini|muse|rovo|agy) printf 'Escape' ;;
    grok) printf 'C-c' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
    claude|codex|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|agy) printf '1' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|rovo|agy) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|rovo|agy) printf 'none' ;;
    *) return 1 ;;
  esac
}
