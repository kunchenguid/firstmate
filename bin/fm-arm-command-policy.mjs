...
function remoteModePathAllowed(value, home) {
  if (["config/x-mode.env", "./config/x-mode.env", "config/telegram-mode.env", "./config/telegram-mode.env"].includes(value)) return true;
  if (!path.isAbsolute(value)) return false;
  const normalized = path.normalize(value);
  return normalized === path.join(path.normalize(home), "config/x-mode.env")
    || normalized === path.join(path.normalize(home), "config/telegram-mode.env");
}
...
function setupKind(info, context) {
  ...
  if ((values[0] === "source" || values[0] === ".") && values.length === 2 && remoteModePathAllowed(values[1], context.home)) return "source";
  if (values[0] === "[" && values[1] === "-f" && values[3] === "]" && values.length === 4 && remoteModePathAllowed(values[2], context.home)) return "test-source";
  return "";
}
