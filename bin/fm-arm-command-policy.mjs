...
function cadenceConfigPathAllowed(value, home) {
  if (["config/x-mode.env", "./config/x-mode.env", "config/topic-mode.env", "./config/topic-mode.env"].includes(value)) return true;
  if (!path.isAbsolute(value)) return false;
  const normalized = path.normalize(value);
  const config = path.join(path.normalize(home), "config");
  return normalized === path.join(config, "x-mode.env") || normalized === path.join(config, "topic-mode.env");
}
...
function setupKind(info, context) {
  ...
  if ((values[0] === "source" || values[0] === ".") && values.length === 2 && cadenceConfigPathAllowed(values[1], context.home)) return "source";
  if (values[0] === "[" && values[1] === "-f" && values[3] === "]" && values.length === 4 && cadenceConfigPathAllowed(values[2], context.home)) return "test-source";
  return "";
}
