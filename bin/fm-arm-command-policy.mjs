...
// The opt-in fast-cadence env files (X mode, Telegram mode) are the only
// sources blessed as setup nodes; see docs/telegram-mode.md for the contract.
const CADENCE_ENV_FILES = ["config/x-mode.env", "config/telegram-mode.env"];

function cadenceEnvPathAllowed(value, home) {
  for (const file of CADENCE_ENV_FILES) {
    if (value === file || value === `./${file}`) return true;
    if (path.isAbsolute(value) && path.normalize(value) === path.join(path.normalize(home), file)) return true;
  }
  return false;
}
