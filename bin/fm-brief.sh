...
The main firstmate's answer normally writes that closing line at answer time; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`resolved [key=<slug>]: {how it cleared}\` yourself if you opened it with a key, or \`resolved: {how it cleared}\` if you did not, as your domain resumes.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved [key=<slug>]: {how it cleared}\` yourself if you opened it with a key, or \`resolved: {how it cleared}\` if you did not, as you resume.
