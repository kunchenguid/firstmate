...
Optional helper: \`bin/fm-secondmate-report.sh\` can append a correlated status line through the same serialized producer path; when it is unavailable, use the status command below and include the same \`corr=<id>\`.
...
Use this helper for every status transition named below; never redirect to the status file directly.
...
The main firstmate's answer normally writes that closing line at answer time; when a blocker or wait clears WITHOUT an answer from the main firstmate, append \`resolved [key=<slug>]: {how it cleared}\` yourself as your domain resumes. Write a blocker with the same canonical key position: \`blocked [key=<slug>]: {reason}\`.
...
6. If a decision belongs to a human (product choices, destructive actions),
   ...
   Firstmate's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a firstmate reply, append \`resolved [key=<slug>]: {how it cleared}\` yourself as you resume. Write a blocker with the same canonical key position: \`blocked [key=<slug>]: {reason}\`.
