...
Your worktree and your temp directory are both removed when this task is cleaned up, so anything the report depends on must live beside it: copy that file into \`$DATA/$ID/\` and cite it there, never a path under the worktree or under a temp directory.
