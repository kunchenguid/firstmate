...
The intent statement you give \`no-mistakes axi run\` is pasted into the PR description, so write it concise and structured, not a run-on paragraph:
- Use exactly these three top-level headings, in this order: \`## What is the problem\`, \`## What was the fix\`, and \`## Proof of work\`.
- Put the problem and fix in their named sections as tight bullets.
- In \`## Proof of work\`, map the latest effective decisions to evidence, name superseded decisions that were not followed, list relevant tests, and include screenshots for UI work.
- Keep tests, screenshots, risk notes, and pipeline evidence inside \`## Proof of work\`, not in separate top-level sections.
- No single multi-paragraph run-on, and no restating the whole investigation.
- If a PR section is auto-generated by the pipeline rather than authored by you, keep the part you control - the intent - concise and structured regardless.
