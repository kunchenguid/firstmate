...
if [ "$EXISTING_PR_SET" -eq 1 ]; then
  ...
  if [ "$MODE" = no-mistakes ]; then
    ...
    BASE_ASSERT="; if [ \"\$PR_PROBE_OK\" -eq 1 ] && ! (cd \"\$PR_PROBE_ROOT/repo\" && gh-axi pr list --state all --base $BASE_BRANCH_QUOTED --head \"\$PR_BRANCH\" --limit 100 --fields url | grep -Fq $EXPECTED_PR_RECORD_QUOTED); then PR_BASE_OK=0; fi"
