...
# Usage: fm-brief.sh <task-id> <repo-name> --mode <no-mistakes|direct-PR|local-only> [--tiered <on|off>] [--ci-tests <on|off>] [--herdr-lab]
...
# A no-mistakes ship brief also takes an optional --tiered <on|off> (default off) and
# --ci-tests <on|off> (default off), resolved by firstmate the same way as --mode from
# the project's registered +tiered/+ci-tests posture (AGENTS.md section 6). When
# tiered=on, the brief's starting-the-run step has the crew classify its diff with
# bin/fm-tier.sh before pushing and trims the skip set (document, plus test under
# ci-tests=on) only at tier=1; review and ci are never in any skip set. tiered=off
# (the default) keeps the plain starting-the-run step, byte-for-byte unchanged.
# --tiered/--ci-tests are refused outside no-mistakes mode and outside ship briefs.
...
if [ "$TIERED_SET" -eq 1 ] || [ "$CITESTS_SET" -eq 1 ]; then
  if [ "$KIND" != ship ] || [ "$MODE" != no-mistakes ]; then
    echo "error: --tiered/--ci-tests apply only to no-mistakes ship briefs; a diff can only run a trimmed gate against the full no-mistakes pipeline" >&2
    exit 1
  fi
fi
...
# explicit --mode before launching. tiered/ci-tests (also validated above) only
# affect the no-mistakes DoD's starting-the-run step (below).
case "$MODE" in
  direct-PR)
    SETUP2=""
...
Classify your diff and supply this run's intent as a base64 push option, then push to start the run so the tier's skip option is applied:
...
This starts the run for you exactly as \`axi run\` would (a malformed or empty intent option blocks the run rather than starting it, which is why the snippet falls back to a plain push when extraction yields nothing). If the push reports "Everything up-to-date" (a stale ref left in the gate mirror by an earlier \`axi run\` attempt), do NOT touch the gate internals yourself - append \`blocked: stale no-mistakes mirror ref for fm/$ID\` and stop; firstmate will clear it and tell you to retry.
EOF
      STARTRUN=${STARTRUN%$'\n'}
    fi
    IFS= read -r -d '' DOD <<EOF || true
...
NEVER cancel, reset, reattach, restart, or start a fresh run mid-validation, and NEVER hand-edit, commit, or fix findings yourself while a run is active - the pipeline applies every fix, and doing it yourself duplicates work and thrashes the run.
Quiet is normal: a run step can take many minutes, so wait for the gate rather than intervening.
If a run genuinely fails, do NOT loop or restart it - append \`failed: {what failed, with evidence}\` and stop; firstmate decides the recovery.
