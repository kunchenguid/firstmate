...
A Lavish review loop is exactly such a deliberate external wait: if you open a Lavish poll loop (foreground \`lavish-axi poll\`, re-running the poll after each shell-tool timeout), immediately append \`$PAUSED_VERB: hosting lavish review loop - waiting on captain annotations, re-running poll after each shell timeout\`.
Never shorten the loop to bounded polls or pass \`--timeout-ms\`: the lavish poll contract forbids both, and the paused line is the fix for supervision noise.
...
4. Report status by appending one line:
   ...
   A Lavish review loop is exactly such a deliberate external wait: if you open a Lavish poll
   loop (foreground \`lavish-axi poll\`, re-running the poll after each shell-tool timeout),
   immediately append \`$PAUSED_VERB: hosting lavish review loop - waiting on captain annotations, re-running poll after each shell timeout\`.
   Never shorten the loop to bounded polls or pass \`--timeout-ms\`: the lavish poll contract
   forbids both, and the paused line is the fix for supervision noise.
