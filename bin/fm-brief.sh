...
Nothing outside \`$DATA/$ID/\` is guaranteed to survive this task's cleanup, so anything the report depends on must live beside it: copy that file into \`$DATA/$ID/\` and cite it there, never a path under the worktree or under a temp directory.
