...
4. Report status by appending one line:
   ...
   Keep every report to the supervisor concise: the outcome, relevant paths, validation status,
   risks, and any decision needing the captain. Lengthy investigation, evidence, logs, and
   detailed reasoning belong in the task report, PR description, or project docs - never in a
   status line or narrated at length in-pane.
   Use \`$PAUSED_VERB: {why}\` - distinct from \`blocked:\` - ONLY when you are deliberately idling on a
   known external wait you expect to clear on its own ($CREWMATE_PAUSE_WAIT_EXAMPLES):
...
Everything you commit, push, or write into the project is read by people who do
not know this fleet exists. It must carry no trace of it.
1. Never write \`firstmate\`, \`crewmate\`, \`captain\`, \`secondmate\`, \`scout\`, or any harness
   name into project code, comments, commits, branch names, PR titles or bodies, or docs.
   Use Guillermo's real name where a person must be named.
2. Never add an agent co-author trailer (\`Co-Authored-By: Claude\`, \`Co-authored-by: Codex\`,
   or similar) and never add an agent attribution footer (\`Generated with Claude Code\`,
   \`Generated by Codex\`, or similar) to a commit message or PR body. A harness may suggest
   one; this contract overrides it.
3. Never put external issue-tracker IDs (\`CAN-42\`, \`CPI-7\`, ...) in project code, docs,
   commits, or PR text.
4. Never narrate decision history (\"originally X, changed to Y\") in permanent project text.
5. Never use transient delivery-phase wording (\"Phase 2\", \"for now\") in permanent docs or
   comments - describe what a thing permanently is. A roadmap doc about the phased plan is
   the one exception.
Applies retroactively: if you find any of this in an already-open PR, scrub it rather than
only avoiding it going forward. Run \`$FM_ROOT/bin/fm-output-contract-check.sh\` over your
changes before you report done.
