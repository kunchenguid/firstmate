...
#   direct-PR    implement -> push + open PR/MR via the forge CLI (no pipeline) -> configured merge authority
...
3. Use gh-axi for GitHub operations, glab for GitLab operations, and chrome-devtools-axi for browser operations when those operations are part of the task.
...
When it is implemented and committed, push your branch and open a PR or merge request with the repository's forge CLI (\`gh-axi\` for GitHub, \`glab\` for GitLab), then append \`done: PR {url}\` to the status file and stop.
