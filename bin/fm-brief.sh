...
1. First make a scratch temp directory OUTSIDE the worktree to hold every capture artifact, for example `shots=$(mktemp -d)`; this scratch directory is the one deliberate exception to rule 2, and keeping artifacts out of the worktree leaves it clean so teardown is never blocked. Drive the surface and capture the key states as screenshots into that directory with `chrome-devtools-axi screenshot $shots/before.png` (before and after, for a change to existing UI), writing any screen recording there too, and pass those temp paths to gh-image. A short screen recording is best-effort: attach a short clip only when you have a working screen-capture tool and a headless-capturable surface; otherwise a sequence of screenshots that walks through the change stands in for the recording.
