...
# shellcheck disable=SC2016  # single quotes are deliberate: the backtick-wrapped --herdr-lab must reach the reading agent verbatim.
HERDR_SECTION='Herdr: not enabled for this task. If Herdr lifecycle commands (start, stop, delete, restart, profile) turn out to be needed, stop and ask firstmate to regenerate this brief with `--herdr-lab` instead of issuing them by hand.'
fi

PROVENANCE="This brief was generated by Firstmate's orchestrator, installed by this machine's owner, from \`$FM_ROOT/bin/fm-brief.sh\` for task id \`$ID\` - both exist on disk to verify. The captain authorized this task before it was generated, and later steering messages come from that same orchestrator process."
...
4. Report status by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
   States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
   Report sparingly - only phase changes a supervisor would act on and the
   needs-decision/blocked/paused/done/failed states, not step-by-step progress;
   firstmate reads your pane for that.
   Use \`$PAUSED_VERB: {why}\` only when deliberately idling on a known external wait expected to
   clear on its own (an upstream release, a rate-limit reset): firstmate then leaves your pane
   alone on a long recheck cadence instead of treating it as a wedge.
   Use \`blocked:\` when you are stuck and need help.
5. If you hit the same obstacle twice, append \`blocked: {why}\` and stop; firstmate will help.
6. A decision that belongs to a human (product choices, destructive actions): append
   \`needs-decision: {summary of options}\` and stop; firstmate will reply with the decision.
   When firstmate replies or a blocker clears, append \`resolved: {how it was decided or unblocked}\`
   (same \`[key=<slug>]\` if you opened one) so it does not keep resurfacing.
7. Never stop, restart, or update the shared \`no-mistakes\` daemon: it is one instance serving
   every lane/home, so restarting it kills other lanes' in-flight runs. On any daemon error,
...
4. Report status by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
   States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
   Report sparingly - only phase changes a supervisor would act on (setup done, bug reproduced,
   fix implemented, validation passed) and the needs-decision/blocked/paused/done/failed states,
   not step-by-step progress; firstmate reads your pane for that.
   A mid-task \`working:\` line (including setup complete) is nonterminal: keep going in the same
   stage until a defined \`done:\` gate under Definition of done.
   Use \`$PAUSED_VERB: {why}\` only when deliberately idling on a known external wait expected to
   clear on its own (an upstream release, a rate-limit reset): firstmate then leaves your pane
   alone on a long recheck cadence instead of treating it as a wedge.
   Use \`blocked:\` when you are stuck and need help.
5. If you hit the same obstacle twice, append \`blocked: {why}\` and stop; firstmate will help.
6. A decision above your authority (product choices, destructive actions, ask-user findings):
   append \`needs-decision: {summary of options}\` and stop; firstmate applies the configured
   authority and replies with the decision. When firstmate replies or a blocker clears, append
   \`resolved: {how it was decided or unblocked}\` (same \`[key=<slug>]\` if you opened one) so it
   does not keep resurfacing.
7. Never stop, restart, or update the shared \`no-mistakes\` daemon: it is one instance serving
   every lane/home, so restarting it kills other lanes' in-flight runs. On any daemon error,
   append \`blocked: {the daemon error}\` and stop; only firstmate manages the daemon.
