...
# Block texts are assigned with `read -d ''` heredocs, never `VAR=$(cat <<EOF`:
# bash 3.2 (macOS system /bin/bash) mis-scans a heredoc nested in $(...) when
# its body contains an apostrophe, backtick, or paren, breaking the parse of
# the entire rest of the script. `read` exits 1 at EOF-without-NUL (hence
# `|| true`), and the trailing newline is stripped to keep the emitted brief
# byte-identical to the former command-substitution form.
