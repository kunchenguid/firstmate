...
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
   Prefer direct, unpiped invocations of common CLI tools (git, docker, npm, npx jest/eslint/tsc, grep, psql, etc.) rather than piping their output through \`tail\`/\`head\` yourself: the environment's automatic output-compaction hook only rewrites simple invocations, so a manual pipe defeats it and the hook's own filtering is usually more effective anyway.
