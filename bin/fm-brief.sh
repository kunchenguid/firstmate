...
The implementation is ready for validation only when committed on your branch; the task is not complete at that commit.
Immediately after committing the implementation, invoke the no-mistakes skill using the current harness's supported skill invocation form.
Do not append \`done:\`, stop, or wait for a separate firstmate instruction at the implementation commit.
The same worker that implemented the change must keep driving validation through every pipeline return until the configured ready point below.

You drive no-mistakes by responding to its gates, not by implementing fixes.
Follow the guidance no-mistakes itself provides for the mechanics once the skill loads; \`no-mistakes axi run --help\` plus the \`help\` lines in each \`axi\` response are authoritative and version-matched to the installed binary.
...
After no-mistakes reports CI green (the configured CI-ready return point - do not wait for it to keep monitoring in the background until merge), append \`done: PR {url} checks green\` and stop. You are finished.
EOF
    ;;
esac
