...
3. Use gh-axi for GitHub operations; use chrome-devtools-axi for every browser operation; never use the claude-in-chrome extension, any \`mcp__claude-in-chrome__*\` tool, or any other MCP browser tool, even when they are available in the session.
