...
For the narrow case where an explicit captain-requested operation unexpectedly gains a new multi-minute phase, append \`working [key=new-phase-<work-slug>]: {condition}; revised rough duration {duration}\` once when that phase appears. Do not append it for an expected wait, background work, ordinary progress, or an unchanged estimate; append it again only with a new key for a genuinely new phase, or when the estimate changes materially. Failure and needed-decision reports keep using their own states unconditionally.
