...
#     is typed, except for the exact Firstmate doorbell, which is submitted
#     and verified before lifecycle control continues.
#
...
do_exit() {
  local state cmd verdict composer_state cancel interrupt_result=not-needed pending_bell
  ...
  while :; do
    ...
    case "$composer_state" in
      ...
      pending|pending-unproven)
        pending_bell=$(fm_task_inbox_doorbell_line "$STATE/$ID.inbox/001.msg" 2>/dev/null) \
          || pending_bell=
        if [ -n "$pending_bell" ] \
           && fm_backend_composer_matches_text "$BACKEND" "$T" "$pending_bell" "$LABEL"; then
          fm_backend_submit_exact_pending "$BACKEND" "$T" "$pending_bell" "$EXIT_RETRIES" "$POLL" "$LABEL" \
            || die "task $ID's exact pending Firstmate doorbell could not be submitted and verified; refusing to type the $cmd exit command"
          continue
        fi
