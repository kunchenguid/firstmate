...
# supported forges: GitHub, Bitbucket Data Center, and GitLab. Callers must
# validate task IDs and raw PR/MR URLs before constructing task paths or
# performing any side effect.
#
# The stored identity is provider-tagged: provider, url, host, path, number.
# "path" is the full project path, which is owner/repository on GitHub and
# Bitbucket Data Center and an arbitrarily nested group/subgroup/project
...
# instead. GitLab and Bitbucket Data Center both run on self-hosted instances,
# so the host is part of that identity rather than a constant. Every consumer
...
#
# FM_PR_OWNER and FM_PR_REPO are additionally set for github and bitbucket-dc
# because bin/fm-pr-merge.sh addresses both forges by owner/repository. A gitlab
# URL leaves them empty; teaching the merge path about GitLab is a separate
# change, and until then it refuses a GitLab URL rather than merging anything.
fm_pr_url_parse() {
  local raw=${1-} pattern host path user slug num
  local LC_ALL=C
  FM_PR_PROVIDER=
  FM_PR_URL=
  FM_PR_HOST=
  ...
  if [[ "$raw" =~ $pattern ]]; then
    ...
    FM_PR_URL=$raw
    FM_PR_HOST=$host
    # Consumed by bin/fm-pr-check.sh and bin/fm-pr-merge.sh after fm_pr_url_parse.
    # shellcheck disable=SC2034
    FM_PR_OWNER="~$user"
    # shellcheck disable=SC2034
    FM_PR_REPO=$slug
    FM_PR_PATH="~$user/$slug"
    FM_PR_NUMBER=$num
    return 0
  fi
