...
# arbitrarily nested group/subgroup/project namespace on GitLab or Codebase. A GitLab
...
fm_pr_url_parse() {
  ...
  pattern='^https://(code\.byted\.org|code-tx\.byted\.org)/([A-Za-z0-9._/-]+)/merge_requests/([1-9][0-9]*)$'
  if [[ "$raw" =~ $pattern ]]; then
    host=${BASH_REMATCH[1]}
    path=${BASH_REMATCH[2]}
    fm_pr_codebase_path_valid "$path" || return 1
    FM_PR_PROVIDER=codebase
    FM_PR_URL=$raw
    FM_PR_HOST=$host
    FM_PR_PATH=$path
    FM_PR_NUMBER=${BASH_REMATCH[3]}
