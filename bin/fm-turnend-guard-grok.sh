...
[ -n "$REASON" ] || REASON='tasks in flight, no live watcher supervision'
...
GROK_TURNEND_GUARD_ACTIVE=1 \
  ...
  grok --resume "$SESSION_ID" \
    --cwd "$ROOT" \
    --output-format plain \
    -p "TURN WOULD END BLIND - supervision is off. Restore watcher supervision before ending the turn.
