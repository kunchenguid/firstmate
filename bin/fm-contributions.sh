...
# 1..25). Every read is capped at five seconds and retried up to three
# attempts with backoff while the deadline allows. A pull observation has
# three dependent waves: core, six independent reads, then the closing head
# read; an issue has two waves. Parallelizing each independent wave bounds
# either observation to 3 * 5 = 15 seconds of successful reads; retries add
# time only on failure. poll reserves min(the configured budget, 15) before
...
# applies to every owner without another forge read. When the budget runs
# out mid-observation, the poll ends with that URL's records untouched; only
# a genuine forge failure or head change records an error.
...
forge() {
  local remaining bounded rc=0 attempt forge_err=${FORGE_ERR:-$TMP/forge.err}
