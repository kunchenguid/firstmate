...
This file is the executable owner of candidate registration, canonical envelope
...
def load_config(home: Path) -> dict[str, Any] | None:
    ...
    if value["sealing_enabled"] or value["delivery_enabled"] or value["consumer_enabled"]:
...
def validate_item(item: Any, config: Mapping[str, Any], *, verify_source: bool = True) -> dict[str, Any]:
    ...
    if item.get("sensitive_scan") != "passed":
        raise HandoffError("SENSITIVE_SCAN", "handoff item lacks the required sensitive scan result")
...
def deliver_pending(home: Path) -> dict[str, Any]:
    ...
    with state_lock(layout):
        ...
        if not ready:
            ...
            write_receipt(layout, "delivery", "pending", reason, pending_count=len(pending))
        ...
        if completed.returncode != 0:
            ...
            write_receipt(layout, "delivery", "pending", "recipient-notification-failed", pending_count=len(pending))
        ...
        write_receipt(layout, "delivery", "notified", "exact-recipient-notified", pending_count=len(pending))
        return {"status": "notified", "pending_count": len(pending)}
...
def hook_output(value: Mapping[str, Any] | None = None) -> None:
    if value:
...
def find_completed_approval(home: Path, config: Mapping[str, Any], record_id: str) -> dict[str, Any] | None:
    layout = StateLayout(home)
    for path in reversed(approval_records(layout, record_id)):
        value = read_json_file(path, max_bytes=64 * 1024)
        if not isinstance(value, dict) or value.get("schema") != APPROVAL_SCHEMA:
            continue
        try:
            return verify_completed_transaction(home, config, record_id, value)
        except HandoffError as exc:
    ...
    try:
        ...
        if args.command == "claude-hook":
            if exc.code in {"STATE_FILE_MODE", "STATE_SYMLINK", "PAYLOAD_MISMATCH"}:
                hook_output(guard_deny("The context handoff safety boundary is unhealthy; mutation was denied."))
                return 0
