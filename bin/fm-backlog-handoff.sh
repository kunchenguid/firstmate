...
#
# It gates the ID-targeted path ahead of the local/remote split, so a remote
# route cannot deliver into a captain-held lane either. --resume-pending exits
# above it: that path finishes an already-staged delivery whose items have
# already left the main backlog, and it targets no single lane.
