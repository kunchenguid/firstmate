...
# backlog - on ANOTHER fleet machine, while the main backlog is here. The move is
# then genuinely two-sided and runs in one fixed order:
...
#
# An SSH-routed remote secondmate takes the other transport: one atomic local
# tasks-axi mv removes the selected set from the dispatchable backlog into
# data/handoff/<id>.outbox.md, then an idempotent confined transfer and
# fm-backlog-receive.sh deliver it. A present outbox is the whole recovery
# record. No two-phase journal exists.
...
usage() {
  ...
  echo "       fm-backlog-handoff.sh --resume-pending" >&2
}
...
secondmate_machine() {
  local id=$1
  [ -f "$REG" ] || return 0
  secondmate_registry_field "$REG" "$id" machine 2>/dev/null || true
}
