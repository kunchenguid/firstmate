...
# It emits exactly one merged line for a merged PR or MR, exactly one closed
# line for a PR or MR closed without merging, and stays silent otherwise,
# including on every error, so a failed lookup can never be read as a merge.
# closed is a terminal read distinct from merged: the watcher wakes on it but
# never retires the poll for it, because a closed-unmerged result alone does
# not prove the work landed elsewhere. The provider-tagged identity is data in
