...
# dirs (data/, state/, config/, projects/), so a secondmate's in-flight work is
# never disrupted. Worktrees of this repo share one object store, so a single
# fetch refreshes them all; standalone-clone homes are fetched on their own.
# Secondmate homes are leased at a detached HEAD on the default branch, so a
# fast-forward there advances HEAD only and never touches
