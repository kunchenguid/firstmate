#!/usr/bin/env python3
"""SQLite ownership for the optional local WhatsApp bridge.

Schema version 1; a database binds permanently to mode, agent, creator and home.
Inbound media uses an additive attachments table; identity binding stays schema 1.
...
class Config:
    def __init__(self, path):
        ...
        if any(r.get(k) for k in ("typing", "read_receipts", "tts", "external_llm")):
            raise BridgeError("optional status/model features are pending a later stage")
        if r.get("media") not in (True, False, None):
            raise BridgeError("media must be boolean")
        self.media = r.get("media", False) is True
        if self.media and self.rates["media-metadata"] < 2:
            raise BridgeError("media-metadata rate must allow the two reserved GETs per attachment")
        command = r.get("media_stt_command")
        self.stt_command = None
        if command is not None:
            if not self.media:
                raise BridgeError("media_stt_command requires media true")
            self.stt_command = self.absolute(command)
        self.media_python = Path(r.get("media_python", sys.executable))
        if not self.media_python.is_absolute():
            raise BridgeError("media_python must be absolute")
        self.stt_model = self.absolute(r["media_stt_model"]) if r.get("media_stt_model") else None
        if self.stt_model and self.stt_command:
            raise BridgeError("choose media_stt_model or media_stt_command, not both")
        if self.stt_model and not self.media:
            raise BridgeError("media_stt_model requires media true")
...
class Store:
    def __init__(self, config, clock=time.time):
        ...
        CREATE TABLE IF NOT EXISTS attachments(
          request TEXT PRIMARY KEY, media_id TEXT NOT NULL, kind TEXT NOT NULL,
          mime TEXT, sha256 TEXT, caption TEXT, filename TEXT, voice INTEGER NOT NULL DEFAULT 0,
          path TEXT, transcript TEXT, extracted TEXT, status TEXT NOT NULL, error TEXT,
          attempts INTEGER NOT NULL DEFAULT 0, due REAL NOT NULL);
        CREATE TABLE IF NOT EXISTS attachment_details(
          request TEXT PRIMARY KEY, details TEXT NOT NULL);
        """)
    ...
    def reserve(self, endpoint, units=1):
        with self.tx():
            now = self.now()
            wait = self.rate_wait(endpoint, now)
            if units > 1:
                rows = self.rows("SELECT at FROM rates WHERE endpoint=? AND at>? ORDER BY at", (endpoint, now - 60))
                shortage = len(rows) + units - self.config.rates[endpoint]
                if shortage > 0:
                    wait = max(0.01, rows[shortage - 1]["at"] + 60 - now)
            if wait:
                return wait
            self.db.execute("DELETE FROM rates WHERE at<=?", (now - 60,))
            self.db.executemany("INSERT INTO rates VALUES(?,?)", [(endpoint, now)] * units)
        return 0
    ...
    def request(self, request):
        ...
        if row is None or row["sender"] != self.config.creator or row["state"] in (
                "quarantined", "historical", "unsupported", "invalid", "media_pending"):
            raise BridgeError("request is not an authorized owner request")
        # A download/decoder failure never reached canonical main intake.
        # Main's own failed outcomes must remain readable for idempotent replay.
        if row["state"] == "failed" and not row["note_id"]:
            raise BridgeError("request is not an authorized owner request")
        return dict(row)
    ...
    def snapshot(self):
        return {"schema": "fm-whatsapp-status.v1", "cursor": int(self.get("cursor")),
                ...
                "requests": self.rows("SELECT request,state,note_id FROM inbound WHERE sender=? ORDER BY received",
                                      (self.config.creator,)),
                "attachments": self.rows("SELECT request,kind,mime,status,error,attempts,due FROM attachments ORDER BY rowid"),
