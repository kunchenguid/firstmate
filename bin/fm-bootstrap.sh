...
x_mode_setup() {
  ...
  # `read -d ''` instead of `VAR=$(cat <<EOF`: bash 3.2 (macOS system bash)
  # mis-scans a heredoc nested in $(...) when its body has unbalanced
  # apostrophes, backticks, or parens, breaking the whole script's parse.
