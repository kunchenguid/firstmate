...
# shellcheck source=bin/fm-trello-lib.sh disable=SC1091
...
control_plane_write_if_changed() {
...
x_mode_setup() {
  ...
  control_plane_write_if_changed "$shim" "$shim_body" 700 || { fmx_arm_failed; return 0; }
...
EOF
  ...
  control_plane_write_if_changed "$cadence" "$cadence_body" 600 || { fmx_arm_failed; return 0; }

  echo "FMX: X mode on - relay poll armed via state/x-watch.check.sh; 30s watcher cadence in config/x-mode.env"
}

# Trello control plane (opt-in): when this home's config/trello.env carries all
# three board credentials, wire the board poll into the EXISTING watcher check
# mechanism without touching fm-watch.sh or any watcher-backbone file - the exact
# additive pattern as x_mode_setup. Drops two idempotent, gitignored artifacts:
#   state/trello-watch.check.sh       - check shim that execs bin/fm-trello-poll.sh
#   state/trello-watch.check-trust    - byte binding required for custom checks
#   config/trello-mode.env            - exports FM_CHECK_INTERVAL=60 for this home's watcher
# On opt-out (config removed or incomplete) it removes those artifacts. Absent the
# credentials AND with no leftover artifacts it is a complete no-op, so a
# non-Trello user sees zero change. Prints one confirmation line on opt-in, and
# one on opt-out only when it actually removed artifacts. It never restarts the
# watcher itself; a cadence transition is the emitted harness protocol's job.
trello_mode_setup() {
  local env_file key token board shim trust cadence shim_body cadence_body tool missing
  env_file="$FM_HOME/config/trello.env"
  shim="$STATE/trello-watch.check.sh"
  trust="$STATE/trello-watch.check-trust"
  cadence="$CONFIG/trello-mode.env"

  key=; token=; board=
  if [ -f "$env_file" ]; then
    key=$(trello_env_get TRELLO_API_KEY "$env_file")
    token=$(trello_env_get TRELLO_TOKEN "$env_file")
    board=$(trello_env_get TRELLO_BOARD_SHORTLINK "$env_file")
  fi

  trello_remove_artifacts() {
    rm -f "$shim" "$trust" "$cadence" 2>/dev/null || true
    [ ! -e "$shim" ] && [ ! -e "$trust" ] && [ ! -e "$cadence" ]
  }
  trello_supervision_repair() {
    local out
    out=$("$SCRIPT_DIR/fm-supervision-instructions.sh" --repair-line 2>/dev/null) \
  ...
  if [ -z "$key" ] || [ -z "$token" ] || [ -z "$board" ]; then
    if [ -e "$shim" ] || [ -e "$trust" ] || [ -e "$cadence" ]; then
      if trello_remove_artifacts; then
        echo "TRELLO: control plane off - removed board poll shim and 60s cadence; default cadence applies on the next supervision cycle; $(trello_supervision_repair)"
      else
        echo "TRELLO: control plane off - failed to remove board poll shim or 60s cadence"
      fi
    fi
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    if [ -e "$shim" ] || [ -e "$trust" ] || [ -e "$cadence" ]; then
      if trello_remove_artifacts; then
        echo "TRELLO: control plane off - missing board poll dependencies; install them and rerun bootstrap"
      else
        echo "TRELLO: control plane off - failed to remove board poll shim or 60s cadence after missing dependencies"
      fi
    fi
    return 0
  fi
  ...
  trello_arm_failed() {
    if trello_remove_artifacts; then
      echo "TRELLO: control plane off - failed to arm board poll shim or 60s cadence"
    else
      echo "TRELLO: control plane off - failed to arm board poll shim or 60s cadence; stale artifacts remain"
    fi
  }

  mkdir -p "$STATE" "$CONFIG" 2>/dev/null || { trello_arm_failed; return 0; }

  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - Trello control-plane board poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-trello-poll.sh")
EOF
)
  control_plane_write_if_changed "$shim" "$shim_body" 700 || { trello_arm_failed; return 0; }
  FM_STATE_OVERRIDE="$STATE" "$SCRIPT_DIR/fm-check-register.sh" trello-watch >/dev/null 2>&1 \
    || { trello_arm_failed; return 0; }

  cadence_body=$(cat <<'EOF'
# Auto-generated by fm-bootstrap.sh - Trello control-plane watcher cadence.
# Source this before the active harness protocol starts a watcher process so
# fm-watch.sh polls the Trello check once per minute. Non-Trello homes have no
# such file and keep the default 300s cadence.
export FM_CHECK_INTERVAL=60
EOF
)
  control_plane_write_if_changed "$cadence" "$cadence_body" 600 || { trello_arm_failed; return 0; }

  echo "TRELLO: control plane on - board poll armed via state/trello-watch.check.sh; 60s watcher cadence in config/trello-mode.env"
}
