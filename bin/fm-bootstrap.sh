...
trello_mode_setup() {
  local env_file key token board shim cadence shim_body cadence_body tool missing
...
EOF
  ...
  write_if_changed "$shim" "$shim_body" || { trello_arm_failed; return 0; }
  chmod +x "$shim" 2>/dev/null || { trello_arm_failed; return 0; }
...
EOF
  ...
  write_if_changed "$cadence" "$cadence_body" || { trello_arm_failed; return 0; }
