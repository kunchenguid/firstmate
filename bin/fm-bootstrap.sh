...
#          (secondmate_sync, x_mode_setup, tg_mode_setup, fleet_sync) while still printing
...
# Telegram mode (opt-in): when this home's .env carries a non-empty FMTG_BOT_TOKEN,
# wire the Telegram poll into the EXISTING watcher check mechanism without touching
# fm-watch.sh or any other watcher-backbone file. Drops two idempotent,
# gitignored artifacts:
#   state/tg-watch.check.sh - check shim that execs bin/fm-tg-poll.sh each cycle
#   config/tg-mode.env      - exports FM_CHECK_INTERVAL=30, sourced by the watcher
# On opt-out (no token, or empty) it removes any such artifacts so the instance
# reverts to the default 300s no-poll behavior. Absent a token AND with no leftover
# artifacts it is a complete no-op (nothing written, nothing printed).
tg_mode_setup() {
  local env_file token shim cadence shim_body cadence_body missing
  env_file="$FM_HOME/.env"
  ...
  if [ -z "$token" ]; then
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if tg_mode_remove_artifacts; then
        echo "FMTG: Telegram mode off - removed poll shim and 30s cadence; restart the watcher (bin/fm-watch-arm.sh --restart) to drop back to the default cadence"
      else
  ...
  if [ "$missing" -ne 0 ]; then
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if tg_mode_remove_artifacts; then
        echo "FMTG: Telegram mode off - missing dependencies; install them and rerun bootstrap"
      else
        echo "FMTG: Telegram mode off - failed to remove poll shim or 30s cadence after missing dependencies"
      fi
    fi
    return 0
  fi
  ...
  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - Telegram mode connector poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-tg-poll.sh")
EOF
)
  write_if_changed "$shim" "$shim_body" || { fmtg_arm_failed; return 0; }
  chmod +x "$shim" 2>/dev/null || { fmtg_arm_failed; return 0; }

  cadence_body=$(cat <<'EOF'
# Auto-generated by fm-bootstrap.sh - Telegram mode watcher cadence.
# Source this before arming the watcher so fm-watch.sh polls the Telegram
# check every 30s. Non-TG instances have no such file and keep the
# default 300s cadence.
export FM_CHECK_INTERVAL=30
EOF
)
  write_if_changed "$cadence" "$cadence_body" || { fmtg_arm_failed; return 0; }

  echo "FMTG: Telegram mode on - poll armed via state/tg-watch.check.sh; 30s watcher cadence in config/tg-mode.env"
}
