...
install_cmd() {
  case "$1" in
    ...
    superset) echo "see https://docs.superset.sh/cli for installation" ;;
...
# never told tmux is missing, and only orca and superset drop treehouse. A
# backend value with no verified dependency set is reported before the
...
detect_local_tools() {
  ...
  # backend actually requires treehouse (every backend except orca and superset,
  # which each own their own worktrees); an orca or superset home must not be
