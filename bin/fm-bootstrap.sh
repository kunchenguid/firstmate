...
#                 "MODEL_POLICY: <where the denied or unnamed model is configured>: <reason>",
...
# Forbidden-model policy (bin/fm-model-policy-lib.sh owns the file format and
# the decision; docs/configuration.md "Forbidden models" owns the operator
# contract). Configuration validation is the earliest point a denied model can
# be caught, which is the whole point: a model this home forbids should cost a
...
model_policy_validate() {
  ...
  if [ -f "$dispatch" ] && command -v jq >/dev/null 2>&1 && jq -e . "$dispatch" >/dev/null 2>&1; then
    while IFS="$(printf '\t')" read -r harness model; do
      [ -n "$harness" ] || continue
      fm_model_policy_check "$CONFIG" "$model" && continue
      findings="${findings}MODEL_POLICY: config/crew-dispatch.json profile harness=$harness: $FM_MODEL_POLICY_ERROR
"
    done <<EOF
$(jq -r '
      def profiles($value):
      ...
      | .[]
      | select(type == "object")
      | ((.harness? // "") | tostring) + "\t" + ((.model? // "") | tostring)
    ' "$dispatch" 2>/dev/null)
EOF
  fi
