...
# authenticated long-poll check in the existing watcher and a one-second poll
# cadence. The poll itself blocks up to ten seconds, so the effective API
...
# The watcher starts the next bounded long poll promptly after the
# prior long poll returns. The poll's own timeout bounds the request rate.
export FM_TELEGRAM_CHECK_INTERVAL=1
EOF
)
