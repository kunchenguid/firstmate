...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    ...
    def malformed_pi_agent_dir($items):
      ($items | any(has("piAgentDir") and (((.piAgentDir | type) != "string") or (.piAgentDir | length) == 0)));
    def invalid_codex_homes:
      configured_profiles
      | map(select(.codexHome != null and .harness != "codex"));
    def invalid_pi_agent_dirs:
      configured_profiles
      | map(select(.piAgentDir != null and (.harness != "pi" and .harness != "pi-signed")));
    def bad_efforts:
      configured_profiles
    ...
    elif malformed_pi_agent_dir([(.rules // [])[]? | profiles(.use?)[]?]) then "use profile piAgentDir must be a non-empty string when present"
    ...
    elif has("default") and malformed_pi_agent_dir([profiles(.default)[]?]) then "default profile piAgentDir must be a non-empty string when present"
    elif (invalid_codex_homes | length) > 0 then "codexHome is supported only for codex profiles"
    elif (invalid_pi_agent_dirs | length) > 0 then "piAgentDir is supported only for pi and pi-signed profiles"
    else
      (configured_profiles
        | map(.harness)
        | map(select(. != null))
  ...
  if [ "${FM_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ]; then
    ...
    def profile($p):
      ...
      + (if ($p.codexHome? != null) then " [codexHome=" + ($p.codexHome | tostring) + "]" else "" end)
      + (if ($p.piAgentDir? != null) then " [piAgentDir=" + ($p.piAgentDir | tostring) + "]" else "" end);
