...
#
# The bridge is implemented (bin/fm-tg-*.sh, .agents/skills/fmtg-respond) but is
# intentionally inert, so a home whose .env carries FMTG_BOT_TOKEN still gets no
# Telegram behavior and no poll artifacts.
#
# Why it is off: messages reached state/tg-inbox/ but never woke firstmate,
# because the wake queue was written with a 'tg' kind that the watcher's
# classifier does not recognize, so no wake ever fired and the inbox silently
# accumulated unread captain messages. A bridge that takes messages and never
# surfaces them is worse than no bridge, so it stays off until that is fixed.
#
# TODO to re-enable, in order:
#   1. Route tg wakes through the check: mechanism the watcher already honors
#      (see x_mode_setup above for the shape that works), rather than a bespoke
#      'tg' wake kind, OR teach bin/fm-classify-lib.sh to classify 'tg' wakes.
#   2. Verify end to end that a real message produces a check: wake, a drained
#      inbox, and a reply.
#   3. Delete the FMTG_ENABLE guard below so .env presence alone arms it again,
#      and restore the full section 15 documentation in AGENTS.md.
#
# Until then the disabled path still runs on every bootstrap so a previously
# armed home is actively cleaned up rather than left with stale artifacts.
# Set FMTG_ENABLE=1 to arm it anyway while working through the TODO above.
tg_mode_setup() {
  ...
  token=
  [ -f "$env_file" ] && token=$(fmx_env_get FMTG_BOT_TOKEN "$env_file")

  # Disabled, or opted out: drop any artifacts and stay silent unless we
  # actually removed something.
  if [ "${FMTG_ENABLE:-0}" != 1 ] || [ -z "$token" ]; then
    if x_mode_artifact_present "$shim" || x_mode_artifact_present "$cadence"; then
      if tg_mode_remove_artifacts; then
        if [ -n "$token" ]; then
          echo "FMTG: Telegram mode DISABLED (TODO: tg wakes are not classified) - removed poll shim and 30s cadence"
        else
          echo "FMTG: Telegram mode off - removed poll shim and 30s cadence"
        fi
      else
        echo "FMTG: Telegram mode off - failed to remove poll shim or 30s cadence"
      fi
    fi
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    tg_mode_remove_artifacts || true
    echo "FMTG: Telegram mode off - missing poll dependencies; install them and rerun bootstrap"
    return 0
  fi

  fmtg_arm_failed() {
    if tg_mode_remove_artifacts; then
      echo "FMTG: Telegram mode off - failed to arm poll shim or 30s cadence"
    else
      echo "FMTG: Telegram mode off - failed to arm poll shim or 30s cadence; stale artifacts remain"
    fi
  }
...
# Source this before the active harness protocol starts a watcher process so
# fm-watch.sh polls the Telegram check every 30s. Non-TG instances have no such
...
EOF
  ...
  echo "FMTG: Telegram mode on (FMTG_ENABLE=1 override) - poll armed via state/tg-watch.check.sh; 30s watcher cadence in config/tg-mode.env"
}
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cmd"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      ...
      elif $h == "opencode" or $h == "kimi" or $h == "cmd" then false
      else true
      end;
    def profiles($value):
