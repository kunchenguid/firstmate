...
#          Intake mode is OPTIONAL and inert unless config/intake.env exists.
#          When opted in, bootstrap requires jq and writes state/intake.check.sh.
...
#          PR-check artifacts, secondmate homes, X-mode/intake artifacts,
...
# shellcheck source=bin/fm-intake-lib.sh disable=SC1091
...
# Intake mode (opt-in): when config/intake.env exists, wire the intake poll into
# the authenticated watcher check mechanism through a generated byte-static
# wrapper in state/intake.check.sh. With no config file, stale wrappers are
# removed and otherwise the default behavior is unchanged.
intake_mode_setup() {
  local env_file shim shim_body missing
  env_file="$CONFIG/intake.env"
  shim="$STATE/intake.check.sh"

  intake_mode_remove_artifacts() {
    x_mode_remove_artifact "$shim"
  }

  if [ ! -f "$env_file" ]; then
    if x_mode_artifact_present "$shim"; then
      if intake_mode_remove_artifacts; then
        echo "INTAKE: intake mode off - removed intake poll shim"
      else
        echo "INTAKE: intake mode off - failed to remove intake poll shim"
      fi
    fi
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    if x_mode_artifact_present "$shim"; then
      if intake_mode_remove_artifacts; then
        echo "INTAKE: intake mode off - missing intake poll dependencies; install them and rerun bootstrap"
      else
        echo "INTAKE: intake mode off - failed to remove intake poll shim after missing intake poll dependencies"
      fi
    fi
    return 0
  fi
  ...
  intake_arm_failed() {
    if intake_mode_remove_artifacts; then
      echo "INTAKE: intake mode off - failed to arm intake poll shim"
    else
      echo "INTAKE: intake mode off - failed to arm intake poll shim; stale artifact remains"
    fi
  }

  mkdir -p "$STATE" 2>/dev/null || { intake_arm_failed; return 0; }

  shim_body=$(fm_intake_poll_shim_content "$FM_HOME" "$FM_ROOT")
  x_mode_write_if_changed "$shim" "$shim_body" 700 || { intake_arm_failed; return 0; }
  fm_intake_poll_shim_valid "$shim" "$FM_HOME" "$FM_ROOT" \
    || { intake_arm_failed; return 0; }

  echo "INTAKE: intake mode on - intake poll armed via state/intake.check.sh"
}
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    ...
    def effort_ok($h; $e):
      ...
      elif $h == "omp" then (["low","medium","high","xhigh"] | index($e))
