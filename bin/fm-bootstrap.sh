...
manual_install_url() {
  case "$1" in
    ...
    agy) echo "https://antigravity.google/download" ;;
    *) return 1 ;;
  esac
}
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cursor","muse","antigravity"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif $h == "claude" then (["low","medium","high","xhigh","max"] | index($e))
      elif $h == "codex" then (["low","medium","high","xhigh"] | index($e))
      elif $h == "grok" or $h == "antigravity" then (["low","medium","high"] | index($e))
...
detect_local_config() {
  ...
  if [ "$crew" = antigravity ] && ! command -v agy >/dev/null 2>&1; then
    echo "MISSING_MANUAL: agy (instructions: $(manual_install_url agy))"
  fi
  crew_dispatch_validate
