...
crew_dispatch_validate() {
  ...
  if [ "${FM_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ]; then
    jq -r '
      . as $policy
      | def resolve($value):
          if ($value | type) == "string" then $policy.profiles[$value] else $value end;
      def profile($value):
        ($value.harness | tostring)
        + (if ($value.model? != null) then "/" + ($value.model | tostring)
           elif ($value.effort? != null) then "/default"
           else "" end)
        + (if ($value.effort? != null) then "/" + ($value.effort | tostring) else "" end);
      def profile_set($value; $selector):
        if ($value | type) == "array" then
          (($selector // "quota-balanced") + "[" + ([$value[] | profile(resolve(.))] | join(", ")) + "]")
