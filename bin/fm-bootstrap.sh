...
#   config/trello-mode.env      - exports FM_CHECK_INTERVAL=30 for this home's watcher
...
trello_mode_setup() {
  ...
  if [ -z "$key" ] || [ -z "$token" ] || [ -z "$board" ]; then
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if trello_remove_artifacts; then
        echo "TRELLO: control plane off - removed board poll shim and 30s cadence; default cadence applies on the next supervision cycle; $(trello_supervision_repair)"
      else
        echo "TRELLO: control plane off - failed to remove board poll shim or 30s cadence"
      fi
    fi
    return 0
  fi
  ...
  if [ "$missing" -ne 0 ]; then
    if [ -e "$shim" ] || [ -e "$cadence" ]; then
      if trello_remove_artifacts; then
        ...
        echo "TRELLO: control plane off - failed to remove board poll shim or 30s cadence after missing dependencies"
      fi
    fi
    return 0
  fi
  ...
  trello_arm_failed() {
    if trello_remove_artifacts; then
      echo "TRELLO: control plane off - failed to arm board poll shim or 30s cadence"
    else
      echo "TRELLO: control plane off - failed to arm board poll shim or 30s cadence; stale artifacts remain"
    fi
  }
...
# Source this before the active harness protocol starts a watcher process so
# fm-watch.sh polls the Trello check every 30s. Non-Trello homes have no such
...
EOF
  ...
  echo "TRELLO: control plane on - board poll armed via state/trello-watch.check.sh; 30s watcher cadence in config/trello-mode.env"
}
