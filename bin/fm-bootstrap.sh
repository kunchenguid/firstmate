...
# Generated config/x-mode.env body. A function, not `VAR=$(cat <<EOF ... EOF)`:
# stock macOS Bash 3.2 scans a command substitution for its closing paren without
# skipping heredoc bodies, so one quote character in a generated body makes the
# whole script unparsable there. tests/fm-stock-bash-parse.test.sh enforces this.
x_mode_cadence_content() {
  cat <<'EOF'
...
x_mode_setup() {
  ...
  cadence_body=$(x_mode_cadence_content)
