...
discord_mode_setup() {
  local env_file token shim cadence shim_home shim_body cadence_body
  env_file="$FM_HOME/.env"
  shim="$STATE/discord-watch.check.sh"
  cadence="$CONFIG/discord-mode.env"

  token=
  [ -f "$env_file" ] && token=$(fmx_env_get FM_DISCORD_BOT_TOKEN "$env_file")

  discord_mode_artifact_present() {
    local file=$1
    [ -e "$file" ] || [ -L "$file" ]
  }

  discord_mode_remove_artifact() {
    local file=$1
  ...
  discord_mode_remove_artifacts() {
    local failed=0
    discord_mode_remove_artifact "$shim" || failed=1
    discord_mode_remove_artifact "$cadence" || failed=1
    [ "$failed" -eq 0 ]
  }
  ...
  if [ -z "$token" ]; then
    if discord_mode_artifact_present "$shim" || discord_mode_artifact_present "$cadence"; then
      if discord_mode_remove_artifacts; then
        ...
        echo "FM_DISCORD: self-hosted Discord mode off - failed to remove poll shim or 30s cadence"
      fi
    fi
    return 0
  fi
  ...
  case "$FM_HOME" in
    /*)
      shim_home=$(CDPATH='' cd -- "$FM_HOME" 2>/dev/null && pwd -P) \
        || { echo "FM_DISCORD: failed to resolve FM_HOME path"; return 0; }
      ;;
  esac
  shim_body=$(fm_discord_poll_shim_content "$shim_home" "$FM_ROOT")
  if ! printf '%s\n' "$shim_body" | fmx_private_artifact_publish_stdin "$STATE" "discord-watch.check.sh" 700 2>/dev/null; then
    echo "FM_DISCORD: self-hosted Discord mode inactive - failed to publish poll shim"
    return 0
  fi

  cadence_body=$(cat <<'EOF'
# Auto-generated by fm-bootstrap.sh - self-hosted Discord mode watcher cadence.
export FM_CHECK_INTERVAL=30
EOF
)
  if ! printf '%s\n' "$cadence_body" | fmx_private_artifact_publish_stdin "$CONFIG" "discord-mode.env" 600 2>/dev/null; then
    echo "FM_DISCORD: self-hosted Discord mode inactive - failed to publish cadence"
    return 0
  fi

  echo "FM_DISCORD: self-hosted Discord mode on - poll armed via state/discord-watch.check.sh; 30s watcher cadence in config/discord-mode.env"
}
