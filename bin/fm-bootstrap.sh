...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    ...
    def invalid_codex_homes:
      configured_profiles
      | map(select(.codexHome != null and (.harness != "pi" and .harness != "pi-signed")));
    def bad_efforts:
      configured_profiles
    ...
    elif (invalid_codex_homes | length) > 0 then "codexHome is supported only for pi and pi-signed profiles"
    else
      (configured_profiles
        | map(.harness)
        | map(select(. != null))
