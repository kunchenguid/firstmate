...
# Browser work runs through the BrowserClaw MCP server, which is configured as an
# MCP server rather than an npm axi CLI, so it is not a bootstrap-installed tool here.
COMMON_TOOLS="node git gh no-mistakes gh-axi lavish-axi tasks-axi quota-axi"
