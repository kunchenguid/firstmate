...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # A mutating sweep like the three above: live probes plus a state write, so it
  # stays behind the same fleet-lock authorization as its siblings.
  if network_phase && network_sweep_authorized 'model entitlement probe'; then
    __fm_timing_stamp=$(fm_timing_now_ms)
    model_probe_sweep
