...
# runtime backend. The local config/scm-host file, or FM_SCM_HOST for one
# launch, selects github (the backward-compatible default), forgejo, or
# local-only. GitHub alone adds gh, gh-axi, and the network auth probe.
fm_scm_host() {
  local host
  host=${FM_SCM_HOST:-}
...
detect_local_tools() {
  ...
  if [ "$SCM_HOST" = github ] && command -v gh-axi >/dev/null 2>&1 && ! tool_version_at_least gh-axi "$GH_AXI_MIN"; then
...
if network_phase && [ "$SCM_HOST" = github ]; then
  __fm_timing_stamp=$(fm_timing_now_ms)
