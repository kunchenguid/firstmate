...
#          fails, so a session without verified ownership never race-mutates
#          PR-check artifacts, secondmate homes, X-mode artifacts, project clones,
...
if [ -n "$tangle_branch" ]; then
  ...
  if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" = 1 ]; then
    echo "TANGLE: primary checkout on feature branch '$tangle_branch' (expected '$tangle_default'); the work is safe on that ref - this read-only session must not restore it without the fleet lock"
  else
