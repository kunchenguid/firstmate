...
crew_dispatch_validate() {
  ...
  if $typed_active; then
    ...
    verified_harnesses='["claude","mirasim","codex","opencode","pi","pi-signed","grok","kimi","cursor","agy","muse","rovo","omp"]'
  fi
  err=$(jq -r --argjson typed "$typed_active" --argjson verified_harnesses "$verified_harnesses" --arg provider_re "$FM_QUOTA_PROVIDER_ID_RE" '
    ...
    def effort_ok($h; $m; $e):
      ...
      elif $h == "claude" or $h == "mirasim" then (["low","medium","high","xhigh","max"] | index($e))
