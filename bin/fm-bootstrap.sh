...
discord_mode_setup() {
  ...
  printf '%s\n' "$shim_body" | fmx_private_artifact_publish_stdin "$STATE" "discord-watch.check.sh" 700 2>/dev/null || true
...
EOF
  ...
  printf '%s\n' "$cadence_body" | fmx_private_artifact_publish_stdin "$CONFIG" "discord-mode.env" 600 2>/dev/null || true
