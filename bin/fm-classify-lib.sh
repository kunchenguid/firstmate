...
# read: it reuses bin/fm-crew-state.sh, which reads authoritative Desktop or
# backend state and may read explicitly enabled inactive migration compatibility,
...
# installation; absent, it points at the real sibling script.
...
#   working - an exact Desktop working state or a busy pane; the crew is
#             legitimately mid-work on a static-looking endpoint;
...
# NOT a pure read: fm-crew-state.sh may query a backend, so callers run it only
