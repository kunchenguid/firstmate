...
# through its own tool; see its own comment for the callers' cost contract. The
# absorb classification (crew_absorb_class and its working/paused wrappers) is
# NOT a pure status-file read: it reuses bin/fm-crew-state.sh, which may make a
# bounded no-mistakes call, to decide whether a crew that just stopped its turn
# or went stale is working, deliberately paused, settled in a terminal state, or
# none of those. Callers run it ONLY on no-verb signal handling, first sighting
# of a stale hash, and immediately before escalating a possible wedge - never on
# every wake - so the per-wake triage stays cheap.
...
# side effect (state/.<task>.open-decisions-cursor), another of the library's
# documented exceptions to the pure-read rule alongside task_hold_kind and
# crew_absorb_class. The write
