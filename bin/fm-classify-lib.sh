...
# Both declarations can intentionally leave an endpoint idle whether or not the
# crew's agent is still live, so the declaration alone - never a backend agent
# liveness verdict - is what earns the watcher's bounded pause cadence. The
# guarantee that no live decision gate is silenced comes instead from
# crew_absorb_class's `actionable` token plus run-step precedence.
# pause_state_class (bin/fm-watch.sh) owns that reconciliation.
