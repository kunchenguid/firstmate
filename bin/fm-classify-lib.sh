...
merge_ready_current_pr_verdict() {
  local url=$1 query
  # shellcheck disable=SC2016
  query='[.headRefOid, ((.statusCheckRollup // []) as $checks | if (($checks | length) > 0 and all($checks[]; ((((.status // "") == "COMPLETED") and (((.conclusion // "") == "SUCCESS") or ((.conclusion // "") == "NEUTRAL") or ((.conclusion // "") == "SKIPPED"))) or ((.state // "") == "SUCCESS")))) then "green" else "not-green" end)] | @tsv'
  if command -v timeout >/dev/null 2>&1; then
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 timeout "$FM_MERGE_READY_HEAD_TIMEOUT" \
      gh pr view "$url" --json headRefOid,statusCheckRollup --jq "$query" 2>/dev/null
    ...
    GH_PROMPT_DISABLED=1 GH_NO_UPDATE_NOTIFIER=1 perl -e \
      ...
      "$FM_MERGE_READY_HEAD_TIMEOUT" gh pr view "$url" --json headRefOid,statusCheckRollup --jq "$query" 2>/dev/null
  fi
}
...
crew_is_merge_ready() {  # <id> <state-dir> <backend> <target>
  ...
  remote_pr=$(merge_ready_current_pr_verdict "$FM_PR_META_URL") || return 1
  remote_head=${remote_pr%%$'\t'*}
  [ "$remote_pr" = "$remote_head"$'\tgreen' ] || return 1
