...
# read: it reuses bin/fm-crew-state.sh, which may make a bounded pane-busy check,
...
# Overridable so tests can stub the pane verdict without a real worktree;
...
# authoritatively (not the status log) is what keeps busy-state precedence: a crew
# that appended paused: but then RESUMED work reports working, never paused.
# NOT a pure read: fm-crew-state.sh may make a bounded pane-busy check, so callers
