...
# bin/fm-crew-state.sh to decide whether a crew that just stopped its turn or
# went stale is working, deliberately paused, or neither. Callers run it ONLY on no-verb signal handling and first
...
# Overridable so tests can stub the pane/status verdict without a real worktree;
...
#   working - a busy pane; the crew is legitimately mid-work on a static-looking
#             pane (e.g. waiting on CI);
#   paused  - the crew's authoritative current state is a declared external-wait
#             pause (paused:), which is EXPECTED to idle;
#   none    - neither, so the wake must surface (a stopped/finished/parked/failed/
#             torn-down/unknown crew, or an unreadable verdict).
# One fm-crew-state.sh read serves BOTH absorb reasons at once.
# Reading the state authoritatively (not only the status log) means a busy crew
# that previously appended paused: reports working, never paused.
