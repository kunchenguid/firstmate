...
# 0 if a status line's leading verb is the pause verb or the legacy parked verb.
# A pure read of the line itself, so the daemon's classify_stale can reuse the last
# line it already read without a fm-crew-state.sh call.
status_is_paused() {  # <status-line>
  local line=$1 verb
  [ -n "$line" ] || return 1
  verb=$(status_line_verb "$line")
  case "$verb" in
    "${FM_CLASSIFY_PAUSED_VERB:-$FM_CLASSIFY_PAUSED_VERB_DEFAULT}"|parked) return 0 ;;
    *) return 1 ;;
  esac
}
...
# 0 when any listed status file's CURRENT last event is actionable. This is the
# compatibility predicate for a status log that has no committed classification
# cursor yet: an old terminal line is not enough to interrupt an actively
# running validation or a worktree that changed after that line was written.
# Once a cursor exists, fm-watch classifies the newly appended span instead, so a
# fresh completion cannot be hidden by later routine cleanup chatter.
