...
# read: it reuses bin/fm-crew-state.sh, which may make a bounded state call,
...
# or runtime install; absent, it points at the real sibling script.
...
# authoritatively (not the status log) is what keeps current-state precedence: a crew
# that appended paused: but then STARTED a run reports working, never paused.
# NOT a pure read: fm-crew-state.sh may make a bounded state call, so callers
