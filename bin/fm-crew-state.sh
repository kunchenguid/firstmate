...
#      blocker also outranks rule 2b's supersession and unknown answers. The
#      same holds for any open decision when the run record itself is
#      UNVERIFIED (its daemon answered down): the crew saw its gate or blocker
#      first hand, so needs-decision stays parked and blocked stays blocked,
#      with the unverified record named as the reason.
