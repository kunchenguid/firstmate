...
#      attributed either: its outcome reports on the run, not on the code or PR.
...
# isolated rendered-tail fallback, and traex reads its own the same way; a
# herdr crew's native `busy` is accepted
...
crew_busy_verdict() {  # <target>
  local tail40=''
  case "$HARNESS" in
    grok*|traex*) tail40=$(fm_backend_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40='' ;;
  esac
