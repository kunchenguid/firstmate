...
bb_read_record_bounded() {  # <workspace> <repo> <number>
  local record state merged
  # shellcheck disable=SC2016  # The inner script expands after bash -c receives positional args.
  if ! record=$(fm_run_timed 5 bash -c '
    . "$1"
    fm_pr_bitbucket_read_record "$2" "$3" "$4" || exit 1
    printf "state=%s\nmerged=%s\n" "$FM_PR_RECORD_STATE" "$FM_PR_RECORD_MERGED"
  ' _ "$SCRIPT_DIR/fm-pr-lib.sh" "$1" "$2" "$3" 2>/dev/null); then
    return 1
  fi
...
passed_pr_detail() {
  ...
  case "$provider" in
    ...
    bitbucket)
      owner=${path%%/*}
      repo=${path#*/}
      if ! bb_read_record_bounded "$owner" "$repo" "$number"; then
        printf 'run passed: PR state unknown (unreadable)'
        return
      fi
      if [ "$FM_PR_RECORD_MERGED" = true ]; then
      ...
      # Bitbucket Cloud spells an open pull request OPEN and a closed one
      # DECLINED (or, when another pull request has landed the same change,
      # SUPERSEDED); both fall through to the generic state line below.
      state_lc=$(printf '%s' "$FM_PR_RECORD_STATE" | tr '[:upper:]' '[:lower:]')
      case "$state_lc" in
        open)     printf 'run passed: PR open' ;;
