...
# git-common-dir, or a linked primary home carrying a genuine .fm-primary-home
# marker. A crewmate/scout task worktree - the shape bin/fm-spawn.sh always hands
# out - is an unmarked linked git worktree where the two differ. This guard does
# not inspect .fm-secondmate-home, so it applies in a git-cloned secondmate home
# but remains inert when the secondmate home is itself a treehouse-leased linked
# worktree. docs/cd-guard.md owns this scope; docs/turnend-guard.md owns the
# turn-end guard's separate marker-aware scope. Any failure to confirm the
...
if ! fm_primary_home_branch "$FM_ROOT" >/dev/null; then
