...
answer_callback_query() {
  ...
  response=$(curl --silent --show-error --connect-timeout 10 --max-time 20 \
    ...
    --data-urlencode "text=${text}" 2>&1) || {
      dm_log "could not acknowledge callback query ${callback_id}: ${response}"
      return 1
    }
  if [ "$(printf '%s' "$response" | jq -r '.ok // false' 2>/dev/null)" != true ]; then
    dm_log "Telegram rejected callback acknowledgement ${callback_id}: $(printf '%s' "$response" | jq -r '.description // "unreadable response"' 2>/dev/null)"
    return 1
  fi
}
...
show_decision_selection() {
  ...
  response=$(curl --silent --show-error --connect-timeout 10 --max-time 20 \
    ...
    --data-urlencode "reply_markup={\"inline_keyboard\":[]}" 2>&1) || {
      dm_log "could not show selected decision on message ${message_id}: ${response}"
      return 1
    }
  if [ "$(printf '%s' "$response" | jq -r '.ok // false' 2>/dev/null)" != true ]; then
    dm_log "Telegram rejected decision display edit for message ${message_id}: $(printf '%s' "$response" | jq -r '.description // "unreadable response"' 2>/dev/null)"
    return 1
  fi
}

persist_captain_callback() {
  local update_id=$1 callback=$2 callback_id sender_id data message chat_id chat_type message_id token choice_key intent choice label destination existing_rc
  ...
  [ -n "$callback_id" ] && [ -n "$data" ] && [ -n "$message" ] || {
    dm_log "ignored malformed callback update ${update_id}"
    return 2
  }
  chat_id=$(printf '%s' "$message" | jq -r '.chat.id | tostring') || return 1
  chat_type=$(printf '%s' "$message" | jq -r '.chat.type // ""') || return 1
  message_id=$(printf '%s' "$message" | jq -r '.message_id') || return 1
  if [ "$chat_type" != private ] || [ "$sender_id" != "$FM_TOPIC_CAPTAIN_ID" ] || [ "$chat_id" != "$FM_TOPIC_CAPTAIN_ID" ]; then
    dm_log "ignored callback ${update_id}: not a captain direct-message tap"
    return 2
  fi
  case "$message_id" in ''|*[!0-9]*) dm_log "ignored callback ${update_id} with malformed message id"; return 2 ;; esac
  IFS=: read -r token choice_key <<< "$data"
  [ "$(fm_topic_callback_data "$token" "$choice_key" 2>/dev/null || true)" = "$data" ] || {
    dm_log "ignored callback ${update_id} with an unknown callback token"
    return 2
  }
  intent=$(fm_topic_find_decision_intent "$token") || {
    dm_log "ignored callback ${update_id} for an unknown decision token"
    return 2
  }
  choice=$(fm_topic_decision_choice "$intent" "$choice_key") || {
    dm_log "ignored callback ${update_id} for an unknown decision choice"
    return 2
  }
  fm_topic_decision_already_recorded "$chat_id" "$message_id" "$token" && return 3
  jq -e --arg chat_id "$chat_id" --argjson message_id "$message_id" '
    .status == "sent"
    and .decision.status == "open"
    and .decision.source_chat_id == $chat_id
    and .decision.source_message_id == $message_id
    and .decision.source_thread_id == null
  ' "$intent" >/dev/null 2>&1 || {
    dm_log "ignored callback ${update_id} for a closed or mismatched decision"
    return 2
  }
  ...
  FM_DM_CALLBACK_INTENT=$intent
  FM_DM_CALLBACK_CHOICE_KEY=$choice_key
  FM_DM_CALLBACK_LABEL=$label
  FM_DM_CALLBACK_CHAT_ID=$chat_id
  FM_DM_CALLBACK_MESSAGE_ID=$message_id
}
...
poll_once() {
  ...
  while IFS= read -r update; do
    ...
    if [ -n "$callback" ]; then
      persist_captain_callback "$update_id" "$callback"
      persist_rc=$?
      callback_id=$(printf '%s' "$callback" | jq -r '.id // empty') || return 1
      case "$persist_rc" in
        0)
          answer_callback_query "$callback_id" 'Decision recorded' || true
          mark_decision_selected "$FM_DM_CALLBACK_INTENT" "$FM_DM_CALLBACK_CHOICE_KEY" "$FM_DM_CALLBACK_LABEL" || return 1
          show_decision_selection "$FM_DM_CALLBACK_INTENT" "$FM_DM_CALLBACK_CHAT_ID" "$FM_DM_CALLBACK_MESSAGE_ID" "$FM_DM_CALLBACK_LABEL" || true
          new=$((new + 1))
          ;;
        ...
        *) dm_log "failed to persist callback update ${update_id}; offset remains ${offset}"; return 1 ;;
      esac
      continue
    fi
