...
# (state/x-watch.check.sh), or a Trello control-plane poll
# (state/trello-watch.check.sh), and whether its watcher has a fresh liveness beacon
...
#                         Trello control-plane poll, or a registered event
#                         source (a source is a wait on an external process,
#                         not a task, so it has no metadata)
...
# Exit 0 (true) exactly when the home needs a watcher (in-flight work, an
# X-mode relay poll, a Trello control-plane poll, or a registered event
# source). Exit 1 (false) for an idle home.
