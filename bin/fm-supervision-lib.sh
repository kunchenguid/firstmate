...
# (state/x-watch.check.sh), or an armed inbound message channel
# (state/wa-watch.check.sh), and whether its watcher has a fresh liveness beacon
...
#                         armed inbound message channel, or a registered event
#                         source (a source is a wait on an external process, not
#                         a task, so it has no metadata). An inbound channel is
#                         the same case as a relay poll: the captain reaches a
#                         home precisely when nothing else is running, so an idle
#                         home with an armed channel still needs a watcher.
