...
# Stop auto-arm and Codex foreground-checkpoint models, where the watcher is
# deliberately absent while the model handles a turn, a fresh beacon with no live
# watcher is healthy; under persistent-watcher harnesses a live identity-matched
# watcher is still required. The status fields here retain the beacon-age details
# used in their messages.
