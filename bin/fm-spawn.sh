...
W="fm-$ID"
T="$SES:$W"
# Create the task window through the tmux backend. The backend captures a stable
# window id and pins the window name (disabling automatic-rename/allow-rename) so a
# captain's non-default tmux config cannot rename it away from fm-<id> once treehouse
# cd's into the worktree, which would break firstmate's name-based targeting ($SES:$W).
# The persisted window= handle below stays the name form; the stable id is used only
# for the rename-critical spawn steps that follow.
WID=$(fm_backend_tmux_create_task "$SES" "$W" "$PROJ_ABS") || exit 1
if [ "$KIND" != secondmate ]; then
  fm_backend_tmux_send_text_line "$WID" 'treehouse get'
  ...
  for _ in $(seq 1 60); do
    p=$(fm_backend_tmux_current_path "$WID" || true)
