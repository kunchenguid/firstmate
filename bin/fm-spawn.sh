...
#   --reuse-worktree <id> is the context-handoff agent swap (AGENTS.md section 8 /
#   context-handoff skill): it takes ONLY <id> (project/harness/model/effort/backend/
#   kind/mode/yolo come from state/<id>.meta), retires the old endpoint, skips
#   treehouse get, relaunches a FRESH agent on the SAME recorded worktree/branch
#   seeded with the original brief + data/<id>/handoff.md, and resets
#   state/<id>.context + state/<id>.turn-ended. It aborts on missing meta, a vanished
#   worktree, or a missing/empty handoff dump; it is refused with --secondmate and on
#   the orca backend (Orca owns the worktree lifecycle).
...
# does NOT kill the old pane in a separate step (it retires the old endpoint itself,
# below), and does NOT destroy the worktree (that is teardown). Missing meta, a
# vanished worktree, or a missing handoff dump abort rather than launching blind.
# project/harness/model/effort/backend are recovered from meta unless the caller
# overrode them on this spawn. Resolved BEFORE backend/harness selection so the
# recovered values feed those code paths unchanged.
REUSE_WT=
REUSE_PROJECT=
REUSE_MODE=
REUSE_YOLO=
...
# not retire the old pane separately. The 4-arg fm_backend_kill form matches
# fm-teardown.sh's own per-backend kill call (target, zellij tab id, expected label).
...
case "$BACKEND" in
  tmux)
    ...
    WID=$(fm_backend_tmux_create_task "$SES" "$W" "$PANE_DIR") || exit 1
    WT_TARGET="$WID"
    ;;
  herdr)
