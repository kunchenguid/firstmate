...
# Fresh task metadata also records launch_epoch= in Unix seconds for usage
# collection; relaunch preserves an existing value without backfilling legacy tasks.
...
}
{
  ...
  [ "$RELAUNCH" -eq 1 ] || printf 'launch_epoch=%s\n' "$(date +%s)"
