...
raw_launch_preflight() {
  ...
  case "$base" in
    claude|env|command|exec|builtin|eval|source|.|coproc|noglob|nocorrect|not|-|nohup|nice|ionice|chrt|setsid|stdbuf|timeout|time|sudo|doas|script|watch|strace|perf|xargs|find|busybox|toybox)
      raw_launch_refuse
      return 1
      ;;
  esac
...
launch_template() {
  ...
  case "$harness" in
    ...
    # crewmate needs. This is Grok-specific and is unchanged by Claude's classified
    # auto permission mode. grok's turn-end signal does NOT ride the
