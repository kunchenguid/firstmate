...
}
{
  ...
  # Preserved lines stay last and in their recorded order: an armed merge poll
  # authenticates only while pr= and pr_head= remain the record's tail
  # (bin/fm-pr-lib.sh fm_pr_metadata_identity_parse), so no spawn-owned line
  # may follow them.
  if [ "$RELAUNCH" -eq 1 ]; then
    preserve_relaunch_meta
  fi
