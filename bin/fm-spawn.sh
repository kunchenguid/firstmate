...
# repeat watch because a pipeline changes state several times per run, and
# --edge (inside bin/fm-nm-watch.sh's own arm call) because
# bin/fm-nm-state-condition.sh already de-dups its own transitions (it
# rewrites its snapshot on every poll, true or false), so the generic repeat
# dedup would risk discarding a real change observed right after a restart
# between fires. It carries FM_HOME because fm-send refuses to resolve a
# target without one. bin/fm-nm-watch.sh owns the watch shape and re-arms
# idempotently, so a relaunch converges on the watch its original spawn
# armed; a failure there queues a durable check wake and never fails the spawn.
if [ "$KIND" = ship ] && [ "$MODE" = no-mistakes ]; then
