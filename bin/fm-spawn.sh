...
#     __CODEX_HOME__ absolute path to the firstmate-managed Codex home used for
#                  codex crewmates/scouts, with no MCP servers and auth inherited
#                  from the user's real ~/.codex/auth.json
...
# omp uses a state-owned extension under state/<id>.omp-ext.ts and passes it with
# --hook for crewmate/scout launches; secondmate launches omit the hook.
...
codex_preserved_config_lines() {  # <source-config>
  local src=$1
  [ -f "$src" ] || return 0
  awk '
    BEGIN { top = 1 }
    /^[[:space:]]*\[/ { top = 0 }
    top != 0 && /^[[:space:]]*(model|model_reasoning_effort)[[:space:]]*=/ { print }
  ' "$src"
}

prepare_codex_home() {  # <state-real>
  local state_real=$1 source_home source_config source_auth codex_home tmp auth_link preserved
  source_home=${FM_CODEX_SOURCE_HOME:-$HOME/.codex}
  if [ -d "$source_home" ]; then
    source_home=$(cd "$source_home" && pwd -P)
  fi
  source_config="$source_home/config.toml"
  source_auth="$source_home/auth.json"
  codex_home="$state_real/codex-home"
  mkdir -p "$codex_home"
  chmod 0700 "$codex_home" 2>/dev/null || true

  tmp=$(mktemp "$codex_home/config.toml.XXXXXX") || return 1
  {
    printf '# Firstmate-managed Codex home for crewmates and scouts.\n'
    printf '# Regenerated by bin/fm-spawn.sh; do not add MCP servers here.\n'
    preserved=$(codex_preserved_config_lines "$source_config")
    if [ -n "$preserved" ]; then
      printf '%s\n' "$preserved"
    fi
    printf '[mcp_servers]\n'
  ...
  mv -f "$tmp" "$codex_home/config.toml"

  auth_link="$codex_home/auth.json"
  if [ -e "$auth_link" ] && [ ! -L "$auth_link" ]; then
    echo "error: managed Codex auth path is not a symlink: $auth_link" >&2
    return 1
  fi
  if [ "$(readlink "$auth_link" 2>/dev/null || true)" != "$source_auth" ]; then
    rm -f "$auth_link"
    ln -s "$source_auth" "$auth_link"
  fi
  printf '%s\n' "$codex_home"
}
...
case "$LAUNCH" in
  *__CODEX_HOME__*)
    CODEX_HOME_FOR_LAUNCH=$(prepare_codex_home "$STATE_REAL") || exit 1
    sq_codex_home=$(shell_quote "$CODEX_HOME_FOR_LAUNCH")
    ;;
esac
...
LAUNCH=${LAUNCH//__CODEX_HOME__/$sq_codex_home}
