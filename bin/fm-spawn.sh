...
spawn_worktree_isolated() {  # <path>
  ...
  # genuine isolated worktree reached under a variant spelling (".../WT1" for
  # ".../wt1") compares unequal as text and was refused with a misleading
  # "not a worktree root". The same identity compare keeps the primary checkout
  # refused under any spelling, and names the real reason rather than that
  # accidental one.
  #
  # `-ef` is also false when either side cannot be stat'd, so each side is
  # confirmed an existing directory first: an unresolvable side must read as
  ...
  if [ "$wt_real" -ef "$PROJ_ABS_REAL" ]; then
    SPAWN_WT_REASON="it is the spawning project itself, under another name"
    return 1
  fi
