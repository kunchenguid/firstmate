...
#   tab. Herdr uses the sanitized value as the task tab name, including in an
#   optional presentation space; other backends retain their stable fm-<id>
#   runtime names until their addressing and recovery surfaces can safely
#   separate identity from display. A respawn with no flag reuses label= from
#   meta. Batch dispatch refuses --label because a display label is per-task.
...
#   (--label is refused in batch mode because a display label names one task, not a set).
