...
}
{
  ...
  # control_relaunch_tx must land before preserve_relaunch_meta's output: that
  # step reproduces every unowned line from the prior meta verbatim, including
  # a pr=/pr_head= block fm-pr-check.sh always keeps last, and
  # fm_pr_metadata_identity_parse (bin/fm-pr-lib.sh) rejects any non-allowlisted
  # line after pr=. Emitting this line after preservation silently disabled the
  # merge poll on every relaunch of a PR-armed task.
