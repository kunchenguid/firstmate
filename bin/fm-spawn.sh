...
#   environment ($TMUX, HERDR_ENV=1, via bin/fm-backend.sh's fm_backend_detect),
#   then tmux.
#   Spawn-capable backends are the reference tmux adapter and experimental
#   herdr. An auto-detected herdr spawn prints a loud stderr notice;
...
# items) and in a home that keeps no
...
spawn_remote_secondmate() {
  ...
  local sm_primary_head sync_out sync_rc
  local -a launch_args
  id=${POS[0]:-}
...
preserve_relaunch_meta() {
  awk -F= '
    BEGIN {
      split("window endpoint_task_id worktree project harness kind mode yolo tasktmp model effort busy_gen spawn_gen backend herdr_session herdr_workspace_id herdr_tab_id herdr_pane_id home projects control_relaunch_tx", keys, " ")
...
if [ "$KIND" = secondmate ]; then
  ...
  LAUNCH="FM_ROOT_OVERRIDE= FM_STATE_OVERRIDE= FM_DATA_OVERRIDE= FM_PROJECTS_OVERRIDE= FM_CONFIG_OVERRIDE= FM_PUBLIC_FOLLOWUP_PRIMARY_HOME=$sq_primary_home FM_HOME=$sq_home FM_SUPERVISION_MODEL=$supervision_model $LAUNCH"
fi
