...
# Usage: fm-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--label <text>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
#        fm-spawn.sh <task-id> <project-dir> --scout [--label <text>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
...
#   Whitespace and control runs collapse to one space, leading and trailing
#   whitespace is removed, and the result is capped at 64 characters.
#   A label may not begin with fm-, which is reserved for stable fm-<id> task
#   identity, and a Herdr spawn refuses a label already recorded by another task
#   in the same home.
#   Herdr uses the sanitized value as the task tab name; other backends retain
#   their stable fm-<id> runtime names.
#   A relaunch with no flag reuses label= from meta, and batch dispatch refuses
#   --label because a display label is per-task rather than shared.
...
#   applies to every pair, while --label is refused. A ship batch therefore carries one delivery contract, and each
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ "$LABEL_SET" -eq 0 ] || {
    echo "error: --relaunch reuses the task's recorded display label; --label cannot rename an existing endpoint" >&2
    exit 1
  }
else
...
spawn_remote_secondmate() {
  ...
  local remote_traceparent remote_recorded_traceparent remote_recorded_label sm_primary_head sync_out sync_rc
  local -a launch_args
  id=${POS[0]:-}
  ...
  *)
  ...
  launch_args=("$id" "$harness" "$model" "$effort" "$backend" "${LABEL:--}")
  ...
  remote_recorded_label=$(printf '%s\n' "$out" | sed -n 's/^label=//p' | tail -1)
  tmp="$meta.tmp.$$"
  {
    ...
    echo "model=${model#-}"
    echo "effort=${effort#-}"
    [ -z "$remote_recorded_label" ] || echo "label=$remote_recorded_label"
    echo "home=$home"
...
if [ "$LABEL_SET" -eq 1 ]; then
  ...
  [ -n "$LABEL" ] || {
    echo "error: --label must contain printable text" >&2
    exit 1
  }
  case "$LABEL" in
  fm-*)
    echo "error: --label must not begin with fm- (reserved for stable fm-<id> task identity)" >&2
    exit 1
    ;;
  esac
...
preserve_relaunch_meta() {
  awk -F= '
    BEGIN {
      split("window endpoint_task_id worktree project harness kind mode yolo tasktmp model effort label busy_gen spawn_gen traceparent backend herdr_session herdr_workspace_id herdr_tab_id herdr_pane_id zellij_session zellij_tab_id zellij_pane_id orca_worktree_id terminal cmux_workspace_id cmux_surface_id home projects control_relaunch_tx", keys, " ")
