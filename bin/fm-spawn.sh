...
#   per task at intake (primary-runtime delivery contract); data/projects.md holds the captain's
...
#   A fresh spawn first takes the per-home task-set lock and refuses rather than
#   waits when forced cleanup owns it; relaunch is exempt because the existing
...
#   see primary-runtime delivery lifecycle); --secondmate records kind=secondmate and launches in a
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # Delivery contract (`primary-runtime`). A ship task's mode and yolo are
...
UNPUBLISHED_RESOURCES_ACTIVE=0
UNPUBLISHED_TASK_TMP_CREATED=0
UNPUBLISHED_BUSY_GEN=
UNPUBLISHED_WIRING_PATHS=
UNPUBLISHED_WIRING_HARNESS=
UNPUBLISHED_TURNEND_AUTH_PATH=
UNPUBLISHED_TURNEND_AUTH_DIR=

remove_unpublished_owned_file() {  # <path> <description>
  local path=$1 description=$2
  [ -e "$path" ] || [ -L "$path" ] || return 0
  if [ -L "$path" ] || [ ! -f "$path" ]; then
    echo "warning: preserving $description for $ID because its exact path is no longer an owned regular file: $path" >&2
    return 1
  fi
  if ! rm -f -- "$path"; then
    echo "warning: preserving $description for $ID because it could not be removed: $path" >&2
    return 1
  fi
  return 0
}

cleanup_unpublished_resources() {
  local cleanup_status=0 path
  [ "$UNPUBLISHED_RESOURCES_ACTIVE" = 1 ] || return 0
  # Clear first so the trap and an explicit abort can both call this function
  # without deleting a later incarnation's resources on the second call.
  UNPUBLISHED_RESOURCES_ACTIVE=0

  if [ -n "$UNPUBLISHED_BUSY_GEN" ]; then
    if ! "$FM_ROOT/bin/fm-busy-event.sh" retire \
        "$STATE_REAL" "$ID" --gen "$UNPUBLISHED_BUSY_GEN"; then
      echo "warning: preserving the busy-state record for $ID because generation $UNPUBLISHED_BUSY_GEN could not be retired exactly" >&2
      cleanup_status=1
    fi
  fi
  ...
  while IFS= read -r path; do
    [ -n "$path" ] || continue
    remove_unpublished_owned_file "$path" "unpublished $HARNESS wiring" \
      || cleanup_status=1
  done <<EOF
$UNPUBLISHED_WIRING_PATHS
EOF

  if [ -n "$UNPUBLISHED_TURNEND_AUTH_PATH" ]; then
    case "$UNPUBLISHED_TURNEND_AUTH_PATH" in
      "$UNPUBLISHED_TURNEND_AUTH_DIR"/fm.*)
        remove_unpublished_owned_file \
          "$UNPUBLISHED_TURNEND_AUTH_PATH" \
          "unpublished $HARNESS turn-end registry entry" \
          || cleanup_status=1
        ;;
      *)
        echo "warning: preserving the unpublished $HARNESS turn-end registry entry for $ID because its ownership path is not exact: $UNPUBLISHED_TURNEND_AUTH_PATH" >&2
        cleanup_status=1
        ;;
    esac
  fi

  if [ "$UNPUBLISHED_TASK_TMP_CREATED" = 1 ]; then
    case "${TASK_TMP:-}" in
      "/tmp/fm-$ID")
        if [ -L "$TASK_TMP" ] || { [ -e "$TASK_TMP" ] && [ ! -d "$TASK_TMP" ]; }; then
          echo "warning: preserving the task temporary root for $ID because its exact path is no longer the directory this spawn created: $TASK_TMP" >&2
          cleanup_status=1
        elif [ -d "$TASK_TMP" ] && ! rm -rf -- "$TASK_TMP"; then
          echo "warning: preserving the task temporary root for $ID because it could not be removed: $TASK_TMP" >&2
          cleanup_status=1
        fi
        ;;
      *)
        echo "warning: preserving the task temporary root for $ID because its path is not the exact /tmp/fm-<id> root: ${TASK_TMP:-<unset>}" >&2
        cleanup_status=1
        ;;
    esac
  elif [ -n "${TASK_TMP:-}" ] && { [ -e "$TASK_TMP" ] || [ -L "$TASK_TMP" ]; }; then
    echo "warning: preserving the pre-existing task temporary root for $ID because this spawn did not create it: $TASK_TMP" >&2
    cleanup_status=1
  fi
...
spawn_abort_cleanup() {
  ...
  meta_publish_cleanup
  if [ "$UNPUBLISHED_RESOURCES_ACTIVE" = 1 ]; then
    cleanup_unpublished_resources || true
...
spawn_herdr_presentation_order_lock_acquire() {
  ...
  max_attempts=${FM_HERDR_PRESENTATION_LOCK_ATTEMPTS:-50}
...
if [ ! -e "$TASK_TMP" ] && [ ! -L "$TASK_TMP" ]; then
  ...
  UNPUBLISHED_TASK_TMP_CREATED=1
else
  mkdir -p "$TASK_TMP/gotmp"
fi
UNPUBLISHED_RESOURCES_ACTIVE=1
...
EOF
    ...
    grok*)
      ...
      umask "$old_umask"
      UNPUBLISHED_TURNEND_AUTH_PATH=$auth_file
      UNPUBLISHED_TURNEND_AUTH_DIR=$GROK_AUTH_DIR
      printf '%s\n' "$TURNEND" > "$auth_file"
...
EOF
    ...
    kimi*)
      ...
      umask "$old_umask"
      UNPUBLISHED_TURNEND_AUTH_PATH=$auth_file
      UNPUBLISHED_TURNEND_AUTH_DIR=$KIMI_AUTH_DIR
      printf '%s\n' "$TURNEND" > "$auth_file"
  ...
  UNPUBLISHED_WIRING_HARNESS=$(fm_control_harness_family "$HARNESS") \
    || UNPUBLISHED_WIRING_HARNESS=
  UNPUBLISHED_WIRING_PATHS=$(fm_control_harness_wiring_paths \
    "$UNPUBLISHED_WIRING_HARNESS" "$WT" "$STATE_REAL" "$ID") || {
    echo "error: could not record the exact unpublished $HARNESS wiring paths for $ID" >&2
    exit 1
  }
fi
