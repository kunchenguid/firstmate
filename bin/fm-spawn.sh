...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    grok)
      ...
      # axis is the reasoning knob. As of grok 1.0.3 (2026-08-15),
      # --reasoning-effort accepts low|medium|high|xhigh and rejects max, so
      # omit max rather than passing a known-bad value. grok 1.0.x is the
      # supported floor; 0.2.x still rejected xhigh.
      case "$effort" in
