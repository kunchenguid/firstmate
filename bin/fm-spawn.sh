...
if [ "${#POS[@]}" -gt 0 ] && [ "${POS[0]}" != "$idpart" ] && case "$idpart" in */*) false ;; *) true ;; esac; then
  rc=0
  _batch_model_args=()
  [ -n "${SPAWN_MODEL:-}" ] && _batch_model_args=("--model=$SPAWN_MODEL")
  for pair in "${POS[@]}"; do
    case "$pair" in
      *=*) : ;;
    ...
    if [ "$KIND" = secondmate ]; then
      ...
      if FM_SPAWN_NO_GUARD=1 "$FM_ROOT/bin/fm-spawn.sh" "${pair%%=*}" "${pair#*=}" --scout "${_batch_model_args[@]+"${_batch_model_args[@]}"}"; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    else
      if FM_SPAWN_NO_GUARD=1 "$FM_ROOT/bin/fm-spawn.sh" "${pair%%=*}" "${pair#*=}" "${_batch_model_args[@]+"${_batch_model_args[@]}"}"; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    fi
  done
  exit "$rc"
fi
...
echo "spawned $ID harness=$HARNESS kind=$KIND mode=$MODE yolo=$YOLO model=${SPAWN_MODEL:-default} window=$T worktree=$WT"
