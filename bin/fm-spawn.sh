...
[ "$INTAKE_RULE_SET" -eq 0 ] || [ -n "$INTAKE_RULE_ARG" ] || {
  echo "error: --intake-rule requires a non-empty value" >&2
  exit 1
}
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ "$INTAKE_RULE_SET" -eq 0 ] || {
    echo "error: --relaunch reuses the task's frozen intake identity; --intake-rule cannot override it" >&2
    exit 1
  }
else
