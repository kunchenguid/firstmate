...
#   Before a fresh remote-backed ship or scout worker starts, its clean task
#   worktree fetches origin, resolves the current remote default branch, and
#   resets to its tip. A local-only worker without origin instead uses the clean
#   local main or master base already supplied by Treehouse. An unreachable or
#   unresolved required base, or a non-clean worktree, refuses the spawn rather
#   than risking work from stale history or discarding local changes.
...
freshen_spawn_worktree_base() {  # <worktree>
  ...
  if git -C "$worktree" remote | grep -Fxq origin; then
