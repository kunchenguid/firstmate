...
# Usage: fm-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--tiered <on|off>] [--ci-tests <on|off>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
...
#   --tiered <on|off> and --ci-tests <on|off> (both default off) apply only to
#   no-mistakes ship spawns and are refused otherwise. Firstmate resolves them the
#   same way as --mode/--yolo, from the project's registered +tiered/+ci-tests
#   posture (AGENTS.md section 6); a no-mistakes ship task always records both
#   (off when not passed), a secondmate records both off, and a scout omits them.
...
#   --tiered/--ci-tests applies to every pair. A ship batch therefore carries one delivery contract, and each
...
# On success prints: spawned <id> harness=<name> kind=<ship|scout|secondmate> [mode=<mode> yolo=<on|off> tiering=<on|off> ci_tests=<on|off>] window=<backend-target> worktree=<path>
# A ship task records the explicit mode/yolo it was passed; a no-mistakes ship task
# additionally records the explicit --tiered/--ci-tests it was passed (off/off when
# omitted); a secondmate spawn records mode=secondmate, yolo=off, tiering=off,
# ci_tests=off, home=, and projects=; a scout records neither, and both the
...
for a in "$@"; do
  if [ -n "$want_value" ]; then
    ...
    case "$want_value" in
      ...
      tiered) TIERED=$a; TIERED_SET=1 ;;
      ci-tests) CITESTS=$a; CITESTS_SET=1 ;;
  ...
  case "$a" in
    ...
    --tiered) want_value=tiered ;;
    --tiered=*) TIERED=${a#--tiered=}; TIERED_SET=1 ;;
    --ci-tests) want_value=ci-tests ;;
    --ci-tests=*) CITESTS=${a#--ci-tests=}; CITESTS_SET=1 ;;
...
if [ "$KIND" = ship ]; then
  ...
  if [ "$TIERED_SET" -eq 1 ] || [ "$CITESTS_SET" -eq 1 ]; then
    [ "$MODE" = no-mistakes ] || {
      echo "error: --tiered/--ci-tests apply only to no-mistakes ship spawns; a diff can only run a trimmed gate against the full no-mistakes pipeline" >&2
      exit 1
    }
  fi
  case "$TIERED" in on|off) ;; *) echo "error: --tiered must be on or off (got '$TIERED')" >&2; exit 1 ;; esac
  case "$CITESTS" in on|off) ;; *) echo "error: --ci-tests must be on or off (got '$CITESTS')" >&2; exit 1 ;; esac
else
  [ "$MODE_SET" -eq 0 ] || {
  ...
  [ "$TIERED_SET" -eq 0 ] || {
    echo "error: --tiered applies only to no-mistakes ship spawns; a scout delivers a report and a secondmate records its own fixed posture" >&2
    exit 1
  }
  [ "$CITESTS_SET" -eq 0 ] || {
    echo "error: --ci-tests applies only to no-mistakes ship spawns; a scout delivers a report and a secondmate records its own fixed posture" >&2
    exit 1
  }
fi
...
if [ "${#POS[@]}" -gt 0 ] && [ "${POS[0]}" != "$idpart" ] && case "$idpart" in */*) false ;; *) true ;; esac; then
  ...
  [ "$TIERED_SET" -eq 0 ] || shared_args+=(--tiered "$TIERED")
  [ "$CITESTS_SET" -eq 0 ] || shared_args+=(--ci-tests "$CITESTS")
  for pair in "${POS[@]}"; do
    case "$pair" in
      *=*) : ;;
...
if [ "$KIND" = ship ]; then
  # basename the logical projects/<name> arg, not the pwd -P-resolved PROJ_ABS: a
  # symlinked projects/<name> can physically resolve to a differently-named
  # directory, and the registry keys on the projects/ entry name.
...
# tiered/ci-tests follow the same shape: fixed off for a secondmate, omitted
# for a scout (meaningless outside a no-mistakes ship task), explicit for ship.
if [ "$KIND" = secondmate ]; then
  MODE=secondmate
  YOLO=off
  TIERED=off
...
[ "$BACKEND" = orca ] && META_WINDOW=$W
{
  ...
  [ -z "$MODE" ] || echo "mode=$MODE"
  [ -z "$YOLO" ] || echo "yolo=$YOLO"
  [ -z "$TIERED" ] || echo "tiering=$TIERED"
  [ -z "$CITESTS" ] || echo "ci_tests=$CITESTS"
  echo "tasktmp=$TASK_TMP"
  echo "model=${MODEL:-default}"
  echo "effort=${EFFORT:-default}"
