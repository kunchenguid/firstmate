...
def context(store, request):
    ...
    return {"schema": "fm-whatsapp-context.v1", "request": request,
            ...
            "history": store.rows("SELECT request,body,state FROM inbound WHERE sender=? AND kind='text' AND state NOT IN ('historical','quarantined','invalid') ORDER BY received DESC,rowid DESC LIMIT 10",
