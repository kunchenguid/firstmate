...
family_for_basename() {
  case "$1" in
    ...
    fm-herdr-endpoint-bind-e2e.test.sh|\
    ...
    fm-herdr-endpoint-bind-locking.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/backends/herdr*|bin/fm-herdr-lab.sh|tests/herdr-test-safety.sh)
      ...
      printf '%s\n' pure-contract-unit
      ;;
    bin/fm-herdr-endpoint-bind.sh|bin/fm-herdr-session-cleanup.sh)
      printf '%s\n' session-bootstrap
      printf '%s\n' real-herdr-gated
      printf '%s\n' backend-dispatch
      ;;
    ...
    bin/fm-bootstrap.sh)
      printf '%s\n' session-bootstrap
      # Bootstrap also owns the X-mode transition messages asserted by the
      # pr-forge X-mode suite.
      printf '%s\n' pr-forge
      ;;
