...
family_for_basename() {
  case "$1" in
    ...
    fm-repo-compatibility-pointers-check.test.sh|\
    fm-kimi-harness.test.sh|fm-muse-harness.test.sh|fm-herdr-lab.test.sh|fm-lint.test.sh|\
    fm-install-herdr.test.sh|fm-lint-workflows.test.sh|\
    fm-operational-input.test.sh|fm-pi-primary-types.test.sh|\
    fm-project-pool.test.sh|fm-quota-alert.test.sh|fm-relay.test.sh|\
    fm-send-popup-settle.test.sh|fm-send-settle.test.sh|\
    fm-subagent-pretool-check.test.sh|\
    fm-supervision-instructions.test.sh|fm-task-delivery.test.sh|\
    fm-tmux-submit-busy.test.sh|fm-trace-context-lib.test.sh|\
    fm-transition-lib.test.sh|fm-treehouse-lib.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh|fm-tmux-fleet-isolation.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    ...
    fm-teardown.test.sh|fm-x-mode.test.sh)
      printf '%s\n' pr-forge
      ;;
    fm-afk-inject-e2e.test.sh|fm-afk-return.test.sh|fm-afk-crosshome-isolation.test.sh)
      printf '%s\n' afk
      ;;
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-lock.sh)
      ...
      printf '%s\n' watcher-wake-lock
      ;;
    bin/fm-session-start.sh|bin/fm-bootstrap.sh|bin/fm-nm-lib.sh|bin/fm-fleet-sync.sh|\
