...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-pi-primary-types.test.sh|fm-shard.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-decision-hold.sh|bin/fm-shard.sh|bin/fm-supervision*|bin/fm-transition-lib.sh|\
