...
#                   (repeatable; applied after selection)
...
# proven-isolated. Watcher, lock, AFK, real tmux, daemon, secondmate lifecycle,
# bootstrap, live-harness opt-in, and other unproven work stays here.
# Derived rather than enumerated so a newly added test lands here by default
# instead of falling out of every lane.
...
run_coverage_guard() {
  ...
  # Serial (whole lane and each CI shard) without disturbing a caller's
  # selection.
  ...
  printf 'FM_TEST_COVERAGE ok total=%s parallel=%s serial=%s serial_shards=%s\n' \
    "$(wc -l <"$tmp/all" | tr -d ' ')" \
