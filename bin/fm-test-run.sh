...
family_for_basename() {
  case "$1" in
    ...
    fm-continuation-check.test.sh|fm-session-lock-ancestry.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-continuation-check.sh|bin/fm-decision-hold.sh|bin/fm-supervision*|bin/fm-transition-lib.sh|\
