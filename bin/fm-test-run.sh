...
family_for_basename() {
  case "$1" in
    ...
    fm-tangle-guard.test.sh|fm-update.test.sh)
      printf '%s\n' session-bootstrap
      ;;
    fm-afk-pi-herdr-return-e2e.test.sh|fm-calm-pi-herdr-live-e2e.test.sh|\
    fm-calm-pi-real-model-live-e2e.test.sh|fm-bearings-board-lavish-live-e2e.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-brief.sh|bin/fm-ensure-agents-md.sh|bin/fm-crew-state.sh|bin/fm-next.sh|\
