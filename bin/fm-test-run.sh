...
family_for_basename() {
  case "$1" in
    ...
    fm-ci-load-guard.test.sh|fm-ci-runner-install.test.sh|fm-ci-workflow.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-nm-run-lib.sh)
      ...
      printf '%s\n' pr-forge
      ;;
    bin/fm-spawn.sh|bin/fm-send.sh|bin/fm-harness.sh|bin/fm-launch-axis-lib.sh|\
