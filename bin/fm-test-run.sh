...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-check-unregister.test.sh|fm-github-write.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      printf '%s\n' "__script__:fm-procevent-quota.test.sh"
      ;;
    bin/fm-github-write*.sh|bin/fm-pr-*|bin/fm-merge-local.sh|bin/fm-teardown.sh|bin/fm-review-diff.sh|\
