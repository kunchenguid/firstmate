...
family_for_basename() {
  case "$1" in
    ...
    fm-pi-primary-types.test.sh|fm-no-mistakes-required-workflow.test.sh|\
    fm-send-popup-settle.test.sh|fm-send-settle.test.sh|fm-stow-contract.test.sh|\
    fm-scope-contract.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-process-environ-lib.sh|\
    bin/fm-worker-isolation-lib.sh|bin/fm-agent-cwd-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
