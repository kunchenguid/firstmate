...
family_for_basename() {
  case "$1" in
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-check-unregister.test.sh|fm-github-read-pause.test.sh|fm-pr-check-security.test.sh|fm-pr-merge.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      printf '%s\n' "__script__:fm-procevent-quota.test.sh"
      ;;
    bin/fm-github-read-pause*)
      printf '%s\n' __script__:fm-github-read-pause.test.sh
      ;;
