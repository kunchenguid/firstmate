...
family_for_basename() {
  case "$1" in
    ...
    fm-operational-input.test.sh|fm-outward-text-check.test.sh|fm-pi-primary-types.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|bin/fm-outward-text-check.sh|\
