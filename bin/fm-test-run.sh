...
family_for_basename() {
  case "$1" in
    ...
    fm-spawn-session-name.test.sh|fm-trace-context-spawn.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-main.sh|bin/fm-ff-lib.sh|bin/fm-gotmp*|bin/*pretool*)
      printf '%s\n' pure-contract-unit
      ;;
    .agents/skills/quota-array-dispatch/SKILL.md)
      ...
      printf '%s\n' live-harness-optin
      ;;
    # Broader than */SKILL.md on purpose: a vendored skill also ships scripts and
    # reference files, and a change to one of those must still select coverage.
    .agents/skills/*)
      printf '%s\n' pure-contract-unit
      ;;
