...
run_coverage_guard() {
  ...
  # Every list above is sorted with LC_ALL=C, so comm must compare under the
  # same collation; a developer locale such as en_US.UTF-8 otherwise rejects
  # C-sorted input as unsorted and fails the guard for no real reason.
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|bin/fm-graphify.sh|\
