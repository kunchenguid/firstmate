...
# (bin/fm-home-seed.sh) and for every Herdr crewmate or scout spawn, whose
# pane must be created in its slot (bin/fm-spawn.sh spawn_lease_worktree). Other
...
# Herdr slot carries the same claim, so teardown reads one record everywhere.
#
