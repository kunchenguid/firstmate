...
# Append at most one pending row for an exact kind/key pair while the caller
# holds FM_WAKE_QUEUE_LOCK. Return 0 when appended, 1 when already pending, and
# 2 on validation or publication failure. This is intentionally separate from
# fm_wake_append_locked because ordinary status wakes keep their existing
# latest-event behavior, while remote request correlations need crash-safe
# idempotency across their inbox and this queue.
fm_wake_append_locked_once() {
  local kind=$1 key=$2 payload=$3 clean_key
  case "$kind" in
    signal|stale|check|heartbeat) ;;
    *) return 2 ;;
  esac
