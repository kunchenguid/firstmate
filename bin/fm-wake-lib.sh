...
#
# Treehouse's own state file records a slot's owner as a live process lease
# (owner_pid plus owner_started_at, and `treehouse status` reports in-use from
# the processes actually running under the path). That answers "is anything
# running here", never "which task owns this". The lease is released by the very
# event that makes a task record stale - the worker exiting - so a slot whose
# lease has lapsed reads identical whether it is still this task's or has since
# been handed to another one. Firstmate therefore keeps its own claim: one file
