...
# releases it. Firstmate uses that for secondmate homes (bin/fm-home-seed.sh)
# and, when the installed treehouse advertises --lease, for crewmate and scout
# spawns (bin/fm-spawn.sh). The spawn then cds the pane into the leased path
# instead of leaving an interactive `treehouse get` running in it. An older
# treehouse without --lease still uses the pane-driven get, whose state entry
# is a live process lease (owner_pid plus owner_started_at, and `treehouse
...
# released by bin/fm-teardown.sh when the slot goes back to the pool.
#
