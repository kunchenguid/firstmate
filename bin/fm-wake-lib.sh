...
# An ordinary watcher start after an announced-but-unacked episode is a new down
# stretch: mint a fresh pending generation so a still-open decision or buried
# note can be presented once more. Handling successors and foreground
# checkpoints must not call this, because neither starts a new down stretch.
