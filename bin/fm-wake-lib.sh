...
fm_wake_commit_remote_reply_quarantine_receipts_through() { # <cutoff> [<rows-file>]
  local cutoff=$1 rows=${2:-} key rest id result_hash receipt tmp notice_state
  while IFS= read -r key; do
...
_fm_wake_legacy_signal_sig() {  # <file> -> pre-v2 size:mtime signature
  if [ "$_FM_UNAME" = Darwin ]; then
...
_fm_wake_legacy_signal_seen_size() {  # <raw-marker> <file>
  local raw=$1 size current_size
  case "$raw" in *:*) size=${raw%%:*} ;; *) printf '0'; return 0 ;; esac
  case "$size" in ''|*[!0-9]*) printf '0'; return 0 ;; esac
  current_size=$(_fm_status_file_size "$2") || { printf '0'; return 0; }
  case "$current_size" in ''|*[!0-9]*) printf '0'; return 0 ;; esac
  [ "$size" -le "$current_size" ] || { printf '0'; return 0; }
  printf '%s' "$size"
}
...
fm_wake_signal_seen_size() {  # <state> <file>
  ...
  case "$2" in
    *.status)
      ...
      if status_presentation_marker_parse "$sig"; then
        status_presentation_marker_offset "$marker" "$2"
      else
        # Homes upgraded in place can still carry the pre-v2 size:mtime
        # suppressor. Preserve its classified byte boundary for one scan; the
        # next commit replaces it with the identity-bound v2 marker.
        _fm_wake_legacy_signal_seen_size "$sig" "$2"
      fi
      ;;
    *)
      sig=$(cat "$marker" 2>/dev/null) || { printf '0'; return 0; }
...
fm_wake_signal_seen_current() {  # <state> <file>
  ...
  case "$2" in
    *.status)
      ...
      legacy_sig=$(_fm_wake_legacy_signal_sig "$2") || return 1
      [ "$(cat "$marker" 2>/dev/null)" = "$legacy_sig" ]
      ;;
