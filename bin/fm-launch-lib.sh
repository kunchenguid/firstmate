...
#   __PIBIN__     shell-quoted concrete Pi-family executable path resolved from PATH
#   __PITUIMODE__ optional --tui-mode regular, present only when that executable
#                 advertises the flag
...
launch_template() {
  ...
  case "$harness" in
    ...
    # surface, never an alias: the selected $harness is both the FM_PI_HARNESS
    # identity marker and the name the caller resolves into __PIBIN__, so a
    # signed primary's environment cannot relabel a plain Pi worker (or vice
    # versa). The marker is part of the verified command, so it lives here, not
    # in any caller. __PITUIMODE__ is the caller's capability-gated
    # --tui-mode regular: Pi's CLI surface is version-dependent, so the flag is
    # only present when the resolved executable advertises it.
    pi|pi-signed)
      printf '%s%s' "FM_PI_HARNESS=$harness " '__PIBIN____PITUIMODE__'
      if [ "$kind" = secondmate ]; then
