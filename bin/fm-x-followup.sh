...
if [ "${#REPLY_ENV[@]}" -gt 0 ]; then
  env "${REPLY_ENV[@]}" "$FM_ROOT/bin/fm-x-reply.sh" "$RID" --reply-audience "${REQ_AUDIENCE:-public}" --followup "${TS_ARGS[@]}" >/dev/null
else
  "$FM_ROOT/bin/fm-x-reply.sh" "$RID" --reply-audience "${REQ_AUDIENCE:-public}" --followup "${TS_ARGS[@]}" >/dev/null
fi
post_rc=$?
