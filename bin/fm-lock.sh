...
# Writes the harness (agent) process PID the shared session-lock lib resolves
# for this session - a pid that lives as long as the firstmate session, unlike
# the transient subshell PID of any one tool call, which is dead moments after
# it is written.
...
# Harness identity (FM_HARNESS_RE, ancestry walk with its harness-published
# session-pid fallback, holder liveness) is owned by the shared session-lock
# lib so the Claude Stop auto-arm applies the exact same identity contract.
