...
# The lock name is derived from the task's canonical meta path (the same
# per-task record lock every other lifecycle participant takes), so it stays
# resolvable even when the identity record being rewritten is the durable
# landing record of an already-released task.
