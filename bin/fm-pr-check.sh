#!/usr/bin/env bash
# Record a PR-ready task: store one validated canonical pr=<url>, its worker
# generation, and the forge's exact pr_head=<sha> when available, then arm its poll.
...
POLL_SPAWN_GEN=$(grep '^spawn_gen=' "$META" | tail -1 | cut -d= -f2- || true)
while IFS= read -r line || [ -n "$line" ]; do
  case "$line" in
    pr=*|pr_head=*|pr_poll_spawn_gen=*) ;;
    *) printf '%s\n' "$line" >> "$META_TMP" || exit 1 ;;
  esac
done < "$META"
[ -z "$POLL_SPAWN_GEN" ] || printf 'pr_poll_spawn_gen=%s\n' "$POLL_SPAWN_GEN" >> "$META_TMP" || exit 1
printf 'pr=%s\n' "$URL" >> "$META_TMP" || exit 1
