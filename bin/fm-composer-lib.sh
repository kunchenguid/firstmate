...
# Cursor's `→` is scoped differently: bare `→` is safe as empty only with Cursor
# harness identity; bordered `→` may classify empty via structural composer proof;
# unscoped bare `→` is `unknown`. Every agent glyph must be listed in ALL THREE
# places below - the ghost-stripped-to-empty fallback, the bare-row case, and the
