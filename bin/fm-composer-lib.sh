...
# Compatibility wrapper for callers that need a printed trimmed value rather
# than the in-place owner above.
# It removes the same Unicode whitespace only at the two ends, preserving an
# interior non-ASCII separator byte-for-byte for older direct callers.
fm_composer_trim() {  # <text>
  local text=$1 previous space
  while :; do
    previous=$text
...
FM_DELIVERY_TRAEX_BUSY_REGEX_DEFAULT='esc to interrupt'
...
fm_busy_lines_match() {  # [harness]
  ...
  if [ -n "${FM_BUSY_REGEX:-}" ]; then
    ...
    case "$harness" in
      ...
      traex) regex=$FM_DELIVERY_TRAEX_BUSY_REGEX_DEFAULT ;;
