...
#   - <name> - <desc> (added <date>)                  -> direct-PR off  (ordinary default)
#   - <name> [<mode>] - <desc> (added <date>)          -> <mode> off
#   - <name> [<mode> +yolo] - <desc> (added <date>)    -> <mode> on
#
# mode = how a finished change reaches main:
#   no-mistakes  full pipeline -> PR -> captain merge (explicit project posture)
#   direct-PR    push + PR via gh-axi, no pipeline -> captain merge (default)
...
# to stderr, so a typo follows the ordinary delivery path visibly.
