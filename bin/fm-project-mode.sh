...
#
# Registered modes:
#   direct-PR    push + PR via gh-axi, plus a mandatory Golden Rules review pass
#                for product-facing, mixed, or uncertain work (default)
#   local-only   local branch, no remote/PR, guarded local merge
#
# A legacy `no-mistakes` or `no-mistakes-prod-only` annotation, from before those
# were retired as task modes, is mapped here to `direct-PR`: never migrated in
# the registry file itself, but always resolved to its current meaning.
#
