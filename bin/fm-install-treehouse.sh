...
# the floor for the global `--root` flag bin/fm-spawn.sh's per-home pool depends
# on: v2.0.1 answers `unknown flag: --root` on both `get` and `status` (measured
# 2026-09-11; docs/verification/runtime-backends.md "Treehouse pool root").
set -eu
