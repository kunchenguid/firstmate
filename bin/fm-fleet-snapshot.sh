...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      record=$(printf '%s\n' \
        "$registered" "$summary" "$summary_valid" "$decisions" "$activities" \
        "$activity_scan" "$reconciliation" "$terminal" "$contradiction" "$event_age" \
        | jq -s \
      ...
      record=$(printf '%s\n' \
        "$registered" "$event_age" "$activities" "$activity_scan" "$decisions" "$terminal" \
        | jq -s \
