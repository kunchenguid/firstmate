...
#     Each row exposes started_at from the task metadata written by the spawn
#     transition owner, or null when that authoritative timestamp is absent.
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    jq -n \
      ...
      '{
        ...
        started_at:($started_at | if . == "" then null else . end),
        backend:$backend,
