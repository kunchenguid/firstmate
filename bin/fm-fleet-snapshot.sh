        ...
        if (.body_lines | length) > 0 then
          ...
          | .local_note = (.local_note
              // (cap(.body_lines[-1]; "^(?<v>local-landing:[^[:space:]]+)$") as $explicit
                  | (cap(.body_lines[-1]; "^Deliverable of the finished work:[[:space:]]+(?<v>local-landing:[^[:space:]]+)$") as $retained
                  | if ($explicit != null or $retained != null) and (.completion.verb == "done") then
                      "local " + (($explicit // $retained) | sub("^local-landing:"; ""))
                    elif any(.body_lines[];
