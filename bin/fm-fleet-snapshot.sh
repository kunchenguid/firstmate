...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    printf '%s\n' \
      "$current_json" "$meta_json" "$status_json" "$report_json" "$worktree_json" \
      "$home_json" "$endpoint_exists" "$open_decisions_json" \
      "$(bool_json "$pending_decision")" "$(bool_json "$blocked_event")" \
      "$(bool_json "$report_present")" \
    | jq -s \
      --arg id "$id" \
      --arg kind "$kind" \
      --arg harness "$harness" \
...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      record=$(printf '%s\n' \
        "$remote" "$registered" "$summary" "$summary_valid" "$decisions" \
        "$activities" "$activity_scan" "$reconciliation" "$terminal" \
        "$contradiction" "$event_age" "$sampled_spawn_gen" \
        | jq -s \
      ...
      record=$(printf '%s\n' \
        "$remote" "$registered" "$event_age" "$activities" "$activity_scan" "$decisions" "$terminal" \
        "$summary" "$summary_sampled" "$sampled_spawn_gen" \
        | jq -s \
