...
local_task_runtime_json() {  # <meta-file> <id>
  ...
  if [ -n "$remote_host" ]; then
    current=$(jq -n '{state:"unknown",source:"none",detail:"remote endpoint liveness not collected by fleet snapshot",raw:""}')
...
summary_file_read() {  # <file> <expected-home>
  ...
  if [ "$bytes" -gt "$FM_SNAPSHOT_SECONDMATE_MAX_BYTES" ] \
    || ! jq -e -s --rawfile home <(printf '%s' "$home") -f "$SNAPSHOT_SUMMARY_FILTER" "$captured" >/dev/null 2>&1; then
    rm -f -- "$captured"
    return 1
  fi
  jq -c -s '.[0]' "$captured"
...
JQ
  ...
  while IFS= read -r row; do
    ...
    slot=$((slot + 1))
    jq -cn --arg id "$id" --rawfile home <(printf '%s' "$home") --arg cache "$cache_path" --argjson slot "$slot" \
...
valid_summary() {  # <file> <home>
  ...
  jq -e -s --rawfile home <(printf '%s' "$home") -f "$filter" "$file" >/dev/null 2>&1
}
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      record=$(jq -n \
        ...
        --rawfile state <(printf '%s' "$state") --rawfile current_reason <(printf '%s' "$current_reason") --arg observed "$summary_observed" \
