...
# A large backlog or tasks document can exceed the kernel's per-argument size
# limit, so every aggregate document below this point crosses into jq through
# a file (--slurpfile), never as a --argjson command-line value.
...
if [ "$OUTPUT_MODE" = contribution-input ]; then
  ...
  CONTRIBUTION_INPUT_TASKS_FILE="$JSON_TRANSPORT_DIR/contribution-input-tasks.json"
  printf '%s\n' "$contribution_tasks" > "$CONTRIBUTION_INPUT_TASKS_FILE" \
    || { echo "fm-fleet-snapshot: contribution task staging failed" >&2; exit 1; }
  jq -n --slurpfile backlog "$BACKLOG_JSON_FILE" --slurpfile tasks "$CONTRIBUTION_INPUT_TASKS_FILE" \
