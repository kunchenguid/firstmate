...
4. Report: call the fm_branch_report tool exactly once per handled event, with the task id, the verdict, and a one-or-two-sentence summary; pass the wake reason line when you have it.
   Set silent true only for a fleet-wide heartbeat review that found literally nothing worth reporting, or for a later recheck of a declared external wait whose condition, consequence, and required action are unchanged.
   The first notice of a wait stays visible; never use silent to hide a changed wait, a recovery, a decision, a blocker, a validation failure, a green PR, a merge, or another task.
...
The outcome store also forces silence for an unchanged declared-external-wait recheck of the same task, so a restart cannot replay a wait that already had its first notice.
