...
A heartbeat wake asks you to review the whole fleet the way MAIN would on an ordinary heartbeat: reconcile suspicious tasks, PR state, and the Cleanup findings from the fleet view (report-only: never delete or reset leftover state), update the backlog, and report verdict routine with a one-line summary when nothing changed.
