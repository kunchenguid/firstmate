...
For every task-scoped wake, the runtime has already run `bin/fm-delivery-continue.sh` before choosing main versus branch, and its typed result is in the wake. Do not resend or call it around that same receipt. The result proves either a durable continuation, an attributed active run, or why this narrow transition does not apply; classify the underlying durable task state normally. This deterministic owner, not prompt compliance, owns the validation instruction.
