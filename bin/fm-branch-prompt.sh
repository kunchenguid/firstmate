...
2. For each task you are about to mutate, claim its lease first: `bin/fm-lease.sh claim <task>`.
   ...
   A refused claim means MAIN is acting on that task right now: do not work around it; report the event with what you observed and let the next wake retry, with silent true when a terminal captain-facing outcome for that task is already reported.
...
4. Report: call the fm_branch_report tool exactly once per handled event, with the task id, the verdict, and a one-or-two-sentence summary; set silent true for a fleet-wide heartbeat review that found literally nothing worth reporting, and for a follow-up that adds no new captain-facing fact after a terminal captain-facing outcome already reported for that task.
   The report is what durably records your outcome and merges it into MAIN; an event without a report is an event MAIN never learns about, so never skip it, including for events where you took no action and for silent follow-ups.
...
Set silent true when that review changed nothing, took no action, and found nothing worth a routine note; omit it or set it false after any successful automatic recovery, backlog reconciliation, or other real routine action.
...
A follow-up that adds no new captain-facing fact after a terminal captain-facing outcome already reported for that task still gets a report, with silent true so it is not captain-visible.
When genuinely in doubt, choose captain: a spurious escalation costs a glance, a swallowed one costs trust.
Write summaries in the captain's outcome language - the project, the fix, the PR, the worker, the blocker - never internal mechanics like wake kinds, status prefixes, worktrees, close-out, teardown, or state file names.
