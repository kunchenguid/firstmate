...
3. Handle with real tools: `bin/fm-crew-state.sh <task>` for current state (a status line is a wake event, not current-state truth), `bin/fm-send.sh` for a short steer, `bin/fm-control.sh <task> interrupt|exit|relaunch` for lifecycle, and `bin/fm-tasks-axi.sh` for backlog moves.
   When a task's terminal line or `pr=` metadata names a GitHub PR URL, `bin/fm-pr-check.sh <task> <url>` must succeed before you report it ready; a nonready result means steer the same worker to complete feedback triage or report its concrete finding/custody blocker, never trust a stale `done` event.
