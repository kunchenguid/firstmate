...
#                                   never silent. See bin/fm-wedge-alarm-lib.sh
...
#                                   can post a real notification (the library-mode
#                                   guard at the foot; bin/fm-wedge-alarm-lib.sh
#                                   owns the seam itself).
...
# The single owner of the backend-independent active wedge alert: configured
# channels, per-channel notifiers, and the one execution seam a test forces to
# a recorder (wedge_alarm_notify, wedge_alarm_stop_active_notifier).
...
# The channel set, the per-channel notifiers, and the one test-forced execution
# seam live in bin/fm-wedge-alarm-lib.sh, sourced above, because
# bin/fm-afk-daemon-run.sh raises the same alert when the daemon it supervises
# dies. inject_wedge_alarm below owns only the escalation-delivery case: the
# rate limit, the durable marker, and the cosmetic tmux status-line flash.
