...
#                                   auto-discovered as "<session>:<pane-id>"
#                                   from $HERDR_PANE_ID under Herdr).
#          FM_SUPERVISOR_BACKEND    supervisor pane backend (Herdr;
...
# instead of silently running unsupported primitives against another pane.
...
# FM_BUSY_REGEX overrides the shared fallback busy set.
...
# Busy and composer-empty detection dispatch through the backend abstraction.
#
...
# The backend defaults to Herdr when omitted.
...
# reader whenever it does not report "busy".
pane_is_busy() {  # <target> [backend]
  local target=$1 backend=${2:-herdr} bs tail40
...
# dispatching through fm_backend_composer_state.
# inject_msg
...
pane_input_pending() {  # <target> [backend]
  local target=$1 backend=${2:-herdr}
...
# The durable wedge marker and active alert cover a wedged primary.
...
# skipped, never crashing the daemon loop - and the durable marker plus the
...
# an active alert channel, and attempts a
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  # The backend-independent active alert can reach the operator even when every pane and
...
#     classifier may have missed). Cheap: status files only.
...
#     For Herdr's normal idle-baseline path it means native agent-state observed
#     a real turn start.
...
inject_msg() {  # <message> [state]
  ...
  # Dispatch through bin/fm-backend.sh so the Herdr supervisor pane is checked
  # through the adapter.
...
# Priority: FM_SUPERVISOR_BACKEND override, then $HERDR_ENV=1, then Herdr.
...
# Herdr-specific call against it (zellij, Orca, and cmux have no verified
...
# for, instead of a confusing target error.
  if ! fm_backend_list_contains "$FM_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $FM_SUPERVISOR_SUPPORTED_BACKENDS); set FM_SUPERVISOR_BACKEND=herdr and FM_SUPERVISOR_TARGET to run Firstmate's own pane under a supported backend" >&2
...
# Priority: FM_SUPERVISOR_TARGET override, then $HERDR_PANE_ID composed into
# "<session>:<pane-id>".
# Exporting the result into FM_SUPERVISOR_TARGET makes inject_msg
  ...
  if discovered=$(discover_supervisor_target); then
    ...
    echo "error: could not auto-discover supervisor pane; set FM_SUPERVISOR_TARGET or run inside Herdr" >&2
  fi
  ...
  # Dispatches through bin/fm-backend.sh so the Herdr supervisor pane is checked
  # through the adapter.
