...
# Busy + composer-empty detection. On tmux, use the shared ghost-text- and
# border-robust primitives from fm-tmux-lib.sh (one source of truth with
# fm-send.sh), so a ghost-only or idle bordered claude composer ("│ > … │") reads
# as empty, not pending (incidents afk-invx-i5 and composer-robust). On other
# backends (wezterm) the tmux-specific captures do not apply, so fall back to the
# multiplexer abstraction's capture-based detection.
#
...
pane_is_busy() {  # <window>
  if [ "$(fm_mux_backend)" = tmux ]; then fm_pane_is_busy "$@"; return; fi
  local win=$1 tail40
...
pane_input_pending() {  # <target>
  if [ "$(fm_mux_backend)" = tmux ]; then fm_pane_input_pending "$@"; return; fi
  local target=$1 cy pane_out line idle_re
  idle_re='^[[:space:]]*(\$|>|❯|%|#)[[:space:]]*$|esc (to )?interrupt|Working\.\.\.'
  ...
  printf '%s' "$line" | grep -qiE "${FM_COMPOSER_IDLE_RE:-$idle_re}" && return 1
  return 0
}
...
inject_msg() {  # <message> [state]
  local msg=$1 state target retries sleep_s verdict i
  state="${2:-$(_state_root)}"
  ...
  if [ "$(fm_mux_backend)" = tmux ]; then
    ...
    # Non-tmux backends (e.g. wezterm): type once, then retry Enter only (never
    # retype) until the composer clears, mirroring the verified tmux core.
