...
# Backend-independent active wedge-alert channels (config/wedge-alarm parsing,
# bounded notifier dispatch, and the FM_WEDGE_ALARM_EXEC test seam). Shared with
# bin/fm-wake-delivery-alarm.sh, which sources the same library directly.
...
# Away-mode wedge-alarm re-alarm rate-limit epoch (the shared channel machinery
# and its notifier tunables live in fm-wedge-alarm-lib.sh).
