...
#      ledger entry already finished) or to record a fresh actionable exit-2 outcome
...
autoarm_owns_recovery() {
  ...
  # decision is still open. Once its own ledger entry records a terminal
  # outcome the lock is abandoned, and treating it as ownership is what let a
  # dead watcher go unnoticed for turn after turn. Fall through instead: the
  # outcome cases below still cover a claim that finished moments ago, so a
  # genuine handoff is not duplicated, while a stale one now reaches the block.
...
terminal_fail_open() {
  ...
  if ! fm_lock_try_acquire "$OWNER_LOCK"; then
    ...
    # is already terminal is not a concurrent owner to step aside for. Stepping
    # aside for one here allows the stop silently, and the episode's one attended
    # alarm would never fire, so clear the abandoned claim and let this decision
