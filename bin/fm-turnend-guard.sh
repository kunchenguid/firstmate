...
block_stop() {
  local afk x_mode topic_board reason rule
  afk=0
  [ -e "$STATE/.afk" ] && afk=1
  x_mode=0
  ...
  rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    if [ "$FM_SUP_TOPIC_BOARD" = true ] && [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
      printf '●  %s task(s) and Telegram intake need supervision, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_IN_FLIGHT" "$FM_SUP_BEACON_DESC"
    elif [ "$FM_SUP_TOPIC_BOARD" = true ]; then
      printf '●  Telegram intake needs supervision, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_BEACON_DESC"
...
if [ "$COUNT" -gt "$BLOCK_BUDGET" ]; then
  budget_reset
  if [ "$FM_SUP_TOPIC_BOARD" = true ] && [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
    NEED_DESC="$FM_SUP_IN_FLIGHT task(s) and Telegram intake"
  elif [ "$FM_SUP_TOPIC_BOARD" = true ]; then
    NEED_DESC="Telegram intake active"
  elif [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
