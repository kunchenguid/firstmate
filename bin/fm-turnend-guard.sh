...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --stow-harness) [ "$#" -ge 2 ] || { echo "usage: $(basename "$0") [--claude|--cursor] [--stow-harness <harness>]" >&2; exit 2; }; STOW_HARNESS=$2; shift 2 ;;
...
# Cursor also loads the Claude-compatible hook registration. Its own adapter
# calls back with --cursor; the unmarked duplicate must remain inert.
