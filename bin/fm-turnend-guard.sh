...
#      fail-open, with a visible systemMessage, only for an already verified
#      failure episode.
# That budget lives in state/.turnend-claude-blocks, written by this script in
# --claude mode only: three lines, "session=<claude session id>",
# "count=<consecutive blocks>" and "epoch=<accounted event epoch>". A different
# session id restarts the count, and the file's absence means no blocks have been
# spent this session. An allow clears it only when no failure notice is pending,
# so a verified failure episode keeps its spent budget across the allow. It
# records only how much of the block budget is left, never whether supervision is
# healthy, so deleting it hides spent blocks rather than repairing anything.
set -u
