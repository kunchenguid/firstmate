...
# surface, then prints one reason line and exits. In daemon-owned away/quiet
# mode the watcher exits on every wake for the daemon to classify; extension
# quiet retains normal triage. Reliability depends on a mechanism that SURVIVES the
