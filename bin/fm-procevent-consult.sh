...
json_wait_timed_out() {
  perl -MJSON::PP -e '
    my $v = eval { decode_json(do { local $/; open my $fh, "<", $ARGV[0] or die; <$fh> }) };
    exit 1 unless ref($v) eq "HASH" && $v->{ok} && ref($v->{data}) eq "HASH" && ref($v->{data}{wait}) eq "HASH" && $v->{data}{wait}{timedOut};
  ' "$1"
}
...
cmd_arm() {
  ...
  FM_HOME="$FM_HOME" "$SCRIPT_DIR/fm-procevent.sh" register consult "$source" \
    -- "$SCRIPT_DIR/fm-procevent-consult.sh" wait "$id" || exit 1
  printf 'armed: %s\n' "$id"
}

cmd_wait() {
  local id=${1-} job raw rc timed_out=false code='' state_dir
  [ "$#" -eq 1 ] || usage
  consult_id_valid "$id" || die "invalid consult id"
  job=$("$SCRIPT_DIR/fm-consult.sh" job-id "$id") || exit 1
  ...
  rm -f -- "$raw"
  printf '{"schema":"fm-consult-wait/1","consult_id":%s,"job_id":%s,"wait_exit":%s,"wait_timed_out":%s,"error_code":%s}\n' \
    "$(json_string "$id")" "$(json_string "$job")" "$rc" "$timed_out" "$( [ -n "$code" ] && json_string "$code" || printf null )"
