...
# available pr_head= through bin/fm-pr-check.sh, so teardown can verify landed
# work after squash merges. The full canonical URL is parsed by bin/fm-pr-lib.sh.
# GitHub and Codebase operations use the provider seam in bin/fm-scm-lib.sh.
# A GitLab merge
...
#
# Codebase defaults to a real merge commit and refuses to squash a head that is,
# or may be, a merge commit.
...
reject_bundled_repo_overrides() {
  local arg
  for arg in "$@"; do
    case "$arg" in
      --*) ;;
      # Both forge CLIs expand a single-dash argument one character at a time,
      # so a cluster such as -yR carries the repository override -R.
      -*R*)
...
reject_bundled_repo_overrides "$@" || exit 1
...
case "$PROVIDER" in
  ...
  codebase)
    ...
    codebase_confirm_rc=0
    fm_scm_pr_merge_status "" "$URL" || codebase_confirm_rc=$?
    if [ "$codebase_confirm_rc" -ne 0 ]; then
      printf 'actionable: Codebase accepted the merge request for %s but its landed state could not be confirmed; the merge poll remains armed\n' \
        "$URL" >&2
      exit 0
    fi
    ;;
