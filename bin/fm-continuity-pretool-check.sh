...
# home has actively progressing work or queued wakes but no identity-matched live
# watcher holds the home lock. Ordinary shell commands, recovery commands,
# healthy supervision, fleet-idle homes, and child worktrees are always allowed.
#
# The existing turn-end guard remains the final backstop. This gate
...
SUPERVISION_SUBJECT='tasks are in flight'
if [ "$FM_SUP_QUEUE_PENDING" = true ]; then
  if [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
    SUPERVISION_SUBJECT='tasks are in flight and queued wakes are pending'
  else
    SUPERVISION_SUBJECT='queued wakes are pending'
  fi
fi
...
case "$REASON_CODE" in
  unsafe-teardown)
    REASON="[watcher-continuity] $SUPERVISION_SUBJECT and no live watcher holds this home lock; during recovery only the ordinary literal bin/fm-teardown.sh is allowed, so drop --force and any shell-expanded arguments and retry the literal invocation (blocked: $BLOCKED_SCRIPT)"
    ;;
  *)
    REASON="[watcher-continuity] $SUPERVISION_SUBJECT and no live watcher holds this home lock; drain wakes with bin/fm-wake-drain.sh, use fail-closed bin/fm-teardown.sh for completed tasks when needed, then re-arm with bin/fm-watch-arm.sh as a tracked Claude background task before running other fleet commands (blocked: $BLOCKED_SCRIPT)"
    ;;
esac
