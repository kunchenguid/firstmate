...
# Before the captain has messaged the bot, nobody knows that chat id, so the
...
# Advance the acknowledgement offset past one update. Every update the collector
# has decided about goes through here, whatever the decision was.
acknowledge() {  # <update-id>
  fm_telegram_write_atomic "$OFFSET_FILE" "$(($1 + 1))" \
    || die "cannot advance the acknowledgement offset"
}

# Write one allowed message as a durable captain note. The text is pulled out of
# the response by its position and piped straight into the note, so no part of
# it is ever seen by a shell, and no command substitution strips the trailing
# blank lines the captain actually typed.
queue_note() {  # <response> <index> <update-id> <from>
  printf '%s' "$1" | jq -j --argjson i "$2" '.result[$i].message.text' \
    | "$FM_ROOT/bin/fm-inbox.sh" note \
      --source telegram \
      --meta "telegram_update_id=$3" \
      --meta "telegram_from=$4" \
      - >/dev/null
}
...
cmd_poll() {
  ...
  local updates index update_id is_message chat_id has_text from message_id
  while [ "$cycle" -lt "$POLL_MAX_CYCLES" ]; do
    ...
    # One TSV row per update - EVERY update, not only messages, because an
    # update left unacknowledged is handed back immediately by the next request
    # and the collector would spin on it instead of long-polling. The rows carry
    # identifiers and flags only; message text never enters this listing, so it
    # cannot break the framing, and queue_note extracts it by position instead.
    updates=$(printf '%s' "$response" | jq -r '
      .result | to_entries[]
      | .key as $i | .value as $u
      | ($u.message | if type == "object" then . else null end) as $m
      | [ ($i | tostring),
          ($u.update_id | tostring),
          (if $m == null then "0" else "1" end),
          (($m.chat.id // "") | tostring),
          (if (($m.text // "") | type) == "string" and (($m.text // "") | test("\\S"))
             then "1" else "0" end),
          (($m.from.id // 0) | tostring),
          (($m.message_id // 0) | tostring) ]
      | @tsv' 2>/dev/null) || updates=

    [ -n "$updates" ] || continue

    while IFS=$'\t' read -r index update_id is_message chat_id has_text from message_id; do
      case "$update_id" in
        ''|*[!0-9]*)
          result error "Telegram returned an update without a valid id" \
            "notes=$notes" "dropped=$dropped"
          return 0
          ;;
      esac
      ...
      if [ "$is_message" != 1 ]; then
        # Not a new message (an edit, a channel post): nothing to queue, and
        # nothing to report even during first-run discovery.
        dropped=$((dropped + 1))
      ...
      if fm_telegram_allowlisted && [ "$chat_id" = "$FM_TG_CHAT" ]; then
        ...
        if [ "$has_text" = 1 ]; then
          if queue_note "$response" "$index" "$update_id" "$from"; then
            notes=$((notes + 1))
