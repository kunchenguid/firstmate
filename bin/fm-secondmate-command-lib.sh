...
# The same read for a caller that has ALREADY established, from its own
# authoritative record, that <id> is a persistent secondmate. It reports which
# of THREE distinct states the lane is in and lets the caller decide, because
# the right answer differs by action. Never collapse the last two: they are not
# the same condition.
#
#   captain    - a recorded, deliberate transfer. Every caller refuses.
#   invalid    - a registry line EXISTS but its command field cannot be parsed.
#                Captain command may be exactly what is unreadable, so an
#                authority that cannot be read is not authority: every caller
#                refuses.
#   unrecorded - NO registry line exists for this lane at all. Command state is
#                representable only as that line's trailing field, so there is
#                nowhere a transfer could have been written: the lane is
#                firstmate-commanded by definition, and hard rule 4 stays
#                conditional on a recorded transfer that demonstrably never
#                happened.
#
# Steering (bin/fm-send.sh) refuses all three: a steer is frequent and cheap to
# retry, so the conservative default costs nothing. Retirement
# (bin/fm-teardown.sh) proceeds on `unrecorded` and says so out loud, because
# refusing there protects an unreachable state at the price of a reachable one -
# a home, worktree and never-recycled treehouse lease that could never be
# reclaimed. Teardown keeps its real protections either way: it still refuses
# while that home has work in flight, and the unlanded-work test is untouched.
