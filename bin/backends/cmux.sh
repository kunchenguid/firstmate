...
# Escape, C-c, Down) onto cmux's `send-key` names. Verified empirically: enter,
# escape, and ctrl-c all work directly; cmux specifies down as a lowercase key
# token. cmux's own key vocabulary is genuinely richer (ctrl-d/ctrl-z/ctrl-\\,
# semantic aliases
# sigint/sigtstp/sigquit - `TerminalSurface+Input.swift`), but firstmate's
# shared vocabulary across backends only needs these four today.
