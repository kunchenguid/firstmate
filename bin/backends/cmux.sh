...
fm_backend_cmux_surface_exists() {  # <workspace_id> <surface_id>
  ...
  # Capture rather than pipe: a pipe would swallow the CLI's failure exit, and
  # jq 1.6's -e exits 0 on the resulting EMPTY input (fixed in jq 1.7), which
  # would misread a dead target as live.
