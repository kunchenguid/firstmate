...
fm_backend_herdr_projection_task_identity_matches() {
  ...
  local info state identity agent agent_status current_path index pid start
  FM_BACKEND_HERDR_BOUND_PID=
  ...
  printf '%s' "$info" | jq -e --arg workspace "$workspace" --arg tab "$tab_id" --arg label "$expected_label" '
  ...
  case "$state" in
    no-agent) ;;
    live)
      [ -n "$expected_harness" ] || return 1
      identity=$(fm_backend_herdr_agent_identity_raw "$session" "$pane_id") || return 1
...
fm_backend_herdr_projection_close_bound_pane() {
  ...
  [ "$required_agent_state" = live ] || return 1
  [ -n "$FM_BACKEND_HERDR_BOUND_PID" ] && [ -n "$FM_BACKEND_HERDR_BOUND_PID_START" ] || return 1
...
fm_backend_herdr_projection_close_pane_focus_preserving() {  # <session> <pane-id> [required-agent-state] [workspace] [tab] [label] [harness] [task] [path] [home]
  ...
  [ -n "$pane_id" ] || return 0
  [ -n "$expected_workspace" ] && [ -n "$expected_tab" ] && [ -n "$expected_label" ] \
    && [ -n "$expected_harness" ] && [ -n "$expected_task" ] \
    && [ -n "$expected_path" ] && [ -n "$expected_home" ] || return 1
...
fm_backend_herdr_projection_teardown_close() {
  ...
  local expected_workspace=${4:-} expected_tab=${5:-} expected_label=${6:-} expected_harness=${7:-}
  local expected_task=${8:-} expected_path=${9:-} expected_home=${10:-}
  local close_status state
  state=$(fm_backend_herdr_pane_agent_state "$session" "$pane_id")
  [ "$state" = dead ] && return 0
  if [ "$state" = no-agent ]; then
    [ -n "$expected_workspace" ] && [ -n "$expected_tab" ] || return 1
    fm_backend_herdr_close_tab_focus_preserving \
      "$session" "$expected_workspace" "$expected_tab" "" husk "$pane_id" || return $?
    [ "$(fm_backend_herdr_pane_agent_state "$session" "$pane_id")" = dead ]
    return $?
  fi
  [ "$state" = live ] || return 1
...
fm_backend_herdr_projection_reclaim_rollback() {  # <session> <new-pane> [workspace] [tab]
  local session=$1 new_pane=$2 workspace=${3:-} tab=${4:-} state
  ...
  fm_backend_herdr_close_tab_focus_preserving \
    "$session" "$workspace" "$tab" "" husk "$new_pane" || return 1
...
fm_backend_herdr_projection_reclaim_rollback_identity() {
  ...
  [ -n "$pane" ] || return 1
  fm_backend_herdr_projection_reclaim_rollback "$session" "$pane" "$workspace" "$tab"
}
...
fm_backend_herdr_projection_reclaim_task() {  # <session> <journal> <task-id> <home> <meta-workspace> <meta-tab> <meta-pane> <parent-label> <task-label> <cwd>
  ...
  if [ -z "$new_tab" ] || [ -z "$new_pane" ]; then
    ...
    if [ -n "$new_pane" ]; then
      fm_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" "$meta_workspace" "$new_tab" || return 1
  ...
  if ! fm_backend_herdr_projection_focus_restore "$session" "$focus_before" "husk replacement create"; then
    fm_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" "$meta_workspace" "$new_tab" || true
    return 1
  fi
  ...
  if fm_backend_herdr_close_tab_focus_preserving \
    "$session" "$meta_workspace" "$meta_tab" "" husk "$meta_pane"; then
    close_status=0
  else
    close_status=$?
  fi
