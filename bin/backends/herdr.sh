...
# An alternation of literal sequences, never a bracket set: under a C/POSIX
# locale grep -E treats a bracket set as its raw BYTES, so [❯›] would match
# any line starting with 0xE2 - including every box-drawing border glyph.
...
fm_backend_herdr_composer_state() {  # <target> -> empty|pending|unknown
  ...
  # Strip a leading prompt glyph before judging what remains. '❯' and '›'
  # are multi-byte, so strip them as literals: a ? glob counts bytes under
  # a C/POSIX locale and would leave a stray byte behind. The trim below
