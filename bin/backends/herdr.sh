...
# ALTERNATION, never a bracket class: both glyphs are 3-byte UTF-8 sequences and
# a bracket expression is matched BYTEWISE under a C/POSIX locale (the fleet
# daemon's own environment), so '^[❯›]' matches any row starting with one of
# their constituent bytes - including every U+25xx box-drawing glyph, which all
# share the 0xE2 lead byte. That made a composer box's own ╰───╯ bottom border
# match as a "bare" composer row and, being the LAST match, outrank the real
# bordered row above it. Alternation compares whole literal sequences instead.
