...
fm_backend_herdr_workspace_find() {  # <session>
  ...
  printf '%s' "$list" | jq -r --arg needle_label "$FM_BACKEND_HERDR_WORKSPACE_LABEL" \
    '.result.workspaces[]? | select(.label == $needle_label) | .workspace_id' 2>/dev/null | head -1
}
...
fm_backend_herdr_create_task() {  # <container> <label> <cwd>
  ...
  dup=$(printf '%s' "$list" | jq -r --arg needle_label "$label" '.result.tabs[]? | select(.label == $needle_label) | .tab_id' 2>/dev/null | head -1)
  if [ -n "$dup" ]; then
...
fm_backend_herdr_resolve_bare_selector() {  # <name>
  ...
  while IFS= read -r session; do
    ...
    tab_id=$(printf '%s' "$tabs" | jq -r --arg needle_label "$name" \
      '.result.tabs[]? | select(.label == $needle_label) | .tab_id' 2>/dev/null | head -1)
    [ -n "$tab_id" ] || continue
