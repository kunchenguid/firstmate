...
# verbatim).
#
# Authoritative task recovery/orphan discovery uses a legacy fm-<id> tab label
# directly or maps a human display label back through state/<id>.meta, with
# recorded ids used only as disambiguators: fm_backend_herdr_list_live and
# docs/herdr-backend.md "Human-readable task tab labels". The presentation
# journal is deliberately excluded from that path.
#
...
fm_backend_herdr_projection_journal_snapshot() {  # <journal> <task-id>
  ...
  # task_label is an exact presentation binding, not task authority. It may be
  # the stable fm-<id> default or fm-spawn's sanitized human display label.
  # Live recovery still requires this exact value plus metadata endpoint,
  # home, session, workspace, tab, pane, parent, shape, focus, and agent checks.
...
# Human-labeled tabs are mapped back to that stable identity through the
...
# reconstruct an id. Out-of-band labels may embed tab/newline controls, so the
# jq filter strips control characters before the label enters this line/tab-
# framed stream. A hostile label can never forge an extra recovery line, while
# firstmate's own sanitized labels contain no controls and pass byte-identical.
...
fm_backend_herdr_list_live() {  # <session>
  local session=$1 wsid tabs tab_id label pane_id state meta backend meta_label
  local meta_session meta_wsid meta_tab id stable candidate matches
