...
#                this safe for an interactive `treehouse get`, where a nested
#                get shell sits under the pane's top shell.
...
# discovery after spawn acquires a worktree.
#
...
# shell `cd`s or enters a subshell (as an interactive `treehouse get` does).
# Reading it here would make fm-spawn.sh's worktree-discovery poll never see
# the pane "leave" the project directory, since `cwd` stays frozen at the
# original path forever. `.result.pane.foreground_cwd` tracks the ACTUALLY
# RUNNING foreground process's cwd instead, which is what changes when spawn
# moves the pane into the acquired worktree - confirmed live against a real
...
# spawn-time commands (cd into a leased worktree or interactive `treehouse
# get`, the GOTMPDIR export). `pane run` types
