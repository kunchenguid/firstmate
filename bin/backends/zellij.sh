...
fm_backend_zellij_supervisor_reconcile() {  # <session> [task-id...]
  ...
  # explicitly by pane id; `rename-pane -p <id>` produces the same pane title
  # as `new-pane --name` (evidence: docs/verification/runtime-backends.md,
  # "Supervisor pane shapes"). Cosmetic, so it is best-effort: a missing pane
  # id or a failed rename never fails reconcile.
