...
#      as a foreground command, which is how a spawn enters its leased
#      Treehouse slot (verified: an interactive `treehouse get` or `treehouse
#      enter` opens exactly such a subshell). `pane_cwd` stays frozen at wherever the
