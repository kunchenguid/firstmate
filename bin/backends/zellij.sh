...
#      pane's own top-level shell within one poll (<0.3s), which is how a
#      spawn enters its leased Treehouse slot - but does NOT reflect a `cd`
#      performed by a NESTED SUBSHELL the pane's shell launched as a
#      foreground command (verified: an interactive `treehouse get` or
#      `treehouse enter` opens exactly such a subshell). `pane_cwd` stays frozen at wherever the
...
# fm-spawn.sh leases a slot and sends the pane a top-shell `cd` into it.
#
...
# `cd` run directly in the pane's own top-level shell (the spawn shape), but
# stays FROZEN at whatever directory the pane's shell was in when it launched
# an interactive `treehouse get` or `treehouse enter` as a foreground command
# - it never follows that command's own internal `cd` into the acquired
# worktree, even after the subshell is fully interactive and a `pwd` typed
# into it prints the correct live path on screen. Zellij's CLI
