...
#
# Worktree acquisition remains outside the session-provider adapter:
# fm-spawn.sh takes the durable lease, sends the pane's top-level shell into
# that path, and polls its cwd through these send/current-path primitives.
#
...
#     the shell cd's into the worktree, which would break name-based targeting.
...
# composer verification - used for fixed spawn-time shell commands
# (worktree entry and the GOTMPDIR export) that already ran this exact sequence
