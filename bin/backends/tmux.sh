...
# clears. Delegates to fm_tmux_submit_core (bin/fm-tmux-lib.sh); see
...
fm_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # cursor-agent is exact: the substring cursor is common in IDE tooling paths.
    cursor-agent) printf 'agent' ;;
