...
fm_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # agy is anchored for the same reason muse is: agy ships as a single Go
    # binary whose live process name is exactly `agy` (verified, agy 1.1.15),
    # so there is no version-named form to tolerate, and the three-letter
    # substring would otherwise claim unrelated commands.
    agy) printf 'agent' ;;
