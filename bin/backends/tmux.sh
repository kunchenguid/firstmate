...
# The trailing colon on the -t targets marks <session> unambiguously as a
# session: tmux parses a bare numeric name (e.g. the unnamed default session
# "0") as a window index instead, so list-windows/new-window would fail with
# "index 0 in use" or silently land the window in the wrong session.
