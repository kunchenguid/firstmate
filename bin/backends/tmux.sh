...
fm_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # agy (Antigravity) is anchored for the same reason: its live process name
    # is the bare word `agy` (verified, agy 1.2.0) and a glob would claim
    # unrelated commands such as magyar or agyw.
