...
#
# Worktree acquisition is a separate concern from this session-provider
# extraction: fm-spawn.sh resolves the worktree itself via a proj subprocess
# (bin/fm-proj-lib.sh, which prints the created path directly - no pane cwd
# polling needed) and only uses this adapter's send primitive to `cd` the
# pane's shell there. fm_backend_tmux_current_path below predates that and is
# no longer part of the spawn path, but remains an independently useful and
# tested primitive.
#
...
#     the pane's shell cd's into the acquired worktree, which would break
...
# empty on any tmux error. Equivalent to
# `tmux display-message -p -t "$T" '#{pane_current_path}'`; no longer called
# by fm-spawn.sh's own worktree acquisition (see the file header) but kept as
# an independently useful, tested primitive.
...
# into the acquired proj worktree, the GOTMPDIR export) that already ran this
