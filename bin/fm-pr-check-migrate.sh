...
# quarantined for private review. Current X-mode and private Telegram shims are
# preserved by exact content, while the recognized older X shim is refreshed.
...
remote_poll_check_valid() { # <check-path>
  local check=$1
  case "$(basename "$check")" in
    x-watch.check.sh) fmx_poll_shim_valid "$check" "$FM_HOME" "$FM_ROOT" ;;
    telegram-watch.check.sh) fmtg_poll_shim_valid "$check" "$FM_HOME" "$FM_ROOT" ;;
    *) return 1 ;;
  esac
}
...
current_checks_authenticated() {
  ...
  for check in "$STATE"/*.check.sh; do
    ...
    if remote_poll_check_valid "$check"; then
      continue
    fi
