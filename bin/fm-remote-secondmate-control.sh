...
cmd_launch() {
  ...
  validate_id "$id"
  validate_home "$id"
  case "$harness" in claude|codex|opencode|pi|pi-signed|grok|kimi|cursor-agent) ;; *) die "unverified remote secondmate harness: $harness" ;; esac
...
# checkout. Ordered by cost: already present, this host's Firstmate copy, that
# copy's forge origin, then a distinct home origin retained for adopted layouts.
# Every fetch requests only the target commit and never advances another HEAD.
import_home_commit() { # <home> <commit>
  local home=$1 commit=$2 root_origin home_origin
  ...
  if [ -n "$root_origin" ] \
    && git -C "$home" fetch --quiet --no-tags -- "$root_origin" "$commit" 2>/dev/null \
    && git -C "$home" cat-file -e "$commit^{commit}" 2>/dev/null; then
    return 0
  fi
  home_origin=$(git -C "$home" remote get-url origin 2>/dev/null || true)
  if [ -n "$home_origin" ] && [ "$home_origin" != "$FM_ROOT" ] \
    && [ "$home_origin" != "$root_origin" ] \
    && git -C "$home" fetch --quiet --no-tags -- "$home_origin" "$commit" 2>/dev/null \
    && git -C "$home" cat-file -e "$commit^{commit}" 2>/dev/null; then
    return 0
  fi
  return 1
...
cmd_sync() {
  ...
  import_home_commit "$TARGET_HOME" "$commit" \
    || die "remote home could not import $commit from this host's Firstmate copy, that copy's origin, or the home's distinct origin; run /updatefirstmate to refresh this host's copy, or push that commit first"
