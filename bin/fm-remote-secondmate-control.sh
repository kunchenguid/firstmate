...
cmd_send() {
  ...
  case "$ring_rc" in
    ...
    4) printf 'notice: doorbell typed into %s but its submit did not land, so that line is stranded in the input and every re-ring stays suppressed until it is submitted or cleared; the steer is durably recorded at %s\n' "$REMOTE_ENDPOINT_TARGET" "$rec" >&2 ;;
  esac
}
