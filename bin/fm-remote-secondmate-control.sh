...
#
# Usage:
#   fm-remote-secondmate-control.sh launch <id> <harness> <model|-> <effort|-> herdr <label|-> [traceparent]
...
cmd_launch() {
  local id=$1 harness=$2 model=$3 effort=$4 selected_backend=$5 label=$6 traceparent=${7:-}
...
case "${1:-}" in
  launch) shift; [ "$#" -ge 6 ] && [ "$#" -le 7 ] || usage; cmd_launch "$@" ;;
