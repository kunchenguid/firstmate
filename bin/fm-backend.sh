...
#   - the proj worktree provider for every session-provider-only backend
#     (tmux, herdr, zellij, cmux); orca owns its own task worktree and terminal,
#     so it drops both proj and any other backend's session CLI.
...
fm_backend_required_tools() {  # <backend>
  case "$1" in
    tmux)   printf '%s' 'tmux proj' ;;
    herdr)  printf '%s' 'herdr jq proj' ;;
    zellij) printf '%s' 'zellij jq proj' ;;
    cmux)   printf '%s' 'cmux jq proj' ;;
    orca)   printf '%s' 'orca' ;;
    *) return 1 ;;
  esac
}
