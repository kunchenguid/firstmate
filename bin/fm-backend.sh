...
# spawn-capable backend, currently experimental herdr,
...
# always wins. Selecting herdr via auto-detect prints one loud stderr
# notice (experimental); auto-detecting tmux stays silent - it is
...
# the session-provider CLI itself (tmux/herdr) plus jq for the JSON-emitting
# herdr adapter and the treehouse worktree provider.
...
# exact window; the Herdr adapter reuses its husk classifier.
