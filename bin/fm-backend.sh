...
# its old no-classifier content-diff reporting. ryder is the one adapter that
# owns its own classifier, because it is the only backend whose capture carries
# both a style channel and a cursor position (docs/ryder-backend.md).
