...
#
# Herdr is the default session provider.
# Zellij, Orca, and cmux remain explicit alternatives, while cmux may also be
# selected by runtime detection when Firstmate itself runs inside cmux.
# Orca owns both the task worktree and terminal endpoint; the other providers
# use treehouse for isolated worktrees.
...
# The watcher poll loop remains a fail-closed fallback for providers without
# native event push.
...
# Extend only after a backend gets its own bin/backends/<name>.sh and empirical
# verification, mirroring AGENTS.md section 4's harness-verification discipline.
# Zellij is verified against the real 0.44.0 binary (docs/zellij-backend.md).
# Orca is spawn-capable and also owns the worktree provider.
# cmux is session-provider-only and verified against the real 0.64.17 binary.
...
# inside, from verified environment markers.
# Prints the detected backend name and returns 0, or returns 1 when nothing is
# detected.
# Herdr injects HERDR_ENV=1 into every managed process.
# cmux injects CMUX_WORKSPACE_ID
...
# multiplexer.
# A Herdr marker alongside CMUX_WORKSPACE_ID means Herdr is the innermost layer
# and must win.
#
...
#      Authoritative in the common wrapper-strip case.
...
#      scrubbed entirely (no bundle id to inherit).
# Callers needing the winning signal read FM_BACKEND_DETECT_SIGNAL (set to
# HERDR_ENV, CMUX_WORKSPACE_ID, bundle-id, or ancestry) and
...
# notice for cmux.
# Herdr is the normal default and stays silent.
...
fm_backend_name() {
  ...
  if fm_backend_detect >/dev/null; then
    ...
    if [ "$detected" = cmux ]; then
      ...
      echo "NOTICE: auto-detected cmux runtime ($marker) - spawning into the cmux backend. Set config/backend or pass --backend herdr to opt out." >&2
    fi
    printf '%s' "$detected"
    return 0
  fi
...
#   - the session-provider CLI itself (herdr/zellij/orca/cmux);
#   - jq, for the JSON-emitting session-provider adapters (herdr, zellij, cmux) whose
...
#     (herdr, zellij, cmux); orca owns its own task worktree and terminal,
...
#                      behavior: recorded targets are trusted without a live
#                      re-check).
...
# rather than hand-writing backend-specific case statements
...
# failures here are best-effort and harmless for already-gone targets).
...
# without duplicating per-backend composer-reading logic.
# Herdr, Orca, and cmux expose named classifiers
...
# probe). An unqueryable Herdr pane, missing zellij pane, or unreadable Orca
# terminal simply fails, which
...
#   dead    - CONFIDENTLY not an agent: a structurally-gone or unregistered pane.
...
# verified classifier: Herdr (docs/herdr-backend.md "Agent liveness probe reuses the husk
