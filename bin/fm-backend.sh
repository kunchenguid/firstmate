  ...
  case "$name" in
    tmux)
      if [ -z "${_FM_BACKEND_TMUX_SOURCED:-}" ]; then
        adapter="$FM_BACKEND_LIB_DIR/backends/tmux.sh"
        [ -f "$adapter" ] || return 1
    ...
    zellij)
      if [ -z "${_FM_BACKEND_ZELLIJ_SOURCED:-}" ]; then
        adapter="$FM_BACKEND_LIB_DIR/backends/zellij.sh"
        [ -f "$adapter" ] || return 1
        # shellcheck source=/dev/null
        . "$adapter" || return 1
    ...
    orca)
      if [ -z "${_FM_BACKEND_ORCA_SOURCED:-}" ]; then
        adapter="$FM_BACKEND_LIB_DIR/backends/orca.sh"
        [ -f "$adapter" ] || return 1
        # shellcheck source=/dev/null
        . "$adapter" || return 1
    ...
    cmux)
      if [ -z "${_FM_BACKEND_CMUX_SOURCED:-}" ]; then
        adapter="$FM_BACKEND_LIB_DIR/backends/cmux.sh"
        [ -f "$adapter" ] || return 1
        # shellcheck source=/dev/null
        . "$adapter" || return 1
