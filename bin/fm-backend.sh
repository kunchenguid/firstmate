...
# spawn-capable backend, currently herdr, zellij, orca, cmux, or paseo, carries
...
# always wins. Auto-detected herdr stays silent like tmux. Selecting cmux or
# paseo via auto-detect prints one loud stderr notice because both remain
# experimental; the notice names the winning signal, so a fallback-detected one
# (bundle id, or cmux ancestry, after a wrapper stripped the primary marker) is
# visibly distinct from the primary-marker case. For a `secondmate` kind, an
