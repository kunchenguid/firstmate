  ...
  case "$harness" in
    ...
    codex)
      if [ "$kind" = secondmate ]; then
        ...
        printf '%s' 'codex __MODELFLAG____EFFORTFLAG____CODEXMCPFLAG__--dangerously-bypass-approvals-and-sandbox -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
      fi
      ;;
