...
#     direct Cursor -> Cursor, and Pi/OpenCode models prefixed anthropic/,
#     openai-codex/, or xai/ -> Claude, Codex, or the xAI API product respectively.
...
selection=$(printf '%s\n' "$quota_json" | jq -ec \
  ...
  def route($profile):
    ...
    | if $h == "claude" then {provider: "claude", model: $model}
      ...
      elif $h == "cursor" then {provider: "cursor", model: $model}
  ...
  def relevant_windows($provider; $route):
    ...
    | if $route.provider == "grok" then
      ...
      elif $route.provider == "cursor" then
        $windows | map(select(
          .id == "included_usage"
          or .id == "spend_limit"
          or ((($route.model | length) == 0 or $route.model == "auto") and .id == "auto_usage")
        ))
      else
        $windows | map(select(
