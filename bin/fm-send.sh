...
if [ "${1:-}" = "--key" ]; then
  ...
  if [ "$(fm_mux_backend)" = tmux ]; then
    ...
    # Non-tmux backends (e.g. wezterm) have no verified tmux submit core, so use the
    # multiplexer abstraction's type-then-Enter. The from-firstmate marker still
    # applies, and the popup-settle pause is honored before Enter.
    fm_mux_send_text "$T" "$MARK_PREFIX$*"
    sleep "$settle"
    fm_mux_send_enter "$T"
  fi
  # Submit landed. The cleared composer only proves the text was submitted; the
  # harness still needs a beat to spin up the turn before its busy footer shows.
  # Pause so an immediate peek catches the crewmate actually working instead of the
  # stale idle pane. FM_SEND_SETTLE=0 disables it. Scoped to this path only, never
