...
help() {
  printf '%s\n' \
    ...
    'FM_HOME selects the operational home; launched tasks inherit FM_TASK_ID. No sender label may be supplied.' \
    'Send/receive/ack bind the calling task or require the supervisor working directory to equal FM_HOME.' \
...
case "${1:-}" in
  ...
  receive|ack)
    ...
    if [ "$actor" = supervisor ]; then [ ! -e "$STATE/supervisor.meta" ] && [ ! -L "$STATE/supervisor.meta" ] || exit 1; fi
  ...
  *) echo 'usage: fm-message.sh read|validate <file> | receive | ack <name> | stats | send <recipients> [options] <text>' >&2; exit 1 ;;
esac
