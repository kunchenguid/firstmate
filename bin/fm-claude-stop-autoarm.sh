...
#   - Need: arms only while work is in flight (state/*.meta) or secure-mode X
#     has a relay poll to run (state/x-watch.check.sh); an idle home exits 0.
...
# --- need: in-flight work or a secure-mode X relay poll ----------------------
need_supervision() {
