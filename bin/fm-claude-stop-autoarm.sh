...
#   - Need: arms only while work is in flight (state/*.meta) or an authenticated
#     remote channel has a poll to run; an idle home exits 0.
...
# --- need: in-flight work or an authenticated remote-channel poll ------------
need_supervision() {
...
# shellcheck source=/dev/null
[ -f "$CONFIG/telegram-mode.env" ] && . "$CONFIG/telegram-mode.env"
