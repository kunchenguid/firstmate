...
#     lock, detached daemon delivery, or unresolved ownership remains inert or
#     visibly failed and never arms or rewakes through inferred authority.
...
# generation and outcome, binds rewake outcomes to the session-lock pid and
# watcher recovery generation, while state/.claude-autoarm-failure-epochs records
...
autoarm_trace_session_probe() {  # <lock-pid>
  ...
  local lock_pid=$1 ancestry pid in_ancestry=0 owned=0
...
EOF
  fm_session_lock_owned_by_self "$STATE" "$FM_ROOT" \
    && owned=1
  {
    printf 'autoarm_trace hook_pid=%s lock_pid=%s foreground_owner_in_hook_ancestry=%s session_lock_owned_by_self=%s\n' \
      "$$" "$lock_pid" "$in_ancestry" "$owned"
...
if ! fm_session_lock_owned_by_self "$STATE" "$FM_ROOT"; then
  ...
  if ! fm_claude_daemon_in_session_ancestry \
...
autoarm_commit() {  # <outcome> [marker-file]
  local commit_rc failure_rc terminal_baseline pid outcome=$1 marker=${2:-} session_pid='' recovery=''
  autoarm_session_current_or_fail 'authenticated session owner died before terminal commit' || return 2
  commit_rc=1
  if [ "$outcome" = rewake ]; then
    fm_session_lock_owned_by_self "$STATE" "$FM_ROOT" || return 2
      ...
      case "$FM_RECOVERY_MARKER_TOKEN" in
        pending:downtime:*|announced:downtime:*)
          ...
          fm_autoarm_write_owned \
            "$STATE" "$MY_GEN" "$outcome" "$marker" "$FM_ROOT" "$SESSION_OWNER_PID" bound \
            "$session_pid" "$recovery"
          commit_rc=$?
          ;;
      esac
    fi
