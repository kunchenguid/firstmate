...
#     relay poll to run (state/x-watch.check.sh), or a registered custom check
#     (state/<id>.check.sh + .check-trust) exists; an idle home exits 0.
...
# --- need: in-flight work, an X-mode relay poll, or a registered custom check -
need_supervision() {
