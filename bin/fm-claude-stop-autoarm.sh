...
#   - Need: arms only while work is in flight (state/*.meta) or a standing poll
#     needs a watcher - an X-mode relay poll (state/x-watch.check.sh) or the
#     operational alert inbox watch (state/ops-watch.check.sh); an idle home
#     with no standing poll exits 0.
...
# --- need: in-flight work or an armed standing poll --------------------------
need_supervision() {
