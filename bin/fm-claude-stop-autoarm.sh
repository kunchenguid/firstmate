...
#   - Need: arms only while work is in flight (state/*.meta) or an always-on
#     channel has a poll to run - X mode (state/x-watch.check.sh) or the
#     Telegram bridge (state/telegram-watch.check.sh); an idle home exits 0.
