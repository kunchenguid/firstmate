...
#     an actionable arm close (timestamped signal:/stale:/check:/heartbeat)
#     prints one rewake banner to stderr and exits 2, which wakes Claude even while idle
...
while [ "$attempt" -lt "$AUTOARM_ATTEMPTS" ]; do
  ...
  ACTIONABLE=0
  if [ -n "$OUT" ]; then
    grep -Eq "$FM_WAKE_REASON_LINE_RE" "$OUT" 2>/dev/null && ACTIONABLE=1
  fi
...
if [ "$ACTIONABLE" -eq 1 ]; then
  write_epoch rewake
  {
    ...
    [ -n "$OUT" ] && grep -E "$FM_WAKE_REASON_LINE_RE" "$OUT" 2>/dev/null | head -8
...
if [ ! -e "$FAILURE_NOTICE" ]; then
  write_epoch failed
  {
    ...
    [ -n "$OUT" ] && grep -E "^(watcher:|(${FM_WAKE_SHORT_TIMESTAMP_RE} — )?(signal:|stale:|check:|heartbeat))" "$OUT" 2>/dev/null | head -8
