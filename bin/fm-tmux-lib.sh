...
# `fm_pane_is_busy` once the Enter-retry budget is spent, scoped to the two
# harnesses with verified Enter-while-busy queuing (opencode and cursor): a
...
fm_tmux_composer_row_state() {  # <raw-row> [bordered] [allow-busy] -> empty|pending|unknown
  ...
  fm_composer_classify_content "$bordered" "$stripped" "${FM_COMPOSER_IDLE_RE:-}" insensitive "$plain" "${FM_COMPOSER_HARNESS:-}"
}
