...
fm_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep> [harness] [baseline-idle]
  local target=$1 retries=$2 sleep_s=$3 harness=${4:-} baseline_idle=${5:-} i=0 j state busy_state busy_harness
  while :; do
  ...
  busy_state=idle
  case "$harness" in
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|cursor-agent) busy_harness=$harness ;;
    *) busy_harness= ;;
  esac
  fm_pane_is_busy "$target" "$busy_harness" && busy_state=busy
...
fm_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle> [harness]
  ...
  sleep "$settle"
  fm_tmux_submit_enter_core "$target" "$retries" "$sleep_s" "$harness" "$baseline_idle"
}
