...
# `agent get`. It answers only for pi and polytoken, the two harnesses that
# draw that shape, from two live signals:
...
#     Polytoken's TUI is the pane's foreground `polytoken` process (its daemon
#     is detached), and its `Running for <n>` turn row is the footer.
# Prints "<agent><TAB>idle" or "<agent><TAB>working" with agent pi or
# polytoken; exits 1 when the pane is neither.
