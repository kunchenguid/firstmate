...
# request created through a bound captured source. `close` is the moot outcome:
...
# original timestamp, so a re-delivered captured selection never resets the clock on
...
reconcile_close() {
  ...
  reconcile_request_read "$id" \
    || fail "task $id has no pending captured-source reconcile request"
  require_tasks_axi
  show=$(task_show "$id") || fail "captain-held task $id is absent from this home's configured backlog (data directory $DATA)"
  state=$(show_field "$show" state)
  hold_kind=$(show_field_value "$show" hold_kind)
