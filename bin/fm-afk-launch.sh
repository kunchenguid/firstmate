...
fm_afk_launch_terminal_absent() {  # <backend> <target>
  ...
  case "$backend" in
    herdr)
      ...
      ;;
    tmux)
      out=$(fm_tmux has-session -t "$target" 2>&1)
      result=$?
...
fm_afk_launch_create_tmux() {  # <captain-target> <captain-backend>
  ...
  cmd=$(printf 'exec env FM_HOME=%q FM_SUPERVISOR_TARGET=%q FM_SUPERVISOR_BACKEND=%q FM_TMUX_SOCKET=%q %q' \
...
fm_afk_launch_main() {
  ...
  # a lock this process owns, so arming it before acquisition is safe, and
  # disarming after a failed acquisition keeps this process from running a
  # release it never owed.
