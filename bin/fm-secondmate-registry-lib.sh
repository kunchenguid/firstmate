...
# A remote record adds its host placement and explicit Herdr session before the existing fields:
#   (host: ...; root: ...; home: ...; session: ...; scope: ...; projects: ...; added YYYY-MM-DD)
...
SECONDMATE_REGISTRY_MATCH_SESSION=
SECONDMATE_REGISTRY_MATCH_PROJECTS=
SECONDMATE_REGISTRY_MATCH_REMOTE=0
SECONDMATE_REGISTRY_ERROR=

_SECONDMATE_REGISTRY_LIB_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
# shellcheck source=bin/fm-herdr-session-lib.sh
. "$_SECONDMATE_REGISTRY_LIB_DIR/fm-herdr-session-lib.sh"
unset _SECONDMATE_REGISTRY_LIB_DIR
...
secondmate_registry_parse_line() {
  ...
  local remote_session_re='^- ([A-Za-z0-9._-]+) - (.+) \(host:[[:space:]]*([^;)]*);[[:space:]]*root:[[:space:]]*([^;)]*);[[:space:]]*home:[[:space:]]*([^;)]*);[[:space:]]*session:[[:space:]]*([^;)]*);[[:space:]]*scope:[[:space:]]*(.*);[[:space:]]*projects:[[:space:]]*([^;)]*);[[:space:]]*added[[:space:]]+([0-9]{4}-[0-9]{2}-[0-9]{2})\)[[:space:]]*$'
  ...
  if [[ "$line" =~ $local_re ]]; then
    ...
    SECONDMATE_REGISTRY_SESSION=${BASH_REMATCH[6]}
    ...
    SECONDMATE_REGISTRY_ROOT=${BASH_REMATCH[4]}
    SECONDMATE_REGISTRY_HOME=${BASH_REMATCH[5]}
    SECONDMATE_REGISTRY_SESSION=$FM_REMOTE_HERDR_LEGACY_SESSION
  ...
  if [ "$SECONDMATE_REGISTRY_REMOTE" -eq 1 ]; then
    ...
    fm_remote_herdr_session_validate "$SECONDMATE_REGISTRY_SESSION" || return 1
  fi
  return 0
}
    ...
    case "$line" in
      "- "*)
        ...
        if [ -n "$expected_id" ] && [ "$id" = "$expected_id" ]; then
          ...
          SECONDMATE_REGISTRY_MATCH_SESSION=$SECONDMATE_REGISTRY_SESSION
