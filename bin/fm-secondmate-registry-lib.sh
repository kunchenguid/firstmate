...
# Either form may carry optional trailing "; <key>: <value>" segments after the
# date, written in that order by their own owners: "; label: <display name>"
# pins the session display name (bin/fm-spawn.sh re-reads it on every relaunch)
# and "; command: <state>" records which lane the captain has taken
# (bin/fm-secondmate-command-lib.sh is the sole reader of that one).
# They are parsed as one tolerated extras group rather than as required fields.
# Without that, a labelled or captain-commanded line would fail the anchors below
# and be reported as a malformed entry, taking the whole secondmate out of
# service - so the group deliberately tolerates a trailing segment this parser
...
# Pull the label out of the tolerated trailing-extras group. Other keys in that
# group belong to their own owners and are deliberately left alone here.
secondmate_registry_extract_label() {
  local extras=$1
  SECONDMATE_REGISTRY_LABEL=
  case "$extras" in
    *'; label:'*|*';label:'*)
      SECONDMATE_REGISTRY_LABEL=$(printf '%s\n' "$extras" \
        | sed -n 's/.*;[[:space:]]*label:[[:space:]]*\([^;)]*\).*/\1/p' | tail -1)
      ;;
  esac
}
...
secondmate_registry_parse_line() {
  local line=$1
  local extras_re='((;[[:space:]]*[a-z-]+:[^;)]*)*)'
  local local_re='^- ([A-Za-z0-9._-]+) - (.+) \(home:[[:space:]]*([^;)]*);[[:space:]]*scope:[[:space:]]*(.*);[[:space:]]*projects:[[:space:]]*([^;)]*);[[:space:]]*added[[:space:]]+([0-9]{4}-[0-9]{2}-[0-9]{2})'"$extras_re"'\)[[:space:]]*$'
  local remote_re='^- ([A-Za-z0-9._-]+) - (.+) \(host:[[:space:]]*([^;)]*);[[:space:]]*root:[[:space:]]*([^;)]*);[[:space:]]*home:[[:space:]]*([^;)]*);[[:space:]]*scope:[[:space:]]*(.*);[[:space:]]*projects:[[:space:]]*([^;)]*);[[:space:]]*added[[:space:]]+([0-9]{4}-[0-9]{2}-[0-9]{2})'"$extras_re"'\)[[:space:]]*$'
  SECONDMATE_REGISTRY_ID=
  SECONDMATE_REGISTRY_SUMMARY=
  SECONDMATE_REGISTRY_HOST=
  SECONDMATE_REGISTRY_ROOT=
  ...
  if [[ "$line" =~ $local_re ]]; then
    ...
    secondmate_registry_extract_label "${BASH_REMATCH[7]}"
    ...
    secondmate_registry_extract_label "${BASH_REMATCH[9]}"
