...
# A remote record adds its SSH host placement before the existing fields:
#   (host: ...; root: ...; home: ...; scope: ...; projects: ...; added YYYY-MM-DD)
# A fleet-peer record instead names the machine the home lives on, between the
# home and scope fields; that home is reached over the relay rather than by SSH:
#   (home: ...; machine: ...; scope: ...; projects: ...; added YYYY-MM-DD)
...
SECONDMATE_REGISTRY_MATCH_MACHINE=
...
secondmate_registry_parse_line() {
  ...
  local machine_re='^- ([A-Za-z0-9._-]+) - (.+) \(home:[[:space:]]*([^;)]*);[[:space:]]*machine:[[:space:]]*([^;)]*);[[:space:]]*scope:[[:space:]]*(.*);[[:space:]]*projects:[[:space:]]*([^;)]*);[[:space:]]*added[[:space:]]+([0-9]{4}-[0-9]{2}-[0-9]{2})\)[[:space:]]*$'
  SECONDMATE_REGISTRY_ID=
  SECONDMATE_REGISTRY_SUMMARY=
  SECONDMATE_REGISTRY_HOST=
  SECONDMATE_REGISTRY_ROOT=
  ...
  elif [[ "$line" =~ $machine_re ]]; then
    SECONDMATE_REGISTRY_ID=${BASH_REMATCH[1]}
    SECONDMATE_REGISTRY_SUMMARY=${BASH_REMATCH[2]}
    SECONDMATE_REGISTRY_HOME=${BASH_REMATCH[3]}
    SECONDMATE_REGISTRY_MACHINE=${BASH_REMATCH[4]}
    SECONDMATE_REGISTRY_SCOPE=${BASH_REMATCH[5]}
    SECONDMATE_REGISTRY_PROJECTS=${BASH_REMATCH[6]}
  ...
  if [ -n "$SECONDMATE_REGISTRY_MACHINE" ]; then
    case "$SECONDMATE_REGISTRY_MACHINE" in ''|-*|*[!A-Za-z0-9._-]*) return 1 ;; esac
  fi
...
secondmate_registry_validate_bindings() {
  ...
  local tmp snapshot bindings line id host root machine home home_key duplicate_homes duplicate_ids overlaps expected_home_key
  SECONDMATE_REGISTRY_MATCH_HOST=
  SECONDMATE_REGISTRY_MATCH_ROOT=
  SECONDMATE_REGISTRY_MATCH_MACHINE=
  SECONDMATE_REGISTRY_MATCH_HOME=
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      "- "*)
        ...
        machine=$SECONDMATE_REGISTRY_MACHINE
        ...
        case "$home$host$root$machine" in
          *$'\t'*|*$'\n'*|*$'\r'*)
            rm -rf -- "$tmp"
        ...
        if [ "$SECONDMATE_REGISTRY_REMOTE" -eq 1 ]; then
          ...
          # A fleet-peer home is a path on ANOTHER machine, so this machine's
          # resolver cannot canonicalize it and must not try. The machine name
          # plus the literal path is the identity that machine itself uses.
          home_key="machine:$machine:$home"
        else
        ...
        if [ -n "$expected_id" ] && [ "$id" = "$expected_id" ]; then
          ...
          SECONDMATE_REGISTRY_MATCH_MACHINE=$machine
  ...
  if [ -n "$expected_home" ]; then
    ...
    elif [ -n "$SECONDMATE_REGISTRY_MATCH_MACHINE" ]; then
      expected_home_key="machine:$SECONDMATE_REGISTRY_MATCH_MACHINE:$expected_home"
    else
