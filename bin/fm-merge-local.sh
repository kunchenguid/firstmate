...
[ "$MODE" = local-only ] || { echo "error: task $ID is mode=$MODE, not local-only; merge PR/MR tasks with bin/fm-pr-merge.sh <id> <PR-or-MR url> --authority <class> after approval" >&2; exit 1; }
