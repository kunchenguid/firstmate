...
#
# On a successful merge, this is one of the two places (with bin/fm-pr-merge.sh)
# that stamps the durable provenance record bin/fm-merge-attribution-lib.sh and
# bin/fm-merge-attribution.sh own; see those for the attribution contract.
...
DEFAULT=$(fm_merge_default_branch "$PROJ") || { echo "error: cannot determine default branch for $PROJ; expected origin/HEAD, main, or master" >&2; exit 1; }
...
fm_merge_prov_write "$STATE" "$ID" local-only "" "" "$after_full" \
