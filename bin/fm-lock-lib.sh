...
# Portable mtime in epoch seconds, from the one probe-bound owner. That library
# is itself a dependency-free leaf, so this lib still drags in no wake-queue
# machinery when a caller only needs the staleness proof.
