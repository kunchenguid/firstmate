...
# Primary config/compact-adviser is the presence flag that lets launched agents
# run the compact adviser instead of the default kill switch (bin/fm-spawn.sh
# header), so it flows down too and a secondmate's own launches follow the
# primary's choice.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode lavish-axi-host compact-adviser}"
