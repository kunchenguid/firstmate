...
# Primary config/claude-setting-sources flows down for the same reason, so a
# secondmate's own claude crewmates load the same settings scopes.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode claude-setting-sources lavish-axi-host}"
