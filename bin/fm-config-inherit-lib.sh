...
# Primary config/claude-accounts maps the Claude account names that inherited
# dispatch profiles select to their config directories on this machine, so an
# inherited profile's account resolves in the secondmate exactly as it does here.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode claude-accounts lavish-axi-host}"
