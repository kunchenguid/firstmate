...
# hand-edit backlog files too, primary config/scm-host=forgejo keeps GitHub
# prerequisites disabled fleet-wide, primary config/backend pins that home's local
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend scm-host backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode}"
