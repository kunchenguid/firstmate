...
# "off" preferences propagate as files. Primary config/nm-review-pane carries
# the Herdr no-mistakes review-pane opt-out the same way (bin/fm-nm-review-pane.sh
# owns its values). Primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces nm-review-pane startup-memory-budget trace-context}"
