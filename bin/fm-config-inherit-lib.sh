...
# spawn on codex too, primary config/model-denylist forbids the same models in
# that home's own dispatch, primary config/backlog-backend=manual makes that home
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness model-denylist backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode}"
