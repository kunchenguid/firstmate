...
# secondmate's crewmates launch their harnesses the same customized way, and
# primary config/herdr-presentation-spaces enables the same default-off Herdr
# presentation projection). It also pushes the one primary-authoritative shared
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend harness-overrides.json herdr-presentation-spaces}"
