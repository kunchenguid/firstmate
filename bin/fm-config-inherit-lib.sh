...
# config/task-tmpdir-pin makes a secondmate's own crewmates pin their agent
# TMPDIR to the task temp root too, and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget task-tmpdir-pin trace-context}"
