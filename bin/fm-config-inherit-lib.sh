...
# config/worker-running-time makes a secondmate home measure the running time of
# its own active children, which is what the primary's bearings projection renders
# for those child rows. Primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist worker-running-time}"
