...
# Primary config/claude-config-dir names the Claude config store (seat) every
# claude launch uses, so it flows down too and a secondmate's own claude
# crewmates spend the same seat's quota rather than falling back to the store
# firstmate is signed in to.
# It also pushes
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode claude-config-dir}"
