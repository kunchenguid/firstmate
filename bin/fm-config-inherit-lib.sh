...
# hand-edit backlog files too, primary config/harness-overrides.json makes a
# secondmate's crewmates launch their harnesses the same customized way, primary
# config/backend pins that home's local runtime-backend default for future
# spawns, primary config/startup-memory-budget bounds that home's startup-memory
# curation, and primary config/herdr-presentation-spaces carries the same Herdr
# presentation-projection preference - an absent primary file and an absent
# destination file both mean the same unconfigured default, so the generic
# absence mirror below converges a secondmate without deciding the
# release-dependent floor; explicit "on" and "off" preferences propagate as
# files. Primary config/trace-context is copied at the launch convergence point
# as part of the default-off W3C trace-context setup, while live convergence
# leaves it unchanged. The primary passes its frozen home-session decision into a
# newly launched Secondmate; see docs/trace-context.md.
# It also pushes the one primary-authoritative shared captain-preference file,
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend harness-overrides.json herdr-presentation-spaces startup-memory-budget trace-context}"
