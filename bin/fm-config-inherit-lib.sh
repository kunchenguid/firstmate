...
# profile rules, primary config/crew-identities.json gives every home the same
# presentation roster assignments, primary config/crew-harness=codex makes a secondmate's crewmates
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-identities.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
