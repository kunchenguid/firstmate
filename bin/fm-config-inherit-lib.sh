...
# bounds that home's startup-memory curation, primary config/context-ceiling
# keeps that home's context routing on the same setting, and primary
...
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/fm-context-lib.sh"
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget context-ceiling}"
...
propagate_inheritable_config() {
  ...
  for item in $FM_INHERITABLE_CONFIG; do
    ...
    # Scalar configs consumed as local safety boundaries reject every unsafe or
    # malformed source/destination artifact before the generic byte-copy behavior
    # below can treat one as ordinary inherited material.
    ...
    if [ "$item" = "$FM_CONTEXT_CEILING_FILE" ]; then
      if [ -e "$src" ] || [ -L "$src" ]; then
        if ! fm_context_ceiling_file_valid "$src"; then
          reason="unsafe or invalid primary source: $FM_CONTEXT_CEILING_ERROR"
          warn_inheritable_config_error "$item" "$src" "$reason"
          record_inheritable_config_result "$item" error "$reason"
          rc=1
          continue
      ...
      if [ -e "$dest" ] || [ -L "$dest" ]; then
        if ! fm_context_ceiling_file_valid "$dest"; then
          reason="unsafe or invalid destination: $FM_CONTEXT_CEILING_ERROR"
          warn_inheritable_config_error "$item" "$dest" "$reason"
          record_inheritable_config_result "$item" error "$reason"
          rc=1
          continue
