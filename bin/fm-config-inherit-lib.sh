...
# runtime-backend default for future spawns, primary config/browser-policy.json
# carries browser-capability policy, primary config/startup-memory-budget bounds
# that home's startup-memory curation, and primary config/herdr-presentation-spaces
# enables the same default-off Herdr presentation projection). It also pushes the one primary-authoritative shared
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend browser-policy.json herdr-presentation-spaces startup-memory-budget}"
