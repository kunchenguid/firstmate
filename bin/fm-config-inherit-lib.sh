...
# spawn on codex too, primary config/crew-claude-agent gives that home's claude
# crewmates the same agent profile, primary config/backlog-backend=manual makes that home
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness crew-claude-agent backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
