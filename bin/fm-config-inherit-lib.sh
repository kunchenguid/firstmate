...
# projection, and primary config/crew-memory-bound gives that home's own
# crewmates the same enforced build-memory ceiling). It also pushes the one
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget crew-memory-bound}"
