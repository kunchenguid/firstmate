...
fm_agent_process_classify_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    *claude*|*codex*|*opencode*|*grok*|*kimi*|*rovo*|pi|pi-signed|pi-launcher|Pi|omp) printf 'agent' ;;
    # agy (Antigravity CLI) and junie (Junie CLI) are anchored for the same reason
    # as muse and omp: their live process name is the bare word (`agy`, `junie`),
    # and a glob would claim unrelated commands containing that fragment.
    agy|junie) printf 'agent' ;;
