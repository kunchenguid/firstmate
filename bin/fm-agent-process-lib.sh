...
fm_agent_process_classify_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # unrelated commands such as ompd or comp. omp 18.1.18 rewrites that title,
    # and macOS reports the argv[0] spelling `OMP` in `ps -o comm=` while the
    # kernel name stays `omp` (verified live, 2026-09-12), so both are accepted.
    *claude*|*codex*|*opencode*|*grok*|*kimi*|*rovo*|pi|pi-signed|pi-launcher|Pi|omp|OMP) printf 'agent' ;;
