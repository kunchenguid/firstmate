...
You own the PR text the pipeline would otherwise write: follow \`.github/PULL_REQUEST_TEMPLATE.md\` for the title prefix, the fixed sections, the one-job size limit, and the mandatory pre-PR checklist, which applies with or without any review pipeline.
