...
fm_voice_check_step() {  # <kind> <publish-action> <what-to-write>
  ...
  printf 'On exit 1, correct only the flagged passages, guided by the categories it printed, and check again; after at most two corrections that are still flagged, or on any other non-zero exit (3 unverified, 2 usage error), do not publish: append `needs-decision [at=<epoch>] [key=voice-check]: %s voice check <status>: <printed categories>` and stop.\n' "$kind"
