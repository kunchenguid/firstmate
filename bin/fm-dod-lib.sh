...
# receives. Both paths must hand the worker the same delivery contract.
# fm_dod_block <direct-PR|local-only> <task-id> prints the block on stdout with no
# trailing blank line. The caller validates the mode; an unknown mode is refused
# rather than silently rendered as a supported delivery contract.
# The block opens with the fixed machine-readable "Delivery contract: mode=<mode>"
# line that bin/fm-spawn.sh checks a ship brief against.
# bin/fm-brief.sh scaffolds the `## Captain's intent` and `## Firstmate spec`
# subsections; bin/fm-spawn.sh and bin/fm-promote.sh refuse leftover `{TASK}` /
# `{FIRSTMATE_SPEC}` placeholders through the helpers below.
...
# secondmate charter. It is a distinctly titled launch section that states its
# own precedence for Firstmate tasks, so a brief that authors its own role
# wording is superseded rather than duplicated.
...
The task is complete only when committed on your branch, pushed, opened as a PR, and that PR's required project CI is green.
When it is implemented and committed, push your branch and open a PR with \`gh-axi\`.
Wait for the required project CI on that PR and confirm it is green before reporting completion.
Only then append \`done: PR {url} checks green\` to the status file and stop.
Do NOT merge the PR. The explicitly configured merge authority decides whether to merge it; firstmate relays the outcome.
EOF
      ;;
    local-only)
      cat <<EOF
...
This task ships **local-only**: no remote and no PR.
The task is complete only when committed on your branch \`fm/$id\`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if \`main\` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append \`done: ready in branch fm/$id\` to the status file and stop.
The explicitly configured merge authority approves the ready branch, then firstmate merges it into local \`main\` through the guarded fast-forward path.
EOF
      ;;
    *)
