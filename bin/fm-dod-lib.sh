...
When it is implemented and committed, push your branch and open a PR with \`$direct_pr_command\`, then append \`done [at=<epoch>]: PR {url}\` to the status file and stop.
...
${no_mistakes_base_steps}After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), append \`done [at=<epoch>]: PR {url} checks green\` and stop. You are finished.
EOF
      ;;
    *)
