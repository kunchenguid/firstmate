...
When it is implemented and committed, push your branch and open a PR with \`$direct_pr_command\` that is ready for review, not a draft.
...
${no_mistakes_base_steps}After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), read the PR back from the forge and confirm it is not a draft (\`gh pr view <url> --json isDraft\` must print false); if it is a draft, mark it ready with \`gh-axi pr ready\`.
A draft cannot be merged, so a done report on one leaves the merge unasked.
