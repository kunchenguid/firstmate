...
fm_ask_user_escalation_block() {  # <data-dir> <task-id>
  ...
  cat <<EOF
   ...
   \`needs-decision [at=\$(date +%s)] [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=$data/$id/nm-<run>-findings.txt\`
...
When it is implemented and committed, push your branch and open a PR with \`gh-axi\`, then append \`done [at=\$(date +%s)]: PR {url}\` to the status file and stop.
...
When it is implemented and committed, append \`done [at=\$(date +%s)]: ready in branch fm/$id\` to the status file and stop.
...
The task is complete only when committed on your branch.
When you believe it is complete, append \`done [at=\$(date +%s)]: {summary}\` to the status file and stop.
...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), append \`done [at=\$(date +%s)]: PR {url} checks green\` and stop. You are finished.
EOF
      ;;
    *)
