...
When it is implemented and committed, push your branch and open a PR with \`gh-axi\`, then append \`done: published; PR {url}\` to the status file and stop.
...
When it is implemented and committed, append \`done: committed; ready in branch fm/$id\` to the status file and stop.
...
The task is complete only when committed on your branch.
When the implementation is committed, append \`done: committed; {summary}\` to the status file and stop.
...
After /no-mistakes reports CI green (the CI-ready return point - do not wait for it to keep monitoring in the background until merge), append \`done: green-CI; PR {url} checks green\` and stop. You are finished.
EOF
      ;;
    *)
...
Every completion handoff must distinguish committed, tested, published, and green-CI evidence.
`done:` ends a worker checkpoint; it does not by itself mean tested, published, or CI green.
Include the commit SHA, tests run and their result (or not-run), publication URL (or unpublished), and CI state in the same handoff.
