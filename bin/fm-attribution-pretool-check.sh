...
# A "Generated with" line counts only in attribution shape: it opens a line, a
# quoted message, or the text after an escaped newline, after nothing but
# emoji, whitespace, or markup, so a subject like "parse the JSON generated by
# codex exec" is not attribution.
ATTRIBUTION="co-authored-by:[^[:alnum:]]*${CLAUDE_AI}|co-authored-by:.*(${CO_AUTHOR_AI})|(^|[\"']|\\\\[nr])[^[:alnum:]]*generated (with|by)[^[:alnum:]]*${AI_NAME}|claude-session:|claude\\.ai/code"
