...
fmx_meta_lock_load() {
  command -v fm_meta_lock_acquire >/dev/null 2>&1 && return 0
  # shellcheck source=bin/fm-wake-lib.sh
  . "$FM_X_LIB_DIR/fm-wake-lib.sh"
}
...
fmx_meta_link_set() {
  local meta=$1 rid=$2 ts=$3 followups=${4:-0} platform=${5:-} reply_max=${6:-} tmp rc=0
  fmx_meta_lock_load
  ...
  tmp=$(fmx_meta_tmp "$meta") || { fm_meta_lock_release "$meta"; return 1; }
  if ! { grep -vE '^x_request=|^x_request_ts=|^x_followups=|^x_platform=|^x_reply_max_chars=' "$meta" || true; } > "$tmp"; then
    rm -f "$tmp"; fm_meta_lock_release "$meta"; return 1
  fi
  printf 'x_request=%s\n' "$rid" >> "$tmp" || rc=1
  [ "$rc" -ne 0 ] || printf 'x_request_ts=%s\n' "$ts" >> "$tmp" || rc=1
  [ "$rc" -ne 0 ] || printf 'x_followups=%s\n' "$followups" >> "$tmp" || rc=1
  if [ -n "$platform" ]; then
    [ "$rc" -ne 0 ] || printf 'x_platform=%s\n' "$platform" >> "$tmp" || rc=1
  fi
  case "$reply_max" in
    ''|*[!0-9]*) ;;
    *) [ "$rc" -ne 0 ] || printf 'x_reply_max_chars=%s\n' "$reply_max" >> "$tmp" || rc=1 ;;
  esac
  [ "$rc" -ne 0 ] || mv -f "$tmp" "$meta" || rc=1
  [ "$rc" -eq 0 ] || rm -f "$tmp"
  fm_meta_lock_release "$meta"
  return "$rc"
}
...
fmx_meta_followups_set() {
  ...
  fmx_meta_lock_load
  fm_meta_lock_acquire "$meta" || return 1
  if [ ! -f "$meta" ]; then fm_meta_lock_release "$meta"; return 1; fi
  tmp=$(fmx_meta_tmp "$meta") || { fm_meta_lock_release "$meta"; return 1; }
  if ! { grep -vE '^x_followups=' "$meta" || true; } > "$tmp"; then
    rm -f "$tmp"; fm_meta_lock_release "$meta"; return 1
  fi
  if ! printf 'x_followups=%s\n' "$n" >> "$tmp" || ! mv -f "$tmp" "$meta"; then
    rm -f "$tmp"; fm_meta_lock_release "$meta"; return 1
  fi
  fm_meta_lock_release "$meta"
}
...
fmx_meta_link_clear() {
  ...
  fmx_meta_lock_load
  fm_meta_lock_acquire "$meta" || return 1
  if [ ! -f "$meta" ]; then fm_meta_lock_release "$meta"; return 0; fi
  tmp=$(fmx_meta_tmp "$meta") || { fm_meta_lock_release "$meta"; return 1; }
  if ! { grep -vE '^x_request=|^x_request_ts=|^x_followups=|^x_platform=|^x_reply_max_chars=' "$meta" || true; } > "$tmp"; then
    rm -f "$tmp"; fm_meta_lock_release "$meta"; return 1
  fi
