...
#   3. its tip is either an ancestor of the given "merged into" ref (a clean
#      fast-forward, or any non-squash merge, already contains it - git's own
#      `branch -d` can verify this itself), OR its upstream tracking branch
#      reads "[gone]" (the remote branch was deleted after merge - this
#      fleet's signal for a squash-merged PR, where the commits themselves are
#      never an ancestor of the default branch).
...
fm_branch_is_safely_merged() {
  local repo=$1 branch=$2 merged_into=$3 track
  [ -n "$branch" ] || return 1
...
# nothing itself (callers report their own outcome). Uses the plain safe `-d`
# when the ancestor proof holds - git double-checks that itself, so this can
# never discard real work - and the forced `-D` only for the "[gone]"-tracking
# proof, which git's own `-d` has no way to verify; the presence check above
# is what makes that one narrow force safe. Returns non-zero, unchanged, for
# anything the proof does not cover.
