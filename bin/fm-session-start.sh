...
#                       secondmate liveness, X-mode artifact writes, Telegram
#                       bridge artifact writes, fleet sync) also run only when
#                       locked.
...
EOF
elif [ "$X_MODE_PRESENT" -eq 1 ] || [ "$TELEGRAM_PRESENT" -eq 1 ]; then
  cat <<EOF
Follow the supervision operating instructions block above for harness '$PRIMARY_HARNESS'.
An always-on channel is active, so the emitted block's cadence instruction applies.
