...
#                       mutating step runs.
#   2. bootstrap      - bootstrap's six MUTATING sweeps
...
Only the six bootstrap mutating sweeps and wake-queue
...
#             mutating sweeps that startup already reconciled - bootstrap's six mutating sweeps (fleet
...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    if fm_backend_target_exists "$backend" "${target:-$window}"; then
