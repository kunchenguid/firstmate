...
if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
  ...
  PI_WATCH_VERSION=$(fm_extension_version "$PI_EXT" || printf '')
  PI_TURNEND_VERSION=$(fm_extension_version "$PI_TURNEND_EXT" || printf '')
  if ! fm_extension_loaded "$PI_WATCH_MARKER" "$PI_WATCH_VERSION" "$PI_LOCK" \
    || ! fm_extension_loaded "$PI_TURNEND_MARKER" "$PI_TURNEND_VERSION" "$PI_LOCK"; then
...
if [ "$PRIMARY_HARNESS" = omp ]; then
  ...
  OMP_LOCK="$STATE/.lock"
  OMP_WATCH_VERSION=$(fm_extension_version "$OMP_EXT" || printf '')
  OMP_TURNEND_VERSION=$(fm_extension_version "$OMP_TURNEND_EXT" || printf '')
  if ! fm_extension_loaded "$OMP_WATCH_MARKER" "$OMP_WATCH_VERSION" "$OMP_LOCK" \
    || ! fm_extension_loaded "$OMP_TURNEND_MARKER" "$OMP_TURNEND_VERSION" "$OMP_LOCK"; then
    printf 'OMP_WATCH_EXTENSION: not loaded - restart omp from %s so %s and %s auto-load; use --extension %s --extension %s for an explicit recovery launch\n' "$FM_ROOT" "$OMP_TURNEND_EXT" "$OMP_EXT" "$OMP_TURNEND_EXT" "$OMP_EXT"
  fi
fi
