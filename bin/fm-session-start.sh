...
if [ "$PRIMARY_HARNESS" = pi ]; then
  ...
  if ! pi_extension_loaded "$PI_WATCH_MARKER" "$PI_WATCH_VERSION" "$PI_LOCK" \
    ...
    printf 'PI_WATCH_EXTENSION: not loaded - approve Pi project trust once per clone, then restart bin/fm-pi-primary.sh so %s and %s auto-load for turn-end guard and background wake coverage; pass -e %s -e %s to that launcher only if project hooks are not trusted\n' "$PI_TURNEND_EXT" "$PI_EXT" "$PI_TURNEND_EXT" "$PI_EXT"
  fi
fi
