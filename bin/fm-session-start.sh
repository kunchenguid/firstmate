...
if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
  ...
  PI_LAVISH_MARKER="$STATE/.pi-lavish-extension-loaded"
  PI_LOCK="$STATE/.lock"
  ...
  PI_LAVISH_VERSION=$(hash_file "$PI_LAVISH_EXT" || printf '')
  ...
  if ! pi_extension_loaded "$PI_LAVISH_MARKER" "$PI_LAVISH_VERSION" "$PI_LOCK"; then
    printf 'PI_LAVISH_EXTENSION: not loaded - approve Pi project trust once per clone, then restart %s so %s auto-load for completion-aware Lavish feedback; use -e %s only if project hooks are not trusted\n' "$PI_RESTART_COMMAND" "$PI_LAVISH_EXT" "$PI_LAVISH_EXT"
  fi
fi
