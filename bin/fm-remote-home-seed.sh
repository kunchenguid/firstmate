...
# status path and captain-preferences path resolve on the remote host rather than
# retaining local paths that cannot exist there.
PARENT_STATUS="$STATE/$ID.status"
REMOTE_STATUS="$REMOTE_HOME/state/parent-replies.status"
PARENT_CAPTAIN="$DATA/captain.md"
REMOTE_CAPTAIN="$REMOTE_HOME/data/captain-shared.md"
while IFS= read -r line || [ -n "$line" ]; do
  line=${line//"$PARENT_STATUS"/"$REMOTE_STATUS"}
  line=${line//"$PARENT_CAPTAIN"/"$REMOTE_CAPTAIN"}
  printf '%s\n' "$line"
