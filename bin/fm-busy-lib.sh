...
#   endpoint-gone, herdr-native, grok-regex, traex-regex, muse-session-log, missing,
...
# fm_busy_traex_tail_busy: the traex rendered-tail fallback, the exact shape of
# the Grok one above. traex is a codex fork with no semantic writer wired, and
# its verified busy line is codex's `esc to interrupt` (traex 0.200.13,
# 2026-07-17). Its `Working...` alternative uses a Unicode ellipsis, so that
# anchor is the only one that fires.
fm_busy_traex_tail_busy() {
  grep -v '^[[:space:]]*$' | tail -12 \
    | grep -qiE "${FM_BUSY_REGEX:-${FM_TMUX_TRAEX_BUSY_REGEX_DEFAULT:-esc to interrupt}}"
}
...
fm_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  case "$harness" in
    ...
    grok*|traex*)
      ...
      case "$harness" in
        traex*)
          if printf '%s' "$tail40" | fm_busy_traex_tail_busy; then
            printf 'busy traex-regex'
          else
            printf 'idle traex-regex'
          fi
          ;;
        *)
