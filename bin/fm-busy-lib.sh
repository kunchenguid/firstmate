...
#                    SessionEnd close)
#   devin-hook       Devin lifecycle hooks (SessionStart and UserPromptSubmit
#                    open; Stop and SessionEnd close). Stop fires on normal
#                    completion only, never on a manual interrupt, so like
#                    Claude an interrupt preserves the adapter-owned state
#                    until the next hook event settles it.
