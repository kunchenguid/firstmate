...
#   endpoint-gone, herdr-native, thurbox-native, grok-regex, muse-session-log,
...
#   4. no record at all: herdr's and thurbox's native busy verdicts are
#      trusted as busy (generation state is sufficient for busy, not for
#      idle), then the
