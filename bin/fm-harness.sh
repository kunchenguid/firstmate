...
# Usage: fm-harness.sh                  print own harness: claude|codex|opencode|pi|grok|kimi|cursor|unknown
...
detect_own() {
  ...
  # Cursor agent sessions expose CURSOR_VERSION; CURSOR_PROJECT_DIR is set in hooks.
  [ -n "${CURSOR_VERSION:-}" ] && { echo cursor; return; }
