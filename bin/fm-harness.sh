...
# kimi is a verified crewmate/scout adapter only; Kimi-hosted primary support is
# intentionally out of scope, so own-harness detection does not return kimi.
...
detect_own() {
  ...
  # Only claude, pi, and grok set verified markers of their own; codex and
  # opencode are markerless, so a foreign marker retained in a terminal
  # multiplexer's stored environment can silently misidentify one before
