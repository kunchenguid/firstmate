...
# the semantic busy-state contract (bin/fm-busy-lib.sh). It caches the exact
# verdict for age handling in the same poll. Only an exact busy
...
# A current pause declaration is authoritative when fm-crew-state explicitly
# reports that the harness has no semantic state source. Generic unknown state
# remains unsafe. After other stopped or unknown verdicts, paused classification
...
pause_state_class() {  # <window> <task>
  local win=$1 task=$2 key last recheck_file class agent_alive kind recheck_class
  key=$(window_key "$win")
...
triage_unreadable_endpoint_window() {  # <window>
  local w=$1 kind task key h sf ssf ewf hf pf last stale_status stale_record stale_rest stale_end='' stale_ident=''
  key=$(window_key "$w")
  kind=$(window_kind "$w")
  task=$(window_to_task "$w" "$STATE")
  [ -n "$task" ] || return 0
...
# Scope a declared wait to its status occurrence so a replacement declaration
# starts a fresh cadence instead of inheriting the old one's throttle.
stale_wait_declaration() {  # <task>
  printf 'declared:%s' "$(fm_wake_signal_sig "$STATE/$1.status" || true)"
}

# A captain call also carries its own lifecycle identity because a task can be
# released and held again without another status append.
...
# Record only after the corresponding wake append succeeds.
stale_wait_record() {  # <window-key>
  [ -n "$STALE_WAIT_DECLARATION" ] || return 0
  printf '%s' "$STALE_WAIT_DECLARATION" > "$STATE/.paused-resurfaced-$1"
}

# Bound repeat stale alarms while an ordinary task has an open captain call.
...
# so a watcher exit cannot suppress an unconsumed channel message.
...
signal_files_actionable() {  # <status-file> ...
  local f task start record rest endpoint ident rc found=1
  ...
  for f in "$@"; do
    ...
    if [ "$rc" -eq 0 ]; then
      # With no committed cursor this may be an old terminal line left behind
      # while validation is still running; retain the fork's positive-evidence
      # suppression. A nonzero cursor proves this event is newly appended, so
      # upstream's span contract wins and later routine chatter cannot hide it.
      if [ "$start" -eq 0 ] && ! signal_reason_is_actionable "$f"; then
        continue
      fi
      found=0
    fi
...
heartbeat_scan_finds_actionable() {
  local f task record rest endpoint ident event rc found=1 sig marker
  FM_HEARTBEAT_SURFACE_ENDPOINTS=''
  for f in "$STATE"/*.status; do
    ...
    endpoint=${record%%$'\t'*}; rest=${record#*$'\t'}; ident=${rest%%$'\t'*}; event=${rest#*$'\t'}
    if [ "$rc" -eq 0 ]; then
      case "$(status_line_verb "$event")" in
          ...
          ;;
        *)
          # Legacy captain-relevant lines have no status verb. Preserve the
          # positive working/write suppression only when that legacy event is
          # still the current last line; an earlier event hidden by a later
          # routine append remains actionable through the span cursor above.
          if ! status_line_is_parseable "$event" \
            && [ "$event" = "$(last_status_line "$f")" ] \
            && ! signal_reason_is_actionable "$f"; then
            continue
          fi
          ;;
      esac
...
# Publish the quota-threshold owner's one bounded result through the existing
...
# Arm cleanup at the lock-acquisition boundary. Recovery-marker setup below can
# outlive a short foreground checkpoint; a signal anywhere after acquisition
# must still release the lock through the ordinary recovery transition.
WATCHER_PID=${BASHPID:-$$}
...
while :; do
  ...
  # successor refreshes it on its next ordinary cycle.
  if [ "$WATCHER_RECOVERY_PENDING" -ne 1 ] \
  ...
  if [ "$WATCHER_RECOVERY_PENDING" -ne 1 ] && reconcile_requests_pending; then
    reconcile_requests_detached
  fi
