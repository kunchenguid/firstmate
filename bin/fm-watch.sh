...
# a braille spinner followed by "Running  <N> tokens".
BUSY_REGEX=${FM_BUSY_REGEX:-'esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|^[[:space:]]*[^[:space:]]+[[:space:]]+Running[[:space:]]+[0-9]+[[:space:]]+tokens([[:space:]]|$)'}
