...
# an end-anchored "ctrl+c to stop" composer footer (verified 2026-07-18 on
# 2026.07.16-899851b), plus the older 2026.07.09 "Running  <N> tokens" line.
BUSY_REGEX=${FM_BUSY_REGEX:-'esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|ctrl\+c to stop[[:space:]]*$|^[[:space:]]*[^[:space:]]+[[:space:]]+Running[[:space:]]+[0-9]+[[:space:]]+tokens([[:space:]]|$)'}
