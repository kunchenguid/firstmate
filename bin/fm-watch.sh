...
# With a trailing "batch" argument the due recheck is queued durably and recorded
# for flush_paused_stale_rechecks instead of exiting the watcher on the spot, so
# several paused panes coming due in one scan cost one interruption, not one each.
...
# above, throttled by this window's own .paused-resurfaced-<key> marker, and it
# only QUEUES the recheck: due rechecks are batched and flushed by the caller, so
# multiple unchanged paused panes from the same scan produce one watcher exit.
...
actionable_run_state_surfaced_path() {  # <task>
  printf '%s/.run-state-surfaced-%s' "$STATE" "$(window_key "$1")"
}

actionable_run_state_already_surfaced() {  # <task> <line>
  [ "$(cat "$(actionable_run_state_surfaced_path "$1")" 2>/dev/null || true)" = "$2" ]
}

mark_actionable_run_state_surfaced() {  # <task> <line>
  printf '%s' "$2" > "$(actionable_run_state_surfaced_path "$1")"
}
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        if ! paused_cadence_holds "$key" && surface_paused_actionable_run_state_if_new "$w" "$task" "$h"; then
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            ...
            # letting the still-captain-relevant log line re-surface it, unless
            # a new actionable validation run outcome has appeared since.
            crew_state=$(crew_state_line "$task" || true)
