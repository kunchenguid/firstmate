...
#                          deferred rather than escalated (wedge_defer writing),
...
#                          same way (wedge_defer live-run), on the SAME clock:
#                          one .deferred-since-<key> chain per window, opened by
#                          the first deferral of the current quiet stretch and
#                          not restarted when the reason changes; a working record
...
# .wedge-escalations-, .paused-*, .deferred-*), and live homes hold those markers on
...
# declared-pause absorb and the wedge deferral so the two cadences cannot drift
...
#             otherwise, and files appearing in it is the harder signal to fake;
#   live-run  its attributed no-mistakes run still reports recent pipeline
#             activity (crew_run_activity_is_recent, which reads fm-crew-state.sh):
#             a live axi-status active_steps table with no quiet prefix, not a
#             working run-step record, not a pane busy verdict.
# Deliberately a DEFERRAL, not a cancellation: the idle timer restarts, so the
# next window re-asks both questions. Both causes share ONE clock: the
# .deferred-since-<key> marker opens with the first deferral of the current quiet
# stretch and is never restarted by the cause changing, so a crew that hands off
# from a live round to writing its worktree (or back) still re-surfaces once
# every PAUSE_RESURFACE_SECS through the shared resurface_absorbed above -
# literally the same bounded cadence a declared pause uses, throttled by one
# .deferred-resurfaced-<key> marker - and a crew whose worktree churns without
# real progress, or whose run stays live, cannot stay invisible. Only a real
# escalation (neither cause defers) or the pane going active clears the chain
# (clear_defer_tracking). The escalation counter is left alone: it is neither
# advanced (this is not an escalation) nor reset (a later genuine escalation
...
clear_defer_tracking() {  # <window-key>
  local key=$1
  rm -f "$STATE/.deferred-since-$key" "$STATE/.deferred-resurfaced-$key"
}
...
# written defers (both on the one shared deferral clock); every other idle pane,
# including a working-looking record the daemon no longer confirms, a quiet
# step, a run waiting on the ci step (no live active_steps table exists during
# that phase, so it is deliberately not covered), a terminal run, and no run at
# all, keeps the existing escalation schedule.
wedge_timer_check() {  # <window> <since-file> <triage-label> <escalation-count-file> <task>
  local win=$1 since_file=$2 label=$3 escalation_file=$4 task=$5 since age n reason
  since=$(cat "$since_file" 2>/dev/null || true)
  case "$since" in
    ''|*[!0-9]*)
      # Publish the repaired timer only after its old deferral chain is gone,
      # so observers cannot mistake a new idle window for the old chain.
...
busy_turn_bound_check() {  # <window> <task> <hash> <since-file> <escalation-file>
  ...
  if status_is_paused_or_captain_held "$(last_status_line "$statusf")"; then
    if afk_present; then
      ...
      # deferral chain are cleared exactly as handle_paused_stale clears
...
# timer and escalation count, and the deferral chain. Split out so a caller
