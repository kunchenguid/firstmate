...
pause_state_class() {  # <window> <task> [tail40]
  ...
  # cannot supply that confirmation. Without the mate case a mate's status-declared
  # `captain-held` transfer - which has no current-state mapping and so arrives as
  # `none` - would be silenced by every caller rather than taking the bounded
  # re-surface cadence, and a forgotten declaration would rot invisibly.
