...
if ! command -v fm_pid_identity >/dev/null 2>&1; then
  fm_pid_identity() {
    local pid=$1 out proc_root stat_line starttime cmdline_hex identity_key uname_value
    local -a stat_fields
    case "$pid" in
      ''|*[!0-9]*) return 1 ;;
    esac
    ...
    if [ -r "$proc_root/$pid/stat" ] && [ -r "$proc_root/$pid/cmdline" ]; then
      ...
      uname_value=$(uname 2>/dev/null || echo unknown)
      [ "$uname_value" != Linux ] || identity_key=linux-starttime
