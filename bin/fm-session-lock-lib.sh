...
# polytoken is anchored exactly like omp: the daemon and the TUI both report
# the bare word `polytoken` (verified, polytoken 0.8.14), and an unanchored
# match would claim ordinary firstmate paths such as bin/fm-polytoken-lib.sh.
FM_HARNESS_RE='claude|codex|opencode|grok|kimi|^pi$|^pi-signed$|^omp$|^polytoken$'

# The same harnesses as exact executable names. Keep in sync with
# FM_HARNESS_RE. Used only for the stricter path evidence below, where the
# loose regex would also match ordinary firstmate paths such as
# bin/fm-claude-stop-autoarm.sh.
FM_HARNESS_NAMES=(claude codex opencode grok kimi pi-signed pi omp polytoken)
