...
def main():
    ...
    commands.add_parser("media-doctor", help="check optional local media tools/model; no network/token read or interpretation proof")
    ...
    store = Store(config)
    try:
        if args.command == "media-doctor":
            from fm_whatsapp_media import media_doctor
            return media_doctor(config)
        if args.command == "doctor":
            ...
            return {"os": platform.system(), "python": platform.python_version(),
                    ...
                    "media": config.media,
                    "media_stt_command": bool(config.stt_command),
        ...
        with singleton(config):
            ...
            try:
                while not bridge.stopping:
                    bridge.tick(asynchronous_media=not args.once)
                    if args.once:
                        return store.snapshot()
                    if bridge.stopping:
                        break
