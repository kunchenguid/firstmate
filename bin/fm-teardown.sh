...
# Remove cursor's launch-time turn-end hook file, so a reused pool worktree cannot
# fire signals for a dead task. Unlike the other hook paths teardown removes
# unconditionally, .cursor/hooks.json is cursor's PROJECT hooks config and some
# projects commit it. fm-spawn refuses to write a tracked hooks.json, so teardown
# must equally refuse to delete one - removing a tracked project file would show
# up as a deletion in the returned worktree.
remove_worktree_cursor_hook() {
  local wt=$1
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        remove_worktree_cursor_hook "$child_wt"
      fi
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    remove_worktree_cursor_hook "$WT"
  fi
