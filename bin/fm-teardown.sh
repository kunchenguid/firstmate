...
# Capture the read-only presentation correlation before any cleanup can reap
# the task process tree. Treehouse/process cleanup may make Herdr remove an
# empty projected workspace before the endpoint is explicitly closed. The
# session lock is already held, so this snapshot remains bound to this teardown.
