...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/.$child_id.branch-outcome-index" "$sub_state/$child_id.agy-turnend-token"
  done
}
...
rm -f "$STATE/$ID.turn-ended" \
  ...
  "$STATE/$ID.kimi-turnend-token" "$STATE/$ID.agy-turnend-token" "$STATE/$ID.muse-session" \
