...
# The PR itself is resolved from the task's recorded pr= when present, or - for
# legacy or externally landed work that never passed the publication gate and
# therefore has no canonical PR metadata - by looking up a merged PR whose head
# branch matches the worktree's branch, fetching its head
