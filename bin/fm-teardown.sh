...
if [ -d "$WT" ] && [ "$FORCE" != "--force" ]; then
  if [ "$KIND" = secondmate ]; then
    ...
    dirty=$(git -C "$WT" status --porcelain 2>/dev/null | grep -vE '^\?\? (\.claude/|\.fm-grok-turnend$|\.fm-orca-session$|\.fm-orca-ui-key$)' | head -1 || true)
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/fm-turn-end.js" "$WT/.fm-grok-turnend" "$WT/.fm-orca-session"
    # Note: .fm-orca-ui-key is owned and cleaned by bin/backends/orca.sh's
    # remove_worktree path; it must NOT be deleted here because the orca
    # backend's P5 unregister reads the key before removing the worktree.
  fi
