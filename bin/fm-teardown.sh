...
validate_worktree_teardown_safety() {
  ...
  local untracked_exempt='^\?\? (\.claude/|\.fm-(grok|kimi)-turnend$)'
  [ -d "$WT" ] || return 0
  ...
  if ! dirty_raw=$(git -C "$WT" status --porcelain "--untracked-files=$untracked_mode" 2>/dev/null); then
  ...
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE "$untracked_exempt" | head -1 || true)
...
cleanup_firstmate_home_children() {
  local home=$1 sub_state child_meta child_id child_t child_wt child_proj child_kind child_home child_backend child_orca_worktree_id child_return_rc child_busy_gen child_owner_rc child_hooks
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        if [ "$(meta_value "$child_meta" harness)" = agy ] \
          && child_hooks=$(fm_control_agy_hooks_path "$child_wt"); then
          rm -f "$child_hooks"
        fi
      fi
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    if [ "$(meta_value "$META" harness)" = agy ] && agy_hooks=$(fm_control_agy_hooks_path "$WT"); then
      rm -f "$agy_hooks"
    fi
  fi
