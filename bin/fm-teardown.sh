...
# The scan also checks the native reservation through bin/fm-wake-lib.sh, which
# owns exact task/home/lease binding and the legacy in-place adoption route.
# A foreign or unreadable claim, missing bound lease, or unmatched native lease
# refuses the entire teardown, preserving this task's record as well as the
# copy. Missing claims are tolerated only on unleased legacy slots, still under
# the exclusive-record, landed-work and authorization gates. A returned native
# lease is always conditional on its exact lease id, including retry attempts;
# its claim is removed only after that return succeeds.
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      # Recheck the child binding under the project lock before any cleanup.
      child_owner_rc=0
      ...
      if [ "$child_owner_rc" -ne 0 ]; then
        ...
        if [ -n "$child_proj" ] && [ -d "$child_proj" ] && command -v treehouse >/dev/null 2>&1; then
          if teardown_treehouse_return "$child_wt" "$child_proj" "child worktree" "" "$child_id" "$home"; then
            ...
            child_return_rc=$?
            if [ "$child_return_rc" -eq "$TEARDOWN_TREEHOUSE_LOCK_REFUSED" ] || fm_treehouse_pool_slot "$child_proj" "$child_wt"; then
