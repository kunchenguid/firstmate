...
# A task's provider-continuity handoff ledger
# (state/provider-continuity/handoff/<id>.attempts) is volatile per-task state
# too, so it is removed with the rest; the per-provider observation records are
# fleet state and are never touched by a task teardown.
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/provider-continuity/handoff/$child_id.attempts"
  done
}
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/provider-continuity/handoff/$ID.attempts"
