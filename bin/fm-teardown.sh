...
# proof of reassignment and refuses before any task cleanup. An unreadable claim
# also refuses. An absent legacy claim keeps the record-scan protection; a new
# claim additionally binds the native lease identity. No refusal is relaxed by
# --force. Reconcile the ownership records before retrying.
...
teardown_treehouse_return_once() {  # <dir> <project> <task-id> <home>
  local dir=$1 project=$2 task_id=$3 home=$4
  local -a args
  args=(return --force "$dir")
  if fm_treehouse_pool_slot "$project" "$dir"; then
    require_owned_worktree_slot_record "$task_id" "$dir" "$home" || return 1
    if [ -n "$FM_TREEHOUSE_SLOT_OWNER_LEASE" ]; then
      args+=(--if-lease-id "$FM_TREEHOUSE_SLOT_OWNER_LEASE")
    fi
  fi
  (cd "$project" && treehouse "${args[@]}")
}

teardown_treehouse_return() {
  local dir=$1 cd_dir=$2 label=$3 post_cleanup_check=${4:-} task_id=${5:-$ID} home=${6:-$FM_HOME}
  local out lock attempt=0 max_retries lock_desc

  # Capture stdout+stderr so non-lock failures stay visible and lock failures can
  # be matched by signature even when the lock file is already gone mid-check.
  if out=$( teardown_treehouse_return_once "$dir" "$cd_dir" "$task_id" "$home" 2>&1 ); then
    [ -n "$out" ] && printf '%s\n' "$out"
    return 0
  fi
  [ -n "$out" ] && printf '%s\n' "$out" >&2
...
require_exclusive_worktree_slot_record() {
  ...
  fm_treehouse_collect_states "$record_state" || return 1
...
# Slot ownership is checked before any cleanup and again at each destructive
# boundary. A task id is only unique within its home. New claims also bind the
# native Treehouse lease; a matching marker cannot authorize a replaced lease.
require_owned_worktree_slot_record() {  # <task-id> <worktree> [home]
  local record_id=$1 worktree=$2 home=${3:-$FM_HOME} marker lease holder
  fm_treehouse_slot_owner_state "$worktree" "$record_id" "$home"
  case "$FM_TREEHOUSE_SLOT_OWNER" in
    absent) return 0 ;;
    mine)
      [ -n "$FM_TREEHOUSE_SLOT_OWNER_LEASE" ] || return 0
      holder=$(fm_treehouse_lease_holder "$record_id" "$home") || return 1
      lease=$(fm_treehouse_slot_lease "$worktree") || lease=
      if [ "$lease" = "$FM_TREEHOUSE_SLOT_OWNER_LEASE"$'\t'"$holder" ]; then
        return 0
      fi
      echo "REFUSED: task $record_id's recorded worktree $worktree no longer holds its Treehouse lease $FM_TREEHOUSE_SLOT_OWNER_LEASE (current lease: ${lease:-absent or unreadable}); nothing was changed - not even with --force." >&2
      return 1
      ;;
    other)
      echo "REFUSED: task $record_id's recorded worktree $worktree was reassigned to task $FM_TREEHOUSE_SLOT_OWNER_ID (home $FM_TREEHOUSE_SLOT_OWNER_HOME); preserving the slot and task records - not even --force authorizes touching another task's work." >&2
      return 1
      ;;
  esac
  marker=$(fm_treehouse_slot_owner_marker "$worktree" 2>/dev/null) || marker="beside $worktree"
  echo "REFUSED: task $record_id's recorded worktree $worktree carries an unreadable slot-owner claim at $marker; reconcile ownership before retrying." >&2
  return 1
}
...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" != secondmate ] && [ "$child_backend" != orca ] \
       && fm_treehouse_pool_slot "$child_proj" "$child_wt"; then
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_proj" ] && [ -d "$child_proj" ] && command -v treehouse >/dev/null 2>&1; then
        if teardown_treehouse_return "$child_wt" "$child_proj" "child worktree" "" "$child_id" "$home"; then
          fm_treehouse_slot_owner_release "$child_wt" "$child_id" "$home"
        else
          child_return_rc=$?
          # A failed conditional return is never permission to bypass the lease
          # check by removing the checkout directly.
          if fm_treehouse_pool_slot "$child_proj" "$child_wt" \
             || [ "$child_return_rc" -eq "$TEARDOWN_TREEHOUSE_LOCK_REFUSED" ]; then
