...
cleanup_firstmate_home_children() {
  local home=$1 sub_state child_meta child_id child_t child_wt child_proj child_kind child_home child_backend child_harness child_placement child_orca_worktree_id child_return_rc child_busy_gen
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    ...
    child_placement=$(meta_value "$child_meta" placement)
    child_backend=$(fm_backend_of_meta "$child_meta")
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        ...
        fm_control_remove_sandbox_turnend_hook "$child_harness" "$child_wt" "$child_placement" || return 1
      fi
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    fm_control_remove_sandbox_turnend_hook "$HARNESS" "$WT" "$PLACEMENT" || exit 1
  fi
