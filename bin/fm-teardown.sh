...
# Remove firstmate's own worktree-resident turn-end hook files so a reused pool
# worktree cannot fire signals for a dead task. Any path the project TRACKS is
# skipped: fm-spawn refuses to write over a tracked hook path, so a tracked file
# here is the project's own content (meshery/meshery tracks
# .claude/settings.local.json), and deleting it during cleanup would destroy it.
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
        remove_worktree_hook_artifacts "$child_wt"
      fi
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" "$sub_state/$child_id.meta" "$sub_state/$child_id.pi-ext.ts" "$sub_state/$child_id.claude-settings.json" "$sub_state/$child_id.grok-turnend-token"
  done
}
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    remove_worktree_hook_artifacts "$WT"
  fi
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" "$STATE/$ID.pi-ext.ts" "$STATE/$ID.claude-settings.json" "$STATE/$ID.grok-turnend-token"
